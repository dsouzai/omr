...
TR::OptionTable OMR::Options::_jitOptions[] = {
    ...
    { "activateCompThreadWhenHighPriReqIsBlocked",
        "M\tactivate another compilation thread when high priority request is blocked",
        SET_OPTION_BIT(TR_ActivateCompThreadWhenHighPriReqIsBlocked), "F", NOT_IN_SUBSET },
    { "aggressiveRecompilationChances=",
        "O<nnn>\tnumber of chances per method to recompile with the "
        "aggressive recompilation mechanism",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_aggressiveRecompilationChances, 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "arraycopyRepMovsByteArrayThreshold=",
        "C<nnn>\tByte array copy threshold for using REP MOVS instructions. Only supports 32 or 64 bytes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _arraycopyRepMovsByteArrayThreshold), 0, "F%d" },
    { "arraycopyRepMovsCharArrayThreshold=",
        "C<nnn>\tChar array copy threshold for using REP MOVS instructions. Only supports 32 or 64 bytes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _arraycopyRepMovsCharArrayThreshold), 0, "F%d" },
    { "arraycopyRepMovsIntArrayThreshold=",
        "C<nnn>\tInt array copy threshold for using REP MOVS instructions. Only supports 32, 64, or 128 bytes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _arraycopyRepMovsIntArrayThreshold), 0, "F%d" },
    { "arraycopyRepMovsLongArrayThreshold=",
        "C<nnn>\tLong Array copy threshold for using REP MOVS instructions. Only supports 32, 64, or 128 bytes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _arraycopyRepMovsLongArrayThreshold), 0, "F%d" },
    { "arraycopyRepMovsReferenceArrayThreshold=",
        "C<nnn>\tReference array copy threshold for using REP MOVS instructions. Only supports 32, 64, or 128 bytes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _arraycopyRepMovsReferenceArrayThreshold), 0, "F%d" },
    { "assignEveryGlobalRegister",
        "I\tnever refuse to assign any possible register for GRA in spite of the resulting potential spills",
        SET_OPTION_BIT(TR_AssignEveryGlobalRegister), "F" },
    { "assumeStartupPhaseUntilToldNotTo",
        "M\tUse compiler.Command("
        "endOfStartup"
        ") to exit startup phase",
    ...
    { "bigCalleeFreqCutoffAtHot=",
        "O<nnn>\tInliner threshold for block frequency for cold callees for opt level higher then warm",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _bigCalleeFreqCutoffAtHot), 40, "F%d" },
    { "bigCalleeFreqCutoffAtWarm=",
        "O<nnn>\tInliner threshold for block frequency for cold callees for opt level less of equal to warm",
    ...
    { "classesWithFoldableFinalFields=",
        ...
        "considered safe.",
        TR::Options::setRegex, offsetof(OMR::Options, _classesWithFolableFinalFields), 0, "F" },
    ...
    { "debugCounters=",
        ...
        "criterion)",
        TR::Options::setRegex, offsetof(OMR::Options, _enabledDynamicCounterNames), 0, "F" },
    { "debugCounterWarmupSeconds=",
        ...
        TR::Options::set64BitSignedNumeric, offsetof(OMR::Options, _debugCounterWarmupSeconds), 0, "F%d" },
    ...
    { "delayCompileWithCPUBurn=",
        "I<nnn>\tDelay compilation by burning the specified amount of CPU (in ms) before compilation is started",
    ...
    { "disableAllocationSinking",
        "O\tdon't delay object allocations until immediately before the corresponding constructor calls",
        TR::Options::disableOptimization, allocationSinking, 0, "P" },
    ...
    { "disableAOTWarmRunThroughputImprovement",
        "O\tdisable change iprofiler entry choosing heuristic to improve aot warm run throughput",
        SET_OPTION_BIT(TR_DisableAOTWarmRunThroughputImprovement), "F" },
    { "disableArrayCopyByteArrayInlineSmallSizeWithoutREPMOVS",
        "O\tdisable array copy optimizations enhancement for 8 bit primitive array",
        SET_OPTION_BIT(TR_Disable8BitPrimitiveArrayCopyInlineSmallSizeWithoutREPMOVS), "F" },
    { "disableArrayCopyCharArrayInlineSmallSizeWithoutREPMOVS",
        "O\tdisable array copy optimizations enhancement for 16 bit primitive array",
        SET_OPTION_BIT(TR_Disable16BitPrimitiveArrayCopyInlineSmallSizeWithoutREPMOVS), "F" },
    { "disableArrayCopyIntArrayInlineSmallSizeWithoutREPMOVS",
        "O\tdisable array copy optimizations enhancement for 32 bit primitive array",
        SET_OPTION_BIT(TR_Disable32BitPrimitiveArrayCopyInlineSmallSizeWithoutREPMOVS), "F" },
    { "disableArrayCopyLongArrayInlineSmallSizeWithoutREPMOVS",
        "O\tdisable array copy optimizations enhancement for 64 bit primitive array",
        SET_OPTION_BIT(TR_Disable64BitPrimitiveArrayCopyInlineSmallSizeWithoutREPMOVS), "F" },
    ...
    { "disableArrayCopyReferenceArrayInlineSmallSizeWithoutREPMOVS",
        "O\tdisable array copy optimizations enhancement for reference array",
        SET_OPTION_BIT(TR_DisableReferenceArrayCopyInlineSmallSizeWithoutREPMOVS), "F" },
    ...
    { "disableDememoization",
        ...
        "noncontiguous stack allocations",
        SET_OPTION_BIT(TR_DisableDememoization), "F" },
    ...
    { "disableDirectToJNIInline",
        "O\tdisable direct calls to JNI methods from jitted methods (but still create thunks)",
        SET_OPTION_BIT(TR_DisableDirectToJNIInline), "F" },
    ...
    { "disableFastDLTOnLongRunningInterpreter",
        "O\tdisable logic to trigger DLT when a compiled body exists, but we're receiving interpreter ticks",
    ...
    { "disableGuardedCallArgumentRemat",
        ...
        "the mainline path",
        SET_OPTION_BIT(TR_DisableGuardedCallArgumentRemat), "F" },
    ...
    { "disableHardwareProfilerDataCollection",
        "O\tdisable the collection of hardware profiler information while maintaining the framework",
        SET_OPTION_BIT(TR_DisableHWProfilerDataCollection), "F", NOT_IN_SUBSET },
    ...
    { "disableHierarchyInlining",
        "O\tdisable inlining of overridden methods not overridden in subclass of the type of this pointer",
        SET_OPTION_BIT(TR_DisableHierarchyInlining), "F" },
    ...
    { "disableInliningUnrecognizedIntrinsics",
        "M\tdisable inlining of IntrinsicCandidate that is not a recognized method",
        SET_OPTION_BIT(TR_DisableInliningUnrecognizedIntrinsics), "F" },
    ...
    { "disableIprofilerDataCollection",
        "M\tdisables the collection of iprofiler information while maintaining the framework",
        SET_OPTION_BIT(TR_DisableIProfilerDataCollection), "F", NOT_IN_SUBSET },
    { "disableIprofilerDataDisclaiming",
        "M\tdisable memory disclaiming for IProfiler memory segments (linux specific).",
        SET_OPTION_BIT(TR_DisableIProfilerDataDisclaiming), "F", NOT_IN_SUBSET },
    ...
    { "disableJProfilingInProfilingCompilations",
        "O\tDisable use of jprofiling instrumentation in profiling compilations",
        SET_OPTION_BIT(TR_DisableJProfilingInProfilingCompilations), "F" },
    ...
    { "disableLoopReplicatorColdSideEntryCheck",
        "I\tdisable cold side-entry check for replicating loops containing hot inner loops",
        SET_OPTION_BIT(TR_DisableLoopReplicatorColdSideEntryCheck), "P" },
    ...
    { "disableMarshallingIntrinsics",
        "O\tDisable packed decimal to binary marshalling and un-marshalling optimization. They will not be inlined.",
        SET_OPTION_BIT(TR_DisableMarshallingIntrinsics), "F" },
    ...
    { "disableMCSBypass",
        ...
        "MutableCallSite's target MethodHandle in jitted code",
        SET_OPTION_BIT(TR_DisableMCSBypass), "F" },
    ...
    { "disableMethodIsCold",
        ...
        "interpreted",
        SET_OPTION_BIT(TR_DisableMethodIsCold), "F" },
    { "disableMHCustomizationLogicCalls",
        "C\tdo not insert calls to MethodHandle.doCustomizationLogic for handle invocations outside of thunks",
        RESET_OPTION_BIT(TR_EnableMHCustomizationLogicCalls), "F" },
    ...
    { "disableMutableCallSiteGuards",
        ...
        SET_OPTION_BIT(TR_DisableMutableCallSiteGuards), "F" },
    ...
    { "disablePackedDecimalIntrinsics",
        ...
        "instructions on z/Architecture.",
        SET_OPTION_BIT(TR_DisablePackedDecimalIntrinsics), "F" },
    ...
    { "disableReducedPriorityForCustomMethodHandleThunks",
        "R\tcompile custom MethodHandle invoke exact thunks at the same priority as normal java methods",
        SET_OPTION_BIT(TR_DisableReducedPriorityForCustomMethodHandleThunks), "F", NOT_IN_SUBSET },
    ...
    { "disableRefinedAliases",
        ...
        "compilations",
        SET_OPTION_BIT(TR_DisableRefinedAliases), "F" },
    ...
    { "disableRegisterPressureSimulation",
        "O\tdon't walk the trees to estimate register pressure during global register allocation",
        SET_OPTION_BIT(TR_DisableRegisterPressureSimulation), "F" },
    ...
    { "disableSIMD",
        "O\tdisable SIMD exploitation and infrastructure on platforms supporting vector register and instructions",
        SET_OPTION_BIT(TR_DisableSIMD), "F" },
    ...
    { "disableSmartPlacementOfCodeCaches",
        "O\tdisable placement of code caches in memory so they are near each other and the DLLs",
        SET_OPTION_BIT(TR_DisableSmartPlacementOfCodeCaches), "F", NOT_IN_SUBSET },
    ...
    { "disableStringBuilderTransformer",
        "O\tenable transforming StringBuilder constructor to preallocate a buffer for String concatenation operations",
        SET_OPTION_BIT(TR_DisableStringBuilderTransformer), "F" },
    ...
    { "disableUpdateJITBytesSize",
        "M\tDon't send VM size of IProfiler Entries and Hints that could have been persisted if the SCC wasn't full",
        SET_OPTION_BIT(TR_DisableUpdateJITBytesSize), "F", NOT_IN_SUBSET },
    ...
    { "disableUpgradingColdCompilations",
        "R\tdisable upgrading to warm those methods compiled at cold due to classLoadPhase",
        SET_OPTION_BIT(TR_DisableUpgradingColdCompilations), "F", NOT_IN_SUBSET },
    ...
    { "disclaimMemoryOnSwap", "M\tuse swap file as a backup for disclaimed memory (linux only).",
        SET_OPTION_BIT(TR_DisclaimMemoryOnSwap), "F", NOT_IN_SUBSET },
    ...
    { "dontActivateCompThreadWhenHighPriReqIsBlocked",
        "M\tdo not activate another compilation thread when high priority request is blocked",
        RESET_OPTION_BIT(TR_ActivateCompThreadWhenHighPriReqIsBlocked), "F", NOT_IN_SUBSET },
    ...
    { "dontDowngradeToColdDuringGracePeriod",
        "M\tdon't downgrade first time compilations from warm to cold during grace period (first second of run)",
        SET_OPTION_BIT(TR_DontDowgradeToColdDuringGracePeriod), "F", NOT_IN_SUBSET },
    ...
    { "dontFoldStaticFinalFields=",
        ...
        "'Foo.BAR:LBar;'",
        TR::Options::setRegex, offsetof(OMR::Options, _dontFoldStaticFinalFields), 0, "P" },
    ...
    { "dontJitIfSlotsSharedByRefAndNonRef",
        "O\tfail the compilation (in FSD mode) if a slot needs to be shared between an address and a nonaddress.",
        SET_OPTION_BIT(TR_DontJitIfSlotsSharedByRefAndNonRef), "F" },
    ...
    { "dontUseHigherCountsForNonSCCMethods",
        "M\tDo not use the default high counts for methods belonging to classes not in SCC",
        RESET_OPTION_BIT(TR_UseHigherCountsForNonSCCMethods), "F", NOT_IN_SUBSET },
    { "dontUseHigherMethodCountsAfterStartup",
        "M\tDo not use the default high counts for methods after startup in AOT mode",
        RESET_OPTION_BIT(TR_UseHigherMethodCountsAfterStartup), "F", NOT_IN_SUBSET },
    ...
    { "dontUseLowerCountsForNonSCCMethodsDuringStartup",
        "M\tDo not use lower counts for methods belonging to classes not in SCC",
        RESET_OPTION_BIT(TR_UseLowerCountsForNonSCCMethodsDuringStartup), "F", NOT_IN_SUBSET },
    ...
    { "enableAggressiveLoopVersioning",
        "O\tOptions and thresholds that result in loop versioning occurring in more cases",
        SET_OPTION_BIT(TR_EnableAggressiveLoopVersioning), "F" },
    ...
    { "enableAllocationSinking",
        "O\tdelay object allocations until immediately before the corresponding constructor calls",
        TR::Options::enableOptimization, allocationSinking, 0, "P" },
    ...
    { "enableCFGEdgeCounters",
        "O\tenable CFG edge counters to keep track of taken and non taken branches in compiled code",
        SET_OPTION_BIT(TR_EnableCFGEdgeCounters), "F" },
    ...
    { "enableDowngradeOnHugeQSZ",
        "M\tdowngrade first time compilations when the compilation queue is huge (1000+ entries)",
        SET_OPTION_BIT(TR_EnableDowngradeOnHugeQSZ), "F", NOT_IN_SUBSET },
    ...
    { "enableGuardedCountingRecompilation",
        "O\tinserts recompilation counters with guards to allow the compiler to selectively trigger recompilation",
        RESET_OPTION_BIT(TR_DisableGuardedCountingRecompilations), "F" },
    ...
    { "enableInterfaceCallCachingSingleDynamicSlot",
        "O\tenable interfaceCall caching with one slot storing J9MethodPtr   ",
        SET_OPTION_BIT(TR_enableInterfaceCallCachingSingleDynamicSlot), "F" },
    ...
    { "enableJITServerFollowRemoteCompileWithLocalCompile",
        "O\tenable JITServer to perform local compilations for its remotely compiled methods",
        SET_OPTION_BIT(TR_JITServerFollowRemoteCompileWithLocalCompile), "F" },
    ...
    { "enableJProfilingInProfilingCompilations",
        "O\tEnable the use of jprofiling instrumentation in profiling compilations",
        RESET_OPTION_BIT(TR_DisableJProfilingInProfilingCompilations), "F" },
    { "enableLastRetrialLogging",
        "O\tenable fullTrace logging for last compilation attempt. Needs to have a log defined on the command line",
        SET_OPTION_BIT(TR_EnableLastCompilationRetrialLogging), "F" },
    ...
    { "enableLoopVersionerCountAllocFences",
        "O\tallow loop versioner to count allocation fence nodes on PPC toward a profiled guard's block total",
        SET_OPTION_BIT(TR_EnableLoopVersionerCountAllocationFences), "F" },
    { "enableLowerCompilationLimitsDecisionMaking",
        "O\tenable the piece of code that lowers compilation limits when low on virtual memory (on Linux and z/OS)",
    ...
    { "enableMHCustomizationLogicCalls",
        "C\tinsert calls to MethodHandle.doCustomizationLogic for handle invocations outside of thunks",
        SET_OPTION_BIT(TR_EnableMHCustomizationLogicCalls), "F" },
    ...
    { "enableMutableCallSiteGuards",
        ...
        "(including invokedynamic)",
        RESET_OPTION_BIT(TR_DisableMutableCallSiteGuards), "F" },
    ...
    { "enableRefinedAliases",
        ...
        "compilations",
        RESET_OPTION_BIT(TR_DisableRefinedAliases), "F" },
    ...
    { "enableRegisterPressureSimulation",
        "O\twalk the trees to estimate register pressure during global register allocation",
        RESET_OPTION_BIT(TR_DisableRegisterPressureSimulation), "F" },
    ...
    { "enableSnapshotBlockOpts",
        "O\tenable block ordering/redirecting optimizations in the presences of snapshot nodes",
        SET_OPTION_BIT(TR_EnableSnapshotBlockOpts), "F" },
    ...
    { "enableUpgradesByJitSamplingWhenHWProfilingEnabled",
        "O\tAllow Jit Sampling to upgrade cold compilations when HW Profiling is on",
    ...
    { "enableVectorAPIExpansion",
        "M\tenable expansion of Vector API, disableVectorAPIExpansion option takes precedence",
        SET_OPTION_BIT(TR_EnableVectorAPIExpansion), "F" },
    ...
    { "enforceVectorAPIExpansion",
        "O\tSets up platform specific sub options to enforce Vector API Expansion of Vector API Intrinsics",
        SET_OPTION_BIT(TR_EnforceVectorAPIExpansion), "F" },
    ...
    { "extractExitsByInvalidatingStructure",
        ...
        SET_OPTION_BIT(TR_ExtractExitsByInvalidatingStructure), "F" },
    ...
    { "fanInCallGraphFactor=",
        "R<nnn>\tFactor by which the weight of the callgraph for a particular caller is multiplied",
    ...
    { "GCRdecCount=",
        "R<nnn>\tthe value by which the counter is decremented by guarded counting recompilations (positive value)",
    ...
    { "highCodeCacheOccupancyBCount=",
        "R<nnn>\tthe initial invocation count used during high code cache occupancy for methods with loops",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_highCodeCacheOccupancyBCount, 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "highCodeCacheOccupancyPercentage=",
        "R<nnn>\tthe percentage at which the code cache is considered to be at high occupancy",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_highCodeCacheOccupancyPercentage, 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "hotMaxStaticPICSlots=",
        ...
        " A negative value -N means use N times the maxStaticPICSlots setting.",
    ...
    { "inlineCntrCalleeTooDeepBucketSize=",
        ...
        "big",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrCalleeTooDeepBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrColdAndNotTinyBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the callee is cold and not tiny",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrColdAndNotTinyBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrDepthExceededBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the depth in the call chain is too much",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrDepthExceededBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrRanOutOfBudgetBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the caller ran out of budget",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrRanOutOfBudgetBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrWarmCalleeHasTooManyNodesBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the warm callee has too many nodes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrWarmCalleeHasTooManyNodesBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrWarmCalleeTooBigBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the warm callee is too big",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrWarmCalleeTooBigBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    { "inlineCntrWarmCallerHasTooManyNodesBucketSize=",
        "R<nnn>\tThe unit used to categorize uninlined calls when the warm caller has too many nodes",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlineCntrWarmCallerHasTooManyNodesBucketSize), 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "inlinerArgumentHeuristicFractionBeyondWarm=",
        "O<nnn>\tAt hotness above warm, decreases estimated size by 1/N when the inliner's argument heuristic kicks in",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlinerArgumentHeuristicFractionBeyondWarm), 0, "F%d" },
    { "inlinerArgumentHeuristicFractionUpToWarm=",
        ...
        "heuristic kicks in",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _inlinerArgumentHeuristicFractionUpToWarm), 0, "F%d" },
    ...
    { "interpreterSamplingDivisorInStartupMode=",
        "R<nnn>\tThe divisor used to decrease the invocation count when an interpreted method is sampled",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_interpreterSamplingDivisorInStartupMode, 0, "F%d",
        NOT_IN_SUBSET },
    { "iprofilerPerformTimestampCheck",
        "O\tInterpreter Profiling will perform some validity checks based on timestamps",
    ...
    { "lastOptSubIndex=",
        "O<nnn>\tindex of the last opt transformation to perform within the last optimization (see lastOptIndex)",
    ...
    { "maxNumVisitedSubclasses=",
        "R<nnn>\tmaximum number of subclasses allowed per chtable lookup for a given call site",
    ...
    { "maxSpreadCountLoopless=",
        "O<nnn>\tnumber of maximum additional invocations before compiling methods without loops",
    ...
    { "maxStaticPICSlots=",
        " <nnn>\tmaximum number of polymorphic inline cache slots pre-populated from profiling info",
        TR::Options::set32BitSignedNumeric, offsetof(OMR::Options, _maxStaticPICSlots), 0, "F%d" },
    { "maxUnloadedAddressRanges=",
        " <nnn>\tmaximum number of entries in arrays of unloaded class/method address ranges",
    ...
    { "memUsage=",
        "D\tgather lexical memory profiling statistics of the list of memory types: stack, heap or persistent",
    ...
    { "mimicInterpreterFrameShape",
        "O\tMake sure all locals are laid out in the stack frame just as they would be for the interpreter",
        SET_OPTION_BIT(TR_MimicInterpreterFrameShape), "F" },
    ...
    { "minProfiledCheckcastFrequency=",
        ...
        "Percentage:  0-100",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_minProfiledCheckcastFrequency, 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "numInterfaceCallCacheSlots=",
        "C<nnn>\tThe number of cache slots allocated per interface callpoint, 64 bit zseries only, default is 4",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _numInterfaceCallCacheSlots), 4, "F%d" },
    { "numInterfaceCallStaticSlots=",
        "C<nnn>\tThe number of static slots allocated per interface callpoint, 64 bit zseries only, default is 1",
    ...
    { "optFile=",
        "O<filename>\tRead in 'Performing' statements from <filename> and perform those opts instead of the usual ones",
    ...
    { "profilingCompNodecountThreshold=",
        "M<nnn>\tthreshold for doubling the method to do a profiling compile is considered expensive",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_profilingCompNodecountThreshold, 0, "F%d",
        NOT_IN_SUBSET },
    ...
    { "randomSeedSignatureHash",
        ...
        SET_OPTION_BIT(TR_RandomSeedSignatureHash), "F" },
    ...
    { "sampleInterval=",
        "R<nnn>\tThe number of samples taken on a method between times when it is considered for recompilation",
    ...
    { "startupMethodDontDowngradeThreshold=",
        "O<nnn> Certain methods below this threshold will not be downgraded during startup",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_startupMethodDontDowngradeThreshold, 0, "F%d",
        NOT_IN_SUBSET },
    { "staticDebugCounters",
        "D\tEnable static versions of all enabled dynamic debug counters (unless staticDebugCounters={regex} is "
        "specified)",
        SET_OPTION_BIT(TR_StaticDebugCountersRequested), "F" },
    ...
    { "suppressEA=",
        "O{regex}\tSuppress stack allocations by Escape Analysis at locations that match the specified regex",
        TR::Options::setRegex, offsetof(OMR::Options, _suppressEA), 0, "P" },
    ...
    { "terseRegisterPressureTrace",
        "L\tinclude only summary info about register pressure tracing when traceGRA is enabled",
        SET_OPTION_BIT(TR_TerseRegisterPressureTrace), "P" },
    ...
    { "tracePREForSubNodeReplacement",
        "L\ttrace partial redundancy elimination focussed on optimal subnode replacement",
        SET_OPTION_BIT(TR_TracePREForOptimalSubNodeReplacement), "P" },
    ...
    { "traceRegisterPressureDetails",
        "L\tinclude extra register pressure annotations in register pressure simulation and tree evaluation traces",
        SET_OPTION_BIT(TR_TraceRegisterPressureDetails), "P" },
    ...
    { "trustAllInterfaceTypeInfo",
        ...
        "much more lackadasical about interface type safety, and requires us to be conservative.",
        SET_OPTION_BIT(TR_TrustAllInterfaceTypeInfo), "F" },
    ...
    { "unleashStaticFieldFolding",
        "O\tbypass the class allow-list, and allow static final fields to be folded aggressively",
        RESET_OPTION_BIT(TR_RestrictStaticFieldFolding), "F" },
    ...
    { "useGlueIfMethodTrampolinesAreNotNeeded",
        "O\tSafety measure; return to the old behaviour of always going through the glue",
        SET_OPTION_BIT(TR_UseGlueIfMethodTrampolinesAreNotNeeded), "F" },
    ...
    { "useLowerCountsForNonSCCMethodsDuringStartup",
        "M\tUse lower counts for methods belonging to classes not present in SCC",
        SET_OPTION_BIT(TR_UseLowerCountsForNonSCCMethodsDuringStartup), "F", NOT_IN_SUBSET },
    ...
    { "userSpaceVirtualMemoryMB=",
        "O<nnn>\tsize of the virtual memory that is user space in MB (not used on Windows, AIX, or 64 bit systems)",
    ...
    { "useSamplingJProfilingForInterpSampledMethods",
        "M\tHeuristic. Use samplingJProfiling for methods sampled by interpreter",
        SET_OPTION_BIT(TR_UseSamplingJProfilingForInterpSampledMethods), "F", NOT_IN_SUBSET },
    ...
    { "virtualMemoryCheckFrequencySec=",
        "O<nnn>\tFrequency of the virtual memory check (only applicable for 32 bit systems)",
        TR::Options::setStaticNumeric, (intptr_t)&OMR::Options::_virtualMemoryCheckFrequencySec, 0, "F%d",
        NOT_IN_SUBSET },
    { "waitOnCompilationQueue",
        "M\tPerform synchronous wait until compilation queue empty. Primarily for use with Compiler.command",
        SET_OPTION_BIT(TR_WaitBit), "F", NOT_IN_SUBSET },
...
bool OMR::Options::jitLatePostProcess(TR::OptionSet* optionSet, void* jitConfig)
{
    ...
    } else // option set processing
    {
        ...
        if (_logFileName) {
            ...
            if (_debug) {
                _logFile = _debug->findLogFile(
                    TR::Options::getAOTCmdLineOptions(), TR::Options::getJITCmdLineOptions(), optionSet, _logFileName);
                if (_logFile == NULL)
                    self()->openLogFile();
                else
            ...
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_INFO, "Trace options require a log file to be specified: log=<filename>");
            return false;
        }
...
TR::OptionSet* OMR::Options::findOptionSet(
    int32_t index, int32_t lineNum, const char* methodSignature, TR_Hotness hotnessLevel, bool isAOT)
{
        ...
        if (index && optionSet->getIndex() == index) {
            ...
            if (TR::SimpleRegex::match(optionSet->getMethodRegex(), methodSignature)) {
                ...
                if (!optionSet->getOptLevelRegex())
                    break;
                if (TR::SimpleRegex::matchIgnoringLocale(
                        optionSet->getOptLevelRegex(), TR::Compilation::getHotnessName(hotnessLevel)))
                    break;
...
const char* OMR::Options::processOptions(
    const char* options, const char* envOptions, void* feBase, TR_FrontEnd* fe, TR::Options* cmdLineOptions)
{
...
const char* OMR::Options::processOptions(const char* options, const char* envOptions, TR::Options* cmdLineOptions)
{
    ...
    options = TR::Options::processOptionSet(
        options, envOptions, cmdLineOptions, (cmdLineOptions == TR::Options::getAOTCmdLineOptions()));
    if (*options)
        return options;
...
const char* OMR::Options::processOptionSet(
    const char* options, const char* envOptions, TR::Options* jitBase, bool isAOT)
{
    ...
    while (*options && *options != ')') {
        ...
        if (!optionSet) {
            ...
            // filter starts with a limitfile linenumber range
                ...
                endOpt = options + 1;
            } else if (!strnicmp_ignore_locale(
                           options, EXCLUDED_METHOD_OPTIONS_PREFIX, strlen(EXCLUDED_METHOD_OPTIONS_PREFIX))) {
            ...
            if (newSet) {
                if (*filterHeader == '{') {
                    ...
                    newSet->setEnd(endLine);
                               ...
                               filterHeader, EXCLUDED_METHOD_OPTIONS_PREFIX, strlen(EXCLUDED_METHOD_OPTIONS_PREFIX))) {
...
const char* OMR::Options::processOptionSetPostRestore(
    void* jitConfig, const char* options, TR::Options* optBase, bool isAOT)
{
...
const char* OMR::Options::processOption(
    const char* startOption, TR::OptionTable* table, void* base, int32_t numEntries, TR::OptionSet* optionSet)
{
...
bool OMR::Options::jitPostProcess()
{
    ...
    if (_logFileName) {
        ...
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_FAILURE, "Log file option must be specified when a trace options is used: log=<filename>");
        return false;
    }
    ...
    if (_optFileName) {
        ...
        if (_debug) {
            ...
            if (_customStrategy) {
                ...
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_INFO, "Ignoring optFile option; unable to read opts from '%s'", _optFileName);
            }
        }
    }
...
bool OMR::Options::requiresLogFile()
{
    ...
    //
    if (self()->getAnyOption(TR_TraceAll) || self()->getAnyOption(TR_TraceBBVA) || self()->getAnyOption(TR_TraceBVA)
        || self()->getAnyOption(TR_TraceCG) || self()->getAnyOption(TR_TraceUseDefs)
        ...
        *((int32_t*)((char*)base + entry->parm1)) = 0; // Reset all bits to zero when this function is called
        ...
        if (!regex)
            ...
            if (*((int32_t*)((char*)base + entry->parm1)) == 0x00000000)
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_INFO, "Address enumeration option not found. No address enumeration option was set.");
        }
    }
...
OMR::Options::TR_OptionStringToBit OMR::Options::_optionStringToBitMapping[] = {
    ...
    // GPU Options
    { "default", TR_EnableGPU }, { "enforce", TR_EnableGPUForce }, { "verbose", TR_EnableGPUVerbose },
    { "details", TR_EnableGPUDetails }, { "safeMT", TR_EnableSafeMT }, { "enableMath", TR_EnableGPUEnableMath },
...
const char* OMR::Options::_verboseOptionNames[TR_NumVerboseOptions] = { "options", "compileStart", "compileEnd",
    ...
    "CheckpointRestore", "CheckpointRestoreDetails", "RSSReport", "RSSReportDetailed" };
...
const char* OMR::Options::setVerboseBitsHelper(
    const char* option, VerboseOptionFlagArray* verboseOptionFlags, uintptr_t defaultVerboseFlags)
{
...
const char* OMR::Options::setHotFieldReductionAlgorithm(const char* option, void* base, TR::OptionTable* entry)
{
    ...
    if (!foundMatch) {
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_INFO, "<JIT: Invalid reduction algorithm option provided. Default max reduction algorithm set.>");
        _hotFieldReductionAlgorithms.set(TR_HotFieldReductionAlgorithmMax);
    }
    return option;
}
