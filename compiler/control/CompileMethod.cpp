...
static void writePerfToolEntry(void *start, uint32_t size, const char *name)
{
    // race condition
    ...
    if (firstAttempt) {
        ...
        bool truncated = TR::snprintfTrunc(perfFilename, maxPerfFilenameSize, "/tmp/perf-%" OMR_PRId64 ".map",
            static_cast<int64_t>(jvmPid));
        if (!truncated) {
    ...
    if (perfFile) {
        ...
        // j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t) getMetadata()->startPC, getMetadata()->endWarmPC -
        // getMetadata()->startPC,
        //               getCompilation()->signature(),
        ...
        //    {
        //    j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t) getMetadata()->startColdPC,
        //    getMetadata()->endPC - getMetadata()->startColdPC,
        //                  TR::comp()()->signature(),
        //                  getCompilation()->getHotnessName(getCompilation()->getMethodHotness())); // should we change
...
int32_t commonJitInit(OMR::FrontEnd &fe, char *cmdLineOptions)
{
    ...
    if (initializedTOC) {
        TR_PPCTableOfConstants *tocManagement
            = static_cast<TR_PPCTableOfConstants *>(fe.getPersistentInfo()->getPersistentTOC());
        pseudoTOC = tocManagement->getTOCBase();
    } else {
        pseudoTOC
            = (void *)TR_PPCTableOfConstants::initTOC(&fe, fe.getPersistentInfo(), jitConfig->getInterpreterTOC());
        initializedTOC = true;
    }
...
static bool methodCanBeCompiled(OMR::FrontEnd *fe, TR_ResolvedMethod &method, TR_FilterBST *&filter,
    TR_Memory *trMemory)
{
...
void registerTrampoline(uint8_t *start, uint32_t size, const char *name)
{
...
uint8_t *compileMethodFromDetails(OMR_VMThread *omrVMThread, TR::IlGeneratorMethodDetails &details, TR_Hotness hotness,
    int32_t &rc)
{
    ...
    if (0 == (plan = TR_OptimizationPlan::alloc(hotness, false, false))) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileExclude))
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, "%s out-of-memory allocating optimization plan",
    ...
    TR::Options options(&trMemory, optionSetIndex, lineNumber, &compilee, 0, plan, false);
    ...
    TR_ASSERT(TR::comp() == NULL,
        "there seems to be a current TLS TR::Compilation object %p for this thread. At this point there should be no "
        "current TR::Compilation object",
        TR::comp());
        ...
        if (rc == COMPILATION_SUCCEEDED) // success!
        {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance)) {
                ...
                TR_VerboseLog::write(TR_Vlog_COMP, "(%s) %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
                    compiler.getHotnessName(compiler.getMethodHotness()), signature, startPC,
                    compiler.cg()->getCodeEnd());

                if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
                    TR_VerboseLog::write(" time=%llu mem=%lluKB", translationTime,
            ...
            if (compiler.getOption(TR_PerfTool) || compiler.getOption(TR_EmitExecutableELFFile)
                ...
                codeCacheManager.registerCompiledMethod(compiler.externalName(), startPC,
                ...
                if (compiler.getOption(TR_PerfTool)) {
                    generatePerfToolEntry(startPC, codeGenerator.getCodeEnd(), compiler.signature(),
