...
static void writePerfToolEntry(void* start, uint32_t size, const char* name)
{
    ...
    if (firstAttempt) {
        ...
        bool truncated = TR::snprintfTrunc(
            perfFilename, maxPerfFilenameSize, "/tmp/perf-%" OMR_PRId64 ".map", static_cast<int64_t>(jvmPid));
        if (!truncated) {
...
static bool methodCanBeCompiled(
    OMR::FrontEnd* fe, TR_ResolvedMethod& method, TR_FilterBST*& filter, TR_Memory* trMemory)
{
...
uint8_t* compileMethodFromDetails(
    OMR_VMThread* omrVMThread, TR::IlGeneratorMethodDetails& details, TR_Hotness hotness, int32_t& rc)
{
    ...
    if (0 == (plan = TR_OptimizationPlan::alloc(hotness, false, false))) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileExclude))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_INFO, "%s out-of-memory allocating optimization plan", compilee.signature(&trMemory));
        return 0;
    }
    ...
    try {
        ...
        if (rc == COMPILATION_SUCCEEDED) // success!
        {
            ...
            if (compiler.getOption(TR_PerfTool) || compiler.getOption(TR_EmitExecutableELFFile)
                ...
                codeCacheManager.registerCompiledMethod(
                    compiler.externalName(), startPC, codeGenerator.getCodeLength());
