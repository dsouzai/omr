...
TR::CodeCache* OMR::CodeCacheManager::initialize(
    bool allocateMonolithicCodeCache, uint32_t numberOfCodeCachesToCreateAtStartup)
{
    ...
    if (_symbolContainer == NULL) {
        TR::CodeCacheSymbolContainer* symbolContainer
            = static_cast<TR::CodeCacheSymbolContainer*>(self()->getMemory(sizeof(TR::CodeCacheSymbolContainer)));
    ...
    if (allocateMonolithicCodeCache) {
        ...
        if (self()->allocateCodeCacheRepository(size)) {
            ...
            if (config.canChangeNumCodeCaches()) {
                // code caches will all be nearby each other in the repository so no need to allocate more than 1 up
                // front
            ...
            if (config.verboseCodeCache()) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "failed to allocate codeCacheRepository of size %u KB",
                    (uint32_t)(config.codeCacheTotalKB()));
            }
        }
    }
...
#define REACHEABLE_RANGE_KB (2048 * 1024)
    ...
    int32_t cachesCreatedOnInit
        = std::min<int32_t>(config.maxNumberOfCodeCaches(), numberOfCodeCachesToCreateAtStartup);
...
void OMR::CodeCacheManager::destroy()
{
#if (HOST_OS == OMR_LINUX)
    // if code cache should be written out as shared object, do that now before destroying anything

    if (_elfRelocatableGenerator) {
        TR_ASSERT(_elfRelocatableGenerator->emitELF((const char*)_objectFileName, _relocatableSymbolContainer->_head,
                      _relocatableSymbolContainer->_numSymbols, _relocatableSymbolContainer->_totalSymbolNameLength,
                      _relocations->_head, _relocations->_numRelocations),
            "Failed to write code cache symbols to executable ELF file.");
    }

    if (_elfExecutableGenerator) {
        pid_t jvmPid = getpid();
        static const int maxElfFilenameSize
            = 15 + sizeof(jvmPid) * 3; // "written to file: /tmp/perf-%d.jit, where d is the pid"
        ...
        if (numCharsWritten > 0 && numCharsWritten < maxElfFilenameSize) {
            TR_ASSERT(_elfExecutableGenerator->emitELF((const char*)&elfFilename, _symbolContainer->_head,
                          _symbolContainer->_numSymbols, _symbolContainer->_totalSymbolNameLength),
...
void* OMR::CodeCacheManager::getMemory(size_t sizeInBytes) { return _rawAllocator.allocate(sizeInBytes, std::nothrow); }

void OMR::CodeCacheManager::freeMemory(void* memoryToFree) { _rawAllocator.deallocate(memoryToFree); }

TR::CodeCache* OMR::CodeCacheManager::allocateCodeCacheObject(
    TR::CodeCacheMemorySegment* codeCacheSegment, size_t codeCacheSize)
{
...
//
void OMR::CodeCacheManager::addCodeCache(TR::CodeCache* codeCache)
{
...
void OMR::CodeCacheManager::unreserveCodeCache(TR::CodeCache* codeCache)
{
    if (!codeCache)
        return;
...
TR::CodeCache* OMR::CodeCacheManager::reserveCodeCache(
    bool compilationCodeAllocationsMustBeContiguous, size_t sizeEstimate, int32_t compThreadID, int32_t* numReserved)
{
            ...
            if (!codeCache->isReserved()) // we cannot touch the reserved ones
            {
                ...
                if (almostFull == TR_no || (almostFull == TR_maybe && !compilationCodeAllocationsMustBeContiguous)) {
                    ...
                    if (sizeEstimate == 0 || // If size estimate is not given we'll blindly pick anything
                        ...
                        || codeCache->getSizeOfLargestFreeWarmBlock()
                            >= sizeEstimate // we don't know yet the warm/cold requirements
...
//----------------------------------------------------------------------------
TR::CodeCache* OMR::CodeCacheManager::getNewCodeCache(int32_t reservingCompThreadID)
{
    TR::CodeCache* codeCache = NULL;
    if (self()->canAddNewCodeCache()) {
        TR::CodeCacheConfig& config = self()->codeCacheConfig();
...
//
uint8_t* OMR::CodeCacheManager::allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize,
    TR::CodeCache** codeCache_pp, uint8_t** coldCode, bool needsToBeContiguous, bool isMethodHeaderNeeded)
{
    ...
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    methodBlockAddress = self()->allocateCodeMemoryWithRetries(warmCodeSize, coldCodeSize, codeCache_pp,
        (int32_t)config.codeCacheMethodBodyAllocRetries(), coldCode, needsToBeContiguous, isMethodHeaderNeeded);
...
void OMR::CodeCacheManager::performSizeAdjustments(
    size_t& warmCodeSize, size_t& coldCodeSize, bool needsToBeContiguous, bool isMethodHeaderNeeded)
{
    TR::CodeCacheConfig config = self()->codeCacheConfig();
...
//
TR::CodeCache* OMR::CodeCacheManager::findCodeCacheFromPC(void* inCacheAddress)
{
    TR::CodeCache* codeCache = self()->getFirstCodeCache();
    if (!codeCache)
        return NULL;
...
// callingPC.
//
OMR::CodeCacheTrampolineCode* OMR::CodeCacheManager::findMethodTrampoline(TR_OpaqueMethodBlock* method, void* callingPC)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
...
intptr_t OMR::CodeCacheManager::findHelperTrampoline(int32_t helperIndex, void* callSite)
{
...
//
void OMR::CodeCacheManager::synchronizeTrampolines()
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCacheManager::replaceTrampoline(TR_OpaqueMethodBlock* method, void* callSite,
    void* oldTrampoline, void* oldTargetPC, void* newTargetPC, bool needSync)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return NULL;
...
//
bool OMR::CodeCacheManager::canAddNewCodeCache()
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
...
//
uint8_t* OMR::CodeCacheManager::allocateCodeMemoryWithRetries(size_t warmCodeSize, size_t coldCodeSize,
    TR::CodeCache** codeCache_pp, int32_t allocationRetries, uint8_t** coldCode, bool needsToBeContiguous,
    bool isMethodHeaderNeeded)
{
    ...
    warmCodeAddress = codeCache->allocateCodeMemory(
        warmCodeSize, coldCodeSize, coldCode, needsToBeContiguous, isMethodHeaderNeeded);
            ...
            for (codeCache = self()->getFirstCodeCache(); codeCache; codeCache = codeCache->next()) {
                ...
                if (!codeCache->isReserved()) {
                    if (codeCache->almostFull() != TR_yes) {
                        ...
                        self()->performSizeAdjustments(warmSize, coldSize, needsToBeContiguous, isMethodHeaderNeeded);
            ...
            *codeCache_pp = codeCache; // code cache has been switched // MCT
            ...
            if (config.verboseCodeCache()) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    "Switching TR::CodeCache to %p @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, codeCache,
                    codeCache->getCodeBase(), codeCache->getCodeTop());
            }

            if (needsToBeContiguous) // We should abort the compilation if we change the cache during a compilation that
                                     // needs contiguous allocations
                return NULL;

            /* try allocating in the new cache */
            warmCodeAddress = self()->allocateCodeMemoryWithRetries(warmCodeSize, coldCodeSize, codeCache_pp,
                allocationRetries, coldCode, needsToBeContiguous, isMethodHeaderNeeded);
    ...
    fprintf(stderr, "Cache %p has %u bytes free. We requested %u bytes\n", *codeCache_pp,
        (*codeCache_pp)->getFreeContiguousSpace(), warmCodeSize + coldCodeSize);
#endif
    ...
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    size_t allocateSize = 2
        * (warmCodeSize + coldCodeSize + config.numRuntimeHelpers() * config.trampolineCodeSize()
...
#ifdef MCT_DEBUG
    fprintf(stderr,
        "cache %p reset reservation in allocateCodeMemory after new code cache allocation. New cache=%p "
        "needsToBeContiguous=%d allocationRetries=%d\n",
    ...
    *codeCache_pp = codeCache;
    ...
    /* try allocating in the new cache */
    warmCodeAddress = self()->allocateCodeMemoryWithRetries(warmCodeSize, coldCodeSize, codeCache_pp, allocationRetries,
        coldCode, needsToBeContiguous, isMethodHeaderNeeded);

    return warmCodeAddress;
}

TR::CodeCacheMemorySegment* OMR::CodeCacheManager::getNewCodeCacheMemorySegment(
    size_t segmentSize, size_t& codeCacheSizeAllocated)
{
...
TR::CodeCache* OMR::CodeCacheManager::allocateRepositoryCodeCache()
{
    TR::CodeCache* cache = static_cast<TR::CodeCache*>(self()->getMemory(sizeof(TR::CodeCache)));
    return cache;
}

TR::CodeCacheMemorySegment* OMR::CodeCacheManager::allocateCodeCacheRepository(size_t repositorySize)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    ...
    _codeCacheRepositorySegment
        = self()->allocateCodeCacheSegment(repositorySize, codeCacheSizeAllocated, startAddress);
    if (_codeCacheRepositorySegment) {
        ...
        *((TR::CodeCache**)start) = self()->getRepositoryCodeCacheAddress();
        ...
        _repositoryCodeCache->setColdCodeAlloc(
            (uint8_t*)(_codeCacheRepositorySegment->segmentTop() - _codeCacheRepositorySegment->segmentAlloc()));

        if (config.verboseCodeCache()) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "allocateCodeCacheRepository: size=%u heapBase=%p heapAlloc=%p heapTop=%p", codeCacheSizeAllocated,
                _codeCacheRepositorySegment->segmentBase(), _codeCacheRepositorySegment->segmentAlloc(),
...
void OMR::CodeCacheManager::registerCompiledMethod(const char* sig, uint8_t* startPC, uint32_t codeSize)
{
#if (HOST_OS == OMR_LINUX)
    ...
    if (_elfRelocatableGenerator) {
        TR::CodeCacheSymbol* newRelocSymbol
            = static_cast<TR::CodeCacheSymbol*>(self()->getMemory(sizeof(TR::CodeCacheSymbol)));
...
void OMR::CodeCacheManager::registerStaticRelocation(const TR::StaticRelocation& relocation)
{
#if (HOST_OS == OMR_LINUX)
    if (_elfRelocatableGenerator) {
        ...
        uint32_t symbolNumber
            = static_cast<uint32_t>(_relocatableSymbolContainer->_numSymbols - 1); // symbol index in the linked list
        uint32_t relocationType = _resolver.resolveRelocationType(relocation);
        TR::CodeCacheRelocationInfo* newRelocation
            = static_cast<TR::CodeCacheRelocationInfo*>(self()->getMemory(sizeof(TR::CodeCacheRelocationInfo)));
...
void* OMR::CodeCacheManager::chooseCacheStartAddress(size_t repositorySize)
{
    // default: NULL means don't try to pick a starting address
    return NULL;
}

void OMR::CodeCacheManager::increaseCurrTotalUsedInBytes(size_t size)
{
...
void OMR::CodeCacheManager::decreaseCurrTotalUsedInBytes(size_t size)
{
...
void OMR::CodeCacheManager::increaseFreeSpaceInCodeCacheRepository(size_t size)
{
...
void OMR::CodeCacheManager::decreaseFreeSpaceInCodeCacheRepository(size_t size)
{
    // VM DUMP agents set the segment->heapAlloc to (segment->heapTop - (coldCodeAlloc-warmCodeAlloc))
    // For the repository segment we will keep warmCodeAlloc at 0 and set coldCodeAlloc to
    // the value of free space in the entire repository
...
bool OMR::CodeCacheManager::isSufficientPhysicalMemoryAvailableForAllocation(size_t requestedCodeCacheSize)
{
    return true;
}

TR::CodeCacheMemorySegment* OMR::CodeCacheManager::carveCodeCacheSpaceFromRepository(
    size_t segmentSize, size_t& codeCacheSizeToAllocate)
{
    uint8_t* start = NULL;
    uint8_t* end = NULL;
    size_t freeSpace = 0;
    ...
    if (start) {
        if (config.verboseCodeCache())
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "carved size=%u range: " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, codeCacheSizeToAllocate,
                start, end);
    } else {
        if (config.verboseCodeCache() || config.verbosePerformance())
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                "failed to carve code cache of size=%zu from the repository. Free space in code cache repository= %zu. "
                "isSufficientPhysicalMemoryAvailableForAllocation = %s",
                codeCacheSizeToAllocate, freeSpace, shouldAllowCarving ? "true" : "false");
    }

    if (start) {
        TR::CodeCacheMemorySegment* memorySegment
            = self()->setupMemorySegmentFromRepository(start, end, codeCacheSizeToAllocate);
...
TR::CodeCacheMemorySegment* OMR::CodeCacheManager::setupMemorySegmentFromRepository(
    uint8_t* start, uint8_t* end, size_t& codeCacheSizeToAllocate)
{
    TR::CodeCacheMemorySegment* memorySegment
        = static_cast<TR::CodeCacheMemorySegment*>(self()->getMemory(sizeof(TR::CodeCacheMemorySegment)));
    new (static_cast<TR::CodeCacheMemorySegment*>(memorySegment)) TR::CodeCacheMemorySegment(start, end);
    return memorySegment;
}

void OMR::CodeCacheManager::freeMemorySegment(TR::CodeCacheMemorySegment* segment) { segment->free(self()); }

//--------------------------- undoCarvingFromRepository ----------------------------
void OMR::CodeCacheManager::undoCarvingFromRepository(TR::CodeCacheMemorySegment* segment)
{
    ...
    if (config.verboseCodeCache()) {
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_CODECACHE, "undoCarving start=%p size=%lu", start, allocatedCodeCacheSize);
    }
...
void OMR::CodeCacheManager::initializeRelocatableELFGenerator(void)
{
    ...
    TR::CodeCacheRelocationInfoContainer* relInfo = static_cast<TR::CodeCacheRelocationInfoContainer*>(
        self()->getMemory(sizeof(TR::CodeCacheRelocationInfoContainer)));
    relInfo->_head = NULL;
    relInfo->_tail = NULL;
    ...
    _elfRelocatableGenerator
        = new (_rawAllocator) TR::ELFRelocatableGenerator(_rawAllocator, _codeCacheRepositorySegment->segmentBase(),
            _codeCacheRepositorySegment->segmentTop() - _codeCacheRepositorySegment->segmentBase());
}

void OMR::CodeCacheManager::initializeExecutableELFGenerator(void)
{

    _elfExecutableGenerator
        = new (_rawAllocator) TR::ELFExecutableGenerator(_rawAllocator, _codeCacheRepositorySegment->segmentBase(),
            _codeCacheRepositorySegment->segmentTop() - _codeCacheRepositorySegment->segmentBase());
}
#endif // HOST_OS==OMR_LINUX

TR::CodeCache* OMR::CodeCacheManager::allocateCodeCacheFromNewSegment(
    size_t segmentSizeInBytes, int32_t reservingCompilationTID)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    ...
    TR::CodeCacheMemorySegment* codeCacheSegment
        = self()->getNewCodeCacheMemorySegment(segmentSizeInBytes, actualCodeCacheSizeAllocated);
        ...
        if (codeCache) {
            ...
            if (verboseCodeCache) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    ...
                    " HelperBase:" POINTER_PRINTF_FORMAT,
...
bool OMR::CodeCacheManager::isStartPCInRXCode(intptr_t startPC, void* jitConfig)
{
    return TR::CodeCacheManager::isAddressInRXCode(startPC, jitConfig);
}

bool OMR::CodeCacheManager::isAddressInRXCode(intptr_t address, void* jitConfig)
{
    /**
