...
public:
    ...
    enum Type { unknown, header, alignment, coldBlocks, overEstimate, OOL, snippet };
...
public:
    ...
    enum Grows { lowToHigh, highToLow };
    ...
    TR_PersistentList<RSSItem> getRSSItemList(uint8_t* address)
    {
        ...
        int32_t offset
            = static_cast<uint32_t>((_grows == lowToHigh) ? addressPage - startPage : startPage - addressPage);
...
public:
    ...
    public:
        RSSReportCriticalSection(RSSReport* rssReport)
            : CriticalSection(rssReport->_mutex)
        { }
    };
