...
//
bool OMR::CodeCache::initialize(TR::CodeCacheManager *manager, TR::CodeCacheMemorySegment *codeCacheSegment,
    size_t allocatedCodeCacheSizeInBytes)
{
    ...
    } else // Trampolines are needed
    {
        ...
        if (!config.needsMethodTrampolines()) {
            ...
            //
            // (_helperTop - segment->heapBase) is heapSize
            config.mccCallbacks().codeCacheConfig(static_cast<int32_t>(_helperTop - _segment->segmentBase()),
                &_tempTrampolinesMax);
        }
    ...
    if (size) {
        ...
        if (_trampolineReservationMark >= _trampolineBase + size) {
            ...
            if (config.verboseCodeCache()) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                    "CodeCache %p marked as full in reserveSpaceForTrampoline", self());
            }
        }
    }
...
//
OMR::CodeCacheTrampolineCode *OMR::CodeCache::replaceTrampoline(TR_OpaqueMethodBlock *method, void *oldTrampoline,
    ...
    if (_flags & CODECACHE_FULL_SYNC_REQUIRED) {
        ...
        for (syncBlock = _trampolineSyncList; syncBlock; syncBlock = syncBlock->_next) {
            ...
            for (auto entryIdx = 0; entryIdx < syncBlock->_entryCount; entryIdx++) {
                ...
                self()->createTrampoline(entry->_info._resolved._currentTrampoline, newPC,
...
void OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock *method, void *callSite, void *newStartPC, void *extraArg)
{
    ...
    // scope to patch trampoline
    {
        ...
        int32_t rc = _manager->codeCacheConfig().mccCallbacks().patchTrampoline(method, callSite, methodRunAddress,
            resolvedTramp, newStartPC, extraArg);
    }
}
...
//
void OMR::CodeCache::createTrampoline(CodeCacheTrampolineCode *trampoline, void *targetStartPC,
...
{
    CodeCacheTempTrampolineSyncBlock *block = static_cast<CodeCacheTempTrampolineSyncBlock *>(
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t *start, uint8_t *end, const char *file, uint32_t lineNumber)
{
    ...
    if (_freeBlockList) {
        ...
        } else if ((uint8_t *)curr < start
            && start - ((uint8_t *)curr + curr->_size) < sizeof(CodeCacheFreeCacheBlock)) {
...
OMR::CodeCacheFreeCacheBlock *OMR::CodeCache::removeFreeBlock(size_t blockSize, CodeCacheFreeCacheBlock *prev,
    CodeCacheFreeCacheBlock *curr)
{
...
uint8_t *OMR::CodeCache::allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize, uint8_t **coldCode,
    bool needsToBeContiguous, bool isMethodHeaderNeeded)
{
    TR::CodeCacheConfig &config = _manager->codeCacheConfig();
    ...
    _manager->performSizeAdjustments(warmSize, coldSize, needsToBeContiguous,
        isMethodHeaderNeeded); // side effect on warmSize and coldSize
...
void *OMR::CodeCache::getCCPreLoadedCodeAddress(TR_CCPreLoadedCode h, TR::CodeGenerator *codeGenerator)
{
    if (!_CCPreLoadedCodeInitialized) {
        if (TR_numCCPreLoadedCode) {
            TR::CodeCacheConfig &config = _manager->codeCacheConfig();
            config.mccCallbacks().createCCPreLoadedCode(_CCPreLoadedCodeBase, _CCPreLoadedCodeTop, _CCPreLoadedCode,
                codeGenerator);
...
//
TR::CodeCache *OMR::CodeCache::allocate(TR::CodeCacheManager *manager, size_t segmentSize,
