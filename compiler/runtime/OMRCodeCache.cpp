...
TR::CodeCache* OMR::CodeCache::self() { return static_cast<TR::CodeCache*>(this); }

void OMR::CodeCache::destroy(TR::CodeCacheManager* manager)
{
...
uint8_t* OMR::CodeCache::getCodeAlloc() { return _segment->segmentAlloc(); }

uint8_t* OMR::CodeCache::getCodeBase() { return _segment->segmentBase(); }
...
void OMR::CodeCache::unreserve()
{
    _reserved = false;
    _reservingCompThreadID = -2;
}

void OMR::CodeCache::writeMethodHeader(void* freeBlock, size_t size, bool isCold)
{
...
bool OMR::CodeCache::trimCodeMemoryAllocation(void* codeMemoryStart, size_t actualSizeInBytes)
{
    if (actualSizeInBytes == 0)
        return false; // nothing to resize, just return
    ...
    TR_ASSERT(cacheHeader->_eyeCatcher[0] == config.warmEyeCatcher()[0],
        "Missing eyecatcher during trimCodeMemoryAllocation");
    ...
    if (config.verboseReclamation()) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
            "--trimCodeMemoryAllocation-- CC=%p cacheHeader=%p oldSize=%u actualSizeInBytes=%d shrinkage=%u", this,
            cacheHeader, oldSize, actualSizeInBytes, shrinkage);
    }
...
//
bool OMR::CodeCache::initialize(
    TR::CodeCacheManager* manager, TR::CodeCacheMemorySegment* codeCacheSegment, size_t allocatedCodeCacheSizeInBytes)
{
    _manager = manager;
    ...
    if (!config.trampolineCodeSize()) {
        ...
        TR_ASSERT((((size_t)_CCPreLoadedCodeBase) & config.codeCacheHelperAlignmentMask()) == 0,
            "Per-code cache helper sizes do not account for alignment requirements.");
    ...
    } else // Trampolines are needed
    {
        ...
        if (!config.needsMethodTrampolines()) {
            ...
            _trampolineBase
                = _helperBase - ((_helperBase - _segment->segmentBase()) * config.trampolineSpacePercentage() / 100);

            // Grab the configuration details from the JIT platform code
            //
            // (_helperTop - segment->heapBase) is heapSize
            config.mccCallbacks().codeCacheConfig(
                static_cast<int32_t>(_helperTop - _segment->segmentBase()), &_tempTrampolinesMax);
        }
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCache::allocateTrampoline()
{
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCache::allocateTempTrampoline()
{
...
OMR::CodeCacheErrorCode::ErrorCode OMR::CodeCache::reserveSpaceForTrampoline_bridge(int32_t numTrampolines)
{
    return self()->reserveSpaceForTrampoline(numTrampolines);
}

OMR::CodeCacheErrorCode::ErrorCode OMR::CodeCache::reserveSpaceForTrampoline(int32_t numTrampolines)
{
        ...
        if (_trampolineReservationMark >= _trampolineBase + size) {
            ...
            if (config.verboseCodeCache()) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_CODECACHE, "CodeCache %p marked as full in reserveSpaceForTrampoline", self());
            }
        }
    }
...
void OMR::CodeCache::unreserveSpaceForTrampoline()
{
...
//---------------------------------------------------------------------------
int32_t OMR::CodeCache::reserveResolvedTrampoline(TR_OpaqueMethodBlock* method, bool inBinaryEncoding)
{
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCache::findTrampoline(TR_OpaqueMethodBlock* method)
{
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCache::findTrampoline(int32_t helperIndex)
{
    CodeCacheTrampolineCode* trampoline;
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
//
OMR::CodeCacheTrampolineCode* OMR::CodeCache::replaceTrampoline(
    TR_OpaqueMethodBlock* method, void* oldTrampoline, void* oldTargetPC, void* newTargetPC, bool needSync)
{
...
//
void OMR::CodeCache::syncTempTrampolines()
{
        ...
        for (uint32_t entryIdx = 0; entryIdx < _resolvedMethodHT->_size; entryIdx++) {
            for (CodeCacheHashEntry* entry = _resolvedMethodHT->_buckets[entryIdx]; entry; entry = entry->_next) {
                ...
                if (trampoline && entry->_info._resolved._currentStartPC != newPC) {
                    self()->createTrampoline(trampoline, newPC, entry->_info._resolved._method);
                    entry->_info._resolved._currentStartPC = newPC;
                }
            }
        }
        ...
        for (syncBlock = _trampolineSyncList; syncBlock; syncBlock = syncBlock->_next) {
            ...
            for (auto entryIdx = 0; entryIdx < syncBlock->_entryCount; entryIdx++) {
                ...
                self()->createTrampoline(
                    entry->_info._resolved._currentTrampoline, newPC, entry->_info._resolved._method);
                entry->_info._resolved._currentStartPC = newPC;
            }
...
//
void OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock* method, void* callSite, void* newStartPC, void* extraArg)
{
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
    ...
    // scope to patch trampoline
    {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePatching)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PATCH,
                "Patching callsite=0x%p using "
                "j9method=0x%p,resolvedTramp=0x%p,methodRunAddress=0x%p,newStartPC=0x%p,extraArg=0x%p",
                callSite, method, resolvedTramp, methodRunAddress, newStartPC, extraArg);
        }

        // Patch the code for a method trampoline
        int32_t rc = _manager->codeCacheConfig().mccCallbacks().patchTrampoline(
            method, callSite, methodRunAddress, resolvedTramp, newStartPC, extraArg);
    }
}
...
//
void OMR::CodeCache::createTrampoline(
    CodeCacheTrampolineCode* trampoline, void* targetStartPC, TR_OpaqueMethodBlock* method)
{
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
    config.mccCallbacks().createMethodTrampoline(trampoline, targetStartPC, method);
}

bool OMR::CodeCache::saveTempTrampoline(CodeCacheHashEntry* entry)
{
...
// of such blocks.
//
bool OMR::CodeCache::allocateTempTrampolineSyncBlock()
{
    CodeCacheTempTrampolineSyncBlock* block
        = static_cast<CodeCacheTempTrampolineSyncBlock*>(_manager->getMemory(sizeof(CodeCacheTempTrampolineSyncBlock)));
    if (!block)
        return false;

    TR::CodeCacheConfig& config = _manager->codeCacheConfig();

    block->_hashEntryArray = static_cast<CodeCacheHashEntry**>(
        _manager->getMemory(sizeof(CodeCacheHashEntry*) * config.codeCacheTempTrampolineSyncArraySize()));
...
OMR::CodeCacheHashEntry* OMR::CodeCache::allocateHashEntry()
{
    CodeCacheHashEntry* entry;
    CodeCacheHashEntrySlab* slab = _hashEntrySlab;
...
void OMR::CodeCache::freeHashEntry(CodeCacheHashEntry* entry)
{
...
//
bool OMR::CodeCache::addResolvedMethod(TR_OpaqueMethodBlock* method)
{
...
// empty by default
OMR::CodeCacheMethodHeader* OMR::CodeCache::addFreeBlock(void* metaData) { return NULL; }
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t* start, uint8_t* end, const char* file, uint32_t lineNumber)
{
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
    ...
    if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
        if (config.verboseReclamation()) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE,
                "addFreeBlock2[%s.%d]: failed to add free block. start = 0x%016x end = 0x%016x alignment = 0x%04x "
                "sizeof(CodeCacheFreeCacheBlock) = 0x%08x",
    ...
    if (_freeBlockList) {
        ...
        if (start < (uint8_t*)curr && (uint8_t*)curr - end < sizeof(CodeCacheFreeCacheBlock)) {
            ...
            if ((start - ((uint8_t*)curr + curr->_size) < sizeof(CodeCacheFreeCacheBlock))
                ...
                mergedBlock = curr;
                // fprintf(stderr, "--ccr-- merging new free block of the size %d with blocks of the size %d and %d at
    ...
    if (config.verboseReclamation()) {
        TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
            "--ccr-- addFreeBlock2WithCallSite CC=%p start=%p end=%p mergedBlock=%p link=%p link->_size=%u, "
            "_sizeOfLargestFreeWarmBlock=%d _sizeOfLargestFreeColdBlock=%d warmCodeAlloc=%p coldBlockAlloc=%p",
            this, (void*)start, (void*)end, mergedBlock, link, (uint32_t)link->_size, _sizeOfLargestFreeWarmBlock,
...
void OMR::CodeCache::updateMaxSizeOfFreeBlocks(CodeCacheFreeCacheBlock* blockPtr, size_t blockSize)
{
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
//
uint8_t* OMR::CodeCache::findFreeBlock(size_t size, bool isCold, bool isMethodHeaderNeeded)
{
    ...
    for (currLink = _freeBlockList, prevLink = NULL; currLink; prevLink = currLink, currLink = currLink->_next) {
        ...
        if (!biggestLink) {
            ...
            if (currLink->_size
                >= biggestLink->_size) // >= is important; do not use > only as secondBiggestLink may be wrong
            {
    ...
    if (!isCold) {
        TR_ASSERT(!config.codeCacheFreeBlockRecylingEnabled() || _sizeOfLargestFreeWarmBlock == biggestLink->_size,
            "_sizeOfLargestFreeWarmBlock=%d  biggestLink->_size=%d", _sizeOfLargestFreeWarmBlock,
            (int32_t)biggestLink->_size);
    } else {
    ...
    if (bestFitLink) {
        ...
        if (config.verboseReclamation()) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
                "--ccr- findFreeBlock: CodeCache=%p size=%u isCold=%d bestFitLink=%p bestFitLink->size=%u leftBlock=%p",
...
OMR::CodeCacheFreeCacheBlock* OMR::CodeCache::removeFreeBlock(
    size_t blockSize, CodeCacheFreeCacheBlock* prev, CodeCacheFreeCacheBlock* curr)
{
...
void OMR::CodeCache::dumpCodeCache()
{
    printf("Code Cache @%p\n", this);
...
void OMR::CodeCache::printFreeBlocks()
{
...
//---------------------------------------------------------------------
void OMR::CodeCache::checkForErrors()
{
    if (_freeBlockList) {
        bool doCrash = false;
        ...
        // scope for cache walk
        {
            ...
            for (CodeCacheFreeCacheBlock* currLink = _freeBlockList; currLink; currLink = currLink->_next) {
                ...
                if (currLink->_size > cacheSize) {
                    fprintf(stderr,
                        "checkForErrors cache %p: Error: Size of the free block %u is bigger than the size of the "
                        "cache %u\n",
                        this, (uint32_t)currLink->_size, (uint32_t)cacheSize);
                    doCrash = true;
                }
                // Is the block fully contained in this code cache?
                if ((uint8_t*)currLink < _segment->segmentBase() + sizeof(void*)
                    || (uint8_t*)currLink > _segment->segmentTop()) {
                ...
                if (endBlock < _segment->segmentBase() + sizeof(void*) || endBlock > _segment->segmentTop()) {
                    fprintf(stderr,
                        "checkForErrors cache %p: Error: End of block %p residing at %p is outside cache boundaries\n",
                ...
                if (currLink->_next) {
                    if ((uint8_t*)currLink->_next == endBlock) {
                        ...
                        if (!((uint8_t*)currLink < _warmCodeAlloc && endBlock >= _coldCodeAlloc)) {
                            fprintf(stderr,
                                "checkForErrors cache %p: Error: missed freed block coalescing opportunity. Next block "
                                "(%p) is adjacent to current one %p-%p\n",
                        ...
                        if ((uint8_t*)currLink->_next <= endBlock) {
                            fprintf(stderr,
                                "checkForErrors cache %p: Error: next block (%p) should come after end of current one "
                                "%p-%p\n",
                                this, currLink->_next, currLink, endBlock);
                            doCrash = true;
                            ;
                        }
                        // A free block is always followed by a used block except when this is the last free block in
                        // the warm region
                        if (endBlock != _warmCodeAlloc) {
                            ...
                            if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
                                fprintf(stderr,
                                    "checkForErrors cache %p: Error: block coming after this free one (%p-%p) does not "
                                    "have the eye catcher but %u\n",
            ...
            if (_sizeOfLargestFreeWarmBlock != maxFreeWarmSize) {
                fprintf(stderr,
                    "checkForErrors cache %p: Error: _sizeOfLargestFreeWarmBlock(%" OMR_PRIuSIZE
                    ") != maxFreeWarmSize(%" OMR_PRIuSIZE ")\n",
                    this, _sizeOfLargestFreeWarmBlock, maxFreeWarmSize);
                doCrash = true;
            }
            if (_sizeOfLargestFreeColdBlock != maxFreeColdSize) {
                fprintf(stderr,
                    "checkForErrors cache %p: Error: _sizeOfLargestFreeColdBlock(%" OMR_PRIuSIZE
                    ") != maxFreeColdSize(%" OMR_PRIuSIZE ")\n",
            ...
            // 3. All used blocks must have a CodeCacheMethodHeader with size as first parameter, folowed by eyeCatcher
            // and metaData
            TR::CodeCacheConfig& config = _manager->codeCacheConfig();
            uint8_t* start
                = (uint8_t*)align(((size_t)_segment->segmentTop()) + sizeof(char*), config.codeCacheAlignment());
            ...
            while (start < this->_trampolineBase) {
                ...
                } else // used seg
                {
                    ...
                    if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
                        ...
                        fprintf(stderr,
                            "checkForErrors cache %p: block %p is not in the free list and does not have eye-catcher; "
                            "prevBlock=%p\n",
...
OMR::CodeCacheHashEntry* OMR::CodeCache::findResolvedMethod(TR_OpaqueMethodBlock* method)
{
    return _resolvedMethodHT->findResolvedMethod(method);
}

void OMR::CodeCache::findOrAddResolvedMethod(TR_OpaqueMethodBlock* method)
{
...
uint8_t* OMR::CodeCache::allocateCodeMemory(
    size_t warmCodeSize, size_t coldCodeSize, uint8_t** coldCode, bool needsToBeContiguous, bool isMethodHeaderNeeded)
{
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
    ...
    _manager->performSizeAdjustments(
        warmSize, coldSize, needsToBeContiguous, isMethodHeaderNeeded); // side effect on warmSize and coldSize
    ...
    if (!warmIsFreeBlock) {
        ...
        warmCodeAddress
            = self()->findFreeBlock(warmSize, false, isMethodHeaderNeeded); // side effect: free block is unlinked
    ...
    if (!coldIsFreeBlock) {
        ...
        coldCodeAddress
            = self()->findFreeBlock(coldSize, true, isMethodHeaderNeeded); // side effect: free block is unlinked
        // Note that findFreeBlock may return a block which is slightly bigger than what we wanted
        // Change the warmSize to the higher size so that the size of the block is correctly maintained
    ...
    if (OMR::RSSReport::instance() && !coldIsFreeBlock && !needsToBeContiguous && _coldCodeRSSRegion) {
        ...
        if (padding > 0) {
            OMR::RSSItem* rssItem = new (TR::Compiler->persistentMemory())
                OMR::RSSItem(OMR::RSSItem::alignment, oldColdAlloc - padding, padding, NULL /* counters */);
        ...
        if (header > 0) {
            OMR::RSSItem* rssItem = new (TR::Compiler->persistentMemory())
                OMR::RSSItem(OMR::RSSItem::header, coldCodeAddress - header, header, NULL /* counters */);
            _coldCodeRSSRegion->addRSSItem(rssItem, self()->getReservingCompThreadID());
        }
    }
...
void* OMR::CodeCache::getCCPreLoadedCodeAddress(TR_CCPreLoadedCode h, TR::CodeGenerator* codeGenerator)
{
    if (!_CCPreLoadedCodeInitialized) {
        if (TR_numCCPreLoadedCode) {
            TR::CodeCacheConfig& config = _manager->codeCacheConfig();
            config.mccCallbacks().createCCPreLoadedCode(
                _CCPreLoadedCodeBase, _CCPreLoadedCodeTop, _CCPreLoadedCode, codeGenerator);
...
//
TR::CodeCache* OMR::CodeCache::allocate(
    TR::CodeCacheManager* manager, size_t segmentSize, int32_t reservingCompThreadID)
{
