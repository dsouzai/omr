...
enum AssumptionSameJittedBodyFlags { MARK_FOR_DELETE = 1 };
...
protected:
    RuntimeAssumption(TR_PersistentMemory*, uintptr_t key)
        : _next(NULL)
        , _key(key)
        , _nextAssumptionForSameJittedBody(NULL)
    { }

    void addToRAT(TR_PersistentMemory* persistentMemory, TR_RuntimeAssumptionKind kind, TR_FrontEnd* fe,
    ...
    {
        _nextAssumptionForSameJittedBody
            = (RuntimeAssumption*)((uintptr_t)link | (((uintptr_t)_nextAssumptionForSameJittedBody) & MARK_FOR_DELETE));
    }
    ...
    {
        _nextAssumptionForSameJittedBody
            = (RuntimeAssumption*)(((uintptr_t)_nextAssumptionForSameJittedBody) | MARK_FOR_DELETE);
    }
...
public:
    ...
    RuntimeAssumption* getSentinel()
    {
        ...
        while (toReturn->getAssumptionKind() != RuntimeAssumptionSentinel) {
            TR_ASSERT_FATAL(toReturn != this,
                "Looped through assumptions for same jitted body without finding sentinel! toReturn=%p\n", toReturn);
            RuntimeAssumption* next
                = (RuntimeAssumption*)(((uintptr_t)toReturn->_nextAssumptionForSameJittedBody) & ~MARK_FOR_DELETE);
    ...
    bool assumptionInRange(uintptr_t start, uintptr_t end)
    {
        return ((uintptr_t)getFirstAssumingPC()) <= end && start <= ((uintptr_t)getLastAssumingPC());
    }
    ...
    bool isAssumingRange(uintptr_t rangeStartPC, uintptr_t rangeEndPC, uintptr_t rangeColdStartPC,
        uintptr_t rangeColdEndPC, uintptr_t rangeStartMD, uintptr_t rangeEndMD);

private:
    RuntimeAssumption* _next;
    RuntimeAssumption*
        _nextAssumptionForSameJittedBody; // links assumptions that pertain to the same method body
...
public:
    SentinelRuntimeAssumption()
        : _owningMetaData(NULL)
        , RuntimeAssumption(NULL, 0)
    {
...
protected:
    PatchNOPedGuardSite(
        TR_PersistentMemory* pm, uintptr_t key, TR_RuntimeAssumptionKind kind, uint8_t* location, uint8_t* destination)
...
protected:
    PatchMultipleNOPedGuardSites(
        TR_PersistentMemory* pm, uintptr_t key, TR_RuntimeAssumptionKind kind, PatchSites* sites)
        : OMR::LocationRedirectRuntimeAssumption(pm, key)
        , _patchSites(sites)
    { }
...
public:
    ...
    virtual void reclaim() { PatchSites::reclaim(_patchSites); }
