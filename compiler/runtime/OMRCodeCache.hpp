...
public:
    CodeCache()
        : _coldCodeRSSRegion(NULL)
    { }

    void* operator new(size_t s, TR::CodeCache* cache) { return cache; }
    void operator delete(void* p, TR::CodeCache* cache)
    { /* do nothing */
    }
    ...
    void alignWarmCodeAlloc(uint32_t round)
    {
        _warmCodeAlloc = reinterpret_cast<uint8_t*>(align(reinterpret_cast<size_t>(_warmCodeAlloc), round));
    }
    void alignColdCodeAlloc(uint32_t round)
    {
        _coldCodeAlloc = reinterpret_cast<uint8_t*>(align(reinterpret_cast<size_t>(_coldCodeAlloc), round));
    }
    ...
    uint8_t* allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize, uint8_t** coldCode, bool needsToBeContiguous,
    ...
    CodeCacheTrampolineCode* replaceTrampoline(
    ...
    void createTrampoline(CodeCacheTrampolineCode* trampoline, void* targetStartPC, TR_OpaqueMethodBlock* method);
    bool allocateTempTrampolineSyncBlock();

    void patchCallPoint(TR_OpaqueMethodBlock* method, void* callSite, void* newStartPC, void* extraArg);
...
private:
    ...
    CodeCacheFreeCacheBlock* removeFreeBlock(
        size_t blockSize, CodeCacheFreeCacheBlock* prev, CodeCacheFreeCacheBlock* curr);

public:
    bool addFreeBlock2WithCallSite(uint8_t* start, uint8_t* end, const char* file, uint32_t lineNumber);
    ...
    // flag accessed under mutex <== This is deceiving! There are two different monitors we may hold (not at the same
    // time!) when we write to this. We can either be holding the code cache monitor *OR* the manager's code cache list
    // monitor.
