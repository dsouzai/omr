...
const char* OMR::RSSItem::itemNames[]
    = { "Unknown", "HEADER", "ALIGNMENT", "COLD BLOCKS", "OVERESTIMATE", "OOL", "SNIPPET" };

void OMR::RSSRegion::addRSSItem(OMR::RSSItem* item, int32_t threadId, const char* methodName)
{
#ifdef DEBUG_RSS_REPORT
    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "RSS adding item addr=%p size=%zu type=%s thread=%d method=%s",
        item->_addr, item->_size, OMR::RSSItem::itemNames[item->_type], threadId,
    ...
    while (remainingSize > 0) {
        ...
        OMR::RSSItem* newItem
            = new (TR::Compiler->persistentMemory()) OMR::RSSItem(item->_type, address, newSize, counters);
        offset = (_grows == lowToHigh) ? ++offset : --offset;

        TR_ASSERT_FATAL(offset >= 0, "Got negative offset %d for addr=%p size=%zu type=%s\n", offset, newItem->_addr,
            newItem->_size, OMR::RSSItem::itemNames[newItem->_type]);

#ifdef DEBUG_RSS_REPORT
        TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
            "RSS adding overflow item addr=%p size=%zu type=%s thread=%d method=%s", newItem->_addr, newItem->_size,
            OMR::RSSItem::itemNames[newItem->_type], threadId, methodName ? methodName : "no method");
#endif
...
void OMR::RSSRegion::addToListSorted(TR_PersistentList<OMR::RSSItem>* itemList, OMR::RSSItem* item)
{
    ...
    //
    for (ListElement<OMR::RSSItem>* nextElement = newElement->getNextElement(); nextElement;
...
size_t OMR::RSSReport::countResidentPages(int fd, OMR::RSSRegion* rssRegion, char* pagesInfo)
{
    size_t rss = 0;
    ...
    for (uint8_t* addr = start; addr < end; addr += pageSize) {
        ...
        if (resident == 1 && itemList.isEmpty()) {
            ...
            uint8_t* prevEnd
                = (addr == start) ? addr : (uint8_t*)(((uintptr_t)addr / pageSize) * pageSize); // current page start
            ...
            for (OMR::RSSItem* item = it.getFirst(); item; item = it.getNext()) {
                ...
                if (item->_counters) {
                    ...
                    for (TR::DebugCounterAggregation* aggr = aggrIt.getFirst(); aggr; aggr = aggrIt.getNext()) {
                        ...
                        if (count > 0)
                            aggr->printCounters(false /*printZeroCounters*/);
                    }
                }
                ...
                if (gap != 0) {
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_PERF, "RSS: gap at addr=%p size=%zu in %s region", prevEnd, gap, rssRegion->_name);
                }
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                "RSS: Page at addr %p has %zu bytes of %d items pageDebugCount=%zu", addr, totalItemSize, countItems,
                pageDebugCount);
...
void OMR::RSSReport::printTitle()
{
    ...
    for (OMR::RSSRegion* rssRegion = it.getFirst(); rssRegion; rssRegion = it.getNext()) {
        ...
        if ((offset + REGION_PRINT_WIDTH + 1) < MAX_RSS_LINE) {
            ...
            //
            TR::snprintfTrunc(rssLine + offset, REGION_PRINT_WIDTH + 1, "%*.*s", REGION_PRINT_WIDTH, REGION_PRINT_WIDTH,
                rssRegion->_name);
        } else {
            break;
        }
...
void OMR::RSSReport::printRegions()
{
    size_t totalKb = 0;
    size_t totalRSSKb = 0;
    int32_t regionNum = 0;
    ...
    for (OMR::RSSRegion* rssRegion = it.getFirst(); rssRegion; rssRegion = it.getNext()) {
        ...
        if ((offset + REGION_PRINT_WIDTH + 1) < MAX_RSS_LINE) {
            ...
            // REGION_PRINT_WIDTH+1 ensures each field will not take more than expected by the check above
            //
            TR::snprintfTrunc(rssLine + offset, REGION_PRINT_WIDTH + 1, " %5zu/%5zu(%3.0f%%)", rssSizeKb, sizeKb,
                (float)rssSizeKb * 100.0 / sizeKb);
        }
    ...
    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF, "RSS Region rss/size:    %s  all %u regions %5zu/%5zu(%3.0f%%) KB",
        rssLine, regionNum, totalRSSKb, totalKb, (float)totalRSSKb * 100.0 / totalKb);
}
