...
bool OMR::Compilation::supportsInduceOSR()
{
    ...
    if (_osrCompilationData && _osrCompilationData->seenClassPreventingInducedOSR()) {
        if (self()->getOption(TR_TraceOSR))
            traceMsg(self(),
                "Cannot guarantee OSR transfer of control to the interpreter will work for calls preventing induced "
                "OSR (e.g. Quad) because of differences in JIT vs interpreter representations\n");
        return false;
    }
...
int32_t OMR::Compilation::compile()
{
    ...
    if (self()->getOption(TR_DebugBeforeCompile)) {
        ...
        self()->getDebug()->setupDebugger(
            (void*)*((long*)&(stopBeforeCompile)), (void*)*((long*)&(stopBeforeCompile)), true);
#else
        ...
        if (_ilGenSuccess) {
            ...
            if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
                ...
                for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
                    ...
                    int32_t calleeLen = self()->fej9()->printTruncatedSignature(
                        calleeBuf, sizeof(calleeBuf) - 1, self()->fej9()->getInlinedCallSiteMethod(&site));
...
int16_t OMR::Compilation::matchingCallStackPrefixLength(TR_ByteCodeInfo& bcInfo)
{
    ...
    TR_ASSERT(
        result <= self()->getInlineDepth(), "matchingCallStackPrefixLength can be no greater than getInlineDepth");
    return result;
}
...
int32_t OMR::Compilation::convertNonDeterministicInput(
    int32_t i, int32_t max, TR_RandomGenerator* randomGenerator, int32_t min, bool emitVerbose)
{
    int32_t answer = i;
...
TR_DevirtualizedCallInfo* OMR::Compilation::findOrCreateDevirtualizedCall(
    TR::Node* callNode, TR_OpaqueClassBlock* thisType)
{
...
TR::Node* OMR::Compilation::createAOTInliningGuard(
    TR::Compilation* comp, int16_t calleeIndex, TR::Node* node, TR::TreeTop* destination, TR_VirtualGuardKind kind)
{
    return TR_VirtualGuard::createAOTInliningGuard(comp, calleeIndex, node, destination, kind);
}

TR::Node* OMR::Compilation::createAOTGuard(
    TR::Compilation* comp, int16_t calleeIndex, TR::Node* node, TR::TreeTop* destination, TR_VirtualGuardKind kind)
{
    return TR_VirtualGuard::createAOTGuard(comp, calleeIndex, node, destination, kind);
}

TR::Node* OMR::Compilation::createDummyGuard(
    TR::Compilation* comp, int16_t calleeIndex, TR::Node* node, TR::TreeTop* destination)
{
...
{
    TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
    return self()->fe()->getLineNumberForMethodAndByteCodeIndex(
        self()->getMethodFromNode(node), bcInfo.getByteCodeIndex());
}
...
int32_t OMR::Compilation::getLineNumberInCurrentMethod(TR::Node* node)
{
    ...
    return self()->fe()->getLineNumberForMethodAndByteCodeIndex(
        self()->getCurrentMethod()->getPersistentIdentifier(), site._byteCodeInfo.getByteCodeIndex());
}
 ...
 */
void OMR::Compilation::getOSRCallSiteRemat(
    uint32_t callSiteIndex, uint32_t slot, TR::SymbolReference*& ppSymRef, TR::SymbolReference*& loadSymRef)
{
 ...
 */
void OMR::Compilation::setOSRCallSiteRemat(
    uint32_t callSiteIndex, TR::SymbolReference* ppSymRef, TR::SymbolReference* loadSymRef)
{
    int32_t* table = _inlinedCallSites[callSiteIndex].osrCallSiteRematTable();
...
void OMR::Compilation::diagnosticImplVA(const char* s, va_list ap)
{
    if (self()->getOutFile() != NULL) {
        ...
        va_copy(copy, ap);
        char buffer[256];
        TR::IO::vfprintf(self()->getOutFile(),
            self()->getDebug()->getDiagnosticFormat(s, buffer, sizeof(buffer) / sizeof(char)), copy);
        trfflush(self()->getOutFile());
        va_end(copy);
    }
}
