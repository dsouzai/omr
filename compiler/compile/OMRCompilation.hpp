...
enum ProfilingMode { DisabledProfiling, JitProfiling, JProfiling };
...
public:
    ...
    bool incInlineDepth(TR::ResolvedMethodSymbol*, TR::Node* callNode, bool directCall, TR_VirtualGuardSelection* guard,
        TR_OpaqueClassBlock* receiverClass, TR_PrexArgInfo* argInfo = 0);
...
protected:
    bool incInlineDepth(TR_OpaqueMethodBlock*, TR::ResolvedMethodSymbol*, TR_ByteCodeInfo&,
        TR::SymbolReference* callSymRef, bool directCall, TR_PrexArgInfo* argInfo, TR_AOTMethodInfo* aotMethodInfo);

public:
    TR_ExternalRelocationTargetKind getReloTypeForMethodToBeInlined(
        TR_VirtualGuardSelection* guard, TR::Node* callNode, TR_OpaqueClassBlock* receiverClass)
    {
    ...
    typedef std::map<const void*, TR::DebugCounterBase*, std::less<const void*>, DebugCounterMapAllocator>
        DebugCounterMap;
    ...
    template<typename Exception> void OMR_NORETURN failCompilation(const char* format, ...)
    {
        char buffer[512];
     ...
     */
    void setGenerateReadOnlyCode() { _flags.set(GenerateReadOnlyCode); }
     ...
     */
    bool getGenerateReadOnlyCode() { return _flags.testAny(GenerateReadOnlyCode); }
...
protected:
    ...
    OMR::RuntimeAssumption*
        _metadataAssumptionList; // A special OMR::RuntimeAssumption to play the role of a sentinel for a linked list
#endif
...
private:
    ...
    // is needed, e.g. for printing optimization trees it gets set to 0 in TR_Debug::printBasicPreNodeInfoAndIndent()
    // which preceeds node opCode printouts. Any methods that print parts of node opCodes should always increment this
    // as there may be information following opCodes that need to be aligned as in the case of trees or in future
    // development.
    ...
    typedef TR::typed_allocator<std::pair<TR_OpaqueClassBlock* const, const TR::TypeLayout*>, TR::Region&>
        LayoutAllocator;
...
inline TR::Compilation* comp() { return TR_TLS_GET(OMR::compilation, TR::Compilation*); }
} // namespace TR
