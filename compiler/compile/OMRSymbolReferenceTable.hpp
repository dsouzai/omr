...
public:
    ...
    TR::SymbolReference *createRuntimeHelper(TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept,
    ...
    TR::SymbolReference *createTemporary(TR::ResolvedMethodSymbol *owningMethodSymbol, TR::DataType,
        bool isInternalPointer = false, size_t size = 0);
     ...
     */
    TR::SymbolReference *createNamedStatic(TR::ResolvedMethodSymbol *owningMethodSymbol, TR::DataType type,
        const char *name);
    ...
    TR::SymbolReference *findOrCreatePendingPushTemporary(TR::ResolvedMethodSymbol *owningMethodSymbol,
        int32_t stackDepth, TR::DataType, size_t size = 0);
    TR::SymbolReference *createLocalPrimArray(int32_t objectSize, TR::ResolvedMethodSymbol *owningMethodSymbol,
        int32_t arrayType);
    ...
    TR::SymbolReference *findOrCreateStaticMethodSymbol(mcount_t owningMethodIndex, int32_t cpIndex,
        TR_ResolvedMethod *resolvedMethod);
    ...
    // optimizer (EA)
    TR::SymbolReference *createLocalObject(int32_t objectSize, TR::ResolvedMethodSymbol *owningMethodSymbol,
        TR::SymbolReference *classSymRef);
    TR::SymbolReference *createLocalAddrArray(int32_t objectSize, TR::ResolvedMethodSymbol *owningMethodSymbol,
        TR::SymbolReference *classSymRef);
    ...
    TR::SymbolReference *findOrCreateGenericIntShadowSymbolReference(intptr_t offset,
        bool allocateUseDefBitVector = false);
    ...
    TR::SymbolReference *findOrCreateClassSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t cpIndex,
        void *classObject, bool cpIndexOfStatic = false);
    TR::SymbolReference *findOrCreateArrayShadowSymbolRef(TR::DataType type, TR::Node *baseArrayAddress, int32_t size,
        TR_FrontEnd *fe);
    ...
    TR::SymbolReference *createRefinedArrayShadowSymbolRef(TR::DataType, TR::Symbol *,
        TR::SymbolReference *original); // TODO: to be changed to a special sym ref
    ...
    TR::SymbolReference *findAvailableAuto(List<TR::SymbolReference> &, TR::DataType dataType, bool behavesLikeTemp,
...
protected:
     ...
     */
    TR::SymbolReference *createTempSymRefWithKnownObject(TR::Symbol *symbol, mcount_t owningMethodIndex, int32_t slot,
        TR::KnownObjectTable::Index knownObjectIndex);
