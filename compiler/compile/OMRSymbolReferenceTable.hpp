...
public:
    ...
    enum CommonNonhelperSymbol {
        ...
        countForRecompileSymbol, // global boolean indicating when methods should start counting for guarded
                                 // recompilations
         ...
         * supported. The transition target bytecode is the bytecode index on the call plus an induction offset which is
         * stored on the call node.
         *
         *  \code
        ...
        /** \brief
         *
         *  A call with this symbol marks a place that has been optimized with runtime assumptions. Such place needs
         * protection of OSR points. When the assumption becomes wrong, the execution of jitted code with the assumption
         * has to be transition to the VM interpreter before running the invalid code.
         *
         *  \code
        ...
        /** \brief
         *
         * A call with this symbol marks a place where we want/need escape analysis to add heapifications for any stack
         * allocated objects. The primary use case is to force escape of all live local objects ahead of a throw to an
         * OSR catch block but they may also be inserted to facilitate peeking of methods under HCR or other uses. Calls
         * to this helper should only exist while escape analysis is running
         *
         * \code
    ...
    CommonNonhelperSymbol getLastCommonNonhelperSymbol() { return lastCommonNonhelperSymbol; }
    ...
    template<class BitVector> void getAllSymRefs(BitVector& allSymRefs)
    {
    ...
    TR::SymbolReference* createRuntimeHelper(
        TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters);
    TR::SymbolReference* findOrCreateRuntimeHelper(TR_RuntimeHelper index, bool canGCandReturn = false,
        bool canGCandExcept = false, bool preservesAllRegisters = false);
    ...
    TR::SymbolReference* findOrCreateAutoSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t slot,
        TR::DataType, bool isReference = true, bool isInternalPointer = false, bool reuseAuto = true,
        bool isAdjunct = false, size_t size = 0);
    TR::SymbolReference* createTemporary(
        TR::ResolvedMethodSymbol* owningMethodSymbol, TR::DataType, bool isInternalPointer = false, size_t size = 0);
     ...
     */
    TR::SymbolReference* createNamedStatic(
     ...
     */
    TR::SymbolReference* findOrCreateShadowSymbol(
    ...
    TR::SymbolReference* findOrCreatePendingPushTemporary(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t stackDepth, TR::DataType, size_t size = 0);
    TR::SymbolReference* createLocalPrimArray(
    ...
    TR::SymbolReference* methodSymRefFromName(TR::ResolvedMethodSymbol* owningMethodSymbol, const char* className,
        const char* methodName, const char* signature, TR::MethodSymbol::Kinds kind, int32_t cpIndex = -1);
    ...
    bool isConstantAreaSymbolReference(TR::SymbolReference* symRef)
    {
        return _constantAreaSymbolReference && _constantAreaSymbolReference == symRef;
    }
    ...
    TR::SymbolReference* createKnownStaticDataSymbolRef(
        void* counterAddress, TR::DataType type, TR::KnownObjectTable::Index knownObjectIndex);
    ...
    TR::SymbolReference* findOrCreateMethodSymbol(
        mcount_t owningMethodIndex, int32_t cpIndex, TR_ResolvedMethod*, TR::MethodSymbol::Kinds, bool = false);
    TR::SymbolReference* findOrCreateComputedStaticMethodSymbol(
        mcount_t owningMethodIndex, int32_t cpIndex, TR_ResolvedMethod* resolvedMethod);
    TR::SymbolReference* findOrCreateStaticMethodSymbol(
        mcount_t owningMethodIndex, int32_t cpIndex, TR_ResolvedMethod* resolvedMethod);
    ...
    TR::SymbolReference* createKnownStaticReferenceSymbolRef(
        void* address, TR::KnownObjectTable::Index knownObjectIndex = TR::KnownObjectTable::UNKNOWN);
    ...
    TR::SymbolReference* findOrCreateSymRefWithKnownObject(
        TR::SymbolReference* original, uintptr_t* referenceLocation, bool isArrayWithConstantElements);
    TR::SymbolReference* findOrCreateSymRefWithKnownObject(
        TR::SymbolReference* original, TR::KnownObjectTable::Index objectIndex);
    /*
     ...
     */
    TR::SymbolReference* findOrCreateTemporaryWithKnowObjectIndex(
    ...
    TR::SymbolReference* createIsOverriddenSymbolRef(
        TR::ResolvedMethodSymbol* calleeSymbol); /* never reuse is overridden symrefs */
    ...
    TR::SymbolReference* findOrCreateGenericIntShadowSymbolReference(
        intptr_t offset, bool allocateUseDefBitVector = false);
    ...
    TR::SymbolReference* findOrCreateClassSymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, void* classObject, bool cpIndexOfStatic = false);
    TR::SymbolReference* findOrCreateArrayShadowSymbolRef(
    ...
    TR::SymbolReference* createRefinedArrayShadowSymbolRef(
        TR::DataType, TR::Symbol*, TR::SymbolReference* original); // TODO: to be changed to a special sym ref
    ...
    TR::SymbolReference* findAvailableAuto(
        List<TR::SymbolReference>&, TR::DataType dataType, bool behavesLikeTemp, bool isAdjunct = false);
...
protected:
     ...
     *
     *  \note
     *    This function should only be called from functions inside symbol reference table when creating new autos or
     * temps. Code outside symbol reference table should use the public API findOrCreateTemporaryWithKnowObjectIndex.
     */
    TR::SymbolReference* createTempSymRefWithKnownObject(
     ...
     *       For symbol reference type other than temp, an original symbol is needed to find its corresponding symbol
     * reference. Take a static field with known object index for example, \p symbol is the original static field
     * symbol.
     *
