...
//
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateVtableEntrySymbolRef(TR::ResolvedMethodSymbol *calleeSymbol,
    int32_t vtableSlot)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateInduceOSRSymbolRef(TR_RuntimeHelper induceOSRHelper)
{
    ...
    TR::SymbolReference *induceOSRSymRef = findOrCreateRuntimeHelper(induceOSRHelper, true /* canGCandReturn */,
        true /* canGCandExcept */, true /* preservesAllRegisters */);
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateArrayShadowSymbolRef(TR::DataType type,
    TR::Node *baseArrayAddress)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateArrayShadowSymbolRef(TR::DataType type,
    TR::Node *baseArrayAddress, int32_t size, TR_FrontEnd *fe)
{
    int32_t index = getArrayShadowIndex(type);
    if (!baseArray.element(index)) {
...
TR::SymbolReference *OMR::SymbolReferenceTable::createRefinedArrayShadowSymbolRef(TR::DataType type, TR::Symbol *sym,
...
TR::SymbolReference *OMR::SymbolReferenceTable::createKnownStaticReferenceSymbolRef(void *dataAddress,
    TR::KnownObjectTable::Index knownObjectIndex)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::createRuntimeHelper(TR_RuntimeHelper index, bool canGCandReturn,
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateRuntimeHelper(TR_RuntimeHelper index, bool canGCandReturn,
    bool canGCandExcept, bool preservesAllRegisters)
{
    ...
    int32_t cpIndex)
{
    ...
    if (!method && comp()->compileRelocatableCode())
        comp()->failCompilation<TR::CompilationException>("Failed to get method %s.%s%s from name", className,
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(TR::SymbolReference *original,
    uintptr_t *referenceLocation, bool isArrayWithConstantElements)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::findSymRefWithKnownObject(TR::Symbol *symbol,
    TR::KnownObjectTable::Index knownObjectIndex)
{
...
{
    TR_ASSERT_FATAL(symbol->isAutoOrParm(), "createTempSymRefWithKnownObject can only be called for temp symbol %p",
        symbol);
    TR_BitVector *bucket = _knownObjectSymrefsByObjectIndex[knownObjectIndex];
    if (!bucket) {
        bucket = new (trHeapMemory())
...
{
    return findOrCreateRuntimeHelper(TR_monitorExit, true /* canGCandReturn */, true /* canGCandExcept */,
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateGenericIntShadowSymbolReference(intptr_t offset,
    bool allocateUseDefBitVector)
{
    // FIXME: we should attempt to find it
    return createGenericIntShadowSymbolReference(offset, allocateUseDefBitVector);
}

TR::SymbolReference *OMR::SymbolReferenceTable::createGenericIntShadowSymbolReference(intptr_t offset,
    bool allocateUseDefBitVector)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::findStaticSymbol(TR_ResolvedMethod *owningMethod, int32_t cpIndex,
    TR::DataType type)
{
    TR::SymbolReference *symRef;
    ...
    while ((symRef = i.getNext())) {
        if (symRef->getSymbol()->getDataType() == type && symRef->getCPIndex() != -1
            && TR::Compiler->cls.jitStaticsAreSame(comp(), owningMethod, cpIndex, symRef->getOwningMethod(comp()),
                symRef->getCPIndex()))
            return symRef;
    }
    return 0;
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateClassSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
    int32_t cpIndex, void *classObject, bool cpIndexOfStatic)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateComputedStaticMethodSymbol(mcount_t owningMethodIndex,
    int32_t cpIndex, TR_ResolvedMethod *resolvedMethod)
{
    return findOrCreateMethodSymbol(owningMethodIndex, cpIndex, resolvedMethod, TR::MethodSymbol::ComputedStatic,
        false /*isUnresolvedInCP*/);
}

TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateStaticMethodSymbol(mcount_t owningMethodIndex,
    int32_t cpIndex, TR_ResolvedMethod *resolvedMethod)
{
    return findOrCreateMethodSymbol(owningMethodIndex, cpIndex, resolvedMethod, TR::MethodSymbol::Static,
        false /*isUnresolvedInCP*/);
}
    ...
    TR_ResolvedMethod *resolvedMethod, TR::MethodSymbol::Kinds callKind, bool isUnresolvedInCP)
{
    ...
    if (resolvedMethod) {
        ...
        sym = TR::MethodSymbol::create(trHeapMemory(), TR_Private,
            fej9->createMethod(trMemory(), owningMethod->containingClass(), cpIndex));
#else
    ...
    size_t size, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    if (!symRef) {
        ...
        if (isInternalPointer) {
            sym = size ? TR::AutomaticSymbol::createInternalPointer(trHeapMemory(), type, static_cast<uint32_t>(size),
                             comp()->fe())
...
TR::SymbolReference *OMR::SymbolReferenceTable::findOrCreateShadowSymbol(TR::ResolvedMethodSymbol *owningMethodSymbol,
...
TR::SymbolReference *OMR::SymbolReferenceTable::findAvailableAuto(TR::DataType type, bool behavesLikeTemp,
    bool isAdjunct)
{
    return findAvailableAuto(_availableAutos, type, behavesLikeTemp, isAdjunct);
}

TR::SymbolReference *OMR::SymbolReferenceTable::findAvailableAuto(List<TR::SymbolReference> &availableAutos,
...
TR::SymbolReference *OMR::SymbolReferenceTable::createTemporary(TR::ResolvedMethodSymbol *owningMethodSymbol,
    TR::DataType type, bool isInternalPointer, size_t size)
{
#ifdef J9_PROJECT_SPECIFIC
    TR_ASSERT(!type.isBCD() || size, "binary coded decimal types must provide a size\n");
#endif

    return findOrCreateAutoSymbol(owningMethodSymbol, owningMethodSymbol->incTempIndex(fe()), type, true,
        isInternalPointer, false, false, size);
}
...
TR::SymbolReference *OMR::SymbolReferenceTable::createNamedStatic(TR::ResolvedMethodSymbol *owningMethodSymbol,
    TR::DataType type, const char *name)
{
    TR::StaticSymbol *sym = TR::StaticSymbol::createNamed(trHeapMemory(), type, name);
    return new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodSymbol->getResolvedMethodIndex(),
        owningMethodSymbol->incTempIndex(fe()));
}

TR::SymbolReference *OMR::SymbolReferenceTable::createLocalObject(int32_t objectSize,
    TR::ResolvedMethodSymbol *owningMethodSymbol, TR::SymbolReference *classSymRef)
{
...
TR::SymbolReference *OMR::SymbolReferenceTable::createLocalAddrArray(int32_t objectSize,
    TR::ResolvedMethodSymbol *owningMethodSymbol, TR::SymbolReference *classSymRef)
{
    int32_t slot = owningMethodSymbol->incTempIndex(fe());
    TR_ResolvedMethod *method = owningMethodSymbol->getResolvedMethod();
    ...
    TR::AutomaticSymbol *sym = TR::AutomaticSymbol::createLocalObject(trHeapMemory(), TR::anewarray, classSymRef,
        TR::Aggregate, objectSize, fe());
    sym->setBehaveLikeNonTemp();
    owningMethodSymbol->addAutomatic(sym);
    TR::SymbolReference *symRef = new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodIndex, slot);
    owningMethodSymbol->getAutoSymRefs(slot).add(symRef);
...
TR::SymbolReference *OMR::SymbolReferenceTable::createLocalPrimArray(int32_t objectSize,
    TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t arrayType)
{
    int32_t slot = owningMethodSymbol->incTempIndex(fe());
    TR_ResolvedMethod *method = owningMethodSymbol->getResolvedMethod();
    mcount_t owningMethodIndex = owningMethodSymbol->getResolvedMethodIndex();
...
void OMR::SymbolReferenceTable::rememberOriginalUnimprovedSymRef(TR::SymbolReference *improved,
    TR::SymbolReference *original)
{
