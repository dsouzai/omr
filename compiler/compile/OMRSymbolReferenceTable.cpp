...
OMR::SymbolReferenceTable::SymbolReferenceTable(size_t sizeHint, TR::Compilation* comp)
    ...
    , _methodsBySignature(8, comp->allocator("SymRefTab"))
    , // TODO: Determine a suitable default size
    _hasImmutable(false)
    , _hasUserField(false)
...
TR::SymbolReferenceTable* OMR::SymbolReferenceTable::self() { return static_cast<TR::SymbolReferenceTable*>(this); }

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateContiguousArraySizeSymbolRef()
{
    if (!element(contiguousArraySizeSymbol)) {
        ...
        element(contiguousArraySizeSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), contiguousArraySizeSymbol, sym);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateContiguousArrayDataAddrFieldShadowSymRef()
{
    if (!element(contiguousArrayDataAddrFieldSymbol)) {
        ...
        element(contiguousArrayDataAddrFieldSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), contiguousArrayDataAddrFieldSymbol, sym);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findContiguousArrayDataAddrFieldShadowSymRef()
{
    return element(contiguousArrayDataAddrFieldSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVftSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findVftSymbolRef() { return element(vftSymbol); }

// Denotes the address of the ramMethod in the vtable.
//
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVtableEntrySymbolRef(
    TR::ResolvedMethodSymbol* calleeSymbol, int32_t vtableSlot)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateIndexableSizeSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findArrayClassRomPtrSymbolRef()
{
    return element(arrayClassRomPtrSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findClassRomPtrSymbolRef() { return element(classRomPtrSymbol); }

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateThisRangeExtensionSymRef(
...
TR::SymbolReference* OMR::SymbolReferenceTable::findThisRangeExtensionSymRef(
    TR::ResolvedMethodSymbol* owningMethodSymbol)
{
    return element(thisRangeExtensionSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findInstanceShapeSymbolRef() { return element(instanceShapeSymbol); }

TR::SymbolReference* OMR::SymbolReferenceTable::findInstanceDescriptionSymbolRef()
{
    return element(instanceDescriptionSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findDescriptionWordFromPtrSymbolRef()
{
    return element(descriptionWordFromPtrSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findJavaLangClassFromClassSymbolRef()
{
    return element(javaLangClassFromClassSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findClassFromJavaLangClassSymbolRef()
{
    return element(classFromJavaLangClassSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findAddressOfClassOfMethodSymbolRef()
{
    return element(addressOfClassOfMethodSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findClassIsArraySymbolRef() { return element(isArraySymbol); }

TR::SymbolReference* OMR::SymbolReferenceTable::findClassDepthAndFlagsSymbolRef()
{
    return element(isClassDepthAndFlagsSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findClassFlagsSymbolRef() { return element(isClassFlagsSymbol); }

TR::SymbolReference* OMR::SymbolReferenceTable::findArrayComponentTypeSymbolRef()
{
    return element(componentClassSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateExcpSymbolRef()
{
    if (!element(excpSymbol)) {
        ...
        aliasBuilder.addressStaticSymRefs().set(
            getNonhelperIndex(excpSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(excpSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateOSRReturnAddressSymbolRef()
{
    if (!element(osrReturnAddressSymbol)) {
        ...
        aliasBuilder.addressStaticSymRefs().set(getNonhelperIndex(
            osrReturnAddressSymbol)); // add the symRef to the statics list to get correct aliasing info
    }
    return element(osrReturnAddressSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateInduceOSRSymbolRef(TR_RuntimeHelper induceOSRHelper)
{
    TR_ASSERT(induceOSRHelper == TR_induceOSRAtCurrentPC || induceOSRHelper == TR_induceOSRAtCurrentPCAndRecompile,
        "unexpected helper index when creating induce OSR helper symbol reference");
    TR::SymbolReference* induceOSRSymRef = findOrCreateRuntimeHelper(
        induceOSRHelper, true /* canGCandReturn */, true /* canGCandExcept */, true /* preservesAllRegisters */);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayletShadowSymbolRef(TR::DataType type)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayShadowSymbolRef(
    TR::DataType type, TR::Node* baseArrayAddress)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayShadowSymbolRef(
    TR::DataType type, TR::Node* baseArrayAddress, int32_t size, TR_FrontEnd* fe)
{
 ...
 */
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateImmutableArrayShadowSymbolRef(TR::DataType elementType)
{
    ...
    aliasBuilder.arrayElementSymRefs().set(
        index); // this ensures the newly created shadow can still be aliased to all other array shadows
...
bool OMR::SymbolReferenceTable::isRefinedArrayShadow(TR::SymbolReference* symRef)
{
    ...
    return aliasBuilder.refinedNonIntPrimitiveArrayShadows().isSet(index)
        || aliasBuilder.refinedIntArrayShadows().isSet(index) || aliasBuilder.refinedAddressArrayShadows().isSet(index);
}

TR::SymbolReference* OMR::SymbolReferenceTable::createRefinedArrayShadowSymbolRef(TR::DataType type)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::createRefinedArrayShadowSymbolRef(
    TR::DataType type, TR::Symbol* sym, TR::SymbolReference* original)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateRecompilationCounterSymbolRef(void* counterAddress)
{
    if (!element(recompilationCounterSymbol)) {
        ...
        element(recompilationCounterSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), recompilationCounterSymbol, sym);
    }
    return element(recompilationCounterSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayCopySymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArraySetSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreatePrefetchSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayTranslateSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayTranslateAndTestSymbol()
{
    if (!element(arrayTranslateAndTestSymbol)) {
        TR::MethodSymbol* sym = TR::MethodSymbol::create(trHeapMemory(), TR_Helper);
        sym->setHelper();

        element(arrayTranslateAndTestSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), arrayTranslateAndTestSymbol, sym);
    }
    return element(arrayTranslateAndTestSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreatelong2StringSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreatebitOpMemSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateSinglePrecisionSQRTSymbol()
{
    if (!element(singlePrecisionSQRTSymbol)) {
        TR::MethodSymbol* sym = TR::MethodSymbol::create(trHeapMemory(), TR_Helper);
        sym->setHelper();

        element(singlePrecisionSQRTSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), singlePrecisionSQRTSymbol, sym);
    }
    return element(singlePrecisionSQRTSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayCmpSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayCmpLenSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateCurrentTimeMaxPrecisionSymbol()
{
    if (!element(currentTimeMaxPrecisionSymbol)) {
        TR::MethodSymbol* sym = TR::MethodSymbol::create(trHeapMemory(), TR_Helper);
        sym->setHelper();

        element(currentTimeMaxPrecisionSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), currentTimeMaxPrecisionSymbol, sym);
    }
...
TR::SymbolReference* OMR::SymbolReferenceTable::createKnownStaticDataSymbolRef(
    void* dataAddress, TR::DataType type, TR::KnownObjectTable::Index knownObjectIndex)
{
    TR::StaticSymbol* sym = TR::StaticSymbol::create(trHeapMemory(), type);
    sym->setStaticAddress(dataAddress);
    sym->setNotCollected();
...
TR::SymbolReference* OMR::SymbolReferenceTable::createKnownStaticReferenceSymbolRef(
    void* dataAddress, TR::KnownObjectTable::Index knownObjectIndex)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::createRuntimeHelper(
    TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters)
{
    ...
    TR::SymbolReference* sr = baseArray.element(index)
        = new (trHeapMemory()) TR::SymbolReference(self(), index, methodSymbol);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateRuntimeHelper(
    TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateCodeGenInlinedHelper(CommonNonhelperSymbol index)
{
    if (!element(index)) {
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewObjectSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newObject, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewObjectNoZeroInitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newObjectNoZeroInit, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewValueSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newValue, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewValueNoZeroInitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newValueNoZeroInit, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewArraySymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newArray, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewArrayNoZeroInitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_newArrayNoZeroInit, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateANewArrayNoZeroInitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_aNewArrayNoZeroInit, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateInstanceOfSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_instanceOf, false, false, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVolatileReadLongSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_volatileReadLong, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVolatileWriteLongSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_volatileWriteLong, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVolatileReadDoubleSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_volatileReadDouble, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateVolatileWriteDoubleSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_volatileWriteDouble, true, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateCheckCastSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_checkCast, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateAThrowSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_aThrow, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNullCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_nullCheck, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateResolveCheckSymbolRef(
    TR::ResolvedMethodSymbol* owningMethodSymbol)
{
    if (!element(resolveCheckSymbol)) {
        ...
        element(resolveCheckSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), resolveCheckSymbol, symRef->getSymbol());
    }
    return element(resolveCheckSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayBoundsCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_arrayBoundsCheck, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateDivCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
 ...
 */
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateOverflowCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_overflowCheck, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateArrayStoreExceptionSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_arrayStoreException, false, true, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateMonitorEntrySymbolRef(TR::ResolvedMethodSymbol*)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::methodSymRefFromName(TR::ResolvedMethodSymbol* owningMethodSymbol,
    ...
    int32_t cpIndex)
{
    ...
    if (!method && comp()->compileRelocatableCode())
        comp()->failCompilation<TR::CompilationException>(
            "Failed to get method %s.%s%s from name", className, methodName, methodSignature);

    TR_ASSERT_FATAL(method, "methodSymRefFromName: method must exist: %s.%s%s", className, methodName, methodSignature);
    TR_ASSERT(kind != TR::MethodSymbol::Virtual,
        "methodSymRefFromName doesn't support virtual methods"); // Until we're able to look up vtable index

    // Note: we use cpIndex=-1 here so we don't end up getting back the original symref (which will not have the
    // signature we want)
    ...
    TR_ASSERT(
        _methodsBySignature.Locate(key), "After _methodsBySignature.Add, _methodsBySignature.Locate must be true");

    return result;
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(
    TR::SymbolReference* original, uintptr_t* referenceLocation)
{
    return findOrCreateSymRefWithKnownObject(original, referenceLocation, false);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(
    TR::SymbolReference* original, uintptr_t* referenceLocation, bool isArrayWithConstantElements)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findSymRefWithKnownObject(
    TR::Symbol* symbol, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (!symbol
            && symRef->getSymbol()
                   ->isAuto()) // For temp, symbol can be NULL. Find any temp with the given knownObjectIndex.
            return symRef;
    }
    return NULL;
}
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(
    TR::SymbolReference* originalSymRef, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    TR::SymbolReference* result
        = new (trHeapMemory()) TR::SymbolReference(self(), *originalSymRef, 0, knownObjectIndex);
    bucket->set(result->getReferenceNumber());

    // If the known object symref is created for an immutable array shadow, it should be aliased to other array shadows
    // as well Put the symRef in arrayElementSymRefs so that its alias set can be built correctly
...
TR::SymbolReference* OMR::SymbolReferenceTable::createTempSymRefWithKnownObject(
    TR::Symbol* symbol, mcount_t owningMethodIndex, int32_t slot, TR::KnownObjectTable::Index knownObjectIndex)
{
    TR_ASSERT_FATAL(
        symbol->isAutoOrParm(), "createTempSymRefWithKnownObject can only be called for temp symbol %p", symbol);
    TR_BitVector* bucket = _knownObjectSymrefsByObjectIndex[knownObjectIndex];
    if (!bucket) {
        bucket = new (trHeapMemory())
            TR_BitVector(baseArray.size(), trMemory(), heapAlloc, growable, TR_MemoryBase::SymbolReference);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateMonitorExitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(
        TR_monitorExit, true /* canGCandReturn */, true /* canGCandExcept */, true /* preservesAllRegisters */);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateTransactionEntrySymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_transactionEntry, true, false, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateTransactionAbortSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_transactionAbort, false, false, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateTransactionExitSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_transactionExit, true, false, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateAsyncCheckSymbolRef(TR::ResolvedMethodSymbol*)
{
    return findOrCreateRuntimeHelper(TR_asyncCheck, true, false, true);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateReportMethodEnterSymbolRef(TR::ResolvedMethodSymbol*)
{
...
char* OMR::SymbolReferenceTable::strdup(const char* arg)
{
...
TR::Symbol* OMR::SymbolReferenceTable::findOrCreateGenericIntShadowSymbol()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGenericIntShadowSymbolReference(
    intptr_t offset, bool allocateUseDefBitVector)
{
    // FIXME: we should attempt to find it
    return createGenericIntShadowSymbolReference(offset, allocateUseDefBitVector);
}

TR::SymbolReference* OMR::SymbolReferenceTable::createGenericIntShadowSymbolReference(
    intptr_t offset, bool allocateUseDefBitVector)
{
    TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
        self(), findOrCreateGenericIntShadowSymbol(), comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGenericIntArrayShadowSymbolReference(intptr_t offset)
{
    TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
        self(), findOrCreateGenericIntShadowSymbol(), comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
    symRef->setOffset(offset);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGenericIntNonArrayShadowSymbolReference(intptr_t offset)
{
    TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
        self(), findOrCreateGenericIntShadowSymbol(), comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
    symRef->setOffset(offset);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateConstantAreaSymbolReference()
{
    if (!_constantAreaSymbolReference) {
        _constantAreaSymbolReference
            = new (comp()->trHeapMemory()) TR::SymbolReference(self(), findOrCreateConstantAreaSymbol());
        _constantAreaSymbolReference->setFromLiteralPool();
    }
    return _constantAreaSymbolReference;
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGCRPatchPointSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateStartPCSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateCounterAddressSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findStaticSymbol(
    TR_ResolvedMethod* owningMethod, int32_t cpIndex, TR::DataType type)
{
    TR::SymbolReference* symRef;
    TR_SymRefIterator i(type == TR::Address
            ? aliasBuilder.addressStaticSymRefs()
            : (type == TR::Int32 ? aliasBuilder.intStaticSymRefs() : aliasBuilder.nonIntPrimitiveStaticSymRefs()),
        self());
    while ((symRef = i.getNext())) {
        if (symRef->getSymbol()->getDataType() == type && symRef->getCPIndex() != -1
            && TR::Compiler->cls.jitStaticsAreSame(
                comp(), owningMethod, cpIndex, symRef->getOwningMethod(comp()), symRef->getCPIndex()))
            return symRef;
    }
    return 0;
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateClassSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, void* classObject, bool cpIndexOfStatic)
{
    ...
    //
    // findOrCreateCPSymbol will reuse an existing resolved symbol reference if a previously created symbol references
    // contains the same resolved address.  As a result we can end up sharing symbol references between an explicit
    // reference to the class and an implicit reference used on a write barrier node.   In such a case we make sure that
    // the shared symref uses the cpindex for the explicit reference and has the 'addressIsCPIndexOfStatic' set to
    // false.
    //
    TR::SymbolReference* symRef
        = findOrCreateCPSymbol(owningMethodSymbol, cpIndex, TR::Address, classObject != 0, classObject);
    ...
    if (cpIndex == -1 && comp()->compileRelocatableCode() && !comp()->getOption(TR_UseSymbolValidationManager)) {
        ...
        TR_ASSERT_FATAL(loader == bootstrapLoader, "class symref cpIndex=-1 in AOT not loaded by bootstrap loader\n");
    }
#endif

    if (cpIndexOfStatic) {
        if (symRef->getCPIndex() == cpIndex
            && symRef->getOwningMethodIndex() == owningMethodSymbol->getResolvedMethodIndex())
    ...
    TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    symRef = new (trHeapMemory()) TR::SymbolReference(
        self(), sym, owningMethodSymbol->getResolvedMethodIndex(), cpIndex, unresolvedIndex, knownObjectIndex);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateComputedStaticMethodSymbol(
    mcount_t owningMethodIndex, int32_t cpIndex, TR_ResolvedMethod* resolvedMethod)
{
    return findOrCreateMethodSymbol(
        owningMethodIndex, cpIndex, resolvedMethod, TR::MethodSymbol::ComputedStatic, false /*isUnresolvedInCP*/);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateStaticMethodSymbol(
    mcount_t owningMethodIndex, int32_t cpIndex, TR_ResolvedMethod* resolvedMethod)
{
    return findOrCreateMethodSymbol(
        owningMethodIndex, cpIndex, resolvedMethod, TR::MethodSymbol::Static, false /*isUnresolvedInCP*/);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateMethodSymbol(mcount_t owningMethodIndex, int32_t cpIndex,
    TR_ResolvedMethod* resolvedMethod, TR::MethodSymbol::Kinds callKind, bool isUnresolvedInCP)
{
    TR::SymbolReference* symRef;
    // We check comp()->getHasMethodHandleInvoke() here only because owningMethodDoesntMatter() could be expensive, and
    // currently only JSR292 uses this facility
    if (comp()->getHasMethodHandleInvoke() && resolvedMethod
        && resolvedMethod->owningMethodDoesntMatter()) // Any symbol with the same TR_OpaqueMethodBlock will do
    {
        ...
        for (symRef = i.getNext(); symRef; symRef = i.getNext()) {
            ...
            if (existingMethod && (existingMethod->getMethodKind() == callKind)
                && resolvedMethod->canAlwaysShareSymbolDespiteOwningMethod(existingMethod->getResolvedMethod())) {
                if (performTransformation(comp(), "O^O SYMBOL SHARING: Reusing #%d M%p for M%p\n",
                    ...
                    TR_ResolvedMethod* owningMethod
                        = comp()->getOwningMethodSymbol(owningMethodIndex)->getResolvedMethod();
    ...
    } else if (cpIndex != -1) // -1 is a indication that the symbol reference shouldn't be shared
    {
        TR_SymRefIterator i(aliasBuilder.methodSymRefs(), self());
        for (symRef = i.getNext(); symRef; symRef = i.getNext()) {
            if (cpIndex == symRef->getCPIndex() && owningMethodIndex == symRef->getOwningMethodIndex() && cpIndex != -1
    ...
    if (resolvedMethod) {
...
#ifdef J9_PROJECT_SPECIFIC
        canGCandReturn
            = !(resSym->canDirectNativeCall() || resSym->getRecognizedMethod() == TR::java_lang_System_arraycopy);
        ...
        sym = TR::MethodSymbol::create(
            trHeapMemory(), TR_Private, fej9->createMethod(trMemory(), owningMethod->containingClass(), cpIndex));
#else
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateAutoSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol,
    int32_t slot, TR::DataType type, bool isReference, bool isInternalPointer, bool reuseAuto, bool isAdjunct,
    size_t size)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateAutoSymbolImpl(TR::ResolvedMethodSymbol* owningMethodSymbol,
    int32_t slot, TR::DataType type, bool isReference, bool isInternalPointer, bool reuseAuto, bool isAdjunct,
    size_t size, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    List<TR::SymbolReference>& list
        = slot < 0 ? owningMethodSymbol->getPendingPushSymRefs(-slot - 1) : owningMethodSymbol->getAutoSymRefs(slot);
    ...
    for (; symRef; symRef = i.getNext())
        ...
        // dual symref match, when original type is an address, and needs to be coerced
        {
            ...
            if (symRef->getSymbol()->getParmSymbol() || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
                comp()->failCompilation<TR::CompilationException>(
                    "stack mapping can't handle a parameter slot that is shared between refs and nonrefs 0");
            }
        ...
        for (; symRef2; symRef2 = i2.getNext()) {
            if (comp()->getOption(TR_MimicInterpreterFrameShape)
                ...
                if (symRef2->getSymbol()->getParmSymbol() || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
                    comp()->failCompilation<TR::CompilationException>(
                        "stack mapping can't handle a parameter slot that is shared between refs and nonrefs 1");
                }
                slotSharedbyRefAndNonRef = true;
                symRef2->getSymbol()->setSlotSharedByRefAndNonRef(
                    true); // An address slot is being shared with an int slot.
                comp()->setSlotsSharedByRefAndNonRef(true);
            }
        }
    }
    ...
    if ((type == TR::Address) && (slot != -1) && (slot != 0)) {
        ...
        for (; symRef2; symRef2 = i2.getNext()) {
            if (comp()->getOption(TR_MimicInterpreterFrameShape)
                ...
                if (symRef2->getSymbol()->getParmSymbol() || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
                    comp()->failCompilation<TR::CompilationException>(
                        "stack mapping can't handle a parameter slot that is shared between refs and nonrefs 3");
                }
                slotSharedbyRefAndNonRef = true;
                symRef2->getSymbol()->setSlotSharedByRefAndNonRef(
                    true); // An address slot is being shared with an int slot.
    ...
    if (!symRef) {
        ...
        if (isInternalPointer) {
            sym = size ? TR::AutomaticSymbol::createInternalPointer(
                      trHeapMemory(), type, static_cast<uint32_t>(size), comp()->fe())
    ...
    List<TR::SymbolReference>& list3
        = slot < 0 ? owningMethodSymbol->getPendingPushSymRefs(-slot - 1) : owningMethodSymbol->getAutoSymRefs(slot);
    list3.add(symRef);
    return symRef;
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateShadowSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, bool isStore)
{
    TR_UNIMPLEMENTED();
    return NULL;
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrFabricateShadowSymbol(TR_OpaqueClassBlock* containingClass,
...
TR::SymbolReference* OMR::SymbolReferenceTable::findAvailableAuto(
...
TR::SymbolReference* OMR::SymbolReferenceTable::findAvailableAuto(
    List<TR::SymbolReference>& availableAutos, TR::DataType type, bool behavesLikeTemp, bool isAdjunct)
{
...
void OMR::SymbolReferenceTable::makeAutoAvailableForIlGen(TR::SymbolReference* a)
{
    if (!a->getSymbol()->isNotCollected() && !_availableAutos.find(a))
        _availableAutos.add(a);
}

TR::ParameterSymbol* OMR::SymbolReferenceTable::createParameterSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol,
    int32_t slot, TR::DataType type, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    if (knownObjectIndex == TR::KnownObjectTable::UNKNOWN)
        ...
        symRef = createTempSymRefWithKnownObject(
            sym, owningMethodSymbol->getResolvedMethodIndex(), slot, knownObjectIndex);
...
TR::SymbolReference* OMR::SymbolReferenceTable::createTemporary(
    TR::ResolvedMethodSymbol* owningMethodSymbol, TR::DataType type, bool isInternalPointer, size_t size)
{
#ifdef J9_PROJECT_SPECIFIC
    TR_ASSERT(!type.isBCD() || size, "binary coded decimal types must provide a size\n");
#endif

    return findOrCreateAutoSymbol(
        owningMethodSymbol, owningMethodSymbol->incTempIndex(fe()), type, true, isInternalPointer, false, false, size);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateTemporaryWithKnowObjectIndex(
    TR::ResolvedMethodSymbol* owningMethodSymbol, TR::KnownObjectTable::Index knownObjectIndex)
{
    TR::SymbolReference* symRef = findTempSymRefWithKnownObject(knownObjectIndex);
    if (symRef)
        return symRef;
    return findOrCreateAutoSymbolImpl(owningMethodSymbol, owningMethodSymbol->incTempIndex(fe()), TR::Address,
        true /* isReference */, false /* isInternalPointer*/, false /* reuseAuto */, false /* isAdjunct */, 0 /*size*/,
        knownObjectIndex);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreatePendingPushTemporary(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t slot, TR::DataType type, size_t size)
{
#ifdef J9_PROJECT_SPECIFIC
    TR_ASSERT(!type.isBCD() || size, "binary coded decimal types must provide a size\n");
    TR_ASSERT_FATAL(!owningMethodSymbol->comp()->getOption(TR_EnableOSR)
            || (slot + TR::Symbol::convertTypeToNumberOfSlots(type) - 1) < owningMethodSymbol->getNumPPSlots(),
        "cannot create a pending push temporary that exceeds the number of slots for this method\n");
#endif
    TR::SymbolReference* tempSymRef
        = findOrCreateAutoSymbol(owningMethodSymbol, -(slot + 1), type, true, false, false, false, size);
    tempSymRef->getSymbol()->setIsPendingPush();
    return tempSymRef;
}

TR::SymbolReference* OMR::SymbolReferenceTable::createNamedStatic(
    TR::ResolvedMethodSymbol* owningMethodSymbol, TR::DataType type, const char* name)
{
    TR::StaticSymbol* sym = TR::StaticSymbol::createNamed(trHeapMemory(), type, name);
    return new (trHeapMemory()) TR::SymbolReference(
        self(), sym, owningMethodSymbol->getResolvedMethodIndex(), owningMethodSymbol->incTempIndex(fe()));
}

TR::SymbolReference* OMR::SymbolReferenceTable::createLocalObject(
    int32_t objectSize, TR::ResolvedMethodSymbol* owningMethodSymbol, TR::SymbolReference* classSymRef)
{
    ...
    TR::AutomaticSymbol* sym
        = TR::AutomaticSymbol::createLocalObject(trHeapMemory(), TR::New, classSymRef, TR::Aggregate, objectSize, fe());
...
TR::SymbolReference* OMR::SymbolReferenceTable::createLocalAddrArray(
    int32_t objectSize, TR::ResolvedMethodSymbol* owningMethodSymbol, TR::SymbolReference* classSymRef)
{
    int32_t slot = owningMethodSymbol->incTempIndex(fe());
    TR_ResolvedMethod* method = owningMethodSymbol->getResolvedMethod();
    ...
    TR::AutomaticSymbol* sym = TR::AutomaticSymbol::createLocalObject(
        trHeapMemory(), TR::anewarray, classSymRef, TR::Aggregate, objectSize, fe());
    sym->setBehaveLikeNonTemp();
    owningMethodSymbol->addAutomatic(sym);
    TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodIndex, slot);
    owningMethodSymbol->getAutoSymRefs(slot).add(symRef);
...
TR::SymbolReference* OMR::SymbolReferenceTable::createLocalPrimArray(
    int32_t objectSize, TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t arrayType)
{
    int32_t slot = owningMethodSymbol->incTempIndex(fe());
    TR_ResolvedMethod* method = owningMethodSymbol->getResolvedMethod();
    mcount_t owningMethodIndex = owningMethodSymbol->getResolvedMethodIndex();
    ...
    TR::AutomaticSymbol* sym
        = TR::AutomaticSymbol::createLocalObject(trHeapMemory(), arrayType, TR::Aggregate, objectSize, fe());
    sym->setBehaveLikeNonTemp();
    sym->setNotCollected();
    owningMethodSymbol->addAutomatic(sym);
    TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodIndex, slot);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateCounterSymRef(char* name, TR::DataType d, void* address)
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::createSymbolReference(TR::Symbol* sym, intptr_t offset)
{
...
{
    if (unsigned(ref) >= _numHelperSymbols && unsigned(ref) < _numHelperSymbols + getLastCommonNonhelperSymbol()
        && s < getLastCommonNonhelperSymbol()) {
...
OMR::SymbolReferenceTable::CommonNonhelperSymbol OMR::SymbolReferenceTable::getNonHelperSymbol(
    TR::SymbolReference* symRef)
{
...
{
    return isNonHelper(ref) ? (CommonNonhelperSymbol)(ref - _numHelperSymbols) : getLastCommonNonhelperSymbol();
}
...
TR::SymbolReference*& OMR::SymbolReferenceTable::element(TR_RuntimeHelper s) { return baseArray.element(s); }
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateJProfileValuePlaceHolderSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateJProfileValuePlaceHolderWithNullCHKSymbolRef()
{
    if (!element(jProfileValueWithNullCHKSymbol)) {
        TR::MethodSymbol* sym = TR::MethodSymbol::create(trHeapMemory(), TR_None);
        sym->setHelper();
        element(jProfileValueWithNullCHKSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), jProfileValueWithNullCHKSymbol, sym);
    }
    return element(jProfileValueWithNullCHKSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreatePotentialOSRPointHelperSymbolRef()
{
    if (!element(potentialOSRPointHelperSymbol)) {
        TR::MethodSymbol* sym = TR::MethodSymbol::create(trHeapMemory(), TR_None);
        sym->setHelper();
        TR::SymbolReference* symRef
            = new (trHeapMemory()) TR::SymbolReference(self(), potentialOSRPointHelperSymbol, sym);
        element(potentialOSRPointHelperSymbol) = symRef;
    }
    return element(potentialOSRPointHelperSymbol);
}

TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateOSRFearPointHelperSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateEAEscapeHelperSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::getOriginalUnimprovedSymRef(TR::SymbolReference* symRef)
{
...
void OMR::SymbolReferenceTable::rememberOriginalUnimprovedSymRef(
    TR::SymbolReference* improved, TR::SymbolReference* original)
{
    ...
    TR_ASSERT_FATAL(insertionSucceeded, "original unimproved symref collision for #%d: originals are #%d and #%d",
        improved->getReferenceNumber(), entryPreventingInsertion->second, original->getReferenceNumber());
}

const char* OMR::SymbolReferenceTable::_commonNonHelperSymbolNames[] = { "<contiguousArraySize>",
    "<discontiguousArraySize>", "<arrayClassRomPtr>", "<javaLangClassFromClass>", "<classFromJavaLangClass>",
    "<addressOfClassOfMethod>", "<componentClass>", "<isArray>", "<isClassDepthAndFlags>", "<isClassFlags>", "<vft>",
    "<currentThread>", "<recompilationCounter>", "<excp>", "<indexableSize>", "<resolveCheck>", "<arrayTranslate>",
    "<arrayTranslateAndTest>", "<long2String>", "<bitOpMem>", "<reverseLoad>", "<reverseStore>",
    "<currentTimeMaxPrecision>", "<headerFlags>", "<singlePrecisionSQRT>", "<countForRecompile>", "<gcrPatchPoint>",
    "<counterAddress>", "<startPC>", "<compiledMethod>", "<thisRangeExtension>", "<profilingBufferCursor>",
    "<profilingBufferEnd>", "<profilingBuffer>", "<osrBuffer>", "<osrScratchBuffer>", "<osrFrameIndex>",
    "<osrReturnAddress>", "<contiguousArrayDataAddrField>", "<potentialOSRPointHelper>", "<osrFearPointHelper>",
...
{
    TR_ASSERT_FATAL(
        nonHelper >= OMRfirstPrintableCommonNonhelperSymbol && nonHelper <= OMRlastPrintableCommonNonhelperSymbol,
