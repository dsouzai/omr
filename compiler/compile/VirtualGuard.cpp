...
TR_VirtualGuard::TR_VirtualGuard(TR_VirtualGuardTestType test, TR_VirtualGuardKind kind, TR::Compilation* comp,
    ...
    ,
#endif
    _mutableCallSiteObject(0)
    , _mutableCallSiteEpoch(0)
    , _evalChildren(true)
...
{
    return createVftGuardWithReceiver(
        kind, comp, calleeIndex, callNode, destination, thisClass, callNode->getFirstArgument());
}
    ...
    TR::Node* receiverNode)
{
    ...
    TR_VirtualGuard* vg = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_VftTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), thisClass);
...
TR::Node* TR_VirtualGuard::createMethodGuard(TR_VirtualGuardKind kind, TR::Compilation* comp, int16_t calleeIndex,
    ...
    TR_OpaqueClassBlock* thisClass)
{
    return createMethodGuardWithReceiver(
        kind, comp, calleeIndex, callNode, destination, calleeSymbol, thisClass, callNode->getFirstArgument());
}
    ...
    TR::TreeTop* destination, TR::ResolvedMethodSymbol* calleeSymbol)
{
    ...
    TR_VirtualGuard* vg = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_FSDTest, TR_BreakpointGuard, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex());
    ...
    TR_OpaqueClassBlock* thisClass, TR::Node* receiverNode)
{
    ...
    if (realThisClass && !TR::Compiler->cls.isInterfaceClass(comp, realThisClass)
        && callSymRef->getSymbol()->castToMethodSymbol()->isInterface())
        offset = callSymRef->getOwningMethod(comp)->getResolvedInterfaceMethodOffset(
            realThisClass, callSymRef->getCPIndex());
    else
    ...
    TR_VirtualGuard* vg = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_MethodTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), thisClass);
    ...
    bool forInline)
{
    ...
    TR_VirtualGuard* vg = (TR_VirtualGuard*)(new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_NonoverriddenTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex()));
...
TR::Node* TR_VirtualGuard::createMutableCallSiteTargetGuard(TR::Compilation* comp, int16_t calleeIndex, TR::Node* node,
    TR::TreeTop* destination, uintptr_t* mcsObject, TR::KnownObjectTable::Index mcsEpoch)
{
    TR::SymbolReferenceTable* symRefTab = comp->getSymRefTab();
    ...
    TR::Node* guard = TR::Node::createif(
        TR::ifacmpne, node, load, destination); // NOTE: take bytecode info from node so findVirtualGuardInfo works
    guard->getAndDecChild(0);
    guard->setAndIncChild(0, receiver);

    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_DummyTest, TR_MutableCallSiteTargetGuard, comp, node, guard, comp->getCurrentInlinedSiteIndex());
...
TR::Node* TR_VirtualGuard::createDummyOrSideEffectGuard(
    TR::Compilation* comp, TR::Node* callNode, TR::TreeTop* destination)
{
    TR::SymbolReferenceTable* symRefTab = comp->getSymRefTab();
...
TR::Node* TR_VirtualGuard::createSideEffectGuard(TR::Compilation* comp, TR::Node* callNode, TR::TreeTop* destination)
{
    ...
    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_NonoverriddenTest, TR_SideEffectGuard, comp, callNode, guard, comp->getCurrentInlinedSiteIndex());
    vguard->dontGenerateChildrenCode();
    return guard;
}

TR::Node* TR_VirtualGuard::createAOTInliningGuard(
    TR::Compilation* comp, int16_t calleeIndex, TR::Node* callNode, TR::TreeTop* destination, TR_VirtualGuardKind kind)
{
    TR::Node* guard = createDummyOrSideEffectGuard(comp, callNode, destination);
    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_NonoverriddenTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex());
...
TR::Node* TR_VirtualGuard::createAOTGuard(
    TR::Compilation* comp, int16_t siteIndex, TR::Node* callNode, TR::TreeTop* destination, TR_VirtualGuardKind kind)
{
    TR::Node* guard = createDummyOrSideEffectGuard(comp, callNode, destination);
...
TR::Node* TR_VirtualGuard::createDummyGuard(
    TR::Compilation* comp, int16_t calleeIndex, TR::Node* callNode, TR::TreeTop* destination)
{
    TR::Node* guard = createDummyOrSideEffectGuard(comp, callNode, destination);
    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_NonoverriddenTest, TR_DummyGuard, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex());
    vguard->dontGenerateChildrenCode();
    if (comp->compileRelocatableCode())
        vguard->setCannotBeRemoved();
...
TR_VirtualGuard* TR_VirtualGuard::createGuardedDevirtualizationGuard(
...
TR_VirtualGuard* TR_VirtualGuard::createArrayStoreCheckGuard(
    TR::Compilation* comp, TR::Node* ascNode, TR_OpaqueClassBlock* clazz)
{
...
TR::Node* TR_VirtualGuard::createOSRGuard(TR::Compilation* comp, TR::TreeTop* destination)
{
    ...
    TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
        TR_DummyTest, TR_OSRGuard, comp, NULL, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), NULL);
