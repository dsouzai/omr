...
void OMR::AliasBuilder::createAliasInfo()
{
    ...
    defaultMethodDefAliasesWithoutImmutable().init(
        symRefTab()->getNumSymRefs(), comp()->trMemory(), heapAlloc, growable);
    defaultMethodDefAliasesWithoutUserField().init(
        symRefTab()->getNumSymRefs(), comp()->trMemory(), heapAlloc, growable);
...
void OMR::AliasBuilder::gatherLocalUseInfo(
    TR::Node* node, TR_BitVector& storeVector, vcount_t visitCount, bool isOSRCatch)
{
