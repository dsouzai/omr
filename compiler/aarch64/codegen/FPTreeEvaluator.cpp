...
TR::Register* OMR::ARM64::TreeEvaluator::ibits2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::ARM64::TreeEvaluator::lbits2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::ARM64::TreeEvaluator::fbits2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpBits2integerHelper(node, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dbits2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpBits2integerHelper(node, true, cg);
}

static uint16_t getImm8forFloat(float value)
{
    if (value == 0.0f) {
        return 256;
    }
...
static uint16_t getImm8forDouble(double value)
{
    if (value == 0.0) {
        return 256;
    }
    ...
    if (value < 0) {
        ieee_double
            = (((uint64_t)obj.raw.sign << 63) | ((uint64_t)obj.raw.exponent << 52)) | (uint64_t)obj.raw.mantissa;
    } else if (value > 0) {
...
TR::Register* OMR::ARM64::TreeEvaluator::fconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* trgReg = cg->allocateSinglePrecisionRegister();
...
TR::Register* OMR::ARM64::TreeEvaluator::dconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* trgReg = cg->allocateRegister(TR_FPR);
...
TR::Register* OMR::ARM64::TreeEvaluator::dreturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return genericReturnEvaluator(node, cg->getProperties().getDoubleReturnRegister(), TR_FPR, TR_DoubleReturn, cg);
}

static TR::Register* commonFpEvaluator(
    TR::Node* node, TR::InstOpCode::Mnemonic op, bool isDouble, TR::CodeGenerator* cg)
{
...
{
    // TODO:ARM64: Enable TR::TreeEvaluator::fremEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::dremEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

static TR::Register* commonFpUnaryEvaluator(
    TR::Node* node, TR::InstOpCode::Mnemonic op, bool isDouble, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
...
{
    // TODO:ARM64: Enable TR::TreeEvaluator::d2cEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::d2sEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::d2sEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::d2bEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::d2bEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}
...
static TR::Instruction* iffcmpHelper(
    TR::Node* node, TR::ARM64ConditionCode cc, bool isDouble, bool needsExplicitUnorderedCheck, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::ARM64::TreeEvaluator::iffcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_EQ, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_NE, false, true, cg); // need to check NaN
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_MI, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_GE, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_GT, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LS, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_EQ, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_NE, true, true, cg); // need to check NaN
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_MI, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_GE, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_GT, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LS, true, false, cg);
    return NULL;
}

static TR::Register* fcmpHelper(
    TR::Node* node, TR::ARM64ConditionCode cc, bool isDouble, bool needsExplicitUnorderedCheck, TR::CodeGenerator* cg)
{
    TR::Register* trgReg = cg->allocateRegister();
    TR::Node* firstChild = node->getFirstChild();
...
TR::Register* OMR::ARM64::TreeEvaluator::fcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_EQ, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_NE, false, true, cg); // need to check NaN
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_MI, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GE, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GT, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_LS, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_EQ, true, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_NE, true, true, cg); // need to check NaN
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_MI, true, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GE, true, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GT, true, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_LS, true, false, cg);
}

static TR::Register* floatThreeWayCompareHelper(TR::Node* node, bool isDouble, bool isCmpl, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::ARM64::TreeEvaluator::fcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return floatThreeWayCompareHelper(node, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpgEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return floatThreeWayCompareHelper(node, false, false, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return floatThreeWayCompareHelper(node, true, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpgEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return floatThreeWayCompareHelper(node, true, false, cg);
}

// also handles dRegLoad
TR::Register* OMR::ARM64::TreeEvaluator::fRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
static TR::Register* fpMinMaxHelper(TR::Node* node, bool isDouble, TR::ARM64ConditionCode cc, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
...
TR::Register* OMR::ARM64::TreeEvaluator::fmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpMinMaxHelper(node, false, TR::CC_GE, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpMinMaxHelper(node, false, TR::CC_LS, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpMinMaxHelper(node, true, TR::CC_GE, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fpMinMaxHelper(node, true, TR::CC_LS, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::b2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::b2dEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::s2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::s2dEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::su2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // TODO:ARM64: Enable TR::TreeEvaluator::su2dEvaluator in compiler/aarch64/codegen/TreeEvaluatorTable.hpp when
    // Implemented.
    return OMR::ARM64::TreeEvaluator::unImpOpEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_EQ, false, true, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_NE, false, false, cg); // no need to check NaN
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LT, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_PL, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_HI, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::iffcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LE, false, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_EQ, true, true, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_NE, true, false, cg); // no need to check NaN
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LT, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_PL, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_HI, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::ifdcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    iffcmpHelper(node, TR::CC_LE, true, false, cg);
    return NULL;
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_EQ, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_NE, false, false, cg); // no need to check NaN
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_MI, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GE, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GT, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::fcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_LS, false, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_EQ, true, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_NE, true, false, cg); // no need to check NaN
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_MI, true, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GE, true, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_GT, true, true, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::dcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fcmpHelper(node, TR::CC_LS, true, true, cg);
}

// also handles dselect
TR::Register* OMR::ARM64::TreeEvaluator::fselectEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
