    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        if (regList.find(reg)) {
            ...
            if (kind == TR_GPR) {
                ...
                iCursor = generateTrg1Src1Instruction(
                    cg, isSinglePrecision ? TR::InstOpCode::fmovs : TR::InstOpCode::fmovd, node, copyReg, reg, iCursor);
            }
...
void OMR::ARM64::RegisterDependencyConditions::addPreCondition(
...
void OMR::ARM64::RegisterDependencyConditions::addPostCondition(
    TR::Register* vr, TR::RealRegister::RegNum rr, uint8_t flag)
{
...
void OMR::ARM64::RegisterDependencyConditions::assignPreConditionRegisters(
...
void OMR::ARM64::RegisterDependencyConditions::assignPostConditionRegisters(
    TR::Instruction* currentInstruction, TR_RegisterKinds kindToBeAssigned, TR::CodeGenerator* cg)
{
...
TR::RegisterDependencyConditions* OMR::ARM64::RegisterDependencyConditions::clone(
    TR::CodeGenerator* cg, TR::RegisterDependencyConditions* added, bool omitPre, bool omitPost)
{
...
void OMR::ARM64::RegisterDependencyConditions::stopUsingDepRegs(
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (_dependencies[i].isSpilledReg()) {
            ...
            if (virtReg->getAssignedRealRegister()) {
                ...
                TR::MemoryReference* tempMR = TR::MemoryReference::createWithSymRef(
                    cg, currentNode, (TR::SymbolReference*)virtReg->getBackingStorage()->getSymbolReference());
...
void TR_ARM64ScratchRegisterDependencyConditions::addDependency(
    TR::CodeGenerator* cg, TR::Register* vr, TR::RealRegister::RegNum rr, uint8_t flag)
{
...
void TR_ARM64ScratchRegisterDependencyConditions::unionDependency(
    TR::CodeGenerator* cg, TR::Register* vr, TR::RealRegister::RegNum rr, uint8_t flag)
{
    TR_ASSERT_FATAL(_numGPRDeps < TR::RealRegister::LastAssignableGPR - TR::RealRegister::FirstGPR + 1,
        "Too many GPR dependencies");
...
void TR_ARM64ScratchRegisterDependencyConditions::addScratchRegisters(
    TR::CodeGenerator* cg, TR_ARM64ScratchRegisterManager* srm)
{
