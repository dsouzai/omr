...
static TR::Register* generateMaddOrMsub(
    TR::Node* node, TR::Node* mulNode, TR::Node* anotherNode, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
    if ((mulNode->getOpCodeValue() == TR::imul || mulNode->getOpCodeValue() == TR::lmul)
        ...
        if (node->getOpCodeValue() != TR::aladd) {
             ...
             * where src1Reg also contains an internal pointer and has the same pinning array as the node.
             */
            if ((1 == mulFirstChild->getReferenceCount()) && node->isInternalPointer()
                && mulSrc1Reg->containsInternalPointer()
 ...
 */
static bool isNot(TR::Node* node)
{
    return node->getOpCode().isXor() && node->getSecondChild()->getOpCode().isLoadConst()
        && (node->getSecondChild()->getConstValue() == -1);
}
 ...
 */
static bool isShiftWithConstAmountNode(TR::Node* node)
{
 ...
 */
static bool isShiftedBinaryOp(TR::Node* node, TR::Node* lhs, TR::Node* rhs, TR::Node*& source1Node,
    TR::Node*& source2Node, TR::Node*& shiftNode, TR::Node*& notNode, int32_t& shiftValue)
{
    if ((!lhs->getOpCode().isLoadConst()) && (rhs->getReferenceCount() == 1) && (rhs->getRegister() == NULL)) {
        if (isShiftWithConstAmountNode(rhs)) {
            ...
            if (node->getOpCode().isBitwiseLogical() && (shiftChild->getReferenceCount() == 1)
                && (shiftChild->getRegister() == NULL) && isNot(shiftChild) && shiftNode->getOpCode().isRightShift()
                && (!shiftNode->getOpCode().isShiftLogical())) {
                notNode = shiftChild;
 ...
 */
static TR::Register* generateShiftedBinaryOperation(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::InstOpCode::Mnemonic notOp, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (!(isShiftedBinaryOp(node, firstChild, secondChild, source1Node, source2Node, shiftNode, notNode, shiftValue)
            || ((!node->getOpCode().isSub())
                && isShiftedBinaryOp(
                    node, secondChild, firstChild, source1Node, source2Node, shiftNode, notNode, shiftValue)))) {
        return NULL;
    }
    ...
    if (shiftNode != NULL) {
        TR::ARM64ShiftCode code = (shiftNode->getOpCode().isLeftShift()
                ? TR::SH_LSL
                : (shiftNode->getOpCode().isShiftLogical() ? TR::SH_LSR : TR::SH_ASR));

        generateTrg1Src2ShiftedInstruction(
            cg, (notNode == NULL) ? op : notOp, node, treg, s1reg, s2reg, code, shiftValue);
    } else {
...
TR::Register* OMR::ARM64::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* retReg;
...
TR::Register* OMR::ARM64::TreeEvaluator::inlineVectorBinaryOp(
...
TR::Register* OMR::ARM64::TreeEvaluator::vsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getDataType().getVectorLength() == TR::VectorLength128,
        "Only 128-bit vectors are supported %s", node->getDataType().toString());
...
TR::Register* OMR::ARM64::TreeEvaluator::vmulInt64Helper(
    TR::Node* node, TR::Register* productReg, TR::Register* lhsReg, TR::Register* rhsReg, TR::CodeGenerator* cg)
{
    /*
...
TR::Register* OMR::ARM64::TreeEvaluator::vmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getDataType().getVectorLength() == TR::VectorLength128,
        "Only 128-bit vectors are supported %s", node->getDataType().toString());
...
TR::Register* OMR::ARM64::TreeEvaluator::vdivIntHelper(
    TR::Node* node, TR::Register* resultReg, TR::Register* lhsReg, TR::Register* rhsReg, TR::CodeGenerator* cg)
{
    ...
    static const struct DivOps ops[4] = { { TR::InstOpCode::smovwb, TR::InstOpCode::sdivw, TR::InstOpCode::vinswb, 16 },
    ...
    if (loopCount > 1) {
        ...
        generateVectorShiftImmediateInstruction(
            cg, TR::InstOpCode::vushr4s, node, resultReg, resultReg, (eType == TR::Int8) ? 8 : 16);
        for (int32_t i = 0; i < numElementsPerLoop; i++) {
            ...
            generateMovGPRToVectorElementInstruction(
                cg, op.gprToVectorOp, node, resultReg, tmp1Reg, (i + 1) * loopCount - 1);
        }
        generateVectorShiftImmediateInstruction(
            cg, TR::InstOpCode::vushr4s, node, tmp1VectorReg, tmp1VectorReg, (eType == TR::Int8) ? 8 : 16);
        generateVectorShiftImmediateInstruction(
            cg, TR::InstOpCode::vushr4s, node, tmp2VectorReg, tmp2VectorReg, (eType == TR::Int8) ? 8 : 16);
...
TR::Register* OMR::ARM64::TreeEvaluator::vdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getDataType().getVectorLength() == TR::VectorLength128,
        "Only 128-bit vectors are supported %s", node->getDataType().toString());
...
/**
 * @brief A helper function for generating instuction sequence for min/max operations for vectors of 64-bit integer
 * elements.
 *
 * @param[in] node: node
 ...
 */
static TR::Register* vminmaxInt64Helper(
    TR::Node* node, bool isMax, TR::Register* resReg, TR::Register* lhsReg, TR::Register* rhsReg, TR::CodeGenerator* cg)
{
    /*
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::vbsl16b, node, resReg, (isMax ? lhsReg : rhsReg), (isMax ? rhsReg : lhsReg));

    return resReg;
}

TR::Register* OMR::ARM64::TreeEvaluator::vminInt64Helper(
    TR::Node* node, TR::Register* resReg, TR::Register* lhsReg, TR::Register* rhsReg, TR::CodeGenerator* cg)
{
    return vminmaxInt64Helper(node, false, resReg, lhsReg, rhsReg, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::vmaxInt64Helper(
    TR::Node* node, TR::Register* resReg, TR::Register* lhsReg, TR::Register* rhsReg, TR::CodeGenerator* cg)
{
    return vminmaxInt64Helper(node, true, resReg, lhsReg, rhsReg, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::vminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getDataType().getVectorLength() == TR::VectorLength128,
        "Only 128-bit vectors are supported %s", node->getDataType().toString());
...
TR::Register* OMR::ARM64::TreeEvaluator::vmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT_FATAL_WITH_NODE(node, node->getDataType().getVectorLength() == TR::VectorLength128,
        "Only 128-bit vectors are supported %s", node->getDataType().toString());
...
TR::Register* OMR::ARM64::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
...
TR::Register* OMR::ARM64::TreeEvaluator::imulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::ARM64::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
...
static TR::Register* idivHelper(TR::Node* node, bool is64bit, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src2Instruction(
        cg, is64bit ? TR::InstOpCode::sdivx : TR::InstOpCode::sdivw, node, trgReg, src1Reg, src2Reg);
...
static TR::Register* iremHelper(TR::Node* node, bool is64bit, bool isUnsigned, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src3Instruction(
        cg, is64bit ? TR::InstOpCode::msubx : TR::InstOpCode::msubw, node, trgReg, tmpReg, src2Reg, src1Reg);
...
TR::Register* OMR::ARM64::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::ARM64::TreeEvaluator::bdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return subInt32DivEvaluator(node, cg);
}

TR::Register* OMR::ARM64::TreeEvaluator::sdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return subInt32DivEvaluator(node, cg);
}
 ...
 * @param[in]        cg: code generator
 *
 * @returns register which contains the result of the operation. NULL if the operation cannot be encoded in ubfm
 * instruction.
 */
static TR::Register* generateUBFMForMaskAndShift(TR::Node* shiftNode, TR::CodeGenerator* cg)
{
    ...
    if (shiftNode->getOpCode().isLeftShift()) {
        ...
        if (((maskValue & 1) == 1) && (((maskValue >> (operandBits - 1)) & 1) == 0) && contiguousBits(maskValue)) {
            /*
             * maskValue has consecutive 1s from the least significant bits. The most significant bit must be 0.
             * Otherwise, mask is all 1s. So, this mask and shift operation is copying consecutive bits starting from
             * the least significant bits of the source register to bit position shiftValue of the destination register.
             */
    ...
    } else /* right shift*/
    {
         ...
         * consecutive 1s starting from the bit position shiftValue of the source register to the least significant bits
                ...
                if (shiftNode->getOpCode().isShiftLogical()) {
                    if (operandBits < 32) {
                        generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::ubfmw, andNode, reg, sreg, operandBits - 1); // uxth or uxtb
                        shiftSrcReg = reg;
                    }
                    ...
                    if (operandBits < 32) {
                        generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::sbfmw, andNode, reg, sreg, operandBits - 1); // sxth or sxtb
                        shiftSrcReg = reg;
                    }
    ...
    if (secondOp == TR::iconst) {
        ...
        if (value == 0) {
            ...
            case TR::SH_LSR:
                if (operandBits < 32) {
                    generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::ubfmw, node, trgReg, srcReg, operandBits - 1); // uxth or uxtb
                    shiftSrcReg = trgReg;
                }
            ...
            case TR::SH_ASR:
                if (operandBits < 32) {
                    generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::sbfmw, node, trgReg, srcReg, operandBits - 1); // sxth or sxtb
                    shiftSrcReg = trgReg;
                }
...
// also handles lshl
TR::Register* OMR::ARM64::TreeEvaluator::ishlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (node->getOpCodeValue() == TR::lshl) {
        TR::Node* firstChild = node->getFirstChild();
        TR::Node* secondChild = node->getSecondChild();
        if (firstChild->getOpCodeValue() == TR::i2l && firstChild->getRegister() == NULL
            && firstChild->getReferenceCount() == 1 && secondChild->getOpCodeValue() == TR::iconst
...
// also handles lshr
TR::Register* OMR::ARM64::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* reg = generateUBFMForMaskAndShift(node, cg);
    if (reg != NULL) {
        return reg;
...
// also handles lushr
TR::Register* OMR::ARM64::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* reg = generateUBFMForMaskAndShift(node, cg);
    if (reg != NULL) {
        return reg;
...
// also handles lrol
TR::Register* OMR::ARM64::TreeEvaluator::irolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (secondChild->getOpCode().isNeg() && secondChild->getRegister() == NULL
 ...
 */
template<typename U> bool decodeBitMasksImpl(bool Nbit, uint32_t immr, uint32_t imms, U& wmask)
{
 ...
 * @return true if successfully decoded
 */
bool decodeBitMasks(bool Nbit, uint32_t immr, uint32_t imms, uint32_t& wmask)
{
 ...
 * @return true if successfully decoded
 */
bool decodeBitMasks(bool Nbit, uint32_t immr, uint32_t imms, uint64_t& wmask)
{
    return decodeBitMasksImpl(Nbit, immr, imms, wmask);
}
...
inline bool findLogicImmediateBitPattern(T value, int elementSize, int32_t& rotateCount, int32_t& onesCount)
{
 ...
 */
bool logicImmediateHelper(uint64_t value, bool is64Bit, bool& n, uint32_t& immEncoded)
{
 ...
 * instruction.
 */
static TR::Register* generateUBFMForShiftAndMask(TR::Node* andNode, TR::CodeGenerator* cg)
{
    TR::Node* shiftNode = NULL;
    TR::Node* maskNode = NULL;
    ...
    if ((shiftNode->getReferenceCount() == 1) && (shiftNode->getRegister() == NULL)
        && shiftNode->getSecondChild()->getOpCode().isLoadConst() && maskNode->getOpCode().isLoadConst()) {
        const uint32_t operandBits = TR::DataType::getSize(shiftNode->getDataType()) * 8;
        const bool is64bit = shiftNode->getDataType().isInt64();
        ...
        if (shiftNode->getOpCode().isLeftShift()) {
            ...
            if (((shiftedMask & 1) == 1) && contiguousBits(shiftedMask)) {
                 ...
                 * So, this shift and mask operation is copying consecutive bits starting from the least significant
                 * bits of the source register to bit position shiftValue of the destination register.
                 */
                uint32_t width = populationCount(shiftedMask);
        ...
        {
            /*
             * If the lsb of maskValue is set and the msb is not set and the maskValue has consecutive 1s, then this
             * operation is copying consecutive 1s starting from the bit position shiftValue of the source register to
             * the least significant bits of the destination register. We consider arithmetic shift only.
             */
            if ((!shiftNode->getOpCode().isShiftLogical()) && ((maskValue & 1) == 1)
                && (((maskValue >> (operandBits - 1)) & 1) == 0) && contiguousBits(maskValue)) {
                uint32_t width = populationCount(maskValue);
                uint32_t shiftRemainderWidth = operandBits - shiftValue;
                ...
                bool isLogicalShiftRight
                    = (width == shiftRemainderWidth); /* In this case, it works as a logical shift right. */
...
TR::Register* OMR::ARM64::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
