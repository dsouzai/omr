...
const TR::ARM64LinkageProperties& TR::ARM64SystemLinkage::getProperties() { return _properties; }

void TR::ARM64SystemLinkage::initARM64RealRegisterLinkage()
{
...
uint32_t TR::ARM64SystemLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }
...
void TR::ARM64SystemLinkage::mapStack(TR::ResolvedMethodSymbol* method)
{
    TR::Machine* machine = cg()->machine();
    ...
    while (localCursor != NULL) {
        if (localCursor->getGCMapIndex() < 0 && localCursor->getDataType() != TR::Int64
            && localCursor->getDataType() != TR::Double && !localCursor->getDataType().isVector()) {
...
void TR::ARM64SystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
...
void TR::ARM64SystemLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method)
{
...
void TR::ARM64SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    createPrologue(cursor, comp()->getJittedMethodSymbol()->getParameterList());
}

void TR::ARM64SystemLinkage::createPrologue(TR::Instruction* cursor, List<TR::ParameterSymbol>& parmList)
{
...
void TR::ARM64SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    TR::CodeGenerator* cg = this->cg();
    const TR::ARM64LinkageProperties& properties = getProperties();
    TR::Machine* machine = cg->machine();
        ...
        case TR::Double:
            ...
            if (numFloatArgs < properties.getNumFloatArgRegs()) {
                ...
                if ((numFloatArgs == 0 && resType.isFloatingPoint())) {
                    ...
                    TR::addDependency(
                        dependencies, argRegister, properties.getFloatArgumentRegister(numFloatArgs), TR_FPR, cg());
                }
            } else {
...
#error Unsupported platform
    ...
    for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::x0 + properties.getNumIntArgRegs());
         i <= TR::RealRegister::LastAssignableGPR; i++) {
