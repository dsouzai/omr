...
TR::Instruction* generateInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Instruction* preced)
{
    if (preced)
...
TR::ARM64ImmInstruction* generateImmInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, uint32_t imm, TR::Instruction* preced)
{
    if (preced)
    ...
    uintptr_t imm, TR_ExternalRelocationTargetKind relocationKind, TR::SymbolReference* sr, TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64RelocatableImmInstruction(op, node, imm, relocationKind, sr, preced, cg);
...
TR::Instruction* generateLabelInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::LabelSymbol* sym, TR::Instruction* preced)
{
    if (preced)
    ...
    TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64TestBitBranchInstruction(op, node, sreg, bitpos, sym, cond, preced, cg);
...
TR::Instruction* generateRegBranchInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Register* treg, TR::Instruction* preced)
{
    if (preced)
...
TR::Instruction* generateAdminInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Node* fenceNode, TR::Instruction* preced)
{
    if (preced)
    ...
    TR::Register* treg, uint32_t imm, uint32_t shiftAmount, TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Trg1ImmShiftedInstruction(op, node, treg, imm, shiftAmount, preced, cg);
    ...
    TR::Register* treg, TR::Register* s1reg, uint32_t imm, TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Trg1Src1ImmInstruction(op, node, treg, s1reg, isShifted, imm, preced, cg);
    ...
    TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Trg1Src2ShiftedInstruction(op, node, treg, s1reg, s2reg, shiftType, shiftAmount, preced, cg);
    return new (cg->trHeapMemory())
    ...
    TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Trg1Src2ExtendedInstruction(op, node, treg, s1reg, s2reg, extendType, shiftAmount, preced, cg);
    return new (cg->trHeapMemory())
    ...
    TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Trg1Src2IndexedElementInstruction(op, node, treg, s1reg, s2reg, index, preced, cg);
...
TR::Instruction* generateSrc1Instruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Register* s1reg, TR::Instruction* preced)
{
    if (preced)
...
TR::Instruction* generateCompareImmInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* sreg, int32_t imm, bool is64bit, TR::Instruction* preced)
{
    ...
    TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Src1ImmCondInstruction(op, node, sreg, imm, cc, conditionFlags, preced, cg);
    ...
    TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::ARM64Src2CondInstruction(op, node, sreg1, sreg2, cc, conditionFlags, preced, cg);
...
TR::Instruction* generateCSetInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* treg, TR::ARM64ConditionCode cc, TR::Instruction* preced)
{
    ...
    uint32_t lsb, uint32_t width, bool is64bit, TR::Instruction* preced)
{
    ...
    return generateTrg1Src1ImmInstruction(
        cg, is64bit ? TR::InstOpCode::ubfmx : TR::InstOpCode::ubfmw, node, treg, sreg, (immr << 6) | imms, preced);
}
    ...
    uint32_t lsb, uint32_t width, bool is64bit, TR::Instruction* preced)
{
    ...
    return generateTrg1Src1ImmInstruction(
        cg, is64bit ? TR::InstOpCode::bfmx : TR::InstOpCode::bfmw, node, treg, sreg, (immr << 6) | imms, preced);
}
    ...
    TR::Node* node, TR::Register* treg, TR::Register* sreg, uint32_t shiftAmount, TR::Instruction* preced)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(node,
        (elementSize == 8) || (elementSize == 16) || (elementSize == 32) || (elementSize == 64),
        "Illegal element size: %d", elementSize);
    ...
    TR_ASSERT_FATAL_WITH_NODE(node, (shiftAmount >= shiftAmountLowerLimit) && (shiftAmount <= shiftAmountUpperLimit),
    ...
    TR::Register* treg, TR::Register* sreg, uint32_t srcIndex, TR::Instruction* preced)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(
        node, isVectorRegister(treg) && isVectorRegister(sreg), "The target and source register must be VRF");
