...
TR::MemoryReference* TR::MemoryReference::createWithIndexReg(
    TR::CodeGenerator* cg, TR::Register* baseReg, TR::Register* indexReg, uint8_t scale, TR::ARM64ExtendCode extendCode)
{
...
TR::MemoryReference* TR::MemoryReference::createWithDisplacement(
    TR::CodeGenerator* cg, TR::Register* baseReg, int64_t displacement)
{
...
TR::MemoryReference* TR::MemoryReference::createWithSymRef(
    TR::CodeGenerator* cg, TR::Node* node, TR::SymbolReference* symRef)
{
...
void OMR::ARM64::MemoryReference::validateImmediateOffsetAlignment(
    TR::Node* node, uint32_t alignment, TR::CodeGenerator* cg)
{
        ...
        if (_baseRegister != NULL) {
            if (constantIsUnsignedImm12(displacement)) {
                ...
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::addx, node, newBase, _baseRegister, node->getRegister());
            } else {
            ...
            if (_baseRegister != NULL) {
                if (constantIsUnsignedImm12(displacement)) {
                    generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addimmx, node, newBase, _baseRegister, displacement);
    ...
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL || shiftUnderAddressNode) {
        ...
        if (subTree->getOpCode().isArrayRef()) {
            ...
            if (subTree->getSecondChild()->getOpCode().isLoadConst()) {
                ...
                if (comp->getOption(TR_TraceCG)) {
                    traceMsg(
                        comp, "Capturing array access with constant index at node %p offset = %d\n", subTree, amount);
                }
...
void OMR::ARM64::MemoryReference::consolidateRegisters(TR::Node* srcTree, TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL) {
        ...
        if (self()->isIndexSignExtended()) {
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::sbfmx, srcTree, tempTargetRegister, tempTargetRegister, imm);
        }
        ...
        TR_ASSERT_FATAL(
            false, "consolidateRegister() expects (_baseRegister != NULL) || (_scale != 0) || isIndexSignExtended()");
    }
...
uint8_t* OMR::ARM64::MemoryReference::generateBinaryEncoding(
        ...
        if (op.getMnemonic() != TR::InstOpCode::addimmx) {
            ...
            if (index) {
                ...
                if (isRegisterOffsetInstruction(enc)) {
                    ...
                    if (scale != 0) {
                        TR_ASSERT_FATAL(
                            isValidScale(scale, op.getMnemonic()), "Invalid scale value %d for mnemonic", scale);
                        // set scale bit
                        *wcursor |= (1 << 12);
                    }
                } else {
                ...
                if (isImm9OffsetInstruction(enc)) {
                    ...
                    if (size > 0) {
                        TR_ASSERT(
                            (displacement & ((1 << size) - 1)) == 0, "Non-aligned offset in 2/4/8-byte memory access.");
                    }
                    ...
                    TR_ASSERT_FATAL(
                        (displacement & ((1 << size) - 1)) == 0, "displacement must be 4/8/16-byte alligned");
...
TR::Instruction* OMR::ARM64::MemoryReference::expandInstruction(
