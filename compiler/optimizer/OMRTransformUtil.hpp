...
public:
    ...
    static TR::Node *scalarizeAddressParameter(TR::Compilation *comp, TR::Node *address, size_t byteLengthOrPrecision,
     ...
     */
    static void transformCallNodeToPassThrough(TR::Optimization *opt, TR::Node *node, TR::TreeTop *anchorTree,
        TR::Node *child);

    /**
     *    \brief
     *       Create a conditional alternate implementation path for existing operations. Two blocks will be inserted,
     * one block with comparison tree (compareBlock) and one block to take when comparison succeeds (ifBlock). The
     ...
     *   \note
     *      After transformation, the original elseBlock will contain only one tree, that is the ifTree. Its original
     * trees will be in a newly created block which is its fall through block after the transformation.
     */
    static void createConditionalAlternatePath(TR::Compilation *comp, TR::TreeTop *ifTree, TR::TreeTop *thenTree,
        TR::Block *elseBlock, TR::Block *mergeBlock, TR::CFG *cfg, bool markCold = true);
     ...
     */
    static TR::Node *generateArrayElementAddressTrees(TR::Compilation *comp, TR::Node *arrayNode,
     ...
     */
    static TR::Node *generateConvertArrayElementIndexToOffsetTrees(TR::Compilation *comp, TR::Node *indexNode,
        TR::Node *elementSizeNode = NULL, int32_t elementSize = 0, bool useShiftOpCode = false);
