...
void TR_ReachabilityAnalysis::propagateOneInput(blocknum_t inputBlockNum, blocknum_t blockNum, int32_t depth,
    blocknum_t* stack, blocknum_t* depthMap, TR_BitVector* result)
{
...
void TR_ReachabilityAnalysis::traverse(
    blocknum_t blockNum, int32_t depth, blocknum_t* stack, blocknum_t* depthMap, TR_BitVector* result)
{
    //
...
void TR_ForwardReachability::propagateInputs(
...
void TR_ForwardReachabilityWithoutExceptionEdges::propagateInputs(
    blocknum_t blockNum, int32_t depth, blocknum_t* stack, blocknum_t* depth_map, TR_BitVector* closure)
{
    TR::Block* block = getBlock(blockNum);
...
void TR_BackwardReachability::propagateInputs(
    blocknum_t blockNum, int32_t depth, blocknum_t* stack, blocknum_t* depth_map, TR_BitVector* closure)
{
    TR::Block* block = getBlock(blockNum);
...
bool TR_CanReachGivenBlocks::isOrigin(TR::Block* block) { return _originBlocks->isSet(block->getNumber()); }

bool TR_CanReachNonColdBlocks::isOrigin(TR::Block* block)
{
    if (block->isCold())
        return false;
...
bool TR_CanBeReachedFromCatchBlock::isOrigin(TR::Block* block) { return block->isCatchBlock(); }

bool TR_CanBeReachedWithoutExceptionEdges::isOrigin(TR::Block* block)
{
