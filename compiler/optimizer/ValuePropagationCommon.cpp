...
OMR::ValuePropagation::ValueConstraint* OMR::ValuePropagation::createValueConstraint(
    int32_t valueNumber, Relationship* relationships, StoreRelationship* storeRelationships)
{
    ValueConstraint* vc;
...
#ifdef J9_PROJECT_SPECIFIC
static TR_ResolvedMethod* findResolvedClassMethod(
    TR::Compilation* comp, char* className, char* methodName, char* methodSig)
{
    ...
    TR::Node* copyLenNode, TR::Node* n)
{
    ...
    if (elementSize == 1) {
        ...
        if (TR::Compiler->om.isOffHeapAllocationEnabled())
            len = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                comp, copyLenNode, stride, elementSize, true);
        else
        ...
        if (TR::Compiler->om.isOffHeapAllocationEnabled())
            len = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                comp, copyLenNode, stride, elementSize, false);
        else
#endif /* OMR_GC_SPARSE_HEAP_ALLOCATION */
            if (is64BitTarget) {
                if (!stride) {
...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node* node)
{
    ...
    if ((primitiveTransform || referenceTransform) && !comp()->getOption(TR_DisableArrayCopyOpts)
        ...
        if (transformTheCall && comp()->generateArraylets()) {
            if (!((primitiveArray1 || primitiveArray2)
                ...
                // printf("primitiveArray1=%d, primitiveArray2=%d, referenceArray1=%d ,
            ...
            default: // TR_no == isDstArrayCompTypePrimitiveValueType
            {
                if (isValueTypeArrayFlatteningEnabled) {
                    if (isSrcArrayCompTypePrimitiveValueType == TR_yes) {
                        ...
                        // The source might or might not be a primitive VT array. If it is primitive VT array that is
                        // flattened, the arraycopy instruction cannot handle copying from flattened array into
                        // non-flattened array since the destination array is identity type. Need to add runtime check
                        // here
                        doRuntimeNullRestrictedTest = true;
        ...
        if (trace())
            traceMsg(comp(),
                "%s: n%dn %p transformTheCall %d isSrcArrayCompTypePrimitiveValueType %d "
                "isDstArrayCompTypePrimitiveValueType %d doRuntimeNullRestrictedTest %d\n",
                __FUNCTION__, node->getGlobalIndex(), node, transformTheCall, isSrcArrayCompTypePrimitiveValueType,
                isDstArrayCompTypePrimitiveValueType, doRuntimeNullRestrictedTest);
    }
#else
    ...
    slot 7>[#433  Auto] n40n      BBEnd </block_-1>
            ...
            if (trace())
                traceMsg(comp(),
                    "%s: n%dn %p current block_%d slowBlock block_%d newCallTree n%dn %p srcObjNode n%dn %p dstObjNode "
                    "n%dn %p prevTT n%dn %p nextTT n%dn %p\n",
            ...
            _needRuntimeTestNullRestrictedArrayCopy.add(
                new (trStackMemory()) TR_NeedRuntimeTestNullRestrictedArrayCopy(dstObjNode, srcObjNode, prevTT, nextTT,
                    _curTree->getEnclosingBlock(), slowBlock, needRuntimeTestDstArray));
            if (trace()) {
        ...
        bool canSkipAllChecksOnArrayCopy = methodSymbol->safeToSkipChecksOnArrayCopies()
        ...
        if (zeroLengthCopy
            && performTransformation(
                comp(), "%sRemoving zero length call %s [%p]\n", OPT_DETAILS, node->getOpCode().getName(), node)) {
        ...
        if (!comp()->generateArraylets()) {
            ...
            src = generateArrayAddressTree(
                comp(), node, srcOffHigh, srcOffNode, srcObjNode, elementSize, stride, hdrSize);

            // -------------------------------------------------------------------
            // Calculate destination address node
            // -------------------------------------------------------------------

            dst = generateArrayAddressTree(
                comp(), node, dstOffHigh, dstOffNode, dstObjNode, elementSize, stride, hdrSize);
        } else {
            // aiadd
            //    iaload
            //        aiadd
            ...
            src = generateArrayletAddressTree(
                comp(), node, type, srcOff, srcObjNode, spineShiftNode, shiftNode, strideShiftNode, hdrSize);

            // -------------------------------------------------------------------
            // Calculate destination address node
            // -------------------------------------------------------------------

            dst = generateArrayletAddressTree(
                comp(), node, type, dstOff, dstObjNode, spineShiftNode, shiftNode, strideShiftNode, hdrSize);

            if (comp()->useCompressedPointers()) {
                TR_ASSERT(
                    (src->getFirstChild()->getOpCodeValue() == TR::aloadi), "didn't find the src arraylet pointer");
                TR_ASSERT(
                    (dst->getFirstChild()->getOpCodeValue() == TR::aloadi), "didn't find the dst arraylet pointer");
...
createExceptionEdgeConstraintsAndReturn:
    ...
    addBlockConstraint(
        copyLenNode, TR::VPIntRange::create(this, 0, static_cast<int32_t>(TR::getMaxSigned<TR::Int32>())));
}
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        if (offHigh > 0) {
            if (elementSize == 1)
                ...
                array = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                    comp, offNode, stride, elementSize, true);
            } else {
                array = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                    comp, offNode, stride, elementSize, false);
            }
        }
...
TR::TreeTop* OMR::ValuePropagation::createPrimitiveOrReferenceCompareNode(TR::Node* node)
{
    ...
    TR::Node* componentTypeLoad = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, vftLoad, comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
    TR::Node* romClassLoad = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, componentTypeLoad, comp()->getSymRefTab()->findOrCreateClassRomPtrSymbolRef());
    TR::Node* isArrayField = TR::Node::createWithSymRef(
        TR::iloadi, 1, 1, romClassLoad, comp()->getSymRefTab()->findOrCreateClassIsArraySymbolRef());
    ...
    TR::SymbolReference* dstOffRef, TR::SymbolReference* lenRef, TR::SymbolReference* tableRef, bool hasTable)
{
    ...
    bool isEncodeUtf16
        = (rm == TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big || rm == TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Little);
    ...
    */
    ...
    if (TR::Compiler->om.isOffHeapAllocationEnabled()) {
        if (encode) {
            srcOff = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                comp(), srcOff, strideNode, 0, false);
            arrayTranslateNode->setSourceIsByteArrayTranslate(false);
            arrayTranslateNode->setTargetIsByteArrayTranslate(true);
        } else {
            dstOff = TR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(
                comp(), dstOff, strideNode, 0, false);
    ...
    if (isEncodeUtf16) {
        TR::SymbolReference* transformedCallSymRef
            = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
                TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big == rm ? "transformedEncodeUTF16Big"
    ...
    TR::SymbolReference* tableRef)
{
    ...
    if (root->getOpCode().isCallIndirect()) {
        TR::Node* vftLoad = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, thisNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
    ...
    TR::SymbolReference* srcOffRef, TR::SymbolReference* dstOffRef, TR::SymbolReference* lenRef, bool primitive)
{
    ...
    srcOff = generateArrayletAddressTree(
        comp(), node, type, srcOff, srcObj, spineShiftNode, shiftNode, strideShiftNode, hdrSize);
    dstOff = generateArrayletAddressTree(
        comp(), node, type, dstOff, dstObj, spineShiftNode, shiftNode, strideShiftNode, hdrSize);
...
void createAndInsertTestBlock(
    TR::Compilation* comp, TR::TreeTop* ifTree, TR::TreeTop* tree, TR::Block* insertAfterBlock, TR::Block* dstBlock)
{
    TR::CFG* cfg = comp->getFlowGraph();
...
TR::TreeTop* OMR::ValuePropagation::createSpineCheckNode(TR::Node* node, TR::SymbolReference* objSymRef)
{
    ...
    TR::Node* sizeNode = TR::Node::createWithSymRef(
        TR::iloadi, 1, 1, objNode, comp()->getSymRefTab()->findOrCreateContiguousArraySizeSymbolRef());
...
void OMR::ValuePropagation::transformArrayCopySpineCheck(TR_ArrayCopySpineCheck* checkInfo)
{
    ...
    ifBlock->append(TR::TreeTop::create(
        comp(), TR::Node::create(arraycopyTree->getNode(), TR::Goto, 0, remainderBlock->getEntry())));
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy* rtArrayCopyTree)
{
    ...
    if (type != TR::Address || !comp()->getOptions()->realTimeGC()) {
        ...
        TR::Node* fragmentParent = TR::Node::createWithSymRef(
            vcallNode, TR::aload, 0, comp()->getSymRefTab()->findOrCreateFragmentParentSymbolRef());
        if (comp()->target().is64Bit()) {
            TR::Node* globalFragment = TR::Node::createWithSymRef(
                TR::lloadi, 1, 1, fragmentParent, comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
            ifTree = TR::TreeTop::create(comp(),
                TR::Node::createif(TR::iflcmpne, globalFragment, TR::Node::create(vcallNode, TR::lconst, 0, 0)));
        } else {
            TR::Node* globalFragment = TR::Node::createWithSymRef(
                TR::iloadi, 1, 1, fragmentParent, comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
            ifTree = TR::TreeTop::create(comp(),
    ...
    if (!comp()->getOptions()->realTimeGC() && ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
        // tarok, need wrtbar
        generateRTArrayNodeWithoutFlags(
            rtArrayCopyTree, dupArraycopyTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, 0);
        ...
        generateRTArrayNodeWithoutFlags(
            rtArrayCopyTree, dupArraycopyTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, 1);
        if (comp()->useCompressedPointers()) {
            TR::TreeTop* prevTree = dupArraycopyTree->getPrevTreeTop();
    ...
    if (type == TR::Address && comp()->getOptions()->realTimeGC() && doMultiLeafArrayCopy) {
        ...
        slowArraycopyBlock = TR::Block::createEmptyBlock(
            origDupVCallTree->getNode(), comp(), origCallBlock->getFrequency(), origCallBlock);
    ...
    TR_NeedRuntimeTestNullRestrictedArrayCopy* nullRestrictedArrayCopy)
{
 ...
 slowBlock-> n39n      BBStart <block_10> (freq 0) (cold)
    ...
    if (needTestDstArray) {
        // Create the test node to check if the destination array component type is primitive VT (null restricted value
        // type).
 ...
 */
void OMR::ValuePropagation::transformObjectCloneCall(
    TR::TreeTop* callTree, OMR::ValuePropagation::ObjCloneInfo* cloneInfo)
{
        ...
        TR::TreeTop* compareTree = TR::TreeTop::create(comp(),
            TR::Node::createif(TR::ifacmpne,
                TR::Node::createWithSymRef(
                    callNode, TR::aloadi, 1, objNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef()),
    ...
    objCopy->setAndIncChild(
        3, TR::Node::create(callNode, (comp()->target().is64Bit() ? TR::a2l : TR::a2i), 1, loadaddr));
 ...
 * https://github.com/eclipse/omr/issues/6623
 */
void OMR::ValuePropagation::transformArrayCloneCall(
    TR::TreeTop* callTree, OMR::ValuePropagation::ArrayCloneInfo* cloneInfo)
{
    ...
    if (!performTransformation(
            comp(), "%sInlining array clone call [%p] as new array and arraycopy\n", OPT_DETAILS, callNode))
        return;
    ...
    if (isPrimitiveClass) {
        ...
        if (disableSkipZeroInitInVP) {
            ...
            TR::SymbolReference* symRef = comp()->getSymRefTab()->findOrCreateNewArrayNoZeroInitSymbolRef(
                objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
            TR::Node::recreateWithoutProperties(callNode, TR::newarray, 2, lenNode, typeConst, symRef);
        ...
        if (isFixedClass) {
            ...
            TR::Node* arrayClassNode = TR::Node::createWithSymRef(
                callNode, TR::aloadi, 1, objNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
...
void OMR::ValuePropagation::transformConverterCall(TR::TreeTop* callTree)
{
    ...
    createStoresForConverterCallChildren(
        comp(), callTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, thisRef, callTree);
    if (hasTable)
        tableNode->createStoresForVar(tableRef, callTree, true);

    // create arraytranslate and cold call
    generateArrayTranslateNode(
        callTree, arrayTranslateTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, tableRef, hasTable);
    createConverterCallNodeAfterStores(
        dupCallTree, callTree, srcRef, dstRef, lenRef, srcOffRef, dstOffRef, thisRef, tableRef);
    ...
    if (comp()->target().cpu.isZ()) {
        ...
        if (threshold > 0) {
            TR::TreeTop* thresholdTestTree = TR::TreeTop::create(comp(),
                TR::Node::createif(
                    TR::ificmplt, TR::Node::createLoad(callNode, lenRef), TR::Node::create(TR::iconst, 0, threshold)));
