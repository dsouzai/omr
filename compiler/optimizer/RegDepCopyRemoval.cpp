...
int32_t TR::RegDepCopyRemoval::perform()
{
        ...
        default:
            if (node->getOpCode().isSwitch()) {
                ...
                for (int i = 0; i < nChildren - 1; i++)
                    TR_ASSERT(node->getChild(i)->getOpCodeValue() != TR::GlRegDeps,
                        "GlRegDeps for branch is not the last child\n");
                if (nChildren > 0) {
...
const char* TR::RegDepCopyRemoval::registerName(TR_GlobalRegisterNumber reg)
{
...
void TR::RegDepCopyRemoval::rangeCheckRegister(TR_GlobalRegisterNumber reg)
{
    TR_ASSERT(reg >= _regBegin && reg < _regEnd, "global register number %d out of range\n", (int)reg);
}

TR::RegDepCopyRemoval::RegDepInfo& TR::RegDepCopyRemoval::getRegDepInfo(TR_GlobalRegisterNumber reg)
{
    rangeCheckRegister(reg);
    return _regDepInfoTable.at(reg - _regBegin);
}

TR::RegDepCopyRemoval::NodeChoice& TR::RegDepCopyRemoval::getNodeChoice(TR_GlobalRegisterNumber reg)
{
    rangeCheckRegister(reg);
    return _nodeChoiceTable.at(reg - _regBegin);
}

void TR::RegDepCopyRemoval::discardAllNodeChoices()
{
    for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++)
        clearNodeChoice(reg);
}

void TR::RegDepCopyRemoval::discardNodeChoice(TR_GlobalRegisterNumber reg)
{
...
void TR::RegDepCopyRemoval::clearNodeChoice(TR_GlobalRegisterNumber reg)
{
    NodeChoice& choice = getNodeChoice(reg);
    choice.original = NULL;
    choice.selected = NULL;
...
void TR::RegDepCopyRemoval::rememberNodeChoice(TR_GlobalRegisterNumber reg, TR::Node* selected)
{
...
void TR::RegDepCopyRemoval::clearRegDepInfo()
{
    for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
...
void TR::RegDepCopyRemoval::readRegDeps()
{
    for (int i = 0; i < _regDeps->getNumChildren(); i++) {
        ...
        NodeChoice& choice = getNodeChoice(reg);
        // In a rare case, for the given register last regStore we have seen used different node then the node under
        // PassThrough Skipping such cases.
...
void TR::RegDepCopyRemoval::ignoreRegister(TR_GlobalRegisterNumber reg)
{
...
void TR::RegDepCopyRemoval::selectNodesToReuse(TR::NodeChecklist& usedNodes)
{
    for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
        RegDepInfo& dep = getRegDepInfo(reg);
...
void TR::RegDepCopyRemoval::updateRegDeps(TR::NodeChecklist& usedNodes)
{
    for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
        RegDepInfo& dep = getRegDepInfo(reg);
        switch (dep.state) {
...
void TR::RegDepCopyRemoval::reuseCopy(TR_GlobalRegisterNumber reg)
{
    RegDepInfo& dep = getRegDepInfo(reg);
    NodeChoice& prevChoice = getNodeChoice(reg);
    ...
    if (performTransformation(comp(), "%schange %s in GlRegDeps n%un to use previous copy n%un of n%un\n",
            optDetailString(), registerName(reg), _regDeps->getGlobalIndex(), prevChoice.selected->getGlobalIndex(),
...
void TR::RegDepCopyRemoval::makeFreshCopy(TR_GlobalRegisterNumber reg)
{
    RegDepInfo& dep = getRegDepInfo(reg);
    if (!performTransformation(comp(), "%schange %s in GlRegDeps n%un to an explicit copy of n%un\n", optDetailString(),
            registerName(reg), _regDeps->getGlobalIndex(), dep.value->getGlobalIndex()))
        return;
    ...
    if (curNode->getOpCodeValue() == TR::BBEnd) {
        ...
        if (curBlock->getLastRealTreeTop() != curBlock->getLastNonControlFlowTreeTop()) {
            ...
            if (trace())
                traceMsg(
                    comp(), "\tsplit fallthrough edge to insert copy, created block_%d\n", fallthrough->getNumber());
        }
    }
    ...
    if (choice.regStoreNode == NULL) {
        // As we walk down in Extended Basic Block, for each register, if exists, we record a regStore node, if we do
        // not have one found for given register, node should be regLoad. It is possible that the child node of GlRegDep
        // is PassThrough with regLoad child where both uses same register. In this case, it won't have the
        // corresponding regStore.
        TR_ASSERT_FATAL_WITH_NODE(dep.node,
            dep.node->getOpCode().isLoadReg()
                || (dep.node->getOpCodeValue() == TR::PassThrough && dep.value->getOpCode().isLoadReg()),
            "Only PassThrough (with corresponding regStore appeared before or contains regLoad child) or regLoad nodes "
            "are expected as children of GlRegDeps.");
        choice.regStoreNode
            = TR::Node::create(dep.node, comp()->il.opCodeForRegisterStore(dep.value->getDataType()), 1, copyNode);
...
void TR::RegDepCopyRemoval::updateSingleRegDep(TR_GlobalRegisterNumber reg, TR::Node* newValueNode)
{
    RegDepInfo& dep = getRegDepInfo(reg);
    TR_ASSERT(_treetop->getNode()->getOpCodeValue() != TR::BBStart,
        "attempted to change %s in incoming GlRegDeps on BBStart n%un\n", registerName(reg),
        _treetop->getNode()->getGlobalIndex());
    ...
    if (newValueNode->getOpCode().isLoadReg() && newValueNode->getGlobalRegisterNumber() == reg) {
...
void TR::RegDepCopyRemoval::generateRegcopyDebugCounter(const char* category)
{
    if (!comp()->getOptions()->enableDebugCounters())
        return;
    const char* fullName = TR::DebugCounter::debugCounterName(comp(), "regcopy/RegDepCopyRemoval/%s/(%s)/%s/block_%d",
        category, comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()),
