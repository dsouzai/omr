...
protected:
    ...
    TR::Block* insertGotoFallThroughBlock(TR::TreeTop* fallThroughTT, TR::Node* node, TR::CFGNode* prevBlock,
        TR::CFGNode* origSucc, TR_RegionStructure* parent = NULL);
};
...
public:
    ...
    void noPeepHoleOptimizations()
    {
        _doPeepHoleOptimizationsBefore = false;
        _doPeepHoleOptimizationsAfter = false;
    }
...
private:
    ...
    bool peepHoleGotoToGoto(TR::CFG* cfg, TR::Block* block, TR::Node* gotoNode, TR::Block* destOfGoto,
        const char* title, TR::BitVector& skippedGotoBlocks);
    bool peepHoleGotoToEmpty(
    ...
    bool peepHoleBranchToLoopHeader(
...
private:
    ...
    void riffle(TR::Block** blocks); // Break into two parts and interleave them randomly.  Blocks generally end up in
                                     // the same order, but with cold interleaved with hot.
