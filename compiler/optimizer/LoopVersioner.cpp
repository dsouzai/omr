    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases()) {
            ...
            if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                ...
                conditionalsWillBeEliminated = detectInvariantConditionals(naturalLoop, &conditionalTrees, true,
                    &containsNonInlineGuard, reverseBranchInLoops);
            } else {
                conditionalsWillBeEliminated = detectInvariantConditionals(naturalLoop, &conditionalTrees, false,
                    &containsNonInlineGuard, reverseBranchInLoops);
        ...
        for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL; nextVirtualGuard = guardsIt.getNext()) {
            ...
            if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall()
                ...
                if (comp()->getOptions()->dynamicDebugCounterIsEnabled(debugCounter)) {
                    TR::Block *newBlock = hotGuardBlock->splitEdge(hotGuardBlock,
                        coldGuard->getBranchDestination()->getNode()->getBlock(), comp(), NULL, false);
...
bool TR_LoopVersioner::detectInvariantNodes(List<TR_NodeParentSymRef> *invariantNodes,
...
bool TR_LoopVersioner::checkProfiledGuardSuitability(TR_ScratchList<TR::Block> *loopBlocks, TR::Node *guardNode,
    ...
    if (comp->getMethodHotness() >= hot && callSymRef) {
        if (callSymRef->getSymbol() && callSymRef->getSymbol()->castToMethodSymbol()
            ...
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(comp, "interfaceGuardCheck/(%s)", comp->signature()));
...
bool TR_LoopVersioner::isBranchSuitableToVersion(TR_ScratchList<TR::Block> *loopBlocks, TR::Node *node,
    TR::Compilation *comp)
{
...
bool TR_LoopVersioner::isBranchSuitableToDoLoopTransfer(TR_ScratchList<TR::Block> *loopBlocks, TR::Node *node,
    TR::Compilation *comp)
{
                ...
                } else if (currentOpCode.isCheckCast()) // (currentOpCode.getOpCodeValue() == TR::checkcast)
                {
                    if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
                        void *clazz = currentNode->getSecondChild()
                                          ...
                                          ->getStaticSymbol()
                                          ->getStaticAddress();
                        if (!comp()->fe()->isUnloadAssumptionRequired((TR_OpaqueClassBlock *)clazz,
...
void TR_LoopVersioner::buildNullCheckComparisonsTree(List<TR::Node> *nullCheckedReferences,
            ...
            if ((nextNode->getData()->getOpCode().hasSymbolReference()
                ...
                if (invariantNullCheckReference) {
                    ...
                    if (nullCheckReferenceisField) {
                        ...
                        dupInvariantNullCheckReference->setAndIncChild(0,
                            invariantNullCheckReference->duplicateTreeForCodeMotion());
        ...
        if (performTransformation(comp(),
            ...
            if (isVariableHeapBase || isVariableHeapSize) {
                ifNode1 = TR::Node::create(TR::acmpge, 2, duplicateBase,
                    TR::Node::createWithSymRef(TR::aload, 0,
                        comp()->getSymRefTab()->findOrCreateThreadLowTenureAddressSymbolRef()));
            } else {
                ifNode1 = TR::Node::create(TR::acmpge, 2, duplicateBase,
                    TR::Node::aconst(duplicateBase, fej9->getLowTenureAddress()));
            }
            ...
            if (isVariableHeapBase || isVariableHeapSize) {
                ifNode2 = TR::Node::create(TR::acmplt, 2, duplicateBase,
                    TR::Node::createWithSymRef(TR::aload, 0,
                        comp()->getSymRefTab()->findOrCreateThreadHighTenureAddressSymbolRef()));
            } else {
                ifNode2 = TR::Node::create(TR::acmplt, 2, duplicateBase,
                    TR::Node::aconst(duplicateBase, fej9->getHighTenureAddress()));
            }
...
void TR_LoopVersioner::buildCheckCastComparisonsTree(List<TR::TreeTop> *checkCastTrees)
{
    ...
    while (nextTree) {
        ...
        TR::Node *ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
            TR::Node::create(checkCastNode, TR::iconst, 0, 0), _exitGotoTarget);
...
void TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(List<TR::TreeTop> *arrayStoreCheckTrees)
{
    ...
    while (nextTree) {
        ...
        TR::Node *duplicateClassPtr = TR::Node::createWithSymRef(TR::aloadi, 1, 1, duplicateSrcArray,
        ...
        TR::Node *instanceofNode = TR::Node::createWithSymRef(
            ...
            comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
        TR::Node *ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
            TR::Node::create(arrayStoreCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
...
void TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop()
{
    ...
    dumpOptDetails(comp(), "Removing array store check n%un [%p]\n", arrayStoreCheckNode->getGlobalIndex(),
        arrayStoreCheckNode);
    ...
    TR::TreeTop *firstNewTree = TR::TreeTop::create(comp(),
        TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getFirstChild()), NULL, NULL);
    ...
    if (arrayStoreCheckNode->getNumChildren() > 1) {
        ...
        secondNewTree = TR::TreeTop::create(comp(),
            TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getSecondChild()), NULL, NULL);
...
bool TR_LoopVersioner::buildLoopInvariantTree(List<TR_NodeParentSymRef> *invariantNodes)
{
    ...
    while (nextInvariantNode) {
        ...
        if (performTransformation(comp(), "%s Attempting to hoist n%un [%p] out of the loop\n",
            ...
            if (prep == NULL) {
                dumpOptDetails(comp(), "failed to privatize n%un [%p]\n", invariantNode->getGlobalIndex(),
                    invariantNode);
            } else {
    ...
    while (nextSpecializedNode) {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is value profiled\n",
            ...
            if (specializedNode->getType().isInt64()) {
                ...
                comparisonNode = TR::Node::createif(TR::iflcmpne, highInt,
                    TR::Node::create(duplicateNode, TR::lconst, 0, 0), _exitGotoTarget);
            } else {
...
void TR_LoopVersioner::buildConditionalTree(List<TR::TreeTop> *conditionalTrees,
    SharedSparseBitVector &reverseBranchInLoops)
{
                ...
                if (thisChild->getOpCode().hasSymbolReference()
                    ...
                    if (dupThisChild) {
                        ...
                        if (guardInfo->getTestType() == TR_VftTest) {
                            ...
                            conditionalNode->getFirstChild()->getFirstChild()->setAndIncChild(0,
                                dupThisChild->duplicateTree());
                        } else {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is conditional\n",
            ...
            if (isVersionableIfWithExtremum(conditionalTree, &excond)) {
                ...
                if (excond.extremumIsFinalValue || conditionalNode->getOpCode().isUnsigned()) {
                    ...
                    TR_ASSERT_FATAL_WITH_NODE(conditionalNode, indexChildIndex >= 0,
                    ...
                    if (duplicateIndexNode->getOpCode().isLoad()
                        ...
                        copyOnWriteNode(origConditionalNode, &conditionalNode);
                        replaceInductionVariable(conditionalNode, duplicateIndexNode, indexChildIndex, indexSymRefNum,
                            finalIv, visitCount);
                    }
                }
...
bool TR_LoopVersioner::replaceInductionVariable(TR::Node *parent, TR::Node *node, int childNum,
    int loopDrivingInductionVar, TR::Node *loopLimit, int visitCount)
{
    if (node->getVisitCount() == visitCount)
        return false;
...
void TR_LoopVersioner::RemoveSpineCheck::improveLoop()
{
    ...
    if (is64BitTarget) {
        ...
        hdrSize = TR::Node::create(spineCheckNode, TR::iconst, 0,
            (int32_t)TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());
            ...
            if (arrayLengthNode->getOpCode().isArrayLength()) {
                ...
                if (arrayObject->getOpCode().hasSymbolReference()
                    ...
                    if (invariantArrayObject) {
                        ...
                        arrayLengthNode = TR::Node::create(arrayLengthNode, arrayLengthNode->getOpCodeValue(), 1,
        ...
        if (isIndexInvariant) {
            ...
            if (isSpecialInductionVariable) {
                ...
                if (changedIndexSymRefAtSomePoint
                    ...
                    if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                        ...
                        TR::Node *replacementLoad = TR::Node::createWithSymRef(storeNode,
                            comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0, indexSymRef);

                        if (!storeRhs->getOpCode().isLoad()) {
                            int visitCount = comp()->incVisitCount();
                            replaceInductionVariable(NULL, storeRhs, -1, changedIndexSymRefNumForStoreRhs,
                                replacementLoad, visitCount);
                        } else
                        ...
                        if (!duplicateIndex->getOpCode().isLoad()) {
                            vcount_t visitCount = comp()->incVisitCount();
                            replaceInductionVariable(NULL, duplicateIndex, -1, origIndexSymRef->getReferenceNumber(),
                                storeRhs, visitCount);
                        } else
            ...
            if (changedIndexSymRefAtSomePoint
                ...
                if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                    ...
                    if (!storeRhs->getOpCode().isLoad()) {
                        int visitCount = comp()->incVisitCount();
                        replaceInductionVariable(NULL, storeRhs, -1, changedIndexSymRefNumForStoreRhs, replacementLoad,
                            visitCount);
                    } else
                        storeRhs = replacementLoad;
                    if (!duplicateIndex->getOpCode().isLoad()) {
            ...
            if (isSpecialInductionVariable) {
                ...
                if (isAddition) {
                    if (!indVarOccursAsSecondChildOfSub) {
                        nextComparisonNode = TR::Node::createif(TR::ifiucmpge, correctCheckNode,
                            arrayLengthNode->duplicateTree(), _exitGotoTarget);
                        nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
                    } else {
            ...
            if (!isSpecialInductionVariable) {
                ...
                if (changedIndexSymRefAtSomePoint
                    ...
                    if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                        ...
                        if (!storeRhs->getOpCode().isLoad()) {
                            int visitCount = comp()->incVisitCount();
                            replaceInductionVariable(NULL, storeRhs, -1, origIndexSymRef->getReferenceNumber(),
                                replacementLoad, visitCount);
                        } else
                            storeRhs = replacementLoad;
                        ...
                        if (!firstChild->getOpCode().isLoad()) {
                            vcount_t visitCount = comp()->incVisitCount();
                            replaceInductionVariable(NULL, firstChild, -1, origIndexSymRef->getReferenceNumber(),
                                storeRhs, visitCount);
                        } else
                            firstChild = storeRhs;
                    }
 ...
 */
bool TR_LoopVersioner::depsForLoopEntryPrep(TR::Node *node, TR::list<LoopEntryPrep *, TR::Region &> *deps,
    TR::NodeChecklist *visited, bool canPrivatizeRootNode)
{
    ...
    if (node->isInternalPointer()
        ...
        if (instanceOfReqd) {
            if (otherClassObject) {
                ...
                TR::Node *instanceofNode = TR::Node::createWithSymRef(
                    ...
                    comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
                TR::Node *ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
                    TR::Node::create(node, TR::iconst, 0, 0), _exitGotoTarget);
                ...
                TR::Node *vftLoad = TR::Node::createWithSymRef(TR::aloadi, 1, 1, firstChild,
                    comp()->getSymRefTab()->findOrCreateVftSymbolRef());
        ...
        TR::Node *ificmpgeNode = TR::Node::createif(type.isInt32() ? TR::ificmpge : TR::iflcmpge, indexNode,
            arrayLengthNode, _exitGotoTarget);
        ...
        if (!divisorIsNonzeroConstant) {
            ...
            if (divisor->getType().isInt64())
                ifNode = TR::Node::createif(TR::iflcmpeq, divisor, TR::Node::create(node, TR::lconst, 0, 0),
                    _exitGotoTarget);
            else
                ifNode = TR::Node::createif(TR::ificmpeq, divisor, TR::Node::create(node, TR::iconst, 0, 0),
                    _exitGotoTarget);
 ...
 */
TR_LoopVersioner::LoopEntryPrep *TR_LoopVersioner::addLoopEntryPrepDep(LoopEntryPrep::Kind kind, TR::Node *node,
    TR::list<LoopEntryPrep *, TR::Region &> *deps, TR::NodeChecklist *visited)
{
...
int32_t TR_LoopVersioner::detectCanonicalizedPredictableLoops(TR_Structure *loopStructure, TR_BitVector **optSetInfo,
...
bool TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum, TR_Structure *loopStructure)
{
    ...
    if (!secondChild->getOpCode().isLoadConst()
        ...
        if (secondChild->getOpCode().isLoadVarDirect() && secondChild->getSymbol()->isAutoOrParm()) {
            ...
            if (!isSymbolReferenceWrittenNumberOfTimesInStructure(loopStructure,
                    secondChild->getSymbolReference()->getReferenceNumber(), &timesWritten, 0))
                return false;
...
bool TR_LoopVersioner::guardOkForExpr(TR::Node *node, bool onlySearching)
{
    ...
    switch (kind) {
        ...
        case TR_HCRGuard:
            TR_ASSERT_FATAL(onlySearching, "guardOkForExpr: should not intern HCR guard n%un [%p]",
                node->getGlobalIndex(), node);
            return false;

        case TR_OSRGuard:
            TR_ASSERT_FATAL(onlySearching, "guardOkForExpr: should not intern OSR guard n%un [%p]",
                node->getGlobalIndex(), node);
            return false;

        // Never versioned, only holds inner assumptions.
        case TR_DummyGuard:
            TR_ASSERT_FATAL(onlySearching, "guardOkForExpr: should not intern dummy guard n%un [%p]",
                node->getGlobalIndex(), node);
            return false;
        ...
        default:
            ...
            //
            TR_ASSERT_FATAL(false, "guardOkForExpr: n%un [%p]: unrecognized nop-able guard kind %d",
                node->getGlobalIndex(), node, (int)kind);
    }
 ...
 * or null if no such Expr has been created
 */
const TR_LoopVersioner::Expr *TR_LoopVersioner::substitutePrivTemps(TR::TreeTop *tt, TR::Node *node,
...
void TR_LoopVersioner::emitPrep(LoopEntryPrep *prep, List<TR::Node> *comparisonTrees)
{
    ...
    if (prep->_kind == LoopEntryPrep::TEST) {
        ...
        TR_ASSERT_FATAL(prep->_kind == LoopEntryPrep::PRIVATIZE, "prep %p has unrecognized kind %d\n", prep,
            prep->_kind);
 ...
 */
void TR_LoopVersioner::unsafelyEmitAllTests(const TR::list<LoopEntryPrep *, TR::Region &> &preps,
    ...
    TR::NodeChecklist *visited, LoopEntryPrep *prev)
{
    ...
    if (existing != _curLoop->_prepTable.end()) {
        ...
        dumpOptDetails(comp(), "Using existing prep %p for n%un [%p]\n", existing->second, node->getGlobalIndex(),
            node);
