...
TR_LoopVersioner::TR_LoopVersioner(TR::OptimizationManager* manager, bool onlySpecialize, bool refineAliases)
    ...
    , _refineLoopAliases(refineAliases)
    , _addressingTooComplicated(false)
    ...
    if (aggressive || comp()->getMethodHotness() <= warm) {
        if (naturalLoop->getParent()) {
            TR_StructureSubGraphNode* loopNode
                = naturalLoop->getParent()->findNodeInHierarchy(naturalLoop->getNumber());
            if (loopNode && (loopNode->getPredecessors().size() == 1)) {
                TR_StructureSubGraphNode* loopInvariantNode
                    = toStructureSubGraphNode(loopNode->getPredecessors().front()->getFrom());
                if (loopInvariantNode->getStructure()->asBlock()
                    ...
                    if ((unimportantLoopCountThreshold * loopInvariantBlock->getFrequency())
...
int32_t TR_LoopVersioner::performWithoutDominators()
{
    ...
    _counter = 0;
    _inNullCheckReference = 0, _containsCall = 0;
    _neitherLoopCold = 0;
    ...
    if (!comp()->getOption(TR_ProcessHugeMethods) && (whileLoops.getSize() > HIGH_LOOP_COUNT / 2)) {
        ...
        while (needToPruneMore) {
            ...
            for (nextWhileLoop = whileLoopsIt2.getFirst(); nextWhileLoop != NULL;
                 nextWhileLoop = whileLoopsIt2.getNext()) {
    ...
    _writtenAndNotJustForHeapification
        = new (trStackMemory()) TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        _unchangedValueUsedInBndCheck
            = new (trStackMemory()) TR_BitVector(comp()->getNodeCount(), trMemory(), stackAlloc);
        ...
        if (detectCanonicalizedPredictableLoops(naturalLoop, NULL, -1)
            < 0) // if there was a problem detecting, move to next iteration of the loop
        {
            _curLoop = NULL;
            continue;
        }
        ...
        if (nullChecksMayBeEliminated && !refineAliases() && !shouldOnlySpecializeLoops())
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases()) {
            ...
            if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                ...
                || boundChecksWillBeEliminated || divChecksWillBeEliminated || awrtBarisWillBeEliminated
                || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated
                || specializedNodesWillBeEliminated || invariantNodesWillBeEliminated
                || _nonInlineGuardConditionalsWillNotBeEliminated || _loopTransferDone) {
                conditionalsWillBeEliminated = detectInvariantConditionals(
                    naturalLoop, &conditionalTrees, true, &containsNonInlineGuard, reverseBranchInLoops);
            } else {
                conditionalsWillBeEliminated = detectInvariantConditionals(
                    naturalLoop, &conditionalTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
        ...
        if (versionedThisLoop) {
            ...
            for (v = naturalLoop->getFirstInductionVariable(); v; v = v->getNext()) {
                ...
                if (inductionSymRef && !inductionVars->get(inductionSymRef->getReferenceNumber())) {
                    ...
                    TR::RegisterCandidate* inductionCandidate
                        = comp()->getGlobalRegisterCandidates()->findOrCreate(inductionSymRef);
                    // dumpOptDetails(comp(), "%s Adding auto %d as a global register candidate in loop %d\n",
                    // OPT_DETAILS_LOOP_VERSIONER, inductionCandidate->getSymbolReference()->getReferenceNumber(),
                    // naturalLoop->getNumber());
                    inductionCandidate->addAllBlocksInStructure(naturalLoop, comp(), trace() ? "auto" : NULL);
    ...
    if (!shouldOnlySpecializeLoops() && enableSpecialization) {
...
void TR_LoopVersioner::performLoopTransfer()
{
    ...
    for (vgInfo = _virtualGuardInfo.getFirst(); vgInfo; vgInfo = vgInfo->getNext()) {
        ...
        for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL; nextVirtualGuard = guardsIt.getNext()) {
            ...
            if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall()
                && coldGuard->getOpCode().isIf() && coldGuard->isTheVirtualGuardForAGuardedInlinedCall()) {
                ...
                if (comp()->getOptions()->dynamicDebugCounterIsEnabled(debugCounter)) {
                    TR::Block* newBlock = hotGuardBlock->splitEdge(
                        hotGuardBlock, coldGuard->getBranchDestination()->getNode()->getBlock(), comp(), NULL, false);
...
bool TR_LoopVersioner::detectInvariantChecks(List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees)
{
    ...
    for (node = nullCheckedReferences->getListHead(); node != NULL;) {
        ...
        if (!isNullCheckReferenceInvariant && node->getData()->getOpCode().hasSymbolReference()
...
bool TR_LoopVersioner::detectInvariantSpecializedExprs(List<TR::Node>* profiledExprs)
{
    ...
    for (node = profiledExprs->getListHead(); node != NULL;) {
        ...
        if (!isProfiledExprInvariant && node->getData()->getOpCode().hasSymbolReference()
...
bool TR_LoopVersioner::detectInvariantNodes(
    List<TR_NodeParentSymRef>* invariantNodes, List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodes)
{
...
bool TR_LoopVersioner::detectInvariantCheckCasts(List<TR::TreeTop>* trees)
{
    ...
    while (curTreesElem != NULL) {
        ...
        TR_ASSERT_FATAL_WITH_NODE(node, node->getOpCode().isCheckCast(), "expected a checkcast");
 ...
 */
bool TR_LoopVersioner::isVersionableIfWithExtremum(TR::TreeTop* ifTree, ExtremumConditional* out)
{
    ...
    *out = excond;
    ...
    TR_ASSERT_FATAL_WITH_NODE(ifNode, !withMin || !withMax, "both min and max flags are set");

    static const bool enable = feGetEnv("TR_dontVersionIfWithExtremum") == NULL;
    if (!enable)
        return false;

    TR_ASSERT_FATAL_WITH_NODE(ifNode, ifNode->getNumChildren() == 2, "unexpected number of children");
    ...
    while (inner->getOpCode().isAdd() || inner->getOpCode().isSub() || inner->getOpCode().isMul()
        ...
        if (inner->getOpCode().isNeg() || (inner->getOpCode().isSub() && nextChildIndex == 1)) {
    ...
    if (oldIvLoad->getOpCodeValue() != TR::iload || oldIvLoad->getSymbolReference() != ivUpdate->getSymbolReference())
        return false;
    ...
    excond.extremumIsFinalValue = ifNode->isVersionableIfWithMaxExpr() == exprIncreasingWithIters;
    ...
    if (excond.extremumIsFinalValue || ifNode->getOpCode().isUnsigned()) {
        ...
        } else if (lhs->getOpCodeValue() == TR::iload && lhs->getSymbolReference() == excond.iv) {
        ...
        if (target != loopHeader) {
            TR_ASSERT_FATAL_WITH_NODE(loopTest, !_curLoop->_loop->contains(target->getStructureOf()),
    ...
    bool takenCold = ifNode->getBranchDestination()->getNode()->getBlock()->isCold();
    bool fallthroughCold = ifTree->getEnclosingBlock()->getNextBlock()->isCold();
    ...
    for (; nextTree;) {
        ...
        if (!node->isTheVirtualGuardForAGuardedInlinedCall()) {
            ...
            if (nextBlock->getEdge(targetBlock)->getFrequency() <= MAX_COLD_BLOCK_COUNT + 1
                ...
                && ((targetBlock->isCold() && !nextBlock->getNextBlock()->isCold())
                    || (!targetBlock->isCold() && nextBlock->getNextBlock()->isCold()))) {
                highlyBiasedBranch = true;
            }
#endif
        ...
        if (onlyDetectHighlyBiasedBranches) {
            ...
            TR::Block* nextBlock = nextTree->getData()->getEnclosingBlock();
            TR::Block* targetBlock = node->getBranchDestination()->getNode()->getBlock();
            if ((targetBlock->isCold() && nextBlock->getNextBlock()->isCold())
                || (!targetBlock->isCold() && !nextBlock->getNextBlock()->isCold()))
                includeComparisonTree = false;
        }
#endif
        ...
        if (includeComparisonTree) {
            ...
            if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode)
                && node->isTheVirtualGuardForAGuardedInlinedCall() && !node->isOSRGuard() && !node->isHCRGuard()
                && !node->isDirectMethodGuard() && !node->isBreakpointGuard()) {
                if (!guardInfo)
                    guardInfo = comp()->findVirtualGuardInfo(node);
                ...
                if (guardInfo->getKind() == TR_DummyGuard
                    || !guardInfo->getInnerAssumptions()
                            .isEmpty()) // inner assumption may have receivers other than 'thisChild'
            ...
            } else if (!node->isOSRGuard() && !node->isHCRGuard() && !node->isDirectMethodGuard()
        ...
        if (!isTreeInvariant) {
            ...
            if (onlyDetectHighlyBiasedBranches || (!node->getOpCode().isBranch()) || (!onlyNonInlineGuardConditional)) {
                ...
                foundInvariantTrees = true;
                prevTree = nextTree;
                if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
                ...
                if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
                    ...
                    if (prevTree) {
                        ...
                        if (!_postDominators
                            || _postDominators->numberOfBlocksControlled(b1)
                                > _postDominators->numberOfBlocksControlled(b2)) {
                            prevTree->setNextElement(nextTree->getNextElement());
...
TR::Node* TR_LoopVersioner::isDependentOnInductionVariable(TR::Node* useNode, bool noArithmeticAllowed,
    bool& isIndexChildMultiplied, TR::Node*& mulNode, TR::Node*& strideNode, bool& indVarOccursAsSecondChildOfSub,
    TR::TreeTop*& outDefTree)
{
    ...
    if (useDefInfo->getUseDef(defs, useIndex) && (defs.PopulationCount() == 1)
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(useNode, defNode == defTree->getNode(),
                "n%un [%p] def discrepancy: use-def n%un [%p] vs. _storeTrees n%un [%p]", useNode->getGlobalIndex(),
                useNode, defNode->getGlobalIndex(), defNode, defTree->getNode()->getGlobalIndex(), defTree->getNode());

            TR::Node* child = defNode->getFirstChild();
            bool goodAccess = isVersionableArrayAccess(child);
            while (!noArithmeticAllowed
                && (child->getOpCode().isAdd() || child->getOpCode().isSub() || child->getOpCode().isMul())) {
            ...
            if (child && child->getOpCode().hasSymbolReference() && goodAccess) {
                outDefTree = defTree;
                return child;
            }
        }
...
bool TR_LoopVersioner::isVersionableArrayAccess(TR::Node* indexChild)
{
    ...
    if (!indexChild->getOpCode().hasSymbolReference()) {
        while (indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub() || indexChild->getOpCode().isMul()) {
...
bool TR_LoopVersioner::detectInvariantBoundChecks(List<TR::TreeTop>* boundCheckTrees)
{
    ...
    for (; nextTree;) {
        ...
        if (!isMaxBoundInvariant) {
            if (boundNode->getOpCode().isArrayLength()) {
                ...
                if (boundNode->getOpCode().hasSymbolReference()
                    && boundNode->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(boundNode))
                    isMaxBoundInvariant = true;
            }
        }
        ...
        if (isMaxBoundInvariant) {
            ...
            if (!isIndexInvariant && _loopConditionInvariant
                ...
                if (indexChild->getOpCode().hasSymbolReference())
                    ...
                    while (indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub()
                        || indexChild->getOpCode().isAnd() || indexChild->getOpCode().isMul()) {
                    ...
                    if (indexChild && indexChild->getOpCode().hasSymbolReference() && goodAccess)
                        indexSymRef = indexChild->getSymbolReference();
                }
                ...
                while (indexSymRef && changedIndexSymRef) {
                    ...
                    if (!isInductionVariable) {
                        ...
                        indexChild = isDependentOnInductionVariable(indexChild, changedIndexSymRefAtSomePoint,
                            isIndexChildMultiplied, mulNode, strideNode, indVarOccursAsSecondChildOfSub, defTree);
                        if (!indexChild || !indexChild->getOpCode().hasSymbolReference()
                            || !indexChild->getSymbolReference()->getSymbol()->isAutoOrParm()
                            || (indexChild->getSymbolReference()->getReferenceNumber()
                                == indexSymRef->getReferenceNumber())) {
                            break;
                        } else {
                            indexSymRef = indexChild->getSymbolReference();
        ...
        if (isIndexInvariant && _checksInDupHeader.find(checkTree))
            isIndexInvariant = false;
...
bool TR_LoopVersioner::isDependentOnAllocation(TR::Node* useNode, int32_t recursionDepth)
{
    ...
    if (useDefInfo->getUseDef(defs, useIndex)) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (heapificationStore || (child->getOpCode().isNew())
                || ((child->getOpCodeValue() == TR::loadaddr) && child->getSymbol()->isLocalObject()))
                pointsToNew = true;
            else {
                if (recursionDepth > 0) {
...
bool opCodeIsHoistable(TR::Node* node, TR::Compilation* comp)
{
    ...
    hasEffects = (opCode.isCall() || opCodeValue == TR::New || opCodeValue == TR::newarray
                     || opCodeValue == TR::anewarray || opCodeValue == TR::multianewarray)
        || symReference->isUnresolved() || symReference->getSymbol()->isLocalObject()
        || (symReference->getSymbol()->isArrayShadowSymbol() && comp->requiresSpineChecks());
    return !hasEffects;
}
...
bool TR_LoopVersioner::isExprInvariantRecursive(TR::Node* node, bool ignoreHeapificationStore)
{
    ...
    // Do not attempt to privatize BCD type nodes because doing so can result in creating direct loads of BCD types
    // which are currently not handled correctly within Java. BCDCHK IL will attempt to recreate a call to the original
    // Java API for the corresponding BCD IL and it needs to be able to materialize the node representing the original
    // byte array object. This is not possible if the byte array is stored on the stack.
#ifdef J9_PROJECT_SPECIFIC
...
bool TR_LoopVersioner::canPredictIters(TR_RegionStructure* whileLoop,
    const TR_ScratchList<TR::Block>& blocksInWhileLoop, bool& isIncreasing, TR::SymbolReference*& firstChildSymRef)
{
    ...
    if (returnValue) {
        ...
        if (firstChild->getOpCode().hasSymbolReference())
            ...
            if (firstChild && firstChild->getOpCode().hasSymbolReference())
                firstChildSymRef = firstChild->getSymbolReference();
        }
...
int32_t* computeCallsiteCounts(TR_ScratchList<TR::Block>* loopBlocks, TR::Compilation* comp)
{
    ...
    for (TR::Block* block = loopItr.getCurrent(); block; block = loopItr.getNext()) {
        for (OMR::TreeTop* tt = block->getFirstRealTreeTop(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
            ...
            if (!countAllocationFenceNodes && (tt->getNode()->getOpCodeValue() == TR::allocationFence)) {
                continue;
            }
...
bool TR_LoopVersioner::checkProfiledGuardSuitability(
    TR_ScratchList<TR::Block>* loopBlocks, TR::Node* guardNode, TR::SymbolReference* callSymRef, TR::Compilation* comp)
{
    ...
    if (comp->getMethodHotness() >= hot && callSymRef) {
        if (callSymRef->getSymbol() && callSymRef->getSymbol()->castToMethodSymbol()
            && callSymRef->getSymbol()->castToMethodSymbol()->isInterface()
#ifdef J9_PROJECT_SPECIFIC
            && comp->fej9()->maybeHighlyPolymorphic(comp, callSymRef->getOwningMethod(comp), callSymRef->getCPIndex(),
                callSymRef->getSymbol()->castToMethodSymbol()->getMethod())
#endif
        ) {
            if (trace()) {
            ...
            TR::DebugCounter::incStaticDebugCounter(
                comp, TR::DebugCounter::debugCounterName(comp, "interfaceGuardCheck/(%s)", comp->signature()));
            ...
            if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
                ...
                TR::DebugCounter::incStaticDebugCounter(comp,
                    TR::DebugCounter::debugCounterName(comp,
                        "profiledVersioning/unsuitableForVersioning/interfaceGuard/(%s)/bci=%d.%d", comp->signature(),
                        guardNode->getByteCodeInfo().getCallerIndex(),
                        guardNode->getByteCodeInfo().getByteCodeIndex()));
            }
...
bool TR_LoopVersioner::isBranchSuitableToVersion(
    TR_ScratchList<TR::Block>* loopBlocks, TR::Node* node, TR::Compilation* comp)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (node->isProfiledGuard() && !node->getBranchDestination()->getNode()->getBlock()->isCold()) {
        bool isGetFieldCacheCounts = !strncmp(comp->signature(),
            "org/apache/solr/request/SimpleFacets.getFieldCacheCounts(Lorg/apache/solr/search/SolrIndexSearcher;Lorg/"
            "apache/solr/search/DocSet;Ljava/lang/String;IIIZLjava/lang/String;Ljava/lang/String;)Lorg/apache/solr/"
        ...
        if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
            ...
            TR_AddressInfo* valueInfo = static_cast<TR_AddressInfo*>(
                TR_ValueProfileInfoManager::getProfiledValueInfo(ics._byteCodeInfo, comp, AddressInfo));

            if (valueInfo) {
                if (trace())
                    traceMsg(
                        comp, "Profiled guard probability %.2f for guard %p\n", valueInfo->getTopProbability(), node);
                if (valueInfo->getTopProbability() >= profiledGuardProbabilityThreshold) {
                    suitableForVersioning = checkProfiledGuardSuitability(
                        loopBlocks, node, comp->getInlinedCallerSymRef(node->getByteCodeInfo().getCallerIndex()), comp);
                } else {
                    suitableForVersioning = false;
                }
                if (suitableForVersioning) {
                    TR::DebugCounter::incStaticDebugCounter(comp,
                        TR::DebugCounter::debugCounterName(comp,
                            "profiledVersioning/suitableForVersioning/probability=%d",
                            ((int32_t)(valueInfo->getTopProbability() * 100))));
                } else {
                    TR::DebugCounter::incStaticDebugCounter(comp,
                        TR::DebugCounter::debugCounterName(comp,
                            "profiledVersioning/unsuitableForVersioning/probability=%d",
                            ((int32_t)(valueInfo->getTopProbability() * 100))));
                }
            } else {
                TR::DebugCounter::incStaticDebugCounter(comp,
                    TR::DebugCounter::debugCounterName(comp, "profiledVersioning/unsuitableForVersioning/noinfo"));
                suitableForVersioning = false;
            }
        } else {
...
bool TR_LoopVersioner::isBranchSuitableToDoLoopTransfer(
    TR_ScratchList<TR::Block>* loopBlocks, TR::Node* node, TR::Compilation* comp)
{
...
bool TR_LoopVersioner::detectChecksToBeEliminated(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences,
    ...
    bool& discontinue)
{
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        if (nextBlockStructure && nextBlockStructure->wasHeaderOfCanonicalizedLoop()
        ...
        if (nextBlock->isCold()) {
            ...
            static const bool useOuterHeader = feGetEnv("TR_loopVersionerUseOuterHeaderForImportance") != NULL;
            ...
            if ((!aggressive || comp()->getMethodHotness() <= warm) && blockFrequency >= 0 // frequency must be valid
                ...
                if (trace() || comp()->getOption(TR_CountOptTransformations)) {
                    for (TR::TreeTop* tt = entryTree; isUnimportant && tt != exitTree; tt = tt->getNextTreeTop()) {
                        ...
                        if (op.isCheck() || op.isCheckCast()
                            || (op.isBranch() && ttNode->getNumChildren() >= 2
                                && (!tt->getNode()->getFirstChild()->getOpCode().isLoadConst()
                                    || !tt->getNode()->getSecondChild()->getOpCode().isLoadConst()))) {
                            if (!performTransformation(comp(), "%s ...disregarding %s n%dn\n",
                                    OPT_DETAILS_LOOP_VERSIONER, ttNode->getOpCode().getName(),
        ...
        while (!(currentTree == exitTree)) {
            ...
            if (currentOpCode.getOpCodeValue() == TR::asynccheck) {
                ...
                if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1) && _asyncCheckTree
                    ...
                    bool _canPredictIters
                        = canPredictIters(whileLoop, blocksInWhileLoop, isIncreasing, firstChildSymRef);
            ...
            updateDefinitionsAndCollectProfiledExprs(NULL, currentNode, visitCount, specializedInvariantNodes,
                invariantNodes, invariantTranslationNodesList, NULL, collectProfiledExprs, nextBlock, warmBranchCount);

            bool disableLT = comp()->getOption(TR_DisableLoopTransfer);
            if (!disableLT && currentNode->isTheVirtualGuardForAGuardedInlinedCall()
            ...
            if ((currentNode->getNumChildren() > 0) && currentNode->getFirstChild()->getOpCode().isCall()) {
                if (!currentNode->getFirstChild()->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                    ...
                    if (!guard) {
                        // printf("\t\tnode %p (nextblock = %p) is hte virtual call node for a guarded inlined call
            ...
            if (!isUnimportant && !_containsCall && !refineAliases()) {
                if (currentOpCode.isNullCheck()) {
                    ...
                    if (isConditionalTreeCandidateForElimination(currentTree)) {
                        ...
                        if (_postDominators) {
                            postDominatesEntry
                                = (_postDominators->dominates(nextBlock, whileLoop->getEntryBlock()) != 0);
                            if (postDominatesEntry) {
                                dumpOptDetails(
                                    comp(), "   block_%d post dominates loop entry\n", nextBlock->getNumber());
                        ...
                        if (trace()) {
                            ...
                            if (_postDominators) {
                                ...
                                traceMsg(
                                    comp(), "    post dominates loop entry: %s\n", postDominatesEntry ? "yes" : "no");
                            }
                        }
                ...
                } else if (currentOpCode.isCheckCast()) // (currentOpCode.getOpCodeValue() == TR::checkcast)
                {
                    if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
                        void* clazz = currentNode->getSecondChild()
                                          ...
                                          ->getStaticSymbol()
                                          ->getStaticAddress();
                        if (!comp()->fe()->isUnloadAssumptionRequired(
                                (TR_OpaqueClassBlock*)clazz, comp()->getCurrentMethod())) {
...
void TR_LoopVersioner::updateDefinitionsAndCollectProfiledExprs(TR::Node* parent, TR::Node* node, vcount_t visitCount,
    List<TR::Node>* profiledNodes, List<TR_NodeParentSymRef>* invariantNodes,
    List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodesList, TR::Node* lastFault, bool collectProfiledExprs,
    TR::Block* block, int32_t warmBranchCount)
{
    ...
    if (defNode->isTheVirtualCallNodeForAGuardedInlinedCall() && block->findVirtualGuardBlock(comp()->getFlowGraph()))
    ...
    if (defNode) {
        ...
        if (opCode.isStore()) {
            ...
            if (!heapificationStore && _writtenAndNotJustForHeapification)
                defAliases.getAliasesAndUnionWith(
                    *_writtenAndNotJustForHeapification); // TODO: avoid getting the same aliases twice
        }
    }
    ...
    if (_inNullCheckReference || !_nullCheckReference) {
        if ((opCode.isCall()) || (opCodeValue == TR::New) || (opCodeValue == TR::newarray)
            || (opCodeValue == TR::anewarray) || (opCodeValue == TR::multianewarray))
            _containsCall = true;
        ...
        if (_inNullCheckReference && comp()->requiresSpineChecks() && node->getOpCode().hasSymbolReference()
    ...
    if (TR_LocalAnalysis::isSupportedOpCode(node->getOpCode(), comp())) {
        if (node->getType().isInt32()
            || (node->getType().isInt64() && node->getOpCode().isLoadVar()
            ...
            if (collectProfiledExprs && !node->getType().isInt64()) {
                ...
                TR_ValueInfo* valueInfo = static_cast<TR_ValueInfo*>(
                    TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(), ValueInfo));
                if (valueInfo) {
                    if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY)
                        && (valueInfo->getTotalFrequency() > 0) && !_containsCall
                        && !node->getByteCodeInfo().doNotProfile() && node->canChkNodeCreatedByPRE()
                        && !node->isNodeCreatedByPRE() &&
                        ...
                        if (!node->getType().isInt64()) {
                            ...
                            //
                            if (comp()->requiresSpineChecks() && node->getOpCodeValue() == TR::contigarraylength
                                && valueInfo->getTopValue() == 0) {
                                dumpOptDetails(comp(),
                                    "From value profiling, NOT USING inconclusive contigarraylength node %p with value "
                                    "0 freq %d total freq %d\n",
                                    node,
                                    ((int32_t)(valueInfo->getTopProbability()
                                        * ((float)valueInfo->getTotalFrequency()))),
                                    valueInfo->getTotalFrequency());
                            } else {
                                ...
                                /////if (debug("traceLVE"))
                                dumpOptDetails(comp(),
                                    "From value profiling, node %p has value %d freq %d total freq %d\n", node,
                                    valueInfo->getTopValue(),
                                    ((int32_t)(valueInfo->getTopProbability()
                                        * ((float)valueInfo->getTotalFrequency()))),
                                    valueInfo->getTotalFrequency());
        ...
        if (invariantNodes && collectProfiledExprs && !_containsCall
            && ((node->getOpCode().getSize() >= 4) || (opCodeValue == TR::computeCC))
    ...
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++)
        updateDefinitionsAndCollectProfiledExprs(node, node->getChild(i), visitCount, profiledNodes, invariantNodes,
            invariantTranslationNodesList, lastFault, collectProfiledExprs, block, warmBranchCount);

    if (node == _nullCheckReference)
        _inNullCheckReference = false;
}

static void traceCannot(const char* what, TR::Node* culprit, TR::Compilation* comp)
{
    ...
    SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        TR::Block* nextClonedBlock
            = TR::Block::createEmptyBlock(entryNode, comp(), nextBlock->getFrequency(), nextBlock);
        ...
        // the loop).
        //
        if (!_neitherLoopCold || nextBlock->isCold()) {
            ...
            //  Set the flag on blocks in the slow version of the loop, so that in the analysis phase, we don't collect
            //  these blocks
        ...
        bool disableLT = comp()->getOption(TR_DisableLoopTransfer);
        if (!disableLT && lastTree->getOpCode().isIf()
            ...
            dumpOptDetails(
                comp(), "hotGuardBlock %d coldGuardBlock %d\n", nextBlock->getNumber(), nextClonedBlock->getNumber());
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
            ...
            if (clonedSucc) {
                ...
                nextClonedBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                    comp(), succ->getEntry(), clonedSucc->getEntry());
            } else {
                ...
                if (!fallsThrough)
                    ...
                    if (trace())
                        traceMsg(
                            comp(), "Creating new goto block : %d for node %p\n", newGotoBlock->getNumber(), lastNode);
    ...
    TR::Block* clonedLoopInvariantBlock = TR::Block::createEmptyBlock(
        invariantBlock->getEntry()->getNode(), comp(), invariantBlock->getFrequency(), invariantBlock);
    ...
    if (_requiresAdditionalCheckForIncrement
        ...
        if (_constNode->getType().isInt32()) {
            ...
            dumpOptDetails(
                comp(), "Node %p has been created testing if increment check is required\n", nextComparisonNode);
    ...
    while (comparisonNode) {
        ...
        TR::Block* comparisonBlock = TR::Block::createEmptyBlock(
            invariantBlock->getEntry()->getNode(), comp(), invariantBlock->getFrequency(), invariantBlock);
    ...
    while (!invariantBlock->getPredecessors().empty()) {
        ...
        if (nextPredBlock != _cfg->getStart()) {
            ...
            if (lastTreeInPred)
                lastTreeInPred->adjustBranchOrSwitchTreeTop(
                    comp(), invariantBlock->getEntry(), chooserBlock->getEntry());
        }
    }
        ...
        if (isTest) {
            if (currCriticalEdgeBlock == NULL) {
                ...
                TR::DebugCounter::prependDebugCounter(
                    comp(), debugCounter, currCriticalEdgeBlock->getData()->getEntry()->getNextTreeTop());
...
void TR_LoopVersioner::RemoveAsyncCheck::improveLoop()
{
    ...
    dumpOptDetails(comp(), "Removing asynccheck n%un [%p]\n", asyncCheckNode->getGlobalIndex(), asyncCheckNode);
...
void TR_LoopVersioner::buildNullCheckComparisonsTree(
    List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees)
{
    ...
    for (; nextNode; nextNode = nextNode->getNextElement(), nextTree = nextTree->getNextElement()) {
            ...
            if ((nextNode->getData()->getOpCode().hasSymbolReference()
                ...
                if (invariantNullCheckReference) {
                    ...
                    if (nullCheckReferenceisField) {
                        ...
                        dupInvariantNullCheckReference->setAndIncChild(
                            0, invariantNullCheckReference->duplicateTreeForCodeMotion());
        ...
        if (performTransformation(comp(),
                ...
                OPT_DETAILS_LOOP_VERSIONER, nextNode->getData()->getGlobalIndex(), nextNode->getData(),
                nextTree->getData()->getNode()->getGlobalIndex(), nextTree->getData()->getNode())) {
            ...
            // TR::Node *nextComparisonNode = TR::Node::createif(TR::ifacmpeq, duplicateNullCheckReference,
            // TR::Node::create(duplicateNullCheckReference, TR::aconst, 0, 0), _exitGotoTarget);
            TR::Node* nextComparisonNode = TR::Node::createif(TR::ifacmpeq, duplicateNullCheckReference,
                TR::Node::aconst(duplicateNullCheckReference, 0), _exitGotoTarget);
            LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::TEST, nextComparisonNode);
            if (prep != NULL) {
...
{
    dumpOptDetails(comp(), "Removing null check n%un [%p]\n", _nullCheckNode->getGlobalIndex(), _nullCheckNode);
...
void TR_LoopVersioner::buildAwrtbariComparisonsTree(List<TR::TreeTop>* awrtbariTrees)
{
    ...
    while (nextTree) {
        ...
        if (performTransformation(comp(),
                ...
                OPT_DETAILS_LOOP_VERSIONER, awrtbariNode->getGlobalIndex(), awrtbariNode)) {
            ...
            if (isVariableHeapBase || isVariableHeapSize) {
                ifNode1 = TR::Node::create(TR::acmpge, 2, duplicateBase,
                    TR::Node::createWithSymRef(
                        TR::aload, 0, comp()->getSymRefTab()->findOrCreateThreadLowTenureAddressSymbolRef()));
            } else {
                ifNode1 = TR::Node::create(
                    TR::acmpge, 2, duplicateBase, TR::Node::aconst(duplicateBase, fej9->getLowTenureAddress()));
            }

            duplicateBase = awrtbariNode->getLastChild()->duplicateTreeForCodeMotion();

            if (isVariableHeapBase || isVariableHeapSize) {
                ifNode2 = TR::Node::create(TR::acmplt, 2, duplicateBase,
                    TR::Node::createWithSymRef(
                        TR::aload, 0, comp()->getSymRefTab()->findOrCreateThreadHighTenureAddressSymbolRef()));
            } else {
                ifNode2 = TR::Node::create(
                    TR::acmplt, 2, duplicateBase, TR::Node::aconst(duplicateBase, fej9->getHighTenureAddress()));
            }

            ifNode = TR::Node::createif(TR::ificmpne, TR::Node::create(TR::iand, 2, ifNode1, ifNode2),
                TR::Node::create(duplicateBase, TR::iconst, 0, 0), _exitGotoTarget);

            LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::TEST, ifNode);
            if (prep != NULL) {
                // nodeWillBeRemovedIfPossible() doesn't apply here.
                _curLoop->_loopImprovements.push_back(
                    new (_curLoop->_memRegion) RemoveWriteBarrier(this, prep, awrtbariNode));
            }
        }
...
{
    dumpOptDetails(comp(), "Removing write barrier n%un [%p]\n", _awrtbariNode->getGlobalIndex(), _awrtbariNode);
...
void TR_LoopVersioner::buildDivCheckComparisonsTree(List<TR::TreeTop>* divCheckTrees)
{
    ...
    while (nextTree) {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is divide by zero\n",
                OPT_DETAILS_LOOP_VERSIONER, divCheckNode->getGlobalIndex(), divCheckNode)) {
            ...
            if (duplicateDivisor->getType().isInt64())
                ifNode = TR::Node::createif(TR::iflcmpeq, duplicateDivisor,
                    TR::Node::create(duplicateDivisor, TR::lconst, 0, 0), _exitGotoTarget);
            else
                ifNode = TR::Node::createif(TR::ificmpeq, duplicateDivisor,
                    TR::Node::create(duplicateDivisor, TR::iconst, 0, 0), _exitGotoTarget);
...
{
    dumpOptDetails(comp(), "Removing div check n%un [%p]\n", _divCheckNode->getGlobalIndex(), _divCheckNode);
...
void TR_LoopVersioner::buildCheckCastComparisonsTree(List<TR::TreeTop>* checkCastTrees)
{
    ...
    while (nextTree) {
        ...
        if (!performTransformation(comp(),
                ...
                OPT_DETAILS_LOOP_VERSIONER, checkCastNode->getGlobalIndex(), checkCastNode)) {
            nextTree = nextTree->getNextElement();
            continue;
        }
        ...
        TR::Node* instanceofNode = TR::Node::createWithSymRef(
            TR:: instanceof, 2, 2, duplicateCheckedValue, duplicateClassPtr,
            comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
        TR::Node* ificmpeqNode = TR::Node::createif(
            TR::ificmpeq, instanceofNode, TR::Node::create(checkCastNode, TR::iconst, 0, 0), _exitGotoTarget);
...
void TR_LoopVersioner::RemoveCheckCast::improveLoop()
{
    ...
    dumpOptDetails(comp(), "Removing checkcast n%un [%p]\n", checkCastNode->getGlobalIndex(), checkCastNode);
    ...
    TR::TreeTop* firstNewTree
        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getFirstChild()), NULL, NULL);
    TR::TreeTop* secondNewTree
        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getSecondChild()), NULL, NULL);
...
void TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(List<TR::TreeTop>* arrayStoreCheckTrees)
{
    ...
    while (nextTree) {
        if (!performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is casted\n",
                OPT_DETAILS_LOOP_VERSIONER, nextTree->getData()->getNode()->getGlobalIndex(),
        ...
        TR::Node* duplicateClassPtr = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, duplicateSrcArray, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
        ...
        TR::Node* instanceofNode = TR::Node::createWithSymRef(
            ...
            comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
        TR::Node* ificmpeqNode = TR::Node::createif(
            TR::ificmpeq, instanceofNode, TR::Node::create(arrayStoreCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
...
void TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop()
{
    ...
    dumpOptDetails(
        comp(), "Removing array store check n%un [%p]\n", arrayStoreCheckNode->getGlobalIndex(), arrayStoreCheckNode);
    ...
    TR::TreeTop* firstNewTree = TR::TreeTop::create(
        comp(), TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getFirstChild()), NULL, NULL);
    ...
    if (child->getOpCodeValue() == TR::awrtbari && TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_none
        && performTransformation(
            comp(), "%sChanging awrtbari node [%p] to an iastore\n", OPT_DETAILS_LOOP_VERSIONER, child)) {
    ...
    if (arrayStoreCheckNode->getNumChildren() > 1) {
        ...
        secondNewTree = TR::TreeTop::create(
            comp(), TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getSecondChild()), NULL, NULL);
...
void TR_LoopVersioner::convertSpecializedLongsToInts(TR::Node* node, vcount_t visitCount, TR::SymbolReference** symRefs)
{
    ...
    if ((node->getType().isInt64()) && node->getOpCode().isLoadVar()) {
...
bool TR_LoopVersioner::buildLoopInvariantTree(List<TR_NodeParentSymRef>* invariantNodes)
{
    ...
    while (nextInvariantNode) {
        ...
        if (performTransformation(comp(), "%s Attempting to hoist n%un [%p] out of the loop\n",
                OPT_DETAILS_LOOP_VERSIONER, invariantNode->getGlobalIndex(), invariantNode)) {
            LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::PRIVATIZE, invariantNode->duplicateTree());

            if (prep == NULL) {
                dumpOptDetails(
                    comp(), "failed to privatize n%un [%p]\n", invariantNode->getGlobalIndex(), invariantNode);
            } else {
                _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) Hoist(this, prep));
            }
        }
...
bool TR_LoopVersioner::buildSpecializationTree(List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* divCheckTrees,
    List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* comparisonTrees,
    List<TR::Node>* specializedNodes, TR::Block* loopInvariantBlock, TR::SymbolReference** symRefs)
{
    ...
    while (nextSpecializedNode) {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is value profiled\n",
            ...
            TR_ValueInfo* valueInfo = static_cast<TR_ValueInfo*>(
                TR_ValueProfileInfoManager::getProfiledValueInfo(specializedNode, comp(), ValueInfo));
            int32_t value = valueInfo->getTopValue();

            TR::Node* comparisonNode = NULL;
            if (specializedNode->getType().isInt64()) {
                TR::Node* highInt
                    = TR::Node::create(TR::land, 2, duplicateNode, TR::Node::create(duplicateNode, TR::lconst, 0));
                highInt->getSecondChild()->setLongInt(((uint64_t)0xffffffff) << 32);
                comparisonNode = TR::Node::createif(
                    TR::iflcmpne, highInt, TR::Node::create(duplicateNode, TR::lconst, 0, 0), _exitGotoTarget);
            } else {
                comparisonNode = TR::Node::createif(TR::ificmpne, duplicateNode,
                    TR::Node::create(duplicateNode, TR::iconst, 0, value), _exitGotoTarget);
            }
            ...
            if (specializedNode->getType().isInt64()) {
                if (specializedNode->getOpCode().isLoadVar()) {
                    TR::SymbolReference* tempSymRef
                        = symRefs[specializedNode->getSymbolReference()->getReferenceNumber()];
                    if (!tempSymRef) {
                        ...
                        TR::Node* newStore = TR::Node::createWithSymRef(TR::istore, 1, 1,
                            TR::Node::create(TR::l2i, 1, specializedNode->duplicateTree()), tempSymRef);
...
void TR_LoopVersioner::buildConditionalTree(
    List<TR::TreeTop>* conditionalTrees, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    while (nextTree) {
        ...
        if (conditionalNode->isTheVirtualGuardForAGuardedInlinedCall() && !conditionalNode->isOSRGuard()
                ...
                if (thisChild->getOpCode().hasSymbolReference()
                    ...
                    if (dupThisChild) {
                        ...
                        if (guardInfo->getTestType() == TR_VftTest) {
                            ...
                            conditionalNode->getFirstChild()->getFirstChild()->setAndIncChild(
                                0, dupThisChild->duplicateTree());
                        } else {
            ...
            if (unmentionedReceiver != NULL && requiresPrivatization(unmentionedReceiver)) {
                chainPrep = true;
                prep = createLoopEntryPrep(LoopEntryPrep::PRIVATIZE, unmentionedReceiver);
            }
        }

        if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is conditional\n",
                OPT_DETAILS_LOOP_VERSIONER, conditionalNode->getGlobalIndex(), conditionalNode)) {
            bool changeConditionalToUnconditionalInBothVersions = false;

            //
            // We pulled out from both on first conditional and all the subsequent conditionals we only pull from fast
            // path This exposed a bug where slow path had an unconditional that removed a valid path. Fix is only
            // allowing unconditioning in both if there is single if conditional tree.
            //
            ...
            if (isVersionableIfWithExtremum(conditionalTree, &excond)) {
                ...
                if (excond.extremumIsFinalValue || conditionalNode->getOpCode().isUnsigned()) {
                    ...
                    TR::Node* initialIv = TR::Node::createLoad(conditionalNode, excond.iv);

                    TR::ILOpCodes exitCmpOp = excond.loopTest.keepLoopingCmpOp.getOpCodeForReverseBranch();
                    ...
                    TR::Node* range = TR::Node::create(conditionalNode, TR::isub, 2, loopLimit, initialIv);

                    TR::ILOpCodes wrongSignCmpOp = excond.step > 0 ? TR::ificmplt : TR::ificmpgt;
                    ...
                    {
                        TR::Node* rem = TR::Node::create(conditionalNode, TR::irem, 2, range, step);
                        finalJ = TR::Node::create(conditionalNode, TR::isub, 2, finalJ,
                    ...
                    if (!excond.loopTest.usesUpdatedIv) {
                        ...
                        finalJ = TR::Node::create(
                            conditionalNode, TR::isub, 2, finalJ, TR::Node::iconst(conditionalNode, -1));
                    }
                    ...
                    TR::Node* finalIv = TR::Node::create(conditionalNode, TR::iadd, 2, initialIv,
                    ...
                    if (ivLoadSeesUpdatedValue(excond.ivLoad, conditionalTree)) {
                        ...
                        finalIv = TR::Node::create(conditionalNode, TR::iadd, 2, finalIv, step);
                    }
                    ...
                    TR_ASSERT_FATAL_WITH_NODE(
                        conditionalNode, indexChildIndex >= 0, "Index child index should be valid at this point");
                    ...
                    if (duplicateIndexNode->getOpCode().isLoad()
                        ...
                        replaceInductionVariable(
                            conditionalNode, duplicateIndexNode, indexChildIndex, indexSymRefNum, finalIv, visitCount);
                    }
                }
                ...
                if (conditionalNode->getOpCode().isUnsigned()) {
                    dumpOptDetails(comp(), "Generating initial/final sign test for unsigned comparison\n");
                    ...
                    TR::Node* xorNode = TR::Node::create(conditionalNode, TR::ixor, 2, initial->duplicateTree(), final);

                    TR::Node* signTestNode = TR::Node::createif(
                        TR::ificmplt, xorNode, TR::Node::iconst(conditionalNode, 0), _exitGotoTarget);
            ...
            duplicateComparisonNode = TR::Node::createif(dupIfOp, dupChildren[0], dupChildren[1], _exitGotoTarget);
            ...
            if (duplicateComparisonNode->isMaxLoopIterationGuard()) {
                duplicateComparisonNode->setIsMaxLoopIterationGuard(
...
bool TR_LoopVersioner::replaceInductionVariable(
    TR::Node* parent, TR::Node* node, int childNum, int loopDrivingInductionVar, TR::Node* loopLimit, int visitCount)
{
    if (node->getVisitCount() == visitCount)
        return false;
 ...
 */
bool TR_LoopVersioner::ivLoadSeesUpdatedValue(TR::Node* ivLoad, TR::TreeTop* occurrenceTree)
{
    TR_ASSERT_FATAL_WITH_NODE(ivLoad, ivLoad->getOpCode().isLoadVarDirect(), "expected a direct load");

    TR::SymbolReference* iv = ivLoad->getSymbolReference();
    TR_ASSERT_FATAL_WITH_NODE(ivLoad, iv->getSymbol()->isAutoOrParm(), "expected an auto");

    bool foundOccurrence = false;
    for (TR::PostorderNodeIterator it(occurrenceTree, comp()); it.isAt(occurrenceTree); it.stepForward()) {
    ...
    bool updateAlwaysExecuted = blockIsAlwaysExecutedInLoop(ivUpdateBlock, _curLoop->_loop, &priorBlocks);

    TR_ASSERT_FATAL(updateAlwaysExecuted, "expected IV #%d to be updated every iteration", ivNum);
...
void TR_LoopVersioner::buildSpineCheckComparisonsTree(List<TR::TreeTop>* spineCheckTrees)
{
    ...
    while (nextTree) {
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] has spine\n",
                OPT_DETAILS_LOOP_VERSIONER, spineCheckNode->getGlobalIndex(), spineCheckNode)) {
            TR::Node* contigArrayLength
                = TR::Node::create(TR::contigarraylength, 1, arrayBase->duplicateTreeForCodeMotion());
            TR::Node* nextComparisonNode = TR::Node::createif(
                TR::ificmpne, contigArrayLength, TR::Node::create(spineCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
...
void TR_LoopVersioner::RemoveSpineCheck::improveLoop()
{
    ...
    dumpOptDetails(comp(), "Removing spine check n%un [%p]\n", spineCheckNode->getGlobalIndex(), spineCheckNode);
    ...
    if (is64BitTarget) {
        ...
        hdrSize = TR::Node::create(
            spineCheckNode, TR::iconst, 0, (int32_t)TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());

    int32_t shift = TR::TransformUtil::convertWidthToShift(TR::Compiler->om.sizeofReferenceField());
    TR::Node* shiftNode
        = is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)shift) : TR::Node::iconst(spineCheckNode, shift);
    ...
    node = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, node, comp()->getSymRefTab()->findOrCreateArrayletShadowSymbolRef(type));
    ...
    if (comp()->useCompressedPointers()) {
        compressTree
            = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(node->getFirstChild()), NULL, NULL);
    }
...
void TR_LoopVersioner::buildBoundCheckComparisonsTree(
    List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* spineCheckTrees, bool reverseBranch)
{
    ListElement<TR::TreeTop>* nextTree = boundCheckTrees->getListHead();
    bool isAddition;
    ...
    while (nextTree) {
                ...
                if (arrayObject->getOpCode().hasSymbolReference()
                    ...
                    if (invariantArrayObject) {
                        ...
                        arrayLengthNode = TR::Node::create(
                            arrayLengthNode, arrayLengthNode->getOpCodeValue(), 1, dupInvariantArrayObject);
                    } else
        ...
        bool performBoundCheck
            = performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] exceeds bounds\n",
                OPT_DETAILS_LOOP_VERSIONER, boundCheckNode->getGlobalIndex(), boundCheckNode);
        ...
        if (isIndexInvariant) {
            nextComparisonNode = TR::Node::createif(TR::ificmplt, indexNode->duplicateTreeForCodeMotion(),
            ...
            if (boundCheckNode->getOpCodeValue() == TR::BNDCHK
                ...
                nextComparisonNode = TR::Node::createif(TR::ificmpge, indexNode->duplicateTreeForCodeMotion(),
                    arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
            else
                nextComparisonNode = TR::Node::createif(TR::ificmpgt, indexNode->duplicateTreeForCodeMotion(),
                    arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
            ...
            while (indexSymRef && changedIndexSymRef) {
                ...
                if (!foundInductionVariable) {
                    ...
                    while (versionableInductionVar) {
                        if (indexSymRefNum == *(versionableInductionVar->getData())) {
                            ...
                            exitValue
                                = _storeTrees[indexSymRefNum]->getNode()->getFirstChild()->getSecondChild()->getInt();
                            foundInductionVariable = true;
                            break;
                        }
                        versionableInductionVar = versionableInductionVar->getNextElement();
                ...
                if ((isDerivedInductionVariable && (loopDrivingInductionVariable == -1)) || !foundInductionVariable) {
                    TR::SymbolReference* symRefInCompare = NULL;
                    if (isDerivedInductionVariable && (loopDrivingInductionVariable == -1)) {
                        if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1)) {
            ...
            if (!valueChild->getOpCode().hasSymbolReference()
                || (valueChild->getSymbolReference()->getReferenceNumber()
                    != indexChild->getSymbolReference()->getReferenceNumber()))
                complexButPredictableForm = false;
            ...
            if (isSpecialInductionVariable) {
                ...
                nextComparisonNode = TR::Node::createif(TR::ificmplt,
                ...
                if (changedIndexSymRefAtSomePoint
                    ...
                    if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                        ...
                        TR::Node* replacementLoad = TR::Node::createWithSymRef(
                            storeNode, comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0, indexSymRef);

                        if (!storeRhs->getOpCode().isLoad()) {
                            int visitCount = comp()->incVisitCount();
                            replaceInductionVariable(
                                NULL, storeRhs, -1, changedIndexSymRefNumForStoreRhs, replacementLoad, visitCount);
                        } else
                        ...
                        if (!duplicateIndex->getOpCode().isLoad()) {
                            vcount_t visitCount = comp()->incVisitCount();
                            replaceInductionVariable(
                                NULL, duplicateIndex, -1, origIndexSymRef->getReferenceNumber(), storeRhs, visitCount);
                        } else
                ...
                if ((isAddition && !indVarOccursAsSecondChildOfSub)
                    ...
                    nextComparisonNode = TR::Node::createif(TR::ificmplt, duplicateIndex,
                        TR::Node::create(boundCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
                    ...
                    nextComparisonNode = TR::Node::createif(
                        TR::ificmpge, duplicateIndex, arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
            ...
            if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
                ...
                if (isIncreasingI == stayInLoopOp.isCompareTrueIfGreater()) {
                    dumpOptDetails(comp(),
                        ...
                        stayInLoopOp.getName(), isIncreasingI ? "an increasing" : "a decreasing");
                 ...
                 *   - (limit + step) should be greater than or equal to the limit, otherwise outside of the
                 * representable range
                 *
                 * Loop test op code: >= or >
                 *   - (limit - step) should be less than or equal to the limit, otherwise outside of the representable
                 * range
                 */
                if (!_storeTrees[loopDrivingInductionVariable]->getNode()->getFirstChild()->cannotOverflow()) {
                    ...
                    if ((stayInLoopOp.getOpCodeValue() == TR::ificmple)
                        || (stayInLoopOp.getOpCodeValue() == TR::ificmplt)) // <=, <
                    {
                        TR::Node* overLimit = TR::Node::create(TR::iadd, 2, loopLimit, incrNode);
                        overflowComparisonNode
                            = TR::Node::createif(TR::ificmplt, overLimit, loopLimit, _exitGotoTarget);
                    } else // >=, >
                    {
                        TR::Node* overLimit = TR::Node::create(TR::isub, 2, loopLimit, incrNode);
                        overflowComparisonNode
                            = TR::Node::createif(TR::ificmpgt, overLimit, loopLimit, _exitGotoTarget);
                    }
            ...
            if (isSpecialInductionVariable) {
                ...
                nextComparisonNode = TR::Node::createif(
                    TR::ifiucmpgt, correctCheckNode, arrayLengthNode->duplicateTree(), _exitGotoTarget);
                if (trace())
                    traceMsg(comp(), "Special Induction variable added in each iter -> Creating %p (%s)\n",
                        nextComparisonNode, nextComparisonNode->getOpCode().getName());
            } else {
                ...
                if (isAddition) {
                    if (!indVarOccursAsSecondChildOfSub) {
                        nextComparisonNode = TR::Node::createif(
                            TR::ifiucmpge, correctCheckNode, arrayLengthNode->duplicateTree(), _exitGotoTarget);
                        nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
                    } else {
                ...
                if (changedIndexSymRefAtSomePoint
                    ...
                    if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                        ...
                        if (!storeRhs->getOpCode().isLoad()) {
                            int visitCount = comp()->incVisitCount();
                            replaceInductionVariable(
                                NULL, storeRhs, -1, origIndexSymRef->getReferenceNumber(), replacementLoad, visitCount);
                        } else
                            storeRhs = replacementLoad;
                        ...
                        if (!firstChild->getOpCode().isLoad()) {
                            vcount_t visitCount = comp()->incVisitCount();
                            replaceInductionVariable(
                                NULL, firstChild, -1, origIndexSymRef->getReferenceNumber(), storeRhs, visitCount);
                        } else
                            firstChild = storeRhs;
                    }
                }
                if (!isIndexChildMultiplied) {
                    ...
                    if (isAddition) {
                        if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple)
                            ...
                            nextComparisonNode
                                = TR::Node::createif(TR::ificmpge, firstChild, secondChild, _exitGotoTarget);
                        ...
                        if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge)
                            ...
                            nextComparisonNode
                                = TR::Node::createif(TR::ificmple, firstChild, secondChild, _exitGotoTarget);
                    ...
                    if ((isAddition && !indVarOccursAsSecondChildOfSub)
                        ...
                        visitCount = comp()->incVisitCount();
                        replaceInductionVariable(NULL, duplicateMulHNode, -1, indexSymRef->getReferenceNumber(),
                            loopLimit->duplicateTree(), visitCount);
                    }
...
void TR_LoopVersioner::createRemoveBoundCheck(
    TR::TreeTop* boundCheckTree, LoopEntryPrep* prep, List<TR::TreeTop>* spineCheckTrees)
{
...
void TR_LoopVersioner::RemoveBoundCheck::improveLoop()
{
    ...
    dumpOptDetails(comp(), "Removing bound check n%un [%p]\n", boundCheckNode->getGlobalIndex(), boundCheckNode);
    ...
    if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
        ...
        TR::TreeTop* firstNewTree
            = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(0)), NULL, NULL);
        TR::TreeTop* secondNewTree
            = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(1)), NULL, NULL);
        prevTree->join(firstNewTree);
        firstNewTree->join(secondNewTree);
 ...
 */
void TR_LoopVersioner::collectAllExpressionsToBeChecked(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees,
    TR::Node* node, List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock, vcount_t visitCount)
{
...
void TR_LoopVersioner::collectAllExpressionsToBeChecked(TR::Node* node, List<TR::Node>* comparisonTrees)
{
    ...
    TR_ASSERT_FATAL(shouldOnlySpecializeLoops() || refineAliases(),
        "versioner itself called collectAllExpressionsToBeChecked() for loop %d", _curLoop->_loop->getNumber());
    ...
    if (!depsForLoopEntryPrep(node, &deps, &visited, true)) {
        comp()->failCompilation<TR::CompilationException>("failed to generate safety tests");
    }
...
{
    static const bool nothingRequiresPrivatization = feGetEnv("TR_nothingRequiresPrivatizationInVersioner") != NULL;
...
void TR_LoopVersioner::dumpOptDetailsCreatingTest(const char* description, TR::Node* node)
{
    dumpOptDetails(comp(), "Creating %s test for n%un [%p]\n", description, node->getGlobalIndex(), node);
}

void TR_LoopVersioner::dumpOptDetailsFailedToCreateTest(const char* description, TR::Node* node)
{
    dumpOptDetails(comp(), "Failed to create %s test for n%un [%p]\n", description, node->getGlobalIndex(), node);
}
...
bool TR_LoopVersioner::depsForLoopEntryPrep(
    TR::Node* node, TR::list<LoopEntryPrep*, TR::Region&>* deps, TR::NodeChecklist* visited, bool canPrivatizeRootNode)
{
    ...
    if (node->isInternalPointer()
        ...
        if (!firstChild->isThisPointer()) {
            ...
            TR::Node* ifacmpeqNode
                = TR::Node::createif(TR::ifacmpeq, firstChild, TR::Node::aconst(node, 0), _exitGotoTarget);
            LoopEntryPrep* nullTestPrep = addLoopEntryPrepDep(LoopEntryPrep::TEST, ifacmpeqNode, deps, visited);
        ...
        if (node->isInternalPointer()
            ...
            if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
                ...
                int32_t otherLen;
                char* otherSig = otherSymRef->getOwningMethod(comp())->classNameOfFieldOrStatic(
                    otherSymRef->getCPIndex(), otherLen);
                ...
                if (!instanceOfReqd) {
                    ...
                    dumpOptDetails(comp(), "Failed to find class from field #%d\n", otherSymRef->getReferenceNumber());
                    return false;
                } else if (cl != NULL && fe()->isInstanceOf(cl, otherClassObject, true) == TR_yes) {
                    instanceOfReqd = false;
                } else {
                    TR::SymbolReference* otherClassSymRef
                        = comp()->getSymRefTab()->findOrCreateClassSymbol(owningMethodSym, classCPI, otherClassObject);

                    duplicateClassPtr = TR::Node::createWithSymRef(node, TR::loadaddr, 0, otherClassSymRef);
                }
            }
        }
        ...
        if (instanceOfReqd) {
            if (otherClassObject) {
                ...
                TR::Node* instanceofNode = TR::Node::createWithSymRef(
                    TR:: instanceof, 2, 2, firstChild, duplicateClassPtr,
                    comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
                TR::Node* ificmpeqNode = TR::Node::createif(
                    TR::ificmpeq, instanceofNode, TR::Node::create(node, TR::iconst, 0, 0), _exitGotoTarget);
                ...
                TR::Node* vftLoad = TR::Node::createWithSymRef(
                    TR::aloadi, 1, 1, firstChild, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
                // TR::Node *componentTypeLoad = TR::Node::create(TR::aloadi, 1, vftLoad,
                ...
                // TR_ASSERT(((node->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef()) ||
                // comp()->getSymRefTab()->findVtableEntrySymbolRef(node->getSymbolReference())), "Not enough
                // information to emit the instanceof test that is reqd\n"); dumpOptDetails(comp(), "otherClassObject is
                // NULL in node %p\n", node); printf("otherClassObject is NULL in %s\n", comp()->signature());
                // fflush(stdout);
            }
        }
        ...
        if (node->getFirstChild()->isDataAddrPointer()) {
            ...
            if (arrayBaseAddressNode->isDataAddrPointer()) {
                ...
                static struct temps {
                    ...
                    int32_t k;
                } a[] = { { TR::iadd, TR::iconst, headerSize }, { TR::isub, TR::iconst, -headerSize },
                    { TR::ladd, TR::lconst, headerSize }, { TR::lsub, TR::lconst, -headerSize } };
            ...
            if (childInRequiredForm) {
                if (childInRequiredForm->getOpCodeValue() == TR::ishl
                    || childInRequiredForm->getOpCodeValue() == TR::lshl) {
                ...
                } else if (childInRequiredForm->getOpCodeValue() == TR::imul
                    || childInRequiredForm->getOpCodeValue() == TR::lmul) {
                    if (childInRequiredForm->getSecondChild()->getOpCode().isLoadConst()
        ...
        TR::Node* ificmpgeNode = TR::Node::createif(
            type.isInt32() ? TR::ificmpge : TR::iflcmpge, indexNode, arrayLengthNode, _exitGotoTarget);
        ...
        TR::Node* ificmpltNode
            = TR::Node::createif(type.isInt32() ? TR::ificmplt : TR::iflcmplt, indexNode, constNode, _exitGotoTarget);
        if (type.isInt64())
    ...
    } else if (((node->getOpCodeValue() == TR::idiv) || (node->getOpCodeValue() == TR::irem)
                   || (node->getOpCodeValue() == TR::lrem) || (node->getOpCodeValue() == TR::ldiv))) {
        // This is a divide; so we need to insert explicit checks
        // to mimic the div check.
        //
        TR::Node* divisor = node->getSecondChild();
        bool divisorIsNonzeroConstant = (divisor->getOpCodeValue() == TR::lconst && divisor->getLongInt() != 0)
        ...
        if (!divisorIsNonzeroConstant) {
            ...
            if (divisor->getType().isInt64())
                ifNode = TR::Node::createif(
                    TR::iflcmpeq, divisor, TR::Node::create(node, TR::lconst, 0, 0), _exitGotoTarget);
            else
                ifNode = TR::Node::createif(
                    TR::ificmpeq, divisor, TR::Node::create(node, TR::iconst, 0, 0), _exitGotoTarget);
    ...
    {
        TR_ASSERT_FATAL(false, "depsForLoopEntryPrep: n%un is a checkcast", node->getGlobalIndex());
    }
...
TR_LoopVersioner::LoopEntryPrep* TR_LoopVersioner::addLoopEntryPrepDep(
    LoopEntryPrep::Kind kind, TR::Node* node, TR::list<LoopEntryPrep*, TR::Region&>* deps, TR::NodeChecklist* visited)
{
...
TR::Node* TR_LoopVersioner::findLoad(TR::Node* node, TR::SymbolReference* symRef, vcount_t origVisitCount)
{
    ...
    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference() == symRef))
        return node;
...
bool TR_LoopVersioner::findStore(TR::TreeTop* start, TR::TreeTop* end, TR::Node* node, TR::SymbolReference* symRef,
    bool needAuxiliaryWalk, bool lastTimeThrough)
{
    ...
    while (cursorTree != end) {
        ...
        //
        if (!needAuxiliaryWalk && findLoad(cursorTree->getNode(), symRef, origVisitCount) == node) {
            if (lastTimeThrough)
                foundLoad = true;
            else
                return false;
...
int32_t TR_LoopVersioner::detectCanonicalizedPredictableLoops(
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    int32_t retval = 1;
    ...
    if (loopStructure->getMaxNestingDepth(&currentNestingDepth, &currentMaxNestingDepth)
        > (MAX_NESTING_DEPTH / hotnessFactor))
        return -2;
    ...
    if (loopInvariantBlock) {
        /////if (isInvariantLoop)
        {
            ...
            //_writtenAndNotJustForHeapification = new (trStackMemory()) TR_BitVector(symRefCount, trMemory(),
            //stackAlloc);
            ...
            if ((isPredictableLoop > 0)
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (storeInRequiredForm /* && !_cannotBeEliminated->get(nextInductionVariableNumber) */) {
                        int32_t* versionableInductionVariable
                            = (int32_t*)trMemory()->allocateStackMemory(sizeof(int32_t));
                        *versionableInductionVariable = nextInductionVariableNumber;
                        ...
                        if ((loopTestNode->getFirstChild()->getOpCode().hasSymbolReference()
                                && nextInductionVariableNumber
                                    == loopTestNode->getFirstChild()->getSymbolReference()->getReferenceNumber())
                            || loopTestNode->getFirstChild()
                                == _storeTrees[nextInductionVariableNumber]->getNode()->getFirstChild())
                    ...
                    for (TR::Block* b = eb.getFirst(); b; b = eb.getNext()) {
                        ...
                        if (ifNode->getOpCode().isIf() && !ifNode->getOpCode().isCompBranchOnly()) {
                            ...
                            if (ivNode->getOpCode().hasSymbolReference())
                                ...
                                if (ivNode && ivNode->getOpCode().hasSymbolReference())
                                    ivSymRef = ivNode->getSymbolReference();
                            }
...
bool TR_LoopVersioner::isStoreInSpecialForm(int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (storeNode->getFirstChild()->getOpCode().isAnd()
        ...
        && (storeNode->getFirstChild()->getFirstChild()->getSymbolReference()->getReferenceNumber()
...
bool TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (!secondChild->getOpCode().isLoadConst()
        && (secondChild->getType().isInt32() || secondChild->getType().isInt64())) {
        ...
        // test comparison.
        static const bool allowVariableStep = feGetEnv("TR_loopVersionerAllowVariableStep") != NULL;
        if (!allowVariableStep)
            return false;

        // This requires isAutoOrParm() because other threads can write to statics
        if (secondChild->getOpCode().isLoadVarDirect() && secondChild->getSymbol()->isAutoOrParm()) {
            int32_t timesWritten = 0;
            if (!isSymbolReferenceWrittenNumberOfTimesInStructure(
                    loopStructure, secondChild->getSymbolReference()->getReferenceNumber(), &timesWritten, 0))
                return false;
...
void TR_LoopVersioner::collectArrayAliasCandidates(TR::Node* node, vcount_t visitCount)
{
    TR_ASSERT(!refineAliases(), "something wrong");
}
void TR_LoopVersioner::buildAliasRefinementComparisonTrees(List<TR::TreeTop>* nullCheckTrees,
    List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees,
...
void TR_LoopVersioner::findAndReplaceContigArrayLen(TR::Node* parent, TR::Node* node, vcount_t visitCount)
{
    ...
    if ((node->getOpCodeValue()
            == TR::contigarraylength)) // future: || (node->getOpCodeValue() == TR::discontigarraylength))
...
{
    if (node->getOpCode().isConversion() && node->getFirstChild()->getOpCode().isConversion()
        ...
        if ((conversion1->getOpCodeValue() == TR::s2i && conversion2->getOpCodeValue() == TR::i2s)
            || (conversion1->getOpCodeValue() == TR::b2i && conversion2->getOpCodeValue() == TR::i2b)
            || // FIXME: why are we ignoring bu2i/i2b here?
...
bool TR_LoopVersioner::initExprFromNode(Expr* expr, TR::Node* node, bool onlySearching)
{
    ...
    if (expr->_op.isLoadConst()) {
        ...
        TR_ASSERT_FATAL(node->getBranchDestination() == _exitGotoTarget,
            "versioning test n%un [%p] does not target _exitGotoTarget", node->getGlobalIndex(), node);
    }
    ...
    TR_ASSERT_FATAL((allFlags & mandatoryFlags) == mandatoryFlags,
        "setting _flags 0x%x would fail to preserve _mandatoryFlags 0x%x\n", allFlags, mandatoryFlags);
...
bool TR_LoopVersioner::guardOkForExpr(TR::Node* node, bool onlySearching)
{
    ...
    case TR_HCRGuard:
        TR_ASSERT_FATAL(
            onlySearching, "guardOkForExpr: should not intern HCR guard n%un [%p]", node->getGlobalIndex(), node);
        return false;

    case TR_OSRGuard:
        TR_ASSERT_FATAL(
            onlySearching, "guardOkForExpr: should not intern OSR guard n%un [%p]", node->getGlobalIndex(), node);
        return false;

    // Never versioned, only holds inner assumptions.
    case TR_DummyGuard:
        TR_ASSERT_FATAL(
            onlySearching, "guardOkForExpr: should not intern dummy guard n%un [%p]", node->getGlobalIndex(), node);
        return false;

    // Never versioned (for now).
    case TR_BreakpointGuard:
        TR_ASSERT_FATAL(onlySearching, "guardOkForExpr: should not intern breakpoint guard n%un [%p]",
            node->getGlobalIndex(), node);
        return false;
    ...
    default:
        ...
        //
        TR_ASSERT_FATAL(false, "guardOkForExpr: n%un [%p]: unrecognized nop-able guard kind %d", node->getGlobalIndex(),
            node, (int)kind);
    }
...
const TR_LoopVersioner::Expr* TR_LoopVersioner::makeCanonicalExpr(TR::Node* node)
{
    ...
    static const bool assertRepresentableEnv = feGetEnv("TR_assertRepresentableInVersioner") != NULL;

    Expr expr;
    bool onlySearching = false; // expr will be inserted into CurLoop::_exprTable
    if (!initExprFromNode(&expr, node, onlySearching)) {
        dumpOptDetails(comp(), "n%un [%p] is unrepresentable\n", node->getGlobalIndex(), node);
        ...
        if (shouldAssert) {
            if (node->isNopableInlineGuard()) {
                TR_VirtualGuard* guard = comp()->findVirtualGuardInfo(node);
                TR_ASSERT_FATAL(false, "n%un [%p] is unrepresentable guard kind=%d, test=%d", node->getGlobalIndex(),
                    node, (int)guard->getKind(), (int)guard->getTestType());
            } else {
                TR_ASSERT_FATAL(false, "n%un [%p] is unrepresentable", node->getGlobalIndex(), node);
            }
        }

        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "loopVersioner.unrepresentable/(%s)/%s/loop=%d/n%un",
                comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()), _curLoop->_loop->getNumber(),
                node->getGlobalIndex()));
...
const TR_LoopVersioner::Expr* TR_LoopVersioner::findCanonicalExpr(TR::Node* node)
{
    ...
    TR::Node* defRHS = NULL;
    if (node->getOpCode().isLoadVarDirect() && node->getSymbol()->isAutoOrParm() && !isExprInvariant(node))
...
const TR_LoopVersioner::Expr* TR_LoopVersioner::substitutePrivTemps(
    TR::TreeTop* tt, TR::Node* node, TR::NodeChecklist* visited)
{
    if (visited->contains(node)) {
        auto cached = _curLoop->_nodeToExpr.find(node);
        if (cached == _curLoop->_nodeToExpr.end())
    ...
    if (defRHS != NULL) {
        ...
        Expr expr;
        bool onlySearching = true;
        bool rootMayBePrivatized = !node->getOpCode().isTreeTop() && initExprFromNode(&expr, node, onlySearching);
    ...
    TR_ASSERT_FATAL(insertResult.second || insertResult.first->second == canonicalExpr,
        "failed to insert n%un [%p] into _nodeToExpr", node->getGlobalIndex(), node);
    ...
    dumpOptDetails(comp(), "Transforming n%un [%p] into a load of privatization temp #%d\n", node->getGlobalIndex(),
        node, temp->getReferenceNumber());
...
{
    TR_ASSERT_FATAL(!prep->_requiresPrivatization || _curLoop->_privatizationOK,
        "should not be emitting prep %p because it requires privatization", prep);
    ...
    if (prep->_kind == LoopEntryPrep::TEST) {
        ...
        dumpOptDetails(comp(), "Emitted prep %p as n%un [%p]\n", prep, node->getGlobalIndex(), node);
    } else {
        TR_ASSERT_FATAL(
            prep->_kind == LoopEntryPrep::PRIVATIZE, "prep %p has unrecognized kind %d\n", prep, prep->_kind);
        ...
        //
        static const bool singleThreaded = feGetEnv("TR_assumeSingleThreadedVersioning") != NULL;
        ...
        //
        TR_ASSERT_FATAL(!value->isInternalPointer(), "prep %p attempting to privatize an internal pointer", prep);
        ...
        TR::SymbolReference* temp = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), tempType);

        if (value->getDataType() == TR::Address && value->isNotCollected())
            temp->getSymbol()->setNotCollected();

        auto insertResult = _curLoop->_privTemps.insert(std::make_pair(prep->_expr, PrivTemp(temp, nodeType)));

        TR_ASSERT_FATAL(insertResult.second, "_privTemps insert failed for expr %p", prep->_expr);
        ...
        dumpOptDetails(comp(), "Emitted prep %p as n%un [%p] storing to temp #%d\n", prep, store->getGlobalIndex(),
            store, temp->getReferenceNumber());
    }
}
...
void TR_LoopVersioner::unsafelyEmitAllTests(
    const TR::list<LoopEntryPrep*, TR::Region&>& preps, List<TR::Node>* comparisonTrees)
{
    for (auto it = preps.begin(); it != preps.end(); ++it) {
        ...
        if (prep->_kind == LoopEntryPrep::TEST) {
            ...
            dumpOptDetails(comp(), "Unsafely emitted prep %p as n%un [%p]\n", prep, node->getGlobalIndex(), node);

            if (!prep->_requiresPrivatization) {
                prep->_emitted = true;
                dumpOptDetails(comp(), "This prep happens to be safe (no privatization required)\n");
            }
        }
    }
}
...
TR_LoopVersioner::LoopEntryPrep* TR_LoopVersioner::createLoopEntryPrep(
    LoopEntryPrep::Kind kind, TR::Node* node, TR::NodeChecklist* visited, LoopEntryPrep* prev)
{
    bool optDetails = comp()->getOutFile() != NULL && (trace() || comp()->getOption(TR_TraceOptDetails));
    ...
    if (existing != _curLoop->_prepTable.end()) {
        ...
        dumpOptDetails(
            comp(), "Using existing prep %p for n%un [%p]\n", existing->second, node->getGlobalIndex(), node);

        return existing->second;
    }

    LoopEntryPrep* prep = new (_curLoop->_memRegion) LoopEntryPrep(kind, expr, _curLoop->_memRegion);
    ...
    if (!ok) {
        dumpOptDetails(comp(), "Failed to create prep for n%un [%p]\n", node->getGlobalIndex(), node);
    ...
    if (optDetails) {
        dumpOptDetails(comp(), "Prep for n%un [%p] is prep %p %s expr %p%s, deps: ", node->getGlobalIndex(), node, prep,
            prep->_kind == LoopEntryPrep::PRIVATIZE ? "PRIVATIZE" : "TEST", prep->_expr,
...
TR_LoopVersioner::LoopEntryPrep* TR_LoopVersioner::createChainedLoopEntryPrep(
    LoopEntryPrep::Kind kind, TR::Node* node, LoopEntryPrep* prev)
{
    if (prev == NULL)
        return NULL;
 ...
 */
TR_LoopVersioner::CurLoop::CurLoop(TR::Compilation* comp, TR::Region& memRegion, TR_RegionStructure* loop)
 ...
 */
TR_LoopVersioner::LoopBodySearch::LoopBodySearch(TR::Compilation* comp, TR::Region& memRegion, TR_RegionStructure* loop,
...
{
    TR_ASSERT_FATAL(isBranchConstant(ifNode), "unexpected branch n%un", ifNode->getGlobalIndex());
...
void TR_LoopVersioner::LoopBodySearch::enqueueReachableSuccessorsInLoopFrom(TR::CFGEdgeList& outgoingEdges)
{
...
{
    if (!_alreadyEnqueuedBlocks.contains(block) && _loop->contains(block->getStructureOf())) {
