...
static TR::Node* testBinary(TR::Compilation* comp, TR::Node* parent, TR::ILOpCodes value, TR::ILOpCodes leftChildValue,
    TR::ILOpCodes rightChildValue, const char* msg)
{
...
static TR::Node* testBinaryIConst(TR::Compilation* comp, TR::Node* parent, TR::ILOpCodes value,
    TR::ILOpCodes leftChildValue, int32_t rightChildValue, const char* msg)
{
    TR::Node* child = parent->getFirstChild();
    if (child->getOpCodeValue() != value || child->getFirstChild()->getOpCodeValue() != leftChildValue
        || child->getSecondChild()->getOpCodeValue() != TR::iconst
        || child->getSecondChild()->getInt() != rightChildValue) {
        if (msg)
            dumpOptDetails(comp, msg);
        return NULL;
    } else {
...
TR_LRAddressTree::TR_LRAddressTree(TR::Compilation* comp, TR_InductionVariable* indVar)
    : TR_AddressTree(heapAlloc, comp)
    , _indVar(indVar)
    , _increment(indVar->getIncr()->getLowInt())
    , _indVarLoad(NULL)
{ }

TR_ArrayLoop::TR_ArrayLoop(TR::Compilation* comp, TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar)
    : _comp(comp)
    , _firstAddress(comp, firstIndVar)
    , _secondAddress(comp, secondIndVar)
    , _thirdAddress(comp, secondIndVar)
    , _finalNode(NULL)
    , _addInc(false)
    , _forwardLoop(false)
{ }
...
TR_ArrayLoop::TR_ArrayLoop(TR::Compilation* comp, TR_InductionVariable* indVar)
    : _comp(comp)
    , _firstAddress(comp, indVar)
    , _secondAddress(comp, indVar)
    , _thirdAddress(comp, indVar)
    , _finalNode(NULL)
    , _addInc(false)
    , _forwardLoop(false)
{ }
...
TR_Arraycopy::TR_Arraycopy(TR::Compilation* comp, TR_InductionVariable* indVar)
    : TR_ArrayLoop(comp, indVar)
{ }
TR_ByteToCharArraycopy::TR_ByteToCharArraycopy(
    TR::Compilation* comp, TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, bool bigEndian)
    : TR_ArrayLoop(comp, firstIndVar, secondIndVar)
    , _bigEndian(bigEndian)
{ }
TR_CharToByteArraycopy::TR_CharToByteArraycopy(
    TR::Compilation* comp, TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, bool bigEndian)
    : TR_ArrayLoop(comp, firstIndVar, secondIndVar)
    , _bigEndian(bigEndian)
{ }
...
TR_Arraytranslate::TR_Arraytranslate(TR::Compilation* comp, TR_InductionVariable* indVar, bool hasBranch, bool hasBreak)
    ...
    , _tableNode(NULL)
    , _resultNode(NULL)
    , _resultUnconvertedNode(NULL)
    , _usesRawStorage(false)
    , _compilerGeneratedTable(false)
    , _hasBranch(hasBranch)
    , _hasBreak(hasBreak)
    , _compareOp(TR::BadILOp)
{ }
...
bool TR_LRAddressTree::processBaseAndIndex(TR::Node* parent)
{
...
bool TR_LRAddressTree::checkAiadd(TR::Node* aiaddNode, int32_t elementSize)
{
    ...
    if (_indVarNode.isNull()
        || (_indVarNode.getChild()->skipConversions()->getSymbol()->getRegisterMappedSymbol() != indSym)) {
        dumpOptDetails(comp(), "checkAiadd: induction variable does not match index variable\n");
        return false;
    }

    TR::RegisterMappedSymbol* loadSym
        = _indVarNode.getChild()->skipConversions()->getSymbol()->getRegisterMappedSymbol();
    if (loadSym != indSym) {
        if (_matIndVarSymRef) {
            if (loadSym != _matIndVarSymRef->getSymbol()->getRegisterMappedSymbol()) {
                dumpOptDetails(
                    comp(), "checkAiadd: load in the aiadd tree does not match materialized induction variable\n");
                return false;
            }
        } else {
            dumpOptDetails(comp(), "checkAiadd: induction variable does not match index variable\n");
    ...
    if (_multiplyNode.isNull() && ((elementSize != _increment) && (elementSize != -_increment))) {
        dumpOptDetails(comp(),
            "checkAiadd: sub-tree does not have induction variable change consistent with increment of multiplier (%d "
            "%d)\n",
...
bool TR_Arrayset::checkArrayStore(TR::Node* storeNode)
{
    ...
    if (opCodeStoreSecondChild == TR::iload
        && storeSecondChild->getSymbol()->getRegisterMappedSymbol() == getIndVar()->getLocal()) {
...
bool TR_Arraycopy::checkArrayStore(TR::Node* storeNode)
{
    if (!storeNode->getOpCode().isStoreIndirect()
        && !(storeNode->getOpCodeValue() == TR::treetop
            && storeNode->getFirstChild()->getOpCodeValue() == TR::awrtbari)) {
    ...
    if (storeFirstChild->getNumChildren() == 0 || loadFirstChild->getNumChildren() == 0
        ...
        || storeFirstChild->getFirstChild()->getSymbol()->getRegisterMappedSymbol()
            == loadFirstChild->getFirstChild()->getSymbol()->getRegisterMappedSymbol()) {
...
bool TR_ByteToCharArraycopy::checkArrayStore(TR::Node* storeNode)
{
...
//              ==>iload at [0x005f1af4]
//             iconst -17
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node* loadNodes)
{
    ...
    if (highByteNode->getOpCodeValue() != TR::imul
        ...
        if (highByteNode->getOpCodeValue() != TR::imul || lowByteNode->getOpCodeValue() != TR::bu2i) {
            dumpOptDetails(
                comp(), "checkByteLoads: byte to char arraycopy byte loads do not have imul/bu2i children\n");
            return false;
        }
    }

    if (highByteNode->getFirstChild()->getOpCodeValue() != TR::bu2i
        || highByteNode->getFirstChild()->getFirstChild()->getOpCodeValue() != TR::bloadi) {
    ...
    if (highByteNode->getSecondChild()->getOpCodeValue() != TR::iconst
        || highByteNode->getSecondChild()->getInt() != 256) {
    ...
    TR::RegisterMappedSymbol* hbs = getHighLoadAddress()->getBaseVarNode()->getChild()
        ? getHighLoadAddress()->getBaseVarNode()->getChild()->getSymbol()->getRegisterMappedSymbol()
        : NULL;
    TR::RegisterMappedSymbol* lbs = getLowLoadAddress()->getBaseVarNode()->getChild()
        ? getLowLoadAddress()->getBaseVarNode()->getChild()->getSymbol()->getRegisterMappedSymbol()
        : NULL;
    if (hbs || lbs) {
        // if either has a base var, both must, and they both must be the same
        if (!hbs || !lbs || hbs != lbs) {
            dumpOptDetails(comp(),
                "checkByteLoads: at least one tree has a base sym, but both trees do not have the same sym (%p %p)\n",
                lbs, hbs);
            return false;
        }
    }
...
bool TR_CharToByteArraycopy::checkArrayStores(TR::Node* origHighStoreNode, TR::Node* origLowStoreNode)
{
    ...
    if (highStoreNode->getOpCodeValue() != TR::bstorei) {
        dumpOptDetails(comp(),
            "checkArrayStores: char to byte arraycopy high arraystore tree does not have an indirect store as root\n");
        return false;
    }
    if (lowStoreNode->getOpCodeValue() != TR::bstorei) {
        dumpOptDetails(comp(),
            "checkArrayStores: char to byte arraycopy low arraystore tree does not have an indirect store as root\n");
        return false;
    }
    ...
    TR::Node* ishr = testNode(
        comp(), origHighStoreNode->getSecondChild(), TR::i2b, "checkArrayStores: high store child is not i2b\n");
    if (!ishr)
        return false;
    TR::Node* iand = testBinaryIConst(
        comp(), ishr, TR::ishr, TR::iand, 8, "checkArrayStores: high store child is not ishr of iand and 8\n");
    if (!iand)
        return false;
    TR::Node* c2i = testBinaryIConst(comp(), iand, TR::iand, TR::su2i, 0xFF00,
        "checkArrayStores: high store child is not iand of su2i and 0xFF00\n");
    if (!c2i)
        return false;
    TR::Node* isload
        = testUnary(comp(), c2i->getFirstChild(), TR::sloadi, "checkArrayStores: high store child is not isload\n");
    if (!isload)
        return false;
    ...
    iand = testNode(
        comp(), origLowStoreNode->getSecondChild(), TR::i2b, "checkArrayStores: low store child is not i2b\n");
    if (!iand)
        return false;
    c2i = testBinaryIConst(
        comp(), iand, TR::iand, TR::su2i, 0xFF, "checkArrayStores: low store child is not iand of su2i and 0xFF\n");
    if (!c2i)
        return false;
    TR::Node* isloadDup
        = testUnary(comp(), c2i->getFirstChild(), TR::sloadi, "checkArrayStores: low store child is not isload\n");
    if (!isloadDup)
        return false;
...
//
bool TR_LRAddressTree::checkIndVarStore(TR::Node* indVarNode)
{
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node* loopCmpNode, TR::Node* indVarStoreNode, TR_InductionVariable* indVar)
{
    ...
    if (compareOp == TR::ificmplt || compareOp == TR::ificmple || compareOp == TR::ifiucmplt
        || compareOp == TR::ifiucmple) {
        _forwardLoop = true;
    }
    ...
    if (opCodeCmpSecondChild != TR::iconst && opCodeCmpSecondChild != TR::iload
        && !cmpSecondChild->getOpCode().isArrayLength()) {
...
int32_t TR_ArrayLoop::checkForPostIncrement(
    TR::Block* loopHeader, TR::Node* indVarStoreNode, TR::Node* loopCmpNode, TR::Symbol* ivSym)
{
...
void TR_ArrayLoop::findIndVarLoads(TR::Node* node, TR::Node* indVarStoreNode, bool& storeFound, List<TR::Node>* ivLoads,
...
TR::Node* TR_ArrayLoop::updateIndVarStore(
    TR_ParentOfChildNode* indVarNode, TR::Node* indVarStoreNode, TR_LRAddressTree* tree, int32_t postIncrement)
{
...
void TR_LRAddressTree::updateAiaddSubTree(TR_ParentOfChildNode* indVarNode, TR_ArrayLoop* loop)
{
...
TR::Node* TR_LRAddressTree::updateMultiply(TR_ParentOfChildNode* multiplyNode)
{
    TR::Node* newMul = NULL;
    if (!multiplyNode->isNull()) {
        ...
        if (multiplyNode->getParent()->getType().isInt32()) {
            TR::Node* initMulVal = TR::Node::create(
                multiplyNode->getParent(), TR::iconst, 0, (_increment > 0 ? _increment : -_increment));
...
//   iconst 0 (or iload <val> if loop invariant)
//
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
    // for aladds
    ...
    {
        dumpOptDetails(
            comp(), "Loop has %d tree tops - no arrayset reduction\n", loopHeader->getNumberOfRealTreeTops());
        return false;
    }
    ...
    if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier && !comp()->cg()->getSupportsReferenceArrayCopy()) {
        dumpOptDetails(comp(),
            "arraycopy arraystore tree has write barrier as root and write barriers are enabled but no support for "
            "this platform- no arraycopy reduction\n");
        return false;
    }

    int32_t postIncrement = arraycopyLoop.checkForPostIncrement(
        loopHeader, indVarStoreNode, loopCmpNode, arraycopyLoop.getStoreAddress()->getIndVar()->getLocal());

    int32_t storeSize = storeNode->getSize();
#ifdef J9_PROJECT_SPECIFIC
    if (storeNode->getType().isBCD() && !(storeSize == 1 || storeSize == 2 || storeSize == 4 || storeSize == 8)) {
        dumpOptDetails(comp(),
            "arraycopy storeNode %p is a BCD type (%s) and the storeSize (%d) is not 1,2,4 or 8 so do not reduce "
            "arraycopy\n",
    ...
    TR::Node* imul = arraycopyLoop.updateIndVarStore(
        arraycopyLoop.getStoreIndVarNode(), indVarStoreNode, arraycopyLoop.getStoreAddress(), postIncrement);
    ...
    if (!(arraycopyLoop.hasWriteBarrier() && needWriteBarrier)) {
        ...
        if (arraycopyElementType.isBCD()) {
            ...
            case 1:
                arraycopyElementType = TR::Int8;
                break;
            case 2:
                arraycopyElementType = TR::Int16;
                break;
            case 4:
                arraycopyElementType = TR::Int32;
                break;
            case 8:
                arraycopyElementType = TR::Int64;
                break;
            default:
        ...
        arraycopy = TR::Node::createArraycopy(
            loadAddr->getFirstChild(), storeAddr->getFirstChild(), loadAddr, storeAddr, imul->duplicateTree());
...
//   iconst 0 (or iload <val> if loop invariant)
//
bool TR_LoopReducer::generateArrayset(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
    // for aladds
    ...
    if (!performTransformation(comp(),
            "%sReducing arrayset %d from storeNode [" POINTER_PRINTF_FORMAT
            "] and copyValueNode [" POINTER_PRINTF_FORMAT "]\n",
    ...
    //   The number of elements to set to the value (induction variable initial value - comparison value) / increment /
    //   multiplier
    ...
    TR::Node* imul
        = arraysetLoop.updateIndVarStore(arraysetLoop.getIndVarNode(), indVarStoreNode, arraysetLoop.getStoreAddress());
    ...
    TR::Node* arrayset = TR::Node::create(TR::arrayset, 3, dst, copyValueNode, imul->duplicateTree());
...
bool TR_Arraycmp::checkElementCompare(TR::Node* elemCmpNode)
{
    TR::ILOpCodes elemOp = elemCmpNode->getOpCodeValue();
    if (elemOp != TR::ificmpne && elemOp != TR::iffcmpne && elemOp != TR::iffcmpneu && elemOp != TR::ifdcmpne
        && elemOp != TR::ifdcmpneu && elemOp != TR::iflcmpne) {
...
bool TR_Arraycmp::checkGoto(TR::Block* gotoBlock, TR::Node* gotoNode, TR::Node* finalNode)
{
    ...
    if (targetNode->getOpCode().getOpCodeValue() != TR::ificmpne) {
        if (targetNode->getOpCode().getOpCodeValue() == TR::istore) {
            ...
            if (targetNode->getOpCode().getOpCodeValue() != TR::icmpeq) {
                dumpOptDetails(
                    comp(), "target of goto is not an ificmpne/istore. It is %s\n", targetNode->getOpCode().getName());
                return false;
            }
        }
    }
    ...
    if (extraStoreNode != NULL) {
        ...
        && !(firstChildSym == indSym && secondChildSym == NULL
            && comparisonNode->getInt() == targetNode->getSecondChild()->getInt())) {
...
//
TR::Node* TR_Arraytranslate::getMulChild(TR::Node* child)
{
    // match the tree
    // lmul
    //   i2l
    ...
    if ((child->getOpCodeValue() == TR::imul || child->getOpCodeValue() == TR::lmul)
        && (child->getSecondChild()->getOpCodeValue() == TR::iconst
            || child->getSecondChild()->getOpCodeValue() == TR::lconst)) {
        ...
        if (constValue == 2) {
            if (child->getFirstChild()->getOpCodeValue() == TR::i2l
                || child->getFirstChild()->getOpCodeValue() == TR::bu2l) {
...
bool TR_Arraytranslate::checkLoad(TR::Node* loadNode)
{
    ...
    //
    if (transLoadNode->getOpCodeValue() == TR::i2b || transLoadNode->getOpCodeValue() == TR::i2s
    ...
    if (aiaddNode->getOpCodeValue() != TR::aiadd && aiaddNode->getOpCodeValue() != TR::aladd
        && aiaddNode->getOpCodeValue() != TR::iadd && aiaddNode->getOpCodeValue() != TR::ladd) {
    ...
    if (inLoadNode->getOpCodeValue() != TR::bloadi) {
        dumpOptDetails(
            comp(), "...load tree does not have 2nd bloadi - check if compiler-generated table lookup match\n");
...
bool TR_Arraytranslate::checkStore(TR::Node* storeNode)
{
    ...
    if (hasBranch()) {
        ...
        if (shrinkNode->getOpCodeValue() != TR::i2s && shrinkNode->getOpCodeValue() != TR::i2b
            && shrinkNode->getOpCodeValue() != TR::sconst && shrinkNode->getOpCodeValue() != TR::bconst) {
        ...
        if (shrinkNode->getOpCodeValue() == TR::i2b || shrinkNode->getOpCodeValue() == TR::i2s) {
            ...
            if (iloadNode->getSymbolReference() != _resultNode->getSymbolReference()) {
                dumpOptDetails(comp(),
                    "...store tree reference does not match load tree reference - no arraytranslate reduction\n");
                return false;
            }
        }
    } else {
...
//
bool TR_Arraytranslate::checkBreak(TR::Block* breakBlock, TR::Node* breakNode, TR::Block* nextBlock)
{
    ...
    if (loadNode->getSymbolReference() != _resultUnconvertedNode->getSymbolReference()
        ...
        dumpOptDetails(
            comp(), "...break tree reference does not match load tree reference - no arraytranslate reduction\n");
        return false;
    }

    TR::Node* valNode = breakNode->getSecondChild();
    if (valNode->getOpCodeValue() != TR::iconst
        || (compilerGeneratedTable() && (valNode->getInt() > 32766 || valNode->getInt() < -32766))) {
...
//
bool TR_Arraytranslate::checkMatIndVarStore(TR::Node* matIndVarStoreNode, TR::Node* indVarStoreNode)
{
    ...
    if (storeFirstChild->getOpCodeValue() != TR::iadd && storeFirstChild->getOpCodeValue() != TR::isub) {
    ...
    if (iaddFirstChild->getOpCodeValue() != TR::iadd && iaddFirstChild->getOpCodeValue() != TR::isub) {
    ...
    if (indVarLoad->getOpCodeValue() != TR::iload || iaddFirstChild->getSecondChild()->getOpCodeValue() != TR::iconst) {
        dumpOptDetails(comp(),
            "primary iv in the materialized tree is not TR::iload %p or second child is not TR::iconst %p\n",
    ...
    if (iaddSecondChild->getOpCodeValue() != TR::iload && iaddSecondChild->getOpCodeValue() != TR::iconst) {
        dumpOptDetails(
            comp(), "second child of the materialized tree is not delta or a constant %p\n", iaddSecondChild);
        return false;
    }
...
bool TR_Arraytranslate::checkGoto(TR::Block* gotoBlock, TR::Node* gotoNode, TR::Block* nextBlock)
{
    TR::ILOpCodes gotoOpCode = gotoNode->getOpCode().getOpCodeValue();
    ...
    if (targetBlock != nextBlock) {
        dumpOptDetails(
            comp(), "...goto tree does not goto the first block after the loop %p %p\n", targetBlock, nextBlock);
        return false;
    }
    return true;
}
...
TR::ILOpCodes TR_LoopReducer::convertIf(TR::ILOpCodes ifCmp)
{
    switch (ifCmp) {
    case TR::ifbcmpeq:
        return TR::bcmpeq;
    ...
    case TR::ifacmpeq:
        return TR::acmpeq;
    default:
        TR_ASSERT(0, "Unexpected TR_ifxcmp operator");
        return TR::BadILOp;
    }
}

bool TR_LoopReducer::blockInVersionedLoop(List<TR::CFGEdge> otherLoopVersionBlockList, TR::Block* block)
{
...
bool TR_LoopReducer::generateArraycmp(
    TR_RegionStructure* whileLoop, TR_InductionVariable* indVar, TR::Block* branchBlock, TR::Block* incrementBlock)
{
    //
    ...
    if (compNum != 1 || incrNum != 2) {
        dumpOptDetails(
            comp(), "Loop has wrong number of tree tops (%d,%d) - no arraycmp reduction\n", compNum, incrNum);
        return false;
    }
    ...
    while (!queue.isEmpty()) {
        ...
        if (block != _cfg->getEnd() && block != branchBlock && block != incrementBlock) {
            ...
            if (blockState == transformerReadFirst) {
                dumpOptDetails(comp(),
                    "induction variable is read before write in block_%d after compare loop - no arraycmp reduction\n",
                    block->getNumber());
                return false;
            }
            ...
            if (blockState == transformerWrittenFirst) {
                dumpOptDetails(comp(),
                    "induction variable is written before read in block_%d after compare loop - pruning successor\n",
                    block->getNumber());
                continue;
            }
        }
    ...
    if ((arraycmpLoop.getFirstAddress()->getIncrement() * arraycmpLoop.getFirstAddress()->getMultiplier() < 0)
    ...
    //   The second address to compare to (the aiadd tree - this may reference the first iload in which case it can't be
    //   updated) The number of elements to compare (induction variable initial value - comparison value) / increment /
    //   multiplier
    //
    // Tree-Top
    // ArrayCmp call
    ...
    TR::Node* mul = arraycmpLoop.updateIndVarStore(
        arraycmpLoop.getFirstIndVarNode(), indVarStoreNode, arraycmpLoop.getFirstAddress());
    if (!comp()->target().is64Bit()) {
        // updateIndVarStore returns an imul on 32 bit, extend as arraycmp takes 64 bit length
        mul = TR::Node::create(TR::i2l, 1, mul);
        // extending the imul is technically unneccessary since the length of an array on 32 bit cannot exceed 32 bit
        // range
    ...
    TR_ASSERT(arraycmpLoop.getFirstLoad()->getOpCode().isLoadVar(),
        "firstLoad %s (%p) is not a loadVar for arraycmp reduction\n",
        arraycmpLoop.getFirstLoad()->getOpCode().getName(), arraycmpLoop.getFirstLoad());
    TR_ASSERT(arraycmpLoop.getSecondLoad()->getOpCode().isLoadVar(),
        "secondLoad %s (%p) is not a loadVar for arraycmp reduction\n",
...
int32_t TR_Arraytranslate::getTermValue()
{
...
TR::Node* TR_Arraytranslate::getTermCharNode()
{
    if (compilerGeneratedTable()) {
...
TR::Node* TR_Arraytranslate::getTableNode()
{
    if (compilerGeneratedTable() && (_tableNode == NULL)) {
        ...
        if (TR::ILOpCode::isLessCmp(_compareOp)) {
            ...
            if ((_compareOp == TR::BadILOp) && (!hasBranch() && !hasBreak())) {
                startA = 0;
                endA = max;
                startB = endB = 0;
            } else
        ...
        if (inputBits == 8) {
            if (startB != endB) {
                TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits, (uint16_t)startA, (uint16_t)endA,
                    (uint16_t)startB, (uint16_t)endB, defaultValue);
                tableSymRef = rangeTable.createSymbolRef();
                rangeTable.dumpTable();
            } else {
                TR_RangeTranslateTable rangeTable(
                    comp(), inputBits, outputBits, (uint16_t)startA, (uint16_t)endA, defaultValue);
                tableSymRef = rangeTable.createSymbolRef();
                rangeTable.dumpTable();
            }
        } else {
            if (startB != endB) {
                TR_RangeTranslateTable rangeTable(
                    comp(), inputBits, outputBits, startA, endA, startB, endB, defaultValue);
                tableSymRef = rangeTable.createSymbolRef();
                rangeTable.dumpTable();
            } else {
...
bool TR_LoopReducer::replaceInductionVariable(TR::Node* parent, TR::Node* node, int32_t childNum,
    int32_t indVarSymRefNumber, TR::Node* replacingNode, vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return false;
...
bool TR_LoopReducer::generateArraytranslate(TR_RegionStructure* whileLoop, TR_InductionVariable* indVar,
    TR::Block* firstBlock, TR::Block* secondBlock, TR::Block* thirdBlock, TR::Block* fourthBlock)
{
    ...
    if (((firstNum == 3 || firstNum == 4 || firstNum == 5) && secondNum == 0)
    ...
    bool hasBranch = true;
    if (((firstNum == 2 || firstNum == 3) && secondNum == 3)) {
        hasBreak = true;
    ...
    if (arraytranslateLoop.compilerGeneratedTable() && !hasBranch && hasBreak) {
        dumpOptDetails(comp(),
            "Attempt to reduce loop marked with no branch but also comp gen'ed table - no arraytranslate reduction\n");
        return false;
    }
    ...
    if (!hasBranch && !arraytranslateLoop.getByteInput() && !cg()->getSupportsTestCharComparisonControl()) {
        dumpOptDetails(
            comp(), "Cannot reduce char->byte loops since hardware does not support termination character control\n");
        return false;
    }
    ...
    if (mayHaveMaterializedIV && !arraytranslateLoop.checkMatIndVarStore(matIndVarStoreNode, indVarStoreNode)) {
    ...
    int32_t minimumSize = cg()->arrayTranslateMinimumNumberOfElements(
        arraytranslateLoop.getByteInput(), arraytranslateLoop.getByteOutput());
    ...
    */
    ...
    int32_t mask = cg()->arrayTranslateTableRequiresAlignment(
        arraytranslateLoop.getByteInput(), arraytranslateLoop.getByteOutput());
    if (mask && !arraytranslateLoop.compilerGeneratedTable()) // compiler-generated tables aligned at allocation
    {
        ...
        if (arraytranslateLoop.tableBackedByRawStorage()) {
            ...
            TR::Node* hdrSizeNode = TR::Node::create(
                loadNode, TR::iconst, 0, static_cast<int32_t>(TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
    ...
    translateNode->setTableBackedByRawStorage(
        arraytranslateLoop.tableBackedByRawStorage() || arraytranslateLoop.compilerGeneratedTable());
    ...
    if (hasBranch) {
        ...
        if (!hasBreak) {
            ...
            TR::Node* icmpNode
                = TR::Node::create(TR::icmpeq, 2, iNode, arraytranslateLoop.getFinalNode()->duplicateTree());
            ...
            replaceInductionVariable(NULL, resCharDupNode->getFirstChild()->skipConversions(), -1,
                indVarSymRef->getReferenceNumber(), decNode, visitCount);
        ...
        if (mayHaveMaterializedIV) {
            ...
            TR::Node* decNode
                = TR::Node::create(TR::isub, 2, firstChild, TR::Node::create(updatedMatIndVar, TR::iconst, 0, 1));
    ...
    if (hasBreak) {
        ...
        TR::Node* matchResultNode
            = TR::Node::createLoad(branchAroundNode, arraytranslateLoop.getResultNode()->getSymbolReference());
        ...
        /// lengthTestNode = TR::Node::create(TR::iadd, 2, lengthTestNode, TR::Node::create(loadNode, TR::iconst, 0,
        /// 1));
        TR::Node* dupFinalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
        TR::TreeTop* lengthTestTree = TR::TreeTop::create(
            comp(), TR::Node::createif(TR::ificmplt, lengthTestNode, dupFinalNode, loadElementBlock->getEntry()));
        ...
        //
        for (; !ivLoadNode->getOpCode().isArrayRef() && ivLoadNode->getNumChildren() >= 1;
             ivLoadNode = ivLoadNode->getFirstChild())
            ;
        TR::Node* parentIV = NULL;
        bool foundIV = true;
        if (ivLoadNode->getOpCode().isArrayRef()) {
            vcount_t visitCount = comp()->incVisitCount();
            TR::Node* decNode = TR::Node::create(
                TR::isub, 2, TR::Node::createLoad(loadNode, indVarSymRef), TR::Node::iconst(loadNode, 1));
        ...
        TR::TreeTop* gotoMatchTree
            = TR::TreeTop::create(comp(), TR::Node::create(loadNode, TR::Goto, 0, matchBlock->getEntry()));
    ...
    if (hasBranch) {
        ...
        if (mayHaveMaterializedIV && indVarStoreIsFirstTreeTop) {
...
bool TR_ArraytranslateAndTest::checkLoad(TR::Block* loadBlock, TR::Node* loadNode)
{
    ...
    if (loadNode->getOpCodeValue() == TR::ificmpeq) {
        if (loadNode->getFirstChild()->getOpCodeValue() != TR::b2i) {
            dumpOptDetails(
                comp(), "...load tree has ificmpeq but no widening from byte - no arraytranslateAndTest reduction\n");
            return false;
        } else {
...
bool TR_ArraytranslateAndTest::checkFrequency(TR::CodeGenerator* cg, TR::Block* loadBlock, TR::Node* loadNode)
{
    ...
    if (((fallThroughFrequency <= 0) && ((loopHeaderFrequency > 0)))
...
//   iconst 1024
// BBEnd (block 16)
bool TR_LoopReducer::generateArraytranslateAndTest(
    TR_RegionStructure* whileLoop, TR_InductionVariable* indVar, TR::Block* firstBlock, TR::Block* secondBlock)
{
    ...
    if (secondNum == 2) {
        ...
        if (!asyncFound) {
            dumpOptDetails(comp(),
                "Loop has wrong number of tree tops for no async-check (%d,%d) - no arraytranslateAndTest reduction\n",
                firstNum, secondNum);
            return false;
        }
    ...
    TR::Node* finalNode = arraytranslateAndTestLoop.updateIndVarStore(
        arraytranslateAndTestLoop.getIndVarNode(), indVarStoreNode, arraytranslateAndTestLoop.getFirstAddress());
    ...
    if (cmpTargetBlockNumber != nextBlock->getNumber()) {
        TR::Node* compareNode = TR::Node::createif(TR::ificmpne,
            arraytranslateAndTestLoop.getStoreAddress()->getIndVarLoad()->duplicateTree(),
...
bool TR_LoopReducer::generateByteToCharArraycopy(
    TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
        return false;
    ...
    {
        dumpOptDetails(comp(),
            "Loop does not have an increment of +/-1 and +/-2, but instead %d and %d - no byte to char arraycopy "
            "reduction\n",
            incrA, incrB);
        return false;
    }
    ...
    if (loopHeader->getNumberOfRealTreeTops()
        != 4) // array store, induction variable store, induction variable store, loop comparison
    {
    ...
    if (!arraycopyLoop.checkByteLoads(storeNode->getSecondChild())) {
        dumpOptDetails(comp(),
            "second child of store does not look like OR of 2 byte loads - no byte to char arraycopy performed\n");
        return false;
    }
    ...
    if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
        ...
        if (swap && arraycopyLoop.getStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
            ...
            dumpOptDetails(comp(),
                "Did not encounter byte array induction variable increment - no byte to char arraycopy performed\n");
            return false;
        }
    } else if (!arraycopyLoop.getHighLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
        dumpOptDetails(comp(),
            "Did not encounter char array induction variable increment - no byte to char arraycopy performed\n");
        return false;
    }
    ...
    if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
        ...
        //   { ... }
        dumpOptDetails(comp(),
            "Loop comparison does not match byte or char induction variable - no byte to char arraycopy performed\n");
        return false;
    }

    if (!performTransformation(
            comp(), "%sReducing byte to char arraycopy %d\n", OPT_DETAILS, loopHeader->getNumber())) {
        return false;
    }
    ...
    TR::Node* charImul = arraycopyLoop.updateIndVarStore(
        arraycopyLoop.getStoreIndVarNode(), charIndVarStoreNode, arraycopyLoop.getStoreAddress());
    TR::Node* byteImul = arraycopyLoop.updateIndVarStore(
        arraycopyLoop.getHighLoadIndVarNode(), byteIndVarStoreNode, arraycopyLoop.getHighLoadAddress());
    arraycopyLoop.getStoreAddress()->updateMultiply(arraycopyLoop.getStoreMultiplyNode());
    ...
    TR::Node* charIndVarTempLoadNode
        = TR::Node::createLoad(storeAddr, arraycopyLoop.getStoreAddress()->getIndVarSymRef());
    ...
    TR::Node* byteIndVarReLoadNode
        = TR::Node::createLoad(storeAddr, arraycopyLoop.getHighLoadAddress()->getIndVarSymRef());
    TR::Node* addNode = TR::Node::create(TR::iadd, 2, byteIndVarReLoadNode, mulNode);
    TR::Node* byteIndVarReStoreNode
        = TR::Node::createStore(arraycopyLoop.getHighLoadAddress()->getIndVarSymRef(), addNode);
...
bool TR_LoopReducer::generateCharToByteArraycopy(
    TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
    ...
    if (loopHeader->getNumberOfRealTreeTops() != 5) // array store low byte, array store high byte, induction variable
                                                    // store, induction variable store, loop comparison
    {
    ...
    if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
        ...
        if (swap && arraycopyLoop.getLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
            ...
            dumpOptDetails(comp(),
                "Did not encounter byte array induction variable increment - no char to byte arraycopy performed\n");
            return false;
        }
    } else if (!arraycopyLoop.getHighStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
        dumpOptDetails(comp(),
            "Did not encounter char array induction variable increment - no char to byte arraycopy performed\n");
        return false;
    }
    ...
    if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
        ...
        //   { ... }
        dumpOptDetails(comp(),
            "Loop comparison does not match byte or char induction variable - no char to byte arraycopy performed\n");
        return false;
    }

    if (!performTransformation(
            comp(), "%sReducing char to byte arraycopy %d\n", OPT_DETAILS, loopHeader->getNumber())) {
        return false;
    }
    ...
    arraycopyLoop.getLoadAddress()->updateAiaddSubTree(arraycopyLoop.getLoadIndVarNode(), &arraycopyLoop);
    TR::Node* charImul = arraycopyLoop.updateIndVarStore(
        arraycopyLoop.getLoadIndVarNode(), charIndVarStoreNode, arraycopyLoop.getLoadAddress());
    TR::Node* byteImul = arraycopyLoop.updateIndVarStore(
        arraycopyLoop.getHighStoreIndVarNode(), byteIndVarStoreNode, arraycopyLoop.getHighStoreAddress());
    ...
    TR::TreeTop* treeList[]
        = { byteIndVarStoreTree, charIndVarStoreTree, highArrayStoreTree, lowArrayStoreTree, loopCmpTree, NULL };

    TR::TreeTop::removeDeadTrees(
        comp(), treeList); // make these trees into nops, but leave them around to keep ref counts right

    TR::SymbolReference* charIndVarTemp = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int32);
    TR::Node* charIndVarTempLoadNode
        = TR::Node::createLoad(storeAddr, arraycopyLoop.getLoadAddress()->getIndVarSymRef());
    TR::Node* charIndVarTempStoreNode = TR::Node::createStore(charIndVarTemp, charIndVarTempLoadNode);

    TR::TreeTop* charIndVarTempStoreTree = TR::TreeTop::create(comp(), charIndVarTempStoreNode);

    // following line is a bit messy - need to skip over the multiply-by-2, since want an unstrided value for ind var
    TR::Node* charIndVarReStoreNode = TR::Node::createStore(
        arraycopyLoop.getLoadAddress()->getIndVarSymRef(), charImul->getFirstChild()->duplicateTree());
    ...
    TR::Node* byteIndVarReLoadNode
        = TR::Node::createLoad(storeAddr, arraycopyLoop.getHighStoreAddress()->getIndVarSymRef());
    TR::Node* addNode = TR::Node::create(TR::iadd, 2, byteIndVarReLoadNode, mulNode);
    TR::Node* byteIndVarReStoreNode
        = TR::Node::createStore(arraycopyLoop.getHighStoreAddress()->getIndVarSymRef(), addNode);
...
bool TR_LoopReducer::constrainedIndVar(TR_InductionVariable* indVar)
{
    if (!indVar) {
        return true;
    }
...
void TR_LoopReducer::removeSelfEdge(TR::CFGEdgeList& succList, int32_t selfNumber)
{
    _cfg->removeEdge(succList, selfNumber, selfNumber);
}

int TR_LoopReducer::addBlock(TR::Block* newBlock, TR::Block** blockList, int numBlocks, const int maxNumBlocks)
{
...
int TR_LoopReducer::addRegionBlocks(
    TR_RegionStructure* region, TR::Block** blockList, int numBlocks, const int maxNumBlocks)
{
...
bool TR_LoopReducer::mayNeedGlobalDeadStoreElimination(TR::Block* storeBlock, TR::Block* compareBlock)
{
    ...
    TR::Block* blocks[numBlocks] = { storeBlock, compareBlock };
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure* whileLoop)
{
    ...
    if ((block2 && (loopHeader->getNextBlock() != block2)) || (block2 && block3 && (block2->getNextBlock() != block3))
    ...
    if (nextIndVar) /* these reductions require 2 induction variables to be considered */
    {
        ...
        if ((numBlocks == 2 /* || (numBlocks == 3 && block3->endsInGoto()) */)
            && mayNeedGlobalDeadStoreElimination(loopHeader, block2)) {
            dumpOptDetails(
                comp(), "Loop matches possible arraytranslate - global deadstore elimination to be performed\n");
        ...
        if (((numBlocks == 1) && generateArrayset(indVar, loopHeader))
            ...
            || (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */)
                && generateArraycmp(whileLoop, indVar, loopHeader, block2))
            || ((numBlocks == 1) && generateArraytranslate(whileLoop, indVar, loopHeader, NULL, NULL, NULL))
            || (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */)
                && generateArraytranslate(whileLoop, indVar, loopHeader, block2, NULL, NULL))
...
int32_t TR_LoopReducer::perform()
{
    ...
    if (!comp()->cg()->getSupportsArraySet() && !comp()->cg()->getSupportsReferenceArrayCopy()
        && !comp()->cg()->getSupportsPrimitiveArrayCopy() && !comp()->cg()->getSupportsArrayCmp()
        && !comp()->cg()->getSupportsArrayCmpLen() && !comp()->cg()->getSupportsArrayTranslateTRxx()
