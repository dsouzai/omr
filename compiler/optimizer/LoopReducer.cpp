...
TR_ByteToCharArraycopy::TR_ByteToCharArraycopy(TR::Compilation *comp, TR_InductionVariable *firstIndVar,
    TR_InductionVariable *secondIndVar, bool bigEndian)
    : TR_ArrayLoop(comp, firstIndVar, secondIndVar)
    , _bigEndian(bigEndian)
{}

TR_CharToByteArraycopy::TR_CharToByteArraycopy(TR::Compilation *comp, TR_InductionVariable *firstIndVar,
    TR_InductionVariable *secondIndVar, bool bigEndian)
    : TR_ArrayLoop(comp, firstIndVar, secondIndVar)
    , _bigEndian(bigEndian)
{}
    ...
    if (loadSym != indSym) {
        if (_matIndVarSymRef) {
            if (loadSym != _matIndVarSymRef->getSymbol()->getRegisterMappedSymbol()) {
                dumpOptDetails(comp(),
                    "checkAiadd: load in the aiadd tree does not match materialized induction variable\n");
                return false;
            }
        } else {
            dumpOptDetails(comp(), "checkAiadd: induction variable does not match index variable\n");
...
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node *loadNodes)
{
    ...
    if (highByteNode->getOpCodeValue() != TR::imul
        ...
        if (highByteNode->getOpCodeValue() != TR::imul || lowByteNode->getOpCodeValue() != TR::bu2i) {
            dumpOptDetails(comp(),
                "checkByteLoads: byte to char arraycopy byte loads do not have imul/bu2i children\n");
            return false;
        }
    }
...
bool TR_CharToByteArraycopy::checkArrayStores(TR::Node *origHighStoreNode, TR::Node *origLowStoreNode)
{
    ...
    TR::Node *ishr = testNode(comp(), origHighStoreNode->getSecondChild(), TR::i2b,
        "checkArrayStores: high store child is not i2b\n");
    if (!ishr)
        return false;
    TR::Node *iand = testBinaryIConst(comp(), ishr, TR::ishr, TR::iand, 8,
        "checkArrayStores: high store child is not ishr of iand and 8\n");
    if (!iand)
        return false;
    ...
    if (!c2i)
        return false;
    TR::Node *sloadi
        = testUnary(comp(), c2i->getFirstChild(), TR::sloadi, "checkArrayStores: high store child is not sloadi\n");
    if (!sloadi)
        return false;
    ...
    iand = testNode(comp(), origLowStoreNode->getSecondChild(), TR::i2b,
        "checkArrayStores: low store child is not i2b\n");
    if (!iand)
        return false;
    c2i = testBinaryIConst(comp(), iand, TR::iand, TR::su2i, 0xFF,
        "checkArrayStores: low store child is not iand of su2i and 0xFF\n");
    if (!c2i)
        return false;
    TR::Node *sloadiDup
        = testUnary(comp(), c2i->getFirstChild(), TR::sloadi, "checkArrayStores: low store child is not sloadi\n");
    if (!sloadiDup)
        return false;
...
int32_t TR_ArrayLoop::checkForPostIncrement(TR::Block *loopHeader, TR::Node *indVarStoreNode, TR::Node *loopCmpNode,
    TR::Symbol *ivSym)
{
...
TR::Node *TR_ArrayLoop::updateIndVarStore(TR_ParentOfChildNode *indVarNode, TR::Node *indVarStoreNode,
...
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable *indVar, TR::Block *loopHeader)
{
    ...
    int32_t postIncrement = arraycopyLoop.checkForPostIncrement(loopHeader, indVarStoreNode, loopCmpNode,
        arraycopyLoop.getStoreAddress()->getIndVar()->getLocal());
    ...
    TR::Node *imul = arraycopyLoop.updateIndVarStore(arraycopyLoop.getStoreIndVarNode(), indVarStoreNode,
        arraycopyLoop.getStoreAddress(), postIncrement);
    ...
    if (!(arraycopyLoop.hasWriteBarrier() && needWriteBarrier)) {
        ...
        arraycopy = TR::Node::createArraycopy(loadAddr->getFirstChild(), storeAddr->getFirstChild(), loadAddr,
            storeAddr, imul->duplicateTree());
...
bool TR_Arraytranslate::checkLoad(TR::Node *loadNode)
{
    ...
    if (inLoadNode->getOpCodeValue() != TR::bloadi) {
        dumpOptDetails(comp(),
            "...load tree does not have 2nd bloadi - check if compiler-generated table lookup match\n");
...
bool TR_Arraytranslate::checkBreak(TR::Block *breakBlock, TR::Node *breakNode, TR::Block *nextBlock)
{
    ...
    if (loadNode->getSymbolReference() != _resultUnconvertedNode->getSymbolReference()
        ...
        dumpOptDetails(comp(),
            "...break tree reference does not match load tree reference - no arraytranslate reduction\n");
        return false;
    }
...
bool TR_LoopReducer::generateArraycmp(TR_RegionStructure *whileLoop, TR_InductionVariable *indVar,
    TR::Block *branchBlock, TR::Block *incrementBlock)
{
    ...
    TR::Node *mul = arraycmpLoop.updateIndVarStore(arraycmpLoop.getFirstIndVarNode(), indVarStoreNode,
        arraycmpLoop.getFirstAddress());
    if (!comp()->target().is64Bit()) {
...
TR::Node *TR_Arraytranslate::getTableNode()
{
    if (compilerGeneratedTable() && (_tableNode == NULL)) {
        ...
        if (inputBits == 8) {
            ...
            if (startB != endB) {
                TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits, startA, endA, startB, endB,
                    defaultValue);
                tableSymRef = rangeTable.createSymbolRef();
                rangeTable.dumpTable();
            } else {
    ...
    TR::Block *firstBlock, TR::Block *secondBlock, TR::Block *thirdBlock, TR::Block *fourthBlock)
{
    ...
    if (!hasBranch && !arraytranslateLoop.getByteInput() && !cg()->getSupportsTestCharComparisonControl()) {
        dumpOptDetails(comp(),
            "Cannot reduce char->byte loops since hardware does not support termination character control\n");
        return false;
    }
    ...
    */
    ...
    int32_t mask = cg()->arrayTranslateTableRequiresAlignment(arraytranslateLoop.getByteInput(),
        arraytranslateLoop.getByteOutput());
    if (mask && !arraytranslateLoop.compilerGeneratedTable()) // compiler-generated tables aligned at allocation
    {
        ...
        if (arraytranslateLoop.tableBackedByRawStorage()) {
            ...
            TR::Node *hdrSizeNode = TR::Node::create(loadNode, TR::iconst, 0,
    ...
    if (hasBreak) {
        ...
        /// 1));
        TR::Node *dupFinalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
        TR::TreeTop *lengthTestTree = TR::TreeTop::create(comp(),
            TR::Node::createif(TR::ificmplt, lengthTestNode, dupFinalNode, loadElementBlock->getEntry()));
...
bool TR_ArraytranslateAndTest::checkLoad(TR::Block *loadBlock, TR::Node *loadNode)
{
    ...
    if (loadNode->getOpCodeValue() == TR::ificmpeq) {
        if (loadNode->getFirstChild()->getOpCodeValue() != TR::b2i) {
            dumpOptDetails(comp(),
                "...load tree has ificmpeq but no widening from byte - no arraytranslateAndTest reduction\n");
            return false;
        } else {
...
//   iconst 1024
// BBEnd (block 16)
bool TR_LoopReducer::generateArraytranslateAndTest(TR_RegionStructure *whileLoop, TR_InductionVariable *indVar,
    TR::Block *firstBlock, TR::Block *secondBlock)
{
    ...
    TR::Node *finalNode = arraytranslateAndTestLoop.updateIndVarStore(arraytranslateAndTestLoop.getIndVarNode(),
        indVarStoreNode, arraytranslateAndTestLoop.getFirstAddress());
...
bool TR_LoopReducer::generateByteToCharArraycopy(TR_InductionVariable *byteIndVar, TR_InductionVariable *charIndVar,
    TR::Block *loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
    ...
    TR::Node *charImul = arraycopyLoop.updateIndVarStore(arraycopyLoop.getStoreIndVarNode(), charIndVarStoreNode,
        arraycopyLoop.getStoreAddress());
    TR::Node *byteImul = arraycopyLoop.updateIndVarStore(arraycopyLoop.getHighLoadIndVarNode(), byteIndVarStoreNode,
        arraycopyLoop.getHighLoadAddress());
    arraycopyLoop.getStoreAddress()->updateMultiply(arraycopyLoop.getStoreMultiplyNode());
...
bool TR_LoopReducer::generateCharToByteArraycopy(TR_InductionVariable *byteIndVar, TR_InductionVariable *charIndVar,
    TR::Block *loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
    ...
    arraycopyLoop.getLoadAddress()->updateAiaddSubTree(arraycopyLoop.getLoadIndVarNode(), &arraycopyLoop);
    TR::Node *charImul = arraycopyLoop.updateIndVarStore(arraycopyLoop.getLoadIndVarNode(), charIndVarStoreNode,
        arraycopyLoop.getLoadAddress());
    TR::Node *byteImul = arraycopyLoop.updateIndVarStore(arraycopyLoop.getHighStoreIndVarNode(), byteIndVarStoreNode,
        arraycopyLoop.getHighStoreAddress());
    ...
    TR::TreeTop::removeDeadTrees(comp(),
        treeList); // make these trees into nops, but leave them around to keep ref counts right
    ...
    TR::Node *charIndVarReStoreNode = TR::Node::createStore(arraycopyLoop.getLoadAddress()->getIndVarSymRef(),
        charImul->getFirstChild()->duplicateTree());
...
int TR_LoopReducer::addRegionBlocks(TR_RegionStructure *region, TR::Block **blockList, int numBlocks,
...
bool TR_LoopReducer::mayNeedGlobalDeadStoreElimination(TR::Block *storeBlock, TR::Block *compareBlock)
{
    ...
    const TR::ILOpCodes pattern[numBlocks][numTreeTops] = {
        {  TR::istore, TR::istore, TR::ificmpeq },
        { TR::bstorei, TR::istore, TR::ificmplt }
    };
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure *whileLoop)
{
    ...
    if (nextIndVar) /* these reductions require 2 induction variables to be considered */
    {
        ...
        if ((numBlocks == 2 /* || (numBlocks == 3 && block3->endsInGoto()) */)
            ...
            dumpOptDetails(comp(),
                "Loop matches possible arraytranslate - global deadstore elimination to be performed\n");
