...
OMR::IDTBuilder::IDTBuilder(TR::ResolvedMethodSymbol* symbol, int32_t budget, TR::Region& region, TR::Compilation* comp,
...
TR::IDTBuilder* OMR::IDTBuilder::self() { return static_cast<TR::IDTBuilder*>(this); }
...
TR::IDT* OMR::IDTBuilder::buildIDT()
{
    ...
    TR_CallSite* rootCallSite = new (region())
        TR_CallSite(rootMethod, NULL, NULL, NULL, NULL, rootMethod->containingClass(), 0, 0, rootMethod, _rootSymbol,
            _rootSymbol->getMethodKind() == TR::MethodSymbol::Kinds::Virtual
                || _rootSymbol->getMethodKind() == TR::MethodSymbol::Kinds::Interface,
            _rootSymbol->getMethodKind() == TR::MethodSymbol::Kinds::Interface, bcInfo, comp());

    TR_CallTarget* rootCallTarget = new (region())
        TR_CallTarget(region(), rootCallSite, _rootSymbol, rootMethod, NULL, rootMethod->containingClass(), NULL);
    ...
    TR::vector<TR::AbsValue*, TR::Region&>* arguments, TR_CallStack* callStack)
{
    ...
    for (int32_t i = 0; i < callSite->numTargets(); i++) {
        ...
        TR::ResolvedMethodSymbol* calleeMethodSymbol
            = TR::ResolvedMethodSymbol::create(comp()->trHeapMemory(), callTarget->_calleeMethod, comp());

        TR::IDTNode* child = parent->addChild(_idt->getNextGlobalIDTNodeIndex(), callTarget, calleeMethodSymbol,
            callSite->_byteCodeIndex, callRatio * callTarget->_frequencyAdjustment, _idt->getRegion());

        _idt->increaseGlobalIDTNodeIndex();
        _idt->addCost(child->getCost());

        if (!comp()->incInlineDepth(
                calleeMethodSymbol, callSite->_bcInfo, callSite->_cpIndex, NULL, !callSite->isIndirectCall(), 0))
            continue;
...
uint32_t OMR::IDTBuilder::computeStaticBenefit(
...
void OMR::IDTBuilder::Visitor::visitCallSite(
    TR_CallSite* callSite, TR::Block* callBlock, TR::vector<TR::AbsValue*, TR::Region&>* arguments)
{
    float callRatio = (float)callBlock->getFrequency()
        / (float)_idtNode->getCallTarget()->_cfg->getStart()->asBlock()->getFrequency();
