...
public:
    ...
    {
        ((TR_DataFlowAnalysis*)ptr)->allocator().deallocate(ptr, size);
    ...
    {
        TR_ASSERT(0, "perform not implemented");
        return 0;
    }
...
#include "optimizer/DataFlowAnalysis.enum"
   ...
   virtual TR_ReachingDefinitions           *asReachingDefinitions();
    ...
    bool treeHasChecks(TR::TreeTop* tt, TR::Node* node = NULL)
    {
        ...
        return (tt->getNode()->exceptionsRaised() != 0 || (comp()->isPotentialOSRPointWithSupport(tt)));
    }
...
//
//
template<class Container> class TR_BasicDFSetAnalysis<Container*> : public TR_DataFlowAnalysis {
public:
    ...
    public:
        ...
        TR_ContainerNodeNumberPair()
            : _container(0)
            , _nodeNumber(-1)
        { }
        TR_ContainerNodeNumberPair(Container* b, int32_t n)
            : _container(b)
            , _nodeNumber(n)
        { }
    ...
    struct ExtraAnalysisInfo {
        ...
        TR_ContainerNodeNumberPair* getContainerNodeNumberPair(
            TR_LinkHead<TR_ContainerNodeNumberPair>* list, int32_t n);
...
//
//
template<class Container> class TR_ForwardDFSetAnalysis<Container*> : public TR_BasicDFSetAnalysis<Container*> {
public:
...
//
template<class Container> class TR_IntersectionDFSetAnalysis<Container*> : public TR_ForwardDFSetAnalysis<Container*> {
public:
...
public:
    TR_ReachingDefinitions(TR::Compilation* comp, TR::CFG* cfg, TR::Optimizer* optimizer, TR_UseDefInfo*,
        TR_UseDefInfo::AuxiliaryData& aux, bool trace);
...
private:
    void initializeGenAndKillSetInfoForNode(
        TR::Node* node, TR_BitVector& defsKilled, bool seenException, int32_t blockNum, TR::Node* parent);
...
//
//
template<class Container> class TR_BackwardDFSetAnalysis<Container*> : public TR_BasicDFSetAnalysis<Container*> {
public:
...
class TR_BackwardIntersectionDFSetAnalysis<Container*> : public TR_BackwardDFSetAnalysis<Container*> {
public:
...
class TR_BackwardIntersectionBitVectorAnalysis : public TR_BackwardIntersectionDFSetAnalysis<TR_BitVector*> {
public:
    typedef TR_BitVector ContainerType;
...
//
template<class Container>
class TR_BackwardUnionDFSetAnalysis<Container*> : public TR_BackwardDFSetAnalysis<Container*> {
public:
...
class TR_BackwardUnionBitVectorAnalysis : public TR_BackwardUnionDFSetAnalysis<TR_BitVector*> {
public:
...
class TR_BackwardUnionSingleBitContainerAnalysis : public TR_BackwardUnionDFSetAnalysis<TR_SingleBitContainer*> {
public:
    TR_BackwardUnionSingleBitContainerAnalysis(
        TR::Compilation* comp, TR::CFG* cfg, TR::Optimizer* optimizer, bool trace)
...
//
class TR_GlobalAnticipatability : public TR_BackwardIntersectionBitVectorAnalysis {
public:
...
//
class TR_Earliestness : public TR_UnionBitVectorAnalysis {
public:
...
//
class TR_Delayedness : public TR_IntersectionBitVectorAnalysis {
public:
...
//
class TR_Latestness : public TR_BackwardIntersectionBitVectorAnalysis {
public:
...
//
class TR_Isolatedness : public TR_BackwardIntersectionBitVectorAnalysis {
public:
...
public:
    ...
    TR_LiveVariableInformation(TR::Compilation* comp, TR::Optimizer*, TR_Structure*, bool splitLongs = false,
        bool includeParms = false, bool ignoreOSRUses = false);
    ...
    void findCommonedLoads(
        TR::Node* node, TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount);
    void findAllUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector* genSetInfo, TR_BitVector* killSetInfo,
        TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount);
    void findLocalUsesInBackwardsTreeWalk(TR::Node* node, int32_t blockNum, TR_BitVector* genSetInfo,
        TR_BitVector* killSetInfo, TR_BitVector* commonedLoads, vcount_t visitCount);

private:
    void visitTreeForLocals(TR::Node* node, TR_BitVector** blockGenSetInfo, TR_BitVector* blockKillSetInfo,
        bool movingForwardThroughTrees, bool visitEntireTree, vcount_t visitCount, TR_BitVector* commonedLoads,
        bool belowCommonedNode);

protected:
    virtual void findUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector** genSetInfo, TR_BitVector** killSetInfo,
        TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount);
    ...
    // once they are allocated via createGenAndKillSetCaches(), the next call to initializeGenAndKillSetInfo() will
    // store
    //   the sets into these caches and set _haveCachedGenAndKillSets.  Subsequent calls to
    //   initializeGenAndKillSetInfo() will change the arrays passed in to refer to the bit vectors in these caches.
...
public:
    ...
    TR_OSRLiveVariableInformation(TR::Compilation* comp, TR::Optimizer*, TR_Structure*, bool splitLongs = false,
        bool includeParms = false, bool ignoreOSRUses = false);
...
private:
    ...
    typedef std::map<int32_t, TR_BitVector*, TR_BCLiveSymbolsMapComparator, TR_BCLiveSymbolsMapAllocator>
        TR_BCLiveSymbolsMap;
...
public:
    TR_Liveness(TR::Compilation* comp, TR::Optimizer* optimizer, TR_Structure*, bool ignoreOSRUses = false,
...
public:
    TR_FlowSensitiveEscapeAnalysis(
        TR::Compilation* comp, TR::Optimizer* optimizer, TR_Structure* rootStructure, TR_EscapeAnalysis* e);
...
private:
    ...
    TR::TreeTop** _syncNodeTTForBlock; // For each BB, the treetop of a node that emits a sync (i.e. monexit or
                                       // preferably a volatile access)
