    ...
    , _trace(o->comp()->getOptions()->trace(optNum))
{
    if (_id < OMR::Optimizations::numGroups)
        TR_ASSERT_SAFE_FATAL(
            _id < OMR::Optimizations::numGroups, "The optimization id requested (%d) is too high", _id);

    // set flags if necessary
    switch (self()->id()) {
    case OMR::inlining:
        _flags.set(doesNotRequireAliasSets | canAddSymbolReference | verifyTrees | verifyBlocks | checkTheCFG
            | requiresAccurateNodeCount);
        break;
...
void OMR::OptimizationManager::setRequested(bool requested, TR::Block* block)
{
    ...
    if (requested && self()->optimizer()->canRunBlockByBlockOptimizations()) {
        if (block) {
            ...
            if (self()->id() == loopVersionerGroup)
                self()->optimizer()->setRequestOptimization(
                    lastLoopVersionerGroup, requested, toBlock(self()->comp()->getFlowGraph()->getStart()));
        }
...
void OMR::OptimizationManager::performChecks()
{
    ...
    if (self()->getCheckStructure() || self()->comp()->getOption(TR_EnableParanoidOptCheck)
        || debug("paranoidOptCheck"))
