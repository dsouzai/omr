    ...
    WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock, bool doThisCheck)
{
    ...
    while (cursor.hasMoreElements()) {
        ...
        //
        if (doThisCheck && next._succ.isEmpty() && next._originalBlock == _cfg->getEnd()) {
        ...
        // so if we come across the same node the second time (nodesInPath[next]) we could say that we are done with
        // this particular node and move on
 ...
 */
TR_RegionStructure* TR_RegionAnalysis::findNaturalLoop(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath)
{
    regionNodes.empty();
    ...
    while (cursor.hasMoreElements()) {
        ...
        if (_dominators.dominates(node._originalBlock, backEdgeNode._originalBlock)) {
            ...
            if (_useNew) {
                addNaturalLoopNodesIterativeVersion(
                    backEdgeNode, regionNodes, nodesInPath, cyclesFound, node._originalBlock);
            } else {
...
void TR_RegionAnalysis::addNaturalLoopNodesIterativeVersion(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock)
{
    ...
    while (!workStack.isEmpty()) {
        ...
        simpleIterator(workStack, next._pred, regionNodes, nodesInPath, cyclesFound, hdrBlock);
        // process the exception preds of next
        simpleIterator(workStack, next._exceptionPred, regionNodes, nodesInPath, cyclesFound, hdrBlock);
    }
}

void TR_RegionAnalysis::addNaturalLoopNodes(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock)
{
 ...
 * to justify it.
 */
TR_RegionStructure* TR_RegionAnalysis::findRegion(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath)
{
...
void TR_RegionAnalysis::addRegionNodesIterativeVersion(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock)
{
    // We will be pushing elements backwards to emulate processing in a depth-first order
    // Also, we will be pushing the same node twice in order to update nodesInPath correctly
    ...
    while (!workStack.isEmpty()) {
        ...
        simpleIterator(workStack, next._succ, regionNodes, nodesInPath, cyclesFound, hdrBlock, true);

        simpleIterator(workStack, next._exceptionSucc, regionNodes, nodesInPath, cyclesFound, hdrBlock);
    }
}

void TR_RegionAnalysis::addRegionNodes(
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock)
{
    int32_t index = node._nodeIndex;
    ...
    while (cursor.hasMoreElements()) {
        ...
        // block. It provides an exit for all other regions.
        //
        if ((next._succ.isEmpty()) && next._originalBlock == _cfg->getEnd()) {
            if (hdrBlock->getNumber() != 0)
                continue;
        }
        if (_dominators.dominates(hdrBlock, next._originalBlock))
 ...
 * flow graph.
 */
void TR_RegionAnalysis::buildRegionSubGraph(TR_RegionStructure* region, StructInfo& entryNode,
    WorkBitVector& regionNodes, SubGraphNodes& cfgNodes, TR::Region& memRegion)
{
        ...
        while (cursor.hasMoreElements()) {
            ...
            if (cfgNodes[toIndex] == NULL) {
                if (regionNodes.get(toIndex))
                    ...
                    cfgNodes[toIndex] = new (_structureMemoryRegion)
                        TR_StructureSubGraphNode(toNode._structure->getNumber(), _structureMemoryRegion);
            }
            to = cfgNodes[toIndex];
