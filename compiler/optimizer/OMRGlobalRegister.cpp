...
TR::Node* OMR::GlobalRegister::createStoreFromRegister(
    vcount_t visitCount, TR::TreeTop* prevTreeTop, int32_t i, TR::Compilation* comp, bool storeUnconditionally)
{
    ...
    TR::Node* store = TR::Node::createWithSymRef(
        comp->il.opCodeForDirectStore(rc->getDataType()), 1, 1, node, rc->getSymbolReference());
