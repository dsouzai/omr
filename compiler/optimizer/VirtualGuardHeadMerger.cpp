...
static bool isStopTheWorldGuard(TR::Node *node) { return node->isHCRGuard() || node->isOSRGuard(); }
...
static void collectColdPathLoads(TR::Block *firstBlock, TR_BitVector &coldPathLoads, TR::Compilation *comp)
{
    ...
    while (!blocksToCheck.isEmpty()) {
        ...
        for (TR::TreeTop *tt = block->getFirstRealTreeTop(); tt->getNode()->getOpCodeValue() != TR::BBEnd;
...
static bool safeToMoveGuard(TR::Block *destination, TR::TreeTop *guardCandidate, TR::TreeTop *branchDest,
    TR_BitVector &privArgSymRefs, TR::Compilation *comp)
{
    ...
    if (guardCandidate->getNode()->isHCRGuard()) {
        ...
        for (TR::TreeTop *tt = start; tt && tt != guardCandidate; tt = tt->getNextTreeTop()) {
            ...
            // ahead of the guard
            if (tt->getNode()->getOpCodeValue() != TR::BBStart && tt->getNode()->getOpCodeValue() != TR::BBEnd
                && !tt->getNode()->chkIsPrivatizedInlinerArg()
                && !(tt->getNode()->getOpCode().hasSymbolReference()
                    && tt->getNode()->getSymbol()->holdsMonitoredObject())
                && !tt->getNode()->isNopableInlineGuard())
                return false;

            if (tt->getNode()->chkIsPrivatizedInlinerArg()
                && (disablePrivArgMovement ||
                    // If the priv arg is not for this guard
                    (guardCandidate->getNode()->getInlinedSiteIndex() > -1 &&
                        // if priv arg store does not have the same inlined site index as the guard's caller, that means
                        // it is not a priv arg for this guard, then we cannot move the guard and its priv args up
                        // across other calls' priv args
                        tt->getNode()->getInlinedSiteIndex()
                            != comp->getInlinedCallSite(guardCandidate->getNode()->getInlinedSiteIndex())
                                   ._byteCodeInfo.getCallerIndex())))
                return false;

            if (tt->getNode()->chkIsPrivatizedInlinerArg())
                privArgSymRefs.set(tt->getNode()->getSymbolReference()->getReferenceNumber());

            if (tt->getNode()->isNopableInlineGuard() && tt->getNode()->getBranchDestination() != branchDest)
                return false;
        }
    }
    return true;
...
int32_t TR_VirtualGuardHeadMerger::perform()
{
    ...
    for (TR::Block *block = optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock(); block;
        ...
        if (isMergeableGuard(guard1)) {
            ...
            if (!disableVGHeadMergerTailSplitting && (cold1->getSuccessors().size() == 1)
                ...
                if ((block->getNextBlock()->getSuccessors().size() == 1
                    ...
                    && performTransformation(comp(),
                        "%sCloning block_%d and placing clone after block_%d to reduce HCR guard nops\n", OPT_DETAILS,
                        block->getNextBlock()->getNumber(), cold1->getNumber()))
            ...
            for (TR::Block *nextBlock = block->getNextBlock(); nextBlock; nextBlock = nextBlock->getNextBlock()) {
                if (block->nodeIsRemoved() || nextBlock->nodeIsRemoved()) {
                    if (trace())
                        traceMsg(comp(), "block block_%d %p (%d) or nextBlock block_%d %p (%d) has been removed\n",
                            block->getNumber(), block, block->nodeIsRemoved(), nextBlock->getNumber(), nextBlock,
                            nextBlock->nodeIsRemoved());
                    break;
                }

                if (!(nextBlock->getPredecessors().size() == 1) || !nextBlock->hasPredecessor(block)) {
                    break;
                }
                ...
                if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
                    if (trace())
                        traceMsg(comp(),
                            "  Guard1 [%p] is guarding the same call as Guard2 [%p] - proceeding with guard merging\n",
                            guard1, guard2);
                } else if (guard2->getInlinedSiteIndex() > -1
                    && guard1->getInlinedSiteIndex()
                        == comp()->getInlinedCallSite(guard2->getInlinedSiteIndex())._byteCodeInfo.getCallerIndex()) {
                    if (trace())
                        traceMsg(comp(), "  Guard1 [%p] is the caller of Guard2 [%p] - proceeding with guard merging\n",
                            guard1, guard2);
                } else if ((cold1->getSuccessors().size() == 1) && cold1->hasSuccessor(cold2)) {
                    if (trace())
                        traceMsg(comp(),
                            "  Guard1 cold destination block_%d has guard2 cold destination block_%d as its only "
                            "successor - proceeding with guard merging\n",
                            cold1->getNumber(), cold2->getNumber());
                } else {
                    if (trace())
                        traceMsg(comp(),
                            "  Cold1 block_%d and cold2 block_%d of guard2 [%p] in unknown relationship - abandon the "
                            "merge attempt\n",
                ...
                if (!isStopTheWorldGuard(guard2)) {
                    ...
                    if (coldPathLoads.intersects(privArgSymRefs)) {
                        if (trace())
                            traceMsg(comp(),
                                "  Recycled temp live in cold1 block_%d and used as privarg before guard2 [%p] - stop "
                                "guard merging",
                                cold1->getNumber(), guard2);
                        break;
                    }
                }

                if (!performTransformation(comp(),
                        "%sRedirecting %s guard [%p] in block_%d to parent guard cold block_%d\n", OPT_DETAILS,
                        isStopTheWorldGuard(guard2) ? "stop the world" : "runtime", guard2, guard2Block->getNumber(),
                        cold1->getNumber()))
                    continue;
                ...
                if (guard2Tree != guard2Block->getFirstRealTreeTop()) {
                    ...
                    // 1, it might have side effect to runtime guards after it, moving it up might cause us to falsely
                    // merge
                    ...
                    if (!isStopTheWorldGuard(guard2)) {
                        ...
                        // Monitor store is generated for the caller of the method guard2 protects, so should appear
                        // before priv arg stores for the method guard2 protects
                ...
                if (insertPoint != guard2Block->getPrevBlock()) {
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "headMerger/%s_%s/(%s)",
                            isStopTheWorldGuard(guard1) ? "stop the world" : "runtime",
                            isStopTheWorldGuard(guard2) ? "stop the world" : "runtime", comp()->signature()));
                    cfg->setStructure(NULL);

                    block = nextBlock = guard2Block->getPrevBlock();
                    if (trace())
                        traceMsg(comp(), "  Moving guard2 block block_%d after block_%d\n", guard2Block->getNumber(),
                            insertPoint->getNumber());
...
void TR_VirtualGuardHeadMerger::tailSplitBlock(TR::Block *block, TR::Block *cold1)
{
    ...
    for (auto e = block->getNextBlock()->getExceptionSuccessors().begin();
         e != block->getNextBlock()->getExceptionSuccessors().end(); ++e)
    ...
    if (tailSplitEnd->getOpCode().isGoto()) {
        tailSplitEnd->setBranchDestination(
            block->getNextBlock()->getLastRealTreeTop()->getNode()->getBranchDestination());
        ...
        gotoBlock->append(TR::TreeTop::create(comp(),
            TR::Node::create(tailSplitEnd, TR::Goto, 0, block->getNextBlock()->getExit()->getNextTreeTop())));
        cfg->addEdge(tailSplitBlock, gotoBlock);
        cfg->addEdge(tailSplitBlock,
            tailSplitBlock->getLastRealTreeTop()->getNode()->getBranchDestination()->getEnclosingBlock());
        cfg->addEdge(gotoBlock, block->getNextBlock()->getNextBlock());
    } else if (!tailSplitEnd->getOpCode().isReturn() && !tailSplitEnd->getOpCode().isJumpWithMultipleTargets()
        && tailSplitEnd->getOpCodeValue() != TR::athrow
        && !(tailSplitEnd->getNumChildren() >= 1 && tailSplitEnd->getFirstChild()->getOpCodeValue() == TR::athrow)) {
        tailSplitBlock->append(TR::TreeTop::create(comp(),
            TR::Node::create(tailSplitEnd, TR::Goto, 0, block->getNextBlock()->getExit()->getNextTreeTop())));
        cfg->addEdge(tailSplitBlock, block->getNextBlock()->getNextBlock());
    } else {
        for (auto e = block->getNextBlock()->getSuccessors().begin(); e != block->getNextBlock()->getSuccessors().end();
             ++e)
