...
uint8_t* TR_TranslateTable::data() { return _table->data; }

TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::table() { return _table; }

void TR_TranslateTable::setTable(TR_TranslateTableData* match) { _table = match; }

void TR_TranslateTable::dumpTable()
{
    int count = tableSize(table()->inSize, table()->outSize);
    dumpOptDetails(comp(), "\n\nTranslation table at address %p, size %d\n", data(), count);
    dumpOptDetails(comp(), "\n  Range %d to %d and %d to %d default value %d\n", table()->startA, table()->endA,
        table()->startB, table()->endB, table()->defaultValue);
    if (table()->outSize == 16) {
...
TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::createTable(
    uint32_t startA, uint32_t endA, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
    return createTable(startA, endA, 0, 0, inputSize, outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::createTable(
    uint16_t startA, uint16_t endA, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
    return createTable((uint32_t)startA, (uint32_t)endA, 0, 0, inputSize, outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::createTable(uint16_t startA, uint16_t endA,
    uint16_t startB, uint16_t endB, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
    return createTable(
        (uint32_t)startA, (uint32_t)endA, (uint32_t)startB, (uint32_t)endB, inputSize, outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::createTable(uint32_t startA, uint32_t endA,
    uint32_t startB, uint32_t endB, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
...
TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::matchTable(uint32_t startA, uint32_t endA, uint32_t startB,
    uint32_t endB, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
    ...
    while (curr) {
        if (curr->startA == startA && curr->endA == endA && curr->startB == startB && curr->endB == endB
            && curr->inSize == inputSize && curr->outSize == outputSize && curr->defaultValue == defaultValue) {
            return curr;
        }
        curr = curr->next;
    }
...
TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::matchTable(
    uint8_t inputSize, uint8_t outputSize, void* array)
{
...
TR_RangeTranslateTable::TR_RangeTranslateTable(
    TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint16_t start, uint16_t end, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(start, end, inputSize, outputSize, defaultValue);
}
TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize,
    uint16_t startA, uint16_t endA, uint16_t startB, uint16_t endB, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
...
TR_RangeTranslateTable::TR_RangeTranslateTable(
    TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint32_t start, uint32_t end, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(start, end, inputSize, outputSize, defaultValue);
}
...
TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize,
    uint32_t startA, uint32_t endA, uint32_t startB, uint32_t endB, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(startA, endA, startB, endB, inputSize, outputSize, defaultValue);
}

TR_SetTranslateTable::TR_SetTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint8_t set[])
    : TR_TranslateTable(comp) // input and output size in bits
{
...
TR_SetTranslateTable::TR_SetTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint16_t set[])
    : TR_TranslateTable(comp) // input and output size in bits
{
...
TR_SetTranslateTable::TR_SetTranslateTable(
    TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, void* set, int copyElemSize)
    : TR_TranslateTable(comp) // input and output size in bits
{
    int tableSizeParm = tableSize(inputSize, outputSize);
    int tableByteSize = tableSizeParm * (outputSize / 8);
