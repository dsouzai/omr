...
bool TR_BackwardDFSetAnalysis<Container*>::canGenAndKillForStructure(TR_Structure* nodeStructure)
{
...
template<class Container>
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForBlock(TR_BlockStructure* s)
{
    ...
    for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
        ...
        typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = new (this->trStackMemory())
            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(NULL, succBlock->getNumber());
...
template<class Container>
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(
    TR_RegionStructure* regionStructure, TR_BitVector& pendingList, TR_BitVector& exitNodes, bool lastIteration)
{
    while (this->_analysisQueue.getListHead()
        ...
        TR_BitVector seenNodes(this->comp()->trMemory()->currentStackRegion());
        {
            ...
            for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
                if (!regionStructure->isExitEdge(succ)) {
                    ...
                    if ((regionStructure->getEntry() == succNode) || exitNodes.get(succNode->getNumber()))
                        continue;
                                                                                                 ...
                                                                                                 (regionStructure->getEntry() != succNode)*/)
                  {
                    ...
                    if (pendingList.get(succStructure->getNumber())
                        && !succStructure->hasBeenAnalyzedBefore()) //_firstIteration)
                    {
                        TR_ASSERT(0, "Should not reach here\n");
                    } else {
                        seenSuccNodes.set(succStructure->getNumber());
                        Container* succBitVector = NULL;
                        typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* succInfo
                            = this->getAnalysisInfo(succStructure);
                    }
                }
                firstSucc = false;
            }
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            ...
            TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_killSetInfo
                ...
                *_regionKillSetInfo
                = normalSucc ? regionAnalysisInfo->_regularKillSetInfo : regionAnalysisInfo->_exceptionKillSetInfo,
                *_currentKillSetInfo = normalSucc ? analysisInfo->_currentRegularKillSetInfo
                                                  : regionAnalysisInfo->_currentExceptionKillSetInfo,
                *_genSetInfo = normalSucc ? analysisInfo->_regularGenSetInfo : analysisInfo->_exceptionGenSetInfo,
                *_regionGenSetInfo
                = normalSucc ? regionAnalysisInfo->_regularGenSetInfo : regionAnalysisInfo->_exceptionGenSetInfo,
                *_currentGenSetInfo = normalSucc ? analysisInfo->_currentRegularGenSetInfo
                                                 : regionAnalysisInfo->_currentExceptionGenSetInfo;
            ...
            if (!seenSuccNodes.get(nodeNumber)) {
                ...
                if (node == regionStructure->getEntry()) {
                    ...
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(NULL, nodeNumber);
            ...
            typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* succInfo
                = this->getAnalysisInfo(succ->getTo()->asStructureSubGraphNode()->getStructure());
            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* nodePair;
            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* killNodePair
                = succInfo->_currentRegularKillSetInfo->getFirst();
            for (nodePair = succInfo->_currentRegularGenSetInfo->getFirst(); nodePair;
                 nodePair = nodePair->getNext(), killNodePair = killNodePair->getNext()) {
                ...
                if (node == regionStructure->getEntry()) {
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                        = regionAnalysisInfo->getContainerNodeNumberPair(
                            regionAnalysisInfo->_regularGenSetInfo, nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = initializeInfo(NULL);
                        }
                    ...
                    pair = regionAnalysisInfo->getContainerNodeNumberPair(
                        regionAnalysisInfo->_regularKillSetInfo, nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = inverseInitializeInfo(NULL);
                        }
                    ...
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                        = analysisInfo->getContainerNodeNumberPair(
                            analysisInfo->_currentRegularGenSetInfo, nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
                                NULL, nodePair->_nodeNumber);
                        analysisInfo->_currentRegularGenSetInfo->add(pair);
                    }
                    if (!pair->_container) {
                        pair->_container = initializeInfo(NULL);
                    ...
                    pair = analysisInfo->getContainerNodeNumberPair(
                        analysisInfo->_currentRegularKillSetInfo, nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
                                NULL, nodePair->_nodeNumber);
            ...
            for (nodePair = succInfo->_currentExceptionGenSetInfo->getFirst(); nodePair;
                 nodePair = nodePair->getNext(), killNodePair = killNodePair->getNext()) {
                if (nodePair->_container)
                    *this->_temp = *(nodePair->_container);
                else
                ...
                if (node == regionStructure->getEntry()) {
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                        = regionAnalysisInfo->getContainerNodeNumberPair(
                            regionAnalysisInfo->_exceptionGenSetInfo, nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = initializeInfo(NULL);
                        }
                    ...
                    pair = regionAnalysisInfo->getContainerNodeNumberPair(
                        regionAnalysisInfo->_exceptionKillSetInfo, nodePair->_nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                            pair->_container = inverseInitializeInfo(NULL);
                        }
                    ...
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                        = analysisInfo->getContainerNodeNumberPair(
                            analysisInfo->_currentExceptionGenSetInfo, nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
                                NULL, nodePair->_nodeNumber);
                    ...
                    pair = analysisInfo->getContainerNodeNumberPair(
                        analysisInfo->_currentExceptionKillSetInfo, nodePair->_nodeNumber);
                    if (!pair) {
                        pair = new (this->trStackMemory())
                            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
                                NULL, nodePair->_nodeNumber);
...
template<class Container>
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure* region)
{
            ...
            if (!exitNodes.get(fromStructureNumber)) {
                ...
                typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                    = new (this->trStackMemory())
                        typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(b, fromStructureNumber);
                analysisInfo->_regularGenSetInfo->add(pair);
...
template<class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeRegionStructure(
    TR_RegionStructure* regionStructure, bool checkForChange)
{
    ...
    if (this->supportsGenAndKillSets()
        && (canGenAndKillForStructure(regionStructure) && regionStructure != this->_cfg->getStructure())) {
...
template<class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeNodeIfSuccessorsAnalyzed(
    TR_RegionStructure* regionStructure, TR_BitVector& pendingList, TR_BitVector& exitNodes)
{
    ...
    while (this->_analysisQueue.getListHead()
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            if (!regionStructure->isExitEdge(succ)) {
                ...
                if (pendingList.get(succStructure->getNumber())
                    && !succStructure->hasBeenAnalyzedBefore()) // this->_firstIteration)
                {
                    if (firstSucc)
        ...
        if (exitNodes.get(nodeStructure->getNumber())) {
            if (!(nodeStructure->getSuccessors().empty() && nodeStructure->getExceptionSuccessors().empty())) {
                ...
                for (pair = regionAnalysisInfo->_outSetInfo->getFirst(); pair; pair = pair->getNext()) {
                    ...
                    for (otherPair = analysisInfo->_outSetInfo->getFirst(); otherPair;
                         otherPair = otherPair->getNext()) {
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            if (nodeStructure->getStructure()->asRegion()
                ...
                Container* killBitVector
                    = analysisInfo->getContainer(analysisInfo->_regularKillSetInfo, node->getNumber());
                Container* genBitVector
                    = analysisInfo->getContainer(analysisInfo->_regularGenSetInfo, node->getNumber());
                Container* exceptionKillBitVector
                    = analysisInfo->getContainer(analysisInfo->_exceptionKillSetInfo, node->getNumber());
                Container* exceptionGenBitVector
                    = analysisInfo->getContainer(analysisInfo->_exceptionGenSetInfo, node->getNumber());

                typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* nodePair;
                for (nodePair = analysisInfo->_regularGenSetInfo->getFirst(); nodePair;
                     nodePair = nodePair->getNext()) {
                ...
                for (nodePair = analysisInfo->_exceptionGenSetInfo->getFirst(); nodePair;
                     nodePair = nodePair->getNext()) {
                    if (!backEdgeNodes.get(nodePair->_nodeNumber))
                        continue;
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            ...
            for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
                ...
                Container* killBitVector = analysisInfo->getContainer(
                    normalSucc ? analysisInfo->_regularKillSetInfo : analysisInfo->_exceptionKillSetInfo, nodeNumber);
                Container* genBitVector = analysisInfo->getContainer(
                    normalSucc ? analysisInfo->_regularGenSetInfo : analysisInfo->_exceptionGenSetInfo, nodeNumber);
...
template<class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    ...
    //
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
        = NULL; // this->getAnalysisInfo(blockStructure);
    ...
    if (block == this->_cfg->getEnd()) {
        ...
        for (auto succ = block->getExceptionSuccessors().begin(); succ != block->getExceptionSuccessors().end();
             ++succ) {
            succBlock = (*succ)->getTo();
    ...
    if (blockNum != 0) {
        ...
        *analysisInfo->_inSetInfo = *this->_regularInfo;
        if (!this->_blockAnalysisInfo[blockStructure->getNumber()])
            this->allocateBlockInfoContainer(
                &this->_blockAnalysisInfo[blockStructure->getNumber()], this->_regularInfo);
...
template<class Container>
void TR_BackwardDFSetAnalysis<Container*>::analyzeNode(
    TR::Node* node, vcount_t visitCount, TR_BlockStructure* blockStructure, Container* _analysisInfo)
{ }

template<class Container>
void TR_BackwardDFSetAnalysis<Container*>::analyzeTreeTopsInBlockStructure(TR_BlockStructure* blockStructure)
{
    TR::Block* block = blockStructure->getBlock();
