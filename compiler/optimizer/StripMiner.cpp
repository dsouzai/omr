...
void TR_StripMiner::collectLoops(TR_Structure* str)
{
    ...
    if (region->isNaturalLoop()) {
        ...
        li->_branchToExit = !region->contains(
            branch->getBranchDestination()->getNode()->getBlock()->getStructureOf(), region->getParent());
        ...
        //  It is ok in case where if the stores are in different paths of the code. Then the value is equal to the
        //  deltaOnBackEdge
        ...
        if ((li->_preOffset != 0) || (li->_postOffset != 0)) {
            if (trace())
                traceMsg(comp(), "\tReject loop %d ==> pre offset = %d, post offsets = %d\n", region->getNumber(),
                    li->_preOffset, li->_postOffset);
            // printf("reject %s: pre %d post %d offset\n",comp()->signature(),li->_preOffset,
            // li->_postOffset);fflush(stdout);
            return;
        }
        ...
        if (li->_preOffset + li->_postOffset >= li->_stripLen) {
            if (trace())
                traceMsg(comp(), "\tReject loop %d ==> pre offset = %d, post offsets = %d, strip length = %d\n",
        ...
        if (trace()) {
            ...
            traceMsg(comp(), "\t\tpre-header = %d, loop test = %d, primary induction variable = %d\n",
                preHeader->getNumber(), loopTest->getNumber(), piv->getSymRef()->getReferenceNumber());
            traceMsg(comp(), "\t\tpre-offset = %d, post-offset = %d, offset = %d, strip length = %d\n", li->_preOffset,
                li->_postOffset, li->_offset, li->_stripLen);
...
void TR_StripMiner::transformLoops()
{
    ...
    for (LoopInfo* li = it.getFirst(); li; li = it.getNext()) {
        if (li != NULL && performTransformation(comp(), "%sTransforming loop %d\n", OPT_DETAILS, li->_regionNum)) {
            ...
            // li->_offset, li->_postOffset, li->_preOffset, li->_increasing);
        }
    }
}
...
void TR_StripMiner::duplicateLoop(LoopInfo* li, TR_ClonedLoopType type)
{
    ...
    if (pEnv && (type == offsetLoop && li->_needOffsetLoop)) {
    ...
    for (block = bIt.getFirst(); block; block = bIt.getNext()) {
        ...
        if (block == li->_loopTest && !isMainOrResidual) {
            if (trace())
                traceMsg(
                    comp(), "\tskipping edge from non-main non-residual loop test block [%d]\n", block->getNumber());
            continue;
        }
...
void TR_StripMiner::transformLoop(LoopInfo* li)
{
    ...
    bool isInt32 = ltTree->getNode()->getOpCode().isBooleanCompare()
        && // isCompare only returns true for signed compare -- a better name would be isCompareArith()
    ...
    if (limitNode->getOpCode().isLoadConst()) {
        intptr_t limit = limitNode->getType().isInt32() ? limitNode->getInt() : limitNode->getLongInt();
        if ((li->_increasing && limit < limit + li->_stripLen) || (!li->_increasing && limit > limit - li->_stripLen)) {
            if (trace())
                traceMsg(comp(), "No overflow test for %s loop %d: limit = %ld, stripLength = %ld\n",
                    li->_increasing ? "increasing" : "decreasing", loop->getNumber(), limit, li->_stripLen);
            testOverflow = false;
        }
    }
    ...
    if (testOverflow) {
        ...
        overflowNode = isInt32 ? TR::Node::iconst(phNode, (int32_t)constVal) : TR::Node::lconst(phNode, constVal);
    ...
    //
    TR::Block* outerHeader = TR::Block::createEmptyBlock(clonedPhNode, comp(), clonedPh->getFrequency(), clonedPh);
        ...
        if (testOverflow) {
            ...
            if (trace()) {
                traceMsg(comp(),
                    ...
                    ifOverBlock->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(),
                    offsetLoopPh->getNumber());
            }
        } else {
    ...
    if (li->_preOffset > 0) {
        ...
        if (trace()) {
            traceMsg(comp(),
                ...
                outerHeader->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(), pre->getNumber());
        }
        ...
        if (trace())
            traceMsg(comp(),
                "\t   adding edges: outer header [%d] => goto [%d]; "
                "goto [%d] => branch [%d]\n",
                outerHeader->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(), branchBlock->getNumber());
    }
...
TR::Block* TR_StripMiner::createStartOffsetLoop(LoopInfo* li, TR::Block* outerHeader)
{
    ...
    TR::Node* zeroNode = isInt32 ? TR::Node::iconst(newLtNode, 0) : TR::Node::lconst(newLtNode, (int64_t)0);
    ...
    if (trace())
        traceMsg(
            comp(), "\t   adding edge: test block [%d] => exit [%d]\n", ltBlock->getNumber(), origExit->getNumber());
...
TR::Block* TR_StripMiner::stripMineLoop(LoopInfo* li, TR::Block* outerHeader)
{
    ...
    TR::Node* constNode
        = isInt32 ? TR::Node::iconst(phNode, (int32_t)(constVal - 1)) : TR::Node::lconst(phNode, constVal - 1);
    ...
    if (TR::ILOpCode::isNotEqualCmp(ltTree->getNode()->getOpCodeValue())
        ...
        cmpOpCode = li->_branchToExit ? ltTree->getNode()->getOpCode().getOpCodeForReverseBranch()
    ...
    if (trace())
        traceMsg(
            comp(), "\tcreating striplength test [%p] in outer loop block [%d]\n", ifNode, branchBlock->getNumber());
    ...
    if (trace()) {
        traceMsg(comp(),
            ...
            branchBlock->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(), residualPh->getNumber());
    }

    ///////////////////////////////////////////////////

    /* Create a new induction variable for the strip mined loops */
    TR::SymbolReference* newCounter
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), isInt32 ? TR::Int32 : TR::Int64);
    ...
    if (trace())
        traceMsg(
            comp(), "\t   changed main loop test [%p] to j < (strip length - pre - post)\n", mainLtTree->getNode());
        ...
        if ((block != lt) && (block->getNumber() < _nodesInCFG)) {
            for (exitEdge = eItExit.getFirst(); exitEdge; exitEdge = eItExit.getNext()) {
                if (exitEdge->getFrom()->getNumber() == block->getNumber()) {
                    ...
                    freq
                        = (block->getFrequency() < dest->getFrequency()) ? block->getFrequency() : dest->getFrequency();

                    TR::Block* fromMainBlock = _mainBlockMapper[block->getNumber()];
                    TR::Block* fromResidualBlock = _residualBlockMapper[block->getNumber()];
                    TR::Block* updateBeforeExitBlock
                        = TR::Block::createEmptyBlock(dest->getEntry()->getNode(), comp(), freq, block);
                    ...
                    TR::TreeTop* iStoreTree
                        = TR::TreeTop::create(comp(), TR::Node::createStore(li->_piv->getSymRef(), iAddNode));
...
TR::Block* TR_StripMiner::createGotoBlock(TR::Block* source, TR::Block* dest)
{
    ...
    int32_t freq = (source->getFrequency() < dest->getFrequency()) ? source->getFrequency() : dest->getFrequency();
...
void TR_StripMiner::redirect(TR::Block* source, TR::Block* oldDest, TR::Block* newDest)
{
    ...
    } else if ((branchNode->getOpCode().isBranch() || branchNode->getOpCode().isGoto())
        && (branchNode->getBranchDestination()->getNode()->getBlock() == oldDest)) {
        if (trace())
    ...
    if (found) {
        ...
        if (trace()) {
            traceMsg(comp(), "\t      adding edges: source [%d] => goto [%d]; goto [%d] => new dest [%d]\n",
                source->getNumber(), gotoBlock->getNumber(), gotoBlock->getNumber(), newDest->getNumber());
        }
    }
}
...
TR::Block* TR_StripMiner::createLoopTest(LoopInfo* li, TR_ClonedLoopType type)
{
    ...
    TR::Node* storeNode = TR::Node::createStore(tempSymRef, TR::Node::create(opCode, 2, iNode, offsetNode));
    ...
    if (trace())
        traceMsg(
            comp(), "\t created a new block [%d] to test if i < temp [%p]\n", ifBlock->getNumber(), ifTree->getNode());
...
{
    if (trace())
        traceMsg(comp(), "   analyze %s loop %d...\n", type == mainLoop ? "main" : "residual", li->_regionNum);
    ...
    for (block = bIt.getFirst(); block; block = bIt.getNext()) {
        if (checkClone)
            block = type == mainLoop ? _mainBlockMapper[block->getNumber()] : _residualBlockMapper[block->getNumber()];
        ...
        while (currentTree != exitTree) {
            ...
            if (comp()->generateArraylets() && (!li->_canMoveAsyncCheck || li->_arrayDataSize < 0))
                return;
...
void TR_StripMiner::examineNode(LoopInfo* li, TR::Node* parent, TR::Node* node, TR::SymbolReference* oldSymRef,
    vcount_t visitCount, TR_ClonedLoopType type, bool checkClone, int32_t childNum)
{
    ...
    if (checkClone && node->getSymbolReference() == oldSymRef) {
        if (node->getOpCode().isLoad()) {
            ...
            type == mainLoop ? li->_mainParentsOfStores.add(new (trStackMemory()) TR_ParentOfChildNode(parent, -1))
    ...
    if (!checkClone) {
        ...
        if (comp()->generateArraylets() && (li->_arrayDataSize < 0 || li->_preOffset < 0 || li->_postOffset < 0))
            return;
        ...
        if (symbol && (symbol->isArrayShadowSymbol())) //|| symbol->isArrayletShadowSymbol()))
        {
                    ...
                    if (node->getFirstChild()->getOpCode().isArrayRef()) {
                        ...
                        if (secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub()) {
                            if (secondChild->getFirstChild()->getOpCode().isMul()
                                ...
                                if (firstChild->getOpCode().isRightShift() || firstChild->getOpCode().isDiv()) {
                                    ...
                                    if ((accessNode->getOpCodeValue() == TR::iload)
                                        ...
                                        if (accessNode->getOpCodeValue() != TR::iload) {
                                            if ((accessNode->getOpCode().isAdd() || accessNode->getOpCode().isSub())
                                        ...
                                        else // if iload
                                        {
                                            if (trace())
                                                traceMsg(
                                                    comp(), "Found iload %p which is not piv ... skipping it\n", node);
                                        }
                                    }
                                } else {
                                    if (trace())
...
{
    if (node->getOpCode().isLoad() && (node->getSymbolReference() == newSymRef))
        return node;
...
void TR_StripMiner::replaceLoopPivs(
    LoopInfo* li, TR::ILOpCodes newOpCode, TR::Node* newConst, TR::SymbolReference* newSymRef, TR_ClonedLoopType type)
{
    ...
    if (trace())
        traceMsg(
            comp(), "   replacing Pivs in %s loop %d...\n", type == mainLoop ? "main" : "residual", li->_regionNum);
    ...
    for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
        ...
        if ((parentNode->getOpCode().isRightShift() || parentNode->getOpCode().isDiv())
            ...
            value = (parentNode->getSecondChild()->getType().isInt32()) ? parentNode->getSecondChild()->getInt()
                                                                        : parentNode->getSecondChild()->getLongInt();
            if (value == comp()->fe()->getArraySpineShift(static_cast<int32_t>(li->_arrayDataSize)))
                skipNode = true;
        } else if (parentNode->getOpCode().isAnd() && parentNode->getSecondChild()->getOpCode().isLoadConst()) {
            value = (parentNode->getSecondChild()->getType().isInt32()) ? parentNode->getSecondChild()->getInt()
                                                                        : parentNode->getSecondChild()->getLongInt();
...

        {
            ...
            if (loadNode == NULL) {
                ...
                loadPair = new (comp()->trStackMemory())
                    TR_Pair<TR::Node, TR::Node>(parentNode->getChild(childNum), loadNode);
...
TR::Block* TR_StripMiner::getLoopTest(TR_Structure* str, TR::Block* preHeader)
{
    ...
    //
    if (loopTest && !loopTest->getLastRealTreeTop()->getNode()->getOpCode().isBooleanCompare()) {
        if (trace())
            ...
            if (node->getOpCode().isStore() && node->getSymbolReference() == oldCounter) {
                if (((node->getFirstChild()->getOpCode().isSub()) || (node->getFirstChild()->getOpCode().isAdd()))
                    ...
                    if (pivIncInStore != li->_piv->getDeltaOnBackEdge()) {
                        if (trace())
                            traceMsg(comp(),
                                "\t loop %d ==> Found a store to induction variable with increment different than "
                                "deltaObBackEdge\n",
...
bool TR_StripMiner::findPivInSimpleForm(TR::Node* node, TR::SymbolReference* pivSym)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        if ((node->getOpCode().getOpCodeValue() == TR::iload) && (node->getSymbolReference() == pivSym))
