    ...
    TR::SymbolReference *targetRef, bool castToIntegral)
{
    ...
    if (comp->target().cpu.isPower())
        if (dataType == TR::Int64 && comp->target().is64Bit()) {
            ...
            if (firstChild->getNumChildren() == 2) {
                ...
                TR_ASSERT(offsetChild->getOpCodeValue() != TR::iconst,
                    "iconst shouldn't be used for 64-bit array indexing");
    ...
    if (performTransformation(comp, "%sScalarize arraycopy 0x%p\n", OPT_DETAILS, node)) {
        TR::Node *store = TR::TransformUtil::scalarizeAddressParameter(comp, node->getSecondChild(), byteLen, dataType,
            targetRef, true);
        TR::Node *load = TR::TransformUtil::scalarizeAddressParameter(comp, node->getFirstChild(), byteLen, dataType,
            sourceRef, false);
...
void OMR::TransformUtil::transformCallNodeToPassThrough(TR::Optimization *opt, TR::Node *node, TR::TreeTop *anchorTree,
    TR::Node *child)
{
...
TR::Node *OMR::TransformUtil::generateArrayElementAddressTrees(TR::Compilation *comp, TR::Node *arrayNode,
    TR::Node *offsetNode)
{
...
TR::Node *OMR::TransformUtil::generateConvertArrayElementIndexToOffsetTrees(TR::Compilation *comp, TR::Node *indexNode,
    TR::Node *elementSizeNode, int32_t elementSize, bool useShiftOpCode)
{
