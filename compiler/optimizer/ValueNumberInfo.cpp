...
TR_ValueNumberInfo::TR_ValueNumberInfo(TR::Compilation *comp, TR::Optimizer *optimizer, bool requiresGlobals,
    bool prefersGlobals, bool noUseDefInfo)
    : _compilation(comp)
    ...
    , _nextInRing(comp->allocator())
{
    ...
    _hashTable._buckets = (CollisionEntry **)trMemory()->allocateStackMemory(
        _hashTable._numBuckets * sizeof(CollisionEntry *), TR_Memory::ValueNumberInfo);
...
bool TR_ValueNumberInfo::congruentNodes(TR::Node *node, TR::Node *entryNode)
{
    ...
    if (node->getType().isBCD()) {
        if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
            if (trace())
                traceMsg(comp(),
                    "BCD node %s (%p) and BCD entryNode %s (%p) have size/shape mismatch -- do not consider as "
                    "matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            return false;
        }
        ...
        if (!node->isSignStateEquivalent(entryNode)) {
            if (trace() || comp()->cg()->traceBCDCodeGen())
                traceMsg(comp(),
                    "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- do not consider as "
                    "matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            return false;
        }
        ...
        if (trace())
            traceMsg(comp(),
                "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch -- do not consider as "
                "matching\n",
                node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
        return false;
    } else if (node->chkOpsCastedToBCD() && node->castedToBCD() != entryNode->castedToBCD()) {
        if (trace())
            traceMsg(comp(),
                "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD %d -- do not "
                "consider as matching\n",
                node->getOpCode().getName(), node, node->castedToBCD(), entryNode->getOpCode().getName(), entryNode,
                entryNode->castedToBCD());
        return false;
    }
#endif
    ...
    if (node->getOpCode().isLoadVar() && node->getSymbolReference() == entryNode->getSymbolReference()
        && (node->getSymbol()->isConst() || node->getSymbol()->isConstObjectRef())) {
        return true;
    }
    ...
    */
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (symRef && entrySymRef && symRef->getSymbol() == entrySymRef->getSymbol()
    ...
    if (node->getOpCode().isLoadConst()) {
        ...
        switch (node->getDataType()) {
            case TR::Int8:
                isSame = (node->getByte() == entryNode->getByte());
                break;
            case TR::Int16:
                isSame = (node->getShortInt() == entryNode->getShortInt());
                break;
            case TR::Int32:
                isSame = (node->getInt() == entryNode->getInt());
                break;
            case TR::Float:
                isSame = (node->getFloatBits() == entryNode->getFloatBits());
                break;
            case TR::Int64:
            case TR::Double:
                isSame = (node->getLongInt() == entryNode->getLongInt());
                break;
            case TR::Address:
                isSame = (node->getAddress() == entryNode->getAddress());
                break;
            default: {
                if (
#ifdef J9_PROJECT_SPECIFIC
...
void TR_ValueNumberInfo::initializeNode(TR::Node *node, int32_t &negativeValueNumber)
{
    ...
    for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
        ...
        if (node->getOpCode().isSetSignOnNode() && node->getSetSign() != entryNode->getSetSign()) {
            if (trace())
                traceMsg(comp(),
                    "Nodes %s (%p) and entryNode %s (%p) with setSignOnNode have sign mismatch -- do not consider as "
                    "matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            continue;
        }
        ...
        if (node->getType().isBCD()) {
            ...
            if (!node->isSignStateEquivalent(entryNode)) {
                if (trace() || comp()->cg()->traceBCDCodeGen())
                    traceMsg(comp(),
                        "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- do not consider "
                        "as matching\n",
                        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
                continue;
            }
        ...
        // other nodes in the chain.
        //
        if (node->getOpCode().isStore() || node->getOpCode().isLoadVar())
            break;
...
void TR_ValueNumberInfo::allocateParmValueNumbers()
{
    ...
    if (_numberOfParms > 0) {
        _parmSymbols = (TR::ParameterSymbol **)trMemory()->allocateHeapMemory(
            _numberOfParms * sizeof(TR::ParameterSymbol *), TR_Memory::ValueNumberInfo);
        parms.reset();
        int32_t i = 0;
        for (p = parms.getFirst(); p; p = parms.getNext()) {
            _parmSymbols[i++] = p;
...
{
    if (node->getOpCode().canShareValueNumber() && !node->getOpCode().isResolveCheck()
...
{
    int32_t index = node->getGlobalIndex();
    if (_valueNumbers.ElementAt(index) >= 0 || _valueNumbers.ElementAt(index) <= -3)
        return;
...
TR::Node *TR_ValueNumberInfo::getValueNumberForLoad(TR::Node *node)
{
    ...
    if (!_useDefInfo || !_useDefInfo->isUseIndex(useDefIndex) || (_recursionDepth > 50))
        return NULL;
    ...
    if (defNode) {
        ...
        if (baseVN == -1 || (defNode->getOpCode().isIndirect() && baseVN == getVN(defNode->getFirstChild()))) {
            ...
            if (value >= 0) {
                ...
                if (trace()) {
                    traceMsg(comp(),
                        "  Change value number for load %d at [%p] to value number %d of dominating load %d at [%p]\n",
    ...
    if (isNonZero && node->getOpCode().isLoadVarDirect()) {
        ...
        if (defValue >= 0) {
            if (value >= 0) {
                ...
                if (trace()) {
                    traceMsg(comp(),
                        "  Change value number for load %d at [%p] to value number %d of dominating loads\n",
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (_useDefInfo->getUsesFromDef(usesFromDefs, index, true)) {
                ...
                for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
                    ...
                    if (useNode && (useNode != node) && useNode->getOpCode().isLoadVar()) {
                        TR_UseDefInfo::BitVector otherUseDefiningLoads(comp()->allocator());
                        if (_useDefInfo->getDefiningLoads(otherUseDefiningLoads, useNode)
                            && (otherUseDefiningLoads == definingLoads)) {
                            ...
                            if (baseVN == -1
                                || (useNode->getOpCode().isIndirect() && baseVN == getVN(useNode->getFirstChild()))) {
                                // If this node now has a value number, we can equate the value numbers
                                // of it and its dominating load
                                //
                                value = getVN(node);
                                if (value >= 0) {
                                    ...
                                    if (trace()) {
                                        traceMsg(comp(),
                                            "  Change value number for load %d at [%p] to value number %d of load %d "
                                            "at [%p] reached by same dominating loads (defs)\n",
                                ...
                                if (trace()) {
                                    traceMsg(comp(),
                                        "  Use value number %d of dominating load %d at [%p] for load %d at [%p]\n",
                                        getVN(useNode), useNode->getGlobalIndex(), useNode, node->getGlobalIndex(),
                                        node);
                                }
                                return useNode;
                            }
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!defNode) {
            ...
            if (loadSymRef->getSymbol() != defSymRef->getSymbol() || loadSymRef->getOffset() != defSymRef->getOffset())
                return NULL;
...
TR_HashValueNumberInfo::VNHashKey::VNHashKey(TR::Node *node, TR_ValueNumberInfo *VN)
    : _node(node)
    , _VN(VN)
{
    _hashVal = 2166136261;
    ...
    for (uint32_t childNum = 0; childNum < node->getNumChildren(); childNum++) {
        ...
        TR_ASSERT(childVN > 0, "Allocating VN for node %p without allocating VN for child %p", node,
            node->getChild(childNum));
        hash(childVN);
    }
}
...
bool TR_HashValueNumberInfo::VNHashKey::operator==(const VNHashKey &other) const
{
    ...
    if (_node->getOpCodeValue() == otherNode->getOpCodeValue()
        && _node->getNumChildren() == otherNode->getNumChildren()) {
        bool childVNEqual = true;
        for (uint32_t childNum = 0; (childNum < _node->getNumChildren() && childVNEqual); childNum++)
            childVNEqual = (_VN->getValueNumber(_node->getChild(childNum))
                == _VN->getValueNumber(otherNode->getChild(childNum)));
        if (childVNEqual) {
...
TR_HashValueNumberInfo::TR_HashValueNumberInfo(TR::Compilation *comp, TR::Optimizer *optimizer, bool requiresGlobals,
    bool prefersGlobals, bool noUseDefInfo)
...
void TR_HashValueNumberInfo::allocateValueNumber(TR::Node *node)
{
    ...
    //
        ...
        if (defNode) {
            setValueNumber(node, defNode);
            // traceMsg(comp(), "For load node %p found defNode %p, setting value number of node to %d
            // ",node,defNode,_valueNumbers.ElementAt(index));
        } else {
            changeValueNumber(node, _nextValue++);
            // traceMsg(comp(), "For load node %p found no defNode , setting value number of node to %d
            // ",node,defNode,_valueNumbers.ElementAt(index));
        }
    }
    ...
    //
        ...
        for (int32_t k = 0; k < node->getNumChildren(); k++) {
            ...
            //            traceMsg(comp(), "JIAG: for node %p, value number =
        ...
        if (isValidToLookIntoHash) {
            ...
            if (_nodeHash.Locate(nodeKey, loc)) {
                ...
                TR_ASSERT((otherNode != NULL), "HASHVN :Non-null nodeTable entry expected for globalIndex:%d",
                    otherNodeIndex);

                //              traceMsg(comp(),"JIAG1: Setting node %p value number to same number at node
                //              %p\n",node,otherNode);
