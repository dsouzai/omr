...
int32_t TR_GlobalRegisterAllocator::perform()
{
    ...
    if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
            ...
            if (numLocals > 0
                && (!trace()
                    || performTransformation(
                        comp(), "%s Performing liveness for Global Register Allocator\n", OPT_DETAILS))) {
        ...
        _registerCandidates = new (trStackMemory()) SymRefCandidateMap(
        ...
        if (canAffordAssignment) {
            ...
            if (!splitSymRefs.isEmpty()) {
                ...
                if (performTransformation(
                        comp(), "%s Restoring original symbols from live range splitter\n", OPT_DETAILS)) {
...
TR::Node* TR_GlobalRegisterAllocator::resolveTypeMismatch(
    TR::DataType inputOldType, TR::Node* oldNode, TR::Node* newNode)
{
    return newNode;
}

static void setAutoContainsRegisterValue(
    TR::RegisterCandidate* rc, TR_Array<TR::GlobalRegister>* extRegisters, int32_t i, TR::Compilation* comp)
{
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch()
            ...
            if (ptrToGr) {
                ...
                if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
                    if (ptrToGr->getValue() && !ptrToGr->getAutoContainsRegisterValue())
                        ptrToGr->createStoreFromRegister(
                            comp()->getVisitCount(), tt->getPrevTreeTop(), 1, comp(), true);
        ...
        if (changeNode) {
            ...
            if (comp()->getOption(TR_CheckGRA)
                ...
                TR::Node* zeroIfEqual = TR::Node::create(
                    comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()), 2, oldValue, value);
                TR::TreeTop::create(comp(), tt->getPrevTreeTop(),
                    TR::Node::createWithSymRef(TR::BNDCHK, 2, 2,
                        TR::Node::iconst(
                            oldValue, 1), // pretend we have an array of size=1 so only zeroIfEqual==0 is acceptable
                        zeroIfEqual,
            ...
            if (value->getOpCode().isLoadReg()) {
                TR_ASSERT(
                    comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
                TR_Array<TR::GlobalRegister>& extRegisters
                    = _candidates->getStartOfExtendedBBForBB()[block->getNumber()]->getGlobalRegisters(comp());
    ...
    TR_Array<TR::GlobalRegister>& registers, bool regStarTransformDone)
{
    ...
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getOpCodeValue() == TR::treetop)
            node = node->getFirstChild();
        TR_ASSERT(
            node->getOpCode().isJumpWithMultipleTargets(), "transform multi-way branch called with unknown op-code");
 ...
 */
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(
    int32_t i, TR::GlobalRegister* extReg, TR::GlobalRegister* reg, TR::Block* block, bool traceIt)
{
    ...
    if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
        ...
        if (traceIt)
            traceMsg(
                comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors next block is not extension\n");
...
void TR_GlobalRegisterAllocator::reloadNonRegStarVariables(
    TR::TreeTop* tt, TR::Node* node, TR::Block* block, bool traceIt)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
...
void TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(
    TR::TreeTop* bbStartTT, TR_Array<TR::GlobalRegister>& registers, TR::Block* block)
{
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(
    TR::TreeTop* appendPoint, TR_Array<TR::GlobalRegister>& registers, TR::Block* throwingBlock)
{
...
void TR_GlobalRegisterAllocator::addRegLoadsToEntry(
    TR::TreeTop* bbStartTT, TR_Array<TR::GlobalRegister>& registers, TR::Block* block)
{
    TR::Node* bbStart = bbStartTT->getNode();
...
void TR_GlobalRegisterAllocator::addGlRegDepToExit(
...
TR::TreeTop* TR_GlobalRegisterAllocator::findPrevTreeTop(
    ...
    TR_Array<TR::GlobalRegister>& registers, TR::Block* successorBlock, TR_Array<TR::Node*>& regDepNodes)
{
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
            ...
            if ((!extgr->getAutoContainsRegisterValue() || mustBeLiveAcrossAllPaths)
                ...
                if (liveOnSomeSucc && !extgr->getAutoContainsRegisterValue() && !currCandidateAlreadySeen) {
                    if ((block->getSuccessors().size() == 1) || rc != gr->getRegisterCandidateOnExit()) {
                        extgr->createStoreFromRegister(
                            _visitCount, extgr->optimalPlacementForStore(block, comp()), i, comp());
                    } else {
                        if (!prevTreeTop)
        ...
        if (successorRC && !regDepNodes[i] && !gr->isUnavailable()) // ensure the global reg is currently available)
        {
            ...
            {
                extgr->setCurrentRegisterCandidate(
                    gr->getRegisterCandidateOnExit(), _visitCount, exitTreeTop->getEnclosingBlock(), i, comp());
...
TR::Block* TR_GlobalRegisterAllocator::extendBlock(TR::Block* block, TR::Block* successorBlock)
{
    ...
    _candidates->setStartOfExtendedBBForBB(
        newBlock->getNumber(), _candidates->getStartOfExtendedBBForBB()[block->getNumber()]);
...
int32_t TR_GlobalRegisterAllocator::numberOfRegistersLiveOnEntry(
    TR_Array<TR::GlobalRegister>& registers, bool countMachineRegs)
{
    int32_t numLoads = 0;
    TR_ScratchList<TR::RegisterCandidate> seenCandidates(trMemory());
    int32_t i;
...
TR::GlobalRegister* TR_GlobalRegisterAllocator::getGlobalRegisterWithoutChangingCurrentCandidate(
    TR::Symbol* symbol, TR_Array<TR::GlobalRegister>& registers, TR::Block* block)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
...
TR::GlobalRegister* TR_GlobalRegisterAllocator::getGlobalRegister(
    TR::Symbol* symbol, TR_Array<TR::GlobalRegister>& registers, TR::Block* block)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
    TR::Block* startOfExtendedBlock = _candidates->getStartOfExtendedBBForBB()[block->getNumber()];
...
void TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmAsCandidates(
    TR::Block** cfgBlocks, int32_t numberOfNodes, bool onlySelectedCandidates)
{
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(
    TR_StructureSubGraphNode* structureNode, vcount_t visitCount, SymRefCandidateMap& registerCandidates)
{
...
bool TR_GlobalRegisterAllocator::isDependentStore(
    TR::Node* node, const TR_UseDefInfo::BitVector& defs, TR::SymbolReference* symRef, bool* seenLoad)
{
    ...
    if (symsThatShouldNotBeAssignedInCurrentLoop && assignedAutosInCurrentLoop
        && ((opCode.hasSymbolReference() && opCode.isIndirect()) || (opCode.isArrayLength())
            || !(opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isBooleanCompare() || opCode.isNullCheck()
                || opCode.isBndCheck())))
        findSymsUsedInIndirectAccesses(
            node, symsThatShouldNotBeAssignedInCurrentLoop, assignedAutosInCurrentLoop, true);
}
    ...
    bool examineChildren)
{
    ...
    *_temp2 &= *assignedAutosInCurrentLoop;
    ...
    if (examineChildren && (node->getNumChildren() > 0) && !_temp2->isEmpty()) {
        ...
        for (i = 0; i < node->getNumChildren(); ++i)
            findSymsUsedInIndirectAccesses(
                node->getChild(i), symsThatShouldNotBeAssignedInCurrentLoop, assignedAutosInCurrentLoop, false);
    }
}

TR_PairedSymbols* TR_GlobalRegisterAllocator::findOrCreatePairedSymbols(
    TR::SymbolReference* symRef1, TR::SymbolReference* symRef2)
{
...
TR_PairedSymbols* TR_GlobalRegisterAllocator::findPairedSymbols(
    TR::SymbolReference* symRef1, TR::SymbolReference* symRef2)
{
    ListIterator<TR_PairedSymbols> pairs(&_pairedSymbols);
    TR_PairedSymbols* p;
...
void TR_GlobalRegisterAllocator::findLoopsAndCreateAutosForSignExt(
    TR_StructureSubGraphNode* structureNode, vcount_t visitCount)
{
    TR_Structure* structure;
    if (structureNode)
        structure = structureNode->getStructure();
                    ...
                    if (!rc) {
                        ...
                        if (!rc) {
                            ...
                            if (!dontCreateCandidatesJustForSplitter
                                ...
                                rc->addAllBlocksInStructure(
                                    loop->getStructure(), comp(), trace() ? "range splitter candidate" : NULL);
                            }
                        }
                        registerCandidates[origSymRef->getReferenceNumber()] = rc;
                    }
    ...
    TR_SymRefCandidatePair** correspondingSymRefs, TR::Block* loopInvariantBlock, TR::Node* node)
{
    ...
    TR::SymbolReference* correspondingSymRef = comp()->getSymRefTab()->createTemporary(
        comp()->getMethodSymbol(), symRef->getSymbol()->getDataType(), symRef->getSymbol()->isPinningArrayPointer());
...
void TR_LiveRangeSplitter::appendStoreToBlock(
    TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node)
{
    TR::Node* initNode
        = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(storeSymRef->getSymbol()->getDataType()), 1, 1,
            TR::Node::createWithSymRef(
                node, comp()->il.opCodeForDirectLoad(loadSymRef->getSymbol()->getDataType()), 0, loadSymRef),
            storeSymRef);
    ...
    while (placeHolderTreeForLoad && placeHolderTreeForLoad->getNode()->getOpCode().isStore()) {
        if ((placeHolderTreeForLoad->getNode()->getSymbolReference() == loadSymRef)
            || (placeHolderTreeForLoad->getNode()->getSymbolReference()->sharesSymbol()
                && placeHolderTreeForLoad->getNode()->getSymbolReference()->getUseDefAliases().contains(
                    loadSymRef, comp())))
            break;
...
void TR_LiveRangeSplitter::prependStoreToBlock(
    TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node)
{
