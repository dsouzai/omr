...
//  for different compilation level then we can have two version of this, one using CPIndex and one  (i.e. with
//  MetaData) using getReferenceNumber
...
public:
    ...
    switchSuccessorIterator(TR::Node *node)
        : node_(node)
        , i_(node_->getCaseIndexUpperBound())
    {}
    ...
    {
        for (i_ = (i_ > 0) ? i_ - 1 : 0; i_ > 0 && !node_->getChild((int32_t)i_)->getOpCode().isCase(); --i_)
            ;
        if (i_ == 0) {
            return NULL;
        } else {
 ...
 */
bool TR_GlobalRegisterAllocator::allocateForSymRef(TR::SymbolReference *symRef) { return true; }
...
bool TR_GlobalRegisterAllocator::allocateForType(TR::DataType dt) { return true; }
...
void TR_GlobalRegisterAllocator::visitNodeForDataType(TR::Node *node)
{
...
void TR_GlobalRegisterAllocator::walkTreesAndCollectSymbolDataTypes()
{
...
int32_t TR_GlobalRegisterAllocator::perform()
{
    ...
    if (comp()->getOption(TR_MimicInterpreterFrameShape)
        && ((!comp()->getOption(TR_EnableOSR) && !comp()->getOption(TR_FSDGRA))
            || comp()->getJittedMethodSymbol()->sharesStackSlots(comp())))
        return 1;
    ...
    if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
        ...
        if (!cg()->getLiveLocals()) {
            ...
            if (numLocals > 0
                && (!trace()
                    || performTransformation(comp(), "%s Performing liveness for Global Register Allocator\n",
                        OPT_DETAILS))) {
                // Perform liveness analysis
                //
                TR_Liveness liveLocals(comp(), optimizer(), comp()->getFlowGraph()->getStructure(), false, NULL, false,
                    comp()->getOption(TR_EnableAggressiveLiveness));
                ...
                for (TR::CFGNode *cfgNode = comp()->getFlowGraph()->getFirstNode(); cfgNode;
        ...
        if (comp()->target().is64Bit() && optimizer()->getUseDefInfo()) {
            _temp = new (trStackMemory())
                TR_BitVector(optimizer()->getUseDefInfo()->getNumDefNodes(), trMemory(), stackAlloc);
            _candidatesNeedingSignExtension
                = new (trStackMemory()) TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
            _candidatesSignExtendedInThisLoop
                = new (trStackMemory()) TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
        }
        ...
        _registerCandidates = new (trStackMemory()) SymRefCandidateMap((SymRefCandidateMapComparator()),
            SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));
        ...
        if (canAffordAssignment) {
            globalFPAssignmentDone
                = _candidates->assign(cfgBlocks, numberOfBlocks, _firstGlobalRegisterNumber, _lastGlobalRegisterNumber);

            if (_lastGlobalRegisterNumber > -1) {
                _visitCount = comp()->incVisitCount();

                _signExtAdjustmentReqd
                    = new (trStackMemory()) TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
                _signExtAdjustmentNotReqd
                    = new (trStackMemory()) TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
                _signExtDifference
                    = new (trStackMemory()) TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
                ...
                // Transform IL
                //
                for (TR::TreeTop *tt = comp()->getStartTree(); tt;
                     tt = tt->getExtendedBlockExitTreeTop()->getNextTreeTop())
                    transformBlock(tt);
            }
            ...
            if (!splitSymRefs.isEmpty()) {
                ...
                // If source and destination locals of a copy receive registers, in other words, a register-register
                // copy exists, and a value is modified in one of locals, another local must be indentified as
                // 'valueModified'.
                //
                if (trace)
                ...
                for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
                    ...
                    if (isSplittingCopy(tt->getNode())) {
                        ...
                        if (store->getOpCode().isStoreDirect() && load->getOpCode().isLoadReg()
                            && !(*_registerCandidates)[store->getSymbolReference()->getReferenceNumber()]
                                    ->extendedLiveRange()) {
                ...
                // If a value is not modified in a local which receives a register, the stores from the register is
                // redundant.
                //
                if (trace)
                ...
                for (tt = itr.getFirst(); tt; tt = itr.getNext()) {
                    if (!_valueModifiedSymRefs->isSet(
                            tt->getNode()->getFirstChild()->getRegLoadStoreSymbolReference()->getReferenceNumber())
                        && !nonSplittingCopyStored.isSet(
                            tt->getNode()->getFirstChild()->getRegLoadStoreSymbolReference()->getReferenceNumber())) {
                        if (trace)
                            traceMsg(comp(), "Remove a redundant store %p\n", tt->getNode());
...
bool TR_GlobalRegisterAllocator::isSplittingCopy(TR::Node *node)
{
    bool trace = comp()->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
    ...
    if ((node->getOpCode().isStoreDirect() || node->getOpCode().isStoreReg())
        ...
        if (trace)
            traceMsg(comp(), "Finding a copy at node %p\n", node);
...
void TR_GlobalRegisterAllocator::restoreOriginalSymbol(TR::Node *node, vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return;
    ...
    if (node->getOpCode().hasSymbolReference() || node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg()) {
        if (node->getSymbolReferenceOfAnyType()) {
            ...
            while (origSymRef && (origSymRef != rc->getSplitSymbolReference())) {
                ...
                if (!origRc || origRc->getValueModified() || origRc->extendedLiveRange()) {
                    if (!foundChangeSymRef) {
                        if (origRc && !origRc->getValueModified() && origRc->getRestoreSymbolReference()) {
                            _valueModifiedSymRefs->set(origRc->getSymbolReference()->getReferenceNumber());
            ...
            if (rc && !rc->extendedLiveRange() && changeSymRef) {
                if (trace)
                    traceMsg(comp(), "Restore an original symbol #%d from #%d at %p\n",
 ...
 */
void TR_GlobalRegisterAllocator::transformBlock(TR::TreeTop *tt)
{
    ...
    do {
            ...
            for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
                if ((*curRegisters)[i].getRegisterCandidateOnEntry()) {
                    (*curRegisters)[i]
                        ...
                        ->getSymbolReference()
                        ->getSymbol()
                        ->setIsInGlobalRegister(true);
                }

                if ((*curRegisters)[i].getRegisterCandidateOnExit()) {
                    (*curRegisters)[i]
                        .getRegisterCandidateOnExit()
                        ->getSymbolReference()
                        ->getSymbol()
                        ->setIsInGlobalRegister(true);
                }
        ...
        && (node = tt->getNode(),
            node->getOpCodeValue() != TR::BBStart || node->getBlock()->isExtensionOfPreviousBlock()));
...
TR::Node *TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::DataType oldType, TR::Node *newNode)
{
    return resolveTypeMismatch(oldType, NULL, newNode);
}

TR::Node *TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::Node *oldNode, TR::Node *newNode)
{
    return resolveTypeMismatch(TR::NoType, oldNode, newNode);
}

TR::Node *TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::DataType inputOldType, TR::Node *oldNode,
    TR::Node *newNode)
{
    return newNode;
}

static void setAutoContainsRegisterValue(TR::RegisterCandidate *rc, TR_Array<TR::GlobalRegister> *extRegisters,
    int32_t i, TR::Compilation *comp)
{
...
void TR_GlobalRegisterAllocator::transformNode(TR::Node *node, TR::Node *parent, int32_t childIndex, TR::TreeTop *tt,
    TR::Block *&block, TR_Array<TR::GlobalRegister> &registers, TR_NodeMappings *extBlockNodeMapping)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
    TR_Array<TR::GlobalRegister> *extRegisters
        = &(_candidates->getStartOfExtendedBBForBB()[block->getNumber()]->getGlobalRegisters(comp()));
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch()
        ...
        if (nextBlock && !nextBlock->isExtensionOfPreviousBlock() && block->hasSuccessor(nextBlock)
            && !ttNode->getOpCode().isJumpWithMultipleTargets()) {
            ...
            if (ptrToGr) {
                ...
                if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
                    if (ptrToGr->getValue() && !ptrToGr->getAutoContainsRegisterValue())
                        ptrToGr->createStoreFromRegister(comp()->getVisitCount(), tt->getPrevTreeTop(), 1, comp(),
                            true);
        ...
        if (changeNode) {
            dumpOptDetails(comp(), "%s change load var [%p] of %s symRef#%d to load reg\n", OPT_DETAILS, node,
                node->getSymbolReference()->getSymbol()->isMethodMetaData()
                    ? node->getSymbolReference()->getSymbol()->castToMethodMetaDataSymbol()->getName()
                    : "",
            ...
            if (!value) {
                ...
                dumpOptDetails(comp(), "%s change load var [%p] of %s symRef#%d to load reg\n", OPT_DETAILS, node,
                    ...
                    node->getSymbolReference()->getReferenceNumber());
                if (!value) {
                    if (parent->getOpCode().isStore() && (parent->getSymbolReference() == node->getSymbolReference())) {
                        ...
                        TR::Node *loadVarNode = node->getSymbolReference()->getSymbol()->isMethodMetaData()
                                && node->getSymbolReference() != node->getSymbolReference()
                            ? 0
                            : node;
                    ...
                    bool needs2Regs = false;
                    if (gr->getCurrentRegisterCandidate() && gr->getCurrentRegisterCandidate()->rcNeeds2Regs(comp()))
                        needs2Regs = true;
                    ...
                    if (node->isDontMoveUnderBranch()) {
                        // dumpOptDetails(comp(), "propagating dontMoveUnderBranch flag from original load node [%p] to
                        // new load node [%p]\n", node, value);
            ...
            if (value->getOpCode().isLoadReg() && !(parent->getOpCode().getOpCodeValue() == TR::GlRegDeps)
            ...
            if (comp()->getOption(TR_CheckGRA)
                && performTransformation(comp(), "%s CheckGRA inserting BNDCHK to verify %s node %p\n", OPT_DETAILS,
                    value->getOpCode().getName(), value)) {
                TR::Node *zeroIfEqual = TR::Node::create(comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()),
                    2, oldValue, value);
                TR::TreeTop::create(comp(), tt->getPrevTreeTop(),
                    TR::Node::createWithSymRef(TR::BNDCHK, 2, 2,
                        TR::Node::iconst(oldValue,
                            1), // pretend we have an array of size=1 so only zeroIfEqual==0 is acceptable
                        zeroIfEqual,
            ...
            if ((oldValue->getReferenceCount() == 0) && (oldValue->getOpCodeValue() == TR::iload)
            ...
            if (value->getOpCode().isLoadReg()) {
                TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
                    "Incorrect value in _startOfExtendedBBForBB");
                TR_Array<TR::GlobalRegister> &extRegisters
                    = _candidates->getStartOfExtendedBBForBB()[block->getNumber()]->getGlobalRegisters(comp());
        ...
        if (symbol->isInGlobalRegister() && (gr = getGlobalRegister(symbol, registers, block))) {
            ...
            if ((!fpStore && !symRef->storeCanBeRemoved())
                ...
                if (_osrCatchSucc || symRef->getSymbol()->dontEliminateStores(comp()) || keepAllStores
            ...
            // traceMsg(comp(), "ptrToGr %p node %p mapping %d\n", ptrToGr, node, ptrToGr ?
            // ptrToGr->getMappings().getTo(node) : 0);
            if (ptrToGr) {
                // printf("Ignoring store\n"); fflush(stdout);
                // traceMsg(comp(), "Ignoring store %p\n", node);
                TR::RegisterCandidate *rc = ptrToGr->getCurrentRegisterCandidate();
                if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
                    if (rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                        && rc->getSymbolReference() != origSymRef && ptrToGr->getValue()
 ...
 */
void TR_GlobalRegisterAllocator::transformMultiWayBranch(TR::TreeTop *exitTreeTop, TR::Node *node, TR::Block *block,
    TR_Array<TR::GlobalRegister> &registers, bool regStarTransformDone)
{
    ...
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getOpCodeValue() == TR::treetop)
            node = node->getFirstChild();
        TR_ASSERT(node->getOpCode().isJumpWithMultipleTargets(),
            "transform multi-way branch called with unknown op-code");
        si = new (comp()->trStackMemory()) multipleJumpSuccessorIterator(block);
    }

    for (TR::Block *successorBlock = si->getFirstSuccessor(); successorBlock != NULL;
    ...
    if (node->getOpCode().getOpCodeValue() != TR::tstart) {
        ...
        if (block->getNextBlock() && !block->getNextBlock()->isExtensionOfPreviousBlock()
            && block->hasSuccessor(block->getNextBlock())) {
            if (!regStarTransformDone) {
                addGlRegDepToExit(regDepNodes, exitNode,
                    _candidates->getStartOfExtendedBBForBB()[block->getNumber()]->getGlobalRegisters(comp()), block);
            }
...
void TR_GlobalRegisterAllocator::transformBlockExit(TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR::Block *block,
 ...
 */
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(int32_t i, TR::GlobalRegister *extReg,
    TR::GlobalRegister *reg, TR::Block *block, bool traceIt)
{
    ...
    if (traceIt)
        traceMsg(comp(),
            "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors block=%d GlobalReg=(%d,symRef=#%d)\n",
    ...
    if (traceIt)
        traceMsg(comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors checking extensions\n");
    ...
    if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
        if (traceIt)
            traceMsg(comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors nextBlock=%d\n",
                nextBlock->getNumber());
        ...
        {
            if (traceIt)
                traceMsg(comp(), "  not live on entry. Ok here.\n");
        ...
        if (traceIt)
            traceMsg(comp(), "  block_%d marked to reload candidate #%d\n", nextBlock->getNumber(),
                rc->getSymbolReference()->getReferenceNumber());
        reg->setUnavailableResolved();
        ...
        if (traceIt)
            traceMsg(comp(),
                "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors next block is not extension\n");
...
void TR_GlobalRegisterAllocator::reloadNonRegStarVariables(TR::TreeTop *tt, TR::Node *node, TR::Block *block,
    bool traceIt)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
...
void TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(TR::TreeTop *bbStartTT,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *block)
{
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(TR::TreeTop *appendPoint,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *throwingBlock)
{
    ...
    //
    _osrCatchSucc = NULL;
    for (auto nextEdge = throwingBlock->getExceptionSuccessors().begin();
         (nextEdge != throwingBlock->getExceptionSuccessors().end()) && !_osrCatchSucc; ++nextEdge) {
    ...
    for (int32_t i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (_osrCatchSucc && rc && !rc->isLiveAcrossExceptionEdge() && rc->symbolIsLive(_osrCatchSucc)
...
void TR_GlobalRegisterAllocator::addRegLoadsToEntry(TR::TreeTop *bbStartTT, TR_Array<TR::GlobalRegister> &registers,
    TR::Block *block)
{
    TR::Node *bbStart = bbStartTT->getNode();
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (rc && !seenCandidates.find(rc) && !currRegisters[i].getReloadRegisterCandidateOnEntry()
            && !currRegisters[i].isUnavailable()) {
...
void TR_GlobalRegisterAllocator::addGlRegDepToExit(TR_Array<TR::Node *> &regDepNodes, TR::Node *exitNode,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *block)
{
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        if (regDepNodes[i]) {
            ...
            // traceMsg(comp(), "real reg %d exit node %p reg dep node %p candidate %d\n", i, exitNode, regDepNodes[i],
            // (candidate ? candidate->getSymbolReference()->getReferenceNumber() : 0));
    ...
    if (numLoads) {
        ...
        dumpOptDetails(comp(),
            "%s create TR::GlRegDeps [" POINTER_PRINTF_FORMAT "] on exit node [" POINTER_PRINTF_FORMAT "]\n",
            OPT_DETAILS, glRegDep, exitNode);
    }
}

TR::TreeTop *TR_GlobalRegisterAllocator::findPrevTreeTop(TR::TreeTop *&exitTreeTop, TR::Node *&exitNode,
    TR::Block *block, TR::Block *successorBlock)
{
...
void TR_GlobalRegisterAllocator::prepareForBlockExit(TR::TreeTop *&exitTreeTop, TR::Node *&exitNode, TR::Block *block,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *successorBlock, TR_Array<TR::Node *> &regDepNodes)
{
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (successorBlock->getGlobalRegisters(comp())[i].getRegisterCandidateOnEntry()
            != gr->getRegisterCandidateOnExit())
            candidate = extgr->getCurrentRegisterCandidate();
        if (candidate && !seenExitCandidates.find(candidate))
            seenExitCandidates.add(candidate);
        else if (candidate)
            exitCandidateAlreadySeen = true; // only true for longs

        candidate = extgr->getCurrentRegisterCandidate();
        if (candidate && !seenCurrCandidates.find(candidate))
        ...
        if (extgr->getValue()) {
            ...
            if ((!extgr->getAutoContainsRegisterValue() || mustBeLiveAcrossAllPaths)
                ...
                if (liveOnSomeSucc && !extgr->getAutoContainsRegisterValue() && !currCandidateAlreadySeen) {
                    if ((block->getSuccessors().size() == 1) || rc != gr->getRegisterCandidateOnExit()) {
                        extgr->createStoreFromRegister(_visitCount, extgr->optimalPlacementForStore(block, comp()), i,
                            comp());
                    } else {
                        if (!prevTreeTop)
        ...
        //
        TR::RegisterCandidate *successorRC
        ...
        ///successorRC, regDepNodes[i], extgr->getAutoContainsRegisterValue());
        // traceMsg(comp(), "exit node %p succ RC %p reg dep nodes %p\n", exitNode, successorRC, regDepNodes[i]);
        if (successorRC && !regDepNodes[i] && !gr->isUnavailable()) // ensure the global reg is currently available)
        {
            ...
            {
                extgr->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(), _visitCount,
                    exitTreeTop->getEnclosingBlock(), i, comp());
                ...
                if (currRC && currRC->rcNeeds2Regs(comp())) {
                    ...
                    if (i == highRegNum) {
                        TR::GlobalRegister *grLow = &extRegisters[currRC->getLowGlobalRegisterNumber()];
                        grLow->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(), _visitCount,
                            exitTreeTop->getEnclosingBlock(), currRC->getLowGlobalRegisterNumber(), comp());
                        // grLow->copyCurrentRegisterCandidate(gr);
                    } else {
                        TR::GlobalRegister *grHigh = &extRegisters[currRC->getHighGlobalRegisterNumber()];
                        grHigh->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(), _visitCount,
                            exitTreeTop->getEnclosingBlock(), currRC->getHighGlobalRegisterNumber(), comp());
                ...
                if (extgr->getCurrentRegisterCandidate() != successorRC) {
                    traceMsg(comp(), "block_%d successorBlock %d exitNode %p\n", block->getNumber(),
                        successorBlock->getNumber(), exitNode);

                    if (extgr->getCurrentRegisterCandidate())
                        traceMsg(comp(), "reg %d current candidate %d\n", i,
                            extgr->getCurrentRegisterCandidate()->getSymbolReference()->getReferenceNumber());
                    else
                        traceMsg(comp(), "reg %d current candidate null\n", i);

                    if (gr->getRegisterCandidateOnExit())
                        traceMsg(comp(), "reg %d exit candidate %d\n", i,
                            gr->getRegisterCandidateOnExit()->getSymbolReference()->getReferenceNumber());
                    else
                        traceMsg(comp(), "reg %d current candidate null\n", i);

                    if (successorRC)
                        traceMsg(comp(), "reg %d successorRC %d\n", i,
                            successorRC->getSymbolReference()->getReferenceNumber());
                    else
                        traceMsg(comp(), "reg %d successorRC null\n", i);
                }

                TR_ASSERT(extgr->getCurrentRegisterCandidate() == successorRC,
                    "Candidate on exit does not match candidate on successor's entry");
            }

            TR::Node *value = extgr->getValue();
            if (!extgr->getValue() && !exitCandidateAlreadySeen) {
                if (!prevTreeTop)
                    prevTreeTop = findPrevTreeTop(exitTreeTop, exitNode, block, successorBlock);

                value = extgr->createStoreToRegister(prevTreeTop, NULL, _visitCount, comp(), this);
                bool needs2Regs = false;
                if (extgr->getCurrentRegisterCandidate() && extgr->getCurrentRegisterCandidate()->rcNeeds2Regs(comp()))
                    needs2Regs = true;
            ...
            if (!exitCandidateAlreadySeen
                && !(value->getOpCode().isLoadReg()
                    && extgr->getCurrentRegisterCandidate()->hasSameGlobalRegisterNumberAs(value, comp()))) {
                TR::Node *actualValue = value;
                value = TR::Node::create(TR::PassThrough, 1, value);

                if (actualValue->requiresRegisterPair(comp())) {
                    value->setLowGlobalRegisterNumber(
                        extgr->getCurrentRegisterCandidate()->getLowGlobalRegisterNumber());
                    value->setHighGlobalRegisterNumber(
                        extgr->getCurrentRegisterCandidate()->getHighGlobalRegisterNumber());

                    TR::RegisterCandidate *currRC = extgr->getCurrentRegisterCandidate();
                    if (currRC && currRC->getType().isInt64()) {
                        int32_t highRegNum = currRC->getHighGlobalRegisterNumber();
                        if (i == highRegNum) {
                            regDepNodes[extgr->getCurrentRegisterCandidate()->getLowGlobalRegisterNumber()] = value;
                            TR::GlobalRegister *grLow
                                = &extRegisters[extgr->getCurrentRegisterCandidate()->getLowGlobalRegisterNumber()];
                            ...
                            TR::GlobalRegister *grHigh
                                = &extRegisters[extgr->getCurrentRegisterCandidate()->getHighGlobalRegisterNumber()];
    ...
    if ((successorBlock != originalSuccessorBlock) && (successorBlock->getPredecessors().size() == 1)
...
bool TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(TR::TreeTop *exitTreeTop, TR::Node *exitNode,
    TR::Block *block, TR::GlobalRegister *extgr, TR::Block *&successorBlock, int32_t i)
{
    TR_Array<TR::GlobalRegister> &registers = block->getGlobalRegisters(comp());
    TR::GlobalRegister *gr = &registers[i];
    ...
    if (!predecessors.empty() && !(predecessors.size() == 1)
        ...
        if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
            if (trace())
                traceMsg(comp(),
                    "Extended live range of #%d into successor since candidate is available in register on all "
                    "predecessor's exits: block=%d succ=%d allPredFreq=%d succFreq=%d\n",
                    rc->getSymbolReference()->getReferenceNumber(), block->getNumber(), successorBlock->getNumber(),
    ...
    int32_t numRegistersLiveOnSuccessor
        = numberOfRegistersLiveOnEntry(successorBlock->getGlobalRegisters(comp()), true);
    if (numRegistersLiveOnSuccessor + (needs2Regs ? 2 : 1) > cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block))
        return false;

    if (!rc->symbolIsLive(successorBlock) && !rc->symbolIsLive(block))
        return false;
            ...
            if (block->getNextBlock()->getFrequency() > successorBlock->getFrequency()) {
                if (((successorBlock->getFrequency() == 0)
                        || ((successorBlock->getFrequency() > 0)
                            && ((block->getNextBlock()->getFrequency() * 100 / (successorBlock->getFrequency()))
                                > 130))))
                    branchIsHotter = TR_no;
                ...
                if (((block->getNextBlock()->getFrequency() == 0)
                        || ((block->getNextBlock()->getFrequency() > 0)
                            && ((successorBlock->getFrequency() * 100 / (block->getNextBlock()->getFrequency()))
                                > 130))))
                    branchIsHotter = TR_yes;
            }
        }
    ...
    if (branchIsHotter == TR_no) {
        ...
        if (!newBlock->getEntry()->getPrevTreeTop()) {
            newBlock->append(
                TR::TreeTop::create(comp(), TR::Node::create(exitNode, TR::Goto, 0, successorBlock->getEntry())));
...
TR::Block *TR_GlobalRegisterAllocator::extendBlock(TR::Block *block, TR::Block *successorBlock)
{
    ...
    _candidates->setStartOfExtendedBBForBB(newBlock->getNumber(),
        _candidates->getStartOfExtendedBBForBB()[block->getNumber()]);
...
TR::Block *TR_GlobalRegisterAllocator::createNewSuccessorBlock(TR::Block *block, TR::Block *successorBlock,
    TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR::RegisterCandidate *rc)
{
    ...
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
    TR_Array<TR::GlobalRegister> &blockRegistersEBB
        = _candidates->getStartOfExtendedBBForBB()[block->getNumber()]->getGlobalRegisters(comp());
    ...
    if (!_newBlocks.isEmpty()) {
        ...
        for (nextNewBlock = newBlocksIt.getFirst(); nextNewBlock != NULL; nextNewBlock = newBlocksIt.getNext()) {
            ...
            if (newBlockCanBeReused) {
                TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
                    "Incorrect value in _startOfExtendedBBForBB");
                TR_Array<TR::GlobalRegister> &predRegistersEBB
                    = _candidates->getStartOfExtendedBBForBB()[predBlock->getNumber()]->getGlobalRegisters(comp());
                ...
                if ((nextNewBlock->getSuccessors().size() == 1)
                    ...
                    for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
                        if (successorRegisters[j].getRegisterCandidateOnEntry()
                            != nextNewRegisters[j].getRegisterCandidateOnExit()) {
                            newBlockCanBeReused = false;
                            break;
                        }
                        if ((blockRegisters[j].getRegisterCandidateOnExit()
                                != predRegisters[j].getRegisterCandidateOnExit())
                            || (blockRegistersEBB[j].getCurrentRegisterCandidate()
                                != predRegisters[j].getRegisterCandidateOnExit())
                            || (blockRegisters[j].getRegisterCandidateOnExit()
                                != nextNewRegisters[j].getRegisterCandidateOnEntry())
                            || (blockRegistersEBB[j].getCurrentRegisterCandidate()
                                != nextNewRegisters[j].getRegisterCandidateOnEntry())) {
                            newBlockCanBeReused = false;
                            break;
                    ...
                    int32_t numRegistersLiveOnNewSuccessor
                        = numberOfRegistersLiveOnEntry(nextNewBlock->getGlobalRegisters(comp()), true);
                    bool needs2Regs = rc->rcNeeds2Regs(comp());
                    if (numRegistersLiveOnNewSuccessor + (needs2Regs ? 2 : 1)
                        > comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block)) {
                        if (trace())
                            traceMsg(comp(),
                                "numRegistersLiveOnNewSuccessor %d on nextNewBlock %d > "
                                "comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block_%d) %d\n",
                                numRegistersLiveOnNewSuccessor, nextNewBlock->getNumber(), block->getNumber(),
                                comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block));
                        newBlockCanBeReused = false;
                    }
                } else
        ...
        for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
            if (successorRegisters[j].getRegisterCandidateOnEntry()) {
                ...
                TR_ASSERT(!blockRegisters[j].getRegisterCandidateOnExit()
                        || (blockRegisters[j].getRegisterCandidateOnExit()
                            == successorRegisters[j].getRegisterCandidateOnEntry())
                        || (blockRegisters[j].getRegisterCandidateOnEntry()
                            == successorRegisters[j].getRegisterCandidateOnEntry()),
                    "Candidates are different in a global register across a CFG edge\n");
    ...
    } else if (exitTreeTop->getNode()->getOpCode().isJumpWithMultipleTargets()
        && !exitTreeTop->getNode()->getOpCode().isSwitch()) {
        int32_t i = 0;
...
TR::Block *TR_GlobalRegisterAllocator::createBlock(TR::Block *block, TR::Block *successorBlock)
{
    // dumpOptDetails(comp(), "successorBlock %d freq %d\n", successorBlock->getNumber(),
    // successorBlock->getFrequency());
    TR::Block *newBlock
        = TR::Block::createEmptyBlock(block->getExit()->getNode(), comp(), block->getFrequency(), block);
    ...
    if (_candidates->getStartOfExtendedBBForBB()[successorBlock->getNumber()] == block && block != successorBlock)
        _candidates->setStartOfExtendedBBForBB(successorBlock->getNumber(), newBlock);

    return newBlock;
}

int32_t TR_GlobalRegisterAllocator::numberOfRegistersLiveOnEntry(TR_Array<TR::GlobalRegister> &registers,
    bool countMachineRegs)
{
    int32_t numLoads = 0;
    TR_ScratchList<TR::RegisterCandidate> seenCandidates(trMemory());
    int32_t i;
...
TR::GlobalRegister *TR_GlobalRegisterAllocator::getGlobalRegisterWithoutChangingCurrentCandidate(TR::Symbol *symbol,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *block)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
        ...
        if (!firstRc || firstRc->getSymbolReference()->getSymbol() != symbol) {
            ...
            if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
                ...
                if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
                    ...
                    TR_ASSERT(!symbol->isInGlobalRegister(),
                        "Current candidate is not live on entry and not live on exit from basic block\n");
            }
        }
        return extgr;
    }
...
TR::GlobalRegister *TR_GlobalRegisterAllocator::getGlobalRegister(TR::Symbol *symbol,
    TR_Array<TR::GlobalRegister> &registers, TR::Block *block)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
    TR::Block *startOfExtendedBlock = _candidates->getStartOfExtendedBBForBB()[block->getNumber()];
...
void TR_GlobalRegisterAllocator::findIfThenRegisterCandidates()
{
    LexicalTimer t("TR_GlobalRegisterAllocator::findIfThenRegisterCandidates", comp()->phaseTimer());

    TR_ScratchList<TR::RegisterCandidate> registerCandidates(trMemory());
    TR::CFG *cfg = comp()->getFlowGraph();
    ...
    if (!debug("oldIfThen")) {
        ...
        while (paramCursor != NULL) {
            if (paramCursor->isReferencedParameter()) {
                TR::RegisterCandidate *rc = comp()->getGlobalRegisterCandidates()->find(paramCursor);
                if (!rc) {
        ...
        while (autoCursor != NULL) {
            TR::RegisterCandidate *rc = comp()->getGlobalRegisterCandidates()->find(autoCursor);
            if (!rc) {
        ...
        for (TR::CFGNode *block = cfg->getFirstNode(); block; block = block->getNext()) {
            ...
            if (cg()->getLiveLocals() && liveLocals) {
                if (block != comp()->getStartBlock() && block != cfg->getStart() && block != cfg->getEnd()) {
        ...
        while (paramCursor != NULL) {
            if (paramCursor->isReferencedParameter()) {
                ...
                for (TR::CFGNode *block = cfg->getFirstNode(); block; block = block->getNext()) {
                    TR_BitVector *liveLocals = toBlock(block)->getLiveLocals();
                    if (cg()->getLiveLocals() && liveLocals && !rc->getBlocks().find(block->getNumber())) {
                        if (block != comp()->getStartBlock() && block != cfg->getStart() && block != cfg->getEnd()
                ...
                if (mergeBlock1 && mergeBlock1 == mergeBlock2 && mergeBlock1 != cfg->getEnd()) {
                    for (TR::TreeTop *tt1 = block1->getEntry(); tt1 != block1->getExit(); tt1 = tt1->getNextTreeTop()) {
                        ...
                        if (storeNode1 && storeNode1->getOpCode().isStoreDirect()) {
                            TR::RegisterCandidate *rc = 0;
                            TR::SymbolReference *symRef = tt1->getNode()->getSymbolReference();
                            if ((symRef->getSymbol()->isAutoOrParm()) && isSymRefAvailable(symRef))
                                for (TR::TreeTop *tt2 = block2->getEntry(); tt2 != block2->getExit();
                                     tt2 = tt2->getNextTreeTop()) {
                                    TR::Node *storeNode2 = tt2->getNode()->getStoreNode();
                                    if (storeNode2 && storeNode2->getOpCode().isStoreDirect()
                                        && storeNode2->getSymbolReference() == symRef) {
...
void TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmAsCandidates(TR::Block **cfgBlocks, int32_t numberOfNodes,
    bool onlySelectedCandidates)
{
    ...
    for (TR::ParameterSymbol *paramCursor = paramIterator.getFirst(); paramCursor != NULL;
        ...
        if ((paramCursor->isReferencedParameter() && isTypeAvailable(symRef)) && !onlySelectedCandidates) {
            ...
            while (bvi.hasMoreElements())
                rc->getBlocks().incNumberOfLoadsAndStores(bvi.getNextElement(), 1);
    ...
    for (int32_t symRefNumber = symRefTab->getIndexOfFirstSymRef(); symRefNumber < symRefCount; symRefNumber++) {
        ...
        if (symRef && isSymRefAvailable(symRef)) {
            ...
            if (sym) {
                ...
                if ((sym->isAuto() && methodSymbol->getAutomaticList().find(sym->castToAutoSymbol())
...
void TR_GlobalRegisterAllocator::offerAllFPAutosAndParmsAsCandidates(TR::Block **cfgBlocks, int32_t numberOfNodes)
{
    ...
    for (int32_t symRefNumber = symRefTab->getIndexOfFirstSymRef(); symRefNumber < symRefCount; symRefNumber++) {
        ...
        if (symRef) {
            sym = symRef->getSymbol();
            if (sym) {
                if ((sym->getDataType() == TR::Float || sym->getDataType() == TR::Double) && isTypeAvailable(symRef)
...
void TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates()
{
    ...
    SymRefCandidateMap *registerCandidates = new (trStackMemory()) SymRefCandidateMap((SymRefCandidateMapComparator()),
        SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));
    findLoopsAndCorrespondingAutos(NULL, visitCount, *registerCandidates);
}

TR_GlobalRegisterAllocator::BlockInfo &TR_GlobalRegisterAllocator::blockInfo(int32_t i)
{
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(TR_StructureSubGraphNode *structureNode,
    vcount_t visitCount, SymRefCandidateMap &registerCandidates)
{
    ...
    if (structure->asRegion()) {
        ...
        if (!regionStructure->isAcyclic() && structureNode) {
            ...
            bool excludeInvariantsEnabled
                = comp()->cg()->excludeInvariantsFromGRAEnabled() && (!comp()->getOption(TR_DisableRXusage));
            ...
            if (excludeInvariantsEnabled) {
                ...
                symsThatShouldBeAssignedInCurrentLoop
                    = new (trStackMemory()) TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
            }
            ...
            if (_candidatesSignExtendedInThisLoop) {
                oldCandidatesSignExtendedInThisLoop
                    = new (trStackMemory()) TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
            ...
            for (nextBlock = blocksIt.getFirst(); nextBlock; nextBlock = blocksIt.getNext()) {
                if (nextBlock->getVisitCount() != visitCount) {
                    ...
                    while (currentTree != exitTree) {
                        ...
                        markAutosUsedIn(currentNode, NULL, NULL, &arrayAccess, nextBlock, &blocksInLoop, visitCount,
                            executionFrequency, registerCandidates, &assignedAutosInCurrentLoop,
                            symsThatShouldNotBeAssignedInCurrentLoop, hasCatchBlock);
            ...
            for (auto succ = structureNode->getSuccessors().begin(); succ != structureNode->getSuccessors().end();
                 ++succ) {
                ...
                // are very hot.
                //
                if (0 && exitBlock && (exitBlock != comp()->getFlowGraph()->getEnd())) {
                    ...
                    while (bvi.hasMoreElements()) {
                        ...
                        TR::RegisterCandidate *rc = registerCandidates[nextCandidate];
                    ...
                    if (!symsThatShouldBeAssignedInCurrentLoop->get(nextCandidate)) {
                        *_temp2 = *symsThatShouldBeAssignedInCurrentLoop;
                        ...
                        for (p = pairs.getFirst(); p; p = pairs.getNext()) {
                            ...
                            if (otherSymRef
                                && symsThatShouldNotBeAssignedInCurrentLoop->get(otherSymRef->getReferenceNumber())) {
                                TR::RegisterCandidate *otherCandidate
                                    = registerCandidates[otherSymRef->getReferenceNumber()];
                                if (otherCandidate->countNumberOfLoadsAndStoresInBlocks(&blocksInLoop)
                                    < rc->countNumberOfLoadsAndStoresInBlocks(&blocksInLoop))
                        ...
                        if (!doNotRemoveThisCandidate) {
                            ...
                            for (auto succ = structureNode->getSuccessors().begin();
                                 succ != structureNode->getSuccessors().end(); ++succ) {
                                TR_Structure *exitStructure
                                    = (*succ)->getTo()->asStructureSubGraphNode()->getStructure();
                                TR::Block *exitBlock = NULL;
                                if (exitStructure)
                                    exitBlock = exitStructure->getEntryBlock();
                                if (exitBlock && (exitBlock != comp()->getFlowGraph()->getEnd())) {
...
bool TR_GlobalRegisterAllocator::isDependentStore(TR::Node *node, const TR_UseDefInfo::BitVector &defs,
    TR::SymbolReference *symRef, bool *seenLoad)
{
...
void TR_GlobalRegisterAllocator::markAutosUsedIn(TR::Node *node, TR::Node *parent, TR::Node *grandParent,
    TR::Node **currentArrayAccess, TR::Block *block, List<TR::Block> *blocksInLoop, vcount_t visitCount,
    int32_t executionFrequency, SymRefCandidateMap &registerCandidates, TR_BitVector *assignedAutosInCurrentLoop,
    TR_BitVector *symsThatShouldNotBeAssignedInCurrentLoop, bool hasCatchBlock)
{
    ...
    if (node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isAuto()) {
        ...
        if (comp()->target().is64Bit() && info && (parent->getOpCodeValue() == TR::i2l) && node->isNonNegative()
            && enableSignExtGRA) {
            ...
            if (info->getUseDef(defs, useIndex)) {
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (defNode->getOpCode().isStore()) {
                        ...
                        // TR_ASSERT((sym == node->getSymbolReference()->getSymbol()), "Symbols must match between a use
                        // and a def\n"); Comment out the above assume because:
                        //   - The assume came from JAVA where there could not be aliasing with autos
                        //   - If there is an alias causing a different sym then we will not have a candidate anyways so
                        //   it does not matter
                        //   - avoid extra computation if symbols do not match by adding extra condition below.
                        if (sym == node->getSymbolReference()->getSymbol() && (sym->isAuto())) {
                            bool seenLoad = false;
                            bool dependentStore
                                = isDependentStore(defNode, defs, node->getSymbolReference(), &seenLoad);
                            if (dependentStore && defNode->getFirstChild()->isNonNegative() && seenLoad) {
                                ...
                                if (comp()->getFlowGraph()->getStructure() && blockStructure && defBlockStructure
    ...
    if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect()) {
        ...
        if (symRef->getSymbol()->isAutoOrParm() && isSymRefAvailable(symRef, blocksInLoop)) {
            ...
            if (!rc) {
                registerCandidates[symRef->getReferenceNumber()]
                    = comp()->getGlobalRegisterCandidates()->findOrCreate(symRef);
            ...
            if (block != cfg->getStart()) {
                ...
                if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node))
                    || (node->getOpCode().isLoadVarDirect() && parent && parent->getOpCode().isStoreDirect()
                        && isSplittingCopy(parent))) {
                    rc->addBlock(block, 0);
                } else {
                    if (comp()->target().cpu.isZ()
                        && rc->getSymbolReference()->getSymbol()->getDataType() == TR::Address && parent
                        && (((parent->getOpCode().isStoreIndirect() || parent->getOpCode().isLoadIndirect())
                                && parent->getFirstChild() == node)
                            || (parent->getOpCode().isAdd() && grandParent
            ...
            if (((node->getReferenceCount() > 1) || node->getOpCode().isStoreDirect())
                && symsThatShouldNotBeAssignedInCurrentLoop)
                symsThatShouldNotBeAssignedInCurrentLoop->reset(symRef->getReferenceNumber());

            if (symsThatShouldNotBeAssignedInCurrentLoop && node->getOpCode().isLoadVarDirect()) {
    ...
    for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
        ...
        markAutosUsedIn(node->getChild(childNum), node, parent, currentArrayAccess, block, blocksInLoop, visitCount,
            executionFrequency, registerCandidates, assignedAutosInCurrentLoop,
            symsThatShouldNotBeAssignedInCurrentLoop, hasCatchBlock);
    ...
    if (symsThatShouldNotBeAssignedInCurrentLoop && assignedAutosInCurrentLoop
        && ((opCode.hasSymbolReference() && opCode.isIndirect()) || (opCode.isArrayLength())
            || !(opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isBooleanCompare() || opCode.isNullCheck()
                || opCode.isBndCheck())))
        findSymsUsedInIndirectAccesses(node, symsThatShouldNotBeAssignedInCurrentLoop, assignedAutosInCurrentLoop,
            true);
}
...
void TR_GlobalRegisterAllocator::signExtendAllDefNodes(TR::Node *defNode, List<TR::Node> *defNodes)
{
    ...
    if (((childOfDefNode->getOpCodeValue() == TR::iadd) || (childOfDefNode->getOpCodeValue() == TR::isub))
        ...
        && ((childOfDefNode->getSecondChild()->getInt() <= 32767)
            && (childOfDefNode->getSecondChild()->getInt() >= -32767))) {
        int32_t useIndex;
        if ((childOfDefNode->getOpCodeValue() == TR::iadd) || (childOfDefNode->getOpCodeValue() == TR::isub)) {
            useIndex = childOfDefNode->getFirstChild()->getUseDefIndex();
            if (((childOfDefNode->getOpCodeValue() == TR::iadd) && (childOfDefNode->getSecondChild()->getInt() < 0))
                || ((childOfDefNode->getOpCodeValue() == TR::isub) && (childOfDefNode->getSecondChild()->getInt() > 0)))
                defNode->setNeedsSignExtension(true);
        } else {
            defNode->setNeedsSignExtension(true);
        ...
        if (info->getUseDef(defs, useIndex)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (nextDefNode->getOpCode().isStore() && !defNodes->find(nextDefNode))
...
void TR_GlobalRegisterAllocator::findSymsUsedInIndirectAccesses(TR::Node *node,
    TR_BitVector *symsThatShouldNotBeAssignedInCurrentLoop, TR_BitVector *assignedAutosInCurrentLoop,
    bool examineChildren)
{
    if (symsThatShouldNotBeAssignedInCurrentLoop && node->getOpCode().hasSymbolReference()
        && (node->getSymbolReference()->getSymbol()->isAutoOrParm())) {
        TR::SymbolReference *symRef = node->getSymbolReference();
        symsThatShouldNotBeAssignedInCurrentLoop->reset(symRef->getReferenceNumber());
    }
    ...
    *_temp2 &= *assignedAutosInCurrentLoop;
    ...
    if (examineChildren && (node->getNumChildren() > 0) && !_temp2->isEmpty()) {
        ...
        int32_t i;
        for (i = 0; i < node->getNumChildren(); ++i)
            findSymsUsedInIndirectAccesses(node->getChild(i), symsThatShouldNotBeAssignedInCurrentLoop,
                assignedAutosInCurrentLoop, false);
    }
}

TR_PairedSymbols *TR_GlobalRegisterAllocator::findOrCreatePairedSymbols(TR::SymbolReference *symRef1,
...
TR_PairedSymbols *TR_GlobalRegisterAllocator::findPairedSymbols(TR::SymbolReference *symRef1,
    TR::SymbolReference *symRef2)
{
    ListIterator<TR_PairedSymbols> pairs(&_pairedSymbols);
    TR_PairedSymbols *p;
    for (p = pairs.getFirst(); p; p = pairs.getNext()) {
        if (((p->_symRef1 == symRef1) && (p->_symRef2 == symRef2))
            || ((p->_symRef1 == symRef2) && (p->_symRef2 == symRef1)))
            return p;
    }
    return NULL;
}
 ...
 */
TR::Block *TR_GlobalRegisterAllocator::getAppendBlock(TR::Block *block)
{
    if (_appendBlock)
        return _appendBlock;
    ...
    for (; block != NULL; prevBlock = block, block = block->getNextBlock()) {
        if (block != inBlock && prevBlock && !prevBlock->hasSuccessor(block))
...
void TR_GlobalRegisterAllocator::appendGotoBlock(TR::Block *gotoBlock, TR::Block *curBlock)
{
...
void TR_GlobalRegisterAllocator::findLoopsAndCreateAutosForSignExt(TR_StructureSubGraphNode *structureNode,
    vcount_t visitCount)
{
    TR_Structure *structure;
    if (structureNode)
            ...
            for (nextBlock = blocksIt.getFirst(); nextBlock; nextBlock = blocksIt.getNext()) {
                if (nextBlock->getVisitCount() != visitCount) {
                    ...
                    while (currentTree != exitTree) {
                        TR::Node *currentNode = currentTree->getNode();
                        TR::Node *arrayAccess = NULL;
                        createStoresForSignExt(currentNode, NULL, NULL, currentTree, &arrayAccess, nextBlock,
                            &blocksInLoop, visitCount, false);
                        currentTree = currentTree->getNextRealTreeTop();
                    }
                }
            }
...
void TR_GlobalRegisterAllocator::createStoresForSignExt(TR::Node *node, TR::Node *parent, TR::Node *grandParent,
    TR::TreeTop *treetop, TR::Node **currentArrayAccess, TR::Block *block, List<TR::Block> *blocksInLoop,
    vcount_t visitCount, bool hasCatchBlock)
{
    ...
    int32_t childNum;
    for (childNum = 0; childNum < node->getNumChildren(); childNum++)
        createStoresForSignExt(node->getChild(childNum), node, parent, treetop, currentArrayAccess, block, blocksInLoop,
...
void TR_LiveRangeSplitter::splitLiveRanges()
{
    ...
    // 0, comp()->getSymRefCount()*sizeof(TR::SymbolReference *));
    _origSymRefs = NULL;
    _origSymRefCount = 0;
...
static bool canSplit(TR::SymbolReference *symRef, TR::Compilation *comp)
{
    ...
    //
    if (symRef->getSymbol()->isAutoOrParm() && !addr_taken && !symRef->getSymbol()->isInternalPointer()
        && !symRef->getSymbol()->dontEliminateStores(comp) && !symRef->getSymbol()->isLocalObject()
...
void TR_LiveRangeSplitter::splitLiveRanges(TR_StructureSubGraphNode *structureNode)
{
    ...
    if (structure->asRegion()) {
        ...
        if (regionStructure->isNaturalLoop() && structureNode) {
            ...
            if (!dontAssignInColdBlocks(comp()) || !regionStructure->getEntryBlock()->isCold()) {
                if ((structureNode->getPredecessors().size() == 1)) {
                    TR_StructureSubGraphNode *loopInvariantNode
                        = toStructureSubGraphNode(structureNode->getPredecessors().front()->getFrom());
            ...
            if (loopInvariantBlock) {
                ...
                SymRefCandidateMap *registerCandidates = new (trStackMemory()) SymRefCandidateMap(
                    (SymRefCandidateMapComparator()), SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));

                TR_BitVector *replacedAutosInCurrentLoop
                    = new (trStackMemory()) TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
                TR_BitVector *autosThatCannotBeReplacedInCurrentLoop
                    = new (trStackMemory()) TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
                _storedSymRefs = new (trStackMemory()) TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);

                TR_SymRefCandidatePair **correspondingSymRefs
                    = (TR_SymRefCandidatePair **)trMemory()->allocateStackMemory(
                        comp()->getSymRefCount() * sizeof(TR_SymRefCandidatePair *));
                ...
                TR_BitVector *exitsFromCurrentLoop = new (trStackMemory())
                    TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
                for (auto succ = structureNode->getSuccessors().begin(); succ != structureNode->getSuccessors().end();
                     ++succ)
                ...
                TR::Block *nextBlock;
                for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
                    for (auto succ = nextBlock->getSuccessors().begin(); succ != nextBlock->getSuccessors().end();
                         ++succ) {
                ...
                for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
                    if (nextBlock->getVisitCount() != visitCount) {
                        ...
                        while (currentTree != exitTree) {
                            TR::Node *currentNode = currentTree->getNode();
                            replaceAutosUsedIn(currentTree, currentNode, NULL, nextBlock, &blocksInLoop, &exitBlocks,
                                visitCount, executionFrequency, *registerCandidates, correspondingSymRefs,
                                replacedAutosInCurrentLoop, autosThatCannotBeReplacedInCurrentLoop, structureNode,
                                loopInvariantBlock);
                            currentTree = currentTree->getNextRealTreeTop();
                        }
                    }
                ...
                while ((i < symRefCount) && (prevSymRefCount == 0)) {
                    ...
                    if (origSymRef && trace())
                        traceMsg(comp(), "orig sym %p (#%d)\n", origSymRef->getSymbol(),
                            origSymRef->getReferenceNumber());

                    bool candidateIsLiveOnExit = false;
                    if (origSymRef && origSymRef->getSymbol()->isAutoOrParm()
                        && comp()->cg()->considerTypeForGRA(origSymRef) && canSplit(symRef, comp())
                        && !replacedAutosInCurrentLoop->get(i) && !autosThatCannotBeReplacedInCurrentLoop->get(i)) {
                        ...
                        if (trace() && origSymRef->getSymbol()->getAutoSymbol())
                            traceMsg(comp(), "2 orig sym %p %d live index %d\n", origSymRef->getSymbol(),
                                origSymRef->getReferenceNumber(),
                                origSymRef->getSymbol()->getAutoSymbol()->getLiveLocalIndex());
                        // if (rc)
                        {
                            ...
                            for (exitBlock = exitBlocksIt.getCurrent(); exitBlock; exitBlock = exitBlocksIt.getNext()) {
                                ...
                                if (liveLocals && origSymRef->getSymbol()->getAutoSymbol()
                    ...
                    if (candidateIsLiveOnExit) {
                        ...
                        if (((!isFloat
                                 && (_numberOfGPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalGPRs() - 2)
                                || (isFloat
                                    && (_numberOfFPRs + numRegsForCandidate)
                                        <= comp()->cg()->getNumberOfGlobalFPRs() - 2))
                            && performTransformation(comp(), "%s replace auto #%d in loop %d (%p)\n", OPT_DETAILS,
                                symRef->getReferenceNumber(), structureNode->getNumber(), structureNode)) {
                            ...
                            TR_SymRefCandidatePair *correspondingSymRefCandidate = splitAndFixPreHeader(symRef,
                                correspondingSymRefs, loopInvariantBlock, loopInvariantBlock->getEntry()->getNode());
                            ...
                            ///symRef->getReferenceNumber(), correspondingSymRef->getReferenceNumber(),
                            ///comp()->signature()); fflush(stdout);
                            fixExitsAfterSplit(symRef, correspondingSymRefCandidate, correspondingSymRefs,
                                loopInvariantBlock, &blocksInLoop, loopInvariantBlock->getEntry()->getNode(),
                                *registerCandidates, structureNode, replacedAutosInCurrentLoop, origSymRef);
                        }
                    }
                ...
                _origSymRefCount
                    = comp()->getSymRefCount(); // _origSymRefCount was always zero becasue there was no definition
                _origSymRefs = (TR::SymbolReference **)trMemory()->allocateStackMemory(
...
void TR_LiveRangeSplitter::replaceAutosUsedIn(TR::TreeTop *currentTree, TR::Node *node, TR::Node *parent,
    TR::Block *block, List<TR::Block> *blocksInLoop, List<TR::Block> *exitBlocks, vcount_t visitCount,
    int32_t executionFrequency, SymRefCandidateMap &registerCandidates, TR_SymRefCandidatePair **correspondingSymRefs,
    TR_BitVector *replacedAutosInCurrentLoop, TR_BitVector *autosThatCannotBeReplacedInCurrentLoop,
    TR_StructureSubGraphNode *loop, TR::Block *loopInvariantBlock)
{
    ...
    if (node->getOpCode().isLoadVarDirect() || (node->getOpCodeValue() == TR::loadaddr)
        ...
        TR::SymbolReference *symRef = node->getSymbolReference();
        comp()->setCurrentBlock(block);
        if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp())
            ...
            if (!correspondingSymRefCandidate) {
                ...
                if (origSymRef) {
                    ...
                    if (!rc) {
                        rc = comp()->getGlobalRegisterCandidates()->find(origSymRef);
                        if (!rc) {
                            static char *dontCreateCandidatesJustForSplitter
                                = feGetEnv("TR_dontCreateCandidatesJustForSplitter");
                            if (!dontCreateCandidatesJustForSplitter
                                && performTransformation(comp(),
                                    "%s live range splitter creating candidate for #%d in loop %d\n", OPT_DETAILS,
                                    origSymRef->getReferenceNumber(), loop->getNumber())) {
                                rc = comp()->getGlobalRegisterCandidates()->findOrCreate(origSymRef);
                                rc->addAllBlocksInStructure(loop->getStructure(), comp(),
                                    trace() ? "range splitter candidate" : NULL);
                            }
                        }
                        registerCandidates[origSymRef->getReferenceNumber()] = rc;
                    }
                ...
                if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalGPRs() - 2)
                        || (isFloat
                            && (_numberOfFPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalFPRs() - 2))
                    &&
                    /* candidateIsLiveOnExit && */ candidateIsLiveOnEntry
                    && performTransformation(comp(), "%s replace auto #%d in loop %d (%p)\n", OPT_DETAILS,
                        symRef->getReferenceNumber(), loop->getNumber(), loop)) {
                    _changedSomething = true;
                    ...
                    correspondingSymRefCandidate
                        = splitAndFixPreHeader(symRef, correspondingSymRefs, loopInvariantBlock, node);
            ...
            if (correspondingSymRef) {
                ...
                if (!node->getOpCode().isStoreDirect()
                    || (!dontReplaceStores
                        || performTransformation(comp(),
                            "%s --- going to replace auto #%d by auto #%d on %s node %p --- \n", OPT_DETAILS,
                            symRef->getReferenceNumber(), correspondingSymRef->getReferenceNumber(),
                            node->getOpCode().getName(), node))) {
                    ...
                    TR::Node *storeNode = TR::Node::createWithSymRef(
                        comp()->il.opCodeForDirectStore(correspondingSymRef->getSymbol()->getDataType()), 1, 1,
                        node->getFirstChild(), correspondingSymRef);
                    traceMsg(comp(),
                        " --- created a store to auto #%d adjacent to existing store to auto #%d at %s node %p --- \n",
                        correspondingSymRef->getReferenceNumber(), symRef->getReferenceNumber(),
                        node->getOpCode().getName(), node);
                ...
                fixExitsAfterSplit(symRef, correspondingSymRefCandidate, correspondingSymRefs, loopInvariantBlock,
                    blocksInLoop, node, registerCandidates, loop, replacedAutosInCurrentLoop, origSymRef);
            }
        }
    }
    ...
    int32_t childNum;
    for (childNum = 0; childNum < node->getNumChildren(); childNum++)
        replaceAutosUsedIn(currentTree, node->getChild(childNum), node, block, blocksInLoop, exitBlocks, visitCount,
            executionFrequency, registerCandidates, correspondingSymRefs, replacedAutosInCurrentLoop,
            autosThatCannotBeReplacedInCurrentLoop, loop, loopInvariantBlock);
}

TR_SymRefCandidatePair *TR_LiveRangeSplitter::splitAndFixPreHeader(TR::SymbolReference *symRef,
    TR_SymRefCandidatePair **correspondingSymRefs, TR::Block *loopInvariantBlock, TR::Node *node)
{
    TR::SymbolReference *correspondingSymRef = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
        symRef->getSymbol()->getDataType(), symRef->getSymbol()->isPinningArrayPointer());

    if (symRef->getSymbol()->isNotCollected() && correspondingSymRef->getSymbol()->isCollectedReference())
    ...
    TR_SymRefCandidatePair *correspondingSymRefCandidate
        = new (trStackMemory()) TR_SymRefCandidatePair(correspondingSymRef, 0);
...
void TR_LiveRangeSplitter::fixExitsAfterSplit(TR::SymbolReference *symRef,
    TR_SymRefCandidatePair *correspondingSymRefCandidate, TR_SymRefCandidatePair **correspondingSymRefs,
    TR::Block *loopInvariantBlock, List<TR::Block> *blocksInLoop, TR::Node *node,
    SymRefCandidateMap &registerCandidates, TR_StructureSubGraphNode *loop, TR_BitVector *replacedAutosInCurrentLoop,
    TR::SymbolReference *origSymRef)
{
    TR::SymbolReference *correspondingSymRef = correspondingSymRefCandidate->_symRef;
    ...
    if (correspondingSymRef) {
        ...
        if (!replacedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
            ...
            if (rc && !correspondingRc) {
                ...
                TR_BitVector *blocksInInnerLoop = new (trStackMemory())
                    TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
                ListIterator<TR::Block> blocksIt(blocksInLoop);
                TR::Block *nextBlock;
                for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
                        ...
                        if (!blocksInInnerLoop->get(nextBlock->getNumber())) {
                            if (trace())
                                traceMsg(comp(), "Adding original candidate #%d in block_%d in outer loop %d (%p)\n",
                                    rc->getSymbolReference()->getReferenceNumber(), nextBlock->getNumber(),
                                    parentOfLoop->getNumber(), parentOfLoop);
                            rc->addBlock(nextBlock, 0);
                        }
                    }
                }
...
void TR_LiveRangeSplitter::appendStoreToBlock(TR::SymbolReference *storeSymRef, TR::SymbolReference *loadSymRef,
    TR::Block *block, TR::Node *node)
{
    TR::Node *initNode
        = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(storeSymRef->getSymbol()->getDataType()), 1, 1,
            TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(loadSymRef->getSymbol()->getDataType()), 0,
                loadSymRef),
            storeSymRef);
    ...
    if (placeHolderNode->getOpCode().isResolveOrNullCheck() || (placeHolderNode->getOpCodeValue() == TR::treetop))
    ...
    while (placeHolderTreeForLoad && placeHolderTreeForLoad->getNode()->getOpCode().isStore()) {
        if ((placeHolderTreeForLoad->getNode()->getSymbolReference() == loadSymRef)
            || (placeHolderTreeForLoad->getNode()->getSymbolReference()->sharesSymbol()
                && placeHolderTreeForLoad->getNode()->getSymbolReference()->getUseDefAliases().contains(loadSymRef,
                    comp())))
            break;
...
void TR_LiveRangeSplitter::prependStoreToBlock(TR::SymbolReference *storeSymRef, TR::SymbolReference *loadSymRef,
    TR::Block *block, TR::Node *node)
{
    ...
    while (placeHolderTreeForStore && (placeHolderTreeForStore->getNode()->getOpCodeValue() == TR::treetop)) {
        treetopsFound = true;
        TR::Node *ttChild = placeHolderTreeForStore->getNode()->getFirstChild();
        if (!ttChild->getOpCode().hasSymbolReference() || !ttChild->getSymbolReference()->getSymbol()->isAutoOrParm()
            || (ttChild->getSymbolReference() == storeSymRef)
            || (ttChild->getSymbolReference()->sharesSymbol()
                && ttChild->getSymbolReference()->getUseDefAliases().contains(storeSymRef, comp())))
            break;
        placeHolderTreeForStore = placeHolderTreeForStore->getNextTreeTop();
    }

    if (!treetopsFound || (placeHolderTreeForStore != nextTree)) {
        TR::Node *ttNode = TR::Node::create(TR::treetop, 1, initNode->getFirstChild());
        if (trace())
            dumpOptDetails(comp(), "creating treetop node %p\n", ttNode);
        TR::TreeTop *ttTree = TR::TreeTop::create(comp(), ttNode, 0, 0);
...
void TR_LiveRangeSplitter::placeStoresInLoopExits(TR::Node *node, TR_StructureSubGraphNode *loop,
    List<TR::Block> *blocksInLoop, TR::SymbolReference *orig, TR::SymbolReference *replacement)
{
    ...
    TR_BitVector *exitsFromCurrentLoop
    ...
    TR::Block *exitBlock;
    for (exitBlock = exitBlocksIt.getCurrent(); exitBlock; exitBlock = exitBlocksIt.getNext()) {
        if ((exitBlock != comp()->getFlowGraph()->getEnd()) && exitsFromCurrentLoop->get(exitBlock->getNumber())) {
            ...
            if (!(exitBlock->getPredecessors().size() == 1)) {
                ...
                for (auto nextEdge = exitBlock->getPredecessors().begin();
                     nextEdge != exitBlock->getPredecessors().end(); ++nextEdge) {
                ...
                for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
                    ...
                    if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && !_splitBlocks.isEmpty()) {
                        ListIterator<TR::Block> splitBlocksIt(&_splitBlocks);
                        TR::Block *nextSplitBlock = splitBlocksIt.getFirst();
                        for (nextSplitBlock = splitBlocksIt.getFirst(); nextSplitBlock != NULL;
                             nextSplitBlock = splitBlocksIt.getNext()) {
                            ...
                            if (exitBlockIsASplitBlock
                                ...
                                if (!exitBlockIsASplitBlock) {
                                    ...
                                    if (pred->getLastRealTreeTop()->getNode()->getBranchDestination()
                                        == exitBlock->getEntry())
                                        pred->getLastRealTreeTop()->getNode()->setBranchDestination(
                                            splitBlock->getEntry());
                                ...
                                dumpOptDetails(comp(),
                                    " place store-back of auto #%d using auto #%d in (existing) loop exit block_%d "
                                    "between pred %d and exit block_%d\n",
                                    orig->getReferenceNumber(), replacement->getReferenceNumber(),
                                    splitBlock->getNumber(), pred->getNumber(), exitBlock->getNumber());
                                break;
                            }
                        }
                    }
                    ...
                    if (!splitBlock) {
                        ...
                        dumpOptDetails(comp(),
                            " place store-back of auto #%d using auto #%d in (new) loop exit block_%d between pred %d "
                            "and exit block_%d\n",
                            orig->getReferenceNumber(), replacement->getReferenceNumber(), splitBlock->getNumber(),
                            pred->getNumber(), exitBlock->getNumber());
                    }
