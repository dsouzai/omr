    ...
    : _compilation(c)
{
    ...
    for (i = 2; i < _topDfNum + 1; i++) {
        ...
        for (j = 0; j < _numBlocks - 1; j++) {
            if (bucket->get(j)) {
                if (debug("traceDOM")) {
                    dumpOptDetails(comp(), "   Dominator of [%d] is [%d] \n", _info[i]._block->getNumber(),
                        _info[j + 1]._block->getNumber());
...
void TR_DominatorsChk::findDominators(TR::Block* start)
{
    ...
    while (change) {
        ...
        for (i = 2; i < _topDfNum + 1; i++) {
            ...
            for (auto pred = w->_block->getExceptionPredecessors().begin();
                 pred != w->_block->getExceptionPredecessors().end(); ++pred) {
                predinfo = _info + (_dfNumbers[toBlock((*pred)->getFrom())->getNumber()] + 1);
                (*intersection) &= (*(predinfo->_bucket));
            }
...
{
    TR_Array<StackInfo>* stack
        = new (trStackMemory()) TR_Array<StackInfo>(trMemory(), _numBlocks / 2, false, stackAlloc);
