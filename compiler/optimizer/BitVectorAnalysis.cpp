...
//
//
template<class Container> bool TR_BasicDFSetAnalysis<Container*>::supportsGenAndKillSets() { return false; }

template<class Container> void TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfo() { }

template<class Container> TR_DataFlowAnalysis::Kind TR_BasicDFSetAnalysis<Container*>::getKind()
{
...
bool TR_BasicDFSetAnalysis<Container*>::performAnalysis(TR_Structure* rootStructure, bool checkForChanges)
{
...
template<class Container> void TR_BasicDFSetAnalysis<Container*>::allocateContainer(Container** result, bool, bool)
{
    *result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}

template<class Container>
void TR_BasicDFSetAnalysis<Container*>::allocateBlockInfoContainer(Container** result, bool, bool)
{
    *result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}

template<class Container>
void TR_BasicDFSetAnalysis<Container*>::allocateBlockInfoContainer(Container** result, Container*)
{
    *result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}

template<class Container> void TR_BasicDFSetAnalysis<Container*>::allocateTempContainer(Container** result, Container*)
{
    *result = new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template<class Container> void TR_BasicDFSetAnalysis<Container*>::initializeBasicDFSetAnalysis()
{
    ...
    if (comp()->getMethodSymbol()->mayHaveNestedLoops() && !comp()->getOption(TR_DisableNewBVA)) {
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForStructure(TR_Structure* s)
{
    TR_RegionStructure* region = s->asRegion();
    if (region) {
        if (region->containsImproperRegion() || !canGenAndKillForStructure(region)) {
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfoPropertyForStructure(TR_Structure* s, bool inLoop)
{
...
template<class Container>
bool TR_ForwardDFSetAnalysis<Container*>::canGenAndKillForStructure(TR_Structure* nodeStructure)
{
    if (!this->supportsGenAndKillSetsForStructures())
        return false;

    if (this->_hasImproperRegion || nodeStructure->containsImproperRegion())
        return false;
    ...
    if (naturalLoop && naturalLoop->isNaturalLoop()) {
        ...
        for (TR::CFGEdge* edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
            ...
            for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
                ...
                if (fromNode->hasSuccessor(entryNode) || fromNode->hasExceptionSuccessor(entryNode)) {
                    if (seenNonBackEdge) {
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure* region)
{
    ...
    analysisInfo->_regularGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_regularKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_exceptionGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_exceptionKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_currentRegularGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_currentRegularKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_currentExceptionGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    analysisInfo->_currentExceptionKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    ...
    for (TR::CFGEdge* edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
        ...
        if (!exitNodes.get(toStructureNumber)) {
            ...
            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = new (this->trStackMemory())
                typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(b, toStructureNumber);
    ...
    if (region->isNaturalLoop()) {
        ...
        Container* exceptionKillBitVector
            = analysisInfo->getContainer(analysisInfo->_exceptionKillSetInfo, region->getNumber());
        Container* exceptionGenBitVector
            = analysisInfo->getContainer(analysisInfo->_exceptionGenSetInfo, region->getNumber());
        ...
        if (analysisInfo->_regularGenSetInfo) {
            for (pair = analysisInfo->_regularGenSetInfo->getFirst(); pair; pair = pair->getNext()) {
                if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
                    this->_temp->empty();
                    if (genBitVector)
                    ...
                    Container* killForThisExit
                        = analysisInfo->getContainer(analysisInfo->_regularKillSetInfo, pair->_nodeNumber);
                    if (killForThisExit)
                        *this->_temp -= *killForThisExit;

                    Container* genForThisExit
                        = analysisInfo->getContainer(analysisInfo->_regularGenSetInfo, pair->_nodeNumber);
                    if (genForThisExit)
            ...
            for (pair = analysisInfo->_exceptionGenSetInfo->getFirst(); pair; pair = pair->getNext()) {
                if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
                    ...
                    if (exceptionGenBitVector)
                        *this->_temp |= *exceptionGenBitVector;
                    Container* killForThisExit
                        = analysisInfo->getContainer(analysisInfo->_exceptionKillSetInfo, pair->_nodeNumber);
                    if (killForThisExit)
                        *this->_temp -= *killForThisExit;
                    Container* genForThisExit
                        = analysisInfo->getContainer(analysisInfo->_exceptionGenSetInfo, pair->_nodeNumber);
                    if (genForThisExit)
                        *this->_temp |= *genForThisExit;
                    compose((pair->_container), this->_temp);
                }
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(
    TR_RegionStructure* regionStructure, TR_BitVector& pendingList)
{
    while (this->_analysisQueue.getListHead()
        ...
        typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
            = this->getAnalysisInfo(regionStructure);
        if (node != regionStructure->getEntry()) {
            ...
            for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
                ...
                if (pendingList.get(predStructure->getNumber())
                    && !predStructure->hasBeenAnalyzedBefore()) //_firstIteration)
                {
                } else {
                    Container* predBitVector = NULL;
                    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* predInfo
                        = this->getAnalysisInfo(predStructure);
                    int32_t nodeNumber = predStructure->asRegion() ? node->getNumber() : predNode->getNumber();
                    TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>
                        *predCurrentGenSetInfo
                        = normalPred ? predInfo->_currentRegularGenSetInfo : predInfo->_currentExceptionGenSetInfo,
                        *predCurrentKillSetInfo
                        = normalPred ? predInfo->_currentRegularKillSetInfo : predInfo->_currentExceptionKillSetInfo;
                ...
                if (this->traceBVA()) {
                    dumpOptDetails(this->comp(), "Node %p (%d) pred %p (%d)\n", nodeStructure->getStructure(),
                        nodeStructure->getStructure()->getNumber(), predStructure, predStructure->getNumber());
        ...
        *temp2 = *_currentRegularKillSetInfo;
        ...
        typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* nodeInfo
            = this->getAnalysisInfo(nodeStructure->getStructure());
        this->_nodesInCycle->empty();
        ...
        *this->_temp2 = *_currentRegularKillSetInfo;
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            ...
            *_currentRegularKillSetInfo = *this->_temp2;
            TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_nodeGenSetInfo
                = normalSucc ? nodeInfo->_regularGenSetInfo : nodeInfo->_exceptionGenSetInfo,
            ...
            if (regionStructure->isExitEdge(succ) || (succNode == regionStructure->getEntry())) {
                TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_genSetInfo
                    = normalSucc ? analysisInfo->_regularGenSetInfo : analysisInfo->_exceptionGenSetInfo,
                    *_killSetInfo
                    = normalSucc ? analysisInfo->_regularKillSetInfo : analysisInfo->_exceptionKillSetInfo;
                typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
                ...
                TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_currentGenSetInfo
                    = normalSucc ? nodeInfo->_currentRegularGenSetInfo : nodeInfo->_currentExceptionGenSetInfo,
                    *_currentKillSetInfo
                    = normalSucc ? nodeInfo->_currentRegularKillSetInfo : nodeInfo->_currentExceptionKillSetInfo;
            ...
            if ((!regionStructure->isExitEdge(succ)) && (pendingList.get(succNode->getNumber()))) {
                this->_nodesInCycle->empty();
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForBlock(TR_BlockStructure* s)
{
    ...
    analysisInfo->_regularGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
        = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
            this->_regularGenSetInfo[s->getNumber()], s->getNumber());
    analysisInfo->_regularGenSetInfo->add(pair);

    analysisInfo->_regularKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
        this->_regularKillSetInfo[s->getNumber()], s->getNumber());
    analysisInfo->_regularKillSetInfo->add(pair);

    analysisInfo->_exceptionGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
        this->_exceptionGenSetInfo[s->getNumber()], s->getNumber());
    analysisInfo->_exceptionGenSetInfo->add(pair);

    analysisInfo->_exceptionKillSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(
        this->_exceptionKillSetInfo[s->getNumber()], s->getNumber());
    analysisInfo->_exceptionKillSetInfo->add(pair);

    analysisInfo->_currentRegularGenSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    pair = new (this->trStackMemory())
        typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(NULL, s->getNumber());
    analysisInfo->_currentRegularGenSetInfo->add(pair);
...
template<class Container>
typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* TR_BasicDFSetAnalysis<Container*>::getAnalysisInfo(
    TR_Structure* s)
{
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
        = (typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo*)s->getAnalysisInfo();
    if (!s->hasBeenAnalyzedBefore()) {
...
template<class Container>
typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* TR_BasicDFSetAnalysis<Container*>::createAnalysisInfo()
{
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
        = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo;
    analysisInfo->_inSetInfo = initializeInfo(NULL);
    analysisInfo->_outSetInfo = new (this->trStackMemory())
        TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR_Structure* s)
{
    TR_RegionStructure* region = s->asRegion();
    if (region)
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR::Block* block)
{
    ...
    for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
        ...
        typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = new (this->trStackMemory())
            typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(b, succBlock->getNumber());
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR_RegionStructure* region)
{
...
template<class Container>
void TR_BasicDFSetAnalysis<Container*>::clearAnalysisInfo(
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info)
{
...
template<class Container>
bool TR_ForwardDFSetAnalysis<Container*>::analyzeRegionStructure(
    TR_RegionStructure* regionStructure, bool checkForChange)
{
    // Use information from last time we analyzed this structure; if
    // analyzing for the first time, initialize information
    //
    typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
        = NULL; // this->getAnalysisInfo(regionStructure);
    ...
    while (changed) {
...

        {
            ...
            if (this->analyzeNodeIfPredecessorsAnalyzed(regionStructure, pendingList)) {
                if (!this->supportsGenAndKillSets() || !this->canGenAndKillForStructure(regionStructure))
                    changed = true;
            }
        }
    ...
    if (regionStructure != this->_cfg->getStructure()) {
        ...
        for (TR::CFGEdge* edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
            ...
            typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* fromAnalysisInfo
                = this->getAnalysisInfo(edgeFrom->getStructure());
            ...
            if (this->supportsGenAndKillSets() && this->canGenAndKillForStructure(regionStructure)) {
                ...
                if (!pendingList.get(toStructureNumber)) {
                    ...
                    if (checkForChange && !changed && !(*fromBitVector == *toBitVector))
                        changed = true;
...
template<class Container>
bool TR_ForwardDFSetAnalysis<Container*>::analyzeNodeIfPredecessorsAnalyzed(
    TR_RegionStructure* regionStructure, TR_BitVector& pendingList)
{
    ...
    while (this->_analysisQueue.getListHead()
        ...
        for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
            ...
            if (pendingList.get(predStructure->getNumber())
                && !predStructure->hasBeenAnalyzedBefore()) // this->_firstIteration)
            {
                if (firstPred)
        ...
        typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* regionAnalysisInfo
            = this->getAnalysisInfo(regionStructure);

        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            ...
            for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
                ...
                TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_killSetInfo
                    = normalSucc ? analysisInfo->_regularKillSetInfo : analysisInfo->_exceptionKillSetInfo,
                    ...
                    *_info -= *bitVector;
                    if (this->traceBVA()) {
                        traceMsg(this->comp(),
                            ...
                            (normalSucc ? "" : "E"), nodeStructure->getStructure(),
                            nodeStructure->getStructure()->getNumber(), succNode->getNumber());
                        bitVector->print(this->comp());
                        traceMsg(this->comp(), "\n%s Info for Region or Block : %p numbered %d and exit %d is : \n",
                            (normalSucc ? "Normal" : "Exception"), nodeStructure->getStructure(),
                            nodeStructure->getStructure()->getNumber(), succNode->getNumber());
        ...
        if (!this->supportsGenAndKillSets() || !canGenAndKillForStructure(regionStructure))
            needToIterate = true;
...
bool TR_ForwardDFSetAnalysis<Container*>::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    if (this->supportsGenAndKillSets() && canGenAndKillForStructure(blockStructure)) {
        ...
        typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo
            = this->getAnalysisInfo(blockStructure);
...
template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::analyzeBlockZeroStructure(TR_BlockStructure* blockStructure)
{
    analyzeTreeTopsInBlockStructure(blockStructure);
}

template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::analyzeNode(
    TR::Node* node, vcount_t visitCount, TR_BlockStructure* blockStructure, Container* analysisInfo)
{ }

template<class Container>
void TR_ForwardDFSetAnalysis<Container*>::analyzeTreeTopsInBlockStructure(TR_BlockStructure* blockStructure)
{
    TR::Block* block = blockStructure->getBlock();
...
template<class Container>
Container* TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo::getContainer(
    TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>* list, int32_t n)
{
    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair;
...
TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo::getContainerNodeNumberPair(
    TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>* list, int32_t n)
{
    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair;
    for (pair = list->getFirst(); pair; pair = pair->getNext()) {
