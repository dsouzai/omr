...
bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
{
    if (!comp()->getOption(TR_EnableOSR)) {
        ...
        } else if (disableOSRPointDeadslotsBookKeeping) // save some compile time
        {
            if (comp()->getOption(TR_TraceOSR))
                traceMsg(comp(),
                    "Dead slots bookkeeping is disabled and therefore OSRLiveRangeAnlysis is not needed\n");
            return false;
        }
    }
 ...
 */
void TR_OSRLiveRangeAnalysis::buildDeadPendingPushSlotsInfo(TR::Node *node, TR_BitVector *livePendingPushSymRefs,
...
void TR_OSRLiveRangeAnalysis::intersectWithExistingDeadSlots(TR_OSRPoint *osrPoint, TR_BitVector *deadPPSSlots,
    TR_BitVector *deadAutoSlots, bool containsPendingPushes)
{
    TR_ByteCodeInfo &bcInfo = osrPoint->getByteCodeInfo();
...
int32_t TR_OSRLiveRangeAnalysis::fullAnalysis(bool includeParms, bool containsPendingPushes)
{
...

    {
        ...
        TR_Liveness liveLocals(comp(), optimizer(), comp()->getFlowGraph()->getStructure(), ignoreOSRuses, NULL, false,
            includeParms);
        ...
        int32_t *liveLocalIndexToSymRefNumberMap
            = (int32_t *)trMemory()->allocateStackMemory(numBits * sizeof(int32_t));
                ...
                if ((isPotentialOSRPoint ||
                    ...
                    if (!_sharedSymRefs->isEmpty())
                        buildOSRSlotSharingInfo(node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap,
                            _sharedSymRefs);
                    if (comp()->getOption(TR_FullSpeedDebug))
                        buildDeadSlotsInfo(node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap,
                            containsPendingPushes);
                }
...
TR::TreeTop *TR_OSRLiveRangeAnalysis::collectPendingPush(TR_ByteCodeInfo bci, TR::TreeTop *tt, TR_BitVector *liveVars)
{
    while (comp()->getMethodSymbol()->isOSRRelatedNode(tt->getNode(), bci)) {
        ...
        if (node->getOpCode().isStoreDirect()) {
            ...
            TR_ASSERT(0, "Unexpected OSR related node %p found, should be either pending push store or anchored load",
                node);
        }
