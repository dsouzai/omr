        ...
        false /* !cannotOmitTrivialDefs */, false /* !conversionRegsOnly */, false /* !doCompletion */)
{
    ...
    prepareUseDefInfo(false /* !requiresGlobals */, false /* !prefersGlobals */, false /* !cannotOmitTrivialDefs */,
...
{
    if (!infoIsValid()) {
        // TR_ASSERT(0, "osr def analysis failed. need to undo whatever we did for OSR and disable OSR but we don't.
    ...
    for (auto i = 0U; i < methodSymbol->getOSRPoints().size(); ++i) {
        ...
        if (info) {
            ...
            while (cursor.hasMoreElements()) {
                ...
                if (j < getNumExpandedDefsOnEntry())
                    continue;
                ...
                if (slot >= methodSymbol->getFirstJitTempIndex())
                    continue;
                ...
                if (methodSymbol->sharesStackSlot(defSymRef)) {
                    ...
                    int symRefOrder = 0;
                    for (TR::SymbolReference* symRef = listIt.getFirst(); symRef;
                         symRef = listIt.getNext(), symRefOrder++)
                        if (symRef == defSymRef)
                            break;
                    TR_ASSERT(symRefOrder < list->getSize(), "symref #%d on node n%dn not found\n",
                        defSymRef->getReferenceNumber(), defNode->getGlobalIndex());
                    comp()->getOSRCompilationData()->addSlotSharingInfo(point->getByteCodeInfo(), slot, symRefNum,
                        symRefOrder, static_cast<int32_t>(defSymRef->getSymbol()->getSize()), takesTwoSlots);
                    if (trace()) {
                        TR_ByteCodeInfo& bcInfo = point->getByteCodeInfo();
                        traceMsg(comp(),
                            "added (callerIndex=%d, bcIndex=%d)->(slot=%d, ref#=%d) at OSR point %d side %d def %d\n",
...
void TR_OSRDefInfo::addSharingInfo(AuxiliaryData& aux)
{
    ...
    TR_BitVector* prevTwoSlotUnionDef = new (comp()->trMemory()->currentStackRegion())
        TR_BitVector(getBitVectorSize(), comp()->trMemory()->currentStackRegion());
    TR_BitVector* twoSlotUnionDef = new (comp()->trMemory()->currentStackRegion())
        TR_BitVector(getBitVectorSize(), comp()->trMemory()->currentStackRegion());
    ...
    for (auto i = 0U; ppsListArray && i < ppsListArray->size(); ++i) {
        ...
        if ((ppsList.getSize() > 1) || isTwoSlotSymRefAtThisSlot || isTwoSlotSymRefAtPrevSlot) {
            unionDef.empty();
            ...
            for (TR::SymbolReference* symRef = ppsIt.getFirst(); symRef; symRef = ppsIt.getNext()) {
                ...
                *aux._defsForSymbol[symIndex] = unionDef;
                ...
                if (!prevTwoSlotUnionDef->isEmpty()) {
                    ...
                    for (TR::SymbolReference* prevSymRef = prevppsIt.getFirst(); prevSymRef;
                         prevSymRef = prevppsIt.getNext()) {
    ...
    for (auto i = 0U; autosListArray && i < autosListArray->size(); ++i) {
        ...
        if ((autosList.getSize() > 1) || isTwoSlotSymRefAtThisSlot || isTwoSlotSymRefAtPrevSlot) {
            ...
            for (TR::SymbolReference* symRef = autosIt.getFirst(); symRef; symRef = autosIt.getNext()) {
                ...
                *aux._defsForSymbol[symIndex] = unionDef;
                ...
                if (!prevTwoSlotUnionDef->isEmpty()) {
                    ...
                    for (TR::SymbolReference* prevSymRef = prevautosIt.getFirst(); prevSymRef;
                         prevSymRef = prevautosIt.getNext()) {
                        TR::DataType prevdt = prevSymRef->getSymbol()->getDataType();
                        bool doesPrevTakesTwoSlots = prevdt == TR::Int64 || prevdt == TR::Double;
                        if (doesPrevTakesTwoSlots) {
                            uint16_t prevSymIndex = prevSymRef->getSymbol()->getLocalIndex();
...
void TR_OSRDefInfo::processReachingDefinition(void* vblockInfo, AuxiliaryData& aux) { buildOSRDefs(vblockInfo, aux); }
...
void TR_OSRDefInfo::buildOSRDefs(void* vblockInfo, AuxiliaryData& aux)
{
    ...
    /*
       for (treeTop = comp()->getStartTree(); treeTop != NULL; treeTop = treeTop->getNextTreeTop())
          ...
          if (node->getOpCodeValue() == TR::BBStart)
             {
             block = node->getBlock();
             TR_ReachingDefinitions::ContainerType* analysisInfo =
    ...
    */
    ...
    if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
        ...
        buildOSRDefs(comp()->getStartTree()->getNode(),
            blockInfo[comp()->getStartTree()->getNode()->getBlock()->getNumber()], NULL, nextOsrPoint, NULL, aux);
        nextOsrPoint = NULL;
    }
...
void TR_OSRDefInfo::buildOSRDefs(TR::Node* node, void* vanalysisInfo, TR_OSRPoint* osrPoint, TR_OSRPoint* osrPoint2,
    TR::Node* parent, AuxiliaryData& aux)
{
    ...
    if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
        ...
        if (!defsForSymbol->isEmpty() && isExpandedDefIndex(expandedNodeIndex) && !sym->isRegularShadow()
            && !sym->isMethod()) {
            if (trace()) {
...
int32_t TR_OSRDefAnalysis::perform()
{
    ...
    if (!requiresAnalysis()) {
        ...
        if (osrMethodData->hasSlotSharingOrDeadSlotsInfo()) {
            if (trace()) {
                traceMsg(comp(),
                    "%s OSR reaching definition analysis is not required as it has already been calculated\n",
                    optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
                traceMsg(comp(), "Returning...\n");
            }
            return 0;
...
bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
{
    if (!comp()->getOption(TR_EnableOSR)) {
        ...
        if (comp()->getOption(TR_TraceOSR))
            traceMsg(comp(),
                "Should not perform OSRLiveRangeAnlysis -- OSR is not supported under the current configuration\n");
        return false;
        ...
        } else if (disableOSRPointDeadslotsBookKeeping) // save some compile time
        {
            if (comp()->getOption(TR_TraceOSR))
                traceMsg(
                    comp(), "Dead slots bookkeeping is disabled and therefore OSRLiveRangeAnlysis is not needed\n");
            return false;
        }
    }
    ...
    if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) {
        if (comp()->getOption(TR_TraceOSR)) {
            if (!_sharedSymRefs->isEmpty())
                traceMsg(comp(),
                    "OSRLiveRangeAnalysis is disabled but it is required for correctness. OSR transitions may not "
                    "work.\n");
            else
...
int32_t TR_OSRLiveRangeAnalysis::partialAnalysis()
{
    ...
    TR_ASSERT(comp()->pendingPushLivenessDuringIlgen(),
        "Partial analysis is only possible if liveness has been solved in Ilgen");
    ...
    TR_BitVector* allPendingPushSymRefs, TR_OSRPoint* osrPoint, TR_OSRMethodData* osrMethodData)
{
    ...
    *_workBitVector |= *allPendingPushSymRefs;
    ...
    if (comp()->getOption(TR_TraceOSR)) {
        traceMsg(
            comp(), "Live PP variables at OSR point %p of %p bytecode offset %d\n", node, osrMethodData, byteCodeIndex);
        if (!liveSymRefs)
            traceMsg(comp(), " NULL");
        else
 ...
 */
void TR_OSRLiveRangeAnalysis::pendingPushSlotSharingInfo(
    TR::Node* node, TR_BitVector* liveSymRefs, TR_BitVector* slotSharingSymRefs, TR_OSRPoint* osrPoint)
{
    if (liveSymRefs && !liveSymRefs->isEmpty()) {
        ...
        *_workBitVector &= *slotSharingSymRefs;
        ...
        while (bvi.hasMoreElements()) {
            ...
            ListIterator<TR::SymbolReference> listIt(list);
            int symRefOrder = 0;
            for (TR::SymbolReference* slotSymRef = listIt.getFirst(); slotSymRef;
                 slotSymRef = listIt.getNext(), symRefOrder++)
            ...
            comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(), slot, symRefNum,
                symRefOrder, static_cast<int32_t>(symRef->getSymbol()->getSize()), takesTwoSlots);
        }
    }
 ...
 */
void TR_OSRLiveRangeAnalysis::buildDeadPendingPushSlotsInfo(
    TR::Node* node, TR_BitVector* livePendingPushSymRefs, TR_OSRPoint* osrPoint)
{
    TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
    if (trace())
    ...
    if (deadPPSSlots && !deadPPSSlots->isEmpty()) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (trace())
                traceMsg(
                    comp(), "ppslot %d is dead at %d:%d\n", slot, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
            // symRefNum = -1 and symRefOrder = -1 indicate those slots should be zeroed in prepareForOSR
            comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(), slot, -1 /* symRefNum */,
                -1 /* symRefOrder */, static_cast<int32_t>(TR::Compiler->om.sizeofReferenceAddress()) /*symSize*/,
                false /*takesTwoSlots*/);
        }
    }
...
void TR_OSRLiveRangeAnalysis::intersectWithExistingDeadSlots(
    TR_OSRPoint* osrPoint, TR_BitVector* deadPPSSlots, TR_BitVector* deadAutoSlots, bool containsPendingPushes)
{
    TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
    ...
    if (slotsInfo) // remove every dead slot that's not in the current dead symbols bit vector from slotInfos
    {
        ...
        for (int i = slotInfos.size() - 1; i >= 0; i--) {
            ...
            if (slotInfo.symRefOrder == -1) {
                TR_ASSERT(slotInfo.symRefNum == -1,
                ...
                if (slotInfo.slot < 0 && containsPendingPushes) {
                    TR_ASSERT(deadPPSSlots,
                        "with slotInfo.slot < 0  and containsPendingPushes, there must be at least on pending push "
                ...
                if (slotInfo.slot >= 0) {
                    TR_ASSERT_FATAL(deadAutoSlots,
                        "with slotInfos.slot >= 0, there must be at least on auto symbol in this method");
 ...
 */
void TR_OSRLiveRangeAnalysis::buildDeadSlotsInfo(TR::Node* node, TR_BitVector* liveVars, TR_OSRPoint* osrPoint,
    int32_t* liveLocalIndexToSymRefNumberMap, bool containsPendingPushes)
{
...
int32_t TR_OSRLiveRangeAnalysis::fullAnalysis(bool includeParms, bool containsPendingPushes)
{
...

    {
        ...
        TR_Liveness liveLocals(
            comp(), optimizer(), comp()->getFlowGraph()->getStructure(), ignoreOSRuses, NULL, false, includeParms);
        ...
        if (containsPendingPushes) {
            for (i = 0; i < numPendingSlots; i++) {
                ...
                for (nextSymRef = symRefsIt.getCurrent(); nextSymRef; nextSymRef = symRefsIt.getNext()) {
                    if (nextSymRef->getSymbol()->isAuto()
                        && !nextSymRef->getSymbol()->castToAutoSymbol()->isLiveLocalIndexUninitialized()) {
                        liveLocalIndexToSymRefNumberMap
                            [nextSymRef->getSymbol()->castToAutoSymbol()->getLiveLocalIndex()]
                            = nextSymRef->getReferenceNumber();
                        nextSymRef->getSymbol()->setLocalIndex(0);
                    }
                }
            }
        ...
        while (block) {
            ...
            for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
                ...
                if (isPotentialOSRPoint && comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
                    ...
                    buildOSRLiveRangeInfo(node, _liveVars, offsetOSRPoint, liveLocalIndexToSymRefNumberMap, numBits,
                        osrMethodData, containsPendingPushes);
                    if (!_sharedSymRefs->isEmpty())
                        buildOSRSlotSharingInfo(
                            node, _liveVars, offsetOSRPoint, liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
                }
                ...
                if ((isPotentialOSRPoint ||
                        ...
                        // In involuntaryOSR mode, there are cases where the first evaluation point of a node is under a
                        // treetop that is not an OSRPoint but another treetop with the same bci is an OSRPoint. Need to
                        // call buildOSRLiveRangeInfo in this case for the first evaluation point to make sure the
                        // liveness info is bookkept correctly for this bci
                        (_visitedBCI->isSet(node->getByteCodeInfo().getByteCodeIndex())
                    ...
                    buildOSRLiveRangeInfo(node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, numBits,
                        osrMethodData, containsPendingPushes);
                    if (!_sharedSymRefs->isEmpty())
                        buildOSRSlotSharingInfo(
                            node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
                    if (comp()->getOption(TR_FullSpeedDebug))
                        buildDeadSlotsInfo(
                            node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, containsPendingPushes);
                }
            ...
            //
            if (comp()->getStartTree()->getNode()->getBlock() == block && comp()->isOutermostMethod()
                ...
                buildOSRLiveRangeInfo(comp()->getStartTree()->getNode(), _liveVars, osrPoint,
                    liveLocalIndexToSymRefNumberMap, numBits, osrMethodData, containsPendingPushes);
                if (!_sharedSymRefs->isEmpty())
                    buildOSRSlotSharingInfo(comp()->getStartTree()->getNode(), _liveVars, osrPoint,
                        liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
            }
...
TR::TreeTop* TR_OSRLiveRangeAnalysis::collectPendingPush(TR_ByteCodeInfo bci, TR::TreeTop* tt, TR_BitVector* liveVars)
{
    while (comp()->getMethodSymbol()->isOSRRelatedNode(tt->getNode(), bci)) {
        ...
        if (node->getOpCode().isStoreDirect()) {
        ...
        } else if (node->getOpCodeValue() == TR::treetop && node->getFirstChild()->getOpCode().isLoad()
            ...
            TR_ASSERT(
                0, "Unexpected OSR related node %p found, should be either pending push store or anchored load", node);
        }
...
void TR_OSRLiveRangeAnalysis::maintainLiveness(TR::Node* node, TR::Node* parent, int32_t childNum, vcount_t visitCount,
    TR_Liveness* liveLocals, TR_BitVector* liveVars, TR::Block* block)
{
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        if (local && !local->isLiveLocalIndexUninitialized()) {
            ...
            if ((!disallowOSRPPS3 || !_pendingPushSymRefs->get(node->getSymbolReference()->getReferenceNumber()))
                && ((node->getLocalIndex() == 1 || node->getOpCodeValue() == TR::loadaddr)
                    && !liveVars->isSet(localIndex))) {
    ...
    } else if (node->exceptionsRaised() && (node->getLocalIndex() <= 1)) {
        TR::Block* succ;
...
{
    TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
        "It is only safe to identify dead stores in OSRCodeBlocks and OSRInduceBlocks");
...
{
    TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
        "It is only safe to remove dead stores from OSRCodeBlocks and OSRInduceBlocks");
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
{
    ...
    if (comp()->getOSRMode() == TR::involuntaryOSR) {
        if (comp()->getOption(TR_TraceOSR))
            traceMsg(comp(),
                "Involuntary OSR enabled -- returning from OSR exception edge removal analysis since we implement "
                "involuntary OSR with exception edges.\n");
        return 0;
    }
    ...
    if (comp()->osrInfrastructureRemoved()) {
        if (comp()->getOption(TR_TraceOSR))
            traceMsg(comp(),
                "OSR infrastructure removed -- returning from OSR exception edge removal analysis since we have "
        ...
        for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();) {
            ...
            if (catchBlock->isOSRCatchBlock()
                && performTransformation(comp(),
                    "%s: Remove redundant exception edge from block_%d at [%p] to OSR catch block_%d at [%p]\n",
        ...
        for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
            ...
            while (tt
                && !((tt->getNode()->getOpCode().isCheck() || tt->getNode()->getOpCodeValue() == TR::treetop)
                    ...
                    && tt->getNode()->getFirstChild()->getSymbolReference()->getReferenceNumber()
                        == TR_prepareForOSR)) {
                tt = tt->getNextTreeTop();
            }
            ...
            for (auto edge = block->getPredecessors().begin(); edge != block->getPredecessors().end(); ++edge) {
                ...
                if (osrBlock->isOSRCodeBlock() && addDeadStores(osrBlock, alwaysDead, !firstPass))
                    ...
                    for (auto exEdge = osrBlock->getExceptionPredecessors().begin();
                         exEdge != osrBlock->getExceptionPredecessors().end(); ++exEdge) {
                        TR::Block* exBlock = toBlock((*exEdge)->getFrom());
                        TR_ASSERT(exBlock && exBlock->isOSRInduceBlock(),
                            "Predecessors to an OSR catch block should be OSR induce blocks");
            ...
            for (int32_t i = 0; i < prepareForOSR->getNumChildren(); ++i) {
                ...
                if (child->getOpCode().isLoadVar()
                    && alwaysDead.get(child->getSymbolReference()->getReferenceNumber())) {
                    if (performTransformation(comp(), "%s: Remove dead OSR load [%p] from OSR code block_%d\n",
                            OPT_DETAILS, child, block->getNumber())) {
                        TR::Node::recreate(prepareForOSR->getChild(i),
                            comp()->il.opCodeForConst(prepareForOSR->getChild(i)->getDataType()));
