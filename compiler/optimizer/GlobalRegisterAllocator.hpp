...
struct TR_PairedSymbols {
    ...
    TR_PairedSymbols(TR::SymbolReference* symRef1, TR::SymbolReference* symRef2)
        : _symRef1(symRef1)
        , _symRef2(symRef2)
    { }
...
struct TR_SymRefCandidatePair {
    ...
    TR_SymRefCandidatePair(TR::SymbolReference* symRef, TR::RegisterCandidate* rc)
        : _symRef(symRef)
        , _rc(rc)
    { }
...
public:
    ...
    virtual void replaceAutosUsedIn(TR::TreeTop* currentTree, TR::Node* node, TR::Node* parent, TR::Block* block,
        List<TR::Block>* blocksInLoop, List<TR::Block>* exitBlocks, vcount_t visitCount, int32_t executionFrequency,
        SymRefCandidateMap& registerCandidates, TR_SymRefCandidatePair** correspondingSymRefs,
    ...
    void placeStoresInLoopExits(TR::Node* node, TR_StructureSubGraphNode* loop, List<TR::Block>* blocksInLoop,
        TR::SymbolReference* orig, TR::SymbolReference* replacement);
    TR_SymRefCandidatePair* splitAndFixPreHeader(TR::SymbolReference* symRef,
        TR_SymRefCandidatePair** correspondingSymRefs, TR::Block* loopInvariantBlock, TR::Node* node);
    void fixExitsAfterSplit(TR::SymbolReference* symRef, TR_SymRefCandidatePair* correspondingSymRefCandidate,
        TR_SymRefCandidatePair** correspondingSymRefs, TR::Block* loopInvariantBlock, List<TR::Block>* blocksInLoop,
        TR::Node* node, SymRefCandidateMap& registerCandidates, TR_StructureSubGraphNode* loop,
        TR_BitVector* replacedAutosInCurrentLoop, TR::SymbolReference* origSymRef);
    void appendStoreToBlock(
        TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node);
    void prependStoreToBlock(
        TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node);
...
struct TR_ReplaceNode {
    ...
    TR_ReplaceNode()
        : _old(NULL)
        , _new(NULL) {};
    TR_ReplaceNode(TR::Node* o, TR::Node* n)
        : _old(o)
        , _new(n) {};
    TR::Node* _old;
    TR::Node* _new;
...
public:
    ...
    {
        return (_candidatesNeedingSignExtension && _candidatesNeedingSignExtension->get(symRefNum));
    }
...
private:
    ...
    virtual void markAutosUsedIn(TR::Node*, TR::Node*, TR::Node*, TR::Node**, TR::Block*, List<TR::Block>*, vcount_t,
        int32_t, SymRefCandidateMap&, TR_BitVector*, TR_BitVector*, bool);
    ...
    virtual void transformNode(
        TR::Node*, TR::Node*, int32_t, TR::TreeTop*, TR::Block*&, TR_Array<TR::GlobalRegister>&, TR_NodeMappings*);
    void addGlRegDepToExit(TR_Array<TR::Node*>&, TR::Node*, TR_Array<TR::GlobalRegister>&, TR::Block*);
    virtual void prepareForBlockExit(
        TR::TreeTop*&, TR::Node*&, TR::Block*, TR_Array<TR::GlobalRegister>&, TR::Block*, TR_Array<TR::Node*>&);
    ...
    virtual bool registerIsLiveAcrossEdge(
        TR::TreeTop*, TR::Node*, TR::Block*, TR::GlobalRegister*, TR::Block*&, int32_t);
    ...
    void createStoresForSignExt(TR::Node* node, TR::Node* parent, TR::Node* grandParent, TR::TreeTop* treetop,
    ...
    void splitLiveRanges(TR_StructureSubGraphNode *structureNode, vcount_t visitCount, TR::RegisterCandidate
    **registerCandidates); void replaceAutosUsedIn(TR::Node *node, TR::Node *parent, TR::Block * block, List<TR::Block>
    *blocksInLoop, vcount_t visitCount, int32_t executionFrequency, TR::RegisterCandidate **registerCandidates,
    TR_SymRefCandidatePair **correspondingSymRefs, TR_BitVector *replacedAutosInCurrentLoop, TR_StructureSubGraphNode
    *loop, TR::Block *loopInvariantBlock); void placeStoresInPreHeaderAndInLoopExits(TR::Node *node,
    TR_StructureSubGraphNode *loop, TR::Block *loopInvariantBlock, List<TR::Block> *blocksInLoop, TR::SymbolReference
    *orig, TR::SymbolReference *replacement); void appendStoreToBlock(TR::SymbolReference *storeSymRef,
    TR::SymbolReference *loadSymRef, TR::Block *block, TR::Node *node);
    */
protected:
    ...
    void transformMultiWayBranch(
        TR::TreeTop*, TR::Node*, TR::Block*, TR_Array<TR::GlobalRegister>&, bool regStarTransformDone = false);
    ...
    void addStoresForCatchBlockLoads(
        TR::TreeTop* appendPoint, TR_Array<TR::GlobalRegister>& extBlockRegisters, TR::Block* throwingBlock);
    bool isNodeAvailable(TR::Node* node);
    TR::GlobalRegister* getGlobalRegister(TR::Symbol*, TR_Array<TR::GlobalRegister>&, TR::Block*);
    TR::GlobalRegister* getGlobalRegisterWithoutChangingCurrentCandidate(
        TR::Symbol*, TR_Array<TR::GlobalRegister>&, TR::Block*);
