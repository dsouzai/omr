...
{
    if (block->getEntry() && !containsBarriers(block))
        transformBlock(block);
    return 0;
}
...
void TR_LocalReordering::postPerformOnBlocks() { comp()->incVisitCount(); }
...
void TR_LocalReordering::delayDefinitions(TR::Block* block)
{
    ...
    while (!(currentTree == entryTree)) {
        ...
        if (currentTree->getNode()->getOpCode().isBranch()
            || currentTree->getNode()->getOpCode().isJumpWithMultipleTargets()) {
            int32_t symRefCount = comp()->getSymRefCount();
...
void TR_LocalReordering::moveStoresEarlierIfRhsAnchored(
    TR::Block* block, TR::TreeTop* treeTop, TR::Node* node, TR::Node* parent, vcount_t visitCount)
{
    ...
    for (int32_t j = 0; j < node->getNumChildren(); j++) {
        ...
        if ((child->getLocalIndex() == 0) && (child->getReferenceCount() > 1) && (!child->getOpCode().isLoadConst())) {
    ...
    if ((numDeadChildren > 1) && (!parent || !parent->getOpCode().isCheck()) && !node->getOpCode().isTreeTop()
        && !node->getOpCode().isCall() && (node->getOpCodeValue() != TR::New)
        && (node->getOpCodeValue() != TR::newvalue) && (node->getOpCodeValue() != TR::newarray)
...
bool TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop* storeTree, TR::TreeTop* treeTop, bool strictCheck)
{
    ...
    while (currentTree != treeTop) {
        ...
        if (!stopHere) {
            if (currentNode->getOpCode()
                    .isCheckCast()) // Moving a store earlier than a checkcast usually results in suboptimal type info
                                    // (because of the way store constraints work, they only pick up the type info when
                                    // the store was encountered)
                stopHere = true;
        }
        ...
        if (stopHere) {
            if (performTransformation(comp(),
                    ...
                    storeTree->getNode(), currentTree->getNode(), currentTree->getNextTreeTop()->getNode(),
                    storeTree->getPrevTreeTop()->getNode(), storeTree->getNextTreeTop()->getNode())) {
                ...
                // dumpOptDetails(comp(), "\n%sInserting Definition @ 1 : [%p] between %p and %p (earlier between %p and
                // %p)\n", OPT_DETAILS, storeTree->getNode(), currentTree->getNode(), nextTree->getNode(),
                // origPrevTree->getNode(), origNextTree->getNode());
    ...
    if (!foundInsertionSpot
        ...
        // dumpOptDetails(comp(), "\n%sInserting Definition @ 2 : [%p] between %p and %p (earlier between %p and %p)\n",
        // OPT_DETAILS, storeTree->getNode(), currentTree->getNode(), nextTree->getNode(), origPrevTree->getNode(),
        // origNextTree->getNode());
    ...
    while (!(currentTree == exitTree)) {
        ...
        if (isAnySymInDefinedOrUsedBy(currentNode, visitCount1)) {
            if (performTransformation(comp(),
                    ...
                    originalNode, currentTree->getPrevTreeTop()->getNode(), currentTree->getNode(),
                    originalTree->getPrevTreeTop()->getNode(), originalTree->getNextTreeTop()->getNode())) {
...
bool TR_LocalReordering::isAnySymInDefinedOrUsedBy(TR::Node* node, vcount_t visitCount)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if ((!(node->getOpCode().isLoadVar() || node->getOpCode().isStore()
...
bool TR_LocalReordering::isAnySymInDefinedBy(TR::Node* node, vcount_t visitCount)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if ((!node->getOpCode().isLoadVar() || node->mightHaveVolatileSymbolReference())
