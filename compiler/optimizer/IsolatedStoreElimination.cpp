...
struct UnsafeSubexpressionRemoval // PR69613
{
    ...
    {
        TR_ASSERT(isVisited(node), "Cannot call isUnsafe on n%dn before anchorSafeChildrenOfUnsafeNodes",
            node->getGlobalIndex());
...
int32_t TR_IsolatedStoreElimination::perform()
{
    ...
    if (useDefInfo) {
        ...
        for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size(); groupIndex++) {
            ...
            if (groupOfStores
                && performTransformation(
                    comp(), "%sGlobal Store Elimination eliminating group %d:\n", optDetailString(), groupIndex)) {
                ...
                while (bviUses.hasMoreElements()) {
                    ...
                    if (treeTop->getNode()->getOpCode().isSpineCheck() && treeTop->getNode()->getFirstChild() == node) {
                        ...
                        child->recursivelyDecReferenceCount();
                        TR::Node::recreate(
                            node, comp()->il.opCodeForConst(node->getSymbolReference()->getSymbol()->getDataType()));
                        node->setFlags(0);
                        ...
                        if (usr.isUnsafe(child)) {
                            ...
                            //
                            child->recursivelyDecReferenceCount();
                            TR::Node* dummyChild = node->setAndIncChild(
                                0, TR::Node::createConstDead(child, TR::Int32, 0xbad1 /* eyecatcher */));
                            if (trace())
        ...
        for (int32_t i = _storeNodes->size() - 1; i >= 0; --i) {
            ...
            if (node
                && performTransformation(
                    comp(), "%s   Global Store Elimination eliminating : %p\n", optDetailString(), node)) {
                if (useDefInfo)
                    useDefInfo->clearNode(node->getUseDefIndex());
                if (node->getReferenceCount() < 1) {
                    node->setFlags(0);
...
bool TR_IsolatedStoreElimination::groupIsolatedStores(
    int32_t defIndex, TR_BitVector* currentGroupOfStores, TR_UseDefInfo* info)
{
...
void TR_IsolatedStoreElimination::performDeadStructureRemoval(TR_UseDefInfo* info)
{
    ...
    findStructuresAndNodesUsedIn(
        info, rootStructure, visitCount, nodesInStructure, defsInStructure, &propagateRemovalToParent);
}
    ...
    vcount_t visitCount, TR_BitVector* nodesInStructure, TR_BitVector* defsInStructure, bool* propagateRemovalToParent)
{
    ...
    if (structure->asRegion()) {
        ...
        for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
            ...
            if (findStructuresAndNodesUsedIn(
                    info, subStruct, visitCount, nodesInSubStructure, defsInSubStructure, &toPropagateRemoval)) {
...
static TR::Node* getLoopIncrementStep(
    TR::Block* block, TR::SymbolReference* symRef, bool shouldBeSub, int32_t* loopStride)
{
                                    ...
                                    if ((lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpgt)
                                        ...
                                        numIterations = TR::Node::create(TR::isub, 2,
                                            TR::Node::createWithSymRef(
                                                secondChildOfLastTree, TR::iload, 0, inductionSymRef),
            ...
            while (treeTop != exit) {
                ...
                if (node->getOpCode().isStoreDirect()) {
                    ...
                    if ((rhsOfStoreDefNode->getOpCode().getOpCodeValue() == TR::ixor)
                        ...
                        TR::Node* remNode = TR::Node::create(
                            TR::irem, 2, numIterations, TR::Node::create(rhsOfStoreDefNode, TR::iconst, 0, 2));
...
bool TR_IsolatedStoreElimination::markNodesAndLocateSideEffectIn(
    TR::Node* node, vcount_t visitCount, TR_BitVector* nodesSeen, TR_BitVector* defsSeen)
{
    if (node->getVisitCount() == visitCount)
        return false;
