...
void TR_ExpressionsSimplification::simplifyInvariantLoopExpressions(ListIterator<TR::Block>& blocks)
{
    ...
    if (!loopInfo) {
        ...
        if (loopInfo->getBoundaryNode()) {
            if (trace())
                traceMsg(
                    comp(), "Variable iterations from node %p has not been handled\n", loopInfo->getBoundaryNode());
        } else if (iters <= 0) {
            if (trace())
    ...
    for (TR::Block* currentBlock = blocks.getFirst(); currentBlock; currentBlock = blocks.getNext()) {
        if (trace())
            traceMsg(
                comp(), "Analyzing block #%d, which must be executed once per iteration\n", currentBlock->getNumber());
...
void TR_ExpressionsSimplification::setSummationReductionCandidates(TR::Node* node, TR::TreeTop* tt)
{
    ...
    if (opNode->getOpCodeValue() == TR::iadd || opNode->getOpCodeValue() == TR::isub) {
        ...
        if (firstNode->getOpCode().hasSymbolReference() && node->getSymbolReference() == firstNode->getSymbolReference()
            ...
            _candidates->add(new (trStackMemory()) SimplificationCandidateTuple(
                tt, flags32_t(SimplificationCandidateTuple::SummationReductionCandidate)));
    ...
    if (node->getOpCode().isStore() && !node->getSymbol()->isStatic() && !node->getSymbol()->holdsMonitoredObject()) {
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            if (!_currentRegion->isExprInvariant(node->getChild(i))) {
                if (trace())
                    traceMsg(
                        comp(), "Node %p: The store is not loop-invariant due to child %p\n", node, node->getChild(i));
                return;
            }
        }
...
bool TR_ExpressionsSimplification::tranformSummationReductionCandidate(
    TR::TreeTop* treeTop, LoopInfo* loopInfo, bool* isPreheaderBlockInvalid)
{
...
void TR_ExpressionsSimplification::tranformStoreMotionCandidate(TR::TreeTop* treeTop, bool* isPreheaderBlockInvalid)
{
    ...
    TR_ASSERT(
        node->getOpCode().isStore() && !node->getSymbol()->isStatic() && !node->getSymbol()->holdsMonitoredObject(),
...
void TR_ExpressionsSimplification::removeUncertainBlocks(
    TR_RegionStructure* region, List<TR::Block>* candidateBlocksList)
{
...
TR::Node* TR_ExpressionsSimplification::iaddisubSimplifier(TR::Node* invariantNode, LoopInfo* loopInfo)
{
    ...
    int32_t iters = loopInfo->getNumIterations();
    TR_ASSERT_FATAL(
        iters > 0, "iteration count (%d) must be known and positive for loop %d", iters, _currentRegion->getNumber());
