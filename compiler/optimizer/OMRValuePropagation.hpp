...
private:
    ...
    TR::TreeTop* createRangeCompareNode(
...
public:
    ...
    //
    enum SpecialRelationships { AbsoluteConstraint = -1 };
    ...
    TR::VPConstraint* addGlobalConstraint(
        TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative);
    TR::VPConstraint* addBlockConstraint(
        TR::Node* node, TR::VPConstraint* constraint, TR::Node* relative = NULL, bool mustBeValid = true);
    TR::VPConstraint* addEdgeConstraint(
        TR::Node* node, TR::VPConstraint* constraint, EdgeConstraints* contraints, TR::Node* relative = NULL);
    ...
    TR::VPConstraint* addBlockOrGlobalConstraint(
        TR::Node* node, TR::VPConstraint* constraint, bool isGlobal, TR::Node* relative = NULL);
    ...
    void removeConstraint(
        int32_t valueNumber, ValueConstraints& valueConstraints, int32_t relative = AbsoluteConstraint);
    ...
    Relationship* findValueConstraint(
        int32_t valueNumber, ValueConstraints& valueConstraints, int32_t relative = AbsoluteConstraint);
    ...
    StoreRelationship* findStoreValueConstraint(
    ...
    TR::VPConstraint* applyGlobalConstraints(
        TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative);
    ...
    struct TR_NeedRuntimeTestNullRestrictedArrayCopy {
        ...
        TR_NeedRuntimeTestNullRestrictedArrayCopy(TR::Node* dstArrRef, TR::Node* srcArrRef, TR::TreeTop* ptt,
            TR::TreeTop* ntt, TR::Block* originBlock, TR::Block* slowBlock, bool testDstArray)
            : _dstArrayRefNode(dstArrRef)
            , _srcArrayRefNode(srcArrRef)
            , _prevTT(ptt)
            , _nextTT(ntt)
    ...
    TR::TreeTop* createSameLeafTestAfterStores(
        TR::TreeTop* tree, TR::SymbolReference* child1Ref, TR::SymbolReference* child2Ref);
