...
void TR_LocalAnticipatability::analyzeBlock(TR::Block *block, vcount_t visitCount1, vcount_t visitCount2,
    TR_BitVector *temp)
{
    ...
    while (currentTree != exitTree) {
        ...
        updateAnticipatabilityForSupportedNodes(currentTree->getNode(), seenDefinedSymbolReferences,
            seenStoredSymbolReferences, block, killedExpressions, allSymbolReferences,
            allSymbolReferencesInNullCheckReference, allSymbolReferencesInStore, storeNodes, visitCount1);
        ...
        if (_isStoreTree) {
            ...
            if (!allSymbolReferencesInStore->isEmpty()) {
                ...
                *tempContainer &= *allSymbolReferencesInStore;
                ...
                if (isCurrentTreeTopAnticipatable) {
                    ...
                    *tempContainer &= *allSymbolReferencesInStore;
                    if (!tempContainer->isEmpty()) {
                        if (storeTreeParticipatesInAnalysis && !storeNodes->get(node->getLocalIndex()))
                            isCurrentTreeTopAnticipatable = false;
                    }
                }
            }
            ...
            if (storeTreeParticipatesInAnalysis) {
                ...
                if (node->getOpCode().isIndirect()
                    && ((node->getFirstChild()->getLocalIndex() != MAX_SCOUNT
                            && node->getFirstChild()->getLocalIndex() != 0)
                        && ((_downwardExposedBeforeButNotAnymore->get(node->getFirstChild()->getLocalIndex())
                            ...
                            || !_info[block->getNumber()]._downwardExposedAnalysisInfo->get(
                                node->getFirstChild()->getLocalIndex()))))
            ...
            if (_isNullCheckTree) {
                ...
                bool downwardExposed = true;
                if (((nullCheckReference->getLocalIndex() != MAX_SCOUNT) && (nullCheckReference->getLocalIndex() != 0))
                    && ((_downwardExposedBeforeButNotAnymore->get(nullCheckReference->getLocalIndex())
                            && _visitedNodes->get(nullCheckReference->getGlobalIndex()))
                        || _notDownwardExposed->get(nullCheckReference->getLocalIndex())
                        || !_info[block->getNumber()]._downwardExposedAnalysisInfo->get(
                            nullCheckReference->getLocalIndex())))
                    downwardExposed = false;

                if (downwardExposed && !_downwardExposedBeforeButNotAnymore->get(treeTopNode->getLocalIndex())
                    && !_notDownwardExposed->get(node->getLocalIndex())) {
            ...
            *tempContainer &= *symbolReferencesInCheck;
                ...
                if (!_isNullCheckTree) {
                    ...
                    for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
                        ...
                        if ((child->getLocalIndex() != MAX_SCOUNT) && (child->getLocalIndex() != 0)) {
                            ...
                            if (!child->getOpCode().isLoadConst() && (child->getOpCodeValue() != TR::PassThrough)) {
                                isCurrentTreeTopAnticipatable = false;
                                break;
                            }
                        }
                    ...
                    if ((nullCheckReference->getLocalIndex() != MAX_SCOUNT)
                        && (nullCheckReference->getLocalIndex() != 0)) {
                        if (killedExpressions->get(nullCheckReference->getLocalIndex()))
                            isCurrentTreeTopAnticipatable = false;
            ...
            if (!_isNullCheckTree) {
                ...
                for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
                    ...
                    if ((child->getLocalIndex() != MAX_SCOUNT) && (child->getLocalIndex() != 0)) {
                        if ((_downwardExposedBeforeButNotAnymore->get(child->getLocalIndex())
                                && _visitedNodes->get(child->getGlobalIndex()))
            ...
            if (downwardExposed && !_downwardExposedBeforeButNotAnymore->get(node->getLocalIndex())
                && !_notDownwardExposed->get(node->getLocalIndex())) {
                _info[block->getNumber()]._downwardExposedAnalysisInfo->set(node->getLocalIndex());
                if (trace())
        ...
        //
        updateUsesAndDefs(currentTree->getNode(), block, seenDefinedSymbolReferences, seenStoredSymbolReferences,
            seenUsedSymbolReferences, tempContainer, temp, storeNodes, visitCount2);
...
bool TR_LocalAnticipatability::updateAnticipatabilityForSupportedNodes(TR::Node *node,
    ContainerType *seenDefinedSymbolReferences, ContainerType *seenStoredSymbolReferences, TR::Block *block,
    ContainerType *killedExpressions, ContainerType *allSymbolReferences,
    ...
    ContainerType *storeNodes, vcount_t visitCount)
{
    ...
    if (visitCount <= node->getVisitCount()) {
        if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)
            ...
            if (opCode.isLoad() || node->getOpCodeValue() == TR::loadaddr) {
                if (opCode.hasSymbolReference() && (loadaddrAsLoad() || node->getOpCodeValue() != TR::loadaddr)) {
                ...
                if (!adjustInfoForAddressAdd(node, firstChild, seenDefinedSymbolReferences, seenStoredSymbolReferences,
                        killedExpressions, storeNodes, block))
                    return false;
                if (!adjustInfoForAddressAdd(node, secondChild, seenDefinedSymbolReferences, seenStoredSymbolReferences,
                        killedExpressions, storeNodes, block))
                    return false;
            } else
                return false;
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        if (!updateAnticipatabilityForSupportedNodes(child, seenDefinedSymbolReferences, seenStoredSymbolReferences,
                block, killedExpressions, allSymbolReferences, allSymbolReferencesInNullCheckReference,
                allSymbolReferencesInStore, storeNodes, visitCount))
            flag = false;
    }
 ...
 */
bool TR_LocalAnticipatability::adjustInfoForAddressAdd(TR::Node *node, TR::Node *child,
    ...
    ContainerType *killedExpressions, ContainerType *storeNodes, TR::Block *block)
{
    ...
    if (childHasSupportedOpCode) {
        if (killedExpressions->get(child->getLocalIndex())) {
            if (trace())
                (comp()->target().is64Bit()) ? traceMsg(comp(),
                                                   "\n330Definition #%d (n%dn) (aladd) is NOT locally anticipatable in "
                                                   "block_%d because of child\n",
                                                   node->getLocalIndex(), block->getNumber())
                                             : traceMsg(comp(),
                                                   "\n330Definition #%d (n%dn) (aiadd) is NOT locally anticipatable in "
                                                   "block_%d because of child\n",
                                                   node->getLocalIndex(), block->getNumber());
            return false;
        }
        ...
        if (childOpCode.isLoad() || child->getOpCodeValue() == TR::loadaddr) {
            if (childOpCode.hasSymbolReference() && (loadaddrAsLoad() || child->getOpCodeValue() != TR::loadaddr)) {
                if (seenDefinedSymbolReferences->get(child->getSymbolReference()->getReferenceNumber())
                    || (seenStoredSymbolReferences->get(child->getSymbolReference()->getReferenceNumber())
                        && ((child->getLocalIndex() == MAX_SCOUNT) || (child->getLocalIndex() == 0)
                            || !storeNodes->get(child->getLocalIndex())))) {
                    if (trace())
                        (comp()->target().is64Bit()) ? traceMsg(comp(),
                                                           "\n330Definition #%d (n%dn) (aladd) is NOT locally "
                                                           "anticipatable in block_%d because of child\n",
                                                           node->getLocalIndex(), block->getNumber())
                                                     : traceMsg(comp(),
                                                           "\n330Definition #%d (n%dn) (aiadd) is NOT locally "
                                                           "anticipatable in block_%d because of child\n",
                                                           node->getLocalIndex(), block->getNumber());
                    return false;
                }
...
void TR_LocalAnticipatability::updateUsesAndDefs(TR::Node *node, TR::Block *block,
    ContainerType *seenDefinedSymbolReferences, ContainerType *seenStoredSymbolReferences,
    ContainerType *seenUsedSymbolReferences, ContainerType *tempContainer, TR_BitVector *temp,
    ContainerType *storeNodes, vcount_t visitCount)
{
    if (visitCount <= node->getVisitCount())
        return;
        ...
        if ((opCode.isLoadVar() || (loadaddrAsLoad() && opCode.getOpCodeValue() == TR::loadaddr))
            ...
            *seenUsedSymbolReferences = *temp;
            ...
            if (opCode.isStore()) {
                ...
                //
                if (_registersScarce || (node->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef())
                    || node->getSymbolReference()->getSymbol()->isAutoOrParm() || symReference->sharesSymbol()
    ...
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++)
        updateUsesAndDefs(node->getChild(i), block, seenDefinedSymbolReferences, seenStoredSymbolReferences,
            seenUsedSymbolReferences, tempContainer, temp, storeNodes, visitCount);
}
...
void TR_LocalAnticipatability::killDownwardExposedExprs(TR::Block *block, TR::Node *node)
{
    ...
    *_temp = *(_info[block->getNumber()]._downwardExposedAnalysisInfo);
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ContainerType *tinfo
            = _localTransparency->getTransparencyInfo(node->getSymbolReference()->getReferenceNumber());
        *(_info[block->getNumber()]._downwardExposedAnalysisInfo) &= *tinfo;
        *(_info[block->getNumber()]._downwardExposedStoreAnalysisInfo) &= *tinfo;
    }
