...
#define SIMPLIFY_BRANCH_ARITHMETIC(Type, Width)                                                                        \
    if ((firstChild->getOpCode().isSub() || firstChild->getOpCode().isAdd())                                           \
        ...
        dumpOptDetails(                                                                                                \
            s->comp(), "%ssimplified arithmetic in branch [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node); \
    }
...
{
    return BinaryOpSimplifier<int8_t>(
        s, BinaryOpSimplifierHelpers::getNodeByteValue, BinaryOpSimplifierHelpers::setNodeByteValue);
}

BinaryOpSimplifier<int16_t> getShortBinaryOpSimplifier(TR::Simplifier* s)
{
    return BinaryOpSimplifier<int16_t>(
        s, BinaryOpSimplifierHelpers::getNodeShortValue, BinaryOpSimplifierHelpers::setNodeShortValue);
}

BinaryOpSimplifier<int32_t> getIntBinaryOpSimplifier(TR::Simplifier* s)
{
    return BinaryOpSimplifier<int32_t>(
        s, BinaryOpSimplifierHelpers::getNodeIntValue, BinaryOpSimplifierHelpers::setNodeIntValue);
}

BinaryOpSimplifier<int64_t> getLongBinaryOpSimplifier(TR::Simplifier* s)
{
    return BinaryOpSimplifier<int64_t>(
        s, BinaryOpSimplifierHelpers::getNodeLongValue, BinaryOpSimplifierHelpers::setNodeLongValue);
}

BinaryOpSimplifier<uint32_t> getFloatBitsBinaryOpSimplifier(TR::Simplifier* s)
{
    return BinaryOpSimplifier<uint32_t>(
        s, BinaryOpSimplifierHelpers::getNodeFloatBits, BinaryOpSimplifierHelpers::setNodeFloatBits);
}

BinaryOpSimplifier<uint64_t> getDoubleBitsBinaryOpSimplifier(TR::Simplifier* s)
{
    return BinaryOpSimplifier<uint64_t>(
        s, BinaryOpSimplifierHelpers::getNodeDoubleBits, BinaryOpSimplifierHelpers::setNodeDoubleBits);
}
...
static void convertToTestUnderMask(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand && node->getChild(1)->getOpCode().isLoadConst()
        ...
        node->setAndIncChild(
            0, TR::Node::create(TR::butest, 2, bloadi, TR::Node::bconst(iand->getChild(1)->getConst<int8_t>())));
    ...
    if (s->reassociate() && node->getOpCode().isArrayRef()) {
        ...
        if (firstChild->getOpCodeValue() == node->getOpCodeValue()
            ...
            if (performTransformation(
                    s->comp(), "%sReordering constant terms in node 0x%p\n", s->optDetailString(), node)) {
    ...
    if (s->reassociate() && node->getOpCode().isAdd()) {
      ...
      //    t   c1             c2 c1
        if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9 to node 0x%p\n", s->optDetailString(), node)) {
                ...
                if (c1->getDataType() != secondChild->getDataType()) {
                    TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
                        c1->getDataType(), secondChild->getDataType(), false /* !wantZeroExtension */);
      ...
      //        t   c2     c1 c2
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_1 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node* newFirstChild = TR::Node::create(firstChild, node->getOpCodeValue(), 2);
      ...
      //    c  t1              t1 t2
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_1_1 to node 0x%p\n", s->optDetailString(), node)) {
      ...
      //    c1  t         c1  c2
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_1_2 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node* newFirstChild = TR::Node::create(firstChild, node->getOpCodeValue(), 2);
                newFirstChild->setAndIncChild(0, firstChild->getFirstChild());
                newFirstChild->setAndIncChild(1, secondChild);
      ...
      //    t1  c1 t2 c2     t1 t2  c1 c2    t1 c1 t2 c2     t1 t2 c1 c2
            ...
            && isExprInvariant(region, secondChild->getSecondChild())) {
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_1_3 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node* newFirstChild = TR::Node::create(firstChild, node->getOpCodeValue(), 2);
                newFirstChild->setAndIncChild(0, firstChild->getFirstChild());
      ...
      //        t   c2     c1 c2
            ...
            && isExprInvariant(region, secondChild->getSecondChild())) {
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_2 to node 0x%p\n", s->optDetailString(), node)) {
      ...
      //                          c2
            ...
            && !isSmallConstant(secondChild->getSecondChild(), s)) {
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 9_3 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node* newFirstChild = TR::Node::create(firstChild, node->getOpCodeValue(), 2);
                TR::Node* newNeg = TR::Node::create(
                    secondChild->getSecondChild(), negOps[secondChild->getSecondChild()->getDataType()], 1);
                TR::Node* zero = TR::Node::create(
                    secondChild->getSecondChild(), constOps[secondChild->getSecondChild()->getDataType()], 0);
...
template<class T> TR::Node* subSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    //   that problem, the test should be moved back down there.
        ...
        && performTransformation(
            s->comp(), "%sReduce xsub from 0 [%s] to xneg \n", s->optDetailString(), node->getName(s->getDebug()))) {
...
static void orderChildrenByHighWordZero(
    TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
...
static TR::Node* foldDemotionConversion(
    TR::Node* node, TR::ILOpCodes opcode, TR::ILOpCodes foldedOpCode, TR::Simplifier* s)
{
...
static void fold2SmallerIntConstant(
    TR::Node* node, TR::Node* child, TR::DataType sdt, TR::DataType tdt, TR::Simplifier* s)
{
    int32_t val;
    switch (sdt) {
    case TR::Int16:
...
static TR::Node* intDemoteSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (sourceIs64Bit
        ...
        uint32_t childSourceSize = TR::ILOpCode::getSize(TR::ILOpCode::getProperConversion(
            childTargetDataType, childSourceDataType, wantZeroExtension)); // size of childSourceDataType (hack)
   ...
   // subtree  lconst 22
    ...
    if (sourceIs64Bit) {
        ...
        if (targetDataType == TR::Int32) {
            ...
            reduceOp = TR::ILOpCode::getProperConversion(
                TR::Int32, targetDataType, false); // appropriate integer to integer op
        }
    ...
    TR_ComparisonTypes compareType, bool isUnsignedCompare, int64_t constant, TR::Simplifier* s)
{
    ...
    if (node->getOpCode().isSelect() && node->getDataType().isIntegral() && node->getReferenceCount() == 1) {
        ...
        toReturn = processSubTreeLeavesForISelectCompare(
            visited, node->getFirstChild(), compareType, isUnsignedCompare, constant, s);
    } else {
        toReturn = false;
    }
    return toReturn;
        ...
        if (canProcessSubTreeLeavesForISelectCompare(safetyVisited, compare->getFirstChild())) {
            ...
            if (performTransformation(s->comp(),
                ...
                compare->setAndIncChild(1,
                    TR::Node::createConstZeroValue(
                        compare->getSecondChild(), compare->getSecondChild()->getDataType()));
                constVal->decReferenceCount();
                TR::Node::recreate(compare,
                    TR::ILOpCode(TR::ILOpCode::compareOpCode(
                                     compare->getFirstChild()->getDataType(), TR_cmpNE, isUnsignedCompare))
                        .convertCmpToIfCmp());
                return true;
            }
        }
                    ...
                    if (predEndWithGoto || isImmediatePred) {
                        ...
                        if (storeNode->getOpCode().isStoreDirect() && storeSymRef == symRef
                            ...
                            if (performTransformation(comp, "%sDuplicating %p and insert it after %p\n",
                                ...
                                TR::Block* newIfCmpBlock = TR::Block::createEmptyBlock(
                                    block->getEntry()->getNode(), comp, predBlock->getFrequency(), predBlock);

                                // Create new gotoBlock
                                TR::Block* gotoBlock = TR::Block::createEmptyBlock(
                                    gotoTree->getNode(), comp, predBlock->getFrequency(), predBlock);
    ...
    if (firstOp == TR::a2i && firstChild->getFirstChild()->getType().isAddress() && s->comp()->target().is32Bit()
        ...
        if ((secondOp == TR::iconst && secondChild->getInt() == 0) || (secondOp == TR::a2i)) {
            ...
            if (secondOp == TR::a2i) {
                ...
                dumpOptDetails(
                    s->comp(), "Address Compare Conversion: found child 1 a2i and child 2 iconst in node %p\n", node);
            }
        }
        ...
        if ((secondOp == TR::lconst && secondChild->getLongInt() == 0) || (secondOp == TR::a2l)) {
            ...
            if (secondOp == TR::a2l) {
                ...
                dumpOptDetails(
                    s->comp(), "Address Compare Conversion: found child 1 a2l and child 2 lconst in node %p\n", node);
            }
        }
    }
}
    ...
    TR::ILOpCodes shortOp, TR::ILOpCodes byteOp)
{
    ...
    //     performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node))
    {
        ...
        if ((firstOp == TR::i2l)
            && performTransformation(
                s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node)) {
...
static void ifjlClassSimplifier(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (node->getFirstChild()->getOpCode().hasSymbolReference()
        ...
        && performTransformation(
            s->comp(), "%sSimplify test on j/l/Class children [%p]\n", s->optDetailString(), node)) {
    ...
    char bitPosition[], char operationType[], int from, int to, int tabspace, bool isLong)
{
    ...
    if (count <= 2) {
        ...
        firstChild = generateDecomposedTree(
            parentNode, multiplierChild, s, bitPosition, operationType, from, from + mid, tabspace + 1, isLong);
        secondChild = generateDecomposedTree(
            parentNode, multiplierChild, s, bitPosition, operationType, from + mid, to, tabspace + 1, isLong);
...
static void printTree(
    TR::Simplifier* s, char bitPosition[], char operationType[], int from, int to, int tabspace, bool isLong)
{
    int count = to - from;
    int i;
...
static void decomposeMultiply(TR::Node* node, TR::Simplifier* s, bool isLong)
{
    ...
    count = decomposeConstant(
        bitPosition, operationType, isLong ? secondChild->getLongInt() : secondChild->getInt(), isLong ? 64 : 32);
    ...
    if ((!s->containingStructure() || s->getLastRun())
        && comp->cg()->mulDecompositionCostIsJustified(
            count, bitPosition, operationType, isLong ? secondChild->getLongInt() : secondChild->getInt())
...
TR::Node* getQuotientUsingMagicNumberMultiply(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::idiv || node->getOpCodeValue() == TR::irem) {
        ...
        if (shiftAmount == 0) {
            ...
            node3 = TR::Node::create(
                TR::lshr, 2, node2, TR::Node::create(node2, TR::iconst, 0, static_cast<int32_t>(shiftAmount)));
...
static void simplifyIntBranchArithmetic(
    TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
    SIMPLIFY_BRANCH_ARITHMETIC(Int, int32_t);
}

static void simplifyLongBranchArithmetic(
    TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
        ...
        if ((*ie) != outEdge) {
            ...
            if ((*ie)->getFrom()->hasSuccessor(block)) {
                ...
                if ((predBlock->getSuccessors().size() == 2) && brNode->getOpCode().isBranch()
                    ...
                    (*ie)->getFrom()->asBlock()->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                        s->comp(), nextBlock->getEntry(), block->getEntry());
...
static void intCompareNarrower(
    TR::Node* node, TR::Simplifier* s, TR::ILOpCodes ushortOp, TR::ILOpCodes shortOp, TR::ILOpCodes byteOp)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    static char* reportCompareDemotions = feGetEnv("TR_ReportCompareDemotions");
...
static TR::Node* integerToFloatHelper(uint32_t absValue, bool isUnsignedOperation, TR::Node* node, TR::Simplifier* s)
{
    ...
    if (leadingZeroes(absValue) + trailingZeroes(absValue) >= 8) {
        ...
        foldFloatConstant(
            node, isUnsignedOperation ? (float)firstChild->getUnsignedInt() : (float)firstChild->getInt(), s);
        return node;
    } else {
...
template<typename T> static bool checkAndReplaceRotation(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (!performTransformation(
            s->comp(), "%sReduced or/xor/add in node [" POINTER_PRINTF_FORMAT "] to rol\n", s->optDetailString(), node))
        return false;
...
static TR::Node* longToFloatHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node* node, TR::Simplifier* s)
{
    ...
    if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
        foldFloatConstant(
            node, isUnsignedOperation ? (float)firstChild->getUnsignedLongInt() : (float)firstChild->getLongInt(), s);
        return node;
    } else {
...
static TR::Node* longToDoubleHelper(uint64_t absValue, bool isUnsignedOperation, TR::Node* node, TR::Simplifier* s)
{
    ...
    if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 53)) {
        foldDoubleConstant(
            node, isUnsignedOperation ? (double)firstChild->getUnsignedLongInt() : (double)firstChild->getLongInt(), s);
        return node;
...
static void bitTestingOp(TR::Node* node, TR::Simplifier* s)
{
    ...
    case TR::Int8:
        foldByteConstant(
            firstChild->getSecondChild(), (bitAmount & 0xFF) >> shiftAmount, s, false /* !anchorChildren */);
        foldByteConstant(secondChild, (cmpAmount & 0xFF) >> shiftAmount, s, false /* !anchorChildren */);
        break;
    case TR::Int16:
        foldShortIntConstant(
            firstChild->getSecondChild(), (bitAmount & 0xFFFF) >> shiftAmount, s, false /* !anchorChildren */);
        foldShortIntConstant(secondChild, (cmpAmount & 0xFFFF) >> shiftAmount, s, false /* !anchorChildren */);
        break;
    case TR::Int32:
        foldIntConstant(
            firstChild->getSecondChild(), (bitAmount & 0xFFFFFFFF) >> shiftAmount, s, false /* !anchorChildren */);
...
static void bitwiseToLogical(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::SymbolReference* temp1 = s->comp()->getSymRefTab()->createTemporary(
        s->comp()->getMethodSymbol(), cmp2->getFirstChild()->getDataType());
    prevTree = TR::TreeTop::create(s->comp(), prevTree, TR::Node::createStore(temp1, cmp2->getFirstChild()));
    TR::SymbolReference* temp2 = s->comp()->getSymRefTab()->createTemporary(
        s->comp()->getMethodSymbol(), cmp2->getSecondChild()->getDataType());
    ...
    TR::Node* newCmp = TR::Node::createif(
        newOp, TR::Node::createLoad(cmp2->getFirstChild(), temp1), TR::Node::createLoad(cmp2->getSecondChild(), temp2));
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (s->comp()->cg()->getSupportsVectorRegisters()) {
        ...
        if ((node->getType().isIntegral() || node->getType().isDouble())
            ...
            TR::Node* vloadiNode = TR::Node::createWithSymRef(
                TR::ILOpCode::createVectorOpCode(TR::vloadi, vectorType), 1, 1, addressNode, newSymRef);
            TR::Node* indexNode = TR::Node::iconst(offset / (node->getSize()));

            TR::Node* vgetelemNode = TR::Node::create(
                TR::ILOpCode::createVectorOpCode(TR::vgetelem, vectorType), 2, vloadiNode, indexNode);
...
TR::Node* indirectStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((node->getOpCodeValue() == TR::awrtbari) && 0
        ...
        if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray)
            ...
            if (removeWrtBar && !s->comp()->getOptions()->realTimeGC()
                && performTransformation(s->comp(),
                    "%sFolded indirect write barrier to iastore because GC could not have occurred enough times to "
    ...
    if (s->comp()->cg()->getSupportsVectorRegisters()) {
        ...
        if ((node->getType().isIntegral() || node->getType().isDouble())
            ...
            TR::Node* vloadiNode = TR::Node::createWithSymRef(
                TR::ILOpCode::createVectorOpCode(TR::vloadi, type), 1, 1, addressNode, newSymRef);
            TR::Node* indexNode = TR::Node::iconst(offset / (node->getSize())); // idx = byte offset / element size
            TR::Node* vsetelemNode = TR::Node::create(
                TR::ILOpCode::createVectorOpCode(TR::vsetelem, type), 3, vloadiNode, indexNode, valueNode);
            auto newVStorei = TR::Node::createWithSymRef(
                TR::ILOpCode::createVectorOpCode(TR::vstorei, type), 2, 2, addressNode, vsetelemNode, newSymRef);
            dumpOptDetails(s->comp(), "[" POINTER_PRINTF_FORMAT "]\n", newVStorei);
...
TR::Node* daddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldDoubleConstant(
            node, TR::Compiler->arith.doubleAddDouble(firstChild->getDouble(), secondChild->getDouble()), s);
        return node;
    }
...
TR::Node* isubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    //   that problem, the test should be moved back down there.
        ...
        && performTransformation(
            s->comp(), "%sReduce isub from 0 [%s] to ineg \n", s->optDetailString(), node->getName(s->getDebug()))) {
        TR::Node::recreate(node, TR::ILOpCode::negateOpCode(node->getDataType()));
        node->setVisitCount(0);
        node->getFirstChild()->recursivelyDecReferenceCount();
        node->setChild(0, node->getSecondChild());
...
TR::Node* fsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldFloatConstant(
            node, TR::Compiler->arith.floatSubtractFloat(firstChild->getFloat(), secondChild->getFloat()), s);
        return node;
    }
...
TR::Node* dsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldDoubleConstant(
            node, TR::Compiler->arith.doubleSubtractDouble(firstChild->getDouble(), secondChild->getDouble()), s);
        return node;
    }
...
TR::Node* imulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        if (node->getOpCode().isUnsigned()) {
            foldUIntConstant(
                node, firstChild->getUnsignedInt() * secondChild->getUnsignedInt(), s, false /* !anchorChildren*/);
        } else {
    ...
    if (secondChildOp == TR::iconst && secondChild->getInt() < 0
        ...
        && performTransformation(
            s->comp(), "%sFound a*(-c) = -(a*c) in imul [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    } else if (secondChildOp == TR::iconst && !s->getLastRun() && secondChild->getInt() != 0
        && !isNonNegativePowerOf2(secondChild->getInt()) && secondChild->getInt() != TR::getMinSigned<TR::Int32>()) {
   ...
   //    e   c1
    ...
    if (s->getLastRun() && node->isNonNegative() && node->getOpCode().isMul()
        ...
        size > 0) {
        if (performTransformation(
                s->comp(), "%sFolded mul and div by the same constant 0x%p\n", s->optDetailString(), node)) {
            TR::Node* firstChild = node->getFirstChild();
            TR::Node* secondChild = node->getSecondChild();
   ...
   //    t   c1             c2 c1
        if (region && node->getOpCodeValue() == TR::imul && isExprInvariant(region, node->getSecondChild())
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 10 to node 0x%p\n", s->optDetailString(), node)) {
   ...
   //    e   c1          e   c2
        if (region && node->getOpCodeValue() == TR::imul && isExprInvariant(region, node->getSecondChild())
            ...
            && node->getFirstChild()->getReferenceCount() == 1) {
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 11 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node::recreate(node, TR::iadd);
   ...
   //    e1  e2          e1  c e2  c
            ...
            && node->getFirstChild()->getOpCodeValue() == TR::iadd) {
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 13 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node::recreate(node, TR::iadd);
                TR::Node* firstChild = node->getFirstChild();
                TR::Node* secondChild = node->getSecondChild();
   ...
   //    e1  e2          e1  c e2  c
            ...
            if (performTransformation(
                    s->comp(), "%sApplied reassociation rule 15 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node::recreate(node, TR::isub);
                TR::Node* firstChild = node->getFirstChild();
                TR::Node* secondChild = node->getSecondChild();
...
TR::Node* lmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    } else if (s->comp()->target().is64Bit() && secondChildOp == TR::lconst && !s->getLastRun()
        && secondChild->getLongInt() != 0 && !isNonNegativePowerOf2(secondChild->getLongInt())
...
TR::Node* fmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldFloatConstant(
            node, TR::Compiler->arith.floatMultiplyFloat(firstChild->getFloat(), secondChild->getFloat()), s);
        return node;
    }
...
TR::Node* dmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldDoubleConstant(
            node, TR::Compiler->arith.doubleMultiplyDouble(firstChild->getDouble(), secondChild->getDouble()), s);
        return node;
    }
...
TR::Node* smulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldShortIntConstant(
            node, firstChild->getShortInt() * secondChild->getShortInt(), s, false /* !anchorChildren */);
        return node;
    }
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0 && permitSimplificationOfConstantDivisor(s, node)) {
            ...
            // Design 1790
                ...
                && performTransformation(
                    s->comp(), "%sMagic number idiv opt in node %p\n", s->optDetailString(), node)) {
                ...
                node1 = TR::Node::create(
                    TR::imulh, 2, firstChild, TR::Node::create(firstChild, TR::iconst, 0, magicNumber));
...
TR::Node* ldivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0 && permitSimplificationOfConstantDivisor(s, node)) {
            if (firstChild->getOpCode().isLoadConst()) {
                ...
                if (performTransformation(
                        s->comp(), "%sReduced ldiv by -1 with lneg in node [%p]\n", s->optDetailString(), node)) {
                    // make sure we don't lose grandchildren in prepareToReplaceNode
                    firstChild->incReferenceCount();
                    ...
                    if (divisor < 0) {
                        ...
                        if (performTransformation(s->comp(),
                            ...
                            if (s->cg()->canUseImmedInstruction(divisor - 1)) {
                                ...
                                node_shl = TR::Node::create(
                                    TR::lshl, 2, node_add, TR::Node::iconst(firstChild, 63 - shiftAmount));
                ...
                node3 = TR::Node::create(
                    TR::lshr, 2, node2, TR::Node::create(node2, TR::iconst, 0, (int32_t)shiftAmount));
    ...
    if (node->getOpCodeValue() == TR::ldiv) {
        ...
        if ((firstChild->getOpCodeValue() == TR::i2l) && (secondChild->getOpCodeValue() == TR::i2l) &&
            ...
            && performTransformation(
                s->comp(), "%sReduced ldiv [%p] of two i2l children to i2l of idiv \n", s->optDetailString(), node)) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10
            ...
            && // transformToLongDiv creates a tonne of nodes.  Not good to do this recursively if you have chained
               // divides
            performTransformation(
                s->comp(), "%sReduced ldiv by 10 [%p] to bitwise ops\n", s->optDetailString(), node)) {
...
TR::Node* fdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getFloatBits() != FLOAT_POS_ZERO
        ...
        if (firstChild->getOpCode().isLoadConst()) {
            foldFloatConstant(
                node, TR::Compiler->arith.floatDivideFloat(firstChild->getFloat(), secondChild->getFloat()), s);
            return node;
        }
...
TR::Node* ddivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
            foldDoubleConstant(
                node, TR::Compiler->arith.doubleDivideDouble(firstChild->getDouble(), secondChild->getDouble()), s);
            return node;
        }
...
TR::Node* bdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (secondChild->getByte() != 0)
        ...
        if (firstChild->getOpCode().isLoadConst()) {
            foldByteConstant(
                node, (int8_t)(firstChild->getByte() / secondChild->getByte()), s, false /* !anchorChildren*/);
            return node;
        }
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0 && permitSimplificationOfConstantDivisor(s, node)) {
            if (divisor == 1 || (!isUnsigned && (divisor == -1))) {
                ...
                && performTransformation(
                    s->comp(), "%sMagic number irem opt in node %p\n", s->optDetailString(), node)) {
...
TR::Node* lremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::lrem) {
        ...
        if ((firstChild->getOpCodeValue() == TR::i2l) && (secondChild->getOpCodeValue() == TR::i2l)
            && performTransformation(
                s->comp(), "%sReduced lrem [%p] of two i2l children to i2l of irem \n", s->optDetailString(), node)) {
            TR::TreeTop* curTree = s->_curTree;
...
TR::Node* fremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getFloatBits() != FLOAT_POS_ZERO
        ...
        if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
            foldFloatConstant(
                node, TR::Compiler->arith.floatRemainderFloat(firstChild->getFloat(), secondChild->getFloat()), s);
            return node;
        }
    }
...
TR::Node* dremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldDoubleConstant(
            node, TR::Compiler->arith.doubleRemainderDouble(firstChild->getDouble(), secondChild->getDouble()), s);
        return node;
    }
...
TR::Node* sremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()
        ...
        foldShortIntConstant(
            node, (int16_t)(firstChild->getShortInt() % secondChild->getShortInt()), s, false /* !anchorChildren */);
        return node;
    }
...
TR::Node* fnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::fneg) {
        if (performTransformation(
                s->comp(), "%sTransforming [" POINTER_PRINTF_FORMAT "] --A -> A\n", s->optDetailString(), node)) {
...
TR::Node* ishlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldIntConstant(
            node, firstChild->getInt() << (secondChild->getInt() & INT_SHIFT_MASK), s, false /* !anchorChildren*/);
        return node;
    }
...
TR::Node* bshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldByteConstant(
            node, firstChild->getByte() << (secondChild->getInt() & INT_SHIFT_MASK), s, false /* !anchorChildren*/);
        return node;
    }
...
template<class T> inline TR::Node* sqrtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->getOpCode().isLoadConst()
        && performTransformation(
            s->comp(), "%sSimplify sqrt of const child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* i2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((firstChild->getOpCodeValue() == TR::isub || firstChild->getOpCodeValue() == TR::iadd)
        ...
        && performTransformation(
            s->comp(), "%sTransforming iu2a  [%s] to aiadd\n", s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node* f2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        foldShortIntConstant(
            node, (floatToInt(firstChild->getFloat(), false) << 16) >> 16, s, false /* !anchorChildren */);
        return node;
    }
...
TR::Node* d2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        foldByteConstant(
            node, (doubleToInt(firstChild->getDouble(), false) << 24) >> 24, s, false /* !anchorChildren*/);
        return node;
    }
...
TR::Node* d2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        foldShortIntConstant(
            node, (doubleToInt(firstChild->getDouble(), false) << 16) >> 16, s, false /* !anchorChildren */);
        return node;
    }
...
static TR::Node* simplifyIficmpneHelper(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()
        && conditionalBranchFold(
            (firstChild->getInt() != secondChild->getInt()), node, firstChild, secondChild, block, s))
        return node;
...
TR::Node* ificmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::ificmplt) {
        if (firstChild->getOpCode().isLoadConst()
            && conditionalBranchFold(
                (originalFirst->getInt() < originalSecond->getInt()), node, firstChild, secondChild, block, s))
            return node;
...
TR::Node* simplifyIflcmpneHelper(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()
        && conditionalBranchFold(
            (firstChild->getLongInt() != secondChild->getLongInt()), node, firstChild, secondChild, block, s))
        return node;
...
TR::Node* iflcmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::iflcmple) {
        if (firstChild->getOpCode().isLoadConst()
            && conditionalBranchFold(
                (originalFirst->getLongInt() <= originalSecond->getLongInt()), node, firstChild, secondChild, block, s))
            return node;
...
static TR::Node* simplifyIfacmpneHelper(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::aconst
        && conditionalBranchFold(
            (firstChild->getAddress() != secondChild->getAddress()), node, firstChild, secondChild, block, s))
        return node;
...
TR::Node* lucmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldIntConstant(
            node, firstChild->getUnsignedLongInt() < secondChild->getUnsignedLongInt(), s, false /* !anchorChildren*/);
        return node;
    }
...
TR::Node* sucmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        foldIntConstant(
            node, firstChild->getConst<uint16_t>() < secondChild->getConst<uint16_t>(), s, false /* !anchorChildren*/);
        return node;
    }
                ...
                if (node->getFirstChild()->getReferenceCount() == 1) {
                    if (performTransformation(s->comp(),
                        ...
                        TR::Node::recreateWithoutProperties(
                            node, oldFirstChild->getOpCode().getOpCodeForReverseBranch(), numChildren);
...
TR::Node* a2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        if (firstChild->getType().isAddress())
            ...
            foldIntConstant(
                node, static_cast<int32_t>(firstChild->get64bitIntegralValue()), s, false /* !anchorChildren */);
    } else {
...
TR::Node* variableNewSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::variableNew && node->getFirstChild()->getOpCodeValue() == TR::loadaddr
        && performTransformation(
            s->comp(), "%sReplacing TR::variableNew %p with TR::New\n", s->optDetailString(), node)) {
...
TR::Node* ifxcmpoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::ILOpCodes opCode = node->getOpCodeValue();
    TR_ASSERT(
        (opCode == TR::ificmpo) || (opCode == TR::ificmpno) || (opCode == TR::iflcmpo) || (opCode == TR::iflcmpno),
...
TR::Node* ifxcmnoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::ILOpCodes opCode = node->getOpCodeValue();
    TR_ASSERT(
        (opCode == TR::ificmno) || (opCode == TR::ificmnno) || (opCode == TR::iflcmno) || (opCode == TR::iflcmnno),
...
TR::Node* bndchkwithspinechkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (removeSpineCheck) {
        ...
        if (s->trace()) {
            traceMsg(
                s->comp(), "removing spine check from node %p, anchoring element child to %p\n", node, treeTopNode);
        }
    }
