...
int32_t OMR_InlinerPolicy::getInitialBytecodeSize(
    TR_ResolvedMethod* feMethod, TR::ResolvedMethodSymbol* methodSymbol, TR::Compilation* comp)
{
...
void TR_InlinerBase::setSizeThreshold(uint32_t size)
{
    ...
    heuristicTrace(
        tracer(), "Setting method size threshold (_methodByteCodeSizeThreshold) to %d\n", _methodByteCodeSizeThreshold);
}
    ...
    TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode, bool allConsts)
{
    ...
    if (block) {
        blockFrequency = comp()->convertNonDeterministicInput(
            block->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, randomGenerator(), 0);
        if (allowBiggerMethods()) {
            ...
            if (blockFrequency > borderFrequency) {
                ...
                bytecodeSize = scaleSizeBasedOnBlockFrequency(
                    bytecodeSize, blockFrequency, borderFrequency, calleeResolvedMethod, callNode);
    ...
    if (inlineThreshold != 0 && (uint32_t)bytecodeSize > unsigned(inlineThreshold)) {
        ...
        if (block) {
            TR::DebugCounter::prependDebugCounter(
                comp(), "inliner.callSites/failed/calleeHasTooManyBytecodes", block->getFirstRealTreeTop());
...
bool TR_DumbInliner::tryToInline(const char* message, TR_CallTarget* calltarget)
{
    ...
    if (getPolicy()->tryToInline(calltarget, NULL, true)) {
        if (comp()->trace(OMR::inlining))
            traceMsg(
                comp(), "tryToInline pattern matched; %s for %s\n", message, method->signature(comp()->trMemory()));
        return true;
    }
...
bool OMR_InlinerPolicy::tryToInlineGeneral(TR_CallTarget* calltarget, TR_CallStack* callStack, bool toInline)
{
    ...
    if (regex && TR::SimpleRegex::match(regex, calltarget->_calleeMethod)) {
        if (comp()->trace(OMR::inlining)) {
            traceMsg(
                comp(), toInline ? "Inliner: tryToInline pattern matched, " : "Inliner: dontInline pattern matched, ");
    ...
    if (callStack && callStack->_inlineFilters) {
        ...
        if (inlineFilters) {
            bool inclusive = comp()->getDebug()->methodSigCanBeFound(
                signature, inlineFilters, filterInfo, calltarget->_calleeMethod->convertToMethod()->methodType());
    ...
    TR_PrexArgInfo* argInfo, int32_t initialMaxSize)
{
    ...
    TR_CallSite* callsite = TR_CallSite::create(
        callNodeTreeTop, parent, callNode, thisClass, symRef, (TR_ResolvedMethod*)0, comp(), trMemory(), stackAlloc);
...
TR_DumbInliner::TR_DumbInliner(
    TR::Optimizer* optimizer, TR::Optimization* optimization, uint32_t initialSize, uint32_t dumbReductionIncrement)
...
bool TR_DumbInliner::inlineCallTargets(
    TR::ResolvedMethodSymbol* callerSymbol, TR_CallStack* prevCallStack, TR_InnerPreexistenceInfo* innerPrexInfo)
{
    int32_t maxCallSize;
...
bool TR_DumbInliner::analyzeCallSite(
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if ((byteCodeSize > maxBCSize)) {
            if (tryToInline("overriding getMaxBytecodeIndex check", calltarget)) {
                ...
                if (comp()->trace(OMR::inlining))
                    traceMsg(
                        comp(), "inliner: overriding getMaxBytecodeIndex check because it's always worth inlining\n");
            } else {
                if (comp()->trace(OMR::inlining))
...
static TR::TreeTop* cloneAndReplaceCallNodeReference(
    TR::TreeTop*, TR::Node*, TR::Node*, TR::TreeTop*, TR::Compilation*);
    ...
    rcount_t originalCallNodeReferenceCount, TR::TreeTop* callNodeTreeTop, TR::Node* parent, TR::Node* callNode)
{
    ...
    if (resultNode) {
        ...
        for (; tt && numberOfReferencesToFind; tt = tt->getNextTreeTop())
            replaceCallNodeReferences(
                tt->getNode(), 0, 0, callNode, resultNode, numberOfReferencesToFind, visitedNodes);
    } else
    ...
    TR::Node* callNode, TR::Node* replacementNode, rcount_t& numberOfReferencesToFind, TR::NodeChecklist& visitedNodes)
{
    ...
    if (!replacedNode) {
        for (int32_t i = 0; i < node->getNumChildren() && numberOfReferencesToFind; ++i)
            replaceCallNodeReferences(
                node->getChild(i), node, i, callNode, replacementNode, numberOfReferencesToFind, visitedNodes);
    }
}
    ...
    TR_VirtualGuardSelection* guard)
{
    ...
    if (guard->_kind == TR_MutableCallSiteTargetGuard) {
        ...
        if (knot) {
            heuristicTrace(tracer(), "  createVirtualGuard: MutableCallSite %p epoch is obj%d",
                guard->_mutableCallSiteObject, guard->_mutableCallSiteEpoch);
        }

        return TR_VirtualGuard::createMutableCallSiteTargetGuard(
            comp(), calleeIndex, callNode, destination, guard->_mutableCallSiteObject, guard->_mutableCallSiteEpoch);
    }
    ...
    if (guard->_type == TR_VftTest) {
        ...
        return TR_VirtualGuard::createMethodGuard(
            guard->_kind, comp(), calleeIndex, callNode, destination, calleeSymbol, thisClass);
        ...
        return TR_VirtualGuard::createNonoverriddenGuard(
            guard->_kind, comp(), calleeIndex, callNode, destination, calleeSymbol);
    }
}
    ...
    if (suitableForRemat) {
        ...
        for (prevTree = NULL, argStoreTree = tif.getParameterMapper().firstTempTreeTop();
            ...
            if (argStore->chkIsPrivatizedInlinerArg()) {
                ...
                if (result == TR_yes) {
                    ...
                    debugTrace(
                        tracer(), "    priv arg remat unsafe for node [%p] - %d", argStore, argStore->getGlobalIndex());
            ...
            for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
                ...
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "privInlinerArg/Failed/%s", argStore->getFirstChild()->getOpCode().getName()),
                ...
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byReason/%s", argStore->getFirstChild()->getOpCode().getName()));
            }
        }
        ...
        for (uint32_t i = 0; i < argSafetyInfo.size(); ++i) {
            ...
            if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
                TR::DebugCounter::incStaticDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byMethod/(%s)/Succeeded", comp()->signature()));
                TR::DebugCounter::incStaticDebugCounter(
                    comp(), TR::DebugCounter::debugCounterName(comp(), "privatizedInlinerArgs.byReason/Success"));
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "privInlinerArg/Succeeded/(%s)", argStore->getFirstChild()->getOpCode().getName()),
                    argStoreTree, 1, TR::DebugCounter::Expensive);
                ...
                if (rematTree == argStoreTree) {
                    ...
                    if (performTransformation(
                            comp(), "O^O GUARDED CALL REMAT: Rematerialize [%p] as [%p]\n", argStore, duplicateStore)) {
    ...
    TR_VirtualGuardSelection* guard, TR::TreeTop** induceOSRCallTree, TR_CallTarget* calltarget)
{
    ...
    getUtil()->refineInlineGuard(
        callNode, block1, block2, appendTestToBlock1, callerSymbol, cursorTree, virtualGuard, block4);
    ...
    TR::DebugCounter::prependDebugCounter(
        comp(), "inliner.callSites/succeeded:guardedCallee", block4->getLastRealTreeTop());
        ...
        while (cursor && comp()->getMethodSymbol()->isOSRRelatedNode(cursor->getNode(), bci)) {
            if (cursor->getNode()->getOpCode().isStoreDirect() && cursor->getNode()->getFirstChild() == callNode) {
                debugTrace(tracer(), "  virtual call node pps: Pending push store of call [%p] found: [%p]", callNode,
                    cursor->getNode());
                block4->append(TR::TreeTop::create(
                    comp(), TR::Node::createStore(cursor->getNode()->getSymbolReference(), callNode)));
                break;
            }
...
void TR_ParameterToArgumentMapper::initialize(TR_CallStack* callStack)
{
    ...
    for (uint32_t argOffset = 0; parmMap; ++argIndex) {
        TR_ASSERT(
            (argIndex < _callNode->getNumChildren()), "Inlining, not able to match-up parameters and arguments\n");
            ...
            if (parmMap->_isConst) {
                ...
                if (parmMap->_replacementSymRef)
                    ...
                    if (!disableUseKnownObjectTempsForParms && prexArgument && !parmMap->_parmIsModified
                        ...
                        symRef = comp()->getSymRefTab()->findOrCreateTemporaryWithKnowObjectIndex(
                            _callerSymbol, prexArgument->getKnownObjectIndex());
                        ...
                        && prexArgument->isTypeInfoForInlinedBody()) // use known object temp in inlined body only
                    {
                        ...
                        TR::SymbolReference* knownObjectTempSymRef
                            = comp()->getSymRefTab()->findOrCreateTemporaryWithKnowObjectIndex(
                                _callerSymbol, prexArgument->getKnownObjectIndex());
                        ...
                        storeToKnownObjTemp->insertNewTreeTop(
                            _calleeSymbol->getFirstTreeTop(), _calleeSymbol->getFirstTreeTop()->getNextTreeTop());
...
void TR_ParameterToArgumentMapper::mapOSRCallSiteRematTable(uint32_t siteIndex)
{
    ...
    for (uint32_t i = 0; i < comp()->getOSRCallSiteRematSize(siteIndex); ++i) {
        ...
        for (; parmMap; parmMap = parmMap->getNext())
            if (symbol == parmMap->_parmSymbol) {
                if (parmMap->_isConst) {
                    ...
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(
                            comp(), "osrCallSiteRemat/mapParm/const/(%s)", comp()->signature()));
                } else if (loadSymRef->getOffset() > 0) {
                    comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, NULL);
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(
                            comp(), "osrCallSiteRemat/mapParm/addr/(%s)", comp()->signature()));
                } else {
                    comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, parmMap->_replacementSymRef);
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(
                            comp(), "osrCallSiteRemat/mapParm/success/(%s)", comp()->signature()));
                }
                break;
            }
        ...
        if (!parmMap)
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrCallSiteRemat/mapParm/missing/(%s)", comp()->signature()));
    }
    ...
    for (TR_ParameterMapping* parmMap = _mappings.getFirst(); parmMap; parmMap = parmMap->getNext()) {
        ...
        if (createNullCheckReference && i == _callNode->getFirstArgumentIndex()) {
            if (parmMap->_replacementSymRef) {
                ...
                TR_ASSERT(
                    parmMap->_parameterNode->getOpCodeValue() == TR::aconst, "a const this arg should be a const");
...
void TR_TransformInlinedFunction::transformNode(
    TR::Node* node, TR::Node* parent, uint32_t childIndex, TR::NodeChecklist& visitedNodes)
{
...
void TR_TransformInlinedFunction::transformReturn(TR::Node* returnNode, TR::Node* parent)
{
    ...
    if (returnNode->getNumChildren() && _callNode->getReferenceCount() > 1) {
        ...
        if (_simpleCallReferenceTreeTop) {
            cloneAndReplaceCallNodeReference(
                _simpleCallReferenceTreeTop, _callNode, returnValue, _currentTreeTop->getPrevTreeTop(), comp());
    ...
    , _mappings(mappings)
{
    ...
    if (!_tracer)
        _tracer = new (comp->trHeapMemory()) TR_InlinerTracer(
            comp, comp->fe(), NULL); // if I wasn't provided a tracer, just use a dummy one.  No tracing will occur.
}
...
void TR_HandleInjectedBasicBlock::findAndReplaceReferences(
...
void TR_HandleInjectedBasicBlock::replaceNodesReferencedFromAbove(
        ...
        if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
            ...
            if (!symRef) {
                ...
                if (comp()->fe()->dataTypeForLoadOrStore(nodeDataType) != nodeDataType) {
                    TR::ILOpCodes convOpCode = TR::ILOpCode::getProperConversion(
                        nodeDataType, comp()->fe()->dataTypeForLoadOrStore(nodeDataType), false);
                ...
                if (value->getOpCode().hasSymbolReference() && value->getSymbolReference()->hasKnownObjectIndex())
                    symRef = comp()->getSymRefTab()->findOrCreateTemporaryWithKnowObjectIndex(
                        _methodSymbol, value->getSymbolReference()->getKnownObjectIndex());

                OMR_InlinerUtil::storeValueInATemp(
                    comp(), value, symRef, tt, _methodSymbol, _injectedBasicBlockTemps, _availableTemps, 0);
            }
...
TR_HandleInjectedBasicBlock::MultiplyReferencedNode::MultiplyReferencedNode(
    TR::Node* node, TR::TreeTop* tt, uint32_t refsToBeFound, bool symCanBeReloaded)
    : _node(node)
    , _treeTop(tt)
...
void TR_HandleInjectedBasicBlock::replace(
    MultiplyReferencedNode* ref, TR::TreeTop* tt, TR::Node* parent, uint32_t childIndex)
{
    ...
    bool isIndirect, int32_t offset)
{
    ...
    if (isIndirect) {
        ...
        storeNode = TR::Node::createWithSymRef(
            comp->il.opCodeForIndirectStore(dataType), 2, 2, storeAddress, value, symRefShadow);
    } else {
...
static void cloneAndReplaceReturnValueReference(
    TR::TreeTop* ifCmpTreeTop, TR::TreeTop* rvStoreTreeTop, TR::Block* curBlock, TR::CFG* cfg, TR::Compilation* comp)
{
...
void TR_InlinerBase::getSymbolAndFindInlineTargets(TR_CallStack* callStack, TR_CallSite* callsite, bool findNewTargets)
{
    ...
    if (callsite->_initialCalleeSymbol) {
        ...
        if (comp()->fe()->isInlineableNativeMethod(comp(), callsite->_initialCalleeSymbol)) {
            ...
            callsite->addTarget(
                trMemory(), this, guard, callsite->_initialCalleeSymbol->getResolvedMethod(), callsite->_receiverClass);
        }
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if (!target->_calleeSymbol
            || !target->_calleeMethod->isSameMethod(target->_calleeSymbol->getResolvedMethod())) {
    ...
    if (getPolicy()->shouldRemoveDifferingTargets(callNode)) {
        ...
        if (callsite->numTargets() > 0) {
            if (callsite->isIndirectCall()) // local opts pre-existence has been performed
            {
                ...
                if (!callsite->getTarget(i)->_calleeMethod->isSameMethod(
                        callsite->_initialCalleeSymbol->getResolvedMethod())) {
                    callsite->getTarget(i)->_myCallees.setFirst(
                        0); // preventing furhter inlining as we now don't know what's beyond the new target
                    callsite->getTarget(i)->_numCallees = 0; // may want to improve this eventually
                    callsite->getTarget(i)->_partialInline
                        = 0; // Can't do a partial inline anymore as the blocks are wrong.
...
void TR_InlinerBase::inlineFromGraph(
    TR_CallStack* prevCallStack, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo* innerPrexInfo)
{
...
static void updateCallersFlags(
    TR::ResolvedMethodSymbol* callerSymbol, TR::ResolvedMethodSymbol* calleeSymbol, TR::Optimizer* optimizer)
{
...
bool TR_InlinerBase::inlineCallTarget2(
    TR_CallStack* callStack, TR_CallTarget* calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t)
{
    ...
    if (!performTransformation(comp(),
            "%sInlining qwerty %s into %s with a virtual guard kind=%s type=%s partialInline=%d\n", OPT_DETAILS,
            calleeSymbol->signature(trMemory()), callerSymbol->signature(trMemory()),
    ...
    */
    ...
    if (guard->_kind != TR_NoGuard) {
        ...
        if (osrMethodData && osrMethodData->getOSRCatchBlock()) {
            ...
            TR::Block* blockOfTreesAfterTheCall = mergedBlock->split(
                callNodeTreeTop, callerCFG, false /*fixupCommoning*/, false /*copyExceptionSuccessors*/);
    ...
    if (callNode->getReferenceCount() > 0)
        replaceCallNode(
            callerSymbol, tif->resultNode(), originalCallNodeReferenceCount, callNodeTreeTop, parent, callNode);
    ...
    if (tif->crossedBasicBlock()) {
        ...
        hibb.findAndReplaceReferences(
            previousBBStartInCaller, nextBBEndInCaller->getNode()->getBlock(), virtualCallSnippet);
        ...
        if (lastNodeInCallBlock->getOpCode().isReturn() || lastNodeInCallBlock->getOpCode().isGoto()) {
            ...
            if (cursorTreeTop) {
                *cursorTreeTop = calltarget->_myCallSite->_callNodeTreeTop->getPrevTreeTop();
                heuristicTrace(
                    tracer(), "Setting cursorTreeTop %p node %p", cursorTreeTop, (*cursorTreeTop)->getNode());
            }
        } else {
 ...
 * by the \p guard
 */
TR_PrexArgInfo* OMR_InlinerUtil::createPrexArgInfoForCallTarget(
    TR_VirtualGuardSelection* guard, TR_ResolvedMethod* implementer)
{
    return NULL;
}
    ...
    TR_ResolvedMethod* implementer, TR_OpaqueClassBlock* receiverClass, TR_AllocationKind allocKind, float ratio)
{
    ...
    TR_CallTarget* result = new (*memRegion)
        TR_CallTarget(*memRegion, this, _initialCalleeSymbol, implementer, guard, receiverClass, myPrexArgInfo, ratio);
    ...
    , _trMemory(_comp->trMemory())
{
    ...
    // traceMsg(comp,"_traceLevel set to %d, trace_heuristic = %d trace_debug =
...
void TR_InlinerTracer::dumpCallGraphs(TR_LinkHead<TR_CallTarget>* targets)
{
    ...
    for (calltarget = targets->getFirst(); calltarget; calltarget = calltarget->getNext()) {
        ...
        do {
            ...
            traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s", currentTarget->_myCallSite->getDepth(),
            ...
            traceMsg(comp(), "\t\t%d\t\t%s\n", currentTarget->_myCallSite->_byteCodeIndex,
                comp()->fe()->sampleSignature(
                    currentTarget->_calleeMethod->getPersistentIdentifier(), nameBuffer, SIGNATURE_SIZE, trMemory()));
            ...
            if (!currentTarget->_myCallees.isEmpty()) {
                ...
                for (callsite = currentTarget->_myCallees.getFirst(); callsite; callsite = callsite->getNext()) {
                    //               traceMsg(comp(),"\t\t\t\tnumtargets = %d numRemovedTargets = %d
                    //               currtarget->numdeletedcallees = %d
            ...
            if (heuristicLevel() && !currentTarget->_deletedCallees.isEmpty()) {
                ...
                for (callsite = currentTarget->_deletedCallees.getFirst(); callsite; callsite = callsite->getNext()) {
                    if (callsite->numTargets()) {
                        ...
                        traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s", callsite->getDepth(), 0, 0, 0,
...
void TR_InlinerTracer::dumpDeadCalls(TR_LinkHead<TR_CallSite>* sites)
{
    ...
    for (callsite = sites->getFirst(); callsite; callsite = callsite->getNext()) {
        ...
        for (int32_t i = 0; i < callsite->numTargets(); i++) {
            ...
            traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s", calltarget->_myCallSite->getDepth(),
            ...
            traceMsg(comp(), "\t\t%d\t\t%s\n", calltarget->_myCallSite->_byteCodeIndex,
                comp()->fe()->sampleSignature(
                    calltarget->_calleeMethod->getPersistentIdentifier(), name, SIGNATURE_SIZE, trMemory()));
        }
...

    {
        ...
        if (callsite->_callerResolvedMethod) {
            ...
            traceMsg(comp(), "\t CALLER signature from method = %s\n",
                fe()->sampleSignature(
                    callsite->_callerResolvedMethod->getPersistentIdentifier(), callerName, 1024, trMemory()));
        }
...
void TR_InlinerTracer::dumpCallTarget(TR_CallTarget* calltarget, const char* fmt, ...)
{
    ...
    traceMsg(comp(), "\tcalltarget= %p\n\t\tguard = %p guard->_kind = %s guard->_type = %s ", calltarget,
        calltarget->_guard, getGuardKindString(calltarget->_guard), getGuardTypeString(calltarget->_guard));
    ...
    traceMsg(comp(), "\t\t_size = %d _partialSize = %d _fullSize = %d _weight = %d ", calltarget->_size,
        calltarget->_partialSize, calltarget->_fullSize, calltarget->_weight);
    traceMsg(comp(),
        "_callGraphAdjustedWeight = %f \n\t\t_frequencyAdjustment = %f _isPartialInliningCandidate = %d _partialInline "
        "= %p\n",
 ...
 */
bool OMR_InlinerPolicy::supressInliningRecognizedInitialCallee(TR_CallSite* callsite, TR::Compilation* comp)
{
    return false;
}

TR_InlinerFailureReason OMR_InlinerPolicy::checkIfTargetInlineable(
    TR_CallTarget* target, TR_CallSite* callsite, TR::Compilation* comp)
{
...
void OMR_InlinerUtil::estimateAndRefineBytecodeSize(
    TR_CallSite* callsite, TR_CallTarget* target, TR_CallStack* callStack, int32_t& bytecodeSize)
{ }
 ...
 */
void OMR_InlinerUtil::clearArgInfoForNonInvariantArguments(
    TR_PrexArgInfo* argInfo, TR::ResolvedMethodSymbol* methodSymbol, TR_InlinerTracer* tracer)
{
    ...
    for (TR::TreeTop* tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (ordinal < argInfo->getNumArgs() && argInfo->get(ordinal)) {
            ...
            if (valueNode->getOpCode().hasSymbolReference() && valueNode->getSymbolReference()->hasKnownObjectIndex()
                ...
                if (tracePrex)
                    traceMsg(comp(), "ARGS PROPAGATION: unsetting an arg [%i] of argInfo %p", parmSymbol->getOrdinal(),
                        argInfo);
 ...
 */
void OMR_InlinerUtil::computeMethodBranchProfileInfo(
    TR::Block* cfgBlock, TR_CallTarget* calltarget, TR::ResolvedMethodSymbol* callerSymbol)
{
    return;
}
 ...
 */
void OMR_InlinerUtil::calleeTreeTopPreMergeActions(
    TR::ResolvedMethodSymbol* calleeResolvedMethodSymbol, TR_CallTarget* calltarget)
{
