...
void collectNodesForIsCorrectChecks(
    TR::Node* n, TR::list<TR::Node*>& checkNodes, TR::SparseBitVector& refsToCheckIfKilled, vcount_t vc);
...
template<class T> TR::Node* getNodeParent(TR::Node* lookForNode, TR::Node* curNode, vcount_t visitCount, T& ci)
{
...
bool foundInterferenceBetweenCurrentNodeAndPropagation(TR::Compilation* comp, bool trace, TR::Node* currentNode,
    TR::Node* storeNode, TR::list<TR::Node*>& checkNodes, TR::SparseBitVector& refsToCheckIfKilled)
{
    bool result = false;
    ...
    if (trace) {
        comp->getDebug()->trace(
            "foundInterferenceBetweenCurrentNodeAndPropagation: currentNode %p storeNode %p checkNodes [", currentNode,
            storeNode);
...
public:
    ...
    void setTargetTreeTop(TR::TreeTop* tt) { _targetTreeTop = tt; }
    ...
    void setSourceTree(TR::Node* st) { _sourceTree = st; }

    void setOriginalDefNodeForSourceTree(TR::Node* dn) { _originalDefNodeForSourceTree = dn; }
...
private:
    ...
    {
        _parentOfTargetNode
            = getNodeParent(_targetNode, _targetTreeTop->getNode(), comp()->incOrResetVisitCount(), _targetChildIndex);

        TR_ASSERT(_parentOfTargetNode != 0,
            "could not find _targetNode " POINTER_PRINTF_FORMAT
            " parent, _targetTreeTop root node " POINTER_PRINTF_FORMAT "!",
            _targetNode, _targetTreeTop->getNode());
        TR_ASSERT(_parentOfTargetNode->getChild(_targetChildIndex) == _targetNode,
            "_targetChildIndex or _parentOfTargetNode is not correct?");
    ...
    // The difference between this function and replaceTargetNodeBySourceTree is that this function does not set any
    // class fields.
    void replaceAllInstancesOfTargetNodeInTreeBySourceTree(
        TR::TreeTop* targetTreeTop, TR::Node* targetNode, TR::Node* sourceTree)
    {
        while (targetTreeTop->getNode()->containsNode(targetNode, comp()->incOrResetVisitCount())) {
            size_t childIndex;
            TR::Node* parentNode
                = getNodeParent(targetNode, targetTreeTop->getNode(), comp()->incOrResetVisitCount(), childIndex);

            TR_ASSERT(parentNode != 0, "parentNode == 0!");

            if (trace()) {
                comp()->getDebug()->trace("%s   Propagating new RHS " POINTER_PRINTF_FORMAT
                                          " in place of old instance location " POINTER_PRINTF_FORMAT
                                          " child index %d\n",
    ...
    void fixupBCDPrecisionIfRequired()
    {
        ...
        if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
            ...
            if (newNode->getNumChildren() == 2) {
                ...
                TR::DataType newDataType = newNode->getOpCode().hasSymbolReference()
                    ? newNode->getSymbolReference()->getSymbol()->getDataType()
                    : newNode->getDataType();
                int32_t newSymSize = newNode->getOpCode().hasSymbolReference()
                    ? newNode->getSymbolReference()->getSymbol()->getSize()
                    : newNode->getSize();
                newPrecision = TR::DataType::getBCDPrecisionFromSize(newDataType, newSymSize);

                // for a oldNode with prec=30 and an newNode with prec=31 then if the symbol sizes match and the other
                // checks below pass then can reduce the newPrecision to 30 so a pdModPrec correction is not needed (as
                // the propagated symbol will not have the top nibble 30->31 set).
                if (newNode->getType().isAnyPacked()) {
                        ...
                        oldSize == newSymSize) // 16 == 16
                    {
                        ...
                        newPrecision
                            = oldPrecision; // 31->30 -- no correction needed for same byte odd to even truncation if
                                            // the origNode was only storing out the even $ of digits
                    }
                }
            }
            ...
            if (needsClean || needsPrecisionCorrection) {
                dumpOptDetails(comp(),
                    "node %p precision %d != propagated symRef #%d precision %d and/or needsClean (%s)\n", oldNode,
                    oldPrecision, newNode->getSymbolReference()->getReferenceNumber(), newPrecision,
                    needsClean ? "yes" : "no");

                if (needsPrecisionCorrection) {
                    TR::ILOpCodes modPrecOp = TR::ILOpCode::modifyPrecisionOpCode(
                    ...
                    TR::Node* pdModPrecNode
                        = TR::Node::create(modPrecOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
                ...
                if (needsClean) {
                    TR::ILOpCodes cleanOp
                        = TR::ILOpCode::cleanOpCode(_parentOfTargetNode->getChild(_targetChildIndex)->getDataType());
                    ...
                    TR::Node* pdCleanNode
                        = TR::Node::create(cleanOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
    ...
    void addConversionIfRequired()
    {
        ...
        if (newNode->getType().getDataType() != oldNode->getType().getDataType()) {
            //                                                     /* source */            /* target */
            TR::ILOpCodes convOp
                = TR::ILOpCode::getProperConversion(newNode->getDataType(), oldNode->getDataType(), true);
    ...
    void propagateThroughToCommoningPoints()
    {
        ...
        if (oldNode->getReferenceCount() > 0) {
            // Step 1: Propagate this RHS change to anything with a commoned old node further down the block. Common the
            // rematerialization instead.

            // The idea is that we have replaced the RHS of some node, but other nodes further down the block could have
            // commoned the old node. We want to replace those commonings with a commoning to the new RHS of the node
            // because the value is the same.

            // Get a list of load sym refs for the new rematerialized RHS - use this to see if any part of the new node
            // is killed during tree traversal.
            ...
            collectNodesForIsCorrectChecks(
                newNode, newCheckNodes, newRefsToCheckIfKilled, comp()->incOrResetVisitCount());

            // Start at _targetTreeTop to replace other instances of _targetNode in _targetTreeTop as well as to the end
            // of the block.
            ...
            while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
                ...
                if (oldNode->getReferenceCount() == 0) {
                    ...
                    if (trace()) {
                        comp()->getDebug()->trace("%s   Propagating new RHS " POINTER_PRINTF_FORMAT
                                                  " stops because oldNode ref count = 0\n",
                            OPT_DETAILS, newNode);
                    }
...
int32_t TR_CopyPropagation::perform()
{
...

    {
        ...
        typedef TR::typed_allocator<std::pair<TR::Node* const, TR::deque<TR::Node*, TR::Region&>*>, TR::Region&>
            StoreNodeMapAllocator;
        typedef std::less<TR::Node*> StoreNodeMapComparator;
        typedef std::map<TR::Node*, TR::deque<TR::Node*, TR::Region&>*, StoreNodeMapComparator, StoreNodeMapAllocator>
            StoreNodeMap;
        ...
        for (TR::TreeTop* tree = comp()->getStartTree(); tree; tree = tree->getNextTreeTop()) {
            ...
            if (storeLookup != storeMap.end()) {
                for (auto itr = storeLookup->second->begin(), end = storeLookup->second->end(); itr != end; ++itr) {
                    ...
                    if (thisDefSlot < 0) // pending pushes
                        ...
                        < comp()
                              ->getOwningMethodSymbol(defNode->getSymbolReference()->getOwningMethodIndex())
                              ->getFirstJitTempIndex()) // autos and parms
                        thisSymRefIsPreferred = true;
                    ...
                    if (prevDefSlot < 0) // pending pushes
                        ...
                        < comp()
                              ->getOwningMethodSymbol(prevDefNode->getSymbolReference()->getOwningMethodIndex())
                              ->getFirstJitTempIndex()) // autos and parms
                        prevSymRefIsPreferred = true;

#ifdef J9_PROJECT_SPECIFIC
                    if (prevDefNode->getType().isBCD()) {
                        TR_ASSERT(defNode->getType().isBCD(),
                            "expecting defNode to be a BCD type (dt=%s) when prevDefNode is a BCD type (dt=%s)\n",
                            defNode->getDataType().toString(), prevDefNode->getDataType().toString());
                        if (defNode->getType().isBCD() && defNode->getDataType() == prevDefNode->getDataType()
                            && prevDefNode->getDecimalPrecision() == defNode->getDecimalPrecision()
                            && (defNode->getDataType() != TR::PackedDecimal
                                || prevDefNode->mustCleanSignInPDStoreEvaluator()
                                    == defNode->mustCleanSignInPDStoreEvaluator())) {
                            dumpOptDetails(comp(),
                                "%s   Def nodes %p (precision = %d) and %p (precision = %d) are equivalent\n",
                                OPT_DETAILS, defNode, defNode->getDecimalPrecision(), prevDefNode,
                    ...
                    {
                        dumpOptDetails(
                            comp(), "%s   Def nodes %p and %p are equivalent\n", OPT_DETAILS, defNode, prevDefNode);
            ...
            if (storeLookup == storeMap.end()) {
                storeMap[rhsOfStoreDefNode]
                    = new (trStackMemory()) TR::deque<TR::Node*, TR::Region&>(trMemory()->currentStackRegion());
        ...
        for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end(); itr != end; ++itr) {
            ...
            for (useCursor.SetToFirstOne(); useCursor.Valid(); useCursor.SetToNextOne()) {
                ...
                if (performTransformation(comp(), "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n",
                    ...
                    comp()->setOsrStateIsReliable(
                        false); // could check if the propagation was done to a child of the OSR helper call here
                    ...
                    dumpOptDetails(comp(), "%s   By Original symRef #%d in Use node : %p\n", OPT_DETAILS,
                        originalSymbolReference->getReferenceNumber(), useNode);
        ...
        for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
            ...
            if (useDefInfo->getUseDef(defs, i)) {
                ...
                int32_t regNumber;
                TR::Node* defNode = areAllDefsInCorrectForm(
                    useNode, defs, useDefInfo, firstRealDefIndex, loadNode, regNumber, baseNode);
                if (!defNode)
                    continue;
                ...
                if (defTree->getEnclosingBlock()->startOfExtendedBlock()
                    == _useTree->getEnclosingBlock()->startOfExtendedBlock()) {
                ...
                if (baseNode) {
                    ...
                    if (!_cleanupTemps || baseNode->getSymbolReference() == defNode->getSymbolReference()
                ...
                TR::Node* rhsOfStoreDefNode
                    = defNode->getOpCode().isStoreIndirect() ? defNode->getSecondChild() : defNode->getFirstChild();
                TR::SymbolReference* copySymbolReference
                    = defNode->getOpCode().hasSymbolReference() ? defNode->getSymbolReference() : NULL;
                ...
                if (performTransformation(comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n", OPT_DETAILS,
                    ...
                    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                        ...
                        dumpOptDetails(comp(), "%s      Def #%d[%p]\n", OPT_DETAILS, defIndexTrace,
                            useDefInfo->getNode(defIndexTrace));
                    }

                    if (!isRegLoad && (loadNode->getOpCodeValue() != TR::loadaddr)
                        && // loadaddr doesn't need to be anchored
                        (loadNode->getOpCode().hasSymbolReference())
                        && (loadNode->getSymbolReference() == copySymbolReference)
                        && (rhsOfStoreDefNode != loadNode)) // Modifications to this condition should be mirrored in the
                                                            // NOTEMPS check above.
                    {
                        ...
                        TR::SymbolReference* newSymbolReference = comp()->getSymRefTab()->createTemporary(
                            comp()->getMethodSymbol(), copySymbolReference->getSymbol()->getDataType(), false,
                            copySymbolReference->getSymbol()->getSize());

                        dumpOptDetails(comp(), "%s      created new temp #%d\n", OPT_DETAILS,
                            newSymbolReference->getReferenceNumber());

                        TR::Node* store = TR::Node::createStore(
                            newSymbolReference, TR::Node::createLoad(defNode, copySymbolReference));
                    ...
                    if (_propagatingWholeExpression) {
                        ...
                        TR::Node* rematExpression = rhsOfStoreDefNode->getOpCode().isStore()
                            ? rhsOfStoreDefNode->getValueChild()
                            : rhsOfStoreDefNode;
                        TR::Node* newExpression = rematExpression->duplicateTree();
                        if (!optimizer()->areSyntacticallyEquivalent(
                                rematExpression, newExpression, comp()->incOrResetVisitCount())) {
                        ...
                        replaceCopySymbolReferenceByOriginalIn(
                            copySymbolReference, rhsOfStoreDefNode, useNode, defNode, baseAddr, baseAddrAvail);
                        ...
                        if (bbstartNode->getNumChildren() > 0) {
                            ...
                            if (regLoadNode) {
                                ...
                                while (cursorTree != exitTree) {
                                    replaceCopySymbolReferenceByOriginalRegLoadIn(
                                        regLoadNode, useNode, copySymbolReference, cursorTree->getNode(), NULL, -1);
                                    cursorTree = cursorTree->getNextTreeTop();
                                }
                            }
                        }
...
void TR_CopyPropagation::rematerializeIndirectLoadsFromAutos()
{
    ...
    while (currentTree) {
        ...
        if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress())
            && node->getFirstChild()->getOpCodeValue() == TR::loadaddr
            && node->getFirstChild()->getSymbol()->isAutoOrParm() && nextNode->getOpCode().isStoreDirect()
            && nextNode->getSymbol()->isAutoOrParm() && nextNode->getFirstChild() == node->getSecondChild()
...
void TR_CopyPropagation::commonIndirectLoadsFromAutos()
{
    ...
    while (currentTree) {
        ...
        if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress())
            && node->getFirstChild()->getOpCodeValue() == TR::loadaddr
            && node->getFirstChild()->getSymbol()->isAutoOrParm() && nextNode->getOpCode().isStoreDirect()
            && nextNode->getSymbol()->isAutoOrParm() && nextNode->getFirstChild()->getOpCode().isLoadIndirect()
    ...
    if (node->getVisitCount() != curVisit) {
        ...
        if (node->getOpCode().hasSymbolReference()) {
...
#endif
            {
                ...
                if (origNode->getOpCode().isLoadIndirect()) {
                ...
                } else if (origNode->getOpCode().isConversion() && origNode->getNumChildren() == 1) {
                    ...
                    if (origNode->getNumChildren() == 2) {
                        ...
                        int32_t origSymPrecision
                            = TR::DataType::getBCDPrecisionFromSize(origNode->getDataType(), origSymSize);
                        // for example for a node with prec=30 and an origSymPrecision with prec=31 then if the symbol
                        // sizes match and the other checks below pass then can reduce the newPrecision to 30 so a
                        // pdModPrec correction is not needed (as the propagated symbol will not have the top nibble
                        // 30->31 set).
                                ...
                                nodeSize == origSymSize) // 16 == 16
                            {
                                if (trace() || comp()->cg()->traceBCDCodeGen())
                                    traceMsg(comp(),
                                        ...
                                        nodePrecision);
                                newPrecision = nodePrecision; // 31->30 -- no correction needed for same byte odd to
                                                              // even truncation if the origNode was only storing out
                                                              // the even $ of digits
                            }
                        }
                    }

                    // if newPrecision < currentPrecision then one or more digits will be lost during the
                    // copyPropagation -- the isCorrect* routines disallow this
                    TR_ASSERT(newPrecision >= currentPrecision,
                        "newPrecision < currentPrecision (%d < %d) during copyPropagation\n", newPrecision,
                        currentPrecision);
                    ...
                    //
                    if (needsClean || needsPrecisionCorrection) {
                        // If the copy propagation is going to change the size of the node being replaced then a
                        // modifyPrecision node must be prepended in case the new parent requires explicit widening
                        // (such as if the parent is a call). Also if the defNode was going to clean must also insert a
                        // pdclean on top of the node with the propagated symRef
                        ...
                        dumpOptDetails(comp(),
                            "node %p precision %d != propagated symRef #%d precision %d and/or needsClean (%s) so "
                            "create nodeCopy %p (isTwoChildrenCase=%s)\n",
                            node, currentPrecision, origNode->getSymbolReference()->getReferenceNumber(), newPrecision,
                            needsClean ? "yes" : "no", nodeCopy, twoChildrenCase ? "yes" : "no");

                        if (needsPrecisionCorrection) {
                            TR::ILOpCodes modPrecOp = TR::ILOpCode::modifyPrecisionOpCode(nodeCopy->getDataType());
                            TR_ASSERT(modPrecOp != TR::BadILOp,
                                "no bcd modify precision opcode found for node %p with type %s\n", nodeCopy,
                                nodeCopy->getDataType().toString());
                            if (needsClean) // i.e. if a clean will be generated below then do not modify node op yet,
                                            // let the top level op (the coming clean) do this
                            {
                                ...
                                dumpOptDetails(comp(), "create %s (0x%p) to correct precision %d->%d\n",
                                    pdModPrecNode->getOpCode().getName(), pdModPrecNode, newPrecision,
                                    currentPrecision);
                            } else {
                                ...
                                dumpOptDetails(comp(), "modify load 0x%p to %s to correct precision %d->%d\n", node,
                                    node->getOpCode().getName(), newPrecision, currentPrecision);
                        ...
                        if (needsClean) {
                            ...
                            TR_ASSERT(cleanOp != TR::BadILOp,
                                "no bcd modify precision cleanOp found for node %p with type %s\n", nodeCopy,
                                nodeCopy->getDataType().toString());
                            // can only clean up to maxPackedPrec -- case allow should have been guaranteed by
                            // isCorrectToPropagate
                            TR_ASSERT(node->getDecimalPrecision() <= TR::DataType::getMaxPackedDecimalPrecision(),
                                "node %p prec %d must be <= max %d\n", node, node->getDecimalPrecision(),
                            ...
                            node->setFlags(0);
                            dumpOptDetails(
                                comp(), "modify load 0x%p to %s to clean\n", node, node->getOpCode().getName());

                            node = nodeCopy; // so the recursion continues from the correct point below
                        }
                    } else if (twoChildrenCase) {
                        dumpOptDetails(comp(),
                            "attach new first %s (%p) and second %s (%p) to node %s (%p) in the twoChildren=true "
                            "case\n",
                            firstChild->getOpCode().getName(), firstChild, secondChild->getOpCode().getName(),
                            secondChild, node->getOpCode().getName(), node);
...
TR::treetop
   iload #378
*/
void TR_CopyPropagation::replaceCopySymbolReferenceByOriginalRegLoadIn(TR::Node* regLoadNode, TR::Node* useNode,
...
bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node* useNode, TR::Node* storeNode, TR::Node* loadNode)
{
    ...
    if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
        dumpOptDetails(comp(),
            "isLoadNodeSuitableForPropagation : useNode %p (prec %d), loadNode %p (prec %d) -- isCorrect=%s (only "
            "correct when useNodePrec <= loadNodePrec)\n",
            useNode, useNode->getDecimalPrecision(), loadNode, loadNode->getDecimalPrecision(),
            useNode->getDecimalPrecision() <= loadNode->getDecimalPrecision() ? "yes" : "no");
        if (useNode->getDecimalPrecision() > loadNode->getDecimalPrecision()) {
            return false;
        } else if (storeNode && storeNode->mustClean()
            && (useNode->getDecimalPrecision() > TR::DataType::getMaxPackedDecimalPrecision())) {
            // have to disallow this case because will not be able to clean useNode after propagation for prec > max
            dumpOptDetails(comp(),
                "isLoadNodeSuitableForPropagation=false for useNode %s (%p) prec %d > max %d and mustClean store %p\n",
                useNode->getOpCode().getName(), useNode, useNode->getDecimalPrecision(),
                TR::DataType::getMaxPackedDecimalPrecision(), storeNode);
            return false;
        }
    }
#endif
    ...
    TR::Node* n, TR::list<TR::Node*>& checkNodes, TR::SparseBitVector& refsToCheckIfKilled, vcount_t vc)
{
    ...
    if ((n->getOpCode().isLoadVar() || n->getOpCode().isLoadAddr()) && (n->getSymbolReference() != NULL)) {
...
// If there are load nodes (loadNode or baseNode) in the RHS, and some parent of those nodes has ref count > 1, then add
// to the list of nodes to perform isSafeToPropagate on.
void collectNodesForIsSafeChecks(
    TR::Node* n, TR::list<TR::Node*>& anchorCheckNodes, vcount_t vc, bool sawRefCountLargerThanOne)
{
    if (n->getVisitCount() == vc) {
        return;
    }
    ...
    //
    if (sawRefCountLargerThanOne && (n->getOpCode().isLoadVar() || n->getOpCode().isLoadAddr())
...
TR::Node* TR_CopyPropagation::areAllDefsInCorrectForm(TR::Node* useNode, const TR_UseDefInfo::BitVector& defs,
    TR_UseDefInfo* useDefInfo, int32_t firstRealDefIndex, TR::Node*& loadNode, int32_t& regNumber, TR::Node*& baseNode)
{
    ...
    //...exception point that could reach a catch
    // catch block
    // load b  //  copy propagating "a" here is incorrect since we will end up propagating rhs2 value (in "a") instead
    // of rhs1 value (in "b") changing program semantics
    //
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defNode->getDataType() != useNode->getDataType() || defNode->getSize() != useNode->getSize())
            return NULL;

        TR::TreeTop* defTree = useDefInfo->getTreeTop(defIndex);

        TR::Node* rhsOfStoreDefNode
            = defNode->getOpCode().isStoreIndirect() ? defNode->getChild(1) : defNode->getChild(0);
...

        {
            ...
            if (lastDefNode == NULL) {
                ...
                if (!optimizer()->areSyntacticallyEquivalent(
                        lastDefNode->getFirstChild(), rhsOfStoreDefNode, visitCount)) {
                    return NULL;
                }
    ...
    if (lastDefNode != NULL) {
        ...
        if (_propagatingWholeExpression) {
            ...
            if (foundInterferenceBetweenCurrentNodeAndPropagation(
                    comp(), trace(), currentNode, lastDefNode, checkNodes, refsToCheckIfKilled)) {
                if (trace()) {
                    comp()->getDebug()->trace(
                        "%s   Cannot propagate RHS of i = i + 1 (node %p)\n", OPT_DETAILS, lastDefNode);
                }
...
bool TR_CopyPropagation::isCorrectToPropagate(TR::Node* useNode, TR::Node* storeNode, TR::list<TR::Node*>& checkNodes,
    TR::SparseBitVector& refsToCheckIfKilled, int32_t regNumber, const TR_UseDefInfo::BitVector& defs,
    TR_UseDefInfo* useDefInfo)
{
    ...
    if (_storeTree->getEnclosingBlock()->startOfExtendedBlock()
        == _useTree->getEnclosingBlock()->startOfExtendedBlock()) {
        for (auto checkNodesCursor = checkNodes.begin(); checkNodesCursor != checkNodes.end(); ++checkNodesCursor) {
    ...
    while (!(currentTree->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)) {
...
#ifdef PROPAGATE_GLOBALS
        else if (_lookForOriginalDefs && currentNode->getOpCode().isStore()
            && defs.ValueAt(currentNode->getUseDefIndex() - useDefInfo->getFirstDefIndex()))
#else
        else if (_lookForOriginalDefs && currentNode->getOpCode().isStoreDirect()
...
#endif
        {
            ...
            if (regNumber == -1) {
                if (foundInterferenceBetweenCurrentNodeAndPropagation(
                        comp(), trace(), currentNode, storeNode, checkNodes, refsToCheckIfKilled)) {
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
            ...
            if (isRedefinedBetweenStoreTreeAnd(
                    checkNodes, refsToCheckIfKilled, storeNode, next->getExit(), regNumber, defs, useDefInfo)) {
                return false;
            }
        }
    }

    for (auto nextEdge = block->getExceptionPredecessors().begin(); nextEdge != block->getExceptionPredecessors().end();
         ++nextEdge) {
        TR::Block* next = toBlock((*nextEdge)->getFrom());
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
            && ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
...
bool TR_CopyPropagation::isRedefinedBetweenStoreTreeAnd(TR::list<TR::Node*>& checkNodes,
    TR::SparseBitVector& refsToCheckIfKilled, TR::Node* storeNode, TR::TreeTop* exitTree, int32_t regNumber,
    const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
    ...
    if (useRecursiveIsRedefinedBetweenStoreTreeAnd)
        return recursive_isRedefinedBetweenStoreTreeAnd(
            checkNodes, refsToCheckIfKilled, storeNode, exitTree, regNumber, defs, useDefInfo);
...
bool TR_CopyPropagation::recursive_isRedefinedBetweenStoreTreeAnd(TR::list<TR::Node*>& checkNodes,
    TR::SparseBitVector& refsToCheckIfKilled, TR::Node* storeNode, TR::TreeTop* exitTree, int32_t regNumber,
    const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
    TR::TreeTop* currentTree = exitTree;
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
            && ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
            if (recursive_isRedefinedBetweenStoreTreeAnd(
                    checkNodes, refsToCheckIfKilled, storeNode, next->getExit(), regNumber, defs, useDefInfo))
                return true;
        }
    }
...
bool TR_CopyPropagation::isCorrectToReplace(
    TR::Node* useNode, TR::Node* storeNode, const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
#ifdef J9_PROJECT_SPECIFIC
    if (useNode->getType().isBCD() && storeNode->getType().isBCD()) {
        dumpOptDetails(comp(),
            "isCorrectToReplace : useNode %p (prec %d), storeNode %p (prec %d) -- isCorrect=%s (only correct when "
            "useNodePrec <= storeNodePrec)\n",
            useNode, useNode->getDecimalPrecision(), storeNode, storeNode->getDecimalPrecision(),
            useNode->getDecimalPrecision() <= storeNode->getDecimalPrecision() ? "yes" : "no");
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())) {
            if (isRedefinedBetweenStoreTreeAnd(
                    checkNodes, refsToCheckIfKilled, storeNode, next->getExit(), -1, defs, useDefInfo))
                return false;
        }
    }
...
TR::Node* TR_CopyPropagation::isLoadVarWithConst(TR::Node* node)
{
    ...
    if (TR::TransformUtil::isNoopConversion(comp(), node) && node->getNumChildren() == 1)
...
TR::Node* TR_CopyPropagation::isIndirectLoadFromRegister(TR::Node* node, TR::Node*& baseNode)
{
    ...
    if (node->getOpCode().isLoadIndirect() && node->getFirstChild()->getOpCode().isLoadVarDirect()) {
...
TR::Node* TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node* defNode, TR::Node* node)
{
    ...
    //
    //  iaload <refined-array-shadow> [id=238:"PSALIT2"] [#1.64 Shadow] [flags 0x80000607 0x0 ]  [0x2B026BFC]
    //  bci=[-1,37,329] rc=2 vc=9438 vn=- sti=- udi=331 nc=1 addr=4
    //    aiadd (internalPtr )                                                            [0x2B016B20] bci=[-1,37,329]
    //    rc=1 vc=9438 vn=- sti=- udi=- nc=2 addr=4 flg=0x8000
    //      loadaddr <auto slot 40> [id=152:"PSA"] [#1.63 Auto] [flags 0x4000008 0x0 ]    [0x2B016AD0] bci=[-1,37,329]
    //      rc=1 vc=9438 vn=- sti=- udi=- nc=0 addr=4 iconst 1212 (X!=0 X>=0 ) [0x2B016B70] bci=[-1,37,329] rc=1 vc=9438
    //      vn=- sti=- udi=- nc=0 flg=0x104
    //
    if (node->getOpCode().isLoadIndirect()) {
        TR::Node* addr = node->getFirstChild();

        if ((addr->getOpCodeValue() == TR::loadaddr) && addr->getSymbol()->isAutoOrParm()) {
    ...
    if (!cheapExpression) {
        if (trace()) {
            comp()->getDebug()->trace(
                "%s   skipping attempt at propagating %p because it is not cheap\n", OPT_DETAILS, node);
        }
