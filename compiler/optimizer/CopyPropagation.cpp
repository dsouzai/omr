...
void collectNodesForIsCorrectChecks(TR::Node *n, TR::list<TR::Node *> &checkNodes,
    TR::SparseBitVector &refsToCheckIfKilled, vcount_t vc);
...
private:
    ...
    // class fields.
    void replaceAllInstancesOfTargetNodeInTreeBySourceTree(TR::TreeTop *targetTreeTop, TR::Node *targetNode,
        TR::Node *sourceTree)
    {
    ...
    void propagateThroughToCommoningPoints()
    {
        ...
        if (oldNode->getReferenceCount() > 0) {
            ...
            collectNodesForIsCorrectChecks(newNode, newCheckNodes, newRefsToCheckIfKilled,
...
int32_t TR_CopyPropagation::perform()
{
...

    {
        ...
        typedef std::map<TR::Node *, TR::deque<TR::Node *, TR::Region &> *, StoreNodeMapComparator,
            StoreNodeMapAllocator>
            StoreNodeMap;
        ...
        for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end(); itr != end; ++itr) {
            ...
            for (useCursor.SetToFirstOne(); useCursor.Valid(); useCursor.SetToNextOne()) {
                ...
                if (performTransformation(comp(), "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n",
                    ...
                    dumpOptDetails(comp(), "%s      Def #%d[%p]\n", OPT_DETAILS, defIndex,
                        useDefInfo->getNode(defIndex));
        ...
        for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
            ...
            if (useDefInfo->getUseDef(defs, i)) {
                ...
                int32_t regNumber;
                TR::Node *defNode = areAllDefsInCorrectForm(useNode, defs, useDefInfo, firstRealDefIndex, loadNode,
                    regNumber, baseNode);
                if (!defNode)
                    continue;
                ...
                if (performTransformation(comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n", OPT_DETAILS,
                                                            ...
                                                            // NOTEMPS check above.
                    {
                        ...
                        TR::Node *store = TR::Node::createStore(newSymbolReference,
                            TR::Node::createLoad(defNode, copySymbolReference));
                    ...
                    if (_propagatingWholeExpression) {
                        ...
                        if (!optimizer()->areSyntacticallyEquivalent(rematExpression, newExpression,
                                comp()->incOrResetVisitCount())) {
                        ...
                        replaceCopySymbolReferenceByOriginalIn(copySymbolReference, rhsOfStoreDefNode, useNode, defNode,
                            baseAddr, baseAddrAvail);
                        ...
                        if (bbstartNode->getNumChildren() > 0) {
                            ...
                            if (regLoadNode) {
                                ...
                                while (cursorTree != exitTree) {
                                    replaceCopySymbolReferenceByOriginalRegLoadIn(regLoadNode, useNode,
                                        copySymbolReference, cursorTree->getNode(), NULL, -1);
                                    cursorTree = cursorTree->getNextTreeTop();
                                }
                            }
                        }
...
void collectNodesForIsSafeChecks(TR::Node *n, TR::list<TR::Node *> &anchorCheckNodes, vcount_t vc,
    bool sawRefCountLargerThanOne)
{
    if (n->getVisitCount() == vc) {
        return;
    }
    ...
    TR_UseDefInfo *useDefInfo, int32_t firstRealDefIndex, TR::Node *&loadNode, int32_t &regNumber, TR::Node *&baseNode)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
...

        {
            ...
            if (lastDefNode == NULL) {
                ...
                if (!optimizer()->areSyntacticallyEquivalent(lastDefNode->getFirstChild(), rhsOfStoreDefNode,
                        visitCount)) {
                    return NULL;
                }
    ...
    if (lastDefNode != NULL) {
        ...
        if (_propagatingWholeExpression) {
            ...
            if (foundInterferenceBetweenCurrentNodeAndPropagation(comp(), trace(), currentNode, lastDefNode, checkNodes,
                    refsToCheckIfKilled)) {
                if (trace()) {
    ...
    TR_UseDefInfo *useDefInfo)
{
    ...
    while (!(currentTree->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)) {
...
#endif
        {
            ...
            if (regNumber == -1) {
                if (foundInterferenceBetweenCurrentNodeAndPropagation(comp(), trace(), currentNode, storeNode,
                        checkNodes, refsToCheckIfKilled)) {
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
            ...
            if (isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode, next->getExit(), regNumber,
                    defs, useDefInfo)) {
                return false;
            }
        }
    ...
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
    ...
    if (useRecursiveIsRedefinedBetweenStoreTreeAnd)
        return recursive_isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode, exitTree, regNumber,
            defs, useDefInfo);
    ...
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())
            && ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
            if (recursive_isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode, next->getExit(),
                    regNumber, defs, useDefInfo))
                return true;
        }
    }
...
bool TR_CopyPropagation::isCorrectToReplace(TR::Node *useNode, TR::Node *storeNode,
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo)
{
#ifdef J9_PROJECT_SPECIFIC
    ...
    while (!(currentTree->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)) {
        if (currentTree == _storeTree)
            ...
            TR::Node *currentNode = skipTreeTopAndGetNode(currentTree);
            if (foundInterferenceBetweenCurrentNodeAndPropagation(comp(), trace(), currentNode, storeNode, checkNodes,
                    refsToCheckIfKilled)) {
                return false; // is not correct
            }
        }
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        TR::Block *next = toBlock((*nextEdge)->getFrom());
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart())) {
            if (isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled, storeNode, next->getExit(), -1, defs,
                    useDefInfo))
                return false;
        }
    }
