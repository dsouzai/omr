...
void TR::LocalDeadStoreElimination::setIsFirstReferenceToNode(TR::Node* parent, int32_t index, TR::Node* node)
{
    parent ? node->setLocalIndex(1) : node->setLocalIndex(0);
}

bool TR::LocalDeadStoreElimination::isFirstReferenceToNode(TR::Node* parent, int32_t index, TR::Node* node)
{
...
bool TR::LocalDeadStoreElimination::isNonRemovableStore(TR::Node* storeNode, bool& seenIdentityStore)
{
    ...
    // istore "b"  // not legal to remove this store as the auto/parm 'escapes' to the parent (owning) routine as they
    // share a stack
    //    z
    // return
    ...
    // except for placeholder stores for monexit, we need to see those in order to pop the live monitor stack the
    ...
    if (!storeNode->getSymbolReference()->getSymbol()->holdsMonitoredObject()) {
        ...
        if (prevStoreNode && !storeNode->getOpCode().isIndirect() && !prevStoreNode->getOpCode().isIndirect()
...
void TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop* entryTree, TR::TreeTop* exitTree)
{
    ...
    while (!(_curTree == entryTree)) {
        ...
        if (currentNode->getOpCode().isReturn()) {
            ...
            for (int32_t symRefNumber = comp()->getSymRefTab()->getIndexOfFirstSymRef(); symRefNumber < symRefCount;
                 symRefNumber++) {
        ...
        if (storeNode) {
            ...
            // the logic below is:
            // when nonRemovableStore=false then removal of the store can happen if one of the following conditions is
            // true: 1) seenIdentityStore=true 2) haven't seen a use and not volatile and one of the following is also
            // true
                ...
                && (seenIdentityStore || // 1)
                    (deadSymbolReferences.get(symRefNum) && // 2)
                        ...
                        && ((_blockContainsReturn && //   a)
                                (symRef->getSymbol()->isAuto() || symRef->getSymbol()->isParm()))
        ...
        //
        if (currentNode->getOpCode().isBranch() || currentNode->getOpCode().isJumpWithMultipleTargets()
            || (currentNode->getOpCodeValue() == TR::treetop
                && currentNode->getFirstChild()->getOpCode().isJumpWithMultipleTargets())) {
            _blockContainsReturn = false;
    ...
    if (_treesAnchored) {
        ...
        while (!(_curTree == exitTree)) {
            ...
            if (firstChild
                && !(callWithSideEffects || firstChild->getOpCodeValue() == TR::New
                         || firstChild->getOpCodeValue() == TR::newarray
                         || firstChild->getOpCodeValue() == TR::anewarray
                         || firstChild->getOpCodeValue() == TR::multianewarray || firstChild->getOpCode().isCheckCast()
                         || firstChild->getOpCodeValue() == TR::
                         instanceof
                     || firstChild->getOpCodeValue() == TR::monent || firstChild->getOpCodeValue() == TR::monexit)) {
                if (firstChild->getReferenceCount() == 0
                    && performTransformation(comp(), "%sRemoving Dead Anchor : %s [0x%p]\n", optDetailString(),
                        currentNode->getOpCode().getName(), currentNode)) {
                    _treesChanged = true;
...
TR::TreeTop* TR::LocalDeadStoreElimination::removeStoreTree(TR::TreeTop* treeTop)
{
    ...
    if (isEntireNodeRemovable(storeNode)) {
        if (performTransformation(comp(), "%sRemoving Dead Store : %s [0x%p]\n", optDetailString(),
                storeNode->getOpCode().getName(), storeNode)) {
        ...
        if (performTransformation(comp(), "%sAnchoring rhs of store : %s [0x%p] in a treetop\n", optDetailString(),
            ...
            if (comp()->useAnchors()) {
                ...
                if (translateNode) {
                    dumpOptDetails(
                        comp(), "removing corresponding translation [%p] for [%p]\n", translateNode, storeNode);
            ...
            if ((curTree != treeTop) || !removedTree) {
                if (nodeInTreeTop->getOpCode().isAnchor()
                    || (nodeInTreeTop->getOpCode().isCheck() && !nodeInTreeTop->getOpCode().isNullCheck()))
...
bool TR::LocalDeadStoreElimination::isIdentityStore(TR::Node* storeNode)
{
    ...
    if (!storedValue->getOpCode().isLoadVar())
        return false;
    if (storeNode->getOpCode().isIndirect() && storeNode->getFirstChild() != storedValue->getFirstChild())
        return false;
    ...
    //
    if ((storedValue->getReferenceCount() == 1)
        || isFirstReferenceToNode(storeNode, storedValueChildIndex, storedValue))
        return true;

    bool doChecksForAnchors = false;
    TR::ILOpCodes anchorOp = TR::BadILOp;
    if (comp()->useCompressedPointers() && (storedValue->getOpCodeValue() == TR::aloadi)
...
void TR::LocalDeadStoreElimination::examineNode(
    TR::Node* parent, int32_t childNum, TR::Node* node, TR_BitVector& deadSymbolReferences)
{
    ...
    if (node->getOpCode().isLoadVar() || (node->getOpCodeValue() == TR::loadaddr)) {
    ...
    if (node->getOpCode().isCall() || node->getOpCode().isCheck() || node->getOpCodeValue() == TR::New
            ...
            node->getOpCode().isCheckCast() || node->getOpCodeValue() == TR::
            instanceof || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit
...
bool TR::LocalDeadStoreElimination::seenIdenticalStore(TR::Node* node)
{
    for (auto it = _storeNodes->rbegin(); it != _storeNodes->rend(); ++it) {
        ...
        if (areLhsOfStoresSyntacticallyEquivalent(storeNode, node))
            return true;
        else if ((node->getSymbolReference()->getReferenceNumber()
...
bool TR::LocalDeadStoreElimination::areLhsOfStoresSyntacticallyEquivalent(TR::Node* node1, TR::Node* node2)
{
    ...
    if (opCode1.hasSymbolReference()) {
        if (!((node1->getOpCodeValue() == node2->getOpCodeValue()
                && node1->getSymbolReference()->getReferenceNumber()
...
void TR::LocalDeadStoreElimination::adjustStoresInfo(TR::Node* node, TR_BitVector& deadSymbolReferences)
{
    ...
    } else if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monent
...
//
TR::Node* TR::LocalDeadStoreElimination::getAnchorNode(
    TR::Node* parentNode, int32_t nodeIndex, TR::Node* node, TR::TreeTop* treeTop, TR::NodeChecklist& visited)
{
    ...
    if (node->getReferenceCount() > 1) {
        ...
        // TR_FirstNodeReferenceTracker (when used) will mark the earliest reference of a tree commoned node (> 1 ref in
        // same tree) as the first overall reference therefore visit children nodes from first (0) to last
        // (node->getNumChildren()-1) so the first reference flag is propagated correctly with setIsFirstReferenceToNode
        // in the code just above. When not using TR_FirstNodeReferenceTracker the future use count is decremented below
        // to handle this case. NOTE: TR_FirstNodeReferenceTracker is now permanently disabled.
...
void TR::LocalDeadStoreElimination::eliminateDeadObjectInitializations()
{
    ...
    for (i = symRefTab->getIndexOfFirstSymRef(); i < symRefCount; i++) {
        ...
        if (symRef) {
            TR::Symbol* sym = symRef->getSymbol();
            if (sym) {
                if (sym->isLocalObject() && sym->getLocalObjectSymbol()->getOpCodeKind() == TR::New)
    ...
    for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (storeNode && storeNode->getOpCode().isStoreIndirect() && !storeNode->getSymbolReference()->isUnresolved()) {
            if (storeNode->getFirstChild()->getOpCode().hasSymbolReference()
                ...
                && (storeNode->getFirstChild()
                        ->getSymbolReference()
                        ->getSymbol()
                        ->getLocalObjectSymbol()
                        ->getOpCodeKind()
                    == TR::New)
                && !usedLocalObjectSymbols.get(
                    storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalIndex()))
        ...
        if (removableZeroStore) {
            ...
            if (initInfo) {
                if (initInfo->zeroInitSlots || (initInfo->numZeroInitSlots > 0)) {
                    int32_t offset = -1;
                    if (storeNode->getFirstChild()->getOpCode().isArrayRef()) {
                        ...
                        offset = static_cast<int32_t>(storeNode->getSymbolReference()->getOffset()
                            - static_cast<intptr_t>(fe()->getObjectHeaderSizeInBytes()));

                    if (removableZeroStore) {
                        if (initInfo->zeroInitSlots && !initInfo->zeroInitSlots->get(offset / 4)) {
        ...
        if ((removableZeroStore || removableLocalObjectStore)
            && performTransformation(comp(), "%sRemoving Dead Store : %s [0x%p]\n", optDetailString(),
                tt->getNode()->getOpCode().getName(), tt->getNode())) {
...
void TR::LocalDeadStoreElimination::findLocallyAllocatedObjectUses(
    LDSBitVector& usedLocalObjectSymbols, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount)
{
    if (node->getOpCode().hasSymbolReference()
    ...
    List<TR::Node>* currentNews, List<TR::Node>* removedNews, TR::Node* parent, int32_t childNum, vcount_t visitCount)
{
    ...
    if (newNode && !removedNews->find(newNode)) {
