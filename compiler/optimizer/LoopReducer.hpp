...
public:
    ...
    int32_t checkForPostIncrement(
        TR::Block* loopHeader, TR::Node* indVarStoreNode, TR::Node* loopCmpNode, TR::Symbol* ivSym);

    void findIndVarLoads(TR::Node* node, TR::Node* indVarStoreNode, bool& storeFound, List<TR::Node>* ivLoads,
        TR::Symbol* ivSym, vcount_t visitCount);

    TR::Node* updateIndVarStore(
        TR_ParentOfChildNode* indVarNode, TR::Node* indVarStoreNode, TR_LRAddressTree* tree, int32_t postIncrement = 0);
...
public:
    TR_ByteToCharArraycopy(
        TR::Compilation*, TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, bool bigEndian);
...
public:
    TR_CharToByteArraycopy(
        TR::Compilation*, TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, bool isBigEndian);
...
private:
    ...
    bool generateArraytranslateAndTest(
    ...
    bool generateByteToCharArraycopy(
        TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, TR::Block* loopHeader);
    bool generateCharToByteArraycopy(
        TR_InductionVariable* firstIndVar, TR_InductionVariable* secondIndVar, TR::Block* loopHeader);

    bool generateArraytranslate(TR_RegionStructure* whileLoop, TR_InductionVariable* indVar, TR::Block* firstBlock,
        TR::Block* secondBlock, TR::Block* thirdBlock, TR::Block* fourthBlock);
