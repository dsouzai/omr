...
//
//
TR_DataFlowAnalysis::Kind TR_GlobalAnticipatability::getKind() { return GlobalAnticipatability; }

TR_GlobalAnticipatability* TR_GlobalAnticipatability::asGlobalAnticipatability() { return this; }

int32_t TR_GlobalAnticipatability::getNumberOfBits() { return _localTransparency.getNumNodes(); }

void TR_GlobalAnticipatability::analyzeNode(TR::Node*, vcount_t, TR_BlockStructure*, TR_BitVector*) { }

TR_GlobalAnticipatability::TR_GlobalAnticipatability(
    TR::Compilation* comp, TR::Optimizer* optimizer, TR_Structure* rootStructure, bool trace)
...
bool TR_GlobalAnticipatability::postInitializationProcessing()
{
    ...
    _checkExpressionsInBlock
        = (ContainerType**)trMemory()->allocateStackMemory(_numberOfNodes * sizeof(ContainerType*));
...
{
    if (nextNode->getOpCode().isIndirect() && nextNode->getOpCode().isLoadVar()
        && nextNode->getOpCode().hasSymbolReference() && !nextNode->getSymbolReference()->isUnresolved()
        && nextNode->getSymbolReference()->getSymbol()->isShadow() && !nextNode->isInternalPointer()
...
static bool nodeCanSurvive(
    TR::Node* nextNode, TR::Node* lastNodeFirstChild, TR::Node* lastNodeSecondChild, TR::Compilation* comp, bool trace)
{
    if (isFieldAccess(nextNode)) {
        ...
        if (seenSimilarAccess) {
            ...
            int32_t otherLen;
            const char* otherSig
                = otherSymRef->getOwningMethod(comp)->classNameOfFieldOrStatic(otherSymRef->getCPIndex(), otherLen);
            if (otherSig) {
...
bool TR_GlobalAnticipatability::isExceptionalInBlock(
    TR::Node* node, int32_t blockNumber, ContainerType* alreadyInBlock, vcount_t visitCount)
{
    ...
    if (node->getLocalIndex() != -1 && alreadyInBlock->get(node->getLocalIndex()))
        return false;
...
void TR_GlobalAnticipatability::analyzeTreeTopsInBlockStructure(TR_BlockStructure* blockStructure)
{
    ...
    if (block != comp()->getFlowGraph()->getEnd()) {
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (!rare && blockWeight > 1 && next->getStructureOf()->hasBeenAnalyzedBefore()) {
        ...
        if (analyzedSucc) {
            ...
            for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
                ...
                if ((!rare && !next->isCold() && blockWeight > 1) || !hasNonColdSuccessor) {
            ...
            if (blockWeight > 1 && block->getFrequency() == blockStructure->getParent()->getEntryBlock()->getFrequency()
                ...
                && block->isPRECandidate()) // set in loop versioner
            {
                ...
                for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
                    ...
                    if (!rare && blockWeight > 1) {
                        dumpOptDetails(comp(),
                            "Ignoring ifcmp within loop in block_%d and adding available expressions from block_%d\n",
