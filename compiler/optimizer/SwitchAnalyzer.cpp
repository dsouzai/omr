...
int32_t TR::SwitchAnalyzer::perform()
{
    ...
    for (tt = comp()->getStartTree(); tt; tt = exitTree->getNextRealTreeTop()) {
...
void TR::SwitchAnalyzer::analyze(TR::Node* node, TR::Block* block)
{
    ...
    // For the unsigned case, we better use unsigned if the numbers straddle the TR::getMaxSigned<TR::Int32>() boundary.
    // If that's the case, the highVal will be less than (in the signed sense) the lowVal.
    //
    if (highVal < lowVal)
        _signed = false;
    else
    ...
    // Caveats are that zero straddling set must be subdivided and TR::getMaxSigned<TR::Int32>() straddling set could be
    // merged back.
    if (!_signed) {
        fixUpUnsigned(chain);
        fixUpUnsigned(bound);
...
void TR::SwitchAnalyzer::findDenseSets(TR_LinkHead<SwitchInfo>* chain)
{
    ...
    //
    for (SwitchInfo *prev = 0, *cur = chain->getFirst(); cur; prev = cur, cur = cur->getNext()) {
        if (cur->_kind == Unique) {
            ...
            if (end != cur) {
                ...
                for (SwitchInfo *t = cur, *next = t->getNext(); t != tail;) {
                    denseInsert(dense, t);
                    t = next;
                    if (!next)
                        break;
...
bool TR::SwitchAnalyzer::mergeDenseSets(TR_LinkHead<SwitchInfo>* chain)
{
    ...
    while (next) {
        ...
        if (newCost < origCost && ((float)count / span) > _minDensity) {
...
TR_LinkHead<TR::SwitchAnalyzer::SwitchInfo>* TR::SwitchAnalyzer::gather(TR_LinkHead<SwitchInfo>* chain)
{
    ...
    for (; cur; cur = next) {
        ...
        //
        if ((cur->_kind == Range) || (cur->_kind == Dense && cur->_count >= _smallDense)) {
            prev = cur;
            continue;
        }
...
TR::SwitchAnalyzer::SwitchInfo* TR::SwitchAnalyzer::getConsecutiveUniques(SwitchInfo* info)
{
    ...
    while (next && next->_kind == Unique && next->_min == cur->_max + 1) {
        cur = next;
        next = next->getNext();
    }
    return cur;
...
bool TR::SwitchAnalyzer::SwitchInfo::operator>(SwitchInfo& other) { return (_min > other._max); }
...
void TR::SwitchAnalyzer::chainInsert(TR_LinkHead<SwitchInfo>* chain, SwitchInfo* info)
{
    ...
    SwitchInfo *cur, *prev;
    for (prev = 0, cur = chain->getFirst(); cur != NULL; prev = cur, cur = cur->getNext())
        if (*cur > *info)
            break;

    if (cur && cur->_target == info->_target && cur->_min == info->_max + 1) {
        if (cur->_kind != Range) {
            cur->_kind = Range;
...
TR::Block* TR::SwitchAnalyzer::checkIfDefaultIsDominant(SwitchInfo* start)
{
    ...
    for (SwitchInfo* temp = start; temp; temp = temp->getNext()) {
        if (temp->_freq >= cutOffFrequency) {
            ...
            if (trace()) {
                traceMsg(
                    comp(), "Found child with frequency of %f. The default case isn't that dominant.\n", temp->_freq);
            }
            break;
        }
    }
...
void TR::SwitchAnalyzer::emit(
    TR_LinkHead<SwitchInfo>* chain, TR_LinkHead<SwitchInfo>* bound, TR_LinkHead<SwitchInfo>* earlyUniques)
{
    ...
    // This flag indicates that the possible values of the switch operand are withing the min max range of all the case
    // statements
    ...
    case TR::Int16:
        rangeLeft
            = static_cast<CASECONST_TYPE>(_signed ? TR::getMinSigned<TR::Int16>() : TR::getMinUnsigned<TR::Int16>());
        rangeRight
            = static_cast<CASECONST_TYPE>(_signed ? TR::getMaxSigned<TR::Int16>() : TR::getMaxUnsigned<TR::Int16>());
    default:
    ...
    _temp = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), _isInt64 ? TR::Int64 : TR::Int32);

    TR::Block* newBlock = 0;
    if (majorsInBound > 0) {
        if (majorsInBound <= MIN_SIZE_FOR_BIN_SEARCH) {
            newBlock = linearSearch(bound->getFirst());
            if (comp()->isOptServer() && _switch->getOpCodeValue() != TR::lookup) {
                TR::Block* peeledOffBlock = peelOffTheHottestValue(bound);
                if (peeledOffBlock)
                    newBlock = peeledOffBlock;
            }
        } else {
            newBlock = binSearch(bound->getFirst(), getLastInChain(bound), majorsInBound, rangeLeft, rangeRight);
            if (comp()->isOptServer()) {
    ...
    if (majorsInChain > 0) {
        if (majorsInChain <= MIN_SIZE_FOR_BIN_SEARCH) {
            ...
            newBlock = binSearch(chain->getFirst(), getLastInChain(chain), majorsInChain, rangeLeft, rangeRight);
            if (comp()->isOptServer()) {
    ...
    if (majorsInEarly > 0) {
        if (majorsInEarly <= MIN_SIZE_FOR_BIN_SEARCH)
            ...
            newBlock = binSearch(
                earlyUniques->getFirst(), getLastInChain(earlyUniques), majorsInEarly, rangeLeft, rangeRight);
    }
...
TR::Block* TR::SwitchAnalyzer::binSearch(
    SwitchInfo* startNode, SwitchInfo* endNode, int32_t numMajors, CASECONST_TYPE rangeLeft, CASECONST_TYPE rangeRight)
{
...
TR::Block* TR::SwitchAnalyzer::addGotoBlock(TR::TreeTop* dest)
{
    ...
    TR::Block* newBlock
        = TR::Block::createEmptyBlock(node, comp(), dest->getNode()->getBlock()->getFrequency(), _block);
...
TR::Block* TR::SwitchAnalyzer::addIfBlock(TR::ILOpCodes opCode, CASECONST_TYPE val, TR::TreeTop* dest)
{
    ...
    TR::Node* node = TR::Node::createif(opCode, TR::Node::createLoad(_switch, _temp), constNode);
...
TR::Block* TR::SwitchAnalyzer::addTableBlock(SwitchInfo* dense)
{
    ...
    if (_switch && _switch->chkCannotOverflow())
        node->setCannotOverflow(
            true); // Pass on info to code gen that table will have all cases covered and not use default case

    node->setAndIncChild(0,
        TR::Node::create(TR::isub, 2,
            (_isInt64 ? TR::Node::create(TR::l2i, 1, TR::Node::createLoad(_switch, _temp))
    ...
    for (int32_t caseIndex = 0; caseIndex <= upperBound; ++caseIndex) {
        TR::TreeTop* dest;
...
int32_t* TR::SwitchAnalyzer::setupFrequencies(TR::Node* node)
{
    ...
    for (i = node->getCaseIndexUpperBound() - 1; i > 0; --i) {
        ...
        TR_ASSERT_FATAL(targetCount > 0, "Successor block_%d of switch statement has non-sense successsor count",
...
// dense set
bool TR::SwitchAnalyzer::keepAsUnique(SwitchInfo* info, int32_t itemNumber) { return false; }
