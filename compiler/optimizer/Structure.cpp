...
void TR_BlockStructure::clearAnalysisInfo() { setAnalysisInfo(NULL); }
...
{
    for (TR_RegionStructure* parent = getParent()->asRegion(); parent; parent = parent->getParent()->asRegion()) {
        if (parent->contains(other))
            return parent;
    }
    TR_ASSERT(this == cfg->getStructure() || other == cfg->getStructure(),
...
void TR_RegionStructure::cleanupAfterEdgeRemoval(TR::CFGNode* node)
{
    ...
    if (node != getEntry() && node->getPredecessors().empty() && node->getExceptionPredecessors().empty()) {
        // The node is now isolated and should be removed
        //
        if (node->getSuccessors().empty() && node->getExceptionSuccessors().empty()) {
    ...
    else if ((numSubNodes() == 1) && !isNaturalLoop() && !getEntry()->hasSuccessor(getEntry())
...
private:
    ...
    void moveOutgoingEdgeToParent(TR_RegionStructure* region, TR_RegionStructure* parent,
        TR_StructureSubGraphNode* node, TR::CFGEdge* edge, bool isExceptionEdge);
...
/* static */ void TR_RegionStructure::extractUnconditionalExits(
    TR::Compilation* const comp, const TR::list<TR::Block*, TR::Region&>& blocks)
{
    if (blocks.empty())
        return;
...
void TR_RegionStructure::ExitExtraction::extractUnconditionalExits(const TR::list<TR::Block*, TR::Region&>& blocks)
{
    collectWork(blocks);
...
void TR_RegionStructure::ExitExtraction::collectWork(const TR::list<TR::Block*, TR::Region&>& blocks)
{
    ...
    for (auto b = blocks.begin(); b != blocks.end(); ++b) {
        ...
        while (s != NULL && relevant.find(s) == relevant.end()) {
            ...
            if (_trace) {
                traceMsg(_comp, "found relevant structure %d:%p, parent %d:%p\n", s->getNumber(), s,
                    parent == NULL ? -1 : parent->getNumber(), parent);
            }
...
void TR_RegionStructure::ExitExtraction::collectWorkFromRegion(TR_RegionStructure* region, const StructureSet& relevant)
{
...
void TR_RegionStructure::ExitExtraction::extractStructure(TR_Structure* const initialStructure)
{
    ...
    for (;;) {
        TR_ASSERT_FATAL(node->getStructure()->getParent() == region,
        ...
        if (!performTransformation(_comp,
                ...
                OPT_DETAILS, node->getNumber(), region->getNumber(), region, parent->getNumber(), parent))
            return;
...
#else
        const bool checkStructure = _comp->getOption(TR_CheckStructureDuringExitExtraction);
#endif
        if (checkStructure) {
...
TR_BitVector& TR_RegionStructure::ExitExtraction::regionContents(TR_RegionStructure* const region)
{
...
void TR_RegionStructure::ExitExtraction::removeContentsFromRegion(
    TR_Structure* const removed, TR_RegionStructure* const region)
{
    ...
    if (removed->asBlock() != NULL) {
        ...
        TR_ASSERT_FATAL(subregionEntry != _regionContents.end(),
            "region %d:%p has contents, "
            "but (previously) contained subregion %d:%p does not\n",
            region->getNumber(), region, removed->getNumber(), removed);

        blocks -= subregionEntry->second;
    }

    if (_trace) {
        traceMsg(_comp, "adjusted contents of region %d:%p:", region->getNumber(), region);
...
void TR_RegionStructure::ExitExtraction::moveNodeIntoParent(
    TR_StructureSubGraphNode* const node, TR_RegionStructure* const region, TR_RegionStructure* const parent)
{
    if (node == region->getEntry()) {
        // Replace this entire region in the parent, since entry can't be removed
        TR_ASSERT_FATAL(region->numSubNodes() == 1,
    ...
    for (auto e = _allPreds.begin(); e != _allPreds.end(); ++e) {
        ...
        if (_trace) {
            traceMsg(_comp, "removed edge (%d->%d):%p from region %d:%p\n", (*e)->getFrom()->getNumber(),
                (*e)->getTo()->getNumber(), *e, region->getNumber(), region);
        }
    }
    ...
    const bool nodeIsHandler = node->getStructure()->getEntryBlock()->isCatchBlock();

    for (auto e = _allPreds.begin(); e != _allPreds.end(); ++e) {
        auto* const src = toStructureSubGraphNode((*e)->getFrom());
        region->addExitEdge(src, node->getNumber(), nodeIsHandler);
        if (_trace) {
            traceMsg(_comp, "added exit edge (%d->%d) to region %d:%p\n", src->getNumber(), node->getNumber(),
    ...
    if (_trace) {
        traceMsg(_comp, "added %sedge (%d->%d) to region %d:%p\n", nodeIsHandler ? "exception " : "",
            regionNode->getNumber(), node->getNumber(), parent->getNumber(), parent);
    }
    ...
    if (region->getParent() == NULL) {
        if (_trace) {
            traceMsg(_comp, "region %d:%p was eliminated by cleanupAfterNodeRemoval\n", region->getNumber(), region);
        }

        return;
    }

    TR_ASSERT_FATAL(region->getParent() == parent,
...
void TR_RegionStructure::ExitExtraction::moveOutgoingEdgeToParent(TR_RegionStructure* const region,
    TR_RegionStructure* const parent, TR_StructureSubGraphNode* const node, TR::CFGEdge* const edge,
    bool isExceptionEdge)
{
    TR_ASSERT_FATAL(region->isExitEdge(edge),
    ...
    if (_trace) {
        traceMsg(_comp, "removed exit edge (%d->%d):%p from region %d:%p\n", edge->getFrom()->getNumber(),
            edge->getTo()->getNumber(), edge, region->getNumber(), region);
    }
    ...
    if (!regionStillHasThisExit) {
        ...
        if (_trace) {
            traceMsg(_comp,
                ...
                region->getNumber(), region, tgtNum);
        }
    }
    ...
    if (redirect == NULL) {
        ...
        if (_trace) {
            traceMsg(_comp, "successor %d does not exist in parent - created new exit %sedge\n", tgtNum,
        ...
        if (_trace) {
            traceMsg(_comp, "parent region contains %d - created internal %sedge\n", tgtNum,
                isExceptionEdge ? "exception " : "");
        }
    }
}
...
void TR_RegionStructure::cleanupAfterNodeRemoval()
{
    ...
    //
    if ((numSubNodes() == 1) && !isNaturalLoop() && !getEntry()->hasSuccessor(getEntry())
        && !getEntry()->hasExceptionSuccessor(getEntry())) {
        TR_RegionStructure::Cursor si(*this);
        TR_ASSERT(si.getFirst() == getEntry(), "bad structure found trying to remove a block");
        if (getParent())
...
TR_StructureSubGraphNode* TR_RegionStructure::subNodeFromStructure(TR_Structure* const structure)
{
    const int32_t num = structure->getNumber();
    TR_StructureSubGraphNode* const node = findSubNodeInRegion(num);
    TR_ASSERT_FATAL(node != NULL && node->getStructure() == structure,
        "subNodeFromStructure: in region %p, "
        "expected node %d to have structure %p, but found %p\n",
        this, num, structure, node->getStructure());
...
void TR_RegionStructure::replacePart(TR_Structure* from, TR_Structure* to)
{
    ...
    TR_ASSERT(node != getEntry() || _nodeIndex == to->getNumber(), "cannot replace the entry node in %d with node %d",
        _nodeIndex, to->getNumber());
    ...
    if (from->getNumber() != to->getNumber()) {
        ...
        for (auto edge = node->getExceptionPredecessors().begin(); edge != node->getExceptionPredecessors().end();
             ++edge) {
            TR_RegionStructure* pred = toStructureSubGraphNode((*edge)->getFrom())->getStructure()->asRegion();
            if (pred)
                pred->replaceExitPart(from->getNumber(), to->getNumber());
...
static bool findCycleDEPRECATED(TR_StructureSubGraphNode* node, TR_BitVector& regionNodes,
    TR_BitVector& nodesSeenOnPath, TR_BitVector& nodesCleared, int32_t entryNode)
{
    ...
    for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
        TR_ASSERT((*edge)->getTo()->asStructureSubGraphNode(),
            "Expecting a CFG node which can be downcast to StructureSubGraphNode");
...
static bool findCycle(
    TR::Compilation* comp, TR_StructureSubGraphNode* origin, TR_BitVector& regionNodes, int32_t entryNode)
{
     ...
     * Processed flag supersedes Discovered flag. If the node is marked as Processed, it counts as being in the
     * Processed state.
     */
     ...
     * If a node is set to Processed, both that node and its children have already been looked at and a cycle was not
     * found.
     */
     ...
     * After all the children have been handled and set to Processed the node on the top of the stack will have
     * childrenProcessed set to true which sets the node to being Processed.
     */
    ...
    while (!nodeStack.empty()) {
        ...
        /*
         * If the current node is set to Discovered then that means it was seen before as part of the path between
         * origin and the current node. This means a cycle has been found. findCycle can end immediately and return
         * true.
         *
         * If the current node is not set to Discovered, set it to Discovered to indicate it is now part of the path
         * back to origin.
         */
...
void TR_RegionStructure::checkForInternalCycles()
{
    ...
    if (!useOldFindCycle) {
        ...
        setContainsInternalCycles(
            findCycleDEPRECATED(getEntry(), regionNodes, nodesSeenOnPath, nodesCleared, getNumber()));
    }
}
...
void TR_RegionStructure::removeEdgeWithoutCleanup(TR::CFGEdge* edge, bool isExitEdge)
{
    ...
    if (std::find(fromNode->getSuccessors().begin(), fromNode->getSuccessors().end(), edge)
        != fromNode->getSuccessors().end()) {
        fromNode->getSuccessors().remove(edge);
        TR_ASSERT((std::find(toNode->getPredecessors().begin(), toNode->getPredecessors().end(), edge)
                      != toNode->getPredecessors().end()),
            "bad structure found trying to remove an out edge");
        toNode->getPredecessors().remove(edge);
    } else {
        TR_ASSERT((std::find(fromNode->getExceptionSuccessors().begin(), fromNode->getExceptionSuccessors().end(), edge)
                      != fromNode->getExceptionSuccessors().end()),
            "bad structure found trying to remove an out edge");
        fromNode->getExceptionSuccessors().remove(edge);
        TR_ASSERT((std::find(toNode->getExceptionPredecessors().begin(), toNode->getExceptionPredecessors().end(), edge)
                      != toNode->getExceptionPredecessors().end()),
            "bad structure found trying to remove an out edge");
...
int TR_RegionStructure::removeExternalEdgeTo(TR_Structure* from, int32_t toNumber)
{
    ...
    if (removedEdgeType == NO_MORE_EDGES_EXIST) {
        ...
        if (moreThanOneExitEdgeToSameToNumber)
            ...
            if (!toNode->getExceptionPredecessors().empty() || !toNode->getPredecessors().empty())
                removedEdgeType = EDGES_STILL_EXIST;
        }
    }
...
void TR_Structure::setConditionalityWeight(int32_t* currentWeight)
{
    ...
    if (region && (!region->isAcyclic() || (region == comp()->getFlowGraph()->getStructure()))) {
        adjustWeightForBranches(region->getEntry(), region->getEntry(), currentWeight);
    } else if (region->isAcyclic())
        isAcyclicRegion = true;

    if (isAcyclicRegion && getParent())
        getParent()->setConditionalityWeight(currentWeight);
}

void TR_Structure::adjustWeightForBranches(
    TR_StructureSubGraphNode* node, TR_StructureSubGraphNode* entry, int32_t* currentWeight)
{
...
bool TR_RegionStructure::containsOnlyAcyclicRegions()
{
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        ...
        if (region && (!region->isAcyclic() || !region->containsOnlyAcyclicRegions()))
            return false;
    }
...
TR_Structure* TR_BlockStructure::cloneStructure(TR::Block** correspondingBlocks,
    TR_StructureSubGraphNode** correspodingSubNodes, List<TR_Structure>* whileLoops,
    List<TR_Structure>* correspondingWhileLoops)
{
    TR::Block* correspondingBlock = correspondingBlocks[getNumber()];
    TR_BlockStructure* clonedBlockStructure = new (cfg()->structureMemoryRegion())
        TR_BlockStructure(comp(), correspondingBlock->getNumber(), correspondingBlock);
...
TR_Structure* TR_RegionStructure::cloneStructure(TR::Block** correspondingBlocks,
    TR_StructureSubGraphNode** correspondingSubNodes, List<TR_Structure>* whileLoops,
    List<TR_Structure>* correspondingWhileLoops)
{
    TR::Region& structureMemoryRegion = cfg()->structureMemoryRegion();
    TR_RegionStructure* clonedRegionStructure
        = new (structureMemoryRegion) TR_RegionStructure(comp(), correspondingBlocks[getNumber()]->getNumber());
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        subStruct = subNode->getStructure();
        TR_Structure* clonedSubStruct = subStruct->cloneStructure(
            correspondingBlocks, correspondingSubNodes, whileLoops, correspondingWhileLoops);
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        ...
        for (auto edge = subNode->getExceptionSuccessors().begin(); edge != subNode->getExceptionSuccessors().end();
             ++edge) {
            TR_StructureSubGraphNode* succ = toStructureSubGraphNode((*edge)->getTo());
            if (!isExitEdge(*edge)) {
                TR::CFGEdge::createExceptionEdge(
                    correspondingSubNode, correspondingSubNodes[succ->getNumber()], trMemory());
            } else {
    ...
    if (isNaturalLoop() && whileLoops->find(this)) {
...
void TR_RegionStructure::addExternalEdge(TR_Structure* from, int32_t toNumber, bool isExceptionEdge)
{
    ...
    si.reset();
    for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
        if (edge->getTo()->getNumber() == toNumber && edge->getFrom() == fromNode)
            return;
    }
...
void TR_RegionStructure::collapseIntoParent()
{
    ...
    for (node = subNodes.getCurrent(); node; node = subNodes.getNext()) {
            ...
            if (regionNodes.get(toNum)) {
                ...
                for (target = pTargets.getCurrent(); target; target = pTargets.getNext()) {
                    if (target->getNumber() == toNum) {
                        ...
                        // removed it.
                        //
                        for (auto pred = target->getPredecessors().begin(); pred != target->getPredecessors().end();
                             ++pred) {
        ...
        for (auto edge = node->getExceptionSuccessors().begin(); edge != node->getExceptionSuccessors().end(); ++edge) {
            ...
            if (regionNodes.get(toNum)) {
                ...
                for (target = pTargets.getCurrent(); target; target = pTargets.getNext()) {
                    if (target->getNumber() == toNum) {
                        for (auto pred = target->getExceptionPredecessors().begin();
                             pred != target->getExceptionPredecessors().end(); ++pred) {
                            if (((*pred)->getFrom()->getNumber() == getNumber())
                                && (toStructureSubGraphNode((*pred)->getFrom())->getStructure() == this)) {
    ...
    //
    for (auto edge = nodeInParent->getExceptionPredecessors().begin();
         edge != nodeInParent->getExceptionPredecessors().end(); ++edge)
    ...
    for (auto edge = nodeInParent->getExceptionSuccessors().begin();
         edge != nodeInParent->getExceptionSuccessors().end();) {
        TR::CFGEdge* current = *(edge++);
        parent->getExitEdges().remove(current);
        TR::CFGNode* toNode = current->getTo();
...
TR::CFGEdge* TR_RegionStructure::addExitEdge(
    TR_StructureSubGraphNode* from, int32_t to, bool isExceptionEdge, TR::CFGEdge* origEdge)
{
    ...
    TR::CFGEdge* edge;
    TR::CFGNode* toNode = cursor ? cursor->getTo()
                                 : new (cfg()->structureMemoryRegion())
                                     TR_StructureSubGraphNode(to, cfg()->structureMemoryRegion());
    ...
    if (origEdge) {
        ...
        edge = isExceptionEdge ? TR::CFGEdge::createExceptionEdge(from, toNode, trMemory())
...
TR_StructureSubGraphNode* TR_StructureSubGraphNode::create(int32_t num, TR_RegionStructure* region)
{
    ...
    return new (region->cfg()->structureMemoryRegion())
        TR_StructureSubGraphNode(num, region->cfg()->structureMemoryRegion());
}
...
{
    TR_ASSERT_FATAL(
        this->getNumber() == _block->getNumber(), "Number of BlockStructure is NOT the same as that of the block");
...
{
    TR_StructureSubGraphNode* node;
    TR_ASSERT_FATAL(this->getNumber() == getEntry()->getStructure()->getNumber(),
        "Entry node does not have same number as this RegionStructure");
    TR_RegionStructure::Cursor si(*this);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
        TR_ASSERT_FATAL(
            this == node->getStructure()->getParent(), "subGraphNode does not have this RegionStructure as its parent");
        TR_ASSERT_FATAL(node->getNumber() == node->getStructure()->getNumber(),
            "subGraphNode does not have the same node number as its structure");
        ...
        for (auto edge = node->getPredecessors().begin(); edge != node->getPredecessors().end(); ++edge) {
            ...
            TR_ASSERT_FATAL(
                isConsistent, "predecessor of this subGraph node not not contain this node in its successors");
        }
        for (auto edge = node->getExceptionPredecessors().begin(); edge != node->getExceptionPredecessors().end();
             ++edge) {
            pred = toStructureSubGraphNode((*edge)->getFrom());
            isConsistent = false;
            for (auto succEdge = pred->getExceptionSuccessors().begin();
                 succEdge != pred->getExceptionSuccessors().end(); ++succEdge) {
                if ((*succEdge) == (*edge)) {
                    isConsistent = true;
                    break;
                }
            }
            TR_ASSERT_FATAL(isConsistent,
                "exception predecessor of this subGraph node not not contain this node in its exception successors");
        }

        for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
            succ = toStructureSubGraphNode((*edge)->getTo());
            isConsistent = false;
            for (auto predEdge = succ->getPredecessors().begin(); predEdge != succ->getPredecessors().end();
                 ++predEdge) {
                if (*predEdge == *edge) {
                    isConsistent = true;
                    break;
            ...
            TR_ASSERT_FATAL(
                isConsistent, "successor of this subGraph node not not contain this node in its predecessors");
        }
        for (auto edge = node->getExceptionSuccessors().begin(); edge != node->getExceptionSuccessors().end(); ++edge) {
            succ = toStructureSubGraphNode((*edge)->getTo());
            isConsistent = false;
            for (auto predEdge = succ->getExceptionPredecessors().begin();
                 predEdge != succ->getExceptionPredecessors().end(); ++predEdge) {
                if ((*predEdge) == *edge) {
                    isConsistent = true;
                    break;
                }
            ...
            TR_ASSERT_FATAL(isConsistent,
                "exception successor of this subGraph node not not contain this node in its exception predecessors");
        }
        ...
        if (node->getStructure()->asRegion() != NULL) {
            ...
            for (auto edge = excSuccs.begin(); edge != excSuccs.end(); ++edge) {
                ...
                TR_ASSERT_FATAL(std::find(excPreds.begin(), excPreds.end(), exit) != excPreds.end(),
                    "exit in subregion %p corresponding to subnode %d:%p edge to %d "
            ...
            for (auto exit = exits.getFirst(); exit != NULL; exit = exits.getNext()) {
                ...
                auto& excPreds = exitNode->getExceptionPredecessors();
                const bool isExc = std::find(excPreds.begin(), excPreds.end(), exit) != excPreds.end();

                TR::CFGEdgeList& succs = isExc ? node->getExceptionSuccessors() : node->getSuccessors();
    ...
    for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
        ...
        for (seenNode = exitNodesIt.getFirst(); seenNode; seenNode = exitNodesIt.getNext()) {
            if ((seenNode->getNumber() == toNode->getNumber()) && (seenNode != toNode))
                TR_ASSERT_FATAL(0,
                    "Exit edges to the same node number %d in region %p must have a unique structure subgraph node "
                    "(node1 %p node2 %p)\n",
...
void TR_BlockStructure::resetAnalysisInfo() { setAnalysisInfo(NULL); }
...
void TR_BlockStructure::resetAnalyzedStatus() { setAnalyzedStatus(false); }
...
bool TR_BlockStructure::changeContinueLoopsToNestedLoops(TR_RegionStructure* root) { return false; }
    ...
    if (isNaturalLoop()) {
        ...
        for (auto nextEdge = entryBlock->getPredecessors().begin(); nextEdge != entryBlock->getPredecessors().end();
            ...
            if (contains(pred->getStructureOf(), getParent())) {
                ...
                TR_ASSERT((*nextEdge)->getTo()->asBlock() == entryBlock,
                    "Natural loop shouldn't have incoming edge to nonheader block");
                incomingEdges.add(*nextEdge);
            }
        }

        if (numNewHeaders == 1
            && performTransformation(comp(), "%s transforming continues in loop %d to %d new nested loops\n",
            ...
            for (TR::CFGEdge* splitEdge = seIterator.getFirst(); splitEdge; splitEdge = seIterator.getNext()) {
                TR::Block* newHeader = TR::Block::createEmptyBlock(
                    entryBlock->getEntry()->getNode(), comp(), splitEdge->getFrequency(), prevHeader);
                ...
                if (traceMyself)
                    diagnostic("Rerouting edge %d->old header %d to new header %d\n",
                        splitEdge->getFrom()->asBlock()->getNumber(), splitEdge->getTo()->asBlock()->getNumber(),
                        newHeader->getNumber());

                // change destination at the end of splitEdge->getFrom()->asBlock() to branch to newHeader rather than
                // entryBlock, updating edges as needed
            ...
            for (TR::CFGEdge* incomingEdge = ieIterator.getFirst(); incomingEdge; incomingEdge = ieIterator.getNext()) {
                if (traceMyself) {
                    diagnostic("Incoming [%p] edge %d->%d\n", incomingEdge,
                        incomingEdge->getFrom()->asBlock()->getNumber(), incomingEdge->getTo()->asBlock()->getNumber());
...
TR_StructureSubGraphNode* TR_RegionStructure::findNodeInHierarchy(int32_t num)
{
    TR_RegionStructure::Cursor it(*this);
    TR_StructureSubGraphNode* node;
    ...
    return getParent() ? getParent()->findNodeInHierarchy(num) : 0;
}

int32_t TR_Structure::getNumberOfLoops()
{
        ...
        for (edge = _exitEdges.getListHead(); edge != NULL; edge = edge->getNextElement()) {
            ...
            if (toNode->getNumber() == mergedInto->getNumber()) {
                if (mergedIsCatch) {
                    ...
                    for (auto normalEdge = toNode->getPredecessors().begin();
                         normalEdge != toNode->getPredecessors().end();) {
...
void TR_RegionStructure::renumber(int32_t num)
{
    ...
    while (edge) {
        ...
        if (edge->getData()->getTo()->getNumber() == num) {
            ...
            if (std::find(edge->getData()->getFrom()->getExceptionSuccessors().begin(),
                    edge->getData()->getFrom()->getExceptionSuccessors().end(), edge->getData())
                == edge->getData()->getFrom()->getExceptionSuccessors().end())
...
void TR_BlockStructure::hoistInvariantsOutOfNestedLoops(TR_LocalTransparency* transparencyInfo,
    TR_BitVector** optSetInfo, bool insideLoop, TR_BlockStructure* outerLoopInvariantBlock,
    TR_RegionStructure* outerLoop, int32_t bitVectorSize)
{ }
...
void TR_RegionStructure::hoistInvariantsOutOfNestedLoops(TR_LocalTransparency* transparencyInfo,
    TR_BitVector** optSetInfo, bool insideLoop, TR_BlockStructure* outerLoopInvariantBlock,
    TR_RegionStructure* outerLoop, int32_t bitVectorSize)
{
    bool isInvariantLoop = false;
    ...
    if (isCanonicalizedLoop() && getEntry()->getSuccessors().size() == 2) {
        ...
        if (loopInvariantBlock) {
            if (loopInvariantNode->getSuccessors().size() == 1) {
                loopStructure = ((TR_StructureSubGraphNode*)loopInvariantNode->getSuccessors().front()->getTo())
                                    ->getStructure()
                                    ->asRegion();
                if (loopStructure) {
        ...
        if (isInvariantLoop) {
            ...
            TR_RegionStructure::Cursor si(*this);
            for (node = si.getCurrent(); node != NULL; node = si.getNext())
                node->getStructure()->hoistInvariantsOutOfNestedLoops(
                    transparencyInfo, optSetInfo, true, loopInvariantBlock, loopStructure, bitVectorSize);
                            ...
                            if (outerLoop->isExpressionTransparentIn(nextOptimalComputation, transparencyInfo)) {
                                ...
                                while (currentTree != exitTree) {
                                    if (currentTree->getNode()->getOpCode().isCheck()) {
                                        ...
                                        if (currentNode->getSymbolReference()->getSymbol()->isAuto()) {
                                            if (currentNode->getFirstChild()->getLocalIndex()
                                ...
                                if (treeToBeHoisted != NULL) {
                                    ...
                                    TR::TreeTop* outerExitTree
                                        = outerLoopInvariantBlock->getBlock()->getLastRealTreeTop();
                                ...
                                dumpOptDetails(comp(),
                                    "\nO^O PARTIAL REDUNDANCY ELIMINATION: Hoisting loop invariant computation %d over "
                                    "nested loops (lifted from inner block_%d to outer block_%d)\n",
                                    nextOptimalComputation, loopInvariantBlock->getNumber(),
                                    outerLoopInvariantBlock->getNumber());
    ...
    if (!isInvariantLoop) {
        ...
        for (node = si.getCurrent(); node != NULL; node = si.getNext())
            node->getStructure()->hoistInvariantsOutOfNestedLoops(
                transparencyInfo, optSetInfo, insideLoop, outerLoopInvariantBlock, outerLoop, bitVectorSize);
    }
}

bool TR_BlockStructure::isExpressionTransparentIn(
    int32_t nextOptimalComputation, TR_LocalTransparency* transparencyInfo)
{
...
bool TR_RegionStructure::isExpressionTransparentIn(
    int32_t nextOptimalComputation, TR_LocalTransparency* transparencyInfo)
{
    TR_StructureSubGraphNode* node;
    TR_RegionStructure::Cursor si(*this);
