 ...
 */
/* static */ void TR_RegionStructure::extractUnconditionalExits(TR::Compilation * const comp,
...
void TR_RegionStructure::ExitExtraction::removeContentsFromRegion(TR_Structure * const removed,
 ...
 */
static bool findCycle(TR::Compilation *comp, TR_StructureSubGraphNode *origin, TR_BitVector &regionNodes,
    int32_t entryNode)
{
...
void TR_Structure::adjustWeightForBranches(TR_StructureSubGraphNode *node, TR_StructureSubGraphNode *entry,
    ...
    List<TR_Structure> *correspondingWhileLoops)
{
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        subStruct = subNode->getStructure();
        TR_Structure *clonedSubStruct = subStruct->cloneStructure(correspondingBlocks, correspondingSubNodes,
            whileLoops, correspondingWhileLoops);
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        ...
        for (auto edge = subNode->getExceptionSuccessors().begin(); edge != subNode->getExceptionSuccessors().end();
            ...
            TR_StructureSubGraphNode *succ = toStructureSubGraphNode((*edge)->getTo());
            if (!isExitEdge(*edge)) {
                TR::CFGEdge::createExceptionEdge(correspondingSubNode, correspondingSubNodes[succ->getNumber()],
                    trMemory());
            } else {
...
TR::CFGEdge *TR_RegionStructure::addExitEdge(TR_StructureSubGraphNode *from, int32_t to, bool isExceptionEdge,
    TR::CFGEdge *origEdge)
{
...
{
    TR_ASSERT_FATAL(this->getNumber() == _block->getNumber(),
        "Number of BlockStructure is NOT the same as that of the block");
...
void TR_RegionStructure::checkStructure(TR_BitVector *_blockNumbers)
{
    ...
    TR_RegionStructure::Cursor si(*this);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
        TR_ASSERT_FATAL(this == node->getStructure()->getParent(),
            "subGraphNode does not have this RegionStructure as its parent");
        ...
        for (auto edge = node->getPredecessors().begin(); edge != node->getPredecessors().end(); ++edge) {
            ...
            TR_ASSERT_FATAL(isConsistent,
                "predecessor of this subGraph node not not contain this node in its successors");
        }
        for (auto edge = node->getExceptionPredecessors().begin(); edge != node->getExceptionPredecessors().end();
             ++edge) {
            pred = toStructureSubGraphNode((*edge)->getFrom());
        ...
        for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
            ...
            TR_ASSERT_FATAL(isConsistent,
                "successor of this subGraph node not not contain this node in its predecessors");
        }
        for (auto edge = node->getExceptionSuccessors().begin(); edge != node->getExceptionSuccessors().end(); ++edge) {
            succ = toStructureSubGraphNode((*edge)->getTo());
            isConsistent = false;
    ...
    if (isNaturalLoop()) {
        ...
        if (numNewHeaders == 1
            ...
            for (TR::CFGEdge *splitEdge = seIterator.getFirst(); splitEdge; splitEdge = seIterator.getNext()) {
                TR::Block *newHeader = TR::Block::createEmptyBlock(entryBlock->getEntry()->getNode(), comp(),
                    splitEdge->getFrequency(), prevHeader);
    ...
    TR_RegionStructure *outerLoop, int32_t bitVectorSize)
{
    ...
    if (isCanonicalizedLoop() && getEntry()->getSuccessors().size() == 2) {
        ...
        if (isInvariantLoop) {
            ...
            TR_RegionStructure::Cursor si(*this);
            for (node = si.getCurrent(); node != NULL; node = si.getNext())
                node->getStructure()->hoistInvariantsOutOfNestedLoops(transparencyInfo, optSetInfo, true,
                    loopInvariantBlock, loopStructure, bitVectorSize);
    ...
    if (!isInvariantLoop) {
        ...
        TR_RegionStructure::Cursor si(*this);
        for (node = si.getCurrent(); node != NULL; node = si.getNext())
            node->getStructure()->hoistInvariantsOutOfNestedLoops(transparencyInfo, optSetInfo, insideLoop,
                outerLoopInvariantBlock, outerLoop, bitVectorSize);
    }
}

bool TR_BlockStructure::isExpressionTransparentIn(int32_t nextOptimalComputation,
    TR_LocalTransparency *transparencyInfo)
{
...
bool TR_RegionStructure::isExpressionTransparentIn(int32_t nextOptimalComputation,
    TR_LocalTransparency *transparencyInfo)
{
    TR_StructureSubGraphNode *node;
    TR_RegionStructure::Cursor si(*this);
