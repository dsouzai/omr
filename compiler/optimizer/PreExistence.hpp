...
public:
    ...
    static void propagateReceiverInfoIfAvailable(
        TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_LogTracer* tracer);

    static void propagateArgsFromCaller(
        TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_LogTracer* tracer);

    static bool validateAndPropagateArgsFromCalleeSymbol(
        TR_PrexArgInfo* argsFromSymbol, TR_PrexArgInfo* argsFromTarget, TR_LogTracer* tracer);

    static TR_PrexArgInfo* buildPrexArgInfoForMethodSymbol(
        TR::ResolvedMethodSymbol* methodSymbol, TR_LogTracer* tracer);
    ...
    void dumpTrace();
    static TR::TreeTop* getCallTree(
        TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_LogTracer* tracer);
...
private:
    ...
    //
#ifdef J9_PROJECT_SPECIFIC
    static TR::Node* getCallNode(
        TR::ResolvedMethodSymbol* methodSymbol, class TR_CallSite* callsite, class TR_LogTracer* tracer);
