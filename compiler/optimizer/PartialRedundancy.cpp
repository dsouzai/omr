...
static bool shouldAllowOptimalSubNodeReplacement(TR::Compilation* comp)
{
    if ((comp->getOptions()->getLastOptTransformationIndex() < comp->getOptions()->getMaxLastOptTransformationIndex())
        || (comp->getOptions()->getFirstOptTransformationIndex()
            > comp->getOptions()->getMinFirstOptTransformationIndex())) {
...
static bool shouldAllowLimitingOfOptimalPlacement(TR::Compilation* comp)
{
    if (shouldAllowOptimalSubNodeReplacement(comp)) {
        if ((comp->getOptions()->getLastOptTransformationIndex()
                < comp->getOptions()->getMaxLastOptTransformationIndex())
            || (comp->getOptions()->getFirstOptTransformationIndex()
                > comp->getOptions()->getMinFirstOptTransformationIndex()))
            return false;
    }
...
bool TR_PartialRedundancy::ignoreNode(TR::Node* node)
{
    ...
    if (ignoreValueOfMultipleReturnNode(node, seenNodes))
        return true;
...
int32_t TR_PartialRedundancy::perform()
{
    ...
    _numProfilingsAllowed
        = (((USHRT_MAX / 2 - 1000) - comp()->getNodeCount()) / NUMBER_OF_NODES_PER_PROFILING_EXPANSION);
    ...
    _newSymbolReferences
        = (TR::SymbolReference**)trMemory()->allocateStackMemory(_numberOfBits * sizeof(TR::SymbolReference*));
    _newSymbolsMap = (int32_t*)trMemory()->allocateStackMemory(_numberOfBits * sizeof(int32_t));
    _registerCandidates
        = (TR::RegisterCandidate**)trMemory()->allocateStackMemory(_numberOfBits * sizeof(TR::RegisterCandidate*));
    ...
    for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
        ...
        *negation -= *temp;
        ...
        // _isolatedness->copyFromInto(localAnticipatability.getAnalysisInfo(blockStructure->asBlock()->getBlock()->getNumber()),
        // _rednSetInfo[i]);
        *_rednSetInfo[i]
            = *localAnticipatability.getDownwardExposedAnalysisInfo(blockStructure->asBlock()->getBlock()->getNumber());
    ...
    for (bvi1.SetToFirstOne(); bvi1.Valid(); bvi1.SetToNextOne()) {
        ...
        if (nextRedundantComputation != 0) {
            supportedNodesAsArray[nextRedundantComputation]
                = supportedNodesAsArray[nextRedundantComputation]->duplicateTreeWithCommoning(comp()->allocator());
    ...
    for (bvi2.SetToFirstOne(); bvi2.Valid(); bvi2.SetToNextOne()) {
        ...
        if (nextOptimalComputation != 0) {
            ...
            if ((_newSymbolsMap[nextOptimalComputation] < 0) && !nextOptimalNode->getOpCode().isCheck()
                && !nextOptimalNode->getOpCode().isAnchor() && !(nextOptimalNode->getOpCodeValue() == TR::treetop)) {
                if (nextOptimalNode->getOpCode().isLoadVarDirect() && !nextOptimalNode->getSymbol()->isStatic()
                    ...
                    if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) != nextOptimalNode->getDataType()
                        && // only do it for Java where we use istore for small intergral types
                    ...
                    TR::SymbolReference* newSymbolReference
                        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType, false, size);
                    TR::Symbol* newAutoSym = newSymbolReference->getSymbol();

                    comp()->AddCopyPropagationRematerializationCandidate(newSymbolReference);

                    if (nextOptimalNode->getOpCode().hasSymbolReference()
                        && nextOptimalNode->getSymbolReference()->getSymbol()->isNotCollected())
    ...
    for (bvi3.SetToFirstOne(); bvi3.Valid(); bvi3.SetToNextOne()) {
        ...
        TR::Node* nextOptimalNode = supportedNodesAsArray[nextOptimalComputation];
        if (nextOptimalNode->getOpCode().isCheck()
            || (comp()->requiresSpineChecks() && nextOptimalNode->getOpCode().hasSymbolReference()
                && nextOptimalNode->getSymbol()->isArrayShadowSymbol()))
            continue;
...
//
TR::Node* TR_PartialRedundancy::duplicateExact(
    TR::Node* node, List<TR::Node>* seenNodes, List<TR::Node>* duplicateNodes, vcount_t visitCount)
{
...
void TR_PartialRedundancy::calculateGlobalRegisterWeightsBasedOnStructure(
    TR_Structure* structure, int32_t* currentWeight)
{
...
bool TR_PartialRedundancy::isNodeAnImplicitNoOp(TR::Node* node)
{
    ...
    if (node->getOpCode().isArrayRef() && node->getSecondChild()->getOpCode().isLoadConst()
...
#ifdef J9_PROJECT_SPECIFIC
    if (!TR::Compiler->cls.romClassObjectsMayBeCollected() && node->getOpCode().hasSymbolReference()
...
//
bool TR_PartialRedundancy::isOpCodeAnImplicitNoOp(TR::ILOpCode& opCode) { return comp()->cg()->opCodeIsNoOp(opCode); }
...
void TR_PartialRedundancy::processReusedNode(
    TR::Node* node, TR::ILOpCodes newOpCode, TR::SymbolReference* newSymRef, int newNumChildren)
{
#ifdef J9_PROJECT_SPECIFIC
...
TR::TreeTop* TR_PartialRedundancy::placeComputationsOptimally(TR::Block* block, TR::Node*** supportedNodesAsArray)
{
    ...
    ContainerType* anticipatabilityInfo
        = _isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability->_localAnticipatability
              .getDownwardExposedAnalysisInfo(block->getNumber());
    ...
    for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement]; seenOptimalElements < numOptimalElements;
        ...
        if (anticipatabilityInfo->get(nextOptimalComputation)) {
            ...
            while (placeToInsertOptimalComputations != exitTree) {
                if (nextOptimalNode->getOpCode().isCheck()) {
                        ...
                        if (nextNode->getLocalIndex() == nextOptimalComputation) {
                            if (nextNode->getOpCode().isNullCheck()) {
                                if (nextNode->getFirstChild()->exceptionsRaised() != 0) {
                                    ...
                                    // TR::Node *duplicateOptimalNode =
                                    // duplicateExact(nextOptimalNode->getNullCheckReference(), &seenNodes,
                                    // &duplicateNodes, visitCount1);
                                    ...
                                    TR::Node* passThroughNode
                                        = TR::Node::create(TR::PassThrough, 1, nextNode->getNullCheckReference());
                                    TR::Node* nodeInTreeTop
                                        = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passThroughNode, newSymRef);
                                    ...
                                    if (!shouldAllowLimitingOfOptimalPlacement(comp())
                                        || performTransformation(
                                            comp(), "%sPlacing NULLCHK optimally : %p\n", OPT_DETAILS, nodeInTreeTop)) {
                                        ...
                                        if (comp()->useCompressedPointers()) {
                                            ...
                                            if (reference->getOpCode().isLoadIndirect()
                                                ...
                                                translateTT = TR::TreeTop::create(comp(),
                                                    TR::Node::createCompressedRefsAnchor(reference), NULL, NULL);
                                            }
                                        }
                    ...
                    TR::Node* optimalNode = getAlreadyPresentOptimalNode(placeToInsertOptimalComputations->getNode(),
                        nextOptimalComputation, comp()->getVisitCount(), rhsOfStore);
                    ...
                    if (optimalNode
                        && ((optimalNode->getLocalIndex() != nextOptimalComputation) || rhsOfStore
                            || !lastInsertionMade)) {
                        //
                        ...
                        if (!shouldAllowLimitingOfOptimalPlacement(comp())
                            || performTransformation(comp(), "%s0Placing %s optimally : %p using symRef #%d\n",
                                OPT_DETAILS, optimalNode->getOpCode().getName(), optimalNode,
                                newSymbolReference->getReferenceNumber())) {
                            _useAliasSetsNotGuaranteedToBeCorrect = true;

                            TR::Node* convertedOptimalNode = optimalNode;
                            if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType())
                                != nextOptimalNode->getDataType()) {
                            ...
                            if (comp()->useCompressedPointers()) {
                                if (convertedOptimalNode->getOpCode().isLoadIndirect()
                                    ...
                                    translateTT = TR::TreeTop::create(
                                        comp(), TR::Node::createCompressedRefsAnchor(convertedOptimalNode), NULL, NULL);
                                }
                            }

                            TR::Node* storeForCommonedNode = TR::Node::createWithSymRef(
                                comp()->il.opCodeForDirectStore(nextOptimalNode->getDataType()), 1, 1,
                            ...
                            // optimalNode->getOpCode().getName(), optimalNode,
                            // newSymbolReference->getReferenceNumber());
        ...
        if (placeToInsertOptimalComputations == block->getExit()) {
            ...
            if (relevantNode->getOpCode().isResolveOrNullCheck() || (relevantNode->getOpCodeValue() == TR::treetop))
                relevantNode = relevantNode->getFirstChild();

            TR::ILOpCode& placeHolderOpCode = relevantNode->getOpCode();
            if (placeHolderOpCode.isBranch() || placeHolderOpCode.isJumpWithMultipleTargets()
                || placeHolderOpCode.isReturn() || placeHolderOpCode.getOpCodeValue() == TR::athrow)
        ...
        if (nextOptimalNode->getOpCode().isStore()) {
            ...
            if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK)
                ...
                TR::Node* duplicateOptimalNode = duplicateExact(
                    nextOptimalNode->getNullCheckReference(), &seenNodes, &duplicateNodes, visitCount1);
                ...
                if (isSupportedOpCode(nullCheckReferenceNode, NULL) && !nullCheckReferenceOpCode.isLoadVarDirect()) {
                    if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT)
                            && (nullCheckReferenceNode->getLocalIndex() != 0))) {
                        if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] > -1)
                            && (((!_unavailableSetInfo[block->getNumber()]->get(
                                     nullCheckReferenceNode->getLocalIndex()))
                                    && (_rednSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex())
                                        || _optSetInfo[block->getNumber()]->get(
                                            nullCheckReferenceNode->getLocalIndex())))
                                || (comp()->requiresSpineChecks()
                                    && nullCheckReferenceNode->getOpCode().hasSymbolReference()
                                    && nullCheckReferenceNode->getSymbol()->isArrayShadowSymbol()))) {
                            replacedNullCheckReference = true;
                            TR::SymbolReference* newSymbolReference
                                = _newSymbolReferences[nullCheckReferenceNode->getLocalIndex()];
                            TR::Node* newLoad = NULL;
                            newLoad = TR::Node::createWithSymRef(nullCheckReferenceNode,
                ...
                if (!replacedNullCheckReference) {
                    ...
                    for (k = 0; k < nextOptimalNode->getNullCheckReference()->getNumChildren(); k++) {
                        ...
                        placeToInsertOptimalComputations = replaceOptimalSubNodes(placeToInsertOptimalComputations,
                            nextOptimalNode->getNullCheckReference(),
                            nextOptimalNode->getNullCheckReference()->getChild(k), k, duplicateOptimalNode,
                            duplicateOptimalNode->getChild(k), false, block->getNumber(), visitCount4);
                    }
                }
                ...
                if (!shouldAllowLimitingOfOptimalPlacement(comp())
                    ...
                    if (comp()->useCompressedPointers()) {
                        TR::Node* reference = passThroughNode->getFirstChild();
                        if (reference->getOpCode().isLoadIndirect() && reference->getDataType() == TR::Address
                            && TR::TransformUtil::fieldShouldBeCompressed(reference, comp())) {
                            translateTT = TR::TreeTop::create(
                                comp(), TR::Node::createCompressedRefsAnchor(reference), NULL, NULL);
                        }
                    }
                    manager()->setAlteredCode(true);
                    TR::TreeTop* secondTreeInBlock = placeToInsertOptimalComputations->getNextTreeTop();
                ...
                TR_ScratchList<TR::Node> seenNodes(trMemory()), duplicateNodes(trMemory());
                TR::Node* duplicateOptimalNode
                    = duplicateExact(nextOptimalNode, &seenNodes, &duplicateNodes, visitCount1);
                ...
                for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
                    vcount_t visitCount4 = comp()->incVisitCount();
                    placeToInsertOptimalComputations = replaceOptimalSubNodes(placeToInsertOptimalComputations,
                        nextOptimalNode, nextOptimalNode->getChild(k), k, duplicateOptimalNode,
                        duplicateOptimalNode->getChild(k), false, block->getNumber(), visitCount4);
                }
                ...
                if (!shouldAllowLimitingOfOptimalPlacement(comp())
                    || performTransformation(comp(), "%sPlacing %s optimally : %p\n", OPT_DETAILS,
                        duplicateOptimalTree->getNode()->getOpCode().getName(), duplicateOptimalTree->getNode())) {
                    TR::TreeTop* translateTT = NULL;
                    if (comp()->useCompressedPointers()) {
        ...
        } else if (isSupportedOpCode(nextOptimalNode, NULL) && (!isNodeAnImplicitNoOp(nextOptimalNode))) {
            //
            ...
            if (fe()->dataTypeForLoadOrStore(duplicateOptimalNode->getDataType())
                != duplicateOptimalNode->getDataType()) {
                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(duplicateOptimalNode->getDataType(),
                    fe()->dataTypeForLoadOrStore(duplicateOptimalNode->getDataType()), false /* !wantZeroExtension */);
                duplicateOptimalNode->setReferenceCount(0);
            ...
            dumpOptDetails(comp(), "%s1Placing %s optimally : %p using symRef #%d\n", OPT_DETAILS,
                convertedOptimalNode->getOpCode().getName(), convertedOptimalNode,
                newSymbolReference->getReferenceNumber());
            ...
            if (comp()->useCompressedPointers()) {
                ...
                if (reference->getOpCode().isLoadIndirect() || reference->getOpCode().isArrayLength()) {
                    if (reference->getFirstChild()->getOpCode().isLoadIndirect()
                        ...
                        translateTT = TR::TreeTop::create(
                            comp(), TR::Node::createCompressedRefsAnchor(reference->getFirstChild()), NULL, NULL);
                        placeToInsertOptimalComputations->join(translateTT);
                        translateTT->join(secondTreeInBlock);
                        placeToInsertOptimalComputations = translateTT;
                    }
                    ...
                    if (reference->getDataType() == TR::Address
                        && TR::TransformUtil::fieldShouldBeCompressed(reference, comp())) {
                        translateTT
                            = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(reference), NULL, NULL);
                        placeToInsertOptimalComputations->join(translateTT);
                        translateTT->join(secondTreeInBlock);
                        placeToInsertOptimalComputations = translateTT;
                    }
            ...
            if (duplicateOptimalNode->getOpCode().isCall()
                ...
                TR::TreeTop* anchorTT
                    = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, duplicateOptimalNode), NULL, NULL);
...
// not found
//
TR::Node* TR_PartialRedundancy::getAlreadyPresentOptimalNode(
    TR::Node* node, int32_t nextOptimalComputation, vcount_t visitCount, bool& rhsOfStore)
{
    ...
    for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
        TR::Node* child
            = getAlreadyPresentOptimalNode(node->getChild(childNum), nextOptimalComputation, visitCount, rhsOfStore);
        if (child)
            return child;
    }
...
static bool isExpressionRedundant(TR::Node* node, TR_PartialRedundancy::ContainerType* redundantComputations,
    TR_PartialRedundancy::ContainerType* anticipatable)
{
    ...
    if (redundantComputations && node->getLocalIndex() != MAX_SCOUNT && node->getLocalIndex() != 0
...
//
void TR_PartialRedundancy::eliminateRedundantComputations(
    TR::Block* block, TR::Node** supportedNodesAsArray, ContainerType** rednSetInfo, TR::TreeTop* startTree)
{
    ...
    ContainerType* anticipatabilityInfo
        = _isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability->_localAnticipatability
              .getAnalysisInfo(block->getNumber());

    {
        ...
        while (currentTree != exitTree) {
            ...
            //
            eliminateRedundantSupportedNodes(NULL, currentTree->getNode(), false, currentTree, block->getNumber(),
                visitCount, shouldHaveRedundantComputations ? redundantComputations : NULL, anticipatabilityInfo,
                supportedNodesAsArray);
            ...
            if ((firstOpCodeInTree.isStore()
                ...
                || ((firstOpCodeInTree.isResolveOrNullCheck()
                        || (firstOpCodeInTree.getOpCodeValue() == TR::ArrayStoreCHK) || firstOpCodeInTree.isAnchor()
                    ...
                    if (!currentTree->getNode()->getOpCode().isStore()) {
                        if (0
                            && performTransformation(comp(),
                                "%sEliminating redundant check computation (resolve or null check) : %p\n", OPT_DETAILS,
                                currentTree->getNode())) // Look at check-in comment for version 1.296
                        {
                            ...
                            // dumpOptDetails(comp(), "%sEliminating redundant check computation (resolve or null check)
		      ...
		      performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()) */)
                     {
                            ...
                            if (currentTree->getNode()->getOpCode().isStoreIndirect()) {
                                ...
                                if (fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType())
                                    != rhsOfStore->getDataType()) {
                                    TR::ILOpCodes conversionOpCode
                                        = TR::ILOpCode::getProperConversion(rhsOfStore->getDataType(),
                                            fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()),
                                            false /* !wantZeroExtension */);
                                    convertedOptimalNode = TR::Node::create(conversionOpCode, 1, rhsOfStore);
                                }

                                storeForCommonedNode = TR::Node::createWithSymRef(
                                    opCodeValue, 1, 1, convertedOptimalNode, newSymbolReference);
                            } else
                                storeForCommonedNode = TR::Node::createWithSymRef(
                                    opCodeValue, 1, 1, currentTree->getNode()->getFirstChild(), newSymbolReference);
#ifdef J9_PROJECT_SPECIFIC
                            correctDecimalPrecision(
                                storeForCommonedNode, storeForCommonedNode->getFirstChild(), comp());
#endif
                ...
                if ((currentTree->getNode()->getNumChildren() > 0)
                    && isExpressionRedundant(
                        currentTree->getNode()->getFirstChild(), redundantComputations, anticipatabilityInfo)
                    && currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
		  ...
		  performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()->getFirstChild()) */)
                 {
                        ...
                        if (currentTree->getNode()->getFirstChild()->getOpCode().isStoreIndirect()) {
                            ...
                            if (fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()) != rhsOfStore->getDataType()) {
                                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
                                    rhsOfStore->getDataType(), fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()),
                                    false /* !wantZeroExtension */);
                                convertedOptimalNode = TR::Node::create(conversionOpCode, 1, rhsOfStore);
                            }
                        ...
                        dumpOptDetails(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS,
                            currentTree->getNode()->getFirstChild());
                ...
                if (isExpressionRedundant(currentTree->getNode(), redundantComputations, anticipatabilityInfo)) {
                    if (firstOpCodeInTree.isResolveOrNullCheck()) {
                        ...
                        if (performTransformation(comp(), "%sEliminating redundant check computation (%s) : %p\n",
                            ...
                            TR::Node* nextAnchor;
                            for (nextAnchor = anchorIt.getFirst(); nextAnchor != NULL;
                                 nextAnchor = anchorIt.getNext()) {
                                TR::TreeTop* anchorTreeTop
                                    = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, nextAnchor));
...
// in that range)
//
void TR_PartialRedundancy::collectAllNodesToBeAnchored(
    List<TR::Node>* anchoredNodes, TR::Node* node, vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return;
...
//
bool TR_PartialRedundancy::eliminateRedundantSupportedNodes(TR::Node* parent, TR::Node* node, bool firstComputation,
    TR::TreeTop* currentTree, int32_t blockNum, vcount_t visitCount, ContainerType* redundantComputations,
    ContainerType* anticipatabilityInfo, TR::Node** supportedNodesAsArray)
{
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        TR::Node* child = node->getChild(i);
        if (!eliminateRedundantSupportedNodes(node, child, flag, currentTree, blockNum, visitCount,
                redundantComputations, anticipatabilityInfo, supportedNodesAsArray))
            flag = false;
    }
    ...
    if (isSupportedOpCode(node, parent)) {
        ...
        if (_profilingWalk
            ...
            // being profiled here
            //
            if (comp()->isProfilingCompilation() && ((node->getType().isInt32()) || ((node->getType().isInt64())))
                && /* &&
node->getOpCode().isLoadVar() &&
node->getSymbolReference()->getSymbol()->isAutoOrParm())) && */
                !node->getOpCode().isLoadConst() && (_numProfilingsAllowed > 0)
                ...
                if (currentNode->getOpCode().isResolveOrNullCheck() || (currentNode->getOpCodeValue() == TR::treetop))
                    currentNode = currentNode->getFirstChild();

                if (!node->getType().isInt64()) {
                    TR::ILOpCode& currentOpCode = currentNode->getOpCode();
                    if (currentOpCode.isBranch() || currentOpCode.isJumpWithMultipleTargets()
                        || currentOpCode.isReturn() || currentOpCode.getOpCodeValue() == TR::athrow)
                    ...
                    if (trace())
                        traceMsg(
                            comp(), "Added profiling instrumentation for %p(%d)\n", node, node->getByteCodeIndex());
                }
            }
        }
#endif
        ...
        //
        if ((node->getOpCode().isLoadVarDirect() && !node->getSymbol()->isStatic()
        ...
        if (isExpressionRedundant(node, redundantComputations, anticipatabilityInfo)) {
            ...
            if (!isNodeAnImplicitNoOp(node)) {
                ...
                if (node->getOpCode().isLoadConst()) {
                    if ((parent->getOpCode().isDiv() || parent->getOpCode().isRem())
                ...
                if (newSymbolReference
                    && performTransformation(comp(),
                        "%sEliminating redundant computation (%s) : %p in block_%d, visit count %d\n", OPT_DETAILS,
                        node->getOpCode().getName(), node, blockNum, node->getVisitCount())) {
                    if (node->getDataType() == TR::NoType) {
                        ...
                        if (parent && parent->getOpCode().isConversion()
                            && (parent->getOpCodeValue()
                                == TR::ILOpCode::getProperConversion(
                                    parent->getDataType(), node->getDataType(), false /*!wantZeroExtension*/))) {
                            if (node->getReferenceCount() > 1) {
                                TR::TreeTop* anchorTreeTop
                                    = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, node));
                                TR::TreeTop* prevTree = currentTree->getPrevTreeTop();
                                anchorTreeTop->join(currentTree);
                                prevTree->join(anchorTreeTop);
                            }
                            ...
                            processReusedNode(
                                parent, comp()->il.opCodeForDirectLoad(nodeDataType), newSymbolReference, 0);
                        } else {
                            for (j = 0; j < node->getNumChildren(); j++)
                                node->getChild(j)->recursivelyDecReferenceCount();

                            TR::Node* newLoad = TR::Node::createWithSymRef(
                                node, comp()->il.opCodeForDirectLoad(node->getDataType()), 0, newSymbolReference);

                            TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
                                newLoad->getDataType(), node->getDataType(), false /* !wantZeroExtension */);
                            if (TR::ILOpCode::isVectorOpCode(conversionOpCode)
                                && TR::ILOpCode::getVectorOperation(conversionOpCode) == TR::vconv) {
                                node = TR::Node::create(TR::ILOpCode::createVectorOpCode(
                                                            TR::vconv, newLoad->getDataType(), node->getDataType()),
                                    1, newLoad);
                            } else {
...
{
    if (_prevTree && (_prevTree->getNode()->getOpCode().isFloat() || _prevTree->getNode()->getOpCode().isDouble())
        && (_prevTree->getNode()->getSymbol() == symbol))
        return false;
...
//
TR::TreeTop* TR_PartialRedundancy::replaceOptimalSubNodes(TR::TreeTop* curTree, TR::Node* parent, TR::Node* node,
    int32_t childNum, TR::Node* duplicateParent, TR::Node* duplicateOptimalNode, bool isNullCheck, int32_t blockNum,
    vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return curTree;
    ...
    if (isSupportedOpCode(node, parent)
        ...
        if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0))
            ...
            // a result of earlier optimal node placement not having been done (because of opt transformation index)
            // that interfere with last opt transformation index based search. This is done because optimal sub node
            // replacement is rarely the cause of bugs on it's own and this issue is just making it harder to debug
            // problems in other PRE transformations. We created another option TR_TracePREForOptimalSubNodeReplacement
            // to continue to allow optimal sub node replacement to be done regardless but in order to make this more
            // useful we turn off limiting optimal placements when this new option is in effect (so that it becomes safe
            // to narrow down to a optimal sub node replacement in the unlikely event that this is the cause of some
            // failure)
            //
            if (shouldAllowOptimalSubNodeReplacement(comp())
                && (((_newSymbolsMap[node->getLocalIndex()] > -1)
                    && ((/* _optSetInfo[blockNum]->get(node->getLocalIndex()) || */ (
                             (!parent || !parent->getOpCode().isSpineCheck() || (childNum != 0))
                             && comp()->requiresSpineChecks() && node->getOpCode().hasSymbolReference()
                             && node->getSymbol()->isArrayShadowSymbol())
                        || !_unavailableSetInfo[blockNum]->get(node->getLocalIndex())))))) {
                TR::SymbolReference* newSymbolReference = _newSymbolReferences[node->getLocalIndex()];
                TR::Node* newLoad = TR::Node::createWithSymRef(
                    node, comp()->il.opCodeForDirectLoad(nodeDataType), 0, newSymbolReference);
                if (fe()->dataTypeForLoadOrStore(node->getDataType()) != node->getDataType())
                    newLoad = TR::Node::create(TR::ILOpCode::getProperConversion(newLoad->getDataType(),
                                                   node->getDataType(), false /* !wantZeroExtension */),
                        1, newLoad);
                ...
                for (i = 0; i < node->getNumChildren(); i++) {
                    ...
                    curTree = replaceOptimalSubNodes(curTree, node, child, i, duplicateOptimalNode, duplicateChild,
                        isNullCheck, blockNum, visitCount);
                }

                if (node->getOpCode().isCall()) {
                    TR::TreeTop* anchorTreeTop
                        = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, duplicateOptimalNode));
            ...
            for (i = 0; i < node->getNumChildren(); i++) {
                TR::Node* child = node->getChild(i);
                TR::Node* duplicateChild = duplicateOptimalNode->getChild(i);
                curTree = replaceOptimalSubNodes(
                    curTree, node, child, i, duplicateOptimalNode, duplicateChild, isNullCheck, blockNum, visitCount);
            }
        }
    } else {
        int32_t i;
...
//
TR_ExceptionCheckMotion::TR_ExceptionCheckMotion(
    TR::Compilation* comp, TR::Optimizer* optimizer, TR_PartialRedundancy* partialRedundancy)
    : TR_DataFlowAnalysis(comp, comp->getFlowGraph(), optimizer, partialRedundancy->trace())
    , _partialRedundancy(partialRedundancy)
    , _workingList(comp->trMemory())
{
    ...
    for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
        ...
        /////_orderedOptNumbersList[i] = (int32_t
        ///*)trMemory()->allocateStackMemory((_optimisticOptSetInfo[i]->elementCount()+_optimisticRednSetInfo[i]->elementCount())*sizeof(int32_t));
        //
...
int32_t TR_ExceptionCheckMotion::areExceptionSuccessorsIdentical(TR::CFGNode* node1, TR::CFGNode* node2)
{
    ...
    _secondSucc->empty();
    _comparison->empty();
    for (auto nextEdge = node1->getExceptionSuccessors().begin(); nextEdge != node1->getExceptionSuccessors().end();
         ++nextEdge) {
        TR::CFGNode* succ = (*nextEdge)->getTo();
        _firstSucc->set(succ->getNumber());
    }

    for (auto nextEdge = node2->getExceptionSuccessors().begin(); nextEdge != node2->getExceptionSuccessors().end();
         ++nextEdge) {
        TR::CFGNode* succ = (*nextEdge)->getTo();
...
int32_t TR_ExceptionCheckMotion::perform()
{
    ...
    for (numIterations = 0; numIterations < NUM_ITERATIONS; numIterations++) {
            ...
            *_tempContainer &= *_exprsUnaffectedByOrder;
            ...
            if (_orderedOptList[i] != NULL) {
                ...
                if (orderedListSize > 0) {
                    ...
                    TR_ASSERT(orderedListSize <= numElements2,
                        "Overwriting elements! orderedListSize = %d numElements2 = %d\n", orderedListSize,
                        numElements2);
                }
            }
        ...
        if (redundantSetAdjustmentRequired || _trySecondBestSolution) {
            ...
            TR_RedundantExpressionAdjustment* redundantExpressionAdjustment = new (comp()->allocator())
                TR_RedundantExpressionAdjustment(comp(), optimizer(), comp()->getFlowGraph()->getStructure(), this);
...
void TR_ExceptionCheckMotion::composeLists(
    List<TR::Node>* targetList, List<TR::Node>* sourceList, ContainerType* availableExprs)
{
    _composeHelper->empty();
    if (!sourceList->isEmpty() && !targetList->isEmpty()) {
        ...
        _definitelyNotKilled->empty();
        while (oldNode && !oldNode->getData()->getOpCode().isCheck()) {
        ...
        for (newNode = sourceList->getListHead(); newNode != NULL;) {
            ...
            while (newNode && !newNode->getData()->getOpCode().isCheck()) {
                if (!_composeHelper->get(newNode->getData()->getLocalIndex())) {
                    ListElement<TR::Node>* newSourceNode
                        = (ListElement<TR::Node>*)trMemory()->allocateStackMemory(sizeof(ListElement<TR::Node>));
            ...
            if ((oldNode == NULL) || (newNode == NULL)
                || (oldNode->getData()->getLocalIndex() != newNode->getData()->getLocalIndex())) {
                ...
                while (oldNode
                    ...
                    if (!_composeHelper->get(oldNode->getData()->getLocalIndex())) {
                        ...
                        if (prevCursorNode) {
                            ...
                            // prevCursorNode->getData(), oldNode, oldNode->getData());
                        } else
        ...
        ListElement<TR::Node>* oldNode = targetList->getListHead();
        while (oldNode
            && ((oldNode->getData()->getOpCode().isIndirect() || oldNode->getData()->getOpCode().isArrayLength())
                && availableExprs->get(oldNode->getData()->getFirstChild()->getLocalIndex()))) {
            ListElement<TR::Node>* nextOldNode = oldNode->getNextElement();
            // if (!_composeHelper->get(oldNode->getData()->getLocalIndex()))
            {
        ...
        while (newNode
            && ((newNode->getData()->getOpCode().isIndirect() || newNode->getData()->getOpCode().isArrayLength())
                && availableExprs->get(newNode->getData()->getFirstChild()->getLocalIndex()))) {
            // if (!_composeHelper->get(newNode->getData()->getLocalIndex()))
            {
                ListElement<TR::Node>* newSourceNode
...
TR_ExceptionCheckMotion::ExprDominanceInfo* TR_ExceptionCheckMotion::createAnalysisInfo()
{
    ...
    analysisInfo->_outList
        = (List<TR::Node>**)trMemory()->allocateStackMemory(_numberOfNodes * sizeof(List<TR::Node>*));
...
void TR_ExceptionCheckMotion::analyzeNodeToInitializeGenAndKillSets(
    TR::TreeTop* tt, vcount_t visitCount, TR_BlockStructure* blockStructure)
{
	      ...
	      !checkIfNodeCanSurvive(node->getNullCheckReference(), _indirectAccessesThatSurvive))
              {
                ...
                if ((opCodeValue == TR::ResolveAndNULLCHK) && (node->getFirstChild()->getOpCode().isCallIndirect()))
                    firstChildNum = 1;
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(
                         node->getLocalIndex()) /* &&
                                                       !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())*/
                        || _resolveCheckKilled->get(blockNum) || nodeKilledByChild)) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                    _divCheckKilled->set(blockNum);
                 ...
                 nodeKilledByChild)
              {
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(
                         node->getLocalIndex()) /* &&
                                                       !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */
                        || _divCheckKilled->get(blockNum) || nodeKilledByChild)
    ...
    // if (!_atLeastOneExceptionPoint)
    {
        ...
        for (i = 0; i < node->getNumChildren(); i++) {
            if (!_atLeastOneExceptionPoint || !checkNode) {
                if (includeRelevantNodes(node->getChild(i), visitCount, blockNum))
                    childRelevant = true;
            } else {
                TR::Node* child = node->getChild(i);
                if (checkNode && (child->getVisitCount() != visitCount)) {
...
{
    ListElement<TR::Node>* newElement
        = (ListElement<TR::Node>*)trMemory()->allocateStackMemory(sizeof(ListElement<TR::Node>));
...
bool TR_ExceptionCheckMotion::isNodeKilledByChild(TR::Node* parent, TR::Node* child, int32_t blockNum)
{
    ...
    if (((child->getLocalIndex() != MAX_SCOUNT)
            && (child->getLocalIndex() != 0)) /* && (!child->getOpCode().isStore()) */) {
        if (_exprsContainingIndirectAccess->get(child->getLocalIndex())) {
        ...
        if (isNodeKilledByChild(parent, child->getFirstChild(), blockNum)
            || isNodeKilledByChild(parent, child->getSecondChild(), blockNum))
            parentKilled = true;
    }
...
bool TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node* node, vcount_t visitCount, int32_t blockNum)
{
    if (node->getVisitCount() == visitCount) {
        if (((node->getLocalIndex() != MAX_SCOUNT)
                && (node->getLocalIndex() != 0)) /* && (!node->getOpCode().isStore()) */) {
            if (_relevantNodes->get(node->getLocalIndex()))
                return true;
        } else if (node->getOpCode().isTwoChildrenAddress()) {
            TR::Node* firstChild = node->getFirstChild();
            TR::Node* secondChild = node->getSecondChild();
            if (((firstChild->getLocalIndex() != MAX_SCOUNT)
                    && (firstChild->getLocalIndex() != 0)) /* && (!firstChild->getOpCode().isStore()) */) {
                if (_relevantNodes->get(firstChild->getLocalIndex()))
                    return true;
            }

            if (((secondChild->getLocalIndex() != MAX_SCOUNT)
                    && (secondChild->getLocalIndex() != 0)) /* && (!secondChild->getOpCode().isStore()) */) {
    ...
    bool childRelevant = false, containsIndirectAccess = false, containsArrayAccess = false, containsDivide = false,
         containsUnresolvedAccess = false;
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++) {
        if (includeRelevantNodes(node->getChild(i), visitCount, blockNum)
            && (!node->getOpCode().isStore() || (i != rhsChild))) {
            TR::Node* child = node->getChild(i);
            childRelevant = true;
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) /* && (!opCode.isStore()) */) {
        if ((!_actualRednSetInfo[blockNum]->get(node->getLocalIndex()))
            ...
            if (containsIndirectAccess
                || ((opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore())) || (opCode.isArrayLength()))) {
                _exprsContainingIndirectAccess->set(node->getLocalIndex());
                if (_indirectAccessesKilled->get(blockNum)) {
...
void TR_ExceptionCheckMotion::initializeOutList(List<TR::Node>* list) { list->deleteAll(); }
...
bool TR_ExceptionCheckMotion::analyzeNodeIfSuccessorsAnalyzed(
    TR::CFGNode* cfgNode, TR_RegionStructure* regionStructure, ContainerType* pendingList, ContainerType* exitNodes)
{
    bool anyNodeChanged = false;

    while (
        _analysisQueue.getListHead() && (_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        {
            for (auto succ = nodeStructure->getSuccessors().begin(); succ != nodeStructure->getSuccessors().end();
                 ++succ) {
                if (!regionStructure->isExitEdge(*succ)) {
                    ...
                    if (!(isNaturalLoop && (succNode == regionStructure->getEntry()))) {
                        if (pendingList->get(succNode->getNumber()) && (!alreadyVisitedNode)) {
                            ...
                            break;
                            // return analyzeNodeIfSuccessorsAnalyzed(succNode, regionStructure, pendingList,
                            // exitNodes);
                        }
            ...
            for (auto succ = nodeStructure->getExceptionSuccessors().begin();
                 succ != nodeStructure->getExceptionSuccessors().end(); ++succ) {
                if (!regionStructure->isExitEdge(*succ)) {
                    TR_StructureSubGraphNode* succNode = toStructureSubGraphNode((*succ)->getTo());
                    TR_Structure* succStructure = succNode->getStructure();
        ...
        for (auto pred = nodeStructure->getPredecessors().begin(); pred != nodeStructure->getPredecessors().end();
             ++pred) {
        ...
        for (auto pred = nodeStructure->getExceptionPredecessors().begin();
             pred != nodeStructure->getExceptionPredecessors().end(); ++pred) {
            TR_StructureSubGraphNode* predNode = (TR_StructureSubGraphNode*)(*pred)->getFrom();
            TR_Structure* predStructure = predNode->getStructure();
            if (pendingList->get(predStructure->getNumber()) || inSetChanged) {
                // comp()->incVisitCount();
...
bool TR_ExceptionCheckMotion::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    ...
    *_tempContainer &= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
                            ->_globalAnticipatability->_localTransparency.getAnalysisInfo(blockNum);
    ...
    *_tempContainer &= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
    ...
    /////for (nextAvailableComputation = bvi.getFirstElement(); j < numAvailExprs; nextAvailableComputation =
    ///bvi.getNextElement())
    ...
    *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
    ...
    if (_orderedOptList[blockNum]) {
        ...
        for (nextOptElement = _orderedOptList[blockNum]->getListHead(); nextOptElement != NULL;
             nextOptElement = nextOptElement->getNextElement()) {
    ...
    for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL; listElem = listElem->getNextElement()) {
        ...
        if (!_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) {
            ...
            if (_exprsContainingIndirectAccess->get(listElem->getData()->getLocalIndex())
                && optimalIndirectAccessesKilled)
                canPlaceOptimally = false;
            else if (_exprsContainingArrayAccess->get(listElem->getData()->getLocalIndex())
                && optimalArrayAccessesKilled)
                canPlaceOptimally = false;
            else if (_exprsContainingDivide->get(listElem->getData()->getLocalIndex()) && optimalDividesKilled)
                canPlaceOptimally = false;
            else if (_exprsContainingUnresolvedAccess->get(listElem->getData()->getLocalIndex())
                && optimalUnresolvedAccessesKilled)
                canPlaceOptimally = false;
            ...
            if (checkElement) {
                ...
                } else if (listElem->getData()->getOpCode().isBndCheck()
                    || listElem->getData()->getOpCode().isSpineCheck()) {
                    nullCheckKilled = true;
                    resolveCheckKilled = true;
                    divCheckKilled = true;
                    arrayStoreCheckKilled = true;
            ...
            if (canPlaceOptimally && !_actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                ...
                if (checkElement) {
                    ...
                    if (opCodeValue == TR::NULLCHK) {
                        ...
                        || listElem->getData()->getOpCode().isBndCheck()) {
                        nullCheckKilledByOptList = true;
                        resolveCheckKilledByOptList = true;
                        divCheckKilledByOptList = true;
                        arrayStoreCheckKilledByOptList = true;
    ...
    *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
    ...
    for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL; listElem = listElem->getNextElement()) {
        ...
        if (_exprsContainingIndirectAccess->get(listElem->getData()->getLocalIndex())) {
            if (indirectAccessesKilledByOptList || genExprThatIsKilled) {
                if (!checkIfNodeCanSomehowSurvive(node, _indirectAccessesThatSurvive)) {
                    checkKilledByChild = true;
                    if (!opCode.isCheck()) {
                        removeFromList(listElem, _blockInfo[blockNum], prevElem);
        ...
        if (_exprsContainingUnresolvedAccess->get(listElem->getData()->getLocalIndex())) {
            if (unresolvedAccessesKilledByOptList || genExprThatIsKilled) {
                if (!checkIfNodeCanSomehowSurvive(node, _unresolvedAccessesThatSurvive)) {
                    checkKilledByChild = true;
                    if (!opCode.isCheck()) {
                        removeFromList(listElem, _blockInfo[blockNum], prevElem);
        ...
        if (_exprsContainingArrayAccess->get(listElem->getData()->getLocalIndex())) {
            if (arrayAccessesKilledByOptList || genExprThatIsKilled) {
                if (!checkIfNodeCanSomehowSurvive(node, _arrayAccessesThatSurvive)) {
                    checkKilledByChild = true;
                    if (!opCode.isCheck()) {
                        removeFromList(listElem, _blockInfo[blockNum], prevElem);
        ...
        if (_exprsContainingDivide->get(listElem->getData()->getLocalIndex())) {
            if (dividesKilledByOptList || genExprThatIsKilled) {
                if (!checkIfNodeCanSomehowSurvive(node, _dividesThatSurvive)) {
                    checkKilledByChild = true;
                    if (!opCode.isCheck())
                        removeFromList(listElem, _blockInfo[blockNum], prevElem);
...
bool TR_ExceptionCheckMotion::checkIfNodeCanSomehowSurvive(TR::Node* node, ContainerType* nodesThatSurvive)
{
    ...
    if (!_exprsUnaffectedByOrder->get(node->getLocalIndex())) {
        ...
        if (((opCode.isIndirect() || (opCode.isArrayLength()))
                && (!node->getFirstChild()->isThisPointer() || !node->getFirstChild()->isNonNull())
                && (!(opCode.hasSymbolReference() && node->getSymbolReference()->getSymbol()->isArrayletShadowSymbol()))
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        } else if ((i == 1) && (node->getOpCode().isDiv() || node->getOpCode().isRem()) && isNodeValueZero(child)) {
            allChildrenSurvive = false;
            break;
        }
        /*
    ...
    if ((allChildrenSurvive) && (node->getNumChildren() > 0)) {
...
bool TR_ExceptionCheckMotion::checkIfNodeCanSurvive(TR::Node* node, ContainerType* nodesThatSurvive)
{
    if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) {
        ...
        //
        if ((node->getOpCode().isDiv() || node->getOpCode().isRem()) && isNodeValueZero(node->getSecondChild()))
            return false;
        ...
        return false;
    } else if ((node->getOpCodeValue() == TR::aconst) && (node->getAddress() == 0)) {
        //
...
void TR_ExceptionCheckMotion::removeFromList(
    ListElement<TR::Node>* listElem, List<TR::Node>* list, ListElement<TR::Node>* prevElem)
{
    if (prevElem)
...
TR_DataFlowAnalysis::Kind TR_RedundantExpressionAdjustment::getKind() { return RedundantExpressionAdjustment; }

TR_RedundantExpressionAdjustment* TR_RedundantExpressionAdjustment::asRedundantExpressionAdjustment() { return this; }

bool TR_RedundantExpressionAdjustment::supportsGenAndKillSets() { return true; }

bool TR_RedundantExpressionAdjustment::supportsGenAndKillSetsForStructures() { return false; }

int32_t TR_RedundantExpressionAdjustment::getNumberOfBits() { return _partialRedundancy->getNumberOfBits(); }
...
//
TR_RedundantExpressionAdjustment::TR_RedundantExpressionAdjustment(TR::Compilation* comp, TR::Optimizer* optimizer,
...
void TR_RedundantExpressionAdjustment::initializeGenAndKillSetInfo()
{
    ...
    for (i = 1; i < _numberOfNodes; ++i) {
        ...
        if (actualOptSetInfo[i]) {
            ...
            *_regularGenSetInfo[i]
                &= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability
                        ->_localAnticipatability.getDownwardExposedAnalysisInfo(i);
            ...
            *_optSetHelper -= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
                                   ->_globalAnticipatability->_localTransparency.getAnalysisInfo(i);
            ...
            // *_optSetHelper -=
            // *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability->_localAnticipatability.getDownwardExposedAnalysisInfo(i);
...
bool TR_RedundantExpressionAdjustment::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    ...
    if (_exceptionCheckMotion->getTrySecondBestSolution()) {
        ...
        int32_t totalMaxSize
            = optimisticOptSetInfo[blockNum]->elementCount() + optimisticRednSetInfo[blockNum]->elementCount();
        ...
        for (listElem = genSetInfo[blockNum]->getListHead(); listElem != NULL; listElem = listElem->getNextElement()) {
            ...
            */
            ...
            if ((prevElem && (prevElem->getNextElement() == listElem))
                || (genSetInfo[blockNum]->getListHead() == listElem))
                prevElem = listElem;
        }
    ...
    if ((block->getSuccessors().size() == 2) && block->getExit()) {
        ...
        if (((branch->getOpCodeValue() == TR::ifacmpne) || (branch->getOpCodeValue() == TR::ifacmpeq))
