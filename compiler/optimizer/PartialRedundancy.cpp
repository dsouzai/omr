...
//
TR::Node *TR_PartialRedundancy::duplicateExact(TR::Node *node, List<TR::Node> *seenNodes,
    List<TR::Node> *duplicateNodes, vcount_t visitCount)
{
...
void TR_PartialRedundancy::calculateGlobalRegisterWeightsBasedOnStructure(TR_Structure *structure,
    ...
    for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement]; seenOptimalElements < numOptimalElements;
                            ...
                            if (nextNode->getOpCode().isNullCheck()) {
                                if (nextNode->getFirstChild()->exceptionsRaised() != 0) {
                                    ...
                                    if (!shouldAllowLimitingOfOptimalPlacement(comp())
                                        || performTransformation(comp(), "%sPlacing NULLCHK optimally : %p\n",
                        ...
                        if (!shouldAllowLimitingOfOptimalPlacement(comp())
                            ...
                            if (comp()->useCompressedPointers()) {
                                if (convertedOptimalNode->getOpCode().isLoadIndirect()
                                    ...
                                    translateTT = TR::TreeTop::create(comp(),
                                        TR::Node::createCompressedRefsAnchor(convertedOptimalNode), NULL, NULL);
                                }
                            }
        ...
        if (nextOptimalNode->getOpCode().isStore()) {
            ...
            if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK)
                ...
                TR::Node *duplicateOptimalNode = duplicateExact(nextOptimalNode->getNullCheckReference(), &seenNodes,
                    &duplicateNodes, visitCount1);
                ...
                if (!shouldAllowLimitingOfOptimalPlacement(comp())
                    || performTransformation(comp(), "%sPlacing NULLCHK optimally : %p\n", OPT_DETAILS,
                        duplicateOptimalNode)) {
                    TR::TreeTop *translateTT = NULL;
                    if (comp()->useCompressedPointers()) {
                        TR::Node *reference = passThroughNode->getFirstChild();
                        if (reference->getOpCode().isLoadIndirect() && reference->getDataType() == TR::Address
                            && TR::TransformUtil::fieldShouldBeCompressed(reference, comp())) {
                            translateTT = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(reference),
                                NULL, NULL);
                        }
                    }
                    manager()->setAlteredCode(true);
            ...
            if (comp()->useCompressedPointers()) {
                ...
                if (reference->getOpCode().isLoadIndirect() || reference->getOpCode().isArrayLength()) {
                    if (reference->getFirstChild()->getOpCode().isLoadIndirect()
                        ...
                        translateTT = TR::TreeTop::create(comp(),
                            TR::Node::createCompressedRefsAnchor(reference->getFirstChild()), NULL, NULL);
                        placeToInsertOptimalComputations->join(translateTT);
                        translateTT->join(secondTreeInBlock);
                        placeToInsertOptimalComputations = translateTT;
                    }
...
// not found
//
TR::Node *TR_PartialRedundancy::getAlreadyPresentOptimalNode(TR::Node *node, int32_t nextOptimalComputation,
    vcount_t visitCount, bool &rhsOfStore)
{
...
//
void TR_PartialRedundancy::eliminateRedundantComputations(TR::Block *block, TR::Node **supportedNodesAsArray,
    ContainerType **rednSetInfo, TR::TreeTop *startTree)
{
            ...
            if ((firstOpCodeInTree.isStore()
		      ...
		      performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()) */)
                ...
                if ((currentTree->getNode()->getNumChildren() > 0)
                    && isExpressionRedundant(currentTree->getNode()->getFirstChild(), redundantComputations,
                        anticipatabilityInfo)
                    && currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
		  ...
		  performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()->getFirstChild()) */)
                 {
                        ...
                        if (currentTree->getNode()->getFirstChild()->getOpCode().isStoreIndirect()) {
                            ...
                            storeForCommonedNode = TR::Node::createWithSymRef(opCodeValue, 1, 1, convertedOptimalNode,
                                newSymbolReference);
                        } else
                            storeForCommonedNode = TR::Node::createWithSymRef(opCodeValue, 1, 1,
...
// in that range)
//
void TR_PartialRedundancy::collectAllNodesToBeAnchored(List<TR::Node> *anchoredNodes, TR::Node *node,
    vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return;
                    ...
                    if (node->getDataType() == TR::NoType) {
                        ...
                        if (parent && parent->getOpCode().isConversion()
                            && (parent->getOpCodeValue()
                                == TR::ILOpCode::getProperConversion(parent->getDataType(), node->getDataType(),
                                    false /*!wantZeroExtension*/))) {
                            if (node->getReferenceCount() > 1) {
                                TR::TreeTop *anchorTreeTop
                            ...
                            processReusedNode(parent, comp()->il.opCodeForDirectLoad(nodeDataType), newSymbolReference,
                                0);
                        } else {
                            for (j = 0; j < node->getNumChildren(); j++)
                                node->getChild(j)->recursivelyDecReferenceCount();

                            TR::Node *newLoad = TR::Node::createWithSymRef(node,
                                comp()->il.opCodeForDirectLoad(node->getDataType()), 0, newSymbolReference);

                            TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(newLoad->getDataType(),
                            ...
                            if (TR::ILOpCode::isVectorOpCode(conversionOpCode)
                                ...
                                node = TR::Node::create(TR::ILOpCode::createVectorOpCode(TR::vconv,
                                                            newLoad->getDataType(), node->getDataType()),
                                    1, newLoad);
                            } else {
    ...
    if (isSupportedOpCode(node, parent)
        ...
        if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0))
            ...
            if (shouldAllowOptimalSubNodeReplacement(comp())
                ...
                TR::Node *newLoad = TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(nodeDataType), 0,
                    newSymbolReference);
        ...
        for (i = 0; i < node->getNumChildren(); i++) {
            TR::Node *child = node->getChild(i);
            TR::Node *duplicateChild = duplicateOptimalNode->getChild(i);
            curTree = replaceOptimalSubNodes(curTree, node, child, i, duplicateOptimalNode, duplicateChild, isNullCheck,
                blockNum, visitCount);
        }
    }
...
//
TR_ExceptionCheckMotion::TR_ExceptionCheckMotion(TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_PartialRedundancy *partialRedundancy)
...
void TR_ExceptionCheckMotion::composeLists(List<TR::Node> *targetList, List<TR::Node> *sourceList,
    ContainerType *availableExprs)
{
    _composeHelper->empty();
...
void TR_ExceptionCheckMotion::analyzeNodeToInitializeGenAndKillSets(TR::TreeTop *tt, vcount_t visitCount,
...
void TR_ExceptionCheckMotion::removeFromList(ListElement<TR::Node> *listElem, List<TR::Node> *list,
    ListElement<TR::Node> *prevElem)
{
    if (prevElem)
