...
bool TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(TR::Node* node, TR::Compilation* comp, TR::Node* parent)
{
    ...
    bool isSupportedStoreNode = node->getOpCode().isStore() && node->getOpCodeValue() != TR::astorei;

    return isSupportedNode(node, comp, parent, isSupportedStoreNode);
}

bool TR_LocalAnalysis::isSupportedNode(
    TR::Node* node, TR::Compilation* comp, TR::Node* parent, bool isSupportedStoreNode)
{
...
bool TR_LocalAnalysis::isSupportedNodeForPREPerformance(TR::Node* node, TR::Compilation* comp, TR::Node* parent)
{
    ...
    if (node->getOpCode().isStore() && symRef && symRef->getSymbol()->isAutoOrParm()) {
...
bool TR_LocalAnalysis::isSupportedNodeForFunctionality(
    TR::Node* node, TR::Compilation* comp, TR::Node* parent, bool isSupportedStoreNode)
{
#if 0
   ...
   if (node->getOpCode().isLoadConst() &&
    ...
    if (comp->requiresSpineChecks() && node->getOpCode().hasSymbolReference()
    ...
    if (node->getOpCode().hasSymbolReference()
        && (node->getSymbolReference()->isSideEffectInfo() || node->getSymbolReference()->isOverriddenBitAddress()
   ...
   comp()->resetVisitCounts(0);
    ...
    while (currentTree) {
        ...
        if ((opCode->isStore() && !firstNodeInTree->getSymbolReference()->getSymbol()->isAutoOrParm())
            ...
            if (opCode->isCheck() && firstNodeInTree->getFirstChild()->getOpCode().isStore()
                && !firstNodeInTree->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm()
                && !_supportedNodesAsArray[firstNodeInTree->getFirstChild()->getLocalIndex()])
                _supportedNodesAsArray[firstNodeInTree->getFirstChild()->getLocalIndex()]
                    = firstNodeInTree->getFirstChild();
        }
...
{
    _info = (TR_LocalAnalysisInfo::LAInfo*)trMemory()->allocateStackMemory(
        _lainfo._numBlocks * sizeof(TR_LocalAnalysisInfo::LAInfo));
...
int TR_LocalAnalysisInfo::hasOldExpressionOnRhs(TR::Node* node, bool recalcContainsCall, bool storeLhsContainsCall)
{
    ...
    if (nodeContainsCall) {
        ...
        //
        if (!node->getOpCode().isStore() || storeLhsContainsCall)
            return 0;
    }
    ...
    /////if (node->getOpCode().isNullCheck())
    {
        ...
        for (k = 0; k < _numNullChecks; k++) {
            if (!(_nullCheckNodesAsArray[k] == NULL)) {
                if (areSyntacticallyEquivalent(
                        _nullCheckNodesAsArray[k]->getNullCheckReference(), node->getNullCheckReference()))
        ...
        // same global symbol.
        //
        if (node->getOpCode().isStore() && !node->getSymbolReference()->getSymbol()->isAutoOrParm()) {
        ...
        for (other = cursor.firstNode(); other; other = cursor.nextNode()) {
            ...
            if (other->getOpCode().isStore() && !other->getSymbolReference()->getSymbol()->isAutoOrParm()) {
            ...
            if (areSame) {
                if (seenWriteBarrier) {
                    ...
                    TR::Node::recreate(
                        node, _compilation->il.opCodeForCorrespondingLoadOrStore(node->getOpCodeValue()));
                }
...
bool TR_LocalAnalysisInfo::isCallLike(TR::Node* node)
{
    ...
    if ((opCode.isCall() && !node->isPureCall()) || opCodeValue == TR::New || opCodeValue == TR::newarray
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (node->getSymbolReference()->isSideEffectInfo() || node->getSymbolReference()->isOverriddenBitAddress())
            return true;
        ...
        // Note that this change is only needed because javaLangClassFromClass loads are being skipped for performance
        // (see earlier code in this file) and we want to avoid strange side effects of a parent being commoned but a
        // child is not a candidate for commoning.
        //
        if (node->getOpCode().hasSymbolReference()
            && (node->getSymbolReference() == comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef())) {
            return true;
    ...
    for (i = 0; i < node1->getNumChildren(); i++) {
        if (node1->getChild(i)->getLocalIndex() != node2->getChild(i)->getLocalIndex())
            ...
            if (node1->getChild(i)->getLocalIndex() == MAX_SCOUNT) {
                if ((node1->getChild(i)->getOpCode().isLoadConst() || node1->getChild(i)->getOpCode().isLoadVarDirect())
                    && (node2->getChild(i)->getOpCode().isLoadConst()
                        || node2->getChild(i)->getOpCode().isLoadVarDirect())) {
                    if (!_optimizer->areNodesEquivalent(node1->getChild(i), node2->getChild(i)))
                        return false;
                } else if ((node1->getChild(i)->getOpCode().isArrayRef())
                    && (node2->getChild(i)->getOpCode().isArrayRef())) {
                    for (int32_t j = 0; j < node1->getChild(i)->getNumChildren(); j++) {
                        ...
                        if (grandChild1->getLocalIndex() != grandChild2->getLocalIndex())
                            ...
                            if (grandChild1->getLocalIndex() == MAX_SCOUNT) {
                                if ((grandChild1->getOpCode().isLoadConst()
                                        || grandChild1->getOpCode().isLoadVarDirect())
                                    && (grandChild2->getOpCode().isLoadConst()
                                        || grandChild2->getOpCode().isLoadVarDirect())) {
...
bool TR_LocalAnalysisInfo::collectSupportedNodes(TR::Node* node, TR::Node* parent)
{
    ...
    if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
        ...
        if (opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore())) {
            ...
            if (node->getFirstChild()->isThisPointer() && node->getFirstChild()->isNonNull()) {
                ...
                int32_t otherLen;
                const char* otherSig = otherSymRef->getOwningMethod(_compilation)
                                           ->classNameOfFieldOrStatic(otherSymRef->getCPIndex(), otherLen);
                if (otherSig) {
                    otherSig = TR::Compiler->cls.classNameToSignature(otherSig, otherLen, _compilation);
                    otherClassObject = _compilation->fe()->getClassFromSignature(
                        otherSig, otherLen, otherSymRef->getOwningMethod(_compilation));
                }

                if (!cl || !otherClassObject || (cl != otherClassObject))
                    indirectionSafe = false;
            }
        }

        if (childRelevant || (!indirectionSafe || (opCode.isArrayLength())) || (node->getOpCode().isArrayRef())
            || (opCode.hasSymbolReference()
                && (node->getSymbolReference()->isUnresolved() || node->getSymbol()->isArrayShadowSymbol()))
