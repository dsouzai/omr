...
bool TR_LocalAnalysis::isSupportedNode(TR::Node *node, TR::Compilation *comp, TR::Node *parent,
...
bool TR_LocalAnalysis::isSupportedNodeForFunctionality(TR::Node *node, TR::Compilation *comp, TR::Node *parent,
    bool isSupportedStoreNode)
{
#if 0
    ...
    /////if (node->getOpCode().isNullCheck())
    {
        ...
        for (k = 0; k < _numNullChecks; k++) {
            if (!(_nullCheckNodesAsArray[k] == NULL)) {
                if (areSyntacticallyEquivalent(_nullCheckNodesAsArray[k]->getNullCheckReference(),
            ...
            if (areSame) {
                if (seenWriteBarrier) {
                    ...
                    TR::Node::recreate(node,
                        _compilation->il.opCodeForCorrespondingLoadOrStore(node->getOpCodeValue()));
                }
        ...
        if (opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore())) {
            ...
            if (node->getFirstChild()->isThisPointer() && node->getFirstChild()->isNonNull()) {
                ...
                if (otherSig) {
                    ...
                    otherClassObject = _compilation->fe()->getClassFromSignature(otherSig, otherLen,
                        otherSymRef->getOwningMethod(_compilation));
                }
