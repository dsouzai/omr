    ...
    : TR_LocalAnalysis(info, t)
{
     ...
     */
    {
        ...
        ContainerType** definedSymbolReferencesInBlock
            = (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes * sizeof(ContainerType*));
        memset(definedSymbolReferencesInBlock, 0, numberOfNodes * sizeof(ContainerType*));
        ContainerType** storedSymRefsInBlock
            = (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes * sizeof(ContainerType*));
        memset(storedSymRefsInBlock, 0, numberOfNodes * sizeof(ContainerType*));
        ContainerType** symRefsDefinedAfterStoredInBlock
            = (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes * sizeof(ContainerType*));
        memset(symRefsDefinedAfterStoredInBlock, 0, numberOfNodes * sizeof(ContainerType*));
        ContainerType** symRefsUsedAfterDefinedInBlock
            = (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes * sizeof(ContainerType*));
        ...
        while (currentTree) {
            ...
            updateUsesAndDefs(currentTree->getNode(), globalDefinedSymbolReferences,
                definedSymbolReferencesInBlock[blockNumber], storedSymRefsInBlock[blockNumber],
                symRefsDefinedAfterStoredInBlock[blockNumber], symRefsUsedAfterDefinedInBlock[blockNumber],
                startVisitCount, tempContainer, temp, allStoredSymRefsInMethod);
        ...
        *_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
        ...
        while (currentTree) {
            ...
            updateInfoForSupportedNodes(currentNode, allSymbolReferences, allSymbolReferencesInNullCheckReference,
                allSymbolReferencesInStore, globalDefinedSymbolReferences, storedSymRefsInBlock[blockNumber],
                allStoredSymRefsInMethod, visitCount3);
            ...
            if (_isStoreTree) {
                ...
                //
                if (!_registersScarce && storeTreeParticipatesInAnalysis
                ...
                for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
                    ...
                    //
                    if (!allSymbolReferencesInStore->isEmpty() && storeTreeParticipatesInAnalysis) {
                        if (allSymbolReferencesInStore->get(newDefinedSymbolReference)) {
                            ...
                            if (trace())
                                traceMsg(comp(), "75757Expression %d killed (n%dn) by symRef #%d  %d\n",
                                    storeNode->getLocalIndex(), storeNode->getGlobalIndex(), newDefinedSymbolReference,
                                    true);
                        }
                    }
                            ...
                            if (allSymbolReferencesInNullCheckReference->get(newDefinedSymbolReference)) {
                                if (trace())
                                    traceMsg(comp(), "76767Expression %d killed (n%dn) by symRef #%d  %d\n",
                                        currentNode->getLocalIndex(), currentNode->getGlobalIndex(),
                                        newDefinedSymbolReference, true);
                    ...
                    if (symbolReferencesInCheck->get(newDefinedSymbolReference)) {
                        if (trace())
                            traceMsg(comp(), "777Expression %d killed (n%dn) by symRef #%d  %d\n",
                                currentNode->getLocalIndex(), currentNode->getGlobalIndex(), newDefinedSymbolReference,
                                true);
                ...
                if (_hasTransparencyInfoFor->get(nextDefinedSymbolReference)) {
                    ...
                    if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(nextDefinedSymbolReference)
                        || (!symRefsDefinedAfterStoredInBlock[nextBlock->getNumber()]->get(nextDefinedSymbolReference)
                            && !symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]->get(
                                nextDefinedSymbolReference))) {
                        ...
                        for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
                            ...
                            if (_info[nextBlock->getNumber()]._analysisInfo->get(storeNode)) {
                                int32_t storeSymRefNum
                                    = supportedNodesAsArray[storeNode]->getSymbolReference()->getReferenceNumber();
                                ...
                                else if (supportedNodesAsArray[storeNode]->mayKill(true).contains(
                                             nextDefinedSymbolReference, comp())) {
                                    {
                                        if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(storeSymRefNum)
                                            || (!symRefsDefinedAfterStoredInBlock[nextBlock->getNumber()]->get(
                                                    storeSymRefNum)
                                                && !symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]->get(
                                                    storeSymRefNum)))
                                            symRefCanSurvive = true;
                                ...
                                if (symRefCanSurvive && !childKilled) {
                                    ...
                                    if (survivingStoreNodes->isEmpty())
                                        ...
                                        if (survivingStoreNodes->elementCount() > 1
                                            || !survivingStoreNodes->get(storeNode)) {
                                            survivingStoreNodes->empty();
                                            break;
                                        }
                                    }
...
TR_LocalAnalysisInfo::ContainerType* TR_LocalTransparency::getTransparencyInfo(int32_t symRefNum)
{
    return _transparencyInfo[symRefNum];
}

void TR_LocalTransparency::updateUsesAndDefs(TR::Node* node, ContainerType* globallySeenDefinedSymbolReferences,
    ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs,
    ContainerType* symRefsDefinedAfterStored, ContainerType* symRefsUsedAfterDefined, vcount_t visitCount,
    ContainerType* tempContainer, TR_BitVector* temp, ContainerType* allStoredSymRefsInMethod)
{
    ...
    for (i = 0; i < node->getNumChildren(); i++)
        updateUsesAndDefs(node->getChild(i), globallySeenDefinedSymbolReferences, seenDefinedSymbolReferences,
            seenStoredSymRefs, symRefsDefinedAfterStored, symRefsUsedAfterDefined, visitCount, tempContainer, temp,
            allStoredSymRefsInMethod);

    TR::ILOpCode& opCode = node->getOpCode();
    if (opCode.hasSymbolReference() && (loadaddrAsLoad() || opCode.getOpCodeValue() != TR::loadaddr)) {
        ...
        if ((opCode.isLoadVar() || (loadaddrAsLoad() && opCode.getOpCodeValue() == TR::loadaddr))
            ...
            if (!opCode.isCheck() && !opCode.isStore()) {
                ...
                if (!tempContainer->isEmpty()) {
                    ...
                    *tempContainer -= *getCheckSymbolReferences();
                    ...
                    bool pureFunction = node->getOpCode().isCall() && symReference
            ...
            if (opCode.isStore()) {
                ...
                //
                if (_registersScarce || node->getSymbolReference()->getSymbol()->isAutoOrParm()
                    || symReference->sharesSymbol() || seenStoredSymRefs->get(symRefNum)) {
...
void TR_LocalTransparency::updateInfoForSupportedNodes(TR::Node* node, ContainerType* allSymbolReferences,
    ContainerType* allSymbolReferencesInNullCheckReference, ContainerType* allSymbolReferencesInStore,
    ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs,
    ContainerType* allStoredSymRefsInMethod, vcount_t visitCount)
{
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        updateInfoForSupportedNodes(child, allSymbolReferences, allSymbolReferencesInNullCheckReference,
            allSymbolReferencesInStore, seenDefinedSymbolReferences, seenStoredSymRefs, allStoredSymRefsInMethod,
            visitCount);
                ...
                if (!_supportedNodes->get(child->getLocalIndex()))
                    ...
                    for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
                        if (!_transparencyInfo[i]->get(child->getLocalIndex())
                            || (child->getOpCode().hasSymbolReference() && !child->getOpCode().isCall()
                            ...
                            if (trace())
                                traceMsg(comp(),
                                    "Expression %d (n%dn) killed by symRef #%d because child %d (n%dn) is already "
                                    "killed by the symRef\n",
                                    node->getLocalIndex(), node->getGlobalIndex(), i, child->getLocalIndex(),
                            ...
                            for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                                ...
                                if (trace())
                                    traceMsg(comp(), "9999Expression %d (n%dn) killed by symRef #%d  %d\n",
                                        node->getLocalIndex(), node->getGlobalIndex(), nextAlias,
                                        seenDefinedSymbolReferences->get(nextAlias));
                            }
            ...
            if (seenDefinedSymbolReferences->get(symRef->getReferenceNumber())
                ...
                if (trace())
                    traceMsg(comp(), "Expression %d (n%dn) killed by symRef #%d  %d\n", node->getLocalIndex(),
                        node->getGlobalIndex(), symRef->getReferenceNumber(),
                        seenDefinedSymbolReferences->get(symRef->getReferenceNumber()));
                ...
                for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "888Expression %d (n%dn) killed by symRef #%d  %d\n", node->getLocalIndex(),
                            node->getGlobalIndex(), nextAlias, seenDefinedSymbolReferences->get(nextAlias));
                }
            }
        }
    }
...
void TR_LocalTransparency::adjustInfoForAddressAdd(
    TR::Node* node, TR::Node* child, ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs)
{
    bool childHasSupportedOpCode = false;
                ...
                if (!_transparencyInfo[i]->get(child->getLocalIndex())) {
                    ...
                    if (trace()) {
                        if (comp()->target().is64Bit())
                            traceMsg(comp(),
                                "Expression %d killed by symRef #%d because grandchild (child of aladd) %d is already "
                                "killed by the symRef\n",
                                node->getLocalIndex(), i, child->getLocalIndex());
                        else
                            traceMsg(comp(),
                                "Expression %d killed by symRef #%d because grandchild (child of aiadd) %d is already "
                                "killed by the symRef\n",
                                node->getLocalIndex(), i, child->getLocalIndex());
                    }
                }
