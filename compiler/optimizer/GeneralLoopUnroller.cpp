...
static TR_ScratchList<TR::CFGEdge>* findCorrespondingCFGEdges(
    TR_Structure* from, TR_Structure* to, TR::Compilation* comp)
{
    if (from->asBlock()) {
            ...
            } else if (succ == loop->getEntry()) // Type C
            {
                ...
                if (finalUnroll) {
                    if (_unrollKind != CompleteUnroll)
                        ...
                        addEdgeAndFixEverything(
                            loop, *edge, clonedSubNode, loop->getEntry(), false, false, true, BackEdgeToEntry);
                    else {
...
void TR_LoopUnroller::modifyBranchTree(
    TR_RegionStructure* loop, TR_StructureSubGraphNode* loopNode, TR_StructureSubGraphNode* branchNode)
{
    ...
    if (pBlock->getEntry() == 0) {
        TR::Block* newBlock = TR::Block::createEmptyBlock(
            branch, comp(), MAX_COLD_BLOCK_COUNT + 1, pBlock); // branchBlock->getFrequency());
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
        ...
        if (getIndexType().isAddress() || getTestChildType().isAddress()) {
            ...
            TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(
                newLimitNode->getDataType(), getTestChildType().getDataType(), true);
...
void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure* loop, TR_StructureSubGraphNode* branchNode)
{
        ...
        if (!loop->isExitEdge(current)) {
            ...
            if (predNode->getNumber() < _numNodes) {
                if (shouldConnectToNextIteration(predNode, loop)) {
                    addEdgeAndFixEverything(
                        loop, current, predNode, _firstEntryNode, true, true, true, BackEdgeFromPrevGeneration);
                    ...
                    if (trace())
                        traceMsg(comp(),
                            ...
                            predNode->getNumber());
                    addEdgeAndFixEverything(
                        loop, current, predNode, loop->getEntry(), true, false, true, BackEdgeToEntry);
                }
            }
        }
    }
    ...
    if (_spillLoopRequired) {
        ...
        if (1) {
            ...
            TR::Block* newIfBlock = TR::Block::createEmptyBlock(
                branchTreeTop->getNode(), comp(), origExitBlock->getFrequency(), origExitBlock);
            ...
            if (1) {
                ...
                if (1) {
                    ...
                    if (getIndexType().isAddress() || getTestChildType().isAddress()) {
                        ...
                        TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(
                            c1->getDataType(), getTestChildType().getDataType(), true);
                        TR_ASSERT(o2xOp != TR::BadILOp, "could not find conv op for %s -> %s conversion\n",
                ...
                if (spillLoop->contains(branchDestination->getStructureOf(), parent)) {
                    TR::Node::recreate(
                        newIfTree->getNode(), newIfTree->getNode()->getOpCode().getOpCodeForReverseBranch());
...
TR::Node* TR_LoopUnroller::createIfNodeForSpillLoop(TR::Node* ifNode)
{
    ...
    return TR::Node::createif(
        ifNode->getOpCodeValue(), c1, ifNode->getSecondChild()->duplicateTree(), ifNode->getBranchDestination());
}
...
void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure* loop, TR_StructureSubGraphNode* branchNode)
{
    ...
    if (trace()) {
        traceMsg(
            comp(), "trees after creating the spill loop %d for loop %d:\n", spillNode->getNumber(), loop->getNumber());
            ...
            if (newFromNextBlock != newTo) // but the copy does not fall through
            {
                ...
                if (edgeContext == BackEdgeFromLastGenerationCompleteUnroll) {
                    if (newFrom->getLastRealTreeTop()->getNode()->getOpCode().isBranch())
                        newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                            comp(), oldDest, gotoBlock->getEntry());
                }
...
TR_Structure* TR_LoopUnroller::cloneRegionStructure(TR_RegionStructure* region)
{
    ...
    for (subNode = si.getFirst(); subNode; subNode = si.getNext()) {
        ...
        TR_ASSERT(
            subNode->getExceptionSuccessors().empty(), "should not reach here - we dont unroll loops with exceptions");
    }
    ...
    UnrollKind unrollKind, int32_t unrollCount, int32_t peelCount, TR::Optimizer* optimizer)
{
    ...
    if (1) {
        ...
        if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
            if (piv->getDeltaOnExitEdge() != 1 && piv->getDeltaOnExitEdge() != -1) {
                dumpOptDetails(
                    comp, "Cannot unroll loop %d: eq loop with non unit inc/dec not supported\n", loop->getNumber());
                return false;
            }
    ...
    if (unrollKind == CompleteUnroll) {
        ...
        if (!loop->contains(destStruct, loop->getParent())) {
            dumpOptDetails(
                comp, "Cannot unroll loop %d: complete unroll of a loop with a reversed branch\n", loop->getNumber());
            return false;
        }
    }
    ...
    if (unroller.isInternalPointerLimitExceeded()) {
        dumpOptDetails(
            comp, "Cannot unroll loop %d: number of internal pointers has been exceeded\n", loop->getNumber());
        return false;
    }
...
bool TR_LoopUnroller::unroll(
    TR::Compilation* comp, TR_RegionStructure* loop, int32_t unrollCount, int32_t peelCount, TR::Optimizer* optimizer)
{
    // We dont do peeling as of yet
    //
    if (peelCount != 0) {
    ...
    if (!branchStr) {
        // if (comp->trace(generalLoopUnroller))
        dumpOptDetails(
            comp, "Cannot unroll loop %d: branchnode %d is not a block\n", loop->getNumber(), branchNode->getNumber());
        return false;
    }

    TR_LoopUnroller unroller(
        comp, optimizer, loop, branchNode, unrollCount, peelCount, loopInvariantBlock, GeneralUnroll);
...
int32_t TR_GeneralLoopUnroller::perform()
{
    ...
    // if (trace())
    dumpOptDetails(
        comp(), "Starting GLU with a budget of %d.  Total number of nodes in method %d\n", budget, nodeCount);
    ...
    while (1) {
        ...
        if (top->_cost > budget) {
            dumpOptDetails(
                comp(), "Loop %d unroll cost %d > %d remaining budget\n", top->_loop->getNumber(), top->_cost, budget);
...
void TR_GeneralLoopUnroller::countNodesAndSubscripts(
    TR::Node* node, int32_t& numNodes, int32_t& numSubscripts, LoopWeightProbe& lwp)
{
    if (!node)
        return;
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(List<TR_RegionStructure>& innerLoops)
{
    ...
    for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
        ...
        entryBlock->prepend(TR::TreeTop::create(comp(),
            TR::Node::create(
                TR::lstore, 1, TR::Node::create(TR::ladd, 2, TR::Node::createLoad(firstNode, counterSymRef), one))));
        ...
        postBlock->prepend(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(
                TR::lstore, 1, 1, TR::Node::createLoad(firstNode, counterSymRef), counterSymRef)));
    ...
    int32_t& unrollCount, int32_t& peelCount, int32_t& cost)
{
    ...
    if (piv) {
        ...
        if (unrollKind != TR_LoopUnroller::CompleteUnroll && growth > maxSizeThreshold) {
            dumpOptDetails(
                comp(), "\tloop unroll size threshold hit: %d using an unroll count of %d\n", growth, unrollCount);
            return -1;
        }
...
//
bool TR_GeneralLoopUnroller::branchContainsInductionVariable(
    TR_RegionStructure* loop, TR::Node* branchNode, bool checkOnlyPiv)
{
    /// ListIterator<TR_BasicInductionVariable> it(&loop->getBasicInductionVariables());
    bool result = false;
