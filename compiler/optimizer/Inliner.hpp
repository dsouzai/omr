...
public:
    ...
    template<typename ObjWithSignatureMethod> const char* traceSignature(ObjWithSignatureMethod* obj)
    {
...
struct TR_VirtualGuardSelection {
    ...
    TR_VirtualGuardSelection(TR_VirtualGuardKind kind, TR_VirtualGuardTestType type = TR_NonoverriddenTest,
    ...
    bool isHighProbablityProfiledGuard() { return _highProbabilityProfiledGuard; }
};
...
public:
    ...
    OMR_InlinerPolicy* getPolicy()
    {
        TR_ASSERT(_policy != NULL, "inliner policy must be set");
        return _policy;
    }
    OMR_InlinerUtil* getUtil()
    {
        TR_ASSERT(_util != NULL, "inliner util must be set");
        return _util;
    }
...
protected:
    ...
    bool inlineCallTarget(TR_CallStack*, TR_CallTarget* calltarget, bool inlinefromgraph, TR_PrexArgInfo* argInfo = 0,
        TR::TreeTop** cursorTreeTop = NULL);
    bool inlineCallTarget2(
        TR_CallStack*, TR_CallTarget* calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t);

    // inlineCallTarget2 methods
    bool tryToInlineTrivialMethod(TR_CallStack* callStack, TR_CallTarget* calltarget);

    bool tryToGenerateILForMethod(
        TR::ResolvedMethodSymbol* calleeSymbol, TR::ResolvedMethodSymbol* callerSymbol, TR_CallTarget* calltarget);

    void inlineFromGraph(TR_CallStack*, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo*);
    TR_CallSite* findAndUpdateCallSiteInGraph(TR_CallStack* callStack, TR_ByteCodeInfo& bcInfo, TR::TreeTop* tt,
        TR::Node* parent, TR::Node* callNode, TR_CallTarget* calltarget);
    ...
    void getBorderFrequencies(int32_t& hotBorderFrequency, int32_t& coldBorderFrequency,
        TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode);
    int32_t scaleSizeBasedOnBlockFrequency(int32_t bytecodeSize, int32_t frequency, int32_t borderFrequency,
        TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode,
        int32_t coldBorderFrequency = 0); // not virtual because we want base class always calling base version of this
    ...
    void addAdditionalGuard(TR::Node* callNode, TR::ResolvedMethodSymbol* calleeSymbol, TR_OpaqueClassBlock* thisClass,
        TR::Block* prevBlock, TR::Block* inlinedBody, TR::Block* slowPath, TR_VirtualGuardKind kind,
        TR_VirtualGuardTestType type, bool favourVftCompare, TR::CFG* callerCFG);
    void rematerializeCallArguments(TR_TransformInlinedFunction& tif, TR_VirtualGuardSelection* guard,
    ...
    TR::Node* createVirtualGuard(TR::Node*, TR::ResolvedMethodSymbol*, TR::TreeTop*, int16_t, TR_OpaqueClassBlock*,
        bool, TR_VirtualGuardSelection*);

private:
    void replaceCallNodeReferences(
        TR::Node*, TR::Node*, uint32_t, TR::Node*, TR::Node*, rcount_t&, TR::NodeChecklist& visitedNodes);
...
public:
    ...
    virtual bool inlineRecognizedMethod(TR::RecognizedMethod method);
    bool inlineCall(TR::TreeTop* callNodeTreeTop, TR_OpaqueClassBlock* thisType = 0, bool recursiveInlining = false,
        TR_PrexArgInfo* argInfo = 0, int32_t initialMaxSize = 0);
};
...
public:
    ...
    TR_ParameterToArgumentMapper(TR::ResolvedMethodSymbol*, TR::ResolvedMethodSymbol*, TR::Node*, TR_PrexArgInfo*,
        List<TR::SymbolReference>&, List<TR::SymbolReference>&, List<TR::SymbolReference>&, TR_InlinerBase*);
...
public:
    ...
    static TR::TreeTop* storeValueInATemp(TR::Compilation* comp, TR::Node*, TR::SymbolReference*&, TR::TreeTop*,
        TR::ResolvedMethodSymbol*, List<TR::SymbolReference>& tempList, List<TR::SymbolReference>& availableTemps,
        List<TR::SymbolReference>* moreTemps, bool behavesLikeTemp = true, TR::TreeTop** newStoreValueATreeTop = NULL,
        bool isIndirect = false, int32_t offset = 0);
    ...
    virtual TR_PrexArgInfo* createPrexArgInfoForCallTarget(
    ...
    virtual void adjustByteCodeSize(
        TR_ResolvedMethod* calleeResolvedMethod, bool isInLoop, TR::Block* block, int& bytecodeSize)
    {
        return;
    }
    virtual void adjustCallerWeightLimit(TR::ResolvedMethodSymbol* callSymbol, int& callerWeightLimit) { return; }
    virtual void adjustMethodByteCodeSizeThreshold(
        TR::ResolvedMethodSymbol* callSymbol, int& methodByteCodeSizeThreshold)
    {
        return;
    }
    ...
    virtual void clearArgInfoForNonInvariantArguments(
     ...
     *
     * \note
     *    Targeted inlining is meant to deal with method call chains where the receiver of current call stores the
     * information of the next call, which is usually the receiver or the method of the next call. Knowing the receiver
     * of the first call means we can devirtualize all the calls along the chain.
     *
     *    Each method except the last one in the chain usually contains simple code that manipulates the arguments for
     * the next call. The last method in the chain is usually the most important one, it does the real job and can
     * contain arbitrary code.
     *
     *    The goal of targeted inlining is to inline along the call chain to get the most important method inlined,
     * which is the last one in the chain. It does not care about other callsites.
     */
    virtual bool needTargetedInlining(TR::ResolvedMethodSymbol* callee);

protected:
    virtual void refineColdness(TR::Node* node, bool& isCold);
    virtual void computeMethodBranchProfileInfo(
    ...
    virtual void calleeTreeTopPreMergeActions(
    ...
    { }
    virtual void estimateAndRefineBytecodeSize(
    ...
    virtual TR_TransformInlinedFunction* getTransformInlinedFunction(TR::ResolvedMethodSymbol*,
        ...
        TR_VirtualGuardSelection*, List<TR::SymbolReference>&, List<TR::SymbolReference>&, List<TR::SymbolReference>&);
};
...
public:
    ...
    virtual int32_t getInitialBytecodeSize(
     ...
     */
    virtual bool skipHCRGuardForCallee(TR_ResolvedMethod* callee) { return false; }
    ...
    /**
     * \brief sometimes we choose not to inline certain callsite because they will be handled specially in other
     * optimizations or in codegen
     *
...
protected:
    ...
    virtual TR_InlinerFailureReason checkIfTargetInlineable(
...
class TR_TransformInlinedFunction {
public:
    TR_TransformInlinedFunction(TR::Compilation*, TR_InlinerTracer* tracer, TR::ResolvedMethodSymbol*,
        TR::ResolvedMethodSymbol*, TR::Block*, TR::TreeTop*, TR::Node*, TR_ParameterToArgumentMapper&,
        TR_VirtualGuardSelection*, List<TR::SymbolReference>&, List<TR::SymbolReference>&, List<TR::SymbolReference>&);
...
#define MAX_FIND_SIMPLE_CALL_REFERENCE_DEPTH 12
    ...
    void setMultiRefNodeIsCallRecursionDepthToMax()
    {
...
public:
    TR_HandleInjectedBasicBlock(TR::Compilation*, TR_InlinerTracer*, TR::ResolvedMethodSymbol*,
        List<TR::SymbolReference>&, List<TR::SymbolReference>&, List<TR::SymbolReference>&, TR_ParameterMapping* = 0);
