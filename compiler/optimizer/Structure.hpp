...
public:
    ...
    enum Kind { Blank = 0, Block = 1, Region = 2 };
    ...
    TR_Structure(TR::Compilation* c, int32_t index)
        ...
        , _parent(NULL)
        , _weight(-1)
        , _versionedStructure(NULL)
        , _trMemory(c->trMemory())
        , _maxNestingDepth(0)
        , _nestingDepth(0)
        , _analyzedBefore(false)
        , _containsImproperRegion(false)
    { }
    ...
    virtual TR_Structure* cloneStructure(
        TR::Block**, TR_StructureSubGraphNode**, List<TR_Structure>*, List<TR_Structure>*)
        = 0;
    ...
    int32_t getMaxNestingDepth() { return _maxNestingDepth; }
    ...
    void setMaxNestingDepth(int16_t f)
    {
        if (f > SHRT_MAX - 1) {
            ...
            comp()->failCompilation<TR::CompilationException>(
                "max nesting depth must be less than or equal to SHRT_MAX-1");
        }
        _maxNestingDepth = f;
    }

    virtual void hoistInvariantsOutOfNestedLoops(
        TR_LocalTransparency*, TR_BitVector**, bool, TR_BlockStructure*, TR_RegionStructure*, int32_t)
        = 0;
...
public:
    ...
    TR_StructureSubGraphNode(TR_Structure* s)
        : TR::CFGNode(s->getNumber(), s->cfg()->structureMemoryRegion())
        , _structure(s)
    {
        s->setSubGraphNode(this);
    }
    ...
    void setStructure(TR_Structure* s)
    {
        _structure = s;
        if (s) {
...
public:
    ...
    TR_InductionVariable(TR::RegisterMappedSymbol* s, TR::VPConstraint* entry, TR::VPConstraint* exit,
        TR::VPConstraint* incr, TR_YesNoMaybe isSigned)
        : _local(s)
        , _isSigned(isSigned)
    {
        setEntry(entry);
...
public:
    ...
    TR_RegionStructure(TR::Compilation* c, int32_t index)
        : TR_Structure(c, index)
        , _invariantSymbols(NULL)
        , _blocksAtSameNestingLevel(NULL)
        , _piv(NULL)
        , _basicIVs(c->getFlowGraph()->structureMemoryRegion())
        , _exitEdges(c->getFlowGraph()->structureMemoryRegion())
        , _subNodes(c->getFlowGraph()->structureMemoryRegion())
    ...
    //
    TR::CFGEdge* addExitEdge(
        TR_StructureSubGraphNode* from, int32_t to, bool isExceptionEdge = false, TR::CFGEdge* origEdge = 0);
    ...
    void addAfterInductionVariable(TR_InductionVariable* prev, TR_InductionVariable* v)
    {
        _inductionVariables.insertAfter(prev, v);
    }
    ...
    void setExprInvariant(TR::Node* expr)
    {
        TR_ASSERT(_invariantExpressions != NULL, "assertion failure");
        _invariantExpressions->set(expr->getGlobalIndex());
    }
    void resetExprInvariant(TR::Node* expr)
    {
        TR_ASSERT(_invariantExpressions != NULL, "assertion failure");
        _invariantExpressions->reset(expr->getGlobalIndex());
    }
...
public:
    ...
    class Cursor {
    public:
        Cursor(TR_RegionStructure& region)
            : _nodes(region._subNodes)
            , _iter(_nodes.begin())
        { }
...
private:
    ...
    {
        TR_ASSERT(
            toStructureSubGraphNode(edge->getTo())->getStructure() == NULL, "Region exit edge must not have structure");
        _exitEdges.add(edge);
    }
