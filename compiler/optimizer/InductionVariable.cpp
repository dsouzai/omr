...
int32_t TR_LoopStrider::detectCanonicalizedPredictableLoops(TR_Structure *loopStructure, TR_BitVector **optSetInfo,
    ...
    if (loopInvariantBlock) {
        ...
        if (1) {
            ...
            if (prevInvariantNode && prevInvariantNode->getStructure()->asBlock()
                ...
                _numInternalPointerOrPinningArrayTempsInitialized += countInternalPointerOrPinningArrayStores(comp(),
                    prevInvariantNode->getStructure()->asBlock()->getBlock());
            }
        }
        ...
        if (isPredictableLoop > 0) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (storeInRequiredForm
                    ...
                    if (replaceAllInductionVariableComputations(loopInvariantBlock->getBlock(), loopStructure,
                            &newSymbolReference, inductionVarSymRef)) {
                        ...
                        for (; k < _nextExpression; k++) {
                            ...
                            if (performTransformation(comp(),
                                    ...
                                    loopStructure->getNumber()))
                                newInductionCandidate->addAllBlocksInStructure(loopStructure, comp(),
                                    trace() ? "strider auto" : NULL);
                        ...
                        if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable
                            ...
                            && performTransformation(comp(), "%sReplacing loop test node in %s\n", OPT_DETAILS,
                                comp()->signature())) {
                            changeLoopCondition(loopInvariantBlock, usingAladd, bestCandidate,
                                storeOfDerivedInductionVariable);
                        }
                    } else {
        ...
        for (j = 0; j < symRefCount; j++) {
            ...
            if (reassociatedAuto) {
                ...
                TR::Node *arrayRefNode = TR::TransformUtil::generateArrayElementAddressTrees(comp(), newArrayObjNode,
                    NULL, byteCodeInfoNode);
                    ...
                    if (symRefPair->_isConst) {
                        ...
                        if (usingAladd) {
                            ...
                            if (TR::Compiler->om.isOffHeapAllocationEnabled())
                                arrayRefNode = TR::TransformUtil::generateDataAddrLoadTrees(comp(), arrayRefNode);
#endif /* OMR_GC_SPARSE_HEAP_ALLOCATION */
                            arrayRefNode = TR::Node::create(TR::aladd, 2, arrayRefNode,
    ...
    TR::Node *storeOfDerivedInductionVariable)
{
    ...
    if ((_linearEquations[bestCandidate][4] > -1)
        ...
        TR::Node *newAload = TR::Node::createLoad(secondChildOfLoopTestNode,
            symRefTab->getSymRef((int32_t)_linearEquations[bestCandidate][4]));
        ...
        newStore = TR::Node::createWithSymRef(addNode->getType().isInt64() ? TR::lstore : TR::istore, 1, 1, addNode,
            newSymbolReference);
    }
    ...
    addNode->setLocalIndex(~0);
    TR::Node *secondOperand = TR::Node::createWithSymRef(secondChildOfLoopTestNode,
        comp()->il.opCodeForDirectLoad(newStore->getDataType()), 0, newSymbolReference);
    ...
    int64_t differenceInAdditiveConstants, bool &isInternalPointer, bool &downcastNode, bool &usingAladd)
{
    if ((replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload)
        ...
        if (canHoistAdditiveTerm) {
            TR::SymbolReference *newLoopInvariant = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
                adjustmentNode->getDataType(), false);
    ...
    int32_t childNum, TR::Node *node, vcount_t visitCount, TR::SymbolReference **newSymbolReference)
{
                    ...
                    if (canCreateNewSymRef) {
                        ...
                        *newSymbolReference = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
                        ...
                        if (isInternalPointer && !pinningArrayPointer) {
                            ...
                            createParmAutoPair(comp()->getSymRefTab()->getSymRef(internalPointerSymbol),
                                newPinningArray);
    ...
    if (examineChildren) {
        ...
        for (i = 0; i < node->getNumChildren(); i++) {
            if (examineTreeForInductionVariableUse(loopInvariantBlock, node, i, node->getChild(i), visitCount,
                    newSymbolReference))
                seenInductionVariableComputation = true;
        }
    }
...
int32_t TR_LoopStrider::findNewInductionVariable(TR::Node *node, TR::SymbolReference **symRef, bool hasAdditiveTerm,
    int32_t internalPointerParentSymbol)
{
    // for aladds
    bool usingAladd = (comp()->target().is64Bit()) ? true : false;
...
bool TR_LoopStrider::reassociateAndHoistComputations(TR::Block *loopInvariantBlock, TR::Node *parent, int32_t childNum,
    TR::Node *node, vcount_t visitCount)
{
    ...
    if (false && // TODO enable storing dataAddrPtr in temps
        TR::Compiler->om.isOffHeapAllocationEnabled() && originalNode
...
static void assertSubtreeDoesNotMentionOriginals(TR::Node *node,
    const TR::list<std::pair<int32_t, int32_t> > &widenedIVs, TR::NodeChecklist &visited)
{
    if (visited.contains(node))
        return;
 ...
 * \see enableExpensiveLoopStriderAssertions()
 */
static void assertStructureDoesNotMentionOriginals(TR_Structure *structure,
...
static bool isLoopRelativelyFrequent(TR_Structure *loop, TR::Block *preheader)
{
    ...
    if (loopFreq < minFreq) {
        dumpOptDetails(TR::comp(), "[Sign-Extn] skip loop %d due to frequency %d < %d\n", loop->getNumber(), loopFreq,
            minFreq);
        return false;
    }
...
void TR_LoopStrider::createConstraintsForNewInductionVariable(TR_Structure *loopStructure,
    TR::SymbolReference *newSymRef, TR::SymbolReference *oldSymRef)
{
...
bool TR_LoopStrider::morphExpressionLinearInInductionVariable(TR::Node *parent, int32_t childNum, TR::Node *node,
    vcount_t visitCount)
{
    // for aladds
    bool usingAladd = (comp()->target().is64Bit()) ? true : false;
 ...
 * \see detectLoopsForIndVarConversion(TR_Structure*, TR::NodeChecklist&)
 */
void TR_LoopStrider::walkTreesAndFixUseDefs(TR_Structure *loopStructure, TR::SymbolReference *newSymbolReference,
...
void TR_LoopStrider::eliminateSignExtensionsInSubtree(TR::Node *parent, TR::NodeChecklist &exLoads,
    TR::NodeChecklist &visited, SignExtMemo &signExtMemo)
{
...
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64, TR::Node *node,
    TR::NodeChecklist &exLoads, SignExtMemo &memo)
{
...
void TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure *structure, TR_RegionStructure *loop)
{
    ...
    if (trace()) {
        ...
        while (it.hasMoreElements()) {
            ...
            traceMsg(comp(), "\t%d %d %p symRef=%p symbol=%p: ", refNum, refIndex, inSymbol, symRef,
                symRef->getSymbol());
            if (inSymbol)
...
//
bool TR_InductionVariableAnalysis::branchContainsInductionVariable(TR_RegionStructure *loop, TR::Node *branchNode,
...
bool TR_InductionVariableAnalysis::branchContainsInductionVariable(TR::Node *node, TR::SymbolReference *ivSymRef,
    int32_t *nodeBudget)
{
...
//
bool TR_InductionVariableAnalysis::getProgression(TR::Node *expr, TR::SymbolReference *storeRef,
    TR::SymbolReference **ref, TR_ProgressionKind *prog, int64_t *incr)
{
    int64_t konst;
    TR_ProgressionKind kind;
...
bool TR_InductionVariableAnalysis::isIVUnchangedInLoop(TR_RegionStructure *loop, TR::Block *loopTestBlock,
    TR::Node *candidateIV)
{
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(TR_RegionStructure *loop, TR_BitVector *candidates,
    TR_Array<TR_BasicInductionVariable *> &basicIVs)
{
...
bool TR_InductionVariableAnalysis::findEntryValues(TR_RegionStructure *loop,
    TR_Array<TR_BasicInductionVariable *> &basicIVs)
{
    TR_ArrayIterator<TR_BasicInductionVariable> it(&basicIVs);
    for (TR_BasicInductionVariable *biv = it.getFirst(); biv; biv = it.getNext()) {
        ...
        if (entryValue) {
            if (trace())
                traceMsg(comp(), "\tFound entry value of BIV %d: %p\n", biv->getSymRef()->getReferenceNumber(),
                    entryValue);
