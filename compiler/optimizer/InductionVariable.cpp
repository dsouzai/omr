...
int32_t TR_LoopStrider::perform()
{
    ...
    _hoistedAutos
        = new (stackMemoryRegion) SymRefPairMap((SymRefPairMapComparator()), SymRefPairMapAllocator(stackMemoryRegion));
    _reassociatedAutos
        = new (stackMemoryRegion) SymRefMap((SymRefMapComparator()), SymRefMapAllocator(stackMemoryRegion));
    _count = 0;
...
bool TR_LoopStrider::foundLoad(TR::TreeTop* storeTreeTop, TR::Node* node, int32_t symRefNum, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isLoadVar() && (node->getSymbolReference()->getReferenceNumber() == symRefNum)
    ...
    for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect()
            && ((node->getSymbol()->isAuto()
                ...
                || (node->getSymbol()->isParm() && node->getSymbol()->castToParmSymbol()->isPinningArrayPointer())))
            count++;
    }
    return count;
}

int32_t TR_LoopStrider::detectCanonicalizedPredictableLoops(
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock) {
        ...
        //_storeTreesList = (List<TR_StoreTreeInfo>
        //**)trMemory()->allocateStackMemory(symRefCount*sizeof(List<TR_StoreTreeInfo> *));
        //_loadUsedInNewLoopIncrementList = (List<TR::Node>
        //**)trMemory()->allocateStackMemory(symRefCount*sizeof(List<TR::Node> *)); memset(_storeTreesList, 0,
        // symRefCount*sizeof(List<TR::TreeTop> *)); int32_t i = 0; while (i < symRefCount)
        //   {
        ...
        if (1) {
            _numInternalPointerOrPinningArrayTempsInitialized
                += countInternalPointerOrPinningArrayStores(comp(), loopInvariantBlock->getBlock());
            ...
            if (prevInvariantNode && prevInvariantNode->getStructure()->asBlock()
                && prevInvariantNode->getStructure()->asBlock()->isLoopInvariantBlock()) {
                _numInternalPointerOrPinningArrayTempsInitialized += countInternalPointerOrPinningArrayStores(
                    comp(), prevInvariantNode->getStructure()->asBlock()->getBlock());
            }
        }
        ...
        int32_t isPredictableLoop
            = checkLoopForPredictability(loopStructure, loopInvariantBlock->getBlock(), NULL, false);
        ...
        if (isPredictableLoop > 0) {
            ...
            _loadUsedInNewLoopIncrement
                = (TR::Node**)trMemory()->allocateStackMemory(_numberOfLinearExprs * sizeof(TR::Node*));
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (_loopTestTree->getNode()->getNumChildren() == 2) {
                    ...
                    if (!secondChildOfLoopTestNode->getOpCode().isLoadConst()
                        && !(secondChildOfLoopTestNode->getOpCode().isLoadVar()
                            ...
                            && _neverWritten->get(
                                secondChildOfLoopTestNode->getSymbolReference()->getReferenceNumber()))) {
                        indexSymRef = NULL;
                    }

                    if (indexSymRef && indexSymRef->getReferenceNumber() == _loopDrivingInductionVar) {
                ...
                // if (_cannotBeEliminated->get(nextInductionVariableNumber))
                {
                    ...
                    for (TR::Block* exitBlock = bi.getCurrent(); exitBlock != NULL; exitBlock = bi.getNext()) {
                        ...
                        if (!exitValueNeeded && !seenStore) {
                            TR::Block* next;
                            for (auto edge = exitBlock->getSuccessors().begin();
                                 edge != exitBlock->getSuccessors().end(); ++edge) {
                ...
                if (storeInRequiredForm
                    ...
                    && performTransformation(comp(), "%s Replacing uses of induction var %d in loop %d\n", OPT_DETAILS,
                        nextInductionVariableNumber, loopStructure->getNumber())) {
                    ...
                    if (replaceAllInductionVariableComputations(
                            loopInvariantBlock->getBlock(), loopStructure, &newSymbolReference, inductionVarSymRef)) {
                        ...
                        for (; k < _nextExpression; k++) {
                            ...
                            TR::RegisterCandidate* newInductionCandidate
                                = comp()->getGlobalRegisterCandidates()->findOrCreate(newSymbolReference);
                            //
                            // Add the derived induction variable as a global register candidate
                            // within the loop
                            //
                            if (performTransformation(comp(),
                                    "%s Adding auto %d (created for striding) as a global register "
                                    "candidate in loop %d\n",
                                    OPT_DETAILS, newInductionCandidate->getSymbolReference()->getReferenceNumber(),
                                    loopStructure->getNumber()))
                                newInductionCandidate->addAllBlocksInStructure(
                                    loopStructure, comp(), trace() ? "strider auto" : NULL);

                            // Add the initialization of the derived induction variable before the
                            // loop is entered
                            //
                            TR::Node* placeHolderNode = placeInitializationTreeInLoopInvariantBlock(
                                loopInvariantBlock, inductionVarSymRef, newSymbolReference, k, symRefTab);
                            ...
                            //
                            TR::Node* storeNode
                                = placeNewInductionVariableIncrementTree(loopInvariantBlock, inductionVarSymRef,
                            ...
                            if (mulConst < 0) {
                                if ((-1 * mulConst > highestFactor)
                                    || ((-1 * mulConst == highestFactor) && (_linearEquations[k][4] > -1))) {
                                    bestCandidate = k;
                                ...
                                if ((mulConst > highestFactor)
                                    || ((mulConst == highestFactor) && (_linearEquations[k][4] > -1))) {
                                    bestCandidate = k;
                            ...
                            if (newSymbolReference->getSymbol()->getDataType() != TR::Address) {
                                ...
                                if (v) {
                                    ...
                                    TR_InductionVariable* iv = new (trHeapMemory()) TR_InductionVariable(
                                        newSymbolReference->getSymbol()->castToRegisterMappedSymbol(), newEntryVal,
                                        newExitVal, newIncr,
                        ...
                        if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable
                            ...
                            && performTransformation(
                                comp(), "%sReplacing loop test node in %s\n", OPT_DETAILS, comp()->signature())) {
                            changeLoopCondition(
                                loopInvariantBlock, usingAladd, bestCandidate, storeOfDerivedInductionVariable);
                        }
                    } else {
                        TR::RegisterCandidate* inductionCandidate
                            = comp()->getGlobalRegisterCandidates()->findOrCreate(inductionVarSymRef);
                        if (performTransformation(comp(),
                                "%s Adding auto %d as a global register candidate in loop %d\n", OPT_DETAILS,
                                inductionCandidate->getSymbolReference()->getReferenceNumber(),
                                loopStructure->getNumber()))
        ...
        for (j = 0; j < symRefCount; j++) {
            ...
            TR::SymbolReference* reassociatedAuto = reassociatedAutoSearchResult != _reassociatedAutos->end()
                ? reassociatedAutoSearchResult->second
                : NULL;
            if (reassociatedAuto) {
                ...
                if (usingAladd) {
                    ...
                    arrayRefNode = TR::Node::create(TR::aladd, 2,
                        TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto), constantNode);
                } else
                ...
                if (!origAuto->getSymbol()->isInternalPointer()) {
                    arrayRefNode->setPinningArrayPointer(
                        reassociatedAuto->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
                    reassociatedAuto->getSymbol()
                        ->castToInternalPointerAutoSymbol()
                        ->getPinningArrayPointer()
                        ->setPinningArrayPointer();
                } else
                    arrayRefNode->setPinningArrayPointer(
                        origAuto->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
                ...
                TR::RegisterCandidate* newInductionCandidate
                    = comp()->getGlobalRegisterCandidates()->findOrCreate(reassociatedAuto);
                //
                // Add the reassociated auto as a global register candidate
                // within the loop
                //
                if (performTransformation(comp(),
                        "%s Adding auto %d (created for striding) as a "
                ...
                while (symRefPair) {
                    ...
                    if (symRefPair->_isConst) {
                        ...
                        if (usingAladd) {
                            ...
                            arrayRefNode = TR::Node::create(TR::aiadd, 2,
                                TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto),
                                TR::Node::create(byteCodeInfoNode, TR::iconst, 0, index));
                    } else {
                        TR::SymbolReference* indexSymRef = symRefPair->_indexSymRef;
                        if (usingAladd)
                            arrayRefNode = TR::Node::create(TR::aladd, 2,
                                TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto),
                                TR::Node::createWithSymRef(byteCodeInfoNode, TR::lload, 0, indexSymRef));
                        else
                            arrayRefNode = TR::Node::create(TR::aiadd, 2,
                                TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto),
                                TR::Node::createWithSymRef(byteCodeInfoNode, TR::iload, 0, indexSymRef));
                    }

                    arrayRefNode->setIsInternalPointer(true);

                    if (!origAuto->getSymbol()->isInternalPointer()) {
                        symRefPair->_derivedSymRef->getSymbol()
                            ->castToInternalPointerAutoSymbol()
                            ->getPinningArrayPointer()
                            ->setPinningArrayPointer();
                        arrayRefNode->setPinningArrayPointer(symRefPair->_derivedSymRef->getSymbol()
                                                                 ->castToInternalPointerAutoSymbol()
                                                                 ->getPinningArrayPointer());
                    } else
                        arrayRefNode->setPinningArrayPointer(
                    ...
                    TR::Node* newStore
                        = TR::Node::createWithSymRef(TR::astore, 1, 1, arrayRefNode, symRefPair->_derivedSymRef);
                    newStore->setLocalIndex(~0);
                    TR::TreeTop* newStoreTreeTop = TR::TreeTop::create(comp(), newStore);
                    placeHolderTree->getPrevTreeTop()->join(newStoreTreeTop);
                    newStoreTreeTop->join(placeHolderTree);
                    ...
                    dumpOptDetails(comp(),
                        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted invariant "
                        "initialization tree : %p for new symRef #%d\n",
                        newStoreTreeTop->getNode(), symRefPair->_derivedSymRef->getReferenceNumber());

                    TR::RegisterCandidate* newInductionCandidate
                        = comp()->getGlobalRegisterCandidates()->findOrCreate(symRefPair->_derivedSymRef);
                    //
                    // Add the reassociated auto as a global register candidate
                    // within the loop
                    //
                    if (performTransformation(comp(),
                            "%s Adding auto %d (created for reassociation) as a global register candidate in loop %d\n",
                            OPT_DETAILS, newInductionCandidate->getSymbolReference()->getReferenceNumber(),
                            loopStructure->getNumber()))
                        newInductionCandidate->addAllBlocksInStructure(
                            loopStructure, comp(), trace() ? "reassociation auto" : NULL);
...
{
    if ((_registersScarce && cg()->getSupportsScaledIndexAddressing())
        || (!cg()->getSupportsScaledIndexAddressing() && cg()->getSupportsConstantOffsetInAddressing()))
        return false;
...
TR::VPIntRange* TR_LoopStrider::genVPIntRange(TR::VPConstraint* cons, int64_t coeff, int64_t additive)
{
    if (cons && cons->asIntConstraint()) {
        ...
        return new (trHeapMemory())
            TR::VPIntRange((int32_t)(low * coeff + additive), (int32_t)(high * coeff + additive));
    }
    return NULL;
}
...
TR::Node* TR_LoopStrider::getNewLoopIncrement(TR::Node* oldLoad, int32_t k, int32_t symRefNum)
{
    ...
    if (!newLoad) {
        if (_storeTreesList) {
            ...
            if (lookup != _storeTreesList->end()) {
                ...
                TR_StoreTreeInfo* storeTree;
                for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                    if (oldLoad == storeTree->_loadUsedInLoopIncrement && storeTree->_load) {
    ...
    if (!newLoad && (oldLoad == _loadUsedInLoopIncrement) && _loadUsedInNewLoopIncrement[k]) {
...
void TR_LoopStrider::changeLoopCondition(TR_BlockStructure* loopInvariantBlock, bool usingAladd, int32_t bestCandidate,
    TR::Node* storeOfDerivedInductionVariable)
{
    TR::SymbolReferenceTable* symRefTab = comp()->getSymRefTab();
    ...
    if (!usingAladd && (_linearEquations[bestCandidate][4] > -1) && secondChildOfLoopTestNode->getType().isInt64()) {
    ...
    if (usingAladd || (secondChildOfLoopTestNode->getType().isInt64())) {
        ...
        if (!secondChildOfLoopTestNode->getType().isInt64()) {
            TR::Node* i2lNode = TR::Node::create(secondChildOfLoopTestNode, TR::i2l, 1);
            i2lNode->setAndIncChild(0, secondChildOfLoopTestNode->duplicateTree());
            mulNode = TR::Node::create(TR::lmul, 2, i2lNode, constantNode);
        } else
    ...
    if ((_linearEquations[bestCandidate][4] > -1)
        ...
        TR::Node* newAload = TR::Node::createLoad(
            secondChildOfLoopTestNode, symRefTab->getSymRef((int32_t)_linearEquations[bestCandidate][4]));
        ...
        if (!newAload->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer()) {
            ...
            addNode->setPinningArrayPointer(newAload->getSymbolReference()
                                                ->getSymbol()
                                                ->castToInternalPointerAutoSymbol()
                                                ->getPinningArrayPointer());
        ...
        if (!pinningArrayPointer->isInternalPointer()) {
            ...
            symbol->castToInternalPointerAutoSymbol()->setPinningArrayPointer(
                pinningArrayPointer->castToInternalPointerAutoSymbol()->getPinningArrayPointer());

        newStore = TR::Node::createWithSymRef(TR::astore, 1, 1, addNode, newSymbolReference);
    } else {
        newSymbolReference
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), addNode->getDataType(), false);
        _newTempsCreated = true;
        _newNonAddressTempsCreated = true;
        newStore = TR::Node::createWithSymRef(
            addNode->getType().isInt64() ? TR::lstore : TR::istore, 1, 1, addNode, newSymbolReference);
    }
    ...
    if (firstChildOfLoopTestNode->getOpCode().hasSymbolReference()) {
        ...
        //
        TR::Node* newLoad = getNewLoopIncrement(firstChildOfLoopTestNode, bestCandidate,
            firstChildOfLoopTestNode->getSymbolReference()->getReferenceNumber());
        /*
        ...
        */
        ...
        if (newLoad) {
            ...
            TR::SymbolReference* derivedSymbolReference
                = symRefTab->getSymRef((int32_t)_linearEquations[bestCandidate][1]);
            firstOperand = TR::Node::createWithSymRef(firstChildOfLoopTestNode,
                comp()->il.opCodeForDirectLoad(derivedSymbolReference->getSymbol()->getDataType()), 0,
                derivedSymbolReference);
        }
    } else
    ...
    addNode->setLocalIndex(~0);
    TR::Node* secondOperand = TR::Node::createWithSymRef(
        secondChildOfLoopTestNode, comp()->il.opCodeForDirectLoad(newStore->getDataType()), 0, newSymbolReference);
    ...
    if (firstChildIsConversion && !usingAladd) {
        ...
        if (firstChildIsConversion->getReferenceCount() == 1)
            ...
            TR::Node* dupOfFirstConversionChild
                = TR::Node::create(firstChildIsConversion->getOpCodeValue(), 1, firstOperand);
    ...
    if (secondChildIsConversion && !usingAladd) {
        ...
        if (secondChildIsConversion->getReferenceCount() == 1)
            ...
            TR::Node* dupOfSecondConversionChild
                = TR::Node::create(secondChildIsConversion->getOpCodeValue(), 1, secondOperand);
...
bool TR_LoopStrider::unchangedValueNeededIn(TR::Block* exitBlock, int32_t nextInductionVariableNumber, bool& seenStore)
{
    ...
    while (cursorTree != exitTree) {
        ...
        if (cursorNode->getOpCode().isNullCheck() || (cursorNode->getOpCodeValue() == TR::treetop))
...
TR::Node* TR_LoopStrider::genLoad(TR::Node* node, TR::SymbolReference* symRef, bool isInternalPointer)
{
    ...
    result = TR::Node::createWithSymRef(node,
        isInternalPointer ? TR::aload : (symRef->getSymbol()->getType().isInt64() ? TR::lload : TR::iload), 0, symRef);
...
void TR_LoopStrider::examineOpCodesForInductionVariableUse(TR::Node* node, TR::Node* parent, int32_t& childNum,
    int32_t& index, TR::Node* originalNode, TR::Node* replacingNode, TR::Node* linearTerm, TR::Node* mulTerm,
    TR::SymbolReference** newSymbolReference, TR::Block* loopInvariantBlock, TR::AutomaticSymbol* pinningArrayPointer,
    int64_t differenceInAdditiveConstants, bool& isInternalPointer, bool& downcastNode, bool& usingAladd)
{
    if ((replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload)
        ...
        if (downcastNode && node->getOpCodeValue() == TR::lload) {
            ...
            //
            if (parent && parent->getType().isInt64() &&
                ...
                (!parent->getOpCode().isShift() || node != parent->getSecondChild()) && !parent->getOpCode().isCall()
                && !parent->getOpCode().isConversion() && parent->getOpCodeValue() != TR::lselect) {
        ...
        if (replacingNode->getOpCodeValue() != TR::iload && replacingNode->getOpCodeValue() != TR::lload) {
            ...
            if (usingAladd) {
                ...
                if (/* debug("enableRednIndVarElim") && */
                    indexNode->getOpCode().isLoadVarDirect() && indexNode->getSymbol()->isAutoOrParm()
                ...
                mulNode = TR::Node::create(node, replacingNode->getType().isInt64() ? TR::lmul : TR::imul, 2);
        ...
        if (isInternalPointer) {
            ...
            if (usingAladd) {
                ...
                if (replacingNode->getOpCodeValue() == TR::isub) {
                    adjustmentNode
                        = TR::Node::create(TR::imul, 2, adjustmentNode, TR::Node::create(node, TR::iconst, 0, -1));
                    adjustmentNode->setLocalIndex(~0);
                    adjustmentNode->getSecondChild()->setLocalIndex(~0);
                }
        ...
        } else if (replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload) {
        ...
        if (differenceInAdditiveConstants != 0) {
            ...
            if (adjustmentNode) {
                if (usingAladd || (adjustmentNode->getType().isInt64())) {
                    ...
                    differenceInAdditiveConstantsNode
                        = TR::Node::create(node, TR::iconst, 0, static_cast<int32_t>(differenceInAdditiveConstants));
        ...
        if (canHoistAdditiveTerm) {
            TR::SymbolReference* newLoopInvariant = comp()->getSymRefTab()->createTemporary(
                comp()->getMethodSymbol(), adjustmentNode->getDataType(), false);
            TR::Node* newLoad = TR::Node::createLoad(node, newLoopInvariant);
            newLoad->setLocalIndex(~0);
            TR::Node* newStore
                = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(adjustmentNode->getDataType()), 1, 1,
                    adjustmentNode->duplicateTree(), newLoopInvariant);
...
bool TR_LoopStrider::examineTreeForInductionVariableUse(TR::Block* loopInvariantBlock, TR::Node* parent,
    int32_t childNum, TR::Node* node, vcount_t visitCount, TR::SymbolReference** newSymbolReference)
{
    ...
    if (cg()->supportsInternalPointers() && (node->isInternalPointer()) && !node->isDataAddrPointer()
    ...
    if ((node->getOpCodeValue() == TR::iadd || node->getOpCodeValue() == TR::isub)
        ...
        if (isExprLoopInvariant(node->getSecondChild())) {
            ...
            if ((node->getFirstChild()->getOpCodeValue() == TR::imul
                ...
                if ((replacingNode = findReplacingNode(node->getFirstChild()->getFirstChild(), usingAladd, -1))) {
                    ...
                    && (replacingNode = findReplacingNode(node->getFirstChild()->getSecondChild(), usingAladd, -1))) {
                    linearTerm = node->getFirstChild()->getSecondChild();
                    mulTerm = node->getFirstChild()->getFirstChild();
                }

                if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral()
                    || // we were checking for signed here -- i made the test a bit weaker, but I don't think we can
                       // reach here with unsigned
                ...
                if (replacingNode) {
                    *newSymbolReference = NULL;
                        ...
                        if (isInternalPointer
                            && ((comp()->getSymRefTab()->getNumInternalPointers() >= maxInternalPointers())
                                || (comp()->cg()->canBeAffectedByStoreTagStalls()
                                    && _numInternalPointerOrPinningArrayTempsInitialized
                                        >= MAX_INTERNAL_POINTER_AUTOS_INITIALIZED)))
                            return seenInductionVariableComputation;
                        else if (!isInternalPointer && !node->isNonNegative() && !node->isNonPositive())
                            return seenInductionVariableComputation;
                    ...
                    if (canCreateNewSymRef) {
                        ...
                        *newSymbolReference = comp()->getSymRefTab()->createTemporary(
                            comp()->getMethodSymbol(), dataType, isInternalPointer);
                        if (isInternalPointer && !pinningArrayPointer) {
                            TR::SymbolReference* newPinningArray = comp()->getSymRefTab()->createTemporary(
                                comp()->getMethodSymbol(), TR::Address, false);
                            pinningArrayPointer = newPinningArray->getSymbol()->castToAutoSymbol();
                            createParmAutoPair(
                                comp()->getSymRefTab()->getSymRef(internalPointerSymbol), newPinningArray);

                            TR::Node* newAload
                                = TR::Node::createLoad(node, comp()->getSymRefTab()->getSymRef(internalPointerSymbol));
                            newAload->setLocalIndex(~0);
                            TR::Node* newStore
                                = TR::Node::createWithSymRef(TR::astore, 1, 1, newAload, newPinningArray);
                            newStore->setLocalIndex(~0);
                            internalPointerSymbol = newPinningArray->getReferenceNumber();
                            placeStore(newStore, loopInvariantBlock);
                            dumpOptDetails(comp(),
                                "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted "
                                "initialization tree : %p for new symRef #%d\n",
                        ...
                        populateLinearEquation(node, _loopDrivingInductionVar,
                            (*newSymbolReference)->getReferenceNumber(), internalPointerSymbol, mulTerm);
                    }
                    ...
                    examineOpCodesForInductionVariableUse(node, parent, childNum, index, originalNode, replacingNode,
                    ...
                    dumpOptDetails(comp(),
                        "O^O INDUCTION VARIABLE ANALYSIS: (add/sub) Replaced node : %p "
        ...
        bool shift = node->getOpCodeValue() == TR::ishl || node->getOpCodeValue() == TR::lshl;
        ...
        if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral()
            || // I changed isSignedInt->isIntegral -- but I don't think unsigned types ever reach here
            (!mulTerm->getOpCode().isLoadConst() && shift))
            replacingNode = NULL;
        ...
        if (replacingNode) {
            *newSymbolReference = NULL;
            ...
            if (canCreateNewSymRef) {
                ...
                *newSymbolReference
                    = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType, isInternalPointer);
                if (isInternalPointer && !pinningArrayPointer) {
                    TR::SymbolReference* newPinningArray
                        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address, false);
                    pinningArrayPointer = newPinningArray->getSymbol()->castToAutoSymbol();
                ...
                index = _nextExpression;
                populateLinearEquation(node, _loopDrivingInductionVar, (*newSymbolReference)->getReferenceNumber(),
                    internalPointerSymbol, mulTerm);
            }
            ...
            dumpOptDetails(comp(),
                "O^O INDUCTION VARIABLE ANALYSIS: (mul/shl) Replaced node : "
                "%p to (%s) by load of symbol #%d\n",
                node, comp()->getDebug()->getName(node->getOpCodeValue()), (*newSymbolReference)->getReferenceNumber());
        }
    }
    ...
    if (examineChildren) {
        ...
        for (i = 0; i < node->getNumChildren(); i++) {
            if (examineTreeForInductionVariableUse(
                    loopInvariantBlock, node, i, node->getChild(i), visitCount, newSymbolReference))
                seenInductionVariableComputation = true;
        }
    }
    ...
    TR::SymbolReferenceTable* symRefTab)
{
    ...
    if (getAdditiveTermNode(k) != NULL) {
        if (usingAladd) {
            addNode = TR::Node::create(TR::ladd, 2, mulNode, duplicateAdditiveTermNode(k, placeHolderNode, TR::Int64));
        } else {
            addNode = TR::Node::create(mulNode->getType().isInt64() ? TR::ladd : TR::iadd, 2, mulNode,
    ...
    if (_linearEquations[k][4] > -1) {
        TR::Node* newAload
            = TR::Node::createLoad(placeHolderNode, symRefTab->getSymRef((int32_t)_linearEquations[k][4]));
        newAload->setLocalIndex(~0);
        addNode = (usingAladd) ? TR::Node::create(TR::aladd, 2, newAload, addNode)
    ...
    dumpOptDetails(comp(),
        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted initialization tree : %p for new "
        "symRef #%d\n",
        newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
    return placeHolderNode;
}

TR::Node* TR_LoopStrider::placeNewInductionVariableIncrementTree(TR_BlockStructure* loopInvariantBlock,
    TR::SymbolReference* inductionVarSymRef, TR::SymbolReference* newSymbolReference, int32_t k,
    TR::SymbolReferenceTable* symRefTab, TR::Node* placeHolderNode, int32_t loopTestHasSymRef)
{
    //
    ...
    int32_t loopTestBlockNum
        = _loopTestTree->getNextTreeTop()->getNode()->getBlock()->startOfExtendedBlock()->getNumber();
    ...
    if (lookup != _storeTreesList->end()) {
        ...
        for (; storeTree != NULL;) {
            ...
            TR::Node* newStore = placeNewInductionVariableIncrementTree(loopInvariantBlock, inductionVarSymRef,
                newSymbolReference, k, symRefTab, placeHolderNode, newLoad, storeTree->getData()->_insertionTreeTop,
                storeTree->getData()->_constNode, storeTree->getData()->_isAddition);

            int32_t storeBlockNum
                = storeTree->getData()->_insertionTreeTop->getEnclosingBlock()->startOfExtendedBlock()->getNumber();

            // only update the return value if the storevalue
            // is going to be used in the new loopTest
            //
            if (!(loopTestHasSymRef > 0) || (storeBlockNum == loopTestBlockNum))
        ...
        returnValue = placeNewInductionVariableIncrementTree(loopInvariantBlock, inductionVarSymRef, newSymbolReference,
            k, symRefTab, placeHolderNode, newLoad, _insertionTreeTop, _constNode, _isAddition);
    }
...
TR::Node* TR_LoopStrider::placeNewInductionVariableIncrementTree(TR_BlockStructure* loopInvariantBlock,
    TR::SymbolReference* inductionVarSymRef, TR::SymbolReference* newSymbolReference, int32_t k,
    TR::SymbolReferenceTable* symRefTab, TR::Node* placeHolderNode, TR::Node* newLoad, TR::TreeTop* insertionTreeTop,
    TR::Node* constNode, bool isAddition)
{
    ...
    if (usingAladd) {
        ...
        newMulNode = TR::Node::create(newLoad->getType().isInt64() ? TR::lmul : TR::imul, 2, constNode,
            duplicateMulTermNode(k, placeHolderNode, newLoad->getDataType()));
    }
    ...
    if (isAddition) {
        if (_linearEquations[k][4] > -1) {
            ...
            loopIncrementer = (usingAladd) ? TR::Node::create(TR::aladd, 2, newIvLoad, newMulNode)
                                           : TR::Node::create(TR::aiadd, 2, newIvLoad, newMulNode);
            loopIncrementer->setIsInternalPointer(true);

            TR::AutomaticSymbol* autoSym
                = symRefTab->getSymRef((int32_t)_linearEquations[k][4])->getSymbol()->castToAutoSymbol();
            if (!autoSym->isInternalPointer()) {
                loopIncrementer->setPinningArrayPointer(autoSym);
                autoSym->setPinningArrayPointer();
            } else
                loopIncrementer->setPinningArrayPointer(
                    autoSym->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
    ...
    dumpOptDetails(comp(),
        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted loop incremental step tree : %p for "
        "new symRef #%d\n",
        newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
    return newStore;
}

int32_t TR_LoopStrider::findNewInductionVariable(
    TR::Node* node, TR::SymbolReference** symRef, bool hasAdditiveTerm, int32_t internalPointerParentSymbol)
{
    // for aladds
    bool usingAladd = (comp()->target().is64Bit()) ? true : false;
...
TR::Node* TR_LoopStrider::setUsesLoadUsedInLoopIncrement(TR::Node* node, int32_t k)
{
    if (_storeTreesList) {
        ...
        if (lookup != _storeTreesList->end()) {
            ...
            TR_StoreTreeInfo* storeTree;
            for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                if (!storeTree->_loadUsedInLoopIncrement && node->getReferenceCount() > 1)
                    return NULL;
...
bool TR_LoopStrider::isExpressionLinearInSomeInductionVariable(TR::Node* node)
{
    ...
    if ((node->getOpCodeValue() == TR::iload) || (node->getOpCodeValue() == TR::lload)) {
        ...
        if ((node->getFirstChild()->getOpCodeValue() == TR::iload)
            || (node->getFirstChild()->getOpCodeValue() == TR::lload)) {
            if ((_storeTreesList
                    || _writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))
                && (node->getSecondChild()->getOpCode().isLoadConst()
                    || (node->getSecondChild()->getOpCode().isLoadVarDirect()
                        && node->getSecondChild()->getSymbol()->isAutoOrParm()
                        && _neverWritten->get(node->getSecondChild()->getSymbolReference()->getReferenceNumber())))) {
...
bool TR_LoopStrider::reassociateAndHoistComputations(
    TR::Block* loopInvariantBlock, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount)
{
    // for aladds
    bool usingAladd = (comp()->target().is64Bit()) ? true : false;
    ...
    if (cg()->supportsInternalPointers() && reassociateAndHoistNonPacked() && node->isInternalPointer()
    ...
    if (isInternalPointer && examineChildren) {
        bool isConst = false;
        if (node->getOpCode().isLoadConst())
            isConst = true;

        if ((isConst
                || (node->getOpCode().hasSymbolReference() && node->getOpCode().isLoadVar() &&
            ...
            while (symRefPair) {
                ...
                if (isConst) {
                    ...
                    if (symRefPair->_isConst && (symRefPair->_index == constNodeValue))
                        matched = true;
                } else {
                    if (!symRefPair->_isConst && (symRefPair->_indexSymRef == node->getSymbolReference()))
                        matched = true;
                }
            ...
            if (!internalPointerSymRef) {
                TR::SymbolReference* newSymbolReference = comp()->getSymRefTab()->createTemporary(
                    comp()->getMethodSymbol(), TR::Address, isInternalPointer);
                if (isInternalPointer && !pinningArrayPointer) {
                    TR::SymbolReference* newPinningArray
                        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address, false);
                    pinningArrayPointer = newPinningArray->getSymbol()->castToAutoSymbol();
            ...
            if ((isInternalPointer && (comp()->getSymRefTab()->getNumInternalPointers() < maxInternalPointers())
                ...
                if (_reassociatedAutos->find(originalInternalPointerSymbol) == _reassociatedAutos->end()) {
                    ...
                    if (isInternalPointer && !pinningArrayPointer) {
                        ...
                        newStoreTreeTop->join(nextTree);
                        dumpOptDetails(comp(),
                            "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted initialization "
                            "tree : %p for new symRef #%d\n",
                            newStoreTreeTop->getNode(), newPinningArray->getReferenceNumber());
                        _numInternalPointerOrPinningArrayTempsInitialized++;
                    }
        ...
        if (lookup != _storeTreesList->end()) {
            ...
            for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                ...
                if ((!storeNode->getType().isInt32() && !storeNode->getType().isInt64())
                    || (!storeNode->getSymbolReference()->getSymbol()->getType().isInt32()
                        && !storeNode->getSymbolReference()->getSymbol()->getType().isInt64()))
                    return false;
...
void TR_LoopStrider::checkIfIncrementInDifferentExtendedBlock(TR::Block* block, int32_t inductionVariable)
{
    ...
    if (block != _storeTrees[inductionVariable]->getEnclosingBlock()->startOfExtendedBlock()) {
    ...
    if (_storeTreesList) {
        ...
        if (lookup != _storeTreesList->end()) {
            ...
            TR_StoreTreeInfo* storeTree;
            for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                if (block != storeTree->_tt->getEnclosingBlock()->startOfExtendedBlock()) {
...
TR::Node* TR_LoopStrider::updateLoadUsedInLoopIncrement(TR::Node* node, int32_t inductionVariable)
{
    ...
    if (_indirectInductionVariable && opCode.isLoadVar()
        ...
        && ((!_storeTreesList && _writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber()))
            || (_storeTreesList
                && _storeTreesList->find(node->getSymbolReference()->getReferenceNumber()) != _storeTreesList->end()
                && (*_storeTreesList)[node->getSymbolReference()->getReferenceNumber()]->isSingleton()))) {
        ...
        if (useDefInfo->getUseDef(defs, useIndex) && (defs.PopulationCount() == 1)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                TR::Node* constNode
                    = containsOnlyInductionVariableAndAdditiveConstant(defNode->getFirstChild(), inductionVariable);
                if (constNode && _storeTrees[defNode->getSymbolReference()->getReferenceNumber()]) {
                    checkIfIncrementInDifferentExtendedBlock(
                        useDefInfo->getTreeTop(defIndex)->getEnclosingBlock()->startOfExtendedBlock(),
                        inductionVariable);
                } else {
                    constNode = NULL;
                }
...
bool TR_LoopStrider::isStoreInRequiredForm(TR::Node* storeNode, int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (_constNode == NULL) {
        ...
        TR_InductionVariable* v
            = loopStructure->asRegion()->findMatchingIV(comp()->getSymRefTab()->getSymRef(symRefNum));
        if (!v)
            return false;
...
static void orderSensitiveDescendantsRec(TR::Node* node, TR::NodeChecklist& out, TR::NodeChecklist& visited)
{
 ...
 */
static void assertSubstPreservesEvalOrder(TR::Node* orig, TR::Node* replacement, const char* operation)
{
    if (enableExpensiveLoopStriderAssertions()) {
        TR_ASSERT_FATAL(substPreservesEvalOrder(orig, replacement), "%s fails to preserve ordering\n", operation);
    }
}
...
static void assertSubtreeDoesNotMentionOriginals(
    TR::Node* node, const TR::list<std::pair<int32_t, int32_t> >& widenedIVs, TR::NodeChecklist& visited)
{
    if (visited.contains(node))
        return;
    ...
    for (auto it = widenedIVs.begin(); it != widenedIVs.end(); ++it) {
        TR_ASSERT_FATAL(symRefNum != it->first, "n%un is not supposed to have mentions of symref #%d\n",
            node->getGlobalIndex(), it->first);
    }
}
...
static void assertStructureDoesNotMentionOriginals(
    TR_Structure* structure, const TR::list<std::pair<int32_t, int32_t> >& widenedIVs)
{
    if (!enableExpensiveLoopStriderAssertions())
        return;
...
static bool isLoopRelativelyFrequent(TR_Structure* loop, TR::Block* preheader)
{
    ...
    static const char* const envMinFreq = feGetEnv("TR_loopStriderSignExtensionMinimumFrequency");
    ...
    static const char* const envMinIters = feGetEnv("TR_loopStriderSignExtensionMinimumIterations");
    ...
    if (loopFreq < minFreq) {
        dumpOptDetails(
            TR::comp(), "[Sign-Extn] skip loop %d due to frequency %d < %d\n", loop->getNumber(), loopFreq, minFreq);
        return false;
    }
    ...
    if (estimatedAverageIters < minIters) {
        dumpOptDetails(TR::comp(), "[Sign-Extn] skip loop %d due to estimated average iterations %f < %f\n",
            loop->getNumber(), estimatedAverageIters, minIters);
        return false;
    }
 ...
 */
void TR_LoopStrider::detectLoopsForIndVarConversion(TR_Structure* loopStructure, TR::NodeChecklist& exLoads)
{
        ...
        if (usingAladd && info) {
            if (isPredictableLoop > 0) {
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    TR_ASSERT(storeTree != NULL, "missing store tree for #%d in _writtenExactlyOnce\n",
                    ...
                    if (storeInRequiredForm
                        && performTransformation(comp(), "%s Analyzing candidate: %d in loop: %d for conversion\n",
                            OPT_DETAILS, _loopDrivingInductionVar, loopStructure->getNumber())) {
                        TR_ASSERT(_loopDrivingInductionVar == nextInductionVariable,
                        ...
                        if (_isInductionVariableMorphed || stressMode) {
                            if (checkStoreOfIndVar(_storeTrees[nextInductionVariable]->getNode())) {
                                ...
                                widenedIVs.push_back(std::make_pair(nextInductionVariable, ext->getReferenceNumber()));
                            }
                        }
                    }
                }
 ...
 */
void TR_LoopStrider::extendIVsOnLoopEntry(const TR::list<std::pair<int32_t, int32_t> >& ivs, TR::Block* preheader)
{
 ...
 * \see detectLoopsForIndVarConversion(TR_Structure*, TR::NodeChecklist&)
 */
void TR_LoopStrider::truncateIVsOnLoopExit(const TR::list<std::pair<int32_t, int32_t> >& ivs, TR_RegionStructure* loop)
{
    ...
    for (TR::Block* src = lit.getFirst(); src != NULL; src = lit.getNext()) {
        ...
        for (TR::Block* dest = eit.getFirst(); dest != NULL; dest = eit.getNext()) {
            ...
            auto preds = dest->getPredecessors().size() + dest->getExceptionPredecessors().size();
            if (preds > 1) {
                TR::Block* origDest = dest;
                dest = src->splitEdge(src, dest, comp());
                dumpOptDetails(comp(), "[Sign-Extn] split loop exit: block_%d [-> block_%d] -> block_%d\n",
                    src->getNumber(), dest->getNumber(), origDest->getNumber());
            }
 ...
 */
void TR_LoopStrider::convertIV(TR::Node* bciNode, TR::TreeTop* nextTT, int32_t ivIn, int32_t ivOut, TR::ILOpCodes op)
{
    ...
    dumpOptDetails(comp(), "[Sign-Extn] convert at loop boundary: #%d <- %s(#%d) in block_%d\n", ivOut,
        TR::ILOpCode(op).getName(), ivIn, nextTT->getEnclosingBlock()->getNumber());
    TR::Node* conv
        = TR::Node::createStore(bciNode, out, TR::Node::create(bciNode, op, 1, TR::Node::createLoad(bciNode, in)));
    nextTT->insertBefore(TR::TreeTop::create(comp(), conv));
}

void TR_LoopStrider::createConstraintsForNewInductionVariable(
    TR_Structure* loopStructure, TR::SymbolReference* newSymRef, TR::SymbolReference* oldSymRef)
{
    ...
    if (v) {
        ...
        TR_InductionVariable* newv
            = new (trHeapMemory()) TR_InductionVariable(newSymRef->getSymbol()->castToRegisterMappedSymbol(),
                newEntryVal, newExitVal, newIncr, oldIV ? oldIV->isSigned() : TR_maybe);
...
bool TR_LoopStrider::morphExpressionLinearInInductionVariable(
    TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount)
{
    // for aladds
    bool usingAladd = (comp()->target().is64Bit()) ? true : false;
        ...
        if (secondChild->getOpCodeValue() == TR::lconst) {
            ...
            if (firstChild->getOpCodeValue() == TR::lmul || firstChild->getOpCodeValue() == TR::lshl) {
                ...
                if (firstChild->getFirstChild()->getOpCodeValue() == TR::i2l) {
                    ...
                    if (isLinearInInductionVariable && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst) {
                        examineChildren = false;
    ...
    //     lconst
        ...
        if (node->getFirstChild()->getOpCodeValue() == TR::i2l) {
            ...
            isLinearInInductionVariable = checkExpressionForInductionVariable(indVarTree);
            if (isLinearInInductionVariable && node->getSecondChild()->getOpCodeValue() == TR::lconst) {
                examineChildren = false;
...
void TR_LoopStrider::walkTreesAndFixUseDefs(
    TR_Structure* loopStructure, TR::SymbolReference* newSymbolReference, TR::NodeChecklist& exLoads)
{
    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "loopStrider.widen/(%s)/%s/loop=%d/iv=%d", comp()->signature(),
            comp()->getHotnessName(comp()->getMethodHotness()), loopStructure->getEntryBlock()->getNumber(),
            _loopDrivingInductionVar));

    // Change (istore i ..) to (lstore k (i2l ..))
    TR::Node* storeNode = _storeTrees[_loopDrivingInductionVar]->getNode();
    TR_ASSERT(storeNode->getOpCodeValue() == TR::istore, "storeNode opcode is %s; expected istore\n",
        storeNode->getOpCode().getName());
    TR_ASSERT(storeNode->getSymbolReference()->getReferenceNumber() == _loopDrivingInductionVar,
    ...
    replaceLoadsInStructure(loopStructure, _loopDrivingInductionVar, newIntValue, newSymbolReference, exLoads, visited);
 ...
 * \see detectLoopsForIndVarConversion(TR_Structure*, TR::NodeChecklist&)
 */
void TR_LoopStrider::replaceLoadsInStructure(TR_Structure* structure, int32_t iv, TR::Node* defRHS,
    TR::SymbolReference* newSR, TR::NodeChecklist& exLoads, TR::NodeChecklist& visited)
{
    if (structure->asBlock() != NULL) {
        TR::Block* b = structure->asBlock()->getBlock();
        TR::TreeTop* end = b->getExit();
...
void TR_LoopStrider::replaceLoadsInSubtree(TR::Node* node, int32_t iv, TR::Node* defRHS, TR::SymbolReference* newSR,
    TR::NodeChecklist& exLoads, TR::NodeChecklist& visited)
{
    if (visited.contains(node))
        return;
    ...
    if (node->getOpCodeValue() == TR::iload && node->getSymbolReference()->getReferenceNumber() == iv) {
 ...
 */
void TR_LoopStrider::widenComparison(TR::Node* node, int32_t iv, TR::Node* defRHS, TR::NodeChecklist& exLoads)
{
    static const char* const disableEnv = feGetEnv("TR_disableLoopStriderWidenComparison");
    ...
    case TR::icmpge:
        ...
        break;
    default:
        return; // Not a 32-bit signed integer comparison.
    }
    ...
    if (!performTransformation(comp(), "%s [Sign-Extn] Changing n%un %s into %s\n", optDetailString(),
            node->getGlobalIndex(), node->getOpCode().getName(), TR::ILOpCode(lcmp).getName()))
        return;
...
void TR_LoopStrider::eliminateSignExtensionsInSubtree(
    TR::Node* parent, TR::NodeChecklist& exLoads, TR::NodeChecklist& visited, SignExtMemo& signExtMemo)
{
    ...
    for (int i = 0; i < parent->getNumChildren(); i++) {
        ...
        if (!performTransformation(comp(),
                ...
                node->getGlobalIndex(), extended->getGlobalIndex(), i, parent->getGlobalIndex()))
            continue;
 ...
 * \see transmuteDescendantsIntoTruncations(TR::Node*, TR::Node*)
 */
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtend(TR::Node* node, TR::NodeChecklist& exLoads, SignExtMemo& memo)
{
    ...
    case TR::l2i:
        if (exLoads.contains(node)) {
            TR::Node* child = node->getFirstChild();
            TR_ASSERT(child->getOpCodeValue() == TR::lload, "exload l2i n%un's child should be an lload\n",
                node->getGlobalIndex());
    ...
    if (result.extended != NULL) {
        ...
        if (trace())
            traceMsg(comp(), "[Sign-Extn] sign-extended n%un %s into n%un %s\n", node->getGlobalIndex(),
...
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtendBinOp(
    TR::ILOpCodes op64, TR::Node* node, TR::NodeChecklist& exLoads, SignExtMemo& memo)
{
    const SignExtEntry rejected;
    if (!node->cannotOverflow())
        return rejected;

    static const char* const moveDisableEnv = feGetEnv("TR_disableLoopStriderMoveSignExtIntoChild");
    static const bool moveDisable = moveDisableEnv != NULL && moveDisableEnv[0] != '\0';
 ...
 */
void TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node* intNode, TR::Node* longNode)
{
    if (longNode->getOpCodeValue() == TR::i2l) {
        ...
        TR_ASSERT(child == intNode, "transmuting n%un %s does not match child n%un %s of n%un i2l\n",
            intNode->getGlobalIndex(), intNode->getOpCode().getName(), child->getGlobalIndex(),
            child->getOpCode().getName(), longNode->getGlobalIndex());
        return;
    }
    ...
    TR_ASSERT(intOp == TR::ILOpCode::integerOpForLongOp(longNode->getOpCodeValue()),
        "attempted to transmute n%un %s into l2i of n%un %s\n", intNode->getGlobalIndex(),
        intNode->getOpCode().getName(), longNode->getGlobalIndex(), longNode->getOpCode().getName());

    int children = intNode->getNumChildren();
    TR_ASSERT(children == longNode->getNumChildren(), "transmute child count differs for n%un vs n%un\n",
        intNode->getGlobalIndex(), longNode->getGlobalIndex());
    ...
    if (!performTransformation(comp(), "%s [Sign-Extn] Transmuting n%un %s into l2i of n%un %s\n", optDetailString(),
            intNode->getGlobalIndex(), intNode->getOpCode().getName(), longNode->getGlobalIndex(),
    ...
    if (firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) {
        ...
        if (firstChild->getFirstChild()->getOpCode().hasSymbolReference()) {
            if (firstChild->getSecondChild()->getOpCode().isLoadConst()) {
                if (firstChild->getFirstChild()->getSymbolReference()->getReferenceNumber()
                    == _loopDrivingInductionVar) {
...
bool TR_LoopStrider::checkExpressionForInductionVariable(TR::Node* node)
{
    ...
    if (node->getOpCodeValue() == TR::iload) {
        if (_writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber()))
            return true;
    } else if (node->getOpCodeValue() == TR::iadd && node->getFirstChild()->getOpCodeValue() == TR::iload
        ...
        if (_writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))
            return true;
    } else if (node->getOpCodeValue() == TR::isub && node->getFirstChild()->getOpCodeValue() == TR::iload
        && node->getSecondChild()->getOpCodeValue() == TR::iconst) {
        if (_writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))
            return true;
    }
...
bool TR_LoopStrider::isAdditiveTermEquivalentTo(int32_t k, TR::Node* node)
{
    if (isAdditiveTermConst(k) && node->getOpCode().isLoadConst()) {
        ...
        if (getAdditiveTermConst(k) == addConst)
            return true;
    } else if (getAdditiveTermNode(k) && getAdditiveTermNode(k)->getOpCode().hasSymbolReference()
...
TR::Node* TR_LoopStrider::duplicateMulTermNode(int32_t k, TR::Node* node, TR::DataType type)
{
    ...
    if (new_node->getDataType() != type)
        new_node = TR::Node::create(
            TR::ILOpCode::getProperConversion(new_node->getDataType(), type, false /* !wantZeroExtension */), 1,
            new_node);
    return new_node;
}
...
bool TR_LoopStrider::isMulTermEquivalentTo(int32_t k, TR::Node* node)
{
    if (isMulTermConst(k) && node->getOpCode().isLoadConst()) {
        ...
        if (getMulTermConst(k) == multiplicativeConstant)
            return true;
    } else if (getMulTermNode(k) && getMulTermNode(k)->getOpCode().hasSymbolReference()
        && node->getOpCode().hasSymbolReference()
...
const char* TR_LoopStrider::optDetailString() const throw() { return "O^O LOOP STRIDER: "; }

// end sign-extension

static inline int64_t ceiling(int64_t numer, int64_t denom)
{
    return (numer % denom == 0) ? numer / denom : numer / denom + 1;
}
...
void TR_InductionVariableAnalysis::gatherCandidates(TR_Structure* s, TR_BitVector* loopLocalDefs, TR_BitVector* allDefs)
{
    ...
    } else if (loopLocalDefs) // ie inside loop
    {
        ...
        for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
            ...
            if (node->getOpCode().isStoreDirect()
                && (node->getType().isIntegral() || node->getSymbolReference()->getSymbol()->isInternalPointer())) {
...
void TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure* structure, TR_RegionStructure* loop)
{
    ...
    if (trace()) {
        ...
        while (it.hasMoreElements()) {
            ...
            traceMsg(
                comp(), "\t%d %d %p symRef=%p symbol=%p: ", refNum, refIndex, inSymbol, symRef, symRef->getSymbol());
            if (inSymbol)
...
void TR_InductionVariableAnalysis::analyzeCyclicRegion(TR_RegionStructure* region, TR_RegionStructure* loop)
{
    ...
    while (it.hasMoreElements()) {
        ...
        if (loopLocalDefs->isSet(refNum)) {
            ...
            // traceMsg(comp(),"For loop %d setting symref %d to unknown
...
TR_InductionVariableAnalysis::DeltaInfo** TR_InductionVariableAnalysis::newBlockInfo(TR_RegionStructure* loop)
{
...
void TR_InductionVariableAnalysis::initializeBlockInfoArray(TR_RegionStructure* loop)
{
...
bool TR_InductionVariableAnalysis::isProgressionalStore(TR::Node* node, TR_ProgressionKind* kind, int64_t* incr)
{
    ...
    if (!getProgression(cursor, storeRef, &symRef, kind, incr)) {
        // traceMsg(comp(),"In isProgressionalStore cursor node %p returning false. getProgression returned
        // false\n",cursor);
        return false;
    }
    ...
    {
        // traceMsg(comp(),"In isProgressionalStore cursor node %p returning false. symRef = %p(%d) is not equal to
...
//
bool TR_InductionVariableAnalysis::branchContainsInductionVariable(
    TR_RegionStructure* loop, TR::Node* branchNode, TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    ...
    for (iv = it.getFirst(); iv; iv = it.getNext()) {
        ...
        if (trace())
            traceMsg(comp(), "\t considering branchnode [%p] and basiciv [%d]\n", branchNode, index);
...
bool TR_InductionVariableAnalysis::branchContainsInductionVariable(
    TR::Node* node, TR::SymbolReference* ivSymRef, int32_t* nodeBudget)
{
...
//
bool TR_InductionVariableAnalysis::getProgression(
    TR::Node* expr, TR::SymbolReference* storeRef, TR::SymbolReference** ref, TR_ProgressionKind* prog, int64_t* incr)
{
    int64_t konst;
    TR_ProgressionKind kind;
    ...
    if (expr->getOpCode().isAdd() && secondChild->getOpCode().isLoadConst() && secondChild->getType().isIntegral()) {
        ...
        if (kind == Geometric)
            return false;
        konst += secondChild->getOpCode().isUnsigned() ? secondChild->get64bitIntegralValueAsUnsigned()
                                                       : secondChild->get64bitIntegralValue();
        *prog = Arithmetic;
    } else if (expr->getOpCode().isSub() && secondChild->getOpCode().isLoadConst()
        && secondChild->getType().isIntegral()) {
        if (!getProgression(expr->getFirstChild(), storeRef, ref, &kind, &konst))
            return false;
        if (kind == Geometric)
            return false;
    ...
    */
        ...
        if (expr->getOpCode().hasSymbolReference()) {
            if (!expr->getSymbolReference()->getSymbol()->isAutoOrParm())
                return false;
            if (storeRef
                && (expr->getSymbolReference() != storeRef || expr->getVisitCount() == comp()->getVisitCount()))
                return false;
...
void TR_InductionVariableAnalysis::analyzeLoopExpressions(TR_RegionStructure* loop, DeltaInfo** loopSet)
{
    TR_BitVector* candidates = ((AnalysisInfo*)loop->getAnalysisInfo())->getLoopLocalDefs();

    comp()->incVisitCount();

    TR_Array<TR_BasicInductionVariable*>* ivs = new (trHeapMemory())
        TR_Array<TR_BasicInductionVariable*>(trMemory(), candidates->elementCount(), true, heapAlloc);
    ...
    while (it.hasMoreElements()) {
        ...
        if (info->isUnknownValue()) {
            if (trace())
                traceMsg(comp(), "----> symRef #%d[%p] is unknown\n", refNum, symRef);
        } else if (info->getKind() == Identity) {
            if (trace())
                traceMsg(comp(), "----> symRef #%d[%p] is using an identity progression\n", refNum, symRef);
...
//
static uint16_t countNodeOccurrencesInSubTree(
    TR::Node* root, TR::Node* node, vcount_t visitCount, TR::Compilation* comp)
{
    uint16_t count = 0;
...
static bool identicalBranchTrees(TR::Node* node1, TR::Node* node2)
{
    ...
    if (node1->getOpCodeValue() == TR::iconst && node1->getInt() != node2->getInt())
        return false;

    if (node1->getOpCodeValue() == TR::lconst && node1->getLongInt() != node2->getLongInt())
        return false;
...
void TR_InductionVariableAnalysis::appendPredecessors(WorkQueue& dest, TR::Block* block)
{
...
bool TR_InductionVariableAnalysis::isIVUnchangedInLoop(
    TR_RegionStructure* loop, TR::Block* loopTestBlock, TR::Node* candidateIV)
{
    ...
    for (TR::Block* block = loopTestBlock->getNextBlock(); block && block->isExtensionOfPreviousBlock();
    ...
    while (!workQueue.empty()) {
        ...
        for (TR::TreeTop* tt = curBlock->getFirstRealTreeTop(); tt->getNode()->getOpCodeValue() != TR::BBEnd;
             tt = tt->getNextTreeTop()) {
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(
    TR_RegionStructure* loop, TR_BitVector* candidates, TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    ...
    for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
        ...
        if (subNode->getStructure()->asRegion()) {
            ...
            if (!_seenInnerRegionExit.get(toNum)) {
                ...
                for (auto predEdge = end->getPredecessors().begin(); predEdge != end->getPredecessors().end();
                     ++predEdge) {
            ...
            if (osrInduceExitEdge) {
                if (trace())
                    traceMsg(comp(), "\tbranch from inner region is to OSR block, ignoring\n");
                continue;
            }

            if (trace())
                traceMsg(comp(), "\tReject - exit edge from inner region\n");
            return false;
        }
        ...
        if (((!node->getOpCode().isBooleanCompare() || !node->getOpCode().isBranch())
            ...
            if (trace())
                traceMsg(comp(), "\tReject - branch block not ending with a branch\n"); // FIXME: can improve a bit here
            return false;
        }

        if (!(node->getFirstChild()->getType().isIntegral()
                || (node->getFirstChild()->getType().isAddress() && node->getFirstChild()->isInternalPointer()
        ...
        if (!node->getOpCode().isJumpWithMultipleTargets()) {
            ...
            if (trace())
                traceMsg(comp(), "\tbranch is a tstart, not a controlling branch. Ignoring\n");
        ...
        if (!branchContainsInductionVariable(loop, node, basicIVs)) {
            ...
            if (trace())
                traceMsg(comp(), "\tBranch [%p] does not involve basic ivs, ignoring\n", node);
            continue;
        }
        ...
        for (auto predEdge = loop->getEntryBlock()->getPredecessors().begin();
             predEdge != loop->getEntryBlock()->getPredecessors().end(); ++predEdge) {
            TR::Block* predBlock = (*predEdge)->getFrom()->asBlock();
            if (loop->contains(predBlock->getStructureOf()) && !_dominators->dominates(block, predBlock)) {
                return false;
            }
        }
        ...
        if (!getProgression(node->getFirstChild(), 0, &symRef, &kind, &incr)) {
            if (trace())
                traceMsg(comp(), "\tReject - branch node %p in unrecognized form\n", node);
            return false;
        }
        ...
        if (!biv) {
            if (trace())
                traceMsg(comp(), "\tReject - test (node %p) on unknown symRef\n", node);
            return false;
        }
    ...
    if (controllingIV == -1) {
        if (trace())
            traceMsg(comp(), "\tReject - could not find a loop controlling test\n");
        return false;
    }
    ...
    if ((TR::ILOpCode::isLessCmp(opCode) && delta > 0) || (TR::ILOpCode::isGreaterCmp(opCode) && delta < 0)) {
        if (trace())
            traceMsg(comp(), "\tReject - uninteresting direction of the loop test\n");
        return false;
    }
    ...
    if (valueNode->getOpCode().isLoadVarDirect()) {
        ...
        if (valueCouldBeEvaluatedBeforeTest || forceILWalk) {
            ...
            if (trace()) {
                traceMsg(comp(), "\twalking EBB to determine when n%un %s #%d is evaluated\n",
            ...
            if (usesUnchangedValueInLoopTest) {
                if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(), valueNode)) {
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "\tReject - IV node has been changed in one of the blocks that is not part of the loop "
                            "test in the loop\n");
                    return false;
                }
            }
        ...
        for (tt = controllingBranch;; tt = tt->getPrevTreeTop()) {
            ...
            if (curNode->getOpCodeValue() == TR::BBStart && !curNode->getBlock()->isExtensionOfPreviousBlock()) {
                if (trace())
                    traceMsg(comp(), "\tRejected - description needed here\n");
            ...
            if (curNode->getOpCode().isStoreDirect() && curNode->getSymbolReference() == biv->getSymRef()) {
                if (curNode->getFirstChild() != valueNode) {
                    if (trace())
                        traceMsg(comp(), "\tRejected - exit value %p maybe not same as tested value %p\n", curNode,
                            valueNode);
    ...
    if (!loop->isExprInvariant(boundNode)) {
        if (trace())
            traceMsg(comp(), "\tReject - loop bound node is not invariant\n");
        return false;
    }

    if ((boundNode->getOpCode().isLoadVarDirect()) && boundNode->getSymbolReference()->getSymbol()->isAutoOrParm()) {
    ...
    if (biv->getEntryValue()) {
        ...
        TR::Node* mulNode = ((entry->getOpCode().isAdd() || entry->getOpCode().isSub())
                                && entry->getSecondChild()->getOpCode().isLoadConst())
            ? mulNode = entry->getFirstChild()
            : entry;

        if (mulNode->getOpCode().isMul() && mulNode->getSecondChild()->getOpCode().isLoadConst()) {
                ...
                if (actualValue && actualValue->getOpCode().isLoadConst()) {
                    if (entry->getType().isInt32()) {
                        ...
                        if (entry->getOpCode().isAdd()) // second child must be a load const
                            ...
                            konst -= entry->getSecondChild()->getInt();
                        TR::Node* node = TR::Node::create(actualValue, TR::iconst, 0);
                        node->setInt(konst);
    ...
    if (biv->getEntryValue() && biv->getSymRef()->getSymbol()->isInternalPointer()) {
        TR::Node* entry = biv->getEntryValue();
        TR::Node* newEntryValue = 0;
        TR::Node* baseLoad = NULL;
        if (entry->getNumChildren() > 0)
            baseLoad = entry->getFirstChild();
        if (entry->getOpCode().isAdd() && baseLoad && baseLoad->getOpCode().isLoad()) {
            ...
            if (offset->getOpCode().isAdd() || offset->getOpCode().isSub()) {
                if (offset->getSecondChild()->getOpCode().isLoadConst()) {
                    ...
                    if (otherNode->getOpCode().isMul() && otherNode->getSecondChild()->getOpCode().isLoadConst()) {
                        ...
                        if (loadNode->getOpCode().isLoadVarDirect()) {
                            ...
                            if (actualValue && actualValue->getOpCode().isLoadConst()) {
                                if ((actualValue->getSize() == 8) || foundConversion) {
                                    ...
                                    // int64_t konst = actualValue->getLongInt() * mulConst->getLongInt(); // FIXME:
                                    // overflow
    ...
    TR_PrimaryInductionVariable* piv = new (trHeapMemory()) TR_PrimaryInductionVariable(biv, controllingBlock,
        boundNode->duplicateTree(), opCode, comp(), optimizer(), usesUnchangedValueInLoopTest, trace());
...
bool TR_InductionVariableAnalysis::isGotoBlock(TR::Block* block)
{
...
bool TR_InductionVariableAnalysis::findEntryValues(
    TR_RegionStructure* loop, TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    TR_ArrayIterator<TR_BasicInductionVariable> it(&basicIVs);
...
TR::Node* TR_InductionVariableAnalysis::findEntryValueForSymRef(TR_RegionStructure* loop, TR::SymbolReference* symRef)
{
...
TR::Node* TR_InductionVariableAnalysis::getEntryValue(TR::Block* block, TR::SymbolReference* symRef,
    TR_BitVector* nodesDone, TR_Array<TR::Node*>& cachedValues, int32_t depth)
{
    ...
    if (!entryTree) {
        if (comp()->isLoopTransferDone()) {
            ...
            TR::Node* dummyNode = TR::Node::create(NULL, ((dataType == TR::Int32) ? TR::iconst : TR::lconst), 0);
    ...
    for (TR::TreeTop* tt = block->getLastRealTreeTop(); tt != entryTree; tt = tt->getPrevRealTreeTop()) {
        TR::Node* node = tt->getNode();
        if (node->getOpCodeValue() == TR::treetop)
            node = node->getFirstChild();
        ...
        if (node->getOpCode().isStoreDirect()) {
        ...
        } else if (node->mayKill(false).contains(symRef, comp())
            || (node->getNumChildren() > 0 && node->getChild(0)->mayKill(false).contains(symRef, comp()))) {
            ...
            // if you find somewhere a call like  foo(&num);  then this kills the num, and we can no longer reason about
            // its value.
...
const char* TR_InductionVariableAnalysis::optDetailString() const throw()
{
    return "O^O INDUCTION VARIABLE ANALYSIS: ";
}

///////////////////////////////////////////
void TR_InductionVariableAnalysis::DeltaInfo::arithmeticDelta(int32_t delta)
{
    if (_kind == Geometric)
        _unknown = true;
...
void TR_InductionVariableAnalysis::DeltaInfo::geometricDelta(int32_t delta)
{
    if (_kind == Arithmetic)
        _unknown = true;
    else if (_kind == Identity)
...
void TR_InductionVariableAnalysis::DeltaInfo::merge(DeltaInfo* other)
{
    if (other->isUnknownValue() || getKind() != other->getKind() || getDelta() != other->getDelta()) {
        setUnknownValue();
    }
}

void TR_InductionVariableAnalysis::printDeltaInfo(DeltaInfo* info)
{
    if (!trace())
        return;
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(TR_BasicInductionVariable* biv, TR::Block* branchBlock,
    TR::Node* exitBound, TR::ILOpCodes exitOp, TR::Compilation* c, TR::Optimizer* opt,
    bool usesUnchangedValueInLoopTest, bool trace)
    : TR_BasicInductionVariable(c, biv)
    , _branchBlock(branchBlock)
    , _exitBound(exitBound)
    , _exitOp(exitOp)
    , _iterCount(-1)
{
    _usesUnchangedValueInLoopTest = usesUnchangedValueInLoopTest;

    if (getEntryValue() && getEntryValue()->getOpCode().isLoadConst()
        ...
        int64_t exitVal = getExitBound()->getOpCode().isUnsigned() ? getExitBound()->get64bitIntegralValueAsUnsigned()
                                                                   : getExitBound()->get64bitIntegralValue();
        ...
        int64_t entryVal = getEntryValue()->getOpCode().isUnsigned()
            ? getEntryValue()->get64bitIntegralValueAsUnsigned()
            : getEntryValue()->get64bitIntegralValue();

        int32_t diff
            = (int32_t)exitVal - (int32_t)entryVal + (getDeltaOnBackEdge() - getDeltaOnExitEdge()); // FIXME: overflow
...
int32_t TR_IVTypeTransformer::perform()
{
    ...
    for (nextDoWhileLoop = doWhileLoopsIt.getFirst(); nextDoWhileLoop != NULL;
         nextDoWhileLoop = doWhileLoopsIt.getNext()) {
...
const char* TR_IVTypeTransformer::optDetailString() const throw()
{
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure* natLoop)
{
    ...
    do {
        ...
        if (node->getOpCodeValue() == TR::astore
            && (node->getSymbolReference() == firstChildSymRef || node->getSymbolReference() == secondChildSymRef)
    ...
    while (!SGNQueue.IsEmpty()) {
        ...
        if (auto region = structure->asRegion()) {
            ...
            for (tt = trblock->getEntry(); tt && tt != exitTT; tt = tt->getNextTreeTop()) {
                ...
                if (node->getOpCode().isLikeDef() && node->getOpCode().hasSymbolReference()) {
     ...
     */
    auto cmpChildWithEndAddr = (astoreNode->getSymbolReference() == firstChildSymRef) ? backEdgeIfNode->getChild(1)
                                                                                      : backEdgeIfNode->getChild(0);
    auto ptrTestChild = (astoreNode->getSymbolReference() == firstChildSymRef) ? backEdgeIfNode->getChild(0)
    ...
    auto baseStoreTT = TR::TreeTop::create(
        cm, TR::Node::createStore(_baseSymRef, TR::Node::createWithSymRef(TR::aload, 0, _addrSymRef)));
    _intIdxSymRef = cm->getSymRefTab()->createTemporary(
        cm->getMethodSymbol(), comp()->target().is64Bit() ? TR::Int64 : TR::Int32);
    auto intIdxStoreTT = TR::TreeTop::create(cm,
        TR::Node::createStore(_intIdxSymRef, comp()->target().is64Bit() ? TR::Node::lconst(0) : TR::Node::iconst(0)));
    ...
    if (!performTransformation(cm, "%s Adding int increment tree\n", optDetailString()))
        return;
    auto intIncrementTT = TR::TreeTop::create(cm,
        TR::Node::createStore(_intIdxSymRef,
            TR::Node::create(comp()->target().is64Bit() ? TR::ladd : TR::iadd, 2, TR::Node::createLoad(_intIdxSymRef),
    ...
    auto a2intOp
        = TR::ILOpCode::getProperConversion(TR::Address, comp()->target().is64Bit() ? TR::Int64 : TR::Int32, false);
    TR::Node* newIf;
    if (astoreNode->getSymbolReference() == firstChildSymRef) // Match if children to original backedgeIf children
    {
        newIf = TR::Node::createif(getIntegralIfOpCode(backEdgeIfNode->getOpCodeValue(), comp()->target().is64Bit()),
            ...
            TR::Node::create(comp()->target().is64Bit() ? TR::lsub : TR::isub, 2,
                ...
                TR::Node::create(a2intOp, 1, TR::Node::createLoad(_baseSymRef))),
        ...
        newIf = TR::Node::createif(getIntegralIfOpCode(backEdgeIfNode->getOpCodeValue(), comp()->target().is64Bit()),
            TR::Node::create(comp()->target().is64Bit() ? TR::lsub : TR::isub, 2,
                TR::Node::create(a2intOp, 1, cmpChildWithEndAddr),
                TR::Node::create(a2intOp, 1, TR::Node::createLoad(_baseSymRef))),
            intNodeForBackEdgeTest, backEdgeIfNode->getBranchDestination());
    }
...
void TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node* node)
{
    ...
    if (child && child->getOpCodeValue() == TR::aload && child->getSymbolReference() == _addrSymRef
        && performTransformation(
            comp(), "%s Replacing n%in aload with base int-index form\n", optDetailString(), child->getGlobalIndex())) {
        auto arrayRef = TR::Node::recreateWithoutProperties(child, comp()->target().is64Bit() ? TR::aladd : TR::aiadd,
            2, TR::Node::createLoad(_baseSymRef), TR::Node::createLoad(_intIdxSymRef));
    }
    for (int i = 0; i < node->getNumChildren(); i++) {
...
bool TR_IVTypeTransformer::isInAddrIncrementForm(TR::Node* node, int32_t& increment)
{
    ...
    if (node->getOpCodeValue() == TR::astore && node->getChild(0)->getOpCode().isArrayRef()
        && (load = node->getChild(0)->getChild(0)) && load->getOpCode().isLoadVar()
        && load->getOpCode().hasSymbolReference() && load->getSymbolReference() == node->getSymbolReference()
...
TR::SymbolReference* TR_IVTypeTransformer::findComparandSymRef(TR::Node* node)
{
    ...
    if (node->getOpCode().isArrayRef() && node->getChild(0)->getOpCode().hasSymbolReference())
