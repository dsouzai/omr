...
TR_LocalLiveRangeReduction::TR_LocalLiveRangeReduction(TR::OptimizationManager* manager)
    ...
    , _movedTreesList(trMemory())
    , _depPairList(trMemory())
{ }
...
void TR_LocalLiveRangeReduction::postPerformOnBlocks() { comp()->incVisitCount(); }
...
int32_t TR_LocalLiveRangeReduction::perform()
{
    ...
    for (tt = comp()->getStartTree(); tt; tt = nextTT) {
        ...
        while ((nextTT = exitTT->getNextTreeTop())
            && (b = nextTT->getNode()->getBlock(), b->isExtensionOfPreviousBlock())) {
...
bool TR_LocalLiveRangeReduction::transformExtendedBlock(TR::TreeTop* entryTree, TR::TreeTop* exitTree)
{
    ...
    // Note that since the list adds elements to the head, we can iterate the list, and by this to cover chain of moves
    // (the later one will be in front) e.g. if a blocked by b and then b moved after c. The list will be b-c;a-b
...
void TR_LocalLiveRangeReduction::collectRefInfo(
    TR_TreeRefInfo* treeRefInfo, TR::Node* node, vcount_t compVisitCount, int32_t* maxRefCount)
{
...
bool TR_LocalLiveRangeReduction::isWorthMoving(TR_TreeRefInfo* tree)
{
    ...
    if (((numLastRefNodesInt < numFirstRefNodesInt) && (numLastRefNodesFloat <= numFirstRefNodesFloat))
        || ((numLastRefNodesFloat < numFirstRefNodesFloat) && (numLastRefNodesInt <= numFirstRefNodesInt)))
        return true;
...
void TR_LocalLiveRangeReduction::populatePotentialDeps(TR_TreeRefInfo* treeRefInfo, TR::Node* node)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (opCode.isCall() || opCode.isResolveCheck() || opCode.isStore()
...
bool TR_LocalLiveRangeReduction::isAnyDataConstraint(
    TR_TreeRefInfo* currentTreeRefInfo, TR_TreeRefInfo* movingTreeRefInfo)
{
...
static bool mayBeObjectHeaderStore(TR::Node* node, TR_FrontEnd* fe)
{
    ...
    TR_ASSERT(
        fe->getObjectHeaderSizeInBytes() <= TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), "assertion failure");
...
bool TR_LocalLiveRangeReduction::isAnySymInDefinedOrUsedBy(
    TR_TreeRefInfo* currentTreeRefInfo, TR::Node* currentNode, TR_TreeRefInfo* movingTreeRefInfo)
{
    ...
    //
    if (movingNode->canGCandReturn() || currentNode->canGCandReturn()) {
        if (trace())
    ...
    //
    if ((currentNode->getOpCode().isWrtBar() || currentNode->canCauseGC())
        && mayBeObjectHeaderStore(movingNode, fe())) {
        if (trace())
    ...
    if ((comp()->target().cpu.isPower() || comp()->target().cpu.isARM64())
        ...
        if (movingNode->getOpCodeValue() == TR::treetop && movingNode->getFirstChild()->getOpCode().isNew()
            && (currentNode->getAllocation() == NULL || currentNode->getAllocation() == movingNode->getFirstChild())) {
            if (trace()) {
        ...
        // Exclude all indirect stores, they may be for stack allocs, in which case the flush is needed at least as a
        // scheduling barrier Direct stores to autos and parms are the only safe candidates
        if (movingNode->getOpCode().isStoreIndirect()
            || (movingNode->getOpCode().isStoreDirect() && !movingNode->getSymbol()->isParm()
                && !movingNode->getSymbol()->isAuto())) {
            if (trace())
                traceMsg(comp(), "cannot move %p beyond flush %p - (flush for possible stack alloc)", movingNode,
                    currentNode);
            return true;
        }
    }
            ...
            if (child->exceptionsRaised()
                ...
                if (trace())
                    traceMsg(comp(), "cannot move %p beyond %p - cannot change evaluation point of %p\n ", movingNode,
                        currentTreeRefInfo->getTreeTop()->getNode(), child);
                return true;
            }
                ...
                if ((comp()->getSymRefTab()->isNonHelper(stSymRefNum, TR::SymbolReferenceTable::vftSymbol))
                    || (comp()->getSymRefTab()->isNonHelper(
                        stSymRefNum, TR::SymbolReferenceTable::contiguousArraySizeSymbol))
                    || (comp()->getSymRefTab()->isNonHelper(
                        stSymRefNum, TR::SymbolReferenceTable::discontiguousArraySizeSymbol))
                ...
                if (trace())
                    traceMsg(comp(), "cannot move %p beyond %p - node %p under ResolveOrNullCheck", movingNode,
                        currentTreeRefInfo->getTreeTop()->getNode(), currentNode);
                return true;
            }

            else if (comp()->target().is64Bit() && movingNode->getOpCode().isBndCheck()
                && ((opCode.getOpCodeValue() == TR::i2l) || (opCode.getOpCodeValue() == TR::iu2l))
                && !child->isNonNegative()) {
                if (trace())
                    traceMsg(comp(),
                        "cannot move %p beyond %p - changing the eval point of %p will casue extra cg instruction ",
...
bool TR_LocalLiveRangeReduction::moveTreeBefore(TR_TreeRefInfo* treeToMove, TR_TreeRefInfo* anchor, int32_t passNumber)
{
    ...
    // verifier
    {
        ...
        int32_t maxRefCount = 0;
        TR::TreeTop* tt;
        TR_TreeRefInfo** treesRefInfoArrayTemp
            = (TR_TreeRefInfo**)trMemory()->allocateStackMemory(_numTreeTops * sizeof(TR_TreeRefInfo*));
        ...
        for (int32_t i = 0; i < _numTreeTops - 1; i++) {
            if (!verifyRefInfo(
                    treesRefInfoArrayTemp[i]->getFirstRefNodesList(), _treesRefInfoArray[i]->getFirstRefNodesList())) {
                printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
                TR_ASSERT(
                    0, "fail to verify firstRefNodesList for %p\n", _treesRefInfoArray[i]->getTreeTop()->getNode());
            }

            if (!verifyRefInfo(
                    treesRefInfoArrayTemp[i]->getMidRefNodesList(), _treesRefInfoArray[i]->getMidRefNodesList())) {
                printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
                TR_ASSERT(0, "fail to verify midRefNodesList for %p\n", _treesRefInfoArray[i]->getTreeTop()->getNode());
            }

            if (!verifyRefInfo(
                    treesRefInfoArrayTemp[i]->getLastRefNodesList(), _treesRefInfoArray[i]->getLastRefNodesList())) {
                printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
                TR_ASSERT(
                    0, "fail to verify lastRefNodesList for %p\n", _treesRefInfoArray[i]->getTreeTop()->getNode());
            }
        }
...
// If there are several nodes with "first reference" in the tree to move, stop before the first tree which contain a
// loast reference to one of those nodes.
...
void TR_LocalLiveRangeReduction::updateRefInfo(
    TR::Node* n, TR_TreeRefInfo* currentTreeRefInfo, TR_TreeRefInfo* movingTreeRefInfo, bool underEval)
{
