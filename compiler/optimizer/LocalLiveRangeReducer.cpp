...
void TR_LocalLiveRangeReduction::collectRefInfo(TR_TreeRefInfo *treeRefInfo, TR::Node *node, vcount_t compVisitCount,
    int32_t *maxRefCount)
{
...
bool TR_LocalLiveRangeReduction::isAnyDataConstraint(TR_TreeRefInfo *currentTreeRefInfo,
...
static bool mayBeObjectHeaderStore(TR::Node *node, TR_FrontEnd *fe)
{
    ...
    TR_ASSERT(fe->getObjectHeaderSizeInBytes() <= TR::Compiler->om.contiguousArrayHeaderSizeInBytes(),
        "assertion failure");
        ...
        if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
            ...
            if (child->exceptionsRaised()
                ...
                if ((comp()->getSymRefTab()->isNonHelper(stSymRefNum, TR::SymbolReferenceTable::vftSymbol))
                    || (comp()->getSymRefTab()->isNonHelper(stSymRefNum,
...
bool TR_LocalLiveRangeReduction::moveTreeBefore(TR_TreeRefInfo *treeToMove, TR_TreeRefInfo *anchor, int32_t passNumber)
{
    ...
    // verifier
    {
        ...
        for (int32_t i = 0; i < _numTreeTops - 1; i++) {
            if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getFirstRefNodesList(),
                    _treesRefInfoArray[i]->getFirstRefNodesList())) {
            ...
            if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getMidRefNodesList(),
                    _treesRefInfoArray[i]->getMidRefNodesList())) {
                printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
                TR_ASSERT(0, "fail to verify midRefNodesList for %p\n", _treesRefInfoArray[i]->getTreeTop()->getNode());
            }

            if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getLastRefNodesList(),
                    _treesRefInfoArray[i]->getLastRefNodesList())) {
                printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
...
void TR_LocalLiveRangeReduction::updateRefInfo(TR::Node *n, TR_TreeRefInfo *currentTreeRefInfo,
    TR_TreeRefInfo *movingTreeRefInfo, bool underEval)
{
