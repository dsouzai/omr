...
static void checkForNonNegativeAndOverflowProperties(
    OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* constraint = NULL)
{
    if (!constraint) {
        bool isGlobal;
    ...
    if (node->getOpCode().isIndirect()) {
        ...
        for (; !baseExpression;) {
            ...
            if (!constraint) {
                if (vp->trace())
                    traceMsg(
                        vp->comp(), "  - FAIL: %s %p has no constraint\n", curNode->getOpCode().getName(), curNode);
                break;
                ...
                if (curNode->getOpCodeValue() == TR::loadaddr
                    ...
                    bool didSomething = TR::TransformUtil::transformIndirectLoadChainAt(
                        vp->comp(), node, curNode, &clazz, &nodeToRemove);
                    if (nodeToRemove) {
                        TR_ASSERT(didSomething,
                            "How can there be a node to remove if transformIndirectLoadChain didn't do something??");
                        vp->removeNode(nodeToRemove, true);
        ...
        if (baseExpression) {
            ...
            //
            TR::Node* nodeToRemove = NULL;
            bool didSomething = TR::TransformUtil::transformIndirectLoadChain(
                vp->comp(), node, baseExpression, baseKnownObject, &nodeToRemove);
            if (nodeToRemove) {
                TR_ASSERT(didSomething,
                    "How can there be a node to remove if transformIndirectLoadChain didn't do something??");
                vp->removeNode(nodeToRemove, true);
...
bool reduceLongOpToIntegerOp(OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* nodeConstraint)
{
    ...
    if (performTransformation(
            vp->comp(), "%sReduce %s (0x%p) to integer arithmetic\n", OPT_DETAILS, node->getOpCode().getName(), node)) {
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            ...
            if (canMoveLongOpChildDirectly(node, i, arithNode)) {
                ...
                dumpOptDetails(
                    vp->comp(), "    Transfer integer child %d %s (0x%p)\n", i, child->getOpCode().getName(), child);
                ...
                TR::Node* newChild = TR::Node::create(
                    node, TR::ILOpCode::getProperConversion(child->getDataType(), arithNode->getDataType(), false), 1);
        ...
        vp->prepareToReplaceNode(
            node, TR::ILOpCode::getProperConversion(arithNode->getDataType(), node->getDataType(), false));
...
TR::Node* constrainAnyIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
            ...
            if (arrayVPConstraint) {
                ...
                if (knot && kobj) {
                    ...
                    if (kobj->isArrayWithConstantElements(vp->comp()) && kobj->isPrimitiveArray(vp->comp())) {
                        TR::VMAccessCriticalSection constrainAnyIntLoadCriticalSection(
    ...
    if (node->isNonNegative())
        vp->addBlockConstraint(
            node, TR::VPIntRange::create(vp, 0, static_cast<int32_t>(TR::getMaxSigned<TR::Int32>())));
...
TR::Node* constrainShortLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->isNonNegative())
        vp->addBlockConstraint(
            node, TR::VPShortRange::create(vp, 0, static_cast<int32_t>(TR::getMaxSigned<TR::Int16>())));
...
TR::Node* constrainLload(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    constrainRangeByPrecision(
        TR::getMinSigned<TR::Int64>(), TR::getMaxSigned<TR::Int64>(), TR_MAX_DECIMAL_PRECISION, lo, hi);
...
static bool addKnownObjectConstraints(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (objectReferenceLocation) {
...

        {
            ...
            uintptr_t objectReference
                = vp->comp()->fej9()->getStaticReferenceFieldAtAddress((uintptr_t)objectReferenceLocation);
        ...
        if (isString && symRef->getSymbol()->isStatic()) {
            ...
            vp->addGlobalConstraint(node,
                TR::VPClass::create(vp, TR::VPConstString::create(vp, symRef), TR::VPNonNullObject::create(vp), NULL,
                    NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::HeapObject)));
        } else if (jlClass) // without a jlClass, we can't tell what kind of constraint to add
        {
            ...
            if (isFixedJavaLangClass) {
                if (performTransformation(vp->comp(),
                        ...
                        node, classSig, knownObjectIndex)) {
                    constraint
                ...
                if (performTransformation(vp->comp(),
                        ...
                        node, knownObjectIndex, classSig)) {
                    constraint = TR::VPClass::create(vp, TR::VPKnownObject::create(vp, knownObjectIndex),
                        TR::VPNonNullObject::create(vp), NULL, NULL,
                                ...
                                if (elementSize != 0) {
                                    constraint = TR::VPClass::create(vp, (TR::VPClassType*)constraint, NULL, NULL,
                                        TR::VPArrayInfo::create(vp, 0,
                                            elementSize == 0 ? TR::getMaxSigned<TR::Int32>()
...
TR::Node* constrainIaload(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        {
            TR::VMAccessCriticalSection constrainIaloadCriticalSection(
                vp->comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
            if (constrainIaloadCriticalSection.hasVMAccess()) {
                ...
                if (baseObject) {
                    ...
                    if (recognizedClassName) {
                        TR_OpaqueClassBlock* recognizedClazz = vp->fe()->getClassFromSignature(
                            recognizedClassName, recognizedClassNameLength, symRef->getOwningMethod(vp->comp()));
                        if (recognizedClazz
        ...
        case TR::Symbol::Java_lang_invoke_MethodHandle_thunks:
            if (!node->isNonNull()
                && performTransformation(vp->comp(), "%s[%p] recognized field is never null: %s\n", OPT_DETAILS, node,
                    symRef->getName(vp->comp()->getDebug()))) {
                vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
                node->setIsNonNull(true);
            }
            break;
                ...
                if (callSiteConstraint && callSiteConstraint->getKnownObject()) {
                    ...
                    if (bypassOnThisRun) {
...

                        {
                            ...
                            if (constrainIaloadCriticalSection.hasVMAccess()) {
                                ...
                                if (!bypassLocation
                                    ...
                                    bypassLocation
                                        = vp->comp()->fej9()->mutableCallSite_findOrCreateBypassLocation(*siteLocation);
                                }
                            } else {
                                if (vp->trace())
                                    traceMsg(vp->comp(),
                        ...
                        if (bypassLocation
                            ...
                            node->setNumChildren(0);
                            node->setSymbolReference(vp->comp()->getSymRefTab()->createKnownStaticReferenceSymbolRef(
                                bypassLocation, targetKOI));
                            return node;
                        }
                    }
                }
    ...
    if (base && base->getClass() && !vp->comp()->getOption(TR_DisableMarkingOfHotFields)
        ...
        vp->comp()->fej9()->markHotField(
            vp->comp(), node->getSymbolReference(), base->getClass(), base->isFixedClass());
    }
        ...
        if (loadNode->getOpCode().isLoadVarDirect() && loadNode->getSymbol() == storeSymbol) {
            ...
            for (bni = vp->_booleanNegationInfo.getFirst(); bni; bni = bni->getNext()) {
                if (bni->_storeValueNumber == loadVN) {
                    ...
                    //
                    if (performTransformation(
                            vp->comp(), "%sRemoving double boolean negation at [%p]\n", OPT_DETAILS, node)) {
...
void canRemoveWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (constraint) {
        if (constraint->isNullObject() && TR::Compiler->om.writeBarrierType() != gc_modron_wrtbar_always
            ...
            if (node->getOpCode().isIndirect()) {
                if (performTransformation(
                        vp->comp(), "%sChanging write barrier store into iastore [%p]\n", OPT_DETAILS, node)) {
                ...
                if (performTransformation(
                        vp->comp(), "%sChanging write barrier store into astore [%p]\n", OPT_DETAILS, node)) {
    ...
    if (doOpt && ((gcMode == gc_modron_wrtbar_cardmark_and_oldcheck) || (gcMode == gc_modron_wrtbar_oldcheck))
        ...
        if (removeWrtBar
            && performTransformation(
                vp->comp(), "%sChanging wrtbar to store because the rhs is a new object [%p]\n", OPT_DETAILS, node)) {
            ...
            if (useDefInfo) {
                ...
                if (useDefInfo->isUseIndex(useIndex)) {
                    ...
                    if (useDefInfo->getUseDef(defs, useIndex)) {
                        ...
                        if (setStackWrtBar
                            && performTransformation(
                                vp->comp(), "%sSetting wrtbar flag to assume stack object [%p]\n", OPT_DETAILS, node)) {
    ...
    if (node->getOpCode().isWrtBar()) {
        ...
        if (constraint) {
            if (constraint->isHeapObject() == TR_yes
                && performTransformation(
                    vp->comp(), "%sMarking the wrtbar node [%p] - destination is a heap object\n", OPT_DETAILS, node)) {
...
TR::Node* constrainMonent(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (constraint && constraint->getClass()) {
        ...
        if ((clazz || !node->hasMonitorClassInNode())
            && (performTransformation(
                vp->comp(), "%sSetting type on MONENTER node [%p] to [%p]\n", OPT_DETAILS, node, clazz)))
...
TR::Node* constrainMonexit(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (constraint && constraint->getClass()) {
        ...
        if ((clazz || !node->hasMonitorClassInNode())
            && performTransformation(
                vp->comp(), "%sSetting type on MONEXIT  node [%p] to [%p]\n", OPT_DETAILS, node, clazz))
            node->setMonitorClassInNode(clazz);
    }
...
TR::Node* constrainThrow(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    for (TR::Block* catchBlock = oehi.getFirst(); catchBlock; catchBlock = oehi.getNext()) {
        ...
        if (catchBlock->getCatchType() == 0) {
            ...
            willCatch = vp->fe()->isInstanceOf(
                constraint->getClass(), catchBlock->getExceptionClass(), constraint->isFixedClass());
            if (willCatch == TR_yes)
...
TR::Node* constrainInstanceOf(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (objectConstraint) {
        ...
        if (objectConstraint->isNullObject()) {
            ...
            TR_YesNoMaybe isInstance = vp->fe()->isInstanceOf(
                objectClass, castClass, objectConstraint->isFixedClass(), castConstraint->isFixedClass(), !disabled);
                    ...
                    if (jlClass) {
                        ...
                        if (classType->getClass() == jlClass) {
                            ...
                            if ((classType->isClassObject() == TR_maybe)
                                ...
                                newClassType = TR::VPResolvedClass::create(
                                    vp, classType->getClass()); // Do not want to get the 'fixed' property
                        }
                    }
                } else {
                    ...
                    if (useDefInfo->getUseDef(defs, useIndex)) {
                        ...
                        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                            ...
                            if (defNode && (defNode->getOpCodeValue() == TR::astore)) {
                                ...
                                if (methodSig
                                    && !strncmp(
                                        methodSig, "java/util/HashMap.get(Ljava/lang/Object;)Ljava/lang/Object;", 59)) {
            ...
            if (!classSymRef->isUnresolved()) {
                ...
                if (classSym) {
                    ...
                    if (vp->cg()->isObjectOfSizeWorthAligning(size)) {
                        vp->comp()->fej9()->markClassForTenuredAlignment(
                            vp->comp(), (TR_OpaqueClassBlock*)classSym->getStaticAddress(), 0);
                    }
                }
            }
        }
...
TR::Node* constrainNewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (sizeConstraint)
        vp->addGlobalConstraint(
            node, TR::VPArrayInfo::create(vp, sizeConstraint->getLowInt(), sizeConstraint->getHighInt(), elementSize));
    else
        vp->addGlobalConstraint(
            node, TR::VPArrayInfo::create(vp, 0, static_cast<int32_t>(TR::getMaxSigned<TR::Int32>()), elementSize));
...
TR::Node* constrainMultiANewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    // remove the rest of the block.
    //
    int32_t maxSize = (int32_t)TR::Compiler->om.maxArraySizeInElementsForAllocation(
        node, vp->comp()); // int32_t is ok because multiANewArray is only useful in Java, where array sizes are bound
                           // by TR::getMaxSigned<TR::Int32>()
    int64_t maxHeapSize;
...
void addValidRangeBlockOrGlobalConstraint(
    OMR::ValuePropagation* vp, TR::Node* node, FUNC createRange, FUNC2 processValue, T low, T high, bool childGlobal)
{
...
TR::Node* constrainArraylength(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (elementSize > 1) {
        ...
        if (maxSize < upperBoundLimit) {
            TR_ASSERT(
                0 <= maxSize && maxSize <= TR::getMaxSigned<TR::Int32>(), "arraylength must be within Int32 range");
    ...
    if (!node->getArrayStride()
        && performTransformation(
            vp->comp(), "%sSetting element width for array [%p] to %d\n", OPT_DETAILS, node, elementSize)) {
...
static void devirtualizeCall(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (!constraint || !(thisType = constraint->getClass())) {
        ...
        if (!dontProfileMore && (vp->comp()->getMethodHotness() == hot) && vp->comp()->getRecompilationInfo()
            ...
            dumpOptDetails(
                vp->comp(), "%s enabling profiling: would be useful to profile arraycopy calls\n", OPT_DETAILS);
        }
...
TR::Node* constrainCall(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (vp->lastTimeThrough() && vp->_isGlobalPropagation) {
        ...
        if (it != map->end()) {
            ...
            for (; pair; pair = iter.getNext()) {
                ...
                TR_ASSERT_FATAL(
                    guardNode->isTheVirtualGuardForAGuardedInlinedCall() && guardNode->isNonoverriddenGuard(),
    ...
    if (symbol) {
        ...
        if (isIntegerAbs || isLongAbs) {
            ...
            if (jitHelpersClass && TR::Compiler->cls.isClassInitialized(vp->comp(), jitHelpersClass)) {
                ...
                for (TR_ResolvedMethod* m = it.getCurrent(); m; m = it.getNext()) {
                    ...
                    if (!strncmp(sig, "hashCodeImpl", 11)) {
                        hashCodeMethodSymRef = vp->comp()->getSymRefTab()->findOrCreateMethodSymbol(
                            JITTED_METHOD_INDEX, -1, m, TR::MethodSymbol::Virtual);
                        ...
                        getHelpersSymRef = vp->comp()->getSymRefTab()->findOrCreateMethodSymbol(
                            JITTED_METHOD_INDEX, -1, m, TR::MethodSymbol::Static);
                    }
                }

                if (vp->comp()->getOption(TR_EnableJITHelpershashCodeImpl) && hashCodeMethodSymRef && getHelpersSymRef
                    && performTransformation(
                        vp->comp(), "%sChanging call to new hashCodeImpl at node [%p]\n", OPT_DETAILS, node)) {
            ...
            Method -728] n577n         aload  java/lang/invoke/MethodHandle$UnsafeGetter.myUnsafe n579n         iaload
    ...
    if (syncRel && syncRel->constraint)
        sync = syncRel->constraint->asVPSync();
    if (sync && sync->syncEmitted() == TR_yes) {
        vp->addConstraintToList(
            NULL, vp->_syncValueNumber, vp->AbsoluteConstraint, TR::VPSync::create(vp, TR_maybe), &vp->_curConstraints);
        if (vp->trace()) {
            traceMsg(vp->comp(), "Setting syncRequired due to node [%p]\n", node);
        }
    } else {
    ...
    if (lhs && rhs) {
        ...
        if (lhs->asLongConst() && rhs->asLongConst()) {
            ...
            if (!(can64BitMultiplyOverflow(lhs->getLowLong(), rhs->getLowLong(), node->getOpCode().isUnsigned())
                    ...
                    || can64BitMultiplyOverflow(
                        lhs->getHighLong(), rhs->getHighLong(), node->getOpCode().isUnsigned()))) {
...
TR::Node* constrainLdiv(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
        ...
        if (vp->lastTimeThrough() && !isUnsigned && lhsConstraint
            ...
            && performTransformation(
                vp->comp(), "%sChange node [" POINTER_PRINTF_FORMAT "] ldiv->i2l of idiv\n", OPT_DETAILS, node)) {
            ...
            if (rhsLow >= 1 && rhsLow <= rhsHigh && lhsLow <= lhsHigh) {
                ...
                vp->addBlockOrGlobalConstraint(
                    node, TR::VPLongRange::create(vp, lhsLow / rhsHigh, lhsHigh / rhsLow), lhsGlobal);
            }
        }
    }
...
static TR::Node* distributeShift(
    OMR::ValuePropagation* vp, TR::Node* node, int32_t shiftAmount, TR::VPConstraint*& constraint)
{
    ...
    if ((nodeOp.isAdd() || nodeOp.isSub()) && node->getFirstChild()->cannotOverflow()) {
        ...
        if (replaceNode
            && performTransformation(
                vp->comp(), "%sDistributing shift across add/sub [%p]\n", OPT_DETAILS, node->getFirstChild())) {
            ...
            if (c) {
                if (node->getOpCodeValue() == TR::iushr) {
                    if (c->getLowInt() >= 0)
                        constraint = TR::VPIntRange::create(
                            vp, ((uint32_t)c->getLowInt()) >> shiftAmount, ((uint32_t)c->getHighInt()) >> shiftAmount);
                    else if (c->getHighInt() < 0)
                        constraint = TR::VPIntRange::create(
                            vp, ((uint32_t)c->getHighInt()) >> shiftAmount, ((uint32_t)c->getLowInt()) >> shiftAmount);
...
TR::Node* constrainIshr(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (firstChild->isNonNegative() /*&& firstChild->getType().isSignedInt()*/ && vp->lastTimeThrough()
        && performTransformation(
            vp->comp(), "%sChange node [" POINTER_PRINTF_FORMAT "] ishr->iushr\n", OPT_DETAILS, node)) {
...
TR::Node* constrainIand(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (canBeRemoved
        && performTransformation(
            vp->comp(), "%sRemoving redundant node [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        TR::Node* firstChild = node->getFirstChild();
...
static TR::Node* constrainWidenToLong(
    OMR::ValuePropagation* vp, TR::Node* node, int64_t low, int64_t high, bool isUnsigned)
{
    if (findConstant(vp, node))
        return node;
    constrainChildren(vp, node);
...
static TR::Node* constrainNarrowIntValue(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(
        node, (uint64_t)high - (uint64_t)low == origDiff, "truncated range is not the same size as the original range");
...
static void generateModifiedNopGuard(OMR::ValuePropagation* vp, TR::Node* guardNode, TR_VirtualGuardKind kind)
{
    ...
    //
    TR_ASSERT_FATAL_WITH_NODE(
        guardNode, comp->getInlineDepth() == 0, "trying to upgrade to a nop guard: VP is running during inlining");
    ...
    case TR_NonoverriddenGuard: {
        newGuardNode = TR_VirtualGuard::createNonoverriddenGuard(TR_NonoverriddenGuard, comp, calleeIndex, fakeCallNode,
            guardNode->getBranchDestination(), inlinedMethodSym, true /* forInline */);
    ...
    case TR_HierarchyGuard: {
        ...
        TR_ASSERT_FATAL_WITH_NODE(guardNode, expectedClass == thisClass, "VFT test class %p differs from thisClass %p",
            expectedClass, thisClass);

        newGuardNode = TR_VirtualGuard::createVftGuardWithReceiver(TR_HierarchyGuard, comp, calleeIndex, fakeCallNode,
            guardNode->getBranchDestination(), thisClass, receiverNode);
    ...
    case TR_HCRGuard: {
        ...
        TR_ASSERT_FATAL_WITH_NODE(
            guardNode, thisClass == inlinedResolvedMethod->containingClass(), "HCR assumption class mismatch");
        ...
        newGuardNode = TR_VirtualGuard::createHCRGuard(
            comp, calleeIndex, fakeCallNode, guardNode->getBranchDestination(), inlinedMethodSym, thisClass);
...
static TR::VPConstraint* passingTypeTestObjectConstraint(
    OMR::ValuePropagation* vp, TR::VPConstraint* classConstraint, bool testingForFixedType, bool constrainVft)
{
    TR_ASSERT_FATAL(
        classConstraint->isClassObject() == TR_yes, "expected a instanceof classConstraint to be a 'ClassObject'");
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if (cannotBranch
        && performTransformation(
            vp->comp(), "%sRemoving conditional branch [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
    ...
    if (cannotFallThrough
        && performTransformation(
            vp->comp(), "%sChanging node [%p] %s into goto\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        vp->printEdgeConstraints(vp->createEdgeConstraints(edge, false));
                ...
                if (typeConstraint) {
                    ...
                    if (!vp->comp()->getGenerateReadOnlyCode() && clazzToBeInitialized
                        && (((clazzNameLen == 35)
                            ...
                            || ((clazzNameLen == 41)
                                && !strncmp(
                                    clazzToBeInitialized, "Lcom/ibm/websphere/ras/TraceEnabledToken;", clazzNameLen))
                            || ((clazzNameLen == 40)
                                && !strncmp(
                                    clazzToBeInitialized, "Ljava/lang/String$StringCompressionFlag;", clazzNameLen)))
                        && performTransformation(vp->comp(),
    ...
    if (node->getOpCodeValue() == TR::ifacmpne && virtualGuard != NULL) {
        ...
        case TR_VftTest: {
            ...
            if (ignoreVirtualGuard && !cannotFallThrough && !cannotBranch && classChild->getOpCodeValue() == TR::aconst
        ...
        case TR_MethodTest: {
            ...
            TR_ASSERT_FATAL_WITH_NODE(
                methodPtrNode, expectedMethodAddr != 0, "method test method pointer constant is null");
    ...
    if (instanceofConstraint && instanceofOnBranch) {
        TR::VPConstraint* newConstraint = passingTypeTestObjectConstraint(
            vp, instanceofConstraint, instanceofDetectedAndFixedType, instanceofObjectRefIsVft);
...
//
static TR::Node* constrainIfcmplessthan(
    OMR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool orEqual)
{
    ...
    if (lhsChild->getOpCode().isLong()) {
        ...
        if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
            if (!vp->addEdgeConstraint(
                    lhsChild, TR::VPLongRange::create(vp, minLongValue, longBoundary), edgeConstraints)) {
                if (!vp->intersectionFailed())
                    cannotBranch = true;
                else
                    vp->setIntersectionFailed(false);
    ...
    if (lhsChild->getOpCode().isLong() && !isUnsigned) {
        ...
        if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
            if (!vp->addBlockConstraint(
                    rhsChild, TR::VPLongRange::create(vp, TR::getMinSigned<TR::Int64>(), longBoundary), NULL, false)) {
                if (!vp->intersectionFailed())
                    cannotFallThrough = true;
                else
                    vp->setIntersectionFailed(false);
        ...
        if (longBoundary != TR::getMinSigned<TR::Int64>()) {
            if (!vp->addBlockConstraint(
                    lhsChild, TR::VPLongRange::create(vp, longBoundary, TR::getMaxSigned<TR::Int64>()), NULL, false)) {
                if (!vp->intersectionFailed())
                    cannotFallThrough = true;
                else
                    vp->setIntersectionFailed(false);
...
//
static TR::Node* constrainCmplessthan(
    OMR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool orEqual)
{
...
static void constrainClassObjectLoadaddr(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
    ...
    TR::VPConstraint* constraint
        = TR::VPClass::create(vp, TR::VPClassType::create(vp, symRef, true), TR::VPNonNullObject::create(vp), NULL,
...
static bool handleResolveCheck(OMR::ValuePropagation* vp, TR::Node* node, bool nullChkToo)
{
    ...
    if (child->getOpCode().isStore())
        vp->addConstraintToList(
            node, index, vp->AbsoluteConstraint, TR::VPIntConst::create(vp, 1), &vp->_curConstraints);
    else if (!rel)
        vp->addConstraintToList(
            node, index, vp->AbsoluteConstraint, TR::VPIntConst::create(vp, 0), &vp->_curConstraints);
...
TR::Node* constrainZeroChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (childConstraint) {
        ...
        if (!zeroConstraint->intersect(childConstraint, vp)) {
            // Can't be zero
            if (performTransformation(
                    vp->comp(), "%sRemoving unnecessary %s [%p]\n", OPT_DETAILS, node->getOpCode().getName(), node)) {
        ...
        if (!nonzeroConstraint->intersect(childConstraint, vp)) {
            // Can't be nonzero
            if (performTransformation(
                    vp->comp(), "%sRemoving inevitable %s [%p]\n", OPT_DETAILS, node->getOpCode().getName(), node))
                vp->mustTakeException();
        }
    }
...
TR::Node* constrainResolveNullChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (handleNullCheck(vp, node, !removeTheCheck) == 1) {
        if (removeTheCheck) {
            if (performTransformation(
                    vp->comp(), "%sChanging ResolveAndNULLCHK node into a treetop node [%p]\n", OPT_DETAILS, node)) {
                TR::Node::recreate(node, TR::treetop);
                vp->setChecksRemoved();
            }
        } else {
            if (performTransformation(
                    vp->comp(), "%sChanging ResolveAndNULLCHK node into a ResolveCHK node [%p]\n", OPT_DETAILS, node)) {
        ...
        if (performTransformation(
                vp->comp(), "%sChanging ResolveAndNULLCHK node into a NULLCHK node [%p]\n", OPT_DETAILS, node)) {
...
TR::Node* constrainBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (relativeConstraint && relativeConstraint->mustBeLessThan()
        && performTransformation(
            vp->comp(), "%sRemoving redundant bound check node (subsumed) [%p]\n", OPT_DETAILS, node)) {
        // Change this node to a treetop containing the index (which is
        // currently the second child of the bounds check).
        //
        TR::Node::recreate(node, TR::treetop);
...
TR::Node* constrainBndChkWithSpineChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (indexConstraint && objConstraint) {
        ...
        if (arrayInfo) {
            ...
            if (indexConstraint->getLowInt() >= 0 && indexConstraint->getHighInt() < lowerBoundLimit
                && performTransformation(
                    vp->comp(), "%sRemoving unnecessary bound check from composite node [%p]\n", OPT_DETAILS, node)) {
                removeBndChk = true;
            }
        }
    }
    ...
    if (!removeBndChk && relativeConstraint && relativeConstraint->mustBeLessThan()
        && performTransformation(
            vp->comp(), "%sRemoving redundant bound check from node (subsumed) [%p]\n", OPT_DETAILS, node)) {
        removeBndChk = true;
    }
...
TR::Node* constrainArrayCopyBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if ((vp->getValueNumber(lhsNode) == vp->getValueNumber(rhsNode))
        ...
        if (performTransformation(
                vp->comp(), "%sRemoving redundant arraycopy bound check node [%p]\n", OPT_DETAILS, node)) {
            vp->removeNode(node);
            vp->setChecksRemoved();
            return NULL;
        }
    ...
    if (isArrayLengthCheck) {
        ...
        vp->addBlockConstraint(
            objectRef, TR::VPArrayInfo::create(vp, lhsConstraint->getLowInt(), lhsConstraint->getHighInt(), 0));
    }
    return node;
}
...
TR::Node* constrainArrayStoreChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (storeClassForCheck != NULL) {
        if (vp->trace()) {
            traceMsg(
                vp->comp(), "Setting arrayStoreClass on ArrayStoreChk node [%p] to [%p]\n", node, storeClassForCheck);
        }
