...
TR_LiveOnNotAllPaths::TR_LiveOnNotAllPaths(
    TR::Compilation* c, TR_Liveness* liveOnSomePaths, TR_LiveOnAllPaths* liveOnAllPaths)
    : _comp(c)
{
    TR_LiveVariableInformation* liveVarInfo = liveOnSomePaths->getLiveVariableInfo();
    TR_ASSERT(
        (liveVarInfo == liveOnAllPaths->getLiveVariableInfo()), "Possibly inconsistent live variable information");
...
ListElement<TR_BlockListEntry>* TR_OrderedBlockList::addInTraversalOrder(
    TR::Block* block, bool forward, TR::CFGEdge* edge)
{
...
void TR_SinkStores::recordPlacementForDefInBlock(TR_BlockStorePlacement* blockPlacement)
{
    ...
    TR_ASSERT(
        _symbolsUsedInBlock[blockNumber] != NULL, "_symbolsUsedInBlock[%d] should have been initialized!", blockNumber);
            ...
            do {
                ...
                if (trace())
                    traceMsg(comp(), "      calling findLocalUses on node %p with treeVisitCount %d\n", tt->getNode(),
                        treeVisitCount);
                _liveVarInfo->findLocalUsesInBackwardsTreeWalk(
                    tt->getNode(), blockNumber, usedSymbols, killedSymbols, treeCommonedLoads, treeVisitCount);
                if (trace()) {
                    ...
                    while (storeElement != NULL) {
                        ...
                        if (trace()) {
                            ...
                            if (store->_movable && store->_commonedLoadsUnderTree
                                ...
                                traceMsg(
                                    comp(), "            killedSymIdx = %d and temp is not needed\n", killedSymIdx);
                            }
                        }
                        if (store->_movable && store->_commonedLoadsUnderTree
                            && store->_commonedLoadsUnderTree->get(killedSymIdx)) {
                ...
                if (canMoveStore) {
                    ...
                    movableStore = new (trStackMemory()) TR_MovableStore(
                        this, useOrKill, symIdx, myCommonedLoads, commonedLoadsAfter, treeDepth, moveStoreWithTemps);
        ...
        if (!block->isExtensionOfPreviousBlock()) {
            ...
            while (!useOrKillInfoList.isEmpty()) {
                ...
                if (isMovableStore) {
                    ...
                    TR_ASSERT(
                        useOrKill == store->_useOrKillInfo, "mismatch useOrKillInfoList in potentiallyMovableStores!");
                    ...
                    if (performTransformation(comp(),
                        ...
                        TR_ASSERT(_killedSymbolsToMove->isSet(symIdx),
                            ...
                            "(symIdx=%d)!",
                            symIdx);
                        if (sinkStorePlacement(store, movedPreviousStore)) {
                            if (trace())
                ...
                if (movedStore) {
                    ...
                    if (trace()) {
                        traceMsg(
                            comp(), "      Did not move node %p so update killed and used symbols\n", tt->getNode());
...
void TR_SinkStores::placeStoresAlongEdges(List<TR_StoreInformation>& stores, List<TR_EdgeInformation>& edges)
{
    ...
    if (!to->getExceptionPredecessors().empty()) {
        ...
        placementBlock->setHandlerInfo(
            to->getCatchType(), to->getInlineDepth(), genHandlerIndex(), to->getOwningMethod(), comp());
        ...
        TR::Node* newAthrow = TR::Node::createWithSymRef(
            TR::athrow, 1, 1, excpNode, symRefTab->findOrCreateAThrowSymbolRef(comp()->getMethodSymbol()));
    ...
    vcount_t& highVisitCount)
{
    ...
    return (symIdx >= 0 && _liveOnNotAllPaths->_outSetInfo[b]->get(symIdx)
        && treeIsSinkableStore(
            node, sinkIndirectLoads, indirectLoadCount, depth, isLoadStatic, comp()->getVisitCount()));
}
...
bool TR_SinkStores::isSafeToSinkThruEdgePlacement(
    int symIdx, TR::CFGNode* block, TR::CFGNode* succBlock, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
    ...
    int32_t sourceBlockFrequency, bool isLoadStatic, vcount_t visitCount, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
    ...
    if (containingLoop != NULL && containingLoop->getEntryBlock() == succBlock) {
        ...
        if (containingLoop->asRegion()->numSubNodes() > 1
            || !storeCanMoveThroughBlock(
                _symbolsKilledInBlock[succBlock->getNumber()], _symbolsUsedInBlock[succBlock->getNumber()], symIdx)) {
            return false;
        }
    }
...
bool TR_SinkStores::checkLiveMergingPaths(TR_BlockListEntry* blockEntry, int32_t symIdx)
{
    ...
    if (trace())
        traceMsg(
            comp(), "            Counting LONAP predecessors to compare to propagation count %d\n", blockEntry->_count);
    ...
    List<TR_MovableStore>& potentiallyMovableStores)
{
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
        TR::Node* child = node->getChild(i);
        genStoreToTempSyms(
            storeLocation, child, commonedLoadsUnderStore, killedLiveCommonedLoads, store, potentiallyMovableStores);
    }
}

// Given a dup tree, replace the symRef for all commoned loads (as in the original store) with the temp symRef
void TR_SinkStores::replaceLoadsWithTempSym(
    TR::Node* newNode, TR::Node* origNode, TR_BitVector* needTempForCommonedLoads)
{
...
bool TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore* movableStore, bool nextStoreWasMoved)
{
    ...
    while (!worklist.isEmpty()) {
        ...
        if (!checkLiveMergingPaths(blockEntry, symIdx)) {
            ...
            if (trace())
                traceMsg(
                    comp(), "              didn't reach from all live predecessors, splitting edges it did reach on\n");
                    ...
                    if (_liveOnSomePaths->_blockAnalysisInfo[succBlockNumber]->get(symIdx)
                        ...
                        //    }
                            ...
                            if (!needTempForCommonedLoads)
                                ...
                                storeInfo->_storeTemp = tt->duplicateTree();
                                replaceLoadsWithTempSym(
                                    storeInfo->_storeTemp->getNode(), tt->getNode(), needTempForCommonedLoads);
                            }
                            TR_BitVector* symsReferenced = new (trStackMemory()) TR_BitVector(*_killedSymbolsToMove);
                            (*symsReferenced) |= (*_usedSymbolsToMove);
                            TR_EdgeInformation* edgeInfo
