...
int32_t TR_FieldPrivatizer::detectCanonicalizedPredictableLoops(
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (canTransform && !loopInvariantBlock && (regionStructure->getEntryBlock() != comp()->getStartBlock())) {
        ...
        TR::Block* newBlock = TR::Block::createEmptyBlock(
            entryBlock->getEntry()->getNode(), comp(), sumPredFreq < 0 ? 0 : sumPredFreq, entryBlock);
        ...
        for (auto pred = entryBlock->getPredecessors().begin(); pred != entryBlock->getPredecessors().end();) {
            ...
            if ((predBlock != newBlock) && !blocksInRegion.find(predBlock)) {
                predBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                    comp(), entryBlock->getEntry(), newBlock->getEntry());
        ...
        TR::TreeTop* gotoTreeTop = TR::TreeTop::create(
            comp(), TR::Node::create(entryBlock->getEntry()->getNode(), TR::Goto, 0, entryBlock->getEntry()));
...
void TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR::Node* node, vcount_t visitCount)
{
    ...
    if (opCode.isStore() || opCode.isLoadVar()) {
        ...
        if ((opCode.isIndirect() || staticSym)) {
            ...
            if (canPrivatize && !sym->isArrayShadowSymbol() && !sym->isVolatile()
        ...
        if (!_fieldsThatCannotBePrivatized->get(symRef->getReferenceNumber())) {
            ...
            if (canPrivatizeBasedOnThisNode) {
                ...
                if (!autoForField) {
                    ...
                    autoForField = comp()->getSymRefTab()->createTemporary(
                        comp()->getMethodSymbol(), nodeDataType, false, symSize);
    ...
    while (currentNodeElem) {
        ...
        if (duplicateNode->getOpCode().isStore()) {
            if (duplicateNode->getOpCode().isIndirect()) {
                TR::Node::recreate(
                    duplicateNode, comp()->il.opCodeForCorrespondingIndirectStore(duplicateNode->getOpCodeValue()));
    ...
    for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
        ...
        for (auto edge = exitBlock->getSuccessors().begin(); edge != exitBlock->getSuccessors().end();) {
            ...
            if (!blocksInsideLoop->get(next->getNumber())
                ...
                // safety
                exitBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                    comp(), next->getEntry(), nnext->getEntry());
    ...
    while (currentNodeElem) {
        ...
        if (_needToStoreBack->get(currentSymRef->getReferenceNumber())) {
            if (currentNode->getOpCode().isIndirect()) {
                if (!duplicateNode->getOpCode().isStore()) {
                    TR::Node::recreate(
                        duplicateNode, comp()->il.opCodeForCorrespondingIndirectLoad(duplicateNode->getOpCodeValue()));
                }
                ...
                TR::Node* storedBackValue = TR::Node::createWithSymRef(
                    duplicateNode, comp()->il.opCodeForDirectLoad(duplicateNode->getDataType()), 0, currentSymRef);
...
bool TR_FieldPrivatizer::storesBackMustBePlacedInExitBlock(
    TR::Block* exitBlock, TR::Block* toBlock, TR_BitVector* blocksInsideLoop)
{
...
void TR_FieldPrivatizer::addPrivatizedRegisterCandidates(TR_Structure* structure)
{
    ...
    while (privatizedCandidate) {
        if (performTransformation(comp(),
                ...
                structure->getNumber()))
            privatizedCandidate->getData()->addAllBlocksInStructure(
                structure, comp(), trace() ? "privatization auto" : NULL);
        privatizedCandidate = privatizedCandidate->getNextElement();
    }
}
...
void TR_FieldPrivatizer::addStringInitialization(TR::Block* block)
{
    ...
    TR::Node* initNode = TR::Node::createWithSymRef(
        TR::New, 1, 1, classObject, comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(methodSymbol));
        ...
        for (TR_ResolvedMethod* method = it.getCurrent(); method; method = it.getNext()) {
            if (method->isConstructor()) {
                ...
                if (!strncmp(sig, "(Ljava/lang/String;)V", 21)) {
                    _initSymRef = getSymRefTab()->findOrCreateMethodSymbol(
                        JITTED_METHOD_INDEX, -1, method, TR::MethodSymbol::Special);
                    break;
                }
            }
        }
        ...
        for (TR_ResolvedMethod* method = it.getCurrent(); method; method = it.getNext()) {
            if (!strncmp(method->nameChars(), "toString", 8)) {
                ...
                if (!strncmp(sig, "()Ljava/lang/String;", 20)) {
                    _toStringSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                        JITTED_METHOD_INDEX, -1, method, TR::MethodSymbol::Virtual);
                    break;
                }
            }
        }
        ...
        for (TR_ResolvedMethod* method = it.getCurrent(); method; method = it.getNext()) {
            if ((method->nameLength() == 15) && !strncmp(method->nameChars(), "jitAppendUnsafe", 15)) {
                ...
                if (!strncmp(sig, "(C)Ljava/lang/StringBuffer;", 27)) {
                    _appendSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                        JITTED_METHOD_INDEX, -1, method, TR::MethodSymbol::Special);
                    break;
                }
            }
