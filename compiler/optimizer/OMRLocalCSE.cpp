...
int32_t OMR::LocalCSE::perform()
{
    ...
    for (tt = comp()->getStartTree(); tt; tt = exitTreeTop->getNextTreeTop()) {
        ...
        if (doExtraPassForVolatiles()) {
            if (trace())
                traceMsg(
                    comp(), "LocalCSE entering 2 pass mode for volatile elimination - pass 1 for volatiles ONLY\n");
    ...
    int32_t childNum, int32_t* nextLoadIndex, bool* parentCanBeAvailable, int32_t depth)
{
    ...
    if (symRef && node->getOpCode().isLoadVar()) {
        ...
        if (result != _storeMap->end()
            ...
            doCopyPropagationIfPossible(
                node, parent, childNum, result->second, symRef, visitCount, doneCopyPropagation);
            ...
            for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end; ++itr) {
                ...
                if (storeNode->getOpCode().isStoreIndirect()
                    ...
                    if (doCopyPropagationIfPossible(
                            node, parent, childNum, storeNode, symRef, visitCount, doneCopyPropagation))
                        break;
                }
            }
        }
    ...
    TR::SymbolReference* symRef, vcount_t visitCount, bool& doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
        ...
        rhsOfStoreDefNode = replaceCopySymbolReferenceByOriginalIn(
            symRef /*, originalSymbolReference*/, storeNode, rhsOfStoreDefNode, node, parent, childNum);
        node->setVisitCount(visitCount);
...
void OMR::LocalCSE::makeNodeAvailableForCommoning(
    TR::Node* parent, TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, bool* canBeAvailable)
{
        ...
        if (symRef->getReferenceNumber() == copySymbolReference->getReferenceNumber()) {
            ...
            if (
                ...
                if (TR::ILOpCode::isVectorOpCode(convOp) && TR::ILOpCode::getVectorOperation(convOp) == TR::vconv)
                    convNode = TR::Node::create(TR::ILOpCode::createVectorOpCode(
                                                    TR::vconv, rhsOfStoreDefNode->getDataType(), node->getDataType()),
                        1, rhsOfStoreDefNode);
                else
                ...
                if (rhsOfStoreDefNode->getType().isBCD()) {
                    if (defNode && defNode->getDataType() == TR::PackedDecimal && defNode->getOpCode().isStore()
                        ...
                        rhsOfStoreDefNode = TR::Node::create(
                            origRhsOfStoreDefNode, TR::ILOpCode::cleanOpCode(origRhsOfStoreDefNode->getDataType()), 1);
...
bool OMR::LocalCSE::killExpressionsIfVolatileLoad(
    TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, TR_UseDefAliasSetInterface& UseDefAliases)
{
...
void OMR::LocalCSE::killAvailableExpressionsAtGCSafePoints(
    TR::Node* node, TR::Node* parent, TR_BitVector& seenAvailableLoadedSymbolReferences)
{
