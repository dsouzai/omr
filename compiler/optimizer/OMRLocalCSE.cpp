...
bool OMR::LocalCSE::isFirstReferenceToNode(TR::Node *parent, int32_t index, TR::Node *node, vcount_t visitCount)
{
...
void OMR::LocalCSE::transformBlock(TR::TreeTop *entryTree, TR::TreeTop *exitTree)
{
    ...
    for (currentTree = entryTree->getNextRealTreeTop(); currentTree != exitTree->getNextTreeTop();
    ...
    int32_t childNum, int32_t *nextLoadIndex, bool *parentCanBeAvailable, int32_t depth)
{
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++)
        examineNode(node->getChild(i), seenAvailableLoadedSymbolReferences, node, i, nextLoadIndex, &nodeCanBeAvailable,
            depth + 1);
    ...
    if (symRef && node->getOpCode().isLoadVar()) {
        ...
        if (result != _storeMap->end()
            && result->second->getSymbolReference()->getReferenceNumber() == symRef->getReferenceNumber()) {
            // Base case: where symrefs match
            doCopyPropagationIfPossible(node, parent, childNum, result->second, symRef, visitCount,
                doneCopyPropagation);
        } else if (node->getOpCode().isLoadIndirect() && node->getFirstChild()->getVisitCount() >= visitCount) {
            ...
            for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end; ++itr) {
                ...
                if (storeNode->getOpCode().isStoreIndirect()
                    ...
                    if (doCopyPropagationIfPossible(node, parent, childNum, storeNode, symRef, visitCount,
                            doneCopyPropagation))
                        break;
                }
            }
    ...
    if (hasAliases && !doneCommoning)
        alreadyKilledAtVolatileLoad
            = killExpressionsIfVolatileLoad(node, seenAvailableLoadedSymbolReferences, UseDefAliases);
    ...
    if (node->getOpCode().isStore() || hasAliases) {
        ...
        if (hasAliases) {
            ...
            if (UseDefAliases.containsAny(tmp, comp())) {
                ...
                if (alreadyKilledAtVolatileLoad) // we want to keep the last load of a particular sym ref alive but no
        ...
        //
        if (symRef && symRef->getSymbol()->isAuto() && _availablePinningArrayExprs.get(symRef->getReferenceNumber())) {
...
bool OMR::LocalCSE::canCommonNodeInVolatilePass(TR::Node *node)
{
    return node->getOpCode().hasSymbolReference()
        && (node->getSymbol()->isVolatile() || node->getSymbol()->isAutoOrParm());
}
...
void OMR::LocalCSE::doCommoningIfAvailable(TR::Node *node, TR::Node *parent, int32_t childNum, bool &doneCommoning)
{
    ...
    if (availableExpression && (availableExpression != node) && shouldCommonNode(parent, node)
        && performTransformation(comp(),
            "%s   Local Common Subexpression Elimination commoning node : %p by available node : %p\n",
        ...
        if (!node->getOpCode().hasSymbolReference()
            ...
            if (parent != NULL) {
                ...
                if ((!parent->getOpCode().isResolveOrNullCheck() && (parent->getOpCodeValue() != TR::DIVCHK))
                    && ((parent->getOpCodeValue() != TR::compressedRefs) || (childNum != 0)))
...
bool OMR::LocalCSE::doCopyPropagationIfPossible(TR::Node *node, TR::Node *parent, int32_t childNum, TR::Node *storeNode,
    TR::SymbolReference *symRef, vcount_t visitCount, bool &doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
            || !symRef->getSymbol()->isAuto() || !symRef->getSymbol()->castToAutoSymbol()->isSlotSharedByRefAndNonRef())
        && shouldCommonNode(parent, node) && isSafeToCommon && canAffordToIncreaseRegisterPressure()
        && (!node->getOpCode().hasSymbolReference()
            || node->getSymbolReference() != comp()->getSymRefTab()->findVftSymbolRef())
        && (symRef->storeCanBeRemoved()
            || (!symRef->getSymbol()->isVolatile() && rhsOfStoreDefNode->getDataType() == TR::Float
                && (rhsOfStoreDefNode->getOpCode().isCall() || (rhsOfStoreDefNode->getOpCode().isLoadConst())
                    || rhsOfStoreDefNode->getOpCode().isLoadVar())))
        && (!parent->getOpCode().isSpineCheck() || childNum != 0)
        && performTransformation(comp(),
            "%s   Local Common Subexpression Elimination propagating local #%d in node : %p PARENT : %p from node %p\n",
        ...
        manager()->setAlteredCode(true);
        rhsOfStoreDefNode = replaceCopySymbolReferenceByOriginalIn(symRef /*, originalSymbolReference*/, storeNode,
            rhsOfStoreDefNode, node, parent, childNum);
        node->setVisitCount(visitCount);
...
//
void OMR::LocalCSE::makeNodeAvailableForCommoning(TR::Node *parent, TR::Node *node,
    TR_BitVector &seenAvailableLoadedSymbolReferences, bool *canBeAvailable)
{
    if (parent && (parent->getOpCodeValue() == TR::Prefetch) && (parent->getFirstChild() == node))
        return;
    ...
    if (node->getOpCode().hasSymbolReference()) {
        if (!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber())) {
            *canBeAvailable = false;
            ...
            if (node->getOpCode().isLoadVar() || node->getOpCode().isCheck() || node->getOpCode().isCall()
                    || node->getOpCodeValue() == TR::
                    instanceof || ((node->getOpCodeValue() == TR::loadaddr)
                                   && (node->getSymbol()->isNotCollected() || node->getSymbol()->isAutoOrParm()))) {
...
void OMR::LocalCSE::commonNode(TR::Node *parent, int32_t childNum, TR::Node *node, TR::Node *replacingNode)
{
    ...
    if ((node->getOpCode().isLoadVar() || node->getOpCode().isLoadReg()) && node->isDontMoveUnderBranch()
        && (replacingNode->getOpCode().isLoadVar() || replacingNode->getOpCode().isLoadReg())) {
...
TR::Node *OMR::LocalCSE::replaceCopySymbolReferenceByOriginalIn(
    TR::SymbolReference *copySymbolReference /*, TR::SymbolReference *originalSymbolReference*/, TR::Node *defNode,
    TR::Node *rhsOfStoreDefNode, TR::Node *node, TR::Node *parent, int32_t childNum)
{
    if (node->getOpCode().hasSymbolReference()) {
        TR::SymbolReference *symRef = node->getSymbolReference();
        ...
        if (symRef->getReferenceNumber() == copySymbolReference->getReferenceNumber()) {
            ...
            if (
                ...
                // sign mismatch
                TR::ILOpCodes convOp
                    = TR::ILOpCode::getProperConversion(rhsOfStoreDefNode->getDataType(), node->getDataType(), false);
                ...
                if (TR::ILOpCode::isVectorOpCode(convOp) && TR::ILOpCode::getVectorOperation(convOp) == TR::vconv)
                    convNode = TR::Node::create(TR::ILOpCode::createVectorOpCode(TR::vconv,
                                                    rhsOfStoreDefNode->getDataType(), node->getDataType()),
                        1, rhsOfStoreDefNode);
                else
                ...
                if (rhsOfStoreDefNode->getType().isBCD()) {
                    if (defNode && defNode->getDataType() == TR::PackedDecimal && defNode->getOpCode().isStore()
                        && defNode->mustCleanSignInPDStoreEvaluator()) {
                        TR_ASSERT(rhsOfStoreDefNode->getDataType() == TR::PackedDecimal,
                            "rhsOfStoreDefNode %p (type %s) must be pd type if defNode %p is pd\n", rhsOfStoreDefNode,
                        ...
                        TR_ASSERT(rhsOfStoreDefNode->getDecimalPrecision()
                            ...
                            rhsOfStoreDefNode->getDecimalPrecision(), TR::DataType::getMaxPackedDecimalPrecision());
                        TR::Node *origRhsOfStoreDefNode = rhsOfStoreDefNode;
                        rhsOfStoreDefNode = TR::Node::create(origRhsOfStoreDefNode,
                            TR::ILOpCode::cleanOpCode(origRhsOfStoreDefNode->getDataType()), 1);
                        ...
                        dumpOptDetails(comp(),
                            ...
                            "] when propagating %s [" POINTER_PRINTF_FORMAT "] to %s [" POINTER_PRINTF_FORMAT
                            ...
                            parent->getOpCode().getName(), parent, rhsOfStoreDefNode->getOpCode().getName(),
                            rhsOfStoreDefNode);
                    }
                    ...
                    if (overrideNodePrecision != 0) {
                        ...
                        TR_ASSERT(node->getType().isBCD(),
                            "node %s (%p) must be a BCD type if overrideNodePrecision == 0\n",
                            node->getOpCode().getName(), node);
                        nodePrecision = node->getDecimalPrecision();
                    }

                    if (rhsOfStoreDefNode->getDecimalPrecision() != nodePrecision) {
                        // If the copy propagation is going to change the size of the node being replaced then a size
                        // changing pdshl must be prepended in case the new parent requires explicit widening (such as
                        // if the parent is a call).
                        TR::Node *origRhsOfStoreDefNode = rhsOfStoreDefNode;
                        TR::ILOpCodes modifyPrecisionOp
                            = TR::ILOpCode::modifyPrecisionOpCode(origRhsOfStoreDefNode->getDataType());
                        ...
                        rhsOfStoreDefNode->setReferenceCount(1);
                        dumpOptDetails(comp(),
                            "%sPrecision mismatch when propagating %s [" POINTER_PRINTF_FORMAT
                            "] to %s [" POINTER_PRINTF_FORMAT "] so create new %s [" POINTER_PRINTF_FORMAT "]\n",
...
//
bool OMR::LocalCSE::canBeAvailable(TR::Node *parent, TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences,
    bool canBeAvailable)
{
    if (!canBeAvailable)
        return false;
    ...
    if (node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg()
        || (node->getOpCodeValue() == TR::PassThrough && parent->getOpCodeValue() != TR::GlRegDeps)
    ...
    if (parent) {
        if (node->getOpCode().isCall()
            && (!node->getSymbol()->isMethod()
                || (node->getSymbol()->isMethod() && !node->getSymbol()->castToMethodSymbol()->isPureFunction()))
...
TR::Node *OMR::LocalCSE::getAvailableExpression(TR::Node *parent, TR::Node *node)
{
    if (node->getOpCodeValue() == TR::NULLCHK) {
        for (int32_t i = 0; i < _numNullCheckNodes; i++) {
            if (!(_nullCheckNodesAsArray[i] == NULL)) {
                if ((_nullCheckNodesAsArray[i]->getSymbolReference()->getReferenceNumber()
    ...
    if (node->hasPinningArrayPointer() && node->computeIsInternalPointer() && cg()->supportsInternalPointers()
        ...
        for (; arrayRefNode; arrayRefNode = arrayRefNodesIt.getNext()) {
            if ((arrayRefNode != node) && (arrayRefNode->getFirstChild() == node->getFirstChild())
                && (arrayRefNode->getSecondChild() == node->getSecondChild())) {
                arrayRefNode->setIsInternalPointer(true);

                TR::AutomaticSymbol *autoSym
                    = node->getFirstChild()->getSymbolReference()->getSymbol()->castToAutoSymbol();
...
//
bool OMR::LocalCSE::killExpressionsIfVolatileLoad(TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences,
...
void OMR::LocalCSE::killAllInternalPointersBasedOnThisPinningArray(TR::SymbolReference *symRef)
{
    ...
    TR::Node *arrayRefNode = arrayRefNodesIt.getFirst();
    for (; arrayRefNode; arrayRefNode = arrayRefNodesIt.getNext()) {
        if ((arrayRefNode->getNumChildren() > 0) && (arrayRefNode->getFirstChild()->getOpCodeValue() == TR::aload)
...
void OMR::LocalCSE::killAvailableExpressionsAtGCSafePoints(TR::Node *node, TR::Node *parent,
    TR_BitVector &seenAvailableLoadedSymbolReferences)
{
...
{
    if ((node->getOpCode().isArrayRef()) || (node->isGCSafePointWithSymRef() && comp()->getOptions()->realTimeGC())
    ...
    if ((node->getOpCode().hasSymbolReference() && node->getOpCode().isLoadVar()) || node->getOpCode().isCall())
...
{
    if (node->getOpCode().isStore() || (node->getOpCode().isVoid() && (node->getOpCodeValue() != TR::PassThrough)))
        return;

    if (node->getOpCode().hasSymbolReference() && !_relevantNodes.get(node->getSymbolReference()->getReferenceNumber()))
        return;
...
bool OMR::LocalCSE::areSyntacticallyEquivalent(TR::Node *node1, TR::Node *node2, bool *remove)
{
    ...
    for (int32_t i = 0; i < node1->getNumChildren(); i++) {
        ...
        if (child1 != child2) {
            if (child1->getOpCode().isArrayRef() && child2->getOpCode().isArrayRef()) {
            ...
            } else if (node1->getOpCodeValue() == node2->getOpCodeValue() && node1->getOpCode().isCommutative()
    ...
    if (node1->getOpCodeValue() == TR::dbits2l && node2->getOpCodeValue() == TR::dbits2l
        && node1->normalizeNanValues() != node2->normalizeNanValues())
        return false;

    if (node1->getOpCodeValue() == TR::fbits2i && node2->getOpCodeValue() == TR::fbits2i
        && node1->normalizeNanValues() != node2->normalizeNanValues())
        return false;
