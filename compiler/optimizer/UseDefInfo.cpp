...
void TR_UseDefInfo::prepareUseDefInfo(
    bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs, bool conversionRegsOnly)
{
    ...
    if (requiresGlobals || prefersGlobals) {
        ...
        if (requiresGlobals && !canBuild) {
            ...
            if (trace())
                traceMsg(
                    comp(), "Use/Def info: cannot build global use/def info as it failed in indexSymbolsAndNodes\n");
            return;
        }
    }
...
void TR_UseDefInfo::findAndPopulateVolatileSymbolsIndex(TR::BitVector& volatileSymbols)
{
    ...
    for (int32_t symRefNumber = comp()->getSymRefTab()->getIndexOfFirstSymRef();
         unsigned(symRefNumber) < comp()->getSymRefCount(); symRefNumber++) {
    ...
    if (_uniqueIndexForDefsOnEntry) {
        for (int32_t i = getFirstDefIndex(); i < _numDefsOnEntry; i++) {
            ...
            for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                ...
                // traceMsg(comp(),"\n _numDefsOnEntry=%d getNumExpandedDefNodes()=%d j=(u%d s#%d) i=(u%d
                // s#%d)",_numDefsOnEntry, getNumExpandedDefNodes(), j, aliasedSymRef->getReferenceNumber(),i,
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData& aux)
{
    ...
    if (trace()) {
        traceMsg(
            comp(), "Trying to index nodes with _indexFields = %d, _indexStatics = %d\n", _indexFields, _indexStatics);
    }
    ...
    while (refIter.hasMoreElements()) {
        ...
        if (trace()) {
            traceMsg(comp(), "Non Trivial symbol Ref [%p:%d] isTrivial=%d isParm=%d _neverWritten=%d", symRef,
...
bool TR_UseDefInfo::skipAnalyzingForCompileTime(
    TR::Node* node, TR::Block* block, TR::Compilation* comp, AuxiliaryData& aux)
{
    ...
    AuxiliaryData& aux, TR::deque<uint32_t, TR::Region&>& symRefToLocalIndexMap, bool considerImplicitStores)
{
    ...
    if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
        if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
            ...
            //       traceMsg(comp(), "UDI: setting useDefIndex to %d _numUseOnlyNodes =
            //       %d\n",useDefIndex,_numUseOnlyNodes);
            ...
            if (num_aliases > 0) {
                if (symRefToLocalIndexMap[symRef->getReferenceNumber()] == 0) {
                    ...
                    // num_aliases, opCode.isFence(), node->getSymbolReference()->getSymbol()->isMethod());
                    useDefIndex = _numDefUseNodes++;
                    //             traceMsg(comp(), "UDI: setting useDefIndex to %d _numDefUseNodes =
                    //             %d\n",useDefIndex,_numDefUseNodes);
    ...
    AuxiliaryData& aux, bool considerImplicitStores)
{
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        bool shouldConsiderLoadAddrChildrenImplicitStores
            = parentCouldHaveImplicitStoreChildren && childIndexIndicatesImplicitStore(node, i);
        if (!assignAdjustedNodeIndex(
                block, node->getChild(i), node, treeTop, aux, shouldConsiderLoadAddrChildrenImplicitStores))
            return false;
    }
    ...
    if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
        if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
            ...
            //         traceMsg(comp(), "node %p setting adjustment to %d _numDefOnlyNodes = %d, _numDefUseNodes =
    ...
    if (node->getLocalIndex() + expandedAdjustment > MAX_SCOUNT) {
        dumpOptDetails(
            comp(), "   use/def failed, local index overflow (%d)\n", node->getLocalIndex() + expandedAdjustment);
        return false;
    }
    ...
    AuxiliaryData& aux, TR::SparseBitVector& expandedLIndexes, bool considerImplicitStores)
{
    ...
    if (trace()) {
        if (!definesMultipleSymbols)
            traceMsg(comp(),
                "Node : %p   opCode = %s useDefIndex = %d localIndex = %d definesMultipleSymbols=%d "
            ...
            traceMsg(comp(),
                "Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d definesMultipleSymbols=%d "
                "isTrivialUseDefNode=%d adjustArray=%d \n",
...
bool isSymRefFromInlinedMethod(
    TR::Compilation* comp, TR::ResolvedMethodSymbol* currentMethod, TR::SymbolReference* symRef)
{
    while (true) {
...
void TR_UseDefInfo::buildUseDefs(void* vblockInfo, AuxiliaryData& aux)
{
    ...
    if (_indexStatics && blockInfo) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            //         traceMsg(comp(), "UDI: setting _useDefInfo[%d}[i=%d] to true getLastUseIndex = %d
            //         getFirstUseIndex = %d numberOfNodes =
    ...
    if (trace()) {
        traceMsg(comp(),
            "\nUse/Def info: firstDefIndex=%d lastDefIndex=%d firstUseIndex=%d lastUseIndex=%d "
            "getNumDefsOnEntry()=%d\n",
...
void TR_UseDefInfo::dereferenceDefs(
    int32_t useIndex, TR_UseDefInfo::BitVector& nodesLookedAt, TR_UseDefInfo::BitVector& loadDefs)
{
    if (trace()) {
...
//
int32_t TR_UseDefInfo::setSingleDefiningLoad(
    int32_t useIndex, TR_UseDefInfo::BitVector& nodesLookedAt, TR_UseDefInfo::BitVector& loadDefs)
{
    TR_UseDefInfo::BitVector& useDefInfo = _useDefInfo[useIndex];
    nodesLookedAt[useIndex] = true;
    ...
    if (0 && !cursor.Valid()) {
        ...
        if (trace())
            traceMsg(
                comp(), "      Use index %d has single defining load %d\n", useIndex + getFirstUseIndex(), defIndex);
 ...
 * in the same block
 */
void TR_UseDefInfo::dereferenceDef(
    TR_UseDefInfo::BitVector& useDefInfo, int32_t defIndex, TR_UseDefInfo::BitVector& nodesLookedAt)
{
...
void TR_UseDefInfo::buildUseDefs(
    TR::Node* node, void* vanalysisInfo, TR_BitVector& nodesToBeDereferenced, TR::Node* parent, AuxiliaryData& aux)
{
    vcount_t visitCount = comp()->getVisitCount();
    ...
    if (node->getOpCode().hasSymbolReference() && isTrivialUseDefNode(node, aux)) {
        ...
        if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
            ...
            if (trace())
                traceMsg(
                    comp(), "For node %p index = %d and first use index = %d\n", node, nodeIndex, getFirstUseIndex());
    ...
    if (isExpandedUseIndex(expandedNodeIndex)
            ...
            while (cursor.hasMoreElements()) {
                ...
                if (i >= _numDefsOnEntry) {
                    ...
                    if (memSymIndex == -1 || getMemorySymbolIndex(defNode) != memSymIndex
                        ...
                        //              traceMsg(comp(), "UDI: setting _useDefInfo[realIndex=%d][j=%d] to
                        //              true\n",realIndex,j);
        ...
        if (numDefs == 0) {
            if (isLoadAddrUse(node)) {
                //        traceMsg(comp(), "UDI: setting _useDefInfo[realIndex=%d][0] to true because of
                //        loadaddr\n",realIndex);
                _useDefInfo[realIndex][0] = true;
            }
        }
...
TR::Node* TR_UseDefInfo::getSingleDefiningLoad(TR::Node* node)
{
    ...
    if (info.PopulationCount() == 1) {
        ...
        if ((firstDef >= getFirstUseIndex()) && (n = getNode(firstDef)) && (n->getUseDefIndex() > 0)
            && (n->getOpCode().isLoadVar()) && (n->getOpCode().hasSymbolReference() && !n->getSymbol()->isVolatile()))
            return n;
    }
    return NULL;
}
...
const TR_UseDefInfo::BitVector& TR_UseDefInfo::getUseDef_ref_body(
    int32_t useIndex, TR_BitVector* visitedDefs, TR_UseDefInfo::BitVector* defs)
{
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (firstDef < 0) {
                ...
                if ((firstDef < getFirstUseIndex())
                    ...
                    //               traceMsg(comp(), "UDI, special case firstDef = %d getFirstUseIndex() = %d
...
const TR_UseDefInfo::BitVector& TR_UseDefInfo::getUsesFromDef_ref(int32_t defIndex, bool loadAsDef)
{
    if ((_defUseInfo.size() > 0) && !loadAsDef) {
        ...
        //      ",defIndex,_defUseInfo.NumberOfElements());
        //      *(comp()) << _defUseInfo;
...
{
    TR_ASSERT(
        index < getTotalNodes(), "TR_UseDefInfo::getNode index(%d) is bigger than total(%d)\n", index, getTotalNodes());
    ...
    for (int32_t i = getFirstUseIndex(); i <= getLastUseIndex(); i++) {
        ...
        if (!defs.IsZero()) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                //         traceMsg(comp(), "UDI: for bit vector at index %d setting %d  (i = %d) to
