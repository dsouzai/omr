...
static void countReplicationFailure(const char* failureReason, int32_t regionNum)
{
    ...
    TR::DebugCounter::incStaticDebugCounter(comp,
        TR::DebugCounter::debugCounterName(comp, "LoopReplicator/%s/%s/(%s)/region_%d", failureReason,
            comp->getHotnessName(comp->getMethodHotness()), comp->signature(), regionNum));
}
...
int32_t TR_LoopReplicator::perform(TR_Structure* str)
{
    ...
    dumpOptDetails(
        comp(), "loop (%d) does not conform to required form & will not be replicated\n", region->getNumber());
    return false;
}
...
bool TR_LoopReplicator::isWellFormedLoop(TR_RegionStructure* region, TR_Structure* condBlock)
{
    ...
    for (b = bilIt.getCurrent(); b; b = bilIt.getNext()) {
        ...
        for (TR::TreeTop* tt = entryTT->getNextRealTreeTop(); tt != exitTT; tt = tt->getNextRealTreeTop()) {
...
static void collectNonColdInnerLoops(
    TR::Compilation* c, TR_RegionStructure* region, List<TR_RegionStructure>& innerLoops)
{
...
bool TR_LoopReplicator::shouldReplicateWithHotInnerLoops(
    TR_RegionStructure* region, LoopInfo* lInfo, TR_ScratchList<TR::Block>* hotInnerLoopHeaders)
{
    ...
    for (;;) {
        ...
        TR_ASSERT(tracePrefixCursor != outerLoopHeader, "cold side-entry detection returned to outerLoopHeader\n");

        for (auto e = tracePrefixCursor->getPredecessors().begin(); e != tracePrefixCursor->getPredecessors().end();
             ++e) {
...
void TR_LoopReplicator::processBlock(TR::Block* X, TR_RegionStructure* region, LoopInfo* lInfo)
{
    ...
    for (auto e = X->getSuccessors().begin(); e != X->getSuccessors().end(); ++e) {
        ...
        if (computeWeight(*e)) {
            if (trace())
                traceMsg(
                    comp(), "   candidate (%d) satisfied weight computation, extending trace\n", dest->getNumber());
...
bool TR_LoopReplicator::gatherBlocksToBeCloned(LoopInfo* lInfo)
{
    ...
    for (be = (lInfo->_nodesCommon).getFirst(); be; be = be->getNext()) {
        ...
        for (auto e = b->getPredecessors().begin(); e != b->getPredecessors().end(); ++e) {
            ...
            /// _blockMapper[source->getNumber()], be->_nonLoop);
            if (!sourceBE || _blockMapper[source->getNumber()] || be->_nonLoop) {
                ...
                if (enableInnerLoopChecks && be->_nonLoop && sourceBE && !_blockMapper[source->getNumber()]) {
...
void TR_LoopReplicator::doTailDuplication(LoopInfo* lInfo)
{
    ...
    for (be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
              ...
              _blockMapper[n->getNumber()])*/
        {
            ...
            newBlock->setFrequency(
                (int32_t)((block->getFrequency() * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
            ...
            for (auto e = newBlock->getExceptionSuccessors().begin(); e != newBlock->getExceptionSuccessors().end();
                 ++e) {
                int32_t edgeFreq = (*e)->getFrequency();
                (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
                if ((*e)->getFrequency() < defaultFrequency)
            ...
            for (auto e = newBlock->getExceptionPredecessors().begin(); e != newBlock->getExceptionPredecessors().end();
                 ++e) {
                int32_t edgeFreq = (*e)->getFrequency();
                (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
                if ((*e)->getFrequency() < defaultFrequency)
    ...
    for (auto e = clonedHeader->getExceptionSuccessors().begin(); e != clonedHeader->getExceptionSuccessors().end();
         ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
    ...
    for (auto e = clonedHeader->getExceptionPredecessors().begin(); e != clonedHeader->getExceptionPredecessors().end();
         ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) / 100)); // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
...
void TR_LoopReplicator::addBlocksAndFixEdges(LoopInfo* lInfo)
{
    ...
    for (BlockEntry* be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
        ...
        for (auto e = origBlock->getSuccessors().begin(); e != origBlock->getSuccessors().end(); ++e) {
            ...
            // correspondingly add C1->C2 where C1 & C2 are the respective clones
            else if ((dest->getNumber() < _nodesInCFG) && _blockMapper[dest->getNumber()] && dest != loopHeader) {
                ...
                else if (newBlock->getNextBlock() != dest && !branchNode->getOpCode().isJumpWithMultipleTargets()
            ...
            // add C1->B2 where C1 is clone of B1
                ...
                if ((branchNode->getOpCode().isJumpWithMultipleTargets() && branchNode->getOpCode().hasBranchChildren())
                    || branchNode->getOpCode().isGoto()) {
                    _cfg->addEdge(TR::CFGEdge::createEdge(newBlock, dest, trMemory()));
                    if (trace())
                        traceMsg(comp(), "   added edge %d -> %d\n", newBlock->getNumber(), dest->getNumber());
...
TR::Block* TR_LoopReplicator::createEmptyGoto(TR::Block* source, TR::Block* dest, bool redirect)
{
    ...
    if (redirect) {
        ...
        target = destEntry;
    TR::Node* gotoNode = TR::Node::create(destEntry->getNextTreeTop()->getNode(), TR::Goto, 0, target);
...
void TR_LoopReplicator::calculateBlockWeights(TR_RegionStructure* region)
{
    ...
    do {
        ...
        if ((node->getStructure()->getParent()->asRegion() == region)
            && !(node->getStructure()->asRegion() && node->getStructure()->asRegion()->isNaturalLoop())) {
        ...
        for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
            ...
            if (!isBackEdgeOrLoopExit(*e, region, true)) {
                bool innerLoop = ((_seenBlocks[dest->getNumber()] == COLOR_BLACK)
                    && (dest->getStructure()->asRegion() && dest->getStructure()->asRegion()->isNaturalLoop()));
...
int32_t TR_LoopReplicator::deriveFrequencyFromPreds(TR_StructureSubGraphNode* curNode, TR_RegionStructure* region)
{
    ...
    for (auto e = curNode->getPredecessors().begin(); e != curNode->getPredecessors().end(); ++e) {
        ...
        if (from->getStructure()->getParent()->asRegion() == region) {
            if (from->getStructure()->asRegion() && from->getStructure()->asRegion()->isNaturalLoop()) {
    ...
    for (TR::Block* b = bIt.getFirst(); b; b = bIt.getNext()) {
        ...
        if (!(b->getSuccessors().size() == 1)) {
            ...
            for (auto e = b->getSuccessors().begin(); e != b->getSuccessors().end(); ++e) {
                ...
                if ((d == curBlock) || (!region->contains(d->getStructureOf(), region->getParent())))
                    continue;
...
bool TR_LoopReplicator::predecessorsNotYetVisited(TR_RegionStructure* r, TR_StructureSubGraphNode* curNode)
{
...
int32_t TR_LoopReplicator::getBlockFreq(TR::Block* X)
{
    ...
    // if (freq >= 0)
    // if (freq)
    if ((freq != (MAX_COLD_BLOCK_COUNT + 1)) && (freq != 0))
        return freq;
