 ...
 */
TR_YesNoMaybe RematTools::gatherNodesToCheck(TR::Compilation* comp, TR::Node* privArg, TR::Node* currentNode,
    ...
    TR::SparseBitVector& visitedNodes)
{
    ...
    if (opCode.hasSymbolReference() && !opCode.isLoad()) {
        ...
        if (gatherNodesToCheck(
                comp, privArg, currentNode->getFirstChild(), scanTargets, symRefsToCheck, trace, visitedNodes)
            != TR_no) {
            scanTargets[currentNode->getGlobalIndex()] = true;
            symRefsToCheck[currentNode->getSymbolReference()->getReferenceNumber()] = true;
            return TR_yes;
        ...
        return TR_maybe;
    } else if (opCode.isArithmetic() || opCode.isConversion() || opCode.isMax() || opCode.isMin()
        ...
        for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
            ...
            TR_YesNoMaybe childResult = gatherNodesToCheck(
                comp, privArg, currentNode->getChild(i), childScanTargets, childSymRefsToCheck, trace, visitedNodes);
    ...
    TR::SparseBitVector& scanTargets, TR::SparseBitVector& symRefsToCheck, bool trace)
{
    ...
    return gatherNodesToCheck(comp, privArg, currentNode, scanTargets, symRefsToCheck, trace, visitedNodes);
}

void RematTools::walkNodesCalculatingRematSafety(TR::Compilation* comp, TR::Node* currentNode,
    ...
    bool trace, TR::SparseBitVector& visitedNodes)
{
    for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
        ...
        walkNodesCalculatingRematSafety(
            comp, currentNode->getChild(i), scanTargets, enabledSymRefs, unsafeSymRefs, trace, visitedNodes);
    }
 ...
 */
bool RematTools::walkTreesCalculatingRematSafety(TR::Compilation* comp, TR::TreeTop* start, TR::TreeTop* end,
    TR::SparseBitVector& scanTargets, TR::SparseBitVector& unsafeSymRefs, TR_BitVector* blocksToVisit, bool trace)
{
...
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
    if (blocks) {
        blocks = new (comp->trStackMemory())
            TR_BitVector(comp->getFlowGraph()->getNextNodeNumber(), comp->trMemory(), stackAlloc);
    ...
    while (start != end) {
        if (start->getNode()) {
            walkNodesCalculatingRematSafety(
                comp, start->getNode(), scanTargets, enabledSymRefs, unsafeSymRefs, trace, visitedNodes);
        }
...
void RematTools::walkTreesCalculatingRematSafety(TR::Compilation* comp, TR::TreeTop* start, TR::TreeTop* end,
 ...
 * Returns true to indicate it successfully completed the analysis.
 */
bool RematTools::walkTreeTopsCalculatingRematFailureAlternatives(TR::Compilation* comp, TR::TreeTop* start,
...
void RematTools::walkTreeTopsCalculatingRematFailureAlternatives(TR::Compilation* comp, TR::TreeTop* start,
    TR::TreeTop* end, TR::list<TR::TreeTop*>& failedArgs, TR::SparseBitVector& scanTargets,
