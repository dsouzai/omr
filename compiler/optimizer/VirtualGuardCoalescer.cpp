...
void TR_VirtualGuardTailSplitter::initializeDataStructures()
{
    ...
    while (!dfsList.isEmpty()) {
        ...
        if (top && block == top->getMergeBlock()) {
            ...
            for (TR::CFGEdge *edge = it.getFirst(); edge && !notDone; edge = it.getNext()) {
    ...
    while (cursor->getExit() != NULL) {
        ...
        if (isMergeBlock) {
            if (!(cursor->getPredecessors().size() == 2))
                break;
        } else if (!(cursor->getPredecessors().size() == 1))
            break;
        ...
        if (!(succs.size() == 1)) {
            ...
            if ((succs.size() == 2) && !lastTree->getNode()->getOpCode().isSwitch()) {
...
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block *first, TR::Block *last)
{
    ...
    while (next != last) {
        ...
        if (_cfg->getStructure()) {
            next->getStructureOf()->getParent()->asRegion()->addSubNode(
                new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(
...
TR_VirtualGuardTailSplitter::VGInfo *TR_VirtualGuardTailSplitter::recognizeVirtualGuard(TR::Block *block,
    VGInfo *parent)
{
    if (!block->getExit())
        return 0;
    ...
    if (call && guardNode->isProfiledGuard()) {
        TR_AddressInfo *valueInfo = static_cast<TR_AddressInfo *>(
            TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(), AddressInfo));
...
bool TR_VirtualGuardTailSplitter::isKill(TR::Node *node)
{
    ...
    if (opcode.hasSymbolReference() && node->getSymbolReference()->isUnresolved())
        return true;
...
int32_t TR_InnerPreexistence::perform()
{
...
int32_t TR_InnerPreexistence::initialize()
{
...
TR_InnerPreexistence::GuardInfo::GuardInfo(TR::Compilation *comp, TR::Block *block, GuardInfo *parent,
    TR_ValueNumberInfo *vnInfo, uint32_t numInlinedSites)
    : _block(block)
    , _parent(parent)
    , _hasBeenDevirtualized(false)
{
    TR::Node *guardNode = block->getLastRealTreeTop()->getNode();
    ...
    _innerSubTree
        = new (comp->trStackMemory()) TR_BitVector(numInlinedSites, comp->trMemory(), stackAlloc, notGrowable);
...
void TR_InnerPreexistence::transform()
{
    int32_t i;
    ...
    for (i = 0; i < _numInlinedSites; ++i) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (!performTransformation(comp(), "%sDevirtualizing call guarded by block_%d preexisting on guard %d\n",
                    OPT_DETAILS, descendant->getBlock()->getNumber(), info->getBlock()->getNumber()))
                continue;
            ...
            if (true) // force block scope
            {
                ...
                int32_t thisVN = _vnInfo->getValueNumber(innerCall->getChild(innerCall->getFirstArgumentIndex()));

                // Find out which argument of the outer call is the same as the this of the inner call
                //
                int16_t ordinal = -1;
                for (int16_t child = outerCall->getNumChildren() - 1; child >= outerCall->getFirstArgumentIndex();
                     child--) {
...
void TR_InnerPreexistence::devirtualize(GuardInfo *info)
{
    TR::Block *guardBlock = info->getBlock();
    TR::Node *guardNode = guardBlock->getLastRealTreeTop()->getNode();
    TR_ASSERT(guardNode->getOpCodeValue() == TR::ificmpne || guardNode->getOpCodeValue() == TR::ifacmpne,
