    ...
    TR::Block* block, VGInfo* parent)
{
    ...
    if (call && guardNode->isProfiledGuard()) {
        TR_AddressInfo* valueInfo
            = static_cast<TR_AddressInfo*>(TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(), AddressInfo));
...
TR_InnerPreexistence::GuardInfo::GuardInfo(
    TR::Compilation* comp, TR::Block* block, GuardInfo* parent, TR_ValueNumberInfo* vnInfo, uint32_t numInlinedSites)
    : _block(block)
    , _parent(parent)
