...
TR_LiveVariableInformation::TR_LiveVariableInformation(TR::Compilation* c, TR::Optimizer* optimizer,
    TR_Structure* rootStructure, bool splitLongs, bool includeParms, bool ignoreOSRUses)
    : _compilation(c)
    , _trMemory(c->trMemory())
    , _ignoreOSRUses(ignoreOSRUses)
{
    ...
    TR_BitVector** regularKillSetInfo, TR_BitVector** exceptionGenSetInfo, TR_BitVector** exceptionKillSetInfo)
{
    if (_haveCachedGenAndKillSets) {
        TR_ASSERT(comp()->getFlowGraph()->getNextNodeNumber() == _numNodes,
            "flow graph has changed since gen and kill sets were initialized");
        ...
        for (int32_t nodeNumber = 0; nodeNumber < _numNodes; nodeNumber++) {
            ...
            if (_cachedExceptionGenSetInfo[nodeNumber] != NULL) {
                exceptionGenSetInfo[nodeNumber]
                    = new (trStackMemory()) TR_BitVector(_numLocals, trMemory(), stackAlloc);
                *(exceptionGenSetInfo[nodeNumber]) = *(_cachedExceptionGenSetInfo[nodeNumber]);
            }

            if (_cachedExceptionKillSetInfo[nodeNumber] != NULL) {
                exceptionKillSetInfo[nodeNumber]
                    = new (trStackMemory()) TR_BitVector(_numLocals, trMemory(), stackAlloc);
    ...
    if (!_localObjects->isEmpty()) {
        for (TR::CFGNode* cfgNode = _compilation->getFlowGraph()->getFirstNode(); cfgNode;
...
void TR_LiveVariableInformation::findCommonedLoads(
...
void TR_LiveVariableInformation::findAllUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector* genSetInfo,
    TR_BitVector* killSetInfo, TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
    visitTreeForLocals(
        node, &genSetInfo, killSetInfo, movingForwardThroughTrees, true, visitCount, commonedLoads, false);
}

void TR_LiveVariableInformation::findLocalUsesInBackwardsTreeWalk(TR::Node* node, int32_t blockNum,
    TR_BitVector* genSetInfo, TR_BitVector* killSetInfo, TR_BitVector* commonedLoads, vcount_t visitCount)
{
...
void TR_LiveVariableInformation::findUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector** genSetInfo,
    TR_BitVector** killSetInfo, TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
    visitTreeForLocals(node, genSetInfo + blockNum, killSetInfo[blockNum], movingForwardThroughTrees, false, visitCount,
        commonedLoads, false);
}
...
// if visitEntireTree is true, then the backward traversal will continue under commoned nodes but belowCommonedNode will
// be set
//    NOTE: forward traversals cannot rely on visitEntireTree
// belowCommonedNode is used to track when we're traversing a commoned part of a node's tree or not (on first call, pass
// in false) if commonedLoads is non-null, then any symbols loaded while belowCommonedNode is set are marked in that bit
// vector if visitCount is >= 0, then it will be used to avoid visiting nodes multiple times
//    NOTE: if a node has its visit count set to visitCount, it will not be visited unless visitEntireTree is set
//    NOTE: for backward traversals a new visit count is needed for each treetop in an EBB to visit correctly and
//    efficiently
void TR_LiveVariableInformation::visitTreeForLocals(TR::Node* node, TR_BitVector** blockGenSetInfo,
    ...
    TR_BitVector* commonedLoads, bool belowCommonedNode)
{
    ...
    if (traceLiveVarInfo()) {
        if (movingForwardThroughTrees)
            traceMsg(comp(),
                "         Looking forward for uses in node %p has visitCount = %d and comp() visitCount = %d\n", node,
                node->getVisitCount(), visitCount);
        else
            traceMsg(comp(),
                "         Looking backward for uses in node %p has future use count = %d and reference count = %d\n",
    ...
    if (movingForwardThroughTrees) {
        ...
        // visit count used to detect commoned nodes WITHIN each tree: visit count must be different for each tree in an
        // EBB for this to work properly ...IF the visit count isn't different for each tree, this DOES NOT WORK
        // PROPERLY
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i)
        visitTreeForLocals(node->getChild(i), blockGenSetInfo, blockKillSetInfo, movingForwardThroughTrees,
            visitEntireTree, visitCount, commonedLoads, belowCommonedNode);
    ...
    if (local) {
...

        {
            ...
            // for it
            //
            if (!movingForwardThroughTrees || blockKillSetInfo == NULL || (local && !blockKillSetInfo->get(localIndex)))
 ...
 */
void TR_OSRLiveVariableInformation::findUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector** genSetInfo,
    TR_BitVector** killSetInfo, TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
    TR_LiveVariableInformation::findUseOfLocal(
        node, blockNum, genSetInfo, killSetInfo, commonedLoads, movingForwardThroughTrees, visitCount);
 ...
 *
 * OSRLiveRangeInfo keeps all the dead symbol reference numbers for a bci in a given caller. This function use the total
 * autos or pend
 */
void TR_OSRLiveVariableInformation::buildLiveSymbolsBitVector(
    TR_OSRMethodData* osrMethodData, int32_t byteCodeIndex, TR_BitVector* liveLocals)
{
 ...
 */
TR_BitVector* TR_OSRLiveVariableInformation::getLiveSymbolsInInterpreter(TR_ByteCodeInfo& byteCodeInfo)
{
    ...
    if (bcLiveSymbolMaps[callerIndex + 1] == NULL) {
        bcLiveSymbolMap
            = new TR_BCLiveSymbolsMap(TR_BCLiveSymbolsMapComparator(), TR_BCLiveSymbolsMapAllocator(comp()->region()));
    ...
    OMR::ResolvedMethodSymbol* methodSymbol
...
TR_OSRLiveVariableInformation::TR_OSRLiveVariableInformation(TR::Compilation* c, TR::Optimizer* optimizer,
    TR_Structure* rootStructure, bool splitLongs, bool includeParms, bool ignoreOSRUses)
    : TR_LiveVariableInformation(c, optimizer, rootStructure, splitLongs, includeParms, ignoreOSRUses)
{
    ...
    bcLiveSymbolMaps
        = (TR_BCLiveSymbolsMap**)comp()->trMemory()->allocateHeapMemory(numOfMethods * sizeof(TR_BCLiveSymbolsMap*));
