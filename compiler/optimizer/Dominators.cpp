...
int TR_Dominators::dominates(TR::Block* block, TR::Block* other)
{
    ...
    if (other == block)
        return 1;
    for (TR::Block* d = other; d != NULL && _dfNumbers[d->getNumber()] >= _dfNumbers[block->getNumber()];
         d = getDominator(d)) {
        if (d == block)
            return 1;
    }
...
void TR_Dominators::initialize(TR::Block* start, BBInfo* nullParent)
{
    ...
    while (!stack.empty()) {
        ...
        //
        StackInfo::list_type& exceptionSuccessors
...
void TR_PostDominators::findControlDependents()
{
    ...
    for (i = 0; i < nextNodeNumber; i++)
        _directControlDependents[i]
            = new (_region) TR_BitVector(nextNodeNumber, _region); //.AddEntry(comp()->allocator("PostDominators"));
