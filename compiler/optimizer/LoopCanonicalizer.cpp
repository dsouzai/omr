...
void TR_LoopTransformer::createWhileLoopsList(TR_ScratchList<TR_Structure> *whileLoops)
{
    ...
    if (asLoopVersioner())
        detectWhileLoops(whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _rootStructure,
            false);
    else
        detectWhileLoops(whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _rootStructure, true);
        ...
        if (!isDoWhileLoop && loopHeader && (numSubNodes > 1)) {
            ...
            if ((hdrBlock->hasExceptionPredecessors() == false) && (hdrBlock->hasExceptionSuccessors() == false)) {
                ...
                if (entryNode->getSuccessors().size() == 2) {
                    ...
                    ++it;
                    TR::CFGEdge *succ2 = *it;
                    bool succ1Internal = region->contains(toStructureSubGraphNode(succ1->getTo())->getStructure(),
                        region->getParent());
                    bool succ2Internal = region->contains(toStructureSubGraphNode(succ2->getTo())->getStructure(),
                        region->getParent());
...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure *whileLoop)
{
    ...
    */
    ...
    TR::Block *splitter1 = TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(), comp(),
        sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    TR::Block *splitter2 = TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(), comp(),
        sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    TR::Block *extraGoto = NULL;
    if (reversedBranch) {
        extraGoto = TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(), comp(),
            sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    }
...
bool TR_LoopCanonicalizer::isLegalToSplitEdges(TR_RegionStructure *doWhileLoop, TR::Block *blockAtHeadOfLoop)
{
    ...
    int32_t sumPredFreq;
    return modifyBranchesForSplitEdges(doWhileLoop, blockAtHeadOfLoop, NULL, NULL, false, &sumPredFreq,
        /* isCheckOnly = */ true);
}
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure *doWhileLoop)
{
    ...
    if (isEntry) {
        dummyEntryBlock = TR::Block::createEmptyBlock(predBlockExitNode, comp(), blockAtHeadOfLoop->getFrequency(),
            blockAtHeadOfLoop);
    ...
    int32_t sumPredFreq;
    modifyBranchesForSplitEdges(doWhileLoop, blockAtHeadOfLoop, loopInvariantBlock, targetBlock, addToEnd,
        &sumPredFreq);
    ...
    if (isEntry) {
        ...
        doWhileNode->getStructure()->renumberRecursively(doWhileNode->getNumber(),
            dummyEntryBlockStructure->getNumber());
                ...
                if (performElimination) {
                    ...
                    if ((!comp()->cg()->hasComplexAddressingMode()) || checkComplexInductionVariableUse(naturalLoop)) {
                        if (performTransformation(comp(), "%sRemoving redundant induction variable #%d\n", OPT_DETAILS,
                            ...
                            replaceAllInductionVariableComputations(loopInvariantBlock->getBlock(), naturalLoop,
                                &newSymbolReference, symRefInCompare);
                    }
    ...
    for (auto edge = _entryBlock->getSuccessors().begin(); edge != _entryBlock->getSuccessors().end();) {
        ...
        if (!structure->contains(block->getStructureOf())) {
            ...
            if (_primaryIncrementedFirst != 0) {
                ...
                subNode = TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2, subNode,
            ...
            TR::Node *storeNode = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, subNode,
                derivedInductionVar);
    ...
    if (node->getOpCode().hasSymbolReference() && !node->getOpCode().isStore()) {
        if ((node->getSymbolReference() == _symRefBeingReplaced)
            ...
            if (!loadOfPrimaryInductionVar)
                loadOfPrimaryInductionVar = TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType),
                    0, _primaryInductionVariable);
    ...
    TR::SymbolReference **newSymbolReference, TR::SymbolReference *inductionVarSymRef)
{
    ...
    if (structure->asBlock() != NULL) {
        ...
        while (currentTree != exitTree) {
            ...
            if (examineTreeForInductionVariableUse(loopInvariantBlock, NULL, -1, currentNode, comp()->getVisitCount(),
                    newSymbolReference))
                seenInductionVariableComputation = true;
        ...
        TR_RegionStructure::Cursor si(*regionStructure);
        for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
            if (replaceAllInductionVariableComputations(loopInvariantBlock, node->getStructure(), newSymbolReference,
                    inductionVarSymRef))
                seenInductionVariableComputation = true;
        }
    }
...
TR_LoopTransformer::TR_TransformerDefUseState TR_LoopTransformer::getSymbolDefUseStateInSubTree(TR::Node *node,
    TR::RegisterMappedSymbol *indVarSym)
{
...
TR_LoopTransformer::TR_TransformerDefUseState TR_LoopTransformer::getSymbolDefUseStateInBlock(TR::Block *block,
    TR::RegisterMappedSymbol *indVarSym)
{
...
bool TR_LoopTransformer::isSymbolReferenceWrittenNumberOfTimesInStructure(TR_Structure *structure, int32_t symRefNum,
    int32_t *numTimesSeen, int32_t maxTimes)
{
    if (structure->asBlock() != NULL) {
        ...
        TR_RegionStructure::Cursor si(*regionStructure);
        for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
            if (!isSymbolReferenceWrittenNumberOfTimesInStructure(node->getStructure(), symRefNum, numTimesSeen,
                    maxTimes))
                return false;
        }
    }
    ...
    switch (node->getOpCodeValue()) {
        case TR::ificmplt: {
            ...
            subNode = TR::Node::create(TR::isub, 2, node->getSecondChild()->duplicateTree(),
                node->getFirstChild()->duplicateTree());
            break;
        }
        case TR::ificmple: {
            // if (!_isAddition)
        ...
        case TR::ificmpgt: {
            ...
            subNode = TR::Node::create(TR::isub, 2, node->getFirstChild()->duplicateTree(),
                node->getSecondChild()->duplicateTree());
            break;
        }
        case TR::ificmpge: {
            // if (_isAddition)
...
TR::Node *TR_LoopTransformer::containsOnlyInductionVariableAndAdditiveConstant(TR::Node *node,
    int32_t inductionVariable)
{
    TR::ILOpCode &opCode = node->getOpCode();
 ...
 */
bool TR_LoopTransformer::blockIsAlwaysExecutedInLoop(TR::Block *queryBlock, TR_RegionStructure *loopStructure,
...
int32_t TR_RedundantInductionVarElimination::perform()
{
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        TR_RegionStructure *naturalLoop = nextWhileLoop->asRegion();
        TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),
            "redundantInductionVarElimination, expecting natural loop");
        if (naturalLoop->getEntryBlock()->isCold())
            continue;
        eliminateRedundantInductionVariablesFromLoop(naturalLoop);
    }
...
int32_t TR_LoopInverter::detectCanonicalizedPredictableLoops(TR_Structure *loopStructure, TR_BitVector **optSetInfo,
        ...
        if (isInvertible
            ...
            if (_loopTestTree->getNode()->getSecondChild()->getOpCode().isLoadConst()) {
                ...
                storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
                    TR::Node::create(loopTestNode, TR::iconst, 0, initValue), nextInductionSymRef);
            } else {
                ...
                for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
                    ...
                    if (!blocksInRegion.find(next)) {
                        ...
                        if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
                            storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1, rhsOfStoreNode->duplicateTree(),
                                nextInductionSymRef);
                        else {
                            if (origOpCodeWasAdd)
                                storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
 ...
 */
bool TR_LoopInverter::checkIfSymbolIsReadInKnownTree(TR::Node *node, int32_t symRefNum, TR::TreeTop *currentTree,
