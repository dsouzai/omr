...
int32_t TR_LoopCanonicalizer::perform()
{
...

    {
        ...
        for (nextCleansedBlock = cleansedBlocksIt.getCurrent(); nextCleansedBlock;
             nextCleansedBlock = cleansedBlocksIt.getNext()) {
        ...
        for (nextInvariantBlock = invariantBlocksIt.getCurrent(); nextInvariantBlock;
             nextInvariantBlock = invariantBlocksIt.getNext()) {
        ...
        /*
           ...
           for (nextDoWhileLoop = doWhileLoopsIt.getFirst(); nextDoWhileLoop != NULL; nextDoWhileLoop =
           doWhileLoopsIt.getNext())
              {
              TR_RegionStructure *naturalLoop = nextDoWhileLoop->asRegion();
              TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),"Loop canonicalizer, expecting natural loop");
              if (naturalLoop->getEntryBlock()->isCold())
...
void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(ListAppender<TR_Structure>& whileLoopsInnerFirst,
    List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst,
    ...
    TR_RegionStructure* region, vcount_t visitCount, TR_BitVector* pendingList, bool innerFirst)
{
    ...
    for (auto pred = rootNode->getPredecessors().begin(); pred != rootNode->getPredecessors().end(); ++pred) {
        ...
        if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
            detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops,
                predStructure, predNode, region, visitCount, pendingList, innerFirst);
            return;
        }
    }

    for (auto pred = rootNode->getExceptionPredecessors().begin(); pred != rootNode->getExceptionPredecessors().end();
         ++pred) {
        TR_StructureSubGraphNode* predNode = (TR_StructureSubGraphNode*)(*pred)->getFrom();
        TR_Structure* predStructure = predNode->getStructure();
        if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
    ...
    for (auto succ = rootNode->getSuccessors().begin(); succ != rootNode->getSuccessors().end(); ++succ) {
        ...
        if ((!isExitEdge) && pendingList->get(succStructure->getNumber())) {
            ...
            _nodesInCycle->empty();
            detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops,
                succStructure, succNode, region, visitCount, pendingList, innerFirst);
        }
    }

    for (auto succ = rootNode->getExceptionSuccessors().begin(); succ != rootNode->getExceptionSuccessors().end();
         ++succ) {
        TR_StructureSubGraphNode* succNode = (TR_StructureSubGraphNode*)(*succ)->getTo();
        TR_Structure* succStructure = succNode->getStructure();
        bool isExitEdge = false;
...
void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(ListAppender<TR_Structure>& whileLoopsInnerFirst,
    List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst,
    List<TR_Structure>& doWhileLoops, TR_RegionStructure* region, vcount_t visitCount, TR_BitVector* pendingList,
    bool innerFirst)
{
    while (!_analysisStack.isEmpty() && _analysisStack.top()->getStructure() != region) {
        ...
        if (trace())
            traceMsg(
                comp(), "Begin looking for canonicalizable loops in node %p numbered %d\n", root, root->getNumber());
        ...
        if (!_nodesInCycle->get(rootNode->getNumber())) {
            ...
            for (auto precedingEdge = precedingEdges.getFirst(); precedingEdge;
                 precedingEdge = precedingEdges.getNext()) {
        ...
        for (auto succeedingEdge = succeedingEdges.getFirst(); succeedingEdge;
             succeedingEdge = succeedingEdges.getNext()) {
...
void TR_LoopTransformer::createWhileLoopsList(TR_ScratchList<TR_Structure>* whileLoops)
{
    ...
    if (asLoopVersioner())
        detectWhileLoops(
            whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _rootStructure, false);
    else
        detectWhileLoops(whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _rootStructure, true);
 ...
 */
void TR_LoopTransformer::detectWhileLoops(ListAppender<TR_Structure>& whileLoopsInnerFirst,
    List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst,
    List<TR_Structure>& doWhileLoops, TR_Structure* root, bool innerFirst)
{
    ...
    // if (debug("nonRecursiveBVA"))
    {
        ...
        detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops,
            region, visitCount, pendingList, innerFirst);
    }
    /*
 else
    detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops,
 region->getEntry()->getStructure(), region->getEntry(), region, visitCount, pendingList, innerFirst);
    */
    ...
    */
    ...
    if (asLoopCanonicalizer()) {
        ...
        /////   return;
        /////
        if ((loopHeader && loopHeader->getBlock()->getNextBlock()
                && loopHeader->getBlock()->getNextBlock()->isExtensionOfPreviousBlock())
            || (loopHeader
                && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets())
            || (loopHeader
                && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isCompBranchOnly())) {
            isDoWhileLoop = true;
        }
        if (!isDoWhileLoop && loopHeader && (numSubNodes > 1)) {
            ...
            if ((hdrBlock->hasExceptionPredecessors() == false) && (hdrBlock->hasExceptionSuccessors() == false)) {
                ...
                if (entryNode->getSuccessors().size() == 2) {
                    ...
                    ++it;
                    TR::CFGEdge* succ2 = *it;
                    bool succ1Internal = region->contains(
                        toStructureSubGraphNode(succ1->getTo())->getStructure(), region->getParent());
                    bool succ2Internal = region->contains(
                        toStructureSubGraphNode(succ2->getTo())->getStructure(), region->getParent());
                    if (succ1Internal ^ succ2Internal) {
                        ...
                        if (firstTree == hdrBlock->getLastRealTreeTop()
                            ...
                            if (!exitBlocks.isSingleton()) {
                                ...
                                bool foundInductionVariable = findMatchingIVInRegion(hdrBlockLoopTestTree, region);
                                    ...
                                    for (exitBlock = blocksIt.getCurrent(); exitBlock; exitBlock = blocksIt.getNext()) {
                                        if (exitBlock != hdrBlock) {
                                            if (exitBlock->getSuccessors().size() == 2) {
                                                ...
                                                bool successor1Internal = region->contains(
                                                    toBlock(exitSucc1->getTo())->getStructureOf(), region->getParent());
                                                bool successor2Internal = region->contains(
                                                    toBlock(exitSucc2->getTo())->getStructureOf(), region->getParent());
                                                if (successor1Internal ^ successor2Internal) {
                                                    TR::TreeTop* exitBlockLoopTestTree
                                                        = exitBlock->getLastRealTreeTop();
                                                    if (exitBlockLoopTestTree->getNode()->getOpCode().isCompBranchOnly()
                                                        || exitBlockLoopTestTree->getNode()
                                                               ->getOpCode()
                                                               .isJumpWithMultipleTargets())
                                                        continue;
            ...
            for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
                ...
                if (!(subNode->getExceptionPredecessors().empty() &&
                    ...
                    */
                    ...
                    TR_BlockStructure* pred = toStructureSubGraphNode(subNode->getPredecessors().front()->getFrom())
                                                  ->getStructure()
                                                  ->asBlock();
                    if (!pred || !pred->isLoopInvariantBlock())
                        return;
        ...
        // bool pred1Internal = region->contains(toStructureSubGraphNode(pred1->getFrom())->getStructure(),
        // region->getParent()); bool pred2Internal =
        // region->contains(toStructureSubGraphNode(pred2->getFrom())->getStructure(), region->getParent()); if
        // (!(pred1Internal ^ pred2Internal))
        //    return;
    }
...
bool TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop* loopTestTree, TR_RegionStructure* region)
{
    ...
    if (firstChild->getOpCode().hasSymbolReference())
        ...
        if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
            && (firstChild->getReferenceCount() > 1) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure* whileLoop)
{
    ...
    for (auto nextSucc = loopHeader->getSuccessors().begin(); nextSucc != loopHeader->getSuccessors().end();
         ++nextSucc) {
    ...
    if (!_blocksToBeCleansed.find(loopHeader))
        ...
        TR_ASSERT(loopHeader->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto,
            "Unexpected opcode instead of TR::Goto\n");
    ...
    for (auto edge = loopHeader->getPredecessors().begin(); edge != loopHeader->getPredecessors().end(); ++edge) {
        ...
        if (!whileLoop->contains(pred->getStructureOf(), whileLoop->getParent())) {
                ...
                if (lastNonFenceTree->getNode()->getOpCode().isBranch()) {
                    ...
                    if (lastNonFenceTree->getNode()->getBranchDestination() != entryTree) {
                        ...
                        TR::TreeTop* gotoTreeTop
                            = TR::TreeTop::create(comp(), TR::Node::create(entryNode, TR::Goto, 0, entryTree));
            ...
            if (pred != _cfg->getStart()) {
                ...
                if (lastNonFenceTree->getNode()->getOpCode().isBranch()
                    || lastNonFenceTree->getNode()->getOpCode().isJumpWithMultipleTargets()) {
                    ...
                    if (!lastNonFenceTree->adjustBranchOrSwitchTreeTop(comp(), entryTree, newEntryTree)) {
                        TR::TreeTop* gotoTreeTop
                            = TR::TreeTop::create(comp(), TR::Node::create(entryNode, TR::Goto, 0, newEntryTree));
                        TR::TreeTop* treeTop = lastNonFenceTree->getNextTreeTop();
                        lastNonFenceTree->join(gotoTreeTop);
                        gotoTreeTop->join(treeTop);
                        _blocksToBeCleansed.add(pred);
    ...
    //
    if (((tempTree != NULL) && (tempTree->getNode()->getBlock() != loopBody))
        && (entryTree != comp()->getStartTree())) {
    ...
    */
    ...
    TR::Block* splitter1 = TR::Block::createEmptyBlock(
        loopHeader->getEntry()->getNode(), comp(), sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    TR::Block* splitter2 = TR::Block::createEmptyBlock(
        loopHeader->getEntry()->getNode(), comp(), sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    TR::Block* extraGoto = NULL;
    if (reversedBranch) {
        extraGoto = TR::Block::createEmptyBlock(
            loopHeader->getEntry()->getNode(), comp(), sumPredFreq < 0 ? 0 : sumPredFreq, loopHeader);
    }
    ...
    //
    TR::CFGEdge* newEdge;
    for (auto nextEdge = loopHeader->getSuccessors().begin(); nextEdge != loopHeader->getSuccessors().end();
         ++nextEdge) {
    ...
    TR_RegionStructure* properRegion
        = new (_cfg->structureMemoryRegion()) TR_RegionStructure(comp(), loopHeader->getNumber());
    ...
    TR_BlockStructure* clonedHdrBlockStructure
        = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), clonedHeader->getNumber(), clonedHeader);
    if (needToSetFlag) {
    ...
    TR_StructureSubGraphNode* hdrNode
        = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(loopHeader->getStructureOf());
    properRegion->addSubNode(hdrNode);
    properRegion->setEntry(hdrNode);

    TR_StructureSubGraphNode* node = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(
        new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), splitter2->getNumber(), splitter2));
    ...
    node = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(
        new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), splitter1->getNumber(), splitter1));
    properRegion->addSubNode(node);
    TR::CFGEdge::createEdge(hdrNode, node, trMemory());
    ...
    if (reversedBranch) {
        ...
        TR_BlockStructure* extraGotoStruct
            = new (_cfg->structureMemoryRegion()) TR_BlockStructure(comp(), extraGoto->getNumber(), extraGoto);
...
bool TR_LoopCanonicalizer::isLegalToSplitEdges(TR_RegionStructure* doWhileLoop, TR::Block* blockAtHeadOfLoop)
{
    ...
    int32_t sumPredFreq;
    return modifyBranchesForSplitEdges(
        doWhileLoop, blockAtHeadOfLoop, NULL, NULL, false, &sumPredFreq, /* isCheckOnly = */ true);
}
 ...
 */
bool TR_LoopCanonicalizer::modifyBranchesForSplitEdges(TR_RegionStructure* doWhileLoop, TR::Block* blockAtHeadOfLoop,
    TR::Block* loopInvariantBlock, TR::Block* targetBlock, bool addToEnd, int32_t* sumPredFreq, bool isCheckOnly)
{
    TR_ASSERT((!isCheckOnly) || (isCheckOnly && (loopInvariantBlock == NULL) && (targetBlock == NULL)),
        "Check should be made only with loopInvariantBlock and targetBlock NULL");

    *sumPredFreq = 0;
    TR::TreeTop* blockHeadTreeTop = blockAtHeadOfLoop->getEntry();

    for (auto edge = blockAtHeadOfLoop->getPredecessors().begin();
         edge != blockAtHeadOfLoop->getPredecessors().end();) {
        TR::Block* pred = toBlock((*edge)->getFrom());
        if ((!doWhileLoop->contains(pred->getStructureOf())) && (pred != loopInvariantBlock)) {
            *sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
            if (pred != _cfg->getStart()) {
                    ...
                    if (!lastTree->adjustBranchOrSwitchTreeTop(comp(), blockHeadTreeTop, targetBlock->getEntry())) {
                        if (addToEnd) {
                            ...
                            if ((targetBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto)
                                && (targetBlock->getLastRealTreeTop()->getNode()->getBranchDestination()
                                    == blockHeadTreeTop))
                ...
                TR_ASSERT(isCheckOnly,
                    "Cannot reach here if preparation work for the loopInvariantBlock has been done correctly\n");
                if (!isCheckOnly) {
                    TR::TreeTop* targetEntry = targetBlock->getEntry();
                    TR::TreeTop* targetExit = targetBlock->getExit();
                    TR::TreeTop* prevTree = targetEntry->getPrevTreeTop();
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure* doWhileLoop)
{
    ...
    {
        for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin();
             iedge != blockAtHeadOfLoop->getPredecessors().end(); ++iedge) {
    ...
    TR::Node* predBlockExitNode
        = blockAtHeadOfLoop->getPredecessors().front()->getFrom()->asBlock()->getExit()->getNode();

    TR::Block* loopInvariantBlock
        = TR::Block::createEmptyBlock(predBlockExitNode, comp(), blockAtHeadOfLoop->getFrequency(), blockAtHeadOfLoop);
    ...
    if (doWhileLoop->contains(cursorTree->getNode()->getBlock()->getStructureOf())) {
        ...
        TR::TreeTop* gotoTreeTop
            = TR::TreeTop::create(comp(), TR::Node::create(blockHeadNode, TR::Goto, 0, blockHeadTreeTop));
    ...
    TR_BlockStructure* invariantBlockStructure = new (_cfg->structureMemoryRegion())
        TR_BlockStructure(comp(), loopInvariantBlock->getNumber(), loopInvariantBlock);
    ...
    if (isEntry) {
        dummyEntryBlock = TR::Block::createEmptyBlock(
            predBlockExitNode, comp(), blockAtHeadOfLoop->getFrequency(), blockAtHeadOfLoop);
        ...
        if (addToEnd) {
            ...
            TR::TreeTop* gotoTreeTop
                = TR::TreeTop::create(comp(), TR::Node::create(blockHeadNode, TR::Goto, 0, newEntry));
        ...
        dummyEntryBlockStructure = new (_cfg->structureMemoryRegion())
            TR_BlockStructure(comp(), dummyEntryBlock->getNumber(), dummyEntryBlock);
        targetBlock = dummyEntryBlock;
    } else
        targetBlock = loopInvariantBlock;

    _cfg->setStructure(NULL);
    TR::CFGEdge* loopInvariantBlock_blockAtHeadOfLoop_Edge
        = TR::CFGEdge::createEdge(loopInvariantBlock, blockAtHeadOfLoop, trMemory());
    _cfg->addEdge(loopInvariantBlock_blockAtHeadOfLoop_Edge);
    TR::CFGEdge* dummyEntryBlock_loopInvariantBlock_Edge = NULL;
    if (isEntry) {
        dummyEntryBlock_loopInvariantBlock_Edge
            = TR::CFGEdge::createEdge(dummyEntryBlock, loopInvariantBlock, trMemory());
        _cfg->addEdge(dummyEntryBlock_loopInvariantBlock_Edge);
    }

    int32_t sumPredFreq;
    modifyBranchesForSplitEdges(
        doWhileLoop, blockAtHeadOfLoop, loopInvariantBlock, targetBlock, addToEnd, &sumPredFreq);
    ...
    if (isEntry) {
        TR_StructureSubGraphNode* dummyEntryNode
            = new (_cfg->structureMemoryRegion()) TR_StructureSubGraphNode(dummyEntryBlockStructure);
        ...
        doWhileNode->getStructure()->renumberRecursively(
            doWhileNode->getNumber(), dummyEntryBlockStructure->getNumber());
    ...
    for (auto predIt = doWhileNode->getExceptionPredecessors().begin();
         predIt != doWhileNode->getExceptionPredecessors().end();) {
        TR::CFGEdge* nextPred = *(predIt++);
...
void TR_LoopCanonicalizer::eliminateRedundantInductionVariablesFromLoop(TR_RegionStructure* naturalLoop)
{
    ...
    if (!naturalLoop->getParent() || naturalLoop->getEntryBlock()->isCold())
        return;
    ...
    for (auto nextEdge = entryBlock->getPredecessors().begin(); nextEdge != entryBlock->getPredecessors().end();
        ...
        if (nextBlock != loopInvariantBlock->getBlock()) {
            //// Back edge found
            ////
            if (nextBlock->getLastRealTreeTop()->getNode()->getOpCode().isIf()
                && !nextBlock->getLastRealTreeTop()->getNode()->getOpCode().isCompBranchOnly()) {
    ...
    if (primaryInductionVariable && bivIterator.getFirst()) {
        ...
        static bool disableIVEPostDominatorsCheck
            = (comp()->cg()->hasComplexAddressingMode() || NULL != feGetEnv("TR_DisableIVEPostDominatorsCheck"));
        ...
        for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
            ...
            if ((nextIndVar->getSymRef()->getSymbol()->getDataType()
                ...
                // comp()->incVisitCount();
                bool performElimination
                    = incrementedInLockStep(naturalLoop, derivedInductionVar, symRefInCompare, nextIndVarIncr,
                        primaryIncr, &derivedInductionVarIncrementBlocks, &primaryInductionVarIncrementBlocks);
                if (performElimination
                    ...
                    if (performElimination)
                        performElimination = checkIfOrderOfBlocksIsKnown(naturalLoop, entryBlock, loopTestBlock,
                            &derivedInductionVarIncrementBlocks, &primaryInductionVarIncrementBlocks,
                            _primaryIncrementedFirst);
                }
                ...
                if (performElimination) {
                    ...
                    if ((!comp()->cg()->hasComplexAddressingMode()) || checkComplexInductionVariableUse(naturalLoop)) {
                        if (performTransformation(comp(), "%sRemoving redundant induction variable #%d\n", OPT_DETAILS,
                                derivedInductionVar->getReferenceNumber()))
                            replaceAllInductionVariableComputations(
                                loopInvariantBlock->getBlock(), naturalLoop, &newSymbolReference, symRefInCompare);
                    }

                    if (newSymbolReference) {
                        optimizer()->setAliasSetsAreValid(false);
                        placeInitializationTreeInLoopPreHeader(loopInvariantBlock->getBlock(),
                            loopInvariantBlock->getBlock()->getEntry()->getNode(), newSymbolReference, symRefInCompare,
                            derivedInductionVar);
                        replaceInductionVariableComputationsInExits(naturalLoop,
                            loopInvariantBlock->getBlock()->getEntry()->getNode(), newSymbolReference, symRefInCompare,
                            derivedInductionVar);
        ...
        if (alteredCode) {
            if (loopInvariantBlock->getBlock()->getLastRealTreeTop()->getNode()->getOpCodeValue() != TR::BBStart) {
                ...
                // TR_ASSERT((loopInvariantBlock->getBlock()->getLastRealTreeTop()->getNode()->getOpCodeValue() ==
                // TR::Goto), "Expect the loop pre-header to end in a goto for call to TR::Block::split to work
                // correctly\n");
                TR::Block* newLoopInvariantBlock = loopInvariantBlock->getBlock()->split(
                    loopInvariantBlock->getBlock()->getLastRealTreeTop(), comp()->getFlowGraph(), false);
...
bool TR_LoopCanonicalizer::replaceInductionVariableComputationsInExits(TR_Structure* structure, TR::Node* node,
    TR::SymbolReference* newSymbolReference, TR::SymbolReference* primaryInductionVar,
    TR::SymbolReference* derivedInductionVar)
{
    ...
    for (auto edge = _entryBlock->getSuccessors().begin(); edge != _entryBlock->getSuccessors().end();) {
        ...
        if (!structure->contains(block->getStructureOf())) {
            TR::Block* newBlock
                = (*edge)->getFrom()->asBlock()->splitEdge((*edge)->getFrom()->asBlock(), block, comp());
            ...
            if (_primaryIncrementedFirst != 0) {
                ...
                subNode = TR::Node::create(
                    ((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2, subNode, adjustmentConstChild);
            ...
            TR::Node* storeNode = TR::Node::createWithSymRef(
                comp()->il.opCodeForDirectStore(dataType), 1, 1, subNode, derivedInductionVar);
...
void TR_LoopCanonicalizer::findIncrements(TR::Node* currentNode, vcount_t visitCount,
    TR::SymbolReference* derivedInductionVar, TR::SymbolReference* primaryInductionVar,
    int64_t& derivedInductionVarIncrement, int64_t& primaryInductionVarIncrement, bool& unknownIncrement)
{
    ...
    if (currentNode->getOpCode().isStore()) {
        ...
        if (currentNode->getSymbolReference() == derivedInductionVar) {
            ...
            if ((derivedInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
            ...
            } else if ((derivedInductionVarIncrement == 0) && child->getOpCode().isSub()) {
                TR::Node* secondChild = child->getSecondChild();
                if (secondChild->getOpCode().isLoadConst()) {
                    TR::Node* firstChild = child->getFirstChild();
                    int64_t curIncr;
            ...
            bool incrInStandardForm = false;
            if ((primaryInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
                TR::Node* secondChild = child->getSecondChild();
                if (secondChild->getOpCode().isLoadConst()) {
                    TR::Node* firstChild = child->getFirstChild();
                    int64_t curIncr;
            ...
            } else if ((primaryInductionVarIncrement == 0) && child->getOpCode().isSub()) {
                TR::Node* secondChild = child->getSecondChild();
                if (secondChild->getOpCode().isLoadConst()) {
                    TR::Node* firstChild = child->getFirstChild();
                    int64_t curIncr;
    ...
    for (i = 0; i < currentNode->getNumChildren(); i++) {
        ...
        if (child->getOpCode().isStore())
            findIncrements(child, visitCount, derivedInductionVar, primaryInductionVar, derivedInductionVarIncrement,
                primaryInductionVarIncrement, unknownIncrement);
    }
}

bool TR_LoopCanonicalizer::incrementedInLockStep(TR_Structure* structure, TR::SymbolReference* derivedInductionVar,
    TR::SymbolReference* primaryInductionVar, int64_t derivedInductionVarLoopIncrement,
    int64_t primaryInductionVarLoopIncrement, TR_ScratchList<TR::Block>* derivedInductionVarIncrementBlocks,
    TR_ScratchList<TR::Block>* primaryInductionVarIncrementBlocks)
{
    if (structure->asBlock() != NULL) {
        ...
        while (currentTree != exitTree) {
            ...
            findIncrements(currentNode, comp()->getVisitCount(), derivedInductionVar, primaryInductionVar,
                derivedInductionVarIncrement, primaryInductionVarIncrement, unknownIncrement);
            if (unknownIncrement)
                return false;
            currentTree = currentTree->getNextTreeTop();
        }
        ...
        for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
            if (!incrementedInLockStep(node->getStructure(), derivedInductionVar, primaryInductionVar,
                    derivedInductionVarLoopIncrement, primaryInductionVarLoopIncrement,
                    derivedInductionVarIncrementBlocks, primaryInductionVarIncrementBlocks))
                return false;
        }
        return true;
...
bool TR_LoopCanonicalizer::checkIfOrderOfBlocksIsKnown(TR_RegionStructure* naturalLoop, TR::Block* entryBlock,
    TR::Block* loopTestBlock, TR_ScratchList<TR::Block>* derivedInductionVarIncrementBlocks,
    TR_ScratchList<TR::Block>* primaryInductionVarIncrementBlocks, uint8_t& primaryFirst)
{
    ...
    for (auto edge = entryBlock->getExceptionSuccessors().begin(); edge != entryBlock->getExceptionSuccessors().end();
         ++edge) {
    ...
    for (auto edge = loopTestBlock->getExceptionSuccessors().begin();
         edge != loopTestBlock->getExceptionSuccessors().end(); ++edge) {
        TR::Block* catchBlock = (*edge)->getTo()->asBlock();
        if (naturalLoop->contains(catchBlock->getStructureOf()))
            return false;
    }
    ...
    if (entryBlock == primaryBlock) {
        ...
        if (loopTestBlock == derivedBlock) {
            ...
            if ((onlyPredOfLoopTestBlock->getSuccessors().size() == 1) && (onlyPredOfLoopTestBlock == derivedBlock)) {
        ...
        if (loopTestBlock == primaryBlock) {
            ...
            TR::Block* onlyPredOfLoopTestBlock = loopTestBlock->getPredecessors().front()->getFrom()->asBlock();
            if ((onlyPredOfLoopTestBlock->getSuccessors().size() == 1) && (onlyPredOfLoopTestBlock == primaryBlock)) {
...
bool TR_LoopCanonicalizer::examineTreeForInductionVariableUse(TR::Block* loopInvariantBlock, TR::Node* parent,
    int32_t childNum, TR::Node* node, vcount_t visitCount, TR::SymbolReference** newSymbolReference)
{
    ...
    if (node->getOpCode().hasSymbolReference() && !node->getOpCode().isStore()) {
        if ((node->getSymbolReference() == _symRefBeingReplaced)
            && performTransformation(comp(), "Replacing use %p of sym ref #%d by sym ref #%d\n", node,
                node->getSymbolReference()->getReferenceNumber(), _primaryInductionVariable->getReferenceNumber())) {
            TR::DataType dataType = node->getDataType();
            if (!(*newSymbolReference))
                *newSymbolReference
                    = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType, false);
                ...
                if (!_derivedInductionVarStoreInBlock) {
                    if (!_primaryInductionVarStoreSomewhereInBlock) {
                        ...
                        while (curTree != exitTree) {
                            ...
                            if (!curNode->getOpCode().isStore() && (curNode->getNumChildren() > 0))
                    ...
                    if (_primaryInductionVarStoreSomewhereInBlock)
                        loadOfPrimaryInductionVar
                            = _primaryInductionVarStoreSomewhereInBlock->getFirstChild()->getFirstChild();
                }
            }

            if (!loadOfPrimaryInductionVar)
                loadOfPrimaryInductionVar = TR::Node::createWithSymRef(
                    node, comp()->il.opCodeForDirectLoad(dataType), 0, _primaryInductionVariable);

            loadOfPrimaryInductionVar
                = TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2, loadOfPrimaryInductionVar,
                    TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType), 0, *newSymbolReference));
            ...
            if (_primaryIncrementedFirst == 0) {
                if (_derivedInductionVarStoreInBlock) {
                    if (!_primaryInductionVarStoreInBlock) {
                        ...
                        TR::Node* adjustmentConstChild
                            = TR::Node::create(constChild, constChild->getOpCodeValue(), 0, 0);
                        if (adjustmentConstChild->getDataType() == TR::Int32)
                            adjustmentConstChild->setInt((int32_t)additiveConstant);
                        else
                            adjustmentConstChild->setLongInt(additiveConstant);
                ...
                } else if (0) // Do not need adjustment in this case since we have obtained the correctly commoned node
                              // in this case
                {
                    if (_primaryInductionVarStoreInBlock) {
                ...
                if (_primaryIncrementedFirst == 1) {
                    if (((_primaryInductionIncrementBlock != _currentBlock) || _primaryInductionVarStoreInBlock)
                        && ((_derivedInductionIncrementBlock != _currentBlock) || !_derivedInductionVarStoreInBlock)
                        && ((_derivedInductionIncrementBlock == _loopTestBlock) || (_currentBlock != _loopTestBlock))) {
                        int64_t additiveConstant = _primaryIncr;
                        TR::Node* adjustmentConstChild = 0;
                        if (dataType == TR::Int32)
                            adjustmentConstChild
                                = TR::Node::create(node, TR::iconst, 0, static_cast<int32_t>(-1 * additiveConstant));
                        else {
                            adjustmentConstChild = TR::Node::create(node, TR::lconst, 0, 0);
                            adjustmentConstChild->setLongInt(-1 * additiveConstant);
                        }
                    ...
                    if (((_primaryInductionIncrementBlock != _currentBlock) || !_primaryInductionVarStoreInBlock)
                        && ((_derivedInductionIncrementBlock != _currentBlock) || _derivedInductionVarStoreInBlock)
                        && ((_primaryInductionIncrementBlock == _loopTestBlock) || (_currentBlock != _loopTestBlock))) {
            ...
            //                                    TR::Node::create(node, comp()->il.opCodeForDirectLoad(dataType), 0,
            //                                    _primaryInductionVariable), TR::Node::create(node,
            //                                    comp()->il.opCodeForDirectLoad(dataType), 0, *newSymbolReference));
...
void TR_LoopCanonicalizer::placeInitializationTreeInLoopPreHeader(TR::Block* b, TR::Node* node,
    TR::SymbolReference* newSymbolReference, TR::SymbolReference* primaryInductionVar,
    TR::SymbolReference* derivedInductionVar)
{
    TR::DataType dataType = newSymbolReference->getSymbol()->getDataType();
    TR::Node* subNode = TR::Node::create((dataType == TR::Int32) ? TR::isub : TR::lsub, 2,
        ...
        TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType), 0, primaryInductionVar));
    TR::Node* storeNode
        = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, subNode, newSymbolReference);
    TR::TreeTop* newStoreTree = TR::TreeTop::create(comp(), storeNode, NULL, NULL);
...
{
    return node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getReferenceNumber() == symrefNum;
}
...
void TR_LoopCanonicalizer::rewritePostToPreIncrementTestInBlock(TR::Block* const block)
{
    ...
    const int ifOpIdx = int(ifOp.isCompareTrueIfGreater()) << 1 | int(ifOp.isCompareTrueIfEqual());
    ...
    if (!performTransformation(comp(), "%sChanging n%un (equivalently %s old-#%d n%un) to (%s n%un n%un)\n",
            optDetailString(), test->getGlobalIndex(), ifOp.getName(), symrefNum, right->getGlobalIndex(),
            adjustedIfOp.getName(), updated->getGlobalIndex(), right->getGlobalIndex()))
        return;
...
const char* TR_LoopCanonicalizer::optDetailString() const throw() { return "O^O LOOP CANONICALIZER: "; }

bool TR_LoopTransformer::replaceAllInductionVariableComputations(TR::Block* loopInvariantBlock, TR_Structure* structure,
    TR::SymbolReference** newSymbolReference, TR::SymbolReference* inductionVarSymRef)
{
    bool seenInductionVariableComputation = false;
    if (structure->asBlock() != NULL) {
        TR_BlockStructure* blockStructure = structure->asBlock();
        ...
        while (currentTree != exitTree) {
            ...
            if (examineTreeForInductionVariableUse(
                    loopInvariantBlock, NULL, -1, currentNode, comp()->getVisitCount(), newSymbolReference))
                seenInductionVariableComputation = true;
        ...
        for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
            if (replaceAllInductionVariableComputations(
                    loopInvariantBlock, node->getStructure(), newSymbolReference, inductionVarSymRef))
                seenInductionVariableComputation = true;
        }
    }
...
bool TR_LoopTransformer::cleanseTrees(TR::Block* block)
{
    ...
    //        treeTop = exitTreeTop->getNextTreeTop())
    {
        ...
        if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
            ...
            {
                TR_ASSERT(targetTree->getPrevTreeTop()->getNode()->getOpCodeValue() == TR::BBEnd,
                    "BBStart is not preceded by a BBEnd");
            ...
            if ((prevTreeOfTarget->getNode()->getOpCode().isBranch()
                    && ((prevTreeOfTarget->getNode()->getOpCodeValue() == TR::Goto)
                        || (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree)))
                || isPrevTreeOfTargetReturn || prevTreeOfTarget->getNode()->getOpCode().isSwitch()
                ...
                // if (isPrevTreeOfTargetReturn || ((prevTreeOfTarget->getNode()->getOpCode().getOpCodeValue() ==
                // TR::Goto) || (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree)))
                {
    ...
    //        treeTop = exitTreeTop->getNextTreeTop())
    {
        ...
        if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
            ...
            if ((prevTreeOfTarget->getNode()->getOpCode().isBranch()
                    && ((prevTreeOfTarget->getNode()->getOpCodeValue() == TR::Goto)
                        || (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree)))
                || isPrevTreeOfTargetReturn || (prevTreeOfTarget->getNode()->getOpCodeValue() == TR::athrow)) {
                // if (isPrevTreeOfTargetReturn || (prevTreeOfTarget->getNode()->getBranchDestination() != targetTree))
                {
                    ...
                    if (treeTop->getPrevTreeTop()) {
                        ...
                        if (!prevNode->getOpCode().isReturn()) {
                            if (!prevNode->getOpCode().isBranch()
                                || ((prevNode->getOpCodeValue() != TR::Goto)
                                    && (prevNode->getBranchDestination() != treeTop)))
...
int32_t TR_LoopTransformer::checkLoopForPredictability(TR_Structure* loopStructure, TR::Block* loopInvariantBlock,
    TR::Node** numberOfIterations, bool returnIfNotPredictable)
{
...
void TR_LoopTransformer::updateInfo(TR::Node* node, vcount_t visitCount, updateInfo_tables& uinfo)
{
    ...
    if ((node->getOpCode().isStore() && !node->isTheVirtualCallNodeForAGuardedInlinedCall()) || !_doingVersioning) {
...
void TR_LoopTransformer::collectSymbolsWrittenAndReadExactlyOnce(
    TR_Structure* structure, vcount_t visitCount, updateInfo_tables& uinfo)
{
    if (structure->asBlock() != NULL) {
        TR_BlockStructure* blockStructure = structure->asBlock();
        TR::Block* block = blockStructure->getBlock();
...
TR_LoopTransformer::TR_TransformerDefUseState TR_LoopTransformer::getSymbolDefUseStateInSubTree(
    TR::Node* node, TR::RegisterMappedSymbol* indVarSym)
{
...
TR_LoopTransformer::TR_TransformerDefUseState TR_LoopTransformer::getSymbolDefUseStateInBlock(
    TR::Block* block, TR::RegisterMappedSymbol* indVarSym)
{
...
bool TR_LoopTransformer::isSymbolReferenceWrittenNumberOfTimesInStructure(
    TR_Structure* structure, int32_t symRefNum, int32_t* numTimesSeen, int32_t maxTimes)
{
    if (structure->asBlock() != NULL) {
        ...
        while (currentTree != exitTree) {
            ...
            if (!defNode) {
                defNode = currentNode;
                if (currentNode->getOpCodeValue() == TR::treetop || currentNode->getOpCode().isCheck()) {
        ...
        for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
            if (!isSymbolReferenceWrittenNumberOfTimesInStructure(
                    node->getStructure(), symRefNum, numTimesSeen, maxTimes))
                return false;
        }
    }
...
TR::Node* TR_LoopTransformer::getCorrectNumberOfIterations(TR::Node* node, TR::Node* constNode)
{
    ...
    case TR::ificmplt: {
        ...
        subNode = TR::Node::create(
            TR::isub, 2, node->getSecondChild()->duplicateTree(), node->getFirstChild()->duplicateTree());
        break;
    }
    case TR::ificmple: {
        // if (!_isAddition)
        ...
        // subNode = TR::Node::create(TR::isub, 2, node->getSecondChild()->duplicateTree(),
        // node->getFirstChild()->duplicateTree()); subNode = TR::Node::create(TR::iadd, 2, subNode,
        // TR::Node::create(node, TR::iconst, 0, 1)); break;
    }
    case TR::ificmpgt: {
        if (_isAddition)
            return 0;
        ...
        subNode = TR::Node::create(
            TR::isub, 2, node->getFirstChild()->duplicateTree(), node->getSecondChild()->duplicateTree());
        break;
    }
    case TR::ificmpge: {
        // if (_isAddition)
        ...
        // subNode = TR::Node::create(TR::isub, 2, node->getFirstChild()->duplicateTree(),
        // node->getSecondChild()->duplicateTree()); subNode = TR::Node::create(TR::iadd, 2, subNode,
        // TR::Node::create(node, TR::iconst, 0, 1)); break;
    }
    default:
        return NULL;
...
TR::Node* TR_LoopTransformer::containsOnlyInductionVariableAndAdditiveConstant(
    TR::Node* node, int32_t inductionVariable)
{
    TR::ILOpCode& opCode = node->getOpCode();
...
const TR::BlockChecklist& TR_LoopTransformer::getLoopBlocksChecklist(TR_RegionStructure* loop)
{
...
bool TR_LoopTransformer::blockIsAlwaysExecutedInLoop(
    TR::Block* queryBlock, TR_RegionStructure* loopStructure, const TR::BlockChecklist** priorBlocks)
{
    ...
    TR_ASSERT_FATAL(loopBlocks.contains(queryBlock), "block_%d is not in loop %d", queryBlock->getNumber(),
    ...
    while (!queue.isEmpty()) {
        ...
        for (auto* edge = it.getFirst(); edge != NULL; edge = it.getNext()) {
            ...
            if (!loopBlocks.contains(succ) || seen.contains(succ) || succ == queryBlock)
                continue;
...
int32_t TR_RedundantInductionVarElimination::perform()
{
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        TR_RegionStructure* naturalLoop = nextWhileLoop->asRegion();
        TR_ASSERT(
            naturalLoop && naturalLoop->isNaturalLoop(), "redundantInductionVarElimination, expecting natural loop");
        if (naturalLoop->getEntryBlock()->isCold())
            continue;
        eliminateRedundantInductionVariablesFromLoop(naturalLoop);
    }
...
const char* TR_RedundantInductionVarElimination::optDetailString() const throw()
{
...
int32_t TR_LoopInverter::detectCanonicalizedPredictableLoops(
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!nextInductionSymRef->getUseonlyAliases().isZero(comp()) || !_storeTrees[nextInductionVariableNumber]
            || !_loopTestTree
            || ((_loopTestTree->getNode()->getOpCodeValue() != TR::ificmplt)
                && (_loopTestTree->getNode()->getOpCodeValue() != TR::ificmple))
            || !_isAddition || (exitBlocks.getListHead()->getData()->getLastRealTreeTop() != _loopTestTree)
        ...
        if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
            ...
            if (v) {
                ...
                if (entryVal && entryVal->asIntConst()
                    && (entryVal->getLowInt() == 0
                        || (exitVal && exitVal->asIntConst()
                            && (llabs((long long)(exitVal->getLowInt()) - (long long)(entryVal->getLowInt())))
                                < INT_MAX))
                    &&
                    // We cannot safely guess exit value of IV if it doesn't land on comparand(RhsOfStoreNode).
                    // Workaround is only allow abs(incr)==1.   This is also a hard requirement for unsigned IV to not
                    // introduce wraparound. For signed case, we may allow abs(incr)>1, but exit value calc is not just
                    // RhsOfStoreNode if it doesn't land on it. todo
                    incrVal->asIntConst() && abs(incrVal->getLowInt()) == 1) {
                    entryValue = entryVal->getLowInt();

                    if (exitVal && exitVal->asIntConst())
                        isInvertible = true;
        ...
        if (isInvertible
            ...
            if (_loopTestTree->getNode()->getSecondChild()->getOpCode().isLoadConst()) {
                ...
                if (incrVal->asIntConst() && incrVal->getLowInt() == 1 && testValue < INT_MAX
                ...
                storeNode = TR::Node::createWithSymRef(
                    TR::istore, 1, 1, TR::Node::create(loopTestNode, TR::iconst, 0, initValue), nextInductionSymRef);
            } else {
            ...
            dumpOptDetails(comp(), "Init node: %p, update node: %p, loopTest node: %p\n", storeNode,
                _storeTrees[nextInductionVariableNumber]->getNode(), _loopTestTree->getNode());
                ...
                for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
                    ...
                    if (!blocksInRegion.find(next)) {
                        ...
                        if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
                            storeNode = TR::Node::createWithSymRef(
                                TR::istore, 1, 1, rhsOfStoreNode->duplicateTree(), nextInductionSymRef);
                        else {
                            if (origOpCodeWasAdd)
                                storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
                                    TR::Node::create(TR::iadd, 2, rhsOfStoreNode->duplicateTree(),
                                        origStoreNode->getFirstChild()->getSecondChild()->duplicateTree()),
                                    nextInductionSymRef);
                            else
                                storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1,
                                    TR::Node::create(TR::isub, 2, rhsOfStoreNode->duplicateTree(),
                                        origStoreNode->getFirstChild()->getSecondChild()->duplicateTree()),
                                    nextInductionSymRef);
                        }
    ...
    if (structure->asBlock() != NULL) {
        ...
        while (currentTree != exitTree) {
            ...
            if (!checkIfSymbolIsReadInKnownTree(currentNode, symRefNum, currentTree, checklist)
                || ((currentNode->getOpCodeValue() != TR::asynccheck) && currentNode->canGCandReturn()))
                return false;
 ...
 */
bool TR_LoopInverter::checkIfSymbolIsReadInKnownTree(
    TR::Node* node, int32_t symRefNum, TR::TreeTop* currentTree, TR::NodeChecklist& checklist)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if ((aliasIndex == symRefNum) || (defAliases.hasAliases() && defAliases.contains(aliasIndex, comp())))
            return false;
    }
...
const char* TR_LoopInverter::optDetailString() const throw() { return "O^O LOOP INVERTER: "; }
