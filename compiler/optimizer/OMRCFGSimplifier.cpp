...
bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
{
    ...
    if (compareNode->getOpCodeValue() == TR::ificmpeq) {
        ...
        if (trace())
            traceMsg(
                comp(), "   Found an ificmpne of an instance of against zero which throws on the fallthrough path\n");
        throwBlock = _next1;
    ...
    catchBlock->setHandlerInfo(
        0, static_cast<uint8_t>(comp()->getInlineDepth()), 0, comp()->getCurrentMethod(), comp());
    ...
    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "cfgSimpCheckcast/(%s)", comp()->signature()));
...
bool OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
{
    ...
    while (treeCursor->getNode()->getOpCode().isStoreDirect() && !treeCursor->getNode()->getOpCode().isWrtBar()) {
        ...
        TR::Node* trueValue = triangle1
            ? TR::Node::createWithSymRef(
                comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0, storeNode->getSymbolReference())
        ...
        TR::TreeTop* insTree = TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(
                storeNode, storeNode->getOpCodeValue(), 1, select, storeNode->getSymbolReference()));
    ...
    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "cfgSimpMovSeq/%d/(%s)", count, comp()->signature()));
    return true;
}
...
bool OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
{
    ...
    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "cfgSimpNULLCHK/nullToException/(%s)", comp()->signature()));
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
{
    ...
    if (!performTransformation(
            comp(), "%sReplace compare-and-branch node [%p] with boolean compare\n", OPT_DETAILS, compareNode))
        return false;
 ...
 */
bool OMR::CFGSimplifier::simplifyCondCodeBooleanStore(
    TR::Block* joinBlock, TR::Node* branchNode, TR::Node* store1Node, TR::Node* store2Node)
{
    ...
    TR_ASSERT(
        joinBlockSuccessors.size() == 2, "Block containing only an if tree doesn't have exactly two successors\n");
