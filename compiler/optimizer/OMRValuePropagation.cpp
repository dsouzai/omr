...
OMR::ValuePropagation::Relationship* OMR::ValuePropagation::createRelationship(
    int32_t relative, TR::VPConstraint* constraint)
{
...
OMR::ValuePropagation::StoreRelationship* OMR::ValuePropagation::createStoreRelationship(
    TR::Symbol* symbol, Relationship* firstRel)
{
...
bool OMR::ValuePropagation::propagateConstraint(
    TR::Node* node, int32_t valueNumber, Relationship* first, Relationship* rel, ValueConstraints* valueConstraints)
{
    ...
    for (cur = first; cur && worthPropagatingConstraints(comp(), _isGlobalPropagation); cur = cur->getNext()) {
        ...
        if (rel->relative == AbsoluteConstraint) {
            ...
            TR_ASSERT(cur->constraint->asRelation(), "assertion failure");
            TR_ASSERT(rel->constraint->asRelation(), "assertion failure");
            c = cur->constraint->asRelation()->propagateRelativeConstraint(
                rel->constraint->asRelation(), cur->relative, rel->relative, this);
            if (c) {
    ...
    if (rel->relative != AbsoluteConstraint) {
        ...
        if (!addConstraintToList(
                node, rel->relative, valueNumber, rel->constraint->asRelation()->getComplement(this), valueConstraints))
            return false;
    }
            ...
            if (updateStoreConstraint) {
                ...
                if (storeConstraint) {
                    if (storeConstraint != storeRel->constraint) {
                        ...
                        if (trace() && node) {
                            traceMsg(
                                comp(), "   %s [%p] gets new store constraint:", node->getOpCode().getName(), node);
...
TR::VPConstraint* OMR::ValuePropagation::addGlobalConstraint(
    TR::Node* node, TR::VPConstraint* constraint, TR::Node* relative)
{
...
TR::VPConstraint* OMR::ValuePropagation::addGlobalConstraint(
    TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative)
{
    ...
    if (newOrChanged) {
        ...
        if (trace() && node) {
            traceMsg(comp(),
                "   %s [%p] found existing global constraint value number %d (%p): ", node->getOpCode().getName(), node,
                valueNumber, c);
...
TR::VPConstraint* OMR::ValuePropagation::addEdgeConstraint(
    TR::Node* node, TR::VPConstraint* constraint, EdgeConstraints* constraints, TR::Node* relative)
{
    if (!_isGlobalPropagation)
        return constraint;
...
TR::VPConstraint* OMR::ValuePropagation::addBlockConstraint(
    TR::Node* node, TR::VPConstraint* constraint, TR::Node* relative, bool mustBeValid)
{
    if (!constraint)
        return NULL;
    int32_t valueNumber = getValueNumber(node);
...
TR::VPConstraint* OMR::ValuePropagation::addBlockOrGlobalConstraint(
    TR::Node* node, TR::VPConstraint* constraint, bool isGlobal, TR::Node* relative)
{
    if (isGlobal)
...
void OMR::ValuePropagation::mergeStoreRelationships(ValueConstraint* fromvc, ValueConstraint* tovc, bool preserveFrom)
{
    ...
    while (from || to) {
        ...
        if (!fromRel) {
            ...
            /// fromRel->isEmpty(), toRel->isEmpty());
            mergeRelationships(
                *fromRel, *toRel, valueNumber, preserve, prev /*mergingStore*/, &storeSymbols, inBothLists);
        }
    }
}
...
//
OMR::ValuePropagation::GlobalConstraint* OMR::ValuePropagation::GlobalConstraint::create(
    TR::Compilation* c, int32_t valueNumber)
{
...
//
OMR::ValuePropagation::EdgeConstraints* OMR::ValuePropagation::EdgeConstraints::create(
    TR::Compilation* c, TR::CFGEdge* edge)
{
...
OMR::ValuePropagation::EdgeConstraints* OMR::ValuePropagation::createEdgeConstraints(
    TR::CFGEdge* edge, bool keepBlockList)
{
    if (!_isGlobalPropagation)
        return NULL;
...
void OMR::ValuePropagation::createExceptionEdgeConstraints(
    uint32_t exceptions, ValueConstraint* extraConstraint, TR::Node* reason)
{
    if (!_isGlobalPropagation)
        return;
...
OMR::ValuePropagation::Relationship* OMR::ValuePropagation::findConstraintInList(
    TR_LinkHead<Relationship>& list, int32_t relative)
{
...
OMR::ValuePropagation::Relationship* OMR::ValuePropagation::findValueConstraint(
    int32_t valueNumber, ValueConstraints& valueConstraints, int32_t relative)
{
    // Find the constraint for the given value number in the given value
    // constraint list.
    //
...
OMR::ValuePropagation::Relationship* OMR::ValuePropagation::findEdgeConstraint(
    int32_t valueNumber, EdgeConstraints* edge, int32_t relative)
{
    return findValueConstraint(valueNumber, edge->valueConstraints, relative);
}

OMR::ValuePropagation::StoreRelationship* OMR::ValuePropagation::findStoreValueConstraint(
    int32_t valueNumber, TR::Symbol* symbol, ValueConstraints& valueConstraints)
{
...
OMR::ValuePropagation::StoreRelationship* OMR::ValuePropagation::findStoreConstraint(
    int32_t valueNumber, TR::Symbol* symbol)
{
    return findStoreValueConstraint(valueNumber, symbol, _curConstraints);
}

OMR::ValuePropagation::StoreRelationship* OMR::ValuePropagation::findStoreEdgeConstraint(
    int32_t valueNumber, TR::Symbol* symbol, EdgeConstraints* edge)
{
...
TR::VPConstraint* OMR::ValuePropagation::applyGlobalConstraints(
    TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative)
{
    ...
    for (rel = gc->constraints.getFirst(); rel; rel = rel->getNext()) {
        ...
        if (reverse && follow) {
            ...
            if (relative == AbsoluteConstraint)
                linkedConstraint = reverse->constraint->asRelation()->propagateAbsoluteConstraint(
                    follow->constraint, valueNumber, this);
            else
            ...
            if (sig && sig[0] == '[') {
                ...
                //
                    ...
                    if (arrayComponentClass) {
                        isInstance = fe()->isInstanceOf(
                            objectClass, arrayComponentClass, object->isFixedClass(), array->isFixedClass());
                    }
...
void OMR::ValuePropagation::checkTypeRelationship(
    TR::VPConstraint* lhs, TR::VPConstraint* rhs, int32_t& value, bool isInstanceOf, bool isCheckCast)
{
    if (trace())
...
TR::VPConstraint* OMR::ValuePropagation::mergeDefConstraints(
    TR::Node* node, int32_t relative, bool& isGlobal, bool forceMerge)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
            ...
            //
            if (trace())
                traceMsg(comp(), "   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(), node,
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound
            ...
            for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent) {
                if (trace())
                    traceMsg(comp(),
                        "   %s [%p] looking at back edges in loop %d for def %s [%p]: ", node->getOpCode().getName(),
                        node, loopInfo->_loop->getNumber(), defNode->getOpCode().getName(), defNode);
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint32_t)defConstraint->getHighInt() < TR::getMaxUnsigned<TR::Int32>()) {
                                        if ((uint32_t)defConstraint->getLowInt() < (uint32_t)constraint->getLowInt()
                                            ...
                                            TR::VPConstraint* newDefConstraint = TR::VPIntRange::create(
                                                this, constraint->getLowInt(), defConstraint->getHighInt());
                                ...
                                if (defConstraint->getLowInt() > TR::getMinSigned<TR::Int32>()) {
                                    if (defConstraint->getLowInt() <= constraint->getHighInt()
                                        ...
                                        TR::VPConstraint* newDefConstraint = TR::VPIntRange::create(
                                            this, defConstraint->getLowInt(), constraint->getHighInt());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint16_t)defConstraint->getHighShort() < TR::getMaxUnsigned<TR::Int16>()) {
                                        if ((uint16_t)defConstraint->getLowShort() < (uint16_t)constraint->getLowShort()
                                            ...
                                            TR::VPConstraint* newDefConstraint = TR::VPShortRange::create(
                                                this, constraint->getLowShort(), defConstraint->getHighShort());
                                            defConstraint = defConstraint->intersect(newDefConstraint, this);
                                        }
                                    }
                                ...
                                if (defConstraint->getLowShort() > TR::getMinSigned<TR::Int16>()) {
                                    if (defConstraint->getLowShort() <= constraint->getHighShort()
                                        ...
                                        TR::VPConstraint* newDefConstraint = TR::VPShortRange::create(
                                            this, defConstraint->getLowShort(), constraint->getHighShort());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint64_t)defConstraint->getHighLong() < TR::getMaxUnsigned<TR::Int64>()) {
                                        if ((uint64_t)defConstraint->getLowLong() < (uint64_t)constraint->getLowLong()
                                            ...
                                            TR::VPConstraint* newDefConstraint = TR::VPLongRange::create(
                                                this, constraint->getLowLong(), defConstraint->getHighLong());
                                            defConstraint = defConstraint->intersect(newDefConstraint, this);
                                        }
                                    }
                                ...
                                if (defConstraint->getLowLong() > TR::getMinSigned<TR::Int64>()) {
                                    if (defConstraint->getLowLong() <= constraint->getHighLong()
                                        ...
                                        TR::VPConstraint* newDefConstraint = TR::VPLongRange::create(
                                            this, defConstraint->getLowLong(), constraint->getHighLong());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
        ...
        if (!_useDefInfo->getUseDef(defs, useIndex))
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if ((defVN != incrementVN) && !notInLoop) {
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "      ivInfo is invalid for sym [%p], def [%d] node %p does not have the same value "
                            "number as another increment\n",
                            sym, defVN, defNode);
                    break;
                }
...
void OMR::ValuePropagation::setUpInductionVariables(TR_StructureSubGraphNode* node)
{
    ...
    while ((ivInfo = _loopInfo->_inductionVariables.pop())) {
        ...
        if (ivInfo->_symbol->getType().isInt32()) {
            ...
            if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
                if (ivInfo->_entryConstraint->asShortConst())
                    ...
                    entry = new (trHeapMemory()) TR::VPShortRange(
                        ivInfo->_entryConstraint->getLowShort(), ivInfo->_entryConstraint->getHighShort());
            } else
                entry = NULL;
            if (ivInfo->_increment->asShortConst())
            ...
            if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
                if (ivInfo->_entryConstraint->asLongConst())
                    ...
                    entry = new (trHeapMemory()) TR::VPLongRange(
                        ivInfo->_entryConstraint->getLowLong(), ivInfo->_entryConstraint->getHighLong());
            } else
                entry = NULL;
            if (ivInfo->_increment->asLongConst())
        ...
        if (trace()) {
            ...
            traceMsg(comp(), "\nFound induction variable %d [%p]",
                ivInfo->_valueNumber - _firstInductionVariableValueNumber, ivInfo->_symbol);
...
void OMR::ValuePropagation::collectBackEdgeConstraints()
{
    ...
    for (edge = pi.getFirst(); edge; edge = pi.getNext()) {
        ...
        if (_loopInfo->_backEdgeConstraints->valueConstraints.isEmpty()) {
            ...
            //
            _vcHandler.setRoot(
                _loopInfo->_backEdgeConstraints->valueConstraints, _vcHandler.getRoot(constraints->valueConstraints));
            _vcHandler.setRoot(constraints->valueConstraints, NULL);
        } else {
        ...
        for (edge = exits.getFirst(); edge; edge = exits.getNext()) {
            if (edge->getTo()->getNumber() == parentTo) {
                ...
                if (isUnreachablePath(parentConstraints)) {
                    ...
                    _vcHandler.setRoot(
                        parentConstraints->valueConstraints, _vcHandler.getRoot(constraints->valueConstraints));
                    _vcHandler.setRoot(constraints->valueConstraints, NULL);
                } else {
...
void TR::GlobalValuePropagation::processAcyclicRegion(
    TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
    if (trace())
...
void TR::GlobalValuePropagation::processNaturalLoop(
    TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
    TR_RegionStructure* region = node->getStructure()->asRegion();
    ...
    for (TR::CFGEdge* edge = it.getFirst(); edge; edge = it.getNext()) {
        if (trace())
            traceMsg(comp(), "   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(),
...
void TR::GlobalValuePropagation::processImproperLoop(
    TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
    if (trace())
        printStructureInfo(node->getStructure(), true, lastTimeThrough);
...
void TR::GlobalValuePropagation::processRegionSubgraph(
    TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop, bool isNaturalLoop)
{
...
void TR::GlobalValuePropagation::processRegionNode(
    TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
...
TR_BitVector* TR::GlobalValuePropagation::mergeDefinedOnAllPaths(TR_StructureSubGraphNode* node)
{
    ...
    for (auto itr = node->getPredecessors().begin(), end = node->getPredecessors().end(); itr != end; ++itr) {
        ...
        if (trace()) {
            traceMsg(comp(), "   inbound seenOnAllpaths for edge %d->%d", (*itr)->getFrom()->getNumber(),
                (*itr)->getTo()->getNumber());
...
void TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
    ...
    if (comp()->getStartBlock()->getNumber() == _curBlock->getNumber()) {
        addConstraintToList(
            NULL, _syncValueNumber, AbsoluteConstraint, TR::VPSync::create(this, TR_no), &_curConstraints);
    }
    ...
    if (insideLoop) {
        for (auto itr = node->getSuccessors().begin(), end = node->getSuccessors().end(); itr != end; ++itr) {
            if (trace()) {
                traceMsg(comp(), "   outbound seenOnAllpaths for edge %d->%d", (*itr)->getFrom()->getNumber(),
                    (*itr)->getTo()->getNumber());
                _curDefinedOnAllPaths->print(comp());
                traceMsg(comp(), "\n");
            }
...
void OMR::ValuePropagation::Relationship::print(OMR::ValuePropagation* vp, int32_t valueNumber, int32_t indent)
{
    ...
    if (valueNumber < vp->_firstUnresolvedSymbolValueNumber) {
        ...
        //
        trfprintf(vp->comp()->getOutFile(), "%*.ssymbol %d is resolved", indent, " ",
        ...
        if (iv)
            trfprintf(vp->comp()->getOutFile(), "%*.sinduction variable %d [%p]", indent, " ",
                valueNumber - vp->_firstInductionVariableValueNumber, iv->_symbol);
        else
            trfprintf(vp->comp()->getOutFile(), "%*.sparent induction variable %d", indent, " ",
...
bool OMR::ValuePropagation::removeConstraints(int32_t valueNumber, ValueConstraints* vc, bool findStores)
{
    if (trace()) {
        ...
        traceMsg(
            comp(), "   Intersection of constraints failed for valueNumber [%d], removing constraints\n", valueNumber);
    }
...
bool OMR::ValuePropagation::removeStoreConstraints(
    ValueConstraints* valueConstraints, int32_t valueNumber, int32_t relative)
{
    ValueConstraint* vc = _vcHandler.find(valueNumber, *valueConstraints);
    if (vc) {
    ...
    TR::SymbolReference* dstObjRef, bool isForward)
{
    ...
    if (!root->isReferenceArrayCopy()) {
        ...
        if (trace())
            traceMsg(comp(), "Created 3-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
        ...
        if (trace())
            traceMsg(comp(), "Created 5-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
                comp()->getDebug()->getName(root));
    }
...
TR::TreeTop* TR::ArraycopyTransformation::createPointerCompareNode(
    TR::Node* node, TR::SymbolReference* srcRef, TR::SymbolReference* dstRef)
{
...
TR::TreeTop* TR::ArraycopyTransformation::createRangeCompareNode(
    TR::Node* node, TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::SymbolReference* lenRef)
{
    bool is64Bit = comp()->target().is64Bit();
    TR::Node* cmp;
    TR::Node* src; // = TR::Node::createLoad(node, srcRef);
...
TR::TreeTop* TR::ArraycopyTransformation::createMultipleArrayNodes(TR::TreeTop* arrayTree, TR::Node* node)
{
    ...
    if (!alreadyForwardArrayCopy) {
        ...
        followOnBlock = arraycopyBlock->createConditionalBlocksBeforeTree(
            arrayTree, compareTree, arraycopyBackward, outerElseTree, cfg);
    ...
    if (specificLength >= 0)
        specializeForLength(
            outerArraycopyTree, node, (uintptr_t)specificLength, srcRef, dstRef, lenRef, srcObjRef, dstObjRef);
...
int32_t TR::ArraycopyTransformation::perform()
{
    ...
    if (!hasChangedTrees()
        || !performTransformation(
            comp(), "%s Arraycopy Transformation for primitive and reference arrays\n", OPT_DETAILS))
        return 0;
    ...
    for (TR::Node* bndchkNode = iter.getFirst(); bndchkNode; bndchkNode = iter.getNext()) {
        ...
        if (!array) {
            createNewBucketForArrayIndex(
                array, arrayLengths, c, baseNode, bndchkNode, instanceOfClass, indexSourceObjectClass);
            continue;
        }
            ...
            if (performTransformation(comp(), "%s Creating tests outside block_%d for versioning arraylenth %p \n",
                ...
                if (arrayIndex->_baseNode) {
                    nextComparisonNode = TR::Node::createif(
                        TR::ificmplt, minIndex, TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0, 0));
                ...
                if (arrayIndex->_baseNode && arrayIndex->_instanceOfClass
                    ...
                    if (indexObjectRefChild) {
                        ...
                        clazz = sig ? fe()->getClassFromSignature(
                                    sig, len, indexObjectRefChild->getSymbolReference()->getOwningMethod(comp()))
                                    : NULL;
        ...
        if (arrayLengthVersioned) {
            if (arrayLength->_arrayLen->getOpCode().isArrayLength()) {
                ...
                if (objectRefChild && !objectRefChild->isNonNull()) {
                    ...
                    TR::Node* nullTestNode = TR::Node::createif(
                        TR::ifacmpeq, objectRefChild->duplicateTree(), TR::Node::aconst(objectRefChild, 0));
                ...
                if (arrayLength->_instanceOfClass && objectRefChild) {
                    ...
                    clazz = sig ? fe()->getClassFromSignature(
                                sig, len, objectRefChild->getSymbolReference()->getOwningMethod(comp()))
                                : NULL;
                    TR_YesNoMaybe result
                    ...
                    if (result == TR_yes) {
                        ...
                        if (result != TR_yes) {
                            ...
                            TR::Node* ificmpeqNode = TR::Node::createif(
                                TR::ificmpeq, instanceofNode, TR::Node::create(objectRef, TR::iconst, 0, 0));
...
void OMR::ValuePropagation::addToSortedList(
    TR_LinkHead<ArrayLengthToVersion>* arrayLengths, ArrayLengthToVersion* newArrayLen)
{
    ...
    TR::SymbolReference* dstRef, TR::SymbolReference* lenRef)
{
    ...
    createReferenceArrayNodeWithoutFlags(
        arrayTree->_treetop, arraycopyArrayStoreCheck, srcObjRef, dstObjRef, lenRef, srcRef, dstRef, true);
    ...
    if ((arrayTree->_flag & NEED_WRITE_BARRIER) == 0)
        createPrimitiveArrayNodeWithoutFlags(
            arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcRef, dstRef, lenRef, true, true);
    else {
        createReferenceArrayNodeWithoutFlags(
            arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcObjRef, dstObjRef, lenRef, srcRef, dstRef, true);
    ...
    arraycopyBlock->createConditionalBlocksBeforeTree(
        arrayTree->_treetop, ifTree, arraycopyArrayStoreCheck, arraycopyNoArrayStoreCheck, cfg, false);
...
void OMR::ValuePropagation::transformUnknownTypeArrayCopy(TR_TreeTopWrtBarFlag* arrayTree)
{
    ...
    TR_ASSERT(dstObjRef, "No destination object ref\n");
    createReferenceArrayNodeWithoutFlags(
        arrayTree->_treetop, arraycopyReference, srcObjRef, dstObjRef, lenRef, srcRef, dstRef, true);

    TR::Node* srcObject = node->getChild(0);
    TR::TreeTop* ifTree = createPrimitiveOrReferenceCompareNode(srcObject);

    arraycopyBlock->createConditionalBlocksBeforeTree(
        arrayTree->_treetop, ifTree, arraycopyReference, arraycopyPrimitive, cfg, false);
...
void OMR::ValuePropagation::doDelayedTransformations()
{
    ...
    for (scalarizedArrayCopy = treesIt1.getFirst(); scalarizedArrayCopy; scalarizedArrayCopy = treesIt1.getNext()) {
        ...
        TR::TransformUtil::scalarizeArrayCopy(
            comp(), scalarizedArrayCopy->_node, scalarizedArrayCopy->_treetop, true, didTransformArrayCopy);
    ...
    if (_blocksToBeRemoved) {
        for (i = _blocksToBeRemoved->size() - 1; i >= 0; --i) {
            ...
            if (performTransformation(
                    comp(), "%sRemoving unreachable block_%d at [%p]\n", OPT_DETAILS, node->getNumber(), node)) {
    ...
    if (!_multiLeafCallsToInline.isEmpty()) {
        ...
        for (multiLeafCall = iter.getCurrent(); multiLeafCall != NULL; multiLeafCall = iter.getNext()) {
            ...
            if (!multiLeafInlineCall.inlineCall(multiLeafCall))
                performTransformation(
                    comp(), "%s WARNING: Inlining of %p failed\n", OPT_DETAILS, multiLeafCall->getNode());
        }
    ...
    for (predictedThrow = throwsIt.getFirst(); predictedThrow; predictedThrow = throwsIt.getNext()) {
        ...
        if (predictedCatchBlock->specializedDesyncCatchBlock())
            dumpOptDetails(
                comp(), "%sChanging a throw [%p] to a goto for specializedDesyncCatchBlock\n", OPT_DETAILS, node);
        ...
        if (treeTop->getNode()->getOpCodeValue() == TR::NULLCHK) {
            ...
            treeTop = TR::TreeTop::create(
                comp(), treeTop, TR::Node::createWithSymRef(TR::astore, 1, 1, node->getFirstChild(), excpSymRef));
        } else {
...
TR_OpaqueClassBlock* OMR::ValuePropagation::findLikelySubtype(
    const char* sig, int32_t len, TR_ResolvedMethod* owningMethod)
{
    return NULL;
}

TR::VPConstraint* OMR::ValuePropagation::createTypeHintConstraint(
    TR_ResolvedMethod* owningMethod, const char* sig, int32_t len)
{
    return NULL;
}
...
OMR::ValuePropagation::InductionVariable::InductionVariable(
    TR::Symbol* sym, TR::Node* entryDef, int32_t incrVN, TR::VPConstraint* incr, OMR::ValuePropagation* vp)
    : _symbol(sym)
    , _entryDef(entryDef)
    , _entryConstraint(0)
    , _incrementVN(incrVN)
...
OMR::ValuePropagation::CallInfo::CallInfo(
    OMR::ValuePropagation* vp, TR_OpaqueClassBlock* thisType, TR_PrexArgInfo* argInfo)
    : _tt(vp->_curTree)
    , _block(vp->_curBlock)
    , _thisType(thisType)
    , _argInfo(argInfo)
...
OMR::ValuePropagation::VirtualGuardInfo::VirtualGuardInfo(
    OMR::ValuePropagation* vp, TR_VirtualGuard* vgOld, TR_VirtualGuard* vgNew, TR::Node* newGNode, TR::Node* cn)
