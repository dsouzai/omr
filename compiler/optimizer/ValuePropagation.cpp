...
void TR::ValuePropagation::doDelayedTransformations()
   ...
   //Object.clone transformation
   ListIterator<TR::Node> clnNodeIt(&_objectCloneCallNodes);
   ListIterator<TR_OpaqueClassBlock>  clnClassIt(&_objectCloneInstanceClasses);
   ListIterator<TR::TreeTop> clnTreeTopIt(&_objectCloneCallTreeTops);

   TR::Node *clnCallNode = clnNodeIt.getCurrent();
   TR_OpaqueClassBlock *optClnClass = comp()->fe()->getClassFromSignature("packagePlaceholder/OptCloneClass", 32, comp()->getCurrentMethod());

   if (clnCallNode && optClnClass)
      {
      ...
      for (method = clnit.getCurrent(); method; method = clnit.getNext())
         ...
         if(!strncmp(sig, "packagePlaceholder/OptCloneClass.OptimizedClone(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;", len))
            {
            optClnClassSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(clnCallNode->getSymbolReference()->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Special);
            break;
            }
         }

      //Traverse the clone call lists and the list of potentail src cln classes and mutated the calls.
      //
      TR_OpaqueClassBlock *srcClass = clnClassIt.getCurrent();
      TR::TreeTop *clnTreeTop = clnTreeTopIt.getCurrent();
      for ( ; clnCallNode && optClnClassSymRef; clnCallNode = clnNodeIt.getNext())
         {
         //Now, try to transform the Object.clone call to a J9VMInternals.optimizedClone call. This call
         //takes two parameters. The second parameter is reserved for when we can create a new "empty"
         //object that can act as the target of the clone. If not, pass NULL.
         //

         //We can pass in a new, empty instance as a parameter to optimizedClone.
         //
         if (srcClass)
            {
            TR::Node *newTreeNode = TR::Node::create(clnCallNode ,TR::treetop, 1);
            TR::TreeTop *newTree = TR::TreeTop::create(comp(), newTreeNode);

            if (!performTransformation(comp(), "%sCreating new object tree [%p] to populate second parameter of optimized clone call [%p] \n", OPT_DETAILS, newTreeNode, clnCallNode))
               {
               srcClass = NULL;
               }
            else
               {
               //Create a new instance and common the node with the second parameter to the optimized clone call.
               //
               if (performTransformation(comp(), "%sTransforming Object.Clone call node [%p] to J9VMIntenrals.optimizedClone call \n", OPT_DETAILS, clnCallNode))
                  {
                  TR::Node *newNode =  TR::Node::createWithSymRef(clnCallNode, TR::New, 1, comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(clnCallNode->getFirstChild()->getSymbolReference()->getOwningMethodSymbol(comp())));
                  TR::Node *classPtr = TR::Node::createWithSymRef(clnCallNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateClassSymbol(clnCallNode->getFirstChild()->getSymbolReference()->getOwningMethodSymbol(comp()),0, srcClass));
                  newNode->setNumChildren(1);
                  newNode->setAndIncChild(0,classPtr);
                  newTreeNode->setNumChildren(1);
                  newTreeNode->setAndIncChild(0,newNode);
                  clnCallNode->setNumChildren(2);
                  clnCallNode->setAndIncChild(1, newNode);
                  clnCallNode->setSymbolReference(optClnClassSymRef);
                  #ifdef DEBUG
                  printf("TreeTop: %p \n", clnTreeTop);
                  printf("New Tree Top: %p \n", newTreeNode);
                  #endif

                  clnTreeTop->insertBefore(newTree);
                  }
               }
            }
         ...
         if (srcClass == NULL)
            {
            //Cannot create a new instance of the source object's class, therefore, second argument is NULL.
            //
            if (performTransformation(comp(), "%sTransforming Object.Clone call node [%p] to J9VMIntenrals.optimizedClone call \n", OPT_DETAILS, node))
               {
               TR::Node *nullNode =  TR::Node::aconst(clnCallNode, 0);
               clnCallNode->setNumChildren(2);
               clnCallNode->setChild(0, clnCallNode->getChild(clnCallNode->getFirstArgumentIndex()));
               //if (clnCallNode->getNumChildren() >1)
               //   {
               //   cg()->decReferenceCount(clnCallNode->getChild(1));
               //  clnCallNode->setNumChildren(2);
               //  }
         ...
         srcClass = clnClassIt.getNext();
         }
      }

   _objectCloneCallTreeTops.deleteAll();
   _objectCloneCallNodes.deleteAll();
   _objectCloneInstanceClasses.deleteAll();
#endif
