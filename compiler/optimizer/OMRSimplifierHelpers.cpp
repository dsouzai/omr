    ...
    if (branchToFollowingBlock(node, block, s->comp())) {
        if (node->getNumChildren() > 0) {
            ...
            if (block->getExit()->getNode()->getNumChildren() == 0)
                return node;
            if (!s->optimizer()->areNodesEquivalent(node->getFirstChild(),
                    block->getExit()->getNode()->getFirstChild()))
                return node;
        }
...
TR::Node *foldRedundantAND(TR::Node *node, TR::ILOpCodes andOpCode, TR::ILOpCodes constOpCode, int64_t andVal,
    TR::Simplifier *s)
{
...
void makeConstantTheRightChildAndSetOpcode(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild,
    TR::Simplifier *s)
{
    if (firstChild->getOpCode().isLoadConst() && !secondChild->getOpCode().isLoadConst()) {
...
bool decodeConversionOpcode(TR::ILOpCode op, TR::DataType nodeDataType, TR::DataType &sourceDataType,
    TR::DataType &targetDataType)
{
    if (!op.isConversion()) {
        return false;
    } else {
    ...
    if (s->comp()->cg()->needsNormalizationBeforeShifts() && !node->isNormalizedShift()) {
        ...
        if ((secondChild->getOpCodeValue() != TR::iconst)
            ...
            if (performTransformation(s->comp(), "%sPlatform specific normalization of shift node [%s]\n",
                ...
                //
                TR::Node *secondChild = node->getSecondChild();
                TR::Node *normalizedNode = TR::Node::create(TR::iand, 2, secondChild,
                    TR::Node::create(secondChild, TR::iconst, 0, normalizationConstant));
