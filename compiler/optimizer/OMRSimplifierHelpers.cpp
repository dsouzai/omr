...
bool performTransformationSimplifier(TR::Node* node, TR::Simplifier* s)
{
    return performTransformation(s->comp(), "%sConstant folding node [%s] %s\n", s->optDetailString(),
        node->getName(s->getDebug()), node->getOpCode().getName());
}

void setIsHighWordZero(TR::Node* node, TR::Simplifier* s)
{
    if (((int32_t)(node->getLongIntHigh() & 0xffffffff) == (int32_t)0) && ((int64_t)node->getLongInt() >= (int64_t)0))
    ...
    if (branchToFollowingBlock(node, block, s->comp())) {
        if (node->getNumChildren() > 0) {
            ...
            if (block->getExit()->getNode()->getNumChildren() == 0)
                return node;
            if (!s->optimizer()->areNodesEquivalent(
                    node->getFirstChild(), block->getExit()->getNode()->getFirstChild()))
                return node;
        }
...
void orderChildren(TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
    ...
    // R2:
    else if ((region = s->containingStructure()) && !isExprInvariant(region, secondChild)
        && isExprInvariant(region, firstChild)) {
        if (performTransformation(
                s->comp(), "%sApplied reassociation rule 2 to node 0x%p\n", s->optDetailString(), node))
            swapChildren(node, firstChild, secondChild, s);
    }
    // R2:
    else if ((region = s->containingStructure()) && isExprInvariant(region, secondChild)
        && !isExprInvariant(region, firstChild)) {
        // do nothing
    } else if (!secondChild->getOpCode().isLoadConst() && shouldSwapChildren(firstChild, secondChild)) {
        if (performTransformation(s->comp(), "%sOrdering children of node 0x%p\n", s->optDetailString(), node))
            swapChildren(node, firstChild, secondChild, s);
    }
}

TR::Node* foldRedundantAND(
    TR::Node* node, TR::ILOpCodes andOpCode, TR::ILOpCodes constOpCode, int64_t andVal, TR::Simplifier* s)
{
    ...
    if (constChild->getOpCodeValue() == constOpCode) {
        switch (constOpCode) {
        case TR::sconst:
            val = constChild->getShortInt();
            break;
        case TR::iconst:
            val = constChild->getInt();
            break;
        case TR::lconst:
            val = constChild->getLongInt();
            break;
        default:
            val = 0;
        }
    ...
    if (((val & andVal) == andVal) && (andChild->getReferenceCount() == 1)
        && performTransformation(s->comp(), "%sFolding redundant AND node [%s] and its children [%s, %s]\n",
            s->optDetailString(), node->getName(s->getDebug()), lhsChild->getName(s->getDebug()),
            constChild->getName(s->getDebug()))) {
...
TR::Node* tryFoldAndWidened(TR::Simplifier* simplifier, TR::Node* node)
{
    if (node->getOpCode().isAnd()) {
        ...
        if (rhsNode->getOpCode().isLoadConst()) {
            ...
            if (lhsNode->getOpCode().isZeroExtension()
                || (lhsNode->getOpCode().isSignExtension() && lhsNode->isNonNegative())) {
                TR::Node* extensionValueNode = lhsNode->getChild(0);

                // Sanity check that this is indeed an extension
                TR_ASSERT(node->getSize() > extensionValueNode->getSize(),
                    "Extended value datatype size must be smaller than the parent's datatype size.");
...
void makeConstantTheRightChildAndSetOpcode(
    TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
    if (firstChild->getOpCode().isLoadConst() && !secondChild->getOpCode().isLoadConst()) {
        TR_ASSERT(
            node->getOpCode().getOpCodeForSwapChildren() != TR::BadILOp, "cannot swap children of irreversible op");
...
TR::Node* postWalkLowerTreeSimplifier(TR::TreeTop* tt, TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (newTree != s->_curTree)
        s->_curTree = newTree
            = newTree->getPrevTreeTop(); // set it to the previous treetop so simplier will next walk the new tree
...
bool decodeConversionOpcode(
    TR::ILOpCode op, TR::DataType nodeDataType, TR::DataType& sourceDataType, TR::DataType& targetDataType)
{
    if (!op.isConversion()) {
        return false;
    } else {
        ...
        for (int i = 0; i < TR::NumAllTypes; i++) {
            ...
            if (opValue
                == TR::ILOpCode::getProperConversion(sourceDataType, targetDataType, false /*!wantZeroExtension*/)) {
                return true;
            }
        }
        return false;
...
void removePaddingNode(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (paddingNode) {
        ...
        dumpOptDetails(s->comp(), "remove paddingNode %s (0x%p) from %s (0x%p) and dec numChildren %d->%d\n",
            paddingNode->getOpCode().getName(), paddingNode, node->getOpCode().getName(), node, oldNumChildren,
            oldNumChildren - 1);
    }
}
...
{
    TR_ASSERT(node->getReferenceCount() == 1,
        "stopUsingSingleNode only valid for nodes with a referenceCount of 1 and not %d\n", node->getReferenceCount());
    if (removePadding)
...
{
    if (s->comp()->cg()->needsNormalizationBeforeShifts() && !node->isNormalizedShift()) {
        ...
        if ((secondChild->getOpCodeValue() != TR::iconst)
            ...
            if (performTransformation(s->comp(), "%sPlatform specific normalization of shift node [%s]\n",
                ...
                //
                TR::Node* secondChild = node->getSecondChild();
                TR::Node* normalizedNode = TR::Node::create(
                    TR::iand, 2, secondChild, TR::Node::create(secondChild, TR::iconst, 0, normalizationConstant));
