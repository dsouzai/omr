...
const char* TR_Debug::getDiagnosticFormat(const char* format, char* buffer, int32_t length)
{
...
void TR_Debug::printSnippetLabel(
    TR::FILE* pOutFile, TR::LabelSymbol* label, uint8_t* cursor, const char* comment1, const char* comment2)
{
    int addressFieldWidth = TR::Compiler->debug.hexAddressFieldWidthInChars();
    int codeByteColumnWidth = TR::Compiler->debug.codeByteColumnWidth();
    int prefixWidth = addressFieldWidth * 2 + codeByteColumnWidth
    ...
    if (_comp->getOption(TR_MaskAddresses)) {
        trfprintf(pOutFile, "\n\n%*s %08x %*s", TR::Compiler->debug.hexAddressFieldWidthInChars(), "*Masked*", offset,
    ...
    if (comment1) {
        trfprintf(
            pOutFile, "\t\t%c %s", (_comp->target().cpu.isX86() && _comp->target().isLinux()) ? '#' : ';', comment1);
        if (comment2)
...
void TR_Debug::print(
    TR::SymbolReference* symRef, TR_PrettyPrinterString& output, bool hideHelperMethodInfo, bool verbose)
{
...
const char* TR_Debug::getName(TR::Instruction* instr)
{
    ...
    if (_comp->getToNumberMap().Locate((void*)instr, hashIndex)) {
        ...
        return getName(
            (void*)instr, "IN_", InstructionNumber, _comp->getAddressEnumerationOption(TR_EnumerateInstruction));
    }
...
const char* TR_Debug::getName(TR_Structure* structure)
{
    ...
    //
    return getName(
        (void*)structure, "ST_", _nextStructureNumber++, _comp->getAddressEnumerationOption(TR_EnumerateStructure));
}
...
const char* TR_Debug::getName(TR::Node* node)
{
    if (!node)
        ...
        return getName(
            (void*)node, "ND_", node->getGlobalIndex(), _comp->getAddressEnumerationOption(TR_EnumerateNode));
}
...
const char* TR_Debug::getAutoName(TR::SymbolReference* symRef)
{
    ...
    if (symRef->getSymbol()->isSpillTempAuto()) {
        ...
        int debugNameLen;
        const char* debugName = getOwningMethod(symRef)->localName(
            slot, 0, debugNameLen, comp()->trMemory()); // TODO: Proper bcIndex somehow; TODO: proper length
        if (!debugName) {
            debugName = "";
            debugNameLen = 0;
        }
...
#ifdef J9_PROJECT_SPECIFIC
            TR::VMAccessCriticalSection getStaticNameCriticalSection(
...
static const char* commonNonhelperSymbolNames[] = { "<contiguousArraySize>", "<discontiguousArraySize>",
    ...
    "<storeFlattenableArrayElementNonHelper>", "<synchronizedFieldLoad>", "<atomicAdd>", "<atomicFetchAndAdd>",
    "<atomicFetchAndAdd32Bit>", "<atomicFetchAndAdd64Bit>", "<atomicSwap>", "<atomicSwap32Bit>", "<atomicSwap64Bit>",
    ...
    "<j9VMThreadFloatTemp1>", "<J9JNIMethodIDvTableIndexFieldSymbol>", "<defaultValue>", "<jitDispatchJ9Method>" };
...
const char* TR_Debug::getShadowName(TR::SymbolReference* symRef)
{
    ...
    static_assert(
        sizeof(commonNonhelperSymbolNames) / sizeof(commonNonhelperSymbolNames[0]) == numCommonNonhelperSymbols,
...
void TR_Debug::print(TR::FILE* pOutFile, List<TR::Snippet>& snippetList)
{
    if (pOutFile == NULL)
        return;
...
const char* TR_Debug::getRuntimeHelperName(int32_t index)
{
    ...
    if (index < TR_FSRH) {
        ...
        if (index < TR_LXRH) {
            ...
            case TR_IA32compressString:
                return "_compressString";
            case TR_IA32compressStringNoCheck:
                return "_compressStringNoCheck";
            case TR_IA32compressStringJ:
                return "_compressStringJ";
            case TR_IA32compressStringNoCheckJ:
                return "_compressStringNoCheckJ";
            case TR_IA32andORString:
                return "_andORString";
            ...
            case TR_AMD64compressString:
                return "_compressString";
            case TR_AMD64compressStringNoCheck:
                return "_compressStringNoCheck";
            case TR_AMD64compressStringJ:
                return "_compressStringJ";
            case TR_AMD64compressStringNoCheckJ:
                return "_compressStringNoCheckJ";
            case TR_AMD64andORString:
                return "_andORString";
        ...
        case TR_PPCcompressString:
            return "__compressString";
        case TR_PPCcompressStringNoCheck:
            return "__compressStringNoCheck";
        case TR_PPCcompressStringJ:
            return "__compressStringJ";
        case TR_PPCcompressStringNoCheckJ:
            return "__compressStringNoCheckJ";
        case TR_PPCandORString:
            return "__andORString";
...
void TR_Debug::startTracingRegisterAssignment(const char* direction, TR_RegisterKinds kindsToAssign)
{
    if (_file == NULL)
        return;
...
void TR_Debug::printDebugCounters(TR::DebugCounterGroup* counterGroup, const char* name)
{
    ...
    TR::DebugCounter** counterArray
        = (TR::DebugCounter**)(TR::Compiler->regionAllocator.allocate(counters.getSize() * sizeof(TR::DebugCounter*)));
