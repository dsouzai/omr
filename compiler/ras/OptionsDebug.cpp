...
static char optionCategories[]
    = { ' ', 'C', 'O', 'L', 'D', 'R', 'I', 'M', 0 }; // Must match categories[] in codegen.dev/Options.cpp
...
int32_t TR_Debug::findLogFile(const char* logFileName, TR::Options* aotCmdLineOptions, TR::Options* jitCmdLineOptions,
...
void TR_Debug::findLogFile(
    const char* logFileName, TR::Options* cmdOptions, TR::Options** optionsArray, int32_t arraySize, int32_t& index)
{
    if (!cmdOptions)
        return;
...
void TR_Debug::dumpOptionHelp(TR::OptionTable* firstOjit, TR::OptionTable* firstOfe, TR::SimpleRegex* nameFilter)
{
    ...
    for (int32_t cat = 0; optionCategories[cat]; cat++) {
        ...
        while (ojit->name || ofe->name) {
            if (ojit->name
                && (ojit->helpText == NULL || ojit->helpText[0] != optionCategories[cat]
                    || (regexExcludes(nameFilter, ojit->name) && regexExcludes(nameFilter, ojit->helpText)))) {
                ojit++;
                continue;
            }
            if (ofe->name
                && (ofe->helpText == NULL || ofe->helpText[0] != optionCategories[cat]
...
void TR_Debug::dumpOptions(const char* optionsType, const char* options, const char* envOptions,
    TR::Options* cmdLineOptions, TR::OptionTable* ojit, TR::OptionTable* ofe, void* feBase, TR_FrontEnd* fe)
{
    TR::OptionTable* entry;
    char* base;
    ...
    while (ojit->name || ofe->name) {
        if (ojit->name && (!ojit->msg || (ojit->msg[0] == 'F' && !(ojit->msgInfo & OPTION_FOUND))) && !ojit->enabled) {
            ojit++;
            continue;
        }
        if (ofe->name && (!ofe->msg || (ofe->msg[0] == 'F' && !(ofe->msgInfo & OPTION_FOUND))) && !ofe->enabled) {
            ofe++;
            continue;
        }
        if (!ojit->name) {
        ...
        if (entry->fcn == TR::Options::setBit) {
            ...
            if (entry->msg[0] == 'P')
                printIt = (value != 0);
        } else if (entry->fcn == TR::Options::set32BitNumeric || entry->fcn == TR::Options::set32BitSignedNumeric
            || entry->fcn == TR::Options::setCount) {
            value = (intptr_t)(*((int32_t*)(base + entry->parm1)));
            if (entry->msg[0] == 'P')
                printIt = (value != 0);
        ...
        if (printIt && (entry->msg[0] == 'F' || extendCheck)) {
            ...
            if (regex)
                ...
                || entry->fcn == TR::Options::setVerboseBitsInJitPrivateConfig) {
    ...
    if (fej9->generateCompressedPointers()) {
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "     compressedRefs shiftAmount=%d", TR::Compiler->om.compressedReferenceShift());
    }
#endif
}
