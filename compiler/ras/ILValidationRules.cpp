...
void TR::SoundnessRule::validate(TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    checkSoundnessCondition(stop, !stop || stop->getNode() != NULL, "Stop tree must have a node");
    ...
    for (TR::TreeTop* currentTree = start; currentTree != stop; currentTree = currentTree->getNextTreeTop()) {
        checkSoundnessCondition(currentTree, currentTree->getNode() != NULL, "Tree must have a node");
        ...
        if (next) {
            checkSoundnessCondition(currentTree, next->getNode() != NULL, "Tree after n%dn must have a node",
            ...
            checkSoundnessCondition(currentTree, next->getPrevTreeTop() == currentTree,
                ...
                currentTree->getNode()->getGlobalIndex(), next->getNode()->getGlobalIndex(),
            ...
            checkSoundnessCondition(currentTree, stop == NULL,
                ...
                currentTree->getNode()->getGlobalIndex(), stop ? stop->getNode()->getGlobalIndex() : 0);
            checkNodeSoundness(currentTree, currentTree->getNode(), ancestorNodes, visitedNodes);
        }
    }
}

void TR::SoundnessRule::checkNodeSoundness(
    TR::TreeTop* location, TR::Node* node, TR::NodeChecklist& ancestorNodes, TR::NodeChecklist& visitedNodes)
{
    ...
    checkSoundnessCondition(
        location, !ancestorNodes.contains(node), "n%dn must not be its own ancestor", node->getGlobalIndex());
    ancestorNodes.add(node);
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        TR::Node* child = node->getChild(i);
        checkSoundnessCondition(location, child != NULL, "n%dn child %d must not be NULL", node->getGlobalIndex(), i);
        checkNodeSoundness(location, child, ancestorNodes, visitedNodes);
    }
    ancestorNodes.remove(node);
}

void TR::SoundnessRule::checkSoundnessCondition(TR::TreeTop* location, bool condition, const char* formatStr, ...)
{
    if (!condition) {
...
void TR::ValidateLivenessBoundaries::validate(TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    TR::TreeTop* start = methodSymbol->getFirstTreeTop();
    TR::TreeTop* stop = methodSymbol->getLastTreeTop();
    for (TR::PostorderNodeOccurrenceIterator iter(start, comp(), "VALIDATE_LIVENESS_BOUNDARIES"); iter != stop;
         ++iter) {
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            ...
            if (nextTree) {
                TR::checkILCondition(
                    node, nextTree->getNode()->getOpCodeValue() == TR::BBStart, comp(), "Expected BBStart after BBEnd");
...
void TR::ValidateLivenessBoundaries::validateEndOfExtendedBlockBoundary(TR::Node* node, TR::LiveNodeWindow& liveNodes)
{
    for (LiveNodeWindow::Iterator lnwi(liveNodes); lnwi.currentNode(); ++lnwi) {
        TR::checkILCondition(node, false, comp(), "Node cannot live across block boundary at n%dn",
...
void TR::ValidateLivenessBoundaries::updateNodeState(
    TR::Node* node, TR::NodeSideTable<TR::NodeState>& nodeStates, TR::LiveNodeWindow& liveNodes)
{
    ...
    if (liveNodes.contains(node)) {
        TR::checkILCondition(node, state._futureReferenceCount >= 1, comp(), "Node already has reference count 0");
        if (--state._futureReferenceCount == 0) {
            liveNodes.remove(node);
        }
    } else {
        TR::checkILCondition(node, node->getOpCode().isTreeTop(), comp(), "Node has already gone dead");
    }
...
void TR::ValidateNodeRefCountWithinBlock::validate(TR::TreeTop* firstTreeTop, TR::TreeTop* exitTreeTop)
{
...
void TR::ValidateNodeRefCountWithinBlock::validateRefCountPass2(TR::Node* node)
{
    ...
    if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
        ...
        TR::checkILCondition(node, node->getLocalIndex() == 0, comp(),
            "Node accessed outside of its (extended) basic block: %d time(s)", node->getLocalIndex());
    }
}
    ...
    if (opcode.expectedChildCount() != ILChildProp::UnspecifiedChildCount) {
        ...
        if (!opcode.canHaveGlRegDeps()) {
            ...
            TR::checkILCondition(node, node->getChild(actChildCount - 1)->getOpCodeValue() == TR::GlRegDeps, comp(),
                "Child count %d does not match expected value of %d (%d without GlRegDeps) and last child is not a "
                "GlRegDeps",
    ...
    if (opcode.expectedChildCount() != ILChildProp::UnspecifiedChildCount) {
        ...
        for (auto i = 0; i < actChildCount; ++i) {
            ...
            if (childOpcode.getOpCodeValue() != TR::GlRegDeps) {
                ...
                const auto expChildTypeName
                    = (expChildType >= TR::NumAllTypes) ? "UnspecifiedChildType" : TR::DataType::getName(expChildType);
                const auto actChildTypeName = TR::DataType::getName(actChildType);
                TR::checkILCondition(node, (expChildType >= TR::NumAllTypes || actChildType == expChildType), comp(),
                 ...
                 */
                TR::checkILCondition(node, opcode.canHaveGlRegDeps() && (i == actChildCount - 1), comp(),
                    "Unexpected GlRegDeps child %d", i);
            }
        }
    }
}
...
void TR::Validate_ireturnReturnType::validate(TR::Node* node)
{
    ...
    if (opcode.getOpCodeValue() == TR::ireturn) {
        ...
        for (auto i = 0; i < childCount; ++i) {
            ...
            TR::checkILCondition(node,
                (actChildType == TR::Int32 || actChildType == TR::Int16 || actChildType == TR::Int8), comp(),
