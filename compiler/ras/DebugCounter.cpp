...
void TR::DebugCounter::prependDebugCounterBump(TR::Compilation *comp, TR::TreeTop *nextTreeTop,
    TR::DebugCounterBase *counter, int32_t delta)
{
...
void TR::DebugCounter::prependDebugCounterBump(TR::Compilation *comp, TR::TreeTop *nextTreeTop,
    TR::DebugCounterBase *counter, TR::Node *deltaNode)
{
...
TR::DebugCounter *TR::DebugCounter::getDebugCounter(TR::Compilation *comp, const char *name, int8_t fidelity,
    int32_t staticDelta)
{
...
void TR::DebugCounter::generateRelocation(TR::Compilation *comp, uint8_t *location, TR::Node *node,
...
bool TR::DebugCounterBase::initializeReloData(TR::Compilation *comp, int32_t delta, int8_t fidelity,
    int32_t staticDelta)
{
    if (!_reloData) {
...
{
    TR::SymbolReference *symRef = comp->getSymRefTab()->findOrCreateCounterSymRef(const_cast<char *>(_name),
        comp->target().is64Bit() ? TR::Int64 : TR::Int32, &_bumpCount);
...
void TR::DebugCounter::getInsertionCounterNames(TR::Compilation *comp, TR_OpaqueMethodBlock *method,
    int32_t bytecodeIndex, const char *(&counterNames)[3])
{
...
void TR::DebugCounterAggregation::aggregateStandardCounters(TR::Compilation *comp, TR::Node *node,
    const char *counterName, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    TR_ByteCodeInfo &bci = node->getByteCodeInfo();
    ...
    int32_t bytecodeIndex, TR::DebugCounter *counter, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    ...
    if (counter && counter->getDenominator() && counter->contributesToDenominator())
        aggregateDebugCounterInsertions(comp, method, bytecodeIndex, counter->getDenominator(), delta, fidelity,
            staticDelta);
...
void TR::DebugCounterAggregation::aggregateDebugCounterHistogram(TR::Compilation *comp, TR::DebugCounter *counter,
    int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    if (comp->getOptions()->debugCounterHistogramIsEnabled(counter->getName(), fidelity)) {
        TR::DebugCounter *bucketCounter = TR::DebugCounter::getDebugCounter(comp,
            TR::DebugCounter::debugCounterBucketName(comp, delta, "%s", counter->getName()), fidelity, 1);
...
TR::DebugCounter *TR::DebugCounterGroup::createCounter(const char *name, int8_t fidelity,
    TR_PersistentMemory *persistentMemory)
{
    ...
    if (separator) // There is a denominator counter
    {
        ...
        if (!denominator) {
            ...
            TR_ASSERT(denominatorName,
                "char *denominatorName must not be null. Ensure availability of persistent memory");
