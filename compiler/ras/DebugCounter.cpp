...
void TR::DebugCounter::prependDebugCounterBump(
    TR::Compilation* comp, TR::TreeTop* nextTreeTop, TR::DebugCounterBase* counter, int32_t delta)
{
...
void TR::DebugCounter::prependDebugCounterBump(
    TR::Compilation* comp, TR::TreeTop* nextTreeTop, TR::DebugCounterBase* counter, TR::Node* deltaNode)
{
...
void TR::DebugCounter::prependDebugCounter(TR::Compilation* comp, const char* name, TR::TreeTop* nextTreeTop,
    int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    ...
    TR::DebugCounterAggregation* aggregatedCounters
        = comp->getPersistentInfo()->getDynamicCounters()->createAggregation(comp, name);
...
void TR::DebugCounter::prependDebugCounter(TR::Compilation* comp, const char* name, TR::TreeTop* nextTreeTop,
    TR::Node* deltaNode, int8_t fidelity, int32_t staticDelta)
{
    if (!comp->getOptions()->enableDebugCounters())
        return;
    if (!nextTreeTop)
...
TR::DebugCounter* TR::DebugCounter::getDebugCounter(
    TR::Compilation* comp, const char* name, int8_t fidelity, int32_t staticDelta)
{
    if (comp->getOptions()->staticDebugCounterIsEnabled(name, fidelity)) {
        TR::DebugCounter* staticCounter
            = comp->getPersistentInfo()->getStaticCounters()->getCounter(comp, name, fidelity);
...
void TR::DebugCounter::generateRelocation(
    TR::Compilation* comp, uint8_t* location, TR::Node* node, TR::DebugCounterBase* counter)
{
    counter->finalizeReloData(comp, node);

    comp->cg()->addExternalRelocation(
        TR::ExternalRelocation::create(location, (uint8_t*)counter, TR_DebugCounter, comp->cg()), __FILE__, __LINE__,
        node);
}

void TR::DebugCounter::generateRelocation(TR::Compilation* comp, TR::Instruction* firstInstruction, TR::Node* node,
    TR::DebugCounterBase* counter, uint8_t seqKind)
{
    counter->finalizeReloData(comp, node, seqKind);

    TR::BeforeBinaryEncodingExternalRelocation* r = new (comp->trHeapMemory())
        TR::BeforeBinaryEncodingExternalRelocation(firstInstruction, (uint8_t*)counter, TR_DebugCounter, comp->cg());

    comp->cg()->addExternalRelocation(r, __FILE__, __LINE__, node);
}

void TR::DebugCounter::generateRelocation(TR::Compilation* comp, TR::Instruction* firstInstruction,
    TR::Instruction* secondInstruction, TR::Node* node, TR::DebugCounterBase* counter, uint8_t seqKind)
{
    counter->finalizeReloData(comp, node, seqKind);

    TR::ExternalOrderedPair32BitRelocation* r = new (comp->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
        (uint8_t*)firstInstruction, (uint8_t*)secondInstruction, (uint8_t*)counter, TR_DebugCounter, comp->cg());

    comp->cg()->addExternalRelocation(r, __FILE__, __LINE__, node);
}

bool TR::DebugCounter::relocatableDebugCounter(TR::Compilation* comp) { return comp->compileRelocatableCode(); }
...
const char* TR::DebugCounter::debugCounterBucketName(TR::Compilation* comp, int32_t value, const char* format, ...)
{
    ...
    char* bucketFormat = (char*)comp->trMemory()->allocateStackMemory(
        strlen(format) + 40); // appending "=%d..%d" where each %d could be 11 characters
...
bool TR::DebugCounterBase::initializeReloData(
    TR::Compilation* comp, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    if (!_reloData) {
...
void TR::DebugCounterBase::finalizeReloData(TR::Compilation* comp, TR::Node* node, uint8_t seqKind)
{
...
{
    TR::SymbolReference* symRef = comp->getSymRefTab()->findOrCreateCounterSymRef(
        const_cast<char*>(_name), comp->target().is64Bit() ? TR::Int64 : TR::Int32, &_bumpCount);
    symRef->getSymbol()->setIsDebugCounter();
    return symRef;
}

intptr_t TR::DebugCounter::getBumpCountAddress() { return (intptr_t)&_bumpCount; }

void TR::DebugCounter::getInsertionCounterNames(
    TR::Compilation* comp, TR_OpaqueMethodBlock* method, int32_t bytecodeIndex, const char* (&counterNames)[3])
{
    memset(counterNames, 0, sizeof(counterNames));
    bool insertByteCode = TR::Options::_debugCounterInsertByteCode
        && TR::SimpleRegex::match(TR::Options::_debugCounterInsertByteCode, getName(), true);
    bool insertJittedBody = TR::Options::_debugCounterInsertJittedBody
        && TR::SimpleRegex::match(TR::Options::_debugCounterInsertJittedBody, getName(), true);
    bool insertMethod = TR::Options::_debugCounterInsertMethod
        && TR::SimpleRegex::match(TR::Options::_debugCounterInsertMethod, getName(), true);
    ...
    if (insertByteCode) {
        counterNames[0] = TR::DebugCounter::debugCounterName(comp,
            comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byByteCode.(%s)=%d"),
            methodName, bytecodeIndex);
    }
    if (insertJittedBody) {
        counterNames[1] = TR::DebugCounter::debugCounterName(comp,
            comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byJittedBody.(%s).%s"),
            jittedBody, comp->getHotnessName());
    }
    if (insertMethod) {
        counterNames[2] = TR::DebugCounter::debugCounterName(comp,
            comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byMethod.(%s)"), methodName);
    }
}
...
void TR::DebugCounterAggregation::aggregateStandardCounters(
    TR::Compilation* comp, TR::Node* node, const char* counterName, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    TR_ByteCodeInfo& bci = node->getByteCodeInfo();
...
void TR::DebugCounterAggregation::aggregateStandardCounters(TR::Compilation* comp, TR_OpaqueMethodBlock* method,
    int32_t bytecodeIndex, const char* counterName, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
...
void TR::DebugCounterAggregation::aggregateDebugCounterInsertions(TR::Compilation* comp, TR_OpaqueMethodBlock* method,
    int32_t bytecodeIndex, TR::DebugCounter* counter, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    ...
    if (counter && counter->getDenominator() && counter->contributesToDenominator())
        aggregateDebugCounterInsertions(
            comp, method, bytecodeIndex, counter->getDenominator(), delta, fidelity, staticDelta);
...
void TR::DebugCounterAggregation::aggregateDebugCounterHistogram(
    TR::Compilation* comp, TR::DebugCounter* counter, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    if (comp->getOptions()->debugCounterHistogramIsEnabled(counter->getName(), fidelity)) {
        TR::DebugCounter* bucketCounter = TR::DebugCounter::getDebugCounter(
            comp, TR::DebugCounter::debugCounterBucketName(comp, delta, "%s", counter->getName()), fidelity, 1);
        aggregate(bucketCounter, 1);
    }
}

intptr_t TR::DebugCounterAggregation::getBumpCountAddress() { return (intptr_t)&_bumpCount; }
...
{
    TR::DebugCounterAggregation* aggregatedCounters
        = new (comp->trPersistentMemory()) TR::DebugCounterAggregation(name, comp->trPersistentMemory());
...
TR::DebugCounter* TR::DebugCounterGroup::createCounter(
    const char* name, int8_t fidelity, TR_PersistentMemory* persistentMemory)
{
    ...
    if (separator) // There is a denominator counter
    {
        ...
        if (!denominator) {
            ...
            TR_ASSERT(
                denominatorName, "char *denominatorName must not be null. Ensure availability of persistent memory");
...
TR::DebugCounter* TR::DebugCounterGroup::getCounter(TR::Compilation* comp, const char* name, int8_t fidelity)
{
    ...
    TR_ASSERT(result->getFidelity() >= fidelity,
        "Request for counter at fidelity %d is too high; counter only has fidelity %d: %s\n", fidelity,
        result->getFidelity(), name);
...
void TR::DebugCounterGroup::accumulate()
{
    ...
    if (!_counters.isEmpty()) {
        ...
        for (TR::DebugCounterAggregation* aggregatedCounters = li2.getCurrent(); aggregatedCounters;
             aggregatedCounters = li2.getNext())
