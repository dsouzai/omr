...
void TR_Debug::clearFilters(TR::CompilationFilters* filters)
{
...
void TR_Debug::clearFilters(bool loadLimit)
{
    if (loadLimit)
...
TR::CompilationFilters* TR_Debug::findOrCreateFilters(TR::CompilationFilters* filters)
{
    if (filters)
        return filters;
    else {
...
TR::CompilationFilters* TR_Debug::findOrCreateFilters(bool loadLimit)
{
    if (loadLimit)
...
TR_FilterBST* TR_Debug::addFilter(const char*& filterString, int32_t scanningExclude, int32_t optionSetIndex,
    int32_t lineNum, TR::CompilationFilters* anyFilters)
{
...
TR_FilterBST* TR_Debug::addFilter(
    const char*& filterString, int32_t scanningExclude, int32_t optionSetIndex, int32_t lineNum, bool loadLimit)
{
    if (loadLimit) {
        // initializing filters using the new interface
...
TR_FilterBST* TR_Debug::addExcludedMethodFilter(bool loadLimit)
{
...
bool TR_Debug::scanInlineFilters(FILE* inlineFile, int32_t& lineNumber, TR::CompilationFilters* filters)
{
 ...
 */
const char* TR_Debug::inlinefileOption(
    const char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions)
{
 ...
 *     Otherwise a pointer to the next comma or NULL.
 */
const char* TR_Debug::limitfileOption(const char* option, void* base, TR::OptionTable* entry,
    TR::Options* cmdLineOptions, bool loadLimit, TR_PseudoRandomNumbersListElement** pseudoRandomListHeadPtr)
{
    const char* endOpt = option;
    const char* name = option;
    const char* fail = option;
...
const char* TR_Debug::limitOption(const char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions,
    TR::CompilationFilters*& filters)
{
    const char* p = option;
...
const char* TR_Debug::limitOption(
    const char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions, bool loadLimit)
{
    if (loadLimit) {
...
bool TR_Debug::methodSigCanBeCompiled(const char* methodSig, TR_FilterBST*& filter, TR::Method::Type methodType)
{
...
bool TR_Debug::methodSigCanBeFound(
    const char* methodSig, TR::CompilationFilters* filters, TR_FilterBST*& filter, TR::Method::Type methodType)
{
    ...
    if (filters->hasClassNameSigFilter() || filters->hasNameSigFilter()) {
        if (filters->hasClassNameSigFilter()) {
            ...
            if (filter)
                filter = filter->find(
                    methodName, methodNameLen, methodClass, methodClassLen, methodSignature, methodSignatureLen);
        }
...
bool TR_Debug::methodCanBeFound(
    TR_Memory* trMemory, TR_ResolvedMethod* method, TR::CompilationFilters* filters, TR_FilterBST*& filter)
{
...
bool TR_Debug::methodSigCanBeCompiledOrRelocated(
    const char* methodSig, TR_FilterBST*& filter, bool loadLimit, TR::Method::Type methodType)
{
...
bool TR_Debug::methodCanBeCompiled(TR_Memory* trMemory, TR_ResolvedMethod* method, TR_FilterBST*& filter)
{
    const char* methodSig = method->signature(trMemory);
    return methodSigCanBeCompiled(methodSig, filter, method->convertToMethod()->methodType());
}

bool TR_Debug::methodCanBeRelocated(TR_Memory* trMemory, TR_ResolvedMethod* method, TR_FilterBST*& filter)
{
    const char* methodSig = method->signature(trMemory);
        ...
        while (fgets(lineBuffer, sizeof(lineBuffer), optFile)) {
            if (optCount >= (sizeof(optNumBuffer) / sizeof(optNumBuffer[0]))) {
                TR_VerboseLog::writeLine(
                    TR_Vlog_INFO, "Reached limit of %d optFile lines; ignoring subsequent lines", optCount);
                break;
            }
