...
void TR_Debug::printBottomLegend(TR::FILE* pOutFile)
{
    ...
    trfprintf(
        pOutFile, "%s=[x,y,z]: byte-code-info [callee-index, %s, %s]\n", bciOrLoc, bciOrLocVerbose, lineOrStatement);
...
char* TR_Debug::formattedString(
    char* buf, uint32_t bufLen, const char* format, va_list args, TR_AllocationKind allocationKind)
{
    va_list args_copy;
    ...
    if (printChildren) {
        ...
        if (node->getOpCode().isSwitch()) {
            ...
            if (node->getSecondChild()->getNumChildren() == 1) // a GlRegDep
            {
                ...
                nodeCount += print(
                    pOutFile, node->getSecondChild()->getFirstChild(), indentation + DEFAULT_INDENT_INCREMENT, true);
            }
            ...
            if (node->getOpCodeValue() == TR::lookup) {
                ...
                for (i = 2; i < upperBound; i++) {
                    ...
                    if (node->getChild(i)->getNumChildren() == 1) // a GlRegDep
                    {
                        ...
                        nodeCount += print(
                            pOutFile, node->getChild(i)->getFirstChild(), indentation + DEFAULT_INDENT_INCREMENT, true);
                    }
                }
                setCurrentParentAndChildIndex(oldParent, oldChildIndex);
            } else {
    ...
    if (printChildren) {
        ...
        if (node->getOpCode().isSwitch()) {
            ...
            if (node->getOpCodeValue() == TR::lookup) {
                ...
                if (upperBound < node->getNumChildren()
                    && node->getChild(upperBound)->getOpCodeValue() != TR::GlRegDeps) {
                    nodeCount += printWithFixedPrefix(
                        pOutFile, node->getChild(upperBound), indentation, true, printRefCounts, prefix);
                }
            }
        } else {
            TR::Node* oldParent = getCurrentParent();
...
void TR_Debug::printBasicPostNodeInfo(TR::FILE* pOutFile, TR::Node* node, uint32_t indentation)
{
    ...
    if ((_comp->getNodeOpCodeLength() + indentation) < DEFAULT_NODE_LENGTH + DEFAULT_INDENT_INCREMENT)
        output.appendf(
            "%*s", DEFAULT_NODE_LENGTH + DEFAULT_INDENT_INCREMENT - (_comp->getNodeOpCodeLength() + indentation), "");
    ...
    if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
        ...
        if (block->isCatchBlock()) {
            ...
            if (classNameChars) {
                ...
                length = 3;
                output.appendf(
                    " (catches %.*s)", length, classNameChars); // passing literal string shouldn't use getName()
            }
        ...
        if (opcode.isTwoTypeVectorOpCode()) {
            ...
            output.appendf(
                "%s_%s", getName(opcode.getVectorSourceDataType()), getName(opcode.getVectorResultDataType()));
        } else {
...
void TR_Debug::printBCDNodeInfo(TR::Node* node, TR_PrettyPrinterString& output)
{
    if (node->getType().isBCD()) {
        if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst()
            ...
            output.appendf(
                " <prec=%d (len=%d) adj=%d> ", node->getDecimalPrecision(), node->getSize(), node->getDecimalAdjust());
...
void TR_Debug::printVCG(TR::FILE* pOutFile, TR::CFG* cfg, const char* sig)
{
    ...
    if (debug("nestedVCG")) {
        ...
        trfprintf(pOutFile, "edge.color: black\n");
        trfprintf(
            pOutFile, "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 textcolor: blue borderwidth: 1}\n", sig);
