...
#define FOR_EACH_RESERVED_REGISTER(machine, props, block) \
    FOR_EACH_REGISTER(                                    \
        machine, if (props._registerFlags[regNum] & RV_Reserved) { block; })

#define FOR_EACH_CALLEE_SAVED_REGISTER(machine, props, block) \
    FOR_EACH_REGISTER(                                        \
        machine, if (props._registerFlags[regNum] == Preserved) { block; })

#define FOR_EACH_ASSIGNED_CALLEE_SAVED_REGISTER(machine, props, block) \
    FOR_EACH_CALLEE_SAVED_REGISTER(                                    \
        machine, props, if (reg->getHasBeenAssignedInMethod()) { block; })
