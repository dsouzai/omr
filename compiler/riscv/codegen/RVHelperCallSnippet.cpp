...
uint8_t* TR::RVHelperCallSnippet::emitSnippetBody()
{
    ...
    if (!VALID_UJTYPE_IMM(distance)) {
        distance = TR::CodeCacheManager::instance()->findHelperTrampoline(
                       getDestination()->getReferenceNumber(), (void*)cursor)
            - (intptr_t)cursor;
    ...
    *(int32_t*)cursor
    ...
    if (_restartLabel != NULL) {
        ...
        if (VALID_UJTYPE_IMM(distance)) {
            // b distance
            *(int32_t*)cursor = TR_RISCV_UJTYPE(
                TR::InstOpCode::_jal, cg()->machine()->getRealRegister(OMR::RealRegister::zero), distance >> 2);
            cursor += RISCV_INSTRUCTION_LENGTH;
        } else {
...
void TR_Debug::print(TR::FILE* pOutFile, TR::RVHelperCallSnippet* snippet)
{
    ...
    trfprintf(
        pOutFile, "jal \tra, " POINTER_PRINTF_FORMAT "\t\t; %s%s", target, getName(snippet->getDestination()), info);
