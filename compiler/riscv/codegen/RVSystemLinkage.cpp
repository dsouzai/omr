        ...
        case TR::Address:
            if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
                ...
                cursor = generateSTORE(op, firstNode, stackSlot,
                    machine->getRealRegister(getProperties().getIntegerArgumentRegister(nextIntArgReg)), cg, cursor);
                nextIntArgReg++;
            } else {
                nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
            }
...
TR::Register* TR::RVSystemLinkage::buildDispatch(TR::Node* callNode)
{
    ...
    if (callNode->getOpCode().isCallIndirect()) {
        ...
        if (targetAddr == NULL) {
             ...
             */
            generateJTYPE(
                TR::InstOpCode::_jal, callNode, ra, 0, dependencies, callNode->getSymbolReference(), NULL, cg());
        } else {
            /*
