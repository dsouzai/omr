...
// MethodBuilder in order to avoid increasing the number of header files needed to compile against the JitBuilder
// library. Because we are storing them as pointers, we cannot rely on the default C++ destruction semantic to destruct
// and deallocate the memory region, but rather have to do it explicitly in the MethodBuilder::MemoryManager destructor.
// And since C++ destroys the other members *after* executing the user defined destructor, we need to make sure that any
// members (and their contents) that are allocated in _memoryRegion are explicitly destroyed and deallocated *before*
// _memoryRegion in the MethodBuilder::MemoryManager destructor.
OMR::MethodBuilder::MemoryManager::MemoryManager()
    : _segmentProvider(new (TR::Compiler->persistentAllocator())
            TR::SystemSegmentProvider(MEM_SEGMENT_SIZE, TR::Compiler->rawAllocator))
...
TR::MethodBuilder* OMR::MethodBuilder::asMethodBuilder() { return static_cast<TR::MethodBuilder*>(this); }

int32_t OMR::MethodBuilder::getNextValueID()
{
...
int32_t OMR::MethodBuilder::getNextInlineSiteIndex()
{
    TR::MethodBuilder* caller = callerMethodBuilder();
    if (caller != NULL)
...
TR::MethodBuilder* OMR::MethodBuilder::callerMethodBuilder()
{
...
void OMR::MethodBuilder::setupForBuildIL()
{
    initSequence();
...
uint32_t OMR::MethodBuilder::countBlocks()
{
    if (_count > -1)
        return _count;
...
bool OMR::MethodBuilder::connectTrees()
{
...
bool OMR::MethodBuilder::symbolDefined(const char* name)
{
...
void OMR::MethodBuilder::defineSymbol(const char* name, TR::SymbolReference* symRef)
{
...
const char* OMR::MethodBuilder::adjustNameForInlinedSite(const char* name)
{
...
TR::SymbolReference* OMR::MethodBuilder::lookupSymbol(const char* name)
{
    ...
    if (_inlineSiteIndex == -1 && paramSlotsIterator != _parameterSlot.end()) {
        ...
        symRef = symRefTab()->findOrCreateAutoSymbol(_methodSymbol, slot, primitiveType, true, false, true);
    } else {
...
TR::ResolvedMethod* OMR::MethodBuilder::lookupFunction(const char* name)
{
...
bool OMR::MethodBuilder::isSymbolAnArray(const char* name) { return _symbolIsArray.find(name) != _symbolIsArray.end(); }

void OMR::MethodBuilder::DefineLine(const char* line)
{
    TR::snprintfNoTrunc(_definingLine, MAX_LINE_NUM_LEN * sizeof(char), "%s", line);
}

void OMR::MethodBuilder::DefineLine(int line)
{
    TR::snprintfNoTrunc(_definingLine, MAX_LINE_NUM_LEN * sizeof(char), "%d", line);
}

void OMR::MethodBuilder::DefineName(const char* name) { _methodName = name; }

void OMR::MethodBuilder::DefineLocal(const char* name, TR::IlType* dt)
{
...
void OMR::MethodBuilder::DefineMemory(const char* name, TR::IlType* dt, void* location)
{
...
void OMR::MethodBuilder::DefineGlobal(const char* name, TR::IlType* dt, void* location)
{
...
void OMR::MethodBuilder::DefineParameter(const char* name, TR::IlType* dt)
{
...
void OMR::MethodBuilder::DefineArrayParameter(const char* name, TR::IlType* elementType)
{
...
void OMR::MethodBuilder::DefineReturnType(TR::IlType* dt) { _returnType = dt; }

void OMR::MethodBuilder::DefineFunction(const char* const name, const char* const fileName,
    const char* const lineNumber, void* entryPoint, TR::IlType* returnType, int32_t numParms, ...)
{
    TR::IlType** parmTypes
        = (TR::IlType**)trMemory()->trPersistentMemory()->allocatePersistentMemory(numParms * sizeof(TR::IlType*));
    va_list parms;
...
void OMR::MethodBuilder::DefineFunction(const char* const name, const char* const fileName,
    const char* const lineNumber, void* entryPoint, TR::IlType* returnType, int32_t numParms, TR::IlType** parmTypes)
{
    ...
    TR::ResolvedMethod* method = new (trMemory()->heapMemoryRegion()) TR::ResolvedMethod(
        (char*)fileName, (char*)lineNumber, (char*)name, numParms, copiedParmTypes, returnType, entryPoint, 0);

    _functions.insert(std::make_pair(name, method));
}

const char* OMR::MethodBuilder::getSymbolName(int32_t slot)
{
...
TR::IlType** OMR::MethodBuilder::getParameterTypes()
{
...
void OMR::MethodBuilder::addToBlockCountingWorklist(TR::BytecodeBuilder* builder)
{
    TraceIL("[ %p ] TR::MethodBuilder::addToBlockCountingWorklist %p\n", this, builder);
    _countBlocksWorklist->add(builder);
}

void OMR::MethodBuilder::addToTreeConnectingWorklist(TR::BytecodeBuilder* builder)
{
    if (!builder->_connectedTrees) {
...
void OMR::MethodBuilder::addToAllBytecodeBuildersList(TR::BytecodeBuilder* bcBuilder)
{
...
void OMR::MethodBuilder::AppendBytecodeBuilder(TR::BytecodeBuilder* builder)
{
...
void OMR::MethodBuilder::addBytecodeBuilderToWorklist(TR::BytecodeBuilder* builder)
{
...
int32_t OMR::MethodBuilder::GetNextBytecodeFromWorklist()
{
