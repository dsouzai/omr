...
#if defined(NEW_MEMORY)
   _segmentProvider(new(TR::Compiler->persistentAllocator()) TestAlloc::OMRSystemSegmentProvider(MEM_SEGMENT_SIZE, TR::Compiler->rawAllocator)),
#else
...
#if defined(NEW_MEMORY)
   reinterpret_cast<TestAlloc::OMRSystemSegmentProvider *>(_segmentProvider)->~OMRSystemSegmentProvider();
#else
