...
void OMR::IlInjector::initPrimitiveTypes()
{
...
void OMR::IlInjector::initialize(TR::IlGeneratorMethodDetails* details, TR::ResolvedMethodSymbol* methodSymbol,
...
TR::CFG* OMR::IlInjector::cfg() { return _methodSymbol->getFlowGraph(); }

TR::Block* OMR::IlInjector::getCurrentBlock() { return _currentBlock; }

bool OMR::IlInjector::genIL()
{
...
void OMR::IlInjector::generateToBlock(int32_t b)
{
    _currentBlockNumber = b;
    _currentBlock = _blocks[b];
}

void OMR::IlInjector::allocateBlocks(int32_t num)
{
    _numBlocks = num;
...
TR::Block* OMR::IlInjector::newBlock() { return TR::Block::createEmptyBlock(comp()); }

void OMR::IlInjector::createBlocks(int32_t num)
{
    allocateBlocks(num);
...
TR::TreeTop* OMR::IlInjector::genTreeTop(TR::Node* n)
{
...
TR::SymbolReference* OMR::IlInjector::newTemp(TR::IlType* dt)
{
    return symRefTab()->createTemporary(_methodSymbol, dt->getPrimitiveType());
}

TR::Node* OMR::IlInjector::parameter(int32_t slot, TR::IlType* dt)
{
    return TR::Node::createLoad(
        symRefTab()->findOrCreateAutoSymbol(_methodSymbol, slot, dt->getPrimitiveType(), true, false, true));
}

TR::Node* OMR::IlInjector::iconst(int32_t value) { return TR::Node::iconst(value); }

TR::Node* OMR::IlInjector::lconst(int64_t value) { return TR::Node::lconst(value); }

TR::Node* OMR::IlInjector::bconst(int8_t value) { return TR::Node::bconst(value); }

TR::Node* OMR::IlInjector::sconst(int16_t value) { return TR::Node::sconst(value); }

TR::Node* OMR::IlInjector::aconst(uintptr_t value) { return TR::Node::aconst(value); }

TR::Node* OMR::IlInjector::dconst(double value)
{
...
TR::Node* OMR::IlInjector::fconst(float value)
{
...
TR::Node* OMR::IlInjector::staticAddress(void* address) { return TR::Node::aconst((uintptr_t)address); }

void OMR::IlInjector::storeToTemp(TR::SymbolReference* tempSymRef, TR::Node* value)
{
    genTreeTop(TR::Node::createStore(tempSymRef, value));
}

TR::Node* OMR::IlInjector::loadTemp(TR::SymbolReference* tempSymRef) { return TR::Node::createLoad(tempSymRef); }

TR::Node* OMR::IlInjector::i2l(TR::Node* n) { return TR::Node::create(TR::i2l, 1, n); }

TR::Node* OMR::IlInjector::iu2l(TR::Node* n) { return TR::Node::create(TR::iu2l, 1, n); }

void OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node* first, TR::Node* second, TR::Block* targetBlock)
{
...
void OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node* first, TR::Node* second, int32_t targetBlockNumber)
{
    ifjump(op, first, second, _blocks[targetBlockNumber]);
}

TR::Node* OMR::IlInjector::shiftLeftBy(TR::Node* value, int32_t shift)
{
    TR::Node* result;
...
TR::Node* OMR::IlInjector::multiplyBy(TR::Node* value, int64_t factor)
{
    TR::Node* result;
    if (value->getDataType() == TR::Int32)
...
TR::Node* OMR::IlInjector::arrayLoad(TR::Node* base, TR::Node* index, TR::IlType* dt)
{
    TR::Node* scaledIndex;
    TR::Node* element;
    if (index->getDataType() == TR::Int32) {
        ...
        scaledIndex
            = createWithoutSymRef(TR::lmul, 2, index, lconst((int64_t)TR::DataType::getSize(dt->getPrimitiveType())));
    ...
    TR::Node* load
        = TR::Node::createWithSymRef(TR::ILOpCode::indirectLoadOpCode(dt->getPrimitiveType()), 1, element, 0, symRef);
    return load;
}

void OMR::IlInjector::returnValue(TR::Node* value)
{
...
void OMR::IlInjector::returnNoValue()
{
...
void OMR::IlInjector::validateTargetBlock()
{
    TR_ASSERT(_currentBlock != NULL, "Current Block is NULL! Probably need to call generateToBlock() beforehand");
    TR_ASSERT(_currentBlockNumber >= 0,
        "Current Block Number is invalid! Probably need to call generateToBlock() beforehand");
}

void OMR::IlInjector::gotoBlock(TR::Block* destBlock)
{
...
void OMR::IlInjector::generateFallThrough()
{
    cfg()->addEdge(_currentBlock, _blocks[_currentBlockNumber + 1]);
    generateToBlock(_currentBlockNumber + 1);
}
 ...
 * */
TR::Node* OMR::IlInjector::createWithoutSymRef(TR::ILOpCodes opCode, uint32_t numArgs, ...)
{
