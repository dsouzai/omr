...
OMR::VirtualMachineOperandArray::VirtualMachineOperandArray(TR::MethodBuilder* mb, int32_t numOfElements,
    TR::IlType* elementType, TR::VirtualMachineRegister* arrayBaseRegister)
...
void OMR::VirtualMachineOperandArray::Commit(TR::IlBuilder* b)
{
    ...
    for (int32_t i = 0; i < _numberOfElements; i++) {
        ...
        if (element != NULL) {
            b->StoreAt(b->IndexAt(pElement, arrayBase, b->ConstInt32(i)), element);
        }
    }
}

void OMR::VirtualMachineOperandArray::Reload(TR::IlBuilder* b)
{
    TR::IlType* Element = _elementType;
    TR::IlType* pElement = _mb->typeDictionary()->PointerTo(Element);
    ...
    for (int32_t i = 0; i < _numberOfElements; i++) {
        _values[i] = b->LoadAt(pElement, b->IndexAt(pElement, array, b->ConstInt32(i)));
    }
}

void OMR::VirtualMachineOperandArray::MergeInto(TR::VirtualMachineState* o, TR::IlBuilder* b)
{
    ...
    for (int32_t i = 0; i < _numberOfElements; i++) {
        if (NULL == other->_values[i]) {
            TR_ASSERT_FATAL(_values[i] == NULL,
                "if an element in the dest array at index is NULL it has to be NULL in the src array");
        } else if (other->_values[i]->getID() != _values[i]->getID()) {
            // Types have to match!!!
            TR_ASSERT_FATAL(_values[i]->getDataType() == other->_values[i]->getDataType(),
                "invalid array merge: primitive type mismatch at same index");
...
void OMR::VirtualMachineOperandArray::UpdateArray(TR::IlBuilder* b, TR::IlValue* array)
{
...
TR::VirtualMachineState* OMR::VirtualMachineOperandArray::MakeCopy()
{
    TR::VirtualMachineOperandArray* copy = (TR::VirtualMachineOperandArray*)TR::comp()->trMemory()->allocateHeapMemory(
        sizeof(TR::VirtualMachineOperandArray));
    new (copy) TR::VirtualMachineOperandArray(static_cast<TR::VirtualMachineOperandArray*>(this));

    return copy;
}

TR::IlValue* OMR::VirtualMachineOperandArray::Get(int32_t index)
{
...
void OMR::VirtualMachineOperandArray::Set(int32_t index, TR::IlValue* value)
{
    TR_ASSERT_FATAL(index < _numberOfElements, "index has to be less than the number of elements");
    TR_ASSERT_FATAL(index >= 0, "index can not be negative");
    ...
    if (NULL != _values[index]) {
        TraceIL("VirtualMachineOperandArray[ %p ]::Set index %d to %p(%d) old value was %p(%d)\n", this, index, value,
            value->getID(), _values[index], _values[index]->getID());
    } else {
...
void OMR::VirtualMachineOperandArray::Move(TR::IlBuilder* b, int32_t dstIndex, int32_t srcIndex)
{
    ...
    TraceIL("VirtualMachineOperandArray[ %p ]::Move builder %p move srcIndex %d %p(%d) to dstIndex %d %p(%d)\n", this,
        b, srcIndex, _values[srcIndex], _values[srcIndex]->getID(), dstIndex, _values[dstIndex],
        _values[dstIndex]->getID());
}

void* OMR::VirtualMachineOperandArray::client()
{
...
void OMR::VirtualMachineOperandArray::init()
{
    int32_t numBytes = _numberOfElements * sizeof(TR::IlValue*);
    _values = (TR::IlValue**)TR::comp()->trMemory()->allocateHeapMemory(numBytes);
