...
void OMR::VirtualMachineOperandStack::Commit(TR::IlBuilder* b)
{
    ...
    for (int32_t i = _stackTop; i >= 0; i--) {
        ...
        b->StoreAt(b->IndexAt(pElement, stack, b->ConstInt32(i - _stackOffset)),
            Pick(_stackTop - i)); // should generalize, maybe delegate element storage ?
    }
}

void OMR::VirtualMachineOperandStack::Reload(TR::IlBuilder* b)
{
    TR::IlType* Element = _elementType;
    TR::IlType* pElement = _mb->typeDictionary()->PointerTo(Element);
    ...
    TR::IlValue* stack = b->Load(_stackBaseName);
    for (int32_t i = _stackTop; i >= 0; i--) {
        _stack[i] = b->LoadAt(pElement, b->IndexAt(pElement, stack, b->ConstInt32(i - _stackOffset)));
    }
}

void OMR::VirtualMachineOperandStack::MergeInto(TR::VirtualMachineState* o, TR::IlBuilder* b)
{
    ...
    for (int32_t i = _stackTop; i >= 0; i--) {
        ...
        if (other->_stack[i]->getID() != _stack[i]->getID()) {
            ...
            TR_ASSERT_FATAL(_stack[i]->getDataType() == other->_stack[i]->getDataType(),
                "invalid stack merge: primitive type mismatch at same depth stack elements");
...
void OMR::VirtualMachineOperandStack::UpdateStack(TR::IlBuilder* b, TR::IlValue* stack)
{
...
TR::VirtualMachineState* OMR::VirtualMachineOperandStack::MakeCopy()
{
    TR::VirtualMachineOperandStack* copy = (TR::VirtualMachineOperandStack*)TR::comp()->trMemory()->allocateHeapMemory(
        sizeof(TR::VirtualMachineOperandStack));
    new (copy) TR::VirtualMachineOperandStack(static_cast<TR::VirtualMachineOperandStack*>(this));

    return copy;
}

void OMR::VirtualMachineOperandStack::Push(TR::IlBuilder* b, TR::IlValue* value)
{
    checkSize();
    _stack[++_stackTop] = value;
}

TR::IlValue* OMR::VirtualMachineOperandStack::Top()
{
    TR_ASSERT_FATAL(_stackTop >= 0, "no top: stack empty");
    return _stack[_stackTop];
}

TR::IlValue* OMR::VirtualMachineOperandStack::Pop(TR::IlBuilder* b)
{
    TR_ASSERT_FATAL(_stackTop >= 0, "stack underflow");
    return _stack[_stackTop--];
}

TR::IlValue* OMR::VirtualMachineOperandStack::Pick(int32_t depth)
{
    TR_ASSERT_FATAL(_stackTop >= depth, "pick request exceeds stack depth");
    return _stack[_stackTop - depth];
}

void OMR::VirtualMachineOperandStack::Drop(TR::IlBuilder* b, int32_t depth)
{
    TR_ASSERT_FATAL(_stackTop >= depth - 1, "stack underflow");
    _stackTop -= depth;
}

void OMR::VirtualMachineOperandStack::Dup(TR::IlBuilder* b)
{
...
void OMR::VirtualMachineOperandStack::copyTo(TR::VirtualMachineOperandStack* copy) { }

void OMR::VirtualMachineOperandStack::checkSize()
{
    if (_stackTop == _stackMax - 1)
        grow();
}

void OMR::VirtualMachineOperandStack::grow(int32_t growAmount)
{
    if (growAmount == 0)
...
void* OMR::VirtualMachineOperandStack::client()
{
...
void OMR::VirtualMachineOperandStack::init()
{
    int32_t numBytes = _stackMax * sizeof(TR::IlValue*);
    _stack = (TR::IlValue**)TR::comp()->trMemory()->allocateHeapMemory(numBytes);
    ...
    TR::SymbolReference* symRef = comp->getSymRefTab()->createTemporary(
        _mb->methodSymbol(), _mb->typeDictionary()->PointerTo(_elementType)->getPrimitiveType());
