...
void OMR::IlBuilder::pullInBuilderTrees(TR::IlBuilder *builder, uint32_t *currentBlock, TR::TreeTop **firstTree,
...
TR::IlValue *OMR::IlBuilder::Add(TR::IlValue *left, TR::IlValue *right)
{
    ...
    if (left->getDataType() == TR::Address) {
        ...
        returnValue = binaryOpFromNodes(TR::Compiler->target.is32Bit() ? TR::aiadd : TR::aladd, loadValue(left),
            loadValue(right));
    } else {
 ...
 */
void OMR::IlBuilder::appendExceptionHandler(TR::Block *blockThrowsException, TR::IlBuilder **handler,
    uint32_t catchType)
{
...
void OMR::IlBuilder::setHandlerInfo(uint32_t catchType)
{
    ...
    catchBlock->setHandlerInfoWithOutBCInfo(catchType, static_cast<uint8_t>(comp()->getInlineDepth()), -1,
        _methodSymbol->getResolvedMethod(), comp());
}
...
TR::IlValue *OMR::IlBuilder::genOperationWithOverflowCHK(TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode,
    TR::IlBuilder **handler, TR::ILOpCodes overflow)
{
    /*
     * BB1:
     *    overflowCHK
...
TR::IlValue *OMR::IlBuilder::SubWithOverflow(TR::IlBuilder **handler, TR::IlValue *left, TR::IlValue *right)
{
    TR::Node *leftNode = loadValue(left);
    TR::Node *rightNode = loadValue(right);
    TR::IlValue *subValue = genOperationWithOverflowCHK(TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode,
        rightNode, handler, TR::OverflowCHK);
...
TR::IlValue *OMR::IlBuilder::SubWithUnsignedOverflow(TR::IlBuilder **handler, TR::IlValue *left, TR::IlValue *right)
{
    TR::Node *leftNode = loadValue(left);
    TR::Node *rightNode = loadValue(right);
    TR::IlValue *unsignedSubValue = genOperationWithOverflowCHK(TR::ILOpCode::subtractOpCode(leftNode->getDataType()),
        leftNode, rightNode, handler, TR::UnsignedOverflowCHK);
...
TR::IlValue *OMR::IlBuilder::MulWithOverflow(TR::IlBuilder **handler, TR::IlValue *left, TR::IlValue *right)
{
    TR::Node *leftNode = loadValue(left);
    TR::Node *rightNode = loadValue(right);
    TR::IlValue *mulValue = genOperationWithOverflowCHK(TR::ILOpCode::multiplyOpCode(leftNode->getDataType()), leftNode,
        rightNode, handler, TR::OverflowCHK);
...
TR::IlValue *OMR::IlBuilder::genCall(TR::SymbolReference *methodSymRef, int32_t numArgs, TR::IlValue **argValues,
    bool isDirectCall /* true by default*/)
{
    TR::DataType returnType = methodSymRef->getSymbol()->castToMethodSymbol()->getMethod()->returnType();
    TR::Node *callNode = TR::Node::createWithSymRef(isDirectCall ? TR::ILOpCode::getDirectCall(returnType)
                                                                 : TR::ILOpCode::getIndirectCall(returnType),
        numArgs, methodSymRef);
...
void OMR::IlBuilder::ifCmpCondition(TR_ComparisonTypes ct, bool isUnsignedCmp, TR::IlValue *left, TR::IlValue *right,
    TR::Block *target)
{
    integerizeAddresses(&left, &right);
    TR::Node *leftNode = loadValue(left);
...
TR::IlBuilder::JBCase *OMR::IlBuilder::MakeCase(int32_t caseValue, TR::IlBuilder **caseBuilder,
...
void OMR::IlBuilder::generateSwitchCases(TR::Node *switchNode, TR::Node *defaultNode, TR::IlBuilder **defaultBuilder,
    uint32_t numCases, JBCase **cases)
{
    ...
    TR::IlBuilder **continueBuilder, TR::IlValue *initial, TR::IlValue *end, TR::IlValue *increment)
{
    ...
    *loopCode = createBuilderIfNeeded(*loopCode);
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(*continueBuilder == NULL,
            "ForLoop returns continueBuilder, cannot provide continueBuilder as input");
...
void OMR::IlBuilder::DoWhileLoop(const char *whileCondition, TR::IlBuilder **body, TR::IlBuilder **breakBuilder,
    TR::IlBuilder **continueBuilder)
{
    ...
    *body = createBuilderIfNeeded(*body);
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(*continueBuilder == NULL,
            "DoWhileLoop returns continueBuilder, cannot provide continueBuilder as input");
    ...
    if (breakBuilder) {
        TR_ASSERT_FATAL(*breakBuilder == NULL,
            "DoWhileLoop returns breakBuilder, cannot provide breakBuilder as input");
        *breakBuilder = OrphanBuilder();
        AppendBuilder(*breakBuilder);
    }
...
void OMR::IlBuilder::WhileDoLoop(const char *whileCondition, TR::IlBuilder **body, TR::IlBuilder **breakBuilder,
    TR::IlBuilder **continueBuilder)
{
    methodSymbol()->setMayHaveLoops(true);
    ...
    if (breakBuilder) {
        TR_ASSERT_FATAL(*breakBuilder == NULL,
            "WhileDoLoop returns breakBuilder, cannot provide breakBuilder as input");
        *breakBuilder = done;
    }
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(*continueBuilder == NULL,
            "WhileDoLoop returns continueBuilder, cannot provide continueBuilder as input");
        *continueBuilder = loopContinue;
    }
