...
bool OMR::IlBuilder::TraceEnabled_log()
{
...
void OMR::IlBuilder::setupForBuildIL()
{
    initSequence();
...
void OMR::IlBuilder::print(const char* title, bool recurse)
{
    if (!TraceEnabled)
        return;
...
void OMR::IlBuilder::printBlock(TR::Block* block)
{
    if (!TraceEnabled)
        return;
 ...
 */
void OMR::IlBuilder::SetCurrentIlGenerator() { comp()->setCurrentIlGenerator((TR_IlGenerator*)this); }

TR::SymbolReference* OMR::IlBuilder::lookupSymbol(const char* name)
{
...
void OMR::IlBuilder::defineSymbol(const char* name, TR::SymbolReference* symRef)
{
...
void OMR::IlBuilder::defineValue(const char* name, TR::IlType* type)
{
...
TR::IlValue* OMR::IlBuilder::newValue(TR::DataType dt, TR::Node* n)
{
...
TR::IlValue* OMR::IlBuilder::newValue(TR::IlType* dt, TR::Node* n) { return newValue(dt->getPrimitiveType(), n); }

TR::IlValue* OMR::IlBuilder::NewValue(TR::IlType* dt)
{
    TR_ASSERT_FATAL(0, "should not create a value without a TR::Node");
    return 0;
}

TR::IlValue* OMR::IlBuilder::Copy(TR::IlValue* value)
{
    ...
    TraceIL("IlBuilder[ %p ]::Copy value (%d) dataType (%d) to newVal (%d) at cpIndex (%d)\n", this, value->getID(),
        dt.getDataType(), newVal->getID(), newSymRef->getCPIndex());

    return newVal;
}

TR::TreeTop* OMR::IlBuilder::getFirstTree()
{
...
TR::TreeTop* OMR::IlBuilder::getLastTree()
{
    TR_ASSERT_FATAL(_blocks, "_blocks not created yet");
...
uint32_t OMR::IlBuilder::countBlocks()
{
...
void OMR::IlBuilder::pullInBuilderTrees(
    TR::IlBuilder* builder, uint32_t* currentBlock, TR::TreeTop** firstTree, TR::TreeTop** newLastTree)
{
...
bool OMR::IlBuilder::connectTrees()
{
    ...
    for (SequenceEntry* entry = iter.getFirst(); !iter.atEnd(); entry = iter.getNext()) {
        ...
        if (lastTree) {
            TraceIL("[ %p ] Connecting tree %p [ node %p ] to new tree %p [ node %p ]\n", this, lastTree,
                lastTree->getNode(), firstTree, firstTree->getNode());
    ...
    if (_methodBuilder != this || _methodBuilder->callerMethodBuilder() != NULL) {
        ...
        TraceIL("[ %p ] Connecting last tree %p [ node %p ] to exit block entry %p [ node %p ]\n", this, lastTree,
            lastTree->getNode(), _exitBlock->getEntry(), _exitBlock->getEntry()->getNode());
        lastTree->join(_exitBlock->getEntry());

        // also add exit block to blocks array and add an edge from last block to EXIT if the builder comes back (i.e.
        // doesn't return or branch off somewhere)
        if (comesBack())
            cfg()->addEdge(blocks[currentBlock - 1], getExit());

        blocks[currentBlock++] = _exitBlock;
        TraceIL("[ %p ] exit block %d is %p (%p -> %p)\n", this, _exitBlock->getNumber(), _exitBlock->getEntry(),
            _exitBlock->getExit());
...
TR::Node* OMR::IlBuilder::zero(TR::DataType dt)
{
    switch (dt) {
    case TR::Int8:
        return TR::Node::bconst(0);
    case TR::Int16:
        return TR::Node::sconst(0);
    case TR::Int32:
        return TR::Node::iconst(0);
    case TR::Int64:
        return TR::Node::lconst(0);
    default:
        return TR::Node::create(TR::ILOpCode::constOpCode(dt), 0, 0);
    }
    TR_ASSERT_FATAL(0, "should not reach here");
}

TR::Node* OMR::IlBuilder::zero(TR::IlType* dt) { return zero(dt->getPrimitiveType()); }

TR::Node* OMR::IlBuilder::zeroNodeForValue(TR::IlValue* v) { return zero(v->getDataType()); }

TR::IlValue* OMR::IlBuilder::zeroForValue(TR::IlValue* v)
{
    TR::IlValue* returnValue = newValue(v->getDataType(), zeroNodeForValue(v));
    return returnValue;
}

TR::IlBuilder* OMR::IlBuilder::OrphanBuilder()
{
...
TR::BytecodeBuilder* OMR::IlBuilder::OrphanBytecodeBuilder(int32_t bcIndex, char* name)
{
...
TR::Block* OMR::IlBuilder::emptyBlock()
{
...
TR::IlBuilder* OMR::IlBuilder::createBuilderIfNeeded(TR::IlBuilder* builder)
{
    if (builder == NULL)
...
OMR::IlBuilder::SequenceEntry* OMR::IlBuilder::blockEntry(TR::Block* block)
{
    return new (_comp->trMemory()->trHeapMemory()) IlBuilder::SequenceEntry(block);
}

OMR::IlBuilder::SequenceEntry* OMR::IlBuilder::builderEntry(TR::IlBuilder* builder)
{
    return new (_comp->trMemory()->trHeapMemory()) IlBuilder::SequenceEntry(builder);
}

void OMR::IlBuilder::appendBlock(TR::Block* newBlock, bool addEdge)
{
    if (newBlock == NULL) {
        newBlock = emptyBlock();
    }
...
void OMR::IlBuilder::AppendBuilder(TR::IlBuilder* builder)
{
    ...
    // when trees are connected, exit block will swing down to become last trees in builder, so it can fall through to
    // this block we're about to create need to add edge explicitly because of this exit block sleight of hand
    appendNoFallThroughBlock();
    cfg()->addEdge(builder->getExit(), _currentBlock);
}

TR::Node* OMR::IlBuilder::loadValue(TR::IlValue* v) { return v->load(_currentBlock); }

void OMR::IlBuilder::storeNode(TR::SymbolReference* symRef, TR::Node* v)
{
    genTreeTop(TR::Node::createStore(symRef, v));
}

void OMR::IlBuilder::indirectStoreNode(TR::Node* addr, TR::Node* v)
{
...
TR::IlValue* OMR::IlBuilder::indirectLoadNode(TR::IlType* dt, TR::Node* addr, bool isVectorLoad)
{
 ...
 * @param varName the name of the local variable to be stored into. If the name has not been used before, this local
 * variable will take the same data type as the value being written to it.
 * @param value IlValue that should be written to the local variable, which should be the same data type
 */
void OMR::IlBuilder::Store(const char* varName, TR::IlValue* value)
{
    ...
    TraceIL("IlBuilder[ %p ]::Store %s %d (%d) gets %d\n", this, varName, symRef->getCPIndex(),
        symRef->getReferenceNumber(), value->getID());
 ...
 */
void OMR::IlBuilder::StoreOver(TR::IlValue* dest, TR::IlValue* value)
{
 ...
 * @param varName the name of the local variable to be vector stored into. if the name has not been used before, this
 * local variable will take the same data type as the value being written to it. The width of this data will be
 * determined by the vector data type of IlValue.
 * @param value IlValue with the vector data that should be written to the local variable, and should have the same data
 * type
 */
void OMR::IlBuilder::VectorStore(const char* varName, TR::IlValue* value)
{
 ...
 */
void OMR::IlBuilder::StoreAt(TR::IlValue* address, TR::IlValue* value)
{
 ...
 * @param address the pointer address through which the vector value will be written. The width of the store will be
 * determined by the vector data type of IlValue.
 * @param value IlValue with the vector data that should be written  at "address"
 */
void OMR::IlBuilder::VectorStoreAt(TR::IlValue* address, TR::IlValue* value)
{
...
TR::IlValue* OMR::IlBuilder::CreateLocalArray(int32_t numElements, TR::IlType* elementType)
{
    ...
    TraceIL("IlBuilder[ %p ]::CreateLocalArray array allocated %d bytes, address in %d\n", this, size,
        arrayAddressValue->getID());
    return arrayAddressValue;
}

TR::IlValue* OMR::IlBuilder::CreateLocalStruct(TR::IlType* structType)
{
    ...
    TraceIL("IlBuilder[ %p ]::CreateLocalStruct struct allocated %d bytes, address in %d\n", this, size,
        structAddressValue->getID());
    return structAddressValue;
}

void OMR::IlBuilder::StoreIndirect(const char* type, const char* field, TR::IlValue* object, TR::IlValue* value)
{
...
TR::IlValue* OMR::IlBuilder::Load(const char* name)
{
    ...
    TraceIL("IlBuilder[ %p ]::Load %s into %d from symref %d\n", this, name, returnValue->getID(),
        symRef->getReferenceNumber());
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::VectorLoad(const char* name)
{
...
TR::IlValue* OMR::IlBuilder::LoadIndirect(const char* type, const char* field, TR::IlValue* object)
{
    TR::IlReference* fieldRef = _types->FieldReference(type, field);
    TR::SymbolReference* symRef = fieldRef->symRef();
    TR::DataType fieldType = symRef->getSymbol()->getDataType();
    TR::IlValue* returnValue = newValue(fieldType,
        TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(fieldType), 1, loadValue(object), 0, symRef));
    TraceIL("IlBuilder[ %p ]::%d is LoadIndirect %s.%s from (%d)\n", this, returnValue->getID(), type, field,
        object->getID());
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::LoadAt(TR::IlType* dt, TR::IlValue* address)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is LoadAt type %d address %d\n", this, returnValue->getID(),
        dt->getPrimitiveType().getDataType(), address->getID());
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::VectorLoadAt(TR::IlType* dt, TR::IlValue* address)
{
    TR_ASSERT_FATAL(address->getDataType() == TR::Address, "LoadAt needs an address operand");
...
TR::IlValue* OMR::IlBuilder::IndexAt(TR::IlType* dt, TR::IlValue* base, TR::IlValue* index)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is IndexAt(%s) base %d index %d\n", this, address->getID(), dt->getName(),
        base->getID(), index->getID());
 ...
 */
TR::IlValue* OMR::IlBuilder::StructFieldInstanceAddress(const char* structName, const char* fieldName, TR::IlValue* obj)
{
 ...
 */
TR::IlValue* OMR::IlBuilder::UnionFieldInstanceAddress(const char* unionName, const char* fieldName, TR::IlValue* obj)
{
    auto ptype = typeDictionary()->PointerTo(typeDictionary()->UnionFieldType(unionName, fieldName));
    return ConvertTo(ptype, obj);
}

TR::IlValue* OMR::IlBuilder::NullAddress()
{
...
TR::IlValue* OMR::IlBuilder::ConstInt8(int8_t value)
{
...
TR::IlValue* OMR::IlBuilder::ConstInt16(int16_t value)
{
...
TR::IlValue* OMR::IlBuilder::ConstInt32(int32_t value)
{
...
TR::IlValue* OMR::IlBuilder::ConstInt64(int64_t value)
{
...
TR::IlValue* OMR::IlBuilder::ConstFloat(float value)
{
...
TR::IlValue* OMR::IlBuilder::ConstDouble(double value)
{
...
TR::IlValue* OMR::IlBuilder::ConstString(const char* const value)
{
...
TR::IlValue* OMR::IlBuilder::ConstAddress(const void* const value)
{
    TR::IlValue* returnValue = newValue(Address, TR::Node::aconst((uintptr_t)value));
...
TR::IlValue* OMR::IlBuilder::ConstInteger(TR::IlType* intType, int64_t value)
{
    if (intType == Int8)
        return ConstInt8((int8_t)value);
    else if (intType == Int16)
        return ConstInt16((int16_t)value);
    else if (intType == Int32)
        return ConstInt32((int32_t)value);
    else if (intType == Int64)
...
TR::IlValue* OMR::IlBuilder::ConvertTo(TR::IlType* t, TR::IlValue* v)
{
    TR::DataType typeFrom = v->getDataType();
    TR::DataType typeTo = t->getPrimitiveType();
    if (typeFrom == typeTo) {
        TraceIL(
            "IlBuilder[ %p ]::%d is ConvertTo (already has type %s) %d\n", this, v->getID(), t->getName(), v->getID());
        return v;
    }
...
TR::IlValue* OMR::IlBuilder::UnsignedConvertTo(TR::IlType* t, TR::IlValue* v)
{
    TR::DataType typeFrom = v->getDataType();
    TR::DataType typeTo = t->getPrimitiveType();
    if (typeFrom == typeTo) {
        TraceIL("IlBuilder[ %p ]::%d is UnsignedConvertTo (already has type %s) %d\n", this, v->getID(), t->getName(),
            v->getID());
        return v;
    }
    TR::IlValue* convertedValue = convertTo(typeTo, v, true);
    TraceIL(
        "IlBuilder[ %p ]::%d is UnsignedConvertTo(%s) %d\n", this, convertedValue->getID(), t->getName(), v->getID());
    return convertedValue;
}

TR::IlValue* OMR::IlBuilder::Negate(TR::IlValue* v)
{
    TR::DataType dataType = v->getDataType();

    TR::ILOpCodes negateOp = ILOpCode::negateOpCode(dataType);
    TR_ASSERT_FATAL(negateOp != TR::BadILOp, "Builder [ %p ] cannot negate value %d of type %s", this, v->getID(),
        dataType.toString());
...
TR::IlValue* OMR::IlBuilder::convertTo(TR::DataType typeTo, TR::IlValue* v, bool needUnsigned)
{
    TR::DataType typeFrom = v->getDataType();

    TR::ILOpCodes convertOp = ILOpCode::getProperConversion(typeFrom, typeTo, needUnsigned);
    TR_ASSERT_FATAL(convertOp != TR::BadILOp, "Builder [ %p ] unknown conversion requested for value %d %s to %s", this,
        v->getID(), typeFrom.toString(), typeTo.toString());
...
TR::IlValue* OMR::IlBuilder::ConvertBitsTo(TR::IlType* t, TR::IlValue* v)
{
    TR::DataType typeFrom = v->getDataType();
    TR::DataType typeTo = t->getPrimitiveType();
    ...
    TR_ASSERT_FATAL(convertOpcode != TR::BadILOp && TR::DataType::getSize(typeTo) == TR::DataType::getSize(typeFrom),
        "Builder [ %p ] requested bit conversion for value %d from type of size %d (%s) to type of size %d (%s) "
        "(consider using ConvertTo() to for narrowing/widening)",
        this, v->getID(), TR::DataType::getSize(typeFrom), typeFrom.toString(), TR::DataType::getSize(typeTo),
        typeTo.toString());
    TR_ASSERT_FATAL(convertOpcode != TR::BadILOp,
        "Builder [ %p ] unknown bit conversion requested for value %d (%s) to type %s", this, v->getID(),
        typeFrom.toString(), t->getName());
...
TR::IlValue* OMR::IlBuilder::unaryOp(TR::ILOpCodes op, TR::IlValue* v)
{
...
void OMR::IlBuilder::doVectorConversions(TR::Node** leftPtr, TR::Node** rightPtr)
{
...
TR::IlValue* OMR::IlBuilder::widenIntegerTo32Bits(TR::IlValue* v)
{
...
TR::IlValue* OMR::IlBuilder::widenIntegerTo32BitsUnsigned(TR::IlValue* v)
{
    if (v->getDataType() == TR::Int8 || v->getDataType() == TR::Int16)
...
TR::Node* OMR::IlBuilder::binaryOpNodeFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
    TR::DataType leftType = leftNode->getDataType();
    TR::DataType rightType = rightNode->getDataType();
    bool isAddressBump = ((leftType == TR::Address) && (rightType == TR::Int32 || rightType == TR::Int64));
    bool isRevAddressBump = ((rightType == TR::Address) && (leftType == TR::Int32 || leftType == TR::Int64));
    TR_ASSERT_FATAL(leftType == rightType || isAddressBump || isRevAddressBump,
...
TR::IlValue* OMR::IlBuilder::binaryOpFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
...
TR::IlValue* OMR::IlBuilder::binaryOpFromOpMap(OpCodeMapper mapOp, TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::binaryOpFromOpCode(TR::ILOpCodes op, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
...
TR::IlValue* OMR::IlBuilder::compareOp(TR_ComparisonTypes ct, bool needUnsigned, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
...
TR::IlValue* OMR::IlBuilder::NotEqualTo(TR::IlValue* left, TR::IlValue* right)
{
...
void OMR::IlBuilder::Goto(TR::IlBuilder** dest)
{
...
void OMR::IlBuilder::Return()
{
    TR::IlBuilder* returnBuilder = _methodBuilder->returnBuilder();
    if (returnBuilder != NULL) {
        TR_ASSERT_FATAL(_methodBuilder->returnSymbol() == NULL,
            "Return() from inlined call did not expect a pre-existing returnSymbol");
...
void OMR::IlBuilder::Return(TR::IlValue* value)
{
    TR::IlBuilder* returnBuilder = _methodBuilder->returnBuilder();
    if (returnBuilder != NULL) {
        ...
        TraceIL("IlBuilder[ %p ]::Return %d back to caller's returnBuilder [ %p ] via return symbol called %s\n", this,
            value->getID(), returnBuilder, returnSymbol);
...
TR::IlValue* OMR::IlBuilder::Sub(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::Add(TR::IlValue* left, TR::IlValue* right)
{
    TR::IlValue* returnValue = NULL;
    if (left->getDataType() == TR::Address) {
        ...
        returnValue = binaryOpFromNodes(
            TR::Compiler->target.is32Bit() ? TR::aiadd : TR::aladd, loadValue(left), loadValue(right));
    } else {
 ...
 */
void OMR::IlBuilder::appendExceptionHandler(
    TR::Block* blockThrowsException, TR::IlBuilder** handler, uint32_t catchType)
{
    ...
    *handler = createBuilderIfNeeded(*handler);
    ...
    TraceIL("blockAfterExceptionHandler block_%d, blockWithGoto block_%d \n", blockAfterExceptionHandler->getNumber(),
        blockWithGoto->getNumber());
    gotoNode->setBranchDestination(blockAfterExceptionHandler->getEntry());
    cfg()->addEdge(blockWithGoto, blockAfterExceptionHandler);
}

void OMR::IlBuilder::setHandlerInfo(uint32_t catchType)
{
    TR::Block* catchBlock = getEntry();
    catchBlock->setIsCold();
    catchBlock->setHandlerInfoWithOutBCInfo(
        catchType, static_cast<uint8_t>(comp()->getInlineDepth()), -1, _methodSymbol->getResolvedMethod(), comp());
}

TR::Node* OMR::IlBuilder::genOverflowCHKTreeTop(TR::Node* operationNode, TR::ILOpCodes overflow)
{
    TR::Node* overflowChkNode = TR::Node::createWithRoomForOneMore(overflow, 3,
        symRefTab()->findOrCreateOverflowCheckSymbolRef(_methodSymbol), operationNode, operationNode->getFirstChild(),
        operationNode->getSecondChild());
    overflowChkNode->setOverflowCheckOperation(operationNode->getOpCodeValue());
    genTreeTop(overflowChkNode);
    return overflowChkNode;
}

TR::IlValue* OMR::IlBuilder::genOperationWithOverflowCHK(
    TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode, TR::IlBuilder** handler, TR::ILOpCodes overflow)
{
    /*
     * BB1:
     *    overflowCHK
...
TR::ILOpCodes OMR::IlBuilder::getOpCode(TR::IlValue* leftValue, TR::IlValue* rightValue)
{
    TR::ILOpCodes op;
    if (leftValue->getDataType() == TR::Address) {
        TR_ASSERT_FATAL((TR::Compiler->target.is64Bit() && rightValue->getDataType() == TR::Int64)
                || (TR::Compiler->target.is32Bit() && rightValue->getDataType() == TR::Int32),
            "the right child type must be either TR::Int32 (on 32-bit ISA) or TR::Int64 (on 64-bit ISA) when the left "
            "child of Add is TR::Address\n");
...
TR::IlValue* OMR::IlBuilder::AddWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
...
TR::IlValue* OMR::IlBuilder::AddWithUnsignedOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
    TR::ILOpCodes opcode = getOpCode(left, right);
    TR::IlValue* addValue = genOperationWithOverflowCHK(opcode, leftNode, rightNode, handler, TR::UnsignedOverflowCHK);
    TraceIL("IlBuilder[ %p ]::%d is AddWithUnsignedOverflow %d + %d\n", this, addValue->getID(), left->getID(),
        right->getID());
    return addValue;
}

TR::IlValue* OMR::IlBuilder::SubWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
    TR::IlValue* subValue = genOperationWithOverflowCHK(
        TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode, rightNode, handler, TR::OverflowCHK);
    TraceIL("IlBuilder[ %p ]::%d is SubWithOverflow %d + %d\n", this, subValue->getID(), left->getID(), right->getID());
    return subValue;
}

TR::IlValue* OMR::IlBuilder::SubWithUnsignedOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
    TR::IlValue* unsignedSubValue = genOperationWithOverflowCHK(
        TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode, rightNode, handler, TR::UnsignedOverflowCHK);
    TraceIL("IlBuilder[ %p ]::%d is UnsignedSubWithOverflow %d + %d\n", this, unsignedSubValue->getID(), left->getID(),
        right->getID());
    return unsignedSubValue;
}

TR::IlValue* OMR::IlBuilder::MulWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
    TR::IlValue* mulValue = genOperationWithOverflowCHK(
        TR::ILOpCode::multiplyOpCode(leftNode->getDataType()), leftNode, rightNode, handler, TR::OverflowCHK);
    TraceIL("IlBuilder[ %p ]::%d is MulWithOverflow %d + %d\n", this, mulValue->getID(), left->getID(), right->getID());
    return mulValue;
}

TR::IlValue* OMR::IlBuilder::Mul(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::Div(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::UnsignedDiv(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::Rem(TR::IlValue* left, TR::IlValue* right)
{
    TR::DataType returnType = left->getDataType();
...
TR::IlValue* OMR::IlBuilder::UnsignedRem(TR::IlValue* left, TR::IlValue* right)
{
    TR::DataType returnType = left->getDataType();
    ...
    if (returnType.isInt64()) {
        ...
        TR::IlValue* truncLeft
            = binaryOpFromOpCode(TR::ILOpCodes::lmul, binaryOpFromOpCode(TR::ILOpCodes::ludiv, left, right), right);
...
TR::IlValue* OMR::IlBuilder::And(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::Or(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::Xor(TR::IlValue* left, TR::IlValue* right)
{
...
TR::Node* OMR::IlBuilder::shiftOpNodeFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
    TR::DataType leftType = leftNode->getDataType();
    TR::DataType rightType = rightNode->getDataType();
    TR_ASSERT_FATAL(leftType.isIntegral() && rightType.isInt32(),
        "shift operation first operand must be an integer, and shift amount must be 32-bit integer");

    return TR::Node::create(op, 2, leftNode, rightNode);
}

TR::IlValue* OMR::IlBuilder::shiftOpFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
...
TR::IlValue* OMR::IlBuilder::shiftOpFromOpMap(OpCodeMapper mapOp, TR::IlValue* left, TR::IlValue* right)
{
    TR::Node* leftNode = loadValue(left);
    TR::DataType leftType = leftNode->getDataType();
...
TR::IlValue* OMR::IlBuilder::ShiftL(TR::IlValue* v, TR::IlValue* amount)
{
...
TR::IlValue* OMR::IlBuilder::ShiftR(TR::IlValue* v, TR::IlValue* amount)
{
...
TR::IlValue* OMR::IlBuilder::UnsignedShiftR(TR::IlValue* v, TR::IlValue* amount)
{
 ...
 */
void OMR::IlBuilder::IfAnd(
    TR::IlBuilder** allTrueBuilder, TR::IlBuilder** anyFalseBuilder, int32_t numTerms, JBCondition** terms)
{
 ...
 */
void OMR::IlBuilder::IfAnd(TR::IlBuilder** allTrueBuilder, TR::IlBuilder** anyFalseBuilder, int32_t numTerms, ...)
{
 ...
 */
void OMR::IlBuilder::IfOr(
    TR::IlBuilder** anyTrueBuilder, TR::IlBuilder** allFalseBuilder, int32_t numTerms, JBCondition** terms)
{
    TR::IlBuilder* mergePoint = OrphanBuilder();
 ...
 */
void OMR::IlBuilder::IfOr(TR::IlBuilder** anyTrueBuilder, TR::IlBuilder** allFalseBuilder, int32_t numTerms, ...)
{
    JBCondition** terms = (JBCondition**)_comp->trMemory()->allocateHeapMemory(numTerms * sizeof(JBCondition*));
    TR_ASSERT_FATAL(NULL != terms, "out of memory");
...
TR::IlBuilder::JBCondition* OMR::IlBuilder::MakeCondition(TR::IlBuilder* conditionBuilder, TR::IlValue* conditionValue)
{
...
TR::IlValue* OMR::IlBuilder::EqualTo(TR::IlValue* left, TR::IlValue* right)
{
...
void OMR::IlBuilder::integerizeAddresses(TR::IlValue** leftPtr, TR::IlValue** rightPtr)
{
...
TR::IlValue* OMR::IlBuilder::LessThan(TR::IlValue* left, TR::IlValue* right)
{
...
TR::IlValue* OMR::IlBuilder::UnsignedLessThan(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
    TR::IlValue* returnValue = compareOp(TR_cmpLT, true, left, right);
    TraceIL("IlBuilder[ %p ]::%d is UnsignedLessThan %d < %d?\n", this, returnValue->getID(), left->getID(),
        right->getID());
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::LessOrEqualTo(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
    TR::IlValue* returnValue = compareOp(TR_cmpLE, false, left, right);
    TraceIL(
        "IlBuilder[ %p ]::%d is LessOrEqualTo %d <= %d?\n", this, returnValue->getID(), left->getID(), right->getID());
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::UnsignedLessOrEqualTo(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
...
TR::IlValue* OMR::IlBuilder::GreaterThan(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
...
TR::IlValue* OMR::IlBuilder::UnsignedGreaterThan(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
...
TR::IlValue* OMR::IlBuilder::GreaterOrEqualTo(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
...
TR::IlValue* OMR::IlBuilder::UnsignedGreaterOrEqualTo(TR::IlValue* left, TR::IlValue* right)
{
    integerizeAddresses(&left, &right);
...
TR::IlValue** OMR::IlBuilder::processCallArgs(TR::Compilation* comp, int numArgs, va_list args)
{
 ...
 */
TR::IlValue* OMR::IlBuilder::ComputedCall(const char* functionName, int32_t numArgs, ...)
{
    ...
    TR::SymbolReference* methodSymRef
        = symRefTab()->findOrCreateComputedStaticMethodSymbol(JITTED_METHOD_INDEX, -1, resolvedMethod);
 ...
 */
TR::IlValue* OMR::IlBuilder::ComputedCall(const char* functionName, int32_t numArgs, TR::IlValue** argValues)
{
    TraceIL("IlBuilder[ %p ]::ComputedCall %s\n", this, functionName);
    TR::ResolvedMethod* resolvedMethod = _methodBuilder->lookupFunction(functionName);
    if (resolvedMethod == NULL && _methodBuilder->RequestFunction(functionName))
 ...
 */
TR::IlValue* OMR::IlBuilder::Call(TR::MethodBuilder* calleeMB, int32_t numArgs, ...)
{
    va_list args;
    va_start(args, numArgs);
    TR::IlValue** argValues = processCallArgs(_comp, numArgs, args);
 ...
 * coming in a subsequent commit).
 */
TR::IlValue* OMR::IlBuilder::Call(TR::MethodBuilder* calleeMB, int32_t numArgs, TR::IlValue** argValues)
{
    ...
    // there shouldn't be any fall-through, but if there is it should go to the return block and we need to put it
    // somewhere anyway
    ...
    TraceIL(
        "IlBuilder[ %p ]::Call callee return value is %d loaded from %s\n", this, returnValue->getID(), returnSymbol);
    return returnValue;
}

TR::IlValue* OMR::IlBuilder::Call(const char* functionName, int32_t numArgs, ...)
{
    TraceIL("IlBuilder[ %p ]::Call %s\n", this, functionName);
    va_list args;
    va_start(args, numArgs);
    ...
    TR::SymbolReference* methodSymRef
        = symRefTab()->findOrCreateStaticMethodSymbol(JITTED_METHOD_INDEX, -1, resolvedMethod);
    methodSymRef->getSymbol()->getMethodSymbol()->setLinkage(TR_System);
    return genCall(methodSymRef, numArgs, argValues);
}

TR::IlValue* OMR::IlBuilder::Call(const char* functionName, int32_t numArgs, TR::IlValue** argValues)
{
    TraceIL("IlBuilder[ %p ]::Call %s\n", this, functionName);
    TR::ResolvedMethod* resolvedMethod = _methodBuilder->lookupFunction(functionName);
    if (resolvedMethod == NULL && _methodBuilder->RequestFunction(functionName))
...
TR::IlValue* OMR::IlBuilder::genCall(
    TR::SymbolReference* methodSymRef, int32_t numArgs, TR::IlValue** argValues, bool isDirectCall /* true by default*/)
{
    ...
    TR::Node* callNode = TR::Node::createWithSymRef(
        isDirectCall ? TR::ILOpCode::getDirectCall(returnType) : TR::ILOpCode::getIndirectCall(returnType), numArgs,
        methodSymRef);
    ...
    for (int32_t a = 0; a < numArgs; a++) {
        ...
        if (arg->getDataType() == TR::Int8 || arg->getDataType() == TR::Int16
            || (Word == Int64 && arg->getDataType() == TR::Int32))
 ...
 */
TR::IlValue* OMR::IlBuilder::AtomicAdd(TR::IlValue* baseAddress, TR::IlValue* value)
{
 ...
 *              |-->persistentFailurie
 */
void OMR::IlBuilder::Transaction(TR::IlBuilder** persistentFailureBuilder, TR::IlBuilder** transientFailureBuilder,
    TR::IlBuilder** transactionBuilder)
{
    ...
    *transactionBuilder = createBuilderIfNeeded(*transactionBuilder);
    ...
    TR::Node* persistentFailureNode
        = TR::Node::create(TR::branch, 0, (*persistentFailureBuilder)->getEntry()->getEntry());
    TR::Node* transientFailureNode
        = TR::Node::create(TR::branch, 0, (*transientFailureBuilder)->getEntry()->getEntry());
    TR::Node* transactionNode = TR::Node::create(TR::branch, 0, (*transactionBuilder)->getEntry()->getEntry());

    TR::Node* tStartNode
        = TR::Node::create(TR::tstart, 3, persistentFailureNode, transientFailureNode, transactionNode);
 ...
 */
void OMR::IlBuilder::TransactionAbort()
{
...
void OMR::IlBuilder::IfCmpNotEqualZero(TR::IlBuilder** target, TR::IlValue* condition)
{
...
void OMR::IlBuilder::IfCmpNotEqualZero(TR::IlBuilder* target, TR::IlValue* condition)
{
    ...
    TraceIL("IlBuilder[ %p ]::IfCmpNotEqualZero %d? -> [ %p ] B%d\n", this, condition->getID(), target,
        target->getEntry()->getNumber());
    ifCmpNotEqualZero(condition, target->getEntry());
}

void OMR::IlBuilder::IfCmpNotEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpNotEqual(TR::IlBuilder* target, TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::IfCmpNotEqual %d == %d? -> [ %p ] B%d\n", this, left->getID(), right->getID(), target,
        target->getEntry()->getNumber());
    ifCmpCondition(TR_cmpNE, false, left, right, target->getEntry());
}

void OMR::IlBuilder::IfCmpEqualZero(TR::IlBuilder** target, TR::IlValue* condition)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpLessThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedLessThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedLessThan(TR::IlBuilder* target, TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::IfCmpUnsignedLessThan %d < %d? -> [ %p ] B%d\n", this, left->getID(), right->getID(),
        target, target->getEntry()->getNumber());
    ifCmpCondition(TR_cmpLT, true, left, right, target->getEntry());
}

void OMR::IlBuilder::IfCmpLessOrEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedLessOrEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpGreaterThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpGreaterOrEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterOrEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterOrEqual(TR::IlBuilder* target, TR::IlValue* left, TR::IlValue* right)
{
    TraceIL("IlBuilder[ %p ]::IfCmpUnsignedGreaterOrEqual %d >= %d? -> [ %p ] B%d\n", this, left->getID(),
        right->getID(), target, target->getEntry()->getNumber());
    ifCmpCondition(TR_cmpGE, true, left, right, target->getEntry());
}

void OMR::IlBuilder::ifCmpCondition(
    TR_ComparisonTypes ct, bool isUnsignedCmp, TR::IlValue* left, TR::IlValue* right, TR::Block* target)
{
    integerizeAddresses(&left, &right);
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
    TR::ILOpCode cmpOpCode(TR::ILOpCode::compareOpCode(leftNode->getDataType(), ct, isUnsignedCmp));
    ifjump(cmpOpCode.convertCmpToIfCmp(), leftNode, rightNode, target);
    appendBlock();
}

void OMR::IlBuilder::ifCmpNotEqualZero(TR::IlValue* condition, TR::Block* target)
{
    ifCmpCondition(TR_cmpNE, false, condition, zeroForValue(condition), target);
}

void OMR::IlBuilder::ifCmpEqualZero(TR::IlValue* condition, TR::Block* target)
{
    ifCmpCondition(TR_cmpEQ, false, condition, zeroForValue(condition), target);
}

void OMR::IlBuilder::appendGoto(TR::Block* destBlock)
{
    gotoBlock(destBlock);
    appendBlock();
}
 ...
 * #1: then path exists "out of line", else path falls through from If and then jumps to merge, followed by then path
 * which falls through to the merge point
 */
void OMR::IlBuilder::IfThenElse(TR::IlBuilder** thenPath, TR::IlBuilder** elsePath, TR::IlValue* condition)
{
...
void OMR::IlBuilder::Switch(
    TR::IlValue* selectorValue, TR::IlBuilder** defaultBuilder, uint32_t numCases, JBCase** cases)
{
...
void OMR::IlBuilder::Switch(TR::IlValue* selectorValue, TR::IlBuilder** defaultBuilder, uint32_t numCases, ...)
{
    va_list args;
...
void OMR::IlBuilder::TableSwitch(TR::IlValue* selectorValue, TR::IlBuilder** defaultBuilder, bool generateBoundsCheck,
    uint32_t numCases, JBCase** cases)
{
...
void OMR::IlBuilder::TableSwitch(
    TR::IlValue* selectorValue, TR::IlBuilder** defaultBuilder, bool generateBoundsCheck, uint32_t numCases, ...)
{
    va_list args;
    va_start(args, numCases);
    JBCase** cases = createCaseArray(numCases, args);
...
TR::IlBuilder::JBCase* OMR::IlBuilder::MakeCase(
    int32_t caseValue, TR::IlBuilder** caseBuilder, int32_t caseFallsThrough)
{
...
TR::IlValue* OMR::IlBuilder::Select(TR::IlValue* condition, TR::IlValue* trueValue, TR::IlValue* falseValue)
{
    ...
    TR_ASSERT_FATAL(dt == falseValue->getDataType(), "Select requires trueValue and falseValue to be of the same type");
...
TR::IlBuilder::JBCase** OMR::IlBuilder::createCaseArray(uint32_t numCases, va_list args)
{
...
void OMR::IlBuilder::generateSwitchCases(
    TR::Node* switchNode, TR::Node* defaultNode, TR::IlBuilder** defaultBuilder, uint32_t numCases, JBCase** cases)
{
...
void OMR::IlBuilder::ForLoop(bool countsUp, const char* indVar, TR::IlBuilder** loopCode, TR::IlBuilder** breakBuilder,
    TR::IlBuilder** continueBuilder, TR::IlValue* initial, TR::IlValue* end, TR::IlValue* increment)
{
    ...
    *loopCode = createBuilderIfNeeded(*loopCode);
    ...
    TraceIL("IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d loopCode %p countsUp %d\n", this, indVar,
        initial->getID(), end->getID(), increment->getID(), *loopCode, countsUp);
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(
            *continueBuilder == NULL, "ForLoop returns continueBuilder, cannot provide continueBuilder as input");
        *continueBuilder = loopContinue;
    }

    if (countsUp) {
        loopContinue->Store(indVar, loopContinue->Add(loopContinue->Load(indVar), increment));
        loopContinue->IfCmpLessThan(&loopBody, loopContinue->Load(indVar), end);
    } else {
        loopContinue->Store(indVar, loopContinue->Sub(loopContinue->Load(indVar), increment));
        loopContinue->IfCmpGreaterThan(&loopBody, loopContinue->Load(indVar), end);
    }
...
void OMR::IlBuilder::DoWhileLoop(
    const char* whileCondition, TR::IlBuilder** body, TR::IlBuilder** breakBuilder, TR::IlBuilder** continueBuilder)
{
    methodSymbol()->setMayHaveLoops(true);
    ...
    *body = createBuilderIfNeeded(*body);
    TraceIL(
        "IlBuilder[ %p ]::DoWhileLoop do body B%d while %s\n", this, (*body)->getEntry()->getNumber(), whileCondition);
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(
            *continueBuilder == NULL, "DoWhileLoop returns continueBuilder, cannot provide continueBuilder as input");
        loopContinue = *continueBuilder = OrphanBuilder();
    } else
        loopContinue = OrphanBuilder();

    AppendBuilder(loopContinue);
    loopContinue->IfCmpNotEqualZero(body, loopContinue->Load(whileCondition));

    if (breakBuilder) {
        TR_ASSERT_FATAL(
            *breakBuilder == NULL, "DoWhileLoop returns breakBuilder, cannot provide breakBuilder as input");
        *breakBuilder = OrphanBuilder();
        AppendBuilder(*breakBuilder);
    }
...
void OMR::IlBuilder::WhileDoLoop(
    const char* whileCondition, TR::IlBuilder** body, TR::IlBuilder** breakBuilder, TR::IlBuilder** continueBuilder)
{
    methodSymbol()->setMayHaveLoops(true);
    ...
    if (breakBuilder) {
        TR_ASSERT_FATAL(
            *breakBuilder == NULL, "WhileDoLoop returns breakBuilder, cannot provide breakBuilder as input");
        *breakBuilder = done;
    }
    ...
    if (continueBuilder) {
        TR_ASSERT_FATAL(
            *continueBuilder == NULL, "WhileDoLoop returns continueBuilder, cannot provide continueBuilder as input");
        *continueBuilder = loopContinue;
    }

    AppendBuilder(loopContinue);
    loopContinue->IfCmpEqualZero(&done, loopContinue->Load(whileCondition));
...
void* OMR::IlBuilder::JBCase::client()
{
    if (_client == NULL && _clientAllocator != NULL)
...
void* OMR::IlBuilder::JBCondition::client()
{
    if (_client == NULL && _clientAllocator != NULL)
