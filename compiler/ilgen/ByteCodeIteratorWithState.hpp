...
template<typename ByteCode, ByteCode BCunknown, class ByteCodeIterator, typename OperandStackElementType>
class TR_ByteCodeIteratorWithState : public ByteCodeIterator {
public:
...
protected:
    enum ByteCodeWithStateFlags { inExceptionRange = 0x01, generated = 0x02 };
    ...
    void markExceptionRange(int32_t index, int32_t start, int32_t end, int32_t handler, uint32_t type)
    {
        ...
        _tryCatchInfo.insert(
            _tryCatchInfo.begin() + index, typename ByteCodeIterator::TryCatchInfo(start, end, handler, type));
    ...
    ///  operand stack slots are saved into locals; other blocks can then access the state of the operand stack by
    ///  loading those locals
    ...
    ///  saveStack to make sure the state of the operand stack is accessible at the beginning of the basic block
    ///  associated with target returns the TR::TreeTop to which new treetops in the target block should be appended
    ...
    /// initializes the operand stack state for the given bytecode index, typically called at the beginning of a basic
    /// block
    ...
    ///   once found, setupBBStartContext is called to ensure everything is ready to start translating bytecodes at that
    ///   index
    /// if nothing left to translate, returns _maxByteCode + 8 as a number bigger than the exit test for the simulation
    /// loop
    ...
    ///   NOTE: if copyCount > shiftCount then (copyCount - shiftCount) elements above the top of the original operand
    ///   stack will not be set
    ...
    ///   first shifts operand stack by shiftCount, then copies the shifted elements underneath the shifted operand
    ///   stack values
