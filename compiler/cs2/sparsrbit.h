...
template<class Allocator> class ASparseBitVector : private Allocator {
    class SparseBitRef;
...
public:
    ...
    ASparseBitVector(const Allocator& a = Allocator())
        : Allocator(a)
        , fBase(NULL)
        , fNumberOfSegments(0)
    { }
    ...
    ~ASparseBitVector() { ClearToNull(); }

    static bool hasFastRandomLookup() { return false; }
    ...
    template<class A2> ASparseBitVector& operator=(const ASparseBitVector<A2>&);

    template<class B2> ASparseBitVector& operator=(const B2&);
    ...
    {
        return false;
    } // returns whether this bit vector is based on an array of bitWords (which it is not)
    int32_t FirstOneWordIndex() const
    {
        CS2Assert(false, ("This method is not implemented"));
        return 0;
    }
    ...
    uint32_t WordAt(uint32_t wordIndex) const
    {
        CS2Assert(false, ("This method is not implemented"));
        return 0;
    }
    ...
    bool And(const ASparseBitVector& inputVector, ASparseBitVector& outputVector) const;
    bool Andc(const ASparseBitVector& inputVector);
    template<class B2> bool Andc(const B2& inputVector);

    bool Andc(const ASparseBitVector& inputVector, ASparseBitVector& outputVector) const;
    bool Or(const ASparseBitVector& inputVector);
    bool Or(const ASparseBitVector& inputVector, ASparseBitVector& outputVector) const;

    bool OrMask(const ASparseBitVector& inputVector, const ASparseBitVector& maskVector);
    ...
    // is non-zero
    template<class B2> bool Intersects(const B2& inputVector) const;

    bool Intersects(const ASparseBitVector& inputVector) const;

    // Determine if the intersection of this vector and the input vector
    // under the given mask is non-zero
    bool IntersectsWithMask(const ASparseBitVector& inputVector, const ASparseBitVector& maskVector) const;
...
private:
    ...
    public:
        ...
        template<class S2> void copy(S2& s, Allocator& a)
        {
        ...
        template<class S2> void append(S2& s, uint16_t fromIndex, uint16_t toIndex)
        {
        ...
        template<class S2> void append(S2& s) { append(s, 0, s.PopulationCount()); }

        void append(uint16_t v) { fSegment[fNumValues++] = v; }
        ...
        {
            fSegment = (uint16_t*)a.reallocate(size * sizeof(uint16_t), fSegment, fSize * sizeof(uint16_t));
            fSize = static_cast<uint16_t>(size);
        }
        ...
        {
            if (fHighBits != s1.fHighBits)
                return false;
            if (fNumValues != s1.fNumValues)
                return false;

            return !memcmp(fSegment, s1.fSegment, sizeof(uint16_t) * PopulationCount());
        }

        size_t MemoryUsage() { return sizeof(Segment) + fSize * sizeof(uint16_t); }
    ...
    //
    // if numValues is given, then it must accommodate numValues. Thus if numValues exceeds given size then grow size by
    // some factor more. The new size is then guaranteed to exceed numValues.
    ...
    template<class S2> void CopySegment(Segment&, const S2&);
    ...
    class SparseBitRef {
    public:
        SparseBitRef(ASparseBitVector& vector, SparseBitIndex index)
            : fIndex(index)
            , fVector(vector)
        { }
        // ~SparseBitRef()  destructor intentionally omitted
        SparseBitRef(const SparseBitRef& r)
            : fIndex(r.fIndex)
            , fVector(r.fVector)
        { }
...
public:
    ...
    public:
        IntersectionCursor(const ASparseBitVector& adoptVector1, const ASparseBitVector& adoptVector2)
            : c1(adoptVector1)
            , c2(adoptVector2)
        { }
        ...
        bool Valid() const { return c1.Valid() && c2.Valid(); }
        operator SparseBitIndex() const { return c1; }
    ...
    private:
        bool FindNextCommonItem()
        {
            while (c1 != c2) {
                if (c1 < c2) {
                    if (!c1.SetToNextOneAfter(c2 - 1))
                        return false;
                } else if (!c2.SetToNextOneAfter(c1 - 1))
                    return false;
            }
            return true;
        }
    ...
    class MaskCursor {
    public:
        MaskCursor(const ASparseBitVector& adoptVector, const ASparseBitVector& maskVector)
            : c(adoptVector)
            , m(maskVector)
        { }
        ...
        operator SparseBitIndex() const { return c; }
    ...
    template<class A2> friend class ASparseBitVector;

    template<class A2> friend inline void Swap(ASparseBitVector<A2>& vectorA, ASparseBitVector<A2>& vectorB);
};

// SparseBitRef methods
template<class Allocator> inline ASparseBitVector<Allocator>::SparseBitRef::operator bool()
{
    return fVector.ValueAt(fIndex);
}

template<class Allocator>
inline typename ASparseBitVector<Allocator>::SparseBitRef& ASparseBitVector<Allocator>::SparseBitRef::operator=(
    bool bitValue)
{
...
template<class Allocator> inline void Swap(ASparseBitVector<Allocator>& vectorA, ASparseBitVector<Allocator>& vectorB)
{
...
template<class Allocator>
template<class B2>
inline ASparseBitVector<Allocator>& ASparseBitVector<Allocator>::operator=(const B2& vector)
{
    Clear();
    ...
    while (c1.Valid()) {
        ...
        for (c1.SetToNextOne(); c1.Valid() && ((c1 & ~kSegmentMask) == highbits); c1.SetToNextOne()) {
            count++;
        }
...
template<class Allocator> inline void ASparseBitVector<Allocator>::Compact()
{
    size_t i;
    for (i = 0; i < fNumberOfSegments; i++)
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::operator[](size_t index) const
{
    return ValueAt(index);
}

template<class Allocator> inline bool ASparseBitVector<Allocator>::ValueAt(size_t elementIndex) const
{
    Segment* s = FindSegment(static_cast<SparseBitIndex>(elementIndex));
    if (s)
        return GetSegment(*s, static_cast<SparseBitIndex>(elementIndex));
    return false;
}

template<class Allocator> inline SparseBitIndex ASparseBitVector<Allocator>::FirstOne() const
{
...
template<class Allocator> inline SparseBitIndex ASparseBitVector<Allocator>::LastOne() const
{
...
template<class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::ClearLastOneIfThereIsOneInRange(
    SparseBitIndex low, SparseBitIndex high, bool& foundone)
{
    if (IsZero()) {
        foundone = false;
...
template<class Allocator> inline void ASparseBitVector<Allocator>::Clear()
{
...
template<class Allocator> inline void ASparseBitVector<Allocator>::ClearToNull()
{
    Clear();
    fBase = NULL;
}
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Cursor::Valid() const
{
    return fSegmentIndex < fVector.fNumberOfSegments;
}

template<class Allocator> inline ASparseBitVector<Allocator>::Cursor::operator SparseBitIndex() const
{
    CS2Assert(Valid(), ("Expecting valid cursor"));
    return fSegmentHighBits | fSegment[fSegmentOffset];
}

template<class Allocator> inline void ASparseBitVector<Allocator>::Cursor::SetToFirstOne()
{
    fSegmentIndex = 0;
    fSegmentOffset = 0;
    if (Valid()) {
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Cursor::SetToNextOne()
{
    ...
    CS2Assert(fSegmentPopCount == fVector.fBase[fSegmentIndex].PopulationCount() - 1,
        ("ASparseBitVector updated durign traversal %d %d", fSegmentPopCount,
            fVector.fBase[fSegmentIndex].PopulationCount() - 1));
    fSegmentIndex += 1;
    if (!Valid())
        return false;
    fSegmentOffset = 0;
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Cursor::SetToPreviousOne()
{
    CS2Assert(Valid(), ("Cannot advance an invalid cursor"));
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Cursor::SetToNextOneAfter(SparseBitIndex next)
{
    CS2Assert(Valid(), ("Cannot advance an invalid cursor"));
...
template<class Allocator> inline void ASparseBitVector<Allocator>::Cursor::SetToLastOne()
{
...
template<class Allocator> inline SparseBitIndex ASparseBitVector<Allocator>::SizeInBits() const
{
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::IsZero() const { return fNumberOfSegments == 0; }

// ASparseBitVector<Allocator>::IsNull
//
// Determine if this vector is null (and also is zero)

template<class Allocator> inline bool ASparseBitVector<Allocator>::IsNull() const
{
    return (fNumberOfSegments == 0) && (fBase == NULL);
}

template<class Allocator>
inline typename ASparseBitVector<Allocator>::Segment* ASparseBitVector<Allocator>::FindSegment(
    SparseBitIndex index) const
{
    size_t i, n = fNumberOfSegments;
    for (i = 0; i < n; i++) {
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::Intersects(const ASparseBitVector<Allocator>& inputVector) const
{
    ...
    while (i1 < n1 && i2 < n2) {
        ...
        if (s1->fHighBits == s2->fHighBits) {
            ...
            if ((s1->Indices()[0] <= s2->Indices()[m2]) && (s2->Indices()[0] <= s1->Indices()[m1])) {
                do {
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::operator!=(const ASparseBitVector& inputVector) const
{
...
template<class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::FindIndex(
    const typename ASparseBitVector<Allocator>::Segment& thisSegment, uint16_t search, SparseBitIndex l,
    SparseBitIndex h) const
{
    size_t low = l, high = h;
    CS2Assert(thisSegment.Indices(), ("Expecting non-null srbv segment"));
    if (high == 0)
        high = thisSegment.fNumValues - 1;

    if (search <= thisSegment.Indices()[low])
        return static_cast<SparseBitIndex>(low);
    if (search == thisSegment.Indices()[high])
        return static_cast<SparseBitIndex>(high);
    if (search > thisSegment.Indices()[high])
        return static_cast<SparseBitIndex>(high + 1);

    while (high - low > 16) {
        size_t mid = (high + low) / 2;
        if (search < thisSegment.Indices()[mid])
            high = mid;
        else if (search > thisSegment.Indices()[mid])
            low = mid;
        else
            return static_cast<SparseBitIndex>(mid);
    }

    while (low < high && thisSegment.Indices()[low] < search)
        low += 1;
    return static_cast<SparseBitIndex>(low);
}
...
// smallest bit in the segment that is larger or equal than search
template<class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::AdvanceIndex(
    const typename ASparseBitVector<Allocator>::Segment& thisSegment, uint16_t s, SparseBitIndex l,
    SparseBitIndex h) const
{
    ...
    while (low + 3 <= high) {
        if (thisSegment.Indices()[low] >= search)
            return low;
        if (thisSegment.Indices()[low + 1] >= search)
            return low + 1;
        if (thisSegment.Indices()[low + 2] >= search)
            return low + 2;
        if (thisSegment.Indices()[low + 3] >= search)
            return low + 3;
        low += 4;
    }
    if (low > high || thisSegment.Indices()[low] >= search)
        return low;
    if (low + 1 > high || thisSegment.Indices()[low + 1] >= search)
        return low + 1;
    if (low + 2 > high || thisSegment.Indices()[low + 2] >= search)
        return low + 2;
    return low + 3;
}

template<class Allocator>
inline void ASparseBitVector<Allocator>::GrowSegment(
    typename ASparseBitVector<Allocator>::Segment& thisSegment, uint32_t size, uint32_t numValues)
{
    if (numValues > 0) {
        if (numValues <= 1024) {
    ...
    if (thisSegment.AllocatedSize() > size)
        return;

    if (size > kSegmentMask)
        size = kSegmentMask;
...
template<class Allocator>
inline void ASparseBitVector<Allocator>::SetSegment(
    typename ASparseBitVector<Allocator>::Segment& thisSegment, SparseBitIndex bit)
{
    ...
    if (thisSegment.AllocatedSize() == thisSegment.fNumValues)
        GrowSegment(thisSegment, thisSegment.AllocatedSize(), thisSegment.fNumValues + 1);

    memmove(&thisSegment.Indices()[t + 1], &thisSegment.Indices()[t], (thisSegment.fNumValues - t) * sizeof(uint16_t));
    CS2Assert(thisSegment.fNumValues < thisSegment.AllocatedSize(),
        ("Segment Overflow %d(%d %d)>=%d", (thisSegment.fNumValues - 1 - t) + (t + 1), thisSegment.fNumValues, t,
            thisSegment.AllocatedSize()));

    thisSegment.Indices()[t] = bit;
    CS2Assert(t < thisSegment.AllocatedSize(), ("Segment Overflow %d>=%d", t, thisSegment.AllocatedSize()));

    thisSegment.fNumValues += 1;
}

template<class Allocator>
inline void ASparseBitVector<Allocator>::ClearSegment(
    typename ASparseBitVector<Allocator>::Segment& thisSegment, SparseBitIndex bit)
{
    SparseBitIndex t = FindIndex(thisSegment, bit);
    ...
    if (t < thisSegment.fNumValues && bit == ValueAtSegment(thisSegment, t)) {
        thisSegment.fNumValues -= 1;
        memmove(
            &thisSegment.Indices()[t], &thisSegment.Indices()[t + 1], (thisSegment.fNumValues - t) * sizeof(uint16_t));
    }
}

template<class Allocator>
inline bool ASparseBitVector<Allocator>::GetSegment(
    typename ASparseBitVector<Allocator>::Segment& thisSegment, SparseBitIndex bit) const
{
    CS2Assert(thisSegment.Indices(), ("Expecting non-null srbv segment"));
    SparseBitIndex t = FindIndex(thisSegment, bit);
...
template<class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::ValueAtSegment(
    const typename ASparseBitVector<Allocator>::Segment& thisSegment, SparseBitIndex i) const
{
    CS2Assert(thisSegment.Indices(), ("Expecting non-null srbv segment"));
...
template<class Allocator>
inline typename ASparseBitVector<Allocator>::Segment* ASparseBitVector<Allocator>::AddSegment(
    SparseBitIndex index, SparseBitIndex count)
{
...
template<class Allocator>
inline typename ASparseBitVector<Allocator>::Segment* ASparseBitVector<Allocator>::AddSegment(
    SparseBitIndex index, SparseBitIndex count, uint16_t* bits)
{
...
template<class Allocator>
inline typename ASparseBitVector<Allocator>::Segment* ASparseBitVector<Allocator>::OrSegment(
    SparseBitIndex index, SparseBitIndex count, uint16_t* bits)
{
    CS2Assert(count > 0 && count <= kSegmentMask, ("OrSegment count OOB=%d", count));
...
template<class Allocator>
template<class S2>
inline void ASparseBitVector<Allocator>::CopySegment(
    typename ASparseBitVector<Allocator>::Segment& thisSegment, const S2& inputSegment)
{
    CS2Assert(thisSegment.HighBits() == inputSegment.HighBits(),
        ("Copying segments with diff hb %d!=%d", thisSegment.HighBits(), inputSegment.HighBits()));
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::OrSegment(typename ASparseBitVector<Allocator>::Segment& thisSegment,
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::OrSegment(const typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment,
    typename ASparseBitVector<Allocator>::Segment& outputSegment)
{
    ...
    if (toAllocate > kSegmentMask)
        toAllocate = kSegmentMask + 1;
...
// Segment methods
template<class Allocator>
inline bool ASparseBitVector<Allocator>::OrSegmentMask(typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment,
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::AndSegment(typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment)
{
    if (thisSegment.IsZero())
        return false;
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::AndSegment(const typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment,
    typename ASparseBitVector<Allocator>::Segment& outputSegment)
{
    if (thisSegment.IsZero()) {
    ...
    for (i = 0; i < thisSegment.PopulationCount(); i++) {
        while (ii < inputSegment.PopulationCount() && inputSegment.Indices()[ii] < thisSegment.Indices()[i]) {
            ii += 1;
        }
        ...
        if (inputSegment.Indices()[ii] == thisSegment.Indices()[i]) {
            CS2Assert(oi < outputSegment.AllocatedSize(),
                ("Writing past the end of segment %d >= %d (is=%d)", oi, outputSegment.AllocatedSize(),
                    PopulationCount()));
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::AndcSegment(typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment)
{
    if (thisSegment.IsZero())
        return false;
    ...
    while (thisSegment.Indices()[i] != inputSegment.Indices()[ii]) {
        i = AdvanceIndex(thisSegment, inputSegment.Indices()[ii], i);
        if (i == myPopCount)
            return false; // reached end of this vector;

        ii = AdvanceIndex(inputSegment, thisSegment.Indices()[i], ii);
        if (ii == inPopCount)
            return false; // reached end of input vector;
    }
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::AndcSegment(const typename ASparseBitVector<Allocator>::Segment& thisSegment,
    const typename ASparseBitVector<Allocator>::Segment& inputSegment,
    typename ASparseBitVector<Allocator>::Segment& outputSegment)
{
    ...
    for (i = 0; i < thisSegment.PopulationCount(); i++) {
        ...
        if (ii == inputSegment.PopulationCount() || inputSegment.Indices()[ii] != thisSegment.Indices()[i]) {
            CS2Assert(oi < outputSegment.AllocatedSize(),
                ("Writing past the end of segment %d >= %d (is=%d)", oi, outputSegment.AllocatedSize(),
                    PopulationCount()));
            outputSegment.Indices()[oi++] = thisSegment.Indices()[i];
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::IsSubset(const typename ASparseBitVector<Allocator>::Segment& thisSegment,
...
template<class Allocator>
template<class str>
inline void ASparseBitVector<Allocator>::DumpSegment(
    str& out, const typename ASparseBitVector<Allocator>::Segment& s) const
{
    SparseBitIndex i;
...
template<class Allocator>
inline void ASparseBitVector<Allocator>::TruncateSegment(typename ASparseBitVector<Allocator>::Segment& thisSegment)
{
    thisSegment.fNumValues = 0;
}

template<class Allocator> inline void ASparseBitVector<Allocator>::RemoveSegmentAt(SparseBitIndex segmentIndex)
{
    ...
    if (fNumberOfSegments > 1) {
        ...
        fBase = (Segment*)Allocator::reallocate(
            sizeof(Segment) * (fNumberOfSegments - 1), fBase, sizeof(Segment) * (fNumberOfSegments));
        fNumberOfSegments -= 1;
    } else {
...
template<class Allocator> inline void ASparseBitVector<Allocator>::RemoveSegment(SparseBitIndex index)
{
    SparseBitIndex i;
...
template<class Allocator> inline unsigned long ASparseBitVector<Allocator>::MemoryUsage() const
{
    unsigned long ret = 0;
    SparseBitIndex i;
    for (i = 0; i < fNumberOfSegments; i++)
...
//
// Print a sparse bit vector.
template<class Allocator, class str> inline str& operator<<(str& out, const ASparseBitVector<Allocator>& vector)
{
    vector.Dump(out);
    return out;
}

template<class Allocator> template<class str> inline void ASparseBitVector<Allocator>::Dump(str& out) const
{
    if (IsNull()) {
        out << "0";
        return;
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Andc(const ASparseBitVector<Allocator>& inputVector)
{
    if (IsZero())
        return false;
...
template<class Allocator> template<class B2> inline bool ASparseBitVector<Allocator>::Andc(const B2& inputVector)
{
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::Andc(
    const ASparseBitVector<Allocator>& inputVector, ASparseBitVector<Allocator>& outputVector) const
{
    ...
    for (i = 0; i < fNumberOfSegments; i++) {
        ...
        if (!is) {
            ...
            typename ASparseBitVector<Allocator>::Segment* os
                = outputVector.AddSegment(s->HighBits(), s->PopulationCount());

            changed = outputVector.AndcSegment(*s, *is, *os) || changed;
            if (os->IsZero())
                outputVector.RemoveSegment(s->HighBits());
        }
    }
    return changed;
}

template<class Allocator> template<class B2> inline bool ASparseBitVector<Allocator>::And(const B2& vector)
{
    if (IsZero())
        return false;
...
template<class Allocator> template<class B2> inline void ASparseBitVector<Allocator>::Or(const B2& abv)
{
    typename B2::Cursor abvc(abv);
    OrCursor(abvc);
}

template<class Allocator> template<class ACursor> inline void ASparseBitVector<Allocator>::OrCursor(ACursor& abvc)
{
    ...
    while (abvc.Valid()) {
        ...
        do {
            ...
            abvc.SetToNextOne();
        } while (bc < BUFFERSIZE && abvc.Valid() && (highBits == (abvc & ~kSegmentMask)));
...
template<class Allocator>
inline void ASparseBitVector<Allocator>::Or(ASparseBitVector<Allocator>** vs, SparseBitIndex nvs)
{
    if (nvs == 0)
        return;
    if (nvs == 1) {
    ...
    for (sc.SetToFirstOne(); sc.Valid(); sc.SetToNextOne()) {
        ...
        for (SparseBitIndex i = 0; i < nvs; i++) {
            typename ASparseBitVector<Allocator>::Segment* s = vs[i]->FindSegment(hbits);
...
template<class Allocator>
inline void ASparseBitVector<Allocator>::Or(
    ASparseBitVector<Allocator>** vs, SparseBitIndex nvs, const ASparseBitVector<Allocator>& mask)
{
    if (nvs == 0 || mask.IsZero())
        return;
    if (nvs == 1) {
        OrMask(*vs[0], mask);
        return;
    ...
    for (sc.SetToFirstOne(); sc.Valid(); sc.SetToNextOne()) {
        ...
        typename ASparseBitVector<Allocator>::Segment* ms = mask.FindSegment(hbits);
        SparseBitIndex mc = 0;
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::Or(const ASparseBitVector<Allocator>& inputVector)
{
    if (inputVector.IsZero())
        return false;
    if (IsZero()) {
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::Or(
    const ASparseBitVector<Allocator>& inputVector, ASparseBitVector<Allocator>& outputVector) const
{
    if (inputVector.IsZero()) {
        outputVector = *this;
    ...
    for (i = 0; i < fNumberOfSegments; i++) {
        ...
        if (is) {
            typename ASparseBitVector<Allocator>::Segment* os
                = outputVector.AddSegment(s->HighBits(), s->PopulationCount());
            if (outputVector.OrSegment(*s, *is, *os))
                changed = true;
        } else {
            outputVector.AddSegment(s->HighBits(), s->PopulationCount(), s->Indices());
        }
    ...
    for (i = 0; i < inputVector.fNumberOfSegments; i++) {
        ...
        if (!s) {
            typename ASparseBitVector<Allocator>::Segment* os
                = outputVector.AddSegment(is->HighBits(), is->PopulationCount(), is->Indices());
            changed = true;
        }
    }
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::OrMask(
    const ASparseBitVector<Allocator>& inputVector, const ASparseBitVector<Allocator>& maskVector)
{
    if (inputVector.IsZero() || maskVector.IsZero())
        return false;
    if (IsZero()) {
...
template<class Allocator> inline bool ASparseBitVector<Allocator>::And(const ASparseBitVector<Allocator>& inputVector)
{
    if (IsZero())
        return false;
    if (inputVector.IsZero()) {
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::And(
    const ASparseBitVector<Allocator>& inputVector, ASparseBitVector<Allocator>& outputVector) const
{
    if (IsZero()) {
        outputVector.Clear();
...
template<class Allocator>
template<class B2>
inline bool ASparseBitVector<Allocator>::Intersects(const B2& inputVector) const
{
    if (IsZero())
        return false;
...
template<class Allocator>
inline bool ASparseBitVector<Allocator>::operator==(const ASparseBitVector<Allocator>& inputVector) const
{
    if (IsZero() && inputVector.IsZero())
        return true;
    if (IsZero() || inputVector.IsZero())
    ...
    if (fNumberOfSegments != inputVector.fNumberOfSegments)
        return false;
    SparseBitIndex i;
    for (i = 0; i < fNumberOfSegments; i++) {
        if (!(fBase[i] == inputVector.fBase[i]))
            return false;
    }
    return true;
}
...
template<class Allocator>
template<class B2>
inline ASparseBitVector<Allocator>& ASparseBitVector<Allocator>::operator-=(const B2& v)
{
    Andc(v);
    return *this;
}
