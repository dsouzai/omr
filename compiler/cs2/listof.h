...
public:
    ...
    template<class Initializer> ListIndex Add(const Initializer&);
    ...
    template<class ostr> friend ostr& operator<<(ostr& out, const CS2_LI_DECL& list)
    {
        uint32_t listIndex;

        for (listIndex = 0; listIndex < list.NumberOfElements(); ++listIndex) {
            out << "[" << listIndex << "]:";
    ...
    public:
        ...
        operator uint32_t() const { return (fSegmentIndex * fList.ElementsPerSegment()) + fElementIndex; }

        AElementType& Data() const { return (fSegment + fElementIndex)->Element(); }
        AElementType& operator*() const { return Data(); }
        AElementType* operator->() const { return &Data(); }

        typename CS2_LI_DECL::DerivedElement* DerivedElement() const { return fSegment + fElementIndex; }
...
CS2_LI_TEMP
template<class Initializer> inline ListIndex CS2_LI_DECL::Add(const Initializer& inputElement)
{
    ListIndex newIndex;
...
CS2_LI_TEMP inline AElementType& CS2_LI_DECL::operator[](ListIndex index) const { return ElementAt(index); }

// ListOf::IsEmpty
//
// Predicate to determine if the list is empty.

CS2_LI_TEMP inline bool CS2_LI_DECL::IsEmpty() const { return (fNextAvailable == 0); }

// ListOf::NumberOfElements
//
// The number of allocated elements in the list.

CS2_LI_TEMP inline uint32_t CS2_LI_DECL::NumberOfElements() const { return fNextAvailable; }

// ListOf::~ListOf
//
// Destroy a list and any currently allocated elements.

CS2_LI_TEMP inline CS2_LI_DECL::~ListOf()
{
    ListIndex listIndex;

    // Destroy the existing elements.
    for (listIndex = 0; listIndex < NumberOfElements(); ++listIndex) {
        typename CS2_AR_DECL::DerivedElement* derivedElement
            = (typename CS2_AR_DECL::DerivedElement*)&ElementAt(listIndex);
...
CS2_LI_TEMP inline CS2_LI_DECL::ListOf(const CS2_LI_DECL& list)
...
CS2_LI_TEMP inline CS2_LI_DECL& CS2_LI_DECL::operator=(const CS2_LI_DECL& list)
{
    ListIndex listIndex;
...
CS2_LI_TEMP inline void CS2_LI_DECL::MakeEmpty(bool freeStorage)
{
    ListIndex listIndex;
...
CS2_LI_TEMP inline unsigned long CS2_LI_DECL::MemoryUsage() const
{
...
CS2_LI_TEMP inline void CS2_LI_DECL::Append(const CS2_LI_DECL& list)
{
    uint32_t listIndex;
