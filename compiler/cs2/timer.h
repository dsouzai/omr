...
public:
    ...
    void Reset(void)
    {
        memset(&start, 0, TIMEBASE_SZ);
        memset(&stop, 0, TIMEBASE_SZ);
    }
...
public:
     ...
     */
    void Start(void) { PlatformTimer::Start(); }
    ...
    static uint32_t sprintfMetric(
        char* line, Metric value, Metric total, bool alternativeFormat = false, bool csv = false)
    {
        uint32_t offset = 0;
        if (csv)
...
template<class Meter> class RunnableMeter : public Meter {
public:
    RunnableMeter(void)
        : Meter()
        , fIsRunning(false)
     ...
     * to be running also.
     */
    RunnableMeter(const RunnableMeter& t)
        : Meter()
     ...
     */
    bool IsRunning(void) const { return fIsRunning; }

private:
    bool fIsRunning;
};

template<class Meter, class Allocator> class PhaseMeasuringNode : private Allocator {
public:
    typedef typename Meter::Metric Metric;

    PhaseMeasuringNode(const char* name = NULL, const Allocator& a = Allocator())
        : Allocator(a)
        , fName(NULL)
        , fParent(0)
        , fChildMap(a)
    ...
    PhaseMeasuringNode(const PhaseMeasuringNode& node)
        : Allocator(node)
        , fName(NULL)
        , fParent(node.fParent)
        , fChildMap(node.fChildMap)
        , fTotalMetric(node.fTotalMetric)
        , fCount(node.fCount)
        , fRunning(node.fRunning)
    {
        if (node.fName) {
    ...
    {
        fCount += 1;
        CS2Assert(!fRunning, ("Starting meter already running: %s", Name()));
        fRunning = true;
        // printf("%-40.40s ", fName);
        fMeter.Start();
    }
    ...
    void Stop(void)
    {
        // printf("%-40.40s ", fName);
        fMeter.Stop();
        CS2Assert(fRunning, ("Starting stopped meter: %s", Name()));
        fRunning = false;
    ...
    void Dump(ostream& out, uint32_t indent = 0, Metric total = 0, bool running = false, bool alternativeFormat = false,
        bool csv = false);
 ...
 */
template<class Meter, class Allocator> class PhaseMeasuringSummary {
...
public:
     ...
     */
    PhaseMeasuringSummary(
        const char* name = "Total elapsed metric", const Allocator& a = Allocator(), bool collect = true)
        : fNodes(a)
        , fNode(0)
        , fCollect(collect)
    {
     ...
     */
    template<class ostream> void DumpSummary(ostream& s, bool alternativeFormat = false, bool csv = false);
    ...
    void Start(ListIndex child)
    {
        CS2Assert(fNodes[child].Parent() == GetCurrentMeter(),
            ("Incorrect meter nesting for meters %s %s", fNodes[GetCurrentMeter()].Name(), fNodes[child].Name()));
        fNodes[child].Start();
    ...
    ListIndex GetCurrentMeter() const { return fNode; }
    void SetCurrentMeter(ListIndex node) { fNode = node; }

    bool Collect() { return fCollect; }

    static PhaseMeasuringSummary<Meter, Allocator> fGlobal;
    static PhaseMeasuringSummary<Meter, Allocator>& Global() { return fGlobal; }

private:
    template<class ostream>
    void DumpSummaryNode(ostream& s, ListIndex root, uint32_t indent, Metric total, bool running = false,
...
template<size_t N> inline HashValue HashFunction(char const (&strVal)[N])
{
    int i;
...
template<class ostream>
inline void PhaseMeasuringNode<Meter, Allocator>::Dump(
    ostream& out, uint32_t indent, Metric total, bool running, bool alternativeFormat, bool csv)
{
    char line[2048];
...
template<class Meter, class Allocator>
template<class ostream>
inline void PhaseMeasuringSummary<Meter, Allocator>::DumpSummaryNode(
    ostream& out, ListIndex root, uint32_t indent, Metric total, bool running, bool alternativeFormat, bool csv)
{
...
public:
     ...
     */
    PhaseProfiler(const char* name = NULL, PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
        : fIndex(0)
        , fSummaryMeter(s)
    {
        if (name)
...
public:
     ...
     */
    template<size_t N>
    LexicalBlockProfiler(const char (&phase)[N], PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
    ...
    LexicalBlockProfiler(const char* phase, PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
        : PhaseProfiler<Meter, Allocator>(phase, s)
    {
        if (s.Collect()) {
            PhaseProfiler<Meter, Allocator>::Start();
    ...
    LexicalBlockProfiler(const char* phase, uint32_t count, PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
    ...
    LexicalBlockProfiler(
        const char* phase, const char* name, PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
        : PhaseProfiler<Meter, Allocator>(NULL, s)
    {
        if (s.Collect()) {
            char pname[1024];
     ...
     */
    ~LexicalBlockProfiler(void) { PhaseProfiler<Meter, Allocator>::Stop(); }
};
...
//
template<class Allocator> class PhaseTimingSummary : public PhaseMeasuringSummary<RunnableTimer, Allocator> {
private:
    // Should not be copy constructed or copied.
...
public:
    template<size_t N>
    LexicalBlockTimer(const char (&phase)[N], PhaseTimingSummary& s = PhaseTimingSummary::Global())
        : LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>(phase, s)
    { }

    LexicalBlockTimer(const char* phase, PhaseTimingSummary& s = PhaseTimingSummary::Global())
        : LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>(phase, s)
    { }

    LexicalBlockTimer(const char* phase, uint32_t count, PhaseTimingSummary& s = PhaseTimingSummary::Global())
        : LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>(phase, count, s)
    { }

    LexicalBlockTimer(const char* phase, const char* name, PhaseTimingSummary& s = PhaseTimingSummary::Global())
