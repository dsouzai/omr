...
public:
    ...
    void deallocate(void* pointer, size_t size) { free(pointer); }
    void* reallocate(size_t newsize, void* pointer, size_t size) { return realloc(pointer, newsize); }
};
    ...
    public:
        ...
        void* allocate(uint32_t index)
        {
            if (freelist) {
                ...
                CS2Assert(holds_address(ret), ("Found pointer outside of segment: %p", ret));
                return ret;
            }
        ...
        bool holds_address(void* pointer) { return (this <= pointer && pointer < ((char*)this) + segmentsize); }
        bool is_full(uint32_t index) { return alloc == element_count(index); }
        ...
        static size_t element_count(uint32_t index) { return (segmentsize - sizeof(Segment)) / element_size(index); }
        Segment* next_segment() { return next; }

        size_t freelist_size(uint32_t index) { return freed * element_size(index); }
...
public:
    heap_allocator(const base_allocator& a = base_allocator())
        : base_allocator(a)
    {
        for (uint32_t i = 0; i < segmentcount; i++)
            segments[i] = NULL;
    }
    heap_allocator(const heap_allocator& c)
        : base_allocator(c)
    {
        for (uint32_t i = 0; i < segmentcount; i++)
            segments[i] = NULL;
...
template<class base_allocator> class shared_allocator {
    base_allocator& base;

public:
    shared_allocator(base_allocator& b = base_allocator::instance())
        : base(b)
    { }

    void* allocate(size_t size) { return base.allocate(size); }

    void deallocate(void* pointer, size_t size) { return base.deallocate(pointer, size); }

    void* reallocate(size_t newsize, void* pointer, size_t size) { return base.reallocate(newsize, pointer, size); }
    ...
    template<typename T> operator TR::typed_allocator<T, shared_allocator>()
    {
        return TR::typed_allocator<T, shared_allocator>(*this);
    }

    friend bool operator==(const shared_allocator& left, const shared_allocator& right)
    {
...
public:
    arena_allocator(base_allocator b = base_allocator())
        : base_allocator(b)
        , segment(NULL)
        , allocated(0)
    { }
