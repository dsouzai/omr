...
template<class Allocator> class ABitVector : private Allocator {
private:
    class BitRef;
...
public:
    ...
    BitRef operator[](BitIndex bitIndex) { return BitRef(*this, bitIndex); }
    ...
    // Bitwise operations
    template<class B2> ABitVector& operator|=(const B2&);
    template<class B2> ABitVector& operator&=(const B2&);
    template<class B2> ABitVector& operator-=(const B2&);
    ...
    bool And(const ABitVector& inputVector, ABitVector& outputVector) const;
    bool Andc(const ABitVector& inputVector);
    bool Andc(const ABitVector& inputVector, ABitVector& outputVector) const;
    bool Or(const ABitVector& inputVector);
    bool Or(const ABitVector& inputVector, ABitVector& outputVector) const;
    bool Xor(const ABitVector& inputVector);
    bool Xor(const ABitVector& inputVector, ABitVector& outputVector) const;
    ...
    template<class B2> void Or(B2** vs, uint32_t nvs);
    ...
    class Cursor {
    public:
        Cursor(const ABitVector& adoptVector)
            : fVector(adoptVector)
            , fWord(0)
            , fIndex(0)
        ...
        bool SetToFirstOne() { return SetToNextOneAfter(0); }
        ...
        bool SetToNextOne()
        {
            ...
            if (fWord == 0) {
                BitWord wordIndex;
                for (wordIndex = (fIndex + kBitWordSize - 1) / kBitWordSize; wordIndex < fWordCount; wordIndex += 1) {
        ...
        bool Valid() const { return fIndex < fWordCount * kBitWordSize; }
        ...
        operator BitIndex() const { return fIndex; }
    ...
    public:
        ...
        // Dereference operator - identical to cast to index operator - note it returns the index, not a reference to
        // it.
        ...
        {
            return this == &other || (Cursor::fVector == other.fVector && Cursor::fIndex == other.fIndex);
        }
    ...
    template<class ostr> friend ostr& operator<<(ostr& out, const ABitVector& vector)
    {
        typename ABitVector<Allocator>::Cursor vectorCursor(vector);
        uint32_t i;

        out << "( ";
        for (vectorCursor.SetToFirstOne(), i = 0; vectorCursor.Valid(); vectorCursor.SetToNextOne(), ++i) {
...
private:
    ...
    public:
        ...
        BitRef& operator=(const BitRef& value) { return *this = (bool)value; }
    ...
    template<class A2> friend inline void Swap(ABitVector<A2>& vectorA, ABitVector<A2>& vectorB);
};

template<class Allocator> inline uint32_t ABitVector<Allocator>::SizeInBits() const { return fNumBits; }

template<class Allocator> inline BitWord& ABitVector<Allocator>::WordAt(uint32_t wordIndex) const
{
...
inline ABitVector<Allocator>::ABitVector(const ABitVector<Allocator>& adoptVector)
    : Allocator(adoptVector)
    , fNumBits(0)
    , fBitWords(NULL)
{
    *this = adoptVector;
...
template<class Allocator>
inline ABitVector<Allocator>::ABitVector(const ABitVector<Allocator>& adoptVector, const Allocator& a)
    : Allocator(a)
    , fNumBits(0)
    , fBitWords(NULL)
{
...
template<class Allocator> inline bool ABitVector<Allocator>::ValueAt(BitIndex bitIndex) const
{
    BitIndex wordIndex;
...
template<class Allocator> inline BitWord ABitVector<Allocator>::WordValueAt(BitIndex bitIndex, BitIndex wordIndex) const
{
    if (bitIndex >= SizeInBits())
        return BitWord(0);
...
template<class Allocator>
inline void ABitVector<Allocator>::GrowTo(BitIndex newBitSize, bool geometric, bool forceGeometric)
{
    ...
    if (geometric && (forceGeometric || newBitSize < 1024)) {
        uint32_t power2Size = 1;

        while (power2Size < newBitSize)
            power2Size <<= 1;
    ...
    if (oldBitSize == 0) {
        ...
        fBitWords = (BitWord*)Allocator::reallocate(SizeInBytes(numBits), fBitWords, SizeInBytes(oldBitSize));
        memset(((char*)fBitWords) + SizeInBytes(oldBitSize), 0, SizeInBytes(numBits) - SizeInBytes(oldBitSize));
    }
    fNumBits = numBits;
}

template<class Allocator> inline void ABitVector<Allocator>::GrowTo(const ABitVector& vector)
{
    GrowTo(vector.fNumBits, false, false);
}

template<class Allocator> inline bool ABitVector<Allocator>::operator!=(const ABitVector& v) const
{
    return !operator==(v);
}

template<class Allocator> inline uint32_t ABitVector<Allocator>::SizeInWords(BitIndex numBits)
{
#ifdef DEV_VERSION
...
template<class Allocator> inline uint32_t ABitVector<Allocator>::SizeInWords() const { return SizeInWords(fNumBits); }

template<class Allocator> inline uint32_t ABitVector<Allocator>::SizeInShortWords(BitIndex numBits)
{
#ifdef DEV_VERSION
    CS2Assert(numBits < 0xFFFFFFFFul - kBitWordSize, ("Illegal vector length"));
#endif
...
template<class Allocator> inline uint32_t ABitVector<Allocator>::SizeInBytes(BitIndex numBits)
{
...
template<class Allocator> inline bool ABitVector<Allocator>::And(const ABitVector& inputVector)
{
...
template<class Allocator> inline bool ABitVector<Allocator>::Andc(const ABitVector& inputVector)
{
...
template<class Allocator> inline bool ABitVector<Allocator>::Or(const ABitVector& inputVector)
{
    GrowTo(inputVector);
    return Or(inputVector, *this);
}

template<class Allocator> template<class B2> inline void ABitVector<Allocator>::Or(const B2& v)
{
...
template<class Allocator> template<class B2> inline void ABitVector<Allocator>::And(const B2& v)
{
...
template<class Allocator> template<class B2> inline void ABitVector<Allocator>::Andc(const B2& v)
{
    if (v.IsZero())
        return;
    if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
...
template<class Allocator> template<class B2> inline ABitVector<Allocator>& ABitVector<Allocator>::operator=(const B2& v)
{
    Clear();
    Or(v);
    return *this;
...
template<class Allocator> template<class B2> inline void ABitVector<Allocator>::Or(B2** vs, uint32_t nvs)
{
    uint32_t i;
    for (i = 0; i < nvs; i++) {
        Or(*(vs[i]));
...
template<class Allocator> inline bool ABitVector<Allocator>::Xor(const ABitVector& inputVector)
{
    GrowTo(inputVector);
...
template<class Allocator> inline void Swap(ABitVector<Allocator>& vectorA, ABitVector<Allocator>& vectorB)
{
...
template<class Allocator>
inline ABitVector<Allocator>& ABitVector<Allocator>::operator=(const ABitVector<Allocator>& adoptVector)
{
...
template<class Allocator> inline bool ABitVector<Allocator>::operator==(const ABitVector<Allocator>& v) const
{
    uint32_t bx = 0, wx = 0, minLen = Minimum(fNumBits, v.fNumBits);

    while (bx < minLen) {
        if (fBitWords[wx] != v.fBitWords[wx])
            return false;
        ++wx;
        bx += kBitWordSize;
    }
...
template<class Allocator> inline unsigned long ABitVector<Allocator>::MemoryUsage() const
{
...
template<class Allocator> inline void ABitVector<Allocator>::Clear()
{
    if (fNumBits) {
...
template<class Allocator> inline void ABitVector<Allocator>::ClearToNull()
{
    Clear();
...
template<class Allocator> inline void ABitVector<Allocator>::SetAll(BitIndex numBits)
{
    if (numBits == 0)
        return;
...
template<class Allocator> inline BitIndex ABitVector<Allocator>::LowestZero() const
{
    ...
    for (wordIndex = 0; (wordIndex < vectorWordSize) && (WordAt(wordIndex) == kFullMask); ++wordIndex)
        ;

    // If we found no zero bits return the index just beyond the vector
    if (wordIndex >= vectorWordSize)
        return fNumBits + 1;

    return wordIndex * kBitWordSize + BitManipulator::LeadingOnes(WordAt(wordIndex));
}
...
template<class Allocator> inline BitIndex ABitVector<Allocator>::FirstOne() const
{
    uint32_t wordIndex, vectorWordSize;
    ...
    for (wordIndex = 0; (wordIndex < vectorWordSize) && (WordAt(wordIndex) == kZeroBits); ++wordIndex)
        ;
    ...
    return wordIndex * kBitWordSize + BitManipulator::LeadingZeroes(WordAt(wordIndex));
}

template<class Allocator> inline BitIndex ABitVector<Allocator>::LastOne() const
{
    ...
    if (lastIndex >= vectorWordSize || WordAt(lastIndex) == kZeroBits)
        return 0;
    return lastIndex * kBitWordSize + kBitWordSize - BitManipulator::TrailingZeroes(WordAt(lastIndex)) - 1;
}
...
template<class Allocator> inline int32_t ABitVector<Allocator>::FirstOneWordIndex() const
{
    int32_t wordIndex;
...
template<class Allocator> inline int32_t ABitVector<Allocator>::LastOneWordIndex() const
{
    int32_t wordIndex;

    // Scan to find the last word which has a one bit
    for (wordIndex = SizeInWords() - 1; (wordIndex >= 0) && (WordAt(wordIndex) == kZeroBits); wordIndex--)
        ;
...
inline BitIndex ABitVector<Allocator>::ClearLastOneIfThereIsOneInRange(BitIndex low, BitIndex high, bool& foundone)
{
    ...
    for (BitIndex wordIndex = highWordIndex; wordIndex >= lowWordIndex;) {
        if (WordAt(wordIndex) != kZeroBits) {
            int32_t rc
                = wordIndex * kBitWordSize + kBitWordSize - BitManipulator::TrailingZeroes(WordAt(wordIndex)) - 1;
...
template<class Allocator> inline bool ABitVector<Allocator>::IsZero() const
{
    uint32_t wordIndex, vectorWordSize;
...
template<class Allocator> inline bool ABitVector<Allocator>::IsNull() const
{
...
template<class Allocator> inline bool ABitVector<Allocator>::Intersects(const ABitVector<Allocator>& vector) const
{
...
template<class Allocator> template<class B2> inline bool ABitVector<Allocator>::Intersects(const B2& vector) const
{
...
template<class Allocator> inline bool ABitVector<Allocator>::operator<=(const ABitVector<Allocator>& vector) const
{
    uint32_t wordIndex, thisWordSize, vectorWordSize, smallerWordSize;
...
template<class Allocator>
inline ABitVector<Allocator>& ABitVector<Allocator>::operator-=(const ABitVector<Allocator>& v)
{
    Andc(v);
    return *this;
}
...
#ifndef USE_POPCNTB
template<class Allocator> inline uint32_t ABitVector<Allocator>::PopulationCount(uint32_t numBits) const
{
    uint32_t popCount = 0;
...
template<class Allocator> inline uint32_t ABitVector<Allocator>::PopulationCount(uint32_t numBits) const
{
        ...
        for (wordIndex = 0; wordIndex < streamWordSize; wordIndex += WORDS_IN_16_BIT_ACCUM) {
            ...
            for (BitIndex i2 = wordIndex; i2 < wordIndex + WORDS_IN_16_BIT_ACCUM; i2 += WORDS_IN_8_BIT_ACCUM) {
                ...
                for (BitIndex i1 = i2; i1 < i2 + WORDS_IN_8_BIT_ACCUM; i1++) {
                    c1 += __popcntb(
                        fBitWords[i1]); // count the number of 1's in each byte of the word at wordIndex index
                    c1a += __popcntb(fBitWords[streamWordSize
                        + i1]); // count the number of 1's in each byte of the word at wordIndex+streamWordSize index
                    c1b += __popcntb(fBitWords[streamWordSize * 2
                        + i1]); // count the number of 1's in each byte of the word at wordIndex+2*streamWordSize index
                    c1c += __popcntb(fBitWords[streamWordSize * 3
                        + i1]); // count the number of 1's in each byte of the word at wordIndex+3*streamWordSize index
                }
#ifdef BITVECTOR_64BIT
    ...
    streamWordSize
        = ((lastWord - wordIndex) - ((lastWord - wordIndex) % (WORDS_IN_8_BIT_ACCUM * 2))) / 2; // for 2 streams
    ...
    if (lastWord - wordIndex >= WORDS_IN_8_BIT_ACCUM * 2) {
        ...
        for (; wordIndex < wordIndex + streamWordSize; wordIndex += WORDS_IN_8_BIT_ACCUM) {
            ...
            for (BitIndex i1 = wordIndex; i1 < wordIndex + WORDS_IN_8_BIT_ACCUM; i1++) {
                ...
                c1a += __popcntb(fBitWords[streamWordSize
                    + i1]); // count the number of 1's in each byte of the word at wordIndex index
            }
#ifdef BITVECTOR_64BIT
            c2_1 += (c1 & 0x00FF00FF00FF00FFul);
            c2_2 += ((c1 >> 8) & 0x00FF00FF00FF00FFul);
    ...
    CS2Assert(checkCount == tempCount + tempCount2 + tempCount3 + tempCount4 + tempCount0,
        ("Wrong result by ABitVector<Allocator>::PopulationCount_pwr5"));
#endif
...
template<class Allocator>
inline uint32_t ABitVector<Allocator>::PopulationCount(const ABitVector<Allocator>& mask) const
{
...
template<class Allocator> inline void ABitVector<Allocator>::CopyToMemory(void* pMemory, BitIndex numBits) const
{
...
template<class Allocator> inline void ABitVector<Allocator>::CopyFromMemory(void* pMemory, BitIndex numBits)
{
    ShortWord* pBitWord = (ShortWord*)pMemory;
...
template<class Allocator>
inline bool ABitVector<Allocator>::And(
    const ABitVector<Allocator>& inputVector, ABitVector<Allocator>& outputVector) const
{
    uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
...
// Place the result in the output vector.
template<class Allocator>
inline bool ABitVector<Allocator>::Andc(
    const ABitVector<Allocator>& inputVector, ABitVector<Allocator>& outputVector) const
{
    uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
    BitWord inputWord, thisWord, newWord;
...
template<class Allocator>
inline bool ABitVector<Allocator>::Or(
    const ABitVector<Allocator>& inputVector, ABitVector<Allocator>& outputVector) const
{
    uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
    BitWord inputWord, thisWord, newWord;
...
template<class Allocator>
inline bool ABitVector<Allocator>::Xor(
    const ABitVector<Allocator>& inputVector, ABitVector<Allocator>& outputVector) const
{
    uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize, largerWordSize, outputWordSize;
    BitWord inputWord, thisWord, newWord;
