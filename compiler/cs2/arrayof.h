...
///
// ------------------------------------------------------------------------
template<class AElementType, class Allocator, size_t segmentBits = 8> struct BaseArrayOf : private Allocator {
public:
    ...
    template<class ostr> friend ostr& operator<<(ostr& out, const CS2_BASEARDECL& table)
    {
    ...
    public:
        ...
        DerivedElement(const Initializer& element)
            : fElement(element)
        { }
        ~DerivedElement() { }
...
CS2_ARTEMP inline size_t CS2_BASEARDECL::ElementsPerSegment() const { return size_t(1) << segmentBits; }

CS2_ARTEMP inline size_t CS2_BASEARDECL::SegmentSize() const { return ElementsPerSegment() * sizeof(AElementType); }

CS2_ARTEMP inline size_t CS2_BASEARDECL::SegmentMask() const { return ~((~size_t(0)) << segmentBits); }
...
CS2_ARTEMP inline typename CS2_BASEARDECL::DerivedElement* CS2_BASEARDECL::Segment(size_t elementIndex) const
{
    ...
    CS2Assert(segmentMapIndex < fNumberOfSegments, ("Index %lu does not exist", elementIndex));

    return fSegmentMap[segmentMapIndex];
}

CS2_ARTEMP inline typename CS2_BASEARDECL::DerivedElement* CS2_BASEARDECL::DerivedElementAt(size_t elementIndex) const
{
...
CS2_ARTEMP inline AElementType& CS2_BASEARDECL::operator[](size_t elementIndex) { return ElementAt(elementIndex); }

CS2_ARTEMP inline size_t CS2_BASEARDECL::NumberOfElements() const { return fNumberOfSegments * ElementsPerSegment(); }

CS2_ARTEMP inline CS2_BASEARDECL::BaseArrayOf(const Allocator& a)
    : Allocator(a)
{
    ...
    CS2Assert(ElementsPerSegment() < 0x10000, ("Too many elements per segment: %lu\n", ElementsPerSegment()));
...
CS2_ARTEMP inline CS2_BASEARDECL& CS2_BASEARDECL::operator=(const CS2_BASEARDECL& inputArray)
{
    ...
    {
        firstSegment
            = (fNumberOfSegments < inputArray.fNumberOfSegments) ? fNumberOfSegments : inputArray.fNumberOfSegments;
        ...
        for (segmentIndex = 0; segmentIndex < firstSegment; ++segmentIndex) {
            {
                ...
                {
                    for (elementIndex = 0; elementIndex < ElementsPerSegment(); ++elementIndex) {
    ...
    for (segmentIndex = firstSegment; segmentIndex < fNumberOfSegments; ++segmentIndex) {
        fSegmentMap[segmentIndex] = (DerivedElement*)Allocator::allocate(SegmentSize());
    }
...
CS2_ARTEMP inline void CS2_BASEARDECL::GrowTo(size_t newSize)
{
    ...
    if (segmentMapIndex < fNumberOfSegments)
        return;
    ...
    if (segmentMapIndex >= fMaxSegments) {
        if (fSegmentMap == NULL) {
            ...
            void* newSegmentMapAllocation = Allocator::reallocate(
                maxSegments * sizeof(DerivedElement*), fSegmentMap, fMaxSegments * sizeof(DerivedElement*));
    ...
    for (newSegmentIndex = fNumberOfSegments; newSegmentIndex < segmentMapIndex + 1; ++newSegmentIndex) {
    ...
    CS2Assert(segmentMapIndex + 1 == fNumberOfSegments,
        ("Number of current segments %d is smaller than expected size of %d.", (int)fNumberOfSegments,
...
CS2_ARTEMP inline void CS2_BASEARDECL::ShrinkTo(size_t newSize)
{
    ...
    if (newSize > 0)
        newSize = ElementsPerSegment() * ((newSize + ElementsPerSegment() - 1) / ElementsPerSegment());
    ...
    if (firstDeadSegment >= fNumberOfSegments)
        return;

    // Destroy all dead segments
    for (segmentIndex = firstDeadSegment; segmentIndex < fNumberOfSegments; ++segmentIndex) {
        Allocator::deallocate(fSegmentMap[segmentIndex], SegmentSize());
    }
...
public:
    ArrayOf(size_t ignore, const Allocator& a = Allocator(), const Initializer i = Initializer())
        : CS2_BASEARDECL(a)
        , fInitializer(i)
        , fNumInitialized(0)
    { }

    ArrayOf(const Allocator& a = Allocator(), const Initializer i = Initializer())
        : CS2_BASEARDECL(a)
        , fInitializer(i)
        , fNumInitialized(0)
    { }
    ...
    ArrayOf<AElementType, Allocator, segmentBits>& operator=(
        const ArrayOf<AElementType, Allocator, segmentBits>& inputArray)
    {
        MakeEmpty();
    ...
    bool operator==(const ArrayOf<AElementType, Allocator, segmentBits>& inputArray) const
    {
        if (NumberOfElements() != inputArray.NumberOfElements())
            return false;
    ...
    const AElementType& operator[](size_t elementIndex) const { return CS2_BASEARDECL::ElementAt(elementIndex); }
    ...
    void Sort(size_t from, size_t to)
    {
        ...
        if (pivot < CS2_BASEARDECL::ElementAt(low))
            low -= 1;
    ...
    void Sort() { Sort(0, NumberOfElements()); }

    template<class ostr> friend ostr& operator<<(ostr& out, const CS2_ARDECL& table)
    {
        Cursor c(table);
    ...
    class Cursor {
    public:
        Cursor(const ArrayOf<AElementType, Allocator, segmentBits, Initializer>& a)
            : fArray(a)
            , fSegmentIndex(0)
            , fElementIndex(0)
            , fSegment(NULL)
        ...
        bool Valid() const { return fElementIndex < fNumElements; }

        bool IsLast() const { return (fSegmentIndex == fNumSegments - 1 && fElementIndex == fNumElements - 1); }
        operator uint32_t() const { return (fSegmentIndex * fArray.ElementsPerSegment()) + fElementIndex; }

        AElementType& operator*() const { return (fSegment + fElementIndex)->Element(); }
    ...
    public:
        BackwardsCursor(const ArrayOf<AElementType, Allocator, segmentBits, Initializer>& a)
            : fArray(a)
            , fSegmentIndex(0)
            , fElementIndex(0)
            , fSegment(NULL)
        ...
        bool Valid() const { return fElementIndex > 0; }

        bool IsLast() const { return (fSegmentIndex == 0 && fElementIndex == 1); }
        operator uint32_t() const { return (fSegmentIndex * fArray.ElementsPerSegment()) + fElementIndex - 1; }

        AElementType& operator*() const { return (fSegment + fElementIndex - 1)->Element(); }
    ...
    public:
        ...
        {
            return this == &other || (fArray == other.fArray && fCursor == other.fCursor);
        }
...
public:
    StaticArrayOf(size_t size, const Allocator& a = Allocator(), const Initializer i = Initializer())
