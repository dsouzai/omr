...
#define CS2Assert(cond, ss)                                                              \
    if (!(cond)) {                                                                       \
        printf("COMPILER ASSERTION|%s|%s\n%s:%d ", "CS2", __func__, __FILE__, __LINE__); \
        printf ss;                                                                       \
        abort();                                                                         \
    }
#else
...
inline void Memory() { CS2Assert(0, ("SystemResourceError::Memory\n")); }
} // namespace SystemResourceError

template<class c> void Swap(c& c1, c& c2)
{
    c ctmp(c2);
    c2 = c1;
    c1 = ctmp;
