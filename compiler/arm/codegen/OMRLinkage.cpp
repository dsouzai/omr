    ...
    TR::Register*& vftReg, TR_LinkageConventions conventions, bool isVirtualOrJNI)
{
    ...
    for (i = from; (isHelper && i > to) || (!isHelper && i < to); i += step) {
        ...
        case TR::Float:
            ...
            if (isSpecialArg) {
                ...
                if (numIntegerArgs < numIntArgRegs) {
                    ...
                    fflush(stdout);
#endif
                    tempMR = self()->getOutgoingArgumentMemRef(
                        totalSize, argSize, reg, TR::InstOpCode::str, pushToMemory[memArg++]);
                }
                numIntegerArgs++;
                argSize += 4;
            }
                ...
                if (numIntegerArgs < numIntArgRegs) {
                    ...
                    if (numIntegerArgs == 0 && resType.isAddress()) {
                        dependencies->addPostCondition(
                            self()->cg()->allocateCollectedReferenceRegister(), TR::RealRegister::gr0);
                    } else {
                        dependencies->addPostCondition(
                            self()->cg()->allocateRegister(), properties.getIntegerArgumentRegister(numIntegerArgs));
                    }
        ...
        case TR::Float:
            ...
            if (numFloatArgs < numFloatArgRegs)
               ...
               {
               tempMR = getOutgoingArgumentMemRef(totalSize - argSize, reg, TR::InstOpCode::stfs, pushToMemory[memArg++]);
               }
            numFloatArgs++;
#endif
            break;
        case TR::Double:
            ...
            if (numFloatArgs < numFloatArgRegs)
               ...
               {
               tempMR = getOutgoingArgumentMemRef(totalSize - argSize, reg, TR::InstOpCode::stfd, pushToMemory[memArg++]);
               }
            numFloatArgs++;
#endif
            break;
    ...
    */
    ...
    for (i = 0; i < 8 /*properties.getNumFloatArgRegs()*/; i++) {
        TR::addDependency(
            dependencies, NULL, (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 + i), TR_FPR, self()->cg());
    }
#endif
      ...
      for (i = memFrom; (isJNI &&  i >= memTo) || (!isJNI && i < memTo); i += memStep)
...
#endif
        {
            ...
            generateMemSrc1Instruction(
                self()->cg(), pushToMemory[i].opCode, callNode, pushToMemory[i].argMemory, pushToMemory[i].argRegister);
#ifdef DEBUG_ARM_LINKAGE
            printf("done\n");
            fflush(stdout);
#endif
