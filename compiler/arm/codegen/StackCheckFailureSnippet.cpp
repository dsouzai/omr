...
uint8_t* TR::ARMStackCheckFailureSnippet::emitSnippetBody()
{
    ...
    bool saveLR = (cg()->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() && !machine->getLinkRegisterKilled())
        ? true
        : false;
    ...
    *(int32_t*)buffer
        = 0xEA000000 | encodeBranchDistance((uintptr_t)buffer, (uintptr_t)getReStartLabel()->getCodeLocation());
...
uint32_t TR::ARMStackCheckFailureSnippet::getLength(int32_t estimatedSnippetStart)
{
    ...
    if (cg()->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() && !cg()->machine()->getLinkRegisterKilled()) {
        length += 8; // str, ldr
    }
