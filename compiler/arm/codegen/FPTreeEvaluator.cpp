...
static TR::Register *singlePrecisionEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic opCode, TR::CodeGenerator *cg)
{
    ...
    if (node->getNumChildren() == 2) {
        ...
        generateTrg1Src2Instruction(cg, opCode, node, floatTrgReg, moveToFloatRegister(node, src1Reg, cg),
            moveToFloatRegister(node, src2Reg, cg));
    } else // 1 child
    {
...
static TR::Register *doublePrecisionEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic opCode, TR::CodeGenerator *cg)
{
    ...
    if (node->getNumChildren() == 2) {
        ...
        generateTrg1Src2Instruction(cg, opCode, node, doubleTrgReg, moveToDoubleRegister(node, src1Reg, cg),
            moveToDoubleRegister(node, src2Reg, cg));
    } else // 1 child
    {
...
static TR::Register *callLong2DoubleHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Double)->getMethodAddress(), dependencies,
...
static TR::Register *callLong2FloatHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Float)->getMethodAddress(), dependencies,
...
static TR::Register *callDouble2LongHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(11, 11, cg->trMemory());
    int i;
    TR::Register *highReg = cg->allocateRegister();
    TR::Register *lowReg = cg->allocateRegister();
    TR::Register *trgReg = cg->allocateRegisterPair(lowReg, highReg);
    ...
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMdouble2Long)->getMethodAddress(), dependencies,
...
static TR::Register *callFloat2LongHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMfloat2Long)->getMethodAddress(), dependencies,
...
static TR::Register *callDoubleRemainderHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    dependencies->stopAddingConditions();
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMdoubleRemainder)->getMethodAddress(), dependencies,
...
static TR::Register *callFloatRemainderHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMfloatRemainder)->getMethodAddress(), dependencies,
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        if (noFPRA) {
            ...
            if (tempMR->getIndexRegister() && tempMR->getBaseRegister()) {
                ...
                generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, tempReg, tempMR->getBaseRegister(),
...
TR::Register *OMR::ARM::TreeEvaluator::lbits2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        if (noFPRA) {
            ...
            highMem = new (cg->trHeapMemory())
                TR::MemoryReference(*tempMR, (cg->comp()->target().cpu.isBigEndian()) ? 0 : 4, 4, cg);
            generateTrg1MemInstruction(cg, TR::InstOpCode::ldr, node, highReg, highMem);
            lowMem = new (cg->trHeapMemory())
                TR::MemoryReference(*tempMR, (cg->comp()->target().cpu.isBigEndian()) ? 4 : 0, 4, cg);
            ...
            if (highMem->getIndexRegister() && highMem->getBaseRegister()) {
                TR::Register *tempReg = cg->allocateRegister();
                generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, tempReg, highMem->getBaseRegister(),
                    highMem->getIndexRegister());
                highMem->decNodeReferenceCounts();
...
// #define STAYINFP
    ...
    if (node->getSymbolReference()->getSymbol()->isSyncVolatile() && cg->comp()->target().isSMP()
        && cg->comp()->target().cpu.id() != TR_DefaultARMProcessor) {
...
TR::Register *OMR::ARM::TreeEvaluator::dreturnEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (returnRegister->getKind() == TR_GPR) {
        TR::addDependency(deps, returnRegister->getLowOrder(), cg->getProperties().getLongLowReturnRegister(), TR_GPR,
            cg);
        TR::addDependency(deps, returnRegister->getHighOrder(), cg->getProperties().getLongHighReturnRegister(), TR_GPR,
            cg);
    } else if (returnRegister->getKind() == TR_FPR) {
...
static TR::Register *generateFusedMultiplyAdd(TR::Node *addNode, TR::InstOpCode::Mnemonic OpCode, TR::CodeGenerator *cg)
{
    ...
    if (addNode->getDataType() == TR::Float) {
        if (noFPRA) {
            TR::Register *floatTrgReg = cg->allocateSinglePrecisionRegister();
            generateTrg1Src2Instruction(cg, OpCode, addNode, floatTrgReg,
                moveToFloatRegister(addNode, source1Register, cg), moveToFloatRegister(addNode, source2Register, cg));
        ...
        if (noFPRA) {
            TR::Register *doubleTrgReg = cg->allocateRegister(TR_FPR);
            generateTrg1Src2Instruction(cg, OpCode, addNode, doubleTrgReg,
                moveToDoubleRegister(addNode, source1Register, cg), moveToDoubleRegister(addNode, source2Register, cg));
            generateTrg2Src1Instruction(cg, TR::InstOpCode::fmrrd, addNode, trgRegister->getLowOrder(),
                trgRegister->getHighOrder(), doubleTrgReg);
            cg->stopUsingRegister(doubleTrgReg);
        } else {
            generateTrg1Src2Instruction(cg, OpCode, addNode, trgRegister, source1Register, source2Register);
        }
...
TR::Register *OMR::ARM::TreeEvaluator::dsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    } else if (isFPStrictMul(node->getSecondChild(), comp) && (node->getFirstChild()->getReferenceCount() == 1)
...
TR::Register *OMR::ARM::TreeEvaluator::fnegEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
    ...
    } else if (isFPStrictMul(firstChild, comp) && firstChild->getReferenceCount() < 2 && !firstChild->getRegister()
        ...
        if (noFPRA) {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::fnmuls, node, floatTrgReg,
                moveToFloatRegister(node, src1Register, cg), moveToFloatRegister(node, src2Register, cg));
...
TR::Register *OMR::ARM::TreeEvaluator::dnegEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
        ...
        TR::Register *src2Register = cg->evaluate(firstChild->getSecondChild());
        if (noFPRA) {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::fnmuld, node, doubleTrgReg,
                moveToDoubleRegister(node, src1Register, cg), moveToDoubleRegister(node, src2Register, cg));
...
TR::Register *OMR::ARM::TreeEvaluator::i2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::InstOpCode::Mnemonic opcode
        = (node->getOpCodeValue() == TR::iu2f) ? TR::InstOpCode::fuitos : TR::InstOpCode::fsitos;
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        ...
        && (firstChild->getNumChildren() > 0) && (firstChild->getFirstChild()->getNumChildren() == 1)
...
TR::Register *OMR::ARM::TreeEvaluator::i2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::InstOpCode::Mnemonic opcode = (node->getOpCodeValue() != TR::iu2d && node->getOpCodeValue() != TR::su2d)
        ? TR::InstOpCode::fsitod
        : TR::InstOpCode::fuitod; // D[t], S[s]
    ...
    TR::Register *doubleTrgReg = cg->allocateRegister(TR_FPR);
    generateTrg1Src1Instruction(cg, opcode, node, doubleTrgReg,
        tempReg); // The conversion.. source is float, result is double
...
TR::Register *OMR::ARM::TreeEvaluator::f2dEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        ...
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcvtds, node, doubleTrgReg,
            moveToFloatRegister(node, src1Reg, cg));
        cg->decReferenceCount(firstChild);
    }
...
TR::Register *OMR::ARM::TreeEvaluator::f2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::Register *trgReg = NULL;
    TR::InstOpCode::Mnemonic opcode
        = (node->getOpCodeValue() == TR::f2i) ? TR::InstOpCode::ftosizs : TR::InstOpCode::ftouizs;
    TR::Register *floatTrgReg = cg->allocateSinglePrecisionRegister();
...
TR::Register *OMR::ARM::TreeEvaluator::d2iEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::Register *trgReg = NULL;
    TR::InstOpCode::Mnemonic opcode
        = (node->getOpCodeValue() == TR::d2i) ? TR::InstOpCode::ftosizd : TR::InstOpCode::ftouizd;
    TR::Register *floatTrgReg = cg->allocateSinglePrecisionRegister();
...
TR::Register *OMR::ARM::TreeEvaluator::d2fEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        ...
        src1Reg = cg->evaluate(firstChild);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcvtsd, node, floatTrgReg,
            moveToDoubleRegister(node, src1Reg, cg));
        cg->decReferenceCount(firstChild);
    }
...
static void ifFloatEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic opCode, TR_ARMConditionCode cond,
    TR::CodeGenerator *cg, bool isDouble, bool unordered)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    TR::Register *src1Reg = cg->evaluate(firstChild);
    ...
    if (noFPRA) {
        if (isDouble) {
            generateSrc2Instruction(cg, opCode, node, moveToDoubleRegister(node, src1Reg, cg),
                moveToDoubleRegister(node, src2Reg, cg));
        } else {
            generateSrc2Instruction(cg, opCode, node, moveToFloatRegister(node, src1Reg, cg),
                moveToFloatRegister(node, src2Reg, cg));
        }
    } else {
    ...
    if (deps) {
        ...
        if (unordered)
            generateConditionalBranchInstruction(cg, node, ARMConditionCodeVS, label, deps);
    } else {
        generateConditionalBranchInstruction(cg, node, cond, label);
        if (unordered)
            generateConditionalBranchInstruction(cg, node, ARMConditionCodeVS, label);
    }
}

static TR::Register *setboolFloatEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic opCode, TR_ARMConditionCode cond,
    TR::CodeGenerator *cg, bool isDouble, bool unordered)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeEQ, cg, isDouble,
        unordered);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeNE, cg, isDouble,
        false);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps,
        unordered ? ARMConditionCodeLT : ARMConditionCodeCC, cg, isDouble, false);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps,
        unordered ? ARMConditionCodeCS : ARMConditionCodeGE, cg, isDouble, false);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps,
        unordered ? ARMConditionCodeHI : ARMConditionCodeGT, cg, isDouble, false);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    ifFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps,
        unordered ? ARMConditionCodeLE : ARMConditionCodeLS, cg, isDouble, false);
    return NULL;
}
...
TR::Register *OMR::ARM::TreeEvaluator::dcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    return setboolFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeEQ, cg,
        isDouble, unordered);
}
...
TR::Register *OMR::ARM::TreeEvaluator::dcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    return setboolFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeNE, cg,
        isDouble, false);
}
...
TR::Register *OMR::ARM::TreeEvaluator::dcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    return setboolFloatEvaluator(node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps,
        unordered ? ARMConditionCodeLT : ARMConditionCodeCC, cg, isDouble, false);
}
...
TR::Register *OMR::ARM::TreeEvaluator::dcmplEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (noFPRA) {
        if (isDouble) {
            generateSrc2Instruction(cg, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, node,
                moveToDoubleRegister(node, src1Reg, cg), moveToDoubleRegister(node, src2Reg, cg));
        } else {
            generateSrc2Instruction(cg, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, node,
                moveToFloatRegister(node, src1Reg, cg), moveToFloatRegister(node, src2Reg, cg));
        }
    } else {
...
static TR::Register *generateFloatMaxMin(TR::Node *node, TR::CodeGenerator *cg, bool max)
{
    ...
    if (noFPRA && reg->getKind() == TR_GPR) {
        ...
        if (cg->canClobberNodesRegister(firstChild)) {
            ...
            if (isFloat) {
                ...
                generateTrg1Src1Instruction(cg, TR::InstOpCode::fcpys, node, trgFloatReg,
                    moveToFloatRegister(node, reg, cg));
            } else {
                trgFloatReg = cg->allocateRegister(TR_FPR);
                generateTrg1Src1Instruction(cg, TR::InstOpCode::fcpyd, node, trgFloatReg,
                    moveToDoubleRegister(node, reg, cg));
            }
        }
        if (max) {
    ...
    if (noFPRA && childReg->getKind() == TR_GPR) {
        ...
        if (!max) {
            childReg = (isFloat) ? moveFromFloatRegister(node, childFloatReg, cg)
                                 : moveFromDoubleRegister(node, childFloatReg, cg);
        }
    }
    ...
    generateSrc2Instruction(cg, (isFloat) ? TR::InstOpCode::fcmps : TR::InstOpCode::fcmpd, node, trgFloatReg,
        trgFloatReg);
    generateInstruction(cg, TR::InstOpCode::fmstat, node);
    generateConditionalBranchInstruction(cg, node, ARMConditionCodeVS, doneLabel, deps);

    // Check for NaN
    cursor = generateSrc2Instruction(cg, (isFloat) ? TR::InstOpCode::fcmps : TR::InstOpCode::fcmpd, node, childFloatReg,
        childFloatReg);
    cursor = generateInstruction(cg, TR::InstOpCode::fmstat, node, cursor);
    cursor = generateTrg1Src1Instruction(cg, (isFloat) ? TR::InstOpCode::fcpys : TR::InstOpCode::fcpyd, node,
        trgFloatReg, childFloatReg, cursor);
    cursor->setConditionCode(ARMConditionCodeVS);
    ...
    if (isFloat) {
        cursor = generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node, max ? trgReg : childReg, 0x80, 24,
            cursor); // 0x80000000
    } else {
        cursor = generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node,
            max ? trgReg->getHighOrder() : childReg->getHighOrder(), 0x80, 24, cursor); // 0x80000000
        cursor = generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node,
            max ? trgReg->getLowOrder() : childReg->getLowOrder(), 0, 0, cursor);
     ...
     *
     */
    cursor = generateSrc1ImmInstruction(cg, (isFloat) ? TR::InstOpCode::fcmpzs : TR::InstOpCode::fcmpzd, node,
        max ? childFloatReg : trgFloatReg, 0, 0, cursor); // Dummy 0
    cursor->setConditionCode(ARMConditionCodeEQ);
    cursor = generateInstruction(cg, TR::InstOpCode::fmstat, node, cursor);
    cursor->setConditionCode(ARMConditionCodeEQ);
    cursor = generateTrg1Src1Instruction(cg, (isFloat) ? TR::InstOpCode::fcpys : TR::InstOpCode::fcpyd, node,
    ...
    if (noFPRA && reg->getKind() == TR_GPR) {
        if (cg->canClobberNodesRegister(firstChild)) {
            if (isFloat) {
                ...
                cursor = generateTrg2Src1Instruction(cg, TR::InstOpCode::fmrrd, node, reg->getLowOrder(),
                    reg->getHighOrder(), trgFloatReg, cursor);
            }
            trgReg = reg;
        } else {
            trgReg = (isFloat) ? moveFromFloatRegister(node, trgFloatReg, cg)
                               : moveFromDoubleRegister(node, trgFloatReg, cg);
...
TR::Register *OMR::ARM::TreeEvaluator::fconstEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dconstEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::floadi
TR::Register *OMR::ARM::TreeEvaluator::floadEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::dloadi
TR::Register *OMR::ARM::TreeEvaluator::dloadEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::ifstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::idstoreEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::freturnEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dreturnEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::faddEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::daddEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dsubEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fsubEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fmulEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dmulEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fdivEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::ddivEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fremEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dremEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fabsEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dabsEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fnegEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dnegEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::i2fEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::i2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::l2fEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::l2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }
...
TR::Register *OMR::ARM::TreeEvaluator::f2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::f2iEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::d2iEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles f2c
TR::Register *OMR::ARM::TreeEvaluator::d2cEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles f2s
TR::Register *OMR::ARM::TreeEvaluator::d2sEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles f2b
TR::Register *OMR::ARM::TreeEvaluator::d2bEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::f2lEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::d2lEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::d2fEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::b2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles s2f
TR::Register *OMR::ARM::TreeEvaluator::s2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles c2f
TR::Register *OMR::ARM::TreeEvaluator::su2dEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpeq
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpeq
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpeq
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpeq
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpgt
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmple
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpequ
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpequEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::iffcmpltu
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpltuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }
...
TR::Register *OMR::ARM::TreeEvaluator::ifdcmpgtuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }
...
TR::Register *OMR::ARM::TreeEvaluator::dcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::fcmpne
TR::Register *OMR::ARM::TreeEvaluator::dcmpneEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::fcmplt
TR::Register *OMR::ARM::TreeEvaluator::dcmpltEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

// also handles TR::fcmpge
TR::Register *OMR::ARM::TreeEvaluator::dcmpgeEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpgtEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpleEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpequEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpltuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpgeuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpgtuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpleuEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmplEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::dcmpgEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fRegStoreEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fmaxEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }

TR::Register *OMR::ARM::TreeEvaluator::fminEvaluator(TR::Node *node, TR::CodeGenerator *cg) { return NULL; }
