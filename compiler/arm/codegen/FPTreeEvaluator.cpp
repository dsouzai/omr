        ...
        if (noFPRA) {
            ...
            if (tempMR->getIndexRegister() && tempMR->getBaseRegister()) {
                ...
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::add, node, tempReg, tempMR->getBaseRegister(), tempMR->getIndexRegister());
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        if (noFPRA) {
            ...
            if (highMem->getIndexRegister() && highMem->getBaseRegister()) {
                TR::Register* tempReg = cg->allocateRegister();
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::add, node, tempReg, highMem->getBaseRegister(), highMem->getIndexRegister());
                highMem->decNodeReferenceCounts();
...
TR::Register* OMR::ARM::TreeEvaluator::dreturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (returnRegister->getKind() == TR_GPR) {
        TR::addDependency(
            deps, returnRegister->getLowOrder(), cg->getProperties().getLongLowReturnRegister(), TR_GPR, cg);
        TR::addDependency(
            deps, returnRegister->getHighOrder(), cg->getProperties().getLongHighReturnRegister(), TR_GPR, cg);
    } else if (returnRegister->getKind() == TR_FPR) {
...
TR::Register* OMR::ARM::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* doubleTrgReg = cg->allocateRegister(TR_FPR);
    generateTrg1Src1Instruction(
        cg, opcode, node, doubleTrgReg, tempReg); // The conversion.. source is float, result is double
...
TR::Register* OMR::ARM::TreeEvaluator::f2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        ...
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::fcvtds, node, doubleTrgReg, moveToFloatRegister(node, src1Reg, cg));
        cg->decReferenceCount(firstChild);
    }
...
TR::Register* OMR::ARM::TreeEvaluator::d2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        ...
        src1Reg = cg->evaluate(firstChild);
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::fcvtsd, node, floatTrgReg, moveToDoubleRegister(node, src1Reg, cg));
        cg->decReferenceCount(firstChild);
    }
    ...
    TR::CodeGenerator* cg, bool isDouble, bool unordered)
{
    ...
    if (noFPRA) {
        if (isDouble) {
            generateSrc2Instruction(
                cg, opCode, node, moveToDoubleRegister(node, src1Reg, cg), moveToDoubleRegister(node, src2Reg, cg));
        } else {
            generateSrc2Instruction(
                cg, opCode, node, moveToFloatRegister(node, src1Reg, cg), moveToFloatRegister(node, src2Reg, cg));
        }
    } else {
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    ifFloatEvaluator(
        node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeEQ, cg, isDouble, unordered);
    return NULL;
}
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    ifFloatEvaluator(
        node, isDouble ? TR::InstOpCode::fcmpd : TR::InstOpCode::fcmps, ARMConditionCodeNE, cg, isDouble, false);
    return NULL;
}
...
static TR::Register* generateFloatMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
    ...
    if (noFPRA && reg->getKind() == TR_GPR) {
        ...
        if (cg->canClobberNodesRegister(firstChild)) {
            ...
            if (isFloat) {
                ...
                generateTrg1Src1Instruction(
                    cg, TR::InstOpCode::fcpys, node, trgFloatReg, moveToFloatRegister(node, reg, cg));
            } else {
                trgFloatReg = cg->allocateRegister(TR_FPR);
                generateTrg1Src1Instruction(
                    cg, TR::InstOpCode::fcpyd, node, trgFloatReg, moveToDoubleRegister(node, reg, cg));
            }
        }
        if (max) {
    ...
    generateSrc2Instruction(
        cg, (isFloat) ? TR::InstOpCode::fcmps : TR::InstOpCode::fcmpd, node, trgFloatReg, trgFloatReg);
    generateInstruction(cg, TR::InstOpCode::fmstat, node);
    generateConditionalBranchInstruction(cg, node, ARMConditionCodeVS, doneLabel, deps);

    // Check for NaN
    cursor = generateSrc2Instruction(
        cg, (isFloat) ? TR::InstOpCode::fcmps : TR::InstOpCode::fcmpd, node, childFloatReg, childFloatReg);
    cursor = generateInstruction(cg, TR::InstOpCode::fmstat, node, cursor);
    cursor = generateTrg1Src1Instruction(
        cg, (isFloat) ? TR::InstOpCode::fcpys : TR::InstOpCode::fcpyd, node, trgFloatReg, childFloatReg, cursor);
    cursor->setConditionCode(ARMConditionCodeVS);
    ...
    if (isFloat) {
        cursor = generateSrc1ImmInstruction(
            cg, TR::InstOpCode::cmp, node, max ? trgReg : childReg, 0x80, 24, cursor); // 0x80000000
    } else {
        ...
        cursor = generateSrc1ImmInstruction(
            cg, TR::InstOpCode::cmp, node, max ? trgReg->getLowOrder() : childReg->getLowOrder(), 0, 0, cursor);
    ...
    cursor->setConditionCode(ARMConditionCodeEQ);
    cursor = generateSrc2Instruction(
        cg, (isFloat) ? TR::InstOpCode::fcmps : TR::InstOpCode::fcmpd, node, childFloatReg, trgFloatReg, cursor);
    ...
    if (noFPRA && reg->getKind() == TR_GPR) {
        if (cg->canClobberNodesRegister(firstChild)) {
            if (isFloat) {
                ...
                cursor = generateTrg2Src1Instruction(
                    cg, TR::InstOpCode::fmrrd, node, reg->getLowOrder(), reg->getHighOrder(), trgFloatReg, cursor);
            }
            trgReg = reg;
        } else {
