...
TR::Register* OMR::ARM::TreeEvaluator::gotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getNumChildren() > 0) {
        ...
        generateLabelInstruction(
            cg, TR::InstOpCode::b, node, gotoLabel, generateRegisterDependencyConditions(cg, child, 0));
...
static TR::Instruction* compareIntsForOrder(
    TR_ARMConditionCode branchType, TR::Node* node, TR::CodeGenerator* cg, TR::SymbolReference* sr = NULL)
{
    ...
    if (firstChild->getOpCodeValue() == TR:: instanceof && firstChild->getRegister() == NULL
            && node->getReferenceCount() <= 1 && (value == 0 || value == 1)) {
        TR_ASSERT(
            node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ificmpne, "Unexpected opcode");

        TR_ASSERT(secondChild->getOpCode().isLoadConst(), "Unexpected second child.\n");
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        negated = cmpValue < 0
            && cmpValue != 0x80000000; // the negative of the maximum negative int is itself so can not use cmn here
    ...
    if (!foundConst)
        generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, src1Reg, cg->evaluate(secondChild));
    ...
    if (node->getNumChildren() == 3) {
        ...
        TR_ASSERT(
            thirdChild->getOpCodeValue() == TR::GlRegDeps, "The third child of a compare must be a TR::GlRegDeps");
    ...
    if (!sr) {
        ...
        result = generateImmSymInstruction(
            cg, TR::InstOpCode::bl, node, (uintptr_t)sr->getMethodAddress(), deps, sr, NULL, NULL, branchType);
...
TR::Instruction* OMR::ARM::TreeEvaluator::compareIntsForEquality(
    TR_ARMConditionCode branchType, TR::Node* node, TR::CodeGenerator* cg, TR::SymbolReference* sr)
{
...
TR::Register* OMR::ARM::TreeEvaluator::iflcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    cursor = generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, src1Reg->getHighOrder(), src2Reg->getHighOrder());
    cursor = generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, src1Reg->getLowOrder(), src2Reg->getLowOrder());
...
static TR::Register* compareLongAndSetOrderedBoolean(
    TR_ARMConditionCode branchOp, TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* src1Reg = cg->evaluate(firstChild);
    ...
    if (true) {
        TR::Register* src2Reg = cg->evaluate(secondChild);
        TR::ARMControlFlowInstruction* cfop = generateControlFlowInstruction(cg, TR::InstOpCode::setblong, node, deps);
...
static TR::Register* compareLongsForOrder(TR_ARMConditionCode branchOp, TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (disableOOLForLongCompares) {
        TR::ARMControlFlowInstruction* cfop = generateControlFlowInstruction(cg, TR::InstOpCode::iflong, node, deps);
        cfop->addSourceRegister(src1Reg->getHighOrder());
        cfop->addSourceRegister(src1Reg->getLowOrder());
        cfop->addSourceRegister(src2Reg->getHighOrder());
        cfop->addSourceRegister(src2Reg->getLowOrder());
        ...
        TR::Instruction* cursor
            = generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, src1Reg->getHighOrder(), src2Reg->getHighOrder());
        cursor = generateConditionalBranchInstruction(
            cg, node, isGT ? ARMConditionCodeLE : ARMConditionCodeGE, startOOLLabel, deps, cursor);
        if (debugObj)
        ...
        // Hot path
        cursor
            = generateTrg1ImmInstruction(cg, TR::InstOpCode::mov, node, tempReg, 1, 0, cursor); // Setting tempReg to 1

        // Start OOL
        // The compare result has to be live until the end of the last instruction in the mainline, so put it as a
        // dependency
        TR::RegisterDependencyConditions* tempDeps
            = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(1, 1, cg->trMemory());
        ...
        TR_ASSERT(!cursorOOL->getLiveLocals() && !cursorOOL->getLiveMonitors(),
        ...
        cursorOOL = generateTrg1ImmInstruction(
            cg, TR::InstOpCode::mov, node, tempReg, 0, 0, cursorOOL); // Setting tempReg to 0
        cursorOOL = generateConditionalBranchInstruction(
            cg, node, isGT ? ARMConditionCodeLT : ARMConditionCodeGT, returnLabel, cursorOOL);
        cursorOOL = generateSrc2Instruction(
            cg, TR::InstOpCode::cmp, node, src1Reg->getLowOrder(), src2Reg->getLowOrder(), cursorOOL);
        cursorOOL = generateTrg1ImmInstruction(
            cg, TR::InstOpCode::mov, node, tempReg, 1, 0, cursorOOL); // Setting tempReg to 1
        cursorOOL->setConditionCode(isGT ? ARMConditionCodeHI : ARMConditionCodeLS);
        cursorOOL = generateLabelInstruction(
            cg, TR::InstOpCode::label, node, returnLabel, oolDeps, cursorOOL); // Need deps 3/21 change
        cursorOOL = generateLabelInstruction(cg, TR::InstOpCode::b, node, doneOOLLabel, cursorOOL);
        if (debugObj)
        ...
        cursor = generateSrc1ImmInstruction(cg, TR::InstOpCode::tst, node, tempReg, 1, 0, cursor);
...
TR::Register* OMR::ARM::TreeEvaluator::iflcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sub_r, node, tempReg, src1Reg->getLowOrder(), src2Reg->getLowOrder());
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::sbc_r, node, tempReg, src1Reg->getHighOrder(), src2Reg->getHighOrder());
...
static TR::Register* icmpHelper(TR::Node* node, TR_ARMConditionCode cond, TR::CodeGenerator* cg)
{
    ...
    generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, src1Reg, src2Reg);
    movInstr = generateTrg1ImmInstruction(cg, TR::InstOpCode::mov, node, trgReg, 1, 0);
...
TR::Register* OMR::ARM::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // For now we just generate pessimistic code.
    TR::Register* src2Reg = cg->evaluate(secondChild);
    TR::ARMControlFlowInstruction* cfop = generateControlFlowInstruction(cg, TR::InstOpCode::lcmp, node, deps);
    cfop->addTargetRegister(trgReg);
    cfop->addSourceRegister(src1Reg->getLowOrder());
    cfop->addSourceRegister(src1Reg->getHighOrder());
    cfop->addSourceRegister(src2Reg->getLowOrder());
...
// Note: When the checks and/or lookupScheme implementations are changed with regards to number of used registers (or
// dependency), the matching GlDepRegs free checks in OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..)
// must be updated to synch.
static void switchDispatch(TR::CodeGenerator* cg, TR::Node* node)
{
...
// Note: When the checks and/or lookupScheme implementations are changed with regards to number of used registers (or
// dependency), the matching GlDepRegs free checks in OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..)
// must be updated to synch. Called by switchDispatch().
static void lookupScheme1(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
    ...
    for (int i = 2; i < node->getNumChildren(); i++) {
        ...
        generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node, selectorReg, base, rotate);
...
static void lookupScheme2(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
    ...
    for (int i = 2; i < numChildren; i++) {
        TR::Node* caseNode = node->getChild(i);
        TR::LabelSymbol* caseTargetLabel = caseNode->getBranchDestination()->getNode()->getLabel();
        generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, selectorReg, caseConstReg);
        if (unbalanced) {
            TR::RegisterDependencyConditions* caseDeps = deps;
            if (caseNode->getNumChildren() > 0) {
        ...
        if (i < numChildren - 1) {
            ...
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, caseConstReg, caseConstReg, base, rotate);
        }
    }
...
static void lookupScheme3(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
    ...
    int32_t* dataTable = (int32_t*)cg->allocateCodeMemory(
        numberOfEntries * sizeof(int32_t), cg->getCurrentEvaluationBlock()->isCold());
    ...
    for (int32_t i = 2; i < total; i++) {
        ...
        generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, selector, dataRegister);
...
static void lookupScheme4(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    /*  TODO implement lookups
       int32_t  total = node->getNumChildren();
       int32_t  numberOfEntries = total - 2;
       int32_t *dataTable = (int32_t *)cg->allocateCodeMemory(numberOfEntries * sizeof(int),
       cg->getCurrentEvaluationBlock()->isCold()); int32_t  address = (int32_t)dataTable; TR::Register *selector =
       cg->evaluate(node->getFirstChild()); TR::Register *cndRegister = cg->allocateRegister(TR_CCR); TR::Register
       *addrRegister = cg->allocateRegister(); TR::Register *dataRegister = cg->allocateRegister(); TR::Register
       *lowRegister = cg->allocateRegister(); TR::Register *highRegister = cg->allocateRegister(); TR::Register
       *pivotRegister = cg->allocateRegister(); TR::RegisterDependencyConditions *conditions = new (cg->trHeapMemory())
       TR::RegisterDependencyConditions(7, 7);
       ...
       generateTrg1ImmInstruction(node, cg, TR::InstOpCode::mov, lowRegister, 0);
       ...
       generateTrg1Src2Instruction(node, cg, TR::InstOpCode::add, pivotRegister, highRegister, lowRegister);
       new (cg->trHeapMemory()) TR::ARMTrg1Src1Imm2Instruction(0xfffffffc, 31, pivotRegister, pivotRegister,
       ARMOp_rlwinm, cg); generateTrg1MemInstruction(cg, ARMOp_lwzx, dataRegister, new (cg->trHeapMemory())
       TR_ARMMemoryReference(addrRegister, pivotRegister, cg)); generateTrg1Src2Instruction(node, cg, ARMOp_cmp4,
       cndRegister, dataRegister, selector); generateConditionalBranchInstruction(node, cg, ARMOp_bne, searchLabel,
       cndRegister); generateLabelInstruction(node, cg, TR::InstOpCode::bl, linkLabel); generateLabelInstruction(node,
       cg, TR::InstOpCode::label, linkLabel); new (cg->trHeapMemory()) TR::ARMTrg1Instruction(ARMOp_mflr, dataRegister,
       cg); generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi, pivotRegister, pivotRegister, 20);
       generateTrg1Src2Instruction(node, cg, TR::InstOpCode::add, dataRegister, pivotRegister, dataRegister);
       ...
       for (int32_t ii=2; ii<total; ii++)
          {
          generateLabelInstruction(node, cg, TR::InstOpCode::b,
       node->getChild(ii)->getBranchDestination()->getNode()->getLabel()); dataTable[ii-2] =
       node->getChild(ii)->getCaseConstant();
          }
       ...
       generateConditionalBranchInstruction(node, cg, ARMOp_beq,
       ...
       generateLabelInstruction(node, cg, TR::InstOpCode::b, backLabel);
       ...
       new (cg->trHeapMemory()) TR::ARMDepLabelInstruction(TR::InstOpCode::b, backLabel, conditions, cg);
       */
}
...
TR::Register* OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (numBranchTableEntries >= 5) {
        ...
        // the matching GlDepRegs free checks in OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must
        // be updated to synch.
    ...
    if (numBranchTableEntries < 5) {
        for (i = 0; i < numBranchTableEntries; i++) {
            generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node, selectorReg, i, 0);
            generateConditionalBranchInstruction(
                cg, node, ARMConditionCodeEQ, node->getChild(2 + i)->getBranchDestination()->getNode()->getLabel());
        }
        generateConditionalBranchInstruction(
            cg, node, ARMConditionCodeAL, secondChild->getBranchDestination()->getNode()->getLabel(), conditions);
    } else {
        ...
        if (!constantIsImmed8r(numBranchTableEntries, &base, &rotate)) {
            ...
            generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, selectorReg, tempRegister);
        } else {
            generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node, selectorReg, base, rotate);
        }
        generateConditionalBranchInstruction(
            cg, node, ARMConditionCodeCS, secondChild->getBranchDestination()->getNode()->getLabel());
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::sub, node, t1Register, selectorReg, 1, 0);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, machineIP, machineIP,
            new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, t1Register, 2));

        for (i = 2; i < total - 1; i++) {
            generateLabelInstruction(
                cg, TR::InstOpCode::b, node, node->getChild(i)->getBranchDestination()->getNode()->getLabel());
        }
        generateLabelInstruction(
            cg, TR::InstOpCode::b, node, node->getChild(i)->getBranchDestination()->getNode()->getLabel(), conditions);
    }
...
TR::Register* OMR::ARM::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    TR_ARMOutOfLineCodeSection* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_ARMOutOfLineCodeSection(node, TR::call, NULL, slowPathLabel, restartLabel, cg);
...
/* We need to change compareIntsForOrder and compareIntsForEquality for the optimization code below to work */
   if (valueToCheck->getOpCode().isBooleanCompare() &&
...
#endif
    {
        ...
        generateSrc1ImmInstruction(cg, TR::InstOpCode::cmp, node, value, 0, 0);
...
TR::Register* OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!secondChild->getOpCode().isLoadConst() || (dtype.isInt32() && secondChild->getInt() == 0)
        || (dtype.isInt64() && secondChild->getLongInt() == 0)) {
        srcReg = cg->evaluate(secondChild);
        if (dtype.isInt64()) {
            TR::Register* trgReg = cg->allocateRegister();
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::orr, node, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder());
            srcReg = trgReg;
        }
        ...
        TR::Instruction* instr1 = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uintptr_t)arithmeticException->getMethodAddress(), NULL, arithmeticException, NULL, NULL,
            ARMConditionCodeEQ);
        ...
        if (secondChild->getOpCode().isLoadConst()) {
            if (firstChild->getInt() < secondChild->getInt()) {
                ...
                instr = generateImmSymInstruction(cg, TR::InstOpCode::bl, node,
                    (uintptr_t)BNDCHKException->getMethodAddress(), NULL, BNDCHKException, NULL, NULL,
                    ARMConditionCodeAL);
                cg->machine()->setLinkRegisterKilled(true);
            } else {
...
static void VMoutlinedHelperArrayStoreCHKEvaluator(
    TR::Node* node, TR::Register* srcReg, TR::Register* dstReg, bool srcIsNonNull, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    ...
    TR::Instruction* gcPoint = generateImmSymInstruction(
        cg, TR::InstOpCode::bl, node, (uintptr_t)arrayStoreChkHelper->getMethodAddress(), deps, arrayStoreChkHelper);
...
TR::Register* OMR::ARM::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* storeMR = new (cg->trHeapMemory())
        TR::MemoryReference(node->getFirstChild(), TR::Compiler->om.sizeofReferenceAddress(), cg);

    generateMemSrc1Instruction(cg, TR::InstOpCode::str, node, storeMR, srcReg);
...
static TR::Register* generateMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
    ...
    if (cg->canClobberNodesRegister(child)) {
        ...
        if (two_reg) {
            generateTrg1Src1Instruction(cg, TR::InstOpCode::mov, node, trgReg->getLowOrder(), reg->getLowOrder());
            generateTrg1Src1Instruction(cg, TR::InstOpCode::mov, node, trgReg->getHighOrder(), reg->getHighOrder());
        } else {
            generateTrg1Src1Instruction(cg, TR::InstOpCode::mov, node, trgReg, reg);
        }
    }

    TR_ARMConditionCode cond = max ? (isUnsigned ? ARMConditionCodeHI : ARMConditionCodeGT)
                                   : (isUnsigned ? ARMConditionCodeLS : ARMConditionCodeLT);
    ...
    for (; i < n; i++) {
        ...
        if (two_reg) {
            ...
            // First compare
            cursor
                = generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, trgReg->getHighOrder(), reg->getHighOrder());
            cursor = generateConditionalBranchInstruction(cg, node, cond, doneLabel, deps, cursor);

            cursor = generateTrg1Src1Instruction(
                cg, TR::InstOpCode::mov, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
            cursor->setConditionCode(ARMConditionCodeNE);
            cursor = generateTrg1Src1Instruction(
                cg, TR::InstOpCode::mov, node, trgReg->getHighOrder(), reg->getHighOrder(), cursor);
            cursor->setConditionCode(ARMConditionCodeNE);
            ...
            cursor = generateSrc2Instruction(
                cg, TR::InstOpCode::cmp, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
            cursor = generateTrg1Src1Instruction(
                cg, TR::InstOpCode::mov, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
            ...
            cursor = generateSrc2Instruction(cg, TR::InstOpCode::cmp, node, trgReg, reg);

            cond = max ? (isUnsigned ? ARMConditionCodeLS : ARMConditionCodeLT)
                       : (isUnsigned ? ARMConditionCodeHI : ARMConditionCodeGT);
            cursor = generateTrg1Src1Instruction(cg, TR::InstOpCode::mov, node, trgReg, reg, cursor);
...
TR::Register* OMR::ARM::TreeEvaluator::igotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* instr = generateTrg1Src1Instruction(cg, TR::InstOpCode::mov, node, gr15, addrReg);
