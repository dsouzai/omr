...
void TR_ARMSubtractAnalyser::integerSubtractAnalyser(
    TR::Node* root, TR::InstOpCode::Mnemonic regToRegOpCode, TR::InstOpCode::Mnemonic memToRegOpCode)
{
    TR::Node* firstChild;
    TR::Node* secondChild;
    ...
    if (getCopyReg1()) {
        ...
        if (getSubReg3Reg2()) {
            new (cg()->trHeapMemory())
                TR::ARMTrg1Src2Instruction(TR::InstOpCode::sub, root, thirdReg, secondRegister, firstRegister, cg());
        ...
        new (cg()->trHeapMemory())
            TR::ARMTrg1Src1Instruction(TR::InstOpCode::sub, root, firstRegister, secondRegister, cg());
...
void TR_ARMSubtractAnalyser::longSubtractAnalyser(TR::Node* root)
{
    ...
    if (getCopyReg1()) {
        ...
        root->setRegister(thirdReg);
        new (cg()->trHeapMemory())
            TR::ARMTrg1Src1Instruction(TR::InstOpCode::mov, root, lowThird, firstRegister->getLowOrder(), cg());
        new (cg()->trHeapMemory())
            TR::ARMTrg1Src1Instruction(TR::InstOpCode::mov, root, highThird, firstRegister->getHighOrder(), cg());
        if (getSubReg3Reg2()) {
            new (cg()->trHeapMemory())
                TR::ARMTrg1Src1Instruction(TR::InstOpCode::sub, root, lowThird, secondRegister->getLowOrder(), cg());
            //         new (cg()->trHeapMemory()) TR::ARMTrg1Src1Instruction(SBB4RegReg, root, highThird,
        ...
        //      new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(root, ARMOp_subf, firstRegister->getLowOrder(),
        //      secondRegister->getLowOrder(), cg()); new (cg()->trHeapMemory()) TR::ARMRegRegInstruction(SBB4RegReg,
        //      firstRegister->getHighOrder(), secondRegister->getHighOrder(), cg());
        root->setRegister(firstRegister);
    } else // assert getSubReg1Mem2() == true
    {
        //      TR_ARMMemoryReference *lowMR = new (cg()->trHeapMemory()) TR_ARMMemoryReference(secondChild);
        //      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SUB4RegMem, firstRegister->getLowOrder(), lowMR,
        //      cg()); TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory()) TR_ARMMemoryReference(*lowMR, 4); new
        //      (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem, firstRegister->getHighOrder(), highMR,
        //      cg()); lowMR->decNodeReferenceCounts(); root->setRegister(firstRegister);
    }
