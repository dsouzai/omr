...
void moveStatusReg(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    } else // Move to status register
    {
        ...
        if (!instr->immForm) {
            ...
            sprintf(
                last_oprnd, "0x%x", rotateRight(concatImm4Imm4(instr->immHi4, instr->immLo4), instr->rotate_imm * 2));
        }
...
void miscInstr(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    case 7: {
        ...
        sprintf(
            iBuf, "0x%x%x%s", bkptInstr->immed12, bkptInstr->immed4, bkptInstr->cond != 14 ? "   ; UNPREDICTABLE" : "");
    } break;
    case 8:
    case 10:
    case 12:
...
void dataProcessing(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    } else if (instr->shifter == 6) //  Rotate right with extend
    {
        ...
        if (instrBits->bit4) // Register shift
        {
            ...
            sprintf(
                sh_oprnd, "%s, %s %s", regStr(instr->Rm, false), shift_rotate[instr->shift], regStr(instr->Rs, false));
...
void vfpExtension(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    case 4:
        ...
        sprintf(iBuf, "%s, %s",
            isDouble ? doubleRegStr(getVFPDestination(instr), false) : singleRegStr(getVFPDestination(instr), false),
            isDouble ? doubleRegStr(getVFPSecondOperand(instr), false)
                     : singleRegStr(getVFPSecondOperand(instr), false));
        break;
    case 5:
    ...
    case 7:
        if (instr->N) {
            ...
            sprintf(iBuf, "%s, %s",
                isDouble ? singleRegStr(getVFPDestination(instr), true) : singleRegStr(getVFPDestination(instr), true),
                isDouble ? doubleRegStr(getVFPSecondOperand(instr), false)
                         : singleRegStr(getVFPSecondOperand(instr), false));
        } else
            constant32(instrPtr, mBuf, iBuf);
