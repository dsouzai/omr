...
OMR::ARM::Instruction::Instruction(
    TR::CodeGenerator* cg, TR::Instruction* precedingInstruction, TR::InstOpCode::Mnemonic op, TR::Node* node)
...
OMR::ARM::Instruction::Instruction(
    TR::Instruction* precedingInstruction, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::CodeGenerator* cg)
...
OMR::ARM::Instruction::Instruction(
    TR::InstOpCode::Mnemonic op, TR::Node* node, TR::RegisterDependencyConditions* cond, TR::CodeGenerator* cg)
    : OMR::InstructionConnector(cg, op, node)
{
    self()->setConditionCode(ARMConditionCodeAL);
...
TR::Register* OMR::ARM::Instruction::getMemoryDataRegister() { return NULL; }
...
TR::ARMConditionalBranchInstruction* OMR::ARM::Instruction::getARMConditionalBranchInstruction() { return NULL; }
...
void OMR::ARM::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    if (self()->getDependencyConditions()) {
        ...
        self()->getDependencyConditions()->assignPreConditionRegisters(
            self()->getPrev(), kindToBeAssigned, self()->cg());
    }
}
...
{
    // If a virtual is live at the end of the hot path and dead at the beginning it will not be killed immediatey after
    // it's first use in the hot path if it's also used on the cold path, since RA still needs to be done on the cold
    // path (which is where it's future use count will drop to 0), which means it will be incorrectly seen as live
    // between it's first use and the top of the hot path. If there are any GC points between the top of the hot path
    // and the first use the real reg holding the virtual will be included, so we need to fix this.
    ...
    while (!instr->isLabel() || !((TR::ARMLabelInstruction*)instr)->getLabelSymbol()->isEndOfColdInstructionStream()) {
        if (instr->needsGCMap()) {
            ...
            // This instruction has a GC map, for every register in the register map check if that register is
            // unassigned at the beginning of the hot path.
            for (uint32_t regNum = TR::RealRegister::FirstGPR; regNum < TR::RealRegister::LastGPR; ++regNum) {
                ...
                if (map->getRegisterMap() & regMask) {
                    ...
                    if (regInRegMap->getState() == TR::RealRegister::Free) {
                        ...
                        // If it wasn't defined before the GC point it died on the cold path and it's first use on the
                        // hot path was after the GC point i.e. it shouldn't be in the register map.
...
bool TR::ARMConditionalBranchInstruction::refsRegister(TR::Register* reg) { return false; }

bool TR::ARMConditionalBranchInstruction::defsRegister(TR::Register* reg) { return false; }

bool TR::ARMConditionalBranchInstruction::defsRealRegister(TR::Register* reg) { return false; }

bool TR::ARMConditionalBranchInstruction::usesRegister(TR::Register* reg) { return false; }
...
// check within "toARMImmInstruction"
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
TR::ARMImmInstruction* TR::ARMImmInstruction::getARMImmInstruction() { return this; }
#endif // defined(DEBUG) || defined(PROD_WITH_ASSUMES)

// TR::ARMImmSymInstruction:: member functions

TR::ARMImmSymInstruction::ARMImmSymInstruction(TR::InstOpCode::Mnemonic op, TR::Node* node, uint32_t imm,
    TR::RegisterDependencyConditions* cond, TR::SymbolReference* sr, TR::CodeGenerator* cg, TR::Snippet* s,
...
TR::ARMImmSymInstruction::ARMImmSymInstruction(TR::Instruction* precedingInstruction, TR::InstOpCode::Mnemonic op,
    TR::Node* node, uint32_t imm, TR::RegisterDependencyConditions* cond, TR::SymbolReference* sr,
    TR::CodeGenerator* cg, TR::Snippet* s, TR_ARMConditionCode cc)
...
void TR::ARMLabelInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (target1Virtual)
        setTarget1Register(assignedTarget1Register);
    if (source1Virtual)
...
{
    return (reg == getTarget1Register() || reg == getSource1Register() || getSource2Operand()->refsRegister(reg));
}

bool TR::ARMTrg1Src2Instruction::defsRegister(TR::Register* reg) { return (reg == getTarget1Register()); }
...
{
    return (reg == getTarget1Register() || reg == getTarget2Register() || reg == getSource1Register());
}
...
bool TR::ARMTrg2Src1Instruction::usesRegister(TR::Register* reg) { return (reg == getSource1Register()); }
...
void TR::ARMTrg2Src1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (source1Virtual) {
        if (target1Virtual)
            target1Virtual->block();
        if (target2Virtual)
            target2Virtual->block();
        assignedSource1Register = machine->assignSingleRegister(source1Virtual, this);
        if (target2Virtual)
            target2Virtual->unblock();
        if (target1Virtual)
            target1Virtual->unblock();
    }
    ...
    if (target1Virtual)
        setTarget1Register(assignedTarget1Register);
    if (target2Virtual)
        setTarget2Register(assignedTarget2Register);
    if (source1Virtual)
        setSource1Register(assignedSource1Register);
}
#endif
...
{
    return (reg == getTargetLoRegister() || reg == getTargetHiRegister() || reg == getSource1Register()
        || reg == getSource2Register());
}

bool TR::ARMMulInstruction::defsRegister(TR::Register* reg)
{
    return (reg == getTargetLoRegister() || reg == getTargetHiRegister());
}

bool TR::ARMMulInstruction::defsRealRegister(TR::Register* reg)
{
    return (reg == getTargetLoRegister()->getAssignedRegister() || reg == getTargetHiRegister()->getAssignedRegister());
}
...
void TR::ARMMulInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (targetHiVirtual)
        setTargetHiRegister(assignedTargetHiRegister);
    setTargetLoRegister(assignedTargetLoRegister);
    setSource1Register(assignedSource1Register);
    setSource2Register(assignedSource2Register);
}

TR::Register* TR::ARMMemInstruction::getMemoryDataRegister() { return getTargetRegister(); }

bool TR::ARMMemInstruction::refsRegister(TR::Register* reg) { return getMemoryReference()->refsRegister(reg); }

bool TR::ARMMemInstruction::usesRegister(TR::Register* reg) { return getMemoryReference()->refsRegister(reg); }
...
bool TR::ARMMemSrc1Instruction::defsRegister(TR::Register* reg) { return false; }

bool TR::ARMMemSrc1Instruction::defsRealRegister(TR::Register* reg) { return false; }
...
bool TR::ARMTrg1Instruction::refsRegister(TR::Register* reg) { return (reg == getTargetRegister()); }

bool TR::ARMTrg1Instruction::usesRegister(TR::Register* reg) { return false; }

bool TR::ARMTrg1Instruction::defsRegister(TR::Register* reg) { return (reg == getTargetRegister()); }
...
bool TR::ARMTrg1MemInstruction::usesRegister(TR::Register* reg) { return getMemoryReference()->refsRegister(reg); }
...
static TR::Instruction* expandBlongLessThan(TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* cursor,
    TR::Register* temp, TR::Register* result, TR::Register* aHi, TR::Register* aLo, TR::Register* bHi,
    TR::Register* bLo, bool trueValue)
{
...
void TR::ARMControlFlowInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    for (i = 0; i < getNumTargets(); i++) {
        TR::Register* targetVirtual = getTargetRegister(i);
        // Because of OOL codegen, if the target virtual is defined at this instruction and this instruction is in the
        // hot path of an OOL section, the virtual's future use count may still be >0 at this point because of
        // outstsanding uses in the cold path. Therefore we have to check if this is the start of it's live range and
        // unlatch it.
        targetVirtual->unblock();
    ...
    case TR::InstOpCode::iflong: {
        ...
        cursor = generateSrc2Instruction(
            cg(), TR::InstOpCode::cmp, currentNode, getSourceRegister(0), getSourceRegister(2), cursor);
        cursor = generateConditionalBranchInstruction(
            cg(), currentNode, isGT ? ARMConditionCodeGT : ARMConditionCodeLT, getLabelSymbol(), cursor);
        cursor = generateConditionalBranchInstruction(
            cg(), currentNode, isGT ? ARMConditionCodeLT : ARMConditionCodeGT, label2, cursor);
        cursor = generateSrc2Instruction(
            cg(), TR::InstOpCode::cmp, currentNode, getSourceRegister(1), getSourceRegister(3), cursor);
        cursor = generateConditionalBranchInstruction(
            cg(), currentNode, isGT ? ARMConditionCodeHI : ARMConditionCodeLS, getLabelSymbol(), cursor);
    ...
    case TR::InstOpCode::setblong:
        ...
        case ARMConditionCodeLT:
            cursor = expandBlongLessThan(currentNode, cg(), cursor, getTargetRegister(1), getTargetRegister(0),
                getSourceRegister(1), getSourceRegister(0), getSourceRegister(3), getSourceRegister(2), 1);
            break;
        case ARMConditionCodeGT:
            // A > B ==> B < A
            cursor = expandBlongLessThan(currentNode, cg(), cursor, getTargetRegister(1), getTargetRegister(0),
                getSourceRegister(3), getSourceRegister(2), getSourceRegister(1), getSourceRegister(0), 1);
            break;
        case ARMConditionCodeGE:
            // A >= B ==> !(B < A)
            cursor = expandBlongLessThan(currentNode, cg(), cursor, getTargetRegister(1), getTargetRegister(0),
                getSourceRegister(1), getSourceRegister(0), getSourceRegister(3), getSourceRegister(2), 0);
            break;
        case ARMConditionCodeLE:
            // A <= B ==> !(B > A)
            cursor = expandBlongLessThan(currentNode, cg(), cursor, getTargetRegister(1), getTargetRegister(0),
                getSourceRegister(3), getSourceRegister(2), getSourceRegister(1), getSourceRegister(0), 0);
            break;
        case ARMConditionCodeEQ:
        case ARMConditionCodeNE:
            cursor = generateTrg1ImmInstruction(
                cg(), TR::InstOpCode::mov, currentNode, getTargetRegister(0), 0, 0, cursor);
            cursor = generateSrc2Instruction(
                cg(), TR::InstOpCode::cmp, currentNode, getSourceRegister(0), getSourceRegister(2), cursor);
            cursor = generateSrc2Instruction(
                cg(), TR::InstOpCode::cmp, currentNode, getSourceRegister(1), getSourceRegister(3), cursor);
            ...
            cursor = generateTrg1ImmInstruction(
                cg(), TR::InstOpCode::mov, currentNode, getTargetRegister(0), 1, 0, cursor);
    ...
    case TR::InstOpCode::setbool:
        ...
        /*
        cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, getCmpOpValue(), currentNode,
        getTargetRegister(0), getSourceRegister(0), getSourceRegister(1), cg()); cursor = new (cg()->trHeapMemory())
        TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), 1, cg()); cursor = new
        (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, getOpCode2Value(), currentNode, label2,
        getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li,
        currentNode, getTargetRegister(1), 0, cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor,
        TR::InstOpCode::label, currentNode, label2, cg()); break;
        */
    case TR::InstOpCode::setbflt:
        ...
        TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), 1, cg()); cursor = new
        (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, getOpCode2Value(), currentNode, label2,
        getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor,
        getOpCode3Value(), currentNode, label2, getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory())
        TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), 0, cg()); cursor = new
        (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, TR::InstOpCode::label, currentNode, label2, cg()); break;
        */
    case TR::InstOpCode::lcmp:
        ...
        /*
              case TR::InstOpCode::flcmpl:
                 cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_fcmpu, currentNode,
           getTargetRegister(0), getSourceRegister(0), getSourceRegister(2), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), 1, cg()); cursor = new
           (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, ARMOp_bgt, currentNode, label2,
           getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li,
           currentNode, getTargetRegister(1), 0, cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode, label2, getTargetRegister(0), cg());
                 cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode,
           getTargetRegister(1), -1, cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor,
           TR::InstOpCode::label, currentNode, label2, cg()); break; case TR::InstOpCode::flcmpg: cursor = new
           (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_fcmpu, currentNode, getTargetRegister(0),
           getSourceRegister(0), getSourceRegister(2), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), -1, cg()); cursor = new
           (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, ARMOp_blt, currentNode, label2,
           getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li,
           currentNode, getTargetRegister(1), 0, cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode, label2, getTargetRegister(0), cg());
                 cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode,
           getTargetRegister(1), 1, cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor,
           TR::InstOpCode::label, currentNode, label2, cg()); break; case TR::InstOpCode::idiv: cursor = new
           (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_eqv, currentNode, getTargetRegister(2),
           getTargetRegister(1), getSourceRegister(1), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1Src2Instruction(cursor, TR::InstOpCode::and_, currentNode, getTargetRegister(2),
           getSourceRegister(2), getTargetRegister(2), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1Src1ImmInstruction(cursor, ARMOp_cmpi4, currentNode, getTargetRegister(0), getTargetRegister(2),
           -1, cg()); cursor = new (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq,
           currentNode, label2, getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1Src2Instruction(cursor, ARMOp_divw, currentNode, getTargetRegister(1), getSourceRegister(1),
           getSourceRegister(2), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor,
           TR::InstOpCode::label, currentNode, label2, cg()); break; case TR::InstOpCode::irem: cursor = new
           (cg()->trHeapMemory()) TR::ARMTrg1Src1ImmInstruction(cursor, ARMOp_cmpi4, currentNode, getTargetRegister(0),
           getSourceRegister(2), -1, cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor,
           ARMOp_li, currentNode, getTargetRegister(1), 0, cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode, label2, getTargetRegister(0), cg());
                 cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_divw, currentNode,
           getTargetRegister(2), getSourceRegister(1), getSourceRegister(2), cg()); cursor = new (cg()->trHeapMemory())
           TR::ARMTrg1Src2Instruction(cursor, ARMOp_mullw, currentNode, getTargetRegister(2), getSourceRegister(2),
           getTargetRegister(2), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor,
           ARMOp_subf, currentNode, getTargetRegister(1), getTargetRegister(2), getSourceRegister(1), cg()); cursor =
           new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, TR::InstOpCode::label, currentNode, label2, cg());
                 break;
        */
    default:
...
bool TR::ARMMultipleMoveInstruction::refsRegister(TR::Register* reg) { return (reg == getMemoryBaseRegister()); }

bool TR::ARMMultipleMoveInstruction::defsRegister(TR::Register* reg) { return (reg == getMemoryBaseRegister()); }
...
bool TR::ARMMultipleMoveInstruction::usesRegister(TR::Register* reg) { return (reg == getMemoryBaseRegister()); }
