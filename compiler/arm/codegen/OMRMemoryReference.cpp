...
static uint8_t *armLoadConstantInstructions(TR::RealRegister *trgReg, int32_t value, uint8_t *cursor, uint32_t *wcursor,
    TR::CodeGenerator *cg);
...
OMR::ARM::MemoryReference::MemoryReference(TR::Node *node, TR::SymbolReference *symRef, uint32_t len,
    TR::CodeGenerator *cg)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
...
OMR::ARM::MemoryReference::MemoryReference(TR::MemoryReference &mr, int32_t displacement, uint32_t len,
    TR::CodeGenerator *cg)
    : _symbolReference(cg->comp()->getSymRefTab())
{
                ...
                if (base != 0) {
                    if (!isFirstDone) {
                        if (_baseRegister == NULL)
                            ...
                            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::add, node, newBase, _baseRegister, base,
                                shift);
                        isFirstDone = true;
                    } else
...
void OMR::ARM::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCodeValue() == TR::aiadd || subTree->getOpCodeValue() == TR::iadd) {
            ...
            if (symbol->isStatic()) {
                if (ref->isUnresolved()) {
                    self()->setUnresolvedSnippet(new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, subTree,
                        &_symbolReference, subTree->getOpCode().isStore(), false));
                    cg->addSnippet(self()->getUnresolvedSnippet());
                } else {
                    if (_baseRegister != NULL) {
...
void OMR::ARM::MemoryReference::consolidateRegisters(TR::Register *srcReg, TR::Node *srcTree, bool srcModifiable,
    TR::CodeGenerator *cg)
{
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
        ...
        if (_indexRegister != NULL) {
            ...
            new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(TR::InstOpCode::add, srcTree, tempTargetRegister,
                _indexRegister, srcReg, cg);
        ...
        if (_indexRegister != NULL) {
            if (self()->isBaseModifiable())
                ...
                tempTargetRegister = cg->allocateRegister();
            new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(TR::InstOpCode::add, srcTree, tempTargetRegister,
                _baseRegister, _indexRegister, cg);
...
uint8_t *OMR::ARM::MemoryReference::generateBinaryEncoding(TR::Instruction *currentInstruction, uint8_t *cursor,
    TR::CodeGenerator *cg)
{
        ...
        *wcursor = 0xEA000000; // insert a branch to the snippet
        ...
        } else if (op == TR::InstOpCode::add) // op is TR::InstOpCode::add for delayed offsets (see loadAddrEvaluator)
        {
            ...
            *wcursor |= (displacement >> 2); // offset for these coprocessor instructions is in words
            ...
            TR_ASSERT(displacement == 0 && index == 0,
                "Load/store exclusive does not have displacement or index register");
            base->setRegisterFieldRN(wcursor);
        } else {
...
uint8_t *OMR::ARM::MemoryReference::encodeLargeARMConstant(uint32_t *wcursor, uint8_t *cursor,
