...
static void loadRelocatableConstant(
    TR::Node* node, TR::SymbolReference* ref, TR::Register* reg, TR::MemoryReference* mr, TR::CodeGenerator* cg);

static uint8_t* armLoadConstantInstructions(
    TR::RealRegister* trgReg, int32_t value, uint8_t* cursor, uint32_t* wcursor, TR::CodeGenerator* cg);
...
//
OMR::ARM::MemoryReference::MemoryReference(TR::Node* rootLoadOrStore, uint32_t len, TR::CodeGenerator* cg)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
    ...
    , _flag(0)
{
    ...
    if (rootLoadOrStore->getOpCode().isIndirect()) {
        if (ref->isUnresolved()) {
            self()->setUnresolvedSnippet(new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
                cg, rootLoadOrStore, rootLoadOrStore->getSymbolReference(), isStore, false));
...
OMR::ARM::MemoryReference::MemoryReference(
    TR::Node* node, TR::SymbolReference* symRef, uint32_t len, TR::CodeGenerator* cg)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
...
OMR::ARM::MemoryReference::MemoryReference(
    TR::MemoryReference& mr, int32_t displacement, uint32_t len, TR::CodeGenerator* cg)
    : _symbolReference(cg->comp()->getSymRefTab())
{
...
#ifdef J9_PROJECT_SPECIFIC
        _unresolvedSnippet = new (cg->trHeapMemory())
            TR::UnresolvedDataSnippet(cg, 0, &_symbolReference, mr.getUnresolvedSnippet()->resolveForStore(), false);
...
void OMR::ARM::MemoryReference::setSymbol(TR::Symbol* symbol, TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL && _indexRegister != NULL && (self()->hasDelayedOffset() || self()->getOffset() != 0)) {
                    ...
                    if (!isFirstDone) {
                        if (_baseRegister == NULL)
                            ...
                            generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::add, node, newBase, _baseRegister, base, shift);
                        isFirstDone = true;
                    } else
...
void OMR::ARM::MemoryReference::addToOffset(TR::Node* node, int32_t amount, TR::CodeGenerator* cg)
{
    ...
    if ((_length == 4 && !constantIsImmed12(displacement))
        || (_length < 4 && (displacement < -128 || displacement > 127))) {
        TR::Register* newBase;
...
void OMR::ARM::MemoryReference::adjustForResolution(TR::CodeGenerator* cg) { _modBase = cg->allocateRegister(); }
...
void OMR::ARM::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCodeValue() == TR::aiadd || subTree->getOpCodeValue() == TR::iadd) {
            ...
            if (symbol->isStatic()) {
                if (ref->isUnresolved()) {
                    self()->setUnresolvedSnippet(new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
                        cg, subTree, &_symbolReference, subTree->getOpCode().isStore(), false));
                    cg->addSnippet(self()->getUnresolvedSnippet());
                } else {
                    if (_baseRegister != NULL) {
...
void OMR::ARM::MemoryReference::consolidateRegisters(
    TR::Register* srcReg, TR::Node* srcTree, bool srcModifiable, TR::CodeGenerator* cg)
{
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
        ...
        if (_indexRegister != NULL) {
            ...
            else if (srcReg->containsCollectedReference() || _indexRegister->containsCollectedReference())
            ...
            new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
                TR::InstOpCode::add, srcTree, tempTargetRegister, _indexRegister, srcReg, cg);
        ...
        if (_indexRegister != NULL) {
            ...
            else if (_baseRegister->containsCollectedReference() || _indexRegister->containsCollectedReference())
                tempTargetRegister = cg->allocateCollectedReferenceRegister();
            else
                tempTargetRegister = cg->allocateRegister();
            new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
                TR::InstOpCode::add, srcTree, tempTargetRegister, _baseRegister, _indexRegister, cg);
            ...
            else if (srcReg->containsCollectedReference() || _baseRegister->containsCollectedReference())
                tempTargetRegister = cg->allocateCollectedReferenceRegister();
            else
                tempTargetRegister = cg->allocateRegister();
            new (cg->trHeapMemory())
                TR::ARMTrg1Src2Instruction(TR::InstOpCode::add, srcTree, tempTargetRegister, _baseRegister, srcReg, cg);
            if (_baseRegister != tempTargetRegister) {
                self()->decNodeReferenceCounts();
                _baseNode = NULL;
            }
...
uint8_t* OMR::ARM::MemoryReference::generateBinaryEncoding(
    TR::Instruction* currentInstruction, uint8_t* cursor, TR::CodeGenerator* cg)
{
    ...
    if (self()->getUnresolvedSnippet()) {
        ...
        cg->addRelocation(new (cg->trHeapMemory())
                TR::LabelRelative24BitRelocation(cursor, self()->getUnresolvedSnippet()->getSnippetLabel()));
        *wcursor = 0xEA000000; // insert a branch to the snippet
        ...
        if (index) {
            ...
            *wcursor = preserve;
            if (op == TR::InstOpCode::ldr || op == TR::InstOpCode::ldrb || op == TR::InstOpCode::str
                || op == TR::InstOpCode::strb || op == TR::InstOpCode::ldrsb || op == TR::InstOpCode::ldrh
                || op == TR::InstOpCode::ldrsh || op == TR::InstOpCode::strh) {
                if (base) {
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
            else if (op == TR::InstOpCode::flds || op == TR::InstOpCode::fldd || op == TR::InstOpCode::fsts
                || op == TR::InstOpCode::fstd) {
                if (base) {
        ...
        if (!index
            && op != TR::InstOpCode::add) // common encoding for all true load/store instructions with immediate offsets
        {
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
            if (!(op == TR::InstOpCode::flds || op == TR::InstOpCode::fldd || op == TR::InstOpCode::fsts
                    || op == TR::InstOpCode::fstd)) {
#endif
                base->setRegisterFieldRN(wcursor);
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
            ...
            TR_ASSERT(!(self()->isImmediatePreIndexed() && self()->isPostIndexed()),
                "write back with post-index transfer redundant");
        ...
        if (op == TR::InstOpCode::ldr || op == TR::InstOpCode::str || op == TR::InstOpCode::ldrb
            || op == TR::InstOpCode::strb) {
            // addressing mode 2
            if (index) {
                if (0 != _scale) {
                    if (4 == _scale) {
            ...
            || op == TR::InstOpCode::strh) // addressing mode 3
        {
            if (index) {
                base->setRegisterFieldRN(wcursor);
                index->setRegisterFieldRM(wcursor);
        ...
        } else if (op == TR::InstOpCode::add) // op is TR::InstOpCode::add for delayed offsets (see loadAddrEvaluator)
        {
        ...
        } else if (op == TR::InstOpCode::ldfd || op == TR::InstOpCode::stfd || op == TR::InstOpCode::ldfs
            ...
            *wcursor |= (displacement >> 2); // offset for these coprocessor instructions is in words
            ...
            TR_ASSERT(
                displacement == 0 && index == 0, "Load/store exclusive does not have displacement or index register");
            base->setRegisterFieldRN(wcursor);
        } else {
...
uint32_t OMR::ARM::MemoryReference::estimateBinaryLength(TR::InstOpCode::Mnemonic op)
{
    ...
    } else if (op == TR::InstOpCode::ldr || op == TR::InstOpCode::str || op == TR::InstOpCode::ldrb
        || op == TR::InstOpCode::strb || op == TR::InstOpCode::ldrex || op == TR::InstOpCode::strex) {
...
uint8_t* OMR::ARM::MemoryReference::encodeLargeARMConstant(
    uint32_t* wcursor, uint8_t* cursor, TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    bool spillNeeded;
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
    vfpInstruction = (op == TR::InstOpCode::flds || op == TR::InstOpCode::fldd || op == TR::InstOpCode::fsts
        || op == TR::InstOpCode::fstd);
#endif

    TR_ASSERT((op == TR::InstOpCode::add || op == TR::InstOpCode::ldr || op == TR::InstOpCode::ldrb
                  || op == TR::InstOpCode::str || op == TR::InstOpCode::strb || op == TR::InstOpCode::ldrsb
                  || op == TR::InstOpCode::ldrh || op == TR::InstOpCode::ldrsh || op == TR::InstOpCode::strh
                  || vfpInstruction),
        "unrecognized opcode");
    ...
    if (op == TR::InstOpCode::str || op == TR::InstOpCode::strh || op == TR::InstOpCode::strb || vfpInstruction
        || toRealRegister(currentInstruction->getMemoryDataRegister())->getRegisterNumber()
            == base->getRegisterNumber()) {
        spillNeeded = true;
...
static TR::RealRegister::RegNum choose_rX(TR::Instruction* currentInstruction, TR::RealRegister* base)
{
    ...
    int32_t exclude = ((TSnum == TR::RealRegister::gr3) ? 1 : 0) | ((TSnum == TR::RealRegister::gr2) ? 2 : 0)
        | ((basenum == TR::RealRegister::gr3) ? 1 : 0) | ((basenum == TR::RealRegister::gr2) ? 2 : 0);
    ...
    TR::RealRegister* trgReg, int32_t value, uint8_t* cursor, uint32_t* wcursor, TR::CodeGenerator* cg)
{
    ...
    // if these large delayed constants start showing up more frequently it would be worth using similar logic as in
    // armLoadConstant for now just pessimistically generate four instructions
    ...
    TR::Node* node, TR::SymbolReference* ref, TR::Register* reg, TR::MemoryReference* mr, TR::CodeGenerator* cg)
{
    ...
    addr = isStatic ? (uintptr_t)symbol->getStaticSymbol()->getStaticAddress()
    ...
    if (ref->isUnresolved() || comp->compileRelocatableCode()) {
        ...
        if (symbol->isCountForRecompile()) {
            ...
            cg->addSnippet(mr->setUnresolvedSnippet(
                new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, node, ref, node->getOpCode().isStore(), false)));
        }
    } else {
