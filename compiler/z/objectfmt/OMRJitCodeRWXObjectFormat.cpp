...
TR::Instruction* OMR::Z::JitCodeRWXObjectFormat::emitFunctionCall(TR::FunctionCallData& data)
{
    ...
    TR_ASSERT_FATAL_WITH_NODE(
        callNode, data.returnAddressReg != NULL, "returnAddressReg should be set to make a function call.");
    ...
    if (data.snippet->getKind() == TR::Snippet::IsUnresolvedCall
        ...
        *reinterpret_cast<int32_t*>(cursor)
        ...
        cg->addProjectSpecializedRelocation(
            cursor, (uint8_t*)callSymRef, NULL, TR_HelperAddress, __FILE__, __LINE__, callNode);
        cursor += sizeof(int32_t);
    }
