...
void TR_S390BinaryCommutativeAnalyser::remapInputs(TR::Node *firstChild, TR::Register *firstRegister,
    TR::Node *secondChild, TR::Register *secondRegister, bool nonClobberingDestination)
{
    if (!cg()->useClobberEvaluate()) {
        ...
        if (firstChild == secondChild && firstChild->getReferenceCount() == 2 && firstRegister
            ...
            // isRegisterClobberable(firstRegister,1) returns false in this latter case as firstRegister is attached to
            // more than one unique node
            setClob1();
            setClob2();
        }
    }
...
TR::Register *TR_S390BinaryCommutativeAnalyser::allocateAddSubRegister(TR::Node *node, TR::Register *src1Reg)
{
...
void TR_S390BinaryCommutativeAnalyser::genericAnalyser(TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
    TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode, bool nonClobberingDestination,
    TR::LabelSymbol *targetLabel, TR::InstOpCode::S390BranchCondition fBranchOpCond,
    TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
    ...
    setInputs(firstChild, firstRegister, secondChild, secondRegister, nonClobberingDestination, false, comp);
    ...
    if (cg()->comp()->target().cpu.supportsFeature(OMR_FEATURE_S390_MISCELLANEOUS_INSTRUCTION_EXTENSION_2)) {
        ...
        if (root->getOpCodeValue() == TR::lmul) {
            if (firstChild->getOpCodeValue() == TR::s2l && firstChild->getFirstChild()->getOpCodeValue() == TR::sloadi
            ...
            } else if (firstChild->getOpCodeValue() == TR::lloadi && firstChild->getReferenceCount() == 1
                && firstChild->getRegister() == NULL) {
            ...
            if (firstChild->getOpCodeValue() == TR::iloadi && firstChild->getReferenceCount() == 1
                && firstChild->getRegister() == NULL) {
    ...
    if ((root->getSize() == 1 || root->getSize() == 2) && root->getOpCode().isBitwiseLogical()) {
        resetMem1();
        resetMem2();
    }

    if (root->getOpCode().isBooleanCompare()) {
        if (firstChild->getSize() == 1 || (root->getOpCode().isUnsigned() && firstChild->getSize() == 2))
            resetMem1();

        if (secondChild->getSize() == 1 || (root->getOpCode().isUnsigned() && secondChild->getSize() == 2))
            resetMem2();
    }
    ...
    if (getOpReg1Reg2()) {
        if (targetLabel != NULL) {
            finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, firstRegister,
                secondRegister, getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel, false, true);
            ...
            if (cg()->comp()->target().cpu.supportsFeature(OMR_FEATURE_S390_MISCELLANEOUS_INSTRUCTION_EXTENSION_2)) {
                ...
                if (root->getOpCodeValue() == TR::lmul && firstRegister != NULL && firstRegister->is64BitReg()
                    && secondRegister != NULL && secondRegister->is64BitReg()) {
                    z14OpCode = TR::InstOpCode::MSGRKC;
                } else if (root->getOpCodeValue() == TR::imul && firstRegister != NULL && secondRegister != NULL) {
            ...
            if (nodeReg == NULL) {
                ...
                if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                    ...
                    && performTransformation(comp,
                        "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].",
                        ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
                        debugObj->getName(secondRegister), ccInst)) {
                    ...
                    && performTransformation(comp,
                        "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].",
                        ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
                        debugObj->getName(secondRegister), ccInst)) {
        ...
        if (targetLabel != NULL) {
            finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, secondRegister,
                firstRegister, (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel, false, true);
            nodeReg = secondRegister;
            notReversedOperands();
        } else {
            ...
            if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister)
                && performTransformation(comp,
                    "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].",
                    ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
                    debugObj->getName(secondRegister), ccInst)) {
                nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
            } else if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister)
                && performTransformation(comp,
                    "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].",
                    ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister),
                    debugObj->getName(secondRegister), ccInst)) {
                nodeReg = secondRegister;
                notReversedOperands(); // CCInfo already exists; don't generate compare op any more;
        ...
        switch (firstRegister->getKind()) {
            ...
            default: {
                TR_ASSERT_FATAL(false, "Generic binary commutative analyser for register kind (%d) is unimplemented",
        ...
        if (targetLabel != NULL)
            finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, nodeReg, secondRegister,
                getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel, false, true);
        else
        ...
        if (targetLabel != NULL)
            finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, nodeReg, firstRegister,
                (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel, false, true);
        else
            ...
            switch (firstRegister->getKind()) {
                ...
                default: {
                    TR_ASSERT_FATAL(false,
                        "Generic binary commutative analyser for register kind (%d) is unimplemented",
                        firstRegister->getKind());
                    break;
                }
            }
    ...
    if (!finalInstr && targetLabel != NULL) {
        generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, getReversedOperands() ? rBranchOpCond : fBranchOpCond,
            root, targetLabel);
    }
 ...
 */
bool TR_S390BinaryCommutativeAnalyser::conversionIsRemoved(TR::Node *root, TR::Node *&child)
{
    if (((root->getDataType() == TR::Int64 && child->getOpCodeValue() == TR::a2l && cg()->comp()->target().is64Bit())
            || (root->getDataType() == TR::Int32 && child->getOpCodeValue() == TR::a2i
                && cg()->comp()->target().is32Bit()))
        && child->getFirstChild()->getOpCodeValue() == TR::aloadi && child->getFirstChild()->getRegister() == NULL
        && child->getFirstChild()->getReferenceCount() == 1 && child->getReferenceCount() == 1) {
 ...
 */
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
    TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    /* bool setsOrReadsCC = NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc) || (node->getOpCodeValue() ==
     * TR::iuaddc); */
    ...
    if (cg()->comp()->target().cpu.supportsFeature(OMR_FEATURE_S390_MISCELLANEOUS_INSTRUCTION_EXTENSION_2)
        && secondChild->getOpCodeValue() == TR::s2l && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi
        && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL
        && secondChild->getFirstChild()->getReferenceCount() == 1
        && secondChild->getFirstChild()->getRegister() == NULL) {
        setMem2();
    ...
    if (getOpReg1Reg2()) {
        ...
        if (cg()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
            if (regToRegOpCode == TR::InstOpCode::AR) {
                cursor
                    = generateRRRInstruction(cg(), TR::InstOpCode::ARK, root, tempReg, secondRegister, firstRegister);
                done = true;
            } else if (regToRegOpCode == TR::InstOpCode::ALR) {
                cursor
                    = generateRRRInstruction(cg(), TR::InstOpCode::ALRK, root, tempReg, secondRegister, firstRegister);
                done = true;
            } else if (regToRegOpCode == TR::InstOpCode::AGR) {
                cursor
                    = generateRRRInstruction(cg(), TR::InstOpCode::AGRK, root, tempReg, secondRegister, firstRegister);
                done = true;
            } else if (regToRegOpCode == TR::InstOpCode::ALGR) {
                cursor
                    = generateRRRInstruction(cg(), TR::InstOpCode::ALGRK, root, tempReg, secondRegister, firstRegister);
                done = true;
            }
        }

        if (!done) {
            auto mnemonic = firstRegister->is64BitReg() ? TR::InstOpCode::LGR : TR::InstOpCode::LR;
...
bool TR_S390BinaryCommutativeAnalyser::getCopyReg1()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getCopyReg2()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg1Reg2()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg2Reg1()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg1Mem2()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg3Mem2()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg2Mem1()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getOpReg3Mem1()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getCopyRegs()
{
    if (!cg()->useClobberEvaluate()) {
...
bool TR_S390BinaryCommutativeAnalyser::getInvalid()
{
    if (!cg()->useClobberEvaluate()) {
