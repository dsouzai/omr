                                                      ...
                                                      // before adding in the temporary negative offset
        {
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    ...
                    self()->getOffset());
            self()->enforceSSFormatLimits(
                node, cg, NULL); // call SSFormatLimits to also take this chance to fold in an index register if needed
        }
    ...
    List<TR::Node>& _incNodesList, List<TR::Node>& nodesToNotRecurse, vcount_t vc, bool startRemoving)
{
    ...
    for (size_t i = 0; i < cur->getNumChildren(); i++) {
        recursivelyDecrementIncrementedNodesIfUnderRegister(
            cg, cur->getChild(i), _incNodesList, nodesToNotRecurse, vc, startRemoving);
    }
}

OMR::Z::MemoryReference::MemoryReference(
    TR::Node* rootLoadOrStore, TR::CodeGenerator* cg, bool canUseRX, TR_StorageReference* storageReference)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
    ...
    , _incrementedNodesList(cg->comp()->trMemory())
{
    ...
    self()->tryForceFolding(
        rootLoadOrStore, cg, storageReference, symRef, symbol, nodesAlreadyEvaluatedBeforeFoldingList);
    ...
    if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
        ...
        if ((symbol && symbol->isStatic()) && symRef && symRef->isFromLiteralPool()
            ...
            TR_ASSERT(
                cg->supportsOnDemandLiteralPool() == true, "May not be here with Literal Pool On Demand disabled\n");
...
OMR::Z::MemoryReference::MemoryReference(
    TR::Node* node, TR::SymbolReference* symRef, TR::CodeGenerator* cg, TR_StorageReference* storageReference)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
...
OMR::Z::MemoryReference::MemoryReference(
    TR::Register* br, int32_t disp, TR::SymbolReference* symRef, TR::CodeGenerator* cg)
    : _baseRegister(br)
    , _symbolReference(symRef)
    , _baseNode(NULL)
    , _indexRegister(NULL)
...
void OMR::Z::MemoryReference::populateAddTree(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    ArtificiallyInflateReferenceCountWhenNecessary(
        self(), "OMR::Z::MemoryReference::populateAddTree", nodesBefore, cg, &_incrementedNodesList);
}
...
bool OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator* cg, TR::Node* loadStore, TR::Node* topAdd)
{
    ...
    ArtificiallyInflateReferenceCountWhenNecessary(
        self(), "OMR::Z::MemoryReference::tryBaseIndexDispl", nodesBefore, cg, &_incrementedNodesList);
    return true;
}
...
bool OMR::Z::MemoryReference::ZeroBasePtr_EvaluateSubtree(
    TR::Node* subTree, TR::CodeGenerator* cg, OMR::Z::MemoryReference* mr)
{
...
void OMR::Z::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    ArtificiallyInflateReferenceCountWhenNecessary(
        self(), "OMR::Z::MemoryReference::populateMemoryReference", nodesBefore, cg, &_incrementedNodesList);
}
    ...
    TR::Register* tempTargetRegister, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    ...
    if (_offset > MINLONGDISP && _offset < MAXLONGDISP) {
        ...
        if (preced)
            ...
            generateRXInstruction(cg, op, node, tempTargetRegister, interimMemoryReference);
        generateS390ImmOp(
            cg, TR::InstOpCode::getAddOpCode(), node, tempTargetRegister, tempTargetRegister, (int64_t)_offset);
    }
 ...
 */
TR::Instruction* OMR::Z::MemoryReference::enforceSSFormatLimits(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    ...
    preced = self()->separateIndexRegister(
        node, cg, true, preced, forceDueToAlignmentBump); // enforce4KDisplacementLimit=true
    preced = self()->enforce4KDisplacementLimit(node, cg, preced, false, forceDueToAlignmentBump);
    return preced;
}

TR::Instruction* OMR::Z::MemoryReference::enforceRSLFormatLimits(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    return self()->enforceSSFormatLimits(node, cg, preced);
}

TR::Instruction* OMR::Z::MemoryReference::enforceVRXFormatLimits(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    bool forceDueToAlignmentBump = self()->alignmentBumpMayRequire4KFixup(node, cg);
    preced = self()->enforce4KDisplacementLimit(node, cg, preced, false, forceDueToAlignmentBump);
 ...
 * region).
 */
TR::Instruction* OMR::Z::MemoryReference::enforce4KDisplacementLimit(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced, bool forcePLXFixup, bool forceFixup)
{
...
TR::Instruction* OMR::Z::MemoryReference::markAndAdjustForLongDisplacementIfNeeded(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced, bool* testLongDisp)
{
    return NULL;
}
 ...
 */
TR::Instruction* OMR::Z::MemoryReference::enforceDisplacementLimit(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    if (self()->ignoreNegativeOffset())
        return preced;
    ...
    bool enforce4KDisplacementLimit, TR::Instruction* preced, bool forceDueToAlignmentBump)
{
    ...
    if (_indexRegister != NULL) {
        ...
        if (!self()->ignoreNegativeOffset() && enforce4KDisplacementLimit
            ...
            if (preced)
                preced = generateRXInstruction(
                    cg, TR::InstOpCode::LA, node, tempTargetRegister, interimMemoryReference, preced);
            else
                generateRXInstruction(cg, TR::InstOpCode::LA, node, tempTargetRegister, interimMemoryReference);
        }
...
void OMR::Z::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
        ...
        if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
            assignedBaseRegister = static_cast<TR::RealRegister*>(machine->assignBestRegisterSingle(
                _baseRegister, currentInstruction, NOBOOKKEEPING, ~TR::RealRegister::GPR0Mask));
    ...
    if (_indexRegister != NULL && _indexRegister->getRealRegister() == NULL) {
        ...
        assignedIndexRegister = static_cast<TR::RealRegister*>(machine->assignBestRegisterSingle(
            _indexRegister, currentInstruction, BOOKKEEPING, ~TR::RealRegister::GPR0Mask));
...
bool OMR::Z::MemoryReference::needsAdjustDisp(
    TR::Instruction* instr, OMR::Z::MemoryReference* mRef, TR::CodeGenerator* cg)
{
    ...
    bool isValidSym = comp->getSymRefTab()->isNonHelper(
                          symRef->getReferenceNumber(), TR::SymbolReferenceTable::contiguousArraySizeSymbol)
        || comp->getSymRefTab()->isNonHelper(
            symRef->getReferenceNumber(), TR::SymbolReferenceTable::discontiguousArraySizeSymbol)
...
int32_t OMR::Z::MemoryReference::estimateBinaryLength(
    int32_t currentEstimate, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    uint8_t length = 0;
    TR::Compilation* comp = cg->comp();
...
int32_t OMR::Z::MemoryReference::calcDisplacement(uint8_t* cursor, TR::Instruction* instr, TR::CodeGenerator* cg)
{
    ...
    if (self()->rightAlignMemRef()) {
        TR_ASSERT(
            !self()->leftAlignMemRef(), "A memory reference should not be marked as both right and left aligned\n");
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
    ...
    if (self()->leftAlignMemRef()) {
        TR_ASSERT(
            !self()->rightAlignMemRef(), "A memory reference should not be marked as both left and right aligned\n");
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
...
static TR::Instruction* generateImmToRegister(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* targetRegister, intptr_t value, TR::Instruction* cursor)
{
    ...
    if (value >= (int32_t)0xFFFF8000 && value <= (int32_t)0x00007FFF) {
        ...
        if (cg->comp()->target().is64Bit()) {
            cursor = generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, targetRegister, targetRegister, 12, cursor); // SLLG Rscrtch,Rscrtch,12
        } else {
        ...
        if (cg->comp()->target().is64Bit()) {
            ...
            cursor = generateRSInstruction(
                cg, TR::InstOpCode::SLL, node, targetRegister, 12, cursor); // SLL Rscrtch,Rscrtch,12
        }
...
int32_t OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t* cursor, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    ...
    if (displacement < MINDISP || displacement >= MAXDISP) {
        ...
        if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
            ...
            if (longDisplacementMnemonic != TR::InstOpCode::bad) {
                TR::DebugCounter::incStaticDebugCounter(comp,
                    TR::DebugCounter::debugCounterName(
                        comp, "z/memref/long-displacement-upgrade/(%s)", comp->signature()));
            }
        } else {
            ...
            if (comp->getOption(TR_TraceCG))
                traceMsg(
                    comp, "[%p] Long Disp Inst using %s as scratch reg\n", instr, cg->getDebug()->getName(scratchReg));
        }
    }
    ...
    if (instructionFormat == TR::Instruction::IsRXY || instructionFormat == TR::Instruction::IsRXYb
        ...
        *reinterpret_cast<uint32_t*>(cursor + 4) |= boi((displacement & 0x000FF000) << 12);
        ...
        TR_ASSERT_FATAL(
            displacement >= MINDISP && displacement < MAXDISP, "Displacement (%d) is out of range", displacement);
    ...
    TR_ASSERT(
        offsetToLongDispSlot < MAXDISP, "Offset to long displacement slot (%d) is out of range", offsetToLongDispSlot);
...
int32_t OMR::Z::MemoryReference::generateBinaryEncodingTouchUpForLongDisp(
    uint8_t* cursor, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    int32_t nbytes = 0;
    TR::Compilation* comp = cg->comp();
    uint8_t* instructionStart = cursor;
...
TR::MemoryReference* OMR::Z::MemoryReference::create(TR::CodeGenerator* cg, TR::Node* node)
{
    ...
    TR_ASSERT_FATAL(
        node->getOpCode().hasSymbolReference(), "Memory reference generation API needs a node with symbol reference\n");
...
TR::MemoryReference* generateS390MemoryReference(
    int32_t iValue, TR::DataType type, TR::CodeGenerator* cg, TR::Register* treg, TR::Node* node)
{
...
TR::MemoryReference* generateS390MemoryReference(
    int64_t iValue, TR::DataType type, TR::CodeGenerator* cg, TR::Register* treg, TR::Node* node)
{
...
TR::MemoryReference* generateS390MemoryReference(
    double dValue, TR::DataType type, TR::CodeGenerator* cg, TR::Node* node)
{
    union {
        double dvalue;
        int64_t lvalue;
...
TR::MemoryReference* generateS390MemoryReference(
    TR::Register* br, int32_t disp, TR::CodeGenerator* cg, const char* name)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(br, disp, cg, name);
}

static TR::SymbolReference* findBestSymRefForArrayCopy(
    TR::CodeGenerator* cg, TR::Node* arrayCopyNode, TR::Node* srcNode)
{
 ...
 * pulled off the node
 */
static TR::MemoryReference* generateS390MemoryReference(
    TR::CodeGenerator* cg, TR::SymbolReference* symRefForAliasing, TR::Node* baseNode, int32_t disp)
{
 ...
 */
TR::MemoryReference* generateS390MemoryReference(
    TR::CodeGenerator* cg, TR::Node* node, TR::Node* baseNode, int32_t disp, bool forSS)
{
    TR::Compilation* comp = cg->comp();
...
TR::MemoryReference* generateS390ConstantAreaMemoryReference(
    TR::Register* br, int32_t disp, TR::Node* node, TR::CodeGenerator* cg, bool forSS)
{
    TR::Compilation* comp = cg->comp();
...
TR::MemoryReference* generateS390MemoryReference(
    TR::Register* br, TR::Register* ir, int32_t disp, TR::CodeGenerator* cg)
{
...
TR::MemoryReference* reuseS390MemoryReference(
    TR::MemoryReference* baseMR, int32_t offset, TR::Node* node, TR::CodeGenerator* cg, bool enforceSSLimits)
{
...
TR::MemoryReference* generateS390MemoryReference(
    TR::Snippet* tas, TR::CodeGenerator* cg, TR::Register* base, TR::Node* node)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(tas, cg, base, node);
}

TR::MemoryReference* generateS390MemoryReference(
    TR::Snippet* tas, TR::Register* indx, int32_t disp, TR::CodeGenerator* cg)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(tas, indx, disp, cg);
}

static TR::MemoryReference* generateS390AlignedMemoryReference(
    TR::MemoryReference& baseMR, TR::Node* node, int32_t offset, TR::CodeGenerator* cg, bool enforceSSLimits)
{
...
TR::MemoryReference* generateS390RightAlignedMemoryReference(
    TR::MemoryReference& baseMR, TR::Node* node, int32_t offset, TR::CodeGenerator* cg, bool enforceSSLimits)
{
    TR_ASSERT(!node->getType().isAggregate(), "do not use aligned memrefs for aggrs on node %p\n", node);
