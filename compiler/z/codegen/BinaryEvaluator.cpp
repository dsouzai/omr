    ...
    TR::RegisterDependencyConditions *dep = 0)
{
    ...
    generateRRInstruction(cg, TR::InstOpCode::LCR, node, targetRegisterPair->getHighOrder(),
        targetRegisterPair->getHighOrder());
    generateRRInstruction(cg, TR::InstOpCode::LCR, node, targetRegisterPair->getLowOrder(),
        targetRegisterPair->getLowOrder());
    ...
    TR::RegisterDependencyConditions *dep = 0)
{
    ...
    // Do complements on reg pair
    generateRRInstruction(cg, TR::InstOpCode::LCGR, node, targetRegisterPair->getHighOrder(),
        targetRegisterPair->getHighOrder());
    generateRRInstruction(cg, TR::InstOpCode::LCGR, node, targetRegisterPair->getLowOrder(),
        targetRegisterPair->getLowOrder());
...
static bool genNullTestForCompressedPointers(TR::Node *node, TR::CodeGenerator *cg, TR::Register *targetRegister,
         ...
         !node->isNonZero() */ )
      {
        ...
        if (!isNonZero) {
            ...
            if (addOrSubNode->getFirstChild()->getOpCode().isShift() && addOrSubNode->getFirstChild()->getRegister()) {
                ...
                generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
                generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, r, 0,
                    TR::InstOpCode::COND_BE, skipAdd);
            } else {
                generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
                generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, targetRegister, 0,
                    TR::InstOpCode::COND_BE, skipAdd);
            }
...
TR::Register *laddHelper64(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
        ...
        TR_S390BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, TR::InstOpCode::ALGR, TR::InstOpCode::ALG,
            TR::InstOpCode::LGR);
 ...
 */
static TR::Instruction *findPreviousAGIDef(TR::Instruction *prev, uint32_t searchLimit, TR::Register *useA,
    TR::Compilation *comp)
{
    bool trace = false;
...
TR::Register *generic32BitAddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        switch (secondChild->getDataType()) {
            ...
            default:
                TR_ASSERT_FATAL(false, "Unexpected data type (%s) in generic32BitAddEvaluator",
                    secondChild->getDataType().toString());
                break;
        }
        ...
        temp.integerAddAnalyser(node, TR::InstOpCode::AR, type == TR::Int16 ? TR::InstOpCode::AH : TR::InstOpCode::A,
            TR::InstOpCode::LR);
        targetRegister = node->getRegister();
    }
...
inline TR::Register *generic32BitSubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        switch (secondChild->getDataType()) {
            ...
            default:
                TR_ASSERT_FATAL(false, "Unexpected data type (%s) in generic32BitSubEvaluator",
                    secondChild->getDataType().toString());
                break;
        }
...
inline TR::Register *lDivRemGenericEvaluator64(TR::Node *node, TR::CodeGenerator *cg, bool isDivision)
{
        ...
        // case A/(+-)2^x - division is done using shifts
            ...
            if (isDivision) {
                ...
                if (!isUnsigned && !firstChild->isNonNegative()) {
                    ...
                    generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
                    cFlowRegionStart->setStartInternalControlFlow();
                    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, firstRegister, 0,
                        TR::InstOpCode::COND_BNL, skipSet);

                    // adjustment to dividend if dividend is negative
                    TR::RegisterDependencyConditions *deps
                        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 2, cg);
                    deps->addPostCondition(firstRegister, TR::RealRegister::AssignAny);
                    generateS390ImmOp(cg, TR::InstOpCode::AG, node, firstRegister, firstRegister,
                        absValueOfDenominator - 1, deps);
            ...
            } else // remainder
            {
                ...
                if (!isUnsigned && !firstChild->isNonNegative()) {
                    ...
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegister2,
                        static_cast<int32_t>(absValueOfDenominator - 1));
    ...
    if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned && !firstChild->isNonNegative()
        ...
        generateS390ImmOp(cg, TR::InstOpCode::CG, node, quoRegister, quoRegister,
            (int64_t)CONSTANT64(0x8000000000000000));
    ...
    if (doConditionalRemainder) {
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK4, node,
            skipDiv); // branch to done on <
    }
 ...
 *               = false we return the remainder register.
 */
TR::Register *iDivRemGenericEvaluator(TR::Node *node, TR::CodeGenerator *cg, bool isDivision,
    TR::MemoryReference *divchkDivisorMR)
{
    ...
    if (divchkDivisorMR && !needCheck) {
        ...
        if (divchkDivisorMR) {
            ...
            generateRXInstruction(cg, dtype.isInt64() ? TR::InstOpCode::LG : TR::InstOpCode::L, secondChild,
    ...
    if (needCheck) {
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node, sourceRegister, -1,
            TR::InstOpCode::COND_BNE, doDiv);
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lshl) {
            if (firstChild->getOpCodeValue() == TR::i2l && firstChild->getReferenceCount() == 1
                ...
                if (trgReg = TR::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(firstChild, cg, value,
...
inline TR::Register *genericIntShift(TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic shiftOp,
    TR::InstOpCode::Mnemonic altShiftOp)
{
    TR::Node *secondChild = node->getSecondChild();
    TR::Node *firstChild = node->getFirstChild();
    TR::Register *trgReg = NULL;
    ...
    if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift())
        ...
        if (popCnt == (64 - lZeros - tZeros)) {
            ...
            if (msBit > lsBit) {
                ...
                generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetReg, sourceReg, msBit, 0x80 + lsBit,
                    shiftAmnt);
            } else {
                auto mnemonic = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12) ? TR::InstOpCode::RISBGN
                                                                                             : TR::InstOpCode::RISBG;
...
TR::Register *OMR::Z::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(TR::Node *node, TR::CodeGenerator *cg,
    int32_t shiftAmount, bool isSignedShift)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    TR::Compilation *comp = cg->comp();
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::dualMulHelper64(TR::Node *node, TR::Node *lmulNode, TR::Node *lumulhNode,
    TR::CodeGenerator *cg)
{
...
TR::Register *OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, sourceRegister, 0,
            TR::InstOpCode::COND_BNL, posMulh);
        ...
        // MLG
        generateS390ImmOp(cg, TR::InstOpCode::MLG, node, targetRegisterPair, targetRegisterPair, absValue,
            dependencies);
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (!isUnsigned && (value == -1)) {
            ...
            if (!isUnsigned && !firstChild->isNonNegative()) {
                ...
                generateShiftThenKeepSelected31Bit(node, cg, targetRegister, targetRegister, 0x3f & (32 - shftAmnt), 31,
                    0);
            }
            generateS390LabelInstruction(cg, TR::InstOpCode::label, node, done, deps);
        } else {
...
TR::Register *OMR::Z::TreeEvaluator::landEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(cg, TR::InstOpCode::getAndOpCode(), node, targetRegister, targetRegister,
...
TR::Register *OMR::Z::TreeEvaluator::lorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(cg, TR::InstOpCode::getOrOpCode(), node, targetRegister, targetRegister,
            secondChild->getLongInt());
...
TR::Register *OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(cg, TR::InstOpCode::getXOROpCode(), node, targetRegister, targetRegister,
            secondChild->getLongInt());
