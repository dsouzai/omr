 ...
 */
TR::RegisterPair* lnegFor32Bit(TR::Node* node, TR::CodeGenerator* cg, TR::RegisterPair* targetRegisterPair,
    TR::RegisterDependencyConditions* dep = 0)
{
    ...
    generateRRInstruction(
        cg, TR::InstOpCode::LCR, node, targetRegisterPair->getHighOrder(), targetRegisterPair->getHighOrder());
    generateRRInstruction(
        cg, TR::InstOpCode::LCR, node, targetRegisterPair->getLowOrder(), targetRegisterPair->getLowOrder());
 ...
 */
TR::RegisterPair* lnegFor128Bit(TR::Node* node, TR::CodeGenerator* cg, TR::RegisterPair* targetRegisterPair,
    TR::RegisterDependencyConditions* dep = 0)
{
    TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
    TR::LabelSymbol* cFlowRegionEnd = generateLabelSymbol(cg);
    ...
    // Do complements on reg pair
    generateRRInstruction(
        cg, TR::InstOpCode::LCGR, node, targetRegisterPair->getHighOrder(), targetRegisterPair->getHighOrder());
    generateRRInstruction(
        cg, TR::InstOpCode::LCGR, node, targetRegisterPair->getLowOrder(), targetRegisterPair->getLowOrder());
 ...
 */
inline TR::RegisterPair* laddConst(TR::Node* node, TR::CodeGenerator* cg, TR::RegisterPair* targetRegisterPair,
    int64_t value, TR::RegisterDependencyConditions* deps)
{
...
static bool genNullTestForCompressedPointers(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* targetRegister, TR::LabelSymbol*& skipAdd)
{
    TR::Compilation* comp = cg->comp();
    ...
    if (cg->comp()->target().is64Bit() &&
         ...
         !node->isNonZero() */ )
      {
        ...
        if ((n->getOpCodeValue() == TR::lshl) || (n->getOpCodeValue() == TR::lushr))
            n = n->getFirstChild();
        ...
        if (!isNonZero) {
            ...
            if (addOrSubNode->getFirstChild()->getOpCode().isShift() && addOrSubNode->getFirstChild()->getRegister()) {
                ...
                generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::getCmpOpCode(), node, r, 0, TR::InstOpCode::COND_BE, skipAdd);
            } else {
                generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
                generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::getCmpOpCode(), node, targetRegister, 0, TR::InstOpCode::COND_BE, skipAdd);
            }
 ...
 */
TR::Register* laddHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    ...
    bool isCompressionSequence
        = cg->comp()->target().is64Bit() && comp->useCompressedPointers() && node->containsCompressionSequence();
    ...
    if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
        ...
        TR_S390BinaryCommutativeAnalyser(cg).integerAddAnalyser(
            node, TR::InstOpCode::ALGR, TR::InstOpCode::ALG, TR::InstOpCode::LGR);
        targetRegister = node->getRegister();
        return targetRegister;
    }

    if (isCompressionSequence && curTreeNode->getOpCode().isNullCheck()
    ...
    if (isCompressionSequence) {
        if (isCompressionSequence && (secondChild->getReferenceCount() > 1))
    ...
    if (hasCompressedPointers
        && ((secondChild->getOpCodeValue() != TR::lconst) || (secondChild->getRegister() != NULL))) {
        secondRegister = cg->evaluate(secondChild);
        ...
        if (firstChild->getReferenceCount() > 1 && firstChild->getRegister()
...
{
    return (op.isStore() || op.isLoad() || op.isBranchOp() || op.getOpCodeValue() == TR::InstOpCode::CHI
        || op.getOpCodeValue() == TR::InstOpCode::CLR || op.getOpCodeValue() == TR::InstOpCode::CLGR
        || op.getOpCodeValue() == TR::InstOpCode::CHI || op.getOpCodeValue() == TR::InstOpCode::CGHI);
}
 ...
 */
static TR::Instruction* findPreviousAGIDef(
    TR::Instruction* prev, uint32_t searchLimit, TR::Register* useA, TR::Compilation* comp)
{
    bool trace = false;
 ...
 */
TR::Register* generic32BitAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::Register* targetRegister = NULL;
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        default:
            TR_ASSERT_FATAL(
                false, "Unexpected data type (%s) in generic32BitAddEvaluator", secondChild->getDataType().toString());
            break;
        }
        ...
        temp.integerAddAnalyser(
            node, TR::InstOpCode::AR, type == TR::Int16 ? TR::InstOpCode::AH : TR::InstOpCode::A, TR::InstOpCode::LR);
        targetRegister = node->getRegister();
    }
 ...
 * This generic subtrac evaluator is invoked from different evaluator functions and handles all subtract types shorter
 * than an int (i.e. byte-bsub, short-ssub, and int-isub)
 *
 * It does not handle long type.
 */
inline TR::Register* generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        default:
            TR_ASSERT_FATAL(
                false, "Unexpected data type (%s) in generic32BitSubEvaluator", secondChild->getDataType().toString());
            break;
        }

        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196) && firstChild->getRegister()
 ...
 */
inline TR::Register* lDivRemGenericEvaluator64(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
    ...
    if (handleConstDenominator) {
        ...
        if (denominator == -1) {
            ...
            if (!isDivision) // remainder
            {
                ...
                generateS390ImmOp(cg, TR::InstOpCode::CG, node, firstRegister, firstRegister,
                    (int64_t)CONSTANT64(0x8000000000000000));
        ...
        // case A/(+-)2^x - division is done using shifts
            ...
            if (isDivision) {
                ...
                // This adjustment only applies to signed division as we are effectively trying to skirt the sign bit
                // during the shift operation below
                if (!isUnsigned && !firstChild->isNonNegative()) {
                    ...
                    cFlowRegionStart->setStartInternalControlFlow();
                    generateS390CompareAndBranchInstruction(
                        cg, TR::InstOpCode::CG, node, firstRegister, 0, TR::InstOpCode::COND_BNL, skipSet);
                    ...
                    deps->addPostCondition(firstRegister, TR::RealRegister::AssignAny);
                    generateS390ImmOp(
                        cg, TR::InstOpCode::AG, node, firstRegister, firstRegister, absValueOfDenominator - 1, deps);
            ...
            } else // remainder
            {
                ...
                if (!isUnsigned && !firstChild->isNonNegative()) {
                    ...
                    generateRILInstruction(cg, TR::InstOpCode::NIHF, node, tempRegister2,
                        static_cast<int32_t>((absValueOfDenominator - 1) >> 32));
                    generateRILInstruction(
                        cg, TR::InstOpCode::NILF, node, tempRegister2, static_cast<int32_t>(absValueOfDenominator - 1));
                    ...
                    if (denominator != static_cast<int32_t>(absValueOfDenominator))
                        generateRILInstruction(cg, TR::InstOpCode::NIHF, node, firstRegister,
                            static_cast<int32_t>((CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenominator + 1) >> 32));
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, firstRegister,
                        static_cast<int32_t>(CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenominator + 1));
    ...
    TR::RegisterDependencyConditions* dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, numPostConditions, cg);
    ...
    if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned && !firstChild->isNonNegative()
        ...
        generateS390ImmOp(
            cg, TR::InstOpCode::CG, node, quoRegister, quoRegister, (int64_t)CONSTANT64(0x8000000000000000));
    ...
    if (doConditionalRemainder) {
        TR_ASSERT(secondChild->getOpCode().isLoadConst() && secondChild->getType().isInt64(),
            "secondChild %s (%p) of rem should be int64 constant\n", secondChild->getOpCode().getName(), secondChild);
        if (skipDiv == NULL)
            skipDiv = generateLabelSymbol(cg);
        ...
        if (firstChild->isNonNegative()) {
            ...
            // LGPR is needed so negative numbers do not always take the slow path after the logical compare (but
            // functionally it is not needed)
        ...
        generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK4, node, skipDiv); // branch to done on <
    }

    // Do division
    if (isUnsigned) {
        if (doConditionalRemainder)
            generateRRInstruction(cg, TR::InstOpCode::LGR, node, quoRegister,
                remRegister); // in doConditionalRemainder case this is the slow path so do move here
 ...
 *               = false we return the remainder register.
 */
TR::Register* iDivRemGenericEvaluator(
    TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR)
{
    ...
    // Note:  D and DR require that R1=0 before executing the division
    // TODO(#3685): Remove this Java-ism from OMR and push it into OpenJ9
    bool needCheck = (!node->isSimpleDivCheck() && !node->getOpCode().isUnsigned() && !firstChild->isNonNegative()
        && !secondChild->isNonNegative());
    ...
    if (divchkDivisorMR && !needCheck) {
        ...
        && secondChild->getOpCode().isMemoryReference() && !needCheck) {
        ...
        if (divchkDivisorMR) {
            ...
            generateRXInstruction(
                cg, dtype.isInt64() ? TR::InstOpCode::LG : TR::InstOpCode::L, secondChild, sourceRegister, tempMR);
    ...
    TR::RegisterDependencyConditions* dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, sourceMR ? 6 : 4, cg);
    dependencies->addPostCondition(targetRegisterPair, TR::RealRegister::EvenOddPair);
    dependencies->addPostCondition(remRegister, TR::RealRegister::LegalEvenOfPair);
    dependencies->addPostCondition(quoRegister, TR::RealRegister::LegalOddOfPair);
    if (sourceRegister)
        dependencies->addPostConditionIfNotAlreadyInserted(sourceRegister, TR::RealRegister::AssignAny);
    ...
    if (needCheck) {
        ...
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::C, node, sourceRegister, -1, TR::InstOpCode::COND_BNE, doDiv);
        cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node, quoRegister, remRegister);
...
inline TR::Register* genericLongShiftSingle(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    TR::Node* secondChild = node->getSecondChild();
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lshl) {
            if (firstChild->getOpCodeValue() == TR::i2l && firstChild->getReferenceCount() == 1
                ...
                auto mnemonic = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12) ? TR::InstOpCode::RISBGN
                                                                                             : TR::InstOpCode::RISBG;

                generateRIEInstruction(cg, mnemonic, node, trgReg, srcReg, (int8_t)(32 - value),
                    (int8_t)((63 - value) | 0x80), (int8_t)value);
                ...
                if (trgReg = TR::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
                        firstChild, cg, value, node->getOpCodeValue() == TR::lshl)) {
                    node->setRegister(trgReg);
                    cg->decReferenceCount(firstChild);
                    cg->decReferenceCount(secondChild);
                    return trgReg;
            ...
            if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1) {
                if (trgReg = TR::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
                        firstChild, cg, -value, node->getOpCodeValue() == TR::lshr)) {
                    node->setRegister(trgReg);
                    cg->decReferenceCount(firstChild);
                    cg->decReferenceCount(secondChild);
                    return trgReg;
        ...
        if ((shiftOp == TR::InstOpCode::SRLG || shiftOp == TR::InstOpCode::SLLG)
...
inline TR::Register* genericIntShift(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp, TR::InstOpCode::Mnemonic altShiftOp)
{
    TR::Node* secondChild = node->getSecondChild();
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* trgReg = NULL;
    ...
    if (node->getOpCodeValue() == TR::bushr) {
        if (node->getFirstChild()->getOpCodeValue() == TR::bconst && node->getReferenceCount() == 1
    ...
    if (!cg->canClobberNodesRegister(firstChild) || srcReg->containsCollectedReference()
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::laddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return laddHelper64(node, cg);
}

TR::Register* genericRotateLeft(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (node->getOpCodeValue() == TR::lor) {
        ...
        if (lushr && lushr->getOpCodeValue() == TR::lushr && lushr->getReferenceCount() == 1
            && lushr->getRegister() == NULL && lshl->getOpCodeValue() == TR::lshl && lshl->getReferenceCount() == 1
            && lshl->getRegister() == NULL) {
            ...
            if (rShftAmnt + lShftAmnt == 64 && lshlSourceNode == lushrSourceNode) {
                ...
                TR::InstOpCode::Mnemonic opCode = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12)
                    ? TR::InstOpCode::RISBGN
                    : TR::InstOpCode::RISBG;
    ...
    if (node->getOpCodeValue() == TR::lor && andChild && andChild->getRegister() == NULL
        ...
        if (shiftChild->getOpCodeValue() == TR::lshl && shiftChild->getRegister() == NULL
            && shiftChild->getReferenceCount() == 1 && shiftChild->getSecondChild()->getOpCode().isLoadConst()) {
            data = shiftChild->getFirstChild();
            shiftBy = shiftChild->getSecondChild()->getLongInt();
            mask1 = 1ULL << shiftBy;
            if (data->getOpCodeValue() == TR::i2l && data->getRegister() == NULL)
                data = data->getFirstChild();
            if (data->getOpCodeValue() != TR::icmpeq && data->getOpCodeValue() != TR::icmpne)
                mask1 = 0;
        }
        ...
        if (mask1 && mask1 == (~mask2) && shiftBy <= 63
            && performTransformation(cg->comp(), "O^O Insert bit using RISBG node [%p]\n", node)) {
            ...
            if (mask1 == mask2
                && performTransformation(cg->comp(), "O^O Insert contiguous bits using RISBG node [%p]\n", node)) {
    ...
    if (node->getOpCodeValue() == TR::lor || node->getOpCodeValue() == TR::lxor) {
        TR::Node* shiftChild = NULL;
        TR::Node* otherChild = NULL;
        if (firstChild->getOpCodeValue() == TR::lshl || firstChild->getOpCodeValue() == TR::lushr) {
            shiftChild = firstChild;
            otherChild = secondChild;
        } else if (secondChild->getOpCodeValue() == TR::lshl || secondChild->getOpCodeValue() == TR::lushr) {
            shiftChild = secondChild;
            otherChild = firstChild;
        }
        if (shiftChild && shiftChild->getSecondChild()->getOpCode().isLoadConst()
            && shiftChild->getReferenceCount() == 1 && shiftChild->getRegister() == NULL
 ...
 * Rotate and insert
 */
TR::Register* genericRotateAndInsertHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    //
    if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift())
        ...
        {
            if (firstChild->getFirstChild()->isNonNegative() || firstChild->chkUnsigned()
        ...
        if (popCnt == (64 - lZeros - tZeros)) {
            ...
            if (msBit > lsBit) {
                ...
                generateRIEInstruction(
                    cg, TR::InstOpCode::RISBLG, node, targetReg, sourceReg, msBit, 0x80 + lsBit, shiftAmnt);
            } else {
                auto mnemonic = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12) ? TR::InstOpCode::RISBGN
                                                                                             : TR::InstOpCode::RISBG;
...
TR::Register* OMR::Z::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
    TR::Node* node, TR::CodeGenerator* cg, int32_t shiftAmount, bool isSignedShift)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Compilation* comp = cg->comp();
    ...
    if (node->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst()) {
        ...
        // RISBG instructions perform unsigned rotation, so if we are selecting the sign bit via the logical AND we
        // cannot perform a signed shift because we must preserve the sign bit. In general we cannot handle this case
        // because we are not certain our input will be non-negative, hence we must conservatively disallow the
        // optimization in such a case.
        if (longConstValue >= 0x8000000000000000LL && shiftAmount != 0 && isSignedShift) {
            return NULL;
        }
        // The Shift amount operand in RISBG only holds 6 bits so the optimization will not work on shift amounts >63 or
        // < 63.
        if (shiftAmount > 63 || shiftAmount < -63) {
            return NULL;
        }
        // If the node entered is a common node, then there is no guarantee that the child of the common node will still
        // be alive. And when we try to access the value in that node, we might receive values from a node that might've
        // been killed already. Thus, the optimization will not work.
        ...
        if (popCnt == (64 - lZeros - tZeros)) {
            ...
            if (firstChild->getReferenceCount() > 1 || lZeros > 31 || tZeros > 31 || (lZeros > 0 && tZeros > 0)) {
                // If the shift amount is NOT zero, then we must account for the shifted range
                // [1] If the resultant shifted msb in a right shift is shifted "too far" to the right, then we are
                // basically zeroing the register. This optimization will not be used. [2] Same case if the resultant
                // lsb of a left shift is shifted out of range. Eg[1]: 000...0011 >> 3. msb = 62, lsb = 63. If we right
                // shift, then msb and lsb will both be > 63 (out of range) Eg[2]: 11100...00 << 10. msb = 0, lsb = 2.
                // If we left shift, then msb and lsb will both be < 0 (out of range)
        ...
        if (doTransformation && performTransformation(comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
            ...
            //
            // If the shift amount is NOT zero, then we must account for the shifted range
            // [1] If the shifted lsb of a left shift is still in range, but the msb is out of range, then we can still
            // "capture" the shifted bits by upperbounding the msb. [2] Similar case if the msb of a right shift is
            // still in range but the lsb is out of range.
            //
            // Eg[1]: 00...0111 >> 2. msb = 61, lsb = 63. If we right shift, we get 000...0001, so we can still preserve
            // the last bit. Eg[2]: 1110...00 << 2. msb = 0, lsb = 2. If we left left, we can still preserve the first
            // bit.
 ...
 */
TR::Register* lsubHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* targetRegister = NULL;
    ...
    if (cg->comp()->target().is64Bit() && comp->useCompressedPointers() && node->containsCompressionSequence())
    ...
    bool bumpedRefCount = false;
    if (isCompressionSequence && curTreeNode->getOpCode().isNullCheck() && (firstChild->getOpCodeValue() == TR::a2l)
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::dualMulHelper64(
    TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::dualMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
 ...
 */
TR::Register* lmulHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChildIsConstant) {
        ...
        if (firstChild->getRegister() != NULL && value == 2
...
TR::Register* OMR::Z::TreeEvaluator::baddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::saddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* lhsChild = node->getChild(0);
    TR::Node* rhsChild = node->getChild(1);
...
TR::Register* OMR::Z::TreeEvaluator::bsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* lhsChild = node->getChild(0);
    TR::Node* rhsChild = node->getChild(1);
...
TR::Register* OMR::Z::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* lhsChild = node->getChild(0);
    TR::Node* rhsChild = node->getChild(1);
 ...
 * (child1 - child2)
 */
TR::Register* OMR::Z::TreeEvaluator::isubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 - child2)
 */
TR::Register* OMR::Z::TreeEvaluator::lsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return lsubHelper64(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool needsUnsignedHighMulOnly = (node->getOpCodeValue() == TR::lumulh) && !node->isDualCyclic();
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CG, node, sourceRegister, 0, TR::InstOpCode::COND_BNL, posMulh);
        ...
        // MLG
        generateS390ImmOp(
            cg, TR::InstOpCode::MLG, node, targetRegisterPair, targetRegisterPair, absValue, dependencies);
 ...
 *
 */
TR::Register* OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::bmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* lhsChild = node->getChild(0);
    TR::Node* rhsChild = node->getChild(1);
...
TR::Register* OMR::Z::TreeEvaluator::smulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* lhsChild = node->getChild(0);
    TR::Node* rhsChild = node->getChild(1);
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* secondChild = node->getSecondChild();
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (firstChild->getOpCodeValue() == TR::s2i && firstChild->getFirstChild()->getOpCodeValue() == TR::sloadi
        ...
        regNode = secondChild;
    } else if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi
        && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        bool create_LA = false;
        if ((firstChild->getRegister() != NULL) && (value == 2)
            && (node->isNonNegative() && cg->comp()->target().is64Bit())) {
            create_LA = true;
        }
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool needsUnsignedHighMulOnly = (node->getOpCodeValue() == TR::lumulh) && !node->isDualCyclic();
    if (node->isDualCyclic() || needsUnsignedHighMulOnly) {
        return TR::TreeEvaluator::dualMulEvaluator(node, cg);
 ...
 * (child1 / child2)
 */
TR::Register* OMR::Z::TreeEvaluator::idivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* targetRegister = NULL;
 ...
 * (child1 / child2)
 */
TR::Register* OMR::Z::TreeEvaluator::ldivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 % child2)
 */
TR::Register* OMR::Z::TreeEvaluator::iremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* targetRegister = NULL;
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (!isUnsigned && (value == -1)) {
            ...
            if (!isUnsigned && !firstChild->isNonNegative()) {
                ...
                generateShiftThenKeepSelected31Bit(
                    node, cg, targetRegister, targetRegister, 0x3f & (32 - shftAmnt), 31, 0);
            }
            generateS390LabelInstruction(cg, TR::InstOpCode::label, node, done, deps);
        } else {
            // Division with no exception check
 ...
 * (child1 % child2)
 */
TR::Register* OMR::Z::TreeEvaluator::lremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::inegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (firstChild->getOpCodeValue() == TR::iabs && firstChild->getReferenceCount() == 1
        && firstChild->getRegister() == NULL) {
    ...
    } else if (firstChild->getOpCodeValue() == TR::imul && firstChild->getSecondChild()->getOpCode().isLoadConst()
        && firstChild->getSecondChild()->getInt() != 0x80000000 && firstChild->getReferenceCount() == 1
        && firstChild->getRegister() == NULL
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* targetRegister = NULL;
    TR::Register* sourceRegister;
    ...
    if (firstChild->getOpCodeValue() == TR::labs && firstChild->getReferenceCount() == 1
        && firstChild->getRegister() == NULL) {
        // Load Negative
        sourceRegister = firstChild->getFirstChild()->getRegister();
        if (sourceRegister == NULL)
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* sourceRegister = cg->evaluate(firstChild);
    TR::Register* targetRegister = cg->allocateRegister();
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::snegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* sourceRegister = cg->evaluate(firstChild);
    TR::Register* targetRegister = cg->allocateRegister();
 ...
 * (child1 << child2)
 */
TR::Register* OMR::Z::TreeEvaluator::ishlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 << child2)
 */
TR::Register* OMR::Z::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 << child2)
 */
TR::Register* OMR::Z::TreeEvaluator::bshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    auto altShiftOp = TR::InstOpCode::SLLG;
 ...
 * (child1 << child2)
 */
TR::Register* OMR::Z::TreeEvaluator::sshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    auto altShiftOp = TR::InstOpCode::SLLG;
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::lshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::bshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::InstOpCode::Mnemonic altShiftOp = TR::InstOpCode::SRAG;
    return genericIntShift(node, cg, TR::InstOpCode::SRA, altShiftOp);
}
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::sshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::InstOpCode::Mnemonic altShiftOp = TR::InstOpCode::SRAG;
    return genericIntShift(node, cg, TR::InstOpCode::SRA, altShiftOp);
}
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::lushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::bushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    auto altShiftOp = TR::InstOpCode::SRLG;
 ...
 * (child1 >> child2)
 */
TR::Register* OMR::Z::TreeEvaluator::sushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    auto altShiftOp = TR::InstOpCode::SRLG;
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::integerRolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sourceRegister = NULL;
    TR::Register* targetRegister = NULL;
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (rotateAmount == 0) {
            ...
            generateRSInstruction(cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL, node, targetRegister,
                sourceRegister, rotateAmount);
        }
    } else {
        sourceRegister = cg->evaluate(firstChild);
        targetRegister = nodeIs64Bit ? cg->allocateRegister() : cg->allocateRegister();
        ...
        generateRSInstruction(
            cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL, node, targetRegister, sourceRegister, memRef);
    }
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(
            cg, TR::InstOpCode::getAndOpCode(), node, targetRegister, targetRegister, secondChild->getLongInt());
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::iandEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::candEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::iandEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(
            cg, TR::InstOpCode::getOrOpCode(), node, targetRegister, targetRegister, secondChild->getLongInt());
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::borEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::iorEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::corEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::iorEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        generateS390ImmOp(
            cg, TR::InstOpCode::getXOROpCode(), node, targetRegister, targetRegister, secondChild->getLongInt());
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ixorEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::cxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ixorEvaluator(node, cg);
}
