...
#define TR_INTEGER_TO_PACKED_SIZE \
    8 ///< The result byte size in memory for a integer to packed conversion (i.e. with CVD)
...
#define TR_PACKED_TO_INTEGER_SIZE \
    8 ///< The source byte size in memory for a packed to integer conversion (i.e. with CVB)
#define TR_ASCII_TO_PACKED_RESULT_SIZE \
    16 ///< The result byte size in memory for an ascii to packed conversion (i.e. with PKA)
#define TR_UNICODE_TO_PACKED_RESULT_SIZE \
    16 ///< The result byte size in memory for a unicode to packed conversion (i.e. with PKU)
#define TR_PACKED_TO_UNICODE_SOURCE_SIZE \
    16 ///< The source byte size in memory for a packed to unicode conversion (i.e. with UNPKU)
...
public:
     ...
     */
    TR::RealRegister* getRealRegister(int32_t regNum) { return _registerFile[regNum]; }
    ...
    TR::Register* assignBestRegister(TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping,
        uint64_t availRegMask = 0xffffffff);

    // EVENODD PAIR REGISTERS methods
    TR::Register* assignBestRegisterPair(
        TR::Register* regPair, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask = 0x0000ffff);
    ...
    bool isLegalEvenRegister(
        TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask = 0x0000ffff, bool allowLocked = false);
    bool isLegalOddRegister(
        TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask = 0x0000ffff, bool allowLocked = false);
    ...
    bool isLegalFirstOfFPRegister(
        TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask = 0x0000ffff, bool allowLocked = false);
    bool isLegalSecondOfFPRegister(
        TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask = 0x0000ffff, bool allowLocked = false);
    ...
    void freeBestRegisterPair(TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR_RegisterKinds rk,
        TR::Instruction* currInst, uint64_t availRegMask = 0x0000ffff);
    void freeBestFPRegisterPair(TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR::Instruction* currInst,
    ...
    TR::Register* assignBestRegisterSingle(TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping,
        uint64_t availRegMask = 0xffffffff);

    TR::RealRegister* findBestFreeRegister(TR::Instruction* currentInstruction, TR_RegisterKinds rk,
        TR::Register* virtualReg = NULL, uint64_t availRegMask = 0xffffffff);

    TR::RealRegister* freeBestRegister(
        TR::Instruction* currentInstruction, TR::Register* virtReg, TR_RegisterKinds rk, bool allowNullReturn = false);

    TR::RealRegister* findBestRegisterForShuffle(
        TR::Instruction* currentInstruction, TR::Register* currentAssignedRegister, uint64_t availRegMask = 0xffffffff);

    TR::RealRegister* shuffleOrSpillRegister(TR::Instruction* currInst, TR::Register* toFreeReg, uint64_t availRegMask);

    void spillRegister(TR::Instruction* currentInstruction, TR::Register* virtReg);

    TR::RealRegister* reverseSpillState(
        TR::Instruction* currentInstruction, TR::Register* spilledRegister, TR::RealRegister* targetRegister = NULL);

    bool isAssignable(TR::Register* virtReg, TR::RealRegister* realReg);

    TR::Instruction* coerceRegisterAssignment(
    ...
    //  Tactical GRA stuff ///////////////////////////////////
    uint32_t* getGlobalRegisterTable() { return _globalRegisterNumberToRealRegisterMap; }

    TR_GlobalRegisterNumber getLastGlobalGPRRegisterNumber() { return _lastGlobalGPRRegisterNumber; }

    TR_GlobalRegisterNumber setLastGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg);

    TR_GlobalRegisterNumber getFirstGlobalGPRRegisterNumber() { return _firstGlobalGPRRegisterNumber; }

    TR_GlobalRegisterNumber setFirstGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg);

    TR_GlobalRegisterNumber getLast8BitGlobalGPRRegisterNumber() { return _last8BitGlobalGPRRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getFirstGlobalFPRRegisterNumber() { return _firstGlobalFPRRegisterNumber; }

    TR_GlobalRegisterNumber setFirstGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg);

    TR_GlobalRegisterNumber getLastGlobalFPRRegisterNumber() { return _lastGlobalFPRRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getFirstGlobalVRFRegisterNumber() { return _firstGlobalVRFRegisterNumber; }

    TR_GlobalRegisterNumber setFirstGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg);

    TR_GlobalRegisterNumber getLastGlobalVRFRegisterNumber() { return _lastGlobalVRFRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getGlobalEnvironmentRegisterNumber() { return _globalEnvironmentRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getGlobalCAARegisterNumber() { return _globalCAARegisterNumber; }
    ...
    TR_GlobalRegisterNumber getGlobalParentDSARegisterNumber() { return _globalParentDSARegisterNumber; }
    ...
    TR_GlobalRegisterNumber getGlobalReturnAddressRegisterNumber() { return _globalReturnAddressRegisterNumber; }
    ...
    TR_GlobalRegisterNumber getGlobalEntryPointRegisterNumber() { return _globalEntryPointRegisterNumber; }

    TR_GlobalRegisterNumber getLastGlobalCCRRegisterNumber() { return _lastGlobalCCRRegisterNumber; }

    TR_GlobalRegisterNumber setLastGlobalCCRRegisterNumber(TR_GlobalRegisterNumber reg);

    TR_GlobalRegisterNumber getLastVolatileNonLinkGPR() { return _lastVolatileNonLinkGPR; }
    ...
    TR_GlobalRegisterNumber getLastLinkageGPR() { return _lastLinkageGPR; }

    TR_GlobalRegisterNumber setLastLinkageGPR(TR_GlobalRegisterNumber reg) { return _lastLinkageGPR = reg; }

    TR_GlobalRegisterNumber getLastVolatileNonLinkFPR() { return _lastVolatileNonLinkFPR; }
    TR_GlobalRegisterNumber setLastVolatileNonLinkFPR(TR_GlobalRegisterNumber reg)
    {
        return _lastVolatileNonLinkFPR = reg;
    }

    TR_GlobalRegisterNumber getLastLinkageFPR() { return _lastLinkageFPR; }

    TR_GlobalRegisterNumber setLastLinkageFPR(TR_GlobalRegisterNumber reg) { return _lastLinkageFPR = reg; }
