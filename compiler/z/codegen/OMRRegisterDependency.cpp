...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::CodeGenerator* cg, TR::Node* node, uint32_t extranum, TR::Instruction** cursorPtr)
{
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        resolveSplitDependencies(
            cg, node, child, regPair, regList, reg, highReg, copyReg, highCopyReg, iCursor, regNum);
...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::RegisterDependencyConditions* iConds, uint16_t numNewPreConds, uint16_t numNewPostConds, TR::CodeGenerator* cg)
    : _preConditions(
        new ((iConds ? iConds->getNumPreConditions() : 0) + numNewPreConds, cg->trMemory()) TR::RegisterDependencyGroup)
...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::RegisterDependencyConditions* conds_1, TR::RegisterDependencyConditions* conds_2, TR::CodeGenerator* cg)
...
TR::RegisterDependencyConditions* OMR::Z::RegisterDependencyConditions::clone(
    TR::CodeGenerator* cg, int32_t additionalRegDeps)
{
...
void OMR::Z::RegisterDependencyConditions::assignPreConditionRegisters(
...
void OMR::Z::RegisterDependencyConditions::assignPostConditionRegisters(
    TR::Instruction* currentInstruction, TR_RegisterKinds kindToBeAssigned, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::RegisterDependencyConditions::searchPreConditionRegister(
...
TR::Register* OMR::Z::RegisterDependencyConditions::searchPostConditionRegister(
    TR::RealRegister::RegNum rr, uint8_t flag)
{
...
void OMR::Z::RegisterDependencyConditions::stopUsingDepRegs(
 ...
 * assignment.
 */
void OMR::Z::RegisterDependencyConditions::bookKeepingRegisterUses(
    TR::Instruction* instr, TR::CodeGenerator* cg, int32_t oldPreCursor, int32_t oldPostCursor)
{
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (regDep.isEvenOddPair() || regDep.isFPPair()) {
         ...
         fprintf(stderr, "DEP: PRE REG PAIR ASS: %p(%p,%p)->(%p,%p)\n", virtRegPair, virtRegPair->getHighOrder(),
            ...
            //
            retPair = (TR::RegisterPair*)machine->assignBestRegister(
                (TR::Register*)virtRegPair, currentInstruction, NOBOOKKEEPING, availRegMask);
 ...
 */
void OMR::Z::RegisterDependencyConditions::createRegisterAssociationDirective(
    TR::Instruction* instruction, TR::CodeGenerator* cg)
{
    TR::Machine* machine = cg->machine();
...
bool OMR::Z::RegisterDependencyConditions::doesConditionExist(
    TR::RegisterDependencyGroup* regDepArr, TR::RegisterDependency* depToCheck, uint32_t numberOfRegisters)
{
...
bool OMR::Z::RegisterDependencyConditions::addPreConditionIfNotAlreadyInserted(
 ...
 */
bool OMR::Z::RegisterDependencyConditions::addPreConditionIfNotAlreadyInserted(
    TR::Register* vr, TR::RealRegister::RegDep rr, uint8_t flag)
{
    int32_t pos = -1;
    if (_preConditions != NULL
...
bool OMR::Z::RegisterDependencyConditions::addPostConditionIfNotAlreadyInserted(
    TR::Register* vr, TR::RealRegister::RegNum rr, uint8_t flag)
{
    return addPostConditionIfNotAlreadyInserted(vr, static_cast<TR::RealRegister::RegDep>(rr), flag);
}
