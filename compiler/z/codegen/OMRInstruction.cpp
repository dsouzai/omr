...
#define CTOR_INITIALIZER_LIST                                                                                          \
    _conditions(NULL), _operands(NULL), _useRegs(NULL), _defRegs(NULL), _sourceUsedInMemoryReference(NULL), _flags(0), \
        _longDispSpillReg1(NULL), _longDispSpillReg2(NULL), _binFreeRegs(0), _targetRegSize(0), _sourceRegSize(0),     \
        _sourceMemSize(0), _targetMemSize(0), _sourceStart(-1), _targetStart(-1)
...
{
    TR_ASSERT_FATAL(cg->comp()->target().cpu.isAtLeast(_opcode.getMinimumALS()),
        "Processor detected (%s) does not support instruction %s\n", cg->comp()->target().cpu.getProcessorName(),
...
TR::RegisterDependencyConditions* OMR::Z::Instruction::setDependencyConditionsNoBookKeeping(
...
TR::RegisterDependencyConditions* OMR::Z::Instruction::setDependencyConditions(TR::RegisterDependencyConditions* cond)
{
    ...
    if (cond->getPostConditions())
        cond->getPostConditions()->incNumUses();

    return _conditions = cond;
}

TR::RegisterDependencyConditions* OMR::Z::Instruction::updateDependencyConditions(
    TR::RegisterDependencyConditions* cond)
{
...
void OMR::Z::Instruction::initialize(
    TR::Instruction* precedingInstruction, bool instFlag, TR::RegisterDependencyConditions* cond, bool condFlag)
{
    ...
    if (cg->getCondCodeShouldBePreserved())
        TR_ASSERT(
            (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() || _opcode.setsSignFlag() || _opcode.setsCarryFlag())
                == false,
    ...
    if (cg->comp()->getOption(TR_EnableEBBCCInfo)) {
        ((self()->isLabel() || self()->isCall()) || (instFlag && precedingInstruction == NULL)) ? self()->clearCCInfo()
                                                                                                : self()->setCCInfo();
    } else {
        (!self()->isBranchOp() && (!instFlag || precedingInstruction == NULL)) ? self()->setCCInfo()
                                                                               : self()->clearCCInfo();
    }
}
...
uint32_t OMR::Z::Instruction::getEstimatedBinaryOffset()
{
    union {
        uint8_t* p;
        uint32_t i;
...
void OMR::Z::Instruction::setEstimatedBinaryOffset(uint32_t l)
{
    union {
        uint8_t* p;
        uint32_t i;
...
void OMR::Z::Instruction::setCCInfo()
{
    ...
    if (_opcode.setsCC()) {
        ...
        if (!self()->cg()->comp()->getOption(TR_EnableEBBCCInfo)) {
            ...
            // because compare immed ops looks for setsSignedFlag or setsZeroFlags - compare ops does not sets these
            // flags
            {
                // we have detailed information on the CC setting
                self()->cg()->setHasCCInfo(true);
...
bool OMR::Z::Instruction::matchesTargetRegister(TR::Register* reg)
{
    if (reg->getRealRegister()) {
        if (self()->implicitlySetsGPR0() && toRealRegister(reg)->getRegisterNumber() == TR::RealRegister::GPR0)
            return true;

        if (self()->implicitlySetsGPR1() && toRealRegister(reg)->getRegisterNumber() == TR::RealRegister::GPR1)
            return true;

        if (self()->implicitlySetsGPR2() && toRealRegister(reg)->getRegisterNumber() == TR::RealRegister::GPR2)
            return true;
    }
    return false;
}

bool OMR::Z::Instruction::matchesAnyRegister(TR::Register* reg, TR::Register* instReg)
{
    if (!instReg)
        return false;
...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register* reg, TR::Register* instReg1, TR::Register* instReg2)
{
    return self()->matchesAnyRegister(reg, instReg1) || self()->matchesAnyRegister(reg, instReg2);
}

TR::Register* OMR::Z::Instruction::getRegForBinaryEncoding(TR::Register* reg)
{
    return (reg->getRegisterPair()) ? reg->getRegisterPair()->getHighOrder() : reg;
}

void OMR::Z::Instruction::useRegister(TR::Register* reg, bool isDummy)
{
    ...
    if (regPair) {
        ...
        // S390RegisterDependency also calls useRegister, in this case we do not want to reset the dummy status of these
        // regs
        //
...
bool OMR::Z::Instruction::defsRegister(TR::Register* reg)
{
    TR::Register** _targetReg = self()->targetRegBase();
...
bool OMR::Z::Instruction::isDefRegister(TR::Register* reg)
{
...
bool OMR::Z::Instruction::usesRegister(TR::Register* reg)
{
    int32_t i;
...
TR::Instruction* OMR::Z::Instruction::getOutOfLineEXInstr()
{
...
TR::S390ImmInstruction* OMR::Z::Instruction::getS390ImmInstruction() { return NULL; }
#endif
...
uint32_t getGPRMaskFromRange(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT(lowRegNum >= minRegNum && lowRegNum <= maxRegNum && highRegNum >= minRegNum && highRegNum <= maxRegNum,
...
uint32_t getVRFMaskFromRange(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT_FATAL(lowRegNum <= highRegNum && (highRegNum - lowRegNum) < 16
        ...
        "%s to %s is an invalid vector range", lowReg->getRegisterName(cg->comp(), TR_VectorReg128),
        highReg->getRegisterName(cg->comp(), TR_VectorReg128));
...
static void handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    // wrap around for loop terminal index to minRegNum
    for (uint32_t i = lowRegNum; i != highRegNum; i = ((i == maxRegNum) ? minRegNum : i + 1)) {
        ...
        // avoid needlessly spilling them. (ex. LM(GPR00F,GPR14P,PSALCCAV->LCCAEMS0); ! PSALCCAV needs a reg and only
        // needed on LM)
        ...
        if (reg->getState() == TR::RealRegister::Assigned && inst != virtualReg->getEndOfRange()) {
            ...
            if (!found) {
                ...
                if (shuffle != NULL) {
                    // if the LM instruction uses this reg we just shuffled, it will have the wrong real reg, need to
                    // fix this up
...
void OMR::Z::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (self()->getOpCodeValue() != TR::InstOpCode::assocreg) {
        ...
        // if (kindToBeAssigned == TR_GPR)
        {
            ...
            // Step 1 : First traverse the existing associations and remove them so that they don't interfere with the
            // new ones
...
const char* OMR::Z::Instruction::getName(TR_Debug* debug)
{
    if (debug)
...
void OMR::Z::Instruction::recordOperand(void* operand, int8_t& operandCount)
{
    ...
    if (_operands == NULL) {
        ...
        for (int i = 0; i < n; i++)
            ((uintptr_t**)p)[i] = ((uintptr_t**)_operands)[i];
...
int OMR::Z::Instruction::gatherRegPairsAtFrontOfArray(TR::Register** regArr, int32_t regArrSize)
{
    int32_t pairCnt = 0;
...
TR::Register* OMR::Z::Instruction::assignRegisterNoDependencies(TR::Register* reg)
{
    ...
    if (reg->getRealRegister() != NULL) {
        ...
        if (!skipBookkeeping && realReg->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtReg
            && virtReg->decFutureUseCount() == 0) {
    ...
    if ((reg->getRegisterPair()
        ...
        if (realRegHigh->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegHigh
            && virtRegHigh->decFutureUseCount() == 0) {
        ...
        if (realRegLow->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegLow
            && virtRegLow->decFutureUseCount() == 0) {
...
//
bool OMR::Z::Instruction::anySpilledRegisters(TR::Register** listReg, int32_t listRegSize)
{
    int32_t i;
    ...
    for (i = 0; i < listRegSize; ++i) {
        ...
        if (reg->getRegisterPair()) {
            ...
            if (regH->getAssignedRealRegister() == NULL && regH->getTotalUseCount() != regH->getFutureUseCount()) {
                return true;
            }

            TR::Register* regL = reg->getLowOrder();
            if (regL->getAssignedRealRegister() == NULL && regL->getTotalUseCount() != regL->getFutureUseCount()) {
                return true;
            }
        } else if (reg->getAssignedRealRegister() == NULL && reg->getTotalUseCount() != reg->getFutureUseCount()) {
            return true;
        }
    }
...
bool OMR::Z::Instruction::anySpilledRegisters(TR::MemoryReference** listMem, int32_t listMemSize)
{
    int32_t i;
    ...
    for (i = 0; i < listMemSize; ++i) {
        ...
        TR::Register* reg;
        if ((reg = memRef->getBaseRegister()) && reg->getAssignedRealRegister() == NULL
            && (reg->getTotalUseCount() != reg->getFutureUseCount())) {
            return true;
        }
        if ((reg = memRef->getIndexRegister()) && reg->getAssignedRealRegister() == NULL
            && (reg->getTotalUseCount() != reg->getFutureUseCount())) {
            return true;
        }
        // for ar register pair spilling
        if ((reg = memRef->getBaseRegister()) && reg->getRegisterPair()) {
            TR::Register* regH = reg->getHighOrder();
            if (regH->getAssignedRealRegister() == NULL && regH->getTotalUseCount() != regH->getFutureUseCount())
                return true;
        ...
        if ((reg = memRef->getIndexRegister()) && reg->getRegisterPair()) {
            TR::Register* regH = reg->getHighOrder();
            if (regH->getAssignedRealRegister() == NULL && regH->getTotalUseCount() != regH->getFutureUseCount())
                return true;
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
    int32_t i;
    ...
    if (numTrgtPairs == 0 && _targetReg && _targetRegSize == 1 && _targetMem == NULL
        ...
        blockTarget = true;
        self()->block(
            _sourceReg, _sourceRegSize, _targetReg, _targetRegSize, self()->sourceMemBase(), self()->targetMemBase());
    }
    ...
    // "The solution is to unblock all _targetRegs before PreCond assignment and reblock them right after (required for
    // srcReg assignment)." "We only unblock those target registers that are not alive anymore. Block them again after
    // pre conds are done."
...
void OMR::Z::Instruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
    //
...
int32_t OMR::Z::Instruction::renameRegister(TR::Register* from, TR::Register* to)
{
    int32_t j, n, i;
...
void OMR::Z::Instruction::block(TR::Register** sourceReg, int sourceRegSize, TR::Register** targetReg,
    int targetRegSize, TR::MemoryReference** sourceMem, TR::MemoryReference** targetMem)
{
    int32_t i;
...
void OMR::Z::Instruction::unblock(TR::Register** sourceReg, int sourceRegSize, TR::Register** targetReg,
    int targetRegSize, TR::MemoryReference** sourceMem, TR::MemoryReference** targetMem)
{
    int32_t i;
...
bool OMR::Z::Instruction::checkRegForGPR0Disable(TR::InstOpCode::Mnemonic op, TR::Register* reg)
{
...
TR::Register* OMR::Z::Instruction::getSourceRegister(uint32_t i)
...
TR::Register* OMR::Z::Instruction::getTargetRegister(uint32_t i)
...
bool OMR::Z::Instruction::sourceRegUsedInMemoryReference(uint32_t i) { return _sourceUsedInMemoryReference->isSet(i); }

// Local-local allocation interface
//  Used to grab regs at genBin time.
//    Returns true if the desired number of regs are found.
//    Returns fale if not.
bool OMR::Z::Instruction::getOneLocalLocalAllocFreeReg(TR::RealRegister** reg)
{
    int32_t i;
...
bool OMR::Z::Instruction::getTwoLocalLocalAllocFreeReg(TR::RealRegister** reg1, TR::RealRegister** reg2)
{
    int32_t i;
    int32_t cnt = 1; // skip GPR0
    int32_t first = TR::RealRegister::FirstGPR + 1; // skip GPR0
...
{
    TR_ASSERT(nibbleIndex >= 0 && nibbleIndex <= 7, "OMR::Z::Instruction::setMaskField: bad nibbleIndex\n");
...
TR::Register* OMR::Z::Instruction::tgtRegArrElem(int32_t i)
{
    return (self()->targetRegBase() != NULL) ? (self()->targetRegBase())[i] : NULL;
}

TR::Register* OMR::Z::Instruction::srcRegArrElem(int32_t i)
{
    return (self()->sourceRegBase() != NULL) ? (self()->sourceRegBase())[i] : NULL;
}

void OMR::Z::Instruction::setThrowsImplicitNullPointerException()
{
    _flags.set(ThrowsImplicitNullPointerException);
