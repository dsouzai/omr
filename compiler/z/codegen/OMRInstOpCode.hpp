 ...
 * The following are the different formats of op-codes (from the 390 POP)
   ...
   RSY-a Format
    ________ ____ ____ ____ _______ _____ _________
...
public:
     ...
     */
    OMRProcessorArchitecture getMinimumALS() const { return metadata[_mnemonic].minimumALS; }
    ...
    uint64_t setsCC()
    {
        return (setsZeroFlag() || setsSignFlag() || setsOverflowFlag() || setsCompareFlag() || setsCarryFlag()
            || (metadata[_mnemonic].properties & S390OpProp_SetsCC));
    }
    ...
    static uint8_t getInstructionLength(
        Mnemonic i_opCode); /* Using a table that maps values of bit 0-1 to instruction length from POP chapter 5,
                               topic: Instruction formats (page 5.5)*/
