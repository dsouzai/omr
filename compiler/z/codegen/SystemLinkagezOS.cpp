...
void TR::S390zOSSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
...
void TR::S390zOSSystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    setStackFrameSize(
        (((self()->getOffsetToFirstParm() + argSize + localSize) + (32 - 1)) & ~(32 - 1)) - XPLINK_STACK_FRAME_BIAS);
    ...
    cg->addRelocation(new (cg->trHeapMemory())
            InstructionLabelRelative32BitRelocation(cursor, -8, _ppa1Snippet->getSnippetLabel(), 1));
...
void TR::S390zOSSystemLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method)
{
    setParameterLinkageRegisterIndex(method, method->getParameterList());
}

void TR::S390zOSSystemLinkage::setParameterLinkageRegisterIndex(
        ...
        default: {
            if (paramCursor->getDataType().isVector()) {
                ...
                if (elementType == TR::Int8 || elementType == TR::Int16 || elementType == TR::Int32
                    ...
                    // On 64-bit XPLINK floating point arguments leave "holes" in the GPR linkage registers, but not
                    // vice versa
                    numGPRArgs++;
                    numVRFArgs++;
                    break;
...
int32_t TR::S390zOSSystemLinkage::getOutgoingParameterBlockSize()
{
    //
 ...
 */
TR::Register* TR::S390zOSSystemLinkage::callNativeFunction(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    ...
    TR::LabelSymbol* returnFromJNICallLabel, TR::Snippet* callDataSnippet, bool isJNIGCPoint)
{
    ...
    /*****************************/
    generateInstructionsForCall(callNode, deps, targetAddress, methodAddressReg, javaLitOffsetReg,
        returnFromJNICallLabel, callDataSnippet, isJNIGCPoint);
    ...
    TR::Register *Real_highReg = NULL, *Real_lowReg = NULL, *Img_highReg = NULL, *Img_lowReg = NULL, *Real = NULL,
                 *Imaginary = NULL;
...
TR::RealRegister::RegNum TR::S390zOSSystemLinkage::getENVPointerRegister() { return TR::RealRegister::GPR5; }

TR::RealRegister::RegNum TR::S390zOSSystemLinkage::getCAAPointerRegister()
{
    return comp()->target().is64Bit() ? TR::RealRegister::NoReg : TR::RealRegister::GPR12;
}

int32_t TR::S390zOSSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
    int32_t offset;
    if ((srcReg >= TR::RealRegister::GPR4) && (srcReg <= TR::RealRegister::GPR15)) {
        ...
        TR_ASSERT(
            false, "ERROR: TR::S390zOSSystemLinkage::getRegisterSaveOffset called for volatile reg: %d\n", srcReg);
        return -1;
    }
}

void TR::S390zOSSystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptr_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR::Snippet* callDataSnippet, bool isJNIGCPoint)
{
    // WCode specific
    ...
    TR::RegisterDependencyConditions* preDeps = new (trHeapMemory())
        TR::RegisterDependencyConditions(deps->getPreConditions(), NULL, deps->getAddCursorForPre(), 0, cg());
    ...
    if (callNode->getOpCode().isIndirect()) {
        ...
        if (comp()->isRecursiveMethodTarget(callSymbol)) {
            ...
            TR::Instruction* callInstr = new (cg()->trHeapMemory())
                TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, raReg, callSymbol, callSymRef, cg());
            ...
            FunctionDescriptor* fd
                = reinterpret_cast<FunctionDescriptor*>(callSymbol->castToMethodSymbol()->getMethodAddress());

            genLoadAddressConstant(cg(), callNode, reinterpret_cast<uintptr_t>(fd), epReg);
            generateRSInstruction(cg(), TR::InstOpCode::getLoadMultipleOpCode(), callNode, aeReg, epReg,
                generateS390MemoryReference(epReg, 0, cg()));

            TR::Instruction* callInstr = new (cg()->trHeapMemory())
                TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, raReg, fd->func, callSymRef, cg());
            callInstr->setDependencyConditions(preDeps);
        }
...
TR::LabelSymbol* TR::S390zOSSystemLinkage::getEntryPointMarkerLabel() const { return _entryPointMarkerLabel; }

TR::LabelSymbol* TR::S390zOSSystemLinkage::getStackPointerUpdateLabel() const { return _stackPointerUpdateLabel; }

TR::PPA1Snippet* TR::S390zOSSystemLinkage::getPPA1Snippet() const { return _ppa1Snippet; }

TR::PPA2Snippet* TR::S390zOSSystemLinkage::getPPA2Snippet() const { return _ppa2Snippet; }

TR::Instruction* TR::S390zOSSystemLinkage::genCallNOPAndDescriptor(
    TR::Instruction* cursor, TR::Node* node, TR::Node* callNode, TR_XPLinkCallTypes callType)
{
    if (comp()->target().is32Bit()) {
        ...
        //
        // 1. The call site is so far removed from the Entry Point Marker of the function that its offset cannot be
        // contained in the space available in the call NOP descriptor following the call site.
        //
        // 2. The call contains a return value or parameters that are passed in registers or in ways incompatible with
        // non- XPLINK code.
        //
        ...
        cursor = generateS390BranchInstruction(
            cg(), InstOpCode::BRC, InstOpCode::COND_BRC, node, xplinkCallDescriptorEndLabel, cursor);
        cursor = generateAlignmentNopInstruction(cg(), node, 8, cursor);
        cursor
            = generateS390LabelInstruction(cg(), TR::InstOpCode::label, node, xplinkCallDescriptorBeginLabel, cursor);
        cursor = generateDataConstantInstruction(cg(), TR::InstOpCode::dd, node, 0x00000000, cursor);
        ...
        cursor = new (cg()->trHeapMemory())
            TR::S390Imm2Instruction(TR::InstOpCode::DC2, node, nopDescriptor, cursor, cg());
    }

    return cursor;
}

uint32_t TR::S390zOSSystemLinkage::generateCallDescriptorValue(TR::Node* callNode)
{
    uint32_t result = 0;
    ...
    if (cg()->comp()->target().is32Bit()) {
        ...
        for (int32_t i = firstArgumentChild; (i <= to) && !done; i++, parmCount++) {
            ...
            TR::SymbolReference* parmSymRef
                = child->getOpCode().hasSymbolReference() ? child->getSymbolReference() : NULL;
            int32_t argSize = 0;
            ...
            if (numFPRsNeeded != 0) {
                ...
                if (wordsToPreviousParm > 0xF) { // to big for descriptor. Will pass in stack
                    done = true; // done
                }
...
TR::Instruction* TR::S390zOSSystemLinkage::addImmediateToRealRegister(TR::RealRegister* targetReg, int32_t value,
    TR::RealRegister* tempReg, TR::Node* node, TR::Instruction* cursor, bool* checkTempNeeded)
{
    ...
    if (smallPositiveValue) {
        if (!checkTempNeeded)
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::LA, node, targetReg, generateS390MemoryReference(targetReg, value, cg()), cursor);
    } else if (largeValue) {
        ...
        if (!checkTempNeeded)
            cursor = generateRXInstruction(cg(), TR::InstOpCode::LAY, node, targetReg,
                generateS390MemoryReference(targetReg, value, cg()), cursor);
    }

    return cursor;
}

TR::Instruction* TR::S390zOSSystemLinkage::fillGPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
    int16_t GPRSaveMask;
    ...
    if (firstSaved >= 0) {
        ...
        if (firstSaved == lastSaved) {
            ...
            cursor = restorePreservedRegs(REGNUM(firstSaved + TR::RealRegister::FirstGPR),
                REGNUM(lastSaved + TR::RealRegister::FirstGPR), blockNumber, cursor, currentNode, spReg, rsa,
                getNormalStackPointerRegister());
        }
    }

    if (!(GPRSaveMask
            & (1 << GPREGINDEX(
                   getNormalStackPointerRegister())))) { // GPR4 not restored by previous and needs restoring
...
TR::Instruction* TR::S390zOSSystemLinkage::fillFPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
    ...
    for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask); i <= TR::Linkage::getLastMaskedBit(FPRSaveMask);
         ++i) {
        if (FPRSaveMask & (1 << (i))) {
            TR::MemoryReference* fillMemRef = generateS390MemoryReference(spReg, offset, cg());

            cursor = generateRXInstruction(cg(), TR::InstOpCode::LD, node,
                getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), fillMemRef, cursor);
...
TR::Instruction* TR::S390zOSSystemLinkage::spillGPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
    ...
    if ((frameType == TR_XPLinkStackCheckFrame)
        || ((frameType == TR_XPLinkIntermediateFrame)
            && needAddTempReg)) { // GPR4 is saved but indirectly via GPR0 in stack check or medium case - so don't save
                                  // GPR4 via STM
    ...
    case TR_XPLinkSmallFrame:
        ...
        cursor = generateRSInstruction(cg(), TR::InstOpCode::getStoreMultipleOpCode(), node,
            getRealRegister(REGNUM(firstSaved + TR::RealRegister::FirstGPR)),
    ...
    case TR_XPLinkIntermediateFrame:
        if (needAddTempReg) {
            ...
            if (saveTempReg || needAddTempReg) {
                cursor = generateRRInstruction(
                    cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, spReg, cursor); // LR R0,R4
            }
            if (saveTempReg) {
                gpr3ParmRef = generateS390MemoryReference(spReg, gpr3ParmOffset, cg()); // used for temporary register
                cursor = generateRXInstruction(
                    cg(), TR::InstOpCode::getStoreOpCode(), node, gpr3Real, gpr3ParmRef, cursor); // ST R3,parmRef(R4)
            }

            _stackPointerUpdateLabel = generateLabelSymbol(cg());
            cursor = generateS390LabelInstruction(cg(), TR::InstOpCode::label, node, _stackPointerUpdateLabel, cursor);

            cursor = addImmediateToRealRegister(
                spReg, (stackFrameSize) * -1, gpr3Real, node, cursor); // R4 <- R4 - stack size

            if (saveTempReg) {
                cursor
                    = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr3Real, gpr0Real, cursor);
                gpr3ParmRef = generateS390MemoryReference(gpr3Real, gpr3ParmOffset, cg());
                cursor
                    = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), node, gpr3Real, gpr3ParmRef, cursor);
            }
        } else { // no temp reg needed
            // (needAddTempReg)    LR R0,R4
            // AHI R4,-stackFrameSize
            // STM rx,ry,offset(R4)
        ...
        cursor = generateRSInstruction(cg(), TR::InstOpCode::getStoreMultipleOpCode(), node,
            getRealRegister(REGNUM(firstSaved + TR::RealRegister::FirstGPR)),
            getRealRegister(REGNUM(lastSaved + TR::RealRegister::FirstGPR)), rsa, cursor);
        if (needAddTempReg) { // ST R0,2048(R4)
    ...
    case TR_XPLinkStackCheckFrame:
        ...
        if (comp()->target().is64Bit()) {
            ...
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::getCmpOpCode(), node, spReg, bosRef, cursor); // C R4,bos(tempreg)
        } else {
            bosOffset = 868; // 868=offset in CAA to BOS
            bosRef = generateS390MemoryReference(caaReal, bosOffset, cg());
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::getCmpOpCode(), node, spReg, bosRef, cursor); // C R4,bos(R12)
        }
        cursor = generateS390BranchInstruction(
            cg(), InstOpCode::BRC, InstOpCode::COND_BNM, node, stmLabel, cursor); // BNM stmLabel
        ...
        if (!needAddTempReg) {
            cursor = generateRRInstruction(
                cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, gpr3Real, cursor); // LR R0,R3
        }
        ...
        if (comp()->target().is64Bit()) {
            ...
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::getLoadOpCode(), node, gpr3Real, extenderRef, cursor); //  L R3,extender(R12)
        } else {
...
TR::Instruction* TR::S390zOSSystemLinkage::spillFPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
    TR::RealRegister* spReg = getNormalStackPointerRealRegister();
    int32_t offset = getFPRSaveAreaEndOffset();
    int16_t FPRSaveMask = getFPRSaveMask();
    ...
    for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask); i <= TR::Linkage::getLastMaskedBit(FPRSaveMask);
        ...
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(cg(), TR::InstOpCode::STD, node,
                getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), spillMemRef, cursor);
...
TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::XPLINKCallDescriptorRelocation(
    TR::Instruction* nop, TR::LabelSymbol* callDescriptor)
    : TR::LabelRelocation(NULL, callDescriptor)
    , _nop(nop)
{ }

uint8_t* TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::getUpdateLocation()
{
...
void TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::apply(TR::CodeGenerator* cg)
{
    int64_t offsetToCallDescriptor
        = static_cast<int64_t>(getLabel()->getCodeLocation() - (_nop->getBinaryEncoding() - 7)) / 8;

    if (offsetToCallDescriptor < std::numeric_limits<int16_t>::min()
        || offsetToCallDescriptor > std::numeric_limits<int16_t>::max()) {
...
uint8_t* TR::S390zOSSystemLinkage::XPLINKFunctionDescriptorSnippet::emitSnippetBody()
{
