...
extern TR::Instruction* generateS390ImmToRegister(
        ...
        if (current->isLoad()) {
            ...
            if (disp == stackOffset) {
                ...
                if (opCode == TR::InstOpCode::L) {
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LGR, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LER, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LDR, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LTR, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LTGR, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                    ...
                    newInst = generateRRInstruction(
                        self()->cg(), TR::InstOpCode::LGFR, current->getNode(), tReg, sReg, prev);
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
 ...
 * the prolog code.
 */
void* OMR::Z::Linkage::saveArguments(
    void* cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol>* parameterList)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (lri >= 0) {
            ...
            if (type.isFloatingPoint()) // float/double/long double
            {
                ...
                TR_ASSERT(
                    paramCursor->getSize() <= 8, "Only aggregates of size 8 bytes or less are passed in registers");
            ...
            if ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor)
                ...
                {
                    if (genBinary) {
                        cursor = (void*)TR::S390CallSnippet::storeArgumentItem(
                            storeOpCode, (uint8_t*)cursor, self()->getRealRegister(regNum), offset, self()->cg());
                    } else {
                        ...
                        if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                            self()->removeOSCOnSavedArgument(
                                (TR::Instruction*)cursor, self()->getRealRegister(regNum), offset);
                    }
                }

                    if (secondStore && fullLong && self()->cg()->comp()->target().is32Bit()) {
                        if (genBinary) {
                            cursor = (void*)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::ST, (uint8_t*)cursor,
                ...
                case TR::Float:
                    if (genBinary) {
                        cursor = (void*)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::STE, (uint8_t*)cursor,
                            self()->getRealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                ...
                case TR::Double:
                    if (genBinary) {
                        cursor = (void*)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD, (uint8_t*)cursor,
                            self()->getRealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
            ...
            if (!(target < 0)
                ...
                case 1: // Int or Addr  Reg from Stack Slot
                {
                    ...
                    if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
                        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM, firstNode,
                            self()->getRealRegister(REGNUM(target)), busyMoves[4][i1], mr, (TR::Instruction*)cursor);
                ...
                case 10: // LongDouble
                    cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                        self()->cg()->allocateFPRegisterPair(
                            self()->getRealRegister(REGNUM(target + 2)), self()->getRealRegister(REGNUM(target))),
                        self()->cg()->allocateFPRegisterPair(
                            self()->getRealRegister(REGNUM(source + 2)), self()->getRealRegister(REGNUM(source))),
                        (TR::Instruction*)cursor);
                    freeScratchable.set(source);
    ...
    TR::Register* argRegister)
{
    ...
    if (isStorePair || isStoreOnlyLow) {
        TR::Register* stackRegister = self()->getStackRegisterForOutgoingArguments(
            callNode, dependencies); // delay (possibly) creating this till needed
 ...
 */
TR::Register* OMR::Z::Linkage::getStackRegisterForOutgoingArguments(
    TR::Node* n, TR::RegisterDependencyConditions* dependencies)
{
    ...
    TR::Register* argRegister)
{
    ...
    if (self()->cg()->comp()->target().is32Bit() && argType.isInt64())
        return self()->pushLongArg32(
            callNode, child, numIntegerArgs, numFloatArgs, stackOffsetPtr, dependencies, argRegister);
    ...
    if (isFloatShadowedToIntRegs) {
        TR::Register* stackRegister = self()->getStackRegisterForOutgoingArguments(
            callNode, dependencies); // delay (possibly) creating this till needed
        self()->loadIntArgumentsFromStack(
            callNode, dependencies, argDataType, (*stackOffsetPtr) - argSize, argSize, numIntegerArgs, stackRegister);
    }
    ...
    int64_t killMask, TR::Register*& vftReg, bool PassReceiver)
{
    ...
    // store env register
    stackOffset = self()->storeExtraEnvRegForBuildArgs(
        callNode, self(), dependencies, isFastJNI, stackOffset, gprSize, numIntegerArgs);
    ...
    for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
        ...
        case TR::Address:
            if (isFastJNI && child->getDataType() == TR::Address) {
                argRegister = self()->pushJNIReferenceArg(
                    callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
                numIntegerArgs++;
                break;
            }
        case TR::Int8:
        ...
        case TR::Int64:
            if (self()->cg()->comp()->target().is32Bit()) {
                argRegister
                    = self()->pushLongArg32(callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
                numIntegerArgs += 2;
            } else {
                argRegister
        ...
        case TR::Aggregate:
            if (self()->isAggregatesPassedInParmRegs() || self()->isAggregatesPassedOnParmStack()) {
                argRegister = self()->pushAggregateArg(
                    callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
    ...
    case TR::Int64:
        if (self()->cg()->comp()->target().is32Bit()) {
            ...
            dependencies->addPostCondition(
                resultRegHigh, self()->getLongHighReturnRegister(), DefinesDependentRegister);
...
{
    return self()->killAndAssignRegister(
        killMask, deps, virtualRegPtr, realReg->getRegisterNumber(), cg, isAllocate, isDummy);
}
    ...
    TR::Node*& GlobalRegDeps, bool& hasGlRegDeps, TR::Register** methodAddressReg, TR::Register*& JavaLitOffsetReg)
{
    ...
    if (!self()->cg()->comp()->target().isZOS()) {
        ...
        killMask = self()->killAndAssignRegister(
            killMask, deps, &parm3VirtualRegister, TR::RealRegister::GPR4, self()->cg(), true);
    }

    if (dispatchType == TR_SystemDispatch) {
        if (!self()->cg()->comp()->target().isZOS())
            killMask = self()->killAndAssignRegister(
                killMask, deps, methodAddressReg, TR::RealRegister::GPR14, self()->cg(), true);
    }
}
    ...
    TR::Node* GlobalRegDeps, bool hasGlRegDeps, TR::SystemLinkage* systemLinkage)
{
    ...
    killMask = self()->killAndAssignRegister(
        killMask, deps, &systemReturnAddressVirtualRegister, systemReturnAddressRegister, self()->cg(), true);
    ...
    TR::RegisterDependencyConditions*& deps, bool isFastJNI, bool isPassReceiver, bool isJNIGCPoint)
{
    ...
    /* Build Arguments               */
    /*********************************/
    self()->setupBuildArgForLinkage(
        callNode, dispatchType, deps, isFastJNI, isPassReceiver, killMask, GlobalRegDeps, hasGlRegDeps, systemLinkage);
