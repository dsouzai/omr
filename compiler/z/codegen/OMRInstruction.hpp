...
protected:
    Instruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n);

    Instruction(
        TR::CodeGenerator* cg, TR::Instruction* precedingInstruction, TR::InstOpCode::Mnemonic op, TR::Node* n = 0);

    void initialize(TR::RegisterDependencyConditions* cond) { _conditions = cond; }
    void initialize(TR::Instruction* precedingInstruction = NULL, bool instFlag = false,
        TR::RegisterDependencyConditions* cond = NULL, bool condFlag = false);
...
public:
    ...
    virtual TR::Register* getRegisterOperand(uint32_t i)
    {
        return (i > 0 && i <= _targetRegSize + _sourceRegSize) ? (TR::Register*)_operands[i - 1] : NULL;
    }
    virtual void setRegisterOperand(uint32_t i, TR::Register* newReg)
    {
        TR_ASSERT((i > 0 && i <= _targetRegSize + _sourceRegSize), "Register operand number %d does not exist.", i);
        _operands[i - 1] = newReg;
    }
...
protected:
     ...
     *   Operands are TR::Register * or any derrived type, TR_MemoryReference * in the order target registers, source
     * registers, source mem, target mem Register operands fall into target or source depending how they are used. If a
     * register operand is set it will be in target category, if register is set and used it will also be in target
     * category but not in source. The opcode property UsesTarget indicates that all target registers are used for
     * input. 390 Instructions have 0,1,2, or 3 register operands often refered in manual as R1,R2,R3. These operands
     * can be retreived using getRegisterOperand(i). Note that a compare or store instruction that does not set operand
     * 1 will put R1 into the sourceReg section of operands. This is not what it was like before. getTargetRegister used
     * to fetch R1 and getSourceRegister used to fetch R2. These were not always meaning source and target so now these
     * methods are disallowed. Register operands can be register pairs for instructions that use an even-odd pair.
     */
    void** _operands;
    ...
    TR::Register** targetRegBase() { return (_targetRegSize != 0) ? (TR::Register**)_operands + _targetStart : NULL; }
    TR::Register** sourceRegBase() { return (_sourceRegSize != 0) ? ((TR::Register**)_operands) + _sourceStart : NULL; }
    ...
    {
        return (_targetMemSize != 0)
            ? ((TR::MemoryReference**)_operands) + _targetRegSize + _sourceRegSize + _sourceMemSize
            : NULL;
    }

    template<typename T> class RegisterArray {
    public:
        ...
        {
            ((RegisterArray*)ptr)->allocator().deallocate(ptr, size);
    ...
    public:
        ...
        T& operator[](size_t i) { return _impl[i]; }
        ...
        void MakeEmpty() { _impl.MakeEmpty(); }
    };
    ...
    public:
        ...
        static void operator delete(void* ptr, size_t size)
        {
            ((RegisterBitVector*)ptr)->allocator().deallocate(ptr, size);
        } /* t->allocator() must return the same allocator as used for new */
    ...
    public:
        ...
        void set(size_t i) { _impl[i] = true; }

        bool isSet(size_t i) { return _impl.ValueAt(i); }
    };
