...
uint8_t* TR::S390LabelInstruction::generateBinaryEncoding()
{
    ...
    if (offset && isNopCandidate()) {
        ...
        if (offset < 238) {
            ...
            instr = generateS390BranchInstruction(
                cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK15, getNode(), getLabelSymbol(), prevInstr);
...
void TR::S390BranchInstruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (getOpCode().isBranchOp() && getLabelSymbol()->isEndOfColdInstructionStream()) {
        ...
        if (cg()->getIsInOOLSection()) {
            ...
            TR_ASSERT(
                cg()->getAppendInstruction() == this, "OOL section must have only one branch to the merge point\n");
...
uint8_t* TR::S390BranchInstruction::generateBinaryEncoding()
{
    ...
    if (doRelocation) {
        if (shortRelocation) {
            ...
            cg()->addProjectSpecializedRelocation(
                relocationPoint, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, getNode());
        }
    }
        ...
        *(int32_t*)(cursor + 2) |= boi(distance);
        ...
        if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
            ...
            *(int16_t*)(cursor + 2) |= bos(distance);
            ...
            TR::Instruction* instr;
            instr = generateS390BranchInstruction(
                cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK15, getNode(), relLabel, this);
            instr->setEstimatedBinaryLength(4);
            instr = generateS390BranchInstruction(
                cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK15, getNode(), label, instr);
...
uint8_t* TR::S390BranchOnIndexInstruction::generateBinaryEncoding()
{
    ...
    if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
        ...
        *(int16_t*)(cursor + 2) |= bos(distance);
        ...
        TR_ASSERT_FATAL(
            false, "Cannot encode branch on index instruction because distance (%d) is out of range", distance);
    }
...
void TR::S390RILInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (std::find(comp->getStaticHCRPICSites()->begin(), comp->getStaticHCRPICSites()->end(), this)
        ...
        if (is32bit)
            cg()->jitAdd32BitPicToPatchOnClassRedefinition(
                reinterpret_cast<void*>(immediateAsAddress), static_cast<void*>(cursor));
        else
            cg()->jitAddPicToPatchOnClassRedefinition(
                reinterpret_cast<void*>(immediateAsAddress), static_cast<void*>(cursor));
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, reinterpret_cast<uint8_t*>(immediateAsAddress), TR_HCR, cg()),
            __FILE__, __LINE__, getNode());
    }
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
        ...
        if (immediateAsAddress
            ...
            if (is32bit)
                cg()->jitAdd32BitPicToPatchOnClassUnload(
                    reinterpret_cast<void*>(immediateAsAddress), static_cast<void*>(cursor));
            else
        ...
        if (methodClass != NULL
            && !TR::Compiler->cls.sameClassLoaders(
                comp, static_cast<TR_OpaqueClassBlock*>(methodClass), comp->getCurrentMethod()->classOfMethod())) {
            if (is32bit)
...
uint8_t* TR::S390RILInstruction::generateBinaryEncoding()
{
    ...
    if (getTargetSnippet() && getTargetSnippet()->getKind() == TR::Snippet::IsConstantData) {
        ...
        || getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
        {
            ...
            if (cg()->comp()->target().cpu.isTargetWithinBranchRelativeRILRange(
                    (intptr_t)getTargetPtr(), (intptr_t)cursor)) {
                getOpCode().copyBinaryToBuffer(instructionStart);
                toRealRegister(getRegisterOperand(1))->setRegister1Field((uint32_t*)cursor);
        ...
        if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
            ...
            unloadableClass = (TR_OpaqueClassBlock*)cg()
                                  ...
                                  ->createResolvedMethod(
                                      cg()->trMemory(), (TR_OpaqueMethodBlock*)getTargetPtr(), comp->getCurrentMethod())
                                  ->classOfMethod();
            doRegisterPIC = true;
        }
        ...
        if (getTargetLabel() != NULL) {
            ...
            if (getTargetSnippet() != NULL) {
                ...
                // trampoline.
                if (!isImmediateOffsetInBytes()
                    && !cg()->comp()->target().cpu.isTargetWithinBranchRelativeRILRange(
                        (intptr_t)getTargetPtr(), (intptr_t)cursor)) {
...
uint8_t* TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void)
{
    ...
    if (isRRE || isRR)
        ...
        currInstr = generateRIInstruction(
            cg(), cmpOp, getNode(), getRegisterOperand(1), (int16_t)getSourceImmediate8(), prevInstr);
    else if (isRIL)
        currInstr = generateRILInstruction(
            cg(), cmpOp, getNode(), getRegisterOperand(1), (uint8_t)getSourceImmediate8(), prevInstr);
    ...
    currInstr = generateS390BranchInstruction(
        cg(), TR::InstOpCode::BRC, getBranchCondition(), getNode(), getBranchDestinationLabel(), prevInstr);
...
void TR::S390RIEInstruction::splitIntoCompareAndBranch(TR::Instruction* insertBranchAfterThis)
{
    ...
    generateS390BranchInstruction(
        cg(), TR::InstOpCode::BRC, getBranchCondition(), getNode(), getBranchDestinationLabel(), insertBranchAfterThis);
...
uint8_t* TR::S390SS1Instruction::generateBinaryEncoding()
{
    ...
    TR_ASSERT(
        getLen() >= 0 && getLen() <= 255, "SS1 L must be in the range 0->255 and not %d on inst %p\n", getLen(), this);
