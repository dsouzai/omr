...
void OMR::Z::CodeGenerator::preLowerTrees()
{
...
void OMR::Z::CodeGenerator::lowerTreesWalk(TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount)
{
...
void OMR::Z::CodeGenerator::checkIsUnneededIALoad(TR::Node* parent, TR::Node* node, TR::TreeTop* tt)
{
    ...
    if (node->getOpCodeValue() == TR::aloadi) {
        ...
        } else // Not in list
        {
            ...
            if (node->getReferenceCount() > 1) {
                uint32_t* temp;
                TR_Pair<TR::Node, int32_t>* newEntry
                    = new (self()->trStackMemory()) TR_Pair<TR::Node, int32_t>(node, (int32_t*)1);
    ...
    if (node->isUnneededIALoad()) {
        if (parent->getOpCodeValue() == TR::ifacmpne || parent->getOpCodeValue() == TR::ificmpeq
            || parent->getOpCodeValue() == TR::ificmpne || parent->getOpCodeValue() == TR::ifacmpeq) {
...
void OMR::Z::CodeGenerator::lowerTreesPropagateBlockToNode(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
...
void OMR::Z::CodeGenerator::lowerTreeIfNeeded(
    TR::Node* node, int32_t childNumberOfNode, TR::Node* parent, TR::TreeTop* tt)
{

    OMR::CodeGenerator::lowerTreeIfNeeded(node, childNumberOfNode, parent, tt);

    // Z
    if (self()->comp()->target().cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore())
        ...
        if (add1 && add2 && const1
            && performTransformation(
                self()->comp(), "%sBase/index/displacement form addressing prep for node [%p]\n", OPT_DETAILS, node)) {
...
bool OMR::Z::CodeGenerator::canTransformUnsafeCopyToArrayCopy()
{
    ...
    return (!disableTran && !self()->comp()->getCurrentMethod()->isJNINative()
        && !self()->comp()->getOption(TR_DisableArrayCopyOpts));
}

bool OMR::Z::CodeGenerator::supportsDirectIntegralLoadStoresFromLiteralPool() { return true; }
...
void OMR::Z::CodeGenerator::initialize()
{
    ...
    bool supportsAutoSIMD
        = !comp->getOption(TR_DisableSIMD) && comp->target().cpu.supportsFeature(OMR_FEATURE_S390_VECTOR_FACILITY);
    ...
    _unlatchedRegisterList = (TR::RealRegister**)cg->trMemory()->allocateHeapMemory(
        sizeof(TR::RealRegister*) * (TR::RealRegister::NumRegisters));
    ...
    if (enableBranchPreload
        || (!disableBranchPreload && comp->isOptServer() && comp->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12)))
    ...
    if ((enableBranchPreload && bpp)
        || (bpp && !disableBranchPreload && comp->isOptServer()
    ...
    if (cg->supportsBranchPreloadForCalls()) {
        _callsForPreloadList = new (cg->trHeapMemory())
            TR::list<TR_BranchPreloadCallData*>(getTypedAllocator<TR_BranchPreloadCallData*>(comp->allocator()));
    }
    ...
    if (comp->getOption(TR_TraceRA)) {
        cg->setGPRegisterIterator(new (cg->trHeapMemory())
                TR::RegisterIterator(cg->machine(), TR::RealRegister::FirstGPR, TR::RealRegister::LastAssignableGPR));
        ...
        cg->setVRFRegisterIterator(new (cg->trHeapMemory())
                TR::RegisterIterator(cg->machine(), TR::RealRegister::FirstVRF, TR::RealRegister::LastVRF));
    }
...
bool OMR::Z::CodeGenerator::getSupportsBitPermute() { return true; }
...
bool OMR::Z::CodeGenerator::prepareForGRA()
{
    ...
    if (!_globalRegisterTable) {
        ...
        if (enableVectorGRA) {
            ...
            self()->setOverlapOffsetBetweenAliasedGRNs(
                self()->getFirstOverlappedGlobalVRF() - self()->getFirstOverlappedGlobalFPR());
        }
        ...
        if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
            ...
            for (grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
                ...
                TR_ASSERT(reg != -1,
                    "Register pressure simulator doesn't support gaps in the global register table; reg %d must be "
                    "removed",
                    grn);
...
bool OMR::Z::CodeGenerator::internalPointerSupportImplemented() { return true; }

bool OMR::Z::CodeGenerator::mulDecompositionCostIsJustified(
    int32_t numOfOperations, char bitPosition[], char operationType[], int64_t value)
{
...
bool OMR::Z::CodeGenerator::considerTypeForGRA(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
    if (node->getType().isBCD())
        return false;
...
bool OMR::Z::CodeGenerator::considerTypeForGRA(TR::DataType dt)
{
    if (
#ifdef J9_PROJECT_SPECIFIC
        dt.isBCD() ||
...
bool OMR::Z::CodeGenerator::considerTypeForGRA(TR::SymbolReference* symRef)
{
...
void OMR::Z::CodeGenerator::enableLiteralPoolRegisterForGRA()
{
    if (self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
...
bool OMR::Z::CodeGenerator::canUseImmedInstruction(int64_t value)
{
...
bool OMR::Z::CodeGenerator::isAddMemoryUpdate(TR::Node* node, TR::Node* valueChild)
{
    ...
    if (!disableASI && self()->isMemoryUpdate(node) && valueChild->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        } else if (self()->comp()->target().is64Bit()
            && (valueChild->getOpCodeValue() == TR::ladd || valueChild->getOpCodeValue() == TR::lsub)) {
...
bool OMR::Z::CodeGenerator::isUsing32BitEvaluator(TR::Node* node)
{
...
TR::Instruction* OMR::Z::CodeGenerator::generateNop(TR::Node* n, TR::Instruction* preced, TR_NOPKind nopKind)
{
    return new (self()->trHeapMemory()) TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, n, preced, self());
}

TR::Instruction* OMR::Z::CodeGenerator::insertPad(
    TR::Node* node, TR::Instruction* cursor, uint32_t size, bool prependCursor)
{
    if (size != 0) {
        if (prependCursor) {
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::beginInstructionSelection()
{
    ...
    if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
        ...
        if (self()->comp()->getJittedMethodSymbol()->isJNI()
            ...
            intptr_t jniMethodTargetAddress
                = (intptr_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(self()->comp());

            cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(
                TR::InstOpCode::dd, startNode, UPPER_4_BYTES(jniMethodTargetAddress), cursor, self());

            if (self()->comp()->target().is64Bit()) {
                cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(
                    TR::InstOpCode::dd, startNode, LOWER_4_BYTES(jniMethodTargetAddress), cursor, self());
            }
        }

        _returnTypeInfoInstruction = new (self()->trHeapMemory())
            TR::S390ImmInstruction(TR::InstOpCode::dd, startNode, 0, NULL, cursor, self());
    }
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::endInstructionSelection() { }

////////////////////////////////////////////////////////////////////////////////
// OMR::Z::CodeGenerator::doInstructionSelection
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::doInstructionSelection()
{
...
bool OMR::Z::CodeGenerator::hasWarmCallsBeforeReturn()
{
...
void OMR::Z::CodeGenerator::createBranchPreloadCallData(
    TR::LabelSymbol* callLabel, TR::SymbolReference* callSymRef, TR::Instruction* instr)
{
    TR_BranchPreloadCallData* data = new (self()->trHeapMemory()) TR_BranchPreloadCallData(instr, callLabel,
        self()->comp()->getCurrentBlock(), callSymRef, self()->comp()->getCurrentBlock()->getFrequency());
...
void OMR::Z::CodeGenerator::insertInstructionPrefetches()
{
    if (self()->supportsBranchPreload()) {
        if (_hottestReturn._frequency > 6) {
            ...
            TR::RealRegister* spReg
                = self()->getS390Linkage()->getRealRegister(self()->getS390Linkage()->getStackPointerRegister());
            ...
            cursor = generateS390BranchPredictionPreloadInstruction(
                self(), TR::InstOpCode::BPP, node, _hottestReturn._returnLabel, (int8_t)0x6, tempMR, cursor);
    ...
    if (self()->supportsBranchPreloadForCalls()) {
        ...
        for (auto iterator = self()->getCallsForPreloadList()->begin();
             iterator != self()->getCallsForPreloadList()->end(); ++iterator)
            self()->insertInstructionPrefetchesForCalls(*iterator);
    }
}

void OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(TR_BranchPreloadCallData* data)
{
    static bool bpp = (feGetEnv("TR_BPRP") != NULL);
    ...
    if (bppMethod && self()->getDebug()
        && (strcmp(self()->getDebug()->getMethodName(data->_callSymRef), "java/lang/String.equals(Ljava/lang/Object;)Z")
            && strcmp(
                self()->getDebug()->getMethodName(data->_callSymRef), "java/util/HashMap.hash(Ljava/lang/Object;)I")
            && strcmp(self()->getDebug()->getMethodName(data->_callSymRef),
    ...
    if (offset2 >= MIN_24_RELOCATION_VAL && offset2 <= MAX_24_RELOCATION_VAL && offset1 >= MIN_24_RELOCATION_VAL
        && offset1 <= MAX_24_RELOCATION_VAL) {
    ...
    if (true) //! isInLoop || !bppMaxPath)
    {
        ...
        while (first != cursor) {
            ...
            if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND && real->getOpCode().isLabel()) {
                real = cursor;
                if (cursor->getNext() && first != cursor->getNext()) // op.getOpCodeValue() == TR::InstOpCode::BPRP ||
                                                                     // op.getOpCodeValue() == TR::InstOpCode::BPP)
                {
    ...
    if (bppMaxPath) {
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            while (first != cursor) {
                ...
                if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND && real->getOpCode().isLabel()) {
                    real = cursor;
                    if (cursor->getNext() && first != cursor) // op.getOpCodeValue() == TR::InstOpCode::BPRP ||
                                                              // op.getOpCodeValue() == TR::InstOpCode::BPP)
                    {
                        op = cursor->getNext()->getOpCode();
                        if (canReachWithBPRP
                            && op.getOpCodeValue()
                                == TR::InstOpCode::BPRP) // || op.getOpCodeValue() == TR::InstOpCode::BPP)
                        {
            ...
            if (bppCall && (!canReachWithBPRP || bppOnly)) {
                ...
                cursor = new (self()->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL, node, tempReg,
                    (data->_callSymRef)->getSymbol(), data->_callSymRef, cursor, self());

                tempMR = generateS390MemoryReference(tempReg, 0, self());
                cursor = generateS390BranchPredictionPreloadInstruction(
                    self(), TR::InstOpCode::BPP, node, data->_callLabel, (int8_t)0xD, tempMR, cursor);
                cursor->setNext(second);
                self()->stopUsingRegister(tempReg);

            } else {
                cursor = generateS390BranchPredictionRelativePreloadInstruction(
                    self(), TR::InstOpCode::BPRP, node, data->_callLabel, (int8_t)0xD, data->_callSymRef, cursor);
                cursor->setNext(second);
            }
        }
    }
 ...
 */
bool OMR::Z::CodeGenerator::isLitPoolFreeForAssignment()
{
...
TR_RegisterKinds OMR::Z::CodeGenerator::prepareRegistersForAssignment()
{
    ...
    if (!self()->isOutOfLineColdPath() && _extentOfLitPool < 0) {
        ...
        TR_ASSERT(_extentOfLitPool < 0,
            "OMR::Z::CodeGenerator::doRegisterAssignment -- extentOfLitPool is assumed unassigned here.");
        ...
        if (self()->comp()->getOption(TR_Randomize)) {
            if (randomizer.randomBoolean(300)
                && performTransformation(self()->comp(),
                    "O^O Random Codegen - Added 6000 dummy slots to  Literal Pool frame to test  Literal Pool "
                    "displacement.\n"))
    ...
    // We go from the end since in most cases, we want to test functionality of the first half i.e 0-15 for overlap with
    // FPR etc
    if (TR::Options::_numVecRegsToLock > 0
        && TR::Options::_numVecRegsToLock
            <= (TR::RealRegister::LastAssignableVRF - TR::RealRegister::FirstAssignableVRF)) {
        for (int32_t i = TR::RealRegister::LastAssignableVRF;
             i > (TR::RealRegister::LastAssignableVRF - TR::Options::_numVecRegsToLock); --i) {
...
void OMR::Z::CodeGenerator::replaceInst(TR::Instruction* old, TR::Instruction* curr)
{
...
void OMR::Z::CodeGenerator::AddFoldedMemRefToStack(TR::MemoryReference* mr)
{
...
void OMR::Z::CodeGenerator::RemoveMemRefFromStack(TR::MemoryReference* mr)
{
    if (_stackOfMemoryReferencesCreatedDuringEvaluation.contains(mr)) {
        for (size_t i = 0; i < _stackOfMemoryReferencesCreatedDuringEvaluation.size(); i++) {
            if (_stackOfMemoryReferencesCreatedDuringEvaluation.element(i) == mr) {
                ...
                // there should be no way for remove(i) to delete something before the top of stack before evaluation
                // mark.
...
void OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(int32_t topOfMemRefStackBeforeEvaluation)
{
    while (_stackOfMemoryReferencesCreatedDuringEvaluation.topIndex() > topOfMemRefStackBeforeEvaluation) {
        ...
        // 2) AddFoldedMemRefToStack is only defined for OMR::Z::CodeGenerator, and requires TR::MemoryReference instead
        // of TR_MemoryReference.
        //
        TR::MemoryReference* potentiallyLeakedMemRef
            = (TR::MemoryReference*)(_stackOfMemoryReferencesCreatedDuringEvaluation.pop());
...
bool OMR::Z::CodeGenerator::supportsNonHelper(TR::SymbolReferenceTable::CommonNonhelperSymbol symbol)
{
    bool result = false;
...
void OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(
    TR::S390ConstantDataSnippet* ifcSnippet, TR::list<TR_OpaqueClassBlock*>* PICSlist)
{
    if (_interfaceSnippetToPICsListHashTab == NULL) {
        _interfaceSnippetToPICsListHashTab
            = new (self()->trHeapMemory()) TR_HashTab(self()->comp()->trMemory(), heapAlloc, 10, true);
    }
...
TR::list<TR_OpaqueClassBlock*>* OMR::Z::CodeGenerator::getPICsListForInterfaceSnippet(
    TR::S390ConstantDataSnippet* ifcSnippet)
{
    if (_interfaceSnippetToPICsListHashTab == NULL)
        return NULL;
 ...
 */
bool OMR::Z::CodeGenerator::anyNonConstantSnippets()
{
 ...
 * @return true if one is found
 */
bool OMR::Z::CodeGenerator::anyLitPoolSnippets()
{
    CS2::HashIndex hi;
    ...
    for (auto writeableiterator = _writableList.begin(); writeableiterator != _writableList.end();
         ++writeableiterator) {
...
bool OMR::Z::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
    if (self()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
...
TR_S390ScratchRegisterManager* OMR::Z::CodeGenerator::generateScratchRegisterManager(int32_t capacity)
{
    return new (self()->trHeapMemory()) TR_S390ScratchRegisterManager(capacity, self());
}

void OMR::Z::CodeGenerator::doBinaryEncoding()
{
    ...
    _methodBegin = new (self()->trHeapMemory())
        TR::S390LabelInstruction(TR::InstOpCode::label, self()->comp()->getStartTree()->getNode(),
            generateLabelSymbol(self()), static_cast<TR::Instruction*>(NULL), self());

    _methodEnd = generateS390LabelInstruction(
        self(), TR::InstOpCode::label, self()->comp()->findLastTree()->getNode(), generateLabelSymbol(self()));
    ...
    while (data.cursorInstruction) {
        if (data.cursorInstruction->getNode() != NULL
            && data.cursorInstruction->getNode()->getOpCodeValue() == TR::BBStart) {
    ...
    if (self()->comp()->getOption(TR_TraceCG))
        self()->comp()->getDebug()->dumpMethodInstrs(
            self()->comp()->getOutFile(), "Post Prologue/epilogue Instructions", false);
    ...
    while (data.cursorInstruction) {
        ...
        TR_ASSERT(
            data.cursorInstruction->getEstimatedBinaryLength() >= self()->getBinaryBufferCursor() - instructionStart,
            "\nInstruction length estimate must be conservatively large \n(instr=" POINTER_PRINTF_FORMAT
            ", opcode=%s, estimate=%d, actual=%d",
            data.cursorInstruction, data.cursorInstruction->getOpCode().getMnemonicName(),
            data.cursorInstruction->getEstimatedBinaryLength(), self()->getBinaryBufferCursor() - instructionStart);
    ...
    while (oiIterator != self()->getS390OutOfLineCodeSectionList().end()) {
        ...
        bool needsETE
            = (firstNode->getOpCode().hasSymbolReference()) ? firstNode->getSymbolReference()->canCauseGC() : false;
#ifdef J9_PROJECT_SPECIFIC
        if (firstNode->getOpCodeValue() == TR::BCDCHK || firstNode->getType().isBCD())
            needsETE = true;
#endif
 ...
 */
TR::RegisterPair* OMR::Z::CodeGenerator::allocateConsecutiveRegisterPair()
{
...
TR::RegisterPair* OMR::Z::CodeGenerator::allocateConsecutiveRegisterPair(
    TR::Register* lowRegister, TR::Register* highRegister)
{
...
void OMR::Z::CodeGenerator::processUnusedNodeDuringEvaluation(TR::Node* node)
{
    ...
    TR_ASSERT(self()->getCodeGeneratorPhase() > TR::CodeGenPhase::SetupForInstructionSelectionPhase,
        "must be past TR::CodeGenPhase::SetupForInstructionSelectionPhase (current is %s)\n",
    ...
    if (node) {
        ...
        if (node->getOpaquePseudoRegister()) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - handle extra ref to "
                    "addr child (ref is node based %s - %s %p)\n",
                    node->getOpCode().getName(), node, self()->getDebug()->getName(reg),
                    ref && ref->isNodeBased() ? "yes" : "no",
                    ref->isNodeBased() ? ref->getNode()->getOpCode().getName() : "",
                    ref->isNodeBased() ? ref->getNode() : NULL);
        ...
        if (node->safeToDoRecursiveDecrement()) {
        ...
        } else if (node->getReferenceCount() == 1 && node->getOpCode().isLoadVar()
            && (node->getNumChildren() == 0
                || (node->getNumChildren() == 1 && node->getFirstChild()->safeToDoRecursiveDecrement()))) {
            // i.e. a direct load that has no future refs or a indirect load with no future refs that itself has a child
            // that is safeToRecDec
            self()->recursivelyDecReferenceCount(node);
        } else {
            self()->evaluate(node);
            ...
            if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - handle extra ref "
                        "to addr child (ref is node based %s - %s %p)\n",
...
TR::RegisterDependencyConditions* OMR::Z::CodeGenerator::createDepsForRRMemoryInstructions(
    TR::Node* node, TR::RegisterPair* sourceReg, TR::RegisterPair* targetReg, uint8_t extraDeps)
{
    uint8_t numDeps = 8
        + extraDeps; // TODO: only 6 deps are created below so why is there is always an implicit 2 extra deps created?
    TR::RegisterDependencyConditions* dependencies
        = new (self()->trHeapMemory()) TR::RegisterDependencyConditions(0, numDeps, self());
 ...
 */
TR::RegisterPair* OMR::Z::CodeGenerator::allocateFPRegisterPair()
{
 ...
 */
TR::RegisterPair* OMR::Z::CodeGenerator::allocateFPRegisterPair(TR::Register* lowRegister, TR::Register* highRegister)
{
...
bool OMR::Z::CodeGenerator::enableRegisterAssociations() { return true; }

bool OMR::Z::CodeGenerator::enableRegisterPairAssociation() { return true; }

////////////////////////////////////////////////////////////////////////////////
//  Tactical GRA
TR_GlobalRegisterNumber OMR::Z::CodeGenerator::getLinkageGlobalRegisterNumber(
    int8_t linkageRegisterIndex, TR::DataType type)
{
    ...
    if (isFloat) {
        ...
        result = self()->getGlobalRegisterNumber(
            self()->getS390Linkage()->getIntegerArgumentRegister(linkageRegisterIndex) - 1);
    }

    return result;
}

inline bool isBetween(int64_t value, int32_t low, int32_t high) { return low <= value && value < high; }
...
void OMR::Z::CodeGenerator::simulateNodeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
bool OMR::Z::CodeGenerator::allowGlobalRegisterAcrossBranch(TR::RegisterCandidate* rc, TR::Node* branchNode)
{
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfAssignableGPRs()
{
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Block* block)
{
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    ...
    if (node->getOpCode().isIf() && node->getNumChildren() > 0 && !node->getOpCode().isCompBranchOnly()) {
        ...
        isFloat = (dt == TR::Float || dt == TR::Double);
    ...
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getSecondChild()->getOpCode().isLoadConst()) {
            ...
            if (self()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12) && value >= MIN_IMMEDIATE_BYTE_VAL
                && value <= MAX_IMMEDIATE_BYTE_VAL) {
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(TR::Node* node) { return 8; }

int32_t OMR::Z::CodeGenerator::getMaximumNumberOfVRFsAllowedAcrossEdge(TR::Node* node)
{
    return 16; // This can be dynamically selected based on context, but for now, keep it conservative..
}

int32_t OMR::Z::CodeGenerator::getMaximumNumbersOfAssignableGPRs()
{
...
int32_t OMR::Z::CodeGenerator::getMaximumNumbersOfAssignableFPRs() { return 16; }

int32_t OMR::Z::CodeGenerator::getMaximumNumbersOfAssignableVRs()
{
    //   TR_ASSERT(false,"getMaximumNumbersOfAssignableVRs unreachable");
    return INT_MAX;
}

void OMR::Z::CodeGenerator::setRealRegisterAssociation(TR::Register* reg, TR::RealRegister::RegNum realNum)
{
...
bool OMR::Z::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i, TR::DataType dt)
{
 ...
 */
TR::Register* OMR::Z::CodeGenerator::allocateClobberableRegister(TR::Register* srcRegister)
{
 ...
 */
TR::Register* OMR::Z::CodeGenerator::gprClobberEvaluate(TR::Node* node, bool force_copy, bool ignoreRefCount)
{
    TR::Instruction* cursor = NULL;
...
#ifdef J9_PROJECT_SPECIFIC
    TR_ASSERT(!(node->getType().isAggregate() || node->getType().isBCD()),
        "BCD/Aggr node (%s) %p should use ssrClobberEvaluate\n", node->getOpCode().getName(), node);
#else
 ...
 * Different from evaluate in that it returns a clobberable register
 */
TR::Register* OMR::Z::CodeGenerator::fprClobberEvaluate(TR::Node* node)
{
...
#ifdef J9_PROJECT_SPECIFIC
TR_OpaquePseudoRegister* OMR::Z::CodeGenerator::ssrClobberEvaluate(
    TR::Node* node, TR::MemoryReference* sourceMR) // sourceMR can be NULL
{
    TR::CodeGenerator* cg = self();
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            "\tneedsClobberEval : %s (%p), storageRef #%d (%p) (isBCD = %s, isReadOnlyTemp = %s, tempRefCount = %d)\n",
            node->getOpCode().getName(), node, srcStorageReference->getReferenceNumber(),
            srcStorageReference->getSymbol(), isBCD ? "yes" : "no",
            srcStorageReference->isReadOnlyTemporary() ? "yes" : "no",
            srcStorageReference->getTemporaryReferenceCount());
    ...
    if (node->getReferenceCount() > 1) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\t\t-->needsClobber=true due to srcStorageReference #%d (%s) owningRegCount %d\n",
                ...
                self()->getDebug()->getName(srcStorageReference->getSymbol()),
                srcStorageReference->getOwningRegisterCount());
    } else if (srcStorageReference->isReadOnlyTemporary() && srcStorageReference->getTemporaryReferenceCount() > 1) {
        needsClobberDueToReuse = true;
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\t\t-->needsClobber=true due to reuse of readOnlyTemp %s with refCount %d > 1\n",
                self()->getDebug()->getName(srcStorageReference->getSymbol()),
                srcStorageReference->getTemporaryReferenceCount());
        // all these needsClobberDueToReuse cases should now be caught by the more general getOwningRegisterCount() > 1
        // above
    ...
    if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
        ...
        // if the srcStorageReference is not a temp or a hint then the recursive dec in setStorageReference() will be
        // wrong
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "ssrClobberEvaluate: node %s (%p) with srcReg %s (srcRegSize %d, srcReg->validSymSize %d, "
                "srcReg->liveSymSize %d) and srcStorageReference #%d with %s (symSize %d)\n",
                node->getOpCode().getName(), node, self()->getDebug()->getName(srcRegister), srcRegister->getSize(),
                srcRegister->getValidSymbolSize(), srcRegister->getLiveSymbolSize(),
                srcStorageReference->getReferenceNumber(),
                self()->getDebug()->getName(srcStorageReference->getSymbol()), srcStorageReference->getSymbolSize());

        int32_t byteLength = srcRegister->getValidSymbolSize();
        TR_StorageReference* copyStorageReference
            = TR_StorageReference::createTemporaryBasedStorageReference(byteLength, self()->comp());
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\tgot copyStorageReference #%d with %s for clobberEvaluate\n",
                copyStorageReference->getReferenceNumber(),
                self()->getDebug()->getName(copyStorageReference->getSymbol()));
        ...
        if (srcStorageReference->isReadOnlyTemporary() && srcRegister->getRightAlignedIgnoredBytes() > 0) {
            // The MVC must copy over the entire source field including any ignored (but still valid) bytes (but dead
            // bytes are not included as these have been invalidated/clobbered). The subsequent uses of this copied
            // field (via copyStorageReference) will adjust based on the owning registers' ignoredBytes/deadBytes
            // setting
            int32_t regIgnoredBytes = srcRegister->getRightAlignedIgnoredBytes();
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tremove %d ignoredBytes from sourceMR->_offset (%d->%d) for readOnlyTemp #%d (%s)\n",
                    regIgnoredBytes, sourceMR->getOffset(), sourceMR->getOffset() + regIgnoredBytes,
                    srcStorageReference->getReferenceNumber(),
                    self()->getDebug()->getName(srcStorageReference->getSymbol()));
        ...
        if (isBCD)
            copyMR = generateS390RightAlignedMemoryReference(
                node, copyStorageReference, cg, true, true); // enforceSSLimits=true, isNewTemp=true
        else
        ...
        // the srcRegister->setStorageReference call will set the temp refCount to the final correct value based on the
        // node's refCount
        copyStorageReference->setTemporaryReferenceCount(0);
        TR_OpaquePseudoRegister* targetRegister = bcdSrcRegister ? self()->allocatePseudoRegister(bcdSrcRegister)
                                                                 : self()->allocateOpaquePseudoRegister(srcRegister);
        ...
        int32_t digitsToClear
            = srcRegister->getLeftAlignedZeroDigits() > 0 ? srcRegister->getDigitsToClear(0, digitsInSrc) : digitsInSrc;
        int32_t alreadyClearedDigits = digitsInSrc - digitsToClear;
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\tupdate srcRegister %s with copy ref #%d (%s)\n",
                self()->getDebug()->getName(srcRegister), copyStorageReference->getReferenceNumber(),
                self()->getDebug()->getName(copyStorageReference->getSymbol()));
        srcRegister->setStorageReference(
            copyStorageReference, node); // clears leftAlignedZeroDigits so query getDigitsToClear above
        if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
            traceMsg(self()->comp(), "\tsrcRegister %s had leftAlignedZeroDigits %d on original ref #%d\n",
                self()->getDebug()->getName(srcRegister), srcRegister->getLeftAlignedZeroDigits(),
                srcStorageReference->getReferenceNumber());
            traceMsg(self()->comp(),
                "\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s with new copy ref #%d (digitsInSrc "
                "- digitsToClear = %d - %d = %d)\n",
                alreadyClearedDigits, self()->getDebug()->getName(srcRegister),
                copyStorageReference->getReferenceNumber(), digitsInSrc, digitsToClear, alreadyClearedDigits);
        }
        ...
        if (srcStorageReference->getNodesToUpdateOnClobber()) {
            ...
            //
            //       AP    t1,t2 (pdadd)
            //       ZAP   t1,t1 mark t1 as readOnly, add pdcleanA (w/regA) to list (pdcleanA), increment t1 refCount by
            //       2 (pdaddA nodeRefCount) nop         mark t1 as readOnly (redundant), add pdcleanB (w/regB) to list
            //       (pdcleanB), increment t1 refCount by 3 (pdcleanA nodeRefCount) UNPK  t3,t1 non-clobber operation,
            //       t1 is still alive and readOnly (pd2zd)
            //    ...
            //    pd2zd
            //       =>pdcleanA
            //
            ...
            //       =>pdcleanB regB
            //
            //       MVC   t4,t1  clobber eval for pdadd parent, regB has other use so updated above, go through list
            //       and update regA with t4 (on clobber eval for pdaddB) AP    t1,tx
            //    ...
            ...
            for (TR::Node* listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
                ...
                if (listReg != srcRegister) {
                    if (listReg->getStorageReference() != srcStorageReference) {
                        // in this case the listReg has already had its storageRef changed (likely by
                        // skipCopyOnStore/changeCommonedChildAddress in pdstoreEvaluator so skip this update (the
                        // reference counts of the address subtree will be wrong if the refCount is update here. See
                        // addsub2 first pdstore on line_no=335 with disableVIP)
                        //
                        // ipdstore p=29 symA (store then does a ZAP and updates child VTS_2 to symA)
                        //    aiadd
                        //       aload
                        //       iconst
                        //    pdModPrecA   p=29 (passThrough so skip ssrClobber evaluate and reuse VTS_2 as a read only
                        //    temp) 1 ref remaining to pdModPrecA
                        ...
                        // ipdstore p=30
                        //    addr
                        //    pdModPrecB   p=30 (non-passThrough, needs NI, so needs clobberEval with copy ref VTS_3 and
                        //    goes to update pdModPrecA symA -> VTS_3)
                        //       =>pdaddOverflowMessage p=31 (VTS_2)
                        //
                        // prevent the symA -> VTS_3 update as symA != the orig VTS_2
                        // if the update is done then the last pdModPrecA ref is now using VTS_3 and the final
                        // decReferenceCount of the symA aload is never done (live reg alive bug)
                        //
                        if (self()->traceBCDCodeGen())
                            traceMsg(self()->comp(),
                                "\ty^y : skip update on reg %s node %s (%p), storageRef has already changed -- was #%d "
                                "(%s), now #%d (%s)\n",
                                self()->getDebug()->getName(listReg), listNode->getOpCode().getName(), listNode,
                                srcStorageReference->getReferenceNumber(),
                                self()->getDebug()->getName(srcStorageReference->getSymbol()),
                                listReg->getStorageReference()->getReferenceNumber(),
                                self()->getDebug()->getName(listReg->getStorageReference()->getSymbol()));
                    } else {
                        if (self()->traceBCDCodeGen())
                            traceMsg(self()->comp(),
                                "\ty^y : update reg %s with ref #%d (%s) on node %s (%p) refCount %d from "
                                "_nodesToUpdateOnClobber with copy ref #%d (%s) (skip if refCount == 0)\n",
                        ...
                        if (listNode->getReferenceCount() > 0)
                            listReg->setStorageReference(copyStorageReference,
                                listNode); // TODO: safe to transfer alreadyClearedDigits in this case too?
                    }
                }
            }
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label, bool isCheckDisp)
{
    if (!isCheckDisp) {
    ...
    if (s != NULL
        && (s->getKind() == TR::Snippet::IsConstantData || s->getKind() == TR::Snippet::IsWritableData
...
/**
 * The following method is used only for zOS 31 bit system/JNI linkage for producing the 4 byte NOP with last two bytes
 * az offset in double words to the calldescriptor
 *
 * field _instructionOffser is used for 16-bit relocation when we need to specify where relocation starts relative to
 * the first byte of the instruction. Eg.: BranchPreload instruction BPP (48-bit instruction), where 16-bit relocation
 * is at 32-47 bits, apply16BitLabelRelativeRelocation(cursor,label,4,true)
 *
 * The regular OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t * cursor, TR::LabelSymbol * label) only
 * supports instructions with 16-bit relocations, where the relocation is 2 bytes (hardcoded value) form the start of
 * the instruction
 */
void OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(
    int32_t* cursor, TR::LabelSymbol* label, int8_t addressDifferenceDivisor, bool isInstrOffset)
{
    if (isInstrOffset) {
        uint16_t* label_cursor = (uint16_t*)label->getCodeLocation();
        if ((*(uint16_t*)label_cursor) == bos(0x1800))
        ...
        *(int16_t*)cursor
            = (int16_t)(((intptr_t)label->getCodeLocation() - (((intptr_t)cursor) - addressDifferenceDivisor))
                / addressDifferenceDivisor);
}

void OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label)
{
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label)
{
 ...
 */
TR::Snippet* OMR::Z::CodeGenerator::getFirstSnippet()
{
    TR::Snippet* s = NULL;
...
// OMR::Z::CodeGenerator::Constant Data List Functions
////////////////////////////////////////////////////////////////////////////////
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node* node, void* c, uint16_t size)
{
    CS2::HashIndex hi;
    ...
    if (self()->profiledPointersRequireRelocation() && node
        && (node->getOpCodeValue() == TR::aconst && (node->isClassPointerConstant() || node->isMethodPointerConstant())
            || node->getOpCodeValue() == TR::loadaddr && node->getSymbol()->isClassObject() || isLiteralPoolAddress))
        key.node = node;
    else
        key.node = NULL;
...
TR::S390ConstantInstructionSnippet* OMR::Z::CodeGenerator::createConstantInstruction(
    TR::Node* node, TR::Instruction* instr)
{
    TR::S390ConstantInstructionSnippet* cis
        = new (self()->trHeapMemory()) TR::S390ConstantInstructionSnippet(self(), node, instr);
    _constantList.push_front(cis);
    return cis;
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::CreateConstant(
    TR::Node* node, void* c, uint16_t size, bool writable)
{
...
void OMR::Z::CodeGenerator::addDataConstantSnippet(TR::S390ConstantDataSnippet* snippet)
{
 ...
 */
int32_t OMR::Z::CodeGenerator::setEstimatedOffsetForConstantDataSnippets()
{
    TR::S390ConstantDataSnippet* cursor;
    bool first;
    int32_t size;
...
#define HANDLE_CONSTANT_SNIPPET(cursor, pow2Size) (cursor->getConstantSize() == pow2Size)
...
int32_t OMR::Z::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart)
{
    TR::S390ConstantDataSnippet* cursor;
    bool first;
    int32_t size;
    ...
    estimatedSnippetStart = ((estimatedSnippetStart + 7) / 8 * 8);
    for (auto snippetDataIterator = _snippetDataList.begin(); snippetDataIterator != _snippetDataList.end();
         ++snippetDataIterator) {
...
void OMR::Z::CodeGenerator::emitDataSnippets()
{
    ...
    for (exp = self()->constantDataSnippetExponent(); exp > 0; exp--) {
        ...
        for (auto iterator = _constantList.begin(); iterator != _constantList.end(); ++iterator) {
            ...
            if (HANDLE_CONSTANT_SNIPPET((*iterator), size)) {
                if (first) {
                    first = false;
                    self()->setBinaryBufferCursor(
                        (uint8_t*)(((uintptr_t)(self()->getBinaryBufferCursor() + size - 1) / size) * size));
                }
...
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::create64BitLiteralPoolSnippet(TR::DataType dt, int64_t value)
{
...
TR::Linkage* OMR::Z::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
    TR::Linkage* linkage;
    TR::Compilation* comp = self()->comp();
    switch (lc) {
...
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::createLiteralPoolSnippet(TR::Node* node)
{
...
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate2ByteConstant(TR::Node* node, int16_t c)
{
    return self()->findOrCreateConstant(node, &c, 2);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate4ByteConstant(TR::Node* node, int32_t c)
{
    return self()->findOrCreateConstant(node, &c, 4);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate8ByteConstant(TR::Node* node, int64_t c)
{
    return self()->findOrCreateConstant(node, &c, 8);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::Create4ByteConstant(TR::Node* node, int32_t c, bool writable)
{
    return self()->CreateConstant(node, &c, 4, writable);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::Create8ByteConstant(TR::Node* node, int64_t c, bool writable)
{
    return self()->CreateConstant(node, &c, 8, writable);
}

TR::S390WritableDataSnippet* OMR::Z::CodeGenerator::CreateWritableConstant(TR::Node* node)
{
    if (self()->comp()->target().is64Bit()) {
...
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::getFirstConstantData()
{
...
void OMR::Z::CodeGenerator::freeAndResetTransientLongs()
{
    int32_t num_lReg;
...
TR_S390OutOfLineCodeSection* OMR::Z::CodeGenerator::findOutLinedInstructionsFromLabel(TR::LabelSymbol* label)
{
    auto oiIterator = self()->getS390OutOfLineCodeSectionList().begin();
    while (oiIterator != self()->getS390OutOfLineCodeSectionList().end()) {
        if ((*oiIterator)->getEntryLabel() == label)
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::dumpDataSnippets(TR::FILE* outFile)
{
...
void OMR::Z::CodeGenerator::setGlobalStaticBaseRegisterOnFlag()
{
 ...
 */
bool OMR::Z::CodeGenerator::canUseRelativeLongInstructions(int64_t value)
{
...
bool OMR::Z::CodeGenerator::supportsOnDemandLiteralPool()
{
...
TR::Register* OMR::Z::CodeGenerator::evaluateLengthMinusOneForMemoryOps(
    TR::Node* node, bool clobberEvaluate, bool& lenMinusOne)
{
    TR::Register* reg;

    if ((node->getReferenceCount() == 1) && self()->supportsLengthMinusOneForMemoryOpts()
        && ((node->getOpCodeValue() == TR::iadd && node->getSecondChild()->getOpCodeValue() == TR::iconst
...
TR_GlobalRegisterNumber OMR::Z::CodeGenerator::getGlobalRegisterNumber(uint32_t realRegNum)
{
    return self()->machine()->getGlobalReg((TR::RealRegister::RegNum)(realRegNum + 1));
}

bool OMR::Z::CodeGenerator::bitwiseOpNeedsLiteralFromPool(TR::Node* parent, TR::Node* child)
{
...
{
    TR_ASSERT(
        _cgFlags.testAny(S390CG_reusableSlotIsFree), "The reusable temp slot was already allocated but not freed\n");

    // allocate the reusable slot
    _cgFlags.set(S390CG_reusableSlotIsFree, false);
    if (_reusableTempSlot == NULL)
        _reusableTempSlot
            = self()->comp()->getSymRefTab()->createTemporary(self()->comp()->getMethodSymbol(), TR::Int64);

    return _reusableTempSlot;
}

void OMR::Z::CodeGenerator::freeReusableTempSlot() { _cgFlags.set(S390CG_reusableSlotIsFree, true); }
...
bool OMR::Z::CodeGenerator::isActiveCompareCC(TR::InstOpCode::Mnemonic opcd, TR::Register* tReg, TR::Register* sReg)
{
    if (self()->hasCCInfo() && self()->hasCCCompare()) {
        ...
        if (opcd == ccInst->getOpCodeValue() && tReg == ccTgtReg && sReg == ccSrcReg
            && performTransformation(self()->comp(),
                "O^O isActiveCompareCC: RR Compare Op [%s\t %s, %s] can reuse CC from ccInstr [%p]\n",
                ccInst->getOpCode().getMnemonicName(), self()->getDebug()->getName(tReg),
                self()->getDebug()->getName(sReg), ccInst)) {
            return true;
        }
    }
    return false;
...
bool OMR::Z::CodeGenerator::isActiveArithmeticCC(TR::Register* tstReg)
{
    if (self()->hasCCInfo() && self()->hasCCSigned() && !self()->hasCCOverflow() && !self()->hasCCCarry()
        && performTransformation(self()->comp(),
            "O^O Arithmetic CC: CCInfo(%d) CCSigned(%d) CCOverflow(%d) CCCarry(%d)\n", self()->hasCCInfo(),
            self()->hasCCSigned(), self()->hasCCOverflow(), self()->hasCCCarry())) {
...
bool OMR::Z::CodeGenerator::excludeInvariantsFromGRAEnabled() { return true; }

uint32_t OMR::Z::CodeGenerator::getJitMethodEntryAlignmentBoundary() { return 256; }

/**
 * This function sign extended the specified number of high order bits in the register.
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(
    TR::Node* node, TR::Register* targetRegister, uint32_t numberOfBits)
{
 ...
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(
    TR::Node* node, TR::Register* targetRegister, TR::Register* srcRegister, uint32_t numberOfBits)
{
    switch (numberOfBits) {
    case 16:
 ...
 */
bool OMR::Z::CodeGenerator::clearHighOrderBits(TR::Node* node, TR::Register* targetRegister, uint32_t numberOfBits)
{
 ...
 * This function zero filled the specified number of high order bits in the register.
 */
bool OMR::Z::CodeGenerator::clearHighOrderBits(
    TR::Node* node, TR::Register* targetRegister, TR::Register* srcRegister, uint32_t numberOfBits)
{
    switch (numberOfBits) {
    case 8:
...
void OMR::Z::CodeGenerator::setUnavailableRegistersUsage(
    TR_Array<TR_BitVector>& liveOnEntryUsage, TR_Array<TR_BitVector>& liveOnExitUsage)
{
...
void OMR::Z::CodeGenerator::resetGlobalRegister(TR::RealRegister::RegNum reg, TR_BitVector& globalRegisters)
{
    int32_t globalRegNum = self()->machine()->getGlobalReg(reg);
    if (globalRegNum != -1)
        globalRegisters.reset(globalRegNum);
}

void OMR::Z::CodeGenerator::setGlobalRegister(TR::RealRegister::RegNum reg, TR_BitVector& globalRegisters)
{
    int32_t globalRegNum = self()->machine()->getGlobalReg(reg);
    if (globalRegNum != -1)
        globalRegisters.set(globalRegNum);
}

void OMR::Z::CodeGenerator::setRegister(TR::RealRegister::RegNum reg, TR_BitVector& registers)
{
    if ((int32_t)reg >= (int32_t)TR::RealRegister::FirstGPR)
        registers.set(REGINDEX(reg));
}

void OMR::Z::CodeGenerator::setUnavailableRegisters(TR::Block* b, TR_BitVector& unavailableRegisters) { }

void OMR::Z::CodeGenerator::removeUnavailableRegisters(
    TR::RegisterCandidate* rc, TR::Block** blocks, TR_BitVector& availableRegisters)
{
    ...
    while (loe.hasMoreElements()) {
        ...
        //      traceMsg(comp(),"For register candidate %d, considering block %d.  Looking at node
        //      %p\n",rc->getSymbolReference()->getReferenceNumber(),blockNumber,lastTreeTopNode);

        if (lastTreeTopNode->getNumChildren() > 0 && lastTreeTopNode->getFirstChild()->getOpCode().isFunctionCall()
            && lastTreeTopNode->getFirstChild()
                   ->getOpCode()
                   .isJumpWithMultipleTargets()) // ensure calls that have multiple targets remove all volatile
                                                 // registers from available registers
        {
            TR_LinkageConventions callLinkage
                = lastTreeTopNode->getFirstChild()->getSymbol()->castToMethodSymbol()->getLinkageConvention();
...
void OMR::Z::CodeGenerator::genMemCpy(TR::MemoryReference* targetMR, TR::Node* targetNode,
    TR::MemoryReference* sourceMR, TR::Node* sourceNode, int64_t copySize)
{
...
void OMR::Z::CodeGenerator::genMemClear(TR::MemoryReference* targetMR, TR::Node* targetNode, int64_t clearSize)
{
 ...
 */
void OMR::Z::CodeGenerator::genCopyFromLiteralPool(TR::Node* node, int32_t bytesToCopy, TR::MemoryReference* targetMR,
    size_t litPoolOffset, TR::InstOpCode::Mnemonic op)
{
    ...
    TR::MemoryReference* paddingBytesMR
        = generateS390ConstantAreaMemoryReference(litPoolBaseReg, litPoolOffset, node, self(), true); // forSS=true
    int32_t mvcSize = bytesToCopy - 1;
    generateSS1Instruction(self(), op, node, mvcSize, targetMR, paddingBytesMR);

    if (stopUsing)
        self()->stopUsingRegister(litPoolBaseReg);
}

TR::Instruction* OMR::Z::CodeGenerator::genLoadAddressToRegister(
    TR::Register* reg, TR::MemoryReference* origMR, TR::Node* node, TR::Instruction* preced)
{
 ...
 */
bool OMR::Z::CodeGenerator::usesImplicit64BitGPRs(TR::Node* node) { return false; }

bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
    if (node->getOpCode().isPackedRightShift()) {
        // The most efficient way to perform a packed decimal right shift by a non-zero even amount when the round
        // amount is also zero is to use an MVN instruction. However since this instruction will perform the 'shift' by
        // moving the sign code to the left there are right aligned dead bytes and implied left aligned zero bytes
        // introduced. These dead and implied bytes cannot be within a user variable as the actual full value must be
        // materialized therefore require a temporary to be used for these even right shifts.
        //
...
#endif
        if (TR::MemoryReference::typeNeedsAlignment(node) && node->getType().isAggregate()
...
bool OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node* node)
{
    ...
    if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
        // phase check as node->getRegister() union is only valid past this point (the
        // TR::CodeGenPhase::SetupForInstructionSelectionPhase will NULL this out before inst selection)
        TR_ASSERT(self()->getCodeGeneratorPhase() > TR::CodeGenPhase::SetupForInstructionSelectionPhase,
            "must be past TR::CodeGenPhase::SetupForInstructionSelectionPhase (current is %s)\n",
            self()->getCodeGeneratorPhase().getName());
        if (node->getRegister() == NULL && endHint) {
            // do not have to fully descend a tree looking for overlapping operations in the special (and actually quite
            // common) cases where a node will initialize to a new storageReference (endHintOnOperation check) and one
            // of the conditions below is true 1) a conversion between types that use storageRefs (BCD and Aggr) and
            // types that don't (e.g. int,float) -- i2pd,i2o etc. It is always safe to end the search
            //    because one side does not use storagRefs (no chance for overlap) and the other will always get a new
            //    storageRef.
            // 2) a conversion between types that both use storageRefs AND the evaluator has been fixed up to detect the
            // overlap conditions and privatize to a new temp (pd2zd,ud2pd etc)
            //
            //    zdstore "a"
            //       pd2zd
            //          pdload "b"
            ...
            // endAccumulatorSearchOnOperation will return true to allow the pd2zd to UNPK (accumulate) right to the
            // store location "a" as the pd2zd will check and deal with any "a" and "b" overlap if any exists.
            //

            if (node->getOpCode().isConversion()
                    && (self()->isStorageReferenceType(node) && !self()->isStorageReferenceType(node->getFirstChild()))
                || // i2pd
                (!self()->isStorageReferenceType(node)
                    && self()->isStorageReferenceType(node->getFirstChild()))) // pd2i
            {
                // case1 : i2pd,pd2l,i2o,o2i...
                return true;
            } else {
                // case2: evaluators below have been fixed up to privatize only if needed during instruction selection
                // (privatizeBCDRegisterIfNeeded)
...
bool OMR::Z::CodeGenerator::endHintOnOperation(TR::Node* node)
{
    ...
    if (node->getOpCode().isConversion() && node->getOpCode().isBinaryCodedDecimalOp()
        && !TR::ILOpCode::isZonedToZonedConversion(node->getOpCodeValue()) && node->getType().isBCD()) {
        return true;
    } else
#endif
        if (node->getOpCode().isConversion() && node->getType().isAggregate()) {
        return true;
    }
#ifdef J9_PROJECT_SPECIFIC
        ...
        && isOdd(node->getDecimalAdjust())) // odd left shifts need a new temp
    {
        ...
        //       pdadd <- if pdadd is tagged with store hint <a> then it will need to be copied to a new temp *and* the
        //       pdshl cannot use <a> either iconst 3 (some oddShiftAmount)
        return true;
    }
#endif
    else {
...
bool OMR::Z::CodeGenerator::useRippleCopyOrXC(size_t size, char* lit)
{
    ...
    char byteOne = lit[0];
    if (size <= TR_MAX_MVC_SIZE && (size >= TR_MIN_RIPPLE_COPY_SIZE || byteOne == 0)) {
        size_t i = 1;
        for (i = 1; i < size; i++) {
            if (lit[i] != byteOne)
                break;
...
void OMR::Z::CodeGenerator::setUsesZeroBasePtr(bool v) { _cgFlags.set(S390CG_usesZeroBasePtr, v); }

bool OMR::Z::CodeGenerator::getUsesZeroBasePtr() { return _cgFlags.testAny(S390CG_usesZeroBasePtr); }
...
/**
 * Create a snippet of the debug counter address and generate a DCB (DebugCounterBump) pseudo-instruction that will be
 * ignored during register assignment Be aware that the constituent add immediate instruction (ASI/AGSI) sets condition
 * codes for sign and overflow In most cases the condition code is 2; for result greater than zero and no overflow
 *
 ...
 * @param cond       Register conditions for debug counters are now deprecated on z should be phased out when P
 * architecture eliminates them
 */
TR::Instruction* OMR::Z::CodeGenerator::generateDebugCounterBump(
    TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR::RegisterDependencyConditions* cond)
{
    TR::Snippet* constant = self()->Create8ByteConstant(cursor->getNode(),
        reinterpret_cast<intptr_t>(
            counter->getBumpCountSymRef(self()->comp())->getSymbol()->getStaticSymbol()->getStaticAddress()),
        false);
    return generateS390DebugCounterBumpInstruction(
        self(), TR::InstOpCode::DCB, cursor->getNode(), constant, delta, cursor);
}

TR::Instruction* OMR::Z::CodeGenerator::generateDebugCounterBump(TR::Instruction* cursor, TR::DebugCounterBase* counter,
...
TR::Instruction* OMR::Z::CodeGenerator::generateDebugCounterBump(
    TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR_ScratchRegisterManager& srm)
{
    return cursor;
}

TR::Instruction* OMR::Z::CodeGenerator::generateDebugCounterBump(
    TR::Instruction* cursor, TR::DebugCounterBase* counter, TR::Register* deltaReg, TR_ScratchRegisterManager& srm)
{
    return cursor;
}

bool OMR::Z::CodeGenerator::opCodeIsNoOpOnThisPlatform(TR::ILOpCode& opCode)
{
...
TR_BackingStore* OMR::Z::CodeGenerator::getLocalF2ISpill()
{
...
TR_BackingStore* OMR::Z::CodeGenerator::getLocalD2LSpill()
{
...
bool OMR::Z::CodeGenerator::getSupportsConstantOffsetInAddressing(int64_t value)
{
 ...
 */
bool OMR::Z::CodeGenerator::isDispInRange(int64_t disp) { return (MINLONGDISP <= disp) && (disp <= MAXLONGDISP); }

bool OMR::Z::CodeGenerator::getSupportsOpCodeForAutoSIMD(
    TR::CPU* cpu, TR::ILOpCode opcode, bool supportsVectorRegisters)
{
    ...
    TR_ASSERT_FATAL(
        et == TR::Int8 || et == TR::Int16 || et == TR::Int32 || et == TR::Int64 || et == TR::Float || et == TR::Double,
     ...
     */
    if (!supportsVectorRegisters
        || (et == TR::Float && !cpu->supportsFeature(OMR_FEATURE_S390_VECTOR_FACILITY_ENHANCEMENT_1))) {
        return false;
    }
...
{
    return TR::CodeGenerator::getSupportsOpCodeForAutoSIMD(
        &self()->comp()->target().cpu, opcode, self()->getSupportsVectorRegisters());
}

// simple syntactic test for matching direct loads when the caller knows (by some mechanism, e.g.
// _symRefsToCheckForKills) that there is no intervening kill to the load memory
bool OMR::Z::CodeGenerator::directLoadAddressMatch(TR::Node* load1, TR::Node* load2, bool trace)
{
...
bool OMR::Z::CodeGenerator::isOutOf32BitPositiveRange(int64_t value, bool trace)
{
...
int32_t OMR::Z::CodeGenerator::getMaskSize(int32_t leftMostNibble, int32_t nibbleCount)
{
    bool leftMostNibbleIsEven = (leftMostNibble & 0x1) == 0;
    int32_t maskNibbles = nibbleCount;
    if (leftMostNibbleIsEven)
        maskNibbles++; // increase maskNibbles to include the top nibble
    if (maskNibbles & 0x1)
        maskNibbles++; // if odd then increase to include the bot nibble
...
// 1) circular evaluation (asserts) : node->getRegister() == NULL && node->getOpCode().canHaveStorageReferenceHint()
// below 2) clobbering of value child during address child evaluation : node->getType().isBCD() below A 'conflicting'
// address node is a BCD/Aggr or canHaveStorageReferenceHint node that is commoned some where under both the address
// child and value child of the same indirect store
//
...
// In either case what happens is that the pd2zd is evaluated first and evaluate its child pdX and use it right away to
// from the source memRef Then shortly after, when a hint is used, the address child is evaluated to form the dest
// memRef. When the parent of the pdX in the address child (the pdadd here) has an already initialized child then it
// does a clobber evaluate. The clobber evaluate will update the storageRef on the pdX register to a new temp and then
// clobber the storageRef already on the pdX Normally this is ok as future references to the pdX node will use the new
// temp BUT in this case the pd2zd in the value child has already used the original storageRef to form its source memRef
// so it's too late to pick up the new temp
//
// The fix for both of these problems is to disallow a hint to break the evaluation relationship/dependency between the
// value child and address child

// Z
bool OMR::Z::CodeGenerator::possiblyConflictingNode(TR::Node* node)
{
    ...
    if (node->getReferenceCount() > 1) {
        if (node->getRegister() == NULL && node->getOpCode().canHaveStorageReferenceHint()) {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tpossiblyConflicting=true %s (%p) : reg==NULL and storageRefHint_Case, refCount %d\n",
                    node->getOpCode().getName(), node, node->getReferenceCount());
            possiblyConflicting = true;
        } else if (node->getType().isBCD()) {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tpossiblyConflicting=true %s (%p) : BCDOrAggrType_Case, reg=%s, refCount %d\n",
                    node->getOpCode().getName(), node,
                    node->getRegister() ? self()->getDebug()->getName(node->getRegister()) : "NULL",
                    node->getReferenceCount());
            possiblyConflicting = true;
        }
    }
...
// Z
bool OMR::Z::CodeGenerator::foundConflictingNode(TR::Node* node, TR::list<TR::Node*>* conflictingAddressNodes)
{
    bool foundConflicting = false;
    if (!conflictingAddressNodes->empty() && self()->possiblyConflictingNode(node)
        && (std::find(conflictingAddressNodes->begin(), conflictingAddressNodes->end(), node)
            != conflictingAddressNodes->end())) {
...
// Z
void OMR::Z::CodeGenerator::collectConflictingAddressNodes(
    TR::Node* parent, TR::Node* node, TR::list<TR::Node*>* conflictingAddressNodes)
{
    if (self()->traceBCDCodeGen())
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node* node1, TR::Node* node2)
{
    TR_ASSERT(node1->getOpCode().isLoadVar() || node1->getOpCode().isStore(),
        "node1 %s (%p) should be a loadVar or a store\n", node1->getOpCode().getName(), node1);
    TR_ASSERT(node2->getOpCode().isLoadVar() || node2->getOpCode().isStore(),
        "node2 %s (%p) should be a loadVar or a store\n", node2->getOpCode().getName(), node2);
    ...
    if (node1->getSize() != node2->getSize()) {
        if (self()->comp()->getOption(TR_TraceCG))
            traceMsg(self()->comp(),
                "\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d and node2 %s (%p) size "
                "= %d\n",
                node1->getOpCode().getName(), node1, node1->getSize(), node2->getOpCode().getName(), node2,
                node2->getSize());
        return false;
    }

    if (node1->getSymbolReference() == node2->getSymbolReference() || !0) // will be removed after code is stable
    {
...
bool OMR::Z::CodeGenerator::reliesOnAParticularSignEncoding(TR::Node* node)
{
    TR::ILOpCode op = node->getOpCode();
    if (op.isBCDStore()
        && !node->chkCleanSignInPDStoreEvaluator()) // most common case of needing so put up-front -- the value escapes
        return true;
    ...
    // left shifts do not rely on a clean sign so the only thing to check is if a clean sign property has been
    // propagated thru it (if not then can return false)
...
{
    TR_ASSERT(store->getOpCode().hasSymbolReference() && store->getSymbolReference(),
        "store %s (%p) must have a symRef\n", store->getOpCode().getName(), store);
...
bool OMR::Z::CodeGenerator::checkIfcmpxx(TR::Node* node)
{
    ...
    bool firstChildOK = (firstChildNode->getOpCode().isLoadVar() || firstChildNode->getOpCode().isLoadConst())
        && firstChildNode->getType().isIntegral() && firstChildNode->getReferenceCount() == 1;

    bool secondChildOK = firstChildOK
        && (secondChildNode->getOpCode().isLoadVar() || secondChildNode->getOpCode().isLoadConst())
        && secondChildNode->getType().isIntegral() && secondChildNode->getReferenceCount() == 1;

    if (secondChildOK && firstChildNode->getSize() == secondChildNode->getSize()) {
        if (secondChildNode->getOpCode().isLoadConst() && secondChildNode->getIntegerNodeValue<int64_t>() == 0) {
...
bool OMR::Z::CodeGenerator::checkBitWiseChild(TR::Node* node)
{
    if (node->getOpCode().isLoadConst() || (node->getOpCode().isLoadVar() && node->getReferenceCount() == 1)) {
...
TR_StorageDestructiveOverlapInfo OMR::Z::CodeGenerator::getStorageDestructiveOverlapInfo(
    TR::Node* src, size_t srcLength, TR::Node* dst, size_t dstLength)
{
    ...
    if (srcLength == 1 && dstLength == 1) {
        ...
        case TR_DestructiveOverlap:
            overlapInfo = TR_DefinitelyDestructiveOverlap;
            break;
        case TR_MayOverlap:
        default:
            overlapInfo = TR_UnknownOverlap;
            break;
        }
    }
...
//     PassThrough GRX
//       => iadd
void OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node* node, vcount_t visitCount)
{
    ...
    for (int32_t i = 0; i < regDeps->getNumChildren(); i++) {
        ...
        if (regLoad->getGlobalRegisterNumber() == passThrough->getGlobalRegisterNumber()
            && passThrough->getNumChildren() >= 1 && passThrough->getFirstChild() == addNode) {
            found = true;
            break;
        }
    }
    ...
    // Make sure that the only uses of the add/sub node, so far, are the three we expect: under the iRegStore, under the
    // compare, and in the GLRegDeps under the compare. futureUseCount should've been initialized to be referenceCount -
    // 1 (which will have to be an iRegStore in order for us to see the add node under GLRegDeps here). The compare node
    // here will account for the other two uses, so as long as there's no other uses between the iRegStore and the
    // ificmpgt, all is well.
    ...
    if (!performTransformation(self()->comp(), "%s Found branch on count opportunity\n", OPT_DETAILS))
        return;
    dumpOptDetails(
        self()->comp(), "Branch on count opportunity found (compare: 0x%p\tincrement: 0x%p)\n", node, addNode);
...
// Z only
bool OMR::Z::CodeGenerator::isCompressedClassPointerOfObjectHeader(TR::Node* node)
{
    return (TR::Compiler->om.generateCompressedObjectHeaders() && node->getOpCode().hasSymbolReference()
        && (node->getSymbol()->isClassObject()
            || node->getSymbolReference() == self()->getSymRefTab()->findVftSymbolRef()));
}

bool OMR::Z::CodeGenerator::directCallRequiresTrampoline(intptr_t targetAddress, intptr_t sourceAddress)
{
    return !self()->comp()->target().cpu.isTargetWithinBranchRelativeRILRange(targetAddress, sourceAddress)
