...
void OMR::Z::CodeGenerator::checkIsUnneededAloadi(TR::Node *parent, TR::Node *node, TR::TreeTop *tt)
{
...
void OMR::Z::CodeGenerator::lowerTreeIfNeeded(TR::Node *node, int32_t childNumberOfNode, TR::Node *parent,
    TR::TreeTop *tt)
{
    ...
    if (self()->comp()->target().cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore())
        ...
        if (add1 && add2 && const1
            && performTransformation(self()->comp(), "%sBase/index/displacement form addressing prep for node [%p]\n",
                OPT_DETAILS, node)) {
    ...
    if (!disableLXAUncommoning && (node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aladd)
        && !parent->getOpCode().isLoad() && self()->getUseLXAInstructions()) {
...
void OMR::Z::CodeGenerator::initialize()
{
    ...
    static bool canEmulateLXA = TR::InstOpCode(TR::InstOpCode::LXAB).canEmulate()
        && TR::InstOpCode(TR::InstOpCode::LXAH).canEmulate() && TR::InstOpCode(TR::InstOpCode::LXAF).canEmulate()
        && TR::InstOpCode(TR::InstOpCode::LXAG).canEmulate() && TR::InstOpCode(TR::InstOpCode::LXAQ).canEmulate();
...
bool OMR::Z::CodeGenerator::mulDecompositionCostIsJustified(int32_t numOfOperations, char bitPosition[],
...
TR::Instruction *OMR::Z::CodeGenerator::insertPad(TR::Node *node, TR::Instruction *cursor, uint32_t size,
    bool prependCursor)
{
    if (size != 0) {
        if (prependCursor) {
        ...
        switch (size) {
            case 2: {
                cursor
                    = new (self()->trHeapMemory()) TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, node, cursor, self());
            } break;

            case 4: {
                cursor
                    = new (self()->trHeapMemory()) TR::S390NOPInstruction(TR::InstOpCode::NOP, 4, node, cursor, self());
            } break;
...
void OMR::Z::CodeGenerator::beginInstructionSelection()
{
    ...
    if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
        ...
        if (self()->comp()->getJittedMethodSymbol()->isJNI()
            ...
            cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(TR::InstOpCode::dd, startNode,
                UPPER_4_BYTES(jniMethodTargetAddress), cursor, self());

            if (self()->comp()->target().is64Bit()) {
                cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(TR::InstOpCode::dd, startNode,
                    LOWER_4_BYTES(jniMethodTargetAddress), cursor, self());
            }
        }
...
void OMR::Z::CodeGenerator::createBranchPreloadCallData(TR::LabelSymbol *callLabel, TR::SymbolReference *callSymRef,
...
void OMR::Z::CodeGenerator::insertInstructionPrefetches()
{
    if (self()->supportsBranchPreload()) {
        if (_hottestReturn._frequency > 6) {
            ...
            cursor = generateS390BranchPredictionPreloadInstruction(self(), TR::InstOpCode::BPP, node,
                _hottestReturn._returnLabel, (int8_t)0x6, tempMR, cursor);
...
void OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(TR_BranchPreloadCallData *data)
{
    ...
    if (bppMaxPath) {
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            if (bppCall && (!canReachWithBPRP || bppOnly)) {
                ...
                tempMR = generateS390MemoryReference(tempReg, 0, self());
                cursor = generateS390BranchPredictionPreloadInstruction(self(), TR::InstOpCode::BPP, node,
                    data->_callLabel, (int8_t)0xD, tempMR, cursor);
                cursor->setNext(second);
                self()->stopUsingRegister(tempReg);

            } else {
                cursor = generateS390BranchPredictionRelativePreloadInstruction(self(), TR::InstOpCode::BPRP, node,
                    data->_callLabel, (int8_t)0xD, data->_callSymRef, cursor);
                cursor->setNext(second);
            }
        }
    }
    if (!bppMaxPath) {
        ...
        if (bppCall && (!canReachWithBPRP || bppOnly)) {
            ...
            tempMR->setCreatedDuringInstructionSelection();
            cursor = generateS390BranchPredictionPreloadInstruction(self(), TR::InstOpCode::BPP, node, data->_callLabel,
                (int8_t)0xD, tempMR, cursor);
            cursor->setNext(second);
            self()->stopUsingRegister(tempReg);
...
void OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(TR::S390ConstantDataSnippet *ifcSnippet,
    TR::list<TR_OpaqueClassBlock *> *PICSlist)
{
...
void OMR::Z::CodeGenerator::doBinaryEncoding()
{
    ...
    _methodEnd = generateS390LabelInstruction(self(), TR::InstOpCode::label, self()->comp()->findLastTree()->getNode(),
        generateLabelSymbol(self()));
    ...
    if (self()->comp()->getOption(TR_TraceCG))
        self()->comp()->getDebug()->dumpMethodInstrs(self()->comp()->getOutFile(),
            "Post Prologue/epilogue Instructions", false);
    ...
    while (data.cursorInstruction) {
        ...
        TR_ASSERT(data.cursorInstruction->getEstimatedBinaryLength()
...
TR::RegisterPair *OMR::Z::CodeGenerator::allocateConsecutiveRegisterPair(TR::Register *lowRegister,
            ...
            if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        ...
                        self()->getDebug()->getName(static_cast<TR::Register *>(reg)),
                        ref && ref->isNodeBased() ? "yes" : "no",
                        ref->isNodeBased() ? ref->getNode()->getOpCode().getName() : "",
                        ref->isNodeBased() ? ref->getNode() : NULL);
                self()->processUnusedStorageRef(ref);
...
TR::RegisterDependencyConditions *OMR::Z::CodeGenerator::createDepsForRRMemoryInstructions(TR::Node *node,
    TR::RegisterPair *sourceReg, TR::RegisterPair *targetReg, uint8_t extraDeps)
{
    uint8_t numDeps = 8
...
////////////////////////////////////////////////////////////////////////////////
//  Tactical GRA
TR_GlobalRegisterNumber OMR::Z::CodeGenerator::getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex,
    TR::DataType type)
{
...
void OMR::Z::CodeGenerator::simulateNodeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
...
#ifdef J9_PROJECT_SPECIFIC
TR_OpaquePseudoRegister *OMR::Z::CodeGenerator::ssrClobberEvaluate(TR::Node *node,
    TR::MemoryReference *sourceMR) // sourceMR can be NULL
{
    TR::CodeGenerator *cg = self();
    ...
    if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
        ...
        if (isBCD)
            copyMR = generateS390RightAlignedMemoryReference(node, copyStorageReference, cg, true,
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\tupdate srcRegister %s with copy ref #%d (%s)\n",
                ...
                self()->getDebug()->getName(copyStorageReference->getSymbol()));
        srcRegister->setStorageReference(copyStorageReference,
            node); // clears leftAlignedZeroDigits so query getDigitsToClear above
...
void OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *label,
    int8_t addressDifferenceDivisor, bool isInstrOffset)
{
    if (isInstrOffset) {
        uint16_t *label_cursor = (uint16_t *)label->getCodeLocation();
        if ((*(uint16_t *)label_cursor) == bos(0x1800))
...
TR::S390ConstantInstructionSnippet *OMR::Z::CodeGenerator::createConstantInstruction(TR::Node *node,
    TR::Instruction *instr)
{
...
TR::S390ConstantDataSnippet *OMR::Z::CodeGenerator::CreateConstant(TR::Node *node, void *c, uint16_t size,
    bool writable)
{
    if (writable) {
...
TR::Register *OMR::Z::CodeGenerator::evaluateLengthMinusOneForMemoryOps(TR::Node *node, bool clobberEvaluate,
    bool &lenMinusOne)
{
    TR::Register *reg;
...
{
    TR_ASSERT(_cgFlags.testAny(S390CG_reusableSlotIsFree),
        "The reusable temp slot was already allocated but not freed\n");
 ...
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(TR::Node *node, TR::Register *targetRegister,
 ...
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(TR::Node *node, TR::Register *targetRegister,
    TR::Register *srcRegister, uint32_t numberOfBits)
{
    switch (numberOfBits) {
        case 16:
 ...
 * This function zero filled the specified number of high order bits in the register.
 */
bool OMR::Z::CodeGenerator::clearHighOrderBits(TR::Node *node, TR::Register *targetRegister, TR::Register *srcRegister,
    uint32_t numberOfBits)
{
    switch (numberOfBits) {
        case 8:
...
void OMR::Z::CodeGenerator::setUnavailableRegistersUsage(TR_Array<TR_BitVector> &liveOnEntryUsage,
    TR_Array<TR_BitVector> &liveOnExitUsage)
{
...
void OMR::Z::CodeGenerator::removeUnavailableRegisters(TR::RegisterCandidate *rc, TR::Block **blocks,
...
TR::Instruction *OMR::Z::CodeGenerator::genLoadAddressToRegister(TR::Register *reg, TR::MemoryReference *origMR,
    TR::Node *node, TR::Instruction *preced)
{
...
TR::Instruction *OMR::Z::CodeGenerator::generateDebugCounterBump(TR::Instruction *cursor, TR::DebugCounterBase *counter,
    int32_t delta, TR::RegisterDependencyConditions *cond)
{
    TR::Snippet *constant = self()->Create8ByteConstant(cursor->getNode(),
        ...
        false);
    return generateS390DebugCounterBumpInstruction(self(), TR::InstOpCode::DCB, cursor->getNode(), constant, delta,
        cursor);
}
...
TR::Instruction *OMR::Z::CodeGenerator::generateDebugCounterBump(TR::Instruction *cursor, TR::DebugCounterBase *counter,
    int32_t delta, TR_ScratchRegisterManager &srm)
{
    return cursor;
}
...
bool OMR::Z::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::CPU *cpu, TR::ILOpCode opcode,
    bool supportsVectorRegisters)
{
...
bool OMR::Z::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode)
{
    return TR::CodeGenerator::getSupportsOpCodeForAutoSIMD(&self()->comp()->target().cpu, opcode,
        self()->getSupportsVectorRegisters());
}
...
// Z
void OMR::Z::CodeGenerator::collectConflictingAddressNodes(TR::Node *parent, TR::Node *node,
...
TR_StorageDestructiveOverlapInfo OMR::Z::CodeGenerator::getStorageDestructiveOverlapInfo(TR::Node *src,
    size_t srcLength, TR::Node *dst, size_t dstLength)
{
