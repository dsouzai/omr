...
extern TR::Instruction* generateS390PackedCompareAndBranchOps(TR::Node* node, TR::CodeGenerator* cg,
...
extern TR::Register* iDivRemGenericEvaluator(
    TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR);
...
void killRegisterIfNotLocked(TR::CodeGenerator* cg, TR::RealRegister::RegNum reg, TR::Instruction* instr,
...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->compileRelocatableCode()) {
        ...
        default:
            TR_ASSERT_FATAL(0,
                "got AOT guard in node but virtual guard not one of known guards supported for AOT. Guard: %d",
                virtualGuard->getKind());
            break;
        }
...
static TR::Register* generateS390lcmpEvaluator64(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic brOp,
    TR::InstOpCode::S390BranchCondition brCond, bool isBoolean)
{
    TR::RegisterDependencyConditions* deps = NULL;
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        generateS390ImmOp(
            cg, isUnsigned ? TR::InstOpCode::CLG : TR::InstOpCode::CG, node, cmpRegister, cmpRegister, long_value);
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
        generateS390CompareAndBranchInstruction(cg, isUnsigned ? TR::InstOpCode::CLGR : TR::InstOpCode::CGR, node,
            cmpRegister, srcReg, brCond, isTrue, false, false);
    }
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
inline TR::InstOpCode::S390BranchCondition generateS390Compare(TR::Node* node, TR::CodeGenerator* cg,
...
static TR::Register* xmaxxminHelper(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic compareRROp,
    TR::InstOpCode::S390BranchCondition branchCond, TR::InstOpCode::Mnemonic moveRROp)
{
    ...
    if (node->getOpCode().isFloatingPoint()) {
        ...
        generateRREInstruction(
            cg, node->getOpCode().isDouble() ? TR::InstOpCode::LTDBR : TR::InstOpCode::LTEBR, node, lhsReg, lhsReg);
...
static TR::Register* imaximinHelper(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic compareRROp,
    TR::InstOpCode::S390BranchCondition branchCond, TR::InstOpCode::Mnemonic moveRROp)
{
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
        ...
        generateRRFInstruction(cg, TR::InstOpCode::SELR, node, resultReg, rhsReg, lhsReg,
            getMaskForBranchCondition(getReverseBranchCondition(branchCond)));
        ...
        generateRRInstruction(cg, TR::InstOpCode::CR, node, lhsReg, rhsReg);
        generateRRFInstruction(cg, TR::InstOpCode::LOCR, node, lhsReg, rhsReg,
            getMaskForBranchCondition(getReverseBranchCondition(branchCond)), true);
...
static TR::Register* lmaxlminHelper(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic compareRROp,
    TR::InstOpCode::S390BranchCondition branchCond, TR::InstOpCode::Mnemonic moveRROp)
{
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
        TR::Node* lhsNode = node->getChild(0);
        ...
        generateRRFInstruction(cg, TR::InstOpCode::SELGR, node, resultReg, rhsReg, lhsReg,
            getMaskForBranchCondition(getReverseBranchCondition(branchCond)));
        ...
        generateRREInstruction(cg, TR::InstOpCode::CGR, node, lhsReg, rhsReg);
        generateRRFInstruction(cg, TR::InstOpCode::LOCGR, node, lhsReg, rhsReg,
            getMaskForBranchCondition(getReverseBranchCondition(branchCond)), true);
...
TR::Register* OMR::Z::TreeEvaluator::imaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return imaximinHelper(node, cg, TR::InstOpCode::CR, TR::InstOpCode::COND_BHR, TR::InstOpCode::LR);
}

TR::Register* OMR::Z::TreeEvaluator::lmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return lmaxlminHelper(node, cg, TR::InstOpCode::CGR, TR::InstOpCode::COND_BHR, TR::InstOpCode::LGR);
}

TR::Register* OMR::Z::TreeEvaluator::fmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::dmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // Passing COND_MASK10 to check CC if operand 1 is already greater than, or equal to operand 2
    return xmaxxminHelper(node, cg, TR::InstOpCode::CDBR, TR::InstOpCode::COND_MASK10, TR::InstOpCode::LDR);
}

TR::Register* OMR::Z::TreeEvaluator::iminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return imaximinHelper(node, cg, TR::InstOpCode::CR, TR::InstOpCode::COND_BLR, TR::InstOpCode::LR);
}

TR::Register* OMR::Z::TreeEvaluator::lminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return lmaxlminHelper(node, cg, TR::InstOpCode::CGR, TR::InstOpCode::COND_BLR, TR::InstOpCode::LGR);
}

TR::Register* OMR::Z::TreeEvaluator::fminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::dminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // Passing COND_MASK12 to check CC if operand 1 is already less than, or equal to operand 2
 ...
 */
TR::Register* lcmpHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::branchEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return NULL; }
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::igotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getNumChildren() > 1) {
        TR_ASSERT(node->getNumChildren() == 2 && node->getChild(1)->getOpCodeValue() == TR::GlRegDeps,
...
TR::Register* OMR::Z::TreeEvaluator::returnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((node->getOpCodeValue() == TR::Return) && node->isReturnDummy())
        return NULL;
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && (node->getNumChildren() > 1)
    ...
    case TR::ireturn:
        ...
        if (linkage->isNeedsWidening())
            new (cg->trHeapMemory())
                TR::S390RRInstruction(TR::InstOpCode::LGFR, node, returnValRegister, returnValRegister, cg);
        break;
    case TR::lreturn:
...
bool OMR::Z::TreeEvaluator::isCandidateForButestEvaluation(TR::Node* node)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    return node->getOpCode().isIf() && firstChild->getOpCodeValue() == TR::butest
        && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL
        && secondChild->getOpCode().isLoadConst() && secondChild->getType().isInt32();
}
...
bool OMR::Z::TreeEvaluator::isCandidateForCompareEvaluation(TR::Node* node)
{
    return node->getOpCode().isIf()
        ...
        && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isInt32();
}
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
            ...
            if (instr && instr->getNode() == node && instr->getDependencyConditions())
                mergedGuardDeps = new (cg->trHeapMemory())
                    TR::RegisterDependencyConditions(instr->getDependencyConditions(), 1, 0, cg);

            TR_VirtualGuardSite* site = virtualGuard->addNOPSite();
            if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifacmpeq
...

            {
                ...
                TR::RegisterDependencyConditions* deps
                    = new (cg->trHeapMemory()) TR::RegisterDependencyConditions((uint16_t)0, (uint16_t)0, cg);

                TR::Instruction* vgnopInstr = generateVirtualGuardNOPInstruction(
                    cg, node, site, deps, fallThroughLabel, instr ? instr->getPrev() : NULL);
                ...
                TR::Instruction* vgnopInstr = generateVirtualGuardNOPInstruction(
                    cg, node, site, mergedGuardDeps, label, instr ? instr->getPrev() : NULL);
                vgnopInstr->setNext(instr);
                cg->setAppendInstruction(instr);
            }
...
TR::Register* OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (cg->profiledPointersRequireRelocation() && node->isProfiledGuard()
    ...
    if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifacmpeq) {
        reg = generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE);
    } else {
        reg = generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
    }
    generateMergedGuardCodeIfNeeded(node, cg);
    return reg;
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ificmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool inlined = false;
    TR::Register* reg;
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ificmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool inlined = false;
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ificmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool inlined = false;
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ificmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    bool inlined = false;
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iflcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iflcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iflcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iflcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::iflcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifacmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifacmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpeqEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifbcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (node->getOpCodeValue() == TR::ifbcmpeq) {
        return generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE);
    } else {
        return generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifbcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifbcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifbcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifbcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifscmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifscmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpltEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifscmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifscmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifscmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpleEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifsucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpltEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifsucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifsucmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifsucmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::ificmpleEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::acmpeq) {
        TR::Node* firstChild = node->getFirstChild();
        TR::Node* secondChild = node->getSecondChild();
        if (node->getOpCodeValue() != TR::acmpeq && cg->comp()->target().is64Bit()
            && firstChild->getOpCodeValue() == TR::iand && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
            && secondChild->getOpCodeValue() == TR::iconst && secondChild->getInt() == 0
        ...
        return generateS390CompareBool(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
    }
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::icmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();

    if (!node->getOpCode().isUnsignedCompare() && firstChild->getReferenceCount() == 1 && firstChild->isNonNegative()
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::icmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::icmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();

    if (!node->getOpCode().isUnsignedCompare() && secondChild->getReferenceCount() == 1 && firstChild->isNonNegative()
        && secondChild->isNonNegative()
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::icmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::acmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::bcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::scmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::scmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpltEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::scmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::scmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::scmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpleEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpltEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sucmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sucmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::icmpleEvaluator(node, cg);
}
 ...
 *  -1 if child1 < child2 or unordered)
 */
TR::Register* OMR::Z::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return lcmpHelper64(node, cg);
}

void OMR::Z::TreeEvaluator::tableEvaluatorCaseLabelHelper(TR::Node* node, TR::CodeGenerator* cg,
    tableKind tableKindToBeEvaluated, int32_t numBranchTableEntries, TR::Register* selectorReg,
    TR::Register* branchTableReg, TR::Register* reg1)
{
    TR::Compilation* comp = cg->comp();
    {
        TR_ASSERT(reg1, "Java must have allocated a temp reg");
        TR_ASSERT(tableKindToBeEvaluated == AddressTable32bit || tableKindToBeEvaluated == AddressTable64bitIntLookup
                || tableKindToBeEvaluated == AddressTable64bitLongLookup,
            "For Java, must be using Address Table");

        new (cg->trHeapMemory()) TR::S390RIInstruction(
            TR::InstOpCode::BRAS, node, reg1, (4 + (sizeof(uintptr_t) * numBranchTableEntries)) / 2, cg);
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    int32_t i;
    ...
    TR::Node* secondChild = node->getSecondChild();
    TR::RegisterDependencyConditions* deps = NULL;
    bool isUnsigned = node->getFirstChild()->getOpCode().isUnsigned()
        || node->getFirstChild()->getOpCodeValue() == TR::bu2i; // TODO: check in  switchAnalyzer
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        TR_ASSERT(
            !node->getFirstChild()->getType().isInt64(), "Unhandled long selector type for table node %p\n", node);
        tableKindToBeEvaluated = AddressTable32bit;
    }

    /*
     * High level idea -- there are two modes, a relative table, where we load a relative offset from the table and
     * branch to it, and an address table, where we load an address out of the table and branch to it since cases are
     * indexed by 1, we need to shift by either 4 or 8 to get the next value in memory.
     */

    switch (tableKindToBeEvaluated) {
    case AddressTable32bit: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CL, node, selectorReg,
                static_cast<int32_t>(numBranchTableEntries), TR::InstOpCode::COND_BNL,
                node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), false,
                false); // make sure the case selector is within range of our case constants
    ...
    case AddressTable64bitIntLookup: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CL, node, selectorReg,
                static_cast<int32_t>(numBranchTableEntries), TR::InstOpCode::COND_BNL,
                node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), false, false);
    ...
    case AddressTable64bitLongLookup: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CLG, node, selectorReg,
                static_cast<int64_t>(numBranchTableEntries), TR::InstOpCode::COND_BNL,
                node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), false, false);
        generateRSInstruction(cg, TR::InstOpCode::SLLG, node, selectorReg, selectorReg, 3);
        generateRRInstruction(cg, TR::InstOpCode::LGFR, node, selectorReg, selectorReg);
    } break;
    ...
    TR::TreeEvaluator::tableEvaluatorCaseLabelHelper(
        node, cg, tableKindToBeEvaluated, numBranchTableEntries, selectorReg, branchTableReg, reg1);
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::lookupEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
        ...
        if (child->getNumChildren() > 0) {
            ...
            if (type.isInt64() && cg->comp()->target().is32Bit()) {
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, brCond, node, child->getBranchDestination()->getNode()->getLabel());
                generateS390LabelInstruction(cg, TR::InstOpCode::label, node, skipLoCompareLabel,
                    generateRegisterDependencyConditions(cg, child->getFirstChild(), 0), cursor);
            } else
                cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, brCond, node,
                    child->getBranchDestination()->getNode()->getLabel(),
 ...
 * Zero check an int.
 */
TR::Register* OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (valueToCheck->getOpCode().getOpCodeValue() == TR::acmpeq) {
        TR::InstOpCode::S390BranchCondition opBranchCond
            = generateS390CompareOps(valueToCheck, cg, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
    ...
    TR_S390OutOfLineCodeSection* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(node, TR::call, NULL, slowPathOOLLabel, doneLabel, cg);
...
void OMR::Z::TreeEvaluator::addToRegDep(
    TR::RegisterDependencyConditions* daaDeps, TR::Register* reg, bool mightUseRegPair)
{
    if (!reg)
        return;
...
void OMR::Z::TreeEvaluator::createDAACondDeps(
    TR::Node* node, TR::RegisterDependencyConditions* daaDeps, TR::Instruction* daaInstr, TR::CodeGenerator* cg)
{
        ...
        if (callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()) {
            if ((callNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                ...
                return TR::Node::create(TR::l2a, 1,
                    TR::Node::create(TR::ladd, 2, pdBufAddressNode,
                        TR::Node::create(TR::i2l, 1, TR::Node::create(TR::iadd, 2, pdBufPositionNode, index))));
            }
        }
    }
#endif
...
TR::Register* OMR::Z::TreeEvaluator::butestEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::inlineIfButestEvaluator(
    TR::Node* node, TR::CodeGenerator* cg, TR::Node* cmpOpNode, TR::Node* cmpValNode)
{
    TR_ASSERT(cmpValNode->getOpCode().isLoadConst() && cmpValNode->getType().isInt32(),
        "Expecting constant second child for if node, found node %s (%p)\n", cmpValNode->getOpCode().getName(),
        cmpValNode);
    ...
    TR::InstOpCode::S390BranchCondition cond
        = getButestBranchCondition(cmpOpNode->getOpCodeValue(), cmpValNode->getInt());
    TR::Instruction* instr = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, cond, cmpOpNode, cmpOpNode->getBranchDestination()->getNode()->getLabel(), deps);
...
TR::Register* OMR::Z::TreeEvaluator::inlineIfBifEvaluator(
    TR::Node* ifNode, TR::CodeGenerator* cg, TR::Node* cmpOpNode, TR::Node* cmpValNode)
{
    TR_ASSERT(cmpValNode->getOpCode().isLoadConst() && cmpValNode->getType().isInt32(),
        "Expecting constant second child for if node, found node %s (%p)\n", cmpValNode->getOpCode().getName(),
        cmpValNode);
    ...
    if (ifNode->getFirstChild()->getOpCodeValue() == TR::bu2i) {
        ...
        //      cmpOpNode
        //    cmpValNode
        // We have previously verified that cmpOpNode has a reference count of 1 and has not previously been evaluator,
        // so therefore the bu2i must also be isSingleRefEvaluated() and we should dec ref count here
 ...
 * we must do the opposite to mimic branch behaviour, which would branch around the load when one of the following is
 * true.
 */
TR::InstOpCode::S390BranchCondition OMR::Z::TreeEvaluator::mapBranchConditionToLOCRCondition(
    TR::InstOpCode::S390BranchCondition incomingBc)
{
    switch (incomingBc) {
...
{
    TR_ASSERT(node->getOpCode().isBooleanCompare() && node->getNumChildren() > 0,
        "node %p is not a compare opcode, or compare opcode has 0 children!\n", node);
    TR_ASSERT(node->getFirstChild()->getType().isIntegral() || node->getFirstChild()->getType().isAddress(),
        "node %p is neither an int nor an address!\n", node->getFirstChild());
...
bool OMR::Z::TreeEvaluator::treeContainsAllOtherUsesForNode(TR::Node* treeNode, TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::selectEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (condition->getOpCode().isBooleanCompare()
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
            ...
            generateRRFInstruction(cg, mnemonic, node, trueReg, falseReg, trueReg,
                getMaskForBranchCondition(TR::TreeEvaluator::mapBranchConditionToLOCRCondition(bc)));
        } else if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
            generateRRInstruction(cg, compareOp, node, firstReg, secondReg);

            auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR;
            generateRRFInstruction(cg, mnemonic, node, trueReg, falseReg,
                getMaskForBranchCondition(TR::TreeEvaluator::mapBranchConditionToLOCRCondition(bc)), true);
        } else {
            TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
            TR::LabelSymbol* cFlowRegionEnd = generateLabelSymbol(cg);
            ...
            generateS390CompareAndBranchInstruction(
                cg, compareOp, node, firstReg, secondReg, bc, cFlowRegionEnd, false);

            TR::RegisterDependencyConditions* conditions = conditions
            ...
            generateRRInstruction(
                cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LGR : TR::InstOpCode::LR, node, trueReg, falseReg);
        ...
        TR::Instruction* compareInst = generateRILInstruction(cg,
            ...
            condition->getRegister(), 0);

        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
            auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::SELGR : TR::InstOpCode::SELR;
            generateRRFInstruction(
                cg, mnemonic, node, trueReg, falseReg, trueReg, getMaskForBranchCondition(TR::InstOpCode::COND_BER));
        } else if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
            auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR;
            generateRRFInstruction(
                cg, mnemonic, node, trueReg, falseReg, getMaskForBranchCondition(TR::InstOpCode::COND_BER), true);
        } else {
            TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
            TR::LabelSymbol* cFlowRegionEnd = generateLabelSymbol(cg);
            ...
            generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
            cFlowRegionStart->setStartInternalControlFlow();
            TR::Instruction* branchInst = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, cFlowRegionEnd);
...
TR::Register* OMR::Z::TreeEvaluator::dselectEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z13) && node->getOpCode().isDouble())
        ...
        if (node->getOpCode().isDouble()) {
            ...
            // Shift left the 32 least significant bits for preserving the float representaion as the hardware only
            // operates on the first 32 bits in a FPR
        ...
        // M4 - Floating-point-format control = getVectorElementSizeMask(node->getSize()) - gets the element size mask
        // for doubles/floats respectively M5 - Single-Element-Control = 0x8, setting bit 0 to one, controlling the
        // operation to take place only on the zero-indexed element in the vector M6 - Condition Code Set = 0, the
        // Condition Code is not set and remains unchanged
        generateVRRcInstruction(cg, TR::InstOpCode::VFCE, node, vectorSelReg, tempReg, vzeroReg, 0, 0x8,
            getVectorElementSizeMask(node->getSize()));
        ...
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CL, node, conditionReg, 0, TR::InstOpCode::COND_BNE, cFlowRegionEnd, false, false);
        generateRRInstruction(
            cg, node->getOpCode().isDouble() ? TR::InstOpCode::LDR : TR::InstOpCode::LER, node, resultReg, falseValReg);
        generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionEnd, conditions);
        cFlowRegionStart->setStartInternalControlFlow();
        cFlowRegionEnd->setEndInternalControlFlow();
    }
