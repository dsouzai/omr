    ...
    TR::InstOpCode::S390BranchCondition brCond, bool isBoolean)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        generateS390ImmOp(cg, isUnsigned ? TR::InstOpCode::CLG : TR::InstOpCode::CG, node, cmpRegister, cmpRegister,
            long_value);
    ...
    TR::InstOpCode::S390BranchCondition branchCond, TR::InstOpCode::Mnemonic moveRROp)
{
    ...
    if (node->getOpCode().isFloatingPoint()) {
        ...
        generateRREInstruction(cg, node->getOpCode().isDouble() ? TR::InstOpCode::LTDBR : TR::InstOpCode::LTEBR, node,
            lhsReg, lhsReg);
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
...

            {
                ...
                TR::Instruction *vgnopInstr = generateVirtualGuardNOPInstruction(cg, node, site, deps, fallThroughLabel,
                    instr ? instr->getPrev() : NULL);
                ...
                TR::Instruction *vgnopInstr = generateVirtualGuardNOPInstruction(cg, node, site, mergedGuardDeps, label,
                    instr ? instr->getPrev() : NULL);
                vgnopInstr->setNext(instr);
                cg->setAppendInstruction(instr);
            }
...
TR::Register *OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifacmpeq) {
        reg = generateS390CompareBranch(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE,
            TR::InstOpCode::COND_BE);
    } else {
        reg = generateS390CompareBranch(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE,
...
TR::Register *OMR::Z::TreeEvaluator::ifbcmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    if (node->getOpCodeValue() == TR::ifbcmpeq) {
        return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE,
            TR::InstOpCode::COND_BE);
    } else {
        return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE,
            TR::InstOpCode::COND_BNE);
    }
}
...
TR::Register *OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::acmpeq) {
        ...
        return generateS390CompareBool(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE,
            TR::InstOpCode::COND_BNE);
    }
}
    ...
    TR::Register *branchTableReg, TR::Register *reg1)
{
    TR::Compilation *comp = cg->comp();
    {
        ...
        new (cg->trHeapMemory()) TR::S390RIInstruction(TR::InstOpCode::BRAS, node, reg1,
            (4 + (sizeof(uintptr_t) * numBranchTableEntries)) / 2, cg);
...
TR::Register *OMR::Z::TreeEvaluator::tableEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::TreeEvaluator::tableEvaluatorCaseLabelHelper(node, cg, tableKindToBeEvaluated, numBranchTableEntries,
        selectorReg, branchTableReg, reg1);
...
void OMR::Z::TreeEvaluator::addToRegDep(TR::RegisterDependencyConditions *daaDeps, TR::Register *reg,
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfButestEvaluator(TR::Node *node, TR::CodeGenerator *cg, TR::Node *cmpOpNode,
    TR::Node *cmpValNode)
{
    ...
    TR::Instruction *instr = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, cond, cmpOpNode,
        cmpOpNode->getBranchDestination()->getNode()->getLabel(), deps);
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfBifEvaluator(TR::Node *ifNode, TR::CodeGenerator *cg, TR::Node *cmpOpNode,
    TR::Node *cmpValNode)
{
    TR_ASSERT(cmpValNode->getOpCode().isLoadConst() && cmpValNode->getType().isInt32(),
        "Expecting constant second child for if node, found node %s (%p)\n", cmpValNode->getOpCode().getName(),
...
TR::Register *OMR::Z::TreeEvaluator::selectEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (condition->getOpCode().isBooleanCompare()
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
            ...
            generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
            cFlowRegionStart->setStartInternalControlFlow();
            generateS390CompareAndBranchInstruction(cg, compareOp, node, firstReg, secondReg, bc, cFlowRegionEnd,
                false);
            ...
            generateRRInstruction(cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LGR : TR::InstOpCode::LR, node,
                trueReg, falseReg);
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z15)) {
            auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::SELGR : TR::InstOpCode::SELR;
            generateRRFInstruction(cg, mnemonic, node, trueReg, falseReg, trueReg,
                getMaskForBranchCondition(TR::InstOpCode::COND_BER));
        } else if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
            auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR;
            generateRRFInstruction(cg, mnemonic, node, trueReg, falseReg,
            ...
            generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionStart);
            cFlowRegionStart->setStartInternalControlFlow();
            TR::Instruction *branchInst = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                TR::InstOpCode::COND_BNE, node, cFlowRegionEnd);
...
TR::Register *OMR::Z::TreeEvaluator::dselectEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if ((cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z13) && node->getOpCode().isDouble())
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CL, node, conditionReg, 0, TR::InstOpCode::COND_BNE,
            cFlowRegionEnd, false, false);
        generateRRInstruction(cg, node->getOpCode().isDouble() ? TR::InstOpCode::LDR : TR::InstOpCode::LER, node,
            resultReg, falseValReg);
        generateS390LabelInstruction(cg, TR::InstOpCode::label, node, cFlowRegionEnd, conditions);
        cFlowRegionStart->setStartInternalControlFlow();
        cFlowRegionEnd->setEndInternalControlFlow();
    }
