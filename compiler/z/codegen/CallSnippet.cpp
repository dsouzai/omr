...
uint8_t* TR::S390CallSnippet::storeArgumentItem(
...
uint8_t* TR::S390CallSnippet::S390flushArgumentsToStack(
    uint8_t* buffer, TR::Node* callNode, int32_t argSize, TR::CodeGenerator* cg)
{
            ...
            if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
                if (cg->comp()->target().is64Bit()) {
                    ...
                    if (intArgNum < linkage->getNumIntegerArgumentRegisters() - 1) {
                        buffer = storeArgumentItem(TR::InstOpCode::ST, buffer,
                            machine->getRealRegister(linkage->getIntegerArgumentRegister(intArgNum + 1)), offset + 4,
                            cg);
                    }
                }
            }
...
int32_t TR::S390CallSnippet::adjustCallOffsetWithTrampoline(
    uintptr_t targetAddr, uint8_t* currentInst, TR::SymbolReference* callSymRef, TR::Snippet* snippet)
{
    ...
    if (cg->directCallRequiresTrampoline(targetAddr, currentInstPtr)) {
        if (callSymRef->getReferenceNumber() < TR_S390numRuntimeHelpers)
            targetAddr = TR::CodeCacheManager::instance()->findHelperTrampoline(
                callSymRef->getReferenceNumber(), reinterpret_cast<void*>(currentInstPtr));
        else
            targetAddr
                = cg->fe()->methodTrampolineLookup(cg->comp(), callSymRef, reinterpret_cast<void*>(currentInstPtr));

        TR_ASSERT_FATAL(cg->comp()->target().cpu.isTargetWithinBranchRelativeRILRange(
                            targetAddr, reinterpret_cast<intptr_t>(currentInst)),
