...
static bool isBarrierToPeepHoleLookback(TR::Instruction* cursor)
{
    if (cursor == NULL)
        return true;
...
static TR::Instruction* realInstructionWithLabelsAndRET(TR::Instruction* cursor)
{
    while (cursor
        && (cursor->getKind() == TR::Instruction::IsPseudo || cursor->getKind() == TR::Instruction::IsNotExtended)
        && !cursor->isRet()) {
...
static bool seekRegInFutureMemRef(TR::Instruction* cursor, int32_t maxWindowSize, TR::Register* targetReg)
{
    TR::Instruction* current = cursor->getNext();
    int32_t windowSize = 0;

    while ((current != NULL) && !current->matchesTargetRegister(targetReg) && !isBarrierToPeepHoleLookback(current)
...
bool OMR::Z::Peephole::performOnInstruction(TR::Instruction* cursor)
{
...
bool OMR::Z::Peephole::tryLoadStoreReduction(TR::InstOpCode::Mnemonic storeOpCode, uint16_t size)
{
    if (cursor->getNext()->getOpCodeValue() == storeOpCode) {
        ...
        // Cannot change to MVC if symbol references are unresolved, MVC doesn't follow same format as load and store so
        // patching won't work
        ...
        // dependencies.
        if (reg->getTotalUseCount() != uses || loadInst->getDependencyConditions()
            || storeInst->getDependencyConditions()) {
            return false;
        }

        if (performTransformation(
                self()->comp(), "O^O S390 PEEPHOLE: Transforming load-store sequence at %p to MVC.", storeInst)) {
            ...
            TR::Instruction* newInst = generateSS1Instruction(self()->cg(), TR::InstOpCode::MVC, loadInst->getNode(),
                size - 1, storeMemRef, loadMemRef, cursor->getPrev());
...
bool OMR::Z::Peephole::tryToFoldLoadRegisterIntoSubsequentInstruction()
{
    ...
    while ((current != NULL) && !current->isLabel() && !current->isCall()
        && !(current->isBranchOp() && !(current->isExceptBranchOp())) && windowSize < maxWindowSize) {
        // do not look across Transactional Regions, the Register save mask is optimistic and does not allow renaming
        if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC
        ...
        if (current->usesRegister(lgrTargetReg)) {
            if (current->defsRegister(lgrTargetReg)) {
                ...
                case TR::InstOpCode::AR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::ARK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::AGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::AGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::ALR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::ALRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::ALGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::ALGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::AHI: {
                    int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
                    newInstr = generateRIEInstruction(self()->cg(), TR::InstOpCode::AHIK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                    break;
                }
                case TR::InstOpCode::AGHI: {
                    int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
                    newInstr = generateRIEInstruction(self()->cg(), TR::InstOpCode::AGHIK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                    break;
                }
                case TR::InstOpCode::NR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::NRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::NGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::NGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::XR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::XRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::XGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::XGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::OR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::ORK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::OGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::OGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::SLA: {
                    ...
                    if (mf != NULL) {
                        ...
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SLAK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, mf, current->getPrev());
                    } else
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SLAK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                    break;
                ...
                case TR::InstOpCode::SLL: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, mf, current->getPrev());
                    } else
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                ...
                case TR::InstOpCode::SRA: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SRAK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, mf, current->getPrev());
                    } else
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SRAK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                ...
                case TR::InstOpCode::SRL: {
                    ...
                    if (mf != NULL) {
                        mf->resetMemRefUsedBefore();
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SRLK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, mf, current->getPrev());
                    } else
                        newInstr = generateRSInstruction(self()->cg(), TR::InstOpCode::SRLK, current->getNode(),
                            lgrTargetReg, lgrSourceReg, imm, current->getPrev());
                ...
                case TR::InstOpCode::SR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::SRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::SGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::SGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::SLR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::SLRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                case TR::InstOpCode::SLGR: {
                    newInstr = generateRRRInstruction(self()->cg(), TR::InstOpCode::SLGRK, current->getNode(),
                        lgrTargetReg, lgrSourceReg, srcReg, current->getPrev());
                    break;
                }
                default:
...
bool OMR::Z::Peephole::tryToForwardBranchTarget()
{
...
bool OMR::Z::Peephole::tryToReduce64BitShiftTo32BitShift()
{
    ...
    TR::Register* sourceReg
        = (shiftInst->getSecondRegister()) ? (shiftInst->getSecondRegister()->getRealRegister()) : NULL;
    TR::Register* targetReg
        = (shiftInst->getRegisterOperand(1)) ? (shiftInst->getRegisterOperand(1)->getRealRegister()) : NULL;
    ...
    if (performTransformation(self()->comp(),
            "O^O S390 PEEPHOLE: Reverting int shift at %p from SLLG/SLAG/S[LR][LA]K to SLL/SLA/SRL/SRA.\n",
            shiftInst)) {
        TR::InstOpCode::Mnemonic newOpCode = TR::InstOpCode::bad;
        switch (oldOpCode) {
        case TR::InstOpCode::SLLG:
        case TR::InstOpCode::SLLK:
            newOpCode = TR::InstOpCode::SLL;
            break;
        case TR::InstOpCode::SLAG:
        case TR::InstOpCode::SLAK:
            newOpCode = TR::InstOpCode::SLA;
            break;
        case TR::InstOpCode::SRLK:
            newOpCode = TR::InstOpCode::SRL;
            break;
        case TR::InstOpCode::SRAK:
            newOpCode = TR::InstOpCode::SRA;
            break;
        default:
        ...
        if (shiftInst->getSourceImmediate()) {
            newInstr = new (self()->cg()->trHeapMemory()) TR::S390RSInstruction(newOpCode, shiftInst->getNode(),
                shiftInst->getRegisterOperand(1), shiftInst->getSourceImmediate(), shiftInst->getPrev(), self()->cg());
        } else if (shiftInst->getMemoryReference()) {
            TR::MemoryReference* memRef = shiftInst->getMemoryReference();
            memRef->resetMemRefUsedBefore();
            newInstr = new (self()->cg()->trHeapMemory()) TR::S390RSInstruction(newOpCode, shiftInst->getNode(),
                shiftInst->getRegisterOperand(1), memRef, shiftInst->getPrev(), self()->cg());
        } else {
...
bool OMR::Z::Peephole::tryToReduceAGI()
{
    bool performed = false;
    int32_t windowSize = 0;
    ...
    while ((current != NULL) && !current->matchesTargetRegister(lgrTargetReg) && !isBarrierToPeepHoleLookback(current)
        && windowSize < MaxWindowSize) {
        // do not look across Transactional Regions, the Register save mask is optimistic and does not allow renaming
        if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC
            || current->getOpCodeValue() == TR::InstOpCode::TEND
            ...
            while (mr) {
                if (mr && mr->getBaseRegister() == lgrTargetReg) {
                    if (!reachedLabel && !reachedBranch && !sourceRegInvalid) {
                        if (performTransformation(self()->comp(),
                                "O^O S390 PEEPHOLE: AGI register renaming on [%p] from source load [%p].\n", current,
                                cursor)) {
    ...
    if (attemptLA) {
        ...
        windowSize = 0;
        while ((current != NULL) && !isBarrierToPeepHoleLookback(current)
            && !current->matchesTargetRegister(lgrSourceReg) && windowSize < MaxLAWindowSize) {
        ...
        if (windowSize == MaxLAWindowSize) {
            if (performTransformation(self()->comp(),
                    "O^O S390 PEEPHOLE: AGI LA reduction on [%p] from source load [%p].\n", current, cursor)) {
...
bool OMR::Z::Peephole::tryToReduceCLRToCLRJ()
{
    ...
    if (cursor->getNext()->getOpCodeValue() == TR::InstOpCode::BRC) {
        ...
        if (labelSymbol->getSnippet()) {
            ...
            for (auto branchTakenInstIndex = 0; branchTakenInstIndex < 5 && NULL != branchInstruction;
                 ++branchTakenInstIndex) {
                if (branchInstruction->getOpCode().readsCC()) {
                    break;
                }
                // CC is set before it is read (ordering of the if checks matter)
                if (branchInstruction->getOpCode().setsCC()
                    || TR::BBEnd == branchInstruction->getNode()->getOpCodeValue()) {
                    branchTakenPerformReduction = true;
                    break;
                }
        ...
        for (auto fallThroughInstIndex = 0; fallThroughInstIndex < 5 && NULL != fallThroughInstruction;
             ++fallThroughInstIndex) {
            if (fallThroughInstruction->getOpCode().readsCC()) {
                break;
            }
            // CC is set before it is read (ordering of the if checks matter)
            if (fallThroughInstruction->getOpCode().setsCC()
                || TR::BBEnd == fallThroughInstruction->getNode()->getOpCodeValue()) {
        ...
        if (fallThroughPerformReduction && branchTakenPerformReduction
            && performTransformation(self()->comp(), "O^O S390 PEEPHOLE: Transforming CLR [%p] and BRC [%p] to CLRJ\n",
                clrInstruction, brcInstruction)) {
            TR_ASSERT_FATAL(clrInstruction->getNumRegisterOperands() == 2,
                "Number of register operands was not 2: %d\n", clrInstruction->getNumRegisterOperands());

            TR::Instruction* clrjInstruction = generateRIEInstruction(self()->cg(), TR::InstOpCode::CLRJ,
                clrInstruction->getNode(), clrInstruction->getRegisterOperand(1), clrInstruction->getRegisterOperand(2),
                labelSymbol, static_cast<TR::S390BranchInstruction*>(brcInstruction)->getBranchCondition(),
...
bool OMR::Z::Peephole::tryToReduceCRJLHIToLOCHI(TR::InstOpCode::Mnemonic compareMnemonic)
{
    ...
    TR::InstOpCode::S390BranchCondition cond
        = getBranchConditionForMask(0xF - (getMaskForBranchCondition(branchInst->getBranchCondition()) & 0xF));

    if (performTransformation(
            self()->comp(), "O^O S390 PEEPHOLE: Conditionalizing fall-through block following [%p].\n", currInst)) {
        ...
        while (currInst = realInstructionWithLabelsAndRET(nextInst)) {
            ...
            auto lochiInst = generateRIEInstruction(self()->cg(),
                RIInst->getOpCode().getOpCodeValue() == TR::InstOpCode::LHI ? TR::InstOpCode::LOCHI
                                                                            : TR::InstOpCode::LOCGHI,
                RIInst->getNode(), RIInst->getRegisterOperand(1), RIInst->getSourceImmediate(), cond,
                RIInst->getPrev());
        ...
        auto crInst = generateRRInstruction(self()->cg(), compareMnemonic, branchInst->getNode(),
            branchInst->getRegisterOperand(1), branchInst->getRegisterOperand(2), branchInst->getPrev());
...
bool OMR::Z::Peephole::tryToReduceLToICM()
{
    ...
    if (mem->getBaseRegister() != NULL && mem->getIndexRegister() != NULL)
        return false;

    // L and LTR/CHI instructions must work on the same register.
    if (load->getRegisterOperand(1) != next->getRegisterOperand(1))
        return false;
    ...
    if (performTransformation(self()->comp(), "\nO^O S390 PEEPHOLE: Reducing L [%p] being reduced to ICM.\n", cursor)) {
        ...
        if ((memcp->getBaseRegister() == NULL) && (memcp->getIndexRegister() != NULL)) {
            memcp->setBaseRegister(memcp->getIndexRegister(), self()->cg());
            memcp->setIndexRegister(0);
        }

        // Do the reduction - create the icm instruction
        TR::S390RSInstruction* icm = new (self()->cg()->trHeapMemory()) TR::S390RSInstruction(
            TR::InstOpCode::ICM, load->getNode(), load->getRegisterOperand(1), 0xF, memcp, prev, self()->cg());
...
bool OMR::Z::Peephole::tryToReduceLToLZRF(TR::InstOpCode::Mnemonic loadAndZeroRightMostByteMnemonic)
{
    // This optimization relies on hardware instructions introduced in z13
    if (!self()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z13))
        return false;
    ...
    if (cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
        ...
        if (performTransformation(
                self()->comp(), "O^O S390 PEEPHOLE: Transforming load-and-mask sequence at [%p].\n", nillInst)) {
            // Remove the NILL instruction
            nillInst->remove();

            loadInst->getMemoryReference()->resetMemRefUsedBefore();

            auto lzrbInst = generateRXInstruction(self()->cg(), loadAndZeroRightMostByteMnemonic, loadInst->getNode(),
                loadTargetReg, loadInst->getMemoryReference(), cursor->getPrev());
...
bool OMR::Z::Peephole::tryToReduceLGRToLGFR()
{
    TR::Register* lgrSourceReg = cursor->getRegisterOperand(2);
    TR::Register* lgrTargetReg = cursor->getRegisterOperand(1);
 ...
 */
bool OMR::Z::Peephole::tryToReduceLHIToXR()
{
    // This optimization is disabled by default because there exist cases in which we cannot determine whether this
    // transformation is functionally valid or not. The issue resides in the various runtime patching sequences using
    // the LHI instruction as a runtime patch point for an offset. One concrete example can be found in the virtual
    // dispatch sequence for unresolved calls on 31-bit platforms where an LHI instruction is used and is patched at
    // runtime.
    //
    // TODO (#255): To enable this optimization we need to implement an API which marks instructions that will be
    // patched at runtime and prevent ourselves from modifying such instructions in any way.
    return false;
    ...
    if (lhiInstruction->getSourceImmediate() == 0) {
        ...
        while (nextInstruction != NULL && !nextInstruction->getOpCode().readsCC()) {
            if (nextInstruction->getOpCode().setsCC() || nextInstruction->getNode()->getOpCodeValue() == TR::BBEnd) {
                ...
                TR::Instruction* xrInstruction
                    = generateRRInstruction(self()->cg(), TR::InstOpCode::XR, lhiInstruction->getNode(),
                        lhiInstruction->getRegisterOperand(1), lhiInstruction->getRegisterOperand(1));
...
bool OMR::Z::Peephole::tryToReduceLLCToLLGC()
{
    TR::Instruction* nextInst = cursor->getNext();
    ...
    if (mnemonic == TR::InstOpCode::LGFR || mnemonic == TR::InstOpCode::LLGTR) {
        ...
        if (llcTgtReg == nextSrcReg && llcTgtReg == nextTgtReg) {
            if (performTransformation(self()->comp(), "O^O S390 PEEPHOLE: Reducing LLC/%s [%p] to LLGC.\n",
                ...
                memRef->resetMemRefUsedBefore();
                auto llgcInst = generateRXInstruction(
                    self()->cg(), TR::InstOpCode::LLGC, cursor->getNode(), llcTgtReg, memRef, cursor->getPrev());
...
bool OMR::Z::Peephole::tryToReduceLRCHIToLTR()
{
    int32_t windowSize = 0;
    ...
    while ((current != NULL) && !isBarrierToPeepHoleLookback(current)
        ...
        if ((curOpCode.getOpCodeValue() == TR::InstOpCode::CHI || curOpCode.getOpCodeValue() == TR::InstOpCode::CGHI)
            && ((curOpCode.is32bit() && lgrOpCode.is32bit()) || (curOpCode.is64bit() && lgrOpCode.is64bit()))) {
            ...
            if (curTargetReg == lgrTargetReg && srcImm == 0) {
                if (performTransformation(
                        self()->comp(), "O^O S390 PEEPHOLE: Transforming LR/CHI to LTR at %p\n", cursor)) {
                    auto ltrInst = generateRRInstruction(self()->cg(),
                        lgrOpCode.is64bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR, cursor->getNode(),
                        lgrTargetReg, lgrSourceReg, cursor->getPrev());
        ...
        if (current->isDefRegister(lgrSourceReg) || current->isDefRegister(lgrTargetReg))
            break;
...
bool OMR::Z::Peephole::tryToReduceLTRToCHI()
{
    // The _defRegs in the instruction records virtual def reg till now that needs to be reset to real reg.
    cursor->setUseDefRegisters(false);
    ...
    if (lgrTargetReg == lgrSourceReg
        ...
        if (seekRegInFutureMemRef(cursor, 4, lgrTargetReg)) {
            if (performTransformation(self()->comp(),
                    ...
                    TR::InstOpCode::metadata[cursor->getOpCodeValue()].name, cursor)) {
                auto chiInst = generateRIInstruction(self()->cg(), TR::InstOpCode::getCmpHalfWordImmOpCode(),
                    cursor->getNode(), lgrTargetReg, 0, cursor->getPrev());
...
bool OMR::Z::Peephole::tryToRemoveDuplicateLR()
{
    // The _defRegs in the instruction records virtual def reg till now that needs to be reset to real reg
    cursor->setUseDefRegisters(false);
    ...
    if (lgrTargetReg == lgrSourceReg
        && (lgrOpCode.getOpCodeValue() == TR::InstOpCode::LR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LGR
...
bool OMR::Z::Peephole::tryToRemoveDuplicateLoadRegister()
{
    bool performed = false;
    int32_t windowSize = 0;
    ...
    while ((current != NULL) && !isBarrierToPeepHoleLookback(current)
        ...
        if (curOpCode.getOpCodeValue() == lgrOpCode.getOpCodeValue() && current->getKind() == TR::Instruction::IsRR) {
            ...
            if (((curSourceReg == lgrTargetReg && curTargetReg == lgrSourceReg)
                ...
                if ((!lgrSetCC || !(setCC || useCC))) {
                    if (performTransformation(
                            self()->comp(), "O^O S390 PEEPHOLE: Duplicate LR/CPYA removal at %p\n", rrInst)) {
                        performed = true;
                        current = current->getNext();
                        windowSize = 0;
...
bool OMR::Z::Peephole::tryToRemoveDuplicateNILF()
{
    if (cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILF) {
        ...
        bool instructionsMatch = (currInst->getTargetPtr() == nextInst->getTargetPtr())
        ...
        if (instructionsMatch) {
            if (currInst->matchesTargetRegister(nextInst->getRegisterOperand(1))
                ...
                if (currInst->getSourceImmediate() == nextInst->getSourceImmediate()) {
                    if (performTransformation(self()->comp(),
                            "O^O S390 PEEPHOLE: deleting duplicate NILF from pair %p %p*\n", currInst, nextInst)) {
                        nextInst->remove();
                ...
                //
                else if (((currInst->getSourceImmediate() & nextInst->getSourceImmediate())
                             == currInst->getSourceImmediate())
                    && ((nextInst->getSourceImmediate() & currInst->getSourceImmediate())
                        != nextInst->getSourceImmediate())) {
                    if (performTransformation(self()->comp(),
                            "O^O S390 PEEPHOLE: deleting unnecessary NILF from pair %p %p*\n", currInst, nextInst)) {
                        nextInst->remove();
...
bool OMR::Z::Peephole::tryToRemoveDuplicateNILH()
{
            ...
            if (currInst->isImm()) {
                if (currInst->getSourceImmediate() == nextInst->getSourceImmediate()) {
                    if (currInst->matchesTargetRegister(nextInst->getRegisterOperand(1))
                        && nextInst->matchesTargetRegister(currInst->getRegisterOperand(1))) {
                        if (performTransformation(self()->comp(),
                                "O^O S390 PEEPHOLE: deleting duplicate NILH from pair %p %p*\n", currInst, nextInst)) {
                            nextInst->remove();
...
bool OMR::Z::Peephole::tryToRemoveRedundantCompareAndTrap()
{
    if (self()->comp()->target().isZOS()) {
        // CLT cannot do the job in zOS because in zOS it is legal to read low memory address (like 0x000000, literally
        // NULL), and CLT will read the low memory address legally (in this case NULL) to compare it with the other
        // operand. The result will not make sense since we should trap for NULLCHK first.
        return false;
    }
    ...
    if (disableRemoveMergedNullCHK != NULL)
        return false;
    ...
    while ((current != NULL) && !isBarrierToPeepHoleLookback(current) && windowSize < maxWindowSize) {
        // do not look across Transactional Regions, the Register save mask is optimistic and does not allow renaming
        if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC
            || current->getOpCodeValue() == TR::InstOpCode::TEND
            || current->getOpCodeValue() == TR::InstOpCode::TABORT) {
...
bool OMR::Z::Peephole::tryToRemoveRedundantLA()
{
    ...
    if (mr && mr->getOffset() == 0 && laBaseReg && laTargetReg && laBaseReg == laTargetReg && laIndexReg == NULL
        && (symRef == NULL || symRef->getOffset() == 0) && (symRef == NULL || symRef->getSymbol() == NULL)) {
...
bool OMR::Z::Peephole::tryToRemoveRedundantShift()
{
    TR::Instruction* currInst = cursor;
    TR::Instruction* nextInst = cursor->getNext();
    ...
    if (currInst->getOpCodeValue() == nextInst->getOpCodeValue()) {
        if ((currInst->getKind() == TR::Instruction::IsRS) && (nextInst->getKind() == TR::Instruction::IsRS)) {
            ...
            bool instrMatch = (currRSInst->getFirstRegister() == nextRSInst->getFirstRegister())
...
bool OMR::Z::Peephole::tryToRemoveRedundantLR()
{
    // The _defRegs in the instruction records virtual def reg till now that needs to be reset to real reg
    cursor->setUseDefRegisters(false);
    ...
    if (lgrTargetReg == lgrSourceReg) {
        ...
        if (nextInst->getOpCodeValue() == TR::InstOpCode::LTR || nextInst->getOpCodeValue() == TR::InstOpCode::LTGR) {
            ...
            if (lgrTargetReg == ltgrTargetReg || lgrTargetReg == ltgrSourceReg) {
                if (performTransformation(self()->comp(),
                        "O^O S390 PEEPHOLE: Removing redundant %s [%p] which is followed by a load and test "
                        "register.\n",
                        TR::InstOpCode::metadata[cursor->getOpCodeValue()].name, cursor)) {
                    cursor->remove();
...
bool OMR::Z::Peephole::tryToRemoveRedundantLTR()
{
    // The _defRegs in the instruction records virtual def reg till now that needs to be reset to real reg
    cursor->setUseDefRegisters(false);
    ...
    if (lgrTargetReg == lgrSourceReg) {
        ...
        // we do have the same restricted register, which is why we are handling it here when all the register
        // assignment is done, and the redundant LR's from the clobber evaluate of the add/sub logical are cleaned up as
        // well
        if (prevInst->getOpCode().setsCC() && prevInst->getOpCode().setsCarryFlag()
            && prevInst->getRegisterOperand(1) == lgrTargetReg && nextInst->getOpCodeValue() == TR::InstOpCode::BRC) {
            TR::InstOpCode::S390BranchCondition branchCond
                = ((TR::S390BranchInstruction*)nextInst)->getBranchCondition();

            if (branchCond == TR::InstOpCode::COND_BE || branchCond == TR::InstOpCode::COND_BNE) {
                if (performTransformation(self()->comp(),
                        "O^O S390 PEEPHOLE: Removing redundant Load and Test instruction at %p, because CC can be "
                        "reused from logical instruction %p\n",
                        cursor, prevInst)) {
                    cursor->remove();
