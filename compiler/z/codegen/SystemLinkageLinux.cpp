    ...
    : TR::SystemLinkage(cg, TR_SystemLinux)
{
    ...
    setRegisterFlag(TR::RealRegister::GPR6, IntegerArgumentAddToPost); // I only need to do this for r6 because r6 is
                                                                       // the only preserved linkage register on zlinux.
    ...
    setRegisterFlag(TR::RealRegister::GPR14,
        Preserved); // Setting the return address register as preserved so that way we do not always have to emit stores
                    // in prologues / epilogues This possibly assumes that we don't shuffle the return address register
                    // around elsewhere
    ...
    if (cg->getSupportsVectorRegisters()) {
        ...
        // The vector registers used for parameter passing are allocated in the following order to enable future
        // exension which might operate on vector register pairs
...
void TR::S390zLinuxSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    cursor = generateS390BranchInstruction(
        cg(), TR::InstOpCode::BCR, node, TR::InstOpCode::COND_BCR, getReturnAddressRealRegister(), cursor);
}
...
void TR::S390zLinuxSystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (cg()->isLiteralPoolOnDemandOn() != true && firstSnippet != NULL)
        cursor = new (trHeapMemory())
            TR::S390RILInstruction(TR::InstOpCode::LARL, node, getLitPoolRealRegister(), firstSnippet, cursor, cg());
#endif
    ...
    if (largeStack) {
        ...
        cursor = generateRXInstruction(cg(), TR::InstOpCode::LA, node, spReg,
            generateS390MemoryReference(spReg, (stackFrameSize) * -1, cg()), cursor);
    }
...
void TR::S390zLinuxSystemLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method)
{
    setParameterLinkageRegisterIndex(method, method->getParameterList());
}

void TR::S390zLinuxSystemLinkage::setParameterLinkageRegisterIndex(
...
void TR::S390zLinuxSystemLinkage::generateInstructionsForCall(TR::Node* callNode,
    TR::RegisterDependencyConditions* deps, intptr_t targetAddress, TR::Register* methodAddressReg,
    TR::Register* javaLitOffsetReg, TR::LabelSymbol* returnFromJNICallLabel, TR::Snippet* callDataSnippet,
    bool isJNIGCPoint)
{
...
#if defined(JITTEST)
    {
        ...
        if (callNode->getOpCode().isIndirect()) {
            ...
            TR::Instruction* callInstr = new (trHeapMemory())
                TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, fpReg, callSymbol, callSymRef, cg);
 ...
 */
TR::Register* TR::S390zLinuxSystemLinkage::callNativeFunction(TR::Node* callNode,
    TR::RegisterDependencyConditions* deps, intptr_t targetAddress, TR::Register* methodAddressReg,
    TR::Register* javaLitOffsetReg, TR::LabelSymbol* returnFromJNICallLabel, TR::Snippet* callDataSnippet,
    bool isJNIGCPoint)
{
    ...
    default:
        if (callNode->getOpCode().isVectorOpCode()
            && (callNode->getOpCode().getVectorOperation() == TR::vcall
                || callNode->getOpCode().getVectorOperation() == TR::vcalli)) {
 ...
 */
void TR::S390zLinuxSystemLinkage::initParamOffset(
    TR::ResolvedMethodSymbol* method, int32_t stackIndex, List<TR::ParameterSymbol>* parameterList)
{
    ...
    while (parmCursor != NULL) {
        ...
        if (parmCursor->getDataType() == TR::Aggregate) {
            ...
            if (dt != TR::NoType)
                parmCursor->setDataType(dt);
            else if (parmCursor->getSize() > 8)
                parmCursor->setDataType(TR::Address);
            else if (parmCursor->getSize() == 8)
                parmCursor->setDataType(TR::Int64);
            else if (parmCursor->getSize() == 4)
                parmCursor->setDataType(TR::Int32);
            else if (parmCursor->getSize() == 2)
                parmCursor->setDataType(TR::Int16);
            else if (parmCursor->getSize() == 1)
                parmCursor->setDataType(TR::Int8);
            else
                parmCursor->setDataType(TR::Address);
        }
...
int32_t TR::S390zLinuxSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
    ...
    case TR::RealRegister::FPR2:
        return fpr0Offset + cg()->machine()->getFPRSize();
    case TR::RealRegister::FPR4:
        return fpr0Offset + 2 * cg()->machine()->getFPRSize();
    case TR::RealRegister::FPR6:
        return fpr0Offset + 3 * cg()->machine()->getFPRSize();
    ...
    case TR::RealRegister::GPR2:
        return gpr2Offset;
    default:
        return gpr2Offset + (srcReg - TR::RealRegister::GPR2) * cg()->machine()->getGPRSize();
    }
}

TR::Instruction* TR::S390zLinuxSystemLinkage::fillGPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
    ...
    for (i = lastReg = firstReg = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; ++i) {
        ...
        if (getPreserved(REGNUM(i))) {
            ...
            if (!findStartReg
                && (!(getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()
                    || (i == TR::RealRegister::LastGPR && firstReg != TR::RealRegister::LastGPR))) {
                ...
                if (lastReg - firstReg == 0)
                    cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), node,
                        getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
                else
                    cursor = generateRSInstruction(cg(), TR::InstOpCode::getLoadMultipleOpCode(), node,
                        getRealRegister(REGNUM(firstReg)), getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    ...
    if (!hasRestoreSP) {
        ...
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), node, getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    }
    return cursor;
}

TR::Instruction* TR::S390zLinuxSystemLinkage::fillFPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
    TR::RealRegister* spReg = getNormalStackPointerRealRegister();
...
TR::Instruction* TR::S390zLinuxSystemLinkage::spillGPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    ...
    if (!hasSavedSP) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getStoreOpCode(), node, getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    }
...
TR::Instruction* TR::S390zLinuxSystemLinkage::spillFPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
    TR::RealRegister* spReg = getNormalStackPointerRealRegister();
    int32_t offset = getFPRSaveAreaEndOffset();
    int16_t FPRSaveMask = getFPRSaveMask();
