...
TR::Instruction* generateS390LabelInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::LabelSymbol* sym, TR::Instruction* preced)
{
    if (preced) {
...
TR::Instruction* generateS390LabelInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Snippet* s, TR::Instruction* preced)
{
    if (preced) {
 ...
 */
TR::InstOpCode::Mnemonic getReplacementCompareAndBranchOpCode(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic compareOpCode)
{
...
TR::InstOpCode::Mnemonic getReplacementLongDisplacementOpCode(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::MemoryReference* memRef)
{
        ...
        if (longDisplacementMnemonic != TR::InstOpCode::bad) {
            ...
            TR::DebugCounter::incStaticDebugCounter(cg->comp(),
                TR::DebugCounter::debugCounterName(
                    cg->comp(), "z/memref/long-displacement-upgrade/(%s)", cg->comp()->signature()));
        }
    }
    ...
    TR::LabelSymbol* branchDestination, bool needsCC, bool targetIsFarAndCold)
{
    ...
    if (!cg->comp()->getOption(TR_DisableCompareAndBranchInstruction) && !needsCC
        ...
        returnInstruction = (TR::S390RIEInstruction*)generateRIEInstruction(
            cg, replacementOpCode, node, first, second, branchDestination, bc);
    }
    // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
        ...
        if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, first, second)
            ...
            && performTransformation(comp,
                ...
                "ccInst [%p].",
                opcTmp.getMnemonicName(), debugObj->getName(first), debugObj->getName(second), ccInst))
            returnInstruction = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, getReverseBranchCondition(bc), node, branchDestination);
        else {
    ...
    TR::RegisterDependencyConditions* cond)
{
    ...
    if (!cg->comp()->getOption(TR_DisableCompareAndBranchInstruction) && !needsCC
        && replacementOpCode != TR::InstOpCode::bad && cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12)) {
        cursor = (TR::S390RIEInstruction*)generateRIEInstruction(
            cg, replacementOpCode, node, first, (int8_t)second, branchDestination, bc, preced);
    }
    // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
    else {
        // we'll generate a compare which sets CC in a manner which flags the
...
TR::Instruction* generateS390RegInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, TR::Instruction* preced)
{
    if (preced) {
    ...
    TR::Register* treg, TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    if (instructionFormat == RXa_FORMAT) {
        ...
        TR_ASSERT_FATAL(instructionFormat == RXYa_FORMAT, "Mnemonic (%s) is incorrectly used as an RXY instruction",
...
TR::Instruction* generateRIInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Instruction* preced)
{
    if (preced) {
...
TR::Instruction* generateRILInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t mask, void* addr, TR::Instruction* preced)
{
    if (preced) {
    ...
    TR::Register* treg, TR::Register* sreg, uint32_t imm, TR::Instruction* preced)
{
    ...
    default:
        TR_ASSERT_FATAL(
            false, "Mnemonic (%s) is incorrectly used as RS instruction", TR::InstOpCode::metadata[op].name);
        break;
    }
    return result;
}
...
TR::Instruction* generateS390MemInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::MemoryReference* mf, TR::Instruction* preced)
{
    TR::Instruction* instr;
    if (preced)
    ...
    TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
    ...
    TR::Instruction* instr;
    if (preced)
        instr = new (INSN_HEAP) TR::S390SS1WithImplicitGPRsInstruction(
            op, n, len, mf1, mf2, cond, preced, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
    else
        instr = new (INSN_HEAP) TR::S390SS1WithImplicitGPRsInstruction(
            op, n, len, mf1, mf2, cond, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
    ...
    TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
    ...
    if (preced)
        ...
        instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
            op, n, op1Reg, mf2, op3Reg, mf4, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
    ...
    TR::Register* implicitRegTrg0, TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
    ...
    if (preced)
        ...
        instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
            op, n, op1Reg, mf2, NULL, NULL, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
    ...
    TR::Instruction* preced)
{
    ...
    if (preced)
        ...
        instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
            op, n, op1Reg, mf2, NULL, mf4, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
...
{
    if (preced)
        return new (INSN_HEAP) TR::S390VRRaInstruction(
            cg, op, n, targetReg, sourceReg2, 0 /* mask5 */, 0 /* mask4 */, 0 /* mask3 */, preced);
    else
...
TR::Instruction* generateVRRgInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* targetReg) /* Vector register */
{
...
TR::Instruction* generateS390PseudoInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Node* fenceNode, TR::Instruction* preced)
{
...
#endif
            {
                tempInst = (new (INSN_HEAP) TR::S390RILInstruction(
                    TR::InstOpCode::BRASL, callNode, RegRA, reinterpret_cast<void*>(imm), callSymRef, cg));
            }
#endif
#if !defined(TR_TARGET_64BIT) || (defined(TR_TARGET_64BIT) && defined(J9ZOS390))
...
TR::Instruction* generateDataConstantInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, uint32_t data, TR::Instruction* preced)
{
    if (preced != NULL) {
...
TR::Instruction* generateLoadLiteralPoolAddress(TR::CodeGenerator* cg, TR::Node* node, TR::Register* treg)
{
    ...
    TR::S390RILInstruction* LARLinst = (TR::S390RILInstruction*)generateRILInstruction(
        cg, TR::InstOpCode::LARL, node, treg, reinterpret_cast<void*>(0xBABE), 0);
...
static void rememberSnippetsToBePatchedOnClassUnloadOrRedefinition(
    TR::CodeGenerator* cg, TR::Node* node, TR::Snippet* snippet, intptr_t classPtr)
{
            ...
            unloadableClass = reinterpret_cast<TR_OpaqueClassBlock*>(
                cg->fe()
                    ->createResolvedMethod(
                        cg->trMemory(), reinterpret_cast<TR_OpaqueMethodBlock*>(classPtr), comp->getCurrentMethod())
                    ->classOfMethod());
        } else {
    ...
    TR::Register* base, bool isPICCandidate)
{
    ...
    if (cg->isLiteralPoolOnDemandOn() && (base == 0)) {
        if (op == TR::InstOpCode::L) {
            ...
            LRLinst = (TR::S390RILInstruction*)generateRILInstruction(
                cg, TR::InstOpCode::LRL, node, treg, targetsnippet, 0);
            cursor = LRLinst;
        } else {
    ...
    TR::Register* base, bool isPICCandidate)
{
    ...
    if (instructionFormat == RILa_FORMAT || instructionFormat == RILb_FORMAT || instructionFormat == RILc_FORMAT) {
        ...
        if (op == TR::InstOpCode::LG) {
            ...
            LGRLinst = (TR::S390RILInstruction*)generateRILInstruction(
                cg, TR::InstOpCode::LGRL, node, treg, targetsnippet, 0);
            cursor = LGRLinst;
        } else {
            base = cg->allocateRegister();
            if (cond != 0) {
    ...
    TR::RegisterDependencyConditions* deps)
{
...

    {
        ...
        cursor = generateRXInstruction(
            cg, TR::InstOpCode::EX, node, maskReg, tempMR, (preced != NULL ? preced : cg->getAppendInstruction()));
...
TR::Instruction* generateS390IInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, uint8_t im, TR::Node* n, TR::Instruction* preced)
{
    if (preced) {
...
TR::Instruction* generateRuntimeInstrumentationInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Register* target, TR::Instruction* preced)
{
    if (preced != NULL)
        if (target != NULL)
...
TR::Instruction* generateS390IEInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, uint8_t im1, uint8_t im2, TR::Node* n, TR::Instruction* preced)
{
    if (preced) {
    ...
    TR::Node* srcElementNode, int elementSize, TR::Register* zeroReg, TR::Instruction* preced)
{
    ...
    if (srcElementNode->getOpCode().isLoadConst()) {
        ...
        {
            cursor = generateVRIaInstruction(
                cg, TR::InstOpCode::VREPI, node, targetVRF, srcElementNode->getIntegerNodeValue<uint16_t>(), mask);
        } else {
            TR::MemoryReference* constMR = generateS390MemoryReference(
                srcElementNode->getIntegerNodeValue<int64_t>(), TR::Int64, cg, NULL, node);
    ...
    TR::Register* sourceRegister, int fromBit, int toBit, int shiftAmount)
{
    ...
    TR_ASSERT_FATAL(
        (0 <= toBit) && (toBit <= 63), "toBit(%d) incorrectly out of the range 0 to 63(0b00111111) inclusive", toBit);
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_ZEC12)) {
        generateRIEInstruction(
            cg, TR::InstOpCode::RISBGN, node, targetRegister, sourceRegister, fromBit, toBit | 0x80, shiftAmount);
    } else {
        generateRIEInstruction(
            cg, TR::InstOpCode::RISBG, node, targetRegister, sourceRegister, fromBit, toBit | 0x80, shiftAmount);
    }
}
    ...
    TR::Register* sourceRegister, int fromBit, int toBit, int shiftAmount)
{
    ...
    TR_ASSERT_FATAL(
        (0 <= toBit) && (toBit <= 31), "toBit(%d) incorrectly out of the range 0 to 31(0b00011111) inclusive", toBit);
    TR_ASSERT_FATAL(fromBit <= toBit, "fromBit(%d) incorrectly larger than toBit(%d)", fromBit, toBit);
    TR_ASSERT_FATAL(shiftAmount >= 0, "shiftAmount(%d) incorrectly less than 0", shiftAmount);
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196)) {
        generateRIEInstruction(
            cg, TR::InstOpCode::RISBLG, node, targetRegister, sourceRegister, fromBit, toBit | 0x80, shiftAmount);
    } else {
...
void generateS390DAAExceptionRestoreSnippet(
    TR::CodeGenerator* cg, TR::Node* n, TR::Instruction* instr, TR::InstOpCode::Mnemonic op, bool hasNOP)
{
...
TR::Instruction* generateAlignmentNopInstruction(
    TR::CodeGenerator* cg, TR::Node* node, uint32_t alignment, TR::Instruction* preced)
{
    if (preced)
