    ...
    TR::Instruction *precedingInstruction)
{
    ...
    // exchange vector registers
        ...
        if (middleReg != NULL) {
            ...
            currentInstruction = generateVRXInstruction(cg, TR::InstOpCode::VST, currentNode, targetReg, tempMR, 0,
                precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
            currentInstruction = generateVRRaInstruction(cg, TR::InstOpCode::VLR, currentNode, targetReg, sourceReg,
                precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
            TR::MemoryReference *tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
            currentInstruction = generateVRXInstruction(cg, TR::InstOpCode::VL, currentNode, sourceReg, tempMR2, 0,
                precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
...
bool OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister *reg, bool allowBlocked, uint64_t availRegMask,
    bool allowLocked)
{
...
bool OMR::Z::Machine::isLegalFirstOfFPRegister(TR::RealRegister *reg, bool allowBlocked, uint64_t availRegMask,
    bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
...
bool OMR::Z::Machine::isLegalSecondOfFPRegister(TR::RealRegister *reg, bool allowBlocked, uint64_t availRegMask,
    bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
...
bool OMR::Z::Machine::isLegalOddRegister(TR::RealRegister *reg, bool allowBlocked, uint64_t availRegMask,
    bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
 ...
 */
TR::Register *OMR::Z::Machine::assignBestRegister(TR::Register *targetRegister, TR::Instruction *currInst,
    bool doBookKeeping, uint64_t availRegMask)
{
...
TR::RealRegister *OMR::Z::Machine::findBestRegisterForShuffle(TR::Instruction *currentInstruction,
    TR::Register *currentAssignedRegister, uint64_t availRegMask)
{
...
*/
TR::RealRegister *OMR::Z::Machine::shuffleOrSpillRegister(TR::Instruction *currInst, TR::Register *toFreeReg,
 ...
 */
TR::Register *OMR::Z::Machine::assignBestRegisterSingle(TR::Register *targetRegister, TR::Instruction *currInst,
    bool doBookKeeping, uint64_t availRegMask)
{
 ...
 *            0      32   63
 */
TR::Register *OMR::Z::Machine::assignBestRegisterPair(TR::Register *regPair, TR::Instruction *currInst,
    bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (freeRegisterLow == NULL && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount())
        ...
        if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst,
                availRegMask)) {
    ...
    if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF)
        ...
        // Assign a real register pair for
        //
        if (!self()->findBestFreeRegisterPair(&freeRegisterHigh, &freeRegisterLow, regPairKind, currInst,
                availRegMask)) {
        ...
        if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
                ...
                if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst,
                        availRegMask)) {
                    self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst,
                        availRegMask);
                }

                self()->coerceRegisterAssignment(currInst, firstReg,
                    (toRealRegister(tfreeRegisterHigh)->getRegisterNumber()));
                self()->coerceRegisterAssignment(currInst, lastReg,
                    (toRealRegister(tfreeRegisterLow)->getRegisterNumber()));
        ...
        if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            // Assign a real register pair for
            //
            if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(), currInst,
                    availRegMask)) {
                self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(), currInst,
                    availRegMask);
            }
...
void OMR::Z::Machine::freeBestFPRegisterPair(TR::RealRegister **firstReg, TR::RealRegister **lastReg,
...
////////////////////////////////////////////////////////////////////////////////
TR::RealRegister *OMR::Z::Machine::findBestFreeRegister(TR::Instruction *currentInstruction, TR_RegisterKinds rk,
    TR::Register *virtualReg, uint64_t availRegMask)
{
    ...
    if (comp->getOption(TR_Randomize)) {
        ...
        if (preference != randomPreference
            && performTransformation(comp, "O^O Random Codegen - Randomizing Preference from: %d to: %d\n", preference,
                randomPreference)) {
...
////////////////////////////////////////////////////////////////////////////////
TR::RealRegister *OMR::Z::Machine::reverseSpillState(TR::Instruction *currentInstruction, TR::Register *spilledRegister,
 ...
 */
TR::Instruction *OMR::Z::Machine::coerceRegisterAssignment(TR::Instruction *currentInstruction,
    TR::Register *virtualRegister, TR::RealRegister::RegNum registerNumber)
{
