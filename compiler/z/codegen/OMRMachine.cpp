    ...
    TR::RealRegister* sourceReg, TR::Instruction* precedingInstruction)
{
    ...
    TR_ASSERT_FATAL(sourceReg->getAssignedRegister() != NULL || targetReg->getAssignedRegister() != NULL,
        "Attempting register copy with source (%s) and target (%s) real registers not corresponding to any virtual "
        "register",
    ...
    case TR_GPR: {
        ...
        auto nonNullAssignedReg = targetReg->getAssignedRegister() != NULL ? targetReg->getAssignedRegister()
                                                                           : sourceReg->getAssignedRegister();

        auto mnemonic = nonNullAssignedReg->is64BitReg() ? TR::InstOpCode::LGR : TR::InstOpCode::LR;
 ...
 */
TR::Instruction* OMR::Z::Machine::registerExchange(TR::CodeGenerator* cg, TR_RegisterKinds rk,
    TR::RealRegister* targetReg, TR::RealRegister* sourceReg, TR::RealRegister* middleReg,
    TR::Instruction* precedingInstruction)
{
    TR_ASSERT_FATAL(sourceReg->getAssignedRegister() != NULL,
        "Attempting register exchange with source real register (%s) not corresponding to any virtual register",
        getRegisterName(sourceReg, cg));
    TR_ASSERT_FATAL(targetReg->getAssignedRegister() != NULL,
        "Attempting register exchange with target real register (%s) not corresponding to any virtual register",
        getRegisterName(targetReg, cg));
    ...
    if (rk == TR_FPR) {
        if (middleReg != NULL) {
            ...
            currentInstruction
                = generateRXInstruction(cg, TR::InstOpCode::STD, currentNode, targetReg, tempMR, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
            currentInstruction
                = generateRRInstruction(cg, TR::InstOpCode::LDR, currentNode, targetReg, sourceReg, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
            TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
            currentInstruction
                = generateRXInstruction(cg, TR::InstOpCode::LD, currentNode, sourceReg, tempMR2, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
    ...
    // exchange vector registers
        ...
        if (middleReg != NULL) {
            ...
            TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
            currentInstruction = generateVRXInstruction(
                cg, TR::InstOpCode::VL, currentNode, sourceReg, tempMR2, 0, precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
        ...
        if (middleReg == NULL) {
            ...
            currentInstruction
                = generateRXInstruction(cg, opLoad, currentNode, sourceReg, tempMR, precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
            currentInstruction
            ...
            TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
            currentInstruction
                = generateRXInstruction(cg, opStore, currentNode, targetReg, tempMR2, precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
...
OMR::Z::Machine::Machine(TR::CodeGenerator* cg)
    : OMR::Machine(cg)
    , _lastGlobalGPRRegisterNumber(-1)
    , _last8BitGlobalGPRRegisterNumber(-1)
    , _lastGlobalFPRRegisterNumber(-1)
    , _lastGlobalCCRRegisterNumber(-1)
    , _lastVolatileNonLinkGPR(-1)
    , _lastLinkageGPR(-1)
    , _lastVolatileNonLinkFPR(-1)
    , _lastLinkageFPR(-1)
    , _globalEnvironmentRegisterNumber(-1)
    , _globalCAARegisterNumber(-1)
    , _globalParentDSARegisterNumber(-1)
    , _globalReturnAddressRegisterNumber(-1)
    , _globalEntryPointRegisterNumber(-1)
{
 ...
 */
TR::RealRegister* OMR::Z::Machine::findBestSwapRegister(TR::Register* reg1, TR::Register* reg2)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        if (_registerFile[i]->getState() == TR::RealRegister::Blocked && _registerFile[i]->getAssignedRegister() != NULL
            && _registerFile[i]->getAssignedRegister()->isPlaceholderReg() && _registerFile[i] != realReg1
 ...
 */
uint32_t OMR::Z::Machine::genBitMapOfAssignableGPRs()
{
 ...
 */
uint32_t OMR::Z::Machine::genBitMapOfAssignableVRFs()
{
    uint32_t availRegMask = 0x0;
 ...
 */
uint8_t OMR::Z::Machine::genBitVectOfLiveGPRPairs()
{
...
bool OMR::Z::Machine::isLegalEvenOddPair(TR::RealRegister* evenReg, TR::RealRegister* oddReg, uint64_t availRegMask)
{
    if (evenReg == NULL || oddReg == NULL) {
        ...
        return self()->isLegalEvenRegister(evenReg, ALLOWBLOCKED, availRegMask)
            && self()->isLegalOddRegister(oddReg, ALLOWBLOCKED, availRegMask);
    } else {
        return false;
    }
}

bool OMR::Z::Machine::isLegalEvenRegister(
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    // Not a legal Even reg if 1) not even or 2) reg+1 is locked/blocked with non-sibling 3) using GPR0 when we
    // shouldn't
    if (regNum % 2 != 0 || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0
        || _registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() == TR::RealRegister::Locked
            && allowLocked == false
...
bool OMR::Z::Machine::isLegalFPPair(TR::RealRegister* firstReg, TR::RealRegister* secondReg, uint64_t availRegMask)
{
    if (firstReg == NULL || secondReg == NULL) {
        ...
        return self()->isLegalFirstOfFPRegister(firstReg, ALLOWBLOCKED, availRegMask)
            && self()->isLegalSecondOfFPRegister(secondReg, ALLOWBLOCKED, availRegMask);
    } else {
        return false;
    }
}

bool OMR::Z::Machine::isLegalFirstOfFPRegister(
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
    ...
    if ((regNum % 4 != 0 && regNum % 4 != 1) || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0
        || _registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() == TR::RealRegister::Locked
            && allowLocked == false
...
bool OMR::Z::Machine::isLegalSecondOfFPRegister(
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
    ...
    // Not a legal Even reg if 1) not even or 2) reg+1 is locked/blocked with non-sibling 3) using GPR0 when we
    // shouldn't 1) must be one of FPR2, FPR6, FPR10, FPR14, FPR3, FPR7, FPR11, FPR15 2) its sibling FP Reg is NOT
    // locked/blocked
    if ((regNum % 4 != 2 && regNum % 4 != 3) || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0
        || _registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Locked
            && allowLocked == false
        || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Blocked
            && allowBlocked == false)) {
 ...
 */
TR::RealRegister* OMR::Z::Machine::findBestLegalOddRegister(uint64_t availRegMask)
{
...
bool OMR::Z::Machine::isLegalOddRegister(
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
    ...
    if (regNum % 2 != 1 || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0
        || _registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Locked
            && allowLocked == false
        || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Blocked
            && allowBlocked == false)) {
 ...
 */
TR::RealRegister* OMR::Z::Machine::findBestLegalEvenRegister(uint64_t availRegMask)
{
    uint32_t bestWeightSoFar = 0xFFFFFFFF;
    TR::RealRegister* freeRegister = NULL;
 ...
 */
TR::RealRegister* OMR::Z::Machine::findBestLegalSiblingFPRegister(bool isFirst, uint64_t availRegMask)
{
    uint32_t bestWeightSoFar = 0xFFFFFFFF;
    TR::RealRegister* freeRegister = NULL;
    ...
    for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
        ...
        if ((lastFirstOfPair->getState() == TR::RealRegister::Free
            ...
            && (lastSecondOfPair->getState() == TR::RealRegister::Free
                || lastSecondOfPair->getState() == TR::RealRegister::Unlatched)
            ...
            && (lastSiblingFPReg->getState() == TR::RealRegister::Free
                || lastSiblingFPReg->getState() == TR::RealRegister::Unlatched)
            && lastSiblingFPReg->getWeight() < bestWeightSoFar) {
            freeRegister = lastSiblingFPReg;
            bestWeightSoFar = freeRegister->getWeight();
        }
 ...
 */
TR::Register* OMR::Z::Machine::assignBestRegister(
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
...
TR::RealRegister* OMR::Z::Machine::findBestRegisterForShuffle(
    TR::Instruction* currentInstruction, TR::Register* currentAssignedRegister, uint64_t availRegMask)
{
    TR_ASSERT(currentAssignedRegister,
        "Register shuffling at instruction %P, but the register is not currently assigned?\n", currentInstruction);
    ...
    if (targetRegister && targetRegister->getRealRegister()
    ...
    if ((newRegister
            = self()->findBestFreeRegister(currentInstruction, kindOfRegister, currentAssignedRegister, availRegMask))
        == NULL) {
...
*/
TR::RealRegister* OMR::Z::Machine::shuffleOrSpillRegister(
    TR::Instruction* currInst, TR::Register* toFreeReg, uint64_t availRegMask)
{
    ...
    TR::RealRegister* bestRegister
        = self()->findBestFreeRegister(currInst, toFreeReg->getKind(), toFreeReg, availRegMask);
 ...
 */
TR::Register* OMR::Z::Machine::assignBestRegisterSingle(
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (kindOfRegister != TR_FPR && kindOfRegister != TR_VRF && assignedRegister != NULL) {
        if ((assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
            ...
            TR::Instruction* cursor = self()->registerCopy(
                self()->cg(), kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, currInst);
    ...
    if (assignedRegister == NULL) {
        ...
        if (targetRegister->getTotalUseCount() != targetRegister->getFutureUseCount()) {
            ...
            if ((assignedRegister
                    = self()->findBestFreeRegister(currInst, kindOfRegister, targetRegister, availRegMask))
                == NULL) {
    ...
    if (doBookKeeping && (assignedRegister->getState() != TR::RealRegister::Locked)) {
        ...
        // OOL: if a register is first defined (became live) in the hot path, no matter how many futureUseCount left (in
        // the code path) the register is considered as dead now in the hot path, so GC map contains the correct list of
        // live registers
 ...
 *            0      32   63
 */
TR::Register* OMR::Z::Machine::assignBestRegisterPair(
    TR::Register* regPair, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (freeRegisterLow == NULL && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount())
        ...
        if (!self()->findBestFreeRegisterPair(
                &tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst, availRegMask)) {
    ...
    if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF)
        ...
        //
        if (!self()->findBestFreeRegisterPair(
                &freeRegisterHigh, &freeRegisterLow, regPairKind, currInst, availRegMask)) {
    ...
    } else if (TR_FPR != regPair->getKind()) { // GPR pair case, real reg assigned but not a legal even-odd pair
        if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
                ...
                if (!self()->findBestFreeRegisterPair(
                        &tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst, availRegMask)) {
                    self()->freeBestRegisterPair(
                        &tfreeRegisterHigh, &tfreeRegisterLow, regPairKind, currInst, availRegMask);
                }

                self()->coerceRegisterAssignment(
                    currInst, firstReg, (toRealRegister(tfreeRegisterHigh)->getRegisterNumber()));
                self()->coerceRegisterAssignment(
                    currInst, lastReg, (toRealRegister(tfreeRegisterLow)->getRegisterNumber()));
        ...
        if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            self()->coerceRegisterAssignment(currInst, lastReg,
                (TR::RealRegister::RegNum)(
                    toRealRegister(freeRegisterHigh->getSiblingRegister())->getRegisterNumber()));
            ...
            //
            if (!self()->findBestFreeRegisterPair(
                    &tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(), currInst, availRegMask)) {
                self()->freeBestRegisterPair(
                    &tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(), currInst, availRegMask);
            }
    ...
    // OOL: if a register is first defined (became live) in the hot path, no matter how many futureUseCount left (in the
    // code path) the register is considered as dead now in the hot path, so GC map contains the correct list of live
    // registers
    if (doBookKeeping) {
        firstReg->setIsLive();
 ...
 */
bool OMR::Z::Machine::findBestFreeRegisterPair(TR::RealRegister** firstRegister, TR::RealRegister** lastRegister,
...
void OMR::Z::Machine::freeBestFPRegisterPair(
    TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
        ...
        if ((_registerFile[firstReg]->getState() == TR::RealRegister::Free
                || _registerFile[firstReg]->getState() == TR::RealRegister::Unlatched)
    ...
    if (bestVirtCandidateLow != NULL) {
        ...
        cursor = generateRXInstruction(
            self()->cg(), TR::InstOpCode::LD, currentNode, bestCandidateLow, tempMRLow, currInst);
        ...
        if (!self()->cg()->isOutOfLineColdPath()) {
            ...
            if (locationLow->getMaxSpillDepth() != 1 && locationLow->getMaxSpillDepth() != 2)
    ...
    if (bestVirtCandidateHigh != NULL) {
        ...
        cursor = generateRXInstruction(
            self()->cg(), TR::InstOpCode::LD, currentNode, bestCandidateHigh, tempMRHigh, currInst);
        if (!cursor->assignFreeRegBitVector()) {
            cursor->assignBestSpillRegister();
        }
        ...
        if (!self()->cg()->isOutOfLineColdPath()) {
            ...
            // if this spill is inside OOL cold path, we do not need to protecting the spill slot
            // because the post condition at OOL entry does not expect this register to be spilled
            if (locationHigh->getMaxSpillDepth() != 1 && locationHigh->getMaxSpillDepth() != 2)
    ...
    TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    if (bestVirtCandidateLow != NULL) {
        ...
        if (bestVirtCandidateLow->is64BitReg()) {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::LG, currentNode, bestCandidateLow, tempMRLow, currInst);
        } else {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::L, currentNode, bestCandidateLow, tempMRLow, currInst);
        }
    ...
    if (bestVirtCandidateHigh != NULL) {
        ...
        if (bestVirtCandidateHigh->is64BitReg()) {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::LG, currentNode, bestCandidateHigh, tempMRHigh, currInst);
        } else {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::L, currentNode, bestCandidateHigh, tempMRHigh, currInst);
        }
...
////////////////////////////////////////////////////////////////////////////////
TR::RealRegister* OMR::Z::Machine::findBestFreeRegister(
    TR::Instruction* currentInstruction, TR_RegisterKinds rk, TR::Register* virtualReg, uint64_t availRegMask)
{
    ...
    if (comp->getOption(TR_Randomize)) {
        ...
        if (preference != randomPreference
            && performTransformation(
                comp, "O^O Random Codegen - Randomizing Preference from: %d to: %d\n", preference, randomPreference)) {
            preference = randomPreference;
        }
    }

    uint64_t prefRegMask = TR::RealRegister::isRealReg((TR::RealRegister::RegNum)preference)
        ? _registerFile[preference]->getRealRegisterMask()
        : 0;
    ...
    if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair) {
        ...
        if ((virtualReg->getSiblingRegister())
            ...
            && self()->isLegalOddRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask
                = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
            ...
            {
                _registerFile[toRealRegister(bestRegister)->getRegisterNumber() + 1]->setWeight(
                    S390_REGISTER_PAIR_SIBLING);
            }
        }
        ...
        if ((virtualReg->getSiblingRegister())
            && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister())
            && self()->isLegalEvenRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask
                = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
        ...
        if ((virtualReg->getSiblingRegister())
            && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister())
            && self()->isLegalSecondOfFPRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask
                = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
            // If the sibling is assigned we choose the even register next to it
            if (availRegMask & sibRegMask) {
            ...
            // Set the pair's sibling to a high weight so that assignment to this real reg is unlikely
            {
                _registerFile[toRealRegister(bestRegister)->getRegisterNumber() + 2]->setWeight(
                    S390_REGISTER_PAIR_SIBLING);
            }
        }
        ...
        if ((virtualReg->getSiblingRegister())
            && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister())
            && self()->isLegalFirstOfFPRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask
                = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
            // If the sibling is assigned we choose the even register next to it
            if (availRegMask & sibRegMask) {
        ...
        // If preference register is also marked as an interference at some point, check to see if the interference
        // still applies.
        if (liveRegOn && preference != 0 && (interference & (1 << (preference - maskI)))) {
            if (!boundNext(currentInstruction, preference, virtualReg)) {
                preference = 0;
            }
            prefRegMask = TR::RealRegister::isRealReg((TR::RealRegister::RegNum)preference)
                ? _registerFile[preference]->getRealRegisterMask()
                : 0;
        }
 ...
 */
TR::RealRegister* OMR::Z::Machine::findRegNotUsedInInstruction(TR::Instruction* currentInstruction)
{
 ...
 */
void OMR::Z::Machine::spillAllVolatileHighRegisters(TR::Instruction* currentInstruction)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (volatileReg && realReg->getState() == TR::RealRegister::Assigned
            && (virtReg = realReg->getAssignedRegister()) && virtReg->is64BitReg()) {
...
TR::RealRegister* OMR::Z::Machine::freeBestRegister(
    TR::Instruction* currentInstruction, TR::Register* virtReg, TR_RegisterKinds rk, bool allowNullReturn)
{
...
////////////////////////////////////////////////////////////////////////////////
TR::RealRegister* OMR::Z::Machine::reverseSpillState(
    TR::Instruction* currentInstruction, TR::Register* spilledRegister, TR::RealRegister* targetRegister)
{
    ...
    if (targetRegister == NULL) {
        ...
        targetRegister
            = self()->findBestFreeRegister(currentInstruction, spilledRegister->getKind(), spilledRegister, regMask);
        if (targetRegister == NULL) {
 ...
 */
bool OMR::Z::Machine::isAssignable(TR::Register* virtReg, TR::RealRegister* realReg)
{
 ...
 */
TR::Instruction* OMR::Z::Machine::coerceRegisterAssignment(
    TR::Instruction* currentInstruction, TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber)
{
    ...
    else if (targetRegister->getState() == TR::RealRegister::Free
    ...
    // the target reg is blocked so we need to guarantee it stays in a reg
        ...
        self()->cg()->traceRegisterAssignment(
            "target %R is blocked, assigned to %R", targetRegister, currentTargetVirtual);
        uint64_t regMask = 0xffffffff;
        ...
        if (spareReg == NULL) {
            ...
            spareReg = self()->freeBestRegister(
                currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind(), true);
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)) {
                {
                    ...
                    cursor = self()->registerCopy(
                        self()->cg(), rk, currentAssignedRegister, targetRegister, currentInstruction);
                ...
                cursor = self()->registerExchange(
                    self()->cg(), rk, targetRegister, currentAssignedRegister, spareReg, currentInstruction);
    ...
    // the target reg is assigned
        ...
        currentTargetVirtual = targetRegister->getAssignedRegister();
        self()->cg()->traceRegisterAssignment(
            "target %R is assigned, assigned to %R", targetRegister, currentTargetVirtual);
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister)
                ...
                if (spareReg == NULL) {
                    ...
                    if (virtualRegister->is64BitReg()) {
                        ...
                        spareReg = self()->freeBestRegister(
                            currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind());
                    } else {
                        spareReg = self()->freeBestRegister(
                            currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind(), true);
                    }
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::Machine::initializeRegisterFile()
{

    // Initialize GPRs
    _registerFile[TR::RealRegister::NoReg] = NULL;

    _registerFile[TR::RealRegister::GPR0] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR0, TR::RealRegister::GPR0Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR1] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR1, TR::RealRegister::GPR1Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR2] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR2, TR::RealRegister::GPR2Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR3] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR3, TR::RealRegister::GPR3Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR4] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR4, TR::RealRegister::GPR4Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR5] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR5, TR::RealRegister::GPR5Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR6] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR6, TR::RealRegister::GPR6Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR7] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR7, TR::RealRegister::GPR7Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR8] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR8, TR::RealRegister::GPR8Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR9] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR9, TR::RealRegister::GPR9Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR10] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR10, TR::RealRegister::GPR10Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR11] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR11, TR::RealRegister::GPR11Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR12] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR12, TR::RealRegister::GPR12Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR13] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR13, TR::RealRegister::GPR13Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR14] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR14, TR::RealRegister::GPR14Mask, self()->cg());

    _registerFile[TR::RealRegister::GPR15] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR15, TR::RealRegister::GPR15Mask, self()->cg());
    ...
    _registerFile[TR::RealRegister::FPR0] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR0, TR::RealRegister::FPR0Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR1] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR1, TR::RealRegister::FPR1Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR2] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR2, TR::RealRegister::FPR2Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR3] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR3, TR::RealRegister::FPR3Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR4] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR4, TR::RealRegister::FPR4Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR5] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR5, TR::RealRegister::FPR5Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR6] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR6, TR::RealRegister::FPR6Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR7] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR7, TR::RealRegister::FPR7Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR8] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR8, TR::RealRegister::FPR8Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR9] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR9, TR::RealRegister::FPR9Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR10] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR10, TR::RealRegister::FPR10Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR11] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR11, TR::RealRegister::FPR11Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR12] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR12, TR::RealRegister::FPR12Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR13] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR13, TR::RealRegister::FPR13Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR14] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR14, TR::RealRegister::FPR14Mask, self()->cg());

    _registerFile[TR::RealRegister::FPR15] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR15, TR::RealRegister::FPR15Mask, self()->cg());
    ...
    _registerFile[TR::RealRegister::VRF16] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF16, TR::RealRegister::VRF16Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF17] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF17, TR::RealRegister::VRF17Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF18] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF18, TR::RealRegister::VRF18Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF19] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF19, TR::RealRegister::VRF19Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF20] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF20, TR::RealRegister::VRF20Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF21] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF21, TR::RealRegister::VRF21Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF22] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF22, TR::RealRegister::VRF22Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF23] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF23, TR::RealRegister::VRF23Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF24] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF24, TR::RealRegister::VRF24Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF25] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF25, TR::RealRegister::VRF25Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF26] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF26, TR::RealRegister::VRF26Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF27] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF27, TR::RealRegister::VRF27Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF28] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF28, TR::RealRegister::VRF28Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF29] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF29, TR::RealRegister::VRF29Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF30] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF30, TR::RealRegister::VRF30Mask, self()->cg());

    _registerFile[TR::RealRegister::VRF31] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF31, TR::RealRegister::VRF31Mask, self()->cg());
...
void OMR::Z::Machine::initializeFPRegPairTable()
{
    ...
    if (linkage->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
    {
        for (int32_t i = TR::RealRegister::LastAssignableGPR; i >= TR::RealRegister::FirstGPR; --i) {
            ...
            if (linkage->getPreserved(regNum)) {
                if (regNum != linkage->getStaticBaseRegister() && regNum != linkage->getPrivateStaticBaseRegister()
        ...
        for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
            ...
            if (linkage->getPreserved(regNum)) {
                if (regNum != linkage->getLitPoolRegister() && regNum != linkage->getStaticBaseRegister()
...
/**
 * Initialize global vectors in a common location since all configurations (zOS/zLinux 31/64) have the same linkage
 * conventions Each register is explicitly set to stay consistent with a majority of the global register number setup
 * procedures across z-supported platforms and configurations
 *
...
uint32_t OMR::Z::Machine::initGlobalVectorRegisterMap(uint32_t vectorOffset)
{
    ...
    // For other machines with peculiar overlapping btwn FPR/VRF, highly recommend overriding
    // initGlobalVectorRegisterMap
    //         .--------.
    ...
    // Colouring will make sure to map global virtual FPRs only to the first 16 and vector global virtuals can map to
    // all 32 registers.
...
TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
    return _lastGlobalGPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
    return _firstGlobalGPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
    self()->setFirstOverlappedGlobalFPRRegisterNumber(reg);
    return _firstGlobalFPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg)
{
    self()->setLastOverlappedGlobalFPRRegisterNumber(reg);
    return _lastGlobalFPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg)
{
    return _firstGlobalVRFRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg)
{
    return _lastGlobalVRFRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalCCRRegisterNumber(TR_GlobalRegisterNumber reg)
{
...
TR::Register* OMR::Z::Machine::setVirtualAssociatedWithReal(TR::RealRegister::RegNum regNum, TR::Register* virtReg)
{
    if ((regNum == TR::RealRegister::EvenOddPair) || (regNum == TR::RealRegister::LegalEvenOfPair)
...
void OMR::Z::Machine::takeRegisterStateSnapShot()
{
...
void OMR::Z::Machine::restoreRegisterStateFromSnapShot()
{
    for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
...
TR::RegisterDependencyConditions* OMR::Z::Machine::createCondForLiveAndSpilledGPRs(
