...
void TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    case TR_MethodObject: {
        TR::SymbolReference* reloSymRef
            = (reloType == TR_ClassAddress) ? getNode()->getSymbolReference() : getSymbolReference();
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t*)reloSymRef,
                                        getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1,
                                        (TR_ExternalRelocationTargetKind)reloType, cg()),
            __FILE__, __LINE__, getNode());
    } break;
    ...
    case TR_HelperAddress:
        if (cg()->comp()->target().is64Bit()) {
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)*((uint64_t*)cursor), TR_AbsoluteHelperAddress, cg()),
                __FILE__, __LINE__, getNode());
        } else {
            cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor,
                                            (uint8_t*)(intptr_t) * ((uint32_t*)cursor), TR_AbsoluteHelperAddress, cg()),
                __FILE__, __LINE__, getNode());
        }
        break;
    ...
    case TR_BodyInfoAddress:
        if (cg()->comp()->target().is64Bit()) {
            ...
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)(intptr_t) * ((uint32_t*)cursor),
                    (TR_ExternalRelocationTargetKind)reloType, cg()),
                __FILE__, __LINE__, getNode());
        }
        break;
    ...
    case TR_CatchBlockCounter: {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, NULL, TR_CatchBlockCounter, cg()), __FILE__, __LINE__, getNode());
    } break;
    ...
    case TR_ClassPointer:
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            // for optimizations where we are trying to relocate either profiled j9class or getfrom signature we can't
            // use node to get the target address so we need to pass it to relocation in targetaddress2 for now two
            // instances where use this relotype in such way are: profile checkcast and arraystore check object check
            // optimiztaions
    ...
    case TR_MethodEnterExitHookAddress: {
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t*)getNode()->getSymbolReference(),
...
uint8_t* TR::S390ConstantDataSnippet::emitSnippetBody()
{
    ...
    case TR_ClassAddress:
    case TR_ClassObject: {
        uintptr_t romClassPtr = TR::Compiler->cls.persistentClassPointerFromClassPointer(
            comp, (TR_OpaqueClassBlock*)(*((uintptr_t*)cursor)));
...
TR::S390ConstantInstructionSnippet::S390ConstantInstructionSnippet(
    TR::CodeGenerator* cg, TR::Node* n, TR::Instruction* instr)
