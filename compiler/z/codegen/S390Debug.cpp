...
void TR_Debug::printInstructionComment(
    TR::FILE* pOutFile, int32_t tabStops, TR::Instruction* instr, bool needsStartComment)
{
    while (tabStops-- > 0) {
        trfprintf(pOutFile, "\t");
    }
    ...
    for (int j = 0; j < last; ++j) {
        ...
        if ((intptr_t)dependency->getRegister() > 0) {
            ...
            printS390RegisterDependency(
                pOutFile, virtReg, j + 1, dependency->getRefsRegister(), dependency->getDefsRegister());
        }
    }
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390ImmInstruction* instr)
{
    ...
    printPrefix(pOutFile, instr);
    trfprintf(
        pOutFile, "%-*s0x%08x", OPCODE_SPACING, instr->getOpCode().getMnemonicName(), instr->getSourceImmediate());
    ...
    if (sym) {
        ...
        if (mr->getUnresolvedSnippet() != NULL) {
            if (mr->getUnresolvedSnippet()->getUnresolvedData() != NULL) {
                trfprintf(
                    pOutFile, " target is [%p]", mr->getUnresolvedSnippet()->getUnresolvedData()->getSnippetLabel());
            }
        }
#endif
    }
...
char* TR_Debug::printSymbolName(
    TR::FILE* pOutFile, TR::Symbol* sym, TR::SymbolReference* symRef, TR::MemoryReference* mr)
{
    char* str;
    bool sawWCodeName = false;
...
void TR_Debug::printS390RegisterDependency(
    TR::FILE* pOutFile, TR::Register* virtReg, int realReg, bool refsReg, bool defsReg)
{
...
uint8_t* TR_Debug::printRuntimeInstrumentationOnOffInstruction(
    TR::FILE* pOutFile, uint8_t* cursor, bool isRION, bool isPrivateLinkage)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390VRIInstruction* instr)
{
    ...
    case TR::Instruction::IsVRIa:
        ...
        break;
    case TR::Instruction::IsVRIb:
        trfprintf(pOutFile, ",0x%x,0x%x", maskHalf(static_cast<TR::S390VRIbInstruction*>(instr)->getImmediateField2()),
    ...
    case TR::Instruction::IsVRIf:
        ...
        break;
    case TR::Instruction::IsVRIg:
        trfprintf(pOutFile, ",0x%x, 0x%x", maskHalf(static_cast<TR::S390VRIgInstruction*>(instr)->getImmediateField3()),
