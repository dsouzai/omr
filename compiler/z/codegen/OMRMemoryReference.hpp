...
/** ExposedConsantAddressing is true BCD and Aggr const nodes that have the lit pool address fully exposed as the first
 * child The alternative is if litPoolOffset is just picked off of the constant node itself */
#define ExposedConstantAddressing 0x400000
#define AdjustedForLongDisplacement 0x800000
/** MemRefCreatedDuringInstructionSelection is true for inserting BPP after a call or a label, we need to load the
 * return address from the stack, this happens during doInstructionSelection when we don't have stack frame, so we need
 * to update the memory reference during binary generation */
...
private:
     ...
     * (such as a load from the getWCodeMainLitSymRef() static processed through populateLoadAddrTree) the
     * _symbolReference is changed to point to a shared symbol with a size with no direct relation to the current
     * instructions use of the symbol. In these cases the size from the original rootLoadOrStore node's symRef is cached
     * in _fixedSizeForAlignment
     */
...
public:
    ...
    TR::Register *setBaseRegister(TR::Register *br, TR::CodeGenerator *cg)
    {
        ...
        if ((_baseRegister != NULL) && (_baseRegister->getRegisterPair() != NULL)) {
     ...
     *
     * \note
     *    The offset may not represent the final displacement which will be used to encode the memory reference until
     * the binary encoding of this memory reference is generated.
     */
    ...
    TR::Instruction *setTargetSnippetInstruction(TR::Instruction *i) { return _targetSnippetInstruction = i; }
    ...
    /**
     * Memory references created for indirect loads/stores have a fixed size set as the memRef may be set with the
     * underlying loadaddr symRef. This loadaddr symRef does not reflect the actual size of the symbol being loaded so
     * the fixed _fixedSizeForAlignment must be used instead.
     */
     ...
     *
     * \note
     *    A return value of `false` does not imply long displacement is not required. This is because the displacement
     * of this memory reference is not fully known until binary encoding. Only after the binary encoding of this memory
     *    reference will a return value of `false` indicate that long displacement is not required. The offset may
     * increase by some amount during code generation, however it will never decrease. This is why a return value of
     * `true` always implies long displacement will be required.
     */
    ...
    TR::Instruction *separateIndexRegister(TR::Node *subTree, TR::CodeGenerator *cg, bool enforce4KDisplacementLimit,
        TR::Instruction *preced, bool forceDueToAlignmentBump = false);

    bool ignoreNegativeOffset();
    bool alignmentBumpMayRequire4KFixup(TR::Node *node, TR::CodeGenerator *cg);
    TR::Instruction *handleLargeOffset(TR::Node *node, TR::MemoryReference *interimMemoryReference,
    ...
    TR::Instruction *enforce4KDisplacementLimit(TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *preced,
        bool forceFixup = false, bool forceDueToAlignmentBump = false);
    ...
    TR::Instruction *markAndAdjustForLongDisplacementIfNeeded(TR::Node *node, TR::CodeGenerator *cg,
        TR::Instruction *preced, bool *testLongDisp = NULL);
    ...
    TR::UnresolvedDataSnippet *createUnresolvedDataSnippetForiaload(TR::Node *node, TR::CodeGenerator *cg,
        TR::SymbolReference *symRef, TR::Register *tempReg, bool &isStore)
    {
        return NULL;
    }
...
TR::MemoryReference *generateS390MemoryReference(TR::Register *, int32_t, TR::CodeGenerator *cg,
...
TR::MemoryReference *generateS390MemoryReference(int32_t, TR::DataType, TR::CodeGenerator *, TR::Register *,
    TR::Node *node = NULL);
TR::MemoryReference *generateS390MemoryReference(int64_t, TR::DataType, TR::CodeGenerator *, TR::Register *,
    TR::Node *node = NULL);
...
TR::MemoryReference *generateS390MemoryReference(TR::CodeGenerator *cg, TR::Node *, TR::Node *, int32_t disp = 0,
    bool forSS = false);
...
TR::MemoryReference *generateS390RightAlignedMemoryReference(TR::MemoryReference &baseMR, TR::Node *node,
    int32_t offset, TR::CodeGenerator *cg, bool enforceSSLimits = true);
TR::MemoryReference *generateS390LeftAlignedMemoryReference(TR::MemoryReference &baseMR, TR::Node *node, int32_t offset,
    TR::CodeGenerator *cg, int32_t leftMostByte, bool enforceSSLimits = true);
#endif
