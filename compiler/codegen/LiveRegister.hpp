...
public:
    ...
    int32_t decNodeCount()
    {
        TR_ASSERT(_numOwningNodes > 0, "Bad node count");
        return --_numOwningNodes;
    }
...
#endif
    void addByteRegisterInterference()
    {
        if (!(_association & 0x80000000))
            _interference |= 0x80000000;
    }
