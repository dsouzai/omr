    ...
    for (tt = self()->comp()->getStartTree(); tt; prev = tt, tt = tt->getNextTreeTop()) {
        ...
        if (opcode.getOpCodeValue() == TR::BBStart) {
            ...
            if (node->getBranchDestination()->getNode()->getLabel() == NULL) {
                ...
                TR::LabelSymbol* label = TR::LabelSymbol::create(
                    self()->trHeapMemory(), self(), node->getBranchDestination()->getNode()->getBlock());
...
bool OMR::CodeGenerator::canClobberNodesRegister(
    TR::Node* node, uint16_t count, TR_ClobberEvalData* data, bool ignoreRefCount)
{
...
bool OMR::CodeGenerator::mulDecompositionCostIsJustified(
...
void OMR::CodeGenerator::apply16BitLabelRelativeRelocation(
    int32_t* cursor, TR::LabelSymbol* label, int8_t d, bool isInstrOffset)
{
    *(int16_t*)cursor += (int16_t)(intptr_t)label->getCodeLocation();
}
...
void OMR::CodeGenerator::traceRegAssigned(
    TR::Register* virtReg, TR::Register* realReg, TR_RegisterAssignmentFlags flags)
{
    if (self()->getDebug())
...
TR_StorageOverlapKind OMR::CodeGenerator::storageMayOverlap(
    TR::Node* node1, size_t length1, TR::Node* node2, size_t length2)
{
...
TR_GlobalRegisterNumber OMR::CodeGenerator::getOverlappedAliasForGRN(TR_GlobalRegisterNumber n)
{
    TR_ASSERT(self()->isAliasedGRN(n), "Can only get alias from supported types. Current grn : %d (%s)", n,
        self()->getDebug()->getGlobalRegisterName(n));
    TR_ASSERT(
        self()->getOverlapOffsetBetweenAliasedGRNs() > 0, "Overlap offset must be set before invoking this method");
        ...
        if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
            if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
                ...
                if (firstChildrenMatch && addr1->getSecondChild() && addr2->getSecondChild()
                    ...
                    secondChildrenMatch = self()->additionsMatch(
                        addr1->getSecondChild(), addr2->getSecondChild(), addressesUnderSameTreeTop);
                }
...
uint8_t* OMR::CodeGenerator::allocateCodeMemoryInner(
    uint32_t warmCodeSizeInBytes, uint32_t coldCodeSizeInBytes, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    ...
    uint8_t* warmCode = TR::CodeCacheManager::instance()->allocateCodeMemory(
        warmCodeSizeInBytes, coldCodeSizeInBytes, &codeCache, coldCode, false, isMethodHeaderNeeded);
...
uint8_t* OMR::CodeGenerator::allocateCodeMemory(
    uint32_t warmCodeSizeInBytes, uint32_t coldCodeSizeInBytes, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    uint8_t* warmCode;
    warmCode
...
#define NUM_32BIT_MAGIC_VALUES 11
    static uint32_t div32BitMagicValues[NUM_32BIT_MAGIC_VALUES][3] =
        ...
        { { 3, 0x55555556, 0 }, { 5, 0x66666667, 1 }, { 7, 0x92492493, 2 }, { 9, 0x38e38e39, 1 }, { 10, 0x66666667, 2 },
            { 12, 0x2aaaaaab, 1 }, { 24, 0x2aaaaaab, 2 }, { 60, 0x88888889, 5 }, { 100, 0x51eb851f, 5 },
            { 1000, 0x10624dd3, 6 }, { 3600, 0x91a2b3c5, 11 } };
...
#define NUM_64BIT_MAGIC_VALUES 6
    static int64_t div64BitMagicValues[NUM_64BIT_MAGIC_VALUES][3] =
        ...
        { { 3, CONSTANT64(0x5555555555555556), 0 }, { 5, CONSTANT64(0x6666666666666667), 1 },
            { 7, CONSTANT64(0x4924924924924925), 1 }, { 9, CONSTANT64(0x1c71c71c71c71c72), 0 },
            { 10, CONSTANT64(0x6666666666666667), 2 }, { 12, CONSTANT64(0x2aaaaaaaaaaaaaab), 1 } };
...
TR::SymbolReference* OMR::CodeGenerator::allocateLocalTemp(TR::DataType dt, bool isInternalPointer)
{
    ...
    if (isInternalPointer) {
        temp = TR::AutomaticSymbol::createInternalPointer(
            self()->trHeapMemory(), dt, TR::Symbol::convertTypeToSize(dt), self()->fe());
    } else {
...
TR::Instruction* OMR::CodeGenerator::generateDebugCounter(
    TR::Instruction* cursor, const char* name, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    if (!cursor)
    ...
    aggregatedCounters->aggregateStandardCounters(
        self()->comp(), cursor->getNode(), name, delta, fidelity, staticDelta);
...
TR::Instruction* OMR::CodeGenerator::generateDebugCounter(
    TR::Instruction* cursor, const char* name, TR::Register* deltaReg, int8_t fidelity, int32_t staticDelta)
{
    if (!cursor)
        cursor = self()->getAppendInstruction();
    if (!self()->comp()->getOptions()->enableDebugCounters())
...
void OMR::CodeGenerator::addExternalRelocation(
    TR::Relocation* r, TR::RelocationDebugInfo* info, TR::ExternalRelocationPositionRequest where)
{
    if (self()->comp()->compileRelocatableCode()) {
...
void OMR::CodeGenerator::redoTrampolineReservationIfNecessary(
    TR::Instruction* callInstr, TR::SymbolReference* instructionSymRef)
{
