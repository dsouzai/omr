 ...
 */
static void lowerNewValue(TR::Compilation* comp, TR::Node* node, TR::TreeTop* tt)
{
    ...
    auto* valueClass
        = static_cast<TR_OpaqueClassBlock*>(node->getFirstChild()->getSymbol()->getStaticSymbol()->getStaticAddress());
    ...
    for (int i = 1; i < node->getNumChildren(); ++i) {
        ...
        auto* symref = comp->getSymRefTab()->findOrFabricateShadowSymbol(valueClass, fieldEntry._datatype,
...
void OMR::CodeGenerator::lowerTreeIfNeeded(TR::Node* node, int32_t childNumberOfNode, TR::Node* parent, TR::TreeTop* tt)
{
    if (node->getOpCodeValue() == TR::loadaddr && node->getOpCode().hasSymbolReference()
        && node->getSymbol()->isLabel()) {
    ...
    if (self()->getLocalsIG() && self()->getSupportsCompactedLocals()) {
        ...
        //  load auto2 (colour x)
        if (node->getOpCode().isStoreDirect() && node->getSymbolReference()->getSymbol()->isAuto()
            && node->getFirstChild()->getReferenceCount() == 1 && node->getFirstChild()->getOpCode().isLoadVarDirect()
            ...
            TR::AutomaticSymbol* loadLocal
                = node->getFirstChild()->getSymbolReference()->getSymbol()->castToAutoSymbol();

            if (!storeLocal->holdsMonitoredObject() && !storeLocal->isInternalPointer()
                && !storeLocal->isPinningArrayPointer() && !loadLocal->holdsMonitoredObject()
                && !loadLocal->isInternalPointer() && !loadLocal->isPinningArrayPointer()) {
    ...
    } else if (node->getOpCode().isStore() && node->getSymbol()->isParm() && node->getSymbol()->isCollectedReference())
        node->getSymbol()->setParmHasToBeOnStack();
...
void OMR::CodeGenerator::identifyUnneededByteConvNodes(
    TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount, TR::DataType storeType)
{
    ...
    if (opCode.isConversion()
        || (opCode.isAnd() && parent->getSecondChild()->getOpCode().isLoadConst()
        ...
        || opCode.isStore() || opCode.isLoad() || ((parent == treeTop->getNode()) && opCode.isBooleanCompare())) {
        ...
        for (int32_t childCount = parent->getNumChildren() - 1; childCount >= 0; childCount--) {
            ...
            if (child->getVisitCount() != visitCount) {
                if (childCount == 0 && skipFirstChild)
                    continue; // skip address of indirect store
                ...
                bool isTwoByte
                    ...
                    && child->getOpCode().isConversion();
                if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i || childOp == TR::bu2i))
                    || (isTwoByte && (childOp == TR::i2s || childOp == TR::s2i || childOp == TR::su2i))) {
                    if (child->getReferenceCount() > 1
                        && (!treeTop->getNode()->getOpCode().isBooleanCompare()
                            || (childOp != TR::bu2i && childOp != TR::su2i))
                        && performTransformation(
                            self()->comp(), "%sReplacing shared i2b/b2i node %p\n", OPT_DETAILS, child)) {
                    ...
                    if ((childOp == TR::b2i || childOp == TR::s2i) && treeTop->getNode()->getOpCode().isBooleanCompare()
                        ...
                        && performTransformation(
                            self()->comp(), "%sChanging b2i node %p to unsigned conversion\n", OPT_DETAILS, child)) {
                        TR::Node::recreate(child, (storeType == TR::Int8 ? TR::bu2i : TR::su2i));
                    } else if (performTransformation(
                                   self()->comp(), "%sMarking i2b/b2i node %p as unneeded\n", OPT_DETAILS, child))
...
void OMR::CodeGenerator::identifyUnneededByteConvNodes()
{
    ...
    if (!performTransformation(
            self()->comp(), "%s ===>   Identify and mark Unneeded b2i/i2b conversions  <===\n", OPT_DETAILS))
        return;
    ...
    for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
...
#endif
                node->getOpCode().isByte() || node->getOpCode().isShort())
        ...
        // Don't do conversion when consuming treetop is a compare == or != 0 to 128
            ...
            && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isIntegral()) {
...
void OMR::CodeGenerator::cleanupFlags(TR::Node* node)
{
    ...
    //
    if (node->isDualHigh()) {
        TR_ASSERT(!node->getChild(2)->isAdjunct(),
            "Code Limitation: Cannot cope with common adjunct between dual highs. node = %p.", node);
                ...
                if (classChild->getOpCodeValue() != TR::loadaddr) {
                    TR::DebugCounter::prependDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(
                            self()->comp(), "allocations/%s/child-%s", opName, classChild->getOpCode().getName()),
                        tt);
                } else if (classChild->getSymbolReference()->isUnresolved()) {
                    TR::DebugCounter::prependDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(
                            self()->comp(), "allocations/%s/unresolved", opName, classChild->getOpCode().getName()),
                        tt);
                } else {
                    int32_t length;
                    ...
                    TR::DebugCounter::prependDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(
                            self()->comp(), "allocations/%s/(%.*s)", opName, length, className),
                        tt);
                }
            } else {
                // newarray
