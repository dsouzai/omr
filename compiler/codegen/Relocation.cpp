...
{
    TR_ASSERT_FATAL(_label->getCodeLocation() != NULL, "cannot relocate reference to undefined label: %s (%p)\n",
        _label->getName(TR::comp()->getDebug()), _label);
}
...
void TR::LabelRelative16BitRelocation::apply(TR::CodeGenerator* cg)
{
    ...
    if (getAddressDifferenceDivisor() == 1)
        ...
        cg->apply16BitLabelRelativeRelocation(
            (int32_t*)getUpdateLocation(), getLabel(), getAddressDifferenceDivisor(), isInstructionOffset());
}
...
TR::InstructionLabelRelative16BitRelocation::InstructionLabelRelative16BitRelocation(
    TR::Instruction* cursor, int32_t offset, TR::LabelSymbol* l, int32_t divisor)
...
uint8_t* TR::InstructionLabelRelative16BitRelocation::getUpdateLocation()
{
...
void TR::InstructionLabelRelative16BitRelocation::apply(TR::CodeGenerator* cg)
{
...
TR::InstructionLabelRelative32BitRelocation::InstructionLabelRelative32BitRelocation(
    TR::Instruction* cursor, int32_t offset, TR::LabelSymbol* l, int32_t divisor)
    : TR::LabelRelocation(NULL, l)
    , _cursor(cursor)
    , _offset(offset)
...
uint8_t* TR::InstructionLabelRelative32BitRelocation::getUpdateLocation()
{
    uint8_t* updateLocation = TR::LabelRelocation::getUpdateLocation();
...
void TR::InstructionLabelRelative32BitRelocation::apply(TR::CodeGenerator* cg)
{
    uint8_t* p = getUpdateLocation();
    assertLabelDefined();
...
void TR::ExternalRelocation::addExternalRelocation(TR::CodeGenerator* cg)
{
    ...
    for (r = aot.getFirst(); r != 0; r = r->getNext()) {
        if (r->full() == false && r->getTargetAddress() == _targetAddress && r->getTargetAddress2() == _targetAddress2
            && r->getTargetKind() == _kind && modifier.getValue() == r->getModifierValue()) {
            if (!r->needsWideOffsets()) {
                if (r->getSizeOfRelocationData() + narrowSize > MAX_SIZE_RELOCATION_DATA) {
                    r->setFull();
                    continue; // look for one that's not full
                }
            } else {
                if (r->getSizeOfRelocationData() + wideSize > MAX_SIZE_RELOCATION_DATA) {
                    r->setFull();
                    continue; // look for one that's not full
                }
            }
            r->setNumberOfRelocationSites(r->getNumberOfRelocationSites() + 1);
            r->setSizeOfRelocationData(r->getSizeOfRelocationData() + (r->needsWideOffsets() ? wideSize : narrowSize));
            _relocationRecord = r;
    ...
    temp->setSizeOfRelocationData(temp->getSizeOfRelocationData() + (temp->needsWideOffsets() ? wideSize : narrowSize));
...
TR::ExternalOrderedPair32BitRelocation::ExternalOrderedPair32BitRelocation(
    uint8_t* location1, uint8_t* location2, uint8_t* target, TR_ExternalRelocationTargetKind k, TR::CodeGenerator* cg)
    : TR::ExternalRelocation()
    , _update2Location(location2)
{
...
TR::ExternalOrderedPair32BitRelocation::ExternalOrderedPair32BitRelocation(uint8_t* location1, uint8_t* location2,
    uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind k, TR::CodeGenerator* cg)
    : TR::ExternalRelocation()
    , _update2Location(location2)
{
    setUpdateLocation(location1);
...
uint8_t TR::ExternalOrderedPair32BitRelocation::collectModifier()
{
    ...
    if (comp->target().cpu.isPower()
        && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethod || kind == TR_GlobalValue
            || kind == TR_BodyInfoAddressLoad || kind == TR_DataAddress || kind == TR_DebugCounter
            || kind == TR_BlockFrequency || kind == TR_RecompQueuedFlag || kind == TR_CatchBlockCounter
            || kind == TR_MethodEnterExitHookAddress)) {
...
void TR::ExternalOrderedPair32BitRelocation::apply(TR::CodeGenerator* cg)
{
    ...
    TR_ExternalRelocationTargetKind kind = getRelocationRecord()->getTargetKind();
    if (comp->target().cpu.isPower()
        && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethodSequence
            || kind == TR_GlobalValue || kind == TR_BodyInfoAddressLoad || kind == TR_DataAddress
            || kind == TR_DebugCounter || kind == TR_BlockFrequency || kind == TR_RecompQueuedFlag
            || kind == TR_CatchBlockCounter || kind == TR_MethodEnterExitHookAddress)) {
        TR::Instruction* instr = (TR::Instruction*)getUpdateLocation();
        TR::Instruction* instr2 = (TR::Instruction*)getLocation2();
...
TR::IteratedExternalRelocation::IteratedExternalRelocation(
    uint8_t* target, TR_ExternalRelocationTargetKind k, flags8_t modifier, TR::CodeGenerator* cg)
...
TR::IteratedExternalRelocation::IteratedExternalRelocation(
    uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind k, flags8_t modifier, TR::CodeGenerator* cg)
    : TR_Link<TR::IteratedExternalRelocation>()
    , _numberOfRelocationSites(0)
    , _targetAddress(target)
...
TR::ExternalRelocation* TR::ExternalRelocation::create(
    uint8_t* codeAddress, uint8_t* targetAddress, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* cg)
{
    return new (cg->trHeapMemory()) TR::ExternalRelocation(codeAddress, targetAddress, kind, cg);
}

TR::ExternalRelocation* TR::ExternalRelocation::create(uint8_t* codeAddress, uint8_t* targetAddress,
    uint8_t* targetAddress2, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* cg)
{
