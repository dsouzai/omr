...
namespace OMR {
Instruction::Instruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node)
    : _opcode(op)
    , _next(0)
    ...
    , _gc()
{
    ...
    if (precedingInstruction != 0) {
        ...
        if (precedingInstruction->getNext()) {
            ...
            // TODO: This will do the wrong thing if nextIndex < prevIndex, but as long as it doesn't affect the flags
            // it won't hurt
