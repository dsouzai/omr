...
OMR::RealRegister::RealRegister(
    TR_RegisterKinds rk, uint16_t w, RegState s, uint16_t a, RegNum n, RegMask m, TR::CodeGenerator* cg)
    : TR::Register(rk, a)
    , _weight(w)
    , _state(s)
    , _realRegFlags(0)
...
TR::RealRegister* OMR::RealRegister::self() { return static_cast<TR::RealRegister*>(this); }
...
TR::RealRegister::RegState OMR::RealRegister::setState(TR::RealRegister::RegState s, bool assignedToDummy)
{
...
bool OMR::RealRegister::setHasBeenAssignedInMethod(bool b)
{
    ...
    if (b)
        _realRegFlags.set(isAssigned);
    else
        _realRegFlags.reset(isAssigned);

    return b;
}

void OMR::RealRegister::block()
{
...
TR::RealRegister* OMR::RealRegister::getRealRegister() { return self(); }

TR::Register* OMR::RealRegister::getRegister() { return NULL; }

// Return the position of the rightmost set bit in the mask.  The first bit
// position is 0.
//
int32_t OMR::RealRegister::getBitPosInMask(TR_RegisterMask mask)
{
    int8_t reg = 0;
    while (mask) {
        if (mask & 1)
