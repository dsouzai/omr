...
void TR_StorageInfo::populateAddress(TR::Node* address)
{
    _address = address;
    if (comp()->cg()->isSupportedAdd(_address) && _address->getFirstChild()->getNumChildren() == 0
    ...
    } else if (comp()->cg()->isSupportedAdd(_address) && comp()->cg()->isSupportedAdd(_address->getFirstChild())
    ...
    if (_address->getOpCode().hasSymbolReference() && _address->getSymbolReference()) {
...
void TR_StorageInfo::populateLoadOrStore(TR::Node* loadOrStore)
{
    if (loadOrStore == NULL)
        return;
...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo* info)
{
    ...
    if (symbolMismatch && _class == TR_DirectMappedAuto && info->_class == TR_DirectMappedAuto) {
        ...
        return TR_NoOverlap; // directly mapped autos from loadaddrs or direct stores cannot interfere with static
                             ...
                             // contain a static address
    }

    // TR_StaticBaseAddress and TR_DirectMappedStatic are unlikely to be found in the same compile unit (different
    // fundamental ways of accessing statics) but if they are both present then they may overlap as the
    // TR_StaticBaseAddress points to the start of the the static area and TR_DirectMappedStatic points to a particular
    // static symbol
    ...
    if (compareRanges) {
        ...
        if (overlapStart >= overlapEnd) // i.e. is this an impossible (>) or trivial range (==) with start >= end? If so
                                        // then there is no overlap
        {
            overlap = TR_NoOverlap;
        } else {
            // If overlapStart < overlapEnd then this is a valid range and there is overlap (overlapStart->overlapEnd is
            // the co-incident)
            if (startOne == startTwo) {
                ...
                if (startOne + std::min(_length, info->_length) - 1 >= unsigned(overlapStart)) {
                    ...
                    //    :   abababab  (i.e. result not the same as if a temp was used in which case answer would be
                    //    ababcd)
