...
public:
     ...
     */
    InstructionLabelRelative16BitRelocation(
...
public:
     ...
     *     at \p cursor + \p offset will be the number of half-words between \p cursor and \p l
     */
    InstructionLabelRelative32BitRelocation(
        TR::Instruction* cursor, int32_t offset, TR::LabelSymbol* l, int32_t divisor);
...
public:
    ...
    ExternalRelocation(
        uint8_t* p, uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* cg)
        : TR::Relocation(p)
        , _targetAddress(target)
...
public:
    ...
    BeforeBinaryEncodingExternalRelocation(
        TR::Instruction* instr, uint8_t* target, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* cg)
