...
struct TR_RegisterPressureState {
    ...
    bool _candidateIsLiveOnEntry; // TODO: Why do we need this?  Why doesn't it work when I check the candidate's
                                  // getBlocksLiveOnEntry?
    bool _pressureRiskFromStart; // True if candidate was live on entry and would not be in a register without GRA
    uint32_t _pressureRiskUntilEnd; // Nonzero if a value has been stored to the candidate, the value would not be in a
                                    // register without GRA, and the candidate is live on an exit we haven't reached yet
    uint32_t _numLiveCandidateLoads; // Number of isCandidateLoad nodes that have been evaluated, but whose refcount is
                                     // not yet zero
    uint32_t _rematNestDepth; // How many nodes between current node and current treetop will be duplicated by
                              // rematerialization
    ...
    TR_RegisterPressureState(TR::RegisterCandidate* candidate, bool candidateIsLiveOnEntry,
        TR_BitVector& alreadyAssignedOnEntry, TR_BitVector& alreadyAssignedOnExit,
        TR_LinkHead<TR::RegisterCandidate>* candidatesAlreadyAssigned, uint32_t gprLimit, uint32_t fprLimit,
        uint32_t vrfLimit, vcount_t visitCountForInit)
        : _currentBlock(NULL)
        ...
        , _currentMemref(0)
    { }
    TR_RegisterPressureState(const TR_RegisterPressureState& other, TR::RegisterCandidate* candidate,
        bool candidateIsLiveOnEntry, vcount_t visitCountForInit)
...
struct TR_RegisterPressureSummary {
    ...
    {
        accumulate(
            other._gprPressure, other._fprPressure, other._vrfPressure, other._linkageConventionMask, other._spillMask);
    }

    void accumulate(TR_RegisterPressureState* state, TR::CodeGenerator* cg, uint32_t gprTemps = 0,
        uint32_t fprTemps = 0, uint32_t vrfTemps = 0);

    void reset(uint8_t gprPressure, uint8_t fprPressure, uint8_t vrfPressure)
    {
        _gprPressure = gprPressure;
        _fprPressure = fprPressure;
        _vrfPressure = vrfPressure;
        _linkageConventionMask = 0;
        _spillMask = 0;
    }

    TR_RegisterPressureSummary() // Initially stale
        : _gprPressure(PRESSURE_LIMIT)
        , _fprPressure(PRESSURE_LIMIT)
        , _vrfPressure(PRESSURE_LIMIT)
        , _linkageConventionMask(0)
        , _spillMask(0)
    { }
...
private:
    void accumulate(uint32_t gprPressure, uint32_t fprPressure = 0, uint32_t vrfPressure = 0,
        uint32_t linkageConventionMask = 0, uint32_t spillMask = 0)
    {
...
void setSpilledRegsForAllPresentLinkages(
...
struct TR_SimulatedNodeState {
    ...
    TR::TreeTop*
        _keepLiveUntil; // Register pressure contribution should not be subtracted until this tree has been simulated
    ...
    bool hasRegister()
    {
        return _liveGPRs + _liveFPRs + _liveVRFs + _liveSSRs + _liveARs >= 1;
    } // Equivalent to "node->getRegister() != NULL" during tree evaluation
    void initialize()
    {
        TR_SimulatedNodeState initialState = { 0 };
        *this = initialState;
    }
...
struct TR_SimulatedMemoryReference {
    TR_SimulatedMemoryReference(TR_Memory* m)
        : _undecrementedNodes(m)
        , _numRegisters(0)
...
void simulateSkippedTreeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary,
    char tagChar = 's'); // 's' for "skipped"
void simulateNodeInitialization(
    TR::Node* node, TR_RegisterPressureState* state); // ok to call if node is already initialized
...
void simulateMemoryReference(TR_SimulatedMemoryReference* memref, TR::Node* node, TR_RegisterPressureState* state,
    TR_RegisterPressureSummary* summary);
void simulationPrePass(
...
{
    return NULL; /* There is really no suitable default */
}
...
virtual void getNumberOfTemporaryRegistersUsedByCall(TR::Node* node, TR_RegisterPressureState* state,
