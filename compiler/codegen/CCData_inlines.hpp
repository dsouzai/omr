...
template<typename T> CCData::key_t CCData::key(const T value)
{
    ...
    static_assert(
        std::has_unique_object_representations<T>::value == true, "T must have unique object representations.");
#endif
...
inline bool CCData::reserve(const size_t sizeBytes, const size_t alignmentBytes, const key_t* const key, index_t& index)
{
