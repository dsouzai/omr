...
TR_BackingStore* OMR::CodeGenerator::allocateSpill(
    int32_t dataSize, bool containsCollectedReference, int32_t* offset, bool reuse)
{
    TR_ASSERT_FATAL(dataSize <= 64, "Spill size must be <= 64 bytes");
    TR_ASSERT_FATAL(
        !containsCollectedReference || (dataSize == TR::Compiler->om.sizeofReferenceAddress()), "assertion failure");
    ...
    if (reuse) {
        ...
        if ((spill && self()->comp()->getOption(TR_TraceRA)
                && !performTransformation(self()->comp(), "O^O SPILL TEMPS: Reuse spill temp %s\n",
        ...
        firstAvailableSpillTemp)
{
    ...
    while (referenceIterator != _liveReferenceList.end()) {
        ...
        if (originalReference->getOpCode().hasSymbolReference() && originalReference->getSymbol()->isSpillTempAuto()) {
            ...
            if (debug("traceSpill"))
                diagnostic(
                    "   Re-use spill temp [%p] for reference [%p] without storing\n", tempSymRef, originalReference);
            ...
            if (!_availableSpillTemps.empty()) {
                ...
                if (debug("traceSpill"))
                    diagnostic(
                        "   Re-use available spill temp [%p] for reference [%p]\n", tempSymRef, originalReference);
            } else {
                int32_t slot;
    ...
    TR_LinkHead<TR::RegisterCandidate>* candidatesAlreadyAssigned)
{
    ...
    if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
        ...
        if (self()->terseSimulateTreeEvaluation()) {
            ...
            if (debug("dumpAllSpillKinds")) {
                for (int32_t i = 0; i < TR_numSpillKinds; i++) {
                    ...
                    self()->getDebug()->print(
                        self()->comp()->getOptions()->getLogFile(), self()->getGlobalRegisters(sk, TR_Private));
                    traceMsg(self()->comp(), "\n");
                }
            }
        }
        ...
        TR_BitVector* spilledRegisters
            = self()->getGlobalRegisters(TR_vmThreadSpill, self()->comp()->getMethodSymbol()->getLinkageConvention());
        if (spilledRegisters) {
                ...
                if (cacheIsInconclusive) {
                    if (canAffordFullSimulation) {
                        TR_RegisterPressureState state(blockEntryState, rc,
                            rc->getBlocksLiveOnEntry().isSet(block->getNumber()), self()->comp()->incVisitCount());
                    ...
                    if (self()->traceSimulateTreeEvaluation()) {
                        traceMsg(self()->comp(), "            using cache for block_%d -- g=%d, f=%d, v=%d",
                            block->getNumber(), cachedSummary->_gprPressure, cachedSummary->_fprPressure,
                                ...
                                if (candidateRegister != -1) {
                                    if (!isUnpreferred) {
                                        if (remainingRegisters.isSet(candidateRegister)) {
                                            ...
                                            if (self()->traceSimulateTreeEvaluation()) {
                                                traceMsg(self()->comp(),
                                                    "            found call parameter index %i, register %d (%s), "
                                                    "weight %d, at %s\n",
                                    ...
                                    } else // candidate is unpreferred
                                    {
                                        if (self()->traceSimulateTreeEvaluation()) {
                                            traceMsg(self()->comp(),
                                                "            unprefer call parameter index %i, register %d (%s), "
                                                "weight %d, at %s\n",
                                                i, candidateRegister,
                                                self()->getDebug()->getGlobalRegisterName(candidateRegister),
        ...
        if (linkageRegister == -1) {
            ...
            //
            unpreferredRegisters = *self()->getGlobalRegisters(
                TR_linkageSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
        ...
        if (remainingRegisters.isEmpty()) {
            if (self()->terseSimulateTreeEvaluation())
                traceMsg(self()->comp(),
                    "         } No good registers for candidate #%d after reg pressure simulation\n",
                    rc->getSymbolReference()->getReferenceNumber());
            else
            ...
            if (self()->terseSimulateTreeEvaluation())
                traceMsg(self()->comp(), "         } Picked register %d (%s) for candidate #%d %s\n", result,
                    self()->getDebug()->getGlobalRegisterName(result), rc->getSymbolReference()->getReferenceNumber(),
                    self()->getDebug()->getName(rc->getSymbolReference()));
    ...
    if (1) {
        ...
        uint8_t regsWithheld = (dtype == TR::Float || dtype == TR::Double) ? fprsWithheldFromPickRegister
            : (self()->comp()->getMethodHotness() == warm)                 ? gprsWithheldFromPickRegisterWhenWarm
                                                                           : gprsWithheldFromPickRegister;

        if (availableRegisters.elementCount() <= regsWithheld) {
            if (debug("tracePickRegister")
                || self()->comp()->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator)) {
        ...
        for (bestLowRegisterNumber = -1; bestLowRegisterNumber == -1 && preferences[0]; preferences++) {
            ...
            case 'N': // Any non-preserved register
                ...
                TR_ASSERT(
                    linkageRegister == -1, "Candidates arriving in linkage regs can't be given other linkage regs");
                //
...
TR::RegisterCandidate* OMR::CodeGenerator::findCoalescenceForRegisterCopy(
    TR::Node* node, TR::RegisterCandidate* rc, bool* isUnpreferred)
{
    ...
    if (node->getOpCode().isStoreDirect() && node->getFirstChild()->getOpCode().isLoadVarDirect()) {
        ...
        TR::RegisterCandidate* loadedCand
            = self()->comp()->getGlobalRegisterCandidates()->find(node->getFirstChild()->getSymbolReference());
        if (loadedCand) {
...
TR_GlobalRegisterNumber OMR::CodeGenerator::findCoalescenceRegisterForParameter(
    TR::Node* callNode, TR::RegisterCandidate* rc, uint32_t childIndex, bool* isUnpreferred)
{
...
TR::RegisterCandidate* OMR::CodeGenerator::findUsedCandidate(
    TR::Node* node, TR::RegisterCandidate* rc, TR_BitVector* visitedNodes)
{
...
void OMR::CodeGenerator::computeSimulatedSpilledRegs(
    TR_BitVector* spilledRegisters, int32_t blockNum, TR::SymbolReference* candidate)
{
...
void OMR::CodeGenerator::computeSpilledRegsForAllPresentLinkages(
    TR_BitVector* spilledRegisters, TR_RegisterPressureSummary& summary)
{
...
void OMR::CodeGenerator::setSpilledRegsForAllPresentLinkages(
    TR_BitVector* spilledRegisters, TR_RegisterPressureSummary& summary, TR_SpillKinds spillKind)
{
...
TR::CodeGenerator::TR_SimulatedNodeState& OMR::CodeGenerator::simulatedNodeState(
    TR::Node* node, TR_RegisterPressureState* state)
{
...
inline void standardNodeSimulationAnnotations(
    OMR::CodeGenerator::TR_RegisterPressureState* state, TR::Compilation* comp)
{
    if (state->_candidate)
        traceMsg(comp, " %c%c",
...
inline void leaveSpaceForRegisterPressureState(
    OMR::CodeGenerator::TR_RegisterPressureState* state, TR::Compilation* comp)
{
...
static void killMostRecentValueIfKeptAliveUntilCurrentTreeTop(
    TR::RegisterCandidate* candidate, OMR::CodeGenerator::TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
...
void OMR::CodeGenerator::simulateBlockEvaluation(
    TR::Block* block, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
void OMR::CodeGenerator::simulationPrePass(
    TR::TreeTop* tt, TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
static bool nodeGotFoldedIntoMemref(
    TR::Node* node, OMR::CodeGenerator::TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
...
void OMR::CodeGenerator::simulateTreeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (nodeState.hasRegister()) {
        ...
        if (self()->nodeWillBeRematerialized(node, state)) {
            ...
            //
            if (isCall) {
                self()->getNumberOfTemporaryRegistersUsedByCall(
                    node, state, numGPRTemporaries, numFPRTemporaries, numVRFTemporaries);
...
void OMR::CodeGenerator::simulateSkippedTreeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary, char tagChar)
{
...
void OMR::CodeGenerator::simulateNodeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (callNodeLinkage) {
        if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
            traceMsg(self()->comp(), " (%s %s linkage)", self()->getDebug()->getName(node),
    ...
    if (evaluateSecondChildFirst) {
        ...
        if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
            traceMsg(self()->comp(), " (%s before %s)", self()->getDebug()->getName(node->getSecondChild()),
            ...
            if (callNodeLinkage) {
                if (childIndex < maxChildrenInRegisters
                    ...
                    if (callNodeLinkage && self()->comp()->getOption(TR_TraceRegisterPressureDetails))
                        traceMsg(self()->comp(), " (%s arg %d in %s, %d left)", self()->getDebug()->getName(node),
                            childIndex,
                    ...
                    if (callNodeLinkage && self()->comp()->getOption(TR_TraceRegisterPressureDetails))
                        traceMsg(self()->comp(), " (%s arg %s in mem)", self()->getDebug()->getName(node),
    ...
    if (nodeGotFoldedIntoMemref(node, state, self())
        ...
        if (callNodeLinkage) {
            ...
            if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
                traceMsg(
                    self()->comp(), " childrenInRegisters=" UINT64_PRINTF_FORMAT_HEX, childrenInRegisters.getValue());
    ...
    uint32_t& numGPRTemporaries, uint32_t& numFPRTemporaries, uint32_t& numVRFTemporaries)
{
...

    {
        ...
        bv = *self()->getGlobalRegisters(
            TR_volatileSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
...
void OMR::CodeGenerator::TR_SimulatedMemoryReference::add(
    TR::Node* node, TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
...
void OMR::CodeGenerator::TR_SimulatedMemoryReference::simulateDecNodeReferenceCounts(
    TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::accumulate(
    TR_RegisterPressureState* state, TR::CodeGenerator* cg, uint32_t gprTemps, uint32_t fprTemps, uint32_t vrfTemps)
{
