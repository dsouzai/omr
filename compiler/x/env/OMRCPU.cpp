...
TR::CPU OMR::X86::CPU::detect(OMRPortLibrary * const omrPortLib)
{
    if (omrPortLib == NULL)
        return TR::CPU();
...
TR::CPU OMR::X86::CPU::customize(OMRProcessorDesc processorDescription)
{
    ...
    const uint32_t enabledFeatures[] = { OMR_FEATURE_X86_FPU, OMR_FEATURE_X86_CX8, OMR_FEATURE_X86_CMOV,
        ...
        OMR_FEATURE_X86_SSE4_2, OMR_FEATURE_X86_POPCNT, OMR_FEATURE_X86_AESNI, OMR_FEATURE_X86_OSXSAVE,
        OMR_FEATURE_X86_AVX, OMR_FEATURE_X86_AVX2, OMR_FEATURE_X86_FMA, OMR_FEATURE_X86_HLE, OMR_FEATURE_X86_RTM,
        OMR_FEATURE_X86_AVX512F, OMR_FEATURE_X86_AVX512VL, OMR_FEATURE_X86_AVX512BW, OMR_FEATURE_X86_AVX512DQ,
        OMR_FEATURE_X86_AVX512CD, OMR_FEATURE_X86_AVX512_VBMI2, OMR_FEATURE_X86_AVX512_VPOPCNTDQ,
    ...
    for (size_t i = 0; i < sizeof(enabledFeatures) / sizeof(uint32_t); i++) {
        TR_ASSERT_FATAL(enabledFeatures[i] < OMRPORT_SYSINFO_FEATURES_SIZE * sizeof(uint32_t) * 8,
            "Illegal cpu feature mask");
...
void OMR::X86::CPU::initializeTargetProcessorInfo(bool force)
{
    OMR::X86::CodeGenerator::initializeX86TargetProcessorInfo(force);
}

TR_X86CPUIDBuffer *OMR::X86::CPU::queryX86TargetCPUID()
{
...
const char *OMR::X86::CPU::getX86ProcessorVendorId()
{
    static char buf[13];
...
uint32_t OMR::X86::CPU::getX86ProcessorSignature() { return self()->queryX86TargetCPUID()->_processorSignature; }

uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags()
{
...
uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags2()
{
    if (TR::Compiler->omrPortLib == NULL)
...
uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags8()
{
    if (TR::Compiler->omrPortLib == NULL)
...
uint32_t OMR::X86::CPU::getX86ProcessorFeatureFlags10()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::getSupportsHardwareSQRT() { return true; }

bool OMR::X86::CPU::hasPopulationCountInstruction()
{
...
bool OMR::X86::CPU::supportsTransactionalMemoryInstructions() { return self()->supportsFeature(OMR_FEATURE_X86_RTM); }

bool OMR::X86::CPU::getSupportsHardware32bitCompress()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::getSupportsHardware64bitCompress()
{
    if (self()->isI386())
        return false;
...
bool OMR::X86::CPU::getSupportsHardware32bitExpand()
{
    if (TR::Compiler->omrPortLib == NULL)
        return TR::CodeGenerator::getX86ProcessorInfo().supportsBMI2();
...
bool OMR::X86::CPU::getSupportsHardware64bitExpand()
{
    if (self()->isI386())
        return false;
...
bool OMR::X86::CPU::isGenuineIntel()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::isAuthenticAMD()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::supportsLFence()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::supportsSFence()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::prefersMultiByteNOP()
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::is(OMRProcessorArchitecture p)
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::X86::CPU::isFeatureDisabledByOption(uint32_t feature)
{
...
bool OMR::X86::CPU::supportsFeature(uint32_t feature)
{
...
bool OMR::X86::CPU::is_test(OMRProcessorArchitecture p)
{
    switch (p) {
...
bool OMR::X86::CPU::is_old_api(OMRProcessorArchitecture p)
{
    bool ans = false;
    switch (p) {
        case OMR_PROCESSOR_X86_INTEL_WESTMERE:
...
bool OMR::X86::CPU::supports_feature_old_api(uint32_t feature)
{
...
const char *OMR::X86::CPU::getProcessorName()
{
