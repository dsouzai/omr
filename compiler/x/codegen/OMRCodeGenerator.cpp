...
static_assert(MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE % 8 == 0,
    "MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE should be multiple of 8");
...
// Hack markers
#define CANT_REMATERIALIZE_ADDRESSES(cg) \
    (cg->comp()->target().is64Bit()) // AMD64 produces a memref with an unassigned addressRegister
...
void TR_X86ProcessorInfo::initialize(bool force)
{
    ...
    if (isGenuineIntel()) {
        ...
        case 0x05:
            _processorDescription |= TR_ProcessorIntelPentium;
            break;
        case 0x06: {
            uint32_t extended_model
                = getCPUModel(_processorSignature) + (getCPUExtendedModel(_processorSignature) << 4);
            uint32_t processorStepping = getCPUStepping(_processorSignature);
            switch (extended_model) {
            case 0xcf:
                _processorDescription |= TR_ProcessorIntelEmeraldRapids;
                break;
            case 0x8f:
                _processorDescription |= TR_ProcessorIntelSapphireRapids;
                break;
            case 0x6a: // IceLake_X
            case 0x6c: // IceLake_D
            case 0x7d: // IceLake
            case 0x7e: // IceLake_L
                _processorDescription |= TR_ProcessorIntelIceLake;
                break;
            case 0x55: // Skylake_X
            ...
            case 0x5e: // Skylake
                ...
                break;
            case 0x4f:
                _processorDescription |= TR_ProcessorIntelBroadwell;
                break;
            case 0x3f:
            case 0x3c:
                _processorDescription |= TR_ProcessorIntelHaswell;
                break;
            case 0x3e:
            case 0x3a:
                _processorDescription |= TR_ProcessorIntelIvyBridge;
                break;
            case 0x2a:
            case 0x2d: // SandyBridge EP
                _processorDescription |= TR_ProcessorIntelSandyBridge;
                break;
            case 0x2c: // WestmereEP
            case 0x2f: // WestmereEX
                _processorDescription |= TR_ProcessorIntelWestmere;
                break;
            case 0x1a: // Nehalem
                _processorDescription |= TR_ProcessorIntelNehalem;
                break;
            case 0x17: // Harpertown
            case 0x0f: // Woodcrest/Clovertown
                _processorDescription |= TR_ProcessorIntelCore2;
                break;
            default:
                _processorDescription |= TR_ProcessorIntelP6;
                break;
            }
            break;
        }
        case 0x0f:
            _processorDescription |= TR_ProcessorIntelPentium4;
            break;
        default:
            _processorDescription |= TR_ProcessorUnknown;
            break;
        }
        ...
        case 0x05:
            ...
            break;
        case 0x06:
            _processorDescription |= TR_ProcessorAMDAthlonDuron;
            break;
        case 0x0f:
...
void OMR::X86::CodeGenerator::initializeX86(TR::Compilation* comp)
{
    ...
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.isGenuineIntel() == getX86ProcessorInfo().isGenuineIntel(),
        "isGenuineIntel() failed\n");
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.isAuthenticAMD() == getX86ProcessorInfo().isAuthenticAMD(),
        "isAuthenticAMD() failed\n");
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.prefersMultiByteNOP() == getX86ProcessorInfo().prefersMultiByteNOP(),
        "prefersMultiByteNOP() failed\n");
...
#if !defined(J9HAMMER)
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE2) == getX86ProcessorInfo().supportsSSE2(),
        "supportsSSE2() failed\n");
    ...
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.supportsFeature(OMR_FEATURE_X86_RTM) == getX86ProcessorInfo().supportsTM(),
        "supportsTM() failed\n");

    if (comp->target().cpu.supportsFeature(OMR_FEATURE_X86_RTM) && !comp->getOption(TR_DisableTM)) {
        /**
         * Due to many verions of Haswell and a small number of Broadwell have defects for TM and then disabled by
         * Intel, we will return false for any versions before Broadwell.
         *
        ...
        TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation()
                ...
                || comp->target().cpu.is(OMR_PROCESSOR_X86_INTEL_HASWELL) == getX86ProcessorInfo().isIntelHaswell(),
            "isIntelHaswell() failed\n");
   ...
   if (comp->target().is64Bit())
    ...
    _unlatchedRegisterList = (TR::RealRegister**)self()->trMemory()->allocateHeapMemory(
    ...
    if (!TR::Compiler->om.canGenerateArraylets()) {
        ...
        if (!disableX86TRTO) {
            TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation()
                    || comp->compilePortableCode()
                    || comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1)
                        == self()->getX86ProcessorInfo().supportsSSE4_1(),
                "supportsSSE4_1() failed\n");
        ...
        if (!disableX86TROT) {
            ...
            TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation()
                    ...
                    || comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE2)
                        == self()->getX86ProcessorInfo().supportsSSE2(),
                "supportsSSE4_1() failed\n");
            if (comp->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1)) {
    ...
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
        ...
        "isAuthenticAMD() failed\n");
    TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation() || comp->compilePortableCode()
            || comp->target().cpu.is(OMR_PROCESSOR_X86_AMD_FAMILY15H) == getX86ProcessorInfo().isAMD15h(),
        "isAMD15h() failed\n");
    int32_t boundary;
    if (comp->target().cpu.isGenuineIntel()
        || (comp->target().cpu.isAuthenticAMD() && comp->target().cpu.is(OMR_PROCESSOR_X86_AMD_FAMILY15H)))
        boundary = 32;
    else {
        // TR_AuthenticAMD
        // TR_UnknownVendor
    ...
    if (!comp->getOption(TR_DisableRegisterPressureSimulation)) {
        ...
        for (TR_GlobalRegisterNumber grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
            ...
            if (self()->getProperties().isIntegerArgumentRegister(reg)
                || self()->getProperties().isFloatArgumentRegister(reg))
    ...
    if (comp->getOption(TR_TraceRA)) {
        self()->setGPRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(
            self()->machine(), TR::RealRegister::FirstGPR, TR::RealRegister::LastAssignableGPR));
        self()->setFPRegisterIterator(new (self()->trHeapMemory())
                TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstXMMR, TR::RealRegister::LastXMMR));
        self()->setVMRegisterIterator(new (self()->trHeapMemory())
                TR::RegisterIterator(self()->machine(), TR::RealRegister::k1, TR::RealRegister::k7));
    }
...
void OMR::X86::CodeGenerator::initialize()
{
...
TR::Linkage* OMR::X86::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
...
void OMR::X86::CodeGenerator::beginInstructionSelection()
{
    TR::Compilation* comp = self()->comp();
    ...
    if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
        generateMemInstruction(TR::InstOpCode::LDCWMem, startNode,
            generateX86MemoryReference(
                self()->findOrCreate2ByteConstant(startNode, SINGLE_PRECISION_ROUND_TO_NEAREST), self()),
            self());
    }
}

void OMR::X86::CodeGenerator::endInstructionSelection()
{
    TR::Compilation* comp = self()->comp();
    ...
    if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
        ...
        generateMemInstruction(self()->getLastCatchAppendInstruction(), TR::InstOpCode::LDCWMem,
            generateX86MemoryReference(
                self()->findOrCreate2ByteConstant(
                    self()->getLastCatchAppendInstruction()->getNode(), DOUBLE_PRECISION_ROUND_TO_NEAREST),
                self()),
            self());
    }
}

TR_X86ProcessorInfo& OMR::X86::CodeGenerator::getX86ProcessorInfo()
{
 ...
 */
static bool willNotInlineCompareAndSwapNative(TR::Node* node, int8_t size, TR::Compilation* comp)
{
#ifdef J9_PROJECT_SPECIFIC
...
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node* node, TR::Compilation* comp)
{
    ...
    case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
        return willNotInlineCompareAndSwapNative(
            node, (comp->target().is64Bit() && !comp->useCompressedPointers()) ? 8 : 4, comp);
...
int32_t OMR::X86::CodeGenerator::getMaximumNumbersOfAssignableVRs() { return INT_MAX; }
...
bool OMR::X86::CodeGenerator::canNullChkBeImplicit(TR::Node* node) { return self()->canNullChkBeImplicit(node, true); }

void OMR::X86::CodeGenerator::clobberLiveDiscardableRegisters(TR::Instruction* instr, TR::MemoryReference* mr)
{
        ...
        while (iterator != self()->getLiveDiscardableRegisters().end()) {
            ...
            if (registerCursor->getRematerializationInfo()->isRematerializableFromMemory()) {
                ...
                if ((rmSymRef->getSymbol() == symbol)
                    ...
                    if (debug("dumpRemat")) {
                        diagnostic("---> Clobbering %s discardable register %s at instruction %p in %s\n",
                            ...
                            self()->getDebug()->getName(registerCursor), instr, self()->comp()->signature());
                    }
                }
                ...
                //
                    ...
                    if ((IGNodeRematSym != NULL) && (IGNodeSym->getColour() == IGNodeRematSym->getColour())) {
                        if (!clob) {
                            clob = new (self()->trHeapMemory()) TR::ClobberingInstruction(instr, self()->trMemory());
                            self()->addClobberingInstruction(clob);
                        }
                        ...
                        if (debug("dumpRemat")) {
                            diagnostic("---> Clobbering %s discardable register %s at instruction %p because of shared "
                                       "slot in %s\n",
                                self()->getDebug()->toString(registerCursor->getRematerializationInfo()),
                                self()->getDebug()->getName(registerCursor), instr, self()->comp()->signature());
                        }
...
//
void OMR::X86::CodeGenerator::clobberLiveDependentDiscardableRegisters(
    TR::ClobberingInstruction* clob, TR::Register* baseReg)
{
    ...
    while (!worklist.isEmpty()) {
        ...
        for (auto iterator = self()->getLiveDiscardableRegisters().begin();
             iterator != self()->getLiveDiscardableRegisters().end();) {
            ...
            if (info->isIndirect() && info->getBaseRegister() == baseReg) {
                ...
                if (debug("dumpRemat")) {
                    diagnostic("---> Clobbering %s discardable register %s at instruction %p in %s\n",
                        self()->getDebug()->toString(info), self()->getDebug()->getName(candidate),
                        clob->getInstruction(), self()->comp()->signature());
                }
            } else
                ++iterator;
        }
...
void OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(TR::Register* baseReg)
{
    ...
    if (debug("dumpRemat"))
        diagnostic(
            "---> Re-activating rematerializable registers dependent on %s: ", self()->getDebug()->getName(baseReg));

    while (!worklist.isEmpty()) {
        baseReg = worklist.pop();

        for (auto iterator = self()->getDependentDiscardableRegisters().begin();
             iterator != self()->getDependentDiscardableRegisters().end(); ++iterator) {
            if ((*iterator)->isDiscardable() && (*iterator)->getRematerializationInfo()->getBaseRegister() == baseReg) {
                if (debug("dumpRemat"))
...
void OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(TR::Register* baseReg)
{
    ...
    if (debug("dumpRemat"))
        diagnostic(
            "---> Deactivating rematerialisable registers dependent on %s: ", self()->getDebug()->getName(baseReg));
...
bool OMR::X86::CodeGenerator::supportsConstantRematerialization()
{
    static bool b = CAN_REMATERIALIZE("constant");
    return b;
}
bool OMR::X86::CodeGenerator::supportsLocalMemoryRematerialization()
{
    static bool b = CAN_REMATERIALIZE("local");
    return b;
}
bool OMR::X86::CodeGenerator::supportsStaticMemoryRematerialization()
{
    static bool b = CAN_REMATERIALIZE("static");
    return !CANT_REMATERIALIZE_ADDRESSES(self()) && b;
}
bool OMR::X86::CodeGenerator::supportsXMMRRematerialization()
{
    static bool b = CAN_REMATERIALIZE("xmmr");
    return b;
}
bool OMR::X86::CodeGenerator::supportsIndirectMemoryRematerialization()
{
    static bool b = ALLOWED_TO_REMATERIALIZE("indirect");
    return !CANT_REMATERIALIZE_ADDRESSES(self()) && b;
}
bool OMR::X86::CodeGenerator::supportsAddressRematerialization()
{
    static bool b = ALLOWED_TO_REMATERIALIZE("address");
    return !CANT_REMATERIALIZE_ADDRESSES(self()) && b;
}
...
bool OMR::X86::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::CPU* cpu, TR::ILOpCode opcode)
{
     ...
     * The cases that return false are placeholders that should be updated as support for more vector evaluators is
     * added.
     */
...
{
    TR_ASSERT_FATAL(self()->comp()->compileRelocatableCode() || self()->comp()->isOutOfProcessCompilation()
            || self()->comp()->compilePortableCode()
            || self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1)
                == TR::CodeGenerator::getX86ProcessorInfo().supportsSSE4_1(),
        "supportsSSE4_1()");
    return self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1)
        && !self()->comp()->getOption(TR_DisableSIMDUTF16LEEncoder);
}

bool OMR::X86::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
    TR_ASSERT_FATAL(self()->comp()->compileRelocatableCode() || self()->comp()->isOutOfProcessCompilation()
            || self()->comp()->compilePortableCode()
            || self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_SSE4_1)
...
bool OMR::X86::CodeGenerator::getSupportsBitPermute() { return true; }
...
bool OMR::X86::CodeGenerator::isILOpCodeSupported(TR::ILOpCodes o)
{
    switch (o) {
    case TR::a2i:
        return false;
...
TR::RealRegister* OMR::X86::CodeGenerator::getMethodMetaDataRegister()
{
    return toRealRegister(self()->getVMThreadRegister());
}

bool OMR::X86::CodeGenerator::canTransformUnsafeCopyToArrayCopy()
{
...
void OMR::X86::CodeGenerator::saveBetterSpillPlacements(TR::Instruction* branchInstruction)
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
        ...
        if (realReg->getAssignedRegister() == NULL) {
            ...
            freeRealRegisters
                |= TR::RealRegister::getRealRegisterMask(realReg->getKind(), realReg->getRegisterNumber());
        }
    }
    ...
    //
    for (auto regElement = _spilledIntRegisters.begin();
         regElement != _spilledIntRegisters.end() && numFreeRealRegisters; ++regElement) {
        ...
        //
        if ((*regElement)->containsCollectedReference() || (*regElement)->containsInternalPointer()
            || (*regElement)->hasBetterSpillPlacement())
            continue;

        self()->traceRegisterAssignment(
            "Saved better spill placement for %R, mask = %x.", *regElement, freeRealRegisters);
...
void OMR::X86::CodeGenerator::removeBetterSpillPlacementCandidate(TR::RealRegister* realReg)
{
    ...
    uint32_t mask = ~TR::RealRegister::getRealRegisterMask(
        realReg->getKind(), realReg->getRegisterNumber()); // TODO:AMD64: Use the proper mask value
...
TR::Instruction* OMR::X86::CodeGenerator::findBetterSpillPlacement(TR::Register* virtReg, int32_t realRegNum)
{
    ...
    if (info
        && (info->_freeRealRegs
            & TR::RealRegister::getRealRegisterMask(virtReg->getKind(), (TR::RealRegister::RegNum)realRegNum))) {
        placement = info->_branchInstruction;
        self()->traceRegisterAssignment(
            "Successful better spill placement for %R at [" POINTER_PRINTF_FORMAT "].", virtReg, placement);
    } else {
        placement = NULL;
...
void OMR::X86::CodeGenerator::performNonLinearRegisterAssignmentAtBranch(
    TR::X86LabelInstruction* branchInstruction, TR_RegisterKinds kindsToBeAssigned)
{
    ...
    TR_OutlinedInstructions* oi = self()->findOutlinedInstructionsFromLabel(branchInstruction->getLabelSymbol());
    ...
    if (deps) {
        TR::Instruction* ins = generateLabelInstruction(
            oi->getFirstInstruction(), TR::InstOpCode::label, generateLabelSymbol(self()), deps, self());
    ...
    oi->assignRegistersOnOutlinedPath(
        kindsToBeAssigned, generateVFPSaveInstruction(branchInstruction->getPrev(), self()));
...
void OMR::X86::CodeGenerator::prepareForNonLinearRegisterAssignmentAtMerge(TR::X86LabelInstruction* mergeInstruction)
{
    TR::Machine* xm = self()->machine();
    ...
    TR_OutlinedInstructions* oi = self()->findOutlinedInstructionsFromMergeLabel(mergeInstruction->getLabelSymbol());
...
void OMR::X86::CodeGenerator::processClobberingInstructions(
    TR::ClobberingInstruction* clobInstructionCursor, TR::Instruction* instructionCursor)
{
    ...
    //
    while (clobInstructionCursor && (clobInstructionCursor->getInstruction() == instructionCursor)
        && self()->enableRematerialisation()) {
            ...
            if (!info->isIndirect() || info->getBaseRegister()->getAssignedRegister()) {
                ...
                if (debug("dumpRemat")) {
                    diagnostic("---> Activating %s discardable register %s at instruction %p in %s\n",
                        self()->getDebug()->toString(info), self()->getDebug()->getName(*regIterator),
                        instructionCursor, self()->comp()->signature());
                }
            }
...
TR::Instruction* OMR::X86::CodeGenerator::generateInterpreterEntryInstruction(TR::Instruction* procEntryInstruction)
{
    ...
    if (self()->comp()->target().is64Bit()) {
        if (self()->comp()->getMethodSymbol()->getLinkageConvention() != TR_System)
            interpreterEntryInstruction
                = self()->getLinkage()->copyStackParametersToLinkageRegisters(procEntryInstruction);
        else
...
void OMR::X86::CodeGenerator::doBackwardsRegisterAssignment(
    TR_RegisterKinds kindsToAssign, TR::Instruction* instructionCursor, TR::Instruction* appendInstruction)
{
    TR::Compilation* comp = self()->comp();
    ...
    if (self()->getUseNonLinearRegisterAssigner()) {
        if (!self()->getSpilledRegisterList()) {
            self()->setSpilledRegisterList(new (self()->trHeapMemory())
                    TR::list<TR::Register*>(getTypedAllocator<TR::Register*>(comp->allocator())));
        }
    }
...
void OMR::X86::CodeGenerator::addItemsToRSSReport(uint8_t* coldCode)
{
        ...
        if (insideColdCode) {
            ...
            if (!coldBlockCountersList)
                coldBlockCountersList
                    = new (comp->trPersistentMemory()) TR_PersistentList<TR::DebugCounterAggregation>();
    ...
    if (self()->getEstimatedColdLength() && OMR::RSSReport::instance()) {
        ...
        if (rssRegion) {
            ...
            if (blocksInsideColdCodeSize != actualColdLength) {
                if (comp->getOption(TR_TraceCG)) {
                    traceMsg(comp,
                        "RSS: blocksInsideColdCodeSize=%zu actualColdLength=%zu coldCode=%p coldCodeEnd=%p\n",
            ...
            if (overEstimate > 0) {
                rssItem = new (comp->trPersistentMemory())
                    OMR::RSSItem(OMR::RSSItem::overEstimate, getBinaryBufferCursor(), overEstimate, NULL);
                rssRegion->addRSSItem(rssItem, codeCache->getReservingCompThreadID(), methodName);
            }

            rssItem = new (comp->trPersistentMemory())
                OMR::RSSItem(OMR::RSSItem::coldBlocks, coldCode, actualColdLength, coldBlockCountersList);
...
#define OVER_ESTIMATION 4
    ...
    if (warmEstimate) {
        ...
        self()->setEstimatedColdLength(
            estimate - warmEstimate - MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE + OVER_ESTIMATION);
    } else {
    ...
    uint8_t* temp
        = self()->allocateCodeMemory(self()->getEstimatedWarmLength(), self()->getEstimatedColdLength(), &coldCode);
    ...
    if (self()->comp()->target().is64Bit() && self()->hasCodeCacheSwitched() && self()->getPicSlotCount() != 0) {
        ...
        TR_ASSERT(numTrampolinesToReserve >= 0,
            "Discrepancy with number of IPIC trampolines to reserve getPicSlotCount()=%d "
            "getNumReservedIPICTrampolines()=%d",
    ...
    self()->comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()->getSymbol()->getStaticSymbol()->setStaticAddress(
    ...
    while (cursorInstruction) {
        ...
        if (self()->comp()->target().is64Bit() && (cursorInstruction->getOpCodeValue() == TR::InstOpCode::proc)) {
        ...
        if (cursorInstruction->isLastWarmInstruction()) {
            ...
            if (self()->comp()->getOption(TR_TraceCG)) {
                traceMsg(self()->comp(), "%s warmCodeEnd = %p, lastWarmInstruction = %p coldCodeStart = %p\n",
                    SPLIT_WARM_COLD_STRING, self()->getWarmCodeEnd(), cursorInstruction, coldCode);
            }
            ...
            //
            self()->setAccumulatedInstructionLengthError(static_cast<uint32_t>(self()->getBinaryBufferStart()
                + self()->getEstimatedWarmLength() + MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE - coldCode));
        }
    ...
    //
    for (auto oiIterator = self()->getOutlinedInstructionsList().begin();
         oiIterator != self()->getOutlinedInstructionsList().end(); ++oiIterator) {
        uint32_t startOffset
            = static_cast<uint32_t>((*oiIterator)->getFirstInstruction()->getBinaryEncoding() - self()->getCodeStart());
        uint32_t endOffset = static_cast<uint32_t>(
            (*oiIterator)->getAppendInstruction()->getBinaryEncoding() - self()->getCodeStart());
    ...
    if (self()->comp()->getOption(TR_EnableGCRPatching) && self()->comp()->getRecompilationInfo()
        && self()->comp()->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR()) {
        void* addrToPatch = self()
                                ->comp()
                                ->getSymRefTab()
        ...
        if (!addrToPatch) {
            ...
            self()->comp()->failCompilation<TR::GCRPatchFailure>(
                "Must have updated gcrPatchPointSymbol with the correct address by now");
        }
    }
#endif
...
TR::Register* OMR::X86::CodeGenerator::gprClobberEvaluate(TR::Node* node, TR::InstOpCode::Mnemonic movRegRegOpCode)
{
    ...
    if (self()->comp()->getOption(TR_TraceCG) && sourceRegister->needsLazyClobbering())
        traceMsg(self()->comp(), "LAZY CLOBBERING: node %s register %s refcount=%d canClobber=%s\n",
            self()->getDebug()->getName(node), self()->getDebug()->getName(sourceRegister), node->getReferenceCount(),
        ...
        if (sourceRegister->containsInternalPointer()) {
            ...
            if (self()->comp()->getOption(TR_TraceCG))
                traceMsg(self()->comp(),
                    ...
                    self()->getDebug()->getName(targetRegister), pinningArrayPointer);
...
TR::Register* OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node* node)
{
    ...
    TR_ASSERT(
        !node->getOpCode().isRef() || self()->comp()->target().is32Bit() || self()->comp()->useCompressedPointers(),
...
{
    TR_ASSERT(
        !node->getOpCode().is4Byte() && !node->getOpCode().is8Byte(), "Ints/Longs must use int/longClobberEvaluate");
    TR_ASSERT(!(node->getOpCode().isRef() && self()->comp()->target().is64Bit()),
...
bool OMR::X86::CodeGenerator::allowGlobalRegisterAcrossBranch(TR::RegisterCandidate* rc, TR::Node* node)
{
    ...
    //
    if (node->getOpCode().isSwitch() && (rc->getDataType() == TR::Float || rc->getDataType() == TR::Double)) {
        for (int32_t i = node->getCaseIndexUpperBound() - 1; i > 0; --i)
            if (!rc->getBlocksLiveOnEntry().get(
...
int32_t OMR::X86::CodeGenerator::branchDisplacementToHelperOrTrampoline(
    uint8_t* branchInstructionAddress, TR::SymbolReference* helper)
{
    intptr_t helperAddress = (intptr_t)helper->getMethodAddress();
    uint8_t* nextInstructionAddress
        = branchInstructionAddress + 5; // 5 == length of wide displacement direct call or jump instruction
    ...
    if (self()->directCallRequiresTrampoline(helperAddress, (intptr_t)branchInstructionAddress)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            helper->getReferenceNumber(), (void*)(nextInstructionAddress - 4));

        TR_ASSERT_FATAL(
            self()->comp()->target().cpu.isTargetWithinRIPRange(helperAddress, (intptr_t)nextInstructionAddress),
...
bool OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(
    TR::Node* node, TR::Node* parent, TR_RegisterPressureState* state)
{
    TR_SimulatedNodeState& nodeState = self()->simulatedNodeState(node, state);
    bool result = (node->getOpCode().isLoadVar() || node->getOpCode().isArrayLength())
        && !self()->isCandidateLoad(node, state) && !nodeState.hasRegister();
    ...
    if (self()->traceSimulateTreeEvaluation() && result)
        traceMsg(self()->comp(), " %s foldable into %s", self()->getDebug()->getName(node),
            self()->getDebug()->getName(parent));
    return result;
}
...
uint8_t OMR::X86::CodeGenerator::nodeResultGPRCount(TR::Node* node, TR_RegisterPressureState* state)
{
    ...
    if (node->getOpCode().isLoadConst()
        ...
        && !(self()->simulatedNodeState(node, state)._keepLiveUntil
                != NULL // Check if parent will become a RegStore that keeps this node live
...
void OMR::X86::CodeGenerator::simulateNodeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if ((node->getNumChildren() == 2 || opCode.isBooleanCompare()) && !opCode.isStore() && !opCode.isIndirect()
        && state->_memrefNestDepth == 0) {
        ...
        //
        if (tryFoldingLeft && (clobbersNothing || right->getFutureUseCount() == 1)
            && self()->nodeIsFoldableMemOperand(left, node, state))
            toFold = 0;
        else if ((clobbersNothing || left->getFutureUseCount() == 1)
            && self()->nodeIsFoldableMemOperand(right, node, state))
            toFold = 1;
    }
    ...
    if ((opCode.isMul() || opCode.isDiv() || opCode.isRem()) && !(opCode.isFloat() || opCode.isDouble())) {
        ...
        bool usesMul = true;
        if (secondChild->getOpCode().isLoadConst() && (self()->comp()->target().is64Bit() || !nodeType.isInt64())
    ...
    } else if ((opCode.isLeftShift() || opCode.isRightShift()) && !node->getSecondChild()->getOpCode().isLoadConst()
...
uint8_t OMR::X86::CodeGenerator::old32BitPaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                                        [PADDING_TABLE_MAX_ENCODING_LENGTH]
    = {
          { 0x90 }, // 1: xchg eax,eax
...
uint8_t OMR::X86::CodeGenerator::intelMultiBytePaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                                              [PADDING_TABLE_MAX_ENCODING_LENGTH]
    = {
          { 0x90 },
          { 0x66, 0x90 },
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable = TR_X86PaddingTable(
    7, (uint8_t***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding, TR_X86PaddingTable::registerMatters, 0x8b, 0x20);
...
{
    TR_ASSERT(paddingTable->_biggestEncoding <= PADDING_TABLE_MAX_ENCODING_LENGTH,
        "Padding table must have _biggestEncoding <= PADDING_TABLE_MAX_ENCODING_LENGTH (%d <= %d)",
        paddingTable->_biggestEncoding, PADDING_TABLE_MAX_ENCODING_LENGTH);
    TR_ASSERT(length <= paddingTable->_biggestEncoding, "Padding length %d cannot exceed %d", length,
        paddingTable->_biggestEncoding);
    return (uint8_t*)((uintptr_t)(paddingTable->_encodings) + (length - 1) * PADDING_TABLE_MAX_ENCODING_LENGTH);
}

uint8_t* OMR::X86::CodeGenerator::generatePadding(
    uint8_t* cursor, intptr_t length, TR::Instruction* neighborhood, TR_PaddingProperties properties, bool recursive)
{
    ...
    if (!recursive && self()->comp()->getOptions()->enableDebugCounters()) {
        if (neighborhood) {
            ...
            if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
                && (guardNode->isTheVirtualGuardForAGuardedInlinedCall() || guardNode->isHCRGuard()
                    || guardNode->isProfiledGuard() || guardNode->isMethodEnterExitGuard())) {
                ...
                case TR_ProfiledGuard:
                    guardKind = "ProfiledGuard";
                    break;
                case TR_InterfaceGuard:
                    guardKind = "InterfaceGuard";
                    break;
                case TR_AbstractGuard:
                    guardKind = "AbstractGuard";
                    break;
                case TR_HierarchyGuard:
                    guardKind = "HierarchyGuard";
                    break;
                case TR_NonoverriddenGuard:
                    guardKind = "NonoverriddenGuard";
                    break;
                case TR_SideEffectGuard:
                    guardKind = "SideEffectGuard";
                    break;
                case TR_DummyGuard:
                    guardKind = "DummyGuard";
                    break;
                case TR_HCRGuard:
                    guardKind = "HCRGuard";
                    break;
                case TR_MutableCallSiteTargetGuard:
                    guardKind = "MutableCallSiteTargetGuard";
                    break;
                case TR_MethodEnterExitGuard:
                    guardKind = "MethodEnterExitGuard";
                    break;
                case TR_DirectMethodGuard:
                    guardKind = "DirectMethodGuard";
                    break;
                case TR_InnerGuard:
                    guardKind = "InnerGuard";
                    break;
                case TR_ArrayStoreCheckGuard:
                    guardKind = "ArrayStoreCheckGuard";
                    break;
                default:
                    guardKind = "Unknown";
                    break;
                ...
                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                    TR::DebugCounter::debugCounterName(
                        self()->comp(), "nopCount/%d/%s/%s", blockFrequency, neighborhood->description(), guardKind));
                if (length > 0) {
                    TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s/%s", blockFrequency,
                            neighborhood->description(), guardKind));
                    ...
                    for (TR::Instruction* ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
                        if (ninst->isVirtualGuardNOPInstruction()) {
                            ...
                            TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                TR::DebugCounter::debugCounterName(
                                    self()->comp(), "vgnopNoPatchReason/%d/vgnop", blockFrequency));
                            break;
                        }
                        if (self()->comp()->isPICSite(ninst)) {
                            TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                TR::DebugCounter::debugCounterName(
                                    self()->comp(), "vgnopNoPatchReason/%d/staticPIC", blockFrequency));
                            break;
                        }
                        if (ninst->isPatchBarrier(self())) {
                            if (ninst->getOpCodeValue() != TR::InstOpCode::label)
                                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                    TR::DebugCounter::debugCounterName(
                                        self()->comp(), "vgnopNoPatchReason/%d/patchBarrier", blockFrequency));
                            else
                                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                    TR::DebugCounter::debugCounterName(
                                        self()->comp(), "vgnopNoPatchReason/%d/controlFlowMerge", blockFrequency));
                            break;
                        }
                        if (ninst->getEstimatedBinaryLength() > 0) {
                            TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/%s_%s",
                                    blockFrequency, (guardNode->isHCRGuard() ? "hcr" : "nohcr"), ninst->description()));
                        }
                    }
                }
            } else {
                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                    TR::DebugCounter::debugCounterName(
                        self()->comp(), "nopCount/%d/%s", blockFrequency, neighborhood->description()));
                if (length > 0)
                    TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(
                            self()->comp(), "nopInst/%d/%s", blockFrequency, neighborhood->description()));
            }
        } else {
...
intptr_t OMR::X86::CodeGenerator::alignment(void* cursor, intptr_t boundary, intptr_t margin)
{
    return self()->alignment((intptr_t)cursor, boundary, margin);
}

bool OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(
    void* cursor, intptr_t length, intptr_t boundary, intptr_t margin)
{
...
bool TR_X86ScratchRegisterManager::reclaimAddressRegister(TR::MemoryReference* mr)
{
...
TR::Instruction* OMR::X86::CodeGenerator::generateDebugCounterBump(
    TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR::RegisterDependencyConditions* cond)
{
    if (delta == 1)
        return generateMemInstruction(cursor, TR::InstOpCode::INC4Mem,
            generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()), self()), self());
    else
        return generateMemImmInstruction(cursor,
            (-128 <= delta && delta <= 127) ? TR::InstOpCode::ADD4MemImms : TR::InstOpCode::ADD4MemImm4,
            generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()), self()), delta, self());
}
    ...
    TR::DebugCounterBase* counter, TR::Register* deltaReg, TR::RegisterDependencyConditions* cond)
{
    if (deltaReg)
        return generateMemRegInstruction(cursor, TR::InstOpCode::ADD4MemReg,
            generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()), self()), deltaReg, self());
    else
        return cursor;
}
...
void OMR::X86::CodeGenerator::removeUnavailableRegisters(
...
//
void OMR::X86::CodeGenerator::dumpPostFPRegisterAssignment(
    TR::Instruction* instructionCursor, TR::Instruction* origPrevInstruction)
{
    if (self()->comp()->getOutFile() == NULL)
        return;
...
void OMR::X86::CodeGenerator::dumpPreGPRegisterAssignment(TR::Instruction* instructionCursor)
{
    ...
    if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
        ...
        if (debug("dumpGPRegStatus")) {
            self()->machine()->printGPRegisterStatus(
                self()->fe(), self()->machine()->registerFile(), self()->comp()->getOutFile());
        }
    }
}
...
//
void OMR::X86::CodeGenerator::dumpPostGPRegisterAssignment(
    TR::Instruction* instructionCursor, TR::Instruction* origNextInstruction)
{
    if (self()->comp()->getOutFile() == NULL)
        return;
...
int32_t OMR::X86::CodeGenerator::arrayTranslateMinimumNumberOfElements(bool isByteSource, bool isByteTarget)
{
...
int32_t OMR::X86::CodeGenerator::arrayTranslateAndTestMinimumNumberOfIterations()
{
    if (TR::CodeGenerator::useOldArrayTranslateMinimumNumberOfIterations())
...
void OMR::X86::CodeGenerator::switchCodeCacheTo(TR::CodeCache* newCodeCache)
{
    self()->setNumReservedIPICTrampolines(0);
    OMR::CodeGenerator::switchCodeCacheTo(newCodeCache);
}

bool OMR::X86::CodeGenerator::directCallRequiresTrampoline(intptr_t targetAddress, intptr_t sourceAddress)
{
    // Adjust the sourceAddress to the start of the following instruction (+5 bytes)
    //
    return !self()->comp()->target().cpu.isTargetWithinRIPRange(targetAddress, sourceAddress + 5)
...
uint32_t OMR::X86::CodeGenerator::getOutOfLineCodeSize()
{
