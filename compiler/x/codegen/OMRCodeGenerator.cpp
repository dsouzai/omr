...
void OMR::X86::CodeGenerator::initializeX86(TR::Compilation *comp)
{
   ...
   if (comp->target().is64Bit())
    ...
    if (comp->getOption(TR_TraceRA)) {
        self()->setGPRegisterIterator(new (self()->trHeapMemory()) TR::RegisterIterator(self()->machine(),
            TR::RealRegister::FirstGPR, TR::RealRegister::LastAssignableGPR));
...
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node *node, TR::Compilation *comp)
{
    ...
    switch (methodSymbol->getRecognizedMethod()) {
        ...
        case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
            return willNotInlineCompareAndSwapNative(node,
                (comp->target().is64Bit() && !comp->useCompressedPointers()) ? 8 : 4, comp);
...
void OMR::X86::CodeGenerator::saveBetterSpillPlacements(TR::Instruction *branchInstruction)
{
    ...
    for (auto regElement = _spilledIntRegisters.begin();
        ...
        self()->traceRegisterAssignment("Saved better spill placement for %R, mask = %x.", *regElement,
            freeRealRegisters);
...
void OMR::X86::CodeGenerator::removeBetterSpillPlacementCandidate(TR::RealRegister *realReg)
{
    ...
    uint32_t mask = ~TR::RealRegister::getRealRegisterMask(realReg->getKind(),
        realReg->getRegisterNumber()); // TODO:AMD64: Use the proper mask value
...
TR::Instruction *OMR::X86::CodeGenerator::findBetterSpillPlacement(TR::Register *virtReg, int32_t realRegNum)
{
    ...
    if (info
        ...
        self()->traceRegisterAssignment("Successful better spill placement for %R at [" POINTER_PRINTF_FORMAT "].",
            virtReg, placement);
    } else {
        placement = NULL;
...
void OMR::X86::CodeGenerator::performNonLinearRegisterAssignmentAtBranch(TR::X86LabelInstruction *branchInstruction,
...
void OMR::X86::CodeGenerator::processClobberingInstructions(TR::ClobberingInstruction *clobInstructionCursor,
    ...
    if (self()->comp()->target().is64Bit()) {
        ...
        if (self()->comp()->target().isSMP()) {
            ...
            if (recompilation && !recompilation->useSampling()) {
                ...
                static const TR_AtomicRegion countingAtomicRegions[] = {
                    { 0, 5 },
                    { 0, 0 }
                };
...
#endif
            {
                ...
                static const TR_AtomicRegion samplingAtomicRegions[] = {
                    { 0, 2 },
                    { 0, 0 }
                };
...
void OMR::X86::CodeGenerator::doBackwardsRegisterAssignment(TR_RegisterKinds kindsToAssign,
    TR::Instruction *instructionCursor, TR::Instruction *appendInstruction)
{
    TR::Compilation *comp = self()->comp();
...
TR::Register *OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node *node)
{
    ...
    TR_ASSERT(!node->getOpCode().isRef() || self()->comp()->target().is32Bit()
...
{
    TR_ASSERT(!node->getOpCode().is4Byte() && !node->getOpCode().is8Byte(),
...
//
int32_t OMR::X86::CodeGenerator::branchDisplacementToHelperOrTrampoline(uint8_t *branchInstructionAddress,
    TR::SymbolReference *helper)
{
    ...
    if (self()->directCallRequiresTrampoline(helperAddress, (intptr_t)branchInstructionAddress)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(helper->getReferenceNumber(),
            (void *)(nextInstructionAddress - 4));
...
bool OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(TR::Node *node, TR::Node *parent,
...
void OMR::X86::CodeGenerator::simulateNodeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable = TR_X86PaddingTable(7,
    (uint8_t ***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding, TR_X86PaddingTable::registerMatters, 0x8b, 0x20);
...
uint8_t *OMR::X86::CodeGenerator::generatePadding(uint8_t *cursor, intptr_t length, TR::Instruction *neighborhood,
    TR_PaddingProperties properties, bool recursive)
{
        ...
        if (neighborhood) {
            ...
            if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
                ...
                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                    TR::DebugCounter::debugCounterName(self()->comp(), "nopCount/%d/%s/%s", blockFrequency,
                    ...
                    for (TR::Instruction *ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
                        if (ninst->isVirtualGuardNOPInstruction()) {
                            if (guardNode->isHCRGuard() && ninst->getNode() && ninst->getNode()->isHCRGuard()
                                ...
                                continue;
                            TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/vgnop",
                                    blockFrequency));
                            break;
                        }
                        if (self()->comp()->isPICSite(ninst)) {
                            TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/staticPIC",
                                    blockFrequency));
                            break;
                        }
                        if (ninst->isPatchBarrier(self())) {
                            if (ninst->getOpCodeValue() != TR::InstOpCode::label)
                                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                    TR::DebugCounter::debugCounterName(self()->comp(),
                                        "vgnopNoPatchReason/%d/patchBarrier", blockFrequency));
                            else
                                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                                    TR::DebugCounter::debugCounterName(self()->comp(),
                                        "vgnopNoPatchReason/%d/controlFlowMerge", blockFrequency));
                            break;
                        }
                ...
                TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                    TR::DebugCounter::debugCounterName(self()->comp(), "nopCount/%d/%s", blockFrequency,
                        neighborhood->description()));
                if (length > 0)
                    TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                        TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s", blockFrequency,
                            neighborhood->description()));
            }
        } else {
...
bool OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(void *cursor, intptr_t length, intptr_t boundary,
    intptr_t margin)
{
...
void OMR::X86::CodeGenerator::removeUnavailableRegisters(TR::RegisterCandidate *rc, TR::Block **blocks,
