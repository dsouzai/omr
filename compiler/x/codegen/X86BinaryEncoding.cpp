...
int32_t memoryBarrierRequired(
    TR::InstOpCode& op, TR::MemoryReference* mr, TR::CodeGenerator* cg, bool onlyAskingAboutFences)
{
    ...
    if (!onlyAskingAboutFences && mr->requiresLockPrefix()) {
    ...
    //
    if ((symRef.isUnresolved() && mbou) || (sym && sym->isVolatile() && !mr->ignoreVolatile())) {
...
OMR::X86::EnlargementResult TR::X86BoundaryAvoidanceInstruction::enlarge(
...
uint8_t* TR::X86BoundaryAvoidanceInstruction::generateBinaryEncoding()
{
    ...
    while (cur->getLength() > 0) {
        TR_ASSERT(cur->getLength() <= _boundarySpacing,
            "Atomic regions of %d bytes can't fit between %d-byte boundaries", cur->getLength(), _boundarySpacing);
...
int32_t TR::X86PatchableCodeAlignmentInstruction::betterPadLength(
    int32_t oldPadLength, const TR_AtomicRegion* unaccommodatedRegion, int32_t unaccommodatedRegionStart)
{
    int32_t result = TR::X86BoundaryAvoidanceInstruction::betterPadLength(
        oldPadLength, unaccommodatedRegion, unaccommodatedRegionStart);
    TR_ASSERT(
        result < getBoundarySpacing(), "Patchable code has no viable padding that will let it be updated atomically");
    return result;
}
...
OMR::X86::EnlargementResult TR::X86AlignmentInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
...
void TR::X86LabelInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{

    if (!getOpCode().hasRelativeBranchDisplacement() && !(getOpCodeValue() == TR::InstOpCode::label)) {
        if (getReloType() == TR_AbsoluteMethodAddress) {
            cg()->addProjectSpecializedRelocation(
                cursor, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, getNode());
        }
    }
}
            ...
            *(uint32_t*)cursor = 0;
            ...
            if (label->getCodeLocation() != NULL) {
                ...
                distance = static_cast<int32_t>(cg()->getBinaryBufferStart()
                    ...
                    //
                    + 4 + label->getEstimatedCodeLocation()
                    - (cursor + IA32LengthOfShortBranch + cg()->getAccumulatedInstructionLengthError()));
            }

            TR_ASSERT(getOpCodeValue() != TR::InstOpCode::XBEGIN4 || !_permitShortening,
                "TR::InstOpCode::XBEGIN4 cannot be shortened and can only be used with a label instruction that cannot "
                "shorten - use generateLongLabel!\n");

            if (distance >= -128 && distance <= 127 && getOpCode().isBranchOp() && _permitShortening) {
...
OMR::X86::EnlargementResult TR::X86LabelInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    ...
    if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement)
        || (requestedEnlargementSize > 4 && !allowPartialEnlargement) || maxEnlargementSize < 4)
        return OMR::X86::EnlargementResult(0, 0);
    ...
    if (getOpCode().isBranchOp()) {
        ...
        if (getOpCode().hasIntImmediate()) {
            ...
            if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
                int32_t distance
                    = getLabelSymbol()->getEstimatedCodeLocation() - (currentEstimate + IA32LengthOfShortBranch);
                if (distance >= -128 && distance < 0 && _permitShortening) {
                    immediateLength = 0; // really 1, but for conditional branches (all excep TR::InstOpCode::JMP4) the
                                         // opcode entry will be 1 too big for short branch because the short branch op
                                         // is 1 byte, but the long branch op for conditionals is 2
...
void TR::X86FenceInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{

    cg()->addProjectSpecializedRelocation(cursor, 0, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, _fenceNode);
}
...
uint8_t* TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
    ...
    if (label->getCodeLocation() == NULL) {
        ...
        //
        offset = cg()->getBinaryBufferStart() + label->getEstimatedCodeLocation()
            - (patchCursor + IA32LengthOfShortBranch + cg()->getAccumulatedInstructionLengthError());
    ...
    // guards that do not require atomic patching have a more relaxed sizing constraing since they are only patched
    // while all threads are stopped
    bool requiresAtomicPatching = this->requiresAtomicPatching();
    uint8_t next = requiresAtomicPatching ? cg()->sizeOfInstructionToBePatched(this)
    ...
    if (instToBePatched && _nopSize > 0 && !cg()->nopsAlsoProcessedByRelocations()) {
        ...
        OMR::X86::EnlargementResult enlargement
            = instToBePatched->enlarge(_nopSize - next, getEstimatedBinaryLength(), !requiresAtomicPatching);
        if (enlargement.getPatchGrowth() + next >= _nopSize) {
            ...
            TR_ASSERT(enlargement.getPatchGrowth() == 0,
                "Instructions should only partially enlarge when dealing with an HCR guard - any other partial "
                "enlargement is a waste of code cache!");
        }
    ...
    cursor
        = cg()->generatePadding(cursor, _nopSize, this, requiresAtomicPatching ? TR_AtomicNoOpPadding : TR_NoOpPadding);
...
void TR::X86ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (getReloKind() != -1) // TODO: need to change Body info one to use this
        {
            ...
            case TR_SpecialRamMethodConst:
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t*)getNode()->getSymbolReference(),
                        ...
                        (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
            ...
            case TR_ClassPointer:
                if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t*)getNode(),
                                                    (TR_ExternalRelocationTargetKind)_reloKind, cg()),
                        __FILE__, __LINE__, getNode());
                }
                break;
            default:
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, 0, (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
                    __FILE__, __LINE__, getNode());
            }
        }
        ...
        if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(), cg()->comp()->getStaticHCRPICSites()->end(), this)
            ...
            cg()->jitAdd32BitPicToPatchOnClassRedefinition(
                ((void*)(uintptr_t)getSourceImmediateAsAddress()), (void*)cursor);
        }
    }
}
...
void TR::X86ImmSnippetInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
...
void TR::X86ImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
            if (!comp->isRecursiveMethodTarget(sym)) {
                ...
                if (labelSym) {
                    ...
                    if (methodSym && methodSym->isHelper()) {
                        cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)getSymbolReference(), NULL,
                        ...
                        static const int reloTypes[methodJNITypes] = { TR_JNIVirtualTargetAddress, 0 /*Interfaces*/,
                            TR_JNIStaticTargetAddress, TR_JNISpecialTargetAddress };
                        ...
                        TR_RelocationRecordInformation* info = reinterpret_cast<TR_RelocationRecordInformation*>(
                        ...
                        cg()->addExternalRelocation(
                            TR::ExternalRelocation::create(startOfInstruction, reinterpret_cast<uint8_t*>(info),
                                static_cast<TR_ExternalRelocationTargetKind>(reloTypes[rType]), cg()),
                            __FILE__, __LINE__, getNode());
                        ...
                        cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)getSymbolReference(), NULL,
                            TR_RelativeMethodAddress, __FILE__, __LINE__, getNode());
                    }
                }
            }
            ...
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)(uintptr_t)getSourceImmediate(),
                    getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool,
                    cg()),
                __FILE__, __LINE__, getNode());
            ...
            if (symbol->isConst()) {
                ...
                if (staticSym && staticSym->isCompiledMethod()) {
                    cg()->addExternalRelocation(
                        TR::ExternalRelocation::create(cursor, 0, TR_RamMethod, cg()), __FILE__, __LINE__, getNode());
                } else if (staticSym && staticSym->isStartPC()) {
                    cg()->addExternalRelocation(
                        TR::ExternalRelocation::create(
                            cursor, (uint8_t*)(staticSym->getStaticAddress()), TR_AbsoluteMethodAddress, cg()),
                        __FILE__, __LINE__, getNode());
                    ...
                    if (counter == NULL) {
                        comp->failCompilation<TR::CompilationException>(
                            ...
                            "TR::X86ImmSymInstruction::addMetaDataForCodeAddress\n");
                    }
        ...
        if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
            ...
            if (comp->isRecursiveMethodTarget(sym)) {
                ...
                if (!labelSym) {
                    ...
                    if (self()->cg()->comp()->target().is64Bit()) {
                        ...
                        if (methodSym && methodSym->isJNI() && symNode && symNode->isPreparedForDirectJNI()) {
                            ...
                            TR_ResolvedMethod* resolvedMethod
                                = resolvedMethodSym ? resolvedMethodSym->getResolvedMethod() : 0;
                    ...
                    bool isTrampolineRequired
                        = cg()->directCallRequiresTrampoline(targetAddress, currentInstructionAddress);

                    if (methodSym && methodSym->isHelper()) {
                        if (isTrampolineRequired) {
                            // TODO:AMD64: Consider AOT ramifications
                            targetAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
                                getSymbolReference()->getReferenceNumber(), (void*)cursor);
                        }
                    } else if (methodSym && methodSym->isJNI() && getNode() && getNode()->isPreparedForDirectJNI()) {
                        if (isTrampolineRequired) {
...
OMR::X86::EnlargementResult TR::X86RegInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    ...
    TR_ASSERT_FATAL(cg()->comp()->compileRelocatableCode() || cg()->comp()->isOutOfProcessCompilation()
            || cg()->comp()->compilePortableCode()
            || cg()->comp()->target().cpu.supportsAVX() == cg()->getX86ProcessorInfo().supportsAVX(),
        "supportsAVX() failed\n");
    ...
    if (comp->target().is64Bit() && !getOpCode().info().hasMandatoryPrefix()
        && performTransformation(
            comp, "O^O Enlarging instruction %p by %d bytes by repeating the REX prefix\n", this, enlargementSize)) {
...
uint8_t* TR::X86RegRegInstruction::generateOperand(uint8_t* cursor)
{
    ...
    if ((getOpCode().isEvexInstruction() && getEncodingMethod() != OMR::X86::Legacy)
        || getEncodingMethod() >= OMR::X86::EVEX_L128) {
...
{
    TR_ASSERT_FATAL(getEncodingMethod() != OMR::X86::Bad && getEncodingMethod() >= OMR::X86::EVEX_L128,
        "Masks can be be used on AVX-512 instructions");
    uint8_t* modRM = cursor - 1;
...
void TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (staticHCRPIC) {
            cg()->addExternalRelocation(TR::ExternalRelocation::create(
                                            (uint8_t*)cursor, (uint8_t*)(uintptr_t)getSourceImmediate(), TR_HCR, cg()),
                __FILE__, __LINE__, getNode());
            ...
            void* classPointer
                = (void*)cg()
                      ...
                      ->createResolvedMethod(cg()->trMemory(),
                          (TR_OpaqueMethodBlock*)(uintptr_t)getSourceImmediateAsAddress(), comp->getCurrentMethod())
                      ->classOfMethod();
        ...
        case TR_HEAP_BASE:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)TR_HeapBase, TR_GlobalValue, cg()), __FILE__, __LINE__,
                getNode());
            break;

        case TR_HEAP_TOP:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)TR_HeapTop, TR_GlobalValue, cg()), __FILE__, __LINE__,
                getNode());
            break;

        case TR_HEAP_BASE_FOR_BARRIER_RANGE:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)TR_HeapBaseForBarrierRange0, TR_GlobalValue, cg()),
                __FILE__, __LINE__, getNode());
            break;

        case TR_HEAP_SIZE_FOR_BARRIER_RANGE:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)TR_HeapSizeForBarrierRange0, TR_GlobalValue, cg()),
                __FILE__, __LINE__, getNode());
            break;
...
void TR::X86RegImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    ...
    case TR_ConstantPool:
        TR_ASSERT(symbol->isConst() || symbol->isConstantPoolAddress(),
            "unknown symbol type for TR_ConstantPool relocation %p\n", this);
    ...
    case TR_ClassPointer:
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)(uintptr_t)getSourceImmediate(), (uint8_t*)symbolKind,
    ...
    case TR_BlockFrequency: {
        ...
        recordInfo->data2 = 0; // seqKind
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t*)recordInfo, TR_BlockFrequency, cg()), __FILE__, __LINE__,
            getNode());
    } break;

    case TR_RecompQueuedFlag: {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, NULL, TR_RecompQueuedFlag, cg()), __FILE__, __LINE__, getNode());
    } break;
...
OMR::X86::EnlargementResult TR::X86MemInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    TR::Compilation* comp = cg()->comp();
    if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement) || requestedEnlargementSize < 1)
...
void TR::X86MemImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (_reloKind == TR_ClassAddress && cg()->needClassAndMethodPointerRelocations()) {
            ...
            if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                ...
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t*)getNode()->getSymbolReference(),
                        (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg()),
                    __FILE__, __LINE__, getNode());
            }
        }
    }
            ...
            if (comp->getOption(TR_EnableGCRPatching)) {
                ...
                if (node
                    && (node->getOpCodeValue() == TR::ificmpeq
                        || node->getOpCodeValue() == TR::ificmpne) /*node->getNumChildren() > 0*/) {
...
void TR::X86MemImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    ...
    if (symbol->isConst()) {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor,
                (uint8_t*)getSymbolReference()->getOwningMethod(comp)->constantPool(),
                getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
            __FILE__, __LINE__, getNode());
    } else if (symbol->isClassObject()) {
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t*)getSymbolReference(),
                getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_MethodObject, cg()),
            __FILE__, __LINE__, getNode());
    } else if (symbol->isDebugCounter()) {
        TR::DebugCounterBase* counter = comp->getCounterFromStaticAddress(getSymbolReference());
        if (counter == NULL) {
            comp->failCompilation<TR::CompilationException>("Could not generate relocation for debug counter in "
                                                            "TR::X86MemImmSymInstruction::addMetaDataForCodeAddress\n");
        }
        TR::DebugCounter::generateRelocation(comp, cursor, getNode(), counter);
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t*)getSymbolReference(),
                getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress, cg()),
            __FILE__, __LINE__, getNode());
    }
}
...
OMR::X86::EnlargementResult TR::X86RegMemInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    TR::Compilation* comp = cg()->comp();
    if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement) || requestedEnlargementSize < 1)
...
void TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    bool staticPIC = false;
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
    ...
    TR::SymbolReference* methodSymRef
        = getNode()->getOpCode().hasSymbolReference() ? getNode()->getSymbolReference() : NULL;

#ifdef J9_PROJECT_SPECIFIC
    if (cg()->needRelocationsForHelpers()) {
        if (getNode()->getOpCode().hasSymbolReference() && methodSymRef
            && (methodSymRef->getReferenceNumber() == TR_referenceArrayCopy
                || methodSymRef->getReferenceNumber()
                    == TR_prepareForOSR)) { // the reference number is set in
                                            // j9x86evaluator.cpp/VMarrayStoreCheckArrayCopyEvaluator
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)methodSymRef, TR_AbsoluteHelperAddress, cg()),
                __FILE__, __LINE__, getNode());
        }
    }
    ...
    if (comp->fej9()->needClassAndMethodPointerRelocations()) {
        if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant()
                && needsAOTRelocation())
            || staticHCRPIC) {
            cg()->addExternalRelocation(
            ...
            case TR_ClassAddress: {
                ...
                if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
                    cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, (uint8_t*)getSourceImmediate(),
                                                    (uint8_t*)TR::SymbolType::typeClass, TR_SymbolFromManager, cg()),
                        __FILE__, __LINE__, getNode());
                } else {
                    cg()->addExternalRelocation(
                        TR::ExternalRelocation::create(cursor, (uint8_t*)methodSymRef,
                            (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg()),
                        __FILE__, __LINE__, getNode());
                }
            } break;
            ...
            case TR_JNIVirtualTargetAddress: {
                ...
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(startOfInstruction, reinterpret_cast<uint8_t*>(info),
                        static_cast<TR_ExternalRelocationTargetKind>(_reloKind), cg()),
                    __FILE__, __LINE__, getNode());
            } break;
            }
        }
    ...
    if (staticHCRPIC) {
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create((uint8_t*)cursor, (uint8_t*)getSourceImmediate(), TR_HCR, cg()), __FILE__,
            __LINE__, getNode());
    ...
    if (staticMethodPIC) {
        void* classPointer = (void*)cg()
                                 ->fe()
                                 ->createResolvedMethod(cg()->trMemory(), (TR_OpaqueMethodBlock*)getSourceImmediate(),
...
void TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    ...
    if (getSymbolReference()->getSymbol()->isLabel()) {
        ...
        cg()->addRelocation(new (cg()->trHeapMemory())
                TR::LabelAbsoluteRelocation(cursor, getSymbolReference()->getSymbol()->castToLabelSymbol()));
        ...
        case TR_NativeMethodAbsolute: {
            if (cg()->comp()->getOption(TR_EmitRelocatableELFFile)) {
                ...
                cg()->addStaticRelocation(TR::StaticRelocation(cursor, target->externalName(cg()->trMemory()),
                    TR::StaticRelocationSize::word64, TR::StaticRelocationType::Absolute));
            }
            break;
        }
...
void TR::AMD64Imm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    if (needsAOTRelocation()) {
        cg()->addProjectSpecializedRelocation(cursor, 0, NULL, TR_BodyInfoAddress, __FILE__, __LINE__, getNode());
    }
}
...
void TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    if (getOpCodeValue() == TR::InstOpCode::DQImm64) {
        cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)(uint64_t)getSourceImmediate(),
            getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Thunks, __FILE__,
            __LINE__, getNode());
    }
}
...
{
    TR_ASSERT(getOpCodeValue() == TR::InstOpCode::DQImm64,
        "TR::AMD64Imm64SymInstruction expected to be TR::InstOpCode::DQImm64 only");
