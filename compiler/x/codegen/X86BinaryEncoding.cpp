...
int32_t memoryBarrierRequired(TR::InstOpCode &op, TR::MemoryReference *mr, TR::CodeGenerator *cg,
...
OMR::X86::EnlargementResult TR::X86BoundaryAvoidanceInstruction::enlarge(int32_t requestedEnlargementSize,
...
int32_t TR::X86PatchableCodeAlignmentInstruction::betterPadLength(int32_t oldPadLength,
    const TR_AtomicRegion *unaccommodatedRegion, int32_t unaccommodatedRegionStart)
{
    int32_t result = TR::X86BoundaryAvoidanceInstruction::betterPadLength(oldPadLength, unaccommodatedRegion,
        unaccommodatedRegionStart);
    TR_ASSERT(result < getBoundarySpacing(),
        "Patchable code has no viable padding that will let it be updated atomically");
    return result;
}
...
OMR::X86::EnlargementResult TR::X86AlignmentInstruction::enlarge(int32_t requestedEnlargementSize,
    int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
        ...
        if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
            ...
            if (symbol->isConst()) {
                ...
                if (staticSym && staticSym->isCompiledMethod()) {
                    ...
                    cg()->addExternalRelocation(
                        TR::ExternalRelocation::create(cursor, (uint8_t *)recordInfo, TR_BlockFrequency, cg()),
                        __FILE__, __LINE__, getNode());
...
void TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (staticPIC) {
            cg()->jitAdd32BitPicToPatchOnClassUnload(((void *)(uintptr_t)getSourceImmediateAsAddress()),
                (void *)cursor);
        }

        if (staticHCRPIC) {
            cg()->addExternalRelocation(TR::ExternalRelocation::create((uint8_t *)cursor,
                                            (uint8_t *)(uintptr_t)getSourceImmediate(), TR_HCR, cg()),
                __FILE__, __LINE__, getNode());
        ...
        switch (_reloKind) {
            case TR_HEAP_BASE:
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t *)TR_HeapBase, TR_GlobalValue, cg()), __FILE__,
                    __LINE__, getNode());
                break;

            case TR_HEAP_TOP:
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, (uint8_t *)TR_HeapTop, TR_GlobalValue, cg()), __FILE__,
                    __LINE__, getNode());
                break;

            case TR_HEAP_BASE_FOR_BARRIER_RANGE:
                cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor,
                                                (uint8_t *)TR_HeapBaseForBarrierRange0, TR_GlobalValue, cg()),
                    __FILE__, __LINE__, getNode());
                break;

            case TR_HEAP_SIZE_FOR_BARRIER_RANGE:
                cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor,
                                                (uint8_t *)TR_HeapSizeForBarrierRange0, TR_GlobalValue, cg()),
                    __FILE__, __LINE__, getNode());
                break;
...
OMR::X86::EnlargementResult TR::X86MemInstruction::enlarge(int32_t requestedEnlargementSize, int32_t maxEnlargementSize,
    bool allowPartialEnlargement)
{
    TR::Compilation *comp = cg()->comp();
    if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement) || requestedEnlargementSize < 1)
...
void TR::X86MemImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor)
{
    ...
    if (symbol->isConst()) {
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)getSymbolReference(),
                getNode() ? (uint8_t *)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_MethodObject,
                cg()),
            __FILE__, __LINE__, getNode());
        ...
        cg()->addExternalRelocation(TR::ExternalRelocation::create(cursor, NULL, TR_RecompQueuedFlag, cg()), __FILE__,
            __LINE__, getNode());
        ...
        cg()->addExternalRelocation(
            TR::ExternalRelocation::create(cursor, (uint8_t *)getSymbolReference(),
                getNode() ? (uint8_t *)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t *)-1, TR_DataAddress,
                cg()),
            __FILE__, __LINE__, getNode());
    }
}
...
OMR::X86::EnlargementResult TR::X86RegMemInstruction::enlarge(int32_t requestedEnlargementSize,
    int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    TR::Compilation *comp = cg()->comp();
    if ((maxEnlargementSize < requestedEnlargementSize && !allowPartialEnlargement) || requestedEnlargementSize < 1)
        ...
        if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant()
            ...
            switch (_reloKind) {
                ...
                case TR_JNIVirtualTargetAddress: {
                    ...
                    cg()->addExternalRelocation(TR::ExternalRelocation::create(startOfInstruction,
    ...
    if (getSymbolReference()->getSymbol()->isLabel()) {
        ...
        switch (getReloKind()) {
            case TR_AbsoluteMethodAddress:
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, NULL, TR_AbsoluteMethodAddress, cg()), __FILE__, __LINE__,
                    getNode());
                break;
            default:
                break;
