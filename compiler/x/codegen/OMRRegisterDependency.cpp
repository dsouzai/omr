...
OMR::X86::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::Node* node, TR::CodeGenerator* cg, uint16_t additionalRegDeps)
    : _numPreConditions(-1)
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
        ...
        if (globalReg->getKind() == TR_GPR || globalReg->getKind() == TR_VRF || XMM_GPRS_USE_DISTINCT_NUMBERS(cg)) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(
                node, false, "invalid global register kind %d, reg=%p", globalReg->getKind(), globalReg);
        }
...
TR::RegisterDependencyConditions* OMR::X86::RegisterDependencyConditions::clone(
    TR::CodeGenerator* cg, uint32_t additionalRegDeps)
{
    ...
    for (int32_t i = _numPreConditions - 1; i >= 0; --i) {
        ...
        other->getPreConditions()->setDependencyInfo(
            i, dep->getRegister(), dep->getRealRegister(), cg, dep->getFlags());
    }

    for (int32_t i = _numPostConditions - 1; i >= 0; --i) {
        TR::RegisterDependency* dep = getPostConditions()->getRegisterDependency(i);
        other->getPostConditions()->setDependencyInfo(
            i, dep->getRegister(), dep->getRealRegister(), cg, dep->getFlags());
    }
...
void OMR::X86::RegisterDependencyConditions::assignPreConditionRegisters(
    TR::Instruction* currentInstruction, TR_RegisterKinds kindsToBeAssigned, TR::CodeGenerator* cg)
{
    if (_preConditions != NULL) {
...
void OMR::X86::RegisterDependencyConditions::assignPostConditionRegisters(
    TR::Instruction* currentInstruction, TR_RegisterKinds kindsToBeAssigned, TR::CodeGenerator* cg)
{
    if (_postConditions != NULL) {
        if ((kindsToBeAssigned & TR_X87_Mask)) {
...
void OMR::X86::RegisterDependencyGroup::unblockRealDependencyRegisters(
...
void OMR::X86::RegisterDependencyConditions::createRegisterAssociationDirective(
...
TR::RealRegister* OMR::X86::RegisterDependencyConditions::getRealRegisterFromVirtual(
    TR::Register* virtReg, TR::CodeGenerator* cg)
{
    TR::Machine* machine = cg->machine();
...
TR::RegisterDependencyConditions* generateRegisterDependencyConditions(
    uint32_t numPreConds, uint32_t numPostConds, TR::CodeGenerator* cg)
{
    return new (cg->trHeapMemory()) TR::RegisterDependencyConditions(numPreConds, numPostConds, cg->trMemory());
}

TR::RegisterDependencyConditions* generateRegisterDependencyConditions(
    TR::Node* node, TR::CodeGenerator* cg, uint32_t additionalRegDeps, List<TR::Register>* popRegisters)
{
