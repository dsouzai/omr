 ...
 */
void TR_X86SubtractAnalyser::integerSubtractAnalyserWithExplicitOperands(TR::Node* root, TR::Node* firstChild,
    ...
    TR::Node* borrow) // 0 by default
{
    TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
        "unsupported opcode %s for integerSubtractAnalyserWithExplicitOperands on node %p\n",
        _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
    TR::Register* tempReg = integerSubtractAnalyserImpl(
        root, firstChild, secondChild, regRegOpCode, regMemOpCode, copyOpCode, needsEflags, borrow);
 ...
 */
void TR_X86SubtractAnalyser::integerSubtractAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic regRegOpCode,
    ...
    TR::Node* borrow) // 0 by default
{
    ...
    targetRegister = integerSubtractAnalyserImpl(
        root, firstChild, secondChild, regRegOpCode, regMemOpCode, copyOpCode, needsEflags, borrow);
...
/*
 * users should call the integerSubtractAnalyser or integerSubtractAnalyserWithExplicitOperands APIs instead of calling
 * this one directly
 */
TR::Register* TR_X86SubtractAnalyser::integerSubtractAnalyserImpl(TR::Node* root, TR::Node* firstChild,
    TR::Node* secondChild, TR::InstOpCode::Mnemonic regRegOpCode, TR::InstOpCode::Mnemonic regMemOpCode,
    TR::InstOpCode::Mnemonic copyOpCode, bool needsEflags, TR::Node* borrow)
{
    ...
    if (loadedConst) {
        if (firstRegister == 0) {
            ...
            TR::TreeEvaluator::insertLoadConstant(
                firstChild, firstRegister, firstChild->getInt(), TR_RematerializableInt, _cg);
        } else {
 ...
 */
void TR_X86SubtractAnalyser::longSubtractAnalyserWithExplicitOperands(
    TR::Node* root, TR::Node* firstChild, TR::Node* secondChild)
{
    TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
        "unsupported opcode %s for longSubtractAnalyserWithExplicitOperands on node %p\n",
        _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
...
/*
 * \brief
 * this API is intended for regular lsub operation nodes where the first child and second child are the operands by
 * default
 */
...
/*
 * users should call the longSubtractAnalyser or longSubtractAnalyserWithExplicitOperands APIs instead of calling this
 * one directly
 */
TR::Register* TR_X86SubtractAnalyser::longSubtractAnalyserImpl(
    TR::Node* root, TR::Node*& firstChild, TR::Node*& secondChild)
{
    TR::Register* firstRegister = firstChild->getRegister();
    TR::Register* secondRegister = secondChild->getRegister();
    ...
    if (root->getOpCodeValue() == TR::lusubb && TR::TreeEvaluator::setCarryBorrow(root->getChild(2), true, _cg)) {
    ...
    if (getCopyReg1()) {
        ...
        if (getSubReg3Reg2()) {
            if (secondHighZero) {
                ...
                generateRegRegInstruction(
                    TR::InstOpCode::SBB4RegReg, root, highThird, secondRegister->getHighOrder(), _cg);
            }
        } else // assert getSubReg3Mem2() == true
        {
             ...
             * first child of the lushr is in a register, code above handles the shift. When the first child of the
             * lushr is in memory, the below ensures that the upper part of the first child of the lushr is used as
             * lowMR.
             */
            if (useSecondHighOrder) {
        ...
        if (secondHighZero) {
            ...
            generateRegRegInstruction(
                regRegOpCode, root, firstRegister->getLowOrder(), secondRegister->getLowOrder(), _cg);
            generateRegRegInstruction(
                TR::InstOpCode::SBB4RegReg, root, firstRegister->getHighOrder(), secondRegister->getHighOrder(), _cg);
        }
        targetRegister = firstRegister;
    } else // assert getSubReg1Mem2() == true
    {
         ...
         * of the lushr is in a register, code above handles the shift. When the first child of the lushr is in memory,
...
const uint8_t TR_X86SubtractAnalyser::_actionMap[NUM_ACTIONS] = { // Reg1 Mem1 Clob1 Reg2 Mem2 Clob2
    EvalChild1 | //  0    0     0    0    0     0
        EvalChild2 | CopyReg1 | SubReg3Reg2,
