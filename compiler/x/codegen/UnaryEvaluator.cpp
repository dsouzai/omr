...
TR::Register* OMR::X86::TreeEvaluator::floatingPointAbsHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cmpEncoding >= EVEX_L128) {
        TR_ASSERT_FATAL(
            cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F), "512-bit vabs requires AVX512");
        ...
        if (cg->comp()->target().cpu.supportsAVX()) {
            ...
            generateRegRegRegInstruction(
                andOpcode.getMnemonic(), node, resultReg, resultReg, valueReg, cg, andEncoding);
        } else {
            generateRegRegImmInstruction(cmpOpcode.getMnemonic(), node, resultReg, resultReg, 0x8, cg, cmpEncoding);
...
TR::Register* OMR::X86::TreeEvaluator::unaryVectorArithmeticEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!opcode.isVectorMasked() && valueNode->getRegister() == NULL && valueNode->getReferenceCount() == 1
        && regMemOpcode.getMnemonic() != TR::InstOpCode::bad) {
...
TR::Register* OMR::X86::TreeEvaluator::negEvaluatorHelper(
    TR::Node* node, TR::InstOpCode::Mnemonic negInstr, TR::CodeGenerator* cg)
{
...
{
    return TR::TreeEvaluator::negEvaluatorHelper(
        node, TR::InstOpCode::NEGReg(TR::TreeEvaluator::getNodeIs64Bit(node, cg)), cg);
}
