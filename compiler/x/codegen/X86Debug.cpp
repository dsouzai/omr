...
void TR_Debug::printDependencyConditions(
    TR::RegisterDependencyGroup* conditions, uint8_t numConditions, char* prefix, TR::FILE* pOutFile)
{
    char buf[32];
    char* cursor;
    int len, i;
...
void TR_Debug::printFullRegisterDependencyInfo(TR::FILE* pOutFile, TR::RegisterDependencyConditions* conditions)
{
    ...
    if (conditions->getNumPostConditions() > 0) {
        printDependencyConditions(
            conditions->getPostConditions(), conditions->getNumPostConditions(), "Post", pOutFile);
    }
}
...
void TR_Debug::printPrefixAndMemoryBarrier(
    TR::FILE* pOutFile, TR::Instruction* instr, int32_t barrier, int32_t barrierOffset)
{
    int32_t barrierLength = ::estimateMemoryBarrierBinaryLength(barrier, _comp->cg());
...
void TR_Debug::print(TR::FILE* pOutFile, TR::MemoryReference* mr, TR_RegisterSizes operandSize)
{
    ...
    if (!hasTerm) {
        ...
        if (mr->getLabel()) {
            ...
            printHexConstant(
                pOutFile, _comp->target().is64Bit() ? disp : (uint32_t)disp, _comp->target().is64Bit() ? 16 : 8, true);
        } else if (cds) {
...
int32_t TR_Debug::printIntConstant(
    TR::FILE* pOutFile, int64_t value, int8_t radix, TR_RegisterSizes size, bool padWithZeros)
{
    if (pOutFile == NULL)
        return 0;
...
void TR_Debug::printRegRegInstruction(
    TR::FILE* pOutFile, const char* opCode, TR::RealRegister* reg1, TR::RealRegister* reg2)
{
...
void TR_Debug::printRegMemInstruction(
    TR::FILE* pOutFile, const char* opCode, TR::RealRegister* reg, TR::RealRegister* base, int32_t offset)
{
    trfprintf(pOutFile, "%s\t", opCode);
    print(pOutFile, reg);
    if (base) {
...
void TR_Debug::printMemRegInstruction(
    TR::FILE* pOutFile, const char* opCode, TR::RealRegister* base, int32_t offset, TR::RealRegister* reg)
{
    trfprintf(pOutFile, "%s\t", opCode);
    trfprintf(pOutFile, "[");
    print(pOutFile, base);
...
void TR_Debug::printMemImmInstruction(
    TR::FILE* pOutFile, const char* opCode, TR::RealRegister* base, int32_t offset, int32_t imm)
{
    trfprintf(pOutFile, "%s\t", opCode);
    trfprintf(pOutFile, "[");
    print(pOutFile, base);
