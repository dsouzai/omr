...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::Node* callNode, TR::ILOpCodes callOp, TR::Register* targetReg,
...
void TR_OutlinedInstructions::generateOutlinedInstructionsDispatch()
{
    ...
    if (_targetReg) {
        ...
        TR_ASSERT(resultReg->getKind() == _targetReg->getKind(),
            "OutlinedInstructions: targetReg must have same kind as resultReg.");
...
void TR_OutlinedInstructions::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned, TR::X86VFPSaveInstruction* vfpSaveInstruction)
{
...
OMR::RegisterUsage* TR_OutlinedInstructions::findInRegisterUsageList(
    TR::list<OMR::RegisterUsage*>* rul, TR::Register* virtReg)
{
...
TR_OutlinedInstructionsGenerator::TR_OutlinedInstructionsGenerator(
    TR::LabelSymbol* entryLabel, TR::Node* node, TR::CodeGenerator* cg)
    : _hasEnded(false)
{
...
TR_OutlinedInstructionsGenerator::~TR_OutlinedInstructionsGenerator()
{
    if (!std::uncaught_exception()) {
        TR_ASSERT_FATAL(_hasEnded,
            "TR_OutlinedInstructionsGenerator object has gone out of scope before call to "
            "endOutlinedInstructionSequence().");
    }
}
