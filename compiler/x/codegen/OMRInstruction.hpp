...
protected:
    Instruction(
        TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, OMR::X86::Encoding encoding = Default);
...
public:
    ...
    virtual EnlargementResult enlarge(
...
public:
    ...
    struct EVEX {
        ...
        inline uint8_t Reg() const { return modrm.Reg(~R); }
        inline uint8_t RM() const { return modrm.RM(~B); }
    };
};
...
template<> struct Instruction::VEX<3> {
    ...
    inline VEX() { }
    inline VEX(const REX& rex, uint8_t ModRMOpCode)
        : modrm(ModRMOpCode)
    {
        escape = '\xC4';
        R = ~rex.R;
...
template<> struct Instruction::VEX<2> {
    ...
    inline uint8_t Reg() const { return modrm.Reg(~R); }
    inline uint8_t RM() const { return modrm.RM(); }
};
}} // namespace OMR::X86
