...
TR::Instruction *OMR::X86::TreeEvaluator::compareGPMemoryToImmediate(TR::Node *node, TR::MemoryReference *mr,
...
void OMR::X86::TreeEvaluator::compareGPRegisterToImmediate(TR::Node *node, TR::Register *cmpRegister, int32_t value,
    TR::CodeGenerator *cg)
{
    // On IA32, this is called to do half of an 8-byte compare, so even though
    // the node is 64 bit, we should do a 32-bit compare
...
void OMR::X86::TreeEvaluator::compareGPRegisterToImmediateForEquality(TR::Node *node, TR::Register *cmpRegister,
    int32_t value, TR::CodeGenerator *cg)
{
    // On IA32, this is called to do half of an 8-byte compare, so even though
    // the node is 64 bit, we should do a 32-bit compare
    ...
    TR_RematerializableTypes type, TR::CodeGenerator *cg, TR::Instruction *currentInstruction)
{
    ...
    static const TR::InstOpCode::Mnemonic ops[TR_NumRematerializableTypes + 1][3] =
        //    load 0      load -1     load c
        {
            ...
            { TR::InstOpCode::XOR4RegReg, TR::InstOpCode::OR8RegImms,         TR::InstOpCode::UD2 }
    }; // Long address constant; MOVs handled specially
    ...
    if (currentInstruction) {
        ...
        if (is64Bit) {
            if (cg->constantAddressesCanChangeSize(node) && node && node->getOpCodeValue() == TR::aconst
                && (node->isClassPointerConstant() || node->isMethodPointerConstant())) {
                movInstruction = generateRegImm64Instruction(currentInstruction, TR::InstOpCode::MOV8RegImm64, target,
                    value, cg, reloKind);
            } else if (IS_32BIT_UNSIGNED(value)) {
                // zero-extended 4-byte MOV
                movInstruction = generateRegImmInstruction(currentInstruction, TR::InstOpCode::MOV4RegImm4, target,
                    static_cast<int32_t>(value), cg, reloKind);
            } else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we could get RIP too?
            {
                movInstruction = generateRegImmInstruction(currentInstruction, TR::InstOpCode::MOV8RegImm4, target,
                    static_cast<int32_t>(value), cg, reloKind);
            } else {
                movInstruction = generateRegImm64Instruction(currentInstruction, TR::InstOpCode::MOV8RegImm64, target,
                    value, cg, reloKind);
            ...
            movInstruction = generateRegImmInstruction(currentInstruction, ops[opsRow][MOV], target,
                static_cast<int32_t>(value), cg, reloKind);
        }
        ...
        if (target && node && node->getOpCodeValue() == TR::aconst && node->isMethodPointerConstant()
            && (cg->fe()->isUnloadAssumptionRequired(cg->fe()
                                                         ->createResolvedMethod(cg->trMemory(),
                                                             (TR_OpaqueMethodBlock *)node->getAddress(),
                                                             comp->getCurrentMethod())
                                                         ->classOfMethod(),
                    comp->getCurrentMethod())
                || cg->profiledPointersRequireRelocation())) {
        ...
        if (value == 0 && canClobberEFLAGS) {
            ...
            if (is64Bit) {
                if (cg->constantAddressesCanChangeSize(node) && node && node->getOpCodeValue() == TR::aconst
                    ...
                    // zero-extended 4-byte MOV
                    movInstruction = generateRegImmInstruction(TR::InstOpCode::MOV4RegImm4, node, target,
                        static_cast<int32_t>(value), cg, reloKind);
                } else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we could get RIP too?
                {
                    movInstruction = generateRegImmInstruction(TR::InstOpCode::MOV8RegImm4, node, target,
                        static_cast<int32_t>(value), cg, reloKind);
                } else {
                    movInstruction
                        = generateRegImm64Instruction(TR::InstOpCode::MOV8RegImm64, node, target, value, cg, reloKind);
                ...
                movInstruction = generateRegImmInstruction(ops[opsRow][MOV], node, target, static_cast<int32_t>(value),
                    cg, reloKind);
            }
...
TR::Register *OMR::X86::TreeEvaluator::loadConstant(TR::Node *node, intptr_t value, TR_RematerializableTypes type,
    TR::CodeGenerator *cg, TR::Register *targetRegister)
{
...
//
void OMR::X86::TreeEvaluator::padUnresolvedDataReferences(TR::Node *node, TR::SymbolReference &symRef,
    TR::CodeGenerator *cg)
{
...
TR::Register *OMR::X86::TreeEvaluator::performIload(TR::Node *node, TR::MemoryReference *sourceMR,
    TR::CodeGenerator *cg)
{
    TR::Register *reg
...
//
bool OMR::X86::TreeEvaluator::genNullTestSequence(TR::Node *node, TR::Register *opReg, TR::Register *targetReg,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateRegMemInstruction(TR::InstOpCode::MOVUPSRegMem, node, xmm2Reg, generateX86MemoryReference(s2Reg, 0, cg),
        cg);
    generateRegMemInstruction(TR::InstOpCode::MOVUPSRegMem, node, xmm1Reg,
        generateX86MemoryReference(s2Reg, deltaReg, 0, cg), cg);
    ...
    generateRegMemInstruction(TR::InstOpCode::L1RegMem, node, s2ByteVer1Reg, generateX86MemoryReference(s2Reg, 0, cg),
        cg);
    generateMemRegInstruction(TR::InstOpCode::CMP1MemReg, node, generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
        s2ByteVer1Reg, cg);
    ...
    generateRegMemInstruction(TR::InstOpCode::L1RegMem, node, s2ByteVer2Reg,
        generateX86MemoryReference(s2Reg, equalTestReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::CMP1MemReg, node, generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
        s2ByteVer2Reg, cg);
...
TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpLenEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateLabelInstruction(TR::InstOpCode::label, node, qwordLoop, cg);
    generateRegMemInstruction(TR::InstOpCode::MOVUPSRegMem, node, xmm1Reg,
        generateX86MemoryReference(s1Reg, resultReg, 0, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::MOVUPSRegMem, node, xmm2Reg,
...
void OMR::X86::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    switch (node->getOverflowCheckOperation()) {
        ...
        case TR::lmul:
            ...
            //  the 32 bits lmul needs several instructions including to multiplications between the lower and higher
            //  parts of the registers and additions of the intermediate results. See
            //  TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser Therefore the usual way of only detecting the OF
            //  for the last instruction of sequence won't work for this case. The implementation needs to detect OF
            //  flags after all the instructions involving higher parts of the registers for both operands and
    ...
    // the operation
    {
        ...
        switch (node->getOverflowCheckOperation()) {
            ...
            case TR::sadd:
            case TR::iadd:
                addMulAnalyser.integerAddAnalyserWithExplicitOperands(node, operand1, operand2, op, TR::InstOpCode::UD2,
                    needsEflags);
                break;
            case TR::ladd:
                cg->comp()->target().is32Bit()
                    ? addMulAnalyser.longAddAnalyserWithExplicitOperands(node, operand1, operand2)
                    : addMulAnalyser.integerAddAnalyserWithExplicitOperands(node, operand1, operand2, op,
                          TR::InstOpCode::UD2, needsEflags);
                break;
            // sub group
            case TR::bsub:
                subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                    TR::InstOpCode::UD2, TR::InstOpCode::MOV1RegReg, needsEflags);
                break;
            case TR::ssub:
            case TR::isub:
                subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                    TR::InstOpCode::UD2, TR::InstOpCode::MOV4RegReg, needsEflags);
                break;
            case TR::lsub:
                cg->comp()->target().is32Bit()
                    ? subAnalyser.longSubtractAnalyserWithExplicitOperands(node, operand1, operand2)
                    : subAnalyser.integerSubtractAnalyserWithExplicitOperands(node, operand1, operand2, op,
                          TR::InstOpCode::UD2, TR::InstOpCode::MOV8RegReg, needsEflags);
                break;
            // mul group
            case TR::imul:
                addMulAnalyser.genericAnalyserWithExplicitOperands(node, operand1, operand2, op, TR::InstOpCode::UD2,
                    TR::InstOpCode::MOV4RegReg);
                break;
            case TR::lmul:
                addMulAnalyser.genericAnalyserWithExplicitOperands(node, operand1, operand2, op, TR::InstOpCode::UD2,
                    TR::InstOpCode::MOV8RegReg);
                break;
            default:
                break;
        }
 ...
 *     The code generator
 */
static void arrayCopy64BitPrimitiveOnIA32(TR::Node *node, TR::Register *dstReg, TR::Register *srcReg,
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    ...
    } else // decide direction during runtime
    {
        ...
        generateRegMemInstruction(TR::InstOpCode::LEARegMem(), node, scratch,
            generateX86MemoryReference(srcReg, sizeReg, 0, -8, cg), cg);
        ...
        generateRegMemInstruction(TR::InstOpCode::LEARegMem(), node, scratch,
            generateX86MemoryReference(NULL, scratch, 1, -1, cg), cg);
    }
    ...
    generateMemRegInstruction(TR::InstOpCode::MOVQMemReg, node, generateX86MemoryReference(dstReg, srcReg, 0, cg), XMM,
        cg);
    generateRegMemInstruction(TR::InstOpCode::LEARegMem(), node, srcReg,
        generateX86MemoryReference(srcReg, scratch, 3, cg), cg);
    ...
    TR::LabelSymbol *repMovsLabel, TR::LabelSymbol *mainEndLabel)
{
    ...
    // 8 or 16 Bytes
    generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg1,
        generateX86MemoryReference(srcReg, sizeReg, 0, -8, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::L8RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::S8MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -8, cg),
        tmpReg1, cg);
    ...
    // 24 or 32 Bytes
    generateRegMemInstruction(TR::InstOpCode::MOVDQURegMem, node, tmpXmmYmmReg1,
        generateX86MemoryReference(srcReg, sizeReg, 0, -16, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::MOVDQURegMem, node, tmpXmmYmmReg2,
        generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::MOVDQUMemReg, node,
        generateX86MemoryReference(dstReg, sizeReg, 0, -16, cg), tmpXmmYmmReg1, cg);
    generateMemRegInstruction(TR::InstOpCode::MOVDQUMemReg, node, generateX86MemoryReference(dstReg, 0, cg),
        tmpXmmYmmReg2, cg);
    generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
    ...
    generateRegMemInstruction(TR::InstOpCode::VMOVDQUYmmMem, node, tmpXmmYmmReg2,
        generateX86MemoryReference(srcReg, 0, cg), cg);
    ...
    generateMemRegInstruction(TR::InstOpCode::VMOVDQUMemYmm, node, generateX86MemoryReference(dstReg, 0, cg),
        tmpXmmYmmReg2, cg);
    generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
    ...
    generateRegMemInstruction(TR::InstOpCode::VMOVDQUZmmMem, node, tmpXmmYmmReg2,
        generateX86MemoryReference(srcReg, 0, cg), cg);
    ...
    generateMemRegInstruction(TR::InstOpCode::VMOVDQUMemZmm, node, generateX86MemoryReference(dstReg, 0, cg),
        tmpXmmYmmReg2, cg);
    generateLabelInstruction(TR::InstOpCode::JMP4, node, mainEndLabel, cg);
}
    ...
    TR::LabelSymbol *repMovsLabel, TR::LabelSymbol *mainEndLabel)
{
    ...
    // 4 or 8 Bytes
    generateRegMemInstruction(TR::InstOpCode::L4RegMem, node, tmpReg1,
        generateX86MemoryReference(srcReg, sizeReg, 0, -4, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::L4RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -4, cg),
        tmpReg1, cg);
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (node->isForwardArrayCopy()) {
        ...
        generateRegMemInstruction(TR::InstOpCode::L2RegMem, node, sizeReg, generateX86MemoryReference(srcReg, 0, cg),
            cg);
        generateMemRegInstruction(TR::InstOpCode::S2MemReg, node, generateX86MemoryReference(dstReg, 0, cg), sizeReg,
            cg);
    } else // decide direction during runtime
    {
...

        {
            ...
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(), node, srcReg,
                generateX86MemoryReference(srcReg, sizeReg, 0, -2, cg), cg);
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(), node, dstReg,
                generateX86MemoryReference(dstReg, sizeReg, 0, -2, cg), cg);
    ...
    TR::LabelSymbol *repMovsLabel, TR::LabelSymbol *mainEndLabel)
{
    ...
    // 1-2 Bytes
    generateRegMemInstruction(TR::InstOpCode::L1RegMem, node, tmpReg1,
        generateX86MemoryReference(srcReg, sizeReg, 0, -1, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::L1RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::S1MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -1, cg),
        tmpReg1, cg);
    ...
    // 3-4 Bytes
    generateRegMemInstruction(TR::InstOpCode::L2RegMem, node, tmpReg1,
        generateX86MemoryReference(srcReg, sizeReg, 0, -2, cg), cg);
    generateRegMemInstruction(TR::InstOpCode::L2RegMem, node, tmpReg2, generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(TR::InstOpCode::S2MemReg, node, generateX86MemoryReference(dstReg, sizeReg, 0, -2, cg),
        tmpReg1, cg);
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg, uint8_t elementSize, int32_t repMovsThresholdBytes)
{
    ...
    switch (elementSize) {
        case 8:
            OMR::X86::TreeEvaluator::arrayCopy64BitPrimitiveInlineSmallSizeWithoutREPMOVSImplRoot16(node, dstReg,
                srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsThresholdBytes,
                repMovsLabel, mainEndLabel);
            repmovs = TR::InstOpCode::REPMOVSQ;
            break;
        case 4:
            OMR::X86::TreeEvaluator::arrayCopy32BitPrimitiveInlineSmallSizeWithoutREPMOVSImplRoot16(node, dstReg,
                srcReg, sizeReg, tmpReg1, tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsThresholdBytes,
                repMovsLabel, mainEndLabel);
            repmovs = TR::InstOpCode::REPMOVSD;
            break;
        case 2:
            arrayCopy16BitPrimitiveInlineSmallSizeWithoutREPMOVSImplRoot16(node, dstReg, srcReg, sizeReg, tmpReg1,
                tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsThresholdBytes, repMovsLabel, mainEndLabel);
            repmovs = TR::InstOpCode::REPMOVSW;
            break;
        default: // 1-byte
            arrayCopy8BitPrimitiveInlineSmallSizeWithoutREPMOVSImplRoot8(node, dstReg, srcReg, sizeReg, tmpReg1,
                tmpReg2, tmpXmmYmmReg1, tmpXmmYmmReg2, cg, repMovsThresholdBytes, repMovsLabel, mainEndLabel);
    ...
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    ...
    if (enablePrimitiveArrayCopyInlineSmallSizeWithoutREPMOVS(elementSize, cg, repMovsThresholdBytes)) {
        arrayCopyPrimitiveInlineSmallSizeWithoutREPMOVS(node, dstReg, srcReg, sizeReg, cg, elementSize,
...
static void arraycopyForShortConstArrayWithDirection(TR::Node *node, TR::Register *dstReg, TR::Register *srcReg,
    uint32_t size, TR::CodeGenerator *cg)
{
 ...
 *     The code generator
 */
static void arraycopyForShortConstArrayWithoutDirection(TR::Node *node, TR::Register *dstReg, TR::Register *srcReg,
    uint32_t size, TR::CodeGenerator *cg)
{
    uint32_t totalSize = size;
    ...
    switch (residueCase) {
        ...
        case 4:
            ...
            generateArrayElementStore(node, dstReg, totalSize - secondLoadSizeForCase4, reg2, secondLoadSizeForCase4,
                cg);
            cg->stopUsingRegister(reg1);
            break;
    }
...
static void packUsingShift(TR::Node *node, TR::Register *tempReg, TR::Register *sourceReg, int32_t size,
    TR::CodeGenerator *cg)
{
...
static void packXMMWithMultipleValues(TR::Node *node, TR::Register *XMMReg, TR::Register *sourceReg, int8_t size,
    TR::CodeGenerator *cg)
{
    switch (size) {
        case 1:
        ...
        case 8:
            generateRegRegInstruction(TR::InstOpCode::MOVQRegReg8, node, XMMReg, sourceReg, cg);
            generateRegRegImmInstruction(TR::InstOpCode::PSHUFDRegRegImm1, node, XMMReg, XMMReg,
                size == 4 ? 0x00 : 0x44, cg);
            break;
        default:
...
static void arraySetToZeroForShortConstantArrays(TR::Node *node, TR::Register *addressReg, uintptr_t size,
    TR::CodeGenerator *cg)
{
...
static void arraySet64BitPrimitiveOnIA32(TR::Node *node, TR::Register *addressReg, TR::Register *valueReg,
    TR::Register *sizeReg, TR::CodeGenerator *cg)
{
    TR::Register *XMM = cg->allocateRegister(TR_FPR);
    ...
    generateMemRegInstruction(TR::InstOpCode::MOVQMemReg, node,
        generateX86MemoryReference(addressReg, sizeReg, 3, -8, cg), XMM, cg);
    generateLabelInstruction(TR::InstOpCode::LOOP1, node, loopLabel, cg);
...
TR::Register *OMR::X86::TreeEvaluator::arraysetEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (isShortConstantArrayWithZero) {
        ...
        if (valueNode->getOpCode().isFloat() || valueNode->getOpCode().isDouble()) {
            ...
            valueReg = TR::TreeEvaluator::intOrLongClobberEvaluate(valueNode,
                TR::TreeEvaluator::getNodeIs64Bit(valueNode, cg), cg);
        }
        ...
        TR::Register *sizeReg = TR::TreeEvaluator::intOrLongClobberEvaluate(sizeNode,
            TR::TreeEvaluator::getNodeIs64Bit(sizeNode, cg), cg);
...
{
    TR_ASSERT((!TR::InstOpCode(op).hasLongSource() && !TR::InstOpCode(op).hasLongTarget())
            || cg->comp()->target().is64Bit(),
 ...
 *     The code generator
 */
static TR::Register *inline64BitAtomicCompareAndMemoryUpdateOn32Bit(TR::Node *node, bool returnValue,
    TR::CodeGenerator *cg)
{
...
TR::Register *OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (SymRef && SymRef->getSymbol()->castToMethodSymbol()->isInlinedByCG()) {
        ...
        if (comp->getSymRefTab()->isNonHelper(SymRef,
                TR::SymbolReferenceTable::atomicCompareAndSwapReturnStatusSymbol)) {
            return inlineAtomicCompareAndMemoryUpdate(node, false, cg);
        }
        if (comp->getSymRefTab()->isNonHelper(SymRef,
                TR::SymbolReferenceTable::atomicCompareAndSwapReturnValueSymbol)) {
...
void OMR::X86::TreeEvaluator::compareGPRegisterToConstantForEquality(TR::Node *node, int32_t value,
    TR::Register *cmpRegister, TR::CodeGenerator *cg)
{
    if (value == 0) {
...
TR::Register *OMR::X86::TreeEvaluator::integerRegLoadEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().is64Bit() && node->getOpCodeValue() == TR::iRegLoad
        && performTransformation(comp, "TREE EVALUATION: setUpperBitsAreZero on iRegLoad %s\n",
...
TR::Register *OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
        ...
        if (node->getNumChildren() > 0)
            labelInst = generateLabelInstruction(TR::InstOpCode::label, node, generateLabelSymbol(cg),
                node->getFirstChild(), true, cg);
        else
        ...
        if (cg->enableRematerialisation() && !cg->getLiveDiscardableRegisters().empty()) {
            if (debug("dumpRemat"))
                diagnostic("\n---> Deleting surviving discardable registers at BBEnd [" POINTER_PRINTF_FORMAT "]",
                    node);
...
TR::Register *OMR::X86::TreeEvaluator::PrefetchEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (prefetchOp.getOpCodeValue() != TR::InstOpCode::bad) {
        ...
        if (secondChild->getOpCode().isLoadConst()) {
            ...
            generateMemInstruction(prefetchOp.getOpCodeValue(), node, generateX86MemoryReference(addrReg, offset, cg),
                cg);
            ...
            generateMemInstruction(prefetchOp.getOpCodeValue(), node,
                generateX86MemoryReference(addrReg, offsetReg, 0, cg), cg);
    ...
    TR::Register *rhs, TR::MemoryReference *mr, TR::CodeGenerator *cg)
{
    ...
    if (cmpEncoding >= OMR::X86::EVEX_L128) {
        ...
        generateRegMaskRegRegImmInstruction(cmpOpcode.getMnemonic(), node, maskReg, dummyMaskReg, lhs, lhs, 0x4, cg,
            cmpEncoding);
...
TR::Register *OMR::X86::TreeEvaluator::vectorCompareEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F)) {
        ...
        generateRegMaskRegRegImmInstruction(cmpOpcode.getMnemonic(), node, resultReg, maskReg, lhsReg, rhsReg,
            predicate, cg, cmpEncoding);
        ...
        if (cmpEncoding == OMR::X86::Legacy || type.getVectorElementType().isFloatingPoint()) {
            ...
            if (type.getVectorElementType().isFloatingPoint()) {
                generateRegRegImmInstruction(cmpOpcode.getMnemonic(), node, resultReg, rhsReg, predicate, cg,
                    cmpEncoding);
            } else {
    ...
    if (cg->comp()->target().cpu.supportsAVX()) {
        if (useRegMemForm && tmpNaNReg) {
            TR::Register *rSrcReg = tmpNaNReg
                ...
                : rhsReg;
            generateRegRegRegInstruction(nativeOpcode.getMnemonic(), node, resultReg, lhsReg, rSrcReg, cg,
                simdEncoding);
...
TR::Register *OMR::X86::TreeEvaluator::floatingPointBinaryArithmeticEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    switch (opcode) {
        ...
        case TR::ddiv:
            ...
            break;
        default:
            TR_ASSERT_FATAL_WITH_NODE(node, false, "Unsupported OpCode %s",
                cg->comp()->getDebug()->getName(node->getOpCode()));
    }
    ...
    TR_ASSERT_FATAL(opCode != TR::InstOpCode::bad,
        "floatingPointBinaryArithmeticEvaluator: unsupported data type or arithmetic.");

    if (cg->comp()->target().cpu.supportsAVX()) {
        if (useRegMemForm)
            generateRegRegMemInstruction(opCode, node, resultReg, operandReg0,
                generateX86MemoryReference(operandNode1, cg), cg);
        else
...
TR::Register *OMR::X86::TreeEvaluator::bitpermuteEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (length->getOpCode().isLoadConst()) {
        ...
        for (uintptr_t x = 0; x < arrayLen; ++x) {
            ...
            if (x > 0)
                generateRegImmInstruction(TR::InstOpCode::SHLRegImm1(nodeIs64Bit), node, tmpReg,
                    static_cast<int32_t>(x), cg);
...
TR::Register *OMR::X86::TreeEvaluator::mToLongBitsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (maskReg->getKind() == TR_VMR) {
        ...
        TR_ASSERT_FATAL_WITH_NODE(maskNode, maskReg->getKind() == TR_VRF,
            "Expected mask register kind of TR_VMR or TR_VRF");
...
TR::Register *OMR::X86::TreeEvaluator::vfmaEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (et.isFloatingPoint() && fmaEncoding != OMR::X86::Encoding::Bad) {
        if (maskReg) {
            ternaryVectorMaskHelper(fmaOpcode.getMnemonic(), fmaEncoding, node, resultReg, leftReg, middleReg, rightReg,
                maskReg, cg);
        } else {
            generateRegRegInstruction(movOpcode.getMnemonic(), node, resultReg, leftReg, cg, movOpcodeEncoding);
            generateRegRegRegInstruction(fmaOpcode.getMnemonic(), node, resultReg, middleReg, rightReg, cg,
                fmaEncoding);
        }
    } else {
        ...
        TR_ASSERT_FATAL(mulEncoding != OMR::X86::Encoding::Bad,
            "No supported encoding method for multiplication opcode");
        ...
        if (mulEncoding == OMR::X86::Legacy) {
            ...
            generateRegRegRegInstruction(mulOpcode.getMnemonic(), node, tmpResultReg, leftReg, middleReg, cg,
                mulEncoding);
        }
...
TR::Register *OMR::X86::TreeEvaluator::SIMDreductionEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::InstOpCode regOpcode = getNativeSIMDOpcode(OMR::ILOpCode::reductionToVerticalOpcode(node->getOpCodeValue(), vl),
        child->getDataType(), false);
    ...
    switch (vl) {
        case TR::VectorLength512:
            ...
            generateRegRegImmInstruction(TR::InstOpCode::VEXTRACTF64X4YmmZmmImm1, node, resultVRF, workingReg, 0xFF,
                cg);
            rSrcReg
                = needsNaNHandling ? vectorFPNaNHelper(child, tmpNaNReg, workingReg, resultVRF, NULL, cg) : resultVRF;
        ...
        case TR::VectorLength128: {
            ...
            if (dt != TR::Int64 && dt != TR::Double) {
                ...
                rSrcReg = needsNaNHandling ? vectorFPNaNHelper(child, tmpNaNReg, resultVRF, workingReg, NULL, cg)
                                           : workingReg;
                generateRegRegInstruction(regOpcode.getMnemonic(), node, resultVRF, rSrcReg, cg, regOpcodeEncoding128);
                if (dt != TR::Int32 && dt != TR::Float) {
                    // reduce from 32-bit to 16-bit
                    generateRegRegImmInstruction(TR::InstOpCode::PSHUFLWRegRegImm1, node, workingReg, resultVRF, 0x1,
                        cg);
                    generateRegRegInstruction(regOpcode.getMnemonic(), node, resultVRF, workingReg, cg,
                        regOpcodeEncoding128);
                    if (dt != TR::Int16) {
    ...
    TR::Register *maskReg, TR::CodeGenerator *cg)
{
    ...
    if (vectorMask) {
        ...
        generateRegMaskRegRegInstruction(opcode.getMnemonic(), node, resultReg, maskReg, middleReg, rhsReg, cg,
            encoding);
    }
    ...
    if (maskReg->getKind() == TR_VRF) {
        ...
        if (zeroMask) {
            if (cg->comp()->target().cpu.supportsAVX() && andEncoding != OMR::X86::Legacy) {
                generateRegRegRegInstruction(andOpcode.getMnemonic(), node, resultReg, srcReg, maskReg, cg,
                    andEncoding);
            } else {
        ...
        TR_ASSERT_FATAL(cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F),
            "Native merge masking requires AVX-512");
        ...
        generateRegMaskRegInstruction(movOpcode.getMnemonic(), node, resultReg, maskReg, srcReg, cg, movEncoding,
            zeroMask);
    }
...
TR::Register *OMR::X86::TreeEvaluator::arrayToVectorMaskHelper(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (nativeMasking) {
        ...
        generateRegImmInstruction(shiftOp.getMnemonic(), node, tmpVectorReg, shiftAmount, cg, TR_NoRelocation,
            shiftEncoding);
...
TR::Register *OMR::X86::TreeEvaluator::vmulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (type.getVectorElementType() == TR::Int8) {
        ...
        TR_ASSERT_FATAL(unpackHEncoding != OMR::X86::Encoding::Bad,
            "No suitable encoding form for punpckhbw instruction");
        TR_ASSERT_FATAL(unpackLEncoding != OMR::X86::Encoding::Bad,
            "No suitable encoding form for punpcklbw instruction");
        ...
        switch (vl) {
            ...
            case TR::VectorLength256: {
                TR_ASSERT_FATAL(cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX2),
                    "256-bit broadcast requires AVX2");
        ...
        if (cg->comp()->target().cpu.supportsAVX()) {
            generateRegRegRegInstruction(unpackLOpcode.getMnemonic(), node, lhsTmpReg, lhsReg, zeroReg, cg,
                unpackLEncoding);
            generateRegRegRegInstruction(unpackLOpcode.getMnemonic(), node, resultReg, rhsReg, zeroReg, cg,
                unpackLEncoding);
        } else {
        ...
        if (cg->comp()->target().cpu.supportsAVX()) {
            generateRegRegRegInstruction(unpackHOpcode.getMnemonic(), node, resultReg, lhsReg, zeroReg, cg,
                unpackHEncoding);
            generateRegRegRegInstruction(unpackHOpcode.getMnemonic(), node, rhsTmpReg, rhsReg, zeroReg, cg,
                unpackHEncoding);
        } else {
            generateRegRegInstruction(TR::InstOpCode::MOVDQURegReg, node, resultReg, lhsReg, cg);
        ...
        if (cg->comp()->target().cpu.supportsAVX()) {
            generateRegRegRegInstruction(packOpcode.getMnemonic(), node, resultReg, lhsTmpReg, rhsTmpReg, cg,
                packEncoding);
        } else {
