...
uint8_t* TR::X86FPConversionSnippet::emitCallToConversionHelper(uint8_t* buffer)
{
    ...
    *buffer++ = 0xe8; // CallImm4
    ...
    if (cg()->directCallRequiresTrampoline(helperAddress, callInstructionAddress)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            getHelperSymRef()->getReferenceNumber(), (void*)buffer);
    ...
    *(int32_t*)buffer = (int32_t)(helperAddress - nextInstructionAddress);
    cg()->addProjectSpecializedRelocation(
        buffer, (uint8_t*)getHelperSymRef(), NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
    buffer += 4;
...
uint8_t* TR::X86FPConvertToIntSnippet::genFPConversion(uint8_t* buffer)
{
    ...
    TR_ASSERT(cg()->getProperties().getIntegerReturnRegister() == TR::RealRegister::eax,
        "Only support integer return in eax");
    ...
    *buffer++ = 0xec;
    ...
    if (_convertInstruction->getIA32RegMemInstruction()) {
        ...
        *buffer++ = 0x24;
        ...
        TR_ASSERT(
            instr != NULL, "f2i conversion instruction must be either TR::InstOpCode::L4RegMem or CVTTSS2SIRegReg\n");
...
//
const uint8_t TR::X86FPConvertToLongSnippet::_registerActions[16] = { 0x1e, // 0
    0x14, // 1
    0x0e, // 2
    0x00, // 3
    0x13, // 4
