...
TR::Register* TR_X86IntegerMultiplyDecomposer::decomposeIntegerMultiplier(
    int32_t& tempRegArraySize, TR::Register** tempRegArray)
{
    ...
    if (disable && absMultiplier != 3 && absMultiplier != 5 && absMultiplier != 9 && !isPowerOf2(absMultiplier)) {
    ...
    if (decompositionIndex != -1) {
        ...
        decompositionIndex = findDecomposition(
        ...
        if (decompositionIndex != -1) {
            ...
            if (composition._subsequentShiftTooExpensive == false) {
                ...
                TR_ASSERT_FATAL(comp->compileRelocatableCode() || comp->isOutOfProcessCompilation()
                        || comp->compilePortableCode()
                        || comp->target().cpu.is(OMR_PROCESSOR_X86_AMD_OPTERON)
                            == cg()->getX86ProcessorInfo().isAMDOpteron(),
                    "isAMDOpteron failed\n");
                ...
                if (_multiplier
                    != absMultiplier) // treats TR::getMinSigned<TR::Int64>() properly, _multiplier < 0 does not
                {
                    generateRegInstruction(TR::InstOpCode::NEGReg(nodeIs64Bit), _node, target, _cg);
                }
                if (report) {
        ...
        if (onesCount == 2) // sum of 2 powers of 2
        {
            if (_sourceRegister) {
                if (!_canClobberSource && (absMultiplier & 1) == 0) {
                    ...
                    generateRegRegInstruction(
                        TR::InstOpCode::MOVRegReg(nodeIs64Bit), _node, temp, _sourceRegister, _cg);
            ...
            if (absMultiplier & 1) {
                ...
                generateRegImmInstruction(TR::InstOpCode::SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
                generateRegRegInstruction(
                    TR::InstOpCode::ADDRegReg(nodeIs64Bit), _node, _sourceRegister, _sourceRegister, _cg);
                generateRegRegInstruction(TR::InstOpCode::ADDRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
                ...
                generateRegImmInstruction(
                    TR::InstOpCode::SHLRegImm1(nodeIs64Bit), _node, _sourceRegister, shiftAmount, _cg);
                generateRegRegInstruction(TR::InstOpCode::ADDRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
            }
...
int32_t TR_X86IntegerMultiplyDecomposer::findDecomposition(int64_t multiplier)
{
    ...
    if ((i < NUM_CONSTS_DECOMPOSED) && (_integerMultiplySolutions[i]._multiplier == multiplier)) {
        ...
        if (registerCostOfDecomposition <= 1 || // mul would have a default register cost, too
            registerCostOfDecomposition < numGPRRegs - // total GPR regs available from machine
    ...
    int32_t index, int32_t& tempRegArraySize, TR::Register** tempRegArray)
{
    ...
    for (int32_t cursor = 0; cursor < MAX_NUM_COMPONENTS; ++cursor) {
        ...
        case shlRegImm:
            generateRegImmInstruction(TR::InstOpCode::SHLRegImm1(nodeIs64Bit), _node, registerMap[component._target],
                component._baseOrImmed, _cg);
            break;
        case addRegReg:
            generateRegRegInstruction(TR::InstOpCode::ADDRegReg(nodeIs64Bit), _node, registerMap[component._target],
                registerMap[component._baseOrImmed], _cg);
            break;
        case subRegReg:
            generateRegRegInstruction(TR::InstOpCode::SUBRegReg(nodeIs64Bit), _node, registerMap[component._target],
                registerMap[component._baseOrImmed], _cg);
            break;
        case movRegReg:
            generateRegRegInstruction(TR::InstOpCode::MOVRegReg(nodeIs64Bit), _node, registerMap[component._target],
                registerMap[component._baseOrImmed], _cg);
            break;
        case leaRegReg2:
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
                generateX86MemoryReference(
                    0, registerMap[component._index], TR::MemoryReference::convertMultiplierToStride(2), _cg),
                _cg);
            break;
        case leaRegReg4:
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
                generateX86MemoryReference(
                    0, registerMap[component._index], TR::MemoryReference::convertMultiplierToStride(4), _cg),
                _cg);
            break;
        case leaRegReg8:
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
                generateX86MemoryReference(
                    0, registerMap[component._index], TR::MemoryReference::convertMultiplierToStride(8), _cg),
                _cg);
            break;
        case leaRegRegReg:
            generateRegMemInstruction(TR::InstOpCode::LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
        ...
        case leaRegRegReg8:
            ...
            break;
        case done: {
            if (component._target != 0 && registerMap[0] != _node->getFirstChild()->getRegister()) {
...
const TR_X86IntegerMultiplyDecomposer::integerMultiplyComposition
    TR_X86IntegerMultiplyDecomposer::_integerMultiplySolutions[NUM_CONSTS_DECOMPOSED]
    = {
          {
              1, // multiplier
