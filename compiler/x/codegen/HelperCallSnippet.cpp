...
TR::X86HelperCallSnippet::X86HelperCallSnippet(TR::CodeGenerator* cg, TR::Node* node, TR::LabelSymbol* restartlab,
    TR::LabelSymbol* snippetlab, TR::SymbolReference* helper, int32_t stackPointerAdjustment)
...
TR::X86HelperCallSnippet::X86HelperCallSnippet(TR::CodeGenerator* cg, TR::LabelSymbol* restartlab,
    TR::LabelSymbol* snippetlab, TR::Node* callNode, int32_t stackPointerAdjustment)
...
void TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t* buffer, TR::Node* child)
{
    TR::StaticSymbol* sym = child->getSymbol()->getStaticSymbol();

    if (cg()->comp()->getOption(TR_EnableHCR) && !child->getSymbol()->isClassObject()) {
...
uint8_t* TR::X86HelperCallSnippet::genHelperCall(uint8_t* buffer)
{
        ...
        for (; i < _callNode->getNumChildren(); ++i) {
            ...
            if (child->getOpCodeValue() == TR::loadaddr) {
                if (!child->getRegister()
                    || child->getRegister()
                        != deps->getPostConditions()->getRegisterDependency(registerArgs)->getRegister()) {
                    TR::StaticSymbol* sym = child->getSymbol()->getStaticSymbol();
            ...
            *buffer = 0x50;
            ...
            cg()->machine()
                ->getRealRegister(deps->getPostConditions()->getRegisterDependency(registerArgs++)->getRealRegister())
                ->setRegisterFieldInOpcode(buffer++);
        }
    }
    ...
    *(int32_t*)buffer = branchDisplacementToHelper(callInstructionAddress, getDestination(), cg());
    ...
    cg()->addProjectSpecializedRelocation(
        buffer, (uint8_t*)getDestination(), NULL, TR_HelperAddress, __FILE__, __LINE__, _callNode);
...
void TR_Debug::printBody(TR::FILE* pOutFile, TR::X86HelperCallSnippet* snippet, uint8_t* bufferPos)
{
    ...
    if (snippet->getCallNode()) {
        if (snippet->getOffset() != -1) {
            ...
            bool useDedicatedFrameReg
                = _comp->cg()->getLinkage()->getProperties().getAlwaysDedicateFramePointerRegister() != 0;
        ...
        TR::RegisterDependencyConditions* deps
            = snippet->getRestartLabel()->getInstruction()->getDependencyConditions();
        int32_t registerArgs = 0;
...
uint32_t TR::X86HelperCallSnippet::getLength(int32_t estimatedSnippetStart)
{
    ...
    if (_callNode) {
        ...
        if (_offset != -1) {
            bool useDedicatedFrameReg
                = cg()->getLinkage()->getProperties().getAlwaysDedicateFramePointerRegister() != 0;
            if (_offset >= -128 && _offset <= 127)
...
int32_t TR::X86HelperCallSnippet::branchDisplacementToHelper(
    uint8_t* callInstructionAddress, TR::SymbolReference* helper, TR::CodeGenerator* cg)
{
    ...
    if (cg->directCallRequiresTrampoline(helperAddress, (intptr_t)callInstructionAddress)) {
        helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
            helper->getReferenceNumber(), (void*)(callInstructionAddress + 1));
