...
public:
    ...
    enum TR_X86ProcessorVendors { TR_AuthenticAMD = 0x01, TR_GenuineIntel = 0x02, TR_UnknownVendor = 0x04 };
...
public:
    TR_X86PaddingTable(
        uint8_t biggestEncoding, uint8_t*** encodings, flags8_t flags, uint16_t sibMask, uint16_t prefixMask)
...
public:
    ...
    typedef enum { Backward = 0, Forward = 1 } RegisterAssignmentDirection;
    ...
    void doBackwardsRegisterAssignment(
        TR_RegisterKinds kindsToAssign, TR::Instruction* startInstruction, TR::Instruction* appendInstruction = NULL);
    ...
    void performNonLinearRegisterAssignmentAtBranch(
    ...
    void processClobberingInstructions(
    ...
    void addMetaDataForBranchTableAddress(
