    ...
    TR::InstOpCode::Mnemonic regMemOpCode, TR::InstOpCode::Mnemonic memRegOpCode)
{
    integerCompareAnalyser(
        root, root->getFirstChild(), root->getSecondChild(), true, regRegOpCode, regMemOpCode, memRegOpCode);
}
    ...
    if (getCmpReg1Reg2()) {
        ...
        if (firstHighZero) {
            ...
            generateRegRegInstruction(
                TR::InstOpCode::CMP4RegReg, root, firstRegister->getHighOrder(), secondRegister->getHighOrder(), cg());
        }
    ...
    TR::LabelSymbol* secondBranchLabel, TR::InstOpCode::Mnemonic secondBranchOp)
{
    ...
    if (getCmpReg1Reg2()) {
        generateRegRegInstruction(
            TR::InstOpCode::CMP4RegReg, root, firstRegister->getLowOrder(), secondRegister->getLowOrder(), cg());
        if (deps != NULL) {
            deps->addPostCondition(firstRegister->getHighOrder(), TR::RealRegister::NoReg, cg());
...
TR::Register* TR_X86CompareAnalyser::longEqualityBooleanAnalyser(
...
TR::Register* TR_X86CompareAnalyser::longOrderedBooleanAnalyser(
