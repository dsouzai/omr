...
void TR::X86SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (properties.getAlwaysDedicateFramePointerRegister()) {
        cursor = new (trHeapMemory()) TR::X86RegInstruction(
            cursor, TR::InstOpCode::PUSHReg, machine()->getRealRegister(properties.getFramePointerRegister()), cg());
    ...
    if (properties.getAlwaysDedicateFramePointerRegister()) {
        debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
            comp(), properties.getOffsetToFirstLocal(), properties.getGPRWidth(), "EBP-save", debugFrameSlotInfo);
    }
    ...
    for (autoCursor = autoIterator.getFirst(); autoCursor != NULL; autoCursor = autoIterator.getNext()) {
        debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
            comp(), autoCursor->getOffset(), autoCursor->getSize(), "Local", debugFrameSlotInfo);
    }
...
void TR::X86SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (comp()->getOption(TR_TraceCG)) {
        traceMsg(
            comp(), "create epilogue using system linkage, after restorePreservedRegisters, cursor is %x.\n", cursor);
    }
    ...
    if (_properties.getAlwaysDedicateFramePointerRegister()) {
        ...
        cursor = new (trHeapMemory()) TR::X86RegInstruction(
            cursor, TR::InstOpCode::POPReg, machine()->getRealRegister(_properties.getFramePointerRegister()), cg());
    } else if (allocSize == 0) {
        // No need to do anything
    } else if (allocSize == singleWordSize) {
        TR::RealRegister* realReg = getSingleWordFrameAllocationRegister();
...
int32_t TR::X86SystemLinkage::layoutTypeOnStack(
    TR::DataType type, int32_t& dataCursor, TR::parmLayoutResult& layoutResult)
{
