...
void OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol* method)
{
    ...
    int32_t* colourToOffsetMap = (int32_t*)self()->trMemory()->allocateStackMemory(
        self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(int32_t));

    uint32_t* colourToSizeMap = (uint32_t*)self()->trMemory()->allocateStackMemory(
        self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(uint32_t));
    ...
    for (IGNodeIndex i = 0; i < self()->cg()->getLocalsIG()->getNumNodes(); i++) {
        ...
        TR_ASSERT(
            colour != UNCOLOURED, "uncoloured local %p (igNode=%p) found in locals IG\n", igNode->getEntity(), igNode);

        if ((colour != UNCOLOURED)
            && ((size = ((TR::Symbol*)(igNode->getEntity()))->castToAutoSymbol()->getRoundedSize())
                > colourToSizeMap[colour])) {
        ...
        if (localCursor->getGCMapIndex() >= 0) {
            ...
            if (igNode) {
                ...
                if (localCursor->isInternalPointer() || localCursor->isPinningArrayPointer()
                    ...
                    performSharing = performTransformation(
                        self()->comp(), "O^O COMPACT LOCALS: Sharing slot for local %p\n", localCursor);
...
#ifdef DEBUG
                if (debug("reportCL")) {
                    diagnostic("%s ref local %p (colour=%d): %s\n", isFirst ? "First" : "Shared", localCursor, colour,
                        self()->comp()->signature());
                    isFirst = false;
                }
#endif
        ...
        if (localCursor->getGCMapIndex() >= 0) {
            ...
            if (self()->cg()->comp()->getOption(TR_TraceRA))
                traceMsg(self()->comp(), "\nmapCompactedStack: changing %s (GC index %d) offset from %d to %d",
                    self()->comp()->getDebug()->getName(localCursor), localCursor->getGCMapIndex(),
                    localCursor->getOffset(), newOffset);
...
void OMR::X86::Linkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
    return self()->mapSingleAutomatic(p, p->getRoundedSize(), stackIndex);
}

void OMR::X86::Linkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t size, uint32_t& stackIndex)
{
...
void OMR::X86::Linkage::stopUsingKilledRegisters(TR::RegisterDependencyConditions* deps, TR::Register* returnRegister)
{
    ...
    for (int32_t i = deps->getNumPostConditions() - 1; i >= 0; --i) {
        ...
        if (!regDep->isNoReg()) {
            ...
            if (reg && reg != firstReturnRegister && reg != secondReturnRegister && reg != vmThreadReg)
                self()->cg()->stopUsingRegister(reg);
        }
    }
}

void OMR::X86::Linkage::associatePreservedRegisters(
    TR::RegisterDependencyConditions* deps, TR::Register* returnRegister)
{
    TR::Register* vmThreadReg = self()->cg()->getVMThreadRegister();
...
TR::Register* OMR::X86::Linkage::findReturnRegisterFromDependencies(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR::MethodSymbol* methodSymbol, bool isDirect)
{
    ...
    case TR::Address:
        ...
        break;
    case TR::Int64:
        returnRegister = self()->cg()->allocateRegisterPair(postConditions->getRegisterDependency(0)->getRegister(),
    ...
    case TR::Double: {
        ...
        //
        int depNum = ((methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative())
                         && methodSymbol->preservesAllRegisters())
            ? 0
            : 3;
...
//
void OMR::X86::Linkage::coerceFPReturnValueToXMMR(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies,
    TR::MethodSymbol* methodSymbol, TR::Register* returnReg)
{
    TR::TreeEvaluator::coerceST0ToFPR(callNode, callNode->getDataType(), self()->cg(), returnReg);
}

uint32_t OMR::X86::Linkage::getRightToLeft() { return self()->getProperties().passArgsRightToLeft(); }

uint8_t* OMR::X86::Linkage::loadArguments(
    TR::Node* callNode, uint8_t* cursor, bool calculateSizeOnly, int32_t* sizeOfFlushArea, bool isReturnAddressOnStack)
{
    return self()->flushArguments(callNode, cursor, calculateSizeOnly, sizeOfFlushArea, isReturnAddressOnStack, true);
}

uint8_t* OMR::X86::Linkage::storeArguments(
...
TR::Instruction* OMR::X86::Linkage::loadArguments(TR::Instruction* prev, TR::ResolvedMethodSymbol* methodSymbol)
{
    return self()->flushArguments(prev, methodSymbol, true, true);
}

TR::Instruction* OMR::X86::Linkage::storeArguments(TR::Instruction* prev, TR::ResolvedMethodSymbol* methodSymbol)
{
    return self()->flushArguments(prev, methodSymbol, true, false);
}

int32_t OMR::X86::Linkage::numArgumentRegisters(TR_RegisterKinds kind)
{
    switch (kind) {
    case TR_GPR:
...
TR_RegisterKinds OMR::X86::Linkage::movRegisterKind(TR_MovDataTypes mdt)
{
    return self()->isFloat(mdt) ? TR_FPR : TR_GPR;
}

TR_MovDataTypes OMR::X86::Linkage::paramMovType(TR::ParameterSymbol* param)
{
