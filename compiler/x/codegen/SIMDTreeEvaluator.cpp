...
static TR::MemoryReference* ConvertToPatchableMemoryReference(
    TR::MemoryReference* mr, TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (maskReg && maskReg->getKind() == TR_VMR) {
        ...
        if (maskReg) {
            ...
            OMR::X86::Encoding andEncoding
                = andOpcode.getSIMDEncoding(&cg->comp()->target().cpu, node->getDataType().getVectorLength());
...
TR::Register* OMR::X86::TreeEvaluator::SIMDstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    case 64:
        TR_ASSERT_FATAL(
            cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F), "512-bit vstore requires AVX-512");
        encoding = EVEX_L512;
        break;
    default:
        if (cg->comp()->getOption(TR_TraceCG))
...
TR::Register* OMR::X86::TreeEvaluator::SIMDsplatsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    case TR::VectorLength128:
        generateRegRegImmInstruction(
            TR::InstOpCode::PSHUFDRegRegImm1, node, resultReg, resultReg, broadcast64 ? 0x44 : 0, cg);
        break;
    case TR::VectorLength256: {
        TR_ASSERT_FATAL(
            cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX2), "256-bit vsplats requires AVX2");
    ...
    case TR::VectorLength512: {
        TR_ASSERT_FATAL(
            cg->comp()->target().cpu.supportsFeature(OMR_FEATURE_X86_AVX512F), "512-bit vsplats requires AVX-512");
...
TR::Register* OMR::X86::TreeEvaluator::SIMDvgetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    case TR::Int16:
        TR_ASSERT(
            false, "unsupported vector type %s in SIMDvgetelemEvaluator.\n", firstChild->getDataType().toString());
        break;
    case TR::Int32:
        elementCount = 4;
    ...
    default:
        TR_ASSERT(
            false, "unrecognized vector type %s in SIMDvgetelemEvaluator.\n", firstChild->getDataType().toString());
    }
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (4 == elementCount) {
            ...
            /*
             * the value to be read (indicated by shufconst) from srcVectorReg is splatted into all 4 slots in the
             * dstReg this puts the value we want in the least significant bits and the other bits should never be read.
            ...
            if (3 == elem) {
                ...
                generateRegRegImmInstruction(
                    TR::InstOpCode::PSHUFDRegRegImm1, node, dstReg, srcVectorReg, shufconst, cg);
            }
        ...
        } else // 2 == elementCount
        {
            ...
            if (TR::Int64 == firstChild->getDataType().getVectorElementType()) {
                if (cg->comp()->target().is32Bit()) {
                    ...
                    generateRegRegImmInstruction(
                        TR::InstOpCode::PSHUFDRegRegImm1, node, dstReg, srcVectorReg, (0 == elem) ? 0x03 : 0x01, cg);
