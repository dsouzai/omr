...
public:
    ...
    //
    uint8_t* loadArguments(TR::Node* callNode, uint8_t* cursor, bool calculateSizeOnly, int32_t* sizeOfFlushArea,
    ...
    void ensureMinimumFirstInstructionSize(uint8_t mifs)
    {
        _minimumFirstInstructionSize = std::max(_minimumFirstInstructionSize, mifs);
    }

    bool isFloat(TR_MovDataTypes mdt) { return (mdt == Float4 || mdt == Float8); }
...
protected:
    ...
    TR::Register* findReturnRegisterFromDependencies(
        TR::Node*, TR::RegisterDependencyConditions*, TR::MethodSymbol*, bool);
