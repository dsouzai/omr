...
OMR::X86::Instruction::Instruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, OMR::X86::Encoding encoding)
...
void OMR::X86::Instruction::initialize(
    TR::CodeGenerator* cg, TR::RegisterDependencyConditions* cond, TR::InstOpCode::Mnemonic op, bool flag)
{
    ...
    if (self()->getOpCodeValue() != TR::InstOpCode::assocreg) {
        if ((self()->cg()->getAssignmentDirection() == self()->cg()->Backward)) {
            ...
            self()->getDependencyConditions()->assignPreConditionRegisters(
                self()->getPrev(), kindsToBeAssigned, self()->cg());
            self()->getDependencyConditions()->assignPostConditionRegisters(self(), kindsToBeAssigned, self()->cg());
        }
...
void OMR::X86::Instruction::adjustVFPStateForCall(
    TR_VFPState* state, int32_t vfpAdjustmentForCall, TR::CodeGenerator* cg)
{
...
uint8_t OMR::X86::Instruction::rexRepeatCount() { return _rexRepeatCount; }
