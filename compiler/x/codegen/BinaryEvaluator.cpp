...
bool OMR::X86::TreeEvaluator::analyseSubForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
        ...
        if (firstOp.isAdd() && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &dummyConstValue, cg)) {
            // sub
            //    add         (firstChild)
            //       ?        (llChild)
            //       ?        (lrchild)
            ...
            if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1
                ...
                //    const       (secondChild)
                //
                leaMR = generateX86MemoryReference(
                    cg->evaluate(lrChild), cg->evaluate(llChild->getFirstChild()), stride, displacement, cg);
                ...
                //
                leaMR = generateX86MemoryReference(
                    cg->evaluate(llChild), cg->evaluate(lrChild->getFirstChild()), stride, displacement, cg);
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((secondOp.isAdd() || secondOp.isSub()) && secondChild->getReferenceCount() == 1
        ...
        if (stride && addFirstChild->getReferenceCount() == 1 && addFirstChild->getRegister() == NULL
    ...
    if (indexNode) {
        ...
        node->setRegister(targetRegister);
        return true;
    } else if (constNode && TR::TreeEvaluator::constNodeValueIs32BitSigned(constNode, &dummyConstValue, cg)) {
        // add
        //    firstChild
        //    const
        //
        if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
            // add
            //    add   (firstChild)
            //       ?
            //       ?
            ...
            if (stride1 | stride2) {
                if (stride1) {
                    ...
                    TR_ASSERT(
                        node->getSize() == indexNode->getSize(), "evaluateAndForceSize never needed for indexNode");
        ...
        targetRegister = TR::TreeEvaluator::generateLEAForLoadAddr(
            firstChild, leaMR, firstChild->getSymbolReference(), cg, isInternal);
        cg->decReferenceCount(firstChild);
        node->setRegister(targetRegister);
        return true;
    } else if ((firstChild->getOpCode().isSub() || firstChild->getOpCode().isAdd())
        && (firstChild->getOpCode().getSize() == node->getOpCode().getSize()) && (firstChild->getRegister() == NULL)
        ...
        if (firstChild->getOpCode().isSub())
            val = -val;
        leaMR = generateX86MemoryReference(
            cg->evaluate(firstChild->getFirstChild()), cg->evaluate(secondChild), 0, val, cg);
        targetRegister = cg->allocateRegister();
        generateRegMemInstruction(TR::InstOpCode::LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
        node->setRegister(targetRegister);
        cg->decReferenceCount(firstChild->getFirstChild());
    ...
    if (pair->getReferenceCount() == 1) {
        ...
        if (node->isDualHigh()) {
            ...
            diagnostic("Evaluating Children of %p (%s) for %p (%s) (not required for quad calc)\n", pair,
                pair->getOpCode().getName(), node, node->getOpCode().getName());
            evaluateIfNotConst(pair->getFirstChild(), cg);
            evaluateIfNotConst(pair->getSecondChild(), cg);
        ...
        diagnostic(
            "Evaluating %p (%s) after %p (%s)\n", node, node->getOpCode().getName(), pair, pair->getOpCode().getName());
        ...
        diagnostic("Found %p (%s) for %p (%s)\n", pair, pair->getOpCode().getName(), node, node->getOpCode().getName());
        ...
        diagnostic("Evaluating Children of %p (%s) for %p (%s)\n", lowNode, lowNode->getOpCode().getName(), highNode,
            highNode->getOpCode().getName());
        ...
        diagnostic("Evaluating Children of %p (%s) after those for %p (%s)\n", highNode,
            highNode->getOpCode().getName(), lowNode, lowNode->getOpCode().getName());
        ...
        cg->setComputesCarry(true);
        cg->evaluate(lowNode);
        diagnostic("Evaluating %p (%s) after %p (%s)\n", highNode, highNode->getOpCode().getName(), lowNode,
            lowNode->getOpCode().getName());
        cg->setRequiresCarry(true);
        cg->setComputesCarry(true);
...
TR::Register* OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
        TR_ASSERT((nodeIs64Bit && opCode == TR::ladd || opCode == TR::luaddc)
            ...
            cg->comp()->getDebug()->getName(opCode));

        // we need eflags from integerAddAnalyser for the CC sequence
        const bool isWithCarry = (opCode == TR::luaddc) || (opCode == TR::iuaddc);
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node,
            TR::InstOpCode::AddRegReg(nodeIs64Bit, isWithCarry), TR::InstOpCode::AddRegMem(nodeIs64Bit, isWithCarry),
            true, /* produce eflags */
    ...
    if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
        ...
        intptr_t constValue;
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
            && ((TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg))
                || (comp->useCompressedPointers() && (constValue == 0) && firstChild->getReferenceCount() > 1
                    && !isMemOp))
            && performTransformation(
                comp, "O^O analyseAddForLEA: checking that second node is a memory reference %x\n", constValue)) {
            if (!isMemOp)
                targetRegister = cg->evaluate(firstChild);

            bool internalPointerMismatch = false;

            if (targetRegister && node->getOpCode().isArrayRef() && node->isInternalPointer()) {
            ...
            if (targetRegister
                ...
                // still
                //
                if (targetRegister->containsCollectedReference() && (node->getOpCode().isArrayRef())
                ...
                if (computesCarry) {
                    generateRegRegInstruction(
                        TR::InstOpCode::MOVRegReg(nodeIs64Bit), node, targetRegister, tempReg, cg);
                ...
                if (constValue >= -128 && constValue <= 127) {
                    if (computesCarry) {
                        ...
                        if (isMemOp)
                            ...
                            instr = generateRegImmInstruction(
                                TR::InstOpCode::ADDRegImms(nodeIs64Bit), node, targetRegister, 1, cg);
                    } else if (constValue == -1) {
                        if (isMemOp)
                            instr = generateMemInstruction(TR::InstOpCode::DECMem(nodeIs64Bit), node, tempMR, cg);
                        else
                            instr = generateRegImmInstruction(
                                TR::InstOpCode::SUBRegImms(nodeIs64Bit), node, targetRegister, 1, cg);
                    } else {
                        if (isMemOp)
                    ...
                    if (isMemOp)
                        instr = generateMemImmInstruction(
                            TR::InstOpCode::SUBMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
                    else
                        instr = generateRegImmInstruction(
                            TR::InstOpCode::SUBRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
                } else {
                    // comp()->useCompressedPointers
                    // ladd
                    //    iu2l
            ...
            instr = generateMemRegInstruction(
                TR::InstOpCode::AddMemReg(nodeIs64Bit, isWithCarry), node, tempMR, cg->evaluate(secondChild), cg);
            if (debug("traceMemOp"))
        ...
        if (isMemOp) {
            ...
            temp.integerAddAnalyser(node, TR::InstOpCode::AddRegReg(nodeIs64Bit, isWithCarry),
                TR::InstOpCode::AddRegMem(nodeIs64Bit, isWithCarry), computesCarry, 0);
            targetRegister = node->getRegister();
        }
    }
...
TR::Register* OMR::X86::TreeEvaluator::baddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node)) {
        TR_ASSERT(node->getOpCodeValue() == TR::badd, "CC computation not supported for this node %p with opcode %s\n",
            node, cg->comp()->getDebug()->getName(node->getOpCode()));

        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
            node, TR::InstOpCode::ADD1RegReg, TR::InstOpCode::ADD1RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        && performTransformation(comp,
            "O^O BaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::saddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    if (NEED_CC(node)) {
        TR_ASSERT(node->getOpCodeValue() == TR::sadd, "CC computation not supported for this node %p with opcode %s\n",
            node, cg->comp()->getDebug()->getName(node->getOpCode()));

        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
            node, TR::InstOpCode::ADD2RegReg, TR::InstOpCode::ADD2RegMem, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
        && performTransformation(comp,
            "O^O SaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
        ...
        if (targetRegister && firstChild->getReferenceCount() > 1) {
            ...
            if (value >= -128 && value <= 127) {
                ...
                if (isMemOp)
                    ...
                    instr = generateRegImmInstruction(
                        TR::InstOpCode::SUB4RegImms, node, targetRegister, (uint32_t)-128, cg);
            } else {
                if (isMemOp)
...
TR::Register* OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) || (node->getOpCodeValue() == TR::iusubb)) {
        ...
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, TR::InstOpCode::SubRegReg(nodeIs64Bit, isWithBorrow),
            TR::InstOpCode::SubRegMem(nodeIs64Bit, isWithBorrow), TR::InstOpCode::MOVRegReg(nodeIs64Bit), true,
    ...
    intptr_t constValue;
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg)
        && performTransformation(comp,
            "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 bit constant. Register "
            "value: %x\n",
            testRegister)) {
        ...
        if (targetRegister && !computesCarry && firstChild->getReferenceCount() > 1) {
            ...
            if (constValue >= -128 && constValue <= 127) {
                ...
                if (isMemOp)
                    instr = generateMemImmInstruction(
                        TR::InstOpCode::ADDMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
                else
                    instr = generateRegImmInstruction(
                        TR::InstOpCode::ADDRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
            } else {
                if (isMemOp)
        ...
        instr = generateMemRegInstruction(
            TR::InstOpCode::SubMemReg(nodeIs64Bit, isWithBorrow), node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
    } else {
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, TR::InstOpCode::SubRegReg(nodeIs64Bit, isWithBorrow),
            TR::InstOpCode::SubRegMem(nodeIs64Bit, isWithBorrow), TR::InstOpCode::MOVRegReg(nodeIs64Bit), computesCarry,
            0);
...
TR::Register* OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node)) {
        TR_ASSERT(node->getOpCodeValue() == TR::bsub, "CC computation not supported for this node %p with opcode %s\n",
            node, cg->comp()->getDebug()->getName(node->getOpCode()));

        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, TR::InstOpCode::SUB1RegReg, TR::InstOpCode::SUB1RegMem,
            TR::InstOpCode::MOV1RegReg, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
        && performTransformation(comp,
            "O^O BSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
        int32_t value = secondChild->getByte();
        if (!isMemOp)
            targetRegister = cg->evaluate(firstChild);
        if (targetRegister && firstChild->getReferenceCount() > 1) {
        ...
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(
            node, TR::InstOpCode::SUB1RegReg, TR::InstOpCode::SUB1RegMem, TR::InstOpCode::MOV1RegReg);
        targetRegister = node->getRegister();
        countsAreDecremented = true;
    }
...
TR::Register* OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node)) {
        TR_ASSERT(node->getOpCodeValue() == TR::ssub, "CC computation not supported for this node %p with opcode %s\n",
            node, cg->comp()->getDebug()->getName(node->getOpCode()));

        // we need eflags from integerAddAnalyser for the CC sequence
        TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, TR::InstOpCode::SUB2RegReg, TR::InstOpCode::SUB2RegMem,
            TR::InstOpCode::MOV4RegReg, true /* produce eflags */);
        targetRegister = node->getRegister();
        return targetRegister;
    }
    ...
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
        && performTransformation(comp,
            "O^O SSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
        int32_t value = secondChild->getShortInt();
        if (!isMemOp)
            targetRegister = cg->evaluate(firstChild);
        if (firstChild->getReferenceCount() > 1) {
        ...
        TR_X86SubtractAnalyser temp(cg);
        temp.integerSubtractAnalyser(node, TR::InstOpCode::SUB4RegReg, TR::InstOpCode::SUB2RegMem,
            TR::InstOpCode::MOV4RegReg); // Use MOV4 to avoid unnecessary size prefix
        return node->getRegister();
    }
...
TR::Register* OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isDualCyclic() || needsHighMulOnly) {
        ...
        generateRegRegInstruction(
            TR::InstOpCode::MUL8AccReg, node, lmulTargetRegister, lumulhTargetRegister, multDependencies, cg);
...
TR::Register* OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (value = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg))) {
        if (value == 0) {
            if (firstChild->getReferenceCount() > 1) {
            ...
            TR_X86IntegerMultiplyDecomposer* mulDecomposer = new (cg->trHeapMemory())
                TR_X86IntegerMultiplyDecomposer(value, firstChild->getRegister(), node, cg, canClobberSource);
            int32_t dummy;
            ...
            if (targetRegister == 0 && IS_32BIT_SIGNED(value) && (dataType != TR::Int8)) // decomposition failed
            {
                ...
                if (firstChild->getReferenceCount() > 1 || firstChild->getRegister() != 0) {
                    if (value >= -128 && value <= 127) {
                    ...
                    generateRegRegImmInstruction(
                        opCode, node, targetRegister, cg->evaluate(firstChild), static_cast<int32_t>(value), cg);
                } else {
                    if (firstChild->getOpCode().isMemoryReference()) {
                        ...
                        targetRegister = cg->allocateRegister();
                        generateRegMemImmInstruction(
                            opCode, node, targetRegister, tempMR, static_cast<int32_t>(value), cg);
                        tempMR->decNodeReferenceCounts(cg);
                    } else {
                        if (value >= -128 && value <= 127) {
                            opCode = TR::InstOpCode::IMulRegRegImms(node->getSize());
                        ...
                        targetRegister = cg->evaluate(firstChild);
                        generateRegRegImmInstruction(
                            opCode, node, targetRegister, targetRegister, static_cast<int32_t>(value), cg);
                    }
                }
            }
        }
    ...
    if (!targetRegister && dataType == TR::Int8) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isMemoryReference()) {
            ...
            generateRegRegInstruction(
                TR::InstOpCode::IMUL1AccReg, node, targetRegister, tempRegister, multDependencies, cg);
        }
    }
    ...
    if (targetRegister) {
        ...
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.genericAnalyser(node, TR::InstOpCode::IMulRegReg(node->getSize()),
            TR::InstOpCode::IMulRegMem(node->getSize()), TR::InstOpCode::MovRegReg(node->getSize()));
        targetRegister = node->getRegister();
    }
...
TR::Register* OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) == 0) {
        // TODO: Why special-case this?  Simplifier should catch this.

        if (firstChild->getReferenceCount() > 1)
            cg->evaluate(firstChild); // Respect the evaluation point of the child -- TODO: this could be sub-optimal
                                      // and is likely best handled elsewhere
        cg->recursivelyDecReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    ...
    generateRegRegInstruction(
        TR::InstOpCode::IMULAccReg(nodeIs64Bit), node, eaxRegister, targetRegister, multDependencies, cg);
    ...
    if (isPowerOf2(dvalue)) {
        ...
        if (rootOpCode.isRem()) {
            ...
            if (isMinInt) {
                ...
                //
                generateRegRegInstruction(
                    TR::InstOpCode::TESTRegReg(nodeIs64Bit), node, tempRegister, tempRegister, cg);
                ...
                if (1 <= dvalue && dvalue <= 0x40000000) {
                    ...
                    generateRegImmInstruction(
                        TR::InstOpCode::AND8RegImm4, node, tempRegister, (uint32_t)(2 * dvalue - 1), cg);
            ...
            if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
                ...
                generateRegImmInstruction(
                    TR::InstOpCode::ORRegImm4(nodeIs64Bit), node, tempRegister, (uint32_t)-dvalue, cg);
        ...
        if (!isMinInt) {
            ...
            if ((dividend->isNonNegative() == false) && (dvalue > 0)) {
                ...
                {
                    generateRegRegInstruction(
                        TR::InstOpCode::SUBRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
                    ...
                    TR_ASSERT(mask >= 0,
                        ...
                        "of tempRegister");
                    generateRegImmInstruction(
                        TR::InstOpCode::ANDRegImm4(nodeIs64Bit), node, edxRegister, (uint32_t)dvalue - 1, cg);
                    generateRegRegInstruction(
                        TR::InstOpCode::ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
                } else {
        ...
        if (!nodeIs64Bit || IS_32BIT_SIGNED(m))
            generateRegImmInstruction(
                TR::InstOpCode::MOVRegImm4(nodeIs64Bit), node, eaxRegister, static_cast<int32_t>(m), cg);
        else
            generateRegMemInstruction(
                TR::InstOpCode::LEA8RegMem, node, eaxRegister, generateX86MemoryReference(m, cg), cg);
        generateRegRegInstruction(
            TR::InstOpCode::IMULAccReg(nodeIs64Bit), node, eaxRegister, dividendRegister, multDependencies, cg);
        ...
        //
        generateRegImmInstruction(
            TR::InstOpCode::SARRegImm1(nodeIs64Bit), node, edxRegister, static_cast<int32_t>(s), cg);
        ...
        if (!(dividend->isNonNegative() && (dvalue > 0))) {
            ...
            generateRegImmInstruction(
                TR::InstOpCode::SHRRegImm1(nodeIs64Bit), node, eaxRegister, nodeIs64Bit ? 63 : 31, cg);
        ...
        if (rootOpCode.isRem()) {
            ...
            generateRegRegInstruction(
                TR::InstOpCode::SUBRegReg(nodeIs64Bit), node, dividendRegister, edxRegister, multDependencies, cg);
            cg->stopUsingRegister(edxRegister);
...
TR::Register* OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
        ...
        } else if (secondChild->getReferenceCount() == 1 && !secondChild->getRegister()
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                divideInstr = generateRegRegInstruction(
                    TR::InstOpCode::DIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
            } else {
                TR::MemoryReference* tempMR = generateX86MemoryReference(secondChild, cg);
                divideInstr = generateRegMemInstruction(
                    TR::InstOpCode::DIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        } else {
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = generateRegRegInstruction(
                        TR::InstOpCode::IDIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
            } else {
                TR::MemoryReference* tempMR = generateX86MemoryReference(secondChild, cg);
                if (node->getFirstChild()->isNonNegative() || node->getOpCode().isUnsigned())
                    generateRegRegInstruction(TR::InstOpCode::XOR4RegReg, node, edxRegister, edxRegister, edxDeps, cg);
                ...
                if (node->getOpCode().isUnsigned()
                    ...
                    divideInstr = generateRegMemInstruction(
                        TR::InstOpCode::IDIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
                tempMR->decNodeReferenceCounts(cg);
            }
        }
        ...
        if (needsExplicitOverflowCheck) {
            ...
            cg->addSnippet(new (cg->trHeapMemory())
                    TR::X86DivideCheckSnippet(restartLabel, overflowSnippetLabel, divisionLabel, node->getOpCode(),
                        node->getType(), divideInstr->getX86RegInstruction()->getIA32RegRegInstruction(), cg));
        }
...
TR::X86RegInstruction* OMR::X86::TreeEvaluator::generateRegisterShift(TR::Node* node,
    TR::InstOpCode::Mnemonic immShiftOpCode, TR::InstOpCode::Mnemonic regShiftOpCode, TR::CodeGenerator* cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        intptr_t shiftAmount
            = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) & TR::TreeEvaluator::shiftMask(nodeIs64Bit);
        if (shiftAmount == 0) {
            targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild,
                (SHIFT_MAY_HAVE_ADDRESS_CHILD) ? TR::TreeEvaluator::getNodeIs64Bit(firstChild, cg) : nodeIs64Bit, cg);
        } else {
            targetRegister = cg->evaluate(firstChild);

            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift()
                && (targetRegister->containsCollectedReference() || node->containsCompressionSequence())) {
                TR::Register* origTargetRegister = targetRegister;
                targetRegister = cg->allocateRegister();
                instr = generateRegRegInstruction(
                    TR::InstOpCode::MOVRegReg(nodeIs64Bit), node, targetRegister, origTargetRegister, cg);
            } else {
                targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild,
            ...
            instr = generateRegImmInstruction(
                immShiftOpCode, node, targetRegister, static_cast<int32_t>(shiftAmount), cg);
        }
    } else {
        ...
        if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i || op == TR::l2i) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
                ...
                if (secondChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1
                    ...
                    if (reportShiftAmount)
                        diagnostic("shift: removed sign or zero extend to shift amount in memory in method %s\n",
                            comp->signature());
                } else if (secondChild->getOpCode().isLoadVarDirect() && grandChild->getReferenceCount() == 1
                    && grandChild->getRegister() == 0) {
                ...
                } else if (op != TR::l2i || comp->target().is64Bit()) // cannot use a reigster pair as the shift amount
                {
                    ...
                    if (reportShiftAmount)
                        diagnostic(
                            "shift: removed sign or zero extend to shift amount in method %s\n", comp->signature());
...
TR::X86MemInstruction* OMR::X86::TreeEvaluator::generateMemoryShift(TR::Node* node,
    TR::InstOpCode::Mnemonic immShiftOpCode, TR::InstOpCode::Mnemonic regShiftOpCode, TR::CodeGenerator* cg)
{
    ...
    if (loadConstant
        && performTransformation(comp, "O^O GenerateMemoryShift: load is not constant %d\n", loadConstant)) {
        intptr_t shiftAmount
            = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) & TR::TreeEvaluator::shiftMask(nodeIs64Bit);
        if (shiftAmount != 0) {
            if (debug("traceMemOp"))
        ...
        if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i || op == TR::l2i) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
                ...
                if (grandChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1
                    && grandChild->getRegister() == 0) {
                    TR::Node::recreate(grandChild, TR::bloadi);
                    secondChild->decReferenceCount();
                    secondChild = grandChild;
                    if (reportShiftAmount)
                        diagnostic("shift: removed sign or zero extend to shift amount in memory in method %s\n",
                            comp->signature());
                } else if (grandChild->getOpCode().isLoadVarDirect() && grandChild->getReferenceCount() == 1
                    && grandChild->getRegister() == 0) {
                    TR::Node::recreate(grandChild, TR::bload);
                    secondChild->decReferenceCount();
                    secondChild = grandChild;
...
TR::Register* OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(
            node, TR::InstOpCode::SHLMemImm1(nodeIs64Bit), TR::InstOpCode::SHLMemCL(nodeIs64Bit), cg);
    } else if (shiftAmountNode->getOpCode().isLoadConst()
        && (shiftAmount
            = TR::TreeEvaluator::integerConstNodeValue(shiftAmountNode, cg) & TR::TreeEvaluator::shiftMask(nodeIs64Bit),
        ...
        TR::TreeEvaluator::generateRegisterShift(
            node, TR::InstOpCode::SHLRegImm1(nodeIs64Bit), TR::InstOpCode::SHLRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register* OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        intptr_t rotateAmount
            = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) & TR::TreeEvaluator::rotateMask(nodeIs64Bit);
        if (rotateAmount == 0) {
            targetRegister = cg->evaluate(firstChild);
        } else {
            targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
            generateRegImmInstruction(
                TR::InstOpCode::ROLRegImm1(nodeIs64Bit), node, targetRegister, static_cast<int32_t>(rotateAmount), cg);
        }
    } else {
        targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
        ...
        generateRegRegInstruction(
            TR::InstOpCode::ROLRegCL(nodeIs64Bit), node, targetRegister, rotateAmountReg, rotateDependencies, cg);
    }
...
TR::Register* OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::TreeEvaluator::generateMemoryShift(
            node, TR::InstOpCode::SARMemImm1(nodeIs64Bit), TR::InstOpCode::SARMemCL(nodeIs64Bit), cg);
    } else {
        TR::TreeEvaluator::generateRegisterShift(
            node, TR::InstOpCode::SARRegImm1(nodeIs64Bit), TR::InstOpCode::SARRegCL(nodeIs64Bit), cg);
    }
    return node->getRegister();
}
...
TR::Register* OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::Instruction* shiftInstruction = TR::TreeEvaluator::generateMemoryShift(
            node, TR::InstOpCode::SHRMemImm1(nodeIs64Bit), TR::InstOpCode::SHRMemCL(nodeIs64Bit), cg);
        if (shiftInstruction)
            faultingInstruction = shiftInstruction;
    } else {
        TR::TreeEvaluator::generateRegisterShift(
            node, TR::InstOpCode::SHRRegImm1(nodeIs64Bit), TR::InstOpCode::SHRRegCL(nodeIs64Bit), cg);
    }

    if (comp->useCompressedPointers() && nodeIs64Bit && (node->getFirstChild()->getOpCodeValue() == TR::a2l)) {
...
TR::Register* OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testOpCode == TR::bconst
        && performTransformation(comp,
            "O^O BSHLEvaluator: second child is not an 8-bit signed, two's complement number: %x\n", testOpCode)) {
        int32_t value = secondChild->getByte();
        if (isMemOp) {
            if (value != 0) {
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SHL1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = generateRegRegInstruction(
                TR::InstOpCode::SHL1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
        }
    }
...
TR::Register* OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::ILOpCodes testOpcode = secondChild->getOpCodeValue();
    if (testOpcode == TR::iconst
        && performTransformation(
            comp, "O^O SSHLEvaluator: second child is not a 16-bit integer constant: %x\n", testOpcode)) {
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SHL2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
        } else {
            targetRegister = cg->intClobberEvaluate(firstChild);
            instr = generateRegRegInstruction(
                TR::InstOpCode::SHL4RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
        }
    }
...
TR::Register* OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testOpcode == TR::bconst
        && performTransformation(
            comp, "O^O BSHREvaluator: second child is not an 8-bit signed Two's complement opcode %x\n", testOpcode)) {
        int32_t value = secondChild->getByte();
        if (value != 0) {
            if (isMemOp) {
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SAR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = generateRegRegInstruction(
                TR::InstOpCode::SAR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register* OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testOpcode == TR::iconst
        && performTransformation(
            comp, "O^O SSHREvaluator: second child is not a 16-bit signed two's complement number %x\n", testOpcode)) {
        int32_t value = secondChild->getInt();
        if (value != 0) {
            if (isMemOp) {
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SAR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
        } else
            instr = generateRegRegInstruction(
                TR::InstOpCode::SAR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register* OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isMemOp) {
        ...
        && performTransformation(comp,
            "O^O BUSHREvaluator: first child is not an 8-bit signed two's complement, or an 8 bit unsigned %x\n",
            testOpcode1)) {
        targetRegister = cg->allocateRegister();
    ...
    if (testOpcode2 == TR::bconst
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SHR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = generateRegRegInstruction(
                TR::InstOpCode::SHR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::Register* OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testOpcode == TR::iconst
        && performTransformation(
            comp, "O^O SUSHREvaluator: opcode is not a 16-bit signed two's complement %x\n", testOpcode)) {
        int32_t value = secondChild->getInt();
        if (isMemOp) {
        ...
        if (isMemOp) {
            instr = generateMemRegInstruction(
                TR::InstOpCode::SHR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
            if (debug("traceMemOp"))
                diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
        } else
            instr = generateRegRegInstruction(
                TR::InstOpCode::SHR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
...
TR::InstOpCode::Mnemonic OMR::X86::TreeEvaluator::_logicalOpPackage[numLogicalOpPackages][numLogicalOpForms] = {
    // band
    { TR::InstOpCode::AND1RegReg, TR::InstOpCode::AND1RegMem, TR::InstOpCode::MOV1RegReg, TR::InstOpCode::AND1RegImm1,
        TR::InstOpCode::UD2, TR::InstOpCode::AND1MemImm1, TR::InstOpCode::AND1MemImm1, TR::InstOpCode::AND1MemReg,
        TR::InstOpCode::NOT1Mem },
    // cand/sand
    { TR::InstOpCode::AND2RegReg, TR::InstOpCode::AND2RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::AND2RegImms,
        TR::InstOpCode::AND2RegImm2, TR::InstOpCode::AND2MemImms, TR::InstOpCode::AND2MemImm2,
        TR::InstOpCode::AND2MemReg, TR::InstOpCode::NOT2Mem },
    // iand
    { TR::InstOpCode::AND4RegReg, TR::InstOpCode::AND4RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::AND4RegImms,
        TR::InstOpCode::AND4RegImm4, TR::InstOpCode::AND4MemImms, TR::InstOpCode::AND4MemImm4,
        TR::InstOpCode::AND4MemReg, TR::InstOpCode::NOT4Mem },
    // land
    { TR::InstOpCode::AND8RegReg, TR::InstOpCode::AND8RegMem, TR::InstOpCode::MOV8RegReg, TR::InstOpCode::AND8RegImms,
        TR::InstOpCode::AND8RegImm4, TR::InstOpCode::AND8MemImms, TR::InstOpCode::AND8MemImm4,
        TR::InstOpCode::AND8MemReg, TR::InstOpCode::NOT8Mem },
    // bor
    { TR::InstOpCode::OR1RegReg, TR::InstOpCode::OR1RegMem, TR::InstOpCode::MOV1RegReg, TR::InstOpCode::OR1RegImm1,
        TR::InstOpCode::UD2, TR::InstOpCode::OR1MemImm1, TR::InstOpCode::OR1MemImm1, TR::InstOpCode::OR1MemReg,
        TR::InstOpCode::NOT1Mem },
    // cor, sor
    { TR::InstOpCode::OR2RegReg, TR::InstOpCode::OR2RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::OR2RegImms,
        TR::InstOpCode::OR2RegImm2, TR::InstOpCode::OR2MemImms, TR::InstOpCode::OR2MemImm2, TR::InstOpCode::OR2MemReg,
        TR::InstOpCode::NOT2Mem },
    // ior
    { TR::InstOpCode::OR4RegReg, TR::InstOpCode::OR4RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::OR4RegImms,
        TR::InstOpCode::OR4RegImm4, TR::InstOpCode::OR4MemImms, TR::InstOpCode::OR4MemImm4, TR::InstOpCode::OR4MemReg,
        TR::InstOpCode::NOT4Mem },
    // lor
    { TR::InstOpCode::OR8RegReg, TR::InstOpCode::OR8RegMem, TR::InstOpCode::MOV8RegReg, TR::InstOpCode::OR8RegImms,
        TR::InstOpCode::OR8RegImm4, TR::InstOpCode::OR8MemImms, TR::InstOpCode::OR8MemImm4, TR::InstOpCode::OR8MemReg,
        TR::InstOpCode::NOT8Mem },
    // bxor
    { TR::InstOpCode::XOR1RegReg, TR::InstOpCode::XOR1RegMem, TR::InstOpCode::MOV1RegReg, TR::InstOpCode::XOR1RegImm1,
        TR::InstOpCode::UD2, TR::InstOpCode::XOR1MemImm1, TR::InstOpCode::XOR1MemImm1, TR::InstOpCode::XOR1MemReg,
        TR::InstOpCode::NOT1Mem },
    // cxor, sxor
    { TR::InstOpCode::XOR2RegReg, TR::InstOpCode::XOR2RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::XOR2RegImms,
        TR::InstOpCode::XOR2RegImm2, TR::InstOpCode::XOR2MemImms, TR::InstOpCode::XOR2MemImm2,
        TR::InstOpCode::XOR2MemReg, TR::InstOpCode::NOT2Mem },
    // ixor
    { TR::InstOpCode::XOR4RegReg, TR::InstOpCode::XOR4RegMem, TR::InstOpCode::MOV4RegReg, TR::InstOpCode::XOR4RegImms,
        TR::InstOpCode::XOR4RegImm4, TR::InstOpCode::XOR4MemImms, TR::InstOpCode::XOR4MemImm4,
        TR::InstOpCode::XOR4MemReg, TR::InstOpCode::NOT4Mem },
    // lxor
    { TR::InstOpCode::XOR8RegReg, TR::InstOpCode::XOR8RegMem, TR::InstOpCode::MOV8RegReg, TR::InstOpCode::XOR8RegImms,
        TR::InstOpCode::XOR8RegImm4, TR::InstOpCode::XOR8MemImms, TR::InstOpCode::XOR8MemImm4,
        TR::InstOpCode::XOR8MemReg, TR::InstOpCode::NOT8Mem },
};

TR::Register* OMR::X86::TreeEvaluator::logicalEvaluator(
    TR::Node* node, TR::InstOpCode::Mnemonic package[], TR::CodeGenerator* cg)
{
    bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
    TR::Register* targetRegister = NULL;
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        && performTransformation(comp,
            "O^O LogicalEvaluator: checking that the store has not happened yet. Target register: %x\n",
            testRegister)) {
    ...
    if (isMemOp) {
        TR_ASSERT(
            !node->nodeRequiresConditionCodes(), "a node requiring CC evaluation should not be a direct mem update");
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
        && performTransformation(
            comp, "O^O checking that the store has not happened yet. Target register: %x\n", testRegister)) {
        if (!isMemOp && node->getOpCode().isAnd() && !firstChild->getRegister()
            && firstChild->getOpCode().isConversion() && firstChild->getType().isIntegral()
        ...
        instr = generateMemRegInstruction(
            package[TR::TreeEvaluator::logicalMemRegOp], node, tempMR, cg->evaluate(secondChild), cg);
        if (debug("traceMemOp"))
            diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] logical op by var", node);
    } else {
        TR_X86BinaryCommutativeAnalyser temp(cg);
        temp.genericAnalyser(node, package[TR::TreeEvaluator::logicalRegRegOp],
            package[TR::TreeEvaluator::logicalRegMemOp], package[TR::TreeEvaluator::logicalCopyOp]);
        targetRegister = node->getRegister();
        return targetRegister;
    }
