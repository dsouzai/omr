 ...
 */
TR::Register* OMR::X86::TreeEvaluator::coerceST0ToFPR(
    TR::Node* node, TR::DataType dt, TR::CodeGenerator* cg, TR::Register* xmmReg)
{
    if (!xmmReg) {
...
TR::Register* OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getFloatBits() == 0) {
        ...
        setDiscardableIfPossible(
            TR_RematerializableFloat, targetRegister, node, instr, (intptr_t)node->getFloatBits(), cg);
    }
...
TR::Register* OMR::X86::TreeEvaluator::performFload(
...
TR::Register* OMR::X86::TreeEvaluator::performDload(
    TR::Node* node, TR::MemoryReference* sourceMR, TR::CodeGenerator* cg)
{
    TR::Register* targetRegister;
    TR::Instruction* instr;
            ...
            if (cg->comp()->target().is64Bit()) {
                ...
                if (valueChild->getLongInt() == 0) {
                    ...
                    generateRegImm64Instruction(
                        TR::InstOpCode::MOV8RegImm64, node, floatConstReg, valueChild->getLongInt(), cg);
                }
                ...
                exceptionPoint = generateMemImmInstruction(
                    TR::InstOpCode::S4MemImm4, node, tempMR, valueChild->getLongIntLow(), cg);
        ...
        if (sourceRegister->getKind() == TR_FPR) {
            if (cg->comp()->target().is64Bit() && tempMR->getSymbolReference().isUnresolved()) {
                ...
                if (!tempMR->getSymbolReference().getSymbol()->isShadow()
                    ...
                    //
                    exceptionPoint = generateMemRegInstruction(
                        TR::InstOpCode::MOVSMemReg(nodeIs64Bit), node, tempMR, sourceRegister, cg);
                } else {
                    ...
                    exceptionPoint = generateMemRegInstruction(
                        TR::InstOpCode::MOVSMemReg(nodeIs64Bit), node, mr, sourceRegister, cg);

                    tempMR->setProcessAsFPVolatile();

                    if (cg->comp()->getOption(TR_X86UseMFENCE))
                        insertUnresolvedReferenceInstructionMemoryBarrier(
                            cg, TR::InstOpCode::MFENCE, exceptionPoint, tempMR, sourceRegister, tempMR);
                    else
                        insertUnresolvedReferenceInstructionMemoryBarrier(
                            cg, LockOR, exceptionPoint, tempMR, sourceRegister, tempMR);

                    cg->stopUsingRegister(memReg);
                }
            } else {
                TR_ASSERT(
                    nodeIs64Bit != sourceRegister->isSinglePrecision(), "Wrong operand type to floating point store\n");
                exceptionPoint = generateMemRegInstruction(
                    TR::InstOpCode::MOVSMemReg(nodeIs64Bit), node, tempMR, sourceRegister, cg);
            }
        } else {
...
{
    TR_ASSERT_FATAL(
        node->getDataType().getVectorElementType().isFloatingPoint(), "Unsupported datatype for vsqrt opcode");
...
//
TR::Register* OMR::X86::TreeEvaluator::fpConvertToLong(
    TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    ...
    if (child->getOpCode().isDouble()) {
        ...
        //
        loadHighInstr = generateRegMemInstruction(
            TR::InstOpCode::L4RegMem, node, highReg, generateX86MemoryReference(*convertedLongMR, 4, cg), cg);

        loadLowInstr = generateRegMemInstruction(
            TR::InstOpCode::L4RegMem, node, lowReg, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
        ...
        //
        cg->addSnippet(new (cg->trHeapMemory()) TR::X86FPConvertToLongSnippet(
            reStartLabel, snippetLabel, helperSymRef, node, loadHighInstr, loadLowInstr, cg));
...
TR::Register* OMR::X86::TreeEvaluator::d2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return TR::TreeEvaluator::fpConvertToLong(
        node, cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToLong), cg);
}
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        if (floatReg->getKind() == TR_FPR) {
            ...
            generateRegMemInstruction(
                TR::InstOpCode::L4RegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
        } else {
            tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
...
TR::Register* OMR::X86::TreeEvaluator::generateBranchOrSetOnFPCompare(
    TR::Node* node, bool generateBranch, TR::CodeGenerator* cg)
{
    ...
    if (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu || cmpOp == TR::ifdcmpneu || cmpOp == TR::dcmpneu) {
        if (generateBranch) {
            ...
            generateLabelInstruction(
                TR::InstOpCode::JPE4, node, node->getBranchDestination()->getNode()->getLabel(), deps1, cg);
            generateLabelInstruction(
                TR::InstOpCode::JNE4, node, node->getBranchDestination()->getNode()->getLabel(), deps, cg);
        } else {
        ...
        if (generateBranch) {
            ...
            generateLabelInstruction(
                TR::InstOpCode::JE4, node, node->getBranchDestination()->getNode()->getLabel(), cg);
...
//
void OMR::X86::TreeEvaluator::compareFloatOrDoubleForOrder(TR::Node* node, TR::InstOpCode::Mnemonic xmmCmpRegRegOpCode,
    TR::InstOpCode::Mnemonic xmmCmpRegMemOpCode, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::TreeEvaluator::compareFloatAndBranchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::TreeEvaluator::compareFloatOrDoubleForOrder(
        node, TR::InstOpCode::UCOMISSRegReg, TR::InstOpCode::UCOMISSRegMem, cg);
...
{
    TR::TreeEvaluator::compareFloatOrDoubleForOrder(
        node, TR::InstOpCode::UCOMISDRegReg, TR::InstOpCode::UCOMISDRegMem, cg);
    return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(node, true, cg);
}
