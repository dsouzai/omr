...
protected:
    AMD64SystemLinkage(TR::CodeGenerator* cg)
        : TR::X86SystemLinkage(cg)
    { }

    virtual int32_t layoutParm(TR::Node* parmNode, int32_t& dataCursor, uint16_t& intReg, uint16_t& floatReg,
        TR::parmLayoutResult& layoutResult);
    virtual int32_t layoutParm(TR::ParameterSymbol* paramSymbol, int32_t& dataCursor, uint16_t& intReg,
        uint16_t& floatRrgs, TR::parmLayoutResult&);
    ...
    {
        return machine()->getRealRegister(TR::RealRegister::r11);
    }
...
class AMD64Win64FastCallLinkage : public virtual TR::AMD64SystemLinkage {
    ...
    // R10:R11                   Volatile      Must be preserved as required by caller; used in syscall/sysret
    // instructions. R12:R15                   Nonvolatile   Must be preserved by called function. RDI Nonvolatile Must
    // be preserved by called function. RSI                       Nonvolatile   Must be preserved by called function.
