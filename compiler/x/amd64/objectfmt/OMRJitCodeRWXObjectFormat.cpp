...
TR::Instruction* OMR::X86::AMD64::JitCodeRWXObjectFormat::emitFunctionCall(TR::FunctionCallData& data)
{
    ...
    if (data.runtimeHelperIndex || methodSymRef->getSymbol()->castToMethodSymbol()->isHelper()) {
        ...
        if (data.prevInstr) {
            callImmSym = generateImmSymInstruction(
                data.prevInstr, op, static_cast<int32_t>(targetAddress), methodSymRef, data.regDeps, data.cg);
        } else {
            callImmSym = generateImmSymInstruction(
                op, data.callNode, static_cast<int32_t>(targetAddress), methodSymRef, data.regDeps, data.cg);
        }
        ...
        if (targetAddress == 0) {
             ...
             */
            if (data.prevInstr) {
                callInstr = generateImmSymInstruction(
            ...
            if (data.prevInstr) {
                ...
                callInstr = generateRegInstruction(
                    data.out_materializeTargetAddressInstr, op, data.scratchReg, data.regDeps, data.cg);
            } else {
...
void OMR::X86::AMD64::JitCodeRWXObjectFormat::printEncodedFunctionCall(
