...
TR::Register* OMR::X86::I386::TreeEvaluator::longArithmeticCompareRegisterWithImmediate(TR::Node* node,
    TR::Register* cmpRegister, TR::Node* immedChild, TR::InstOpCode::Mnemonic firstBranchOpCode,
    TR::InstOpCode::Mnemonic secondBranchOpCode, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::I386::TreeEvaluator::compareLongAndSetOrderedBoolean(TR::Node* node,
    TR::InstOpCode::Mnemonic highSetOpCode, TR::InstOpCode::Mnemonic lowSetOpCode, TR::CodeGenerator* cg)
{
...
void OMR::X86::I386::TreeEvaluator::compareLongsForOrder(TR::Node* node, TR::InstOpCode::Mnemonic highOrderBranchOp,
...
TR::Register* OMR::X86::I386::TreeEvaluator::ipopcntEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return TR::TreeEvaluator::badILOpEvaluator(node, cg);
}
...
void OMR::X86::I386::TreeEvaluator::lStoreEvaluatorSetHighLowMRIfNeeded(
    TR::Node* node, TR::MemoryReference* lowMR, TR::MemoryReference* highMR, TR::CodeGenerator* cg)
{ }
        ...
        if (valueReg) {
            ...
            if (isVolatile) {
                if (performTransformation(
                        comp, "O^O Using SSE for volatile store %s\n", cg->getDebug()->getName(node))) {
                    // Get stack piece
                    ...
                    instr = generateMemRegInstruction(
                        TR::InstOpCode::S4MemReg, node, stackLow, valueReg->getLowOrder(), cg);
                    ...
                    instr = generateRegMemInstruction(
                        TR::InstOpCode::L4RegMem, node, eaxReg, lowMR, cg); // forming the EDX:EAX pair
                    generateRegMemInstruction(TR::InstOpCode::L4RegMem, node, edxReg, highMR, cg);
                    lowMR->setIgnoreVolatile();
                    highMR->setIgnoreVolatile();
                    generateRegRegInstruction(TR::InstOpCode::MOV4RegReg, node, ebxReg, valueReg->getLowOrder(),
                    ...
                    generateMemInstruction(
                        cg->comp()->target().isSMP() ? TR::InstOpCode::LCMPXCHG8BMem : TR::InstOpCode::CMPXCHG8BMem,
                        node, cmpxchgMR, deps, cg);
                ...
                generateRegRegInstruction(
                    TR::InstOpCode::MOV4RegReg, node, ebxReg, valueReg->getLowOrder(), cg); // forming the ECX:EBX pair
                generateRegRegInstruction(TR::InstOpCode::MOV4RegReg, node, ecxReg, valueReg->getHighOrder(), cg);
                ...
                TR::MemoryReference* cmpxchgMR = generateX86MemoryReference(node, cg);
                generateMemInstruction(
                    cg->comp()->target().isSMP() ? TR::InstOpCode::LCMPXCHG8BMem : TR::InstOpCode::CMPXCHG8BMem, node,
                    cmpxchgMR, deps, cg);
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            if (isMemOp)
                ...
                instr = generateRegImmInstruction(
                    TR::InstOpCode::ADD4RegImms, node, targetRegister->getLowOrder(), lowValue, cg);
        } else if (lowValue == 128) {
            if (isMemOp)
                instr = generateMemImmInstruction(TR::InstOpCode::SUB4MemImms, node, lowMR, -128, cg);
            else
                instr = generateRegImmInstruction(
                    TR::InstOpCode::SUB4RegImms, node, targetRegister->getLowOrder(), -128, cg);
            ...
            if (isMemOp)
                ...
                instr = generateRegImmInstruction(
                    TR::InstOpCode::ADD4RegImm4, node, targetRegister->getLowOrder(), lowValue, cg);
        }
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL
        ...
        if (lowValue >= -128 && lowValue <= 127) {
            if (isMemOp)
                ...
                instr = generateRegImmInstruction(
                    TR::InstOpCode::SUB4RegImms, node, targetRegister->getLowOrder(), lowValue, cg);
        } else if (lowValue == 128) {
            if (isMemOp)
                instr = generateMemImmInstruction(TR::InstOpCode::ADD4MemImms, node, lowMR, lowValue, cg);
            else
                instr = generateRegImmInstruction(
                    TR::InstOpCode::ADD4RegImms, node, targetRegister->getLowOrder(), -128, cg);
            ...
            if (isMemOp)
                ...
                instr = generateRegImmInstruction(
                    TR::InstOpCode::SUB4RegImm4, node, targetRegister->getLowOrder(), lowValue, cg);
        }
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        if (lowValue == 0 && highValue == 0) {
            ...
            generateRegRegInstruction(opCode, node, lowRegister, sourceLow, dependencies, cg);
            if (!firstIU2L && !intMul) {
                generateRegRegInstruction(TR::InstOpCode::TEST4RegReg, node, multiplierRegister->getHighOrder(),
                ...
                if (tempRegister == 0) // decomposition failed
                {
                    ...
                    generateRegRegImmInstruction(
                        opCode, node, tempRegister, multiplierRegister->getHighOrder(), lowValue, cg);
                }
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairShlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0) {
            ...
            generateRegRegInstruction(
                TR::InstOpCode::MOV4RegReg, node, targetRegister->getHighOrder(), tempRegister->getHighOrder(), cg);
            TR::MemoryReference* tempMR = generateX86MemoryReference(cg);
            ...
            generateRegRegImmInstruction(TR::InstOpCode::SHLD4RegRegImm1, node, targetRegister->getHighOrder(),
                tempRegister->getLowOrder(), value, cg);
        } else {
            targetRegister = cg->longClobberEvaluate(firstChild);
            if (value < 32) {
                generateRegRegImmInstruction(TR::InstOpCode::SHLD4RegRegImm1, node, targetRegister->getHighOrder(),
                    targetRegister->getLowOrder(), value, cg);
                generateRegImmInstruction(TR::InstOpCode::SHL4RegImm1, node, targetRegister->getLowOrder(), value, cg);
            } else {
                if (value != 32) {
                    value -= 32;
                    generateRegImmInstruction(
                        TR::InstOpCode::SHL4RegImm1, node, targetRegister->getLowOrder(), value, cg);
                }
        ...
        generateRegRegInstruction(TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
            targetRegister->getLowOrder(), shiftDependencies, cg);
        ...
        generateRegRegInstruction(
            TR::InstOpCode::CMOVNE4RegReg, node, targetRegister->getHighOrder(), targetRegister->getLowOrder(), cg);
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (value != 0) {
            ...
            if (value != 0) {
                ...
                generateRegRegInstruction(
                    TR::InstOpCode::MOV4RegReg, node, tempHighReg, targetRegister->getHighOrder(), cg);
                ...
                generateRegRegImmInstruction(
                    TR::InstOpCode::SHLD4RegRegImm1, node, targetRegister->getLowOrder(), tempHighReg, value, cg);
        ...
        generateRegRegInstruction(TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getHighOrder(),
            targetRegister->getLowOrder(), shiftDependencies, cg);
        generateRegRegInstruction(
            TR::InstOpCode::SHLD4RegRegCL, node, targetRegister->getLowOrder(), scratchReg, shiftDependencies, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairShrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        if (value == 0)
            ...
            generateRegRegImmInstruction(TR::InstOpCode::SHRD4RegRegImm1, node, targetRegister->getLowOrder(),
                targetRegister->getHighOrder(), value, cg);
            ...
            generateRegRegInstruction(
                TR::InstOpCode::MOV4RegReg, node, targetRegister->getLowOrder(), targetRegister->getHighOrder(), cg);
        ...
        TR::Register* scratchReg = cg->allocateRegister();
        generateRegRegInstruction(TR::InstOpCode::SHRD4RegRegCL, node, targetRegister->getLowOrder(),
            targetRegister->getHighOrder(), shiftDependencies, cg);
        ...
        generateRegImmInstruction(TR::InstOpCode::TEST1RegImm1, node, shiftAmountReg, 32, cg);
        generateRegRegInstruction(
            TR::InstOpCode::CMOVNE4RegReg, node, targetRegister->getLowOrder(), targetRegister->getHighOrder(), cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == -1)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::AND4MemImms : TR::InstOpCode::AND4MemImm4;
                lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::AND4RegImms : TR::InstOpCode::AND4RegImm4;
        ...
        if (highValue == -1)
            ...
            if (isMemOp) {
                opCode = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::AND4MemImms
                                                                 : TR::InstOpCode::AND4MemImm4;
                highInstr = generateMemImmInstruction(opCode, node, highMR, highValue, cg);
            } else {
                opCode = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::AND4RegImms
                                                                 : TR::InstOpCode::AND4RegImm4;
        ...
        temp.genericLongAnalyser(node, TR::InstOpCode::AND4RegReg, TR::InstOpCode::AND4RegReg,
            TR::InstOpCode::AND4RegMem, TR::InstOpCode::AND2RegMem, TR::InstOpCode::AND1RegMem,
            TR::InstOpCode::AND4RegMem, TR::InstOpCode::MOV4RegReg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::OR4MemImms : TR::InstOpCode::OR4MemImm4;
                lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::OR4RegImms : TR::InstOpCode::OR4RegImm4;
                lowInstr = generateRegImmInstruction(opCode, node, lowReg, lowValue, cg);
            }
        }
        ...
        if (highValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::OR4MemImms : TR::InstOpCode::OR4MemImm4;
                highInstr = generateMemImmInstruction(opCode, node, highMR, highValue, cg);
            } else {
                opCode
                    = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::OR4RegImms : TR::InstOpCode::OR4RegImm4;
                highInstr = generateRegImmInstruction(opCode, node, highReg, highValue, cg);
            }
        }
        ...
        temp.genericLongAnalyser(node, TR::InstOpCode::OR4RegReg, TR::InstOpCode::OR4RegReg, TR::InstOpCode::OR4RegMem,
            TR::InstOpCode::OR2RegMem, TR::InstOpCode::OR1RegMem, TR::InstOpCode::OR4RegMem,
...
TR::Register* OMR::X86::I386::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (lowValue == 0)
            ...
            if (isMemOp) {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::XOR4MemImms : TR::InstOpCode::XOR4MemImm4;
                lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
            } else {
                opCode
                    = (lowValue >= -128 && lowValue <= 127) ? TR::InstOpCode::XOR4RegImms : TR::InstOpCode::XOR4RegImm4;
                lowInstr = generateRegImmInstruction(opCode, node, lowReg, lowValue, cg);
            }
        }
        ...
        if (highValue == 0)
            ...
            if (isMemOp) {
                opCode = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::XOR4MemImms
                                                                 : TR::InstOpCode::XOR4MemImm4;
                highInstr = generateMemImmInstruction(opCode, node, highMR, highValue, cg);
            } else {
                opCode = (highValue >= -128 && highValue <= 127) ? TR::InstOpCode::XOR4RegImms
                                                                 : TR::InstOpCode::XOR4RegImm4;
                highInstr = generateRegImmInstruction(opCode, node, highReg, highValue, cg);
            }
        }
        ...
        temp.genericLongAnalyser(node, TR::InstOpCode::XOR4RegReg, TR::InstOpCode::XOR4RegReg,
            TR::InstOpCode::XOR4RegMem, TR::InstOpCode::XOR2RegMem, TR::InstOpCode::XOR1RegMem,
            TR::InstOpCode::XOR4RegMem, TR::InstOpCode::MOV4RegReg);
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getReferenceCount() != 1) {
                targetRegister = cg->allocateRegister();
                generateRegRegInstruction(
                    TR::InstOpCode::MOV4RegReg, node, targetRegister, cmpRegister->getLowOrder(), cg);
            }
...
TR::Register* OMR::X86::I386::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        targetRegister = longArithmeticCompareRegisterWithImmediate(
            node, cg->evaluate(firstChild), secondChild, TR::InstOpCode::JAE4, TR::InstOpCode::JGE4, cg);
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    } else if (firstChild->getOpCodeValue() == TR::lconst && firstChild->getRegister() == NULL) {
        targetRegister = longArithmeticCompareRegisterWithImmediate(
            node, cg->evaluate(secondChild), firstChild, TR::InstOpCode::JBE4, TR::InstOpCode::JLE4, cg);
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    } else {
        TR_X86CompareAnalyser temp(cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::b2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        generateRegRegInstruction(
            TR::InstOpCode::MOVSXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
    }
...
TR::Register* OMR::X86::I386::TreeEvaluator::bu2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child), cg->allocateRegister());
        generateRegRegInstruction(
            TR::InstOpCode::MOVZXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
    }
...
TR::Register* OMR::X86::I386::TreeEvaluator::s2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        node->setRegister(longReg);
        generateRegRegInstruction(
            TR::InstOpCode::MOVSXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
    }
...
TR::Register* OMR::X86::I386::TreeEvaluator::su2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getRegister() == NULL && child->getReferenceCount() == 1) {
        ...
        node->setRegister(longReg);
        generateRegRegInstruction(
            TR::InstOpCode::MOVZXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
    }
    ...
    if ((valueChild->getOpCodeValue() == TR::lbits2d) && !valueChild->getRegister()) {
        ...
        if (valueChild->getOpCode().isLoadConst()) {
            ...
            if (valueChildReg && valueChildReg->getKind() == TR_X87 && valueChild->getReferenceCount() == 1)
                instr = generateFPSTiST0RegRegInstruction(
                    TR::InstOpCode::DSTRegReg, valueChild, valueChildReg, valueChildReg, cg);
            ...
            generateRegMemInstruction(
                TR::InstOpCode::L4RegMem, node, tempRegister, generateX86MemoryReference(*loadLowMR, 4, cg), cg);
            instr = generateMemRegInstruction(
                TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*storeLowMR, 4, cg), tempRegister, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        generateMemRegInstruction(
            TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*lowMR, 4, cg), longReg->getHighOrder(), cg);
        generateFPRegMemInstruction(
            TR::InstOpCode::FLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
        cg->decReferenceCount(child);
    }
...
TR::Register* OMR::X86::I386::TreeEvaluator::l2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        generateMemRegInstruction(
            TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*lowMR, 4, cg), longReg->getHighOrder(), cg);
        generateFPRegMemInstruction(
            TR::InstOpCode::DLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
        cg->decReferenceCount(child);
    }
...
TR::Register* OMR::X86::I386::TreeEvaluator::performLload(
...
TR::Register* OMR::X86::I386::TreeEvaluator::lbits2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && child->getReferenceCount() == 1) {
        ...
        generateMemRegInstruction(
            TR::InstOpCode::S4MemReg, node, generateX86MemoryReference(*tempMR, 4, cg), longReg->getHighOrder(), cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::dbits2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadVar() && (child->getReferenceCount() == 1)) {
        ...
        generateRegMemInstruction(
            TR::InstOpCode::L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg), cg);
        ...
        generateRegMemInstruction(
            TR::InstOpCode::L4RegMem, node, lowReg, generateX86MemoryReference(*tempMR, 0, cg), cg);
        generateRegMemInstruction(
            TR::InstOpCode::L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg), cg);
    }
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1
                ...
                if (landFirstChild->getReferenceCount() == 1 && landFirstChild->getRegister() == NULL
                    ...
                    generateRegMemInstruction(TR::InstOpCode::L4RegMem, landFirstChild, targetRegister, tempMR, cg);
                ...
                generateRegRegInstruction(
                    TR::InstOpCode::OR4RegReg, node, targetRegister, cmpRegister->getHighOrder(), cg);
            }
...
TR::Register* OMR::X86::I386::TreeEvaluator::iflcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (generateLAddOrSubForOverflowCheck(node, cg)) {
        ...
        TR::InstOpCode::Mnemonic reverseCompareOp
            = node->getOpCode().isUnsigned() ? TR::InstOpCode::JA4 : TR::InstOpCode::JG4;
...
TR::Register* OMR::X86::I386::TreeEvaluator::iflcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (generateLAddOrSubForOverflowCheck(node, cg)) {
        ...
        TR::InstOpCode::Mnemonic reverseCompareOp
            = node->getOpCode().isUnsigned() ? TR::InstOpCode::JB4 : TR::InstOpCode::JL4;
...
TR::Register* OMR::X86::I386::TreeEvaluator::lselectEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!longCompare && condOp.isCompareForEquality() && condition->getFirstChild()->getOpCode().isIntegerOrAddress()) {
        ...
        if (condOp.isCompareTrueIfEqual()) {
            generateRegRegInstruction(TR::InstOpCode::CMOVNE4RegReg, node, trueReg->getRegisterPair()->getLowOrder(),
                falseReg->getRegisterPair()->getLowOrder(), cg);
            generateRegRegInstruction(TR::InstOpCode::CMOVNE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
                falseReg->getRegisterPair()->getHighOrder(), cg);
        } else {
            generateRegRegInstruction(TR::InstOpCode::CMOVE4RegReg, node, trueReg->getRegisterPair()->getLowOrder(),
                falseReg->getRegisterPair()->getLowOrder(), cg);
            generateRegRegInstruction(TR::InstOpCode::CMOVE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
                falseReg->getRegisterPair()->getHighOrder(), cg);
        }
        ...
        generateRegRegInstruction((condOp.isCompareTrueIfEqual())
                ...
                : ((condOp.isCompareTrueIfGreater()) ? TR::InstOpCode::CMOVLE4RegReg : TR::InstOpCode::CMOVGE4RegReg),
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairMinMaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRegRegInstruction(TR::InstOpCode::CMOVE4RegReg, node, result->getHighOrder(), result->getLowOrder(),
...
TR::Register* OMR::X86::I386::TreeEvaluator::lcmpsetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateMemInstruction(cg->comp()->target().isSMP() ? TR::InstOpCode::LCMPXCHG8BMem : TR::InstOpCode::CMPXCHG8BMem,
        node, memRef, deps, cg);
