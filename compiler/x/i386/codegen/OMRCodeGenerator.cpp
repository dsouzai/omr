...
void OMR::X86::I386::CodeGenerator::initialize()
{
...
TR::Register* OMR::X86::I386::CodeGenerator::longClobberEvaluate(TR::Node* node)
{
...
TR_GlobalRegisterNumber OMR::X86::I386::CodeGenerator::pickRegister(TR::RegisterCandidate* rc, TR::Block** allBlocks,
    ...
    TR_LinkHead<TR::RegisterCandidate>* candidates)
{
    if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
        if (self()->comp()->getOption(TR_AssignEveryGlobalRegister)) {
            ...
            //
            availableRegisters -= *self()->getGlobalRegisters(
                TR_vmThreadSpill, self()->comp()->getMethodSymbol()->getLinkageConvention());
        }
        return OMR::CodeGenerator::pickRegister(rc, allBlocks, availableRegisters, highRegisterNumber, candidates);
    }

    if ((rc->getSymbol()->getDataType() == TR::Float) || (rc->getSymbol()->getDataType() == TR::Double)) {
...
#endif
    {
        ...
        if (!availableRegisters.get(4) && availableRegisters.get(3))
            assigningEDX = true;
        ...
        while (bvi.hasMoreElements()) {
            ...
            maxRegisterPressure = self()->estimateRegisterPressure(block, visitCount, maxStaticFrequency, maxFrequency,
                vmThreadUsed, numAssignedGlobalRegs, _assignedGlobalRegisters, rc->getSymbolReference(), assigningEDX);
        ...
        // comp()->cg()->getMaximumNumbersOfAssignableGPRs(), numExtraRegs);
...
int32_t OMR::X86::I386::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    ...
    if (node->getOpCode().isIf()) {
        ...
        if (node->getFirstChild()->getType().isInt64()) {
            if (node->getOpCode().isBranch()) {
                ...
                if (firstChild->getOpCodeValue() == TR::d2l || secondChild->getOpCodeValue() == TR::d2l) {
                    return 1;
                }

                if ((firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isLoadVarDirect())
                    || (secondChild->getReferenceCount() == 1 && firstChild->getOpCode().isLoadVarDirect()))
                    extraRegsAvailable
                        += 0; // TODO: put it back to 2 when looking at GRA, GRA pushes allocation of 8 registers
