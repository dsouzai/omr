...
//----------------------------------------------------------------------------
template<int N> class TR_StatsHisto : public TR_Stats {
public:
    TR_StatsHisto(const char* name, double minBin, double maxBin)
        : TR_Stats(name)
    {
        for (int i = 0; i < N + 2; i++)
            _bins[i] = 0;
...
public:
    TR_StatsInterval(const char* name)
        : TR_Stats(name)
        , _intervalerr(0)
    { }
    void update(double v, double t) // Updates a statrec
    {
        ...
        } else // This is not the first sample
        {
            ...
            if (interval < 0.0)
                _intervalerr = 1; // Not a valid interval
...
//--------------------------------------------------------------------------
template<int N> class TR_StatsEvents {
public:
    enum { NAME_LEN = 31 };
    TR_StatsEvents() { }
