...
SimpleRegex::Simple *SimpleRegex::processSimple(const char *&s, TR_YesNoMaybe allowAlternates, bool &foundError)
{
    ...
    if (s[0] == '[' && allowAlternates == TR_yes) {
        ...
        for (i = 0; i < sizeof(m->component->data.bit_map) / sizeof(m->component->data.bit_map[0]);
        ...
        ++s;
        if (invert) {
            for (i = 0; i < sizeof(m->component->data.bit_map) / sizeof(m->component->data.bit_map[0]); i++) {
        ...
        i = 0;
        while (s[0] != '\0' && s[0] != '*' && s[0] != '?' && s[0] != ',' && s[0] != '|' && s[0] != '}') {
    ...
    if (m->remainder == NULL)
        ...
        if ((m->remainder->component->type == wildcards && (m->remainder->component->data.counts & 1) != 0)
            || (m->remainder->fixed_chars_right == 0 && m->remainder->remainder != NULL)) {
            m->fixed_chars_right = 0;
        } else {
            i = 0;
...
bool SimpleRegex::Simple::matchesRemainder(const char *s, bool isCaseSensitive, bool useLocale)
{
    if (remainder == NULL)
        return s[0] == '\0';
    ...
    switch (component->type) {
        case simple_string:
            for (i = 0; component->data.str[i]; ++i, ++s) {
                if (isCaseSensitive) {
                    ...
                    char lower_stri = useLocale ? (tolower(component->data.str[i]))
                                                : (::tolower_ignore_locale(component->data.str[i]));
        ...
        case char_alternatives: {
            ...
            if (!isCaseSensitive && !charIsInBitMap) {
                ...
                charIsInBitMap = ((component->data.bit_map[BWORD(lower_i)] & (CONSTANT64(1) << BBIT(lower_i))) != 0)
...
bool SimpleRegex::match(TR::SimpleRegex *regex, TR_ResolvedMethod *feMethod, bool isCaseSensitive)
{
...
bool SimpleRegex::match(TR::SimpleRegex *regex, TR_ByteCodeInfo &bcinfo, bool isCaseSensitive)
{
    TR::Compilation *comp = TR::comp();
    TR::StackMemoryRegion stackMemoryRegion(*comp->trMemory());
...
bool SimpleRegex::matchIgnoringLocale(TR::SimpleRegex *regex, const char *s, bool isCaseSensitive)
{
