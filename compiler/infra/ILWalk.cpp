...
TR::Node* TR::TreeTopIteratorImpl::currentNode() { return currentTree()->getNode(); }

bool TR::TreeTopIteratorImpl::isAt(PreorderNodeIterator& other) { return other.isAt(currentTree()); }
...
{
    TR_ASSERT(_current,
        "Cannot stepForward a TreeTopIterator that has already moved beyond the first or last tree of the method");
...
{
    TR_ASSERT(_current,
        "Cannot stepBackward a TreeTopIterator that has already moved beyond the first or last tree of the method");
...
bool TR::PreorderNodeIterator::alreadyBeenPushed(TR::Node* node) { return _checklist.contains(node); }
...
bool TR::PostorderNodeIterator::alreadyBeenPushed(TR::Node* node) { return _checklist.contains(node); }
...
TR::PreorderNodeOccurrenceIterator::PreorderNodeOccurrenceIterator(
    TR::TreeTop* start, TR::Compilation* comp, const char* name)
    : NodeOccurrenceIterator(start, comp, name)
{
    logCurrentLocation();
}

bool TR::PreorderNodeOccurrenceIterator::alreadyPushedChildren(TR::Node* node) { return _checklist.contains(node); }
...
TR::PostorderNodeOccurrenceIterator::PostorderNodeOccurrenceIterator(
    TR::TreeTop* start, TR::Compilation* comp, const char* name)
    : NodeOccurrenceIterator(start, comp, name)
{
...
bool TR::PostorderNodeOccurrenceIterator::alreadyPushedChildren(TR::Node* node) { return _checklist.contains(node); }

void TR::PostorderNodeOccurrenceIterator::pushLeftmost(TR::Node* nodeArg)
{
    for (TR::Node* node = nodeArg; node->getNumChildren() >= 1 && !alreadyPushedChildren(node);
...
bool TR::BlockIterator::isLoggingEnabled() { return (_name && _comp && _comp->getOption(TR_TraceILWalks)); }

TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
    TR::Block* start, TR::Compilation* comp, const char* name)
...
TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
    TR::CFG* cfg, TR::Compilation* comp, const char* name)
    : BlockIterator(comp, name)
...
{
    // TODO: This should use an iterative algorithm with a worklist, but the recursive algo is so much easier to
    // write...
...
{
    TR_ASSERT(currentBlock(),
        "cannot stepForward a ReversePostorderSnapshotBlockIterator that has already finished its walk");
    do
        --_currentIndex;
...
TR::AllBlockIterator::AllBlockIterator(TR::CFG* cfg, TR::Compilation* comp, const char* name)
    : BlockIterator(comp, name)
    , _cfg(cfg)
    , _currentBlock(cfg->getFirstNode()->asBlock())
    , _nextBlock(_currentBlock->getNext()->asBlock())
    , _alreadyVisited(cfg->comp())
{
    _alreadyVisited.add(_currentBlock);
    logCurrentLocation();
}

TR::Block* TR::AllBlockIterator::currentBlock() { return _currentBlock; }
...
TR::TreeTopOrderExtendedBlockIterator::TreeTopOrderExtendedBlockIterator(TR::Compilation* comp, const char* name)
    : BlockIterator(comp, name)
    , _currBlock(comp->getStartBlock())
    , _nextBlock(_currBlock->getNextExtendedBlock())
{
    logCurrentLocation();
}

TR::Block* TR::TreeTopOrderExtendedBlockIterator::getFirst() { return _currBlock; }
...
TR::Block* TR::TreeTopOrderExtendedBlockIterator::getLast()
{
    if (_nextBlock != NULL) {
        ...
        for (TR::Block* nextBlock = _currBlock->getNextBlock(); nextBlock != NULL;
             lastBlock = nextBlock, nextBlock = lastBlock->getNextBlock()) {
            // Void
        }
...
{
    TR_ASSERT(_currBlock != NULL,
        "Cannot increment an TreeTopOrderExtendedBlockIterator that has already finished iterating");
