...
public:
    ...
    bool verify() { return _root ? _root->verify(NULL) : true; }
...
public:
    ...
    void reset(TR_HedgeTree<T>& tree)
    {
        _tree = &tree;
        _next = getLeftmost(tree.getRoot());
    }
...
public:
    ...
    T* find(int32_t key, TR_HedgeTree<T>& tree) { return tree.find(key); }
    ...
    T* copyAll(TR_HedgeTree<T>& tree) { return copySubtree(tree.getRoot()); }
    ...
    void empty(TR_HedgeTree<T>& tree) { emptySubtree(tree.rootReference()); }
...
template<class T> void TR_HedgeTreeHandler<T>::swapRightmost(T*& node, T*& swap)
{
    ...
    if (node->right()) {
        ...
        //
        bool b;
        b = node->isLeftSibling();
        node->setLeftSibling(swap->isLeftSibling());
        swap->setLeftSibling(b);
        b = node->isRightSibling();
        node->setRightSibling(swap->isRightSibling());
        swap->setRightSibling(b);

        T* t = swap->right();
        swap->setRight(node->right());
        node->setRight(t);
        T* t1 = swap->getParent();
        if (node == swap->left()) {
