...
void TR_HashTab::init(uint32_t newSize, bool growth)
{
    ...
    _table = (TR_HashTableEntry**)trMemory()->allocateMemory(
        _tableSize * sizeof(TR_HashTableEntry*), _allocType, TR_Memory::HashTableEntry);
...
void TR_HashTab::growAndRehash(uint32_t newSize)
{
    ...
    if ((_allocType == persistentAlloc) && (oldTable != NULL)) {
