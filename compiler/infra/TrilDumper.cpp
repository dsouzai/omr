...
TR::TrilDumper::TrilDumper(FILE* file)
    : _outputFile(file)
{ }
...
void TR::TrilDumper::visitingMethod(TR::ResolvedMethodSymbol* method)
{
    ...
    if (method->getName()) {
        fprintf(_outputFile, "name=%s ", method->getName());
    }
...
void TR::TrilDumper::returnedToMethod(TR::ResolvedMethodSymbol* method) { fprintf(_outputFile, ") "); }
