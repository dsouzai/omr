...
template<typename T, class Alloc = TR::Allocator> class deque : public std::deque<T, typed_allocator<T, Alloc> > {
public:
...
template<typename T, class Alloc> TR::deque<T, Alloc>::~deque() { }

template<typename T, class Alloc>
typename TR::deque<T, Alloc>::reference TR::deque<T, Alloc>::operator[](size_type index)
{
...
template<typename T, class Alloc>
typename TR::deque<T, Alloc>::const_reference TR::deque<T, Alloc>::operator[](size_type index) const
{
// In DEBUG, at() is used for correctness due to its bound checking
// whilst [] is used in PROD for performance
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
