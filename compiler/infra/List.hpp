...
public:
    ...
    ListElement(T* p, ListElement<T>* q = NULL)
        : _pNext(q)
        , _pDatum(p)
    { }
...
public:
    ...
    bool isDoubleton() const
    {
        return _pHead && _pHead->getNextElement() && !_pHead->getNextElement()->getNextElement();
    }
...
public:
    ...
    {
        this->_pHead = NULL;
        this->_region = &(m->heapMemoryRegion());
    }
    List(TR::Region& region)
    {
        this->_pHead = NULL;
        this->_region = &region;
    }
    ...
    ListElement<T>* add(T* p)
    {
        TR_ASSERT(this->_region != NULL,
            "Should not be using heap allocation to add to a persistent list. The 'add' routine in TR_PersistentList "
            "should be called\n");
    ...
    {
        return this->_phead = new (freeList) ListElement<T>(p, this->_pHead);
    }

    void deleteAll() { this->_pHead = NULL; }
    ...
    ListElement<T>* addAfter(T* elem, ListElement<T>* previous)
    {
        ...
        if (previous == NULL)
            ...
            TR_ASSERT(this->_region != NULL,
                "Should not be using heap allocation to add to a persistent list. The 'addAfter' routine in "
                "TR_PersistentList should be called\n");
    ...
    {
        for (ListElement<T>* cursor = other._pHead; cursor; cursor = cursor->getNextElement())
            add(cursor->getData());
    }
...
public:
    ...
    ListIterator()
        ...
        , _pCursor(0)
    { }
    ListIterator(ListBase<T>* p)
        : _pHead(p->getListHead())
    ...
    {
        _pCursor = _pHead;
        return (_pHead ? _pHead->getData() : NULL);
    }
...
public:
    ...
    bool find(T* elem) { return _pList->find(elem); }
    ...
    ListElement<T>* add(T* p)
    {
        TR_ASSERT(_pList->_region != NULL,
            "Should not be using heap allocation to append to a persistent list. Should use 'addPersistent' instead\n");
    ...
    {
        TR_ASSERT(
            _pList->_region == NULL, "Should not be using persistent allocation to append to a non-persistent list.\n");
...
public:
    ...
    TR_TwoListIterator(List<T>& l1, List<T>& l2)
        : _firstListHead(l1.getListHead())
        , _secondListHead(l2.getListHead())
    { }

    T* getFirst()
    {
        if (_firstListHead) {
            _cursor = _firstListHead;
            _iteratingThruSecondList = false;
        } else {
...
public:
    ...
    TR_PersistentList(T* p)
        : List<T>()
    {
        List<T>::_pHead = new (PERSISTENT_NEW) ListElement<T>(p);
    }
...
public:
    TR_Queue(TR_Memory* m, bool useFreeList = false)
        : TR_ScratchList<T>(m)
        , _pTail(NULL)
        , _useFreeList(useFreeList)
        , _freeList(m)
    { }
    ...
    {
        if (_pTail == NULL) {
            List<T>::_pHead = _pTail = _useFreeList ? new (_freeList) ListElement<T>(elem, List<T>::_pHead)
                                                    : new (List<T>::getRegion()) ListElement<T>(elem, List<T>::_pHead);
        } else {
            ListElement<T>* p = _useFreeList ? new (_freeList) ListElement<T>(elem, NULL)
    ...
    bool isEmpty() { return !List<T>::_pHead; }
...
public:
    ...
    ListHeadAndTail(TR_Memory* m)
        : List<T>(m->heapMemoryRegion())
    {
        init();
    }
    ListHeadAndTail(TR_Memory* m, T* p)
        : List<T>(m->heapMemoryRegion())
    {
        init();
        add(p);
    }
    ListHeadAndTail(TR::Region& region)
        : List<T>(region)
    {
        init();
    }
    ListHeadAndTail(TR::Region& region, T* p)
        : List<T>(region)
    {
        init();
        add(p);
