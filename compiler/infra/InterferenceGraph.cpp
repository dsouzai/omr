    ...
    , TR_IGBase()
{
    ...
    _entityHash._buckets
        = (HashTableEntry**)trMemory()->allocateHeapMemory(_entityHash._numBuckets * sizeof(HashTableEntry*));
...
TR_IGNode* TR_InterferenceGraph::add(void* entity, bool ignoreDuplicates)
{
    ...
    TR_ASSERT(!igNode, "entity %p already exists in this interference graph\n", entity);
...
//
void TR_InterferenceGraph::addInterferenceBetween(TR_IGNode* igNode1, TR_IGNode* igNode2)
{
...
bool TR_InterferenceGraph::hasInterference(void* entity1, void* entity2)
{
...
//
void TR_InterferenceGraph::partitionNodesIntoDegreeSets(
    TR_BitVector* workingSet, TR_BitVector* colourableDegreeSet, TR_BitVector* notColourableDegreeSet)
{
    int32_t i;
    TR_BitVectorIterator bvi(*workingSet);

    TR_ASSERT(getNumColours() > 0, "can't partition without knowing the number of available colours\n");
...
//
IGNodeColour TR_InterferenceGraph::findMinimumChromaticNumber() { return 0; }

// Perform Simplify colouring phase on an interference graph.
//
bool TR_InterferenceGraph::simplify()
{
    TR_IGNode *igNode, *bestSpillNode;
    ...
    while (!workingSet->isEmpty()) {
        ...
        //
        TR_ASSERT(!notColourableDegreeSet->isEmpty(), "not colourable set must contain at least one member\n");
        ...
        if (!notColourableDegreeSet->isEmpty()) {
            ...
            while (bvi.hasMoreElements()) {
                ...
                // TODO: This unsigned conversion was inserted while addressing warnings. The warning at this line
                // warned us about signed/unsigned comparison. The C++ standard dictates that signed values are always
                // converted to unsigned before the comparison. Because `degree` is initialized to -1, the unsigned
                // conversion will produce UINT_MAX in this case, so the comparison below will never succeed and we
                // should always assert.
