...
bool TR_AddressTree::isILLoad(TR::Node* node)
{
...
bool TR_AddressTree::findComplexAddressGenerationTree(TR::Node* node, vcount_t visitCount, TR::Node* parent)
{
...
bool TR_AddressTree::processBaseAndIndex(TR::Node* parent)
{
...
bool TR_AddressTree::processMultiplyNode(TR::Node* multiplyNode)
{
...
//    ilload <DirectByteBuffer>
//       aload <Buffer>
bool TR_AddressTree::process(TR::Node* aiaddNode, bool onlyConsiderConstAiaddSecondChild)
{
        ...
        if ((opCodeAiaddSecondChild == TR::isub || opCodeAiaddSecondChild == TR::lsub
            ...
            if (opCodeIsubSecondChild != TR::iconst && opCodeIsubSecondChild != TR::lconst) {
                ...
                if (opCodeIsubFirstChild == TR::imul || opCodeIsubFirstChild == TR::lmul) {
                    ...
                    if (indSymFromSubTree->getOpCodeValue() == TR::iload
                        || indSymFromSubTree->getOpCodeValue() == TR::lload) {
                    ...
                    } else if (indSymFromSubTree->getOpCodeValue() == TR::iadd
                        || indSymFromSubTree->getOpCodeValue() == TR::ladd) {
                        ...
                        // but not:
                        //   a[j + indVar]
                        if ((indSymFromSubTree->getFirstChild()->getOpCodeValue() == TR::iload
                                || indSymFromSubTree->getFirstChild()->getOpCodeValue() == TR::lload)
                            && (indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::iload
                                || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::lload
                                || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::iconst
                                || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::lconst)) {
                            validAiaddSubTree = true;
                    ...
                    validAiaddSubTree
                        = findComplexAddressGenerationTree(isubFirstChild, comp()->incVisitCount(), aiaddSecondChild);
...
bool TR_CommutativePattern::thisMatches(TR::Node* node, TR_Unification& uni, TR::Compilation* comp)
{
    ...
    TR_Unification::TR_Mark mark = uni.mark();
    if (_leftPattern->matches(node->getFirstChild(), uni, comp)
        ...
        return (_leftPattern->matches(node->getSecondChild(), uni, comp)
            && _rightPattern->matches(node->getFirstChild(), uni, comp));
    }
}
