...
enum TR_BitContainerType { singleton, bitvector };
...
public:
    ...
    TR_BitContainer()
        ...
        , _type(bitvector)
    { }
    TR_BitContainer(int32_t index)
        : _singleBit(index)
        , _type(singleton)
    { }
    TR_BitContainer(TR_BitVector* bv)
        : _bitVector(bv)
        , _type(bitvector)
    { }
...
public:
    ...
    TR_SingleBitContainer()
        : _value(false)
    { }
    TR_SingleBitContainer(int64_t initBits, TR_Memory* m, TR_AllocationKind allocKind = heapAlloc)
        : _value(false)
    { }
    TR_SingleBitContainer(int64_t initBits, TR::Region& region)
        : _value(false)
    { }
    int32_t get(int32_t n)
    {
        TR_ASSERT(n == 0, "SingleBitContainers only contain one bit\n");
        return _value;
    }
    ...
    void operator&=(TR_SingleBitContainer& other) { _value = _value && other._value; }
    void operator-=(TR_SingleBitContainer& other)
    {
        if (other._value) {
            _value = false;
        }
    ...
    {
        TR_ASSERT(n < 2, "SingleBitContainers only contain one bit\n");
        if (n > 0) {
            _value = true;
        }
    }
...
enum TR_BitVectorGrowable { notGrowable, growable };
...
public:
    ...
    //
    TR_BitVector(int64_t initBits, TR_Memory* m, TR_AllocationKind allocKind = heapAlloc,
        TR_BitVectorGrowable growableOrNot = growable, TR_MemoryBase::ObjectType ot = TR_MemoryBase::BitVector)
    {
        _chunks = NULL;
        ...
        if (_numChunks) {
            if (_region) {
                ...
                TR_ASSERT(
                    allocKind == persistentAlloc, "Should not allocate through trMemory except for persistent memory");
    ...
    TR_BitVector(int64_t initBits, TR::Region& region, TR_BitVectorGrowable growableOrNot = growable,
        TR_MemoryBase::ObjectType ot = TR_MemoryBase::BitVector)
    {
        _chunks = NULL;
        _numChunks = getChunkIndex(initBits - 1) + 1;
        _firstChunkWithNonZero = _numChunks;
    ...
    // to be empty.
    //
    void init(int64_t initBits, TR_Memory* m, TR_AllocationKind allocKind = heapAlloc,
        TR_BitVectorGrowable growableOrNot = notGrowable)
    {
    ...
    bool isSet(int64_t n) { return get(n) != 0; }
    ...
    template<class BitVector> TR_BitVector& operator=(const BitVector& sparse);
    template<class BitVector> TR_BitVector& operator-=(const BitVector& sparse);
    template<class BitVector> TR_BitVector& operator&=(const BitVector& sparse);
    template<class BitVector> TR_BitVector& operator|=(const BitVector& sparse);
    ...
    //
    bool isEmpty() { return _lastChunkWithNonZero < 0; }
    ...
    //
    void pack() { setChunkSize(numUsedChunks()); }
    ...
    //
    int32_t numUsedChunks() { return _lastChunkWithNonZero + 1; }
    ...
    int32_t chunkSize() { return sizeof(chunk_t); }

#ifdef TRACK_TRBITVECTOR_MEMORY
    uint32_t MemoryUsage() { return _memoryUsed; }
#endif
...
private:
    ...
    {
#ifdef DEBUG
        TR_ASSERT((((bitIndex >> SHIFT) & ~((int64_t)0x7fffffff)) == 0) || (bitIndex < 0),
            "Chunk index out of int32_t range. bitIndex=%lx\n", bitIndex);
#endif
    ...
    //
    static inline chunk_t getBitMask(int32_t bitIndex)
    {
        return (chunk_t)1 << ((BITS_IN_CHUNK - 1) - (bitIndex & (BITS_IN_CHUNK - 1)));
    }
    ...
    // Given a mask for a bitIndex, calculate the mask for bitIndex+1
    //
    static inline chunk_t incrementBitMask(chunk_t mask)
    {
        return mask + mask;
    } // additions are faster than shifts on P4
#endif
...
public:
    ...
    // being used.
    //
    TR_BitVectorIterator() { _bitVector = NULL; }

    TR_BitVectorIterator(TR_BitVector& bv, int32_t startBit = 0) { setBitVector(bv, startBit); }
    ...
    int hasMoreElements() { return (_bitVector->getChunkIndex(_curIndex) < _bitVector->_numChunks); }
...
class TR_BitVectorCursor : public TR_BitVectorIterator {
    // CS2-like iterator
public:
    TR_BitVectorCursor(TR_BitVector& bv)
        : TR_BitVectorIterator(bv)
    {
        SetToFirstOne();
    }
...
public:
    ...
    TR_BitContainerIterator(TR_BitContainer& bc) { setBitContainer(bc); }
...
public:
    ...
    TR_BitMatrix(int32_t rows, int32_t bitsPerRow, TR_Memory* m, TR_AllocationKind allocKind = heapAlloc)
        ...
        , _vector(rows * bitsPerRow, m, allocKind, notGrowable)
    { }
    ...
    bool isSet(int32_t row, int32_t bitIndex) { return get(row, bitIndex) != 0; }
...
//
template<class BitVector> inline TR_BitVector& TR_BitVector::operator=(const BitVector& sparse)
{
...
public:
    CS2_TR_BitVector(TR_BitVector& _bv)
        : bv(_bv)
    { }
    ...
    {
        return true;
    } // returns whether this bit vector is based on an array of bitWords (ie chunks of bits)
    ...
    struct Cursor {
        Cursor(const CS2_TR_BitVector& vector)
            : bvi(vector.bv)
        { }
        bool Valid() { return valid; }
        operator uint32_t() { return value; }
...
//
template<class BitVector> inline void Assign(BitVector& cs2bv, TR_BitVector& trbv, bool clear = true)
{
    if (clear)
...
//
template<class BitVector> inline void Assign(BitVector& cs2bv, TR_BitContainer& trbc, bool clear = true)
{
...
class Operator {
public:
    template<class T> static void operate(T& inplace, const T& operand);
};

class SetDiffOperator {
public:
    template<class T> static void operate(T& inplace, const T& operand) { inplace &= ~operand; }
};
