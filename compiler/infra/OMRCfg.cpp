...
TR::CFG* OMR::CFG::self() { return static_cast<TR::CFG*>(this); }

const char* OMR::CFG::blockFrequencyNames[NUMBER_BLOCK_FREQUENCIES]
    = { "UNKNOWN_COLD_BLOCK_COUNT", "VERSIONED_COLD_BLOCK_COUNT", "UNRESOLVED_COLD_BLOCK_COUNT",
          "CATCH_COLD_BLOCK_COUNT", "INTERP_CALLEE_COLD_BLOCK_COUNT", "REVERSE_ARRAYCOPY_COLD_BLOCK_COUNT" };

TR::CFGNode* OMR::CFG::addNode(TR::CFGNode* n, TR_RegionStructure* parent, bool isEntryInParent)
{
    _nodes.add(n);
...
void OMR::CFG::addSuccessorEdges(TR::Block* block)
{
...
void OMR::CFG::join(TR::Block* b1, TR::Block* b2)
{
    if (b2)
...
void OMR::CFG::insertBefore(TR::Block* b1, TR::Block* b2)
{
    self()->addNode(b1);
    self()->join(b1, b2);
}

void OMR::CFG::copyExceptionSuccessors(TR::CFGNode* from, TR::CFGNode* to, bool (*predicate)(TR::CFGEdge*))
{
...
TR_Structure* OMR::CFG::invalidateStructure()
{
    setStructure(NULL);
...
TR_Structure* OMR::CFG::setStructure(TR_Structure* p)
{
    if (_rootStructure && !p) {
 ...
 */
bool OMR::alwaysTrue(TR::CFGEdge* e) { return true; }

TR_OrderedExceptionHandlerIterator::TR_OrderedExceptionHandlerIterator(
    TR::Block* tryBlock, TR::Region& workingMemoryRegion)
{
    if (tryBlock->getExceptionSuccessors().empty())
        ...
        for (auto e = tryBlock->getExceptionSuccessors().begin(); e != tryBlock->getExceptionSuccessors().end(); ++e) {
            ...
            if (!b->isOSRCatchBlock()) {
                TR_ASSERT(
                    (_handlers[((inlineDim - b->getInlineDepth() - 1) * handlerDim) + b->getHandlerIndex()] == NULL),
                    "handler entry is not NULL\n");
...
TR::Block* TR_OrderedExceptionHandlerIterator::getFirst()
{
    _index = 0;
...
TR::Block* TR_OrderedExceptionHandlerIterator::getNext()
{
    ++_index;
    return getCurrent();
}

TR::Block* TR_OrderedExceptionHandlerIterator::getCurrent()
{
    TR::Block* handler = 0;
...
TR::CFGEdge* TR::CFGEdge::createExceptionEdge(
    TR::CFGNode* pF, TR::CFGNode* pT, TR_Memory* trMemory, TR_AllocationKind allocKind)
{
    TR::CFGEdge* newEdge = new (trMemory, allocKind) TR::CFGEdge(pF, pT);
...
{
    int32_t totalPredFreq
        = getTotalEdgesFrequency(block->getPredecessors()) + getTotalEdgesFrequency(block->getExceptionPredecessors());
    int32_t totalSuccFreq
        = getTotalEdgesFrequency(block->getSuccessors()) + getTotalEdgesFrequency(block->getExceptionSuccessors());
...
bool TR::CFGNode::hasSuccessor(TR::CFGNode* n) { return getSuccessorEdge(n) != NULL; }

bool TR::CFGNode::hasExceptionSuccessor(TR::CFGNode* n) { return getExceptionSuccessorEdge(n) != NULL; }

bool TR::CFGNode::hasPredecessor(TR::CFGNode* n) { return getPredecessorEdge(n) != NULL; }

bool TR::CFGNode::hasExceptionPredecessor(TR::CFGNode* n) { return getExceptionPredecessorEdge(n) != NULL; }
...
TR::CFGNode* OMR::CFG::removeNode(TR::CFGNode* node)
{
    ...
    //
    TR_ASSERT(node->getPredecessors().empty(), "CFG, removing a node that still has predecessors");
    TR_ASSERT(node->getExceptionPredecessors().empty(), "CFG, removing a node that still has predecessors");
...
//
OMR::CFG::OrphanType OMR::CFG::unreachableOrphan(TR::CFG* cfg, TR::CFGNode* from, TR::CFGNode* to)
{
    ...
    // to be unreachable.
    //
    if ((to->getPredecessors().empty() && to->getExceptionPredecessors().empty() && to != cfg->getEnd())
    ...
    if (cfg->getStructure()) {
        TR_RegionStructure* parent
            = from->asBlock()->getStructureOf()->findCommonParent(to->asBlock()->getStructureOf(), cfg);
        ...
        if (toRegion) {
            ...
            for (edge = pit.getFirst(); edge; edge = pit.getNext()) {
                if (!toRegion->contains(edge->getFrom()->asBlock()->getStructureOf()) && (edge->getFrom() != from)) {
                    onlyBackEdges = false;
                    break;
                }
            }
...
bool OMR::CFG::removeEdge(TR::CFGEdge* edge)
{
    ...
    } else if (std::find(from->getExceptionSuccessors().begin(), from->getExceptionSuccessors().end(), edge)
        != from->getExceptionSuccessors().end()) {
        found = true;
    ...
    } else if (std::find(to->getExceptionPredecessors().begin(), to->getExceptionPredecessors().end(), edge)
        != to->getExceptionPredecessors().end()) {
        found = true;
    ...
    if (doWalk) {
        ...
        do {
            ...
            for (TR::CFGEdge* e = edgesIt.getFirst(); e; e = edgesIt.getNext()) {
                ...
                if (std::find(to->getPredecessors().begin(), to->getPredecessors().end(), e)
                    != to->getPredecessors().end())
...
bool OMR::CFG::removeEdge(TR::CFGEdge* edge, bool recursiveImpl)
{
    ...
    //
    if (_removeEdgeNestingDepth > (comp()->getOption(TR_ProcessHugeMethods) ? (MAX_REMOVE_EDGE_NESTING_DEPTH * 2)
                                                                            : MAX_REMOVE_EDGE_NESTING_DEPTH)) {
    ...
    if (getStructure()) {
        TR_RegionStructure* parent
            = from->asBlock()->getStructureOf()->findCommonParent(to->asBlock()->getStructureOf(), self());
        TR_StructureSubGraphNode* toSubNode = parent->findSubNodeInRegion(to->getNumber());
        TR_ASSERT(toSubNode, "cannot have cfg edge a->b in region p where b is not a direct subnode of p");
    ...
    // to be unreachable.
    //
    if ((to->getPredecessors().empty() && to->getExceptionPredecessors().empty() && to != getEnd())
        || (to->getPredecessors().empty() && (to->getExceptionPredecessors().size() == 1)
            && to->getExceptionPredecessors().front()->getFrom() == to)
        || (to->getExceptionPredecessors().empty() && (to->getPredecessors().size() == 1)
            && to->getPredecessors().front()->getFrom() == to)) {
...
void OMR::CFG::removeUnreachableBlocks()
{
    // Prevent recursion
    //
...
bool OMR::CFG::consumePseudoRandomFrequencies()
{
    ...
    for (; nextNode != NULL; nextNode = nextNode->getNext()) {
        count++;
        int32_t f = comp()->convertNonDeterministicInput(
            nextNode->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false);
        ...
        for (TR::CFGEdge* nextEdge = sit.getFirst(); nextEdge != NULL; nextEdge = sit.getNext()) {
            count++;
            nextEdge->setFrequency(comp()->convertNonDeterministicInput(
                nextEdge->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false));
            edgeIndex++;
        }
    }
    ...
    int32_t f2
        = comp()->convertNonDeterministicInput(maxFrequency, MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false);
    count++;
    ...
    if (((origIndex + count) != curIndex) || ((origNumbers + count) == curNumbers))
        return false;

    return true;
}

bool OMR::CFG::setFrequencies()
{
...
int32_t OMR::CFG::compareExceptionSuccessors(TR::CFGNode* node1, TR::CFGNode* node2)
{
    ...
    //
    if (node1->getExceptionSuccessors().empty() && node2->getExceptionSuccessors().empty())
        return BOTH_SAME;

    if ((node1->getExceptionSuccessors().size() == 1) && (node2->getExceptionSuccessors().size() == 1)) {
        if (node1->getExceptionSuccessors().front() == node2->getExceptionSuccessors().front())
            return BOTH_SAME;
    }

    if (node2->getExceptionSuccessors().empty() && (node1->getExceptionSuccessors().size() == 1))
        return FIRST_LARGER;

    if (node1->getExceptionSuccessors().empty() && (node2->getExceptionSuccessors().size() == 1))
...
bool alwaysTrue(TR::CFGEdge* e) { return true; }
...
TR::Block** OMR::CFG::createArrayOfBlocks(TR_AllocationKind allocKind)
{
                        ...
                        if (itr->second->top >= index) {
                            if (trace)
                                dumpOptDetails(comp(), "Block %d in a loop based at block %d\n", number,
                                    stack[itr->second->bottom]->getNumber());
...
// haven't yet all been visited.  Blocks are pushed on the stack to be visited.  The iteratorStack holds the iterators
// for the out edges for the blocks on the blockStack.
int32_t OMR::CFG::createTraversalOrder(bool forward, TR_AllocationKind allocationKind, TR_BitVector* backEdges)
{
    ...
    /*
     * This is allocated before entering the new stack memory region so that the caller can allocate it on its stack
     * scope if desired.
     */
    TR::CFGNode** traversalOrder = (TR::CFGNode**)trMemory()->allocateMemory(
        numberOfBlocks * sizeof(TR::CFGNode*), allocationKind, TR_MemoryBase::UnknownType);
    ...
    while (!blockStack.isEmpty()) {
        ...
        while (edge) {
            ...
            if ((DERIVE_BACKWARD_FROM_FORWARD && !forward) && edge == fallThroughEdge) {
                edge = iter->getNext();
                continue;
            }
            ...
            if (nextBlock->getVisitCount() != traversalVisitCount) {
                ...
                if (_compilation->getOption(TR_TraceBFGeneration))
                    dumpOptDetails(comp(), "\nFound backedge from %d to %d", edge->getFrom()->getNumber(),
        ...
        if ((DERIVE_BACKWARD_FROM_FORWARD && !forward) && !foundUnvisitedBlock && fallThroughEdge) {
            ...
            if (fallThroughBlock->getVisitCount() != traversalVisitCount) {
                ...
                if (_compilation->getOption(TR_TraceBFGeneration))
                    dumpOptDetails(comp(), "\nFound backedge from %d to %d", fallThroughEdge->getFrom()->getNumber(),
        ...
        if (!foundUnvisitedBlock) {
            // we've visited all of the top block's out edges, so add to the traversal order and remove from the block
            // stack
            TR_ASSERT(traversalInsertPosition > 0, "traversed more blocks than CFG claims to have");
            --traversalInsertPosition;
            TR_ASSERT(
                traversalOrder[traversalInsertPosition] == NULL, "inserted a block into traversal more than once");
    ...
    // TR_ASSERT(traversalInsertPosition == 0, "not all blocks in CFG found by traversal, unreachable blocks should be
    // removed first");
...
public:
    ...
    bool operator<(CFGNodeFrequencyPair& right) { return _frequency < right._frequency; }
};

TR_BlockCloner* OMR::CFG::clone()
{
    ...
    TR::Block* clonedBlock
        = cloner->cloneBlocks(comp()->getStartTree()->getNode()->getBlock(), lastTreeTop->getNode()->getBlock());
    lastTreeTop->join(clonedBlock->getEntry());

    return cloner;
}

void OMR::CFG::propagateColdInfo(bool generateFrequencies)
{
    vcount_t visitCount = comp()->incVisitCount();
    ...
    for (int b = 0; b < _forwardTraversalLength; b++) {
        ...
        if (!block->asBlock()->isCold()
            || (haveSuperColdBlocks && !block->asBlock()->isSuperCold() && !block->asBlock()->hasCallToSuperCold())) {
            ...
            if (allPredsCold)
                for (TR::CFGEdge* edge = pit.getFirst(); edge; edge = pit.getNext()) {
                    ...
                    //
                    if (haveSuperColdBlocks
                        && (block->getForwardTraversalIndex() <= edge->getFrom()->getForwardTraversalIndex())) {
                        if (comp()->getOption(TR_TraceBFGeneration))
                    ...
                    if (comp()->getOption(TR_TraceBFGeneration))
                        dumpOptDetails(comp(), "\t\tpredecessor %d coldness %d\n", edge->getFrom()->getNumber(),
                            (int)edge->getFrom()->asBlock()->isCold());
            ...
            if (allPredsCold
                && performTransformation(
                    comp(), "%smarked block_%d cold (all preds were cold)\n", OPT_DETAILS, block->getNumber())) {
                block->asBlock()->setIsCold();
                if (allPredsSuperCold && !noPropagateSuperCold)
                    block->asBlock()->setIsSuperCold();
    ...
    for (int b = 0; b < _backwardTraversalLength; b++) {
        ...
        if (!block->asBlock()->isCold()
                ...
                for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
                    if (comp()->getOption(TR_TraceBFGeneration))
                        dumpOptDetails(comp(), "\t\tsuccessor %d coldness %d very coldness %d\n",
                            edge->getTo()->getNumber(), (int)edge->getTo()->asBlock()->isCold(),
                            (int)edge->getTo()->asBlock()->isSuperCold());
            ...
            if (allSuccsCold
                && (!comp()->ilGenTrace()
                    || performTransformation(
                        comp(), "%smarked block_%d cold (all succs were cold)\n", OPT_DETAILS, block->getNumber()))) {
                block->asBlock()->setIsCold();
                if (allSuccsSuperCold && !noPropagateSuperCold)
                    block->asBlock()->setIsSuperCold();
...
void OMR::CFG::setUniformEdgeFrequenciesOnNode(
    TR::CFGNode* node, int32_t branchToCount, bool addFrequency, TR::Compilation* comp)
{
    TR::CFG* cfg = self();
...
void OMR::CFG::setEdgeFrequenciesOnNode(
    TR::CFGNode* node, int32_t branchToCount, int32_t fallThroughCount, TR::Compilation* comp)
{
    TR::Block* block = node->asBlock();
    ...
    for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
        if ((*e)->getTo() == branchToBlock) {
            (*e)->setFrequency(branchToCount);
            comp->getFlowGraph()->setEdgeProbability(
                *e, (float)branchToCount / (float)(branchToCount + fallThroughCount));
        } else {
            (*e)->setFrequency(fallThroughCount);
            comp->getFlowGraph()->setEdgeProbability(
                *e, (float)fallThroughCount / (float)(branchToCount + fallThroughCount));
        }
...
bool OMR::CFG::setEdgeFrequenciesFrom()
{
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (block->getEntry() != NULL && (node->getSuccessors().size() == 2)
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (block->getEntry() != NULL && (node->getSuccessors().size() == 1) && node->getSuccessors().front()->getTo()
            ...
            if (edgeFrequency > _max_edge_freq)
                edgeFrequency = _max_edge_freq;
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
                dumpOptDetails(comp(), "Edge %p between %d and %d has freq %d (GOTO forward pass)\n", es,
                    es->getFrom()->getNumber(), es->getTo()->getNumber(), es->getFrequency());
        }
    }
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            if (pred->getEntry() != NULL && (pred->getSuccessors().size() == 1)
                ...
                if (_compilation->getOption(TR_TraceBFGeneration))
                    dumpOptDetails(comp(), "Edge %p between %d and %d has freq %d (GOTO backward pass)\n", *e,
                        (*e)->getFrom()->getNumber(), (*e)->getTo()->getNumber(), (*e)->getFrequency());
            }
        }
    }
...
void OMR::CFG::computeEntryFactorsFrom(TR_Structure* str, float& factor)
{
    if (!str)
        return;
...
void OMR::CFG::propagateEntryFactorsFrom(TR_Structure* str, float factor)
{
    if (!str)
        return;
    TR_RegionStructure* region = str->asRegion();
    if (region == NULL) {
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(), "Set block frequency on block_%d to %d, current factor %lf\n", block->getNumber(),
                block->getFrequency(), factor);
        return;
    }

    factor *= region->getFrequencyEntryFactor();

    if (factor > MAX_REGION_FACTOR)
        factor = MAX_REGION_FACTOR;
...
float OMR::CFG::computeOutsideEdgeFactor(TR::CFGEdge* outEdge, TR::CFGNode* pred)
{
...
float OMR::CFG::computeInsideEdgeFactor(TR::CFGEdge* inEdge, TR::CFGNode* pred)
{
    float factor = (float)pred->asBlock()->getFrequency() / (float)INITIAL_BLOCK_FREQUENCY_FACTOR;

    // dumpOptDetails(comp(),"For pred %d inEdgeFrequency %d block freq %d factor %f\n", pred->getNumber(),
    // inEdge->getFrequency(), pred->asBlock()->getFrequency(), factor);
...
void OMR::CFG::computeEntryFactorsAcyclic(TR_RegionStructure* region)
{
...
void OMR::CFG::computeEntryFactorsLoop(TR_RegionStructure* region)
{
    float factor = region->getFrequencyEntryFactor();
    TR::Block* start = region->getEntryBlock();
...
void OMR::CFG::propagateFrequencyInfoFrom(TR_Structure* str)
{
    if (!str)
        return;
    TR_RegionStructure* region = str->asRegion();
...
void OMR::CFG::processAcyclicRegion(TR_RegionStructure* region) { walkStructure(region); }

void OMR::CFG::processNaturalLoop(TR_RegionStructure* region) { walkStructure(region); }

static void setFrequencyOnSuccessor(TR_RegionStructure* region, TR::CFGNode* succ, TR::CFGNode* start, int32_t succNum,
    int32_t sumEdgeFrequency, int32_t parentFrequency, int32_t* _regionFrequencies, ListIterator<TR::CFGEdge> i,
    TR::Compilation* _compilation, int32_t max_edge_freq)
{
    if (toStructureSubGraphNode(succ)->getStructure() && toStructureSubGraphNode(succ)->getStructure()->getParent()
        ...
        int32_t factoredFrequency
            = (int32_t)((float)parentFrequency * (float)thisEdgeFrequency / (float)sumEdgeFrequency);
        ...
        if ((thisEdgeFrequency == max_edge_freq) && (sumEdgeFrequency == succNum * max_edge_freq)) {
            if (succBlock) {
                if (!succBlock->isCold()) {
...
static void summarizeEdgeFrequencies(ListIterator<TR::CFGEdge> i, int32_t* sumEdgeFreq, int32_t* edgeCount)
{
    for (TR::CFGEdge* e = i.getFirst(); e; e = i.getNext()) {
...
*/
static void addAllInnerBlocksToLoop(TR_RegionStructure* region, TR_SuccessorIterator sit, TR_BitVector* _seenNodes,
    TR_ScratchList<TR::CFGNode>* stack, TR::Compilation* _compilation)
{
    for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
        TR::CFGNode* succ = edge->getTo();

        if (!_seenNodes->isSet(succ->getNumber()) && toStructureSubGraphNode(succ)->getStructure()
...
static void generalFrequencyPropagation(TR_RegionStructure* region, TR_SuccessorIterator sit, TR::Block* block,
    TR_BitVector* _seenNodes, int32_t* _regionFrequencies, TR_ScratchList<TR::CFGNode>* stack,
...
int32_t OMR::CFG::getLowFrequency() { return LOW_FREQ; }

int32_t OMR::CFG::getAvgFrequency() { return AVG_FREQ; }

void OMR::CFG::walkStructure(TR_RegionStructure* region)
{
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
    ...
    for (int32_t k = 0; k < numRegions; k++)
        _regionFrequencies[k] = INITIAL_BLOCK_FREQUENCY_FACTOR;
    ...
    while (!stack.isEmpty() && !_foundIllegalStructure) {
        ...
        if (regionStr != NULL) {
            ...
            if (sumEdgeFrequency > 0) {
                for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
                    ...
                    setFrequencyOnSuccessor(region, succ, start, succNum, sumEdgeFrequency, parentFrequency,
                        _regionFrequencies, i, _compilation, _max_edge_freq);
                }
            }
        ...
        if (block->getEntry()) {
            ...
            if ((block->getSuccessors().size() == 2)
                ...
                addAllInnerBlocksToLoop(region, sit, _seenNodes, &stack, _compilation);
            } else if ((block->getSuccessors().size() == 1) && block->getSuccessors().front()->getTo()
                    ...
                    if (branchToBlock) {
                        if (!branchToBlock->isCold()) {
                            if ((branchToBlock->getFrequency() + blockFreq) > INITIAL_BLOCK_FREQUENCY_FACTOR) {
                                if (_compilation->getOption(TR_TraceBFGeneration))
                                    dumpOptDetails(comp(),
                                        "Setting frequency of %d on block_%d (GOTO block) - MAX block frequency\n",
                                ...
                                if (_compilation->getOption(TR_TraceBFGeneration))
                                    dumpOptDetails(comp(),
                                        "Adding frequency of %d on block_%d (GOTO block), prev frequency was %d\n",
                                        blockFreq, branchToBlock->getNumber(),
                                        branchToBlock->getFrequency() - blockFreq);
                            }
                        }
                    } else {
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure()
{
    if (_compilation->getOption(TR_TraceBFGeneration))
    ...
    if (setBlockFrequencies) {
        if (_compilation->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(),
                "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating block and edge frequencies within "
                "regions...\n");
        ...
        if (getStructure()) {
            ...
            for (node = getFirstNode(); node; node = node->getNext()) {
                ...
                if ((frequency < 0) || (frequency > MAX_COLD_BLOCK_COUNT))
...

        {
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
                dumpOptDetails(comp(),
                    "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Computing region weight factors based on CFG "
                    "structure, num regions=%d...\n",
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
                dumpOptDetails(comp(),
                    "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating weight factors based on CFG "
                    "structure...\n");
...
void OMR::CFG::getBranchCounters(
    TR::Node* node, TR::Block* block, int32_t* taken, int32_t* notTaken, TR::Compilation* comp)
{
    ...
    if (((branchToBlock->getFrequency() >= 0) && (fallThroughBlock->getFrequency() >= 0))
        ...
        *notTaken = fallThroughFrequency; //(fallThroughFrequency*blockFreq)/(branchToFrequency + fallThroughFrequency);
        ...
        return;
    } else if ((branchToBlock->getPredecessors().size() == 1) && (fallThroughBlock->getPredecessors().size() == 1)
        ...
        *taken = branchToBlock->getFrequency(); // (branchToBlock->getFrequency()*blockFreq)/(branchToBlock->getFrequency()
                                                // + fallThroughBlock->getFrequency());
        *notTaken = fallThroughBlock->getFrequency(); //(fallThroughBlock->getFrequency()*blockFreq)/(branchToBlock->getFrequency()
                                                      //+ fallThroughBlock->getFrequency());
        ...
        while (parent) {
            ...
            if (parent->isNaturalLoop()) {
                if (node->getOpCode().isIf()) {
                    ...
                    // branchToBlock->getNumber(), fallThroughBlock->getNumber());
...
int32_t TR::CFGNode::denormalizedFrequency(int32_t normalizedFrequency, int32_t maxFrequency)
{
    ...
    // so cast to int64_t
    int32_t frequency = (int32_t)((int64_t)((maxFrequency * (int64_t)(normalizedFrequency - MAX_COLD_BLOCK_COUNT)))
        / MAX_BLOCK_COUNT);
    return frequency;
}
...
{
    int32_t frequency = getFrequency();
    // dumpOptDetails("Edge %p between %d and %d has freq %d (_maxEdgeFrequency %d)\n", this, getFrom()->getNumber(),
    // getTo()->getNumber(), getFrequency(), maxEdgeFrequency);

    if (frequency <= MAX_COLD_BLOCK_COUNT) {
        if (!getTo()->asBlock()->isCold() && !getFrom()->asBlock()->isCold())
    ...
    if (_maxFrequency < 0) {
        for (node = getFirstNode(); node; node = node->getNext()) {
            ...
            if (frequency > _maxFrequency) {
                if (comp()->getOption(TR_TraceBFGeneration))
                    traceMsg(
                        comp(), "22maxFrequency old %d new %d node %d\n", _maxFrequency, frequency, node->getNumber());
...
void OMR::CFG::resetFrequencies()
{
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        if (nodesToBeReset->get(node->getNumber()) && !node->asBlock()->isCold()) {
            node->setFrequency(-1);
        }
...
{
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
    TR_Array<TR::CFGEdge*>* edgesArray
        = new (trStackMemory()) TR_Array<TR::CFGEdge*>(trMemory(), _numEdges, true, stackAlloc);
...
void OMR::CFG::findReachableBlocks(TR_BitVector* result)
{
    ...
    if (edgesToBeRemoved.isEmpty())
        return;
    // the below code is just a second pass to verify that removing edges from start to the alwayskeepblocks doesn't
    // make the alwayskeepblocks unreachable this is possible when there is a cycle in the CFG
...
TR::Region& OMR::CFG::getInternalMemoryRegion() { return self()->_internalMemoryRegion; }
