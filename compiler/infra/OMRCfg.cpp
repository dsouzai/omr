...
TR::CFGEdge *TR::CFGEdge::createExceptionEdge(TR::CFGNode *pF, TR::CFGNode *pT, TR_Memory *trMemory,
...
bool OMR::CFG::consumePseudoRandomFrequencies()
{
    ...
    for (; nextNode != NULL; nextNode = nextNode->getNext()) {
        count++;
        int32_t f = comp()->convertNonDeterministicInput(nextNode->getFrequency(),
            MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false);
        ...
        for (TR::CFGEdge *nextEdge = sit.getFirst(); nextEdge != NULL; nextEdge = sit.getNext()) {
            count++;
            nextEdge->setFrequency(comp()->convertNonDeterministicInput(nextEdge->getFrequency(),
                MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false));
            edgeIndex++;
        }
    }
...
int32_t OMR::CFG::createTraversalOrder(bool forward, TR_AllocationKind allocationKind, TR_BitVector *backEdges)
{
     ...
     * scope if desired.
     */
    TR::CFGNode **traversalOrder = (TR::CFGNode **)trMemory()->allocateMemory(numberOfBlocks * sizeof(TR::CFGNode *),
        allocationKind, TR_MemoryBase::UnknownType);
    ...
    while (!blockStack.isEmpty()) {
        ...
        if (!foundUnvisitedBlock) {
            ...
            TR_ASSERT(traversalOrder[traversalInsertPosition] == NULL,
                "inserted a block into traversal more than once");
...
void OMR::CFG::setUniformEdgeFrequenciesOnNode(TR::CFGNode *node, int32_t branchToCount, bool addFrequency,
    TR::Compilation *comp)
{
    TR::CFG *cfg = self();
...
void OMR::CFG::setEdgeFrequenciesOnNode(TR::CFGNode *node, int32_t branchToCount, int32_t fallThroughCount,
    TR::Compilation *comp)
{
    TR::Block *block = node->asBlock();
    ...
    for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
        if ((*e)->getTo() == branchToBlock) {
            (*e)->setFrequency(branchToCount);
            comp->getFlowGraph()->setEdgeProbability(*e,
                (float)branchToCount / (float)(branchToCount + fallThroughCount));
        } else {
            (*e)->setFrequency(fallThroughCount);
            comp->getFlowGraph()->setEdgeProbability(*e,
                (float)fallThroughCount / (float)(branchToCount + fallThroughCount));
        }
...
void OMR::CFG::getBranchCounters(TR::Node *node, TR::Block *block, int32_t *taken, int32_t *notTaken,
    TR::Compilation *comp)
{
