...
public:
    ...
    TR_HashTabIterator()
        : _curHashIndex(0)
    { }
    TR_HashTabIterator(TR_HashTab* h)
        : _baseHashTab(h)
        , _curHashIndex(0)
    { }
    void* getCurrent();
...
protected:
    enum { kMinimumSize = 16, kDefaultSize = 64 };
...
public:
    ...
    // assume a pointer
    virtual TR_HashId calculateHash(const void* key) const
    {
        return (TR_HashId((uintptr_t)key) >> 2) % _closedAreaSize;
    }
    virtual bool isEqual(const void* key1, const void* key2) const { return key1 == key2; }

    TR_HashTab(TR_Memory* mem, TR_AllocationKind a = heapAlloc, uint32_t initialSize = kDefaultSize, bool g = true)
        : _tableSize(0)
        , _allocType(a)
        , _table(NULL)
        , _closedAreaSize(0)
    ...
    {
        _tableSize = 0;
        if ((_allocType == persistentAlloc) && (_table != NULL)) {
    ...
    void setKey(TR_HashId id, void* key)
    {
        _table[id]->_key = key;
        return;
    };
...
public:
    TR_HashTabInt(TR_Memory* mem, TR_AllocationKind allocType = heapAlloc, uint32_t initialSize = kDefaultSize,
    ...
    bool locate(int32_t key, TR_HashId& hashIndex)
    {
        return TR_HashTab::locate((const void*)(uintptr_t)key, hashIndex);
    }
    bool add(int32_t key, TR_HashId hashIndex, void* data)
    {
        return TR_HashTab::add((void*)(uintptr_t)key, hashIndex, data);
    }
};

class TR_HashTabString : public TR_HashTab {
public:
    TR_HashTabString(TR_Memory* mem, TR_AllocationKind allocType = heapAlloc, uint32_t initialSize = kDefaultSize,
        bool allowGrowth = true)
        : TR_HashTab(mem, allocType, initialSize, allowGrowth)
    { }
...
public:
    TR_HashTabDouble(TR_Memory* mem, TR_AllocationKind allocType = heapAlloc, uint32_t initialSize = kDefaultSize,
        bool allowGrowth = true)
        : TR_HashTab(mem, allocType, initialSize, allowGrowth)
    { }
    ...
    bool isEqual(const void* key1, const void* key2) const
    {
...
public:
    TR_HashTabLong(TR_Memory* mem, TR_AllocationKind allocType = heapAlloc, uint32_t initialSize = kDefaultSize,
        bool allowGrowth = true)
        : TR_HashTab(mem, allocType, initialSize, allowGrowth)
    { }
...
public:
    TR_HashTabFloat(TR_Memory* mem, TR_AllocationKind allocType = heapAlloc, uint32_t initialSize = kDefaultSize,
        bool allowGrowth = true)
        : TR_HashTab(mem, allocType, initialSize, allowGrowth)
    { }
