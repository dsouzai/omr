...
static inline bool contiguousBits(uint32_t lmask) { return contiguousBits((int32_t)lmask); }
...
static inline bool contiguousBits(uint64_t lmask) { return contiguousBits((int64_t)lmask); }
...
static inline int32_t leadingZeroes(int32_t inputWord) { return __cntlz4(inputWord); }

#ifdef TR_HOST_64BIT
// Return a count 0..64 of leading zeroes in the given doubleword
static inline int32_t leadingZeroes(int64_t input) { return __cntlz8(input); }
#else
// Return a count 0..64 of leading zeroes in the given doubleword
...
static inline int32_t leadingZeroes(uint64_t input) { return (leadingZeroes((int64_t)input)); }

static inline int32_t leadingZeroes(uint32_t inputWord) { return leadingZeroes((int32_t)inputWord); }
...
static inline int32_t trailingZeroes(uint32_t inputWord) { return trailingZeroes((int32_t)inputWord); }
...
static inline int32_t trailingZeroes(uint64_t input) { return trailingZeroes((int64_t)input); }

static inline int32_t ceilingPowerOfTwo(int32_t inputWord) { return 1 << (32 - leadingZeroes(inputWord - 1)); }

static inline int32_t floorPowerOfTwo(int32_t inputWord) { return 1 << (31 - leadingZeroes(inputWord)); }

static inline int64_t floorPowerOfTwo64(int64_t inputWord) { return 1LL << (63 - leadingZeroes(inputWord)); }

static inline int32_t leadingOnes(int32_t inputWord) { return leadingZeroes(~inputWord); }

static inline int32_t leadingOnes(uint32_t inputWord) { return leadingZeroes(~inputWord); }

static inline int32_t leadingOnes(int64_t input) { return leadingZeroes(~input); }

static inline int32_t leadingOnes(uint64_t input) { return leadingZeroes(~input); }

#if defined(TR_HOST_ARM64) && (defined(__GNUC__) || defined(__clang__))
// return the number of 1-bits in the argument
static inline int32_t populationCount(int32_t inputWord) { return __builtin_popcount(inputWord); }
#else
// return the number of 1-bits in the argument
...
static inline int32_t populationCount(uint32_t inputWord) { return populationCount((int32_t)inputWord); }

#if defined(TR_HOST_ARM64) && (defined(__GNUC__) || defined(__clang__))
// return the number of 1-bits in the argument
static inline int32_t populationCount(int64_t inputWord) { return __builtin_popcountll(inputWord); }
#else
// return the number of 1-bits in the argument
...
static inline int32_t populationCount(uint64_t inputWord) { return populationCount((int64_t)inputWord); }

// return 10^exponent
static inline uint64_t computePositivePowerOfTen(int32_t exponent)
{
    // TODO: there is a better algorithm to reduce the number of multiplies -- see Simplifier.cpp
    // reduceExpTwoAndGreaterToMultiplication
    TR_ASSERT(exponent >= 0 && exponent <= TR::getMaxSignedPrecision<TR::Int64>(),
        "exponent %d should be in the inclusive range 0->%d\n", exponent, TR::getMaxSignedPrecision<TR::Int64>());
    uint64_t base = 1;
...
static inline bool isEven(int32_t input) { return (input & 0x1) == 0; }

static inline bool isOdd(int32_t input) { return (input & 0x1) != 0; }

static inline bool isEven(uint32_t input) { return isEven((int32_t)input); }

static inline bool isOdd(uint32_t input) { return isOdd((int32_t)input); }

static inline bool isEven(int64_t input) { return (input & 0x1) == 0; }

static inline bool isOdd(int64_t input) { return (input & 0x1) != 0; }

static inline bool isEven(uint64_t input) { return isEven((int64_t)input); }

static inline bool isOdd(uint64_t input) { return isOdd((int64_t)input); }
...
static inline bool isNonPositivePowerOf2(int32_t input) { return (input & -input) == -input; }
 ...
 *         \c false, otherwise
 */
static inline bool isPowerOf2(uint32_t input) { return (input != 0) && (input & (input - 1)) == 0; }
...
static inline bool isNonPositivePowerOf2(int64_t input) { return (input & -input) == -input; }
 ...
 *         \c false, otherwise
 */
static inline bool isPowerOf2(uint64_t input) { return (input != 0) && (input & (input - 1)) == 0; }
