...
public:
    ...
    template<typename AllocatorType> static TR::Symbol* create(AllocatorType, TR::DataType);
...
public:
    ...
    enum {
        ...
        SlotSharedByRefAndNonRef
        = 0x00400000, ///< used in fsd to indicate that an reference symbol shares a slot with a nonreference symbol
        ...
        SpillTempLoaded = 0x04000000, ///< share bit with loadaddr because spill temps will never have their address
                                      ///< taken. Used to remove store to spill if never loaded
        // Available              = 0x02000000,
        VariableSizeSymbol = 0x01000000, ///< non-java only?: specially managed automatic symbols that contain both an
        ...
        StaticAddressWithinMethodBounds
        = 0x00800000, // Address is inside a method body and can be accessed with RIP addressing without relocations
        ...
        NonLinear
        = 0x01000000, // TAROK and temporary.  This bit is used in conjunction with StartOfColdInstructionStream
...
protected:
     ...
     */
public:
    template<typename AllocatorType> static TR::Symbol* createShadow(AllocatorType m);

    template<typename AllocatorType> static TR::Symbol* createShadow(AllocatorType m, TR::DataType d);

    template<typename AllocatorType> static TR::Symbol* createShadow(AllocatorType m, TR::DataType d, uint32_t);
