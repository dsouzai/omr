...
uint16_t OMR::Node::addChildrenAndSymRef(
    uint16_t childIndex, TR::Node* child, ChildrenAndSymRefType... childrenAndSymRef)
{
...
template<class... Children>
TR::Node* OMR::Node::createWithoutSymRef(
...
TR_UseDefAliasSetInterface OMR::Node::mayKill(bool gcSafe)
{
    if (self()->getOpCode().hasSymbolReference()
        && (self()->getOpCode().isLikeDef()
            || self()->mightHaveVolatileSymbolReference())) // we want the old behavior in these cases
    {
        bool shares_symbol = self()->getSymbolReference()->sharesSymbol(gcSafe);
        TR_UseDefAliasSetInterface aliasSetInterface(
            shares_symbol, self()->getSymbolReference(), self()->getOpCode().isCallDirect(), gcSafe);
        return aliasSetInterface;
    } else {
        // if there is no symbolreference, then return an empty aliseset
