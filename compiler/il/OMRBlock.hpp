 ...
 * --> At any given point during compilation, the code compiled is represented by a linked list of treetops, which are
...
public:
    ...
    {
        TR_ASSERT(
            0, "blocks should not be freed until the destructor is updated to also free the catch block extension");
    ...
    static void redirectFlowToNewDestination(
    ...
    TR::Block* split(TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool fixupCommoning = false,
        bool copyExceptionSuccessors = true, TR::ResolvedMethodSymbol* methodSymbol = NULL);

    TR::Block* splitPostGRA(TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool copyExceptionSuccessors = true,
        TR::ResolvedMethodSymbol* methodSymbol = NULL);

    TR::Block* splitWithGivenMethodSymbol(TR::ResolvedMethodSymbol* methodSymbol, TR::TreeTop* startOfNewBlock,
        TR::CFG* cfg, bool fixupCommoning = false, bool copyExceptionSuccessors = true);

    TR::Block* createConditionalSideExitBeforeTree(TR::TreeTop* tree, TR::TreeTop* compareTree, TR::TreeTop* exitTree,
        TR::TreeTop* returnTree, TR::CFG* cfg, bool markCold = true);

    TR::Block* createConditionalBlocksBeforeTree(TR::TreeTop* tree, TR::TreeTop* compareTree, TR::TreeTop* ifTree,
        TR::TreeTop* elseTree, TR::CFG* cfg, bool changeBlockExtensions = true, bool markCold = true);

    TR::Block* splitEdge(TR::Block*, TR::Block*, TR::Compilation*, TR::TreeTop** lastTreeTop = NULL,
        bool findOptimalInsertionPoint = true);
    ...
    public:
        ...
        TR_CatchBlockExtension()
            ...
            , _exceptionClassNameChars(NULL)
            , _exceptionClassNameLength(0)
            , _catchType(0)
            , _exceptionsCaught(0)
            , _handlerIndex(0)
            , _inlineDepth(0)
            , _owningMethod(NULL)
            , _isSyntheticHandler(false)
        { }

        TR_CatchBlockExtension(TR_CatchBlockExtension& other)
            : _exceptionClass(other._exceptionClass)
            , _exceptionClassNameChars(other._exceptionClassNameChars)
            , _exceptionClassNameLength(other._exceptionClassNameLength)
            , _catchType(other._catchType)
            , _exceptionsCaught(other._exceptionsCaught)
            , _handlerIndex(other._handlerIndex)
            , _inlineDepth(other._inlineDepth)
            , _owningMethod(other._owningMethod)
            , _byteCodeInfo(other._byteCodeInfo)
            , _isSyntheticHandler(other._isSyntheticHandler)
        { }
        ...
        bool _isSyntheticHandler; // indicate whether the exception handler is inserted by the compiler rather than
    ...
    void setHandlerInfoWithOutBCInfo(uint32_t c, uint8_t d, int32_t i, TR_ResolvedMethod* m,
        TR::Compilation* comp); // also used for estimatecodesize dummy blocks
...
private:
    ...
    TR::Block* splitBlockAndAddConditional(
        TR::TreeTop* tree, TR::TreeTop* compareTree, TR::CFG* cfg, bool newBlockShouldExtend);
...
struct BlockMapper : TR_Link<BlockMapper> {
    BlockMapper(TR::Block* f, TR::Block* t)
        : _from(f)
        , _to(t)
    { }
    TR::Block* _from;
