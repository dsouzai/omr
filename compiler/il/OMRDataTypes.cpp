...
const char* OMR::DataType::getName(TR::DataType dt)
{
    if (dt.isVector()) {
        ...
        TR_ASSERT_FATAL(
            staticallyInitialized && (OMRDataTypeNames[dt] != NULL), "Vector names should've been initialized");
    } else if (dt.isMask()) {
        // to avoid any race conditions, initialize all mask names once,
        // as soon as first one is requested
        static bool staticallyInitialized = initMaskNames();
        TR_ASSERT_FATAL(
            staticallyInitialized && (OMRDataTypeNames[dt] != NULL), "Mask names should've been initialized");
    }
