...
TR::SymbolReference* OMR::SymbolReference::self() { return static_cast<TR::SymbolReference*>(this); }
 ...
 * constructors.
 */
void OMR::SymbolReference::init(TR::SymbolReferenceTable* symRefTab, uint32_t refNumber, TR::Symbol* sym,
    intptr_t offset, mcount_t owningMethodIndex, int32_t cpIndex, int32_t unresolvedIndex, bool checkNoNamedShadow)
{
...
OMR::SymbolReference::SymbolReference(TR::SymbolReferenceTable* symRefTab, TR::Symbol* symbol, intptr_t offset)
{
    self()->init(symRefTab, symRefTab->assignSymRefNumber(self()), symbol, offset);
}

OMR::SymbolReference::SymbolReference(
    TR::SymbolReferenceTable* symRefTab, int32_t refNumber, TR::Symbol* ps, intptr_t offset)
{
    self()->init(symRefTab, refNumber, ps, offset);
}

OMR::SymbolReference::SymbolReference(TR::SymbolReferenceTable* symRefTab,
    TR::SymbolReferenceTable::CommonNonhelperSymbol number, TR::Symbol* ps, intptr_t offset)
{
 ...
 */
OMR::SymbolReference::SymbolReference(
    TR::SymbolReferenceTable* symRefTab, TR::Symbol* symbol, intptr_t offset, const char* name)
{
    self()->init(symRefTab, symRefTab->assignSymRefNumber(self()), symbol, offset,
 ...
 */
void* OMR::SymbolReference::getMethodAddress()
{
 ...
 */
TR::ResolvedMethodSymbol* OMR::SymbolReference::getOwningMethodSymbol(TR::Compilation* c)
{
 ...
 */
TR_ResolvedMethod* OMR::SymbolReference::getOwningMethod(TR::Compilation* c)
{
    return self()->getOwningMethodSymbol(c)->getResolvedMethod();
}

bool OMR::SymbolReference::maybeVolatile()
{
    if (_symbol->isVolatile())
        return true;

    if (self()->isUnresolved() && !_symbol->isConstObjectRef() && (_symbol->isShadow() || _symbol->isStatic()))
        return true;

    return false;
}

bool OMR::SymbolReference::isUnresolvedFieldInCP(TR::Compilation* c)
{
...
bool OMR::SymbolReference::isUnresolvedMethodInCP(TR::Compilation* c)
{
    TR_ASSERT(c->compileRelocatableCode() && self()->getSymbol()->isMethod(),
        "isUnresolvedMethodInCP only callable in AOT compiles on method symbols");
...
bool OMR::SymbolReference::reallySharesSymbol(TR::Compilation* c)
{
    return (
        _flags.testAny(ReallySharesSymbol) || (c->hasUnsafeSymbol() && (_symbol->isStatic() || _symbol->isShadow())));
}
 ...
 */
void OMR::SymbolReference::copyFlags(TR::SymbolReference* sr) { _flags.set(sr->_flags); }

bool OMR::SymbolReference::isThisPointer()
{
    TR::Compilation* c = TR::comp();
    TR::ParameterSymbol* p = _symbol->getParmSymbol();
    return p && p->getSlot() == 0 && !self()->getOwningMethod(c)->isStatic();
}

bool OMR::SymbolReference::isTemporary(TR::Compilation* c)
{
...
TR::SymbolReference* OMR::SymbolReference::create(
    TR::SymbolReferenceTable* symRefTab, TR::Symbol* sym, TR::KnownObjectTable::Index koi)
{
...
TR::KnownObjectTable::Index OMR::SymbolReference::getKnownObjectIndex()
{
    if (self()->getSymbol()) {
        ...
        if (parm && parm->hasKnownObjectIndex()) {
            if (_knownObjectIndex != TR::KnownObjectTable::UNKNOWN)
                TR_ASSERT(self()->getKnownObjectIndex() == parm->getKnownObjectIndex(),
                    "Parm symbol and symref known-object indexes must match (%d != %d)", self()->getKnownObjectIndex(),
                    parm->getKnownObjectIndex());
...
bool OMR::SymbolReference::hasKnownObjectIndex()
{
    return self()->getKnownObjectIndex() != TR::KnownObjectTable::UNKNOWN;
}

uintptr_t* OMR::SymbolReference::getKnownObjectReferenceLocation(TR::Compilation* comp)
{
...
void OMR::SymbolReference::setAliasedTo(TR::SymbolReference* other, bool symmetric)
{
    {
        TR_ASSERT(_useDefAliases, "this symref doesn't have its own aliasing bitvector");
        _useDefAliases->set(other->getReferenceNumber());
        if (symmetric)
            other->setAliasedTo(self(), false);
    }
}

void OMR::SymbolReference::setEmptyUseDefAliases(TR::SymbolReferenceTable* symRefTab)
{
    TR::Compilation* comp = symRefTab->comp();
    _useDefAliases = new (comp->trHeapMemory())
        TR_BitVector(comp->getSymRefTab()->getNumSymRefs(), comp->trMemory(), heapAlloc, growable);
}

void OMR::SymbolReference::setUseDefAliases(TR_BitVector* bv)
{
    _useDefAliases = bv;
...
bool OMR::SymbolReference::canCauseGC() { return self()->canGCandReturn() || self()->canGCandExcept(); }

bool OMR::SymbolReference::isLitPoolReference()
{
    return self()->isLiteralPoolAddress() || self()->isFromLiteralPool();
}

void OMR::SymbolReference::setHasBeenAccessedAtRuntime(TR_YesNoMaybe ynm)
{
    _flags.setValue(AccessedAtRuntimeMask, ynm * AccessedAtRuntimeBase);
}

TR_YesNoMaybe OMR::SymbolReference::hasBeenAccessedAtRuntime()
{
    return (TR_YesNoMaybe)(_flags.getValue(AccessedAtRuntimeMask) / AccessedAtRuntimeBase);
}

bool OMR::SymbolReference::isOSRInductionHelper()
{
