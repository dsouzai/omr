...
TR::Block* OMR::Block::asBlock() { return self(); }
    ...
    : TR::CFGNode(m)
{
    ...
    self()->setFrequency(-1);
    self()->setUnrollFactor(0);
    if (entry && entry->getNode())
        entry->getNode()->setBlock(self());
    if (exit && exit->getNode())
        exit->getNode()->setBlock(self());
}
...
void OMR::Block::init(TR::TreeTop* entry, TR::TreeTop* exit)
{
    _pEntry = entry;
    _pExit = exit;
    _pStructureOf = NULL;
...
TR::Block* OMR::Block::createBlock(TR::TreeTop* entry, TR::TreeTop* exit, TR::CFG& cfg)
{
    ...
    , _catchBlockExtension(NULL)
{
    ...
    if (other._catchBlockExtension) {
        _catchBlockExtension = new (other._region) TR_CatchBlockExtension(*other._catchBlockExtension);
    }
...
TR::Block* OMR::Block::getNextBlock()
{
    TR::TreeTop* t = _pExit->getNextTreeTop();
    return t ? t->getNode()->getBlock() : 0;
}

TR::Block* OMR::Block::getPrevBlock()
{
    TR::TreeTop* t = _pEntry->getPrevTreeTop();
    return t ? t->getNode()->getBlock() : 0;
}

TR::Block* OMR::Block::getNextExtendedBlock()
{
...
TR::TreeTop* OMR::Block::append(TR::TreeTop* tt) { return self()->getExit()->insertBefore(tt); }

/**
 * Prepend a OMR::Block::node under a treetop immediately after the bbStart
 */
TR::TreeTop* OMR::Block::prepend(TR::TreeTop* tt) { return self()->getEntry()->insertAfter(tt); }

TR::TreeTop* OMR::Block::getFirstRealTreeTop()
{
...
TR::TreeTop* OMR::Block::getLastRealTreeTop()
{
...
TR::TreeTop* OMR::Block::getLastNonControlFlowTreeTop()
{
    TR::TreeTop* tt = self()->getLastRealTreeTop();
    while (tt->getNode()->getOpCode().isBranch() || tt->getNode()->getOpCode().isReturn()
...
int32_t OMR::Block::getNumberOfRealTreeTops()
{
...
TR::Block* OMR::Block::createEmptyBlock(TR::Node* n, TR::Compilation* comp, int32_t frequency, TR::Block* block)
{
    ...
    TR::Block* b = new (comp->trHeapMemory()) TR::Block(TR::TreeTop::create(comp, TR::Node::create(n, TR::BBStart)),
        TR::TreeTop::create(comp, TR::Node::create(n, TR::BBEnd)), comp->trMemory());
    b->getEntry()->join(b->getExit());
    b->setFrequency(frequency);
    return b;
}

int32_t OMR::Block::getMaxColdFrequency(TR::Block* b1, TR::Block* b2)
{
    int32_t max = -1;
...
int32_t OMR::Block::getMinColdFrequency(TR::Block* b1, TR::Block* b2)
{
    int32_t min = INT_MAX;
    int32_t frequency = b1->getFrequency();
    if (b1->isCold())
...
int32_t OMR::Block::getScaledSpecializedFrequency(int32_t fastFrequency)
{
    return (int32_t)((float)(1.0f - MIN_PROFILED_FREQUENCY) * fastFrequency);
}

bool OMR::Block::isGotoBlock(TR::Compilation* comp, bool allowPrecedingSnapshots)
{
    bool isGoToBlock
        = (self()->getEntry() != NULL && self()->getLastRealTreeTop()->getPrevTreeTop() == self()->getEntry()
            && self()->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto);
    if (!isGoToBlock && allowPrecedingSnapshots && comp->getOption(TR_EnableSnapshotBlockOpts)
        && comp->getMethodSymbol()->hasSnapshots()) {
        if (self()->getEntry() != NULL && self()->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto) {
            for (TR::TreeTop* tt = self()->getLastRealTreeTop()->getPrevTreeTop(); tt != self()->getEntry();
                 tt = tt->getPrevTreeTop()) {
                return false;
            }
            isGoToBlock = true;
...
bool OMR::Block::canFallThroughToNextBlock()
{
    TR::Node* lastNode = self()->getLastRealTreeTop()->getNode();
    if (lastNode->getOpCodeValue() == TR::treetop)
        lastNode = lastNode->getFirstChild();
    if (lastNode->getOpCode().isGoto() || lastNode->getOpCode().isReturn() || lastNode->getOpCodeValue() == TR::athrow
 ...
 */
bool OMR::Block::doesNotNeedLabelAtStart()
{
    ...
    for (auto thisEdge = self()->getPredecessors().begin(); thisEdge != self()->getPredecessors().end(); ++thisEdge) {
        ...
        if (from == prevBlock && from->canFallThroughToNextBlock()) {
            ...
            if (lastNode->getOpCodeValue() == TR::treetop)
                lastNode = lastNode->getFirstChild();
            if (lastNode->getOpCodeValue() != TR::lookup && lastNode->getOpCodeValue() != TR::table
 ...
 */
TR::CFGEdge* OMR::Block::getFallThroughEdgeInEBB()
{
...
TR::Block* OMR::Block::breakFallThrough(TR::Compilation* comp, TR::Block* faller, TR::Block* fallee)
{
    TR::Node* lastNode = faller->getLastRealTreeTop()->getNode();
    if (lastNode->getOpCode().isCheck() || lastNode->getOpCodeValue() == TR::treetop)
        lastNode = lastNode->getFirstChild();

    if (lastNode->getOpCode().isReturn() || lastNode->getOpCode().isGoto() || lastNode->getOpCode().isSwitch()
        || lastNode->getOpCode().isJumpWithMultipleTargets() || lastNode->getOpCodeValue() == TR::athrow)
 ...
 */
void OMR::Block::insertBlockAsFallThrough(TR::Compilation* comp, TR::Block* block, TR::Block* newFallThroughBlock)
{
    ...
    TR::TreeTop::insertTreeTops(
        comp, block->getExit(), newFallThroughBlock->getEntry(), newFallThroughBlock->getExit());
    if (!block->hasSuccessor(newFallThroughBlock))
        comp->getFlowGraph()->addEdge(block, newFallThroughBlock);
}

/**
 * comment about when edge from from -> origTo will be removed (clients should be careful that origTo doesn't
 * unexpectedly go poof!)
 * @note  redirectFlowToNewDestination will both create a new edge from "from" to "newTo" as well as remove the original
 * edge from "from" to origTo.  BE CAREFUL when calling this function that there is another predecessor edge for
 * "origTo" or else when redirectFlowToNewDestination removes the predecessor "from", "origTo" might be deleted.
 */
void OMR::Block::redirectFlowToNewDestination(
    TR::Compilation* comp, TR::CFGEdge* origEdge, TR::Block* newTo, bool useGotoForFallThrough)
{
    ...
    if (from->getEntry() == NULL) {
        ...
        // from falls through to origTo
        {
            ...
            if (useGotoForFallThrough) {
                TR::TreeTop* gotoTreeTop
                    = TR::TreeTop::create(comp, TR::Node::create(lastNode, TR::Goto, 0, newTo->getEntry()));
                if (lastNode->getOpCode().isBranch()) {
 ...
 */
void OMR::Block::removeFromCFG(TR::Compilation* c)
{
...
TR_RegionStructure* OMR::Block::getParentStructureIfExists(TR::CFG* cfg)
{
    TR_RegionStructure* parent = NULL;
    if (self()->getStructureOf() && cfg->getStructure())
        parent = self()->getStructureOf()->getParent()->asRegion();

    return parent;
}

TR_RegionStructure* OMR::Block::getCommonParentStructureIfExists(TR::Block* b, TR::CFG* cfg)
{
    TR_RegionStructure* parent = NULL;
    if (self()->getStructureOf() && b && b->getStructureOf() && cfg->getStructure())
        parent = self()->getStructureOf()->findCommonParent(b->getStructureOf(), cfg);

    return parent;
}

void OMR::Block::removeBranch(TR::Compilation* c)
{
    TR::TreeTop* tt = self()->getLastRealTreeTop();
...
TR::Block* OMR::Block::findVirtualGuardBlock(TR::CFG* cfg)
{
        ...
        //    return prevBlock;
            ...
            if (node->isTheVirtualGuardForAGuardedInlinedCall()) {
                ...
                if (node->isProfiledGuard()) {
                    TR_ASSERT(((node->getOpCodeValue() == TR::ifacmpne) || (node->getOpCodeValue() == TR::ifacmpeq)),
                        "Profile guards must use TR::ifacmpne || TR::ifacmpeq");
...
void OMR::Block::changeBranchDestination(TR::TreeTop* newDestination, TR::CFG* cfg, bool callerFixesRegdeps)
{

    TR::Node* branchNode = self()->getLastRealTreeTop()->getNode();
    TR_ASSERT(
        branchNode->getOpCode().isBranch(), "OMR::Block::changeBranchDestination: can't find the existing branch node");
    ...
    if (!self()->hasSuccessor(newDestinationBlock)) {
        ...
        if (prevDestinationBlock->getFrequency() > 0) {
            int32_t newEdgeFrequency
                = (prevEdgeFrequency * newDestinationBlock->getFrequency()) / prevDestinationBlock->getFrequency();
    ...
    if (numChildren > 0 && branchNode->getChild(numChildren - 1)->getOpCodeValue() == TR::GlRegDeps) {
        TR::Node* fromGlRegDep = branchNode->getChild(numChildren - 1);
        TR_ASSERT(
            fromGlRegDep->getOpCodeValue() == TR::GlRegDeps, "expected the branch child to be a GlRegDep.  It's not!");
        ...
        if (newDestination->getNode()->getNumChildren() == 0) {
            ...
            TR_ASSERT(toGlRegDep->getOpCodeValue() == TR::GlRegDeps,
                "expected the bbstart child to be a GlRegDep.  It's not!");
...
void OMR::Block::uncommonNodesBetweenBlocks(
    TR::Compilation* comp, TR::Block* newBlock, TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    TR_HandleInjectedBasicBlock ibb(comp, NULL, methodSymbol ? methodSymbol : comp->getMethodSymbol(),
        symbolReferenceTempsA, injectedBasicBlockTemps, symbolReferenceTempsC);
...
bool OMR::Block::hasExceptionSuccessors()
{
    bool hasExceptionSuccessors = (self()->getExceptionSuccessors().empty() == false);
    return hasExceptionSuccessors;
}

bool OMR::Block::isTargetOfJumpWhoseTargetCanBeChanged(TR::Compilation* comp)
{
    ...
    for (auto predEdge = self()->getPredecessors().begin(); predEdge != self()->getPredecessors().end(); ++predEdge) {
        ...
        if (predBlock != startBlock
            && predBlock->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets()
...
TR::Block* OMR::Block::splitWithGivenMethodSymbol(TR::ResolvedMethodSymbol* methodSymbol, TR::TreeTop* startOfNewBlock,
    TR::CFG* cfg, bool fixupCommoning, bool copyExceptionSuccessors)
{
...
static void replaceNodesInTrees(TR::Compilation* comp, TR::TreeTop* start, NodeTable* nodeInfo)
{
    ...
    while (start) {
        if (start->getNode()->getOpCodeValue() == TR::BBStart
            && !start->getNode()->getBlock()->isExtensionOfPreviousBlock())
            break;
 ...
 */
static std::pair<TR_GlobalRegisterNumber, TR_GlobalRegisterNumber> findAvailableRegister(
    TR::Compilation* comp, TR::Node* node, TR_BitVector& unavailableRegisters)
{
    ...
    std::pair<TR_GlobalRegisterNumber, TR_GlobalRegisterNumber> toReturn
        = std::make_pair<TR_GlobalRegisterNumber, TR_GlobalRegisterNumber>(-1, -1);
...
{
    // Following assert is to make sure that this function is called with node which have Global Registers associated
    // with it.
    TR_ASSERT_FATAL(node->getOpCode().isStoreReg(),
        "checkIfRegisterIsAvailable is used with %s while it is intended to use with RegStore nodes only",
 ...
 * @param StoreRegNodeTable Table containing information about node which are stored into the registers before split
 * point
...
static void gatherUnavailableRegisters(TR::Compilation* comp, TR::Node* regDeps, TR::TreeTop* currentTT,
    NodeTable* nodeInfo, StoreRegNodeTable* storeNodeInfo, List<TR::Node>* storeRegNodePostSplitPoint,
    TR_BitVector& unavailableRegisters)
{
    TR_ASSERT_FATAL(
        regDeps->getOpCodeValue() == TR::GlRegDeps, "A GlRegDeps node is required to gather unavaialble registers\n");
    for (int i = 0; i < regDeps->getNumChildren(); i++) {
        ...
        if (dep->getOpCodeValue() == TR::PassThrough) {
            ...
            if (nodeInfoEntry != nodeInfo->end()) {
                ...
                if (nodeInfoEntry->second.second == NULL) {
                    ...
                    if (storeRegNodeInfoEntry == storeNodeInfo->end()
                        && checkStoreRegNodeListForNode(dep, storeRegNodePostSplitPoint)) {
                        // Node under the PassThrough is not stored into register before the split point which means we
                        // must have stored it into the register after split point. In this case we do not have to do
                        // anything as when we replace the node post split point, it will be replaced under
                        // corresponding regstore as well.
                        needToCreateRegStore = false;
                    } else {
                        // Node under passthrough is stored into some set of registers (Not necessarily same set of
                        // registers as we only record last regStore for the node) and if those set of registers are
                        // available, then use them to create a replacement.
                        TR_ASSERT_FATAL(storeRegNodeInfoEntry != storeNodeInfo->end(),
                            "We should have a regStore pre split point that can be used to allocate register for "
                            "replacement node");
                        TR::Node* storeNode = storeRegNodeInfoEntry->second;
                        if (checkIfRegisterIsAvailable(comp, storeNode, unavailableRegisters)) {
                            // Whether the PassThrough uses same register or not, use the last recorded regStore to
                            // assign register to the node.
                            TR::Node* regLoad
                                = TR::Node::create(value, comp->il.opCodeForRegisterLoad(originalNode->getDataType()));
                            ...
                            // Course of action to analyze reg store for block splitter is to check for the registers
                            // used for the node post split point before we check the regStore information pre split
                            // point. As regStore post split point is most likely to be used later. This will reduce
                            // number of shuffles. So if we get the candidate from before split point, we do not need to
                            // check list of RegStores post split point.
                ...
                if (nodeInfoEntry->second.second != NULL && nodeInfoEntry->second.second->getOpCode().isLoadReg()
                    ...
                    && dep->getHighGlobalRegisterNumber()
                        == nodeInfoEntry->second.second->getHighGlobalRegisterNumber()) {
                    regDeps->setAndIncChild(i, nodeInfoEntry->second.second);
                    dep->recursivelyDecReferenceCount();
                } else if (needToCreateRegStore
                    && (!needToCheckStoreRegPostSplitPoint
                        || !checkStoreRegNodeListForNode(dep, storeRegNodePostSplitPoint))) {
                    // We have either used different register for replacement to value node after split point or
                    // Passthrough has corresponding regStore before spilt point, but registers were unavailable. In
                    // these cases we need to create a regStore for the PassThrough. If we have replacement then need to
                    // store the replacement to register, else, value needs to be stored.
                    TR::Node* nodeToBeStored
                        = nodeInfoEntry->second.second != NULL ? nodeInfoEntry->second.second : value;
                    TR::Node* regStore = TR::Node::create(
                        value, comp->il.opCodeForRegisterStore(originalNode->getDataType()), 1, nodeToBeStored);
                    ...
                    if (nodeToBeStored->getOpCode().isLoadConst() || nodeToBeStored->getOpCodeValue() == TR::loadaddr
                        || nodeInfoEntry->second.second == NULL) {
                        // See if we have stored constants in register before split point.
                        auto storeRegNodeInfoEntry = storeNodeInfo->find(value);
                        TR_ASSERT_FATAL(storeRegNodeInfoEntry != storeNodeInfo->end(),
                            "We have a node n%dn under PassThrough node n%dn but we did not find a regStore that is "
                            "using the info.",
                            value->getGlobalIndex(), dep->getGlobalIndex());
            ...
            TR_ASSERT_FATAL(false,
                "Expected to find only PassThrough and regLoad operations under a GlRegDepNode, but got %s\n",
 ...
 */
static TR::SymbolReference* createSymRefForNode(
    TR::Compilation* comp, TR::ResolvedMethodSymbol* methodSymbol, TR::Node* value, TR::TreeTop* insertBefore)
{
    ...
    if (value->isInternalPointer() && value->getPinningArrayPointer()) {
        ...
        TR::Symbol* symbol = symRef->getSymbol()->castToInternalPointerAutoSymbol()->setPinningArrayPointer(
        ...
        if (isInternalPointer) {
            ...
            if (value->getOpCode().isArrayRef()) {
                ...
                if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
                    ...
                    // If the node we are uncommoning is internal pointer and while iterating a child we found a node
                    // that is not Array Reference, There are only two possibilities.
                    ...
                    if (valueChildSymRef != NULL && valueChildSymRef->getSymbol()->isAuto()
                        && (valueChild->getOpCode().isLoadVarDirect()
                            || (valueChild->getOpCode().isLoadReg()
                                && valueChildSymRef->getSymbol()->castToAutoSymbol()->isInternalPointer()))) {
                        if (valueChildSymRef->getSymbol()->castToAutoSymbol()->isInternalPointer()) {
                            pinningArray = valueChildSymRef->getSymbol()
 ...
 * If there are available global registers which can be used, it will utililze them to uncommon nodes across split
 * points.
 *
 * @details
 * It uses following simplified algorithm to split blocks and uncommon nodes.
 * 1. Analyze trees before the split point to gather information about nodes which are referenced in new block and needs
 * replacement after split.
 * 2. Now as it has information about all the nodes which are required to be uncommoned, it starts cretaing replacment
 * nodes as per following rules. a. If node to be uncommoned is regLoad / load of constant then replacement node will be
 * copy of the original node and also mark the used register unavailable. b. If node to be uncommoned is stored into a
 * register after split point and that register is available, then replacement node will be regLoad from that register.
 * It also makes sure that regStore tree is moved to before split point also marks used register unavailable. c. If node
 * to be uncommoned is stored into a register before split point and node does not succeeds in [b] case and register is
 * available then replacement node will be regLoad from that register also marks used register unavailable.
 ...
 *       then it stores replacement node into a register used by passThrough before the tree containing that passthrough
 * node. b. If node under regStore does not need to be uncommoned, then mark the used register unavailable.
 * 4. For the remaining of nodes to be uncommoned, it tries to find the available register using list of unavailable
 * registers and if it can find one, then stores node into that register otherwise creates a auto/temp slot to uncommon
 * them.
 * 5. Last step is to start walking treetops after split point and replace nodes to be uncommoned with corresponding
 * replacement nodes.
 *
 * @param startOfNewBlock TreeTop from which new block will start
 * @param cfg TR::CFG object
 * @param copyExceptionSuccessors Boolean stating if we need to copy the exceptionSuccessors of the original blocks to
 * new block
 * @param methodSymbol TR::ResolvedMethodSymbol object
 * @return TR::Block* Returns new Block
 */
TR::Block* OMR::Block::splitPostGRA(
    TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool copyExceptionSuccessors, TR::ResolvedMethodSymbol* methodSymbol)
{
...

    {
        ...
        NodeTable* nodeInfo
            = new (stackMemoryRegion) NodeTable(NodeTableComparator(), NodeTableAllocator(stackMemoryRegion));
        StoreRegNodeTable* storeNodeInfo = new (stackMemoryRegion)
            StoreRegNodeTable(NodeTableComparator(), StoreRegNodeTableAllocator(stackMemoryRegion));
        ...
        /**
         * Step-1 : Analyze the trees before split point to record nodes which will be referenced in new block and needs
         * replacement after split It collects following information. a. Node which are referenced after split point in
         * nodeInfo b. Node which are referenced after split point and stored into register in storeNodeInfo
         */
        for (TR::PostorderNodeOccurrenceIterator iter(start, comp, "FIX_POSTGRA_SPLIT_COMMONING"); iter != end;
             ++iter) {
         ...
         *       1. If node under PassThrough node is stored into the same register after split point, we do not have to
         * do anything.
         *       2. If node is stored into same register before split point and if register is available, then use the
         * information to store the node. In other cases where register used in the PassThrough is not available, then
         * create a Store into a register same as PassThrough before the tree containing the passThrough node.
         */
        List<TR::Node> storeRegNodePostSplitPoint(stackMemoryRegion);
        TR::TreeTop* nextExtendedBlockEntry
            = newBlock->getNextExtendedBlock() ? newBlock->getNextExtendedBlock()->getEntry() : NULL;
        for (TR::TreeTop* iter = newBlock->getEntry(); iter != nextExtendedBlockEntry;) {
            ...
            if (node->getOpCode().isStoreReg()) // Case - 1 : TreeTop Node is RegStore
            {
                ...
                // If storedNode is needed to be uncommoned and we do not have found replacement node yet and it is
                // stored in register which is still available then use the register to store the node.
                if (nodeInfoEntry != nodeInfo->end() && nodeInfoEntry->second.second == NULL
                    && checkIfRegisterIsAvailable(comp, node, unavailableRegisters)) {
                    TR::Node* regLoad
                        = TR::Node::create(storedNode, comp->il.opCodeForRegisterLoad(storedNode->getDataType()));
            ...
            } else if (node->getOpCode().isBranch() && node->getNumChildren() > 0) // Case - 2 : TreeTop Node is Branch
            {
                ...
                if (regDeps->getOpCodeValue() == TR::GlRegDeps) {
                    gatherUnavailableRegisters(comp, regDeps, iter, nodeInfo, storeNodeInfo,
                        &storeRegNodePostSplitPoint, unavailableRegisters);
                }
            ...
            if (nextTreeTop
                == nextExtendedBlockEntry) // TreeTop Node is BBExit and next tree top is nextExtendedBlockEntry
            {
                TR::Node* regDeps = iter->getNode()->getNumChildren() > 0 ? iter->getNode()->getChild(0) : NULL;
                if (regDeps && regDeps->getOpCodeValue() == TR::GlRegDeps) {
                    // If the previous tree top of BBEnd contains branch, we need to make sure any regStore we add are
                    // added before that.
        ...
        for (auto iter = nodeInfo->begin(), end = nodeInfo->end(); iter != end; ++iter) {
            ...
            // For all the nodes in the nodeTable for which we can not find replacement yet, create replacement node
            // using available registers.
            if (!iter->second.second) {
                std::pair<TR_GlobalRegisterNumber, TR_GlobalRegisterNumber> regInfo
                    = findAvailableRegister(comp, iter->first, unavailableRegisters);
                if (regInfo.first > -1) {
                    TR::SymbolReference* ref = value->getOpCode().isLoadReg()
                        ? value->getRegLoadStoreSymbolReference()
                        : createSymRefForNode(comp, methodSymbol, value, self()->getExit());
                    TR::Node* regLoad = TR::Node::create(value, comp->il.opCodeForRegisterLoad(value->getDataType()));
                    TR::Node* regStore
                        = TR::Node::create(value, comp->il.opCodeForRegisterStore(value->getDataType()), 1, value);
                ...
                // If we can not find a register, store it into temp slot.
                    ...
                    // Also we need to make sure that replacement node is a node that is converted back to original type
                    // of the node.
                    if (convertedValDataType != originalValDataType) {
                        TR::ILOpCodes convertOpCode
                            = TR::ILOpCode::getProperConversion(originalValDataType, convertedValDataType, false);
                        convertedValueNode = TR::Node::create(convertOpCode, 1, value);
                    }
                    TR::SymbolReference* storedValueSymRef
                        = createSymRefForNode(comp, methodSymbol, convertedValueNode, self()->getExit());
                    TR::Node* storeNode = TR::Node::createWithSymRef(value,
                        comp->il.opCodeForDirectStore(convertedValDataType), 1, convertedValueNode, storedValueSymRef);
#ifdef J9_PROJECT_SPECIFIC
                    if (convertedValueNode->getType().isBCD())
                        storeNode->setDecimalPrecision(convertedValueNode->getDecimalPrecision());
#endif
                    self()->getExit()->insertBefore(TR::TreeTop::create(comp, storeNode));
                    TR::Node* replacementNode = TR::Node::createLoad(value, storedValueSymRef);
                    if (convertedValDataType != originalValDataType) {
                        TR::ILOpCodes convertOpCode
                            = TR::ILOpCode::getProperConversion(convertedValDataType, originalValDataType, false);
            ...
            // Now as we have created a node to replace all the references of value node after split point, add all
            // nodes which are not stored into the temp slot to the list for entry and exit dependency which will be
            // used to create GlRegDeps for exit node of the original block and entry node of new block.
...
TR::Block* OMR::Block::split(TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool fixupCommoning,
    bool copyExceptionSuccessors, TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    TR::Block* block2 = new (comp->trHeapMemory()) TR::Block(
        TR::TreeTop::create(comp, TR::Node::create(startNode, TR::BBStart)), self()->getExit(), comp->trMemory());
    block2->inheritBlockInfo(self());

    cfg->addNode(block2);

    self()->setExit(
        TR::TreeTop::create(comp, startOfNewBlock->getPrevTreeTop(), TR::Node::create(startNode, TR::BBEnd)));
    ...
    if (structure) {
        ...
        if (thisBlockStructure) {
            TR_BlockStructure* blockStructure2
                = new (cfg->structureMemoryRegion()) TR_BlockStructure(comp, block2->getNumber(), block2);
            TR_RegionStructure* parentStructure = thisBlockStructure->getParent()->asRegion();
            TR_StructureSubGraphNode* blockStructureNode2
                = new (cfg->structureMemoryRegion()) TR_StructureSubGraphNode(blockStructure2);
            ...
            for (auto nextSucc = subNode->getSuccessors().begin(); nextSucc != subNode->getSuccessors().end();
                 ++nextSucc)
            ...
            for (auto nextSucc = subNode->getExceptionSuccessors().begin();
                 nextSucc != subNode->getExceptionSuccessors().end(); ++nextSucc) {
                ...
                for (auto e1 = block2->getExceptionSuccessors().begin(); e1 != block2->getExceptionSuccessors().end();
                     ++e1) {
...
TR::Block* OMR::Block::splitBlockAndAddConditional(
    TR::TreeTop* tree, TR::TreeTop* compareTree, TR::CFG* cfg, bool remainderBlockShouldExtend)
{
    cfg->setStructure(0);
 ...
 */
TR::Block* OMR::Block::createConditionalSideExitBeforeTree(TR::TreeTop* tree, TR::TreeTop* compareTree,
    TR::TreeTop* exitTree, TR::TreeTop* returnTree, TR::CFG* cfg, bool markCold)
 ...
 *   \param cfg           the cfg to update with the new information
 */
TR::Block* OMR::Block::createConditionalBlocksBeforeTree(TR::TreeTop* tree, TR::TreeTop* compareTree,
    TR::TreeTop* ifTree, TR::TreeTop* elseTree, TR::CFG* cfg, bool changeBlockExtensions, bool markCold)
{
    auto comp = TR::comp();
...
#define MIN_EXCEPTION_NAME_SIZE 5
    ...
    { 9, "Exception",
        CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck | CanCatchArrayNew
            | CanCatchCheckCast | CanCatchMonitorExit },
    { 9, "Throwable", CanCatchEverything }, { 12, "UnknownError", CanCatchNew | CanCatchArrayNew | CanCatchNewvalue },
    ...
    { 16, "RuntimeException",
        CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck | CanCatchArrayNew
            | CanCatchCheckCast | CanCatchMonitorExit },
    { 18, "ClassCastException", CanCatchCheckCast }, { 18, "IllegalAccessError", CanCatchArrayNew | CanCatchNewvalue },
    { 18, "InstantiationError", CanCatchNew | CanCatchNewvalue },
    { 18, "StackOverflowError", CanCatchNew | CanCatchArrayNew | CanCatchNewvalue },
    { 19, "ArithmeticException", CanCatchDivCheck }, { 19, "ArrayStoreException", CanCatchArrayStoreCheck },
    { 19, "VirtualMachineError", CanCatchNew | CanCatchArrayNew | CanCatchNewvalue },
    { 20, "NullPointerException", CanCatchNullCheck }, { 25, "IndexOutOfBoundsException", CanCatchBoundCheck },
    { 26, "NegativeArraySizeException", CanCatchArrayNew }, { 28, "IllegalMonitorStateException", CanCatchMonitorExit },
...
static TR::Node* findFirstReference(TR::Node* n, TR::Symbol* sym, vcount_t visitCount)
{
...
TR::Node* OMR::Block::findFirstReference(TR::Symbol* sym, vcount_t visitCount)
{
    TR::Node* node = 0;
...
void OMR::Block::collectReferencedAutoSymRefsIn(
    TR::Compilation* comp, TR_BitVector* referencedAutoSymRefs, vcount_t visitCount)
{
...
void OMR::Block::collectReferencedAutoSymRefsIn(
    TR::Compilation* comp, TR::Node* node, TR_BitVector* referencedAutoSymRefs, vcount_t visitCount)
{
...
TR::Block* OMR::Block::splitEdge(
    TR::Block* from, TR::Block* to, TR::Compilation* c, TR::TreeTop** lastTreeTop, bool findOptimalInsertionPoint)
{
    ...
    if (entryTree) {
        ...
        if (isExceptionEdge) {
            newBlock->setHandlerInfo(
                to->getCatchType(), to->getInlineDepth(), to->getHandlerIndex(), to->getOwningMethod(), c);
            ...
            TR::TreeTop* gotoTreeTop
                = TR::TreeTop::create(c, TR::Node::create(from->getExit()->getNode(), TR::Goto, 0, to->getEntry()));
    ...
    // splitting the block at the point of the return or throw is the safest way to do this
        ...
        TR::ILOpCode& opCode
            = lastTree->getNode()->getOpCodeValue() == TR::treetop || lastTree->getNode()->getOpCode().isCheck()
            ? lastTree->getNode()->getFirstChild()->getOpCode()
            : lastTree->getNode()->getOpCode();
        TR_ASSERT(
            opCode.getOpCodeValue() == TR::athrow || opCode.isReturn(), "edge to EXIT should end in throw or return");
...
void OMR::Block::takeGlRegDeps(TR::Compilation* comp, TR::Node* glRegDeps)
{
    if (!glRegDeps)
        return;
...
TR::TreeTop* OMR::Block::getExceptingTree()
{
    TR_ASSERT(self()->getExceptionSuccessors().size() <= 1,
        "routine only works when block has 0 or 1 (and not %d) exception edges\n",
...
const char* OMR::Block::getName(TR_Debug* debug)
{
    if (debug)
...
void OMR::Block::inheritBlockInfo(TR::Block* org, bool inheritFreq)
{
...
TR::Block* OMR::Block::startOfExtendedBlock()
{
...
bool OMR::Block::canCatchExceptions(uint32_t flags)
{
    return _catchBlockExtension && ((flags & _catchBlockExtension->_exceptionsCaught) != 0);
}

void OMR::Block::setByteCodeIndex(int32_t index, TR::Compilation* comp)
{
    self()->getEntry()->getNode()->setByteCodeIndex(index);
}

bool OMR::Block::endsInGoto()
{
    TR::TreeTop* lastTT = NULL;
    if (self()->getEntry() != NULL)
        lastTT = self()->getLastRealTreeTop();
    return (lastTT != NULL && lastTT->getNode()->getOpCodeValue() == TR::Goto);
}

bool OMR::Block::endsInBranch()
{
    TR::TreeTop* lastTT = NULL;
    ...
    return (lastTT != NULL && lastNode->getOpCode().isBranch() && lastNode->getOpCodeValue() != TR::Goto);
}

bool OMR::Block::isEmptyBlock()
{

    return (self()->getEntry() != NULL && self()->getEntry()->getNextTreeTop() == self()->getExit());
}

void OMR::Block::ensureCatchBlockExtensionExists(TR::Compilation* comp)
{
...
bool OMR::Block::hasExceptionPredecessors() { return !self()->getExceptionPredecessors().empty(); }

/**
 * Field functions
 */

int32_t OMR::Block::getNestingDepth()
{
    if (_pStructureOf)
...
TR_Array<TR::GlobalRegister>& OMR::Block::getGlobalRegisters(TR::Compilation* c)
{
    if (!_globalRegisters)
        _globalRegisters = new (c->trStackMemory())
            TR_Array<TR::GlobalRegister>(c->trMemory(), c->cg()->getNumberOfGlobalRegisters(), true, stackAlloc);
    return *_globalRegisters;
}

void OMR::Block::setInstructionBoundaries(uint32_t startPC, uint32_t endPC)
{
    _instructionBoundaries._startPC = startPC;
    _instructionBoundaries._endPC = endPC;
}

void OMR::Block::addExceptionRangeForSnippet(uint32_t startPC, uint32_t endPC)
{
    _snippetBoundaries.add(new (TR::comp()->trHeapMemory()) InstructionBoundaries(startPC, endPC));
}

void OMR::Block::setHandlerInfo(uint32_t c, uint8_t d, int32_t i, TR_ResolvedMethod* m, TR::Compilation* comp)
{
...
void OMR::Block::setHandlerInfoWithOutBCInfo(
    uint32_t c, uint8_t d, int32_t i, TR_ResolvedMethod* m, TR::Compilation* comp)
{
    self()->ensureCatchBlockExtensionExists(comp);
    TR_CatchBlockExtension* cbe = _catchBlockExtension;
    cbe->_catchType = c;
...
bool OMR::Block::isCatchBlock() { return !self()->getExceptionPredecessors().empty(); }

uint32_t OMR::Block::getCatchType()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_catchType;
}

uint8_t OMR::Block::getInlineDepth()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_inlineDepth;
}

int32_t OMR::Block::getHandlerIndex()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_handlerIndex;
}

TR_ByteCodeInfo OMR::Block::getByteCodeInfo()
{
    TR_ASSERT(_catchBlockExtension, "Catch block extension does not exist");
    return _catchBlockExtension->_byteCodeInfo;
}

TR_OpaqueClassBlock* OMR::Block::getExceptionClass()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_exceptionClass;
}

char* OMR::Block::getExceptionClassNameChars()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_exceptionClassNameChars;
}

int32_t OMR::Block::getExceptionClassNameLength()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_exceptionClassNameLength;
}

void OMR::Block::setExceptionClassName(char* name, int32_t length, TR::Compilation* comp)
{
    self()->ensureCatchBlockExtensionExists(comp);
    ...
    if (length < 10 + MIN_EXCEPTION_NAME_SIZE || length > 10 + MAX_EXCEPTION_NAME_SIZE
...
TR_ResolvedMethod* OMR::Block::getOwningMethod()
{
    TR_ASSERT(_catchBlockExtension, "catch block extension does not exist");
    return _catchBlockExtension->_owningMethod;
}

int32_t OMR::Block::getNormalizedFrequency(TR::CFG* cfg)
{
...
int32_t OMR::Block::getGlobalNormalizedFrequency(TR::CFG* cfg)
{
 ...
 */
bool OMR::Block::verifyOSRInduceBlock(TR::Compilation* comp)
{
...
void OMR::Block::setIsExtensionOfPreviousBlock(bool b)
{
...
bool OMR::Block::isExtensionOfPreviousBlock() { return _flags.testAny(_isExtensionOfPreviousBlock); }

void OMR::Block::setIsSuperCold(bool b)
{
...
bool OMR::Block::isSuperCold() { return _flags.testAny(_isSuperCold); }

void OMR::Block::setIsSyntheticHandler()
{
...
TR::Block* TR_BlockCloner::cloneBlocks(TR::Block* from, TR::Block* lastBlock)
{
    ...
    for (; from; from = from->getNextBlock()) {
        ...
        TR::Block* to = new (comp->trHeapMemory())
            TR::Block(*from, TR::TreeTop::create(comp, NULL), TR::TreeTop::create(comp, NULL));
...
TR::Block* TR_BlockCloner::cloneBlocks(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
    TR::Block* toReturn = doBlockClone(bMap);

    // for a path of blocks we're cloning that doesn't follow the fall-through path, the branches will be set to fall
    // through node (incorrectly) by cloneNode.  This code patches up the branches so they point to the right target
    // (the old fall through node) we don't mess with the last node as the connection is handled by whoever called us
    //
    for (BlockMapper* itr = bMap->getFirst(); itr->getNext(); itr = itr->getNext()) {
        ...
        if (lastRealNode->getOpCode().isIf()
            && lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber()
                == itr->_to->getNextBlock()->getNumber()) {
...
TR::Block* TR_BlockCloner::getToBlock(TR::Block* from)
{
...
TR::Block* TR_BlockCloner::doBlockClone(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
    _blockMappings = *bMap;
    TR::Compilation* comp = _cfg->comp();
    BlockMapper* m;
    ...
    //
    // Some Extra remarks about this code.  This cfg code looks insufficient to really handle the case when you are not
    // cloning along a fall through path, and you are trying to preserve branches exactly (ie _cloneBranchesExactly is
    // set to true Luckily, so far no code has ever not done that.  Most code that clonesBranchesExactly clones a single
    // block, and the few cases that don't appear to be cloning along a fall through path
    //
    for (m = _blockMappings.getFirst(); m; m = m->getNext()) {
        _cfg->addNode(m->_to);
        if (comp->ilGenTrace())
        ...
        if (_cloneSuccessorsOfLastBlock || m != _blockMappings.getLast()) {
            for (auto e = m->_from->getSuccessors().begin(); e != m->_from->getSuccessors().end(); ++e)
                _cfg->addEdge(m->_to, getToBlock(toBlock((*e)->getTo())));

            for (auto e = m->_from->getExceptionSuccessors().begin(); e != m->_from->getExceptionSuccessors().end();
                 ++e)
                _cfg->addExceptionEdge(m->_to, getToBlock(toBlock((*e)->getTo())));

            for (auto e = m->_from->getExceptionPredecessors().begin(); e != m->_from->getExceptionPredecessors().end();
                 ++e)
...
TR::Node* TR_BlockCloner::cloneNode(TR::Node* from)
{
    TR::Node* to;
...
TR::Block* TR_ExtendedBlockSuccessorIterator::getFirst()
{
...
TR::Block* TR_ExtendedBlockSuccessorIterator::getNext()
{
    TR::CFGEdge* edge;
...
void TR_ExtendedBlockSuccessorIterator::setCurrentBlock(TR::Block* b)
{
