...
void OMR::Block::insertBlockAsFallThrough(TR::Compilation *comp, TR::Block *block, TR::Block *newFallThroughBlock)
{
    ...
    TR::TreeTop::insertTreeTops(comp, block->getExit(), newFallThroughBlock->getEntry(),
        newFallThroughBlock->getExit());
 ...
 */
void OMR::Block::redirectFlowToNewDestination(TR::Compilation *comp, TR::CFGEdge *origEdge, TR::Block *newTo,
    bool useGotoForFallThrough)
{
...
void OMR::Block::changeBranchDestination(TR::TreeTop *newDestination, TR::CFG *cfg, bool callerFixesRegdeps)
{
    ...
    TR_ASSERT(branchNode->getOpCode().isBranch(),
        "OMR::Block::changeBranchDestination: can't find the existing branch node");
    ...
    if (numChildren > 0 && branchNode->getChild(numChildren - 1)->getOpCodeValue() == TR::GlRegDeps) {
        ...
        TR_ASSERT(fromGlRegDep->getOpCodeValue() == TR::GlRegDeps,
            "expected the branch child to be a GlRegDep.  It's not!");
...
void OMR::Block::uncommonNodesBetweenBlocks(TR::Compilation *comp, TR::Block *newBlock,
...
typedef TR::typed_allocator<std::pair<TR::Node * const, std::pair<int32_t, TR::Node *> >, TR::Region &>
    NodeTableAllocator;
 ...
 */
static std::pair<TR_GlobalRegisterNumber, TR_GlobalRegisterNumber> findAvailableRegister(TR::Compilation *comp,
    TR::Node *node, TR_BitVector &unavailableRegisters)
{
...
{
    TR_ASSERT_FATAL(regDeps->getOpCodeValue() == TR::GlRegDeps,
        "A GlRegDeps node is required to gather unavaialble registers\n");
        ...
        if (dep->getOpCodeValue() == TR::PassThrough) {
            ...
            if (nodeInfoEntry != nodeInfo->end()) {
                ...
                if (nodeInfoEntry->second.second != NULL && nodeInfoEntry->second.second->getOpCode().isLoadReg()
                    ...
                    TR::Node *regStore = TR::Node::create(value,
                        comp->il.opCodeForRegisterStore(originalNode->getDataType()), 1, nodeToBeStored);
 ...
 */
static TR::SymbolReference *createSymRefForNode(TR::Compilation *comp, TR::ResolvedMethodSymbol *methodSymbol,
    TR::Node *value, TR::TreeTop *insertBefore)
{
 ...
 */
TR::Block *OMR::Block::splitPostGRA(TR::TreeTop *startOfNewBlock, TR::CFG *cfg, bool copyExceptionSuccessors,
    TR::ResolvedMethodSymbol *methodSymbol)
{
...
TR::Block *OMR::Block::splitBlockAndAddConditional(TR::TreeTop *tree, TR::TreeTop *compareTree, TR::CFG *cfg,
...
void OMR::Block::collectReferencedAutoSymRefsIn(TR::Compilation *comp, TR_BitVector *referencedAutoSymRefs,
    vcount_t visitCount)
{
...
void OMR::Block::collectReferencedAutoSymRefsIn(TR::Compilation *comp, TR::Node *node,
    TR_BitVector *referencedAutoSymRefs, vcount_t visitCount)
{
    ...
    bool findOptimalInsertionPoint)
{
    ...
    if (entryTree) {
        ...
        if (isExceptionEdge) {
            newBlock->setHandlerInfo(to->getCatchType(), to->getInlineDepth(), to->getHandlerIndex(),
                to->getOwningMethod(), c);
    ...
    // splitting the block at the point of the return or throw is the safest way to do this
        ...
        TR_ASSERT(opCode.getOpCodeValue() == TR::athrow || opCode.isReturn(),
            "edge to EXIT should end in throw or return");
...
void OMR::Block::setHandlerInfoWithOutBCInfo(uint32_t c, uint8_t d, int32_t i, TR_ResolvedMethod *m,
    TR::Compilation *comp)
{
    self()->ensureCatchBlockExtensionExists(comp);
    TR_CatchBlockExtension *cbe = _catchBlockExtension;
