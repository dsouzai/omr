...
protected:
    ...
    static TR::Node* createInternal(
        TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* originalNode = 0);

    TR_YesNoMaybe computeIsCollectedReferenceImpl(
...
public:
    ...
    static TR::Node* create(TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, int32_t intValue,
        TR::TreeTop* dest = 0);
    static TR::Node* createWithSymRef(
    ...
    static TR::Node* recreateWithoutProperties(
        TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren = 0, TR::SymbolReference* symRef = 0);
...
private:
    static TR::Node* recreateWithoutSymRef_va_args(
    ...
    static TR::Node* recreateWithoutSymRef(
        TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...);
    static TR::Node* recreateWithSymRefWithoutProperties(
        TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...);
...
private:
    ...
    template<class... Children>
    static TR::Node* createWithoutSymRef(
        TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, TR::Node* first, Children... children);
...
public:
    ...
    static TR::Node* createWithRoomForThree(
        TR::ILOpCodes op, TR::Node* first, TR::Node* second, void* symbolRefOrBranchTarget = 0);
    ...
    static TR::Node* createAddressNode(
...
private:
    ...
    static TR::Node* recreateAndCopyValidPropertiesImpl(
...
public:
    ...
    bool containsNode(
        TR::Node* searchNode, vcount_t visitCount); // Careful how you use this: it doesn't account for aliasing
    ...
    void rotateChildren(
        int32_t first, int32_t last); ///< @note when finsihed, last child ends up where first child used to be
...
protected:
    enum {
        ...
        = 0x00020000, ///< flag used by TR::arrayset to guarantee that checking for lengths smaller than pointer size is
                      ///< not needed (leading to more efficient code being generated, e.g. REP TR::InstOpCode::STOSQ on
                      ///< X86
        ...
        = 0x00002000, ///< Tactical x86 codegen flag.  Only when refcount <= 1.  Indicates that parent will consult the
                      ///< register's node count before clobbering it (not just the node's refcount).
        ...
        referenceIsNonNull
        = 0x00008000, ///< Sometimes we can't mark the child as non-null (because it's not provably non-null everywhere)
                      ///< but we know it's non-null by the time we run this parent
