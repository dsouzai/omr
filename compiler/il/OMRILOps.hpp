...
public:
    ...
    {
        TR_ASSERT_FATAL(
            vectorType.isVector() || vectorType.isMask(), "createVectorOpCode should take vector or mask type\n");

        TR_ASSERT_FATAL(
            operation < TR::firstTwoTypeVectorOperation, "Vector operation should be one vector type operation\n");

        if (vectorType.isMask())
            vectorType = TR::DataType::vectorFromMaskType(vectorType);

        return (TR::ILOpCodes)(
            TR::NumScalarIlOps + operation * TR::NumVectorTypes + (vectorType - TR::FirstVectorType));
    }
     ...
     *     Vector opcode
     */
    static TR::ILOpCodes createVectorOpCode(
        TR::VectorOperation operation, TR::DataType srcVectorType, TR::DataType resVectorType)
    {
        ...
        TR_ASSERT_FATAL(
            operation >= TR::firstTwoTypeVectorOperation, "Vector operation should be two vector type operation\n");
        ...
        return (TR::ILOpCodes)(TR::NumScalarIlOps + TR::NumOneVectorTypeOps
            ...
            + (srcVectorType - TR::FirstVectorType) * TR::NumVectorTypes + (resVectorType - TR::FirstVectorType));
    }
     ...
     *     Vector operation
     */
    TR::VectorOperation getVectorOperation() const { return getVectorOperation(_opCode); }
    ...
    static TR::VectorOperation getVectorOperation(ILOpCode op)
    {
        ...
        return (TR::VectorOperation)((opcode < (TR::NumScalarIlOps + TR::NumOneVectorTypeOps))
                ...
                : (opcode - TR::NumScalarIlOps - TR::NumOneVectorTypeOps) / (TR::NumVectorTypes * TR::NumVectorTypes)
                    + TR::firstTwoTypeVectorOperation);
    }
     ...
     *     Vector type
     */
    TR::DataType getVectorDataType() const { return getVectorDataType(_opCode); }
     ...
     *     Vector type
     */
    static TR::DataType getVectorDataType(ILOpCode op) { return getVectorResultDataType(op); }
     ...
     *     Vector type
     */
    TR::DataType getVectorResultDataType() const { return getVectorResultDataType(_opCode); }
     ...
     *     Vector type
     */
    TR::DataType getVectorSourceDataType() const { return getVectorSourceDataType(_opCode); }
    ...
    static TR::DataType getVectorSourceDataType(ILOpCode op)
    {
        ...
        TR_ASSERT_FATAL(opcode >= (TR::NumScalarIlOps + TR::NumOneVectorTypeOps),
            "getVectorSourceDataType() can only be called for two vector type opcodes (e.g. vconv)\n");
    ...
    /// opcode.
    /// e.g. ificmplt --> icmplt
    TR::ILOpCodes convertIfCmpToCmp() { return _opCodeProperties[getTableIndex()].booleanCompareOpCode; }

    /// Get the compare-and-branch opcode that corresponds to this boolean compare
    /// e.g. icmplt --> ificmplt
    TR::ILOpCodes convertCmpToIfCmp() { return _opCodeProperties[getTableIndex()].ifCompareOpCode; }

    TR::DataType getDataType() const { return getDataType(_opCode); }

    static TR::DataType getDataType(TR::ILOpCodes op)
    {
        if (!isVectorOpCode(op))
            return _opCodeProperties[op].dataType;
    ...
    {
        return isVectorOpCode(op) ? OMR::DataType::getSize(getDataType(op)) : ILOpCode(op).getSize();
    }
    ...
    bool isByte() const
    {
        return (typeProperties().testAny(ILTypeProp::Integer | ILTypeProp::Unsigned)
            && typeProperties().testAll(ILTypeProp::Size_1));
    }
    bool isShort() const
    {
        return (typeProperties().testAny(ILTypeProp::Integer | ILTypeProp::Unsigned)
            && typeProperties().testAll(ILTypeProp::Size_2));
    }
    bool isInt() const
    {
        return (typeProperties().testAny(ILTypeProp::Integer | ILTypeProp::Unsigned)
            && typeProperties().testAll(ILTypeProp::Size_4));
    }
    bool isLong() const
    {
        return (typeProperties().testAny(ILTypeProp::Integer | ILTypeProp::Unsigned)
            && typeProperties().testAll(ILTypeProp::Size_8));
    }
    ...
    bool canHaveGlRegDeps() const
    {
        return getOpCodeValue() == TR::BBStart || getOpCodeValue() == TR::BBEnd || isCall() || isBranch();
    }
    ...
    bool isIf() const
    {
        return properties1().testAll(ILProp1::BooleanCompare | ILProp1::Branch) && !isCompBranchOnly();
    }
    bool isGoto() const
    {
        return properties1().testAll(ILProp1::Branch | ILProp1::TreeTop) && !isCompBranchOnly() && !isIf();
    }
    ...
    bool isLoadVarDirect() const
    {
        return properties1().testValue(ILProp1::Indirect | ILProp1::LoadVar, ILProp1::LoadVar);
    }
    ...
    {
        return isAdd() || isSub() || isMul() || isDiv() || isRem() || isLeftShift() || isRightShift()
            || isShiftLogical() || isAnd() || isXor() || isOr() || isNeg() || isSelectAdd() || isSelectSub();
    }
    bool isCondCodeComputation() const { return properties2().testAny(ILProp2::CondCodeComputation); }
    bool isJumpWithMultipleTargets() const
    {
        return properties2().testAny(ILProp2::JumpWithMultipleTargets);
    } // Transactional Memory uses this
    ...
    bool isSignedExponentiation() const
    {
        return properties3().testAny(ILProp3::SignedExponentiation);
    } // denotes the power (not the base) is signed
    bool isUnsignedExponentiation() const
    {
        return properties3().testAny(ILProp3::UnsignedExponentiation);
    } // denotes the power (not the base) is unsigned
    ...
    bool isCompareForEquality() const
    {
        return isBooleanCompare() && (isCompareTrueIfLess() == isCompareTrueIfGreater());
    }
    ...
    {
        return (isAdd() && isCommutative() && isAssociative() && isRef());
    } // returns true if TR::aiadd or TR::aladd
    ...
    bool isOverflowCheck() const
    {
        return (getOpCodeValue() == TR::OverflowCHK) || (getOpCodeValue() == TR::UnsignedOverflowCHK);
    }

    bool hasPinningArrayPointer() { return (getOpCodeValue() == TR::aiadd) || (getOpCodeValue() == TR::aladd); }

    bool isLongCompare() const
    {
        return (getOpCodeValue() == TR::lcmp) || (getOpCodeValue() == TR::lcmpeq) || (getOpCodeValue() == TR::lcmpne)
            || (getOpCodeValue() == TR::lcmplt) || (getOpCodeValue() == TR::lcmpge) || (getOpCodeValue() == TR::lcmpgt)
            || (getOpCodeValue() == TR::lcmple) || (getOpCodeValue() == TR::lucmp) || (getOpCodeValue() == TR::lucmplt)
            || (getOpCodeValue() == TR::lucmpge) || (getOpCodeValue() == TR::lucmpgt)
            || (getOpCodeValue() == TR::lucmple);
    }

    bool isFunctionCall() const
    {
        return isCall() && getOpCodeValue() != TR::arraycopy && getOpCodeValue() != TR::arrayset
            && getOpCodeValue() != TR::bitOpMem && getOpCodeValue() != TR::arraycmp
    ...
    {
        return getOpCodeValue() == TR::ificmpgt || getOpCodeValue() == TR::iflcmpgt || getOpCodeValue() == TR::ifiucmpgt
            || getOpCodeValue() == TR::ifacmpgt || getOpCodeValue() == TR::iflucmpgt || getOpCodeValue() == TR::iadd
            || getOpCodeValue() == TR::ladd || getOpCodeValue() == TR::iuaddc || getOpCodeValue() == TR::aiadd
            || getOpCodeValue() == TR::luaddc || getOpCodeValue() == TR::aladd || getOpCodeValue() == TR::isub
            || getOpCodeValue() == TR::lsub || getOpCodeValue() == TR::iusubb || getOpCodeValue() == TR::asub
            || getOpCodeValue() == TR::lusubb;
    }

    bool isStrictWidenConversion()
    {
        return getOpCodeValue() == TR::f2d || getOpCodeValue() == TR::b2i || getOpCodeValue() == TR::s2i
            || getOpCodeValue() == TR::b2l || getOpCodeValue() == TR::bu2i || getOpCodeValue() == TR::bu2l
            || getOpCodeValue() == TR::s2l || getOpCodeValue() == TR::su2i || getOpCodeValue() == TR::su2l
            || getOpCodeValue() == TR::i2l;
    }

    bool isMemToMemOp()
    {
        return getOpCodeValue() == TR::bitOpMem || getOpCodeValue() == TR::arrayset || getOpCodeValue() == TR::arraycmp
            || getOpCodeValue() == TR::arraycopy || getOpCodeValue() == TR::arraycmplen;
    }
    ...
    static TR::ILOpCodes getProperConversion(
        TR::DataType sourceDataType, TR::DataType targetDataType, bool needUnsignedConversion)
    {
        ...
        case TR::b2l:
            return TR::bu2l;
        case TR::b2a:
            return TR::bu2a; // do we care about this?
        case TR::b2f:
            return TR::bu2f;
        case TR::b2d:
            return TR::bu2d;
    ...
    {
        if (type.isVector())
            return createVectorOpCode(TR::vabs, type);
        ...
        case TR::Double:
            return TR::dabs;
        default:
            TR_ASSERT(false, "no abs opcode for this datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    {
        if (type.isVector())
            return createVectorOpCode(TR::vadd, type);
        ...
        case TR::Int64:
            return TR::ladd;
        case TR::Address:
            return (is64Bit) ? TR::aladd : TR::aiadd;
        case TR::Float:
            return TR::fadd;
        case TR::Double:
            return TR::dadd;
        default:
            TR_ASSERT(0, "no add opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    {
        if (type.isVector())
            return createVectorOpCode(TR::vsub, type);
        ...
        case TR::Double:
            return TR::dsub;
        default:
            TR_ASSERT(0, "no sub opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    {
        if (type.isVector())
            return createVectorOpCode(TR::vmul, type);
        ...
        case TR::Double:
            return TR::dmul;
        default:
            TR_ASSERT(0, "no mul opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    {
        if (type.isVector())
            return createVectorOpCode(TR::vdiv, type);
        ...
        case TR::Double:
            return TR::ddiv;
        default:
            TR_ASSERT(0, "no div opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes unsignedDivideOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::ludiv;
        default:
            TR_ASSERT(0, "no unsigned div opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes andOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::land;
        default:
            TR_ASSERT(false, "no and opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes orOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::lor;
        default:
            TR_ASSERT(false, "no or opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes xorOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::lxor;
        default:
            TR_ASSERT(false, "no xor opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes shiftLeftOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::lshl;
        default:
            TR_ASSERT(false, "no shl opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes shiftRightOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::lshr;
        default:
            TR_ASSERT(false, "no shr opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes unsignedShiftRightOpCode(TR::DataType type)
    {
        ...
        case TR::Int64:
            return TR::lushr;
        default:
            TR_ASSERT(false, "no ushr opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes negateOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::dneg;
        default:
            TR_ASSERT(0, "no negate opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes integerOpForLongOp(TR::ILOpCodes longOp)
    {
        ...
        case TR::lshl:
            return TR::ishl;
        default:
            TR_ASSERT(0, "unexpected long arithmetic op\n");
        }
    ...
    static TR::ILOpCodes ifcmpgeOpCode(TR::DataType type, bool isUnsigned)
    {
        switch (type) {
        case TR::Int8:
            return isUnsigned ? TR::ifbucmpge : TR::ifbcmpge;
        case TR::Int16:
            return isUnsigned ? TR::ifsucmpge : TR::ifscmpge;
        case TR::Int32:
            return isUnsigned ? TR::ifiucmpge : TR::ificmpge;
        case TR::Int64:
            return isUnsigned ? TR::iflucmpge : TR::iflcmpge;
        case TR::Address:
            return TR::ifacmpge;
        case TR::Float:
            return TR::iffcmpge;
        case TR::Double:
            return TR::ifdcmpge;
        default:
            TR_ASSERT(0, "no ifcmpge opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes ifcmpleOpCode(TR::DataType type, bool isUnsigned)
    {
        switch (type) {
        case TR::Int8:
            return isUnsigned ? TR::ifbucmple : TR::ifbcmple;
        case TR::Int16:
            return isUnsigned ? TR::ifsucmple : TR::ifscmple;
        case TR::Int32:
            return isUnsigned ? TR::ifiucmple : TR::ificmple;
        case TR::Int64:
            return isUnsigned ? TR::iflucmple : TR::iflcmple;
        case TR::Address:
            return TR::ifacmple;
        case TR::Float:
            return TR::iffcmple;
        case TR::Double:
            return TR::ifdcmple;
        default:
            TR_ASSERT(0, "no ifcmple opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes ifcmpgtOpCode(TR::DataType type, bool isUnsigned)
    {
        switch (type) {
        case TR::Int8:
            return isUnsigned ? TR::ifbucmpgt : TR::ifbcmpgt;
        case TR::Int16:
            return isUnsigned ? TR::ifsucmpgt : TR::ifscmpgt;
        case TR::Int32:
            return isUnsigned ? TR::ifiucmpgt : TR::ificmpgt;
        case TR::Int64:
            return isUnsigned ? TR::iflucmpgt : TR::iflcmpgt;
        case TR::Address:
            return TR::ifacmpgt;
        case TR::Float:
            return TR::iffcmpgt;
        case TR::Double:
            return TR::ifdcmpgt;
        default:
            TR_ASSERT(0, "no ifcmpgt opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes ifcmpltOpCode(TR::DataType type, bool isUnsigned)
    {
        switch (type) {
        case TR::Int8:
            return isUnsigned ? TR::ifbucmplt : TR::ifbcmplt;
        case TR::Int16:
            return isUnsigned ? TR::ifsucmplt : TR::ifscmplt;
        case TR::Int32:
            return isUnsigned ? TR::ifiucmplt : TR::ificmplt;
        case TR::Int64:
            return isUnsigned ? TR::iflucmplt : TR::iflcmplt;
        case TR::Address:
            return TR::ifacmplt;
        case TR::Float:
            return TR::iffcmplt;
        case TR::Double:
            return TR::ifdcmplt;
        default:
            TR_ASSERT(0, "no ifcmplt opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes ifcmpeqOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::ifdcmpeq;
        default:
            TR_ASSERT(0, "no ifcmpeq opcode for this datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes ifcmpneOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::ifdcmpne;
        default:
            TR_ASSERT(0, "no ifcmpne opcode for this datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes cmpeqOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::dcmpeq;
        default:
            TR_ASSERT(0, "no cmpeq opcode for this datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes constOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::dconst;
        default:
            TR_ASSERT(0, "no const opcode for this datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes returnOpCode(TR::DataType type)
    {
        ...
        case TR::Double:
            return TR::dreturn;
        case TR::NoType:
            return TR::Return; // void return
        default:
            TR_ASSERT(0, "no return opcode for this datatype");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes getDirectCall(TR::ILOpCodes ind)
    {
        ...
        case TR::calli:
            return TR::call;
        default:
            TR_ASSERT(0, "no direct call for this op-code");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes getIndirectCall(TR::DataType type)
    {
        ...
        case TR::NoType:
            return TR::calli;
        default:
            TR_ASSERT(0, "no indirect call for this type");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes getDirectCall(TR::DataType type)
    {
        ...
        case TR::NoType:
            return TR::call;
        default:
            TR_ASSERT(0, "no direct call for this type");
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes reductionToVerticalOpcode(TR::ILOpCodes op, TR::VectorLength vectorLength)
    {
        ...
        return ILOpCode::createVectorOpCode(
            operation, TR::DataType::createVectorType(opcode.getDataType().getDataType(), vectorLength));
    }
