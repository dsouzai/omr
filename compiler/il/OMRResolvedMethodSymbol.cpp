...
TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::self() { return static_cast<TR::ResolvedMethodSymbol*>(this); }

template<typename AllocatorType>
TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::create(
    AllocatorType m, TR_ResolvedMethod* rm, TR::Compilation* comp)
{
    return new (m) TR::ResolvedMethodSymbol(rm, comp);
}

static TR::TreeTop* findEndTreeTop(TR::ResolvedMethodSymbol* rms)
{
    TR::TreeTop* end = NULL;
...
void OMR::ResolvedMethodSymbol::initForCompilation(TR::Compilation* comp)
{
    ...
    , _bytecodeProfilingOffsets(comp->allocator())
{
    ...
    //
    if ((_methodIndex > JITTED_METHOD_INDEX
            && !_resolvedMethod->isSameMethod(comp->getJittedMethodSymbol()->getResolvedMethod()))
        || comp->isDLT() || comp->compileRelocatableCode()) {
    ...
    _properties.set(
        CanSkipFlattenableArrayElementNonHelperCall, self()->safeToSkipFlattenableArrayElementNonHelperCall());
...
int32_t OMR::ResolvedMethodSymbol::getSyncObjectTempIndex()
{
    int32_t delta = 0;
...
int32_t OMR::ResolvedMethodSymbol::getThisTempForObjectCtorIndex()
{
    int32_t delta = 0;
    if (self()->comp()->getOption(TR_MimicInterpreterFrameShape)) {
        delta++;
...
template<typename AllocatorType>
TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::createJittedMethodSymbol(
    AllocatorType m, TR_ResolvedMethod* method, TR::Compilation* comp)
{
...
void bcIndexForFakeInduce(
    TR::Compilation* comp, int16_t* callSiteInsertionPoint, int16_t* bcIndexInsertionPoint, char* childPath)
{
...
bool OMR::ResolvedMethodSymbol::canInjectInduceOSR(TR::Node* node)
{
 ...
 */
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCallNode(
    TR::TreeTop* insertionPoint, int32_t numChildren, bool copyChildren, bool shouldSplitBlock, TR::CFG* cfg)
{
    ...
    else if ((refNode->getNumChildren() > 0) && refNode->getFirstChild()->getOpCode().isCall()) {
    ...
    TR::Node* induceOSRCallNode
        = TR::Node::createWithSymRef(refNode, TR::call, numChildren - firstArgIndex, induceOSRSymRef);
    TR_OSRPoint* point = self()->findOSRPoint(refNode->getByteCodeInfo());
    TR_ASSERT(point, "Could not find osr point for bytecode %d:%d!", refNode->getByteCodeInfo().getCallerIndex(),
        refNode->getByteCodeInfo().getByteCodeIndex());
    ...
    if (self()->comp()->getOptions()->getVerboseOption(TR_VerboseOSRDetails))
        TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD, "Injected induceOSR call at %3d:%d in %s",
            refNode->getInlinedSiteIndex(), refNode->getByteCodeIndex(), self()->comp()->signature());
 ...
 */
bool OMR::ResolvedMethodSymbol::induceOSRAfterAndRecompile(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI,
    TR::TreeTop* branch, bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
    TR_ASSERT(self()->comp()->allowRecompilation(),
        "request OSR and recompilation at node %p when recomp is disabled\n", insertionPoint);
    TR::TreeTop* induceOSRCallTree
        = self()->induceOSRAfterImpl(insertionPoint, induceBCI, branch, extendRemainder, offset, lastTreeTop);
    ...
    TR_ASSERT_FATAL(induceOSRCallNode->getOpCode().isCall() && symRef->getReferenceNumber() == TR_induceOSRAtCurrentPC,
        "induceOSRCallNode %p (n%dn) under induceOSRCallTree %p should be a call node with TR_induceOSRAtCurrentPC "
        "helper call",
        induceOSRCallNode, induceOSRCallNode->getGlobalIndex(), induceOSRCallTree->getNode());
    induceOSRCallNode->setSymbolReference(
        self()->comp()->getSymRefTab()->findOrCreateInduceOSRSymbolRef(TR_induceOSRAtCurrentPCAndRecompile));
    return true;
}
 ...
 *          is generated successfully
 */
bool OMR::ResolvedMethodSymbol::induceOSRAfter(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI,
    TR::TreeTop* branch, bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
 ...
 *
 *  \return The treetop of the inserted induceOSR helper call if OSR transition is supported at the requested \parm
 * induceBCI. NULL if OSR transition is not supported at the requested \parm induceBCI.
 */
TR::TreeTop* OMR::ResolvedMethodSymbol::induceOSRAfterImpl(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI,
    TR::TreeTop* branch, bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
    ...
    if (self()->supportsInduceOSR(induceBCI, block, self()->comp())) {
        ...
        return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
            osrBlock->getExit(), induceBCI, false, self()->comp());
    }
    return NULL;
}

TR::TreeTop* OMR::ResolvedMethodSymbol::induceImmediateOSRWithoutChecksBefore(TR::TreeTop* insertionPoint)
{
    if (self()->supportsInduceOSR(
            insertionPoint->getNode()->getByteCodeInfo(), insertionPoint->getEnclosingBlock(), self()->comp()))
        return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
            insertionPoint, insertionPoint->getNode()->getByteCodeInfo(), false, self()->comp());
    if (self()->comp()->getOption(TR_TraceOSR))
        traceMsg(self()->comp(),
...
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(
    TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, bool shouldSplitBlock, TR::Compilation* comp)
{
    TR::TreeTop* induceOSRCallTree
        = self()->genInduceOSRCall(insertionPoint, induceBCI.getCallerIndex(), 0, false, shouldSplitBlock);
 ...
 *
 * \param insertionPoint Tree to prepend the OSR call to. It may also copy children from this tree if it is a call.
 * \param inlinedSiteIndex The inlined site index of the desired transition target. This should match the insertion
 * points inlined site index. \param numChildren The number of children to copy from the insertionPoint to the induce
 * OSR call. \param copyChildren Flag controlling whether to copy children to the induce OSR call. \param
 * shouldSplitBlock Flag controlling whether to split the block after the induce OSR call. \param cfg CFG used for the
 * block modifications. This will default to the outermost CFG, but can be specified if adding induce OSR calls when it
 * is not fully formed, such as during inlining.
 */
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCall(TR::TreeTop* insertionPoint, int32_t inlinedSiteIndex,
    int32_t numChildren, bool copyChildren, bool shouldSplitBlock, TR::CFG* cfg)
{
    TR_OSRMethodData* osrMethodData
        = self()->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(inlinedSiteIndex, self());
    return self()->genInduceOSRCall(
        insertionPoint, inlinedSiteIndex, osrMethodData, numChildren, copyChildren, shouldSplitBlock, cfg);
}
    ...
    TR_OSRMethodData* osrMethodData, int32_t numChildren, bool copyChildren, bool shouldSplitBlock, TR::CFG* callerCFG)
{
    ...
    TR::TreeTop* induceOSRCallTree
        = self()->genInduceOSRCallNode(insertionPoint, numChildren, copyChildren, shouldSplitBlock, callerCFG);
    ...
    if (!enclosingBlock->getLastRealTreeTop()->getNode()->getOpCode().isReturn()) {
        ...
        for (auto nextEdge = enclosingBlock->getSuccessors().begin();
             nextEdge != enclosingBlock->getSuccessors().end();) {
    ...
    for (auto nextEdge = enclosingBlock->getExceptionSuccessors().begin();
         nextEdge != enclosingBlock->getExceptionSuccessors().end();) {
    ...
    TR::Node* loadExcpSymbol = TR::Node::createWithSymRef(
        insertionPointNode, TR::aload, 0, self()->comp()->getSymRefTab()->findOrCreateExcpSymbolRef());
    ...
    enclosingBlock->append(TR::TreeTop::create(self()->comp(),
        TR::Node::createWithSymRef(
            TR::athrow, 1, 1, loadExcpSymbol, self()->comp()->getSymRefTab()->findOrCreateAThrowSymbolRef(self()))));
    ...
    // TODO: The late addition of OSR infrastructure based on the lack of OSR points should not be possible, as we
    // shouldn't have been able to induce without an OSR point in the first place. This should be removed, as well as
    // the genOSRHelperCall's cfg argument.
    if (firstOSRPoint) {
    ...
    self()->insertStoresForDeadStackSlotsBeforeInducingOSR(
        self()->comp(), inlinedSiteIndex, insertionPoint->getNode()->getByteCodeInfo(), induceOSRCallTree);

    if (self()->comp()->getOption(TR_TraceOSR))
        traceMsg(
            self()->comp(), "last real tree n%dn\n", enclosingBlock->getLastRealTreeTop()->getNode()->getGlobalIndex());
    return induceOSRCallTree;
}
 ...
 */
int OMR::ResolvedMethodSymbol::matchInduceOSRCall(
    TR::TreeTop* insertionPoint, int16_t callerIndex, int16_t byteCodeIndex, const char* childPath)
{
    TR::Node* refNode = insertionPoint->getNode();
...
void OMR::ResolvedMethodSymbol::genAndAttachOSRCodeBlocks(int32_t currentInlinedSiteIndex)
{
    ...
    TR_OSRMethodData* osrMethodData
        = self()->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(currentInlinedSiteIndex, self());
    ...
    if (self()->comp()->isOutermostMethod() && self()->comp()->getHCRMode() == TR::osr) {
        ...
        if (!firstBlock->hasExceptionSuccessor(OSRCatchBlock))
            self()->comp()->getFlowGraph()->addEdge(
                TR::CFGEdge::createExceptionEdge(firstBlock, OSRCatchBlock, self()->comp()->trMemory()));
        ...
        TR_OSRPoint* osrPoint
            = new (self()->comp()->trHeapMemory()) TR_OSRPoint(firstBCI, osrMethodData, self()->comp()->trMemory());
    ...
    for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
            ...
            if (matchCode > 0) {
                ...
                if (trace) {
                    ...
                    traceMsg(self()->comp(), "fake induce %p generated for %s at callsite %d bytecode %x\n", ttnode,
                        mSignature, callSiteInsertionPoint, bcIndexInsertionPoint);
                }
                skipNextTT = true;
            }
        } else
        ...
        if (self()->comp()->isPotentialOSRPoint(tt->getNode())) {
            ...
            if (self()->comp()->isOSRTransitionTarget(TR::preExecutionOSR)
                || self()->comp()->requiresAnalysisOSRPoint(ttnode)) {
            ...
            if (self()->comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
                ...
                offsetBCI.setByteCodeIndex(
                    offsetBCI.getByteCodeIndex() + self()->comp()->getOSRInductionOffset(ttnode));
                osrPoint = new (self()->comp()->trHeapMemory())
            ...
            if (edge == block->getExceptionSuccessors().end())
                self()->comp()->getFlowGraph()->addEdge(
                    TR::CFGEdge::createExceptionEdge(block, OSRCatchBlock, self()->comp()->trMemory()));
        }
    ...
    TR_ASSERT_FATAL((osrCodeBlock != NULL) == (osrCatchBlock != NULL),
...
static void getNeighboringSymRefLists(int32_t index, TR_Array<List<TR::SymbolReference> >* listArray,
    List<TR::SymbolReference>** listForPrevSlot, List<TR::SymbolReference>** list,
...
bool OMR::ResolvedMethodSymbol::sharesStackSlot(TR::SymbolReference* symRef)
{
    TR_ASSERT(symRef->getSymbol()->isAutoOrParm(), "sharesStackSlot requires #%d to be on the stack",
    ...
    if (slot < 0)
        getNeighboringSymRefLists(
            -slot - 1, self()->getPendingPushSymRefs(), &listForPrevSlot, &list, &listForNextSlot);
    else
...
void OMR::ResolvedMethodSymbol::genOSRHelperCall(
    int32_t currentInlinedSiteIndex, TR::SymbolReferenceTable* symRefTab, TR::CFG* cfg)
{
    // If not specified use the outermost CFG
    if (!cfg)
        cfg = self()->comp()->getFlowGraph();
    ...
    TR::Node* vmThread = TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0,
        new (self()->comp()->trHeapMemory()) TR::SymbolReference(symRefTab,
            TR::RegisterMappedSymbol::createMethodMetaDataSymbol(self()->comp()->trHeapMemory(), "vmThread")));
    ...
    if (self()->comp()->cg()->getSupportsTM() && !self()->comp()->getOption(TR_DisableTLE) && !disableOSRwithTM) {
        ...
        tabortNode->setSymbolReference(
            self()->comp()->getSymRefTab()->findOrCreateTransactionAbortSymbolRef(self()->comp()->getMethodSymbol()));
        if (trace)
    ...
    if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
        ...
        TR::Node* osrReturnAddress
            = TR::Node::createLoad(firstNode, self()->comp()->getSymRefTab()->findOrCreateOSRReturnAddressSymbolRef());
        ...
        TR_OSRMethodData* callerOSRData
            = self()->comp()->getOSRCompilationData()->findCallerOSRMethodData(osrMethodData);
        TR_InlinedCallSite& callSiteInfo = self()->comp()->getInlinedCallSite(currentInlinedSiteIndex);
        callerOSRData->getMethodSymbol()->insertStoresForDeadStackSlots(
            self()->comp(), callSiteInfo._byteCodeInfo, OSRCodeBlock->getExit(), false);
        ...
        TR::Node* retNode
            = TR::Node::create(firstNode, self()->getResolvedMethod()->returnOpCode(), voidReturn ? 0 : 1);
        if (!voidReturn) {
            TR::Node* retValNode = TR::Node::create(
                firstNode, self()->comp()->il.opCodeForConst(self()->getResolvedMethod()->returnType()), 0);
...
bool OMR::ResolvedMethodSymbol::genIL(
    TR_FrontEnd* fe, TR::Compilation* comp, TR::SymbolReferenceTable* symRefTab, TR::IlGenRequest& customRequest)
{
    ...
    try {
        if (!_firstTreeTop || !comp->isPeekingMethod()) {
            ...
            //_automaticList.setListHead(0); // what's the point ? sym ref tab can use the sym refs anyway by using
            //methodSymbol's auto sym refs and pending push sym refs list; this only confuses analyses into thinking
            //these autos cannot be used when they actually can be
            ...
            if (_methodFlags.testAny(IlGenSuccess)) {
                if (!comp->isPeekingMethod()) {
                    if (self()->catchBlocksHaveRealPredecessors()) {
                        comp->failCompilation<TR::NoRecompilationRecoverableILGenException>(
                            "Catch blocks have real predecessors");
                    }
                }

                // If OSR is enabled, need to create an OSR helper call to keep the pending pushes,
                // parms, and locals alive. Only do this in non HCR mode since under HCR we will be more selective about
                // where to add OSR points.
                //
                const int32_t siteIndex = comp->getCurrentInlinedSiteIndex();
                bool doOSR = comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod()
                ...
                if (doOSR) {
                    ...
                    // to the current inlined call site, unlike getOSRPoints().
                    TR_OSRMethodData* osrMethodData = osrCompData->findOrCreateOSRMethodData(siteIndex, self());
...
bool OMR::ResolvedMethodSymbol::sharesStackSlots(TR::Compilation* comp)
{
    ...
    for (auto i = 0U; !isRequired && ppsListArray && i < ppsListArray->size(); ++i) {
        ...
        if ((ppsList.getSize() > 1) || (prevTakesTwoSlots && !ppsList.isEmpty())) {
            ListIterator<TR::SymbolReference> ppsIt(&ppsList);
            if (comp->getOption(TR_TraceOSR)) {
    ...
    for (auto i = 0U; !isRequired && autosListArray && i < autosListArray->size(); ++i) {
        ...
        if ((symRef->getCPIndex() < methodSymbol->getFirstJitTempIndex())
            && ((autosList.getSize() > 1) || (prevTakesTwoSlots && !autosList.isEmpty()))) {
            if (comp->getOption(TR_TraceOSR)) {
...
int32_t OMR::ResolvedMethodSymbol::getNumberOfBackEdges()
{
    ...
    for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if ((tt->getNode()->getOpCodeValue() == TR::asynccheck) && !inColdBlock)
            numBackEdges++;
    }
    return numBackEdges;
}

void OMR::ResolvedMethodSymbol::resetLiveLocalIndices()
{
...
bool OMR::ResolvedMethodSymbol::supportsInduceOSR(
    TR_ByteCodeInfo& bci, TR::Block* blockToOSRAt, TR::Compilation* comp, bool runCleanup)
{
...
bool OMR::ResolvedMethodSymbol::hasOSRProhibitions()
{
 ...
 */
void OMR::ResolvedMethodSymbol::setCannotAttemptOSR(int32_t n) { _cannotAttemptOSR->set(n); }
...
bool OMR::ResolvedMethodSymbol::cannotAttemptOSRDuring(int32_t callSite, TR::Compilation* comp, bool runCleanup)
{
    ...
    while (osrMethodData->getInlinedSiteIndex() > -1) {
        TR_ASSERT(callSite == osrMethodData->getInlinedSiteIndex(),
            "OSR method data and inlined call sites array being walked out of sync\n");
        ...
        if (!cannotAttemptOSR) {
            ...
            if (comp->getOption(TR_TraceOSR))
                traceMsg(
                    comp, "Checking if OSR can be attempted at caller bytecode index %d:%d\n", callSite, byteCodeIndex);
...
void OMR::ResolvedMethodSymbol::cleanupUnreachableOSRBlocks(int32_t inlinedSiteIndex, TR::Compilation* comp)
{
    ...
    while (osrMethodData) {
        ...
        if (!CallerOSRCodeBlock || CallerOSRCodeBlock->isUnreachable()) {
            if (comp->getOption(TR_TraceOSR))
                traceMsg(
                    comp, "Osr catch block at inlined site index %d is absent\n", osrMethodData->getInlinedSiteIndex());
            ...
            traceMsg(
                comp, "Osr catch block at inlined site index %d is present\n", osrMethodData->getInlinedSiteIndex());
    ...
    if (!allCallersOSRCodeBlocksAreStillInCFG) {
        TR_OSRMethodData* cursorOsrMethodData
            = comp->getOSRCompilationData()->findOrCreateOSRMethodData(inlinedSiteIndex, self());
...
void OMR::ResolvedMethodSymbol::insertRematableStoresFromCallSites(
    TR::Compilation* comp, int32_t siteIndex, TR::TreeTop* induceOSRTree)
{
 ...
 * point.
 */
TR_ByteCodeInfo& OMR::ResolvedMethodSymbol::getOSRByteCodeInfo(TR::Node* node)
{
 ...
 */
bool OMR::ResolvedMethodSymbol::isOSRRelatedNode(TR::Node* node)
{
    return (node->getOpCode().isStoreDirect() && node->getOpCode().hasSymbolReference()
               && node->getSymbolReference()->getSymbol()->isPendingPush())
        || (node->getOpCodeValue() == TR::treetop && node->getFirstChild()->getOpCode().isLoadVarDirect()
 ...
 */
bool OMR::ResolvedMethodSymbol::isOSRRelatedNode(TR::Node* node, TR_ByteCodeInfo& osrBCI)
{
    TR_ByteCodeInfo& bci = node->getByteCodeInfo();
    bool byteCodeIndex
        = osrBCI.getCallerIndex() == bci.getCallerIndex() && osrBCI.getByteCodeIndex() == bci.getByteCodeIndex();
 ...
 */
TR::TreeTop* OMR::ResolvedMethodSymbol::getOSRTransitionTreeTop(TR::TreeTop* tt)
{
    if (self()->comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
 ...
 * \param keepStashedArgsLive Flag to toggle how symrefs used as arguments to this BCI are treated. If transitioning
 * directly to this BCI, it should be true. If transitioning within a deeper frame to the current, it should be false.
 * Defaults true.
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlots(
    TR::Compilation* comp, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* insertTree, bool keepStashedArgsLive)
{
    ...
    if (comp->getOption(TR_TraceOSR))
        traceMsg(comp,
            "Inserting stores for dead stack slots in method at caller index %d and bytecode index %d for induceOSR "
            "call %p\n",
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (!nextSymRef->getSymbol()->isParm()
            && performTransformation(comp,
                "OSR LIVE RANGE ANALYSIS : Local %d is reset before tree [%p] (caller index %d bytecode index %d)\n",
                nextSymRef->getReferenceNumber(), insertTree->getNode(), callSite, byteCodeIndex)) {
            TR::Node* storeNode = TR::Node::createWithSymRef(
                comp->il.opCodeForDirectStore(nextSymRef->getSymbol()->getDataType()), 1, 1,
 ...
 * It does not have to concern itself with dead stores for any outer call sites, as they are located in the
 * OSRCodeBlocks for each frame, as demonstrated in the above example. This is safe as all OSR transitions in a callee
 * will have the same liveness in the caller's frame.
 *
 * \param byteCodeInfo BCI the OSR transition targets, used to look up dead symrefs in OSRLiveRangeAnalysis results.
 * \param induceOSRTree Tree to prepend dead stores to.
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(
    TR::Compilation* comp, int32_t inlinedSiteIndex, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* induceOSRTree)
{
...
TR_OSRPoint* OMR::ResolvedMethodSymbol::findOSRPoint(TR_ByteCodeInfo& bcInfo)
{
...
void OMR::ResolvedMethodSymbol::addAutomatic(TR::AutomaticSymbol* p)
{
...
void OMR::ResolvedMethodSymbol::addVariableSizeSymbol(TR::AutomaticSymbol* s)
{
...
int32_t OMR::ResolvedMethodSymbol::setTempIndex(int32_t index, TR_FrontEnd* fe)
{
...
ncount_t OMR::ResolvedMethodSymbol::generateAccurateNodeCount()
{
...
ncount_t OMR::ResolvedMethodSymbol::recursivelyCountChildren(TR::Node* node)
{
    ncount_t count = 1;
...
List<TR::SymbolReference>& OMR::ResolvedMethodSymbol::getAutoSymRefs(int32_t slot)
{
    ...
    if (!_autoSymRefs) {
        if (self()->comp()->getJittedMethodSymbol() == self())
            ...
            _autoSymRefs = new (m->trHeapMemory()) TR_Array<List<TR::SymbolReference> >(
                m, _resolvedMethod->numberOfParameterSlots() + _resolvedMethod->numberOfTemps() + 5, true);
    }
...
TR::SymbolReference* OMR::ResolvedMethodSymbol::getParmSymRef(int32_t slot)
{
    TR_ASSERT(_parmSymRefs, "_parmSymRefs should not be NULL!");
    return (*_parmSymRefs)[slot];
}

void OMR::ResolvedMethodSymbol::setParmSymRef(int32_t slot, TR::SymbolReference* symRef)
{
    if (!_parmSymRefs)
        _parmSymRefs = new (self()->comp()->trHeapMemory())
            TR_Array<TR::SymbolReference*>(self()->comp()->trMemory(), self()->getNumParameterSlots());
    (*_parmSymRefs)[slot] = symRef;
}

List<TR::SymbolReference>& OMR::ResolvedMethodSymbol::getPendingPushSymRefs(int32_t slot)
{
    TR_Memory* m = self()->comp()->trMemory();
...
void OMR::ResolvedMethodSymbol::removeTree(TR::TreeTop* tt)
{
    TR::Node* node = tt->getNode();
    if (node != NULL) {
    ...
    if (prev != NULL)
        ...
        TR_ASSERT(_firstTreeTop == tt,
            "treetops are not forming a single doubly linked list: node address = %p, global index = %d", node,
            (node ? node->getGlobalIndex() : -1));
        _firstTreeTop = next;
    }
    if (next != NULL)
...
TR::TreeTop* OMR::ResolvedMethodSymbol::getLastTreeTop(TR::Block* b)
{
    if (!b)
...
TR::Block* OMR::ResolvedMethodSymbol::prependEmptyFirstBlock()
{
    TR::Node* firstNode = self()->getFirstTreeTop()->getNode();
...
void OMR::ResolvedMethodSymbol::setFirstTreeTop(TR::TreeTop* tt)
{
    _firstTreeTop = tt;
    if (tt)
...
bool OMR::ResolvedMethodSymbol::detectInternalCycles()
{
                ...
                if (!excepSucc.empty()) {
                    for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
                        ...
                        if (dest == node) {
                            ...
                            for (TR::TreeTop* tt = clonedCatch->getEntry(); tt != clonedCatch->getExit();
                            ...
                            //
                            if (containsMonexit) {
                                TR::TreeTop* storeTT = TR::TreeTop::create(
                                    comp, (TR::Node::create(clonedCatch->getEntry()->getNode(), TR::monexitfence, 0)));
                            ...
                            TR::Node* asyncNode = TR::Node::createWithSymRef(
                                dest->getEntry()->getNextTreeTop()->getNode(), TR::asynccheck, 0,
                            ...
                            TR::Node* gotoNode = TR::Node::create(
                                dest->getEntry()->getNextTreeTop()->getNode(), TR::Goto, 0, gotoBlock->getEntry());
...
bool OMR::ResolvedMethodSymbol::catchBlocksHaveRealPredecessors()
{
...
void OMR::ResolvedMethodSymbol::removeUnusedLocals()
{
    ...
    while (cursor) {
        ...
        if (local->getReferenceCount() == 0
            && !(liveButMaybeUnreferencedLocals && !local->isLiveLocalIndexUninitialized()
                && liveButMaybeUnreferencedLocals->isSet(local->getLiveLocalIndex()))) {
#ifdef DEBUG
    ...
    if (debug("traceUnusedLocals")) {
        ...
        diagnostic("%08d(%08d) %08d(%08d)   %s\n", beforeSize - removedSize, afterTotalSize, removedSize,
            removedTotalSize, self()->comp()->signature());
    }
#endif
}

int32_t OMR::ResolvedMethodSymbol::incTempIndex(TR_FrontEnd* fe) { return self()->setTempIndex(_tempIndex + 1, fe); }
...
int32_t OMR::ResolvedMethodSymbol::getArrayCopyTempSlot(TR_FrontEnd* fe)
{
...
uint32_t& OMR::ResolvedMethodSymbol::getLocalMappingCursor()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _localMappingCursor;
}
void OMR::ResolvedMethodSymbol::setLocalMappingCursor(uint32_t i)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    _localMappingCursor = i;
}

uint32_t OMR::ResolvedMethodSymbol::getProloguePushSlots()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _prologuePushSlots;
}
uint32_t OMR::ResolvedMethodSymbol::setProloguePushSlots(uint32_t s)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return (_prologuePushSlots = s);
}

uint32_t OMR::ResolvedMethodSymbol::getScalarTempSlots()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _scalarTempSlots;
}
uint32_t OMR::ResolvedMethodSymbol::setScalarTempSlots(uint32_t s)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return (_scalarTempSlots = s);
}

uint32_t OMR::ResolvedMethodSymbol::getObjectTempSlots()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _objectTempSlots;
}
uint32_t OMR::ResolvedMethodSymbol::setObjectTempSlots(uint32_t s)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return (_objectTempSlots = s);
}

bool OMR::ResolvedMethodSymbol::containsOnlySinglePrecision()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _methodFlags.testAny(OnlySinglePrecision);
}
void OMR::ResolvedMethodSymbol::setContainsOnlySinglePrecision(bool b)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    _methodFlags.set(OnlySinglePrecision, b);
}

bool OMR::ResolvedMethodSymbol::usesSinglePrecisionMode()
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    return _methodFlags.testAny(SinglePrecisionMode);
}
void OMR::ResolvedMethodSymbol::setUsesSinglePrecisionMode(bool b)
{
    TR_ASSERT(self()->isJittedMethod(), "Should have been created as a jitted method.");
    _methodFlags.set(SinglePrecisionMode, b);
}

bool OMR::ResolvedMethodSymbol::isParmVariant(TR::ParameterSymbol* parmSymbol)
{
    ...
    TR_ASSERT_FATAL(unsigned(parmSymbol->getOrdinal()) < numberOfParameters,
        "Parm %d (%p) cannot be owned by current method that only has %d parms", parmSymbol->getOrdinal(), parmSymbol,
        numberOfParameters);
...
void OMR::ResolvedMethodSymbol::detectVariantParms()
{
    TR_ASSERT_FATAL(self()->getFirstTreeTop() != NULL && self()->getLastTreeTop() != NULL,
        "Can only detect variant parms for methods with Trees.");
    if (_variantParms == NULL)
        _variantParms = new (self()->comp()->trHeapMemory()) TR_BitVector(
            self()->getResolvedMethod()->numberOfParameters(), self()->comp()->trMemory(), heapAlloc, notGrowable);
    for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
...
static bool compareProfilingOffsetInfo(
    const std::pair<int32_t, int32_t>& first, const std::pair<int32_t, int32_t>& second)
{
 ...
 */
void OMR::ResolvedMethodSymbol::clearProfilingOffsetInfo() { _bytecodeProfilingOffsets.clear(); }

TR::KnownObjectTable::Index OMR::ResolvedMethodSymbol::getKnownObjectIndexForParm(int32_t ordinal)
{
...
template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::create(
    TR_StackMemory m, TR_ResolvedMethod* rm, TR::Compilation* comp);
template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::create(
    TR_HeapMemory m, TR_ResolvedMethod* rm, TR::Compilation* comp);
template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::create(
    PERSISTENT_NEW_DECLARE m, TR_ResolvedMethod* rm, TR::Compilation* comp);

template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::createJittedMethodSymbol(
    TR_StackMemory m, TR_ResolvedMethod* method, TR::Compilation* comp);
template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::createJittedMethodSymbol(
    TR_HeapMemory m, TR_ResolvedMethod* method, TR::Compilation* comp);
template TR::ResolvedMethodSymbol* OMR::ResolvedMethodSymbol::createJittedMethodSymbol(
    PERSISTENT_NEW_DECLARE m, TR_ResolvedMethod* method, TR::Compilation* comp);
