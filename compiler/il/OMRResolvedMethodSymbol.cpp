...
TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::create(AllocatorType m, TR_ResolvedMethod *rm,
    TR::Compilation *comp)
{
    ...
    , _bytecodeProfilingOffsets(comp->allocator())
{
    ...
    _properties.set(CanSkipFlattenableArrayElementNonHelperCall,
        self()->safeToSkipFlattenableArrayElementNonHelperCall());
...
template<typename AllocatorType>
TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::createJittedMethodSymbol(AllocatorType m,
 ...
 */
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCallNode(TR::TreeTop *insertionPoint, int32_t numChildren,
    bool copyChildren, bool shouldSplitBlock, TR::CFG *cfg)
{
    ...
    TR::TreeTop *branch, bool extendRemainder, int32_t offset, TR::TreeTop **lastTreeTop)
{
    ...
    if (self()->supportsInduceOSR(induceBCI, block, self()->comp())) {
        ...
        return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(osrBlock->getExit(), induceBCI, false,
            self()->comp());
    }
    return NULL;
}
...
{
    if (self()->supportsInduceOSR(insertionPoint->getNode()->getByteCodeInfo(), insertionPoint->getEnclosingBlock(),
            self()->comp()))
...
TR::TreeTop *OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(TR::TreeTop *insertionPoint,
    TR_ByteCodeInfo induceBCI, bool shouldSplitBlock, TR::Compilation *comp)
{
    TR::TreeTop *induceOSRCallTree
    ...
    int32_t numChildren, bool copyChildren, bool shouldSplitBlock, TR::CFG *cfg)
{
    ...
    return self()->genInduceOSRCall(insertionPoint, inlinedSiteIndex, osrMethodData, numChildren, copyChildren,
        shouldSplitBlock, cfg);
}
 ...
 */
int OMR::ResolvedMethodSymbol::matchInduceOSRCall(TR::TreeTop *insertionPoint, int16_t callerIndex,
    int16_t byteCodeIndex, const char *childPath)
{
    TR::Node *refNode = insertionPoint->getNode();
...
bool OMR::ResolvedMethodSymbol::sharesStackSlot(TR::SymbolReference *symRef)
{
    ...
    if (slot < 0)
        getNeighboringSymRefLists(-slot - 1, self()->getPendingPushSymRefs(), &listForPrevSlot, &list,
            &listForNextSlot);
    else
...
void OMR::ResolvedMethodSymbol::genOSRHelperCall(int32_t currentInlinedSiteIndex, TR::SymbolReferenceTable *symRefTab,
    TR::CFG *cfg)
{
    // If not specified use the outermost CFG
    if (!cfg)
    ...
    if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
        ...
        if (!voidReturn) {
            TR::Node *retValNode = TR::Node::create(firstNode,
                self()->comp()->il.opCodeForConst(self()->getResolvedMethod()->returnType()), 0);
...
bool OMR::ResolvedMethodSymbol::genIL(TR_FrontEnd *fe, TR::Compilation *comp, TR::SymbolReferenceTable *symRefTab,
    TR::IlGenRequest &customRequest)
{
...
bool OMR::ResolvedMethodSymbol::supportsInduceOSR(TR_ByteCodeInfo &bci, TR::Block *blockToOSRAt, TR::Compilation *comp,
    bool runCleanup)
{
...
void OMR::ResolvedMethodSymbol::insertRematableStoresFromCallSites(TR::Compilation *comp, int32_t siteIndex,
    TR::TreeTop *induceOSRTree)
{
 ...
 * Defaults true.
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlots(TR::Compilation *comp, TR_ByteCodeInfo &byteCodeInfo,
    TR::TreeTop *insertTree, bool keepStashedArgsLive)
{
 ...
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(TR::Compilation *comp,
...
List<TR::SymbolReference> &OMR::ResolvedMethodSymbol::getAutoSymRefs(int32_t slot)
{
    ...
    if (!_autoSymRefs) {
        if (self()->comp()->getJittedMethodSymbol() == self())
            ...
            _autoSymRefs = new (m->trHeapMemory()) TR_Array<List<TR::SymbolReference> >(m,
                _resolvedMethod->numberOfParameterSlots() + _resolvedMethod->numberOfTemps() + 5, true);
    }
                ...
                if (!excepSucc.empty()) {
                    for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
                        ...
                        if (dest == node) {
                            ...
                            //
                            if (containsMonexit) {
                                TR::TreeTop *storeTT = TR::TreeTop::create(comp,
                                    (TR::Node::create(clonedCatch->getEntry()->getNode(), TR::monexitfence, 0)));
...
static bool compareProfilingOffsetInfo(const std::pair<int32_t, int32_t> &first,
...
template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::create(TR_StackMemory m, TR_ResolvedMethod *rm,
    TR::Compilation *comp);
template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::create(TR_HeapMemory m, TR_ResolvedMethod *rm,
    TR::Compilation *comp);
template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::create(PERSISTENT_NEW_DECLARE m, TR_ResolvedMethod *rm,
    TR::Compilation *comp);

template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::createJittedMethodSymbol(TR_StackMemory m,
    TR_ResolvedMethod *method, TR::Compilation *comp);
template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::createJittedMethodSymbol(TR_HeapMemory m,
    TR_ResolvedMethod *method, TR::Compilation *comp);
template TR::ResolvedMethodSymbol *OMR::ResolvedMethodSymbol::createJittedMethodSymbol(PERSISTENT_NEW_DECLARE m,
    TR_ResolvedMethod *method, TR::Compilation *comp);
