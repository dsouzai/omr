...
#define DOUBLE_ORDER(d) \
    (TR::Compiler->host.cpu.isLittleEndian() ? ((((uint64_t)(d)) >> 32) | (((uint64_t)(d)) << 32)) : (d))
#else
...
// These are the max sizes representable on a TR::Node a codegen may have smaller limits when deciding what operations
// to inline If these sizes are changed then the sizes in the Node::DecimalInfo bit container below may have to be
// changed
#define TR_MAX_DECIMAL_PRECISION \
    63 // NOTE: update J9::DataType::getMaxPackedDecimalPrecision() when changing this constant
...
#define TR_IGNORED_SIGN_CODE \
    (-1) // a special marker value for when a setSign operation can set any or no sign code value (as it will be ignored
         // by consumer)

#define TR_BCD_SIGN_CHAR_SIZE 1 // one byte for leading leading +/-/u char
#define TR_BCD_STRLEN                                 \
    (TR_MAX_DECIMAL_PRECISION + TR_BCD_SIGN_CHAR_SIZE \
        + 1) // +1 for sign, +1 for terminating null -- for pretty printed string e.g. "+1234"

#define TR_MAX_OTYPE_SIZE TR::getMaxSigned<TR::Int32>()

enum TR_YesNoMaybe { TR_no, TR_yes, TR_maybe };
...
// And ensure the bit fields in Node::DecimalInfo are still big enough -- keeping in mind that the implementation
// defined choice for the underlying type may be signed (see note below next to 'last_raw_bcd_sign    =
// raw_bcd_sign_0xf')
...
enum TR_RawBCDSignCode {
    ...
    raw_bcd_sign_0xd = 2,
    raw_bcd_sign_0xf = 3,
    last_raw_bcd_sign = raw_bcd_sign_0xf, // NOTE: the TR_RawBCDSignCode fields in Node::DecimalInfo are 3 bits so the
                                          // max value for a raw_bcd_sign_* is 7 (enum range is -8->7)
    num_raw_bcd_sign_codes
};

enum TR_SignCodeSize { UnknownSignCodeSize, EmbeddedHalfByte, SeparateOneByte, SeparateTwoByte };

enum TR_DigitSize { UnknownDigitSize, HalfByteDigit, OneByteDigit, TwoByteDigit };
...
template<TR::DataTypes T> inline int64_t getMinSigned()
{
    static_assert(T == -1, "getMinSigned is not valid for this type");
    return 0;
}
template<TR::DataTypes T> inline uint64_t getMinUnsigned()
{
    static_assert(T == -1, "getMinUnsigned is not valid for this type");
    return 0;
}
template<TR::DataTypes T> inline int64_t getMaxSigned()
{
    static_assert(T == -1, "getMaxSigned is not valid for this type");
    return 0;
}
template<TR::DataTypes T> inline uint64_t getMaxUnsigned()
{
    static_assert(T == -1, "getMaxUnsigned is not valid for this type");
    return 0;
}
template<TR::DataTypes T> inline int32_t getMaxSignedPrecision()
{
    static_assert(T == -1, "getMaxSignedPrecision is not valid for this type");
    return 0;
}
template<TR::DataTypes T> inline int32_t getMaxUnsignedPrecision()
{
    static_assert(T == -1, "getMaxUnsignedPrecision is not valid for this type");
    return 0;
}
...
template<> inline int64_t getMaxSigned<TR::Int64>()
{
    return ((int64_t)(((uint64_t)((int64_t)-1)) >> 1)) /**<  9223372036854775807 */;
}
...
public:
    ...
    template<typename T> static bool isSignedInt()
    {
        return isSignedInt8<T>() || isSignedInt16<T>() || isSignedInt32<T>() || isSignedInt64<T>();
    }
    ...
    template<typename T> static bool isUnsignedInt()
    {
