    ...
    , _unionPropertyA()
{
    ...
    if (ilGen) {
        ...
        TR_ASSERT(
            _byteCodeInfo.getCallerIndex() < (SHRT_MAX / 4), "Caller index too high; cannot set high order bit\n");
 ...
 */
void OMR::Node::copyChildren(
    TR::Node* from, uint16_t numChildren /* ignored if !forNodeExtensionOnly */, bool forNodeExtensionOnly)
{
    ...
    if (from->hasNodeExtension()) {
        ...
        if (numChildren > from->_unionBase._extension.getNumElems()) {
            ...
            self()->copyNodeExtension(
                from->_unionBase._extension.getExtensionPtr(), from->_unionBase._extension.getNumElems(), size);
        }
    } else {
 ...
 */
TR::Node* OMR::Node::recreateAndCopyValidPropertiesImpl(
    TR::Node* originalNode, TR::ILOpCodes op, TR::SymbolReference* newSymRef)
{
...
TR::Node* OMR::Node::createInternal(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* originalNode)
{
    if (!originalNode)
...
TR::Node* OMR::Node::create(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::TreeTop* dest)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    TR::Node* node = TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
    if (dest)
...
TR::Node* OMR::Node::create(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, int32_t intValue, TR::TreeTop* dest)
{
...
TR::Node* OMR::Node::create(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, TR::Node* second)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    TR::Node* node = TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
    node->setAndIncChild(0, first);
...
TR::Node* OMR::Node::createWithSymRef(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, TR::SymbolReference* symRef)
{
...
TR::Node* OMR::Node::recreateWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node* OMR::Node::createWithSymRef(
    TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, uintptr_t extraChildren, TR::SymbolReference* symRef)
{
...
TR::Node* OMR::Node::createWithSymRef(
    TR::ILOpCodes op, uint16_t numChildren, TR::SymbolReference* symRef, uintptr_t extraChildrenForFixup)
{
...
TR::Node* OMR::Node::recreateWithoutSymRef_va_args(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, va_list& args)
{
...
TR::Node* OMR::Node::recreateWithoutSymRef(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...)
{
    va_list args;
    va_start(args, numChildArgs);
...
TR::Node* OMR::Node::recreateWithSymRefWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...)
{
    va_list args;
    va_start(args, numChildArgs);
...
TR::Node* OMR::Node::create(
    TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::createWithoutSymRef(
        op, numChildren, 8, first, second, third, fourth, fifth, sixth, seventh, eighth);
}

TR::Node* OMR::Node::recreateWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* first)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node* OMR::Node::recreateWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, TR::Node* second)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
TR::Node* OMR::Node::recreateWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, TR::Node* second, TR::Node* third)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithoutSymRef(
        originalNode, op, numChildren, 6, first, second, third, fourth, fifth, sixth);
}
    ...
    TR::Node* seventh)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithoutSymRef(
        originalNode, op, numChildren, 7, first, second, third, fourth, fifth, sixth, seventh);
}
    ...
    TR::Node* seventh, TR::Node* eighth)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithoutSymRef(
        originalNode, op, numChildren, 8, first, second, third, fourth, fifth, sixth, seventh, eighth);
}

TR::Node* OMR::Node::recreateWithoutProperties(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, TR::Node* first, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
...
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 3, first, second, third, symRef);
}
    ...
    TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 4, first, second, third, fourth, symRef);
}
    ...
    TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, TR::Node* fifth, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 5, first, second, third, fourth, fifth, symRef);
}
    ...
    TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 6, first, second, third, fourth, fifth, sixth, symRef);
}
    ...
    TR::Node* seventh, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 7, first, second, third, fourth, fifth, sixth, seventh, symRef);
}
    ...
    TR::Node* seventh, TR::Node* eighth, TR::SymbolReference* symRef)
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRefWithoutProperties(
        originalNode, op, numChildren, 8, first, second, third, fourth, fifth, sixth, seventh, eighth, symRef);
}
...
TR::Node* OMR::Node::createWithRoomForThree(
    TR::ILOpCodes op, TR::Node* first, TR::Node* second, void* symbolRefOrBranchTarget)
{
...
{
    TR_ASSERT(
        (first != NULL) && (second != NULL) && (third != NULL) && (fourth != NULL), "must supply all four children");
...
TR::Node* OMR::Node::createCase(
    TR::Node* originatingByteCodeNode, TR::TreeTop* destination, CASECONST_TYPE caseConstant)
{
 ...
 */
TR::Node* OMR::Node::createArrayOperation(
    TR::ILOpCodes arrayOp, TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, TR::Node* fifth)
{
...
TR::Node* OMR::Node::createPotentialOSRPointHelperCallInILGen(
    TR::Node* originatingByteCodeNode, int32_t osrInductionOffset)
{
    TR::Compilation* comp = TR::comp();
...
TR::Node* OMR::Node::createOSRFearPointHelperCall(TR::Node* originatingByteCodeNode)
{
    ...
    TR::Node* callNode = TR::Node::createWithSymRef(
        originatingByteCodeNode, TR::call, 0, TR::comp()->getSymRefTab()->findOrCreateOSRFearPointHelperSymbolRef());
    return callNode;
}
...
TR::Node* OMR::Node::createLoad(TR::Node* originatingByteCodeNode, TR::SymbolReference* symRef)
{
    TR::Node* load = TR::Node::createWithSymRef(
        originatingByteCodeNode, TR::comp()->il.opCodeForDirectLoad(symRef->getSymbol()->getDataType()), 0, symRef);
...
{
    return TR::Node::createStore(
        symRef, value, TR::comp()->il.opCodeForDirectStore(symRef->getSymbol()->getDataType()), 0);
}
...
TR::Node* OMR::Node::createAddressNode(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uintptr_t address, uint8_t precision)
{
 ...
 */
bool OMR::Node::mightHaveVolatileSymbolReference()
{
    if (self()->getOpCode().hasSymbolReference())
...
TR_YesNoMaybe OMR::Node::computeIsCollectedReferenceImpl(
    TR::NodeChecklist& processedNodesCollected, TR::NodeChecklist& processedNodesNotCollected)
{
    ...
    while (curNode) {
        ...
        if (op.isSelect()) {
            ...
            if (TR_maybe == secondChildResult) {
                TR_YesNoMaybe thirdChildResult = curNode->getThirdChild()->computeIsCollectedReferenceImpl(
                    processedNodesCollected, processedNodesNotCollected);
                return recordProcessedNodeResult(
                    receiverNode, thirdChildResult, processedNodesCollected, processedNodesNotCollected);
            } else
                return recordProcessedNodeResult(
                    receiverNode, secondChildResult, processedNodesCollected, processedNodesNotCollected);
        }
        ...
        if (op.isLoadVar() || op.isLoadAddr() || op.isLoadReg()) {
            ...
            if (opValue == TR::aloadi && symbol == TR::comp()->getSymRefTab()->findGenericIntShadowSymbol())
                return recordProcessedNodeResult(
                    receiverNode, TR_yes, processedNodesCollected, processedNodesNotCollected);
            else
        ...
        case TR::aconst:
            ...
            if (curNode->getAddress() != 0)
                return recordProcessedNodeResult(
                    receiverNode, TR_no, processedNodesCollected, processedNodesNotCollected);
            else {
                ...
                if (self() != curNode)
                    ...
                    return recordProcessedNodeResult(
                        receiverNode, TR_maybe, processedNodesCollected, processedNodesNotCollected);
            }
        default:
...
bool OMR::Node::mayModifyValue(TR::SymbolReference* symRef)
{
    ...
    if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monexit
        || (node->getOpCode().hasSymbolReference() && node->getSymbol()->isVolatile()) || symbolIsUnresolved) {
...
bool OMR::Node::performsVolatileAccess(vcount_t visitCount)
{
...
bool OMR::Node::isClassUnloadingConst()
{
    ...
    if ((((constNode->isClassPointerConstant())
             && !TR::Compiler->cls.sameClassLoaders(
                 c, (TR_OpaqueClassBlock*)constNode->getAddress(), c->getCurrentMethod()->classOfMethod()))
            || ((constNode->isMethodPointerConstant()) && !c->compileRelocatableCode()
                && !TR::Compiler->cls.sameClassLoaders(c,
                    c->fe()
                        ->createResolvedMethod(
                            c->trMemory(), (TR_OpaqueMethodBlock*)constNode->getAddress(), c->getCurrentMethod())
                        ->classOfMethod(),
...
{
    TR_ASSERT(
        self()->getOpCode().isCall(), "getFirstArgumentIndex called for non-call node " POINTER_PRINTF_FORMAT, self());
...
void OMR::Node::setSignExtendTo32BitAtSource(bool v)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting signExtendedTo32BitAtSource flag on node %p to %d\n", self(), v)) {
...
void OMR::Node::setSignExtendTo64BitAtSource(bool v)
{
    ...
    TR_ASSERT(self()->getOpCode().isLoadVar(), "Can only call this for variable loads\n");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting signExtendedTo64BitAtSource flag on node %p to %d\n", self(), v)) {
...
void OMR::Node::setZeroExtendTo32BitAtSource(bool v)
{
    ...
    TR_ASSERT(self()->getOpCode().isLoadVar(), "Can only call this for variable loads\n");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting zeroExtendTo32BitAtSource flag on node %p to %d\n", self(), v)) {
...
void OMR::Node::setZeroExtendTo64BitAtSource(bool v)
{
    ...
    TR_ASSERT(self()->getOpCode().isLoadVar(), "Can only call this for variable loads\n");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting zeroExtendTo64BitAtSource flag on node %p to %d\n", self(), v)) {
...
void OMR::Node::setIsInvalid8BitGlobalRegister(bool v)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting invalid8BitGlobalRegister flag on node %p to %d\n", self(), v))
...
void OMR::Node::setIsSafeToSkipTableBoundCheck(bool b)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting canSkipTableBoundCheck flag on node %p to %d\n", self(), b))
...
void OMR::Node::setStoredValueIsIrrelevant(bool v)
{
    ...
    if (self()->getOpCode().isStore() && self()->getSymbolReference()->getSymbol()->isAutoOrParm()) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting StoredValueIsIrrelevant flag on node %p to %d\n", self(), v))
...
void OMR::Node::setLiveMonitorInitStore(bool v)
{
    ...
    if (self()->getOpCode().hasSymbolReference() && self()->getSymbol()->holdsMonitoredObject()
        && performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting liveMonitorInitStore flag on node %p to %d\n", self(), v))
...
void OMR::Node::setUseSignExtensionMode(bool b)
{
    ...
    if (self()->getOpCode().isLoadVar() && self()->getType().isInt32()
        && performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting useSignExtensionMode flag on node %p to %d\n", self(), b)) {
...
void OMR::Node::setHasFoldedImplicitNULLCHK(bool v)
{
    ...
    TR_ASSERT(self()->getOpCode().isBndCheck() || self()->getOpCode().isSpineCheck(), "assertion failure");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting foldedImplicitNULLCHK flag on node %p to %d\n", self(), v))
...
void OMR::Node::setThrowInsertedByOSR(bool v)
{
    ...
    if (c->getOption(TR_EnableOSR) && (self()->getOpCodeValue() == TR::athrow)) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting ThrowInsertedByOSR flag on node %p to %d\n", self(), v))
...
{
    TR::Compilation* c = TR::comp();
    if (performNodeTransformation1(
            c, "O^O NODE FLAGS: Setting virtualCallNodeForAGuardedInlinedCall flag on node %p\n", self()))
...
void OMR::Node::resetIsTheVirtualCallNodeForAGuardedInlinedCall()
{
    TR::Compilation* c = TR::comp();
    if (performNodeTransformation1(
            c, "ReO^O NODE FLAGS: Setting virtualCallNodeForAGuardedInlinedCall flag on node %p\n", self()))
...
void OMR::Node::setDontTransformArrayCopyCall()
{
    ...
    if (self()->isArrayCopyCall()) {
        if (performNodeTransformation1(
                c, "O^O NODE FLAGS: Setting dontTransformArrayCopyCall flag on node %p\n", self()))
...
void OMR::Node::setNodeIsRecognizedArrayCopyCall(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::call, "Opcode must be vcall to arraycopy");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting nodeIsRecognizedArrayCopyCall flag on node %p to %d\n", self(), v))
...
void OMR::Node::setContainsCompressionSequence(bool v)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting isCompressionSequence flag on node %p to %d\n", self(), v))
...
void OMR::Node::setSourceIsByteArrayTranslate(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate, "Opcode must be arraytranslate");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting sourceIsByteArrayTranslate flag on node %p to %d\n", self(), v))
...
void OMR::Node::setTargetIsByteArrayTranslate(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate, "Opcode must be arraytranslate");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting targetIsByteArrayTranslate flag on node %p to %d\n", self(), v))
...
void OMR::Node::setSourceCellIsTermChar(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate, "Opcode must be arraytranslate");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting sourceCellIsTermChar flag on node %p to %d\n", self(), v))
...
void OMR::Node::setTableBackedByRawStorage(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate, "Opcode must be arraytranslate");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting tableBackedByRawStorage flag on node %p to %d\n", self(), v))
...
void OMR::Node::setHalfWordElementArrayCopy(bool v)
{
    ...
    if (v) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting halfWordElementArrayCopy flag on node %p to %d\n", self(), v))
...
void OMR::Node::setWordElementArrayCopy(bool v)
{
    ...
    if (v) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting wordElementArrayCopy flag on node %p to %d\n", self(), v))
...
void OMR::Node::setForwardArrayCopy(bool v)
{
    ...
    if (v) {
        if (!wasRarePathForwardArrayCopy
            && performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting forwardArrayCopy flag on node %p to %d\n", self(), v))
...
void OMR::Node::setBackwardArrayCopy(bool v)
{
    ...
    if (v) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting backwardArrayCopy flag on node %p to %d\n", self(), v))
            _flags.setValue(arraycopyDirectionMask, arraycopyDirectionBackward);
        TR_ASSERT(
            self()->isBackwardArrayCopy() && !self()->isForwardArrayCopy() && !self()->isRarePathForwardArrayCopy(),
            "assertion failure");
    } else {
        if (self()->isBackwardArrayCopy()
            && performNodeTransformation2(
...
void OMR::Node::setRarePathForwardArrayCopy(bool v)
{
    ...
    if (v != self()->isRarePathForwardArrayCopy()
        && performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting rarePathForwardArrayCopy flag on node %p to %d\n", self(), v))
        _flags.setValue(arraycopyDirectionMask, v ? arraycopyDirectionForwardRarePath : arraycopyDirectionUnknown);
    if (v)
        TR_ASSERT(
            self()->isRarePathForwardArrayCopy() && self()->isForwardArrayCopy() && !self()->isBackwardArrayCopy(),
            "assertion failure");
    else
...
void OMR::Node::setNoArrayStoreCheckArrayCopy(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::arraycopy, "Opcode must be arraycopy");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting noArrayStoreCheckArrayCopy flag on node %p to %d\n", self(), v))
...
void OMR::Node::setArrayChkPrimitiveArray1(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK, "Opcode must be ArrayCHK");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting arrayChkPrimitiveArray1 flag on node %p to %d\n", self(), v))
...
void OMR::Node::setArrayChkReferenceArray1(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK, "Opcode must be ArrayCHK");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting arrayChkReferenceArray1 flag on node %p to %d\n", self(), v))
...
void OMR::Node::setArrayChkPrimitiveArray2(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK, "Opcode must be ArrayCHK");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting arrayChkPrimitiveArray2 flag on node %p to %d\n", self(), v))
...
void OMR::Node::setArrayChkReferenceArray2(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK, "Opcode must be ArrayCHK");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting arrayChkReferenceArray2 flag on node %p to %d\n", self(), v))
...
void OMR::Node::setIsDontMoveUnderBranch(bool v)
{
    ...
    if (self()->getOpCode().isLoadVarDirect()
        && performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting dontMoveUnderBranch flag on node %p to %d\n", self(), v)) {
...
void OMR::Node::setIsMaxLoopIterationGuard(bool v)
{
    ...
    TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting maxLoopIterationGuard flag on node %p to %d\n", self(), v))
...
bool OMR::Node::vftEntryIsInBounds()
{
    TR_ASSERT_FATAL_WITH_NODE(
        self(), self()->isTheVirtualGuardForAGuardedInlinedCall(), "vftEntryIsInBounds can only be queried on guards");
...
{
    TR_ASSERT_FATAL_WITH_NODE(
        self(), self()->isTheVirtualGuardForAGuardedInlinedCall(), "vftEntryIsInBounds can only be set on guards");
...
void OMR::Node::setStoreAlreadyEvaluated(bool b)
{
    ...
    TR_ASSERT(self()->getOpCode().isStore(), "assertion failure");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting storeAlreadyEvaluated flag on node %p to %d\n", self(), b))
...
void OMR::Node::setNormalizeNanValues(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::fbits2i || self()->getOpCodeValue() == TR::dbits2l,
        "Opcode must be fbits2i or dbits2l");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting mustNormalizeNanValues flag on node %p to %d\n", self(), v))
...
void OMR::Node::setIsClassPointerConstant(bool b)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting classPointerConstant flag on node %p to %d\n", self(), b))
...
void OMR::Node::setIsMethodPointerConstant(bool b)
{
    ...
    TR_ASSERT((self()->getOpCodeValue() == TR::aconst), "Can only call this for aconst\n");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting methodPointerConstant flag on node %p to %d\n", self(), b))
...
bool OMR::Node::isUnneededIALoad()
{
    return (self()->getOpCodeValue() == TR::aloadi && _flags.testAny(unneededIALoad));
}

void OMR::Node::setUnneededIALoad(bool v)
{
    TR::Compilation* c = TR::comp();
...
void OMR::Node::setAllocationCanBeRemoved(bool v)
{
    ...
    TR_ASSERT(self()->getOpCodeValue() == TR::New || self()->getOpCodeValue() == TR::newarray
        ...
        "Opcode must be new, newarray, anewarray, or newvalue");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting allocationCanBeRemoved flag on node %p to %d\n", self(), v))
...
void OMR::Node::setIsFPStrictCompliant(bool v)
{
    ...
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting resultFPStrictCompliant flag on node %p to %d\n", self(), v))
...
void OMR::Node::oneParentSupportsLazyClobber(TR::Compilation* comp)
{
    if (self()->getOpCode().isConversion() && self()->getReferenceCount() <= 1
        && performTransformation(
            comp, "O^O LAZY CLOBBERING: setParentSupportsLazyClobber(%s)\n", comp->getDebug()->getName(self())))
...
{
    TR_ASSERT(
        self()->getOpCodeValue() == TR::PassThrough, "can only set copyToNewVirtualRegister flag on PassThrough\n");
...
bool OMR::Node::isOSRFearPointHelperCall()
{
    ...
    if (self()->getOpCode().isCall() && self()->getSymbol()->isMethod()
        && c->getSymRefTab()->isNonHelper(
            self()->getSymbolReference(), TR::SymbolReferenceTable::osrFearPointHelperSymbol))
        return true;
...
bool OMR::Node::isPotentialOSRPointHelperCall()
{
    ...
    if (self()->getOpCode().isCall() && self()->getSymbol()->isMethod()
        && c->getSymRefTab()->isNonHelper(
            self()->getSymbolReference(), TR::SymbolReferenceTable::potentialOSRPointHelperSymbol))
        return true;
