...
public:
    template<typename AllocatorType> static TR::AutomaticSymbol* create(AllocatorType);

    template<typename AllocatorType> static TR::AutomaticSymbol* create(AllocatorType, TR::DataType);

    template<typename AllocatorType> static TR::AutomaticSymbol* create(AllocatorType, TR::DataType, uint32_t);
...
public:
     ...
     */
    template<typename AllocatorType>
    static TR::AutomaticSymbol* createLocalObject(
     ...
     */
    template<typename AllocatorType>
    static TR::AutomaticSymbol* createLocalObject(AllocatorType m, TR::ILOpCodes kind, TR::SymbolReference* classSymRef,
...
public:
    ...
    template<typename AllocatorType>
    static TR::AutomaticSymbol* createInternalPointer(
        AllocatorType m, TR::DataType d, TR::AutomaticSymbol* pinningArrayPointer = 0);
...
#define TR_VSS_NAME "VTS"
    ...
    template<typename AllocatorType> static TR::AutomaticSymbol* createVariableSized(AllocatorType m, uint32_t s);
    ...
    enum {
        IsReferenced
        = 0x01, ///< was this symbol ever actually used in any instruction? -- if not then it does not have to be mapped
        IsAddressTaken = 0x02, ///< address taken symbols have to be kept alive longer
        IsSingleUse = 0x04, ///< a temp ref created for a one off privatization in a single evaluator (doesn't use
                            ///< refCounts so automatically freed)
    };
