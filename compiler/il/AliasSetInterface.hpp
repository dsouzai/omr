...
public:
    ...
    TR_AliasSetInterface(
        bool shares_symbol, TR::SymbolReference* symRef, bool isDirectCall = false, bool includeGCSafePoint = false)
        : _isDirectCall(isDirectCall)
        , _includeGCSafePoint(includeGCSafePoint)
        , _symbolReference(symRef)
    ...
    TR_BitVector* getTRAliases() { return getTRAliases_impl(_isDirectCall, _includeGCSafePoint); }

    template<class BitVector> bool getAliases(BitVector& aliases)
    {
    ...
    bool getAliases(TR_BitVector& aliases)
    {
        TR::Compilation* comp = TR::comp();
    ...
    bool getAliasesAndUnionWith(TR::SparseBitVector& aliases)
    {
        TR::Compilation* comp = TR::comp();
    ...
    template<class BitVector> bool getAliasesAndSubtractFrom(BitVector& v)
    {
        TR::Compilation* comp = TR::comp();
    ...
    template<class BitVector> bool getAliasesAndIntersectWith(BitVector& v)
    {
        TR::Compilation* comp = TR::comp();
    ...
    bool getAliasesAndSubtractFrom(TR_BitVector& bitvector)
    {
        TR::Compilation* comp = TR::comp();
    ...
    bool contains(TR::SymbolReference* symRef, TR::Compilation* comp)
    {
        return contains(symRef->getReferenceNumber(), comp);
    }

    bool contains(uint32_t refNum, TR::Compilation* comp)
    {
    ...
    template<class BitVector> bool containsAny(const BitVector& refs, TR::Compilation* comp)
    {
    ...
    void removeAliases(TR::SparseBitVector& aliasesToModify)
    {
        TR::Compilation* comp = TR::comp();
    ...
    void addAliases(TR::SparseBitVector& aliasesToModify)
    {
        TR::Compilation* comp = TR::comp();
    ...
    void addAlias(TR::SymbolReference* symRef2)
    {
        TR::Compilation* comp = TR::comp();
    ...
    void removeAlias(TR::SymbolReference* symRef2)
    {
        TR::Compilation* comp = TR::comp();
    ...
    void setAlias(TR::SymbolReference* symRef2, bool value)
    {
        setAlias_impl(symRef2, value, _isDirectCall, _includeGCSafePoint);
    }

    template<class BitVector> bool getAliasesWithClear(BitVector& aliases)
    {
        TR_BitVector* bc_aliases = getTRAliases();
        if (bc_aliases)
    ...
    bool getAliasesWithClear(TR_BitVector& aliases)
    {
        TR_BitVector* bc_aliases = getTRAliases();
        if (bc_aliases)
            aliases = *bc_aliases;
...
public:
    ...
    void setAlias_impl(TR::SymbolReference* symRef2, bool value, bool isDirectCall, bool includeGCSafePoint);

    void setAliases_impl(TR::SparseBitVector& aliasesToModify, bool value, bool isDirectCall, bool includeGCSafePoint)
    {
        TR::Compilation* comp = TR::comp();
        static_assert(_AliasSetInterface == UseDefAliasSet || _AliasSetInterface == UseOnlyAliasSet,
            "failed: can only call setAliases_impl for UseOnly or useDef presently");
    ...
    void removeAllAliases()
    {
        ...
        static_assert(_AliasSetInterface == UseDefAliasSet || _AliasSetInterface == UseOnlyAliasSet,
            "failed: can only call removeAllAliases for UseOnly or useDef presently");
    }

private:
    static void removeSymRef1KillsSymRef2Asymmetrically(
        TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint);
    static void addSymRef1KillsSymRef2Asymmetrically(
        TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint);
    static void setSymRef1KillsSymRef2Asymmetrically(
        TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint, bool value);
...
public:
    TR_UseDefAliasSetInterface(TR::SymbolReference* symRef, bool isDirectCall = false, bool includeGCSafePoint = false)
        : TR_AliasSetInterface<UseDefAliasSet>(symRef, isDirectCall, includeGCSafePoint)
    { }

    TR_UseDefAliasSetInterface(
        bool shares_symbol, TR::SymbolReference* symRef, bool isDirectCall = false, bool includeGCSafePoint = false)
        : TR_AliasSetInterface<UseDefAliasSet>(shares_symbol, symRef, isDirectCall, includeGCSafePoint)
    { }
};

class TR_UseOnlyAliasSetInterface : public TR_AliasSetInterface<UseOnlyAliasSet> {
public:
    TR_UseOnlyAliasSetInterface(TR::SymbolReference* symRef, bool isDirectCall = false, bool includeGCSafePoint = false)
        : TR_AliasSetInterface<UseOnlyAliasSet>(symRef, isDirectCall, includeGCSafePoint)
    { }
};
 ...
 */
template<>
inline TR_BitVector* TR_AliasSetInterface<UseDefAliasSet>::getTRAliases_impl(bool isDirectCall, bool includeGCSafePoint)
{
    if (_symbolReference) {
        if (_shares_symbol)
...
template<>
inline TR_BitVector* TR_AliasSetInterface<UseOnlyAliasSet>::getTRAliases_impl(
    bool isDirectCall, bool includeGCSafePoint)
{
    if (!_symbolReference)
...
inline void TR_AliasSetInterface<_AliasSetInterface>::setAlias_impl(
    TR::SymbolReference* symRef2, bool value, bool isDirectCall, bool includeGCSafePoint)
{
    TR::Compilation* comp = TR::comp();
    if (symRef2 == NULL)
        return;
    ...
    static_assert(_AliasSetInterface == UseDefAliasSet || _AliasSetInterface == UseOnlyAliasSet,
        "failed: can only call setAlias_impl for UseOnly or useDef presently");

    {
        if (_AliasSetInterface == UseDefAliasSet) {
            ...
            if (_symbolReference->getSymbol()->isMethod()
                && !symRef2->getSymbol()->isMethod()) { // foo uses x, same as x kills foo
                symRef2->getUseDefAliases(includeGCSafePoint).setAlias(_symbolReference, value);
            } else if (!_symbolReference->getSymbol()->isMethod()
                && symRef2->getSymbol()->isMethod()) { // x uses foo, same as foo kills x
                symRef2->getUseDefAliases(includeGCSafePoint).setAlias(_symbolReference, value);
            } else if (!_symbolReference->getSymbol()->isMethod()
                && !symRef2->getSymbol()->isMethod()) { // x uses y, same as y kills x. Its symmetric
                symRef2->getUseDefAliases(includeGCSafePoint).setAlias(_symbolReference, value);
            } else { // foo uses bar, bar kills foo
                symRef2->getUseDefAliases(includeGCSafePoint).setAlias(_symbolReference, value);
            }
        }
    }
...
template<AliasSetInterface _AliasSetInterface>
inline void TR_AliasSetInterface<_AliasSetInterface>::removeSymRef1KillsSymRef2Asymmetrically(
    TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint)
{
...
template<AliasSetInterface _AliasSetInterface>
inline void TR_AliasSetInterface<_AliasSetInterface>::addSymRef1KillsSymRef2Asymmetrically(
    TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint)
{
    TR::Compilation* comp = TR::comp();
    TR_BitVector* symRef1_killedAliases = symRef1->getUseDefAliases(includeGCSafePoint).getTRAliases();
...
template<AliasSetInterface _AliasSetInterface>
inline void TR_AliasSetInterface<_AliasSetInterface>::setSymRef1KillsSymRef2Asymmetrically(
    TR::SymbolReference* symRef1, TR::SymbolReference* symRef2, bool includeGCSafePoint, bool value)
{
    TR::Compilation* comp = TR::comp();
...
template<class T> void CountUseDefAliases(T& t, const TR::SparseBitVector& syms)
{
    TR::Compilation* comp = TR::comp();
...

    {
        ...
        TR_BitVector* tr_syms
            = new (comp->trStackMemory()) TR_BitVector(syms.PopulationCount(), comp->trMemory(), stackAlloc, growable);
        *tr_syms = syms;
 ...
 */
inline TR_UseOnlyAliasSetInterface OMR::SymbolReference::getUseonlyAliases()
{
    TR_UseOnlyAliasSetInterface aliasSetInterface(self());
    return aliasSetInterface;
}

inline TR_UseDefAliasSetInterface OMR::SymbolReference::getUseDefAliases(bool isDirectCall, bool includeGCSafePoint)
{
    TR_UseDefAliasSetInterface aliasSetInterface(self(), isDirectCall, includeGCSafePoint);
    return aliasSetInterface;
}

inline TR_UseDefAliasSetInterface OMR::SymbolReference::getUseDefAliasesIncludingGCSafePoint(bool isDirectCall)
{
