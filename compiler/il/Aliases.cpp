...
TR_BitVector* OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall, bool includeGCSafePoint)
{
    ...
    if (!self()->reallySharesSymbol(comp)) {
        ...
        case TR::Symbol::IsStatic: {
            ...
            if ((self()->isUnresolved() && (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef()))
                || _symbol->isVolatile() || self()->isLiteralPoolAddress() || self()->isFromLiteralPool()
    ...
    case TR::Symbol::IsShadow: {
        if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) || _symbol->isVolatile()
...
#ifdef J9_PROJECT_SPECIFIC
TR_BitVector* addVeryRefinedCallAliasSets(
    TR::ResolvedMethodSymbol* methodSymbol, TR_BitVector* aliases, List<void>* methodsPeeked)
{
    TR::Compilation* comp = TR::comp();
        ...
        if (node->getOpCode().isStore()) {
            ...
            if (symInCallee->isShadow()) {
                if (symInCallee->isArrayShadowSymbol())
                    ...
                    symRefInCaller = symRefTab->findShadowSymbol(
                        symRefInCallee->getOwningMethod(comp), symRefInCallee->getCPIndex(), type);
                ...
                symRefInCaller = symRefTab->findStaticSymbol(
                    symRefInCallee->getOwningMethod(comp), symRefInCallee->getCPIndex(), type);
                if (symRefInCaller) {
                    if (symRefInCaller->reallySharesSymbol(comp))
