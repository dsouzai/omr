...
bool OMR::Power::Linkage::hasToBeOnStack(TR::ParameterSymbol* parm) { return (false); }

TR::MemoryReference* OMR::Power::Linkage::getOutgoingArgumentMemRef(int32_t argSize, TR::Register* argReg,
    TR::InstOpCode::Mnemonic opCode, TR::PPCMemoryArgument& memArg, uint32_t length)
{
    ...
    TR::MemoryReference* result = TR::MemoryReference::createWithDisplacement(self()->cg(),
        machine->getRealRegister(properties.getNormalStackPointerRegister()), argSize + self()->getOffsetToFirstParm(),
        length);
...
TR::Instruction* OMR::Power::Linkage::saveArguments(
    TR::Instruction* cursor, bool fsd, bool saveOnly, List<TR::ParameterSymbol>& parmList)
{
...
#define REGNUM(ri) ((TR::RealRegister::RegNum)(ri))
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (lri >= 0) {
            ...
            bool twoRegs
                = (self()->comp()->target().is32Bit() && type.isInt64() && lri < properties.getNumIntArgRegs() - 1);
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
                ...
                if (regNum != ai) // Equal assignment: do nothing
                {
                    if (freeScratchable.isSet(ai)) {
                        cursor = generateTrg1Src1Instruction(self()->cg(),
                            (type.isFloatingPoint()) ? TR::InstOpCode::fmr : TR::InstOpCode::mr, firstNode,
                            REAL_REGISTER(REGNUM(ai)), REAL_REGISTER(regNum), cursor);
                ...
                if (self()->comp()->target().is32Bit() && type.isInt64()) {
                    ...
                    if (!twoRegs) // Low part needs to come from memory
                    {
                        ...
                        if (freeScratchable.isSet(aiLow)) {
                            cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lwz, firstNode,
                                REAL_REGISTER(REGNUM(aiLow)),
                    ...
                    {
                        if (freeScratchable.isSet(aiLow)) {
                            cursor = generateTrg1Src1Instruction(self()->cg(), TR::InstOpCode::mr, firstNode,
                                REAL_REGISTER(REGNUM(aiLow)), REAL_REGISTER(REGNUM(regNum + 1)), cursor);
                            freeScratchable.reset(aiLow);
                            freeScratchable.set(regNum + 1);
                        } else {
        ...
        else if (ai >= 0 && (!fsd || !saveOnly)) // lri<0: arg needs to come from memory
        {
            ...
            case TR::Address:
                if (freeScratchable.isSet(ai)) {
                    cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::Op_load, firstNode,
                        REAL_REGISTER(REGNUM(ai)),
                        TR::MemoryReference::createWithDisplacement(
                            self()->cg(), stackPtr, offset, TR::Compiler->om.sizeofReferenceAddress()),
                        cursor);
                    freeScratchable.reset(ai);
                } else {
                    busyMoves[0][busyIndex] = offset;
            ...
            case TR::Int64:
                if (self()->comp()->target().is64Bit()) {
                    if (freeScratchable.isSet(ai)) {
                        cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::ld, firstNode,
                            REAL_REGISTER(REGNUM(ai)),
            ...
            case TR::Float:
                if (freeScratchable.isSet(ai)) {
                    cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lfs, firstNode,
                        REAL_REGISTER(REGNUM(ai)),
                        TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, 4), cursor);
                    freeScratchable.reset(ai);
                } else {
            ...
            case TR::Double:
                if (freeScratchable.isSet(ai)) {
                    cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lfd, firstNode,
                        REAL_REGISTER(REGNUM(ai)),
                        TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, 8), cursor);
                    freeScratchable.reset(ai);
                } else {
        ...
        while (freeMore && numMoves > 0) {
            ...
            for (i1 = 0; i1 < busyIndex; i1++) {
                ...
                if (!(target < 0) && freeScratchable.isSet(target)) {
                    ...
                    case 0:
                        cursor = generateTrg1Src1Instruction(self()->cg(),
                            (source <= TR::RealRegister::LastGPR) ? TR::InstOpCode::mr : TR::InstOpCode::fmr, firstNode,
                            REAL_REGISTER(REGNUM(target)), REAL_REGISTER(REGNUM(source)), cursor);
                    ...
                    if (busyMoves[2][i1] != 0) {
                        cursor = generateTrg1MemInstruction(self()->cg(), op, firstNode, REAL_REGISTER(REGNUM(target)),
...
TR::Instruction* OMR::Power::Linkage::loadUpArguments(TR::Instruction* cursor)
{
    ...
    while ((paramCursor != NULL)
        ...
        case TR::Float:
        case TR::Double:
            if (hasToLoadFromStack && numFloatArgs < properties.getNumFloatArgRegs()) {
...
TR::Instruction* OMR::Power::Linkage::flushArguments(TR::Instruction* cursor)
{
    ...
    while ((paramCursor != NULL)
        ...
        case TR::Int16:
        case TR::Int32:
            if (hasToStoreToStack && numIntArgs < properties.getNumIntArgRegs()) {
                argRegister = machine->getRealRegister(properties.getIntegerArgumentRegister(numIntArgs));
                cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::stw, firstNode,
                    TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, 4), argRegister,
                    cursor);
            }
            numIntArgs++;
            break;
        ...
        case TR::Int64:
            if (hasToStoreToStack && numIntArgs < properties.getNumIntArgRegs()) {
                ...
                if (self()->comp()->target().is64Bit())
                    cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::Op_st, firstNode,
                        TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, 8), argRegister,
                        cursor);
                else {
                    cursor = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::stw, firstNode,
                        TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, 4), argRegister,
        ...
        case TR::Float:
        case TR::Double:
            if (hasToStoreToStack && numFloatArgs < properties.getNumFloatArgRegs()) {
                argRegister = machine->getRealRegister(properties.getFloatArgumentRegister(numFloatArgs));
                ...
                cursor = generateMemSrc1Instruction(self()->cg(), op, firstNode,
                    TR::MemoryReference::createWithDisplacement(self()->cg(), stackPtr, offset, length), argRegister,
                    cursor);
            }
            numFloatArgs++;
            break;
...
int32_t OMR::Power::Linkage::numArgumentRegisters(TR_RegisterKinds kind)
{
    switch (kind) {
    case TR_GPR:
