...
TR::Register* OMR::Power::TreeEvaluator::arraysetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    const uint32_t fillNodeSize
        = fillNode->getOpCode().isRef() ? TR::Compiler->om.sizeofReferenceField() : fillNode->getSize();
    ...
    if (constFill) {
        ...
        case 1: {
            ...
            if (dofastPath && cg->comp()->target().is64Bit()) {
                ...
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldimi, node, fillReg, fillReg, 32, 0xffffffff00000000ULL);
            } else {
        ...
        case 4: {
            uint64_t word
                = (uint64_t)(fillNode->getOpCodeValue() == TR::fconst ? fillNode->getFloatBits() & 0xffffffff
                                                                      : fillNode->getUnsignedInt() & 0xffffffff);
            if (dofastPath) {
    ...
    } else // variable fill value
    {
        ...
        case 8: {
            if (fillNode->getDataType() == TR::Double) {
                ...
                TR::MemoryReference* tempMRStore1
                    = TR::MemoryReference::createWithSymRef(cg, node, location->getSymbolReference(), 4);
                TR::MemoryReference* tempMRStore2
                    = TR::MemoryReference::createWithMemRef(cg, node, *tempMRStore1, 4, 4);
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, fp1Reg,
                    TR::MemoryReference::createWithSymRef(cg, node, location->getSymbolReference(), 8));
    ...
    if (dofastPath) {
        ...
        generateTrg1Src1ImmInstruction(cg,
            lenNode->getOpCode().isInt() ? TR::InstOpCode::cmpli4 : TR::InstOpCode::cmpli8, node, condReg, lenReg, 0);
        ...
        if (1 == fillNodeSize) {
            ...
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stb, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0, 1), fillReg);
        ...
        if (1 == fillNodeSize || 2 == fillNodeSize) {
            generateTrg1Src1ImmInstruction(cg,
                lenNode->getOpCode().isInt() ? TR::InstOpCode::cmpli4 : TR::InstOpCode::cmpli8, node, condReg, lenReg,
                2);
            ...
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::sth, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0, 2), fillReg);
        ...
        if (1 == fillNodeSize || 2 == fillNodeSize || 4 == fillNodeSize) {
            ...
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stw, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0, 4), fillReg);
        ...
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 8, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 16, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 24, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 32, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 40, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 48, 8), dblFillReg);
        generateMemSrc1Instruction(
            cg, dblStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 56, 8), dblFillReg);
    ...
    } else // regular word store path
    {
        ...
        generateLabelInstruction(cg, TR::InstOpCode::label, node, label3); // L3
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 4, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 8, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 12, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 16, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 20, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 24, 4), fillReg);
        generateMemSrc1Instruction(
            cg, wdStoreOp, node, TR::MemoryReference::createWithDisplacement(cg, addrReg, 28, 4), fillReg);
