...
TR::Instruction* generateMvFprGprInstructions(TR::CodeGenerator* cg, TR::Node* node, MvFprGprMode mode, bool nonops,
    TR::Register* reg0, TR::Register* reg1, TR::Register* reg2, TR::Register* reg3, TR::Instruction* cursor)
{
    ...
    bool checkp8DirectMove = cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8) && !disableDirectMove
    ...
    TR_ASSERT(
        !(checkp8DirectMove && (reg3 == NULL || reg2 == NULL) && (mode == gpr2fprHost32 || mode == fpr2gprHost32)),
...
TR::Instruction* generateAlignmentNopInstruction(
    TR::CodeGenerator* cg, TR::Node* n, uint32_t alignment, TR::Instruction* preced)
{
...
TR::Instruction* generateImmInstruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t imm, TR::Instruction* preced)
{
    if (preced)
...
TR::Instruction* generateImm2Instruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t imm,
    uint32_t imm2, TR::Instruction* preced)
{
    if (preced)
...
TR::Instruction* generateTrg1MemInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, int32_t hint,
    TR::Node* n, TR::Register* treg, TR::MemoryReference* mf, TR::Instruction* preced)
{
    if (preced)
...
TR::Instruction* generateConditionalBranchInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op,
    bool likeliness, TR::Node* n, TR::LabelSymbol* sym, TR::Register* cr, TR::Instruction* preced)
{
    ...
    TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::PPCDepConditionalBranchInstruction(op, n, sym, cr, cond, preced, cg, likeliness);
...
TR::Instruction* generateTrg1Src1Imm2Instruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* trgReg, TR::Register* srcReg, TR::Register* cr0reg, int32_t imm1, int64_t imm2,
    TR::Instruction* preced)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::PPCTrg1Src1Imm2Instruction(op, n, trgReg, srcReg, cr0reg, imm1, imm2, preced, cg);
...
TR::Instruction* generateShiftLeftImmediate(TR::CodeGenerator* cg, TR::Node* n, TR::Register* trgReg,
    TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
    int32_t temp = ((int32_t)0x80000000) >> (31 - shiftAmount);
    if (shiftAmount == 1) {
        if (preced)
            return new (cg->trHeapMemory())
                TR::PPCTrg1Src2Instruction(TR::InstOpCode::add, n, trgReg, srcReg, srcReg, preced, cg);
        return new (cg->trHeapMemory()) TR::PPCTrg1Src2Instruction(TR::InstOpCode::add, n, trgReg, srcReg, srcReg, cg);
    }
    if (preced)
        return new (cg->trHeapMemory()) TR::PPCTrg1Src1Imm2Instruction(
            TR::InstOpCode::rlwinm, n, trgReg, srcReg, shiftAmount % 32, temp, preced, cg);
    return new (cg->trHeapMemory())
        TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rlwinm, n, trgReg, srcReg, shiftAmount % 32, temp, cg);
}
    ...
    TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rlwinm, n, trgReg, srcReg, temp1 % 32, temp2, preced, cg);
    return new (cg->trHeapMemory())
        TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rlwinm, n, trgReg, srcReg, temp1 % 32, temp2, cg);
}
    ...
    TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory()) TR::PPCTrg1Src1Imm2Instruction(
            TR::InstOpCode::rldicr, n, trgReg, srcReg, shiftAmount % 64, mask, preced, cg);
    return new (cg->trHeapMemory())
        TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rldicr, n, trgReg, srcReg, shiftAmount % 64, mask, cg);
}
    ...
    TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory())
            TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rldicl, n, trgReg, srcReg, temp1 % 64, mask, preced, cg);
    return new (cg->trHeapMemory())
        TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rldicl, n, trgReg, srcReg, temp1 % 64, mask, cg);
}
    ...
    TR::RegisterDependencyConditions* deps, TR::Instruction* preced, bool useRegPairForResult, bool useRegPairForCond)
{
    if (preced)
        return new (cg->trHeapMemory())
            TR::PPCControlFlowInstruction(op, n, preced, cg, deps, useRegPairForResult, useRegPairForCond);
    return new (cg->trHeapMemory())
        TR::PPCControlFlowInstruction(op, n, cg, deps, useRegPairForResult, useRegPairForCond);
}
...
TR::Instruction* generateTrg1Instruction(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* trg, TR::Instruction* preced)
{
    if (preced)
    ...
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t bitsInTarget, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src1Imm2Instruction(cg,
        cg->comp()->target().is64Bit() ? TR::InstOpCode::rldicl : TR::InstOpCode::rlwinm, node, trgReg, srcReg, 0,
        mask);
}
