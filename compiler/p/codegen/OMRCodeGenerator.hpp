...
extern void loadFloatConstant(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic loadOp, TR::Node* node, TR::DataType type,
    void* value, TR::Register* trgReg);

extern TR::Instruction* loadAddressConstantInSnippet(TR::CodeGenerator* cg, TR::Node* node, intptr_t address,
    TR::Register* targetRegister, TR::Register* tempRegister, TR::InstOpCode::Mnemonic opCode,
    bool isUnloadablePicSite = false, TR::Instruction* cursor = NULL);

extern TR::Instruction* loadAddressConstant(TR::CodeGenerator* cg, TR::Node* node, intptr_t value,
    TR::Register* targetRegister, TR::Instruction* cursor = NULL, bool isPicSite = false, int16_t typeAddress = -1);

extern TR::Instruction* loadAddressConstant(TR::CodeGenerator* cg, bool isRelocatable, TR::Node* node, intptr_t value,
    TR::Register* targetRegister, TR::Instruction* cursor = NULL, bool isPicSite = false, int16_t typeAddress = -1);

extern TR::Instruction* loadActualConstant(TR::CodeGenerator* cg, TR::Node* node, intptr_t value,
    TR::Register* targetRegister, TR::Instruction* cursor = NULL, bool isPicSite = false);

extern TR::Instruction* loadConstant(TR::CodeGenerator* cg, TR::Node* node, int32_t value, TR::Register* targetRegister,
    TR::Instruction* cursor = NULL, bool isPicSite = false);

extern TR::Instruction* loadConstant(TR::CodeGenerator* cg, TR::Node* node, int64_t value, TR::Register* targetRegister,
    TR::Instruction* cursor = NULL, bool isPicSite = false, bool useTOC = true);

extern TR::Instruction* fixedSeqMemAccess(TR::CodeGenerator* cg, TR::Node* node, intptr_t addr,
    TR::Instruction** nibbles, TR::Register* srcOrTrg, TR::Register* baseReg, TR::InstOpCode::Mnemonic opCode,
    int32_t opSize, TR::Instruction* cursor = NULL, TR::Register* tempReg = NULL);

extern uint8_t* storeArgumentItem(
    TR::InstOpCode::Mnemonic op, uint8_t* buffer, TR::RealRegister* reg, int32_t offset, TR::CodeGenerator* cg);

extern uint8_t* loadArgumentItem(
    TR::InstOpCode::Mnemonic op, uint8_t* buffer, TR::RealRegister* reg, int32_t offset, TR::CodeGenerator* cg);
...
public:
    ...
    void findOrCreateFloatConstant(
        void* v, TR::DataType t, TR::Instruction* n0, TR::Instruction* n1, TR::Instruction* n2, TR::Instruction* n3);
    void findOrCreateAddressConstant(void* v, TR::DataType t, TR::Instruction* n0, TR::Instruction* n1,
        TR::Instruction* n2, TR::Instruction* n3, TR::Node* node, bool isUnloadablePicSite);
    ...
    virtual bool getSupportsTLE() { return false; }
    ...
    TR::SymbolReference& getDouble2LongSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCdouble2Long);
    }
    TR::SymbolReference& getDoubleRemainderSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCdoubleRemainder);
    }
    TR::SymbolReference& getInteger2DoubleSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinteger2Double);
    }
    TR::SymbolReference& getLong2DoubleSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPClong2Double);
    }
    TR::SymbolReference& getLong2FloatSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPClong2Float);
    }
    TR::SymbolReference& getLong2Float_mvSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPClong2Float_mv);
    }
    TR::SymbolReference& getLongMultiplySymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPClongMultiply);
    }
    TR::SymbolReference& getLongDivideSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPClongDivide);
    }
    TR::SymbolReference& getUnresolvedStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedStaticGlue);
    }
    TR::SymbolReference& getUnresolvedSpecialGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedSpecialGlue);
    }
    TR::SymbolReference& getUnresolvedDirectVirtualGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedDirectVirtualGlue);
    }
    TR::SymbolReference& getUnresolvedClassGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedClassGlue);
    }
    TR::SymbolReference& getUnresolvedClassGlue2SymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedClassGlue2);
    }
    TR::SymbolReference& getUnresolvedStringGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedStringGlue);
    }
    TR::SymbolReference& getUnresolvedStaticDataGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedStaticDataGlue);
    }
    TR::SymbolReference& getUnresolvedStaticDataStoreGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedStaticDataStoreGlue);
    }
    TR::SymbolReference& getUnresolvedInstanceDataGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedInstanceDataGlue);
    }
    TR::SymbolReference& getUnresolvedInstanceDataStoreGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedInstanceDataStoreGlue);
    }
    TR::SymbolReference& getVirtualUnresolvedHelperSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCvirtualUnresolvedHelper);
    }
    TR::SymbolReference& getInterfaceCallHelperSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterfaceCallHelper);
    }
    TR::SymbolReference& getItrgSendVirtual0SymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual0);
    }
    TR::SymbolReference& getItrgSendVirtual1SymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual1);
    }
    TR::SymbolReference& getItrgSendVirtualJSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualJ);
    }
    TR::SymbolReference& getItrgSendVirtualFSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualF);
    }
    TR::SymbolReference& getItrgSendVirtualDSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualD);
    }
    TR::SymbolReference& getVoidStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterVoidStaticGlue);
    }
    TR::SymbolReference& getSyncVoidStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterSyncVoidStaticGlue);
    }
    TR::SymbolReference& getGPR3StaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterGPR3StaticGlue);
    }
    TR::SymbolReference& getSyncGPR3StaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterSyncGPR3StaticGlue);
    }
    TR::SymbolReference& getGPR3GPR4StaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterGPR3GPR4StaticGlue);
    }
    TR::SymbolReference& getSyncGPR3GPR4StaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterSyncGPR3GPR4StaticGlue);
    }
    TR::SymbolReference& getFPR0FStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterFPR0FStaticGlue);
    }
    TR::SymbolReference& getSyncFPR0FStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterSyncFPR0FStaticGlue);
    }
    TR::SymbolReference& getFPR0DStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterFPR0DStaticGlue);
    }
    TR::SymbolReference& getSyncFPR0DStaticGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinterpreterSyncFPR0DStaticGlue);
    }
    TR::SymbolReference& getNativeStaticHelperForUnresolvedGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCnativeStaticHelperForUnresolvedGlue);
    }
    TR::SymbolReference& getNativeStaticHelperSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCnativeStaticHelper);
    }
    TR::SymbolReference& getCollapseJNIReferenceFrameSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCcollapseJNIReferenceFrame);
    }
    ...
    TR::SymbolReference& getReferenceArrayCopySymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCreferenceArrayCopy);
    }
    TR::SymbolReference& getGeneralArrayCopySymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCgeneralArrayCopy);
    }
    TR::SymbolReference& getSamplingPatchCallSiteSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCsamplingPatchCallSite);
    }
    TR::SymbolReference& getSamplingRecompileMethodSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCsamplingRecompileMethod);
    }
    TR::SymbolReference& getCountingPatchCallSiteSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCcountingPatchCallSite);
    }
    TR::SymbolReference& getCountingRecompileMethodSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCcountingRecompileMethod);
    }
    TR::SymbolReference& getInduceRecompilationSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCinduceRecompilation);
    }
    TR::SymbolReference& getRevertToInterpreterGlueSymbolReference()
    {
        return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCrevertToInterpreterGlue);
    }
    ...
    TR_GlobalRegisterNumber _gprLinkageGlobalRegisterNumbers[TR::RealRegister::NumRegisters],
        _fprLinkageGlobalRegisterNumbers[TR::RealRegister::NumRegisters]; // these could be smaller
    TR_GlobalRegisterNumber getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex, TR::DataType type);

    virtual void simulateNodeEvaluation(
    ...
    {
        return (value >= LOWER_IMMED) && (value <= UPPER_IMMED);
    }
    ...
    TR::Instruction* generateDebugCounterBump(
    ...
    TR::Instruction* loadAddressConstantFixed(TR::Node* node, intptr_t value, TR::Register* targetRegister,
        TR::Instruction* cursor = NULL, TR::Register* tempReg = NULL, int16_t typeAddress = -1,
        bool doAOTRelocation = true);

    TR::Instruction* loadIntConstantFixed(TR::Node* node, int32_t value, TR::Register* targetRegister,
        TR::Instruction* cursor = NULL, int16_t typeAddress = -1);

    TR::Instruction* fixedLoadLabelAddressIntoReg(TR::Node* node, TR::Register* trgReg, TR::LabelSymbol* label,
        TR::Instruction* cursor = NULL, TR::Register* tempReg = NULL, bool useADDISFor32Bit = false);

    void addMetaDataForLoadIntConstantFixed(TR::Node* node, TR::Instruction* firstInstruction,
...
public:
    TR_PPCScratchRegisterManager(int32_t capacity, TR::CodeGenerator* cg)
...
void mulConstant(TR::Node*, TR::Register* trgReg, TR::Register* sourceReg, int32_t value, TR::CodeGenerator* cg);

void mulConstant(TR::Node*, TR::Register* trgReg, TR::Register* sourceReg, int64_t value, TR::CodeGenerator* cg);

TR::Register* addConstantToLong(
    TR::Node* node, TR::Register* srcReg, int64_t value, TR::Register* trgReg, TR::CodeGenerator* cg);

TR::Register* addConstantToLong(TR::Node* node, TR::Register* srcHigh, TR::Register* srcLow, int32_t valHigh,
    int32_t valLow, TR::CodeGenerator* cg);
