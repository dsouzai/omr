...
#if defined(JITTEST)
    // Store at the index of TR_PPCnumRuntimeHelpers, the systemTOC. Note: Slot tocBase[TR_PPCnumRuntimeHelpers-1] is
    // free. See TR::TreeEvaluator::restoreTOCRegister.
    tocBase[TR_PPCnumRuntimeHelpers] = systemTOC;
    // Set Down Cursor to be TR_PPCnumRuntimeHelpers+1, so nothing between [0..TR_PPCnumRuntimeHelpers] should be used
    // for hashtable.
...
void TR_PPCTableOfConstants::reinitializeMemory()
{
    ...
    memset(tocManagement->getTOCPtr(), 0,
        (tocManagement->getTOCSize() >> 1)
            - (tocManagement->getUpCursorAfterPermanentEntries() + 1) * sizeof(uintptr_t));
...
void TR_PPCTableOfConstants::shutdown(TR_FrontEnd* fe) { TR_ASSERT(0, "shutdown only enabled for zEmulator.\n"); }
...
int32_t TR_PPCTableOfConstants::lookUp(
    int32_t val, struct TR_tocHashEntry* tmplate, int32_t* offsetInSlot, TR::CodeGenerator* cg)
{
    ...
    if (comp->isOptServer()) {
        if (comp->getMethodHotness() < warm || comp->isDLT() || comp->isProfilingCompilation()
            || cg->getCurrentBlock()->isCold())
            return PTOC_FULL_INDEX;
    }
    ...
    *offsetInSlot = 0;
    ...
    while (chain != CHAIN_END) {
        ...
        case TR_FLAG_tocStatic2ClassKey:
            if (hash[idx]._keyTag == tmplate->_keyTag && hash[idx]._cpKey == tmplate->_cpKey
    ...
    case TR_FLAG_tocFloatKey:
        ...
        if (comp->target().cpu.isBigEndian())
            slotValue = (*offsetInSlot == 0) ? (((uintptr_t)hash[idx]._fKey) << 32)
                                             : (getTOCSlot(rval * sizeof(intptr_t)) | hash[idx]._fKey);
        else
            slotValue = (*offsetInSlot == 0)
                ? ((uintptr_t)hash[idx]._fKey)
                : (getTOCSlot(rval * sizeof(intptr_t)) | (((uintptr_t)hash[idx]._fKey) << 32));
...
int32_t TR_PPCTableOfConstants::lookUp(TR::SymbolReference* symRef, TR::CodeGenerator* cg)
{
    ...
    if (!symRef->isUnresolved() || symRef->getCPIndex() < 0 || sym->isAddressOfClassObject() || sym->isConstObjectRef()
        || sym->isConst()) {
        ...
        *(intptr_t*)local_buffer = addr;
        ...
        TR_OpaqueClassBlock* myClass
            = comp->getOwningMethodSymbol(symRef->getOwningMethodIndex())->getResolvedMethod()->containingClass();
        ...
        if (sym->isClassObject()) {
            ...
            name = (int8_t*)comp->getOwningMethodSymbol(symRef->getOwningMethodIndex())
                       ->getResolvedMethod()
                       ->staticName(symRef->getCPIndex(), cg->trMemory());
...
void TR_PPCTableOfConstants::onClassUnloading(void* tagPtr)
{
    TR::PersistentInfo* pinfo = TR_PersistentMemory::getNonThreadSafePersistentInfo();
    ...
    for (idx = 0; idx < endIx; idx++) {
        if ((hash[idx]._flag & (TR_FLAG_tocNameKey | TR_FLAG_tocStatic2ClassKey))
            && hash[idx]._keyTag == (intptr_t)tagPtr) {
            hash[idx]._keyTag = -1;
