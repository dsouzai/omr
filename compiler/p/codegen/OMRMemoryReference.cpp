...
TR::MemoryReference* TR::MemoryReference::createWithLabel(
    TR::CodeGenerator* cg, TR::LabelSymbol* label, int64_t offset, int8_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(label, offset, length, cg);
}

TR::MemoryReference* TR::MemoryReference::createWithIndexReg(
    TR::CodeGenerator* cg, TR::Register* baseReg, TR::Register* indexReg, uint8_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(baseReg, indexReg, length, cg);
}

TR::MemoryReference* TR::MemoryReference::createWithDisplacement(
    TR::CodeGenerator* cg, TR::Register* baseReg, int64_t displacement, int8_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(baseReg, displacement, length, cg);
}

TR::MemoryReference* TR::MemoryReference::createWithRootLoadOrStore(
    TR::CodeGenerator* cg, TR::Node* rootLoadOrStore, uint32_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(rootLoadOrStore, length, cg);
}

TR::MemoryReference* TR::MemoryReference::createWithSymRef(
    TR::CodeGenerator* cg, TR::Node* node, TR::SymbolReference* symRef, uint32_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(node, symRef, length, cg);
}

TR::MemoryReference* TR::MemoryReference::createWithMemRef(
    TR::CodeGenerator* cg, TR::Node* node, TR::MemoryReference& memRef, int32_t displacement, uint32_t length)
{
...
OMR::Power::MemoryReference::MemoryReference(
    TR::Node* node, TR::MemoryReference& mr, int32_t displacement, uint32_t len, TR::CodeGenerator* cg)
{
    ...
    if (mr.getUnresolvedSnippet() != NULL) {
        _unresolvedSnippet = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
            cg, node, _symbolReference, mr.getUnresolvedSnippet()->isUnresolvedStore(), false);
            ...
            if (node->getOpCode().isLoadConst() && node->getRegister()) {
                ...
                if (cg->comp()->target().is64Bit() && (upper < LOWER_IMMED || upper > UPPER_IMMED)) {
                    ...
                    generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addis, node, newBase, _baseRegister, (int16_t)upper);
...
void OMR::Power::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (subTree->getOpCodeValue() == TR::i2l && subTree->isNonNegative()
        ...
        if (performTransformation(
                cg->comp(), "O^O PPC Evaluator: Skip unecessary sign extension on index [%p].\n", subTree)) {
    ...
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCode().isArrayRef()
            ...
            if (cg->comp()->target().is64Bit()) { // 64-bit
                ...
                self()->addToOffset(
                    subTree, subTree->getFirstChild()->getInt() << subTree->getSecondChild()->getInt(), cg);
...
void OMR::Power::MemoryReference::consolidateRegisters(
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
        ...
        if (_indexRegister != NULL) {
            ...
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::add, tempNode, tempTargetRegister, _baseRegister, _indexRegister);
    ...
    if (_baseRegister != NULL) {
        ...
        if ((assignedBaseRegister != NULL)
            ...
            machine->coerceRegisterAssignment(
                currentInstruction, _baseRegister, toRealRegister(alternativeRegister)->getRegisterNumber());
...
TR::Instruction* OMR::Power::MemoryReference::expandForUnresolvedSnippet(
    TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* firstInstruction = generateLabelInstruction(
        cg, TR::InstOpCode::bl, node, getUnresolvedSnippet()->getSnippetLabel(), prevInstruction);
    ...
    if (comp->target().is64Bit() && self()->isTOCAccess()) {
        ...
        if (displacement == PTOC_FULL_INDEX) {
            ...
            prevInstruction = generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicr, node, data, data, 32, 0xffffffff00000000ULL, prevInstruction);
            prevInstruction
            ...
            newMR = TR::MemoryReference::createWithDisplacement(
                cg, self()->getModBase(), LO_VALUE(displacement), self()->getLength());
        }
    } else if (index != NULL || isUsingDelayedIndexedForm()) {
        if (index == NULL) {
            newMR = TR::MemoryReference::createWithIndexReg(
                cg, cg->machine()->getRealRegister(TR::RealRegister::gr0), base, self()->getLength());
        } else {
    ...
    if (newMR != NULL) {
        ...
        case TR::Instruction::IsMem:
            prevInstruction = generateMemInstruction(
                cg, currentInstruction->getOpCodeValue(), currentInstruction->getNode(), newMR, prevInstruction);
            break;
        ...
        case TR::Instruction::IsTrg1Mem:
            prevInstruction
                = generateTrg1MemInstruction(cg, currentInstruction->getOpCodeValue(), currentInstruction->getNode(),
...
TR::Instruction* OMR::Power::MemoryReference::expandInstruction(
    TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    if (!self()->getBaseRegister() && !self()->getLabel()) {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            currentInstruction, !self()->getModBase(), "Cannot have mod base and base register");
    }
    ...
    if (comp->target().is64Bit() && self()->isTOCAccess()) {
        ...
        if (displacement != PTOC_FULL_INDEX) {
            ...
            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, node, data, base, cg->hiValue(displacement), prevInstruction);
            ...
            if (cg->comp()->compileRelocatableCode() && symbol->isStatic() && symbol->isClassObject()) {
                prevInstruction = generateLabelInstruction(
                    cg, TR::InstOpCode::label, node, generateLabelSymbol(cg), prevInstruction);
            ...
            TR::Instruction* newInstruction = prevInstruction = generateTrg1ImmInstruction(
                cg, TR::InstOpCode::lis, node, data, (int16_t)(addr >> 48), prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::ori, node, data, data, (addr >> 32) & 0xffff, prevInstruction);
            prevInstruction = generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicr, node, data, data, 32, 0xffffffff00000000ULL, prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::oris, node, data, data, (addr >> 16) & 0xffff, prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::ori, node, data, data, addr & 0xffff, prevInstruction);
        ...
        if (index != NULL) {
            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                prevInstruction = generateTrg1ImmInstruction(
                    cg, TR::InstOpCode::lis, node, index, (int16_t)(displacement >> 16), prevInstruction);
                prevInstruction = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::ori, node, index, index, displacement & 0xffff, prevInstruction);
            ...
            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                prevInstruction = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, node, base, base, cg->hiValue(displacement), prevInstruction);
                displacement = LO_VALUE(displacement);
            }

            if (displacement != 0)
                prevInstruction = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, base, base, displacement, prevInstruction);
        ...
        if (self()->isBaseModifiable()) {
            ...
            prevInstruction = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addis, node, rX, base, cg->hiValue(displacement), prevInstruction);
...
extern "C" void voom() { }
void OMR::Power::MemoryReference::accessStaticItem(
    TR::Node* node, TR::SymbolReference* ref, bool isStore, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::Symbol* symbol;
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (symbol->isCountForRecompile() && cg->needRelocationsForPersistentInfoData()) {
            ...
            TR_ASSERT_FATAL(
                !comp->getOption(TR_UseSymbolValidationManager) || ref->isUnresolved(), "SVM relocation unhandled");
        }
        ...
        if ((refIsUnresolved || comp->compileRelocatableCode()) && symbol->isDebugCounter()) {
            TR::Register* reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(
                cg, comp->compileRelocatableCode(), nodeForSymbol, 1, reg, NULL, false, TR_DebugCounter);
            return;
            ...
            TR::Register* reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(
                cg, cg->needRelocationsForBodyInfoData(), nodeForSymbol, 0, reg, NULL, false, TR_BodyInfoAddressLoad);
            return;
            ...
            TR::Register* reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(
                cg, cg->needRelocationsForCurrentMethodPC(), nodeForSymbol, 0, reg, NULL, false, TR_RamMethodSequence);
            return;
        ...
        if (cg->needClassAndMethodPointerRelocations()) {
            TR_ASSERT_FATAL(
                symbol->isClassObject(), "expected a class symbol for symref #%d\n", ref->getReferenceNumber());
...
void OMR::Power::MemoryReference::setOffsetRequiresWordAlignment(
    TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* cursor)
{
