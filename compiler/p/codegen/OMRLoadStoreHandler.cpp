...
void OMR::Power::LoadStoreHandler::generateComputeAddressSequence(
    TR::CodeGenerator* cg, TR::Register* addrReg, TR::Node* node, int64_t extraOffset)
{
...
TR::MemoryReference* createAddressMemoryReference(
    TR::CodeGenerator* cg, TR::Register* addrReg, uint32_t length, bool requireIndexForm)
{
    if (requireIndexForm)
    ...
    TR::Node* node, TR::Register* addrReg, TR::InstOpCode::Mnemonic loadOp, uint32_t length, bool requireIndexForm)
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isLoadVar(), "Attempt to use generateLoadAddressSequence for non-load node");
    TR::LoadStoreHandlerImpl::generateLoadSequence(
        cg, trgReg, node, createAddressMemoryReference(cg, addrReg, length, requireIndexForm), loadOp);
}

void OMR::Power::LoadStoreHandler::generatePairedLoadAddressSequence(
    TR::CodeGenerator* cg, TR::Register* trgReg, TR::Node* node, TR::Register* addrReg)
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isLoadVar(), "Attempt to use generatePairedLoadAddressSequence for non-load node");
    TR::LoadStoreHandlerImpl::generatePairedLoadSequence(
        cg, trgReg, node, createAddressMemoryReference(cg, addrReg, 8, false));
}
...
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isStore(), "Attempt to use generateStoreAddressSequence for non-store node");
    TR::LoadStoreHandlerImpl::generateStoreSequence(
        cg, srcReg, node, createAddressMemoryReference(cg, addrReg, length, requireIndexForm), storeOp);
}

void OMR::Power::LoadStoreHandler::generatePairedStoreAddressSequence(
    TR::CodeGenerator* cg, TR::Register* srcReg, TR::Node* node, TR::Register* addrReg)
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isStore(), "Attempt to use generatePairedStoreAddressSequence for non-store node");
    TR::LoadStoreHandlerImpl::generatePairedStoreSequence(
        cg, srcReg, node, createAddressMemoryReference(cg, addrReg, 8, false));
}
...
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isLoadVar(), "Attempt to use generateLoadNodeSequence for non-load node");
...
void OMR::Power::LoadStoreHandler::generatePairedLoadNodeSequence(
    TR::CodeGenerator* cg, TR::Register* trgReg, TR::Node* node)
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isLoadVar(), "Attempt to use generatePairedLoadNodeSequence for non-load node");
...
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isStore(), "Attempt to use generateStoreNodeSequence for non-store node");
    auto ref = TR::LoadStoreHandlerImpl::generateMemoryReference(cg, node, length, requireIndexForm, extraOffset);
...
void OMR::Power::LoadStoreHandler::generatePairedStoreNodeSequence(
    TR::CodeGenerator* cg, TR::Register* srcReg, TR::Node* node)
{
    TR_ASSERT_FATAL_WITH_NODE(
        node, node->getOpCode().isStore(), "Attempt to use generatePairedStoreNodeSequence for non-store node");
    auto ref = TR::LoadStoreHandlerImpl::generateMemoryReference(cg, node, 8, false, 0);
...
OMR::Power::NodeMemoryReference OMR::Power::LoadStoreHandler::generateSimpleLoadMemoryReference(
...
OMR::Power::NodeMemoryReference OMR::Power::LoadStoreHandlerImpl::generateMemoryReference(
    TR::CodeGenerator* cg, TR::Node* node, uint32_t length, bool requireIndexForm, int64_t extraOffset)
{
...
void OMR::Power::LoadStoreHandlerImpl::generatePairedLoadSequence(
...
#ifdef J9_PROJECT_SPECIFIC
enum class StoreSyncRequirements { None, AtomicWrite, LazySet, Full };
...
StoreSyncRequirements getStoreSyncRequirements(TR::CodeGenerator* cg, TR::Node* node)
{
    if (node->getSymbol()->isSyncVolatile()) {
        ...
        if (caller && !cg->comp()->compileRelocatableCode()) {
            TR_ResolvedMethod* m = cg->comp()->fe()->createResolvedMethod(
                cg->trMemory(), caller, node->getSymbolReference()->getOwningMethod(cg->comp()));
...
void OMR::Power::LoadStoreHandlerImpl::generatePairedStoreSequence(
