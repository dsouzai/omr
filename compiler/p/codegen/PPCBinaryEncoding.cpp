...
static void fillFieldRT(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RT field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRTP(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RTp field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, (toRealRegister(reg)->getRegisterNumber() - TR::RealRegister::gr0) % 2 == 0,
        "Attempt to fill RTp field with %s, which is an odd GPR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRT(cursor);
}
...
static void fillFieldFRT(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_FPR,
        "Attempt to fill FRT field with %s, which is not an FPR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRT(cursor);
}
...
static void fillFieldVRT(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_VRF,
        "Attempt to fill VRT field with %s, which is not a VR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRT(cursor);
}
...
static void fillFieldXT(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, canUseAsVsxRegister(reg),
        "Attempt to fill XT field with %s, which is not a VSR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRS(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RS field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRSP(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RSp field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, (toRealRegister(reg)->getRegisterNumber() - TR::RealRegister::gr0) % 2 == 0,
        "Attempt to fill RSp field with %s, which is an odd GPR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRS(cursor);
}
...
static void fillFieldFRS(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_FPR,
        "Attempt to fill FRS field with %s, which is not an FPR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRS(cursor);
}
...
static void fillFieldVRS(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_VRF,
        "Attempt to fill VRS field with %s, which is not a VR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRS(cursor);
}
...
static void fillFieldXS(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, canUseAsVsxRegister(reg),
        "Attempt to fill XS field with %s, which is not a VSR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRA(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RA field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldFRA(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_FPR,
        "Attempt to fill FRA field with %s, which is not an FPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldVRA(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_VRF,
        "Attempt to fill VRA field with %s, which is not a VR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRA(cursor);
}
...
static void fillFieldXA(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, canUseAsVsxRegister(reg),
        "Attempt to fill XA field with %s, which is not a VSR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRB(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RB field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldFRB(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_FPR,
        "Attempt to fill FRB field with %s, which is not an FPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldVRB(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_VRF,
        "Attempt to fill VRB field with %s, which is not a VR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRB(cursor);
}
...
static void fillFieldXB(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, canUseAsVsxRegister(reg),
        "Attempt to fill XB field with %s, which is not a VSR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldRC(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_GPR,
        "Attempt to fill RC field with %s, which is not a GPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldFRC(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_FPR,
        "Attempt to fill FRC field with %s, which is not an FPR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldVRC(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_VRF,
        "Attempt to fill VRC field with %s, which is not a VR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRC(cursor);
}
...
static void fillFieldXC(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, canUseAsVsxRegister(reg),
        "Attempt to fill XC field with %s, which is not a VSR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldBI(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_CCR,
        "Attempt to fill BI field with %s, which is not a CCR", reg->getRegisterName(instr->cg()->comp()));
...
static void fillFieldBF(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_CCR,
        "Attempt to fill BF field with %s, which is not a CCR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRT(cursor);
}
...
static void fillFieldBFA(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_CCR,
        "Attempt to fill BFA field with %s, which is not a CCR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRA(cursor);
}
...
static void fillFieldBFB(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_CCR,
        "Attempt to fill BFB field with %s, which is not a CCR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRB(cursor);
}
...
static void fillFieldBFC(TR::Instruction* instr, uint32_t* cursor, TR::RealRegister* reg)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, reg->getKind() == TR_CCR,
        "Attempt to fill BFC field with %s, which is not a CCR", reg->getRegisterName(instr->cg()->comp()));
    reg->setRegisterFieldRC(cursor);
}
...
static void fillFieldFXM1(TR::Instruction* instr, uint32_t* cursor, uint32_t val)
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, populationCount(val) == 1, "0x%x is invalid for FXM field, expecting exactly 1 bit set", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0x1fu), "0x%x is out-of-range for SIM field", val);
    *cursor |= (val & 0x1f) << 16;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0xffffu), "0x%x is out-of-range for D(16) field", val);
    *cursor |= val & 0xffff;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0xffffu), "0x%x is out-of-range for DS field", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0xffffu), "0x%x is out-of-range for DQ field", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0x3ffffffffull), "0x%llx is out-of-range for D(34) field", val);
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0xffffu), "0x%x is out-of-range for SI(16) field", val);
    *cursor |= val & 0xffff;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, isValidInSignExtendedField(val, 0x1fu), "0x%x is out-of-range for SI(5) field", val);
    *cursor |= (val & 0x1f) << 11;
}
...
static void fillFieldUIM(TR::Instruction* instr, uint32_t* cursor, int32_t numBits, uint32_t val)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, (val & fieldMask) == val, "0x%x is out-of-range for UIM(%d) field", val, numBits);
    *cursor |= val << 16;
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, (val & 0xffu) == val || isValidInSignExtendedField(val, 0xffu),
        "0x%x is out-of-range for IMM8(8) field", val);
    *cursor |= (val & 0xff) << 11;
}
...
void OMR::Power::Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by Instruction", self()->getOpCode().getFormat());
    }
}
...
uint8_t* TR::PPCAlignmentNopInstruction::generateBinaryEncoding()
{
    ...
    if (currentMisalign) {
        ...
        if (trace)
            traceMsg(
                cg()->comp(), "Expanding alignment nop %p into %u instructions: [ %p ", self(), nopsToAdd, lastNop);
        ...
        if (trace)
            traceMsg(
                cg()->comp(), "Eliminating alignment nop %p, since the next instruction is already aligned\n", self());
    }
...
void TR::PPCLabelInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    case FORMAT_I_FORM:
        if (label->getCodeLocation())
            ...
            cg()->addRelocation(
                new (cg()->trHeapMemory()) TR::LabelRelative24BitRelocation(reinterpret_cast<uint8_t*>(cursor), label));
        break;

    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCLabelInstruction", getOpCode().getFormat());
    }
}
...
static bool reversedConditionalBranchOpCode(TR::InstOpCode::Mnemonic op, TR::InstOpCode::Mnemonic* rop)
{
    ...
    default:
        TR_ASSERT(
            0, "New PPC conditional branch opcodes have to have corresponding reversed opcode: %d\n", (int32_t)op);
...
void TR::PPCConditionalBranchInstruction::expandIntoFarBranch()
{
    ...
    TR::Instruction* branchInstr = generateLabelInstruction(
        cg(), wasLinkForm ? TR::InstOpCode::bl : TR::InstOpCode::b, getNode(), getLabelSymbol(), self());
    branchInstr->setEstimatedBinaryLength(4);

    TR::Instruction* labelInstr
        = generateLabelInstruction(cg(), TR::InstOpCode::label, getNode(), skipBranchLabel, branchInstr);
...
void TR::PPCConditionalBranchInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    case FORMAT_B_FORM: {
        ...
        if (label->getCodeLocation())
            ...
            cg()->addRelocation(
                new (cg()->trHeapMemory()) TR::LabelRelative16BitRelocation(reinterpret_cast<uint8_t*>(cursor), label));
        break;
    }
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), false,
            "Format %d cannot be binary encoded by PPCConditionalBranchInstruction", getOpCode().getFormat());
    }
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), getOpCode().getMaxBinaryLength() == PPC_INSTRUCTION_LENGTH,
        "Format %d cannot be binary encoded by PPCConditionalBranchInstruction", getOpCode().getFormat());

    // Conditional branches can be expanded into a conditional branch around an unconditional branch if the target label
    // is out of range for a simple bc instruction. This is done by expandFarConditionalBranches, which runs after
    // binary length estimation but before binary encoding and will call
    // PPCConditionalBranchInstruction::expandIntoFarBranch to expand the branch into two instructions. For this reason,
    // we conservatively assume that any conditional branch could be expanded to ensure that the binary length estimates
    // are correct.
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), getOpCode().getFormat() == FORMAT_NONE,
        "Format %d cannot be binary encoded by PPCAdminInstruction", getOpCode().getFormat());
    ...
    if (getOpCodeValue() == TR::InstOpCode::fence) {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), _fenceNode->getRelocationType() == TR_EntryRelative32Bit,
            "Unhandled relocation type %u", _fenceNode->getRelocationType());
...
void TR::PPCImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (needsAOTRelocation()) {
        ...
        case TR_AbsoluteHelperAddress:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, (uint8_t*)getSymbolReference(), TR_AbsoluteHelperAddress, cg()),
        ...
        case TR_RamMethod:
            if (comp()->getOption(TR_UseSymbolValidationManager)) {
                ...
                cg()->addExternalRelocation(
                    TR::ExternalRelocation::create(cursor, NULL, TR_RamMethod, cg()), __FILE__, __LINE__, getNode());
            }
            break;
        case TR_BodyInfoAddress:
            cg()->addExternalRelocation(
                TR::ExternalRelocation::create(cursor, 0, TR_BodyInfoAddress, cg()), __FILE__, __LINE__, getNode());
            break;
        default:
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
        ...
        void* addressToPatch
            = comp->target().is64Bit() ? (comp->target().cpu.isBigEndian() ? cursor : (cursor - 4)) : cursor;
...
void TR::PPCImmInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCImmInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCImm2Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCImm2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCSrc1Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCSrc1Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src1Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src1Instruction", getOpCode().getFormat());
    }
}

void TR::PPCTrg1ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
        ...
        cg()->jitAddPicToPatchOnClassUnload(
            (void*)(comp->target().is64Bit() ? node->getLongInt() : node->getInt()), (void*)cursor);
    }
        ...
        cg()->jitAddPicToPatchOnClassUnload(
            (void*)(cg()->fe()
                        ->createResolvedMethod(cg()->trMemory(),
                            (TR_OpaqueMethodBlock*)(comp->target().is64Bit() ? node->getLongInt() : node->getInt()),
...
void TR::PPCTrg1ImmInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    TR::RealRegister* trg = toRealRegister(getTargetRegister());
    uint32_t imm = getSourceImmediate();
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1ImmInstruction", getOpCode().getFormat());
    }
}

void TR::PPCTrg1Src1ImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    TR::Compilation* comp = cg()->comp();
        ...
        cg()->jitAddPicToPatchOnClassUnload(
            (void*)(cg()->fe()
                        ->createResolvedMethod(
                            cg()->trMemory(), (TR_OpaqueMethodBlock*)(getSourceImmPtr()), comp->getCurrentMethod())
                        ->classOfMethod()),
            (void*)cursor);
    }
}
...
void TR::PPCTrg1Src1ImmInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src1ImmInstruction", getOpCode().getFormat());
    }
}
...
static std::pair<int32_t, int32_t> getMaskEnds64(TR::Instruction* instr, uint64_t mask)
{
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(instr,
        mask == ((0xffffffffffffffffuLL >> lead) & (0xffffffffffffffffuLL << trail)),
        "Mask of 0x%llx has more than one group of 1 bits", mask);
...
static std::pair<int32_t, int32_t> getMaskEnds32(TR::Instruction* instr, uint64_t mask)
{
    ...
    if (mask32 != 0xffffffffu && (mask32 & 0x80000001u) == 0x80000001u) {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(instr, mask32 == ~((0xffffffffu >> lead) & (0xffffffffu << trail)),
            "Mask of 0x%x has more than one group of 1 bits", mask32);
...
void TR::PPCTrg1Src1Imm2Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    case FORMAT_RLDIC: {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), maskEnds.second == 63 - imm1 && maskEnds.first <= maskEnds.second,
            "Mask of 0x%llx does not match rldic-form for shift by %u", imm2, imm1);
    ...
    case FORMAT_RLDICL: {
        ...
        auto maskEnds = getMaskEnds64(self(), imm2);
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), maskEnds.second == 63 && maskEnds.first <= maskEnds.second,
            "Mask of 0x%llx does not match rldicl-form", imm2);
        fillFieldMDM(self(), cursor, maskEnds.first);
        break;
    }
    ...
    case FORMAT_RLDICR: {
        ...
        auto maskEnds = getMaskEnds64(self(), imm2);
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), maskEnds.first == 0 && maskEnds.first <= maskEnds.second,
            "Mask of 0x%llx does not match rldicr-form", imm2);
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src1Imm2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src2Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src2ImmInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    case FORMAT_RLDCL: {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(self(), maskEnds.second == 63 && maskEnds.first <= maskEnds.second,
            "Mask of 0x%llx does not match rldcl-form", imm);
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src2ImmInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1Src3Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1Src3Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCSrc2Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCSrc2Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCSrc3Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCSrc3Instruction", getOpCode().getFormat());
    }
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, !mr->getLabel(), "Cannot use PC-relative load with non-prefixed instruction");
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, !mr->getIndexRegister(), "Cannot use index-form MemoryReference with non-index-form instruction");
...
void fillMemoryReferenceD34RAR(TR::Instruction* instr, uint32_t* cursor, TR::MemoryReference* mr)
{
    ...
    if (mr->getLabel()) {
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            instr, !mr->getBaseRegister(), "Cannot have base register on PC-relative MemoryReference");

        if (mr->getLabel()->getCodeLocation())
            fillFieldD34(instr, cursor,
                static_cast<uint64_t>(mr->getLabel()->getCodeLocation() - reinterpret_cast<uint8_t*>(cursor))
                    + mr->getOffset());
        else
            instr->cg()->addRelocation(new (instr->cg()->trHeapMemory())
                    TR::PPCD34LabelRelocation(instr, cursor, mr->getLabel(), mr->getOffset()));
...
void fillMemoryReferenceRARB(TR::Instruction* instr, uint32_t* cursor, TR::MemoryReference* mr)
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, !mr->getLabel(), "Cannot use PC-relative load with non-prefixed instruction");
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        instr, mr->getOffset() == 0, "Cannot use non-index-form MemoryReference with index-form instruction");
    fillFieldRA(instr, cursor, toRealBaseRegister(instr, mr->getBaseRegister()));
...
void TR::PPCMemSrc1Instruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCMemSrc1Instruction", getOpCode().getFormat());
    }
}
...
void TR::PPCMemInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCMemInstruction", getOpCode().getFormat());
    }
}
...
void TR::PPCTrg1MemInstruction::fillBinaryEncodingFields(uint32_t* cursor)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_INSTRUCTION(
            self(), false, "Format %d cannot be binary encoded by PPCTrg1MemInstruction", getOpCode().getFormat());
    }
}
...
{
    TR_ASSERT_FATAL_WITH_INSTRUCTION(
        _instr, getLabel()->getCodeLocation(), "Attempt to relocate against an unencoded label");

    fillFieldD34(_instr, reinterpret_cast<uint32_t*>(getUpdateLocation()),
