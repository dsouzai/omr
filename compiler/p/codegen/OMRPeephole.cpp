...
static bool isWAWOrmrPeepholeCandidateInstr(TR::Instruction* instr)
{
    if (instr != NULL && !instr->willBePatched()
        ...
        (instr->getMemoryReference() == NULL || instr->getMemoryReference()->getUnresolvedSnippet() == NULL)) {
        return true;
    }
    return false;
}
...
bool OMR::Power::Peephole::performOnInstruction(TR::Instruction* cursor)
{
...
bool OMR::Power::Peephole::tryToReduceCompareToRecordForm()
{
    ...
    while (current != NULL && !current->isLabel()
        && (!isEBBTerminatingBranch(current) || current->getKind() == OMR::Instruction::IsControlFlow)
        ...
        if (current->getPrimaryTargetRegister() != NULL && current->getPrimaryTargetRegister() == cmpiSourceReg)
        // if target reg is same as source of compare
        {
            ...
            if (current->getOpCode().hasRecordForm()) {
                ...
                if (self()->comp()->target().cpu.is(OMR_PROCESSOR_PPC_GP)
                    ...
                    if (opCode == TR::InstOpCode::rlwinm) {
                        ...
                        if (inst->getSourceImmediate() == 0 && (inst->getMask() & 0xffff0000) == 0) {
                            if (performTransformation(self()->comp(),
                                    "O^O PPC PEEPHOLE: Change %p to andi_r, remove compare immediate %p.\n", current,
                                    cmpiInstruction)) {
                                generateTrg1Src1ImmInstruction(self()->cg(), TR::InstOpCode::andi_r, inst->getNode(),
                                    inst->getPrimaryTargetRegister(), inst->getSourceRegister(0), cmpiTargetReg,
                                    inst->getMask(), current);

                                // Removing consecutive instructions in a backwards peephole window requires us to
                                // update the restart point so the peephole traversal is able to continue
                            ...
                            return false;
                        } else if (inst->getSourceImmediate() == 0 && (inst->getMask() & 0x0000ffff) == 0) {
                            if (performTransformation(self()->comp(),
                                    "O^O PPC PEEPHOLE: Change %p to andis_r, remove compare immediate %p.\n", current,
                                    cmpiInstruction)) {
                                generateTrg1Src1ImmInstruction(self()->cg(), TR::InstOpCode::andis_r, inst->getNode(),
                                    inst->getPrimaryTargetRegister(), inst->getSourceRegister(0), cmpiTargetReg,
                                    ((uint32_t)inst->getMask()) >> 16, current);
                ...
                if (performTransformation(self()->comp(),
                        "O^O PPC PEEPHOLE: Change %p to record form, remove compare immediate %p.\n", current,
                        cmpiInstruction)) {
            ...
            if (current->defsRegister(toRealRegister(cmpiSourceReg))
                || current->defsRegister(toRealRegister(cmpiTargetReg)) || current->getOpCode().isRecordForm())
                return false;
        }
        current = current->getPrev();
    }
...
bool OMR::Power::Peephole::tryToRemoveRedundantLoadAfterStore()
{
    ...
    if (cursor->getKind() != TR::Instruction::IsMemSrc1 || !cursor->getNext()
        || cursor->getNext()->getKind() != TR::Instruction::IsTrg1Mem)
        return false;
    ...
    // TODO: Relax the first constraint a bit and handle cases were unrelated instructions appear between the store and
    // the load
    if (!loadOp.isLoad() || storeMemRef->getLength() != loadMemRef->getLength()
        || !(storeOp.gprOp() == loadOp.gprOp() || storeOp.fprOp() == loadOp.fprOp()) || storeOp.isUpdate()
        || loadOp.isUpdate() || storeMemRef->getUnresolvedSnippet() || loadMemRef->getUnresolvedSnippet()) {
        return false;
    }
    ...
    if (srcReg == trgReg) {
        ...
        if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
            if (performTransformation(self()->comp(),
                    "O^O PPC PEEPHOLE: Replace lwz " POINTER_PRINTF_FORMAT
                    " with rlwinm after store " POINTER_PRINTF_FORMAT ".\n",
                    loadInstruction, storeInstruction)) {
                generateTrg1Src1Imm2Instruction(self()->cg(), TR::InstOpCode::rlwinm, loadInstruction->getNode(),
                    trgReg, srcReg, 0, 0xffffffff, storeInstruction);
                loadInstruction->remove();
                return true;
            }
        } else if (performTransformation(self()->comp(),
                       "O^O PPC PEEPHOLE: Remove redundant load " POINTER_PRINTF_FORMAT
                       " after store " POINTER_PRINTF_FORMAT ".\n",
                       loadInstruction, storeInstruction)) {
            loadInstruction->remove();
            return true;
        }
    ...
    if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
        if (performTransformation(self()->comp(),
                "O^O PPC PEEPHOLE: Replace redundant load " POINTER_PRINTF_FORMAT " after store " POINTER_PRINTF_FORMAT
                " with rlwinm.\n",
                loadInstruction, storeInstruction)) {
            generateTrg1Src1Imm2Instruction(self()->cg(), TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg,
                srcReg, 0, 0xffffffff, storeInstruction);
            loadInstruction->remove();
            return true;
        }
    } else if (performTransformation(self()->comp(),
                   "O^O PPC PEEPHOLE: Replace redundant load " POINTER_PRINTF_FORMAT
                   " after store " POINTER_PRINTF_FORMAT " with mr.\n",
                   loadInstruction, storeInstruction)) {
        generateTrg1Src1Instruction(
            self()->cg(), TR::InstOpCode::mr, loadInstruction->getNode(), trgReg, srcReg, storeInstruction);
        loadInstruction->remove();
        return true;
    }
...
bool OMR::Power::Peephole::tryToRemoveRedundantMoveRegister()
{
    ...
    while (isWAWOrmrPeepholeCandidateInstr(current) && !isEBBTerminatingBranch(current) && windowSize < maxWindowSize) {
        // Keep track of any GC points we cross in order to fix them if we remove a register move
        // Note: Don't care about GC points in snippets, since we won't remove a reg move if inEBB, i.e. if we've
        // encountered a branch (to a snippet or elsewhere)
        if (current->getGCMap()
            && current->getGCMap()->getRegisterMap()
        ...
        if (current->getOpCodeValue() == TR::InstOpCode::mr && current->getSourceRegister(0) == mrTargetReg
        ...
        if (memRef != NULL) {
            ...
            if (memRef->isUsingDelayedIndexedForm()
                && (memRef->getBaseRegister() == mrSourceReg || memRef->getIndexRegister() == mrSourceReg
                    || memRef->getBaseRegister() == mrTargetReg || memRef->getIndexRegister() == mrTargetReg))
                return performed;
            if (current->isUpdate()
                && (memRef->getBaseRegister() == mrSourceReg || memRef->getBaseRegister() == mrTargetReg))
                return performed;
            if (memRef->getBaseRegister() == mrTargetReg) {
                if (toRealRegister(mrSourceReg)->getRegisterNumber() == TR::RealRegister::gr0)
                    all_mr_source_uses_rewritten = false;
                             ...
                             self()->comp(), "O^O PPC PEEPHOLE: Rewrite base register %p.\n", current)) {
            ...
            if (current->getKind() == OMR::Instruction::IsMemSrc1
                ...
                if (performTransformation(
                        self()->comp(), "O^O PPC PEEPHOLE: Rewrite store source register %p.\n", current)) {
            ...
            case OMR::Instruction::IsSrc1: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Src1 source register %p.\n", current)) {
            ...
            case OMR::Instruction::IsTrg1Src1: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src1 source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (toRealRegister(mrSourceReg)->getRegisterNumber() == TR::RealRegister::gr0
                        ...
                        all_mr_source_uses_rewritten = false;
                                 ...
                                 "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
            ...
            case OMR::Instruction::IsTrg1Src1Imm2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm2 source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (current->getOpCode().usesTarget() && inst->getTargetRegister() == mrTargetReg)
                    return performed;
                break;
            }
            case OMR::Instruction::IsSrc2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Src2 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Src2 2nd source register %p.\n", current)) {
            ...
            case OMR::Instruction::IsTrg1Src2: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src2 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src2 2nd source register %p.\n", current)) {
                        inst->setSource2Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
            ...
            case OMR::Instruction::IsTrg1Src3: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src3 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src3 2nd source register %p.\n", current)) {
                        inst->setSource2Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource3Register() == mrTargetReg) {
                    if (performTransformation(
                            self()->comp(), "O^O PPC PEEPHOLE: Rewrite Trg1Src3 3nd source register %p.\n", current)) {
            ...
            case OMR::Instruction::IsTrg1Src2Imm: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (performTransformation(self()->comp(),
                            "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 1st source register %p.\n", current)) {
                        inst->setSource1Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
                ...
                if (inst->getSource2Register() == mrTargetReg) {
                    if (performTransformation(self()->comp(),
                            "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 2nd source register %p.\n", current)) {
                        inst->setSource2Register(mrSourceReg);
                        extendWindow = true;
                        performed = true;
                    } else
        ...
        if (current->defsRegister(toRealRegister(mrTargetReg))) {
            ...
            if (!inEBB && all_mr_source_uses_rewritten
                && performTransformation(self()->comp(),
                    "O^O PPC PEEPHOLE: Remove mr %p when target redefined at %p.\n", mrInstruction, current)) {
                ...
                if (!stackMaps.empty()) {
                    for (auto stackMapIter = stackMaps.begin(); stackMapIter != stackMaps.end(); ++stackMapIter) {
                        ...
                        (*stackMapIter)
                            ->setRegisterBits(
                                self()->cg()->registerBitMask(toRealRegister(mrSourceReg)->getRegisterNumber()));
                    }
                }
...
bool OMR::Power::Peephole::tryToRemoveRedundantSync(int32_t window)
{
    ...
    while (visited < window) {
        ...
        if (removeFirst) {
            if (performTransformation(
                    self()->comp(), "O^O PPC PEEPHOLE: Remove redundant syncronization instruction %p.\n", first)) {
            ...
            if (performTransformation(self()->comp(),
                    "O^O PPC PEEPHOLE: Remove redundant syncronization instruction %p.\n",
...
bool OMR::Power::Peephole::tryToRemoveRedundantWriteAfterWrite()
{
    ...
    if (currTargetReg && currTargetReg->getKind() != TR_CCR && isWAWOrmrPeepholeCandidateInstr(cursor)
        && !cursor->isBranchOp() && !cursor->isCall() && !cursor->getOpCode().isRecordForm()
        && !cursor->getOpCode().setsCarryFlag()) {
        TR::Instruction* current = cursor->getNext();

        while (isWAWOrmrPeepholeCandidateInstr(current) && !current->isBranchOp() && !current->isCall()
            ...
            case OMR::Instruction::IsTrg1Mem:
                if (currTargetReg == current->getSourceRegister(0) || currTargetReg == current->getSourceRegister(1))
                    return false;
                break;
            case OMR::Instruction::IsTrg1Src3:
            ...
            if (current->getTargetRegister(0) == currTargetReg) {
                ...
                if (isConstData) {
                    if (performTransformation(self()->comp(),
                            "O^O PPC PEEPHOLE: Remove dead instrcution group from WAW %p -> %p.\n", cursor, current)) {
                    ...
                    if (performTransformation(self()->comp(),
                            "O^O PPC PEEPHOLE: Remove dead instrcution from WAW %p -> %p.\n", cursor, current)) {
                        cursor->remove();
                        return true;
                    }
                }
...
bool OMR::Power::Peephole::tryToSwapTrapAndLoad()
{
    ...
    if (instructionCursor->getSourceRegister(1) != NULL || instructionCursor->getOffset() != 0) {
        return false;
    }
    ...
    for (int i = 0; i < WINDOW && next != NULL; i++) {
        ...
        if (!next->isLoad() && !next->isStore() && !next->isSyncSideEffectFree())
            return false;
        if (NULL != next->getMemoryReference() && NULL != next->getMemoryReference()->getUnresolvedSnippet())
            return false;
        ...
        if (next->isLoad() && next->getSourceRegister(0) != NULL
            && next->getSourceRegister(0)->getRealRegister() == RAr) {
                ...
                for (TR::Instruction* prev = load->getPrev(); prev != trap->getPrev(); prev = prev->getPrev()) {
                    for (int k = 0; load->getTargetRegister(k) != NULL; k++) {
                        for (int l = 0; prev->getSourceRegister(l) != NULL; l++) {
                            if (prev->getSourceRegister(l)->getRealRegister()
                                == load->getTargetRegister(k)->getRealRegister())
                                return false;
                        }
                        for (int m = 0; prev->getTargetRegister(m) != NULL; m++) {
                            if (prev->getTargetRegister(m)->getRealRegister()
                                == load->getTargetRegister(k)->getRealRegister())
                                return false;
                        }
                    }
