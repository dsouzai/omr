...
static void genericLongAnalyzer(
    TR::CodeGenerator* cg, TR::Node*& child, TR::Register*& lowReg, TR::Register*& highReg, bool& highZero)
{
        ...
        if (!setsOrReadsCC && secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (!setsOrReadsCC && (firstChild->isHighWordZero() || secondChild->isHighWordZero())) {
                ...
                generateTrg1Src2Instruction(
                    cg, regToRegOpCode, node, lowReg, src1Reg->getLowOrder(), src2Reg->getLowOrder());
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::adde, node, highReg, src1Reg->getHighOrder(), src2Reg->getHighOrder());
            }
        }
    } else // 64 bit target
    {
        ...
        if (!setsOrReadsCC && firstChild->getOpCodeValue() == TR::lconst && firstChild->getRegister() == NULL) {
            ...
            if (lowValue >= LOWER_IMMED && lowValue <= UPPER_IMMED) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::subfic, node, lowReg, src2Reg->getLowOrder(), lowValue);
            } else {
                TR::Register* tempReg = cg->allocateRegister();
            ...
            generateTrg1Src2Instruction(
                cg, regToRegOpCode, node, lowReg, src2Reg->getLowOrder(), src1Reg->getLowOrder());
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::subfe, node, highReg, src2Reg->getHighOrder(), src1Reg->getHighOrder());
        }
        trgReg = cg->allocateRegisterPair(lowReg, highReg);
    }
...
//
TR::Register* OMR::Power::TreeEvaluator::dualMulHelper64(
    TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
    ...
    if (!needsUnsignedHighMulOnly) {
        ...
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::mulld, node, lmulTargetRegister, lumulhTargetRegister, secondRegister);
        lmulNode->setRegister(lmulTargetRegister);
    }
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mulhdu, node, lumulhTargetRegister, lumulhTargetRegister, secondRegister);
...
//
TR::Register* OMR::Power::TreeEvaluator::dualMulHelper32(
    TR::Node* node, TR::Node* lmulNode, TR::Node* lumulhNode, TR::CodeGenerator* cg)
{
    bool needsUnsignedHighMulOnly = (lmulNode == NULL);
    // requires:
    ...
    TR::Register* divisorReg = NULL, TR::Register* tmp1Reg = NULL, TR::Register* tmp2Reg = NULL)
{
    ...
    if (divisorValue == 1) {
        ...
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::srawi, node, trgReg, dividendReg, trailingZeroes(divisorValue));
...
static TR::Register* signedLongDivisionOrRemainderAnalyser(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isPowerOf2(divisor)) {
        ...
        if (rootOpCode == TR::lrem) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::sradi, node, temp1Reg, dividendReg, trailingZeroes(divisor));
...
TR::Register* OMR::Power::TreeEvaluator::idivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (divisor != 0) {
        trgReg = signedIntegerDivisionOrRemainderAnalyser(
            node, cg, dividendReg, divisor, false, NULL, secondChild->getRegister());
    } else {
    ...
    *dr_lowReg = dr_l;
    ...
    if (!isDividendImpossible32Bit && !isDivisorImpossible32Bit) {
        ...
        if (secondChild->getOpCode().isLoadConst() && drConst <= (int64_t)TR::getMaxSigned<TR::Int32>()
            ...
            signedIntegerDivisionOrRemainderAnalyser(
                node, cg, dd_l, drConst, isRemainder, isRemainder ? dr_l : dd_l, dr_l, tmp1Reg, tmp2Reg);
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::srawi, node, isRemainder ? dr_h : dd_h, isRemainder ? dr_l : dd_l, 31);
        } else {
...
TR::Register* OMR::Power::TreeEvaluator::ldivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    strengthReducingLongDivideOrRemainder32BitMode(
        node, cg, dependencies, &dd_highReg, &dd_lowReg, &dr_highReg, &dr_lowReg, signed_div, false);
...
TR::Register* OMR::Power::TreeEvaluator::iremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (divisor != 0) {
        trgReg = signedIntegerDivisionOrRemainderAnalyser(
            node, cg, dividendReg, divisor, true, NULL, secondChild->getRegister());
    } else {
        bool testNeeded = (!(secondChild->isNonNegative()) && !(firstChild->isNonNegative()));
        TR::Register* condReg;
        TR::Register* divisorReg = cg->evaluate(secondChild);
...
TR::Register* OMR::Power::TreeEvaluator::lremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    strengthReducingLongDivideOrRemainder32BitMode(
        node, cg, dependencies, &dd_highReg, &dd_lowReg, &dr_highReg, &dr_lowReg, signed_rem, true);
...
static bool isMaskThenShiftRightCandidate(
    TR::CodeGenerator* cg, TR::Node* node, uint32_t operandBits, uint64_t shiftAmount, bool isUnsigned)
{
    static bool disableMaskAndShiftMerging = feGetEnv("TR_DisableMaskAndShiftMerging");
    if (disableMaskAndShiftMerging)
        return false;
...
TR::Register* OMR::Power::TreeEvaluator::irolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, targetRegister, srcRegister, shiftAmount, 0xffffffff);
    } else {
        TR::Register* shiftAmountReg = cg->evaluate(secondChild);
        generateTrg1Src2ImmInstruction(
            cg, TR::InstOpCode::rlwnm, node, targetRegister, srcRegister, shiftAmountReg, 0xffffffff);
    }
...
TR::Register* OMR::Power::TreeEvaluator::lrolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicl, node, targetRegister, srcRegister, shiftAmount, CONSTANT64(0xffffffffffffffff));
    } else {
        TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    TR::Node* constNode)
{
    ...
    if (localVal.getValue() == 0) {
        ...
        TR::Register* tmpReg = cg->allocateRegister(TR_CCR);
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andis_r, node, trgReg, srcReg, tmpReg, localVal.getHighBits());
        cg->stopUsingRegister(tmpReg);
    } else {
        TR::Register* tmpReg;
        if (constNode != NULL) {
...
void simplifyANDRegImm(
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, TR::Node* constNode, TR::CodeGenerator* cg)
{
    int32_t lowValue;
    int32_t highValue;
    ...
    if (lowSpecial == true || highSpecial == true) {
        ...
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::AND, node, trgReg->getLowOrder(), srcReg->getLowOrder(), constReg->getLowOrder());
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::AND, node, trgReg->getHighOrder(), srcReg->getHighOrder(), constReg->getHighOrder());
    }
}
...
TR::Register* OMR::Power::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else // 32 bit target
    {
        if (secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (firstChild->isHighWordZero() || secondChild->isHighWordZero()) {
                return carrylessLongEvaluatorWithAnalyser(
                    node, cg, TR::InstOpCode::AND, TR::InstOpCode::AND, TR::InstOpCode::mr);
            } else {
                TR::Register* src1Reg = cg->evaluate(firstChild);
                TR::Register* src2Reg = cg->evaluate(secondChild);
        ...
        if (secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (localVal.getValue() == 0) {
                generateTrg1Src1Instruction(
                    cg, TR::InstOpCode::mr, node, trgReg->getLowOrder(), src1Reg->getLowOrder());
                ...
                if (localVal.getHighBits() == 0) {
                    generateTrg1Src1ImmInstruction(
                        cg, immedOp, node, trgReg->getLowOrder(), src1Reg->getLowOrder(), localVal.getLowBits());
                } else if (localVal.getLowBits() == 0) {
                    ...
                    TR::Register* tempReg = cg->allocateRegister();
                    generateTrg1Src1ImmInstruction(
                        cg, immedOp, node, tempReg, src1Reg->getLowOrder(), localVal.getLowBits());
                    generateTrg1Src1ImmInstruction(
                        cg, immedShiftedOp, node, trgReg->getLowOrder(), tempReg, localVal.getHighBits());
                    cg->stopUsingRegister(tempReg);
                }
            }
            ...
            if (localVal.getValue() == 0) {
                generateTrg1Src1Instruction(
                    cg, TR::InstOpCode::mr, node, trgReg->getHighOrder(), src1Reg->getHighOrder());
            } else if (localVal.getValue() == -1 && (node->getOpCodeValue() == TR::lor)) {
                generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg->getHighOrder(), -1);
            } else {
                if (localVal.getHighBits() == 0) {
                    generateTrg1Src1ImmInstruction(
                        cg, immedOp, node, trgReg->getHighOrder(), src1Reg->getHighOrder(), localVal.getLowBits());
                } else if (localVal.getLowBits() == 0) {
                    ...
                    TR::Register* tempReg = cg->allocateRegister();
                    generateTrg1Src1ImmInstruction(
                        cg, immedOp, node, tempReg, src1Reg->getHighOrder(), localVal.getLowBits());
                    generateTrg1Src1ImmInstruction(
                        cg, immedShiftedOp, node, trgReg->getHighOrder(), tempReg, localVal.getHighBits());
                    cg->stopUsingRegister(tempReg);
                }
            }
        } else {
            TR::Register* src2Reg = cg->evaluate(secondChild);
            generateTrg1Src2Instruction(
                cg, regOp, node, trgReg->getLowOrder(), src2Reg->getLowOrder(), src1Reg->getLowOrder());
            generateTrg1Src2Instruction(
                cg, regOp, node, trgReg->getHighOrder(), src2Reg->getHighOrder(), src1Reg->getHighOrder());
        }
    }
    node->setRegister(trgReg);
    cg->decReferenceCount(firstChild);
...
TR::Register* OMR::Power::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...

    {
        ...
        return lorTypeEvaluator(
            node, TR::InstOpCode::ori, TR::InstOpCode::oris, TR::InstOpCode::OR, TR::InstOpCode::or_r, cg);
    }
}
...
TR::Register* OMR::Power::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((node->getFirstChild()->isHighWordZero() || node->getSecondChild()->isHighWordZero())
        ...
        && !(cg->comp()->target().is64Bit())) {
        return carrylessLongEvaluatorWithAnalyser(
            node, cg, TR::InstOpCode::XOR, TR::InstOpCode::XOR, TR::InstOpCode::mr);
    } else {
        return lorTypeEvaluator(
            node, TR::InstOpCode::xori, TR::InstOpCode::xoris, TR::InstOpCode::XOR, TR::InstOpCode::xor_r, cg);
    }
}
...
TR::Register* OMR::Power::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        if (cg->isRotateAndMask(node)) {
            ...
            simplifyANDRegImm(
                node, trgReg, cg->evaluate(firstChild), (int32_t)secondChild->get64bitIntegralValue(), cg, secondChild);
    } else {
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::AND, node, trgReg, cg->evaluate(firstChild), cg->evaluate(secondChild));
    }
