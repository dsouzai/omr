...
TR::Register* OMR::Power::TreeEvaluator::iu2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicl, node, trgReg, cg->evaluate(child), 0, CONSTANT64(0x00000000ffffffff));
        node->setRegister(trgReg);
        cg->decReferenceCount(child);
        return trgReg;
    } else // 32 bit target
...
TR::Register* passThroughLongLowEvaluator(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic loadOp, int64_t srcLen)
{
    if (cg->comp()->target().is64Bit())
        return TR::TreeEvaluator::passThroughEvaluator(node, cg);
...
TR::Register* OMR::Power::TreeEvaluator::su2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && (temp == NULL)) {
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicl, node, trgReg, cg->evaluate(child), 0, CONSTANT64(0x000000000000ffff));
            cg->decReferenceCount(child);
        }
    ...
    } else // 32 bit target
    {
        ...
        if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && (temp == NULL)) {
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, trgReg->getLowOrder(), cg->evaluate(child), 0, 0xffff);
            decSharedNode = true;
        }
        generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg->getHighOrder(), 0);
...
TR::Register* OMR::Power::TreeEvaluator::bu2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        if (child->getOpCode().isMemoryReference()) {
            ...
            trgReg = cg->allocateRegister();
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicl, node, trgReg, cg->evaluate(child), 0, CONSTANT64(0x00000000000000ff));
        }
    } else // 32 bit target
    {
        TR::Register* lowReg;
...
TR::Register* OMR::Power::TreeEvaluator::dfloorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::fadd, node, tmp2Reg, trgReg, tmp1Reg); // tmp2  = trunc(x) -1  for floor
                                                                   //       = trunc(x) +1  for ceil
    if ((node->getOpCodeValue() == TR::dfloor) || (node->getOpCodeValue() == TR::ffloor))
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::fsub, node, tmp3Reg, srcReg, trgReg); // tmp3  = (x-trunc(x)) for floor
    else
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::fsub, node, tmp3Reg, trgReg, srcReg); // tmp3  = (trunc(x)-x) for ceil
