...
TR::Register* vcmpHelper(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, bool complement, bool switchOperands)
{
...
static TR::Register* vreductionAddSubWordHelper(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic sumOp)
{
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::vsumsws, node, tempRes, tempRes, zeroReg); // Sum word elements across entire vector operand
...
TR::Register* OMR::Power::TreeEvaluator::vreductionAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    switch (type) {
    case TR::Int8:
        return vreductionAddSubWordHelper(
            node, cg, TR::InstOpCode::vsum4sbs); // since the results will never overflow, it is safe to use a saturate
                                                 // sum instruction here
    case TR::Int16:
        return vreductionAddSubWordHelper(
            node, cg, TR::InstOpCode::vsum4shs); // since the results will never overflow, it is safe to use a saturate
                                                 // sum instruction here
    case TR::Int32:
    ...
    void* value, TR::Register* trgReg)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_NODE(
            node, false, "Cannot call loadFloatConstant with data type %s", TR::DataType::getName(type));
    }
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        case TR::InstOpCode::lxvdsx:
            ...
            generateTrg1MemInstruction(
                cg, loadOp, node, trgReg, TR::MemoryReference::createWithIndexReg(cg, NULL, tmpReg, length));
            break;
        default:
        ...
        default:
            TR_ASSERT_FATAL_WITH_NODE(
                node, false, "Invalid data type %s in loadFloatConstant", TR::DataType::getName(type));
        }
        ...
        if (tocOffset != PTOC_FULL_INDEX) {
            ...
            if (tocOffset < LOWER_IMMED || tocOffset > UPPER_IMMED) {
                ...
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, node, tmpReg, cg->getTOCBaseRegister(), HI_VALUE(tocOffset));
...
TR::Instruction* loadConstant(
    TR::CodeGenerator* cg, TR::Node* node, int32_t value, TR::Register* trgReg, TR::Instruction* cursor, bool isPicSite)
{
    ...
    TR::Instruction* cursor, bool isPicSite, bool useTOC)
{
    ...
    // interchangeable.
    TR_ASSERT_FATAL_WITH_NODE(
        node, cg->comp()->target().is64Bit(), "Should not use 64-bit loadConstant on 32-bit builds");
    ...
    if (offset != PTOC_FULL_INDEX) {
        ...
        if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
            ...
            cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addis, node, trgReg, cg->getTOCBaseRegister(), cg->hiValue(offset), cursor);
        ...
        if ((hhval == -1 && (hlval & 0x8000) != 0) || (hhval == 0 && (hlval & 0x8000) == 0)) {
            ...
            cursor = generateTrg1ImmInstruction(
                cg, TR::InstOpCode::lis, node, trgReg, static_cast<int16_t>(hhval), cursor);
        ...
        cursor = generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicr, node, trgReg, trgReg, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
    ...
    TR::Instruction* cursor, TR::Register* tempReg)
{
    ...
    if (cg->comp()->target().is32Bit()) {
        ...
        if (tempReg == NULL) {
            ...
            nibbles[1] = cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::ori, node, baseReg, baseReg, (hiAddr >> 16) & 0x0000FFFF, cursor);
            cursor = generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicr, node, baseReg, baseReg, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
            nibbles[2] = cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::oris, node, baseReg, baseReg, hiAddr & 0x0000FFFF, cursor);
        } else {
            nibbles[0] = cursor
            ...
            nibbles[2] = cursor
                = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, baseReg, (int16_t)hiAddr, cursor);
            nibbles[1] = cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::ori, node, tempReg, tempReg, (hiAddr >> 16) & 0x0000FFFF, cursor);
            cursor = generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldimi, node, baseReg, tempReg, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
        }
        idx = 3;
    }
    ...
    if (opCode == TR::InstOpCode::lxvdsx) {
        ...
        if (cg->comp()->target().is64Bit() && tempReg) {
            nibbles[idx] = cursor = generateTrg1ImmInstruction(
                cg, TR::InstOpCode::li, node, tempReg, ((int32_t)loAddr) << 16 >> 16, cursor);
            memRef = TR::MemoryReference::createWithIndexReg(cg, baseReg, tempReg, opSize);
        } else {
            nibbles[idx] = cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, baseReg, baseReg, ((int32_t)loAddr) << 16 >> 16, cursor);
    ...
    if (tempReg) {
        ...
        cursor = generateDepLabelInstruction(
            cg, TR::InstOpCode::label, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), dep);
    }
...
TR::Register* OMR::Power::TreeEvaluator::istoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!reverseStore && valueChild->getRegister() == NULL && valueChild->getReferenceCount() == 1) {
        ...
        if (valueChild->getOpCodeValue() == TR::fbits2i && !valueChild->normalizeNanValues()) {
            TR::LoadStoreHandler::generateStoreNodeSequence(
                cg, cg->evaluate(valueChild->getFirstChild()), node, TR::InstOpCode::stfs, 4);
...
TR::Register* OMR::Power::TreeEvaluator::lstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!reverseStore && valueChild->getRegister() == NULL && valueChild->getReferenceCount() == 1) {
        ...
        if (valueChild->getOpCodeValue() == TR::dbits2l && !valueChild->normalizeNanValues()) {
            TR::LoadStoreHandler::generateStoreNodeSequence(
                cg, cg->evaluate(valueChild->getFirstChild()), node, TR::InstOpCode::stfd, 8);
            cg->decReferenceCount(valueChild->getFirstChild());
            cg->decReferenceCount(valueChild);
            return NULL;
        }
...
TR::Register* OMR::Power::TreeEvaluator::inlineVectorUnaryOp(
...
TR::Register* OMR::Power::TreeEvaluator::inlineVectorBinaryOp(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::Power::TreeEvaluator::inlineVectorBitSelectOp(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::Power::TreeEvaluator::vgetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!disableDirectMove && cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
        ...
        return TR::TreeEvaluator::vgetelemMemoryMoveHelper(
            node, cg); // transfers data through memory instead of via direct move instructions.
    }
}
...
TR::Register* OMR::Power::TreeEvaluator::vgetelemDirectMoveHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (elementCount == 4) {
            ...
            if (!skipSplat) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::xxspltw, node, intermediateResReg, srcVectorReg, elem);
            }

            if (firstChild->getDataType().getVectorElementType() == TR::Int32) {
                generateMvFprGprInstructions(
                    cg, node, fpr2gprLow, false, resReg, skipSplat ? srcVectorReg : intermediateResReg);
            } else // TR::Float
            {
                generateTrg1Src1Instruction(
                    cg, TR::InstOpCode::xscvspdp, node, resReg, skipSplat ? srcVectorReg : intermediateResReg);
            }
        } else // elementCount == 2
        {
            /*
            ...
            if (readElemOne) {
                generateTrg1Src2ImmInstruction(
                    cg, TR::InstOpCode::xxsldwi, node, intermediateResReg, srcVectorReg, srcVectorReg, 0x2);
            }
            ...
            if (cg->comp()->target().is32Bit() && firstChild->getDataType().getVectorElementType() == TR::Int64) {
                ...
                generateMvFprGprInstructions(
                    cg, node, fpr2gprHost64, false, resReg, readElemOne ? intermediateResReg : srcVectorReg);
            } else // TR::Double
            {
        ...
        if (firstChild->getDataType().getVectorElementType() == TR::Int32) {
            ...
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::xxlor, node, intermediateResReg, srcVectorReg, srcVectorReg);
            ...
            if (cg->comp()->target().is32Bit() && firstChild->getDataType().getVectorElementType() == TR::Int64) {
                generateMvFprGprInstructions(
                    cg, node, fpr2gprHost32, false, highResReg, lowResReg, intermediateResReg, tempVectorReg);
            } else if (firstChild->getDataType().getVectorElementType() == TR::Int64) {
                generateMvFprGprInstructions(cg, node, fpr2gprHost64, false, resReg, intermediateResReg);
            } else {
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::xxlor, node, resReg, intermediateResReg, intermediateResReg);
            }
        }
...
TR::Register* OMR::Power::TreeEvaluator::vgetelemMemoryMoveHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::addi2, node, resReg, TR::MemoryReference::createWithSymRef(cg, node, localTemp, 16));
    generateMemSrc1Instruction(
        cg, vecStoreOpCode, node, TR::MemoryReference::createWithIndexReg(cg, NULL, resReg, 16), vectorReg);
    ...
    if (firstChild->getDataType().getVectorElementType() == TR::Float
        ...
        if (!cg->comp()->target().cpu.isLittleEndian()) {
            ...
            lowResReg = resReg;
            generateTrg1MemInstruction(
                cg, loadOpCode, node, highResReg, TR::MemoryReference::createWithIndexReg(cg, resReg, offsetReg, 4));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, offsetReg, offsetReg, 4);
            generateTrg1MemInstruction(
                cg, loadOpCode, node, lowResReg, TR::MemoryReference::createWithIndexReg(cg, resReg, offsetReg, 4));
        } else {
            highResReg = resReg;
            lowResReg = cg->allocateRegister();
            generateTrg1MemInstruction(
...
TR::Register* OMR::Power::TreeEvaluator::visetelemHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::addi2, node, addrReg, TR::MemoryReference::createWithSymRef(cg, node, localTemp, 16));
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::stxvw4x, node, TR::MemoryReference::createWithIndexReg(cg, NULL, addrReg, 16), vectorReg);
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::lxvw4x, node, resReg, TR::MemoryReference::createWithIndexReg(cg, NULL, addrReg, 16));
...
TR::Register* OMR::Power::TreeEvaluator::vabsIntHelper(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shift, TR::InstOpCode::Mnemonic add)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
...
TR::Register* OMR::Power::TreeEvaluator::vdivInt32Helper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::addi2, node, srcV1IdxReg, TR::MemoryReference::createWithSymRef(cg, node, srcV1, 16));
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::addi2, node, srcV2IdxReg, TR::MemoryReference::createWithSymRef(cg, node, srcV2, 16));

    // store the src regs to memory
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::stxvw4x, node, TR::MemoryReference::createWithIndexReg(cg, NULL, srcV1IdxReg, 16), lhsReg);
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::stxvw4x, node, TR::MemoryReference::createWithIndexReg(cg, NULL, srcV2IdxReg, 16), rhsReg);
    ...
    TR::Register* resReg = cg->allocateRegister(TR_VRF);
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::lxvw4x, node, resReg, TR::MemoryReference::createWithIndexReg(cg, NULL, srcV1IdxReg, 16));
...
static void inlineArrayCopy(
    TR::Node* node, int64_t byteLen, TR::Register* src, TR::Register* dst, TR::CodeGenerator* cg)
{
    if (byteLen == 0)
        return;
    ...
    if (postP10CopyInline) {
        ...
        if (iteration64 > 0) {
            ...
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lxv, node, regs[1], TR::MemoryReference::createWithDisplacement(cg, src, 0, 16));
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stxv, node, TR::MemoryReference::createWithDisplacement(cg, dst, 0, 16), regs[1]);
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lxv, node, regs[1], TR::MemoryReference::createWithDisplacement(cg, src, 16, 16));
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stxv, node, TR::MemoryReference::createWithDisplacement(cg, dst, 16, 16), regs[1]);
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lxv, node, regs[1], TR::MemoryReference::createWithDisplacement(cg, src, 32, 16));
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stxv, node, TR::MemoryReference::createWithDisplacement(cg, dst, 32, 16), regs[1]);
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lxv, node, regs[1], TR::MemoryReference::createWithDisplacement(cg, src, 48, 16));
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stxv, node, TR::MemoryReference::createWithDisplacement(cg, dst, 48, 16), regs[1]);
        ...
        if ((residue64 & 0xF) != 0) {
            ...
            default:
                ...
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldicr, node, regs[0], regs[0], 56, CONSTANT64(0xff00000000000000));
                if (standingOffset != 0) {
        ...
        if (groups != 0) {
            ...
            if (supportsLEArrayCopyInline) {
                ...
                generateTrg1MemInstruction(
                    cg, load, node, regs[3], TR::MemoryReference::createWithDisplacement(cg, src, 0, memRefSize));
            ...
            if (supportsLEArrayCopyInline) {
                ...
                generateMemSrc1Instruction(
                    cg, store, node, TR::MemoryReference::createWithDisplacement(cg, dst, 0, memRefSize), regs[3]);
        ...
        for (; residual >= memRefSize; residual -= memRefSize, ix += memRefSize) {
            if (supportsLEArrayCopyInline) {
                ...
                if (regIx > 3 || regs[regIx] == NULL)
                    regIx = 0;
                generateTrg1MemInstruction(
                    cg, load, node, oneReg, TR::MemoryReference::createWithDisplacement(cg, src, ix, memRefSize));
                generateMemSrc1Instruction(
                    cg, store, node, TR::MemoryReference::createWithDisplacement(cg, dst, ix, memRefSize), oneReg);
            }
        }
        ...
        if (residual != 0) {
            ...
            if (residual & 2) {
                ...
                if (regIx > 3 || regs[regIx] == NULL)
                    regIx = 0;
                generateTrg1MemInstruction(
                    cg, TR::InstOpCode::lhz, node, oneReg, TR::MemoryReference::createWithDisplacement(cg, src, ix, 2));
                generateMemSrc1Instruction(
                    cg, TR::InstOpCode::sth, node, TR::MemoryReference::createWithDisplacement(cg, dst, ix, 2), oneReg);
                ix += 2;
            }
            if (residual & 1) {
...
static bool loadValue2(
    TR::Node* node, int valueSize, int length, bool constFill, TR::Register*& tmp1Reg, TR::CodeGenerator* cg)
{
    bool retVal;
...
static TR::Register* inlineArrayCmpP10(TR::Node* node, TR::CodeGenerator* cg, bool isArrayCmpLen)
{
    ...
    generateTrg1Src1ImmInstruction(
        cg, is64bit ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpli4, node, condReg, tempReg, 16);
...
static TR::Register* inlineArrayCmp(TR::Node* node, TR::CodeGenerator* cg, bool isArrayCmpLen)
{
    ...
    if (byteLen == 4) {
        ...
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::ldu, node, src1Reg, TR::MemoryReference::createWithDisplacement(cg, src1AddrReg, 8, 8));
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::ldu, node, src2Reg, TR::MemoryReference::createWithDisplacement(cg, src2AddrReg, 8, 8));
    }
    ...
    generateTrg1Src2Instruction(
        cg, (byteLen == 8) ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, condReg, src1Reg, src2Reg);
    ...
    generateTrg1Src1ImmInstruction(
        cg, is64bit ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpli4, node, condReg2, byteLenRemainingRegister, 0);
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, byteLenRemainingRegister, byteLenRemainingRegister, tempReg);

    if (is64bit)
        generateShiftLeftImmediateLong(
            cg, node, byteLenRemainingRegister, byteLenRemainingRegister, (byteLen == 8) ? 3 : 2);
    else
        generateShiftLeftImmediate(
            cg, node, byteLenRemainingRegister, byteLenRemainingRegister, (byteLen == 8) ? 3 : 2);
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, byteLenRemainingRegister, byteLenRemainingRegister, byteLenRegister);
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::lbzu, node, src1Reg, TR::MemoryReference::createWithDisplacement(cg, src1AddrReg, 1, 1));
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::lbzu, node, src2Reg, TR::MemoryReference::createWithDisplacement(cg, src2AddrReg, 1, 1));
    ...
    if (isArrayCmpLen) {
        if (is64bit) {
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::subf, node, ccReg, byteLenRemainingRegister, byteLenRegister);
        } else {
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::subf, node, lowReturnReg, byteLenRemainingRegister, byteLenRegister);
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, highReturnReg, 0);
        }
    } else {
        generateTrg1Src1ImmInstruction(
            cg, is64bit ? TR::InstOpCode::cmpli8 : TR::InstOpCode::cmpli4, node, condReg2, byteLenRemainingRegister, 0);
...
TR::Instruction* OMR::Power::TreeEvaluator::generateHelperBranchAndLinkInstruction(
    TR_RuntimeHelper helperIndex, TR::Node* node, TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg)
{
    TR::SymbolReference* helperSym = cg->symRefTab()->findOrCreateRuntimeHelper(helperIndex);

    return generateDepImmSymInstruction(
        cg, TR::InstOpCode::bl, node, (uintptr_t)helperSym->getMethodAddress(), conditions, helperSym);
}
...
TR::Register* OMR::Power::TreeEvaluator::setmemoryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src1ImmInstruction(cg,
        lengthNode->getType().isInt32() ? TR::InstOpCode::cmpli4 : TR::InstOpCode::cmpli8, node, cndReg, lengthReg, 32);
    ...
    generateTrg1Src1ImmInstruction(cg, lengthNode->getType().isInt32() ? TR::InstOpCode::srawi : TR::InstOpCode::sradi,
        node, tempReg, lengthReg, 5);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, loopStartLabel);
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 8), valueReg);
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 8, 8), valueReg);
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 16, 8), valueReg);
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 24, 8), valueReg);
    ...
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::stw, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 4), valueReg);
    ...
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::sth, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 2), valueReg);
    ...
    generateMemSrc1Instruction(
        cg, TR::InstOpCode::stb, node, TR::MemoryReference::createWithDisplacement(cg, dstAddrReg, 0, 1), valueReg);
...
static TR::Register* inlineNumberOfTrailingZeros(
    TR::Node* node, TR::InstOpCode::Mnemonic op, int32_t subfconst, TR::CodeGenerator* cg)
{
...
static TR::Register* inlineLongNumberOfTrailingZeros(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src2Instruction(
        cg, TR::InstOpCode::andc, node, targetRegister, targetRegister, srcRegister->getHighOrder());
...
static TR::Register* inlineLongHighestOneBit(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, targetRegister, 0xffff8000u);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicr, node, targetRegister, targetRegister, 32, CONSTANT64(0x8000000000000000));
        ...
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::srw, node, targetRegister->getLowOrder(), targetRegister->getLowOrder(), tempRegister);
...
static TR::Register* inlineSimpleAtomicUpdate(
    TR::Node* node, bool isAddOp, bool isLong, bool isGetThenUpdate, TR::CodeGenerator* cg)
{
...
bool TR_PPCComputeCC::setCarryBorrow(
    TR::Node* flagNode, bool invertValue, TR::Register** flagReg, TR::CodeGenerator* cg)
{
    *flagReg = NULL;
...
void OMR::Power::TreeEvaluator::preserveTOCRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
    ...
    cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
        TR::MemoryReference::createWithDisplacement(
            cg, grSysStackReg, callerSaveTOCOffset, TR::Compiler->om.sizeofReferenceAddress()),
        grTOCReg, cursor);

    cg->setAppendInstruction(cursor);
}

void OMR::Power::TreeEvaluator::restoreTOCRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
...
TR::Register* OMR::Power::TreeEvaluator::retrieveTOCRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
...
TR::Register* OMR::Power::TreeEvaluator::sbyteswapEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (reverseLoadEnabled && !firstNonConversionOpCodeNode->getRegister()
        ...
        TR::LoadStoreHandler::generateLoadNodeSequence(
            cg, tgtRegister, firstNonConversionOpCodeNode, TR::InstOpCode::lhbrx, 2, true, extraOffset);
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcRegister, 24, CONSTANT64(0x00000000ff));
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcRegister, 8, CONSTANT64(0x000000ff00));
        } else {
            TR::Register* tmpRegister = cg->allocateRegister();

            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcRegister, 24, CONSTANT64(0x00000000ff));
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmpRegister, srcRegister, 8, CONSTANT64(0x000000ff00));
...
TR::Register* OMR::Power::TreeEvaluator::ibyteswapEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (reverseLoadEnabled && !firstChild->getRegister() && firstChild->getOpCode().isMemoryReference()
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcRegister, 24, CONSTANT64(0x00ffffff00));
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcRegister, 8, CONSTANT64(0x0000ff0000));
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcRegister, 8, CONSTANT64(0x00000000ff));
        } else {
            TR::Register* tmp1Register = cg->allocateRegister();

            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcRegister, 8, CONSTANT64(0x00000000ff));
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 8, CONSTANT64(0x0000ff0000));
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 24, CONSTANT64(0x000000ff00));
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister, 24, CONSTANT64(0x00ff000000));
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
...
TR::Register* OMR::Power::TreeEvaluator::lbyteswapEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->target().is64Bit()) {
        ...
        if (reverseLoadEnabled && comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P7) && !firstChild->getRegister()
            ...
            if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
                ...
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldicl, node, srcHRegister, srcLRegister, 32, CONSTANT64(0x00ffffffff));

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmpRegister, srcLRegister, 24, CONSTANT64(0x00ffffff00));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcHRegister, 24, CONSTANT64(0x00ffffff00));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwimi, node, tmpRegister, srcLRegister, 8, CONSTANT64(0x0000ff0000));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcHRegister, 8, CONSTANT64(0x0000ff0000));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwimi, node, tmpRegister, srcLRegister, 8, CONSTANT64(0x00000000ff));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwimi, node, tgtRegister, srcHRegister, 8, CONSTANT64(0x00000000ff));

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldimi, node, tgtRegister, tmpRegister, 32, CONSTANT64(0xffffffff00000000));
                ...
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcHRegister, 8, CONSTANT64(0x00000000ff));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tgtHRegister, srcLRegister, 8, CONSTANT64(0x00000000ff));

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcHRegister, 8, CONSTANT64(0x0000ff0000));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcLRegister, 8, CONSTANT64(0x0000ff0000));
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtHRegister, tgtHRegister, tmp2Register);

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcHRegister, 24, CONSTANT64(0x000000ff00));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcLRegister, 24, CONSTANT64(0x000000ff00));
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtHRegister, tgtHRegister, tmp2Register);

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcHRegister, 24, CONSTANT64(0x00ff000000));
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcLRegister, 24, CONSTANT64(0x00ff000000));
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
                generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtHRegister, tgtHRegister, tmp2Register);

                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldimi, node, tgtRegister, tgtHRegister, 32, CONSTANT64(0xffffffff00000000));
    ...
    } else // 32-Bit Target
    {
        ...
        if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            generateTrg1Src1Instruction(
                cg, TR::InstOpCode::brw, node, tgtRegister->getLowOrder(), srcRegister->getHighOrder());
            generateTrg1Src1Instruction(
                cg, TR::InstOpCode::brw, node, tgtRegister->getHighOrder(), srcRegister->getLowOrder());
        } else if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)) {
            ...
            generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcRegister->getLowOrder(),
                8, CONSTANT64(0x0000ff0000));
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::OR, node, tgtRegister->getLowOrder(), tgtRegister->getLowOrder(), tmp1Register);
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::OR, node, tgtRegister->getHighOrder(), tgtRegister->getHighOrder(), tmp2Register);
