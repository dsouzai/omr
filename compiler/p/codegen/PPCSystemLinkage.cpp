...
uint32_t mappedOffsetToFirstLocal(TR::CodeGenerator* cg, const TR::PPCLinkageProperties& linkage)
{
    TR::Machine* machine = cg->machine();

    uint32_t min_arg_area = machine->getLinkRegisterKilled() ? (cg->comp()->target().is64Bit() ? 8 * 8 : 8 * 4) : 0;
...
const TR::PPCLinkageProperties& TR::PPCSystemLinkage::getProperties() { return _properties; }

void TR::PPCSystemLinkage::initPPCRealRegisterLinkage()
{
    ...
    for (icount = TR::RealRegister::LastGPR; icount >= TR::RealRegister::gr14; icount--)
        machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(0xf000 - icount);

    for (icount = TR::RealRegister::FirstFPR; icount <= TR::RealRegister::fp13; icount++)
        machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(icount);

    for (icount = TR::RealRegister::LastFPR; icount >= TR::RealRegister::fp14; icount--)
        machine->getRealRegister((TR::RealRegister::RegNum)icount)->setWeight(0xf000 - icount);
...
uint32_t TR::PPCSystemLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }

bool TR::PPCSystemLinkage::hasToBeOnStack(TR::ParameterSymbol* parm)
{
    return (parm->getAssignedGlobalRegisterIndex() >= 0 && parm->isParmHasToBeOnStack());
}

static bool hasParmsPassedOnStack(List<TR::ParameterSymbol>& parmList)
{
...
void TR::PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
    ...
    // The 64-bit ELF V2 ABI Specification makes having a parameter save area optional if all parameters can be passed
    // in registers. As a result, we cannot use the caller's parameter save area on 64-bit Linux if all parameters were
    // passed in registers. However, we *must* use the caller's parameter save area if one or more parameters were
    // passed on the stack to load those values correctly.
    bool saveParmsInLocalArea
        = comp()->target().isLinux() && comp()->target().is64Bit() && !hasParmsPassedOnStack(parmList);
    ...
    if (linkage.getRightToLeft()) {
        while (parmCursor != NULL) {
            ...
            if (linkage.getSmallIntParmsAlignedRight() && parmCursor->getType().isIntegral()
                && parmCursor->getSize() < slot_size)
        ...
        while (parmCursor != NULL) {
            parmCursor->setParameterOffset(
                sizeOfParameterArea - offset_from_top - parmCursor->getSize() + stackIndex + offsetToFirstParm);
            offset_from_top += (parmCursor->getSize() + slot_size - 1) & (~(slot_size - 1));
            parmCursor = parameterIterator.getNext();
        }
    }
...
void TR::PPCSystemLinkage::mapStack(TR::ResolvedMethodSymbol* method)
{
    ...
    if (!method->isEHAware()) {
        while (
            regIndex <= TR::RealRegister::LastGPR && !machine->getRealRegister(regIndex)->getHasBeenAssignedInMethod())
    ...
    if (!method->isEHAware()) {
        while (
            regIndex <= TR::RealRegister::LastFPR && !machine->getRealRegister(regIndex)->getHasBeenAssignedInMethod())
            regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1);
    }
...
void TR::PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
    size_t align = 1;
...
void TR::PPCSystemLinkage::createPrologue(TR::Instruction* cursor)
{
...
void TR::PPCSystemLinkage::createPrologue(TR::Instruction* cursor, List<TR::ParameterSymbol>& parmList)
{
    TR::Machine* machine = cg()->machine();
    ...
    if (machine->getLinkRegisterKilled())
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
            TR::MemoryReference::createWithDisplacement(
                cg(), sp, 2 * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress()),
            gr0, cursor);
    ...
    if (!bodySymbol->isEHAware()) {
        while (savedFirst <= TR::RealRegister::LastFPR
            && !machine->getRealRegister(savedFirst)->getHasBeenAssignedInMethod())
            savedFirst = (TR::RealRegister::RegNum)((uint32_t)savedFirst + 1);
    }
    for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
         regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex - 1)) {
        argSize = argSize - 8;
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::stfd, firstNode,
            TR::MemoryReference::createWithDisplacement(cg(), sp, argSize, 8), machine->getRealRegister(regIndex),
            cursor);
    }

    savedFirst = TR::RealRegister::gr13;
    if (!bodySymbol->isEHAware()) {
        while (savedFirst <= TR::RealRegister::LastGPR
            && !machine->getRealRegister(savedFirst)->getHasBeenAssignedInMethod())
            savedFirst = (TR::RealRegister::RegNum)((uint32_t)savedFirst + 1);
    }
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (cg()->comp()->target().cpu.is(OMR_PROCESSOR_PPC_GP) || cg()->comp()->target().is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                 regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex - 1)) {
                argSize = argSize - TR::Compiler->om.sizeofReferenceAddress();
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    TR::MemoryReference::createWithDisplacement(
                        cg(), sp, argSize, TR::Compiler->om.sizeofReferenceAddress()),
            ...
            cursor = generateMemSrc1Instruction(cg(),
                (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::stw : TR::InstOpCode::stmw, firstNode,
                TR::MemoryReference::createWithDisplacement(
                    cg(), sp, argSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1)),
                machine->getRealRegister(savedFirst), cursor);
        }
    }

    if (bodySymbol->isEHAware() || machine->getRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod()
        ...
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
            TR::MemoryReference::createWithDisplacement(
                cg(), sp, TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress()),
            gr0, cursor);
    }
    ...
    if (size > self()->getOffsetToFirstParm() - argSize) {
        if (size > (-LOWER_IMMED)) {
            ...
            cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stux, firstNode,
                TR::MemoryReference::createWithIndexReg(cg(), sp, gr11, TR::Compiler->om.sizeofReferenceAddress()), sp,
                cursor);
        } else {
            cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, firstNode,
                TR::MemoryReference::createWithDisplacement(cg(), sp, -size, TR::Compiler->om.sizeofReferenceAddress()),
                sp, cursor);
        }
    } else // leaf routine
    {
...
void TR::PPCSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    TR::Machine* machine = cg()->machine();
    const TR::PPCLinkageProperties& properties = getProperties();
    TR::ResolvedMethodSymbol* bodySymbol = comp()->getJittedMethodSymbol();
    ...
    for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
         regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex - 1)) {
        saveSize = saveSize - 8;
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lfd, currentNode, machine->getRealRegister(regIndex),
            TR::MemoryReference::createWithDisplacement(cg(), sp, saveSize, 8), cursor);
    }
        ...
        if (cg()->comp()->target().cpu.is(OMR_PROCESSOR_PPC_GP) || cg()->comp()->target().is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                    ...
                    TR::MemoryReference::createWithDisplacement(
                        cg(), sp, saveSize, TR::Compiler->om.sizeofReferenceAddress()),
                    cursor);
            }
        else {
            ...
            cursor = generateTrg1MemInstruction(cg(),
                ...
                TR::MemoryReference::createWithDisplacement(
                    cg(), sp, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1)),
                cursor);
        }
    }
...
int32_t TR::PPCSystemLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)

{
    ...
    bool aix_style_linkage
        = (cg()->comp()->target().isAIX() || (cg()->comp()->target().is64Bit() && cg()->comp()->target().isLinux()));
        ...
        case TR::Address: // have to do something for GC maps here
            ...
            if (numIntegerArgs < properties.getNumIntArgRegs()) {
                ...
                if (numIntegerArgs == 0 && (resType.isAddress() || resType.isInt32() || resType.isInt64())) {
                    ...
                    TR::addDependency(
                        dependencies, argRegister, properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR, cg());
                }
            } else // numIntegerArgs >= properties.getNumIntArgRegs()
            {
                mref = getOutgoingArgumentMemRef(argSize, argRegister, TR::InstOpCode::Op_st, pushToMemory[argIndex++],
        ...
        case TR::Int64:
            ...
            if (!aix_style_linkage) {
                if (numIntegerArgs & 1) {
                    if (numIntegerArgs < properties.getNumIntArgRegs())
                        TR::addDependency(
                            dependencies, NULL, properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR, cg());
                    numIntegerArgs++;
                }
            }
            if (numIntegerArgs < properties.getNumIntArgRegs()) {
                if (!cg()->canClobberNodesRegister(child, 0)) {
                    if (cg()->comp()->target().is64Bit()) {
                        ...
                        argRegister
                            = cg()->allocateRegisterPair(argRegister->getRegisterPair()->getLowOrder(), tempRegister);
                ...
                if (numIntegerArgs == 0 && (resType.isAddress() || resType.isInt32() || resType.isInt64())) {
                    ...
                    if (cg()->comp()->target().is64Bit())
                        ...
                        dependencies->addPreCondition(
                            argRegister->getRegisterPair()->getHighOrder(), TR::RealRegister::gr3);
                    dependencies->addPostCondition(resultReg, TR::RealRegister::gr3);
                } else if (cg()->comp()->target().is32Bit() && numIntegerArgs == 1 && resType.isInt64()) {
                    TR::Register* resultReg = cg()->allocateRegister();
                    dependencies->addPreCondition(argRegister, TR::RealRegister::gr4);
                ...
                if (cg()->comp()->target().is32Bit()) {
                    if (numIntegerArgs < properties.getNumIntArgRegs() - 1) {
                        ...
                        if (numIntegerArgs == 0 && resType.isInt64()) {
                            TR::Register* resultReg = cg()->allocateRegister();
                            dependencies->addPreCondition(
                                argRegister->getRegisterPair()->getLowOrder(), TR::RealRegister::gr4);
                            dependencies->addPostCondition(resultReg, TR::RealRegister::gr4);
                        } else
                    ...
                    } else // numIntegerArgs == properties.getNumIntArgRegs()-1
                    {
                        mref = getOutgoingArgumentMemRef(argSize + 4, argRegister->getRegisterPair()->getLowOrder(),
                            TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
                    }
                    numIntegerArgs++;
                }
            } else // numIntegerArgs >= properties.getNumIntArgRegs()
                ...
                if (numFloatArgs < properties.getNumFloatArgRegs()) {
                    ...
                    if ((numFloatArgs == 0 && resType.isFloatingPoint())) {
                        ...
                        dependencies->addPostCondition(
                            resultReg, (r == 0) ? TR::RealRegister::fp1 : TR::RealRegister::fp2);
                    } else
                        TR::addDependency(
                            dependencies, argReg, properties.getFloatArgumentRegister(numFloatArgs), TR_FPR, cg());
                } else if (!aix_style_linkage)
                // numFloatArgs >= properties.getNumFloatArgRegs()
                {
                    mref
                        = getOutgoingArgumentMemRef(argSize, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4);
                    argSize += 4;
                }
                ...
                if (aix_style_linkage) {
                    ...
                    } else // numIntegerArgs >= properties.getNumIntArgRegs()
                    {
                        if (cg()->comp()->target().is64Bit() && cg()->comp()->target().isLinux()) {
                            mref = getOutgoingArgumentMemRef(
                                argSize + 4, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4);
                        } else {
                            mref = getOutgoingArgumentMemRef(
                                argSize, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4);
                        }
                    }
        ...
        case TR::Double:
            ...
            for (int r = 0; r < ((childType == TR::Double) ? 1 : 2); r++) {
                ...
                // numFloatArgs >= properties.getNumFloatArgRegs()
                {
                    argSize = (argSize + 4) & (~7);
                    mref
                        = getOutgoingArgumentMemRef(argSize, argReg, TR::InstOpCode::stfd, pushToMemory[argIndex++], 8);
                    argSize += 8;
                }
                    ...
                    if (numIntegerArgs < properties.getNumIntArgRegs()) {
                        ...
                        if (cg()->comp()->target().is32Bit()) {
                            if ((numIntegerArgs + 1) < properties.getNumIntArgRegs())
                                ...
                                mref = getOutgoingArgumentMemRef(
                                    argSize, argReg, TR::InstOpCode::stfd, pushToMemory[argIndex++], 8);
                            }
                        }
                    } else // numIntegerArgs >= properties.getNumIntArgRegs()
                    {
        ...
        default:
            ...
            if (numVectorArgs < properties.getNumVectorArgRegs()) {
                ...
                if (numVectorArgs == 0) {
                    ...
                    TR::addDependency(
                        dependencies, argReg, properties.getVectorArgumentRegister(numVectorArgs), TR_VRF, cg());
            } else {
    ...
    while (numIntegerArgs < properties.getNumIntArgRegs()) {
        if (numIntegerArgs == 0 && resType.isAddress()) {
            ...
            dependencies->addPostCondition(
                cg()->allocateCollectedReferenceRegister(), properties.getIntegerArgumentRegister(0));
        } else {
    ...
    if (memArgs > 0) {
        for (argIndex = 0; argIndex < memArgs; argIndex++) {
            ...
            generateMemSrc1Instruction(
                cg(), pushToMemory[argIndex].opCode, callNode, pushToMemory[argIndex].argMemory, aReg);
...
void TR::PPCSystemLinkage::buildDirectCall(TR::Node* callNode, TR::SymbolReference* callSymRef,
    ...
    if (aix_style_linkage) {
        ...
        if (cg()->comp()->target().cpu.isBigEndian()) {
            ...
            if (!cg()->comp()->getOption(TR_DisableTOC) && !cg()->comp()->compilePortableCode())
                ...
                loadAddressConstant(cg(), callNode, (int64_t)runtimeHelperValue((TR_RuntimeHelper)refNum), geReg, NULL,
                    false, TR_AbsoluteHelperAddress);
        }
    }

    generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode, (uintptr_t)callSymbol->getMethodAddress(),
...
TR::Register* TR::PPCSystemLinkage::buildDirectDispatch(TR::Node* callNode)
{
    ...
    case TR::call:
        ...
        break;
    default:
        if (callNode->getOpCode().isVectorOpCode() && callNode->getOpCode().getVectorOperation() == TR::vcall) {
            returnRegister = dependencies->searchPostConditionRegister(pp.getVectorReturnRegister());
            break;
        }
        returnRegister = NULL;
...
void TR::PPCSystemLinkage::buildVirtualDispatch(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, uint32_t sizeOfArguments)
{
    bool aix_style_linkage
        = (cg()->comp()->target().isAIX() || (cg()->comp()->target().is64Bit() && cg()->comp()->target().isLinux()));
    TR_ASSERT(callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isComputed(),
        "system linkage only supports computed indirect call for now %p\n", callNode);
    ...
    TR_ASSERT((grTOCReg != NULL),
        "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on Library TOC register gr2.");
    TR_ASSERT((grSysStackReg != NULL),
        "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on Native C Stack register gr1.");
    ...
    if (cg()->comp()->target().cpu.isBigEndian()) {
        ...
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
            TR::MemoryReference::createWithDisplacement(
                cg(), callNode->getChild(0)->getRegister(), 0, TR::Compiler->om.sizeofReferenceAddress()));
    ...
    if (cg()->comp()->target().cpu.isBigEndian())
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, grTOCReg,
            TR::MemoryReference::createWithDisplacement(cg(), callNode->getChild(0)->getRegister(),
                TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress()));
...
void TR::PPCSystemLinkage::setParameterLinkageRegisterIndex(
    TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
