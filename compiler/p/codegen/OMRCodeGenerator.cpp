...
TR::Instruction *OMR::Power::CodeGenerator::generateSwitchToInterpreterPrePrologue(TR::Instruction *cursor,
    TR::Node *node)
{
...
bool OMR::Power::CodeGenerator::mulDecompositionCostIsJustified(int numOfOperations, char bitPosition[],
...
void OMR::Power::CodeGenerator::findOrCreateFloatConstant(void *v, TR::DataType t, TR::Instruction *n0,
    TR::Instruction *n1, TR::Instruction *n2, TR::Instruction *n3)
{
...
void OMR::Power::CodeGenerator::simulateNodeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
...
TR_GlobalRegisterNumber OMR::Power::CodeGenerator::getLinkageGlobalRegisterNumber(int8_t linkageRegisterIndex,
    TR::DataType type)
{
...
void OMR::Power::CodeGenerator::addMetaDataForLoadAddressConstantFixed(TR::Node *node,
    TR::Instruction *firstInstruction, TR::Register *tempReg, int16_t typeAddress, intptr_t value)
{
    if (value == 0x0)
        return;
    ...
    switch (typeAddress) {
        case TR_DataAddress: {
            ...
            relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
                (uint8_t *)recordInfo, TR_DataAddress, self());
            break;
        }
        ...
        case TR_BlockFrequency: {
            ...
            relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
                (uint8_t *)recordInfo, TR_BlockFrequency, self());
            break;
        }
        ...
        case TR_CallsiteTableEntryAddress: {
            relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
                (uint8_t *)node->getSymbolReference(), (uint8_t *)seqKind, TR_CallsiteTableEntryAddress, self());
            break;
        }

        case TR_MethodTypeTableEntryAddress: {
            relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
                (uint8_t *)node->getSymbolReference(), (uint8_t *)seqKind, TR_MethodTypeTableEntryAddress, self());
            break;
        }
    }
    ...
    if (comp->getOption(TR_UseSymbolValidationManager) && !relo) {
        switch (typeAddress) {
            case TR_ClassAddress: {
                ...
                recordInfo->data3 = (uintptr_t)seqKind;
                relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
                    (uint8_t *)recordInfo, TR_DiscontiguousSymbolFromManager, self());
                break;
            }
    ...
    if (!relo) {
        relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
            (uint8_t *)value, (uint8_t *)seqKind, (TR_ExternalRelocationTargetKind)typeAddress, self());
    }
    ...
    TR::Register *trgReg, TR::Instruction *cursor, TR::Register *tempReg, int16_t typeAddress, bool doAOTRelocation)
{
    ...
    if (tempReg == NULL) {
        ...
        cursor = firstInstruction = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, trgReg,
            canEmitData ? (value >> 48) : 0, cursor);

        // ori trgReg, trgReg, next 16-bits
        cursor = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::ori, node, trgReg, trgReg,
            canEmitData ? ((value >> 32) & 0x0000ffff) : 0, cursor);
        // shiftli trgReg, trgReg, 32
        cursor = generateTrg1Src1Imm2Instruction(self(), TR::InstOpCode::rldicr, node, trgReg, trgReg, 32,
            CONSTANT64(0xFFFFFFFF00000000), cursor);
        // oris trgReg, trgReg, next 16-bits
        cursor = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::oris, node, trgReg, trgReg,
            canEmitData ? ((value >> 16) & 0x0000ffff) : 0, cursor);
        // ori trgReg, trgReg, last 16-bits
        cursor = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::ori, node, trgReg, trgReg,
            canEmitData ? (value & 0x0000ffff) : 0, cursor);
    } else {
        // lis tempReg, bits[0-15]
        cursor = firstInstruction = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, tempReg,
            canEmitData ? (value >> 48) : 0, cursor);

        // lis trgReg, bits[32-47]
        cursor = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, trgReg,
            canEmitData ? ((int16_t)(value >> 16)) : 0, cursor);
        ...
        cursor = generateTrg1Src1Imm2Instruction(self(), TR::InstOpCode::rldimi, node, trgReg, tempReg, 32,
            CONSTANT64(0xFFFFFFFF00000000), cursor);
    }
    ...
    TR::Instruction *secondInstruction, int16_t typeAddress, int32_t value)
{
    ...
    if (typeAddress == TR_DataAddress) {
        ...
        self()->addExternalRelocation(new (self()->trHeapMemory())
                                          TR::ExternalOrderedPair32BitRelocation((uint8_t *)firstInstruction,
                                              (uint8_t *)secondInstruction, (uint8_t *)recordInfo,
        ...
        TR::DebugCounter::generateRelocation(comp, firstInstruction, secondInstruction, node, counter,
            orderedPairSequence2);
        ...
        self()->addExternalRelocation(new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
                                          (uint8_t *)firstInstruction, (uint8_t *)secondInstruction,
                                          (uint8_t *)recordInfo, (TR_ExternalRelocationTargetKind)typeAddress, self()),
            __FILE__, __LINE__, node);
...
//
TR::Instruction *OMR::Power::CodeGenerator::loadIntConstantFixed(TR::Node *node, int32_t value, TR::Register *trgReg,
    TR::Instruction *cursor, int16_t typeAddress)
{
    TR::Instruction *temp = cursor;
...
void OMR::Power::CodeGenerator::addMetaDataFor32BitFixedLoadLabelAddressIntoReg(TR::Node *node, TR::LabelSymbol *label,
    TR::Instruction *firstInstruction, TR::Instruction *secondInstruction)
{
    TR::Compilation *comp = self()->comp();
    ...
    self()->getAheadOfTimeCompile()->getRelocationList().push_front(
        new (self()->trHeapMemory()) TR::PPCPairedRelocation(firstInstruction, secondInstruction, (uint8_t *)recordInfo,
...
void OMR::Power::CodeGenerator::addMetaDataFor64BitFixedLoadLabelAddressIntoReg(TR::Node *node, TR::LabelSymbol *label,
    TR::Register *tempReg, TR::Instruction **q)
{
    ...
    self()->addExternalRelocation(new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(q[0],
                                      (uint8_t *)(label), (uint8_t *)(tempReg ? fixedSequence4 : fixedSequence2),
    ...
    TR::LabelSymbol *label, TR::Instruction *cursor, TR::Register *tempReg, bool useADDISFor32Bit)
{
    if (self()->comp()->target().is64Bit()) {
        ...
        if (self()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            generateTrg1MemInstruction(self(), TR::InstOpCode::paddi, node, trgReg,
                TR::MemoryReference::createWithLabel(self(), label, 0, 0));
        } else {
...
void TR_PPCScratchRegisterManager::addScratchRegistersToDependencyList(TR::RegisterDependencyConditions *deps,
    bool excludeGPR0)
{
...
TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcReg, int64_t value, TR::Register *trgReg,
    TR::CodeGenerator *cg)
{
    if (!trgReg)
...
TR::Register *addConstantToInteger(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t value,
    TR::CodeGenerator *cg)
{
