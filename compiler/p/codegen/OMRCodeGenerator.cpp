...
void OMR::Power::CodeGenerator::initialize()
{
    ...
    // disabled for now
    //
    if (comp->getOption(TR_AggressiveOpts) && !comp->getOption(TR_DisableArraySetOpts)) {
    ...
    if (!disableDCAS && comp->target().is64Bit() && TR::Options::useCompressedPointers()) {
    ...
    /*
     * TODO: TM is currently not compatible with read barriers. If read barriers are required, TM is disabled until the
     * issue is fixed. TM is now disabled by default, due to various reasons (OS, hardware, etc), unless it is
     * explicitly enabled.
     */
    ...
    if (comp->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_ALTIVEC)
...
TR_PPCScratchRegisterManager* OMR::Power::CodeGenerator::generateScratchRegisterManager(int32_t capacity)
{
...
//
void OMR::Power::CodeGenerator::ppcCGOnClassUnloading(void* loaderPtr)
{
...
TR::Instruction* OMR::Power::CodeGenerator::generateSwitchToInterpreterPrePrologue(
    TR::Instruction* cursor, TR::Node* node)
{
    ...
    TR::SymbolReference* revertToInterpreterSymRef
        = self()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCrevertToInterpreterGlue);
    ...
    cursor = generateDepImmSymInstruction(self(), TR::InstOpCode::bl, node,
    ...
    if (self()->comp()->target().is64Bit()) {
        ...
        cursor = generateImmInstruction(
            self(), TR::InstOpCode::dd, node, self()->comp()->target().cpu.isBigEndian() ? lowBits : highBits, cursor);
    } else {
    ...
    if (self()->comp()->target().is64Bit()) {
        ...
        cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
            self()->comp()->target().cpu.isBigEndian() ? highBits : lowBits, TR_AbsoluteHelperAddress, helperSymRef,
            cursor);
        cursor = generateImmInstruction(
            self(), TR::InstOpCode::dd, node, self()->comp()->target().cpu.isBigEndian() ? lowBits : highBits, cursor);
    } else {
        cursor = generateImmInstruction(
            self(), TR::InstOpCode::dd, node, (int32_t)helperAddr, TR_AbsoluteHelperAddress, helperSymRef, cursor);
    }
...
void OMR::Power::CodeGenerator::beginInstructionSelection()
{
    ...
    if ((self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private)) {
        _returnTypeInfoInstruction
            = new (self()->trHeapMemory()) TR::PPCImmInstruction(TR::InstOpCode::dd, firstNode, 0, NULL, self());
    }

    generateAdminInstruction(self(), TR::InstOpCode::proc, firstNode);
}

void OMR::Power::CodeGenerator::endInstructionSelection()
{
...
void OMR::Power::CodeGenerator::staticTracking(TR::SymbolReference* symRef)
{
    if (self()->comp()->target().is64Bit()) {
...
void OMR::Power::CodeGenerator::itemTracking(int32_t offset, TR::LabelSymbol* label)
{
    if (self()->comp()->target().is64Bit()) {
...
bool OMR::Power::CodeGenerator::mulDecompositionCostIsJustified(
    int numOfOperations, char bitPosition[], char operationType[], int64_t value)
{
...
TR::CodeGenerator::TR_RegisterPressureSummary* OMR::Power::CodeGenerator::calculateRegisterPressure() { return NULL; }

TR::Linkage* OMR::Power::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
...
void OMR::Power::CodeGenerator::expandInstructions()
{
    _binaryEncodingData.estimate = 0;
    self()->generateBinaryEncodingPrologue(&_binaryEncodingData);

    generateLabelInstruction(self(), TR::InstOpCode::label, self()->comp()->getStartTree()->getNode(),
        self()->getStartPCLabel(), _binaryEncodingData.preProcInstruction);
...
void OMR::Power::CodeGenerator::generateBinaryEncodingPrologue(TR_PPCBinaryEncodingData* data)
{
    TR::Compilation* comp = self()->comp();
    data->recomp = NULL;
    ...
    while (data.cursorInstruction) {
        ...
        if (data.cursorInstruction == data.preProcInstruction) {
            self()->setPrePrologueSize(self()->getBinaryBufferCursor() - self()->getBinaryBufferStart()
                - self()->getJitMethodEntryPaddingSize());
            self()
                ->comp()
                ->getSymRefTab()
...
TR::Register* OMR::Power::CodeGenerator::gprClobberEvaluate(TR::Node* node)
{
    ...
    TR_ASSERT(resultReg->getKind() == TR_GPR || resultReg->getKind() == TR_FPR,
        "gprClobberEvaluate() called for non-GPR or non-FPR.");
    ...
    if (!self()->canClobberNodesRegister(node)) {
        if (self()->comp()->target().is32Bit() && node->getType().isInt64()) {
            ...
            generateTrg1Src1Instruction(self(), TR::InstOpCode::mr, node, lowReg, resultReg->getLowOrder());

            generateTrg1Src1Instruction(self(), TR::InstOpCode::mr, node, highReg, resultReg->getHighOrder());
            return longReg;
        } else {
            ...
            generateTrg1Src1Instruction(self(),
                resultReg->getKind() == TR_GPR ? TR::InstOpCode::mr : TR::InstOpCode::fmr, node, targetReg, resultReg);
            return targetReg;
        }
    } else {
        return resultReg;
...
bool OMR::Power::CodeGenerator::canTransformUnsafeSetMemory() { return (self()->comp()->target().is64Bit()); }
...
void OMR::Power::CodeGenerator::findOrCreateFloatConstant(
    void* v, TR::DataType t, TR::Instruction* n0, TR::Instruction* n1, TR::Instruction* n2, TR::Instruction* n3)
{
...
void OMR::Power::CodeGenerator::findOrCreateAddressConstant(void* v, TR::DataType t, TR::Instruction* n0,
    TR::Instruction* n1, TR::Instruction* n2, TR::Instruction* n3, TR::Node* node, bool isUnloadablePicSite)
{
    if (_constantData == NULL)
        _constantData = new (self()->trHeapMemory()) TR::ConstantDataSnippet(self());
...
void OMR::Power::CodeGenerator::emitDataSnippets() { self()->setBinaryBufferCursor(_constantData->emitSnippetBody()); }
...
void OMR::Power::CodeGenerator::simulateNodeEvaluation(
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    TR::Node* immediateChild
        = (node->getNumChildren() == 2 && !node->getOpCode().isCall() && !node->getOpCode().isLoad())
        ? node->getSecondChild()
        : NULL;
    if (immediateChild && immediateChild->getOpCode().isLoadConst()
        && (immediateChild->getType().isIntegral() && !immediateChild->getType().isInt64()
            || immediateChild->getType().isAddress())) {
...
TR_GlobalRegisterNumber OMR::Power::CodeGenerator::pickRegister(TR::RegisterCandidate* regCan, TR::Block** barr,
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getOpCodeValue() == TR::table) {
            ...
            if (total <= 9) {
                for (ii = 3; ii < total
                     && (node->getChild(ii)->getCaseConstant() - node->getChild(ii - 1)->getCaseConstant())
                         <= UPPER_IMMED
                     && (node->getChild(ii)->getCaseConstant() - node->getChild(ii - 1)->getCaseConstant())
        ...
        return (self()->comp()->target().is64Bit()
                ? _numGPR
                : _numGPR - 4); // 4 GPRs are needed for long-compare arguments themselves.
    }
...
TR_GlobalRegisterNumber OMR::Power::CodeGenerator::getLinkageGlobalRegisterNumber(
...
void OMR::Power::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label)
{
    ...
    TR_ASSERT_FATAL((*cursor & 0x0000fffcu) == 0u,
        "Attempt to relocate into an instruction with existing data in the distance field");
...
{
    TR_ASSERT_FATAL(label->getCodeLocation(), "Attempt to relocate a label with a NULL address");
    TR_ASSERT_FATAL((*cursor & 0x03fffffcu) == 0u,
        "Attempt to relocate into an instruction with existing data in the distance field");
...
void OMR::Power::CodeGenerator::setRealRegisterAssociation(TR::Register* reg, TR::RealRegister::RegNum realNum)
{
...
void OMR::Power::CodeGenerator::addRealRegisterInterference(TR::Register* reg, TR::RealRegister::RegNum realNum)
{
    if (!reg->isLive() || realNum == TR::RealRegister::NoReg)
        return;
    TR::RealRegister* realReg = self()->machine()->getRealRegister(realNum);
...
TR_BackingStore* OMR::Power::CodeGenerator::allocateStackSlot()
{
    ...
    if (tempElement->getData() != NULL) {
        ...
        return tempElement->setData(
            new (self()->trHeapMemory()) TR_BackingStore(self()->comp()->getSymRefTab(), spillSymbol, 0));
    }
}
...
bool OMR::Power::CodeGenerator::isRotateAndMask(TR::Node* node)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL &&
        ...
        (secondOp == TR::iconst && contiguousBits(secondChild->getInt()) && firstChild->getReferenceCount() == 1
            && firstChild->getRegister() == NULL
            && ((firstOp == TR::imul && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
                    && firstChild->getSecondChild()->getInt() > 0
                    && isNonNegativePowerOf2(firstChild->getSecondChild()->getInt()))
                || ((firstOp == TR::ishr || firstOp == TR::iushr)
                    && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
                    && firstChild->getSecondChild()->getInt() > 0
                    && (leadingZeroes(secondChild->getInt()) >= firstChild->getSecondChild()->getInt())))))
        return true;
    else
        return false;
}
...
TR::Instruction* OMR::Power::CodeGenerator::generateDebugCounterBump(
    TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR::RegisterDependencyConditions* cond)
{
    ...
    cursor = generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, counterReg,
        TR::MemoryReference::createWithDisplacement(self(), addrReg, 0, 4), cursor);
    ...
    cursor = generateMemSrc1Instruction(self(), TR::InstOpCode::stw, node,
        TR::MemoryReference::createWithDisplacement(self(), addrReg, 0, 4), counterReg, cursor);
    if (cond) {
...
TR::Instruction* OMR::Power::CodeGenerator::generateDebugCounterBump(TR::Instruction* cursor,
...
bool OMR::Power::CodeGenerator::canEmitDataForExternallyRelocatableInstructions()
{
...
int32_t OMR::Power::CodeGenerator::getPreferredLoopUnrollFactor()
{
    return self()->comp()->getMethodHotness() > warm ? (self()->comp()->isOptServer() ? 12 : 8) : 4;
}

bool OMR::Power::CodeGenerator::needsNormalizationBeforeShifts() { return true; }

uint32_t OMR::Power::CodeGenerator::registerBitMask(int32_t reg)
{
    return 1 << (31 - (reg - TR::RealRegister::FirstGPR));
}

bool OMR::Power::CodeGenerator::doRematerialization() { return true; }

void OMR::Power::CodeGenerator::freeAndResetTransientLongs()
{
    int32_t num_lReg;
...
bool OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::CPU* cpu, TR::ILOpCode opcode)
{
    ...
    // alignment issues
    if (!cpu->isAtLeast(OMR_PROCESSOR_PPC_P8) && et != TR::Double && et != TR::Int64)
        return false;
...
bool OMR::Power::CodeGenerator::getSupportsEncodeUtf16LittleWithSurrogateTest()
{
    return self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX)
        && !self()->comp()->getOption(TR_DisableSIMDUTF16LEEncoder);
}

bool OMR::Power::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
    return self()->comp()->target().cpu.supportsFeature(OMR_FEATURE_PPC_HAS_VSX)
        && !self()->comp()->getOption(TR_DisableSIMDUTF16BEEncoder);
}

void OMR::Power::CodeGenerator::addMetaDataForLoadAddressConstantFixed(
    TR::Node* node, TR::Instruction* firstInstruction, TR::Register* tempReg, int16_t typeAddress, intptr_t value)
{
    if (value == 0x0)
        return;
    ...
    case TR_DataAddress: {
        ...
        relo = new (self()->trHeapMemory())
            TR::BeforeBinaryEncodingExternalRelocation(firstInstruction, (uint8_t*)recordInfo, TR_DataAddress, self());
        break;
    }
    ...
    case TR_BlockFrequency: {
        ...
        relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
            firstInstruction, (uint8_t*)recordInfo, TR_BlockFrequency, self());
        break;
    }
    ...
    case TR_MethodEnterExitHookAddress: {
        relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(firstInstruction,
            (uint8_t*)node->getSymbolReference(), (uint8_t*)seqKind, TR_MethodEnterExitHookAddress, self());
        break;
    }
    }
    ...
    if (comp->getOption(TR_UseSymbolValidationManager) && !relo) {
        ...
        case TR_ClassAddress: {
            ...
            recordInfo->data3 = (uintptr_t)seqKind;
            relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                firstInstruction, (uint8_t*)recordInfo, TR_DiscontiguousSymbolFromManager, self());
            break;
        }
        ...
        case TR_RamMethodSequence: {
            TR_RelocationRecordInformation* recordInfo
                = (TR_RelocationRecordInformation*)comp->trMemory()->allocateMemory(
                    sizeof(TR_RelocationRecordInformation), heapAlloc);
            recordInfo->data1
                = (uintptr_t)self()->comp()->getJittedMethodSymbol()->getResolvedMethod()->resolvedMethodAddress();
    ...
    if (!relo) {
        relo = new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
            firstInstruction, (uint8_t*)value, (uint8_t*)seqKind, (TR_ExternalRelocationTargetKind)typeAddress, self());
    }

    self()->addExternalRelocation(relo, __FILE__, __LINE__, node);
}

TR::Instruction* OMR::Power::CodeGenerator::loadAddressConstantFixed(TR::Node* node, intptr_t value,
    TR::Register* trgReg, TR::Instruction* cursor, TR::Register* tempReg, int16_t typeAddress, bool doAOTRelocation)
{
    TR::Compilation* comp = self()->comp();
    ...
    if (tempReg == NULL) {
        ...
        cursor = firstInstruction = generateTrg1ImmInstruction(
            self(), TR::InstOpCode::lis, node, trgReg, canEmitData ? (value >> 48) : 0, cursor);

        // ori trgReg, trgReg, next 16-bits
        cursor = generateTrg1Src1ImmInstruction(
            self(), TR::InstOpCode::ori, node, trgReg, trgReg, canEmitData ? ((value >> 32) & 0x0000ffff) : 0, cursor);
        // shiftli trgReg, trgReg, 32
        cursor = generateTrg1Src1Imm2Instruction(
            self(), TR::InstOpCode::rldicr, node, trgReg, trgReg, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
        // oris trgReg, trgReg, next 16-bits
        cursor = generateTrg1Src1ImmInstruction(
            self(), TR::InstOpCode::oris, node, trgReg, trgReg, canEmitData ? ((value >> 16) & 0x0000ffff) : 0, cursor);
        // ori trgReg, trgReg, last 16-bits
        cursor = generateTrg1Src1ImmInstruction(
            self(), TR::InstOpCode::ori, node, trgReg, trgReg, canEmitData ? (value & 0x0000ffff) : 0, cursor);
    } else {
        // lis tempReg, bits[0-15]
        cursor = firstInstruction = generateTrg1ImmInstruction(
            self(), TR::InstOpCode::lis, node, tempReg, canEmitData ? (value >> 48) : 0, cursor);

        // lis trgReg, bits[32-47]
        cursor = generateTrg1ImmInstruction(
            self(), TR::InstOpCode::lis, node, trgReg, canEmitData ? ((int16_t)(value >> 16)) : 0, cursor);
        // ori tempReg, tempReg, bits[16-31]
        cursor = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::ori, node, tempReg, tempReg,
            canEmitData ? ((value >> 32) & 0x0000ffff) : 0, cursor);
        // ori trgReg, trgReg, bits[48-63]
        cursor = generateTrg1Src1ImmInstruction(
            self(), TR::InstOpCode::ori, node, trgReg, trgReg, canEmitData ? (value & 0x0000ffff) : 0, cursor);
        // rldimi trgReg, tempReg, 32, 0
        cursor = generateTrg1Src1Imm2Instruction(
            self(), TR::InstOpCode::rldimi, node, trgReg, tempReg, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
    }
...
void OMR::Power::CodeGenerator::addMetaDataForLoadIntConstantFixed(TR::Node* node, TR::Instruction* firstInstruction,
    TR::Instruction* secondInstruction, int16_t typeAddress, int32_t value)
{
    ...
    if (typeAddress == TR_DataAddress) {
        ...
        self()->addExternalRelocation(new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
                                          (uint8_t*)firstInstruction, (uint8_t*)secondInstruction, (uint8_t*)recordInfo,
        ...
        TR::DebugCounter::generateRelocation(
            comp, firstInstruction, secondInstruction, node, counter, orderedPairSequence2);
        ...
        self()->addExternalRelocation(
            new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation((uint8_t*)firstInstruction,
                (uint8_t*)secondInstruction, (uint8_t*)node->getSymbolReference(), (uint8_t*)orderedPairSequence2,
...
//
TR::Instruction* OMR::Power::CodeGenerator::loadIntConstantFixed(
    TR::Node* node, int32_t value, TR::Register* trgReg, TR::Instruction* cursor, int16_t typeAddress)
{
    ...
    cursor = firstInstruction
        = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, trgReg, value >> 16, cursor);
    cursor = secondInstruction
        = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::ori, node, trgReg, trgReg, value & 0x0000ffff, cursor);
...
void OMR::Power::CodeGenerator::addMetaDataFor32BitFixedLoadLabelAddressIntoReg(
    TR::Node* node, TR::LabelSymbol* label, TR::Instruction* firstInstruction, TR::Instruction* secondInstruction)
{
    TR::Compilation* comp = self()->comp();
    ...
    self()->getAheadOfTimeCompile()->getRelocationList().push_front(
        new (self()->trHeapMemory()) TR::PPCPairedRelocation(
            firstInstruction, secondInstruction, (uint8_t*)recordInfo, TR_AbsoluteMethodAddressOrderedPair, node));

    self()->addRelocation(new (self()->trHeapMemory())
            TR::PPCPairedLabelAbsoluteRelocation(firstInstruction, secondInstruction, NULL, NULL, label));
}

void OMR::Power::CodeGenerator::addMetaDataFor64BitFixedLoadLabelAddressIntoReg(
    TR::Node* node, TR::LabelSymbol* label, TR::Register* tempReg, TR::Instruction** q)
{

    if (self()->canEmitDataForExternallyRelocatableInstructions()) {
        self()->addRelocation(
            new (self()->trHeapMemory()) TR::PPCPairedLabelAbsoluteRelocation(q[0], q[1], q[2], q[3], label));
    }

    self()->addExternalRelocation(
        new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(q[0], (uint8_t*)(label),
            (uint8_t*)(tempReg ? fixedSequence4 : fixedSequence2), TR_FixedSequenceAddress, self()),
        __FILE__, __LINE__, node);
}

TR::Instruction* OMR::Power::CodeGenerator::fixedLoadLabelAddressIntoReg(TR::Node* node, TR::Register* trgReg,
    TR::LabelSymbol* label, TR::Instruction* cursor, TR::Register* tempReg, bool useADDISFor32Bit)
{
    if (self()->comp()->target().is64Bit()) {
        ...
        if (self()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            generateTrg1MemInstruction(
                self(), TR::InstOpCode::paddi, node, trgReg, TR::MemoryReference::createWithLabel(self(), label, 0, 0));
        } else {
            ...
            if (offset != PTOC_FULL_INDEX) {
                ...
                if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                    TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != self()->hiValue(offset),
                        "TOC offset (0x%x) is unexpectedly high. Can not encode upper 16 bits into an addis "
                        "instruction.",
                        offset);
                    generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addis, node, trgReg,
                        self()->getTOCBaseRegister(), self()->hiValue(offset));
                    generateTrg1MemInstruction(self(), TR::InstOpCode::Op_load, node, trgReg,
                        TR::MemoryReference::createWithDisplacement(self(), trgReg, LO_VALUE(offset), 8));
                } else {
                    generateTrg1MemInstruction(self(), TR::InstOpCode::Op_load, node, trgReg,
                        TR::MemoryReference::createWithDisplacement(self(), self()->getTOCBaseRegister(), offset, 8));
                }
            } else {
                TR::Instruction* q[4];
    ...
    TR::RegisterDependencyConditions* deps, bool excludeGPR0)
{
    ...
    while (msr) {
        ...
        if (excludeGPR0 && msr->_reg->getKind() == TR_GPR) {
            TR_ASSERT(deps->getPostConditions()->getRegisterDependency(deps->getAddCursorForPost() - 1)->getRegister()
                    == msr->_reg,
                "Virtual register for last post condition dependency is not the expected one");
...
{
    /*
     * If the target is 64 bit, the CPU must also be 64 bit (even if we don't specifically know which Power CPU it is)
     * so this can just return true. If the target is not 64 bit, we need to check the CPU properties to try and find
     * out if the CPU is 64 bit or not.
     */
    if (self()->comp()->target().is64Bit()) {
        return true;
    } else {
...
bool OMR::Power::CodeGenerator::supportsNonHelper(TR::SymbolReferenceTable::CommonNonhelperSymbol symbol)
{
    bool result = false;
...
uint32_t OMR::Power::CodeGenerator::getJitMethodEntryAlignmentBoundary() { return 128; }

bool OMR::Power::CodeGenerator::directCallRequiresTrampoline(intptr_t targetAddress, intptr_t sourceAddress)
{
    return !self()->comp()->target().cpu.isTargetWithinIFormBranchRange(targetAddress, sourceAddress)
        || self()->comp()->getOption(TR_StressTrampolines);
}

uint32_t OMR::Power::CodeGenerator::getHotLoopAlignment()
{
...
TR::LabelSymbol* OMR::Power::CodeGenerator::getStartPCLabel()
{
    if (!_startPCLabel)
...
TR::Register* addConstantToLong(
    TR::Node* node, TR::Register* srcReg, int64_t value, TR::Register* trgReg, TR::CodeGenerator* cg)
{
    if (!trgReg)
...
TR::Register* addConstantToLong(TR::Node* node, TR::Register* srcHighReg, TR::Register* srclowReg, int32_t highValue,
    int32_t lowValue, TR::CodeGenerator* cg)
{
...
TR::Register* addConstantToInteger(
    TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t value, TR::CodeGenerator* cg)
{
