 ...
 */
enum class CompareCondition { eq, ne, lt, ge, gt, le };
...
struct CRCompareCondition {
    ...
    CRCompareCondition(TR::RealRegister::CRCC crcc, bool isReversed)
        : crcc(crcc)
...
struct CompareInfo {
    ...
    CompareInfo(CompareCondition cond, TR::DataTypes type, bool isUnsigned, bool isUnorderedTrue)
        ...
        , isUnorderedTrue(isUnorderedTrue)
    { }
};
 ...
 */
bool is16BitSignedImmediate(int64_t value) { return value >= -0x8000 && value < 0x8000; }
 ...
 */
bool is16BitUnsignedImmediate(uint64_t value) { return value < 0x10000; }
 ...
 *    was placed.
 */
CompareCondition evaluateDualIntCompareToConditionRegister(TR::Register* condReg, TR::Node* node, TR::Node* firstChild,
    TR::Node* secondChild, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && !secondChild->getRegister()
        ...
        if (compareInfo.isUnsigned && is16BitUnsignedImmediate(secondHi)) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpli4, node, condReg, firstReg->getHighOrder(), secondHi);
        } else if (!compareInfo.isUnsigned && is16BitSignedImmediate(secondHi)) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpi4, node, condReg, firstReg->getHighOrder(), secondHi);
        } else {
            TR::Register* secondHiReg = cg->allocateRegister();

            loadConstant(cg, node, secondHi, secondHiReg);
            generateTrg1Src2Instruction(cg, compareInfo.isUnsigned ? TR::InstOpCode::cmpl4 : TR::InstOpCode::cmp4, node,
                condReg, firstReg->getHighOrder(), secondHiReg);
        ...
        if (is16BitUnsignedImmediate(secondLo)) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpli4, node, condReg2, firstReg->getLowOrder(), secondLo);
        } else {
            TR::Register* secondLoReg = cg->allocateRegister();

            loadConstant(cg, node, secondLo, secondLoReg);
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::cmpl4, node, condReg2, firstReg->getHighOrder(), secondLoReg);
        ...
        generateTrg1Src2Instruction(cg, compareInfo.isUnsigned ? TR::InstOpCode::cmpl4 : TR::InstOpCode::cmp4, node,
            condReg, firstReg->getHighOrder(), secondReg->getHighOrder());
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::cmpl4, node, condReg2, firstReg->getLowOrder(), secondReg->getLowOrder());
    }
    ...
    case CompareCondition::eq:
        ...
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::crand, node, condReg, condReg2, condReg,
            (TR::RealRegister::CRCC_EQ << TR::RealRegister::pos_RT)
                | (TR::RealRegister::CRCC_EQ << TR::RealRegister::pos_RA)
                | (TR::RealRegister::CRCC_EQ << TR::RealRegister::pos_RB));
        break;
    ...
    case CompareCondition::lt:
        ...
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::crand, node, condReg2, condReg, condReg2,
            (TR::RealRegister::CRCC_EQ << TR::RealRegister::pos_RT)
                | (TR::RealRegister::CRCC_EQ << TR::RealRegister::pos_RA)
                | (TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RB));
 ...
 */
bool evaluateThreeWayIntCompareToConditionRegister(TR::Register* condReg, TR::Node* node, TR::Node* firstChild,
    TR::Node* secondChild, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    ...
    default:
        TR_ASSERT_FATAL_WITH_NODE(node, false,
            "Cannot call evaluateThreeWayIntCompareToConditionRegister with data type %s",
            TR::DataType::getName(compareInfo.type));
    }

    TR_ASSERT_FATAL(!is64Bit || cg->comp()->target().is64Bit(),
        "Cannot call evaluateThreeWayIntCompareToConditionRegister for 64-bit values on 32-bit");
    ...
    bool canUseCmpi = secondChild->getOpCode().isLoadConst()
        && (compareInfo.isUnsigned ? is16BitUnsignedImmediate(secondChild->get64bitIntegralValueAsUnsigned())
    ...
    if (canUseCmpi) {
        ...
        && performTransformation(cg->comp(),
            "O^O evaluateIntCompareToConditionRegister: flipping order of compare operands (n%dn, n%dn) while "
            "evaluating n%dn to avoid P6 FXU reject",
            firstChild->getGlobalIndex(), secondChild->getGlobalIndex(), node->getGlobalIndex())) {
 ...
 *    was placed.
 */
CompareCondition evaluateIntCompareToConditionRegister(TR::Register* condReg, TR::Node* node, TR::Node* firstChild,
    TR::Node* secondChild, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
 ...
 *    was placed.
 */
CompareCondition evaluateFloatCompareToConditionRegister(TR::Register* condReg, TR::Node* node, TR::Node* firstChild,
    TR::Node* secondChild, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    ...
    if (crCond.isReversed != compareInfo.isUnorderedTrue)
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::crxor, node, condReg, condReg, condReg,
            (crCond.crcc << TR::RealRegister::pos_RT) | (crCond.crcc << TR::RealRegister::pos_RA)
                | (TR::RealRegister::CRCC_FU << TR::RealRegister::pos_RB));
 ...
 *    was placed.
 */
CompareCondition evaluateCompareToConditionRegister(TR::Register* condReg, TR::Node* node, TR::Node* firstChild,
    TR::Node* secondChild, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    switch (compareInfo.type) {
    case TR::Int8:
    ...
    default:
        TR_ASSERT_FATAL_WITH_NODE(
            node, false, "Unrecognized comparison type %s", TR::DataType::getName(compareInfo.type));
    }
}
...
CompareCondition evaluateToConditionRegister(
    TR::Register* condReg, TR::Node* node, TR::Node* condNode, TR::CodeGenerator* cg)
{
    ...
    if (!disableCondRegEval && !condNode->getRegister() && condNode->getReferenceCount() == 1) {
        ...
        if (compareInfo.type != TR::DataTypes::NoType
            && performTransformation(cg->comp(),
                "O^O evaluateToConditionRegister: evaluating n%dn directly to a condition register\n",
                condNode->getGlobalIndex())) {
            auto cond = evaluateCompareToConditionRegister(
                condReg, condNode, condNode->getFirstChild(), condNode->getSecondChild(), compareInfo, cg);
...
TR::Register* OMR::Power::TreeEvaluator::compareIntsForOrder(TR::InstOpCode::Mnemonic branchOp,
    TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isSigned, bool isHint, bool likeliness)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (isSigned) {
            ...
            bool newReg = false;
            if (node->getOpCodeValue() == TR::ifbucmplt || node->getOpCodeValue() == TR::ifbucmple
                || node->getOpCodeValue() == TR::ifbucmpgt || node->getOpCodeValue() == TR::ifbucmpge) {
                tReg = cg->allocateRegister();
                newReg = true;
                generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tReg, src1Reg, 0, 0xff);
                value &= (uint64_t)0xff;
            } else if (node->getOpCodeValue() == TR::ifsucmplt || node->getOpCodeValue() == TR::ifsucmple
                || node->getOpCodeValue() == TR::ifsucmpgt || node->getOpCodeValue() == TR::ifsucmpge) {
                tReg = cg->allocateRegister();
                newReg = true;
    ...
    if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
        ...
        if (isHint)
            ...
            generateDepConditionalBranchInstruction(
                cg, branchOp, node, dstLabel, condReg, generateRegisterDependencyConditions(cg, thirdChild, 0));
...
TR::Register* OMR::Power::TreeEvaluator::compareIntsForOrder(
    TR::InstOpCode::Mnemonic branchOp, TR::Node* node, TR::CodeGenerator* cg, bool isSigned)
{
    return TR::TreeEvaluator::compareIntsForOrder(
        branchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg, isSigned, false, false);
}

static void fixDepsForLongCompare(TR::RegisterDependencyConditions* deps, TR::Register* src1High, TR::Register* src1Low,
    TR::Register* src2High, TR::Register* src2Low)
{
    if (deps == NULL)
        return;
...
static TR::Register* compareLongsForOrderWithAnalyser(TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::Register* condReg, TR::LabelSymbol* destinationLabel,
    TR::RegisterDependencyConditions* deps, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (firstChild->isHighWordZero()) {
        ...
        if (firstChild->getReferenceCount() == 1 && src1Reg == NULL) {
            if (firstOp == TR::iu2l || firstOp == TR::su2l
                || (firstOp == TR::lushr && firstChild->getSecondChild()->getOpCode().isLoadConst()
    ...
    if (secondChild->isHighWordZero()) {
        ...
        if (secondChild->getReferenceCount() == 1 && src2Reg == NULL) {
            if (secondOp == TR::iu2l || secondOp == TR::su2l
                || (secondOp == TR::lushr && secondChild->getSecondChild()->getOpCode().isLoadConst()
    ...
    if (firstHighZero && secondHighZero) {
        ...
        if (deps) {
            if (isHint)
                generateDepConditionalBranchInstruction(
                    cg, branchOp, likeliness, node, destinationLabel, condReg, deps);
            else
        ...
        TR::PPCControlFlowInstruction* cfop
            = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::iflong, node, deps);
...
TR::Register* OMR::Power::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg,
    bool isSigned, bool isHint, bool likeliness)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* src1Reg;
    TR::Node* secondChild = node->getSecondChild();
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
            ...
            if (isSigned)
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::cmp8, node, condReg, src1Reg, cg->evaluate(secondChild));
            else
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::cmpl8, node, condReg, src1Reg, cg->evaluate(secondChild));
        }
        if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
            TR::Node* thirdChild = node->getChild(2);
            TR_ASSERT(thirdChild->getOpCodeValue() == TR::GlRegDeps,
    ...
    } else // 32 bit target
    {
        ...
        if ((firstChild->isHighWordZero() || secondChild->isHighWordZero())
            && !(secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL)) {
            return compareLongsForOrderWithAnalyser(
                branchOp, reversedBranchOp, condReg, dstLabel, deps, node, cg, isHint, likeliness);
        }

        TR::Register* src2Reg;
        src1Reg = cg->evaluate(firstChild);
        bool useImmed = secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && value >= 0
            && value <= 0xffff;
        ...
        fixDepsForLongCompare(deps, src1Reg->getHighOrder(), src1Reg->getLowOrder(),
            useImmed ? NULL : src2Reg->getHighOrder(), useImmed ? NULL : src2Reg->getLowOrder());
        TR::PPCControlFlowInstruction* cfop
            = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::iflong, node, deps);
        cfop->addTargetRegister(condReg);
...
TR::Register* OMR::Power::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::Node* node, TR::CodeGenerator* cg, bool isSigned)
{
    return TR::TreeEvaluator::compareLongsForOrder(branchOp, reversedBranchOp,
        node->getBranchDestination()->getNode()->getLabel(), node, cg, isSigned, false, false);
}
...
TR::Register* OMR::Power::TreeEvaluator::gotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getNumChildren() > 0) {
        ...
        cg->evaluate(child);
        generateDepLabelInstruction(
            cg, TR::InstOpCode::b, node, temp, generateRegisterDependencyConditions(cg, child, 0));
...
TR::Register* OMR::Power::TreeEvaluator::ireturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::RealRegister::RegNum machineReturnRegister = linkageProperties.getIntegerReturnRegister();
...
bool checkSelectReverse(TR::CodeGenerator* cg, TR::Node* node, TR::Node*& trueNode, TR::Node*& falseNode)
{
    ...
    if (!disableSelectReverse && !cg->canClobberNodesRegister(trueNode) && cg->canClobberNodesRegister(falseNode)
        && performTransformation(cg->comp(),
            "O^O checkSelectReverse: reversing condition on n%dn to avoid a register shuffle\n",
            node->getGlobalIndex())) {
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        if (node->getOpCodeValue() == TR::lselect && !cg->comp()->target().is64Bit()) {
            ...
            if (crCond.isReversed) {
                generateTrg1Src3Instruction(
                    cg, iselOp, node, trgReg->getLowOrder(), falseReg->getLowOrder(), trueReg->getLowOrder(), condReg);
                generateTrg1Src3Instruction(cg, iselOp, node, trgReg->getHighOrder(), falseReg->getHighOrder(),
                    trueReg->getHighOrder(), condReg);
            } else {
                generateTrg1Src3Instruction(
                    cg, iselOp, node, trgReg->getLowOrder(), trueReg->getLowOrder(), falseReg->getLowOrder(), condReg);
                generateTrg1Src3Instruction(cg, iselOp, node, trgReg->getHighOrder(), trueReg->getHighOrder(),
                    falseReg->getHighOrder(), condReg);
            }
        } else {
        ...
        TR_ASSERT_FATAL_WITH_NODE(node, !trgReg->containsInternalPointer() && !falseReg->containsInternalPointer(),
            "Select nodes cannot have children that are internal pointers");
...
TR::Register* OMR::Power::TreeEvaluator::compareIntsForEquality(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::InstOpCode::Mnemonic branchOp
        = node->getOpCode().isCompareTrueIfEqual() ? TR::InstOpCode::beq : TR::InstOpCode::bne;
    return TR::TreeEvaluator::compareIntsForEquality(
        branchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg, false, false);
}

TR::Register* OMR::Power::TreeEvaluator::compareIntsForEquality(TR::InstOpCode::Mnemonic branchOp,
    TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (firstChild->getOpCodeValue() == TR::b2i && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() &&
        ...
        TR::InstOpCode::Mnemonic opCode
            = (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifscmpeq
        ...
        if (node->getNumChildren() == 3) {
            ...
            cg->evaluate(thirdChild);
            generateDepConditionalBranchInstruction(
                cg, opCode, node, label, condReg, generateRegisterDependencyConditions(cg, thirdChild, 0));
        ...
        // 3) had we evaluated the child, the grandchild's refcount would have been decremented, so we have to decrement
        // it instead If a child is in a register, regardless of whether or not we evaluated it, we don't have to
        // decrement the grandchild
    ...
    if ((firstChild->getOpCodeValue() == TR:: instanceof) && !comp->getOption(TR_DisableInlineIfInstanceOf)
        ...
        && secondChild->getOpCode().isLoadConst() && (value == 0 || value == 1)) {
#ifdef J9_PROJECT_SPECIFIC
    ...
    if (isSigned) {
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && value >= LOWER_IMMED
            && value <= UPPER_IMMED) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, src1Reg, value);
        } else {
            TR::Register* src2Reg = cg->evaluate(secondChild);
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
            && secondChild->get64bitIntegralValue() >= 0 && secondChild->get64bitIntegralValue() <= 0xFFFF) {
...
TR::Register* OMR::Power::TreeEvaluator::compareLongsForEquality(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::InstOpCode::Mnemonic branchOp
        = node->getOpCode().isCompareTrueIfEqual() ? TR::InstOpCode::beq : TR::InstOpCode::bne;
    return TR::TreeEvaluator::compareLongsForEquality(
        branchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg, false, false);
}

TR::Register* OMR::Power::TreeEvaluator::compareLongsForEquality(TR::InstOpCode::Mnemonic branchOp,
    TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    if (cg->comp()->target().is64Bit()) {
        if (virtualGuardHelper(node, cg))
    ...
    if (!disableCompareToOpt && cg->comp()->target().is32Bit() && firstChild->getOpCodeValue() == TR::lshr
        && secondChild->getOpCodeValue() == TR::lshr && firstChild->getReferenceCount() == 1
        && secondChild->getReferenceCount() == 1) {
        ...
        if (firstShiftAmountNode->getOpCode().isLoadConst() && secondShiftAmountNode->getOpCode().isLoadConst()
            ...
            if (firstShiftValue > 32 && secondShiftValue > 32) {
                ...
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, firstShiftResultReg,
                    firstShiftSourceReg->getHighOrder(), firstShiftValue - 32);
                firstChild->setRegister(firstShiftResultReg);
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, secondShiftResultReg,
                    secondShiftSourceReg->getHighOrder(), secondShiftValue - 32);
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (cg->profiledPointersRequireRelocation() && secondChild->getOpCodeValue() == TR::aconst
            ...
            if (node->isProfiledGuard()) {
                ...
                TR_ASSERT(!(node->isNopableInlineGuard()),
                    "Should not evaluate class or method pointer constants underneath NOPable guards as they are "
                    "runtime assumptions handled by virtualGuardHelper");
                cg->evaluate(secondChild);
            }
        }
#endif
 ...
 *    The code generator.
 */
void generateCompareBranchSequence(TR::Register* trgReg, TR::Node* node, TR::Node* firstChild, TR::Node* secondChild,
    const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    TR::Register* condReg = cg->allocateRegister(TR_CCR);
 ...
 *    The code generator.
 */
void generateCompareSetBoolean(TR::Register* trgReg, TR::Node* node, TR::Node* firstChild, TR::Node* secondChild,
    const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    TR::Register* condReg = cg->allocateRegister(TR_CCR);
    CRCompareCondition cond = compareConditionInCR(
        evaluateCompareToConditionRegister(condReg, node, firstChild, secondChild, compareInfo, cg));

    generateTrg1Src1ImmInstruction(
        cg, cond.isReversed ? TR::InstOpCode::setbcr : TR::InstOpCode::setbc, node, trgReg, condReg, cond.crcc);
 ...
 */
TR::Register* intOrderZeroToSignBit(
    TR::Node* node, CompareCondition cond, TR::Register* srcReg, TR::Register* tempReg, TR::CodeGenerator* cg)
{
    switch (cond) {
    case CompareCondition::lt: {
        return srcReg;
    ...
    default:
        TR_ASSERT_FATAL_WITH_NODE(
            node, false, "Invalid compare condition %d for intOrderZeroToSignBit", static_cast<int>(cond));
    }
}
...
TR::Register* intOrderEvaluator(TR::Node* node, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        if (crCond.crcc == TR::RealRegister::CRCC_LT) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(
                node, crCond.crcc == TR::RealRegister::CRCC_GT, "Invalid CRCC %d in intOrderEvaluator", crCond.crcc);

            generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::crxor, node, condReg, condReg, condReg,
                (TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RT)
                    | (TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RA)
                    | (TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RB));
            generateTrg1Src1Instruction(cg, TR::InstOpCode::setb, node, trgReg, condReg);
            if (crCond.isReversed)
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::xori, node, trgReg, trgReg, 1);
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->get64bitIntegralValue() == 0) {
            ...
            if (secondChild->getOpCode().isLoadConst()) {
                ...
                useAddImmediate = is16BitSignedImmediate(addImmediate)
                    || (!secondChild->getRegister() && secondChild->getReferenceCount() == 1);
            } else {
            ...
            if (isNonNegative)
                generateTrg1Src2Instruction(
                    cg, flipResult ? TR::InstOpCode::nor : TR::InstOpCode::OR, node, trgReg, trgReg, src1Reg);
            else
                generateTrg1Src2Instruction(
                    cg, flipResult ? TR::InstOpCode::nand : TR::InstOpCode::AND, node, trgReg, trgReg, src1Reg);

            signReg = trgReg;
        }

        generateTrg1Src1Imm2Instruction(cg, bitLength > 32 ? TR::InstOpCode::rldicl : TR::InstOpCode::rlwinm, node,
            trgReg, signReg, bitLength > 32 ? (65 - bitLength) : (33 - bitLength), 1ull);
        ...
        if (flipOrder && isConst && is16BitSignedImmediate(secondChild->get64bitIntegralValue())) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::subfic, node, trgReg, src1Reg, secondChild->get64bitIntegralValue());
        } else if (!flipOrder && isConst && is16BitSignedImmediate(-secondChild->get64bitIntegralValue())) {
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, trgReg, src1Reg, -secondChild->get64bitIntegralValue());
        } else {
...
TR::Register* floatCompareEvaluator(TR::Node* node, const CompareInfo& compareInfo, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        case TR::RealRegister::CRCC_GT:
            ...
            if (crCond.isReversed != compareInfo.isUnorderedTrue)
                generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::crxor, node, condReg, condReg, condReg,
                    (TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RT)
                        | (TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RA)
                        | (TR::RealRegister::CRCC_FU << TR::RealRegister::pos_RB));
    ...
    if (cg->comp()->target().is64Bit()) {
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P9)) {
            ...
            if (secondChild->getOpCode().isLoadConst()
                && is16BitSignedImmediate(secondChild->get64bitIntegralValue())) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::cmpi8, node, condReg, src1Reg, secondChild->get64bitIntegralValue());
            } else {
                TR::Register* src2Reg = cg->evaluate(secondChild);
                generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, condReg, src1Reg, src2Reg);
            }
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->get64bitIntegralValue() == 0) {
            ...
            generateLabelInstruction(cg, TR::InstOpCode::label, node, startLabel);
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::cmp4, node, condReg, src1Reg->getHighOrder(), src2Reg->getHighOrder());
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
            ...
            generateTrg1Src2Instruction(
                cg, TR::InstOpCode::cmpl4, node, condReg, src1Reg->getLowOrder(), src2Reg->getLowOrder());
            generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, endLabel, condReg);
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, endLabel, condReg);
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
            ...
            TR::RegisterDependencyConditions* deps
                = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 6, cg->trMemory());
...
static TR::Register* floatThreeWayCompareEvaluator(TR::Node* node, bool unorderedIsLess, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P9)) {
        ...
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::cror, node, condReg, condReg, condReg,
            (crcc << TR::RealRegister::pos_RT) | (crcc << TR::RealRegister::pos_RA)
                | (TR::RealRegister::CRCC_FU << TR::RealRegister::pos_RB));
        generateTrg1Src1Instruction(cg, TR::InstOpCode::setb, node, trgReg, condReg);
    } else {
        TR::LabelSymbol* startLabel = generateLabelSymbol(cg);
...
static void collectGlDeps(TR::Node* node, TR_BitVector* vector)
{
    int32_t ii;
...
static void lookupScheme1(TR::Node* node, bool unbalanced, bool fromTableEval, TR::CodeGenerator* cg)
{
    ...
    for (int i = 2; i < total; i++) {
        ...
        if (isInt64) {
            ...
            if (cg->comp()->target().is64Bit()) {
                ...
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::cmpi4, node, cndRegister, selector->getHighOrder(), 0);
                generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, toDefaultLabel, cndRegister);
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::cmpi4, node, cndRegister, selector->getHighOrder(), value);
            } // 64bit target?
        } // 64bit int selector
        else {
        ...
        if (unbalanced) {
            ...
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::beq, node, child->getBranchDestination()->getNode()->getLabel(), cndRegister);
        }
    }
    ...
    generateDepLabelInstruction(
        cg, TR::InstOpCode::b, node, secondChild->getBranchDestination()->getNode()->getLabel(), acond);
...
static void lookupScheme3(TR::Node* node, bool unbalanced, TR::CodeGenerator* cg)
{
        ...
        if (offset != PTOC_FULL_INDEX) {
            ...
            if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != cg->hiValue(offset),
                    ...
                    offset);
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, node, addrRegister, cg->getTOCBaseRegister(), cg->hiValue(offset));
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
                    TR::MemoryReference::createWithDisplacement(cg, addrRegister, LO_VALUE(offset), 8));
            } else {
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
    ...
    else // if 32bit mode
    {
        ...
        cg->getAheadOfTimeCompile()->getRelocationList().push_front(new (cg->trHeapMemory())
                TR::PPCPairedRelocation(rel1, rel2, (uint8_t*)recordInfo, TR_AbsoluteMethodAddressOrderedPair, node));
    ...
    for (int32_t ii = 2; ii < total; ii++) {
        ...
        if (isInt64) {
            if (cg->comp()->target().is64Bit()) {
                ...
                generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, toDefaultLabel, cndRegister);
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::cmp4, node, cndRegister, selector->getLowOrder(), dataRegister->getLowOrder());
            }
        } else {
...
static void lookupScheme4(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (offset != PTOC_FULL_INDEX) {
            ...
            if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != cg->hiValue(offset),
                    ...
                    offset);
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, node, pivotRegister, cg->getTOCBaseRegister(), cg->hiValue(offset));
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
                    TR::MemoryReference::createWithDisplacement(cg, pivotRegister, LO_VALUE(offset), 8));
            } else {
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
                    TR::MemoryReference::createWithDisplacement(cg, cg->getTOCBaseRegister(), offset, 8));
            }
        ...
        rel2 = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi2, node, addrRegister, pivotRegister, LO_VALUE(address));
    ...
    if (isInt64) {
        if (two_reg) {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, dataRegister->getHighOrder(),
                TR::MemoryReference::createWithIndexReg(cg, addrRegister, pivotRegister, 4));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, pivotRegister, pivotRegister, 4);
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, dataRegister->getLowOrder(),
                TR::MemoryReference::createWithIndexReg(cg, addrRegister, pivotRegister, 4));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, pivotRegister, pivotRegister, -4);
        } else {
            generateTrg1MemInstruction(cg, TR::InstOpCode::ldx, node, dataRegister,
                TR::MemoryReference::createWithIndexReg(cg, addrRegister, pivotRegister, 8));
        }
    } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, dataRegister,
            TR::MemoryReference::createWithIndexReg(cg, addrRegister, pivotRegister, 4));
    }
    ...
    if (isInt64) {
        if (two_reg) {
            generateTrg1Src2Instruction(
                cg, cmp_opcode, node, cndRegister, dataRegister->getHighOrder(), selector->getHighOrder());
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, searchLabel, cndRegister);
            generateTrg1Src2Instruction(
                cg, cmp_opcode, node, cndRegister, dataRegister->getLowOrder(), selector->getLowOrder());
        } else {
    ...
    for (int32_t ii = 2; ii < total; ii++) {
        generateLabelInstruction(
            cg, TR::InstOpCode::b, node, node->getChild(ii)->getBranchDestination()->getNode()->getLabel());
        if (isInt64)
            dataTable64[ii - 2] = node->getChild(ii)->getCaseConstant();
        else
            dataTable[ii - 2] = node->getChild(ii)->getCaseConstant();
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node,
        node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), cndRegister);
...
TR::Register* OMR::Power::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isSafeToSkipTableBoundCheck()) {
        ...
        generateConditionalBranchInstruction(
            cg, TR::InstOpCode::bge, node, secondChild->getBranchDestination()->getNode()->getLabel(), ccReg);
    }

    {
        ...
        for (; i < numCases - 1; ++i)
            generateLabelInstruction(
                cg, TR::InstOpCode::b, node, node->getChild(i + 2)->getBranchDestination()->getNode()->getLabel());

        generateDepLabelInstruction(cg, TR::InstOpCode::b, node,
            node->getChild(i + 2)->getBranchDestination()->getNode()->getLabel(), conditions);
    }
...
TR::Instruction* OMR::Power::TreeEvaluator::generateNullTestInstructions(
    TR::CodeGenerator* cg, TR::Register* trgReg, TR::Node* node, bool nullPtrSymRefRequired)
{
    ...
    if (cg->getHasResumableTrapHandler()) {
        ...
        gcPoint = generateDepConditionalBranchInstruction(
            cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely, node, snippetLabel, condReg, conditions);
...
TR::Register* OMR::Power::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_PPCOutOfLineCodeSection* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(node, TR::call, NULL, slowPathLabel, restartLabel, cg);
    ...
    generateConditionalBranchInstruction(
        cg, compareConditionToBranch(reverseCondition(cond)), PPCOpProp_BranchUnlikely, node, slowPathLabel, condReg);
...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->comp()->compileRelocatableCode()) {
        ...
        default:
            TR_ASSERT(0, "got AOT guard in node but virtual guard not one of known guards supported for AOT. Guard: %d",
...
static TR::Register* generateMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
    ...
    case TR::imax:
    case TR::imin:
        cmp_op = TR::InstOpCode::cmp4;
        break;
    case TR::iumax:
    case TR::iumin:
        cmp_op = TR::InstOpCode::cmpl4;
        break;
    case TR::lmax:
    case TR::lmin:
        cmp_op = TR::InstOpCode::cmp8;
        break;
    case TR::lumax:
    case TR::lumin:
        cmp_op = TR::InstOpCode::cmpl8;
        break;
    case TR::fmax:
    case TR::fmin:
    case TR::dmax:
    case TR::dmin:
        cmp_op = TR::InstOpCode::fcmpu;
        break;
    default:
    ...
    if (two_reg) {
        TR::PPCControlFlowInstruction* cfop = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(
            cg, TR::InstOpCode::iflong, node, NULL /*deps */);
        cfop->addTargetRegister(condReg);
        ...
        generateConditionalBranchInstruction(
            cg, max ? TR::InstOpCode::bge : TR::InstOpCode::ble, node, end_label, condReg);
...
TR::Register* OMR::Power::TreeEvaluator::ifxcmpoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getNumChildren() == 3) {
        ...
        cg->evaluate(thirdChild);
        generateDepConditionalBranchInstruction(
            cg, branchOp, node, dstLabel, cr, generateRegisterDependencyConditions(cg, thirdChild, 0));
        cg->decReferenceCount(thirdChild);
    } else {
