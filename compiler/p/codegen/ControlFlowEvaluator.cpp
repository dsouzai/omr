    ...
    TR::Node *secondChild, const CompareInfo &compareInfo, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && !secondChild->getRegister()
        ...
        if (compareInfo.isUnsigned && is16BitUnsignedImmediate(secondHi)) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg, firstReg->getHighOrder(),
                secondHi);
        } else if (!compareInfo.isUnsigned && is16BitSignedImmediate(secondHi)) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, firstReg->getHighOrder(),
                secondHi);
        } else {
        ...
        if (is16BitUnsignedImmediate(secondLo)) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg2, firstReg->getLowOrder(),
                secondLo);
        } else {
            TR::Register *secondLoReg = cg->allocateRegister();

            loadConstant(cg, node, secondLo, secondLoReg);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg2, firstReg->getHighOrder(),
                secondLoReg);
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg2, firstReg->getLowOrder(),
            secondReg->getLowOrder());
    }
    ...
    TR::Node *secondChild, const CompareInfo &compareInfo, TR::CodeGenerator *cg)
{
    switch (compareInfo.type) {
        ...
        default:
            TR_ASSERT_FATAL_WITH_NODE(node, false, "Unrecognized comparison type %s",
                TR::DataType::getName(compareInfo.type));
    }
}
...
CompareCondition evaluateToConditionRegister(TR::Register *condReg, TR::Node *node, TR::Node *condNode,
    TR::CodeGenerator *cg)
{
    ...
    if (!disableCondRegEval && !condNode->getRegister() && condNode->getReferenceCount() == 1) {
        ...
        if (compareInfo.type != TR::DataTypes::NoType
            ...
            auto cond = evaluateCompareToConditionRegister(condReg, condNode, condNode->getFirstChild(),
                condNode->getSecondChild(), compareInfo, cg);
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForOrder(TR::InstOpCode::Mnemonic branchOp, TR::Node *node,
    TR::CodeGenerator *cg, bool isSigned)
{
    return TR::TreeEvaluator::compareIntsForOrder(branchOp, node->getBranchDestination()->getNode()->getLabel(), node,
        cg, isSigned, false, false);
}
    ...
    TR::RegisterDependencyConditions *deps, TR::Node *node, TR::CodeGenerator *cg, bool isHint, bool likeliness)
{
    ...
    if (firstHighZero && secondHighZero) {
        ...
        if (deps) {
            if (isHint)
                generateDepConditionalBranchInstruction(cg, branchOp, likeliness, node, destinationLabel, condReg,
                    deps);
            else
    ...
    bool isSigned, bool isHint, bool likeliness)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
            ...
            if (isSigned)
                generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, condReg, src1Reg,
                    cg->evaluate(secondChild));
            else
                generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl8, node, condReg, src1Reg,
                    cg->evaluate(secondChild));
        }
        if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
            TR::Node *thirdChild = node->getChild(2);
    ...
    } else // 32 bit target
    {
        ...
        if ((firstChild->isHighWordZero() || secondChild->isHighWordZero())
            ...
            return compareLongsForOrderWithAnalyser(branchOp, reversedBranchOp, condReg, dstLabel, deps, node, cg,
                isHint, likeliness);
        }
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        if (node->getOpCodeValue() == TR::lselect && !cg->comp()->target().is64Bit()) {
            ...
            if (crCond.isReversed) {
                generateTrg1Src3Instruction(cg, iselOp, node, trgReg->getLowOrder(), falseReg->getLowOrder(),
                    trueReg->getLowOrder(), condReg);
                generateTrg1Src3Instruction(cg, iselOp, node, trgReg->getHighOrder(), falseReg->getHighOrder(),
                    trueReg->getHighOrder(), condReg);
            } else {
                generateTrg1Src3Instruction(cg, iselOp, node, trgReg->getLowOrder(), trueReg->getLowOrder(),
                    falseReg->getLowOrder(), condReg);
    ...
    const CompareInfo &compareInfo, TR::CodeGenerator *cg)
{
    ...
    generateTrg1Src1ImmInstruction(cg, cond.isReversed ? TR::InstOpCode::setbcr : TR::InstOpCode::setbc, node, trgReg,
        condReg, cond.crcc);
 ...
 */
TR::Register *intOrderZeroToSignBit(TR::Node *node, CompareCondition cond, TR::Register *srcReg, TR::Register *tempReg,
    TR::CodeGenerator *cg)
{
    switch (cond) {
        case CompareCondition::lt: {
        ...
        default:
            TR_ASSERT_FATAL_WITH_NODE(node, false, "Invalid compare condition %d for intOrderZeroToSignBit",
...
TR::Register *intOrderEvaluator(TR::Node *node, const CompareInfo &compareInfo, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        ...
        if (crCond.crcc == TR::RealRegister::CRCC_LT) {
            ...
            TR_ASSERT_FATAL_WITH_NODE(node, crCond.crcc == TR::RealRegister::CRCC_GT,
                "Invalid CRCC %d in intOrderEvaluator", crCond.crcc);
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->get64bitIntegralValue() == 0) {
            ...
            if (isNonNegative)
                generateTrg1Src2Instruction(cg, flipResult ? TR::InstOpCode::nor : TR::InstOpCode::OR, node, trgReg,
                    trgReg, src1Reg);
            else
                generateTrg1Src2Instruction(cg, flipResult ? TR::InstOpCode::nand : TR::InstOpCode::AND, node, trgReg,
                    trgReg, src1Reg);
        ...
        if (flipOrder && isConst && is16BitSignedImmediate(secondChild->get64bitIntegralValue())) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::subfic, node, trgReg, src1Reg,
                secondChild->get64bitIntegralValue());
        } else if (!flipOrder && isConst && is16BitSignedImmediate(-secondChild->get64bitIntegralValue())) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, trgReg, src1Reg,
                -secondChild->get64bitIntegralValue());
        } else {
...
TR::Register *OMR::Power::TreeEvaluator::lcmpEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->get64bitIntegralValue() == 0) {
            ...
            generateLabelInstruction(cg, TR::InstOpCode::label, node, startLabel);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg, src1Reg->getHighOrder(),
            ...
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg, src1Reg->getLowOrder(),
                src2Reg->getLowOrder());
            generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, endLabel, condReg);
            generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, endLabel, condReg);
        ...
        if (offset != PTOC_FULL_INDEX) {
            ...
            if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != cg->hiValue(offset),
                    ...
                    offset);
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, addrRegister, cg->getTOCBaseRegister(),
                    cg->hiValue(offset));
...
static void lookupScheme4(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (offset != PTOC_FULL_INDEX) {
            ...
            if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                TR_ASSERT_FATAL_WITH_NODE(node, 0x00008000 != cg->hiValue(offset),
                    "TOC offset (0x%x) is unexpectedly high. Can not encode upper 16 bits into an addis instruction.",
                    offset);
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, pivotRegister, cg->getTOCBaseRegister(),
                    cg->hiValue(offset));
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
        ...
        rel2 = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, addrRegister, pivotRegister,
            LO_VALUE(address));
    ...
    if (isInt64) {
        if (two_reg) {
            generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister, dataRegister->getHighOrder(),
                selector->getHighOrder());
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, searchLabel, cndRegister);
            generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister, dataRegister->getLowOrder(),
                selector->getLowOrder());
        } else {
    ...
    for (int32_t ii = 2; ii < total; ii++) {
        generateLabelInstruction(cg, TR::InstOpCode::b, node,
            node->getChild(ii)->getBranchDestination()->getNode()->getLabel());
        if (isInt64)
            dataTable64[ii - 2] = node->getChild(ii)->getCaseConstant();
        else
            dataTable[ii - 2] = node->getChild(ii)->getCaseConstant();
...
TR::Register *OMR::Power::TreeEvaluator::tableEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!node->isSafeToSkipTableBoundCheck()) {
        ...
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node,
            secondChild->getBranchDestination()->getNode()->getLabel(), ccReg);
    }

    {
        ...
        for (; i < numCases - 1; ++i)
            generateLabelInstruction(cg, TR::InstOpCode::b, node,
                node->getChild(i + 2)->getBranchDestination()->getNode()->getLabel());
...
TR::Instruction *OMR::Power::TreeEvaluator::generateNullTestInstructions(TR::CodeGenerator *cg, TR::Register *trgReg,
    TR::Node *node, bool nullPtrSymRefRequired)
{
    ...
    if (cg->getHasResumableTrapHandler()) {
        ...
        gcPoint = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely, node,
            snippetLabel, condReg, conditions);
...
TR::Register *OMR::Power::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    generateConditionalBranchInstruction(cg, compareConditionToBranch(reverseCondition(cond)), PPCOpProp_BranchUnlikely,
        node, slowPathLabel, condReg);
...
static TR::Register *generateMaxMin(TR::Node *node, TR::CodeGenerator *cg, bool max)
{
    ...
    if (two_reg) {
        TR::PPCControlFlowInstruction *cfop = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(cg,
            TR::InstOpCode::iflong, node, NULL /*deps */);
        cfop->addTargetRegister(condReg);
        ...
        generateConditionalBranchInstruction(cg, max ? TR::InstOpCode::bge : TR::InstOpCode::ble, node, end_label,
            condReg);
...
TR::Register *OMR::Power::TreeEvaluator::ifxcmpoEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getNumChildren() == 3) {
        TR::Node *thirdChild = node->getChild(2);
        cg->evaluate(thirdChild);
        generateDepConditionalBranchInstruction(cg, branchOp, node, dstLabel, cr,
            generateRegisterDependencyConditions(cg, thirdChild, 0));
        cg->decReferenceCount(thirdChild);
    } else {
