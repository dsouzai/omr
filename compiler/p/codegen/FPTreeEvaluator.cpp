...
static void ifFloatEvaluator(
    TR::Node* node, TR::InstOpCode::Mnemonic branchOp1, TR::InstOpCode::Mnemonic branchOp2, TR::CodeGenerator* cg);
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()
        ...
        if (node->normalizeNanValues()) {
            ...
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::beq, PPCOpProp_BranchLikely, node, nanNormalizeEndLabel, condReg);
...
TR::Register* OMR::Power::TreeEvaluator::lbits2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
        ...
        generateMvFprGprInstructions(
            cg, node, gpr2fprHost32, false, trgReg, longReg->getHighOrder(), longReg->getLowOrder(), tempReg);
...
TR::Register* dbits2l32Evaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isLoadVar() && !child->getRegister() && child->getReferenceCount() == 1
        ...
        generateMvFprGprInstructions(
            cg, node, fpr2gprHost32, false, trgReg->getHighOrder(), trgReg->getLowOrder(), doubleReg, tempReg);
    ...
    if (node->getDataType().getVectorElementType() == TR::Int8) {
        ...
        if (!disableDirectMove && cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)
            ...
            generateMemSrc1Instruction(
                cg, TR::InstOpCode::stw, node, TR::MemoryReference::createWithSymRef(cg, node, localTemp, 4), tempReg);
...
TR::Register* OMR::Power::TreeEvaluator::int2dbl(
    TR::Node* node, TR::Register* srcReg, bool canClobber, TR::CodeGenerator* cg)
{
        ...
        if (cg->comp()->target().is64Bit()) {
            if (node->getOpCodeValue() == TR::iu2f || node->getOpCodeValue() == TR::iu2d) {
                ...
                generateTrg1Src1Imm2Instruction(
                    cg, TR::InstOpCode::rldicl, node, tempReg, srcReg, 0, CONSTANT64(0x00000000ffffffff));
    ...
    if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && cg->comp()->target().is64Bit())) {
        if (cg->comp()->target().is64Bit())
            ...
            if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P8)) {
                TR::Register* tmp1 = cg->allocateRegister(TR_FPR);
                generateMvFprGprInstructions(
                    cg, node, gpr2fprHost32, false, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder(), tmp1);
                cg->stopUsingRegister(tmp1);
            } else
                generateMvFprGprInstructions(
                    cg, node, gpr2fprHost32, false, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder());
        }
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfid, node, trgReg, trgReg);
    } else {
...
TR::Register* OMR::Power::TreeEvaluator::xfRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (globalReg == NULL) {
        globalReg = cg->allocateFloatingPointRegisterPair(
            cg->allocateSinglePrecisionRegister(), cg->allocateSinglePrecisionRegister());
        node->setRegister(globalReg);
    }
    return (globalReg);
}
