...
bool OMR::Power::CPU::getPPCSupportsAES()
{
    return self()->supportsFeature(OMR_FEATURE_PPC_HAS_ALTIVEC) && self()->isAtLeast(OMR_PROCESSOR_PPC_P8)
        && self()->supportsFeature(OMR_FEATURE_PPC_HAS_VSX);
}

bool OMR::Power::CPU::hasPopulationCountInstruction()
{
#if defined(J9OS_I5)
    return false;
#else
...
bool OMR::Power::CPU::supportsDecimalFloatingPoint() { return self()->supportsFeature(OMR_FEATURE_PPC_HAS_DFP); }

bool OMR::Power::CPU::getSupportsHardware32bitCompress() { return self()->isAtLeast(OMR_PROCESSOR_PPC_P10); }

bool OMR::Power::CPU::getSupportsHardware64bitCompress()
{
    return self()->isAtLeast(OMR_PROCESSOR_PPC_P10) && TR::Compiler->target.is64Bit();
}

bool OMR::Power::CPU::getSupportsHardware32bitExpand() { return self()->isAtLeast(OMR_PROCESSOR_PPC_P10); }

bool OMR::Power::CPU::getSupportsHardware64bitExpand()
{
    return self()->isAtLeast(OMR_PROCESSOR_PPC_P10) && TR::Compiler->target.is64Bit();
}

bool OMR::Power::CPU::getSupportsHardwareSQRT() { return self()->isAtLeast(OMR_PROCESSOR_PPC_HW_SQRT_FIRST); }

bool OMR::Power::CPU::getSupportsHardwareRound() { return self()->isAtLeast(OMR_PROCESSOR_PPC_HW_ROUND_FIRST); }

bool OMR::Power::CPU::getSupportsHardwareCopySign() { return self()->isAtLeast(OMR_PROCESSOR_PPC_HW_COPY_SIGN_FIRST); }

bool OMR::Power::CPU::supportsTransactionalMemoryInstructions() { return self()->supportsFeature(OMR_FEATURE_PPC_HTM); }

bool OMR::Power::CPU::isTargetWithinIFormBranchRange(intptr_t targetAddress, intptr_t sourceAddress)
{
...
bool OMR::Power::CPU::supportsFeature(uint32_t feature)
{
...
bool OMR::Power::CPU::getSupportsHardware64bitRotate(bool requireRotateToLeft)
{
    return TR::Compiler->target.is64Bit();
}

bool OMR::Power::CPU::is(OMRProcessorArchitecture p)
{
    if (TR::Compiler->omrPortLib == NULL)
...
bool OMR::Power::CPU::isAtLeast(OMRProcessorArchitecture p)
{
    if (TR::Compiler->omrPortLib == NULL)
        return self()->id() >= self()->getOldProcessorTypeFromNewProcessorType(p);
...
bool OMR::Power::CPU::isAtMost(OMRProcessorArchitecture p)
{
    if (TR::Compiler->omrPortLib == NULL)
        return self()->id() <= self()->getOldProcessorTypeFromNewProcessorType(p);
...
TR_Processor OMR::Power::CPU::getOldProcessorTypeFromNewProcessorType(OMRProcessorArchitecture p)
{
    switch (p) {
...
const char *OMR::Power::CPU::getProcessorName()
{
