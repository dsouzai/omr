...
public:
    ...
    virtual TR_ResolvedMethod* createResolvedMethod(
        TR_Memory*, TR_OpaqueMethodBlock*, TR_ResolvedMethod* = 0, TR_OpaqueClassBlock* = 0);
    virtual OMR::MethodMetaDataPOD* createMethodMetaData(TR::Compilation* comp) { return NULL; }

    virtual TR_OpaqueMethodBlock* getMethodFromName(
    ...
    virtual const char* sampleSignature(
        TR_OpaqueMethodBlock* aMethod, char* bug = 0, int32_t bufLen = 0, TR_Memory* memory = NULL);
    ...
    virtual TR_OpaqueClassBlock* getClassFromSignature(
        const char* sig, int32_t length, TR_ResolvedMethod* method, bool callSiteVettedForAOT = false);
    virtual TR_OpaqueClassBlock* getClassFromSignature(
        const char* sig, int32_t length, TR_OpaqueMethodBlock* method, bool callSiteVettedForAOT = false);
