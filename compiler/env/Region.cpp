...
Region::~Region() throw()
{
    ...
    for (TR::reference_wrapper<TR::MemorySegment> latestSegment(_currentSegment);
         latestSegment.get() != _initialSegment; latestSegment = _currentSegment) {
...
size_t Region::round(size_t bytes) { return (bytes + 15) & (~15); }
} // namespace TR
