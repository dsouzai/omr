...
{
    return round_t<alignment, decltype(allocationSize)>(
        allocationSize, alignmentRemainder<alignment>(allocationSize), alignDown);
}
...
#define TR_ALLOC_IMPL(a)                                                                                             \
    ...
    void operator delete(void* p, TR_ArenaAllocator* m)                                                              \
    { /* TR_ArenaAllocator contains an empty deallocator */                                                          \
    }                                                                                                                \
...
public:
    ...
    static uint32_t sprintfMetric(
        char* line, Metric value, Metric total, bool alternativeFormat = false /* ignored */, bool csv = false)
    {
        uint32_t offset = 0;
        if (csv) {
